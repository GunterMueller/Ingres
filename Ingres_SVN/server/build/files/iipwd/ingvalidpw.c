...
#if defined(ix3_us5) || defined(sqs_ptx) || defined(dr6_us5) || \
    defined(dra_us5) || defined(su4_us5) || defined(dr6_ues) || \
    defined(usl_us5) || defined(nc4_us5) || defined(pym_us5) || \
    defined(sui_us5) || defined(rmx_us5) || defined(ts2_us5) || \
    defined(rux_us5) || defined(dgi_us5) || defined(a64_sol) || \
    ((defined(int_lnx) || defined(ibm_lnx) || defined(axp_lnx) || \
      defined(i64_lnx) || defined(a64_lnx) || defined(hp8_us5) || \
...
#if defined(dr6_us5) || defined(dra_us5) || defined(dr6_ues) || \
    defined(su4_us5) || defined(sui_us5) || defined(usl_us5) || \
    defined(nc4_us5) || defined(pym_us5) || defined(rmx_us5) || \
    defined(ts2_us5) || defined(a64_sol) || \
    ((defined(int_lnx) || defined(int_rpl) || defined(ibm_lnx) || \
    defined(axp_lnx) || defined(i64_lnx)) && defined(SHADOW_EXISTS))
# include <sys/time.h>
# define SECHOUR		3600
# define HOURDAY		24
...
#if defined(hp8_us5) || defined (hpb_us5) || defined(i64_hpu) || \
    defined(hp2_us5)
...
#endif /* #if ((defined(hp8_us5) || defined (hpb_us5) ... */
...
int	iscan(char *input, char *username, char *password);
FILE *	mklogfile(char *ptr);
void handler();
...
# if defined(odt_us5) || defined(sco_us5) || defined(sos_us5) || defined(axp_osf)
	...
	*/
	...
	if ((cnt = read(0,input, sizeof(input))) < 0)
	{
		if (fp)
		{
			fprintf(fp,"ingvalidpw: read failed errno %d\n",errno);
			fclose(fp);
	        }
		exit(E_NOUSER);
	}
	if (iscan(input, user,password) != E_OK)
		exit(E_BADINPUT);
#ifdef	xDEBUG 
	printf("user: %s, passwd: %s\n", user, password);
#endif 
	if (strlen(user) == 0 && fp) 
	   fprintf(fp,"error: strlen(user) = 0\n");
	if (strlen(password) == 0 && fp)
	    fprintf(fp,"error: strlen(password) = 0\n");
	/*
	...
	*/
	if(getuid() && geteuid())
	{
		if (fp)
		{
		    fprintf(fp,"error: you are not root: getuid = %d, geteuid = %d\n", getuid(),geteuid());
		    fclose(fp);
	        }
		return(E_NOTROOT);
	}
...
/*  */
	      ...
	      if (fp)
	      {
	        fprintf(fp,"error: getpwnam failed: can't get password\n");
		fclose(fp);
	      }
	      exit (E_NOUSER);
/*  */
	...
	*/
	...
	if( crypted == NULL)
	{
		if (fp)
		{
		    fprintf(fp,"error: crypt returned NULL\n");
		    fclose(fp);
		}
		exit(E_NOCRYPT);
	}
...
# ifdef CHECK_HPEXP
	 ...
	 */
#ifdef xDEBUG
	fprintf(fp,"HP specific values being tested for uid=%ld:\n\
 ...
 Invalid logins=%d set?=%d user fd_max_tries=%d set?=%d system fd_max_tries=%d set?=%d\n",
...
#endif
	{
	    ...
	    if (fp) {
	        fprintf(fp,"Error: password expired\n");
	        fclose(fp);
	    }
	    exit(E_EXPIRED);
	    }
...

	{
	    ...
	    */
	    ...
	    if ( pw->uflg.fg_nlogins ) {
	       if( pw->pw_nlogin >= maxtries ) {
	            if (fp) {
	                fprintf(fp,"Error: Login Account locked due to failed login attempts\n");
	            fclose(fp);
	            }
  	            exit(E_LOCKED);
	       }
  	...
  	if ( pw->pw_lock ) {
	  if (fp) {
	    fprintf(fp,"Error: Login Account locked\n");
	    fclose(fp);
	  }
	  exit(E_LOCKED);
	}
...
# endif
        ...
        if( strncmp( crypted, encrypted_pwd, strlen(crypted)) != 0 )
	{
		if (fp)
		{
		    fprintf(fp,"error: password mis-match\n");
		    fclose(fp);
		}
		exit(E_NOMATCH);
	}
