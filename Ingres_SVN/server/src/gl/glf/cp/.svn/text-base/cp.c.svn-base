	...
	CPFILE		*user_file )
{
	...
	itemlist[0].buflen = sizeof(procprv);
	itemlist[0].itmcod = JPI$_PROCPRIV;
	itemlist[0].bufadr = &procprv;
	itemlist[0].reslen = NULL;

	itemlist[1].buflen = sizeof(priv_mask);
	itemlist[1].itmcod = JPI$_CURPRIV;
	itemlist[1].bufadr = &priv_mask;
	...
	ret_val = sys$getjpi(0, 0, 0, &itemlist, 0, 0, 0);
	/* this may become asynchronous (fhc) */
	sys$waitfr(0);
