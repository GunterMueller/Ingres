...
*concat_strings( PM_CONTEXT *context, char *s1, char *s2 )
{
	...
	if( s1 == NULL )
		return( s2 );
	if( (s = alloc_mem( context, STlength( s1 ) + STlength( s2 ) + 1 ))
		== NULL )
	{
		return( NULL );
	} 
...

/*
**  Name: Local_CMcpychar 
**
**  Description:
**	A revised version of CMcpychar.
**
**	Local_CMcpychar assume this is an in place situation and 
**	take it into account that the string has mix single byte 
**	characters and double bytes characters.
**
**  History:
**	22-apr-93 (penga03)
**		Created.
*/
void 
Local_CMcpychar(char *src, char *dst)
{
	if (CMdbl1st(src) && !CMdbl1st(dst))
	{
		u_char *p=dst+1, *temp;
		i4 count=0;
		...
		temp=MEreqmem(0, count, TRUE, NULL);
		MEcopy(dst+1, count, temp);
		
		CMcpychar(src,dst);
		
		MEcopy(temp, count, dst+2);
		dst[count+2]='\0';
		MEfree(temp);
	}
	else if (!CMdbl1st(src) && CMdbl1st(dst))
	{
		u_char *p=dst+2, *temp;
		i4 count=0;
		...
		temp=MEreqmem(0, count, TRUE, NULL);
		MEcopy(dst+2, count, temp);
		
		CMcpychar(src, dst);
		
		MEcopy(temp, count, dst+1);
		dst[count+1]='\0';
		MEfree(temp);
	}
	else
...
*/
static void
head_remove( char *c, char **s )
{
	char *p1, *p2;

	for( p1 = p2 = c, CMprev( p1, *s );
		p1 >= *s; CMprev( p1, *s ),
		CMprev( p2, *s )
	)
		Local_CMcpychar( p1, p2 );
	CMnext( *s );
}
...
wrap:
	switch( yywraps )
	{
		case 0:
			...
			NMloc( FILES, FILENAME, ERx( "protect.dat" ),
				&yyloc );
...
{
	bool component;
	char yytext[ PM_MAX_LINE + 1 ], *yytemp;
	i4 yyleng;
	...
	if( yycol == 1 )
	{
		...
		if( yyleng > 0 )
		{
			...
			STnmbcpy( yytext, yytemp, yyleng );
			for( p = yytext, i = 0; i < yyleng; CMnext( p ), i++ );
			*p = EOS;	
	
			if( (yylval.string = alloc_mem( context,
				STlength( yytext ) + 1 )) == NULL )
			{ 
				/* NEED TO LOG OUT OF MEMORY ERROR HERE */ 
				return( YY_FAIL );
			}
			STcopy( yytext, yylval.string );
		...
		if( *yyp == '\'' )
		{
				...
				CMbyteinc( yyleng, yyp), CMnext( yyp ) )
			{
				if( *yyp == '\'' && *yyprev == '\\' )
				{
					...
					CMbytedec( yyleng, yyprev );
					head_remove( yyprev, &head );
				}
				else if( *yyprev == '\\' )
				{
					bool check_octal = TRUE;
...
PMmSetDefault( PM_CONTEXT *context, i4  idx, char *s )
{
	...
	if( (context->def_elem[ idx ] = alloc_mem( context,
		STlength( s ) + 1 )) == NULL )
	{
		return( PM_NO_MEMORY );
	}

	STcopy( s, context->def_elem[ idx ] );
	       ...
	       char *(*elements)[PM_MAX_ELEM] )
{
	...
	while ( EOS != *piece )
	{
		...
		*piece = EOS;
		...
		{
		    if ( 0 == STcompare((*elements)[count], ERx("$")) )
		    {
...
PMmExpandRequest( PM_CONTEXT *context, char *request )
{
	...
	for( i = 0; i < len; i++ )
	{
		...
		if( STcompare( ERx( "$" ), elem[ i ] ) == 0 )
		{
			if( context->def_elem[ i ] != NULL )
			{
				STcopy( context->def_elem[ i ], c );
				c += STlength( context->def_elem[ i ] );
			}
			else
			{
				STcopy( ERx( "*" ), c );
...
PMmInsert( PM_CONTEXT *context, char *request, char *value )
{
	...
	if( (elembuf = alloc_mem( context,
			STlength( request ) + 1 )) == NULL )
	{
	    return( PM_NO_MEMORY );
	}
	STcopy( request, elembuf );
...
PMmLoad( PM_CONTEXT *context, LOCATION *loc, PM_ERR_FUNC *err_func )
{
	...
	if( loc == NULL )
	{
		...
		NMloc( FILES, FILENAME, ERx( "config.dat" ), &yyloc );
		LOcopy( &yyloc, yyloc_buf, &yyloc );
		LOtos( &yyloc, &yyloc_path );
...
PMmWrite( PM_CONTEXT *context, LOCATION *loc )
{
	...
	for( i = 0; i < context->num_name; i++ )
	{
		...
		** whitespace.
		*/
		STprintf( text, "%s:", context->list[ i ]->name );
		for( p = text + (len = STlength( text ));
			p < &text[ len <= 39 ? 40 : len + 1 ]; CMnext( p ) )
		{
			Local_CMcpychar( ERx( " " ), p );
		}
		*p = EOS;

		/* determine whether to delimit resource value string */ 
		for( p = context->list[ i ]->value; *p != EOS; CMnext( p ) )
		{
			if( yyreserved( p ) )
			{
				delimit = TRUE;
		...
		for( p = text; *p != ':'; CMnext( p ) );
		if( delimit )
		{
			STcat( p, ERx( "'" ) );		
			for( ; *p != '\''; CMnext( p ) );
		}
		STcat( p, context->list[ i ]->value );
		if( delimit )
		{
			char *q;

			/* embedded single-quotes must be escaped */
			for( q = CMnext( p ); *q != EOS; CMnext( q ) )
			{
				char *r;
				char special = *q;
