    ...
    CX_LOCK_LIST **lock_list)
{
    ...
    *lock_list = NULL;
    ...
    cursor = 0;
    while (sys$getlkiw(0, &cursor, itmlist1, 0, 0, 0, 0) == SS$_NORMAL)
    {
	int cmpres;
	int count;
	...
	for (;;)
	{
	    ...
	    itmlist2[0].buflen = count * sizeof(LKIDEF);
	    lkilist = (LKIDEF *)MEreqmem(0, itmlist2[0].buflen, FALSE, &status);
	    if (status)
	    {
		TRdisplay("Insufficient memory to allocate lock lists\n");
		return FAIL;
	    }
	    itmlist2[0].bufadr = (char *)lkilist;
	    status = sys$getlkiw(0, &lockid, itmlist2, 0, 0, 0, 0);
