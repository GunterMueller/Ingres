...
char	*string)	/* String to translate (input and output) */
{
	...
	u_i2	result_len;
	II_VMS_ITEM_LIST_3	itemlist[4];	/* Logical name itemlist */
	...
	itemlist[0].buflen = MAX_LOC;
	itemlist[0].bufadr = result;
	itemlist[0].reslen = &result_len;

	itemlist[1].itmcod = LNM$_MAX_INDEX;
	itemlist[1].buflen = sizeof(i4);
	itemlist[1].bufadr = (PTR)&max_index;
	itemlist[1].reslen = NULL;

	itemlist[2].itmcod = LNM$_ATTRIBUTES;
	itemlist[2].buflen = sizeof(i4);
	itemlist[2].bufadr = (PTR)&attribs;
	itemlist[2].reslen = NULL;

	itemlist[3].itmcod = itemlist[3].buflen = 0;
