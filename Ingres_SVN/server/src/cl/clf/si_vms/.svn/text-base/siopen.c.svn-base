...
FILE		**desc)
{
	...
	*/
    ...
    /* Obtain the filename to determine if the file should be opened shared */
    LOdetail(location, null_buf, null_buf, fname, null_buf, null_buf);
    l_fname = STlength(fname);
    ...
    */
	...
	/*
	** If filename == IIACP, IIRCP, IICSP or IIDBMS...
	** allow shared read: at some cost in disk i/o: bug 75975
        */
	if ( ( (l_fname == 5) && (
		(STbcompare(fname, 5, "IIACP",  5, TRUE) == 0) ||
		(STbcompare(fname, 5, "IIRCP",  5, TRUE) == 0) ||
		(STbcompare(fname, 5, "IICSP",  5, TRUE) == 0)) ) ||
		( (l_fname >= 6) &&
		(STbcompare(fname, 6, "IIDBMS", 6, TRUE) == 0) ) )
	    p->fx_fab.fab$b_shr |= FAB$M_SHRGET;
