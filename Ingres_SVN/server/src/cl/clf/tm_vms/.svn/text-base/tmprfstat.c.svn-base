/*
** Copyright 1993, 2003 Ingres Corporation
*/
    ...
    CL_SYS_ERR  *sys_err)
{
    ...
    if (getrusage(RUSAGE_SELF, &pstat) != -1)
    {
        ...
        stat->tmperf_utime.TM_msecs     = pstat.ru_utime.tv_usec / 1000;
        stat->tmperf_stime.TM_secs      = pstat.ru_stime.tv_sec;
        stat->tmperf_stime.TM_msecs     = pstat.ru_stime.tv_usec / 1000;
    ...
    CL_SYS_ERR  *sys_err)
{
	...
	stat->tmperf_utime.TM_msecs = 
			    prusage.pr_utime.tv_nsec * (MILLISEC / NANOSEC);
	stat->tmperf_stime.TM_secs = prusage.pr_stime.tv_sec;
	stat->tmperf_stime.TM_msecs = 
			    prusage.pr_stime.tv_nsec * (MILLISEC / NANOSEC);
...
vms_perfstat(
    ...
    CL_SYS_ERR  *sys_err)
{
    II_VMS_ITEM_LIST_3	syi_itmlst[2];
    int		system_status;
    STATUS	status = OK;
