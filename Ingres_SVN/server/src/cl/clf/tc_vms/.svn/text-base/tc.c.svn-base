...
TCFILE	*desc;
{
	...
	status = sys$qiow(0, thechannel, writemode, 0, 0,
...
i4	seconds;
{
	...
	else
	{
	    ...
	    for (;;)
	    {
		status = sys$qiow(0, thechannel, readmode, &iosb, 0, 0,
                        cptr, TCBUFSIZE, 0, 0, 0, 0);

		if ((status & 1) == 0)
		    lib$signal(status);

		num_chars = iosb.byte_count;
		if (iosb.status == SS$_ENDOFFILE && iosb.sender_pid)
		{
		    achar = TC_EOF;
		    break;
		}
