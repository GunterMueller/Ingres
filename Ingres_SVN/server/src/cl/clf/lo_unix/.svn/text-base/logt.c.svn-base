...
LOgt ( char *buf, LOCATION *loc )
{
        ...
        { /* directory was changed */
                ...
                /* popen executes the shell command and sets up read pipe */
 
                if ((fp = popen("/bin/pwd", "r")) == NULL )
                                status = FAIL;
 
                /* read from the pipe the output of "pwd" */
                else if ((status = SIread(fp, MAX_LOC+1, &count, path)) == OK
                                || count > 0 )
                {
                        status = OK;
                        if ( count >= MAX_LOC+1 )
