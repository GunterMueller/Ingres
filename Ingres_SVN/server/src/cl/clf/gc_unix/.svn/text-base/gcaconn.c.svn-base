...
static GC_RQ_ASSOC	rq_assoc ZERO_FILL;
...
# endif /* OS_THREADS_USED */
    ...
    /*
    ** Receive the GC peer info from partner.  
    ** Hijack svc_parms.
    */
...
GC_recvpeer_sm( PTR closure )
{
    ...
    */
	...
	{
	    /*
	    ** Validate the peer information and extract 
	    ** info needed for GCA.
	    */
	    GC_RQ_ASSOC		*peer = &gcb->ccb.assoc_info.rq_assoc;
	    GC_RQ_ASSOC1	*peer1 = &gcb->ccb.assoc_info.rq_assoc1;

	    if ( 
		 peer->length < sizeof( GC_RQ_ASSOC )  ||
		 peer->id != GC_RQ_ASSOC_ID  ||
		 (peer->version == GC_ASSOC_V1  &&  
		     peer->length < sizeof( GC_RQ_ASSOC1 ))
	       )
	    {
		GCTRACE(1)( "GC_recvpeer_sm %d: invalid peer info!\n", 
			    gcb->id );
	    ...
	    GCTRACE(2)
		( "GC_recvpeer_sm %d: host %s, user %s, tty %s\n",
		  gcb->id, peer->host_name, peer->user_name, peer->term_name );

	    svc_parms->user_name = peer->user_name;
	    svc_parms->account_name = peer->user_name;
	    svc_parms->access_point_identifier = peer->term_name;
	    ...
	    */
	    ...
	    {
		if ( ! STcompare( peer->host_name, rq_assoc.host_name ) )
		    svc_parms->flags.trusted = 
			! STcompare(svc_parms->user_name, rq_assoc.user_name);
		else
		{
		    GCTRACE(1)( "GC_recvpeer_sm %d: invalid peer %s!\n", 
				gcb->id, peer->host_name );
		    ...
		    break;
		       ...
		       remote_access != GC_RA_ALL  && 
		       (remote_access != GC_RA_ENABLED  ||
			   peer->version == GC_ASSOC_V0  ||
			   ! (peer1->flags & GC_RQ_REMOTE)) 
		     )
	    {
		/*
	...
	else
	{
	    ...
	    */
	    ...
	    GCTRACE(1)( "GC_recvpeer_sm %d: received GCA peer info!\n", 
			gcb->id );
...
# endif /* OS_THREADS_USED */
	...
	GCTRACE(1)( "GCrequest %d: connecting on %s\n", gcb->id, bsp.buf );
		
	(*GCdriver->request)( &bsp );

	/* check status */
        
	GCTRACE(1)( "GCrequest %d: status %x\n", gcb->id, bsp.status );
...
GC_request_sm( SVC_PARMS *svc_parms )
{
    ...
    ** Hijack svc_parms.  Must allow room 
    ** for MTYP at head of buffer.
    */
    svc_parms->flags.flow_indicator = 0;
	...
	** Local connection.
	*/
	svc_parms->svc_send_length = sizeof( rq_assoc );
	MEcopy( (PTR)&rq_assoc, 
		sizeof( rq_assoc ), (PTR)svc_parms->svc_buffer );
    }
    else
    {
	/*
	...
	*/
	MEcopy( (PTR)&rq_assoc, 
		sizeof( rq_assoc ), (PTR)&rq_assoc1.rq_assoc0 );
	rq_assoc1.rq_assoc0.length = sizeof( rq_assoc1 );
	rq_assoc1.rq_assoc0.version = GC_ASSOC_V1;
	rq_assoc1.flags = GC_RQ_REMOTE;
	svc_parms->svc_send_length = sizeof( rq_assoc1 );
	MEcopy( (PTR)&rq_assoc1, 
		sizeof( rq_assoc1 ), (PTR)svc_parms->svc_buffer );
    }
...
GC_whoami( VOID )
{
    ...
    */
    ...
    if ( tty )
    {
	STcopy(tty, ttyname_buf);
	tty = ttyname_buf;
    }

    /* strip /dev/ from tty name */
    if ( tty && !STscompare( tty, 5, "/dev/", 5 ) )  tty += 5;

    gethostname( rq_assoc.host_name, sizeof( rq_assoc.host_name ) );
    rq_assoc.host_name[ sizeof( rq_assoc.host_name ) - 1 ] = EOS;
    STcopy( pw ? pw->pw_name : "nobody", rq_assoc.user_name );
    rq_assoc.user_name[ sizeof( rq_assoc.user_name ) - 1 ] = EOS;
    STcopy( tty ? tty : "batch", rq_assoc.term_name );
    rq_assoc.term_name[ sizeof( rq_assoc.term_name ) - 1 ] = EOS;
...
GC_check_uid( SVC_PARMS	*svc_parms )
{
	...
	*/
	if(!STbcompare(rq_assoc.user_name, 0, 
		user_name, info.len_user_id, TRUE))
	{
		/* Trusted */
		break;
