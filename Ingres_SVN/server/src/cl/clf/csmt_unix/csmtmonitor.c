    ...
    i4	    powerful)
{
	...
	else switch (an_scb->cs_state)
	{
	    ...
	    case CS_UWAIT:
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		    "Session %p:%s (%24s) cs_state: %w cs_mask: %v",
			    an_scb->cs_self,
#if defined(axp_osf)
			    STprintf(tidbuf, TIDFMT, an_scb->cs_thread_id->__sequence),
#else
	    ...
	    case CS_EVENT_WAIT: 
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		    "Session %p:%s (%24s) cs_state: %w (%s) cs_mask: %v",
			    an_scb->cs_self,
#if defined(axp_osf)
			    STprintf(tidbuf, TIDFMT, an_scb->cs_thread_id->__sequence),
#else
	    ...
	    case CS_MUTEX:
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		    "Session %p:%s (%24s) cs_state: %w ((%s) %p) cs_mask: %v",
			    an_scb->cs_self,
#if defined(axp_osf)
			    STprintf(tidbuf, TIDFMT, an_scb->cs_thread_id->__sequence),
#else
	    ...
	    case CS_CNDWAIT:
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		    "Session %p:%s (%24s) cs_state: %w (%p) cs_mask: %v",
			    an_scb->cs_self,
#if defined(axp_osf)
			    STprintf(tidbuf, TIDFMT, an_scb->cs_thread_id->__sequence),
#else
		...
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		    "Session %p:%s (%24s) cs_state: <invalid> %x",
		    an_scb->cs_self,
#if defined(axp_osf)
		    STprintf(tidbuf, TIDFMT, an_scb->cs_thread_id->__sequence),
#else
