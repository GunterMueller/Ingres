...

/*}
** Name: DI_CHECK_COMPLETE - Ensure DI operatin has completed
**
** Description:
**      This a VMS specific internal structure.
**      This structure is used to contain information about the
**      current status of a DI operation
**
** History:
**     28-Jan-2000 (horda03)
**          Created.
...
{
   int status;
#define DI_INPROGRESS 0
#define DI_COMPLETED  1
   CS_SID sid;
   char *descrip;
}
DI_CHECK_COMPLETE;
...
static i4	DI_qio(
		    ...
		    IOSB *iosb,
                    ...
                    i4   p5,
                    i4   p6);
static void	DI_resume(
		    DI_CHECK_COMPLETE *di_complete );
    ...
    CL_ERR_DESC		*err_code )
{
    ...
    s = DI_qio( "DI!DI.C::DIgalloc_1",
                CACHE_IO_EF, f->io_channel, IO$_ACCESS, &local_iosb,
	...
	if (local_iosb.iosb$w_status & 1)
	{
	    ...
	    */
	    ...
	    MEfill(sizeof(FAT), NULL, (char *)&io_rec);
	    ...
	    */
		...
		s = DI_qio("DI!DI.C::DIgalloc_2",
                           CACHE_IO_EF, f->io_channel, IO$_MODIFY, &local_iosb,
                         (i4) &f->io_fib_desc, 0, 0, 0, (i4) io_att, 0);
		if (s & 1)
		{
		    if (local_iosb.iosb$w_status & 1)
...
CL_ERR_DESC     *err_code)
{
    ...
    long	  device_class;
    long	  dvi_length;
    II_VMS_ITEM_LIST_3  itmlst[2] = { {4, DVI$_DEVCLASS, &device_class, &dvi_length},
				{0, 0, 0, 0} };
#endif
	...
	*/
	...
	s = DI_qio("DI!DI.C::DIclose_1",
                   CACHE_IO_EF, f->io_channel, IO$_DEACCESS, &local_iosb,
		0, 0, 0, 0, 0, 0);
...
CL_ERR_DESC	*err_code)
{
    ...
    MEfill (sizeof(DI_IO), NULL, (char *)f);
	...
	s = DI_qio("DI!DI.C::DIcreate_2",
                   CACHE_IO_EF, f->io_channel, IO$_CREATE | IO$M_CREATE, &local_iosb,
                (i4) &f->io_fib_desc, (i4) &file_desc, 0, 0, (i4) io_att, 0);
	if (s & 1)
	{
	    if (local_iosb.iosb$w_status & 1)
...
CL_ERR_DESC     *err_code)
{
	...
	*/
	...
	s = DI_qio("DI!DI.C::DIdelete_1",
                    CACHE_IO_EF, f->io_channel, IO$_ACCESS | IO$M_ACCESS, &local_iosb,
	...
	if (s & 1)
	{
		...
		*/
		...
		s = DI_qio("DI!DI.C::DIdelete_2",
                           CACHE_IO_EF, f->io_channel, IO$_DELETE | IO$M_DELETE, &local_iosb,
		     (i4) &f->io_fib_desc, (i4) &file_desc, 0, 0, 0, 0);
...
CL_ERR_DESC     *err_code)
{
    ...
    s = rms_suspend(sys$parse(&fab, rms_resume, rms_resume), &fab);
    if ((s & 1) == 0)
    {
	/*
	...
	*/
    ...
    rms_suspend(sys$parse(&fab, rms_resume, rms_resume), &fab);
...
CL_ERR_DESC     *err_code)
{
	...
	*/
	...
	s = DI_qio("DI!DI.C::DIread_1",
                   CACHE_IO_EF, f->io_channel, IO$_READVBLK, &local_iosb,
...
CL_ERR_DESC     *err_code)
{
    ...
    s = rms_suspend(sys$rename(&oldfab, rms_resume, rms_resume, &newfab), &oldfab);
    if ((s & 1) == 0)
    {
	/*
	**  If RMS gets SS$_NOIOCHAN error it returns a incomplete
...
CL_ERR_DESC     *err_code)
{
	...
	*/
	...
	if ( last_block_to_write > f->io_alloc_eof )
	{
	    i4	real_eof;
		...
		*n = 0;
		if ( s )
		    return(s);
		TRdisplay("%@ DIwrite %d: io_alloc_eof: %d io_system_eof: %d\n",
		    __LINE__, f->io_alloc_eof, f->io_system_eof);
		return(DI_ENDFILE);
	    }
	}
	...
	s = DI_qio("DI!DI.C::DIwrite_1",
                   CACHE_IO_EF, f->io_channel, IO$_WRITEVBLK, &local_iosb,
                 (i4) buf, f->io_block_size * *n, (page << f->io_blks_log2) + 1,
		 0, 0, 0);
...
CL_ERR_DESC	    *err_code)
{
	...
	*/
	...
	i4	    i;
        ...
        s = rms_suspend(sys$search(fab, rms_resume, rms_resume), fab);
	if ((s & 1) == 0)
	{
            err_code->error = s;
...
i4 p6)
{
   ...
   di_complete.status  = DI_INPROGRESS;
   di_complete.descrip = descrip;
   CSget_sid(&di_complete.sid);

   CSnoresnow(descrip, 1);

   s = sys$qio( efn, channel, func, iosb, DI_resume, &di_complete,
                p1, p2, p3, p4, p5, p6 );
   ...
   if (s & 1)
   {
      ...
      do
      {
         ...
         if (di_complete.status == DI_INPROGRESS)
         {
            if (!scb) CSget_scb( &scb );
            TRdisplay("[%x, %x] %@ - DI_QIO resumed in error at %s\n",
                       scb->cs_pid, di_complete.sid, descrip );
         }
       } while (di_complete.status == DI_INPROGRESS);
    }
...
void
DI_resume( DI_CHECK_COMPLETE *di_complete )
{
   di_complete->status = DI_COMPLETED;

   CSresume( di_complete->sid );
}
