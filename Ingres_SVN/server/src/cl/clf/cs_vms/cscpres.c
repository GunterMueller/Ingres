...
CScp_resume( CS_CPID *cpid )
{
	...
	{
	    /* If reader already found to be dead, do nothing */
	    if ( !(CPchan->IsDead) )
	    {
		...
		*/
		...
		vms_status = sys$qio(0, CPchan->chan, 
		...
		if ( vms_status != SS$_NORMAL 
		     || (vms_status = cpres_write_iosb.iosb$w_status) != SS$_NORMAL )
		{
		    /*
		    ** If reading process has died, mark it's PID as dead,
		    ** cancel any leftover I/O, deassign our channel.
		    */
		    if ( vms_status == SS$_NOREADER )
		    {
			/* Keep others out while we deadify the CP_CHANNEL */
			while ( !CS_TAS(&cpres_channels_sem) );

			CPchan->IsDead = TRUE;
			sys$cancel(CPchan->chan);
			sys$dassgn(CPchan->chan);

			CS_ACLR(&cpres_channels_sem);

			STprintf(msg_buf, "PID %x on channel %d is dead, deassigned",
				CPchan->pid, CPchan->chan);
		    ...
		    {
			STprintf(msg_buf, "Error (%x) queueing write to %x on channel %d",
...
clean_channels(void)
{
	...
	if ( CPchan->IsDead || !(PCis_alive(CPchan->pid)) )
	{
	    ...
	    */
		...
		cpres_channels[j - 1].IsDead  = cpres_channels[j].IsDead;
