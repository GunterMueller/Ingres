/*
** Copyright (c) 1998, 2005 Ingres Corporation
**
*/
...
static u_i2 IDuuid_sequence(i4 seed);
...
)
{
    ...
    clock_seq = IDuuid_sequence(uuid->Data1);
...
IDuuid_time()
{
    ...
    if( cur.tv_sec < static_time.tv_sec ||
	(cur.tv_sec == static_time.tv_sec &&
	(cur.tv_nsec - static_time.tv_nsec) < 100) )
    {
	cur = static_time;
...
# define have_uuid_node_code
    ...
    int dev_char;
    uint64 contxt = 0;
    II_VMS_EF_NUMBER efn = EFN$C_ENF;
    II_VMS_ITEM_LIST_3 devclass[2];
    II_VMS_ITEM_LIST_3 devinfo[3];
...
#pragma member_alignment __restore
    ...
    devclass[0].buflen = sizeof(devclass_code);
    devclass[0].itmcod = DVS$_DEVCLASS;
    devclass[0].bufadr = &devclass_code;
    devclass[0].reslen = &reslen[0];
    devclass[1].buflen = devclass[1].itmcod = 0;
    /* initialize item list for sys$getdviw() */
    devinfo[0].buflen = sizeof(dev_status);
    devinfo[0].itmcod = DVI$_STS;
    devinfo[0].bufadr = &dev_status;
    devinfo[0].reslen = &reslen[1];
    devinfo[1].buflen = sizeof(dev_char);
    devinfo[1].itmcod = DVI$_DEVCHAR;
    devinfo[1].bufadr = &dev_char;
    devinfo[1].reslen = &reslen[2];
    devinfo[2].buflen = devinfo[2].itmcod = 0;
	...
	status = sys$getdviw(efn, 0, &devnam, devinfo, &iosb, 0, 0, 0); 
	    ...
	    (dev_char & DEV$M_MBX | DEV$M_NET) == DEV$M_NET)
	{
	    ...
	    status = sys$qiow(efn, chan, IO$_SENSEMODE | IO$M_CTRL, &iosb, 0,
		0, 0, &attdsc, 0, 0, 0, 0);
	    if (status == SS$_NORMAL && iosb.iosb$w_status == SS$_NORMAL)
	    {
