         ...
         CL_SYS_ERR *err_code)
{
    ...
    if ( f->io_sem.cs_sem_scribble_check != CS_SEM_LOOKS_GOOD)
    {
        /* need to initialize the file descriptor mutex semaphore */
        STprintf( sem_name, "DI %12.12s", filename, &f->io_sem );

        if (CSw_semaphore(&f->io_sem, CS_SEM_SINGLE, sem_name))
        {
                if (f->io_sem.cs_thread_id == ERROR_NOT_ENOUGH_MEMORY)
                    return DI_EXCEED_LIMIT;  /* some callers have retry logic */
                else
                    return DI_BADOPEN;
        }
    }
