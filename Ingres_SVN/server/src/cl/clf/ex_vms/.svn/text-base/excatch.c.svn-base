...
EXcatch(struct chf$signal_array *sigs, struct chf$mech_array *mechs)
{
	...
	*/
	...
	for ( env = i_EXtop();; env = i_EXnext( env )) {
	    jmpbuf = env->iijmpbuf;
	...
	*/
	...
	case EX_CONTINUES:
	    if ( dont_continue )
		TRdisplay( "EXcatch(): ignoring EX_CONTINUES return from user\
