...
# define	CTRLC		0x651	/* Operation completed under Ctrl-C */
# define	DATAOVERRUN	0x838   /* Data overrun */
...
i4			*delay_constant;
{
	...
	for (;;)
	{
		...
		status = sys$qiow(0, channel, IO$_SENSEMODE, &tt_iosb, 0, 0, buffer, 8, 0, 0, 0, 0);
		if ((status & 1) == 0)
		{
			sys$dassgn(channel);
...
TEout()
{
	...
	status = sys$qiow(0, te_out_channel, IO$_WRITEVBLK|IO$M_NOFORMAT, 0, 0, 0,
...
i4	seconds;
{
	...
	*/
	...
	for (;;)
	{
		status = sys$qiow(0, te_in_channel, readmod, &iosb, 0, 0,
...
i4	which;
{
	...
	if (channel != 0)
	{
	    sys_stat = sys$qiow(0, channel, IO$_SETMODE, 0,
...
char	*terminal;
{
	...
	jpi_list[0].buffer_length = 8;
	jpi_list[0].item_code = JPI$_TERMINAL;
	jpi_list[0].buffer_address = terminal;
	jpi_list[0].return_length = (i4 *)&terminal_length;
	jpi_list[1].buffer_length = 4;
	jpi_list[1].item_code = JPI$_OWNER;
	jpi_list[1].buffer_address = (PTR)&parent_pid;
	jpi_list[1].return_length = 0;
	jpi_list[2].buffer_length = 0;
	jpi_list[2].item_code = 0;

	/*	see if we are a login process	*/

	status = sys$getjpi(0, 0, 0, jpi_list, 0, 0, 0);
	if ((status & 1) == 0)
		return (FAIL);
	sys$waitfr(0);
	...
	{
		status = sys$getjpi(0, &parent_pid, 0, jpi_list, 0, 0, 0);
		if ((status & 1) == 0)
			return (FAIL);
		sys$waitfr(0);
	}
...
struct	dsc$descriptor_s	*term;
{
	...
	dvi_list[0].buffer_length = sizeof(class);
	dvi_list[0].item_code = DVI$_DEVCLASS;
	dvi_list[0].buffer_addr = (PTR)&class;
	dvi_list[0].return_length = 0;
	dvi_list[1].buffer_length = 0;
	dvi_list[1].item_code = 0;
	...
	*/
	...
	status = sys$getdvi(0, 0, term, dvi_list, &iosb, 0, 0, 0);
	if ((status & 1) == 0)
	{
		return(retval);
	}
...
TEsave()
{
	...
	if (te_in_channel != 0)
		...
		return(OK);
	sys_stat = sys$qiow(0, channel, IO$_SENSEMODE, 0, 0, 0,
...
TEcmdmode()
{
	...
	sys_stat = sys$qiow(0, channel, IO$_SETMODE, 0, 0, 0,
			    &_tty, sizeof(_tty), 0, 0, 0, 0);
	if (! (sys_stat & SS$_NORMAL))
	{
		return(FAIL);
