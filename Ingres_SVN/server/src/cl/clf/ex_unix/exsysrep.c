...
# ifdef sur_u42
    ...
    int                 eax = 0;
    int                 edx = 0;
# define        REGFMT "@ PC = 0x%x, SP = 0x%x, PSL 0x%x, EAX 0x%x, EDX 0x%x"
# endif	/* sur_u42 */
...
# define        REGFMT "@ SP = 0x%x, SXIP = 0x%x, SNIP = 0x%x, SFIP = 0x%x"
# endif /* dg8_us5 dgi_us5 */
# if defined(ris_us5) || defined(rs4_us5) || defined(ris_u64) /*see <mstsave.h>*/
...
# endif
# ifdef sur_u42
	STprintf(buffer, fmt, pc, sp, psl, eax, edx );
# define gotfmt
# endif
# if defined(pyr_us5)
...
#   if ( defined(xCL_072_SIGINFO_H_EXISTS) || defined(LNX) ) \
	 && defined(xCL_071_UCONTEXT_H_EXISTS)
#    if (defined(dr6_us5) || defined(hp8_us5)) && !defined(LP64)
...
# elif defined(hp8_us5) || defined(int_lnx) || defined(int_rpl) || \
