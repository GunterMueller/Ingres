...
report_wakeups(CS_SMCNTRL	*sysseg)
{
    ...
    */
    ...
    wb_mem = MEreqmem( 0, no_wakeups * sizeof(CS_SM_WAKEUP_CB), FALSE, NULL);
    ...
    if (wb_mem)
    {
       ...
       MEcopy( (PTR) wakeup_blocks, no_wakeups * sizeof(CS_SM_WAKEUP_CB), wb_mem);

       CS_relspin(&sysseg->css_spinlock);

       wakeup_blocks = (CS_SM_WAKEUP_CB *) wb_mem;
