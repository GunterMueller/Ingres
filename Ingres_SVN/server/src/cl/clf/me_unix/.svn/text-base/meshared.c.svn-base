...
# if !(defined(hp8_us5) || defined(axp_osf) || defined(hp8_bls))
    if ((flags & ME_ADDR_SPEC) && *addr == 0)
    {
	return(ME_BAD_PARAM);
    }
# endif /* hp8_us5 hp8_bls axp_osf */
...
# if defined(dr6_us5) ||  \
     ...
     defined(sui_us5) || defined(xCL_SHMDT_ONE_ARG) 
	SHMDT(*addr);
# else
	...
	char	 loc_buf[])
{
    ...
    {
	if ((ret_val = NMloc(FILES, PATH, (char *)NULL, &temp_loc)) == OK)
	{
	    LOcopy(&temp_loc, loc_buf, location);
