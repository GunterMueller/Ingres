...
GCwinsock_init(GCC_PCE * pptr)
{
	...
	*/
	...
	{
	    status = GetLastError();
	    GCTRACE(1)("GCwinsock_init: Couldn't create thread status = %d\n",
	    		status );
	    return FAIL;
	}
...
GCwinsock( i4 function_code, GCC_P_PLIST * parm_list)
{
	...
	*/
	...
	GCTRACE(1)("GCconn_connect: Tptr=%8x, plist->compl_id=%8x ->%8x,%8x,%8x hEvent=%8x\n",
		...
		/*
		** Spawn off a thread to handle the listem request
		*/
		hThread = CreateThread(&sa,
			       GC_STACK_SIZE,
	...
	*/
	...
	for (;;)
	{
	    ...
	    */
		...
		status = WSAGetLastError();
		/*	
		** Re-try only for alphanumeric ports.
		*/
