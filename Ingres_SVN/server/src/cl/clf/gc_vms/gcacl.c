...
SVC_PARMS          *svc_parms;
{
	...
	*/
	...
	qiostat = sys$getdviw(0, a_chan->chan, 0, dvi_list, &ioxb, 0, 0, 0);
...
i4	sync;
{
    ...
    ** do the zombie check now--but only if this process is a server.
    */

    if( GCzomb_time && !GCzomb_tmrset && mode == GC_SERVER_MODE)
	GCqio_zombie();
...
i4		channel;
{
    ...
    if( VMS_FAIL( sys$getdviw(0, channel, 0, dvi_list, &ioxb, 0, 0, 0) ) )
	return -1;
...
char	*invoker;
{
    ...
    */
	...
	status = sys$getdviw(0, 0, &term_desc, dviget2,	&ioxb, 0, 0, 0);
	if (VMS_FAIL(status))
	    return status;
