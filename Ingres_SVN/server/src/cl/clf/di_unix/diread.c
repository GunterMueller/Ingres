...
#ifdef LNX
	IIdio_alloc_realign(unix_fd, bytes_to_read, 
			    &apages, buf, &abuf, 
			    f->io_fprop,
			    __FUNCTION__);
#else
...
/* Note: might need to refine this test to avoid pread on platforms where
** it's slower than lseek+read.  Supposedly this is true on Linux, although
** I'm not entirely sure I believe that.  (schka24 Sep '05)
*/

# if defined( OS_THREADS_USED ) && (! defined (xCL_NO_ATOMIC_READ_WRITE_IO))
#ifdef LARGEFILE64
    bytes_read = pread64( unix_fd, abuf, bytes_to_read, lseek_offset );
#else /* LARGEFILE64 */
    bytes_read = pread( unix_fd, abuf, bytes_to_read, lseek_offset );
#endif /* LARGEFILE64 */
...
# else /* OS_THREADS_USED */
    ...
    bytes_read = IIdio_read( unix_fd, abuf, bytes_to_read,
 	    			  lseek_offset, 0, 
				  f->io_fprop,
				  err_code );
