...
GLOBALREF PTR Di_zero_buffer;		/* Zeroing buffer and its size */
...
# endif /* OS_THREADS_USED */
	...
	/*
	** If we failed to guarantee the space do not bother updating
	** the following
	*/
	if ( status == OK )
	{
	    ...
	    /*
	    ** The following check should be false as we should have either
...
#ifdef xDEV_TST
	    TRdisplay(
	    "DIguarantee: file: %t/%t, start: %d, number: %d, alloc_eof: %d\n",
		      f->io_l_pathname, f->io_pathname,
		      f->io_l_filename, f->io_filename,
		      start_pageno, number_pages, f->io_alloc_eof );
#endif
	}
# ifdef OS_THREADS_USED
