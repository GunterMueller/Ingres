...
**						of writing.
**				DI_USER_SYNC_MASK Caller will sync when he
**						  need to.
**      
** Outputs:
...
# endif /* OS_THREADS_USED */
	...
	FPROP_DIRECT_REQ_SET(f->io_fprop);
	...
	/*
	** if both options have been specified then only use O_SYNC since the
	** fsync() does not provide any more functionality.
	*/
...
#if defined(xCL_010_FSYNC_EXISTS) && !defined(O_SYNC)
	/*
	** If FSYNC exists but O_SYNC does not, use fsync.  This
  	** is a bad performance configuration and should probably never occur.
	*/
	f->io_open_flags |= DI_O_FSYNC_MASK;
#endif
...
#if defined(xCL_010_FSYNC_EXISTS) && defined(O_SYNC)
	/*
	** Both O_SYNC and fsync() exist, set di user sync mask.
	** At this point, there are no known platforms on which
	** an fsync() is required when the file is LRU's out, but
	** this open flag is set in case such a platform exists.
	*/
...
# ifdef OS_THREADS_USED
    if ( ret_val == OK )
    {
        /* need to initialize the file descriptor mutex semaphore */
        STprintf( sem_name, "DI %12.12s", filename );
         
        if (CSw_semaphore(&f->io_sem, CS_SEM_SINGLE, sem_name))
        {
            ret_val = DI_BADOPEN; 
        }
    }
