    ...
    CL_ERR_DESC    *err_code)
{
	...
	*/
	    ...
	    (last_page_to_write <= f->io_alloc_eof))
	{
	    ...
	    TRdisplay(
		"DIwrite(): Returning false DI_EXCEED_LIMIT, page %d\n", page);
	}
	else
...
# endif /* OS_THREADS_USED && !xCL_NO_ATOMIC_READ_WRITE_IO */
	...
	/* ensure we're writing to aligned pages on fds that require it */
#ifdef LNX
	if (IIdio_alloc_realign(diop->di_fd, bytes_to_write, &apages, 
			buf, &abuf, f->io_fprop, __FUNCTION__))
		MEcopy (buf, bytes_to_write, abuf);
#else
	abuf = buf;
#endif

/* Note: might need to refine this test to avoid pwrite on platforms where
** it's slower than lseek+write.  Supposedly this is true on Linux, although
** I'm not entirely sure I believe that.  (schka24 Sep '05)
*/

# if  defined(OS_THREADS_USED) && !defined(xCL_NO_ATOMIC_READ_WRITE_IO)
    bytes_written =
#ifdef LARGEFILE64
 	 pwrite64( (int)diop->di_fd, abuf, bytes_to_write, lseek_offset );
#else /*  LARGEFILE64 */
 	 pwrite( (int)diop->di_fd, abuf, bytes_to_write, lseek_offset );
#endif /* LARGEFILE64 */
# else /* OS_THREADS_USED  !xCL_NO_ATOMIC_READ_WRITE_IO */
    bytes_written =
 	 IIdio_write( (int)diop->di_fd, abuf, bytes_to_write,
 	 	       lseek_offset, 0, 
		       f->io_fprop,
		       err_code );
