...
CSn_semaphore(CS_SEMAPHORE *sem, char *string)
{
    ...
    if (VersionString[0] == '\0' || sem_name[0] == '\0')
    {
	...
	if (short_sem == -1)
	{
		NMgtAt("II_SHORT_SEM_NAME", &env);
	    if ( env && *env)
		{
			if ( CMcmpnocase(env, "Y") == 0)
			{
				TRdisplay("%@ II_SHORT_SEM_NAME enabled.\n");
				short_sem = 1;
			}
			else
				short_sem = 0;
	...
	{
		STlpolycat( 2, sizeof(sem->cs_sem_name), sem_name, string, sem->cs_sem_name );
	}
}
