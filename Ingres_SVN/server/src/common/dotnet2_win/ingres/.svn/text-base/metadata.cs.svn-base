...
namespace Ingres.Client
{
	...
	internal static class IngresMetaData
	{
			...
			string[]     restrictionValues)
		{
			...
			IngresConnection ingresConn = (IngresConnection)conn;
			if (conn != null)
			{
				...
				{
					string s;
					s = advanConn.conn.dbCaps.getDbCap(
						DbmsConst.DBMS_DBCAP_UCS2_TYPES);
					if (s == null  ||  s[0] == 'N')
						isUnicodeSupported = false;  // no NCHAR, ... support

					if (advanConn.conn.db_protocol_level >= DbmsConst.DBMS_API_PROTO_4)
						isANSIDateTimeSupported = true;	// TODO: check for 'LEVEL 1'
				}
			}
				...
				in providerTypeList)
			{
				...
				Int64 columnSize =
					ProviderTypeMgr.ColumnSize(providerType);
				Object createFormat =
				...
				Int16 maximumScale =
					ProviderTypeMgr.MaximumScale(providerType);
				Int16 minimumScale =
			...
			string[] restrictionValues)
		{
			...
			while (rdr.Read())
			{
				...
				if (typeName == "INTEGER")  // use specific INT typename
