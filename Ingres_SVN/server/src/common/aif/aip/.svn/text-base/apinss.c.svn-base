/*
** Copyright (c) 1993, 2004 Ingres Corporation All Rights Reserved.
*/
...
static II_VOID		ns_read_result( IIAPI_STMTHNDL *, IIAPI_GCA_BUFF * );
...
# define NS_SA_DELH	((IIAPI_ACTION) 14)	/* Delete statement handle */
# define NS_SA_ERQA	((IIAPI_ACTION) 15)	/* Query active */
# define NS_SA_ERQD	((IIAPI_ACTION) 16)	/* Query done */
# define NS_SA_ERQC	((IIAPI_ACTION) 17)	/* Query cancelled */
# define NS_SA_CBOK	((IIAPI_ACTION) 18)	/* Callback Success */
# define NS_SA_CBFL	((IIAPI_ACTION) 19)	/* Callback Failure */
# define NS_SA_CBND	((IIAPI_ACTION) 20)	/* Callback No Data */
# define NS_SA_CBGC	((IIAPI_ACTION) 21)	/* Callback getColumns status */
# define NS_SA_CBIF	((IIAPI_ACTION) 22)	/* Callback Invalid Function */
# define NS_SA_CNOK	((IIAPI_ACTION) 23)	/* Callback Cancel Success */
# define NS_SA_CNWN	((IIAPI_ACTION) 24)	/* Callback Cancel Warning */
# define NS_SA_CNFL	((IIAPI_ACTION) 25)	/* Callback Cancel Failure */
# define NS_SA_RCVE	((IIAPI_ACTION) 26)	/* Receive Error */
# define NS_SA_RLSB	((IIAPI_ACTION) 27)	/* Release Buffer */

# define NS_SA_CNT     (NS_SA_RLSB + 1)

static char *ns_sa_id[] =
{
    "REMC", "CNCL", "RECV", "RCVB", "SQRY", "SETD", "SPRM", "RERR", "RRES", 
    "BRES", "INFO", "DESC", "RSLT", "BUFF", "DELH", "ERQA", "ERQD", "ERQC", 
    "CBOK", "CBFL", "CBND", "CBGC", "CBIF", "CNOK", "CNWN", "CNFL", "RCVE",
    "RLSB"
};
...
static IIAPI_SM_OUT ns_act_seq[] =
{
    ...
    { 0, 2, { NS_SA_RERR, NS_SA_RCVB, 0 } },			/* SAS_23 */
	...
	case IIAPI_EV_GCN_RESULT_RCVD :
		...
		case NS_SS_WQDR :
		    ...
		    */ 
			...
			value = ! GCN_RET;
		    else  if ( ! gcaBuff->formatted )
			gcu_get_int( gcaBuff->buffer, &value );
		    else
		    {
			resData = (GCD_GCN_OPER_DATA *)gcaBuff->buffer;
			value = resData->gcn_op;
		    }
		    ...
		    else  if ( NS_EOG( connHndl, 
				       (GCA_RV_PARMS *)gcaBuff->gca_parm ) )
		    {
		...
		case NS_SS_WTR :
			...
			*/
		    ...
		    if ( value == GCN_RET )
			if ( ns_eval_data( stmtHndl, getColParm, gcaBuff ) )
			{
			    /*
...
)
{
	...
	case NS_SA_RECV :
	    ...
	    */
	    gcaBuff = IIapi_allocGCABuffer( API_GCN_BUFF_SIZE, 
	...
	case NS_SA_SQRY :
	    ...
	    */
	    if ( ! (parmNmem = IIapi_createGCNOper( stmtHndl )) )
	    {
		status = IIAPI_ST_OUT_OF_MEMORY;
		success = FALSE;
		break;
	...
	case NS_SA_RERR :
		...
		*/
	    ...
	    /*
	    ** We be done with the message, so be sure 
	    ** to mark the GCA buffer as consumed.
	    */
	    gcaBuff->data_len = 0;
	    break;
	...
	case NS_SA_RRES :
	    ...
	    */
	    ns_read_result( stmtHndl, (IIAPI_GCA_BUFF *)parmBlock );
	    break;
	...
	case NS_SA_BRES :
	    ...
	    */
	    if ( stmtHndl->sh_gcaBuff )
		ns_read_result( stmtHndl, stmtHndl->sh_gcaBuff );
	    break;
	...
	case NS_SA_BUFF :
	    ...
	    */
	    if ( gcaBuff != stmtHndl->sh_gcaBuff )
	    {
		if ( stmtHndl->sh_gcaBuff )
...
static II_VOID
ns_read_result( IIAPI_STMTHNDL *stmtHndl, IIAPI_GCA_BUFF *gcaBuff )
{
    ...
    */
	...
	else
	{
	    ...
	    */
	    ...
	    gcaBuff->data += gcu_get_int( gcaBuff->data, &row_count );
	    gcaBuff->data += gcu_get_int( gcaBuff->data, &row_count );
	}
...
)
{
    ...
    */
    ...
    if ( gcaBuff->formatted )
	end = gcaBuff->data + gcaBuff->data_len;
    else
    {
	/*
    ...
    */
	...
	{
	    gcaBuff->data = 
		(char *)&((GCD_GCN_OPER_DATA *)gcaBuff->data)->gcn_tuple[0];
	    gcaBuff->data_len = end - gcaBuff->data;
	}

    if ( gcaBuff->formatted )
	row_count = (end - gcaBuff->data) / sizeof( GCD_GCN_REQ_TUPLE );
	...
	*/
	IIapi_loadNSColumns( stmtHndl, getColParm, gcaBuff->formatted 
			     ? (GCD_GCN_REQ_TUPLE *)gcaBuff->data : &tuple );
	...
	*/
	if ( stmtHndl->sh_colIndex >= stmtHndl->sh_colCount )
	{
	    ...
	    */
	    if ( gcaBuff->formatted )
		gcaBuff->data += sizeof( GCD_GCN_REQ_TUPLE );
	    else
		gcaBuff->data = ptr;
