...
static SM_TRANSITION smt_list[] =
{
    ...
    */
    ...
    { IIAPI_EV_REJECT_RCVD,	SQL_TS_CONN,	SQL_TS_IDLE,	SQL_TAS_6  },
    { IIAPI_EV_RELEASE_RCVD,	SQL_TS_IDLE,	SQL_TS_IDLE,	SQL_TAS_6  },
    { IIAPI_EV_RELEASE_RCVD,	SQL_TS_CONN,	SQL_TS_IDLE,	SQL_TAS_6  },
...
)
{
	...
	*/
	case IIAPI_EV_RESPONSE_RCVD :
	    ...
	    if ( reData  &&  reData->gca_rqstatus & GCA_LOG_TERM_MASK )
	    {
		/*
...
)
{
	...
	case SQL_TA_SCOM :
	    ...
	    */
	    if ( ! (parmNmem = IIapi_createGCAEmpty( sm_hndl )) )
	    {
	    ...
	    parmNmem->pm_msgType = GCA_COMMIT;
	...
	case SQL_TA_SRB :
	    ...
	    */
	    ...
	    parmNmem->pm_msgType = GCA_ROLLBACK;
	    status = IIapi_sndGCA( sm_hndl, parmNmem, NULL );
	...
	case SQL_TA_SSP :
	{
	    ...
	    */
	    ...
	    if ( ! ( parmNmem = IIapi_createGCAQuery( NULL, IIAPI_QT_QUERY, 
						      queryText, TRUE,
						      sm_hndl, (II_INT2)0 ) ) )
	    {
	...
	case SQL_TA_SSEC :
	    ...
	    */
	    if ( ! ( parmNmem = IIapi_createGCASecure( tranHndl ) ) )
	    {
		status = IIAPI_ST_OUT_OF_MEMORY;
		success = FALSE;
		break;
	...
	case SQL_TA_SXAS :
	{
	    ...
	    */
	    ...
	    if ( ! (parmNmem = IIapi_createGCAxa( GCA_XA_START,
	...
	case SQL_TA_SXAE :
	{
	    ...
	    */
	    ...
	    if ( ! (parmNmem = IIapi_createGCAxa( GCA_XA_END,
	...
	case SQL_TA_RRSP :
	{
	    ...
	    */
	    if ( 
	         ...
	         rspData->gca_errd5  &&
	         ! IIapi_xaError( sm_hndl, rspData->gca_errd5 ) 
	       )
	    {
	    	status = IIAPI_ST_OUT_OF_MEMORY;
		success = FALSE;
	...
	case SQL_TA_RXAR :
	{
	    ...
	    */
	    ...
	    if ( ! IIapi_xaError( sm_hndl, rspData->gca_errd5 ) )
	    {
	    	status = IIAPI_ST_OUT_OF_MEMORY;
		success = FALSE;
	    }
