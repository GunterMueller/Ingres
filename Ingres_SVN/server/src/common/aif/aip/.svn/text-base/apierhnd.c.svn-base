...
** Local function
*/
II_VOID deleteErrorHndl (IIAPI_ERRORHNDL *errorHndl);
...
IIapi_createErrorHndl( IIAPI_HNDL *handle )
{
    ...
    if ( IIapi_isQueEmpty( &handle->hd_errorList ) )
...
serverErrorText( IIAPI_ERRORHNDL *errorHndl )
{
    ...
    */
    ...
    {
	if ( ! ( errorHndl->er_message = MEreqmem(0, 33, TRUE, &status) ) )
	{
	    IIAPI_TRACE( IIAPI_TR_FATAL )
		( "serverErrorText: can't alloc error text\n" );
	    return( FALSE );
	...
	STcopy( "Message text not available!", errorHndl->er_message );
    }
...
**	from a server GCA_ERROR message.
**
**	handle		handle to which the error is added
**      errorData	error information from GCA
**
** Return value:
**      status     TRUE if errors are added to error list, FALSE otherwise.
**
...
)
{
    ...
    for( ; errorCount > 0; errorCount--, elem++ )
    {
	/*
	...
	*/
	...
	if ( ! ( errorHndl->er_serverInfo = (IIAPI_SVR_ERRINFO *)
		 MEreqmem( 0, sizeof(IIAPI_SVR_ERRINFO), TRUE, &status ) ) )
	{
	    IIAPI_TRACE( IIAPI_TR_FATAL )
		( "IIapi_serverError: can't alloc server info\n" );
	    goto allocServerFail;
	}
	...
	if ( elem->gca_l_error_parm )
	{
	    if ( ! ( errorHndl->er_serverInfo->svr_parmDescr = 
			(IIAPI_DESCRIPTOR *)MEreqmem( 0, 
			    elem->gca_l_error_parm * sizeof(IIAPI_DESCRIPTOR),
			    TRUE, &status ) ) )
	    {
		IIAPI_TRACE( IIAPI_TR_FATAL )
	    ...
	    if ( ! ( errorHndl->er_serverInfo->svr_parmValue = 
			(IIAPI_DATAVALUE *)MEreqmem( 0, 
			    elem->gca_l_error_parm * sizeof(IIAPI_DATAVALUE),
			   TRUE, &status ) ) )
	    {
		IIAPI_TRACE( IIAPI_TR_FATAL )
	...
	else  if ( elem->gca_severity & GCA_ERWARNING )
	    errorHndl->er_type = IIAPI_GE_WARNING;
	else
	    errorHndl->er_type = IIAPI_GE_ERROR;
	
	if ( errorHndl->er_type == IIAPI_GE_MESSAGE )
	    MEcopy( II_SS5000G_DBPROC_MESSAGE, II_SQLSTATE_LEN,
		    errorHndl->er_SQLSTATE );
	else  if ( protocolLevel < GCA_PROTOCOL_LEVEL_60 )
	    MEcopy( IIapi_gen2IntSQLState( elem->gca_id_error, 
					   elem->gca_local_error ),
		    II_SQLSTATE_LEN, errorHndl->er_SQLSTATE );
	else
	    ss_decode( errorHndl->er_SQLSTATE, elem->gca_id_error );
	
	errorHndl->er_SQLSTATE[ II_SQLSTATE_LEN ] = '\0';
	errorHndl->er_errorCode = elem->gca_local_error;

	errorHndl->er_serverInfo->svr_id_error = elem->gca_id_error;
	errorHndl->er_serverInfo->svr_local_error = elem->gca_local_error;
	errorHndl->er_serverInfo->svr_id_server = elem->gca_id_server;
	errorHndl->er_serverInfo->svr_server_type = elem->gca_server_type;
	errorHndl->er_serverInfo->svr_severity = elem->gca_severity;
	errorHndl->er_serverInfo->svr_parmCount = elem->gca_l_error_parm;
	
	for( j = 0; j < errorHndl->er_serverInfo->svr_parmCount; j++ )
	{
	    dataValue = &elem->gca_error_parm[j];

	    if ( ! IIapi_cnvtGDV2Descr( 
			envHndl,
			(IIAPI_DT_ID)dataValue->gca_type, 
			dataValue->gca_l_value,
			dataValue->gca_precision, 0, NULL,
			&errorHndl->er_serverInfo->svr_parmDescr[ j ] ) )
	    {
		IIAPI_TRACE( IIAPI_TR_FATAL )
		    ( "IIapi_serverError: can't alloc error parm descr\n" );
		goto valueFail;
	    }

	    if ( ! ( errorHndl->er_serverInfo->svr_parmValue[ j ].dv_value =
		     MEreqmem( 0, 
		     errorHndl->er_serverInfo->svr_parmDescr[ j ].ds_length, 
		     FALSE, &status ) ) )
	    {
		IIAPI_TRACE( IIAPI_TR_FATAL )
	    ...
	    IIapi_cnvtGDV2DataValue(
			&errorHndl->er_serverInfo->svr_parmDescr[ j ],
			dataValue->gca_value,
			&errorHndl->er_serverInfo->svr_parmValue[ j ] );
	}

	if ( ! serverErrorText( errorHndl ) )  goto valueFail;
  ...
  valueFail:
    ...
    while( --j >= 0 )  
	MEfree( errorHndl->er_serverInfo->svr_parmValue[ j ].dv_value );
...
IIapi_saveErrors( IIAPI_HNDL *handle )
{
	...
	while( handle->hd_errorQue != &handle->hd_errorList )
	{
	    ...
	    if ( IIapi_isQueEmpty( &er_hndl->hd_errorList ) )
