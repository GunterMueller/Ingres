...
**	IIapi_createParmNMem	Create message parameter block.
...

/*
** Name: IIapi_createParmNMem
**
** Description:
**	This function creates the parameter memory block for messages.
**
** Input:
**	None.
II
...
**
** Returns:
**	IIAPI_PARMNMEM *	Parameter memory, NULL if error.
**
** History:
**      30-sep-93 (ctham)
**          creation
...
**	    Fixed tracing.
**	21-Dec-95 (gordy)
**	    More work on tracing.
**	24-May-96 (gordy)
**	    Removed hd_memTag, replaced by new tagged memory support.
**	11-Nov-96 (gordy)
**	    Check for successful tag allocation.
**	18-Feb-97 (gordy)
**	    Renamed and made globally accessible.
*/

IIAPI_PARMNMEM *
IIapi_createParmNMem( II_VOID )
{
    IIAPI_PARMNMEM	*parmNmem;
    STATUS		status;
    II_UINT2		memTag;

    if( ! (memTag = IIapi_newMemTag()) )
    {
	IIAPI_TRACE( IIAPI_TR_FATAL )
	    ( "IIapi_createParmNMem: can't allocate memory tag\n" );

	return( NULL );
    }

    if ( ! ( parmNmem = (IIAPI_PARMNMEM *)
	     MEreqmem( memTag, sizeof(IIAPI_PARMNMEM), TRUE, &status ) ) )
    {
	IIAPI_TRACE( IIAPI_TR_FATAL )
	    ( "IIapi_createParmNMem: can't allocate memory block\n" );

	IIapi_freeMemTag( memTag );
	return( NULL );
    }
    
    parmNmem->pm_memTag = memTag;
    parmNmem->pm_endFlag = TRUE;
    parmNmem->pm_formatted = TRUE;
