...
static char	uid[ GCN_UID_MAX_LEN ] = { "" };
...
struct _API_PARMS
{
    ...
    i4		parm_min;
    i4		parm_max;
    char *	(*parms)( IIAPI_PARMNMEM *, API_PARSE * );

};

static	char *	ns_value( IIAPI_PARMNMEM *, API_PARSE * );
static	char *	ns_login( IIAPI_PARMNMEM *, API_PARSE * );
...
static IIAPI_DESCRIPTOR ns_login_desc[] =
{
    { IIAPI_CHA_TYPE, FALSE, 1, 0, 0, IIAPI_COL_TUPLE, "type" },
    { IIAPI_VCH_TYPE, FALSE, 34, 0, 0, IIAPI_COL_TUPLE, "vnode" },
    { IIAPI_VCH_TYPE, FALSE, 66, 0, 0, IIAPI_COL_TUPLE, "login" },
};
...
IIapi_createGCNOper( IIAPI_STMTHNDL *stmtHndl )
{
    ...
    /*
    ** We only send one gcn tuple in the request,
    ** which is already included in the structure.
    */
    if ( ! (gcnData = (GCD_GCN_D_OPER *)
		      MEreqmem( parmNmem->pm_memTag, 
			        sizeof( GCD_GCN_D_OPER ), FALSE, &status ) ) )
    {
	IIAPI_TRACE( IIAPI_TR_FATAL )
	    ( "IIapi_createGCNOper: can't allocate GCN_D_OPER\n" );

	IIapi_freeMemTag( parmNmem->pm_memTag );
	return( NULL );
    }

    parmNmem->pm_msgType = GCN_NS_OPER;
    parmNmem->pm_parmLen = sizeof( *gcnData );
    parmNmem->pm_parmBlock = (II_PTR)gcnData;

    /*
    ** Enter GCN_NS_OPER data.
    */
    switch( parse->opcode )
    {
	case API_KW_ADD :  gcnData->gcn_opcode = GCN_ADD;  break;
	case API_KW_DEL :  gcnData->gcn_opcode = GCN_DEL;  break;
	case API_KW_GET :  gcnData->gcn_opcode = GCN_RET;  break;
    ...
    */
    gcnData->gcn_flag = GCN_DEF_FLAG;

    if ( parse->opcode == API_KW_ADD  && 
	( parse->object == API_KW_NODE || parse->object == API_KW_ATTR ) )  
	gcnData->gcn_flag |= GCN_MRG_FLAG;

    if ( connHndl->ch_username  &&  ! connHndl->ch_password )  
	gcnData->gcn_flag |= GCN_UID_FLAG;

    if ( parse->type == API_KW_GLOBAL )  gcnData->gcn_flag |= GCN_PUB_FLAG;

    if ( parse->object != API_KW_SERVER )
	gcnData->gcn_flag |= GCN_NET_FLAG;
    ...
    */
    ...
    {
	case API_KW_NODE :
	    gcnData->gcn_tuple[0].gcn_type.gcn_value = GCN_NODE;
	    break;

	case API_KW_LOGIN :
	    gcnData->gcn_tuple[0].gcn_type.gcn_value = GCN_LOGIN;
	    break;

	case API_KW_ATTR :
	    gcnData->gcn_tuple[0].gcn_type.gcn_value = GCN_ATTR;
	    break;

	case API_KW_SERVER :
	    gcnData->gcn_tuple[0].gcn_type.gcn_value = 
			ns_resolve_param( parse, API_FIELD_OBJ, FALSE );
	    break;
	...
	default :
	    ...
	    */
	    ...
	    gcnData->gcn_tuple[0].gcn_type.gcn_value = empty;
	    break;
    }

    gcnData->gcn_tuple[0].gcn_type.gcn_l_item = 
		STlength( gcnData->gcn_tuple[0].gcn_type.gcn_value ) + 1;

    gcnData->gcn_tuple[0].gcn_obj.gcn_value = 
		ns_resolve_param( parse, API_FIELD_VNODE, 
				  (parse->opcode != API_KW_ADD) );
    gcnData->gcn_tuple[0].gcn_obj.gcn_l_item = 
		STlength( gcnData->gcn_tuple[0].gcn_obj.gcn_value ) + 1;

    /*
    ** The tuple value must be built up from
    ** various parameters and can be function
    ** and object dependent.  Call the appropriate
    ** function for the current request to
    ** process the parameters.
    */
    gcnData->gcn_tuple[0].gcn_val.gcn_value = 
			(*parse->parms->parms)( parmNmem, parse );

    if ( gcnData->gcn_tuple[0].gcn_val.gcn_value )
	gcnData->gcn_tuple[0].gcn_val.gcn_l_item = 
			STlength(gcnData->gcn_tuple[0].gcn_val.gcn_value) + 1;
    else
    {
	IIapi_freeMemTag( parmNmem->pm_memTag );
	return( NULL );
    ...
    */
	...
	gcnData->gcn_tuple[0].gcn_uid.gcn_value = connHndl->ch_username;
    else
    {
	if ( ! uid[0] )
	{
	...
	gcnData->gcn_tuple[0].gcn_uid.gcn_value = uid;
    }

    gcnData->gcn_tuple[0].gcn_uid.gcn_l_item = 
		STlength( gcnData->gcn_tuple[0].gcn_uid.gcn_value ) + 1;

    /*
    ** Installation ID is currently ignored.
    */
    gcnData->gcn_install.gcn_value = install_id;
    gcnData->gcn_install.gcn_l_item = STlength( install_id ) + 1;

    gcnData->gcn_tup_cnt = 1;
...
**
** Returns:
**	char *		Combined parameters, NULL if error.
**
** History:
**	19-Feb-97 (gordy)
**	    Created.
...
static char *
ns_value( IIAPI_PARMNMEM *parmNmem, API_PARSE *parse )
{
    ...
    ** retrieved above.
    */
    value = (char *)MEreqmem( parmNmem->pm_memTag, len, FALSE, &status );
	...
	STpolycat( 3, value, ",", parms[ fld ], value );
...
**	format required for LOGIN operation requests.
**
** Input:
**	parmNmem	Parameter memory.
**	parse		Parser info.
...
static char *
ns_login( IIAPI_PARMNMEM *parmNmem, API_PARSE *parse )
{
    char	*user, *pwd, *value;
    char	buff[ 64 ];
    i4		len;
	...
	*/
    ...
    len = STlength( user ) + STlength( pwd ) + 2;
