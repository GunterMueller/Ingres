...
public sealed class IngresDataReader : MarshalByRefObject,
	IDataReader, IDisposable, IDataRecord, IEnumerable
{
	...
	*/
	...
	/// <summary>
	/// Release allocated resources of the DataReader.
	/// </summary>
	public void Dispose()
	{
		Dispose(true);
	...
	private DataTable GetSchemaTable(MetaData keyInfoMetaData)
	{
		try
		{
			...
			for (int i = 0; i < fieldCount; i++)
			{
				...
				bool hasScale      = (providerType == ProviderType.Decimal ||
					providerType == ProviderType.DateTime);
				...
				row["DataType"]        =   // .NET data type
					ProviderTypeMgr.GetNETType(_resultset.rsmd.getColumnType(i));
	...
	*/
	...
	public int GetOrdinal(string name)
	{
		...
		try
		{
			return _resultset.getColByName(name);
		}
		catch (SqlEx)
		{
