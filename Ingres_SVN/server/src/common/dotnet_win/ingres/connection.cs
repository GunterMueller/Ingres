...
namespace Ca.Ingres.Client
{
	...
	*/
	...
	[ToolboxItem(typeof(Ca.Ingres.Client.Design.IngresConnectionToolboxItem))]
	public sealed class IngresConnection :
		Component, IDbConnection, IDisposable, ICloneable
	{
		/// <summary>
		...
		*/
		...
		[Editor(
			typeof(Ca.Ingres.Client.Design.ConnectionStringUITypeEditor),
		...
		*/
		...
		/// </summary>
		/// <returns></returns>
		public IngresTransaction BeginTransaction()
		{
		...
		public IngresTransaction BeginTransaction(IsolationLevel isoLevel)
		{
			...
			return new IngresTransaction(this, isoLevel);
		}
		...
		*/
		...
		public void Open()
		{
			...
			if (EnlistDistributedTransactionIsNeeded())
			{
				...
				if (transaction != null)
					EnlistDistributedTransaction(transaction, false);
			}
		}
		...
		*/
		...
		public void Close()
		{
			...
			// give MSDTC chance to finish up prepare/commit before we disconnect
			if (this.DTCenlistment != null)
			{

				this.DTCenlistment.Delist(true);
			...
			if (advanConnect != null)
			{
				...
				// put the advantage connection back in pool or close it
		...
		*/
		...
		public IngresCommand CreateCommand()
		{
		...
		*/
		...
		/// Contextutil.Transaction if false.</param>
		private void EnlistDistributedTransaction(
			System.EnterpriseServices.ITransaction transaction,
			bool                                   isExplicitTransaction)
		{
			// Check that auto-enlistment is disabled (Enlist=false).
			...
			try
			{
				...
				if (this.DTCenlistment != null)
				{
					this.DTCenlistment.Delist(false);
					this.DTCenlistment = null;
				}

				advanConnect.setAutoCommit(false);   // turn autocommit off

				this.DTCenlistment = new DTCEnlistment();
				DTCenlistment.Enlist(
					this.advanConnect, transaction, config, isExplicitTransaction);
			}
			catch( SqlEx ex )  { throw ex.createProviderException(); }
		}
		...
		*/
		...
		/// <summary>
		/// Determine if running is a context of a transaction.
		/// </summary>
		/// <returns>True if need to enlist in a
		/// ServicedComponent transaction.</returns>
		private bool EnlistDistributedTransactionIsNeeded()
		{
			if (Transaction != null)
				return false;
			...
			if (System.EnterpriseServices.ContextUtil.IsInTransaction == false)
				return false;
		...
		*/
			...
			CreateComponentsCore(System.ComponentModel.Design.IDesignerHost host)
		{
			...
			Type t = assemDesign.GetType(
				"Ca.Ingres.Client.Design.IngresConnectionToolboxItem");
			if (t == null)
				return null;
	...
	*/
		...
		System.Drawing.Design.UITypeEditor
	{
			...
			object                 value)
		{
			...
			Type t = assemDesign.GetType(
				"Ca.Ingres.Client.Design.ConnectionStringUITypeEditor");
			if (t == null)
				return null;
			object o = Activator.CreateInstance(t);
