...
namespace Ca.Ingres.ProviderInternals
{
	...
	**  Private Data
	**
	**	is_dtmc			Distributed Transaction Management Connection?
	...
	**	    Added getDbmsInfo() cover method which restricts request to cache.*/
	...
	internal class AdvanConnect : DrvObj
	{
		...
		*/
		...
		/// <summary>
		/// If a Distributed Transaction Management Connection then true.
		/// </summary>
		private  bool   is_dtmc = false;
		/// <summary>
...
//							msg.write( msg.encode( user, conn.session_mask, stringValue ) );
							msg.write( msg.toBytes( user, conn.session_mask, stringValue ) );
						}
					...
					case MSG_CP_TIMEZONE :
						/*
							...
							*/
						conn.ingres_tz = true;
						goto case MSG_CP_MNY_FRMT;
						/*
							** Fall through to general configuration parameters.
							*/
...
//				msg.write( msg.encode( userID, conn.session_mask, userID ) );
				msg.write(msg.toBytes( userID, conn.session_mask, userID ));
			}
		...
		**	   is different than current.*/
			...
			System.Data.IsolationLevel level)
		{
			lock(this)
			{
				...
				*/
				if ((stringValue = conn.dbCaps.getDbCap("INGRES/SQL_LEVEL")) != null)
					sqlLevel = System.Int32.Parse(stringValue);

				if (level != System.Data.IsolationLevel.Serializable &&
						sqlLevel < OI_SQL_LEVEL)
					throw SqlEx.get( ERR_GC4010_PARAM_VALUE );
		...
		**	    Added support for Distributed Transaction Management Connections.*/
		...
		internal virtual void  startTransaction(AdvanXID xid)
		{
			warnings = null;
