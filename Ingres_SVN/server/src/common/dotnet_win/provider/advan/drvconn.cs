...
namespace Ca.Ingres.ProviderInternals
{
	...
	**	ingres_tz	    Ingres timezone sent to DBMS?
	...
	*/
		...
		DrvConn : MsgConst
	{
		...
		*/
		...
		public  bool	use_gmt_tz		= true;
		public  bool	ingres_tz		= false;
		public  bool	ucs2_supported		= false;
		public  bool	select_loops		= true;
		public  int		cursor_mode		= DrvConst.DRV_CRSR_DBMS;
		...
		*/
			...
			DrvConn( String host, IConfig config, ITrace trace, bool dtmc )
		{
			...
			/*
			** See if character encoding is configured.
			*/
			CharSet charset = null;
			String  encode = config.get( DrvConst.DRV_PROP_ENCODE );

			if ( encode != null  &&  encode.Length > 0 )
				try { charset = CharSet.getCharSet( encode ); }
				catch( Exception )
				{
					if ( trace.enabled( 1 ) )
						trace.write( tr_id + ": invalid character encoding '" + 
							encode + "'" );
		...
		*/
		...
		private void
		connect( CharSet charset, bool dtmc )
		{
			byte[] 	msg_cp; 
			int    	len;
			...
			*/
			...
			msg = new MsgConn( host, ref msg_cp, charset );
			...
			loadDbCaps()
		{
			...
			*/
			...
			if ( (value = dbCaps.getDbCap( DBMS_DBCAP_OSQL_DATES )) != null )  
				use_gmt_tz = false;	// TODO: check for 'LEVEL 1'
