...
namespace Ca.Ingres.Client.Design
{
	...
	*/
	...
	{
		const string bitmapName = "Ca.Ingres.Client.Design.ingresbox.bmp";
	...
	internal class TableMappings
	{
			...
			string cmdText)
		{
			...
			try
			{
				...
				Ca.Ingres.ProviderInternals.MetaData md =
					new Ca.Ingres.ProviderInternals.MetaData(cmdText);
				if (md        != null  &&
					md.Tables != null  &&
					md.Tables.Count > 0)
				{
					Ca.Ingres.ProviderInternals.MetaData.Table t =
						(Ca.Ingres.ProviderInternals.MetaData.Table)md.Tables[0];
