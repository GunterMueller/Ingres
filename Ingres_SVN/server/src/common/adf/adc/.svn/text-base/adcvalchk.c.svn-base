...
*/
	...
	case DB_ADTE_TYPE:
	    nd = &ndt;
	    MEfill((u_i2) sizeof(ndt), NULLCHAR, (PTR) nd);
	    adu_6to_dtntrnl (adc_dv, nd);
	...
	case DB_TME_TYPE:
            ...
            tzmins = 60 * nd->dn_tzhour;
            if(tzmins < 0)
            {
                tzmins -= nd->dn_tzminute;
            }
            else
            {
                tzmins += nd->dn_tzminute;
		...
		||
	        (   nd->dn_tzhour > AD_35DTE_MAX_TZ_HR
                    || nd->dn_tzhour < AD_34DTE_MIN_TZ_HR)
		||
	        (   nd->dn_tzminute > AD_37DTE_MAX_TZ_MN 
                    || nd->dn_tzminute < AD_36DTE_MIN_TZ_MN)
		|| 
                (   tzmins > AD_48DTE_MAX_TZ_MN
                    || tzmins < AD_47DTE_MIN_TZ_MN)
	       )
               {
                  bad = TRUE;
                  break;
    ...
    if (bad)
	db_stat = adu_error(adf_scb, E_AD1014_BAD_VALUE_FOR_DT, 0);
    else
	db_stat = E_DB_OK;
