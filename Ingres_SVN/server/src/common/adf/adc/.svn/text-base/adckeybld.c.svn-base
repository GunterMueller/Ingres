...
# endif
{
	...
	*err_code = E_AD0000_OK;
    ...
    if	(   (adc_kblk->adc_opkey != ADI_EQ_OP)
	 && (adc_kblk->adc_opkey != ADI_NE_OP)
	 && (adc_kblk->adc_opkey != ADI_LT_OP)
	 && (adc_kblk->adc_opkey != ADI_LE_OP)
	 && (adc_kblk->adc_opkey != ADI_GT_OP)
	 && (adc_kblk->adc_opkey != ADI_GE_OP)
	 && (adc_kblk->adc_opkey != ADI_LIKE_OP)
	 && (adc_kblk->adc_opkey != ADI_NULJN_OP)
	)
    ...
    */
    ...
    /* Special short-cut for cases where the the .db_data PTR is NULL in
    ** the input key value:  This can happen for user parameters in repeat
    ** queries.  In this case, we just want to return the type of key that
	...
	*/
	    ...
	    )
	{
        ...
        /* If input type is INTEGER or FLOAT and output type
        ** is CHARACTER, returns ADC_KALLMATCH.  This change will
        ** make the return key type consistence with the adu_cbldkey()
        ** routine.
        */
	if ((in_bdt == DB_INT_TYPE || in_bdt == DB_FLT_TYPE) &&
	    (ky_bdt == DB_CHR_TYPE || ky_bdt == DB_CHA_TYPE
	    || ky_bdt == DB_VCH_TYPE || ky_bdt == DB_TXT_TYPE))
	{
	    adc_kblk->adc_tykey = ADC_KALLMATCH;
	    return(E_DB_OK);
	}
	...
	*err_code = E_AD0000_OK;
	return (E_DB_OK);   /* Do not set low/high key for this special case */
    }
    ...
    */
    ...
    /* Check for possibility of pattern matching */
    if (in_bdt == DB_FLT_TYPE  		||  
	in_bdt == DB_INT_TYPE		||
	in_bdt == DB_DEC_TYPE		||
	in_bdt == DB_MNY_TYPE		||
	in_bdt == DB_DTE_TYPE		||
	...
	switch(adc_kblk->adc_opkey)
	{
	  ...
	  case ADI_GE_OP:
	    ...
	    db_stat = adc_minmaxdv(adf_scb,
		 ...
		 !(adf_scb->adf_warncb.ad_1rsvd_cnt & AD_1RSVD_EXEDBP))
