...
/* look thru fi table for operations with matching number of inputs */
	...
	*/
	...
	for (j = num_cdts; j < ADI_MAX_OPERANDS; j++)
	{
	    cdts[j] = DB_NODT_TYPE;
	}
	...
	*/
	for (curr_dt = 0; curr_dt < num_cdts; curr_dt++)
	{
		...
		*/
		i4 cl_0 = ad0_type_class(idts[0]);
		i4 cl_1 = ad0_type_class(idts[1]);
	...
	*/
	if (curr_coerces == best_coerces  &&  curr_rank == best_rank)
	{
	    ...
	    */
	    ...
	    i4 par;
	    for (par = 0; par < ADI_MAX_OPERANDS; par++)
	    {
		if (best_fi->adi_dt[par] != cdts[par])
		{
			...
			if (ad0_type_class(best_fi->adi_dt[par]) != ad0_type_class(cdts[par]))
			    state = REJECT;
			else if (ad0_type_class(best_fi->adi_dt[par]) < ad0_type_class(cdts[par]))
			    state = KEEP;
			break;
		    case REJECT:
			break;
