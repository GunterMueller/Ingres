...
**	values. The comparison is a simple byte compare (could be made
**	type sensitive in the future) and is used to determine 
**	partition compatibility of 2 joined tables.
**
** Inputs:
...

{
    ...
    DB_PART_LIST	*plist1p, *plist2p;
    i4			i, vallen;

    /* First, verify the column sets are identically typed and compute 
    ** length of each value break. */

    for (i = 0, vallen = 0; i < pdim1p->ncols; i++)
    {
	plist1p = &pdim1p->part_list[i];
	plist2p = &pdim2p->part_list[i];
	if (plist1p->type != plist2p->type ||
	    plist1p->length != plist2p->length ||
	    plist1p->precision != plist2p->precision)
	  return(FALSE);	/* type info must be identical */
	vallen += plist1p->length;
    }
	...
	if (pbrk1p->oper != pbrk2p->oper ||
	    ...
	    MEcmp(pbrk1p->val_base, pbrk2p->val_base, vallen) != 0)
