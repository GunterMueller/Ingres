	...
	switch (bdt)
	{
	  ...
	  case DB_TME_TYPE:
	    ...
	    (*fcn)(arg,"dn_seconds = %d\tdn_nsecond = %d\n",
		      ndatep->dn_seconds, ndatep->dn_nsecond );
	  ...
	  case DB_INDS_TYPE:
	    ndatep = &newdt;
	    adu_6to_dtntrnl(db_dv, ndatep);
	    (*fcn)(arg,"dn_day = %d\tdn_seconds = %d\tdn_nsecond = %d\n",
		      ndatep->dn_day, ndatep->dn_seconds, ndatep->dn_nsecond);
	    break;
...
DB_DATA_VALUE      *db_dv)
{
	...
	switch (bdt)
	{
	  ...
	  case DB_INDS_TYPE:
	    ndatep = &newdt;
	    adu_6to_dtntrnl(db_dv, ndatep);
	    (*fcn)(STprintf(stbuf,"dn_day = %d\tdn_seconds = %d\tdn_nsecond = %d\n",
		      ndatep->dn_day, ndatep->dn_seconds, ndatep->dn_nsecond));
	    break;
