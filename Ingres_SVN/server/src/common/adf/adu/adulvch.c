...
**      This file defines:
**
**      adu_0lo_setup -- set's up workspace for ...
...

/*{
** Name:	adu_0lo_setup - Perform common wksp setup for filters
**
** Description:
...
**      remaining after the workspace equally between the input and
			...
			DB_DATA_VALUE 	*dv_work)
{
    ...
    char                *buffer;
    DB_DT_ID            dtid =
	ADI_DT_MAP_MACRO(abs(dv_lo->db_datatype));
	...
	if (DB_SUCCESS_MACRO(status))
	{
	    ...
	    STRUCT_ASSIGN_MACRO(work->adw_fip.fip_under_dv,
				work->adw_fip.fip_work_dv);
	    work->adw_fip.fip_work_dv.db_data = 
		(PTR) work->adw_shared.shd_o_area;
	}
    }
    return(status);
}    
	      ...
	      ADP_POP_CB     *pop_cb)
{
    ...
    pop_cb_ptr = ( pop_cb ? pop_cb : &in_pop_cb );

    for(;;)
    {
	/* Make sure parameters are OK */

	if ( ((Adf_globs->Adi_dtptrs[dtid]->adi_dtstat_bits
					      & AD_PERIPHERAL) == 0)
	  || (dv_in->db_datatype != dv_out->db_datatype))
	{
	    status = adu_error(scb, E_AD9999_INTERNAL_ERROR, 0);
	    break;
	}
	...
	/*
	** Now that things are basically doable, initialize the
	** pop_cb's
	*/

	STRUCT_ASSIGN_MACRO(work->adw_fip.fip_pop_cb, *pop_cb_ptr);
	STRUCT_ASSIGN_MACRO(work->adw_fip.fip_pop_cb, out_pop_cb);
	...
	out_pop_cb.pop_user_arg = work->adw_filter_space;
	      ...
	      )
	{
		...
		if (status)
		{
		    ...
		    else if (pop_cb_ptr->pop_error.err_code ==
			     E_AD7001_ADP_NONEXT)
		    {
			done = 1;
			status = E_DB_OK;
		...
		*/
		...
		work->adw_fip.fip_under_dv.db_length;

	    status = (*fcn)(scb,
			    pop_cb_ptr->pop_segment,
			    out_pop_cb.pop_segment,
			    work);
	    if (status)
		break;

	    if (   (work->adw_shared.shd_exp_action ==
						  ADW_FLUSH_STOP)
		|| (work->adw_shared.shd_exp_action ==
						  ADW_STOP))
	    {
		done = 1;
	    }
	...
	status =
	  (*Adf_globs->Adi_fexi[ADI_01PERIPH_FEXI].adi_fcn_fexi)
	  (ADP_CLEANUP, pop_cb_ptr);
	break;
    }
    work->adw_filter_space = out_pop_cb.pop_user_arg; /* save in case */
						      /* needed */
