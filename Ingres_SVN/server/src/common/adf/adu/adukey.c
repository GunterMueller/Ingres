...
ADC_KEY_BLK	*adc_kblk)
{
	...
	** handle the key building.
	*/
	return ( adu_utf8cbldkey( adf_scb, adc_kblk) );
    }
...
ADC_KEY_BLK	*key_block)
{
    ...
    u_i2	    workspace[1000] = {0};
    u_i2 	    *wkspace = &workspace;
    ...
    {
      key_block->adc_lokey.db_data = &wkspace[lo_pos];
    ...
    {
      key_block->adc_hikey.db_data = &wkspace[hi_pos];
    ...
    if (result = adu_nvchbld_key( adf_scb, key_block) != E_DB_OK)
    {
...
ADC_KEY_BLK	    *adc_kblk)
{
    ...
    {
	adu_6to_dtntrnl (&(adc_kblk->adc_kdv), &tempdt);
	dptr = &tempdt;
    }
    else 
    {
	...
	if (lo_date != NULL)
	   adu_7from_dtntrnl (&adc_kblk->adc_lokey, &tempdt);
	break;
	...
	break;
      ...
      case ADI_LT_OP:
	    adc_kblk->adc_tykey = ADC_KHIGHKEY;
	    if (hi_date != NULL)
	      adu_7from_dtntrnl (&adc_kblk->adc_hikey, &tempdt);
	break;
...
ADC_KEY_BLK	*adc_kblk)
{
    ...
    bool	    isescape = adc_kblk->adc_isescape;
