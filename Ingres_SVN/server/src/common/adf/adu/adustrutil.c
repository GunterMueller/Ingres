...
register    char	    **addr)
{
    ...
    ** 3str/5str. */
	...
	case DB_VBYTE_TYPE:
	case DB_UTF8_TYPE:
            *len = ((DB_TEXT_STRING *) dv->db_data)->db_t_count;
	    if (*len < 0  ||  *len > adf_scb->adf_maxstring)
		return (adu_error(adf_scb, E_AD1014_BAD_VALUE_FOR_DT, 0));
	    *addr = (char *) ((DB_TEXT_STRING *) dv->db_data)->db_t_text;
            break;

	case DB_NVCHR_TYPE:
            *len = (((DB_NVCHR_STRING *) dv->db_data)->count) * sizeof(UCS2);
	    if (*len < 0  ||  *len > DB_MAXSTRING)
		return (adu_error(adf_scb, E_AD1014_BAD_VALUE_FOR_DT, 0));
...
register DB_DATA_VALUE	    *dest)
{
	...
	** byte, byte varying, long byte, long text, 
	...
	while (outstring < endout)
     ...
     case DB_NVCHR_TYPE: 
        ((DB_NVCHR_STRING *)dest->db_data)->count =
                                             unistring - (UCS2 *)ucptr;
      /* Fall through and execute the loop to fill out the string. */
...
i4			*size)
{
	...
	*/
	...
	break;
      ...
      case DB_LTXT_TYPE:
      case DB_VBYTE_TYPE:
        *size = ((DB_TEXT_STRING *)dv->db_data)->db_t_count;
        break;

      case DB_NVCHR_TYPE:
         /*  modify the size as it is ELMSZ times count */	
         *size = ((DB_NVCHR_STRING *)dv->db_data)->count;
         break;
...
i4		    *str_len)
{
	...
	case DB_UTF8_TYPE:
	    ...
	    if (*str_len < 0  ||  *str_len > adf_scb->adf_maxstring)
		return (adu_error(adf_scb, E_AD1014_BAD_VALUE_FOR_DT, 0));
            break;

	case DB_NVCHR_TYPE:
            *str_len = ((DB_NVCHR_STRING *) dv->db_data)->count;
            if (*str_len < 0  ||  *str_len > adf_scb->adf_maxstring)
                return (adu_error(adf_scb, E_AD1014_BAD_VALUE_FOR_DT, 0));
            break;
