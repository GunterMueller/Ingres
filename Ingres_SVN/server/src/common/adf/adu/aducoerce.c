...
DB_DATA_VALUE	    *rdv)
{
	...
	break;
      ...
      case DB_TXT_TYPE:
      case DB_LTXT_TYPE:
        if (((DB_TEXT_STRING *) dv->db_data)->db_t_count > DB_MAXTUP)
	    return(adu_error(adf_scb, E_AD5004_OVER_MAXTUP, 0));

        cp = temp;
        count = ((DB_TEXT_STRING *) dv->db_data)->db_t_count;
...
DB_DATA_VALUE	    *rdv)
{
	...
	/* convert money from cents to dollars */
        fval = ((AD_MONEYNTRNL *) dv->db_data)->mny_cents / 100.0;
...
DB_DATA_VALUE	    *rdv)
{
        ...
        (dv->db_datatype == DB_NVCHR_TYPE))
    {
      ...
      {
        cdata.db_datatype = DB_VCH_TYPE;
        if ((((DB_NVCHR_STRING *) dv->db_data)->count * sizeof(UCS2) + DB_CNTSIZE) > DB_MAXTUP)
            return(adu_error(adf_scb, E_AD5004_OVER_MAXTUP, 0));
	else
            cdata.db_length = dv->db_length/sizeof(UCS2);
      }
	...
	if (fval < MINI8 || fval > MAXI8)
      ...
      case DB_MNY_TYPE:
	/* convert money from cents to dollars */
        val8 = ((AD_MONEYNTRNL *) dv->db_data)->mny_cents / 100.0;
        break;
