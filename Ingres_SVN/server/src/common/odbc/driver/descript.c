        ...
        if (ulenValuePtr)
		{
            *ulenValuePtr = pird->Length;
			if( option_bigint && (pird->VerboseType == SQL_BIGINT))
				*ulenValuePtr = sizeof(SQL_INTEGER);
		}
        if (StringLengthPtr)
    ...
    */
    ...
    case SQL_DESC_LENGTH:
        ...
        if (ValuePtr)
           {
#ifdef _WIN64
            *ulenValuePtr = pxxd->Length;
#else
            I4ASSIGN_MACRO(pxxd->Length, *u4ValuePtr);
#endif
           }
        if (StringLengthPtr)
           *StringLengthPtr = sizeof(SQLUINTEGER);
    ...
    SQLINTEGER  BufferLength)
{
    ...
    LPDESCFLD     pxxd;
    SQLRETURN     rc         = SQL_SUCCESS;
    SQLSMALLINT   i2Value    = (SQLSMALLINT) (SQLINTEGER)ValuePtr;
    SQLINTEGER    i4Value    = (SQLINTEGER ) (SQLINTEGER)ValuePtr;
    SQLUINTEGER   ui4Value   = (SQLUINTEGER) (SQLUINTEGER)ValuePtr;
    SQLINTEGER        lenValue   = (SQLINTEGER )     ValuePtr;
    SQLUINTEGER       ulenValue  = (SQLUINTEGER)     ValuePtr;
    SQLUSMALLINT  ui2Value   = (SQLUSMALLINT)(SQLINTEGER)ValuePtr;
    i2            Type;
    BOOL          foundit;
...
/* ????? trigger unbind check if newCount <= OldCount ?????*/
    ...
    case SQL_DESC_CONCISE_TYPE:
        ...
        ptype = CvtGetSqlType(ui2Value, IIAPI_LEVEL_4);
