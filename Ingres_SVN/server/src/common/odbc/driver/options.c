...
scrollopt_return:
    if (SQL_SUCCEEDED(rc))
		pstmt->cRowsetSize = crowRowset > 1 ? crowRowset : 1;
...
*/
    ...
    case SQL_ROWSET_SIZE:               /* kludge for VInterdev, per thoda04... */
        pstmt->pARD->ArraySize   = vParamuint;   /* ignored... */
        break;
    ...
    SQLINTEGER StringLength)
{
    ...
    case SQL_ATTR_CURSOR_SENSITIVITY:
        ...
        if (pstmt->fAPILevel >= IIAPI_LEVEL_5)
            ...
            pstmt->fCursorSensitivity = (UWORD)SQL_UNSPECIFIED;
        break;
