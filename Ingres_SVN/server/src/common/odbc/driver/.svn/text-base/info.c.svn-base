...
#define INGRES_CVT_ALL SQL_CVT_INTEGER | SQL_CVT_LONGVARBINARY | SQL_CVT_LONGVARCHAR | \
SQL_CVT_DECIMAL | SQL_CVT_SMALLINT | SQL_CVT_TINYINT | SQL_CVT_VARBINARY | SQL_CVT_VARCHAR | \
...
/*      if (cbInfoValueMax < sizeof (UDWORD))  we may assume it's large enough
        ...
        */
        ...
        {
            if (fInfoType == SQL_CURSOR_SENSITIVITY)
            {
                i4temp = SQL_UNSPECIFIED | SQL_INSENSITIVE | SQL_SENSITIVE;
	        I4ASSIGN_MACRO(i4temp, *rgbInfoValue);
            }
            else if (fInfoType == SQL_SCROLL_OPTIONS)
            {
            ...
            {
                i4temp = SQL_CA1_LOCK_NO_CHANGE | SQL_CA1_LOCK_EXCLUSIVE |
    ...
    */
    ...
    if (fSqlType == SQL_ALL_TYPES)
    {
        ...
        */
        while (*(SWORD     *)p)
        {
            GetSQLMaxLen(pstmt, *(SWORD     *)(p + cb), (UCHAR *)p);
            pstmt->crowBuffer++;
            p += pstmt->cbrow;          
        }
    }
    else
    {
        ...
        */
        while (*(SWORD     *)p && *(SWORD *)(p + cb) == tempType)
        {
            GetSQLMaxLen(pstmt, fSqlType, (UCHAR     *)p);
            pstmt->crowBuffer++;
            p += pstmt->cbrow;
        }
    ...
    UCHAR     * p)
{
    ...
    LPDESCFLD pird = pIRD->pcol + QRY_TYPE_PRECISION;    /* pird -> DATA_TYPE (SQL data type) */
    UWORD cb = (UWORD)pird->cbDataOffset;

    switch ( fSqlType )
    {
        case SQL_CHAR :
            if(pstmt->pdbcOwner->max_char_column_length > 0)
                MEcopy((PTR)&pstmt->pdbcOwner->max_char_column_length, 
		    sizeof(i2), (PTR)(p + cb));
            break;
        case SQL_VARCHAR:
            if(pstmt->pdbcOwner->max_vchr_column_length > 0)
                MEcopy((PTR)&pstmt->pdbcOwner->max_vchr_column_length, 
		    sizeof(i2), (PTR)(p + cb));
            break;
        case SQL_BINARY:
            if(pstmt->pdbcOwner->max_byte_column_length > 0)
                MEcopy((PTR)&pstmt->pdbcOwner->max_byte_column_length, 
		    sizeof(i2), (PTR)(p + cb));
             break;
        case SQL_VARBINARY:
            if(pstmt->pdbcOwner->max_vbyt_column_length > 0)
                MEcopy((PTR)&pstmt->pdbcOwner->max_vbyt_column_length, 
		    sizeof(i2), (PTR)(p + cb));
             break;
    }
    return;
