...
**              SQLGetFunctions - Query driver API capability.
...
**
**              Since the CLI is a derived driver manager, no actual
**              discovery is performed.  Instead, the response is hard-coded.
**
** Inputs:
...
**
** Outputs: 
**              Supported - Points to TRUE if supported; FALSE otherwise.
**
** Returns: 
**              SQL_SUCCESS
**              SQL_SUCCESS_WITH_INFO
    ...
    SQLUSMALLINT *Supported)
{
    ...
    ODBC_TRACE_ENTRY(ODBC_TR_TRACE, IITraceSQLGetFunctions(ConnectionHandle,
        FunctionId, Supported ), traceRet);

    if (validHandle(ConnectionHandle, SQL_HANDLE_DBC) != SQL_SUCCESS)
    {
        ODBC_TRACE(ODBC_TR_TRACE, IITraceReturn(traceRet, SQL_INVALID_HANDLE));
        return SQL_INVALID_HANDLE;
    }

    resetErrorBuff(ConnectionHandle, SQL_HANDLE_DBC);
