...
# endif /* GCXDEBUG */
	...
	if (ls_parms->gca_status == OK)
	{
	    GCC_ERLIST	erlist;
	    bool	remote = FALSE;
	    char	tmp_passwd[ GCC_L_PWD ];
	    char	emode[ GCC_L_EMODE ];
	    ...
	    */
		...
		STcopy( ls_parms->gca_user_name, ccb->ccb_hdr.username );
		break;
	    }
		...
		*/
		...
		STcopy( rmt_addr->partner_id, ccb->ccb_hdr.target );
		...
		{
		    STcopy( rmt_addr->user_id, ccb->ccb_hdr.username );
		    STcopy( rmt_addr->password, tmp_passwd );
		}
		else  if ( ls_parms->gca_user_name )
		{
		    STcopy( ls_parms->gca_user_name, ccb->ccb_hdr.username );
		    tmp_passwd[0] = EOS;
		}
	    ...
	    case GCA_ID_RMT_INFO:
	    {
	      ...
	      */
		  ...
		  {
	      	      ( VOID ) MEcopy( val, len, ccb->ccb_hdr.target );
		      break;
		  }
		  ...
		  case GCA_RMT_USER:
		  {
	      	      ( VOID ) MEcopy( val, len, ccb->ccb_hdr.username );
		      break;
		  }
		  case GCA_RMT_PASSWD:
		  {
		      ( VOID ) MEcopy( val, len, tmp_passwd );
		      break;
		  ...
		  case GCA_RMT_EMECH:
		  {
		      ( VOID ) MEcopy( val, len, ccb->ccb_hdr.emech );
		      break;
		  }
		  case GCA_RMT_EMODE:
		  {
		      ( VOID ) MEcopy( val, len, emode );
		      status = gcc_encrypt_mode( emode, &ccb->ccb_hdr.emode );
		      if( status != OK )
		      {
			  GCC_ERLIST	erlist;
			  STATUS 	tmp_status;
			  ...
			  erlist.gcc_erparm[0].size = STlength( emode );
			  erlist.gcc_erparm[0].value = emode;
	      ...
	      if ( ! ccb->ccb_hdr.username[0]  &&  ls_parms->gca_user_name )
	      {
		    STcopy( ls_parms->gca_user_name, ccb->ccb_hdr.username );
		    tmp_passwd[0] = EOS;
	      }
		...
		*/
		char buff[16];
		MOulongout(0, (u_i4)ccb->ccb_hdr.lcl_cei, sizeof(buff), buff);
		...
		** Negotiate down the GCA protocol if its greater than what
		    ...
		    ? GCC_GCA_PROTOCOL_LEVEL : ls_parms->gca_partner_protocol;
                ...
                */
                if ( ! ccb->ccb_hdr.username[0]  ||  ! tmp_passwd[0] )
                    ccb->ccb_hdr.password[0] = EOS;
		else
		{
		    char	ns_id[ GCC_L_LCL_ID ];
		    u_char	pbuff[ GCC_L_PWD ];
		    CL_ERR_DESC	sys_err;
		    i4		len;
		    ...
		    */
		    CItobin( tmp_passwd, &len, (u_char *)pbuff );
		    gcc_decode( pbuff, len, ccb->ccb_hdr.username, 
				IIGCc_global->gcc_ns_mask,
...
gcc_alout_exec( i4  output_event_id, GCC_MDE *mde, GCC_CCB *ccb )
{
	...
	*/
	...
	str_len = STlength( ccb->ccb_hdr.target );
	pci += GCaddn2( str_len, pci );
        GCnadds( ccb->ccb_hdr.target, str_len, pci );
...
# endif /* GCXDEBUG */
	...
	*/
	...
	/*
	** No security label initially
	*/
	ccb->ccb_aae.a_flags &= ~GCCAL_SECLABEL;
	...
	** as we specifically did not convert the null.
        */
	aarq_pci.l_ae_title = min( aarq_pci.l_ae_title, GCC_L_TARGET );
	...
	*/
    	...
    	if ( ccb->ccb_aae.a_version >= AL_PROTOCOL_VRSN_65 )
	{
	    ...
	    /*
	    ** Check if passed a security label from the other side
	    */
	    if ( ccb->ccb_hdr.gca_flags & GCA_LS_SECLABEL )  passed_sl = TRUE;
	...
	*/
	else  if ( ! ccb->ccb_hdr.username[0]  || 
	           (! ccb->ccb_hdr.password[0]  &&  ! ccb->ccb_hdr.authlen) )
	{
	...
	*/
	...
	rqo_parms->gca_password = ccb->ccb_hdr.password[0]
				  ? ccb->ccb_hdr.password : NULL;
	...
	if ( ccb->ccb_aae.a_flags & GCCAL_SECLABEL )
	{
	    /*
	    ** Tell GCA_REQUEST we are passing in a security label
	    */
	    rqo_parms->gca_modifiers |= GCA_RQ_SECLABEL;
	    rqo_parms->gca_sec_label = ccb->ccb_hdr.seclab;
	}
	...
	*/
	...
	STcopy( IIGCc_global->gcc_ob_mech, ccb->ccb_hdr.emech );
...
gcc_aaerr( GCC_MDE *mde, GCC_CCB *ccb, STATUS generic_status )
{
	...
	**  and send back to the client comm. server. Set up the generic
	    ...
	    ** Invoke ule_format to translate the error code into text and
	    ...
	    gcu_erfmt( &tmpp2, tmpp2 + GCC_L_BUFF_DFLT, 1,
