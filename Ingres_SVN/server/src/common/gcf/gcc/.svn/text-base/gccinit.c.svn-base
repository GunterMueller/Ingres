...
)
{
    ...
    */
    ...
    STncpy( IIGCc_global->gcc_installation, env ? env : "",
	     sizeof( IIGCc_global->gcc_installation ) - 1 );
    IIGCc_global->gcc_installation[ sizeof( IIGCc_global->gcc_installation ) -
	1 ] = EOS;

    if ( *IIGCc_global->gcc_installation )
	STprintf( chname, ERx("II_CHARSET%s"), IIGCc_global->gcc_installation );
    else
	STcopy( ERx("II_CHARSET"), chname );

    NMgtAt( chname, &env );
    if ( env && *env)
    {
        STlcopy( env, chname, sizeof(chname)-1);

	if ( CMset_attr( chname, &cl_err ) != OK )
	{
	...
	STcopy( env, IIGCc_global->gcc_ib_mech );
    else
	STcopy( GCC_ANY_MECH, IIGCc_global->gcc_ib_mech );

    if ( PMget( ERx("!.ob_encrypt_mech"), &env ) == OK )
	STcopy( env, IIGCc_global->gcc_ob_mech );
    else
	STcopy( GCC_ANY_MECH, IIGCc_global->gcc_ob_mech );
