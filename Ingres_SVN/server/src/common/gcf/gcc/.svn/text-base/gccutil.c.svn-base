...
#define         GCC_KEY_LEN     CRYPT_SIZE
#define         GCC_MAX_STR     128
...
gcc_del_ccb( GCC_CCB *ccb )
{
    ...
    {
	char	cei_buff[16];
	MOulongout(0, (u_i4)ccb->ccb_hdr.lcl_cei, sizeof(cei_buff), cei_buff);
...
gcc_set_conn_info( GCC_CCB *ccb, GCC_MDE *mde )
{
    ...
    */
	...
	GCnadds( ccb->ccb_hdr.seclab, ccb->ccb_hdr.seclen, mde->p2 );
    else
	GCnadds( "", 1, mde->p2 );

    mde->p2 += GCC_L_SECLABEL;
    tmpi2 = ccb->ccb_hdr.sectyp;
    mde->p2 += GCaddi2( &tmpi2, mde->p2 );
...
gcc_get_conn_info( GCC_CCB *ccb, GCC_MDE *mde )
{
    ...
    mde->p1 += GCngets( mde->p1, (i4)GCC_L_UNAME, ccb->ccb_hdr.username );

    if ( (mde->p1 + GCC_L_PWD) > mde->p2 )  return;
    mde->p1 += GCngets( mde->p1, (i4)GCC_L_PWD, ccb->ccb_hdr.password );

    if ( (mde->p1 + GCC_L_ACCTNAME) > mde->p2 )  return;
    mde->p1 += GCC_L_ACCTNAME;

    if ( (mde->p1 + GCC_L_SECLABEL) > mde->p2 )  return;
    mde->p1 += GCngets( mde->p1, (i4)GCC_L_SECLABEL, ccb->ccb_hdr.seclab);
...
{
    char	kbuff[ GCC_KEY_LEN ];
    char	ibuff[ GCC_L_ENCODE + CRYPT_SIZE ];
    CI_KS	ksch;
    char	*src;
    i4		len;
    bool	done;
    ...
    */
    for( src = key, len = 0; len < GCC_KEY_LEN; len++ )
    {
	if ( ! *src )  src = key;
	kbuff[ len ] = *src++ ^ (mask ? mask[len] : 0);
    }
    ...
    */
    for( done = FALSE, len = 0; ! done  &&  len <= GCC_L_ENCODE; )
    {
	/*
	** First character in each encryption block is random.
	*/
	ibuff[ len++ ] = (i4) ( CSET_RANGE * MHrand() ) + CSET_BASE;
    ...
    */
    ibuff[ len ] = EOS;
    CIencode( ibuff, len, ksch, (PTR)buff );
    return( len );
}
...
gcc_decode( u_i1 *e_str, i4 e_len, char *key, u_i1 *mask, char *dbuff )
{
    ...
    ** the correct size. Decrypt.
    */
    e_len = min( e_len, GCC_L_ENCODE );
    CIdecode( (PTR)e_str, (i4)e_len, ksch, (PTR)obuff );
    
    /*
    ** Remove padding and delimiter 
    ** from last encryption block.
    */
    while( e_len  &&  obuff[ --e_len ] != CRYPT_DELIM );
    obuff[ e_len ] = EOS;
    ...
    */
	...
	if ( len % CRYPT_SIZE )  *dbuff++ = (char)obuff[ len ];
