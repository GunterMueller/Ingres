...
**	    LOAD_FRAGMENT	Load atomic fragment for continued message.
**	    SAVE_FRAGMENT	Save atomic fragment for continued message.
...
#define	GCC_DWN_LOAD_FRAGMENT		8
#define	GCC_DWN_SAVE_FRAGMENT		9
#define	GCC_DWN_SEND_OUTPUT		10
#define	GCC_DWN_EXIT			11
	...
	{ GCC_DWN_LOAD_FRAGMENT,"LOAD_FRAGMENT" },
	{ GCC_DWN_SAVE_FRAGMENT,"SAVE_FRAGMENT" },
...
# endif /* GCXDEBUG */
    ...
    else  if ( doc->frag_bytes )
    {
	/* 
	** Pre-pend any atomic fragment left over from previous segment.
	*/
	action[ actions++ ] = GCC_DWN_LOAD_FRAGMENT;
    }
...
# endif /* GCXDEBUG */
	...
	case GCC_DWN_LOAD_FRAGMENT :	/* Load saved atomic fragment */
	    /* 
	    ** We no longer need the PCI of the input MDE, 
	    ** so use the PCI area to pre-pend the fragment.
	    ** Requires sizeof( doc->frag_buffer ) <= GCC_OFF_DATA.
	    */
	    mde->p1 -= doc->frag_bytes;
	    MEcopy((PTR)doc->frag_buffer, doc->frag_bytes, (PTR)mde->p1);
	    doc->frag_bytes = 0;
	    break;

	case GCC_DWN_SAVE_FRAGMENT :	/* Save atomic fragment in DOC */
	    if ( (mde->p2 - mde->p1) <= sizeof( doc->frag_buffer ) )
	    {
		doc->frag_bytes = (u_i2)(mde->p2 - mde->p1);
		MEcopy( mde->p1, doc->frag_bytes, doc->frag_buffer );
		mde->p1 += doc->frag_bytes;
	    }
	    else
	    {
# ifdef GCXDEBUG
		if ( IIGCc_global->gcc_trace_level >= 1 )
		    TRdisplay("cnvrt2net: atom fragment overflow, %d bytes\n",
			      (i4)(mde->p2 - mde->p1));
# endif /* GCXDEBUG */
		QUinsert( &doc->q, &IIGCc_global->gcc_doc_q );
		if ( mde_out )  gcc_del_mde( mde_out );
	...
	case GCC_DWN_SEND_OUTPUT :	/* Flush output MDE to SL */
#ifndef DOUBLEBYTE
...
#else
	    /*
	    ** Output buffers are queued and then sent as
	    ** a batch when no (special) object is split
	    ** between buffers.
	    */
	    QUinsert( mde_out, ccb->ccb_pce.mde_q[ OUTFLOW ].q_prev );
	    mde_out = NULL;

	    if ( ! ccb->ccb_pce.split )
	    {
		QUEUE *mde_ptr;
    ...
    /*
    ** Call GCO to perform HET/NET conversions on message data.
    */
    doc->src1 = mde->p1;
    doc->src2 = mde->p2;
    doc->dst1 = mde_out->p2;
    ...
    status = gco_convert( doc, OUTFLOW, ccb->ccb_pce.p_csetxlt
# ifdef DOUBLEBYTE
			  , &ccb->ccb_pce.split
# endif /* DOUBLEBYTE */
			);
    ...
    /*
    ** We are either done or need more input or output.
    ** In all cases we are done with the current output segment.
    */
    actions = 0;
    ...
    */
	...
	/*
	** If HET/NET processing is complete, there should 
	** not be any message data remaining.  If there is 
	** data, we can't convert to network format so we 
	** must ignore it (with suitable trace).
	*/
	if ( doc->state == DOC_DONE  &&  mde->p1 < mde->p2 )
	{
# ifdef GCXDEBUG
	    if ( IIGCc_global->gcc_trace_level >= 2 )
...
# endif /* GCXDEBUG */
	...
	*/
	if ( ! eod )
	{
	    ...
	    if ( doc->state == DOC_MOREIN  &&  mde->p1 < mde->p2 )
		action[ actions++ ] = GCC_DWN_SAVE_FRAGMENT;
...
#define	GCC_UP_LOAD_FRAGMENT		6
#define	GCC_UP_SAVE_FRAGMENT		7
#define	GCC_UP_SEND_OUTPUT		8
#define	GCC_UP_EXIT			9
	...
	{ GCC_UP_LOAD_FRAGMENT,	"LOAD_FRAGMENT" },
	{ GCC_UP_SAVE_FRAGMENT,	"SAVE_FRAGMENT" },
...
# endif /* GCXDEBUG */
	...
	** Pre-pend any atomic fragment left over from previous segment.
	*/
	action[ actions++ ] = GCC_UP_LOAD_FRAGMENT;
    }
...
# endif /* GCXDEBUG */
	...
	case GCC_UP_LOAD_FRAGMENT :	/* Load saved atomic fragment */
	    /* 
	    ** We no longer need the PCI of the input MDE, 
	    ** so use the PCI area to pre-pend the fragment.
	    ** Requires sizeof( doc->frag_buffer ) <= GCC_OFF_DATA.
	...
	case GCC_UP_SAVE_FRAGMENT :	/* Save atomic fragment in DOC */
	    if ( (mde->p2 - mde->p1) <= sizeof( doc->frag_buffer ) )
	    {
		doc->frag_bytes = (u_i2)(mde->p2 - mde->p1);
		MEcopy( mde->p1, doc->frag_bytes, doc->frag_buffer );
...
# ifdef GCXDEBUG
		if ( IIGCc_global->gcc_trace_level >= 1 )
		    TRdisplay("cnvrt2lcl: atom fragment overflow, %d bytes\n",
			      (i4)(mde->p2 - mde->p1));
# endif /* GCXDEBUG */
		QUinsert( &doc->q, &IIGCc_global->gcc_doc_q );
		if ( mde_out )  gcc_del_mde( mde_out );
...
# else
	    ...
	    ** between buffers.
	    */
	    QUinsert( mde_out, ccb->ccb_pce.mde_q[ INFLOW ].q_prev );
	    mde_out = NULL;
...
# endif /* DOUBLEBYTE */
    ...
    */
    ...
    status = gco_convert( doc, INFLOW, ccb->ccb_pce.p_csetxlt
# ifdef DOUBLEBYTE
			  , &ccb->ccb_pce.split
...
# endif /* GCXDEBUG */
	...
	*/ 
	if ( ! eod )
	{
	    /* Save unprocessed atomic fragment for later processing */
	    if ( doc->state == DOC_MOREIN  &&  mde->p1 < mde->p2 ) 
		action[actions++] = GCC_UP_SAVE_FRAGMENT;
...
gcc_pl_lod( GCC_MDE *mde )
{
	...
	*/
	tod_prog_len = count * GCO_L_OP_MAX * sizeof( GCO_OP );
