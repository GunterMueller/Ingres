...
gcn_make_auth( GCN_MBUF *mbin, GCN_MBUF *mbout, i4 prot )
{
    ...
    char	client_inst[ GCN_OBJ_MAX_LEN + 1 ];
    char	server_inst[ GCN_OBJ_MAX_LEN + 1 ];
    i4		rq_count;
    i4		auth_type;
    STATUS	status;
    GCN_MBUF	*mbuf;
    ...
    char	*m;
    i4		i;
    char	rtickvec[ GCN_DFLT_L_TICKVEC ][ GCN_VAL_MAX_LEN ];
    ...
    */
    ...
    m += gcn_copy_str( m, username );
    m += gcn_copy_str( m, client_inst );
    m += gcn_copy_str( m, server_inst );
    m += gcu_get_int( m, &rq_count );
    if ( rq_count > GCN_DFLT_L_TICKVEC )  
        rq_count = GCN_DFLT_L_TICKVEC;
    ...
    */
    status = gcn_make_tickets( username, IIGCn_static.lcl_vnode,
			       &rtickvec[0][0], rq_count, GCN_VAL_MAX_LEN,
			       prot );
    if ( status != OK )
    {
...
gcn_can_auth( GCN_RESOLVE_CB *grcb )
{
    ...
    */
    ...
    {
	grcb->passwd_out = grcb->ip_pwd;
	grcb->flags &= ~GCN_RSLV_PWD_ENC;
    }
    else  if ( *grcb->vnode  &&  ! STcompare( grcb->user_out, GCN_NOUSER ) )
    {
	/*
	** Save installation password.
	*/
	STcopy( grcb->passwd_out, grcb->ip_pwd );

	/*
	** Don't treat as IP if this is the 
	** reserved internal login.
	*/
	if ( STcompare( grcb->ip_pwd, GCN_NOPASS ) )
	{
	    /*
	    ** Flag the fact that the installation
	    ** password has been decoded in case
...
gcn_use_auth( GCN_RESOLVE_CB *grcb, i4 prot )
{
    ...
    */
    status = gcn_use_rticket( grcb->user, grcb->vnode, grcb->passwd_out, 
			      ticket, &ticket_len );
    if ( status != OK )  return( status );
	...
	*/
    ...
    /* 
    ** Identify password as a ticket of type GCN_AUTH_TICKET. 
    */
    MEcopy( GCN_AUTH_TICKET_MAG, GCN_L_AUTH_TICKET_MAG, grcb->usrpwd );
    CItotext( ticket, ticket_len, grcb->usrpwd + GCN_L_AUTH_TICKET_MAG );

    /*
    ** Send the ticket as the connection password.
    */
    grcb->user_out = grcb->user;
    grcb->passwd_out = grcb->usrpwd;
    grcb->flags &= ~GCN_RSLV_PWD_ENC;
    return( OK );
}
...
gcn_get_auth( GCN_RESOLVE_CB *grcb, GCN_MBUF *mbout )
{
    ...
    /* 
    ** Encrypt the NOUSER password, so the 
    ** local GCC can decrypt it.
    */
    ...
    */
    if ( (status = gcn_connect_info( grcb, mbout, GCN_NOUSER, passwd )) != OK )
	return( status );
...
gcn_got_auth( GCN_RESOLVE_CB *grcb, GCN_MBUF *mbin )
{
    ...
    */
	...
	m += gcn_copy_str( m, rtickvec[ i ] );

    status = gcn_save_rtickets( grcb->user, grcb->vnode, 
				&rtickvec[0][0], rq_count, GCN_VAL_MAX_LEN );
...
gcn_decrypt( char *key, char *pbuff, char *passwd )
{
    i4		klen, plen;
    CI_KS	ksch;
    char	keystr[ GC_L_USER_ID + 1 ];
    char	buff[ GC_L_PASSWORD + 1 ];
	...
	*passwd = EOS;
    ...
    if ( klen > GC_L_USER_ID )  return( FAIL );
    if ( plen > GC_L_PASSWORD )  return( FAIL );

    /*
    ** Adjust key to CRYPT_SIZE by padding/truncation.
    */
    STcopy( key, keystr );
    for( ; klen < CRYPT_SIZE; klen++ )  keystr[ klen ] = ' ';
    keystr[ CRYPT_SIZE ] = EOS;
    ...
    */
    ...
    if ( plen % CRYPT_SIZE )  return( FAIL );
    ...
    */
    ...
    if ( passwd[ --plen ] != GCN_PASSWD_END )  return( FAIL );
    passwd[ plen ] = EOS;
...
gcn_encode( char *key, u_i1 *mask, char *ipb, char *opb )
{
	...
	*opb = EOS;
    ...
    if ( STlength( ipb ) > GC_L_PASSWORD )  return( FAIL );
    if ( mask == NULL )  mask = zeros;

    /*
    ** Adjust key to CRYPT_SIZE by duplication/truncation.
    */
    for( src = key, len = 0; len < CRYPT_SIZE; len++ )
    {
	if ( ! *src )  src = key;
	kbuff[ len ] = *src++ ^ mask[ len ];
    }
    ...
    */
    for( done = FALSE, len = 0; ! done  &&  len < (sizeof(pbuff) - 1); )
    {
	/*
	** First character in each encryption block is random.
	*/
	pbuff[ len++ ] = (i4)(CSET_RANGE * MHrand()) + CSET_BASE;

	/*
	** Load string into remainder of encryption block.
	*/
        while( *ipb  &&  len % CRYPT_SIZE )  pbuff[ len++ ] = *ipb++;
    ...
    */
    pbuff[ len ] = EOS;
    CIencode( pbuff, len, ksch, (PTR)obuff );
    CItotext( (u_char *)obuff, len, opb );
...
gcn_decode( char *key, u_i1 *mask, char *ipb, char *opb )
{
    ...
    char	kbuff[ CRYPT_SIZE + 1 ];
    char	pbuff[ GC_L_PASSWORD * 2 + 1 ];
    u_char	obuff[ GC_L_PASSWORD * 2 + 1 ];
	...
	*opb = EOS;
    ...
    if ( STlength( ipb ) > (GC_L_PASSWORD * 2) )  return( FAIL );
    if ( mask == NULL )  mask = zeros;
    ...
    ** Decrypt password.
    */
    CItobin( ipb, &len, (u_char *)pbuff );
    if ( len % CRYPT_SIZE )  return( FAIL );
    CIdecode( (PTR)pbuff, len, ksch, (PTR)obuff );
    ...
    */
    for( len = 0; obuff[ len ]; len++ )
	if ( len % CRYPT_SIZE )  *opb++ = obuff[ len ];
