...
**	gcn_pad_crypt() - pad key/text for CIencode()
...
static i4	gcn_pad_crypt( char *, char * );
static STATUS	gcn_get_instpwd( char *, char * );
...
STATUS
gcn_make_tickets( char *user, 
		  char *inst, char *rtickvec, i4  l_rtickvec, i4  l_rticket,
		  i4 prot )
{
	GCN_QUE_NAME	*nq;
	GCN_TUP		tup;
	char		passwd[ GC_L_PASSWORD + 1 ];
	char		expiration[ 32 ];
	char		timetolive[ 32 ];
	STATUS		status;
	...
	if( l_rticket < GCN_L_TICKET )
	    return E_GC0144_GCN_INPW_BROKEN;

	/* Look up installation passwd */
	if( ( status = gcn_get_instpwd( inst, passwd ) ) != OK )
	    return status;
	...
	*/
	CVla( IIGCn_static.ticket_exp_time, timetolive );
        CVla( IIGCn_static.now.TM_secs + (IIGCn_static.ticket_exp_time +
            IIGCn_static.ticket_exp_interval), expiration );
...

/*
** Name: gcn_pad_crypt() - pad key/text for CIencode()
**
** Description:
**	CI encryption routines need their inputs blank padded to CRYPT_SIZE
**	bytes.  This routine pads and terminates a null-terminated input
**	string.
**
** Inputs:
**	inbuf		string to pad
**
** Outputs:
**	outbug		padded and terminated string
**
** Returns:
**	length of outbuf
*/
...
static i4
gcn_pad_crypt( char *inbuf, char *outbuf )
{
	char	*in = inbuf;
	char	*out = outbuf;
	i4	padlen;
	...
	/* Round up to CRYPT_SIZE. */

	padlen = ( in - inbuf ) % CRYPT_SIZE;

	/* Put blanks into the new void. */

	if( padlen )
	    for( padlen = CRYPT_SIZE - padlen; padlen--; )
		*out++ = ' ';
...
**
** Inputs:
**	inst		the installation name
**
** Outputs:
**	passwd		the password for the installation
**
** Returns:
**	STATUS
**
...
static STATUS
gcn_get_instpwd( char *inst, char *passwd )
{
    GCN_QUE_NAME	*nq;
    GCN_TUP		*tup, tupmask;
    STATUS		status;
    ...
    STpolycat( 2, GCN_GLOBAL_USER, ",", usrpwd );
    ...
    /* Copy and return.  */
    count = gcu_words( tup->val, usrpwd, pv, ',', 3 );
    if ( count > 2  &&  pv[2][0] == 'V'  &&  
