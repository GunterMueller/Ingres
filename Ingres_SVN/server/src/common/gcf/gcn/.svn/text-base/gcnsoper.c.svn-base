	...
	do
	{
		...
		*/
		if ( opflags & GCN_OPFLAG_LOGIN )
		{
		    ...
		    (void)gcu_words( tup->val, val, pv, ',', 2 );
		    newtup.val = pv[0];
		}
...
)
{
	...
	*/
	if ( (row_count = gcn_nq_del( nq, masktup )) )
	    status = gcn_result( mbout, GCN_DEL, row_count );

	/*
	** Insert the tuples into the name queue.
	*/
	for( row_count = tup_count = 0; tup_count < num_tuple; tup_count++ )
	{
	    GCN_TUP	tup;
	    char	login[ GCN_VAL_MAX_LEN ];
...
{
    char	*pv[ 2 ];
    char	pwd[ GC_L_PASSWORD + 1 ];
    char	buff[ GC_L_PASSWORD * 2 + 1 ];
    STATUS	status;
    ...
    */
    ...
    /*
    ** Re-encrypt password for storage and
    ** rebuild the login tuple value.
    */
    if ( status == OK  &&  (status = gcn_login( GCN_VLP_LOGIN, 1, TRUE, 
						pv[0], pwd, buff )) == OK )
	STpolycat( 5, pv[0], ",", buff, ",", "V1", obuff );
