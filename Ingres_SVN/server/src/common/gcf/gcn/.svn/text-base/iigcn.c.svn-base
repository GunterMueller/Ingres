...
char 	**argv;
{
    ...
    */
    ...
    IIGCn_static.listen_addr = STalloc( name );
    STcopy( name, IIGCn_static.registry_addr );
	...
	*/
	if ( 
	    ...
	    STcopy( info.mech_info[0]->mech_name, IIGCn_static.rmt_mech );
	else
	    STcopy( ERx("none"), IIGCn_static.rmt_mech );
    }
    ...
    */
    ...
    IIGCa_call( GCA_TERMINATE, &parms, GCA_SYNC_FLAG, NULL, -1, &status );
    gcn_srv_term();
    gcn_adm_term();
