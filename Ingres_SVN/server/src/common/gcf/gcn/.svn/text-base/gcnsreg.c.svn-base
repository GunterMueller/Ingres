...
**
** Description:
**	Load the resolve control block remote protocol tables
**	from the registry protocol tables.
**
** Input:
...
gcn_ir_proto( GCN_RESOLVE_CB *grcb, char *host )
{
    ...
    for(
	 reg = (GCN_REG *)IIGCn_static.registry.q_next, grcb->catr.tupc = 0;
	 (QUEUE *)reg != &IIGCn_static.registry;
	 reg = (GCN_REG *)reg->reg_q.q_next, grcb->catr.tupc++
       )
    {
	STprintf( request, "%s,%s,%s", 
		  (host  &&  *host) ? host : 
		      ((reg->reg_host  &&  *reg->reg_host) ? reg->reg_host :
			  IIGCn_static.hostname), 
		  reg->reg_proto, reg->reg_port );

	gcu_words( request, grcb->catr.tupv[ grcb->catr.tupc ], pv, ',', 3 );
...
gcn_ir_comsvr( GCN_RESOLVE_CB *grcb )
{
    ...
    */
    ...
    save_dlen = grcb->deleg_len;	/* Must save delegation info */
    save_dptr = grcb->deleg;
    MEfill( sizeof( *grcb ), EOS, (PTR)grcb );
    grcb->deleg_len = save_dlen;
    grcb->deleg = save_dptr;
    ...
    */
	...
	{
	    STcopy( tupv[ grcb->catl.tupc ].val, 
		    grcb->catl.tupv[ grcb->catl.tupc ] );
	    grcb->catl.host[ grcb->catl.tupc ] = "";
	    grcb->catl.addr[ grcb->catl.tupc ] = 
					grcb->catl.tupv[ grcb->catl.tupc ];
	}
...
gcn_ir_bedcheck( GCN_RESOLVE_CB *grcb, GCN_MBUF *mbreq, GCN_MBUF *mbmsg )
{
    ...
    STATUS		status;
    GCN_REG		*reg;
    char		*m, passwd[ GC_L_PASSWORD * 2 + 1 ];
    ...
    */
    ...
    if ( (status = gcn_connect_info( grcb, mbreq, GCN_NOUSER, passwd )) != OK )
	return( status );
...
gcn_ir_update( GCN_MBUF *mbin )
{
	...
	{
	    STcopy( addr, IIGCn_static.registry_addr );
	    STcopy( id, IIGCn_static.registry_id );
...
gcn_ir_register( GCN_RESOLVE_CB *grcb, GCN_MBUF *mbout )
{
    ...
    */
    ...
    if ( (status = gcn_connect_info( grcb, mbout, "", "" )) != OK )
	return( status );
...
gcn_ir_master( GCN_RESOLVE_CB *grcb, GCN_MBUF *mbreq, GCN_MBUF *mbmsg )
{
    ...
    ** Build GCA_FASTSELECT resolved connection info.
    */
    if ( (status = gcn_connect_info( grcb, mbreq, "", "" )) != OK )
	return( status );
