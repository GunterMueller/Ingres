...
static VOID	gcn_pwd_override( GCN_RESOLVE_CB *, char * );
...
gcn_rslv_init(GCN_RESOLVE_CB *grcb, GCN_MBUF *mbin, char *account, char *user)
{
    ...
    STcopy( account, grcb->account );
    STcopy( user, grcb->user );  
    grcb->user_in[0] = grcb->passwd_in[0] = EOS;
    grcb->user_out = grcb->passwd_out = "";
    grcb->flags &= ~GCN_RSLV_PWD_ENC;
    grcb->auth = NULL;
    grcb->auth_len = 0;
    grcb->emode[0] = grcb->emech[0] = EOS;
    ...
    */
    ...
    if ( grcb->gca_message_type == GCN_NS_2_RESOLVE )
    {
	msg_in += gcn_copy_str( msg_in, grcb->user_in );
	msg_in += gcn_copy_str( msg_in, grcb->passwd_in );
    }
...
gcn_rslv_parse( GCN_RESOLVE_CB *grcb )
{
    ...
    */
    gcn_pwd_override( grcb, grcb->vnode );

    if ( IIGCn_static.trace >= 4 )
	TRdisplay( "gcn_resolve: '%s'::'%s'/'%s' for user %s\n", 
		    grcb->vnode, grcb->dbname, grcb->class, grcb->user );
    ...
    */
    if ( ! STcasecmp( grcb->class, GCN_STSYN ) )
	grcb->class = GCN_STAR;
...
gcn_rslv_node( GCN_RESOLVE_CB *grcb )
{
	...
	**	 3	<host>,<proto>,<port>
	*/
	count = gcu_words( &grcb->vnode[1], grcb->catr.tupv[0], pv, ',', 4 );

	switch( count )
	{
	case 1 :	/* Host provided, use registry protocols & ports */
	    gcn_ir_proto( grcb, &grcb->vnode[1] );
	    break;
    ...
    */
	...
	pv[0] = pv[1] = pv[2] = "";
	gcu_words( tupv[ tupc ]->val, grcb->catr.tupv[ tupc ], pv, ',', 3 );
	grcb->catr.node[ tupc ] = pv[0];
	grcb->catr.protocol[ tupc ] = pv[1];
	grcb->catr.port[ tupc ] = pv[2];
    }
...
gcn_rslv_login( GCN_RESOLVE_CB *grcb )
{
    ...
    */ 
    ...
    else  if ( grcb->vnode[0]  &&  grcb->vnode[0] != '@' )
...
gcn_use_login( GCN_RESOLVE_CB *grcb )
{
    ...
    */
    if ( ! STcompare( GCN_NOUSER, grcb->user_in ) )
	if ( STcompare( GCN_NOPASS, grcb->passwd_in ) )
	    return( E_GC0145_GCN_INPW_NOT_ALLOWED );
	else  if ( ! grcb->vnode[0] )
	    return( E_GC000B_RMT_LOGIN_FAIL );

    grcb->user_out = grcb->user_in;

    if ( grcb->vnode[0] )
    {
	grcb->passwd_out = grcb->passwd_in;
	grcb->flags &= ~GCN_RSLV_PWD_ENC;
    }
    else
    {
	...
	*/
	...
	pwd.user = grcb->user_in;
	pwd.password = grcb->passwd_in;
	...
	if ( status == OK )
	{
	    ...
	    valid.user = grcb->user_in;
	    valid.alias = grcb->user_in;
...
gcn_get_login( GCN_RESOLVE_CB *grcb )
{
    ...
    */
	...
	/*
	** Extract user ID and encrypted password.
	*/
	count = gcu_words( tup->val, grcb->usrpwd, pv, ',', 3 );
	grcb->user_out = pv[0];
	grcb->passwd_out = pv[1];
	if ( count > 2  &&  pv[2][0] == 'V'  &&  
	...
	*/
	gcn_login( GCN_VLP_LOGIN, version, FALSE, pv[0], pv[1], pv[1] );
	grcb->flags &= ~GCN_RSLV_PWD_ENC;

# ifdef NT_GENERIC
	if ( STcasecmp(pv[0], "IIPROMPT1" ) == 0  ||
	     STcasecmp(pv[0], "IIPROMPTALL" ) == 0  ||
	     STcasecmp(pv[0], "IIPROMPTONCE" ) == 0 )
	    grcb->passwd_out = pv[0] + (GCN_VAL_MAX_LEN / 2);
# endif
    }
...
gcn_rslv_attr( GCN_RESOLVE_CB *grcb )
{
    ...
    /*
    ** Nothing to do for local requests
    ** or pre-resolved vnodes.
    */
    if ( grcb->vnode[0] == EOS  ||  grcb->vnode[0] == '@' )
	return( OK );
    ...
    */
    ...
    {
	char	*pv[2];
	char	tmp_val[ GCN_VAL_MAX_LEN + 1 ];

	pv[0] = pv[1] = "";
	gcu_words( tupv[ i ]->val, tmp_val, pv, ',', 2 );
...
gcn_rslv_server( GCN_RESOLVE_CB *grcb )
{
	...
	*/
	if ( grcb->class[0] != EOS  &&
	     ! STcasecmp( grcb->class, GCN_NMSVR ) )
	{
	    grcb->catl.tupc = 1;
	    STcopy( IIGCn_static.registry_addr, grcb->catl.tupv[0] );
	    grcb->catl.host[0] = "";
	    grcb->catl.addr[0] = grcb->catl.tupv[0];
    ...
    */
    ...
    {
	grcb->catl.tupc = 1;
	STcopy( class + 1, grcb->catl.tupv[0] );
	grcb->catl.host[0] = "";
	grcb->catl.addr[0] = grcb->catl.tupv[0];
	grcb->catl.auth[0] = NULL;
	grcb->catl.auth_len[0] = 0;
    ...
    */
	...
	if ( remote )
	    ...
	    else
	    {
	    	...
	    	*/
	    	grcb->catl.tupc = 1;
	    	STcopy( ERx( "Unknown" ), grcb->catl.tupv[0] );
	    	grcb->catl.host[0] = "";
	    	grcb->catl.addr[0] = grcb->catl.tupv[0];
	    	grcb->catl.auth[0] = NULL;
	    	grcb->catl.auth_len[0] = 0;
    ...
    */
    ...
    while( tupc-- )  
    {
	STcopy( tupv[ tupc ].val, grcb->catl.tupv[ tupc ] );
	grcb->catl.host[ tupc ] = "";
	grcb->catl.addr[ tupc ] = grcb->catl.tupv[ tupc ];
...
STATUS
gcn_rslv_done( GCN_RESOLVE_CB *grcb, GCN_MBUF *mbout, i4  protocol )
{
    char	pbuff[ GC_L_PASSWORD * 2 + 1 ];
    char	*pwd = pbuff;
    char	partner[ GCN_EXTNAME_MAX_LEN + 1 ];
	...
	*/
	...
	/*
	** Mock resolved message with 0 rows 
	** for GCA_REQUESTS that can't handle 
	** a GCA_ERROR message.
	*/
	grcb->catl.tupc = 0;
	grcb->catr.tupc = 1;
	...
	grcb->user_out = grcb->passwd_out = "";
	grcb->flags &= ~GCN_RSLV_PWD_ENC;
	grcb->catr.tupv[0][0] = EOS;
	grcb->catr.protocol[0] = grcb->catr.node[0] = grcb->catr.port[0] = "";
    }
    ...
    */
    if ( ! *grcb->user_out  ||  ! *grcb->passwd_out )
	*pwd = EOS;
	...
	*/
	if ( protocol < GCA_PROTOCOL_LEVEL_40 )
	    grcb->catr.tupc = min( grcb->catr.tupc, 1 );
	else
	    msg_out += gcu_put_int( msg_out, max( grcb->catr.tupc, 1 ) );
		
	for( i = 0; i < max( grcb->catr.tupc, 1 ); i++ )
	{
	    char *empty = "";
	...
	** User info.
	*/
	msg_out += gcu_put_str( msg_out, grcb->user_out );
	...
	*/
	user =  grcb->catr.tupc ? grcb->user : grcb->user_out;
	...
	*/
	...
	else
	{
	    ...
	    */
	    ...
	    else  if ( grcb->rmech[0]  &&  grcb->deleg_len > 0 )
	    {
		char	*ptr = msg_out;
		char	*len_ptr;
		i4	len;
		...
		if ( gcn_rem_auth( grcb->rmech, grcb->catr.node[0], 
				   grcb->deleg_len, grcb->deleg, 
				   &len, ptr ) == OK )
		{
		    gcu_put_int( len_ptr, len );
		    msg_out = ptr + len;
		    grcb->user_out = grcb->user;   /* Authenticated client */
		    count++;
		}
		else
		{
...
gcn_pwd_override( GCN_RESOLVE_CB *grcb, char *vnode )
{
    char	*bptr, *eptr;
    char	*pv[ 3 ];
    char	buff[ GCN_EXTNAME_MAX_LEN + 1 ];
    i4		count;
    ...
    */
    ...
    {
	*eptr = EOS;
	count = gcu_words( bptr + 1, buff, pv, ',', 3 );
	...
	{
	    /*
	    ** We fake the control block to look as
	    ** if a resolve message matching the info
	    ** provided has been received.
	    ** when the message_type is forced to GCN_NS_2_RESOLVE
	    ** set the rmech to EOS just to be sure 
	    */
	    *bptr = EOS;
	    STcopy( pv[0], grcb->user_in );
	    STcopy( pv[1], grcb->passwd_in );

	    if ( *grcb->user_in  &&  *grcb->passwd_in )
	    {
	    ...
	    {
		*grcb->user_in = *grcb->passwd_in = EOS;
