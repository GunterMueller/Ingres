	...
	*/
	...
	for( int col = 0; col < rsmd.count; col++ )
	{
		...
		break;
            ...
            case Types.NUMERIC :
            case Types.DECIMAL :
		row.columns[ col ] = new SqlDecimal();
		break;

	    case Types.DATE :
		row.columns[ col ] = new SqlDate();
		break;

	    case Types.TIME :
		row.columns[ col ] = new SqlTime( rsmd.desc[ col ].dbms_type );
		break;
		...
		case DBMS_TYPE_IDATE :
		    row.columns[ col ] = new IngresDate( conn.osql_dates, 
						conn.timeValuesInGMT(),
						conn.cnf_empty_date );
		    break;

		default :
		    row.columns[col] = new SqlTimestamp(
    ...
    throws SqlEx
{
	...
	*/
	try { closeCursor(); } catch( SqlEx ignore ) {}
    }
