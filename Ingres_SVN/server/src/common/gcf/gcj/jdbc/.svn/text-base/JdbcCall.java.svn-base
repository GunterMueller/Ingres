    ...
    throws SQLException
{
    ...
    */
    ...
    {
	/*
	** Calling setBinaryStream() ensures the target
	** type supports coercion of the parameter value.
	*/
        params.init( paramIndex, Types.LONGVARBINARY );
	params.setBinaryStream( paramIndex, null );
    }
    ...
    throws SQLException
{
    ...
    */
    ...
    {
	/*
	** Calling setCharacterStream() ensures the target
	** type supports coercion of the parameter value.
	*/
        params.init( paramIndex, Types.LONGVARCHAR );
	params.setCharacterStream( paramIndex, null );
    }
	       ...
	       ((JdbcClob)clob).isValidLocator( conn ) )
    {
        ...
        */
        ...
        if ( lob instanceof DrvNlob )
        {
            params.init( paramIndex, Types.CLOB, false );
            params.setClob( paramIndex, (DrvNlob)lob );
        }
        else
        {
            params.init( paramIndex, Types.CLOB, true );
...
{ 
    ...
    /*
    ** Blob/Clob require special handling because they can
    ** represent LOB Locators as well as stream data.  The
    ** underlying object needs to be extracted and saved.
    */
    if ( value != null )
	if ( value instanceof Blob )
	{
	    /*
	    ** If the Blob represents a valid Locator, use
	    ** the driver Locator Blob.  Otherwise, use a 
	...
	{
	    /*
	    ** If the Clob represents a valid Locator, use
	    ** the driver Locator Clob.  Otherwise, use a
	    ** stream to save the data.
	    */
    ...
    if ( force )
	params.init( paramIndex, value, alt );
    else
    ...
    throws SQLException
{
    ...
    /*
    ** Blob/Clob require special handling because they can
    ** represent LOB Locators as well as stream data.
    */
    if ( value != null )
	if ( value instanceof Blob )
	{
    ...
    switch( sqlType )
    {
    // !!! TODO: handle coercion of BLOB/CLOB types and Blob/Clob objects.
    case Types.OTHER :
	/*
    ...
    throws SQLException
{
	...
	*/
	...
	case Types.BIT :
	case Types.BOOLEAN :
	    obj = new Boolean( (rslt_val_proc == 0) ? false : true );
	    break;
	
	case Types.TINYINT :
	case Types.SMALLINT :
	case Types.INTEGER :
	    obj = new Integer( rslt_val_proc );
	    break;
	
	case Types.BIGINT :
	    obj = new Long( (long)rslt_val_proc );
	    break;
	
	case Types.REAL :
	    obj = new Float( (float)rslt_val_proc );
	    break;
	
	case Types.FLOAT :
	case Types.DOUBLE :
	    obj = new Double( (double)rslt_val_proc );
	    break;
    ...
    throws SQLException
{
    ...
    case Types.BIT :
    case Types.BOOLEAN :
	obj = new Boolean( results.getBoolean( index ) );
	break;
	
    case Types.TINYINT :
    case Types.SMALLINT :
    case Types.INTEGER :
	obj = new Integer( results.getInt( index ) );
	break;
	
    case Types.BIGINT :
	obj = new Long( results.getLong( index ) );
	break;
	
    case Types.REAL :
	obj = new Float( results.getFloat( index ) );
	break;
	
    case Types.FLOAT :
    case Types.DOUBLE :
	obj = new Double( results.getDouble( index ) );
	break;
