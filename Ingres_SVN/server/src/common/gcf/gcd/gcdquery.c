...
gcd_msg_query( GCD_CCB *ccb )
{
    ...
    */
	...
	case MSG_QP_CRSR_NAME :
	    if ( qp.val_len >= sizeof( ccb->qry.crsr_name )  ||
	    	 ! gcd_get_bytes(ccb, qp.val_len, (u_i1 *)ccb->qry.crsr_name) )
		break;
    ...
    */
    ...
    case MSG_QRY_DESC:	
	if ( ! gcd_expand_qtxt( ccb, stmt_name_len + 16, FALSE ) )
	{
	    status = E_GC4808_NO_MEMORY;
	    break;
	}
	...
	STprintf( ccb->qry.qtxt, "describe %s%s", 
	    	      (flags & MSG_QF_DESC_OUTPUT) ? "" : "input ", 
		      stmt_name );
    ...
    case MSG_QRY_EXS :	
	if ( ! gcd_expand_qtxt( ccb, stmt_name_len + 9, FALSE ) )
	{
	    status = E_GC4808_NO_MEMORY;
	    break;
	}
	...
	{
	    u_i4 len = stmt_name_len + (flags & MSG_QF_READONLY)  
    ...
    case MSG_QRY_CDEL :
    case MSG_QRY_CUPD :
	if ( ! (scb = gcd_find_cursor( ccb, ccb->qry.crsr_name )) )
	{
	...
	if ( ccb->qry.flags & (QRY_FETCH_FIRST | QRY_FETCH_ALL) )
	{
	    ...
	    */
	    u_i4 pre_fetch = (pcb->result.flags & PCB_RSLT_FETCH_LIMIT) 
			     ? pcb->result.fetch_limit : 1;
	    gcd_send_result( ccb, &pcb->rcb, pcb, FALSE );
...
gcd_msg_data( GCD_CCB *ccb )
{
	...
	/*
	** Maximum precision is 31 digits.  Formatting
	** may include sign, leading zero and decimal
	** point.  Also provide space for terminator.
	*/
	char	dbuff[ 35 ];
    ...
    */
    ...
    more_data = (ccb->msg.msg_len || ! (ccb->msg.msg_flags & MSG_HDR_EOG));

    if ( ccb->qry.qry_parms.col_cnt <= 0  || 
	 (! done  &&  ! more_data)  ||  (done  &&  more_data) )
    {
	if ( GCD_global.gcd_trace_level >= 1 )
	    TRdisplay("%4d    GCD invalid parameter set: %d,%d,%d%s (%d%s)\n", 
		      ccb->id, ccb->qry.qry_parms.cur_col, 
