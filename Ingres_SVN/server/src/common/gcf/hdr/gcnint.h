...
#define	GCN_RSLV_PWD_ENC	0x0100		/* passwd_out is encrypted */
    ...
    i4		gca_message_type;
    PTR		target;
    char	account[ GCN_VAL_MAX_LEN + 1 ];		/* GCA_LISTEN user */
    char	user[ GCN_VAL_MAX_LEN + 1 ];		/* GCA_LISTEN alias */
    char	user_in[ GCN_VAL_MAX_LEN + 1 ];		/* specified user ID */
    char	passwd_in[ GCN_VAL_MAX_LEN + 1 ];	/* specified password */
    char	usrpwd[ GCN_VAL_MAX_LEN + 1 ];		/* user/pwd buffer */
    char	ip_pwd[ GCN_VAL_MAX_LEN + 1 ];		/* installation pwd */
    char	*user_out;
    char	*passwd_out;
    char	*vnode;
    char	*dbname;
    ...
    {
	i4	tupc;
	char	tupv[ GCN_SVR_MAX ][ GCN_VAL_MAX_LEN + 1];
...
typedef struct _GCN_TUP_QUE
{
    ...
    QUEUE	gcn_q;
    char	gcn_type[ GCN_TYP_MAX_LEN ];	/* For MO, same as name queue */
...
** Used by gcn_nq_lock, gcn_nq_unlock. The structure member gcn_added
** is no longer used and is now renamed to gcn_obsolete;
*/
...
typedef struct
{
    PTR 	sess_id;
    i4		assoc_id;
    char 	user_id[ GCN_UID_MAX_LEN + 1 ];
    char	state[ GCN_VAL_MAX_LEN ];
    bool        is_system;
    char        svr_class[ GCN_VAL_MAX_LEN + 1 ];
    char        svr_addr[ GCN_VAL_MAX_LEN + 1 ];
} GCN_SESS_INFO;
...
typedef struct 
{
	...
	char		registry_addr[ GCN_VAL_MAX_LEN + 1 ];
	char		registry_id[ GCN_VAL_MAX_LEN + 1 ];
	char		svr_type[ GCN_TYP_MAX_LEN + 1 ];
	char		lcl_vnode[ GCN_TYP_MAX_LEN + 1 ];
	char		rmt_vnode[ GCN_TYP_MAX_LEN + 1 ];
	char		rmt_mech[ GCN_TYP_MAX_LEN + 1 ];
...
#define	GCN_SVR_SHUT	0x0100		/* mionitor: server shutdown  */
#define	GCN_SVR_QUIESCE	0x0200		/* mionitor: server quiesce  */
#define	GCN_SVR_CLOSED	0x0400		/* mionitor: server closed  */
...
FUNC_EXTERN	STATUS		gcn_rslv_done(GCN_RESOLVE_CB *,GCN_MBUF *,i4);
...
FUNC_EXTERN	STATUS		gcn_direct( GCN_RESOLVE_CB *, GCN_MBUF * );
FUNC_EXTERN	STATUS		gcn_resolved( GCN_RESOLVE_CB *, GCN_MBUF * );
FUNC_EXTERN	STATUS		gcn_connect_info( GCN_RESOLVE_CB *, 
						  GCN_MBUF *, char *, char * );
...
FUNC_EXTERN	i4		gcn_copy_str( char *, char * );
