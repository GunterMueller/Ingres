...
** possibly fragmented across many segments.
*/
...
# define DOC_MOREOUT	2		/* need more output buffer */

    /* Atomic fragment handling */

    u_i2	frag_bytes;		/* Count of fragmentary bytes stored */
    char	frag_buffer[8];		/* Storage area for fragment */

    /* Conversion interpreter section */

    i4		depth;			/* tracing obj descr depth */
    GCO_OP	*pc;			/* program counter */
    GCO_OP	*call_stack[3];		/* pushed pc's */
    GCO_OP	**csp;			/* top of call stack */

    i4		val_stack[8];		/* value stack */
    i4		*sp;			/* top of value stack */

    char	*ptr_stack[6];		/* VAREXPAR pointer stack */
    char	**psp;			/* top of ptr_stack */

    /* 
    ** Maximum operations needed to compile a single
    ** data value or tuple attribute.
    */
# define	GCO_L_OP_MAX		16

    GCO_OP	od_dv_prog[ GCO_L_OP_MAX ];	/* compiling GCA_DATA_VALUES */
...
# ifdef DOUBLEBYTE
    /* Special character transliteration stuff */
    i4		type_flag;
    char	*i2_ptr;
    i4		data_len_flag;
    char	*data_len_ptr;
    i4		len_adjust;	/* for char blank padding */
    i4		max_len;
    i4		check_len;
    i4		tuple_flag;
...
#define	DOC_CLEAR_MACRO( doc_ptr ) ((doc_ptr)->state = DOC_IDLE, \
				    ...
				    (doc_ptr)->type_flag = 0, \
				    (doc_ptr)->i2_ptr = NULL, \
				    (doc_ptr)->data_len_flag = 0, \
				    (doc_ptr)->data_len_ptr = NULL, \
				    (doc_ptr)->len_adjust = 0, \
				    (doc_ptr)->max_len = 0, \
				    (doc_ptr)->check_len = 0, \
				    (doc_ptr)->tuple_flag = 0)
...
** number of times.  Must be preceded by an
** element of type integer.  Must be followed 
** by an element with array status GCA_VARTPLAR 
** (padding).  The triad length and explicit 
** length are used to determine the length of 
** the element which follows.  Objects of this 
** type may only be used to build datatype 
** object descriptors and can not be used in 
** message object descriptors.  Must be accom-
** panied by a triad descriptor (GCA_DATA_VALUE 
** or GCA_TUP_DESCR) to determine actual length.
*/
...
FUNC_EXTERN	STATUS	gco_convert( GCO_DOC *doc, i4  inout, PTR char_tbl
# ifdef DOUBLEBYTE
				     , i4  *split
# endif /* ifdef DOUBLEBYTE */
				   );
