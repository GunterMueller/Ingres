...
**	
**	The use of src1, src2, dst1, dst2, dst3, eod and state need
**	some explanation:
**
...
**	    input buffer.  The caller should not set dst3.
**
...
**	    Space at the start of the buffer is reserved (dst3),
...
gco_decode( GCO_DOC *doc )
{
    ...
    */
    ...
    {
	doc->state = DOC_MOREOUT;	/* Pick up dst3 below */
	...
	** which case dst3 will be set shortly.  But we could have 
	** just executed OP_BREAK, in which case we need to set dst3.  
	** In all cases, setting dst3 is a prudent thing to do.  In 
...
# ifdef GCXDEBUG
	 ...
	 *doc->pc != OP_DEBUG_TYPE  &&  *doc->pc != OP_DEBUG_BUFS )
    {
	i4 *i;

	TRdisplay( "%s-- %23s\t[", 
		   tabs( doc->depth ), gcx_look(gcx_gco_ops, (i4)*doc->pc) );
...
# endif /* GCXDEBUG */
	...
	*/
	doc->dst3 = dst + *(doc->pc++);
	if ( doc->dst3 > doc->dst2 )  doc->pc = &op_rewind[0];
	break;
	...
	*/
	...
	if ( (dst + arg) > doc->dst2 )
	    ...
	    *doc->sp++ = max( 1, (min( (doc->src2 - src) + doc->frag_bytes, 
			              doc->dst2 - dst )) / arg );
	break;
...
# ifdef GCXDEBUG
	if ( gco_trace_level >= 3 )
	    TRdisplay( "%s++ expar %d els * %d bytes align(%s, %d) olen %d\n", 
		       ...
		       (i4)doc->pc[2], (i4)(doc->dst2 - doc->dst3) );
# endif /* GCXDEBUG */

	i = *(doc->pc++) * doc->sp[-1];		/* Size of array */
	...
	*/
	...
	for( 
	     t_i4 = 0; 
	     gco_align_pad( &ela, (u_i2)(doc->dst2 - i - t_i4) ); 
	     t_i4++ 
	   );
...
# endif /* GCXDEBUG */
	...
	*/
	if ( doc->psp != doc->ptr_stack )
	{
	    ...
	    **
	    ** doc->sp[-1]	Length of variable portion of object.
	    ...
	    */
	    *((char **)doc->psp[-1]) = doc->dst2 - doc->sp[-1];

	    /*
	    ** Check for buffer overflow on the new data.
	    */
	    if ( doc->dst3 > *((char **)doc->psp[-1]) )  
		doc->pc = &op_rewind[0];
	    else
	    {
		/*
	...
	else
	{
	    ...
	    ** no allocation to ajust, but we do need to check 
	    ** for possible buffer overflow.
	    */
	    if ( dst + doc->sp[-1] > doc->dst2 )  
		doc->pc = &op_rewind[0];
	}
	break;
...
# ifdef DOUBLEBYTE
    ...
    ** does not apply here.  Skip op-codes
    ** unique to double byte.
    */
    case OP_CHAR_TYPE:
	doc->pc += 2;
	break;
...
# endif /* #ifdef DOUBLEBYTE */
    ...
    */
    ...
    case OP_CV_I1: 	item_size = sizeof( i1 ); goto cv;
    case OP_CV_I2: 	item_size = sizeof( i2 ); goto cv;
    case OP_CV_I4: 	item_size = sizeof( i4 ); goto cv;
    case OP_CV_I8: 	item_size = sizeof( i8 ); goto cv;
    case OP_CV_F4: 	item_size = sizeof( f4 ); goto cv;
    case OP_CV_F8: 	item_size = sizeof( f8 ); goto cv;
    case OP_CV_CHAR:	item_size = sizeof( char ); goto cv; 
    case OP_CV_BYTE: 
    case OP_CV_PAD: 	item_size = 1; goto cv;
...
# endif /* GCXDEBUG */
	...
	if ( doc->sp[-1] )	/* More elements to convert? */
	{
	    /*
...
# endif /* GCXDEBUG */
	...
	*/
	...
	if ( (src + arg) > doc->src2 )
	    ...
	    *doc->sp++ = (doc->src2 - src) / arg;	/* Number of elements */
	break;
