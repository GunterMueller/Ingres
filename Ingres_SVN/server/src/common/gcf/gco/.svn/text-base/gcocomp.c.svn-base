...
/* 
** Longest compiled message.
*/

# define GCO_L_MSG_MAX	400	/* empirically derived! */
...
# ifdef DOUBLEBYTE
static bool	gco_pushed_char_type = FALSE;
...
static	GCO_OP	gco_push_numeric( GCA_ELEMENT * );
...
# ifdef GCXDEBUG
	...
	TRdisplay( "gco_comp_msg: message %s, length %d\n", 
...
# ifdef GCXDEBUG
	if ( gco_trace_level >= 1 )
	    TRdisplay( "gco_comp_msg: message %s, length %d too long!\n", 
		       gcx_look( gcx_gca_msg, msg_type ), length );
# endif /* GCXDEBUG */
    }
    else
    {
...
# ifdef DOUBLEBYTE
    if ( 
	 gco_type == GCO_DT_C      ||  gco_type == GCO_DT_C_N     ||
	 gco_type == GCO_DT_CHAR   ||  gco_type == GCO_DT_CHAR_N  ||
	 gco_type == GCO_DT_TEXT   ||  gco_type == GCO_DT_TEXT_N  ||
	 gco_type == GCO_DT_TEXT_C ||  gco_type == GCO_DT_TEXT_CN ||
	 gco_type == GCO_DT_VCHR   ||  gco_type == GCO_DT_VCHR_N  ||
	 gco_type == GCO_DT_VCHR_C ||  gco_type == GCO_DT_VCHR_CN ||
	 gco_type == GCO_DT_LCHR   ||  gco_type == GCO_DT_LCHR_N
       )
	gco_pushed_char_type = TRUE;
# endif /* ifdef DOUBLEBYTE */
...
# ifdef DOUBLEBYTE
    gco_pushed_char_type = FALSE;
# endif /* ifdef DOUBLEBYTE */

# ifdef GCXDEBUG
    if ( gco_trace_level >= 3 )
	TRdisplay( "gco_comp_ddt: type %s, size %d, length %d\n", 
...
gco_comp_dv( GCA_ELEMENT *ela, GCO_OP *in_pc )
{
    ...
    *pc++ = OP_DV_BEGIN;	/* Begin processing GCA_DATA_VALUE */
...
**
** Description:
**	Compiles a GCA_OBJECT_DESC message object
**	describing a row of tuple data.
**
** Inputs:
...
**
** Description:
**	Compile a GCA_TUP_DESCR message object 
**	describing a row of tuple data.
**
** Inputs:
...
# ifdef DOUBLEBYTE
    if ( 
	 ela->tpl.type == DB_CHR_TYPE  || 
	 ela->tpl.type == -DB_CHR_TYPE ||
	 ela->tpl.type == DB_CHA_TYPE  ||
	 ela->tpl.type == -DB_CHA_TYPE ||
	 ela->tpl.type == DB_TXT_TYPE  || 
	 ela->tpl.type == -DB_TXT_TYPE ||
	 ela->tpl.type == DB_VCH_TYPE  || 
	 ela->tpl.type == -DB_VCH_TYPE ||
	 ela->tpl.type == DB_LVCH_TYPE ||
	 ela->tpl.type == -DB_LVCH_TYPE
       )
    {
	*pc++ = OP_CHAR_TYPE;
	*pc++ = (GCO_OP)ela->tpl.type;

	if ( ela->tpl.type == DB_TXT_TYPE  ||  ela->tpl.type == DB_VCH_TYPE )
	    *pc++ = (GCO_OP)ela->tpl.length - 2;
	else  if ( ela->tpl.type == -DB_TXT_TYPE  ||
		   ela->tpl.type == -DB_VCH_TYPE )
	    *pc++ = (GCO_OP)ela->tpl.length - 3;
	else  if ( ela->tpl.type < 0 )
	    *pc++ = (GCO_OP)ela->tpl.length - 1;
	else
	    *pc++ = (GCO_OP)ela->tpl.length;
    }
# endif /* #ifdef DOUBLEBYTE */

    /*
    ** Fixed size types return the compiled object
    ** length.  Variable sized objects return the
    ** TPL length.  We also need to push the length
    ** (don't include the NULL byte) of variable 
    ** sized objects for use during processing.
    */
...
# ifdef GCXDEBUG
    if ( gco_trace_level >= 1  &&  (pc - *in_pc) > GCO_L_OP_MAX )
...
)
{
	...
	*/
	...
	if ( (*pc++ = gco_push_numeric( &od->ela[0] )) == OP_ERROR )
	...
	*/
	...
	if ( (*pc++ = gco_push_numeric( &od->ela[1] )) == OP_ERROR )
	    *pc++ = EOP_BADDVOD;	/* arg1: error ID */
...
# endif /* #ifdef DOUBLEBYTE */
	...
	if ( (*pc++ = gco_push_numeric( &od->ela[2] )) == OP_ERROR )
	    *pc++ = EOP_BADDVOD;	/* arg1: error ID */
	...
	*/
	...
	if ( flags & GCO_ALIGN_MASK )
	{
	    ...
	    {
# ifdef GCXDEBUG
		if ( gco_trace_level >= 4 )  *pc++ = OP_DEBUG_BUFS;	/* Trace */
# endif /* GCXDEBUG */
	...
	*pc++ = OP_CALL_DV;	/* Compile and call GCA_DATA_VALUE program */
    ...
    {
# ifdef DOUBLEBYTE
	if ( od == gco_od_obj_desc )  *pc++ = OP_OBJ_DESC;
# endif /* #ifdef DOUBLEBYTE */
...
# ifdef DOUBLEBYTE
	if ( od == gco_od_obj_desc )  *pc++ = OP_OBJ_DESC_DONE;
# endif /* #ifdef DOUBLEBYTE */
    }
...
gco_comp_list( GCA_ELEMENT *ela, GCO_OP **in_pc, i4  flags )
{
    ...
    */
    if ( (*pc++ = gco_push_numeric( &ela->od_ptr->ela[0] )) == OP_ERROR )
	*pc++ = EOP_VARLSTAR;	    /* arg1: error ID */
...
# endif /* GCXDEBUG */
	...
	*/
	if ( flags & GCO_ALIGN_MASK )
		...
		case GCA_VAREXPAR:
		    /*
		    ** Need to push the array length prior 
		    ** to adding padding (fall through to
		    ** following case).
		    */
		    length_pushed = TRUE;
		    if ( (*pc++ = gco_push_numeric( prev )) == OP_ERROR )
...
# endif /* GCXDEBUG */
	...
	case GCA_VAREXPAR:
	    ...
	    */
	    if ( ! length_pushed  && 
		 (*pc++ = gco_push_numeric(prev)) == OP_ERROR )
		*pc++ = EOP_NOT_INT;	/* arg1: error ID */
...
# endif /* GCXDEBUG */
	...
	case GCA_VARSEGAR:
	    ...
	    ** array.
	    **
	    ** Push the array length on the stack.  Since
	    ** the size is not known until runtime, need
	    ** to adjust buffer allocations at runtime.
	    */
	    if ( (*pc++ = gco_push_numeric( prev )) == OP_ERROR )
		*pc++ = EOP_NOT_INT;	/* arg1: error ID */
	...
	case GCA_VARVCHAR:
	    ...
	    *pc++ = OP_PUSH_VCH;	/* Calculate lengths for varchar */
	    ...
	    /*
	    ** Process the data, length already on stack.
	    */
	    sz_sub_struct = gco_comp_atomic( ela, &pc );
	    *size_pc = (GCO_OP)sz_sub_struct;
	    mul_sub_struct = 1;		/* size of 1 element, caller has TPL */
	    ...
	    /*
	    ** Process the padding, length already on stack.
	    */
	    *pc++ = OP_CV_PAD; 
	    break;
	...
	case GCA_VARIMPAR:
	    ...
	    *pc++ = 0;			    /* arg1: pc offset */
	    *pc++ = OP_MARK;		/* Mark object boundary */
	    *pc++ = OP_COPY_MSG;	/* Calc array length from msg length */
	    size_pc = pc++;		    /* arg1: object size */
...
# ifdef DOUBLEBYTE
		...
		*pc++ = (GCO_OP)ela->tpl.type;
		...
		{
		    /* 
		    ** QTXT can grow to any length
		    ** VAREXPAR is variable length 
		    */
		    *pc++ = (GCO_OP)0;
		}
		else if (ela->arr_stat > 0)
		    *pc++ = (GCO_OP)ela->arr_stat; /* array of len arr_stat */
		else
		    *pc++ = (GCO_OP)ela->tpl.length;
	    }
# endif /* # ifdef DOUBLEBYTE */
...
static GCO_OP
gco_push_numeric( GCA_ELEMENT *ela )
{
    GCO_OP op = OP_ERROR;
	...
	switch( ela->tpl.type )
	{
		...
		switch( ela->tpl.length )
		{
		case 2 : op = OP_PUSH_I2;	break;
		case 4 : op = OP_PUSH_I4;	break;
		}
		break;
	}
