		...
		OPC_NODE    *cnode)
{
    ...
    ** estimate rows to be read from table to help group read determination. */
    ...
    /* If potential qualification predicates were identified, build a CX
    ** to generate values for selecting partitions.  We pass the PR node's
    ** boolean factor bitmap as well, since pqbf might include predicates
    ** that are really outer join ON clause predicates;  we should not
    ** generate qualifications that aren't part of the orig qual.
...

{
    ...
    */
	...
	for (i = 0; i < pqbfp->opb_pbfcount; i++)
	{
	    ...
	    /* Only use a BF if it's a regular orig qualification too */
	    if (!BTtest(keyp->opb_gbf, (char *)bfactbm))
	    {
		ok = FALSE;
		break;
	    }
