		...
		OPC_NODE    *cnode)
{
	...
	*/
	...
	case DB_ORIG:
	    ...
	    ** subselect, then this must be an orig node. Otherwise,
	    ** the subselect must mean that this is a QEN_QP node (like,
	    ** for a view).
	    */
	    if (subqry->ops_vars.opv_base->opv_rt[bco->opo_union.opo_orig]->opv_subselect
									== NULL
		)
	    {
		qntype = QE_ORIG;
		...
		OPE_BMEQCLS	*fattmap)
{
	...
	*/
	...
	opc_adbase(global, &cadf, QEN_ROW, qp->qp_row_cnt, 0, 0);
