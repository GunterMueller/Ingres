		...
		i4	    *szconst)
{
	...
	*ninstr += 1;
	...
	{
	    PST_QNODE	*cptr;
	    /* Iterate down left side in case this is a IN-list
	    ** generated constant chain. It avoids recursive stack
	    ** overflow. */
	    for (cptr = root; cptr; cptr = cptr->pst_left)
	    {
		*nconst += 1;
	...
	*nops += 2;
    ...
    if (root->pst_left != NULL)
    {
	opc_cxest(global, cnode, root->pst_left, ninstr, nops, nconst, szconst);
    }
    if (root->pst_right != NULL)
    {
		...
		i4	    *tidatt)
{
    ...
    ** on to opc_cqual1. */
    ...
    if (resop != (ADE_OPERAND *)NULL) resop->opr_offset = OPR_NO_OFFSET;
						/* init result offset */
    result = opc_cqual1(global, cnode, root, cadf, segment, resop, FALSE);
		...
		bool	    incase)
{
	...
	** AND/OR execution may involve branching through ADF expr */
	...
	ADE_OPERAND	result, *oldres, *newres;
		...
		whlistp = whlistp->pst_left)
	{
		...
		** is made between SEARCHED and SIMPLE cases. */
				...
				PST_SEARCHED_CASE)
		{
			...
			cadf, segment, (ADE_OPERAND *)NULL, TRUE);
	    ...
	    ** OR true labels encountered in the when expression. */
	    ...
	    (VOID) opc_cqual1(global, cnode, thenp, cadf, segment, resop, TRUE);
	...
	*/
	if (root->pst_sym.pst_value.pst_s_op.pst_isescape == PST_HAS_ESCAPE)
	...
	*/
	...
	else
	{
	    ...
	    /* If result location was specified, we're done unless coercion needed. */
	...
	switch (root->pst_sym.pst_value.pst_s_cnst.pst_tparmtype)
	{
	 case PST_RQPARAMNO:
	    ...
	    */
	    ...
	    {
		opc_adbase(global, cadf, QEN_PARM, parm_no, 0, 0);
		ops[0].opr_base = cadf->opc_repeat_base[parm_no];
	    }
	    opc_adseglang(global, cadf,
		    root->pst_sym.pst_value.pst_s_cnst.pst_cqlang, segment);
	...
	** no Unicode normalization is required. */
	...
	if (resop != NULL)
	{
	    ...
	    ** result location. */
		...
		else if ((ops[0].opr_len != ADE_LEN_UNKNOWN) && !(utf8unorm))
		{
			...
			*/
			uniop.opr_len = (uniop.opr_len - 1 - DB_CNTSIZE) * 4 
					+ 1 + DB_CNTSIZE;
		    ...
		    {
			uniop.opr_len = (uniop.opr_len - DB_CNTSIZE) * 4
					+ DB_CNTSIZE;
			if(uniop.opr_len > DB_MAXSTRING)
                        {
...

{
    ...
    escape_dv.db_data = &root->pst_sym.pst_value.pst_s_op.pst_escape;
    opc_adconst(global, cadf, &escape_dv, &escape_ops[0],
						    DB_SQL, segment);
    opc_adinstr(global, cadf, ADE_ESC_CHAR, segment, 1, escape_ops, 0);
}

/*{
	...
	*/
	    ...
	    abs(rdi->opc_srcop.opr_dt) == DB_NVCHR_TYPE)
	{
	    ...
	    ** operate on a large enough buffer. */
	    ...
	    {
		rdi->opc_srcop.opr_len = min(4 * resdom->pst_right->pst_sym.
					pst_dataval.db_length, DB_MAXSTRING); /* kludge */
		...
		if (abs(restype) != DB_NCHR_TYPE )
		{
		  rdi->opc_srcop.opr_len = (rdi->opc_srcop.opr_len - DB_CNTSIZE)
				* 4 + DB_CNTSIZE;
		...
		{
		  rdi->opc_srcop.opr_len = (rdi->opc_srcop.opr_len) * 4;
		  if (rdi->opc_srcop.opr_len > DB_MAXSTRING)
		    rdi->opc_srcop.opr_len = DB_MAXSTRING;   
		}
		...
		if (abs(restype) != DB_NCHR_TYPE)
	        {
		  rdi->opc_srcop.opr_len = (rdi->opc_srcop.opr_len - 1 - DB_CNTSIZE) * 4 + 1 + DB_CNTSIZE;
		  if (rdi->opc_srcop.opr_len > DB_MAXSTRING)
                   rdi->opc_srcop.opr_len = DB_MAXSTRING + 1;
	        }
		else if (abs(restype) != DB_NVCHR_TYPE )
		{
		  rdi->opc_srcop.opr_len = (rdi->opc_srcop.opr_len - 1) * 4 + 1;
		  if (rdi->opc_srcop.opr_len > DB_MAXSTRING)
                   rdi->opc_srcop.opr_len = DB_MAXSTRING + 1;
		}
		...
		abs(resdom->pst_sym.pst_dataval.db_datatype) != DB_LVCH_TYPE)
	{
	    ...
	    */
	    ...
	    if ((abs(restype) == DB_CHA_TYPE || abs(restype) == DB_CHR_TYPE) &&
	        (abs(rdi->opc_srcop.opr_dt) == DB_VCH_TYPE ||
	         abs(rdi->opc_srcop.opr_dt) == DB_TXT_TYPE))
	      rdi->opc_srcop.opr_len += 2;    /* add length for var result */

	    if (rdi->opc_srcop.opr_len > DB_UTF8_MAXSTRING)
	      rdi->opc_srcop.opr_len = DB_UTF8_MAXSTRING+1;
	}

	(VOID) opc_cqual1(global, cnode, resdom->pst_right, 
		cadf, segment, &rdi->opc_srcop, FALSE);
		...
		QEN_ADF	    **qadf)
{
    ...
    */
	...
	opc_adbase(global, &cadf, QEN_ROW, outrow, 0, 0);
	break;
	...
	*/
	...
	opc_adbase(global, &cadf, QEN_ROW, qn->qen_row, 0, 0);
		...
		OPC_ADF	    *cadf)
{
	...
	*/
    ...
    {
	if (attno == DB_IMTID || 
		rel->rdr_attr[attno]->att_flags & DMT_F_NDEFAULT
	    )
	{
    ...
    ** else, the corresponding default value). */
	...
	if (passed_params_left == 0)
	{
	    ...
	    ** Otherwise, compile in the default/null value. */
	    if (attr->attr_flags & DMT_F_KNOWN_NOT_NULLABLE &&
	    ...
	    if (attr->attr_flags & DMT_F_NDEFAULT)
		...
		RDR_INFO	*rel)
{
    ...
    ** expression contains nested CASE expressions. */
    ...
    opc_crupcurs1(global, root, cadf, resop, rel);
		...
		RDR_INFO	*rel)
{
     ...
     case PST_AND:
        ...
        if(global->ops_statement->pst_stmt_mask & PST_NONCNF)
        {
            ...
            ** join the false list. */
            ...
            truelab = cadf->opc_truelab;        /* save across call */
            cadf->opc_truelab = (char *)&locallab;
            opc_crupcurs1(global, root->pst_left, cadf, resop, rel);
            opc_adlabres(global, cadf, &locallab);
            cadf->opc_truelab = truelab;                /* restore after call */
            opc_adinstr(global, cadf, ADE_ANDL, ADE_SMAIN, 1, 
			(ADE_OPERAND *)cadf->opc_falselab, 0);
            opc_crupcurs1(global, root->pst_right, cadf, resop, rel);
        }
        else
        {
	...
	** init'ed. */
		...
		whlistp = whlistp->pst_left)
	{
	    ...
	    ** OR true labels encountered in the when expression. */
	    ...
	    (VOID) opc_crupcurs1(global, thenp, cadf, resop, rel);
	...
	{	       
	    ...
	    opc_crupcurs1(global, lqnode, cadf, &ops[opnum], rel);
	...
	** problems forever. */
	if (inlist != (PST_QNODE *) NULL)
	{
		...
		** for a binary operation (major kludge). */
		...
		opc_crupcurs1(global, inlist, cadf, &ops[1], rel);
					/* materialize/coerce constant */
		global->ops_gmask &= (~OPS_DECCOERCION);
	...
	*/
	    ...
	    global->ops_cstate.opc_before_row == -1)
	{
		...
		*/
		...
		opc_adbase(global, cadf, QEN_ROW,
			   global->ops_cstate.opc_before_row, 0, 0);
	    }
	...
	*/
	if (attno == DB_IMTID)
	{
	    ...
	    */
		...
		{
		    opc_adbase(global, cadf, QEN_ROW,
			global->ops_cstate.opc_bef_tid, 0, 0);
		    ops[0].opr_base =
			cadf->opc_row_base[global->ops_cstate.opc_bef_tid];
		}
	    }
		...
		{
		    opc_adbase(global, cadf, QEN_ROW,
			global->ops_cstate.opc_aft_tid, 0, 0);
		    ops[0].opr_base =
			cadf->opc_row_base[global->ops_cstate.opc_aft_tid];
		}
	    }
	    ...
	    */
		...
		ops[0].opr_base =
		    cadf->opc_row_base[global->ops_cstate.opc_after_row];
                ...
                {
                    opc_adbase(global, cadf, QEN_ROW,
			global->ops_cstate.opc_after_row, 0, 0);
		    ops[0].opr_base =
			cadf->opc_row_base[global->ops_cstate.opc_after_row];
		}
	    }
	...
	*/
	...
	opc_crupcurs1(global, root->pst_left, cadf, &ops[0], rel);
	    ...
	    root->pst_sym.pst_value.pst_s_rsdm.pst_ttargtype == PST_DBPARAMNO)
	{
	    ...
	    QEF_CP_PARAM *param = &opc_pst->opc_stmtahd->qhd_obj.qhd_callproc.
	    ...
	    else param->ahd_parm.parm_flags = 0;
	...
	opc_crupcurs1(global, root->pst_right, cadf, &ops[1], rel);
...

{
    ...
    ** INOUT/OUT parameter. */
	...
	{
	    opc_adbase(global, cadf, QEN_ROW, param->ahd_parm_row, 0, 0);
	    ops[1].opr_base = cadf->opc_row_base[param->ahd_parm_row];
	}
	opc_adinstr(global, cadf, ADE_MECOPY, ADE_SMAIN, 2, ops, 1);
...

{
    ...
    ** to map the dsh row to our ADF CX. */
    ...
    opc_adbase(global, cadf, QEN_ROW, dshrow, 0, 0);
