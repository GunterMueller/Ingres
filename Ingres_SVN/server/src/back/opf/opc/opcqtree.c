		...
		i4	    tid_key)
{
    ...
    */
	...
	for (keyno = 0; keyno < nkey_atts; keyno += 1)
	{
		...
		kconst->opc_ndbpvars = 0;

		kconst->opc_qparms = NULL;
		kconst->opc_nqparms = 0;
	    }
	    else
	    {
		cqual->opc_qsinfo.opc_kptype = PST_RQPARAMNO;
    ...
    */
	...
	for (cqconst = kconst->opc_qconsts, constno = 0;
		cqconst != NULL;
		cqconst = cqconst->opc_qnext, constno += 1
	    )
	{
	    cqconst->opc_tconst_type = PST_USER;
	    cqconst->opc_const_no = constno;
	    kconst->opc_nqconsts += 1;
	}

	/* count up the number of qparm and dbpvar values. */
	for (parmno = 0; parmno < szparm; parmno += 1)
	{
	    if (kconst->opc_qparms != NULL)
	    {
		for (cqconst = kconst->opc_qparms[parmno];
			cqconst != NULL;
			cqconst = cqconst->opc_qnext
		    )
		{
		    kconst->opc_nqparms += 1;
		}
	    }

	    if (kconst->opc_dbpvars != NULL)
	    {
		for (cqconst = kconst->opc_dbpvars[parmno];
			cqconst != NULL;
			cqconst = cqconst->opc_qnext
		    )
		{
		    kconst->opc_ndbpvars += 1;
		}
	    }
	}
    }
    ...
    switch (root->pst_sym.pst_type)
    {
     ...
     case PST_BOP:
		...
		*/
		...
		if (root->pst_sym.pst_value.pst_s_op.pst_isescape == 
								PST_HAS_ESCAPE
		    )
		{
		    kconst->opc_keyblk.adc_escape = 
			root->pst_sym.pst_value.pst_s_op.pst_escape;
		    kconst->opc_keyblk.adc_isescape = TRUE;
		}
		else
		{
		    kconst->opc_keyblk.adc_isescape = FALSE;
		}
		...
		PST_QNODE	*qconst)
{
    ...
    ** data.
    */
    if ((ret = adi_fidesc(global->ops_adfcb, adffn.adf_fi_id, &fdesc)) 
								!= E_DB_OK
	)
    {
	/* EJLFIX: get a better error no */
...
**	    Current cost node.
**	qen_type -
**	    The type of QEN node that is currently being built.
**	outer_map -
**	    The first map of eqcs that are available at the current node.
**	inner_map - 
**	    Inner_map may be NULL, in which case outer_map 
**	    holds the only available
**	    eqcs. Otherwise, inner_map holds an additional set of eqcs that are
**	    available at the current node. If given (ie non-null) then each
**	    boolfact must require both outer_map and inner_map for evaluation.
**	bfactbm -
