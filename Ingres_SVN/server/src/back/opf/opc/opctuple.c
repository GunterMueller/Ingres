		...
		bool		projone)
{
	...
	*qadf = (QEN_ADF *)NULL;
	    ...
	    )
	{
	    ...
	    opc_adbase(global, &cadf, QEN_ROW, ceq[eqcno].opc_dshrow, 0, 0);
	}
...
**		  - it adds a I1 column init'ed to 0 at end of each row
**		    for outer join processing (to build row, if left
**		    join; to probe row for right join; or not at all)
**
{@comment_line@}...
**
** Inputs:
		...
		bool		oj)
{
    ...
    */
    rowsz = &(((QEF_HASH_DUP *)0)->hash_key[0]) - (char*)0;
    keystart = rowsz;	
    rowno = qp->qp_row_cnt;
	...
	for (eqcno = 0, i = 0; eqcno < subqry->ops_eclass.ope_ev; eqcno += 1)
	{
		...
		** that cleans it up. */
		if (cmp->cmp_type != ceq[eqcno].opc_eqcdv.db_datatype ||
		    ...
		    keytype == DB_ADTE_TYPE || keytype == DB_TME_TYPE ||
    ...
    /* Always treat the single or compound hash key as a single byte(n) key */

    ops[1].opr_dt = DB_BYTE_TYPE;
    ops[1].opr_len = keysz;
    ops[1].opr_prec = 0;
    ops[1].opr_collID = -1;
    ops[1].opr_base = cadf.opc_row_base[rowno];
    ops[1].opr_offset = keystart;

    /* Source operands are built, now do result and emit the instr. */
    ...
    ops[0].opr_collID = -1;
    ops[0].opr_base = cadf.opc_row_base[rowno];
    ops[0].opr_offset = 0;	/* first attr in buffer */
    opc_adinstr(global, &cadf, 807 /* ADFI_807_HASH_ALL */, ADE_SMAIN, 
								2, ops, 1);

    /* Add a byte in case it's outer join.  We now add this
    ** all the time, so that the runtime logic doesn't have to
    ** flail around testing whether it's there or not.
    */

    rowsz++;

    rowsz = (i4)ME_ALIGN_MACRO(rowsz,sizeof(ALIGN_RESTRICT));	
				/* align the buffer */
    opc_ptrow(global, &rowno, rowsz);
    *prowsz = rowsz;
    *prowno = rowno;
