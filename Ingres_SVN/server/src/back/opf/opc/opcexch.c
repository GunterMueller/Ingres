		...
		OPC_NODE    *cnode)
{
    ...
    ** partitioned ORIG nodes and partition compatible joins. */
    if (ex->exch_ccount > 1)
	opc_pushcount(ex->exch_out, co->opo_variant.opo_local->opo_pgcount,
			ex->exch_ccount);
    /* if (ex->exch_out->qen_type == QE_ORIG && 
	ex->exch_out->node_qen.qen_orig.orig_part &&
	ex->exch_ccount > 1)
      ex->exch_out->node_qen.qen_orig.orig_part->part_threads =
				ex->exch_ccount; */
	...
	** of subarray counts for each class of buffers/structs. */
	...
	else opc_exnodearrcnt(global, qn->node_qen.qen_exch.exch_out, arrcnts);
...
opc_eexatts(
	...
	OPE_BMEQCLS	*eeqcmp,
	OPC_EQ		*ceq,
	DMT_ATTR_ENTRY	***patts,    /* ptr to a ptr to an array of ptrs 
					to atts */
	i4		*pacount )
{
    OPS_SUBQUERY	*subqry = global->ops_cstate.opc_subqry;
