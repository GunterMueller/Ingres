	...
	PST_QNODE          *newaop)
{
    ...
    */
	...
	{
# ifdef OPT_F006_AGGCOMPAT
	    if (opt_strace( subquery->ops_global->ops_cb, OPT_F006_AGGCOMPAT ) )
		TRdisplay("found identical aop %d\n", newaop);
# endif
	    return (subquery);
	}
    }
...
# ifdef OPT_F006_AGGCOMPAT
	if (opt_strace( subquery->ops_global->ops_cb, OPT_F006_AGGCOMPAT ) )
	    TRdisplay("Simple aggregates are IDENTICAL\n");
# endif
    }
    else
    {
...
# ifdef OPT_F006_AGGCOMPAT
	if (opt_strace( subquery->ops_global->ops_cb, OPT_F006_AGGCOMPAT) )
	    TRdisplay("Simple aggregates may be run together... add to aggregate list\n");
# endif
        {
...
# ifdef OPT_F006_AGGCOMPAT
	if (global->ops_cb->ops_check 
            && 
 	    opt_strace( global->ops_cb, OPT_F006_AGGCOMPAT ) )
	    TRdisplay("Function aggregates are IDENTICAL\n");
# endif
...
# ifdef OPT_F006_AGGCOMPAT
	if (global->ops_cb->ops_check 
            && 
	    opt_strace( global->ops_cb, OPT_F006_AGGCOMPAT ) )
	    TRdisplay("add aggregate into tree\n");
# endif
...
# ifdef OPT_F006_AGGCOMPAT
    if (opt_strace( subquery->ops_global->ops_cb, OPT_F006_AGGCOMPAT ) )
	TRdisplay("Subselects are IDENTICAL\n");
# endif
    subquery->ops_agg.opa_redundant = TRUE; /* set flag indicating that
...
# ifdef OPT_F006_AGGCOMPAT
    ...
    trace = opt_strace( subquery->ops_global->ops_cb, OPT_F006_AGGCOMPAT );
# endif
    global = subquery->ops_global;
...
# ifdef OPT_F006_AGGCOMPAT
	if (trace)
	    TRdisplay("Candidate contains nested aggregates -- so exclude it.\n");
# endif
	return(FALSE);
    }
...
# ifdef OPT_F006_AGGCOMPAT
	if (trace)
	    TRdisplay("Cannot combine a simple and function aggregate\n");
# endif
	return(FALSE); 
    }
...
# ifdef OPT_F006_AGGCOMPAT
	if (trace)
	    TRdisplay("Cannot combine - corelation variables differ \n");
# endif
	return(FALSE); 
    }
...
# ifdef OPT_F006_AGGCOMPAT
	if (trace)
	    TRdisplay("Cannot combine - range variables do not match \n");
# endif
	return(FALSE); 
    }
...
# ifdef OPT_F006_AGGCOMPAT
	if (trace)
	    TRdisplay("Cannot combine - unions \n");
# endif
	return(FALSE); 
    }
...
# ifdef OPT_F006_AGGCOMPAT
	if (trace)
	    TRdisplay("Cannot combine - duplicate handling does not match \n");
# endif
	return(FALSE); 
    }
...
# ifdef OPT_F006_AGGCOMPAT
	    if (trace)
	       TRdisplay("aggregates incompatible due to PRIME/NONPRIME rules\n");
# endif
...
# ifdef OPT_F006_AGGCOMPAT
	    if (trace)
		TRdisplay("aggregates do not range over same variables ",
		        "-- so exclude it.\n");
# endif
	    return(FALSE);
	}
...
# ifdef OPT_F006_AGGCOMPAT
	    if (trace)
		TRdisplay("aggregate qualification lists differ ",
		        "-- so exclude it.\n");
# endif
	    return(FALSE);
	}
...
# ifdef OPT_F006_AGGCOMPAT
	if (trace)
	    TRdisplay("Candidate by lists differ",
    	        "-- so exclude it.\n");
# endif
	return(FALSE);
...
# ifdef OPT_F006_AGGCOMPAT
	if (trace)
	    TRdisplay("Candidate subselect target lists differ",
    	        "-- so exclude it.\n");
# endif
	return(FALSE);
    }
...
# ifdef OPT_F006_AGGCOMPAT
	if (opt_strace( (*previous)->ops_global->ops_cb, OPT_F006_AGGCOMPAT ) )
	{
	    TRdisplay("Candidate identical aggregate is ...\n");
	    opu_qtprint( (*previous)->ops_root);
	}
# endif
...
# ifdef OPT_F003_AGGREGATE
	if (opt_strace( subquery->ops_global->ops_cb, OPT_AGLOOP ) )
