...
**
** Description:
**      This structure is based on the OPB_MBF and describes columns on
**	which a table is partitioned and on which there are boolean factors
**	that may be used for partition qualification (sometimes also known
**	as partition elimination).
**
**      The columns are represented as joinop attributes and are a subset
**	of the partitioning columns for a single dimension of a table
**	partitioning scheme. There is one OPB_PQBF structure instance for
**	each dimension of each partitioned table in the query. The columns
**	in the list all have at least one limiting predicate comparing
**	it to either a constant or a repeat query parameter. 
**
** History:
**	14-jan-04 (inkdo01)
...
{
    struct _OPB_PQBF *opb_pqbfnext;	/* pointer to next OPB_PQBF for
					** current table
					*/
    i4             opb_pbfcount;        /* first "n" elements in
                                        ** partitioning column list which
                                        ** have BFs associated with them
                                        */
    i4             opb_pccount;		/* total number of elements defined
                                        ** in the partitioning column list
                                        ** whether boolean factors exist or not
                                        */
    i4		   opb_pqcount;		/* number of value sets (or "rows")
					*/
    i2		   opb_pdim;		/* partition dimension
					*/
#define OPB_BETWEEN_OP	99	/* special value to show between's */
    ADI_OP_ID	   opb_pqopno;		/* operator code for rightmost
					** comparison (=, <, <=, >= or >)
					*/
    OPB_KA	   *opb_pcbase;		/* list of joinop attributes
                                        ** and respective limiting predicates
                                        ** on those attributes, in order of
                                        ** partitioning column sequence
                                        */
}   OPB_PQBF;

/*}
...
#define                 OPB_FALIKE      0x00000080L
/* SET when the special case STAR problem occurs when the left operand of
** the LIKE operator contains an expression */
