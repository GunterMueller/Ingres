...
typedef struct _OPS_SUBQUERY
{
    ...
    PST_QNODE	    *ops_bygsets;	/* NULL if this subquery doesn't 
					...
					*/
    i4              ops_mode;           /* subquery mode - retrieve, delete,
    ...
    OPN_JTREE	    *ops_jtreep;	/* ptr to JTREE node of plan fragment 
					...
					** enumeration
					*/
    OPO_CO	    *ops_tempco;	/* lower left join CO-node addr for
					** "in progress" compile with new
...
#define                 OPS_CCARTPROD	    0x000000100L
/* TRUE - if at least one relation exists which has a multi-attribute
** storage structure and if the relation is removed then the remaining
** relations would form a cart prod; this is used to check using a
** cart prod in order to key into a relation is desirable */
...
#define                 OPS_FALIKE	    0x000004000L
/* TRUE - if this subquery has a LIKE operator with an expression as an operand */
...
/* TRUE - if aggregate result cost estimate produced by opn_modcost(). */
    ...
    i4         ops_timeout;        /* TRUE - if enumeration timed out
