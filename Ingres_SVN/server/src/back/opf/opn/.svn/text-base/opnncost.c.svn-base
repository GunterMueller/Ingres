	...
	OPN_STATUS	   *sigstat)
{
    ...
    *sigstat = opx_cinter( subquery );	/* this routine may not return if
	...
	np->opn_rlasg, np->opn_sbstruct, (OPL_BMOJ *)NULL, subtp, eqclp, rlmp, 
	&icheck);
	...
	*rlmp = (OPN_RLS *) NULL;
	...
	opn_ncommon (subquery, np, rlmp, eqclp, subtp, eqr, rv, 
	    np->opn_nleaves, &blocks, selapplied, sel, &tups, (OPL_BMOJ *)NULL); 
					/* common routine 
	...
	if (sbp->opn_coback == (OPO_CO *)sbp)
	    opn_prleaf (subquery, sbp, ecp, rlp, blocks, selapplied, sel, varp, keyed_only);
	...
	opn_ncommon (subquery, np, rlmp, eqclp, subtp, eqr, rv, np->opn_nleaves,
	    &blocks, selapplied, sel, &tups, (OPL_BMOJ *)NULL);
        if (rsubtp->opn_coforw == (OPO_CO *) rsubtp)
        {
