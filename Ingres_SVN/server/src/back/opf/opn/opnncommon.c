	...
	OPL_BMOJ	   *filtermap)
{
	...
	*tuples = tups;
	...
	if (filtermap)
	    MEcopy((PTR)filtermap, sizeof(eqsp->opn_maps.opo_filter),
		(PTR)&eqsp->opn_maps.opo_filter);
	else
	    MEfill(sizeof(eqsp->opn_maps.opo_filter), (u_char)0,
		(PTR)&eqsp->opn_maps.opo_filter);
	...
	opx_error(E_OP04B5_FILTEREOUTERJOIN);	/* expecting to create a new structure */
