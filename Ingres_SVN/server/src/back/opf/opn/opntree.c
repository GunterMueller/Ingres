...

/*{
** Name: opn_thandler	- exception handler for search space traversal
**
** Description:
**	It order to traverse the search space in a slightly different
**	order, i.e. look at unbalanced trees with indexes first, prior
**	to looking at balanced trees, this exception handler is called
**	to unwind the stack and traverse the search space in a more
**	desirable order.
** Inputs:
...
**          initial creation
**	16-mar-92 (rog)
**          Remove EX_DECLARE case and bring code up to spec.
[@history_line@]...
*/
static STATUS
opn_thandler(
	EX_ARGS            *args)
{
	...
	if ((args->exarg_count > 0)
	    &&
	    (args->exarg_array[0] == E_OP0401_SEARCHSPACE)
	    )
    ...
    case 2:
				    ...
				    ** cleared the stack so return RESIGNAL
