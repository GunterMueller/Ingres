	...
	OPS_SUBQUERY       *subquery)
{
	...
	** in the ON clause */
	...
	outerp5->opl_rmaxojmap = outerp5->opl_maxojmap;
	      /* following line removed in conjunction with above change
	      ** for bug 63997. If OPL_OJFILTER code is ever implemented,
	      ** this stuff will be redundant, anyway.   */
	/*terp5->opl_maxojmap = outerp5->opl_minojmap; /* remove this
				** line when OPC filter processing is
				** enabled, this prevents associative
				** optimizations */
#endif
    }
#ifdef OPL_OJFILTER
    /* enable when filter processing supported in OPC */
    {	/* check for need for top sort node duplicate handling requirements
	** of outer joins... in order to allow extended use of secondary
	** indices, it may be necessary to add a top sort node to remove
	** duplicates... select r.a from (r left join s on s.b=r.b and s.c=5)
	** if a secondary index existed on s.b, it may be worthwhile to
	** probe first, and then do a tid join to s and apply s.c, however
	** this would require duplicates to be eliminated by null tuples
	** which are duplicates introduced by applying s.c=5
	**                Tjoin
        **               /     \
        **              /       s
        **	  Kjoin (s.b)
	...
	*/
	...
	{   /* look at each outer join and include the equivalence
	    ...
	    ** current set */
	    ojp1 = lbase->opl_ojt[ojid1];
	    if ((ojp1->opl_type == OPL_LEFTJOIN) 
		&&
		(BTcount((char *)ojp1->opl_maxojmap, (i4)maxvar) > 1)
		)
	    {	/* there is a possibility of a split in the outer join
		** since more than one relation exists in the inner
		** map */
		subquery->ops_oj.opl_smask |= OPL_OJSPLIT;
		/* FIXME - get smarter about this by checking
		** that qualifications can be applied to the base
		** relation and not the secondary index */
	    }
	...
	{   /* this implies that all tids need to be visible in the
	    ** target list as non-printing resdoms and duplicates may
	    ** need to be removed */
	    ...
	    if (opa_rsmap( subquery->ops_global, subquery->ops_root, &attrmap)) /* attrmap has 
	    ...
	    MEfill(sizeof(subquery->ops_agg.opa_keeptids), (u_char)0,
		(PTR)&subquery->ops_agg.opa_keeptids);
	    subquery->ops_duplicates = OPS_DREMOVE;
	    for (varno3 = subquery->ops_vars.opv_prv; --varno3 >= 0;)
	    {
		OPV_VARS	*var3p;
		var3p = vbase->opv_rt[varno3];
		if (var3p->opv_grv->opv_relation
		    ||
		    (var3p->opv_grv->opv_created == OPS_FAGG)
		    ||
		    (var3p->opv_grv->opv_created == OPS_HFAGG)
		    ||
		    (var3p->opv_grv->opv_created == OPS_RFAGG)
		    ||
		    (var3p->opv_grv->opv_created == OPS_VIEW)
		    )
		{   /* tids exist for real relations */
			...
			opx_error(E_OP0201_ATTROVERFLOW); /* too many attributes
						** are being defined */
		    BTset ((i4)var3p->opv_gvar, 
			(char *)&subquery->ops_agg.opa_keeptids);
		    opa_tnode(subquery->ops_global, nextattr, (OPV_IGVARS)varno3, 
			subquery->ops_root);
		    if ((subquery->ops_root->pst_left->pst_right->pst_sym.pst_type != PST_VAR)
			||
			(subquery->ops_root->pst_left->pst_right->pst_sym.
			    pst_value.pst_s_var.pst_atno.db_att_id != DB_IMTID))
			opx_error(E_OP04B2_TUPLEID);
		    if (var3p->opv_grv->opv_created == OPS_VIEW)
		    {
			subquery->ops_mask |= OPS_TIDVIEW;
			opv_tidview(subquery, subquery->ops_root->pst_left->pst_right, 
			    varno3, TRUE);
		    }
		    else
			subquery->ops_root->pst_left->pst_right->pst_sym.pst_value.pst_s_var.
			pst_atno.db_att_id = opz_addatts( subquery, varno3, 
			    imdmfattr.db_att_id, &tid);
						/* create joinop attribute number of the
						** implicit TID.
						*/
		}
		else if (var3p->opv_grv->opv_created == OPS_SELECT)
		    ;
		else
		    opx_error(E_OP04B2_TUPLEID);
	    }
	...
	{   /* for each full join, check that the opz_outervars map of the
	    ...
	    ** the full join */
		...
		if (BTtest((i4)attrp->opz_varnm, (char *)outerp2->opl_p1))
		    ...
		    continue;
		MEcopy((PTR)maskp, sizeof(mask_map), (PTR)&mask_map);
		BTnot((i4)BITS_IN(mask_map), (char *)&mask_map);
		BTand((i4)maxvar, (char *)&mask_map, (char *)&attrp->opz_outervars);
	...
	OPV_BMVARS         *varmap)
{
    ...
    {
	OPV_BMVARS	tempmap;
	MEcopy( (PTR)varmap, sizeof(*varmap), (PTR)&tempmap);
	BTnot((i4)BITS_IN(tempmap), (char *)&tempmap);
	BTand( (i4)BITS_IN(*varmap), (char *)&tempmap, (char *)ojp->opl_ovmap);
    }
    else
    ...
    ** into the parent */
	...
	for (ojid1 = maxoj; --ojid1>=0;)
	{
		...
		** two sets of variables partitioned by this outer join */
		...
		varno1 = BTnext((i4)-1, (char *)ojp1->opl_p2, (i4)maxvars);
		if (varno1 < 0)
		    opx_error(E_OP0399_FJ_VARSETS);	/* there should be at least two disjoint sets */
		{
		    ...
		    if (MEcmp((PTR)&tempmap, (PTR)ojp1->opl_p2, sizeof(tempmap)))
