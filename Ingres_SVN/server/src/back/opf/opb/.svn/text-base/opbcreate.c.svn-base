...
{   /* this subtree has operators which depend on NULL
    ...
    ** operations */
    ...
    OPL_OJT	    *lbase;
    OPL_BMOJ	    *ijmap;
    OPS_SUBQUERY    *uv_p = (OPS_SUBQUERY *)0;     /* ptr to union view subquery */
    ...
    /* if the current subquery is in a union view, find that view.
    ** if the current subquery has no special_eqc, use the view's
    ** special_eqc to check if it is being referenced in an oj. 
    */ 
    if (union_view)
    {
       uv_p = subquery; 
       ...
       if (uv_p && special_eqc == OPE_NOEQCLS)
       {
           lbase = uv_p->ops_oj.opl_base; 
           maxoj = uv_p->ops_oj.opl_lv; 
           varno = uv_p->ops_root->pst_sym.pst_value.pst_s_var.pst_vno;
           varp  = uv_p->ops_vars.opv_base->opv_rt[varno]; 
           ijmap = varp->opv_ijmap; 
           special_eqc = varp->opv_ojeqc; 
       }
    }
	    ...
	    (i4)maxoj))>= 0;)
	{
		...
		if (BTtest((i4)ojid, (char *)ijmap))
		{
		    ...
		    BTset((i4)ojid, (char *)&bfp->opb_ojmap);
             ...
             subquery->ops_oj.opl_pinner &&
             uv_p && 
             uv_p->ops_oj.opl_pinner)
    {
       ...
       */
       if (BTnext((i4)-1, 
             (char *)&uv_p->ops_oj.opl_pinner->opl_parser[varp->opv_gvar], 
             (i4)BITS_IN(uv_p->ops_oj.opl_pinner->opl_parser[varp->opv_gvar])) >= 0
           ||
           BTnext((i4)-1, 
             (char *)&uv_p->ops_oj.opl_pouter->opl_parser[varp->opv_gvar], 
             (i4)BITS_IN(uv_p->ops_oj.opl_pouter->opl_parser[varp->opv_gvar])) >= 0
          ) 
          bfp->opb_mask |= OPB_OJMAPACTIVE;
    }
	...
	PST_QNODE          *constant)
{
    ...
    if (opqnodep->pst_sym.pst_value.pst_s_op.pst_isescape == PST_HAS_ESCAPE)
    {
	adc_kblk.adc_isescape = TRUE;
	adc_kblk.adc_escape = opqnodep->pst_sym.pst_value.pst_s_op.pst_escape;
    }
    else
	adc_kblk.adc_isescape = FALSE;
	...
	*/
    ...
    {
	if (bfp->opb_orcount)
	    bfp->opb_orcount--;		/* part of b21160 is that the
	...
	{
	    keyhdr->opb_sargtype = ADC_KNOKEY;
	    if (constant->pst_sym.pst_dataval.db_datatype == DB_VCH_TYPE
		&&
		constant->pst_sym.pst_dataval.db_data)
	    {
		i4	i;
	...
	*/
           ...
           )
	{
        ...
        */
        ...
        if (opno == subquery->ops_global->ops_cb->ops_server->opg_ifnull)
        {   /* do not eliminate null join operator if ifnull operator used
            ** over an expression */
            /* FIXME - when UDT support nulls then any user defined function
            ** cannot be trusted either, and reset this flag */
	...
	OPB_IBF            bfi)
{
    ...
    ** "c1 is null and c1 is not null". */
    ...
    /* Do not process outer join attributes because:
    ** t1 left join t2 on t1.col1=t2.col1 where
    ** t1.col1='value1' and t2.col1='value2' will return zero rows */
