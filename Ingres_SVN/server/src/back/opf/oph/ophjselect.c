...
# endif
		    ...
		    tocell += tolength;     /* increment to next cell */
                ...
                /*  
                    For DB_CHA_TYPES, to avoid errors in opn_chardiff we have 
...
{
    OPH_DOMAIN	mu1, mu2, q1m, q2m, c1m, c2m, lcount, rcount;
	...
	** value and accumulate the totals. */
	...
	for (j = 1; j < rattrp->opz_histogram.oph_numcells; j++)
	{
	    ...
	    ** the product of proportions. */
	    rcount = rhistp->oph_creptf[j] / rtuples;
	    q1m += lcount;
	    q2m += rcount;
	    mu2 += (lcount * rcount);
    ...
    jtups = rtuples * ltuples * (c1m * mu1 + c2m * mu2);
    return(jtups);
}
	...
	OPL_OJHISTOGRAM    *ojhistp)
{
	...
	** histogram. */
	if (!(lhistp->oph_unique) && !(rhistp->oph_unique) &&
