	...
	OPE_BMEQCLS        *eqcmap)
{
	...
	** pick the first, regardless of cost characteristics. */
	    ...
	    sortp++)
	{
	    ...
	    if (!found)
	    {
		if (count == 0 && subquery->ops_bfs.opb_mask &
			OPB_SUBSEL_CNST) return(OPE_NOEQCLS);
				    /* if 1st key is only covered by a constant,
				    ** AND there's a subselect constant, we can't
				    ** guarantee the 1st col is covered (87509) */
