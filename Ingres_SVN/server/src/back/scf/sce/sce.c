...
#define	SCE_RETRY_MAX	10		/* Maximum attempts */
	...
	if (Sc_main_cb->sc_events < 0)
	{
	    ...
	    sc0e_put(E_SC0284_BUCKET_INIT_ERROR, 0, 1,
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ...
	    sc0e_put(E_SC0285_EVENT_MEMORY_ALLOC, 0, 2,
	...
	*/
	...
	if (status != OK)
	{
	    sc0e_put(E_SC0286_EVENT_SEM_INIT, 0, 1,
	...
	sce_hash->sch_evc->evc_mem_sem = &Sc_main_cb->sc_sc0m.sc0m_semaphore;
    	...
    	sce_hash->sch_evc->evc_errf = (void((*)()))sc0e_put;	/* For errs */
	...
	if ( !is_an_rcp )
	{
	    ...
	    */
	    ...
	    {
		sc0e_put(E_SC0273_EVENT_RETRY, 0, 1,
...
sce_register( SCF_CB *scfcb, SCD_SCB *scb )
{
    ...
    if (ult_check_macro(&scb->scb_sscb.sscb_trace, SCS_TALERT, 0, 0))
    ...
    {
	scfcb->scf_error.err_code = E_SC0280_NO_ALERT_INIT;
	return (status);
    }
    ...
    */
    ...
    {
	scfcb->scf_error.err_code = E_SC0270_NO_EVENT_MESSAGE;
	return (status);
    }
	...
	*/
	...
	if (reg != NULL)
	{
	    ...
	    scfcb->scf_error.err_code = E_SC002B_EVENT_REGISTERED;
	    status = E_DB_ERROR;
	    break;
	}
	if (alert == NULL)	/* Alert doesn't exist - create a new one */
	{
	    ...
	    if (status != E_DB_OK)
	    {
		scfcb->scf_error.err_code = E_SC0281_ALERT_ALLOCATE_FAIL;
		break;
	    }
	...
	if (status != E_DB_OK)
	{
	    scfcb->scf_error.err_code = E_SC0282_RSES_ALLOCATE_FAIL;
...
sce_remove( SCF_CB *scfcb, SCD_SCB *scb )
{
	...
	if (reg == NULL)
	{
	    ...
	    scfcb->scf_error.err_code = E_SC002C_EVENT_NOT_REGISTERED;
	    status = E_DB_ERROR;
	    break;
	}
...
sce_raise( SCF_CB  *scfcb, SCD_SCB *scb )
{
	...
	if ( CXcluster_enabled() )
	{
	    ...
	    */
	    ...
	    if ( OK != retval )
	    {
		scfcb->scf_error.err_code = E_SC0276_CX_RAISE_FAIL;
		status = E_DB_ERROR;
	    }
	}
	else
...
sce_resignal( SCF_CB  *scfcb, SCD_SCB *scb )
{
	     ...
	     NULL == sce_hash->sch_evc->evc_root )
	{
	    ...
	    */
	    ...
	    if ( E_DB_OK != status )
	    {
		scfcb->scf_error.err_code = E_SC028A_XSEV_CRLOCKLIST;
		break;
	    }
	    sce_hash->sch_flags = SCE_FACTIVE;	/* We're done */
	}

	scfcb->scf_error.err_code = E_DB_OK;
...
sce_thread(void)
{
	...
	*/
    ...
    sc0e_put(E_SC0271_EVENT_THREAD, 0, 1,
...
sce_alter( i4 op, i4	value )
{
	...
	else
	{
		...
		{
		    sc0e_put(E_SC0272_EVENT_THREAD_ADD, &clerr, 0,
			     0, (PTR)0,
			     0, (PTR)0,
			     0, (PTR)0,
			     0, (PTR)0,
		...
		DB_ERROR	*error )
{
	...
	if ((reg->scse_flags & SCE_RORPHAN) == 0)
	{
	    sc0e_put(E_SC0287_AINST_SESSION_ID, 0, 1,
	...
	error->err_code = E_SC0287_AINST_SESSION_ID;
	return (status);
    }
    sscb = &scb->scb_sscb;
    ...
    */
	...
	if (status != E_DB_OK)
	{
	    ...
	    sc0e_put(E_SC0283_AINST_ALLOCATE_FAIL, 0, 2,
		     sizeof(reg->scse_session), (PTR)&reg->scse_session,
		     ...
		     0, (PTR)0,
		     0, (PTR)0);
	    error->err_code = E_SC0283_AINST_ALLOCATE_FAIL;
	    status = E_DB_ERROR;
	    break;
	}
