...
)
{
	...
	*/
	if((ops&SC0A_QT_NEWREC) && qtacb->qta_len>0)
	{
		...
		if(status!=E_DB_OK)
		{
			_VOID_ ule_format(err_code.err_code, &syserr, 
				ULE_LOG, NULL,
			    (char *)0, 0L, (i4 *)0, &local_error, 0);
			_VOID_ ule_format(E_SC023F_QRYTEXT_WRITE, NULL, ULE_LOG, NULL,
			    (char *)0, 0L, (i4 *)0, &local_error, 0);
		}
		/*
...
) 
{
	...
	if (status != E_DB_OK) 
	{
		_VOID_ ule_format(sxfrcb.sxf_error.err_code, &syserr, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &local_error, 0);
		_VOID_ ule_format(E_SC023E_SXF_BAD_WRITE, NULL, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &local_error, 0);
		/*
		**	Pass error back to caller
		*/
		STRUCT_ASSIGN_MACRO(sxfrcb.sxf_error, *err_code);
