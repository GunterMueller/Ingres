...
sc0m_add_pool(i4 flag)
{
    ...
    /* If a pool is being added, just return */
    if (Sc_main_cb->sc_sc0m.sc0m_status & SC0M_ADD_POOL)
	return(E_SC_OK);

    /* Lock sc0m_semaphore, check again */
	      ...
	      PTR *block )
{
    ...
    */
	     ...
	     pool = next_pool)
	{
	    ...
	    /*
	    ** Pool is not busy.
	    ** Release pool list sem, get pool sem and recheck
	    */
	    if ((flag & SC0M_NOSMPH_MASK) == 0)
	    {
		CSv_semaphore(&Sc_main_cb->sc_sc0m.sc0m_semaphore);
		...
		CSp_semaphore(SEM_SHARE, &Sc_main_cb->sc_sc0m.sc0m_semaphore);
	    }
	...
	if (current == (SC0M_OBJECT*)NULL)
	{
	    ...
	    if (busy_pools == 0)
	    {
		/* No pool with sufficient space; allocate another */
	       ...
	       PTR *addr )
{
    ...
    */
    if ( Sc_main_cb != NULL &&
	(ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_GUARD_PAGES, 0, 0)))
    {
	    in_pages += 2;
    }
...
sc0m_initialize(void)
{
    ...
    if( status != OK )
    {
	TRdisplay( "sc0m_initalize semaphore init error %x\n", status );
	return( E_DB_ERROR );
    }
