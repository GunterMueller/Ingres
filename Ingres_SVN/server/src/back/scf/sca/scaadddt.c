...
sca_trace( i4  dispose_mask, i4  length, char *message )
{
    ...
    {
	sc0e_put(E_SC0318_SCA_USER_TRACE, 0, 1, length, message);
    }
    return(E_DB_OK);
}

		  ...
		  i4  *new_size )
{
    ...
    *new_svcb = adf_svcb;   /* Start out with no change */
	...
	error->err_code = E_SC026A_SCA_REGISTER_ERROR;
	if( i)
	{
	...
	{
	    sc0e_put(E_SC0263_SCA_RISK_CONSISTENCY, 0, 0);
	}

	if (add_block->add_major_id <= 0)
	{
	    sc0e_put(E_SC0264_SCA_ILLEGAL_MAJOR, 0, 1,
	...
	if (status != OK)
	{
	    error->err_code = scf_cb.scf_error.err_code;
	    break;
	}
	...
	{
	    sc0e_put(E_SC0265_SCA_STATE, 0, 3,
				    sizeof(add_block->add_major_id),
				    &add_block->add_major_id,
	    ...
	    sc0e_put(E_SC024D_SCA_ADDING, 0, 4,
	...
	*new_svcb = new_adf_block;
	...
	if (old_adf_block && deallocate_flag)
	{
	    ...
	    if (int_status)
	    {
		sc0e_put(scf_cb.scf_error.err_code, 0, 0);
		sc0e_put(E_SC024C_SCA_DEALLOCATE, 0, 0);
	    }
	}
	old_size = cur_size;
    ...
    if (status && error->err_code)
    {
	sc0e_put(error->err_code, 0, 0);
	sc0e_put(E_SC024E_SCA_NOT_ADDED, 0, 0);
    }
    ...
    {
	sc0e_put(E_SC0266_SCA_USER_SHUTDOWN, 0, 0);
	status = E_DB_FATAL;
    }
    else
    {
...
sca_check(i4 major_id, i4 minor_id)
{
	...
	sc0e_put(E_SC0267_SCA_ID_UNKNOWN, 0, 0);
    ...
    {
    	if (Sc_main_cb->sc_capabilities & (SC_INGRES_SERVER | SC_STAR_SERVER))
	    sc0e_put(E_SC0268_SCA_RISK_INVOKED, 0, 0);
	status = 1;
    }
    return(status);
}
