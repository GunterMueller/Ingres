...
#define			SCS_MOVING_DATA 0x7	/*
						** moving data from
						** tupbuf to odarea 
						*/ 
#define			SCS_ENDQUERY	0x8
#define			SCS_SEND_DESC	0x9	/* send
						** descriptor longer than comm  
						** size	
						*/
#define			SCS_CP_GETSTAT	0xA	/* reading status block during
						** a copy 
						*/
#define			SCS_CP_ERROR	0xB	/* waiting for termination
						** status during a copy
						*/
#define			SCS_ENDRET	0xC	/* end retrieve operation */
#define			SCS_INTERRUPT	0xD	/* interrupt the query */
#define			SCS_ADDPROT	0xE	/* add sys update capability */
#define			SCS_DELPROT	0xF	/*
						** delete syscat update
						** capability 
						*/
#define			SCS_PROCESS	0x10	    /*
						    ** now run the query
						    ** This value is an error if
						    ** ever set outside the
						    ** sequencer 
						    */
#define                 SCS_ACCEPT      0x11	/* Accept connection and ask for
						** initiation parameters
						*/
#define                 SCS_SPECIAL      0x12	/* Non-user DBMS thread - run
						** specialized processing
						** depending on thread type
						*/
#define			SCS_DCONNECT	0x13	/* Direct Connect mode.
						** Distributed server only.
						** User is connected directly
						** to the LDB.
						*/
#define                 SCS_IERROR      0x14    /* Error found in */
						/* processing input */
						/* blocks. */

#define			SCS_SPASSWORD	0x15	/* Send password prompt */

#define			SCS_RPASSWORD	0x16	/* Recieve password reply */
#define			SCS_CTHREAD	0x17	/* Coordinating thread sent an
						** event */
/*
