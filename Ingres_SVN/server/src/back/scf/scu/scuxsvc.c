...
scu_xencode(SCF_CB *scf_cb, SCD_SCB *scb )
{
    STATUS	status;
    CI_KS	KS;
    char	inbuffer[DB_MAXNAME+1];
    char	outbuffer[DB_MAXNAME+1];
    char	keybuffer[DB_MAXNAME];
    u_i2	i2_size;
    i4		longnat_size;
    i4		nat_size;
    char	*char_ptr;
...
#define	PASSINIT    "hjodvwHOJHOJhodh498032&*&*#)$&*jpkshghjlg58925fjkdjkpg"
    ...
    {
	sc0e_0_put(E_SC0261_XENCODE_BAD_PARM, 0);
	scf_cb->scf_error.err_code = E_SC0261_XENCODE_BAD_PARM;
	return(E_DB_ERROR);
    }
    ...
    i2_size = nat_size;
    MEcopy((PTR)outbuffer, i2_size, (PTR)inbuffer);
    ...
    */
    ...
    longnat_size = ((sizeof(inbuffer) + 7) / 8) * 8;
	...
	sc0e_0_put(E_SC0262_XENCODE_BAD_RESULT, 0);
	scf_cb->scf_error.err_code = E_SC0262_XENCODE_BAD_RESULT;
	status = E_DB_ERROR;
    }
