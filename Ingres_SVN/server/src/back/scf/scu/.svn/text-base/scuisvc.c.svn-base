...
#else
static  READONLY SCI_INFO_TABLE Sci_info_table[] = {
#endif
    { 0, 0 },
...
# endif  /* LP64 */
    ...
    { SCI_DATE_FORMAT, 15 /* = strlen("multinational4")+1 */ },
    { SCI_MONEY_FORMAT, 10 },
    { SCI_MONEY_PREC, 4 },
...
scu_information(SCF_CB	*cb, SCD_SCB *scb )
{
    ...
    {
	cb->scf_error.err_code = E_SC000B_NO_REQUEST;
	return(E_DB_ERROR);
    }
	...
	status = E_SC0027_BUFFER_LENGTH;
	if (item->sci_length <= 0)
	    break;
	status = E_SC0009_BUFFER;
	if (!item->sci_aresult)
	    break;
	tm_needed = 0;
	status = E_SC_OK;
...
/*	    where = (PTR) &scb->scb_cscb.cscb_method;*/
	...
	case SCI_CPUTIME:
	    ...
	    */
	    ...
	    {
		ule_format(stat, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		    (i4)0, (i4 *)0, &error, 0);
		status = E_SC0007_INFORMATION;
		ret_val = E_DB_ERROR;
		break;
	    }
	...
	case SCI_SCB:
		...
		case DB_SCF_ID:
		    ...
		    break;
		case DB_ULF_ID:
		    where = (PTR) &scb->scb_sscb.sscb_ulscb;
		    break;
		case DB_GWF_ID:
		...
		case DB_SXF_ID:
		    ...
		    break;
		default:
		    status = E_SC0018_BAD_FACILITY;
		    cb->scf_error.err_data = index;
		    ret_val = E_DB_ERROR;
		    break;
	    }
	    break;
...
?
	...
	case SCI_AUDIT_LOG:
		...
		SXF_ACCESS access;
		    ...
		    if ((ret_val = (*Sc_main_cb->sc_sxf_cptr)
	...
	case SCI_AUDIT_STATE:
		...
		SXF_ACCESS access;
			...
			if(sxfrcb.sxf_auditstate==SXF_S_SUSPEND_AUDIT)
			    STmove("SUSPEND",ERx(' '),item->sci_length,where);
			else if(sxfrcb.sxf_auditstate==SXF_S_STOP_AUDIT)
			    STmove("STOP",ERx(' '),item->sci_length,where);
			else  if(Sc_main_cb->sc_capabilities & SC_C_C2SECURE)
			    STmove("ACTIVE",ERx(' '),item->sci_length,where);
			else
			    STmove("",ERx(' '),item->sci_length,where);
		    }
                    access = (SXF_A_SUCCESS | SXF_A_SELECT);
	        }
	        else
	...
	case SCI_DATE_FORMAT:
	    {
	    ADF_CB *adfcb = (ADF_CB*)scb->scb_sscb.sscb_adscb;
	    where = (PTR) char_maxbuf;
	...
	if (status == E_SC_OK)
	{
		...
		*item->sci_rlength = len;
	    ...
	    {
		cb->scf_error.err_data = index;
		cb->scf_error.err_code = E_SC0008_INFO_TRUNCATED;
		ret_val = E_DB_WARN;
	    }
	    else
		ret_val = E_DB_OK;
