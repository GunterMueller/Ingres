...
scs_initiate(SCD_SCB *scb )
{
    ...
    */
	...
	scb->scb_cscb.cscb_gcp.gca_it_parms.gca_buffer =
			    (PTR) scb->scb_cscb.cscb_inbuffer;
	...
	if (status)
	{
	    ...
	    sc0e_0_put(E_SC0123_SESSION_INITIATE, 0 );
	    error.err_code = E_SC0225_BAD_GCA_INFO;
	    return(E_SC0225_BAD_GCA_INFO);
	}
	error.err_code = E_DB_OK;
	if (scb->scb_cscb.cscb_gcp.gca_it_parms.gca_message_type == GCA_RELEASE)
	{
	...
	if (scb->scb_cscb.cscb_gcp.gca_it_parms.gca_message_type != GCA_MD_ASSOC)
	{
	...
	else if ((!scb->scb_cscb.cscb_gcp.gca_it_parms.gca_end_of_data)
		    || (scb->scb_sscb.sscb_tptr))
	{
		...
		status = sc0m_allocate(0,
		      ...
		      (2 * scb->scb_cscb.cscb_gcp.gca_it_parms.gca_d_length))),
					DB_SCF_ID,
					(PTR) SCS_MEM,
		...
		MECOPY_VAR_MACRO(scb->scb_cscb.cscb_gcp.gca_it_parms.gca_data_area,
			scb->scb_cscb.cscb_gcp.gca_it_parms.gca_d_length,
	    ...
	    if (!scb->scb_cscb.cscb_gcp.gca_it_parms.gca_end_of_data)
		return(E_DB_OK);
	}
	...
	*/
	...
	if (scb->scb_sscb.sscb_need_facilities & (1 << DB_ADF_ID))
	{
		...
		sc0e_uput(E_SC037B_BAD_DATE_CUTOFF, 0, 2,
				  ...
				  SystemVarPrefix,
				  0, 
				  (PTR)scb->scb_sscb.sscb_ics.ics_year_cutoff,
				  0, (PTR)0,
				  0, (PTR)0,
				  0, (PTR)0,
				  0, (PTR)0);		
	...
	if (scb->scb_sscb.sscb_need_facilities & (1 << DB_DMF_ID))
	{
	    ...
	    dmc->dmc_adf_cb = scb->scb_sscb.sscb_adscb;
	    /*
	...
	{
	    gw_rcb = (GW_RCB *) scb->scb_sscb.sscb_gwccb;
	...
	{
	    sxf_cb = (SXF_RCB *) scb->scb_sscb.sscb_sxccb;
	...
	if (scb->scb_sscb.sscb_need_facilities & (1 << DB_QEF_ID))
	{
	    ...
	    qef_cb->qef_adf_cb = (ADF_CB *) scb->scb_sscb.sscb_adscb;
	    qef_cb->qef_asize = 0;
	    qef_cb->qef_alt = 0;
		...
		*/
	    ...
	    status = qef_call(QEC_BEGIN_SESSION, ( PTR ) qef_cb);
	    if (status)
	    {
	...
	{
	    rdf_cb = (RDF_CCB *) scb->scb_sscb.sscb_rdccb;
	    ...
	    rdf_cb->rdf_adf_cb = scb->scb_sscb.sscb_adscb;
	    ...
	    */
		     ...
		     */
		{
			...
			else
			{
			    ...
			    status = qef_call(QED_DESCPTR, ( PTR ) qef_cb);
			    if (status)
			    {
				STRUCT_ASSIGN_MACRO(qef_cb->error, error);
				break;
	...
	*/
	    ...
	    */
		...
		if (!qef_call(QEC_END_SESSION, ( PTR ) qef_cb))
		    scb->scb_sscb.sscb_facility &= ~(1 << DB_QEF_ID);
	    }
	...
	*/
	...
	{
	    psq_cb = (PSQ_CB *) scb->scb_sscb.sscb_psccb;
	    ...
	    psq_cb->psq_adfcb = scb->scb_sscb.sscb_adscb;
	...
	{
	    opf_cb = (OPF_CB *) scb->scb_sscb.sscb_opccb;
	    ...
	    opf_cb->opf_adfcb = (ADF_CB *) scb->scb_sscb.sscb_adscb;
	...
	*/
	...
	if(!scb->scb_sscb.sscb_qeccb)
	    ...
	    status = qef_call(QET_RESUME, ( PTR ) qef_cb);
	    if (status)
	    {
		STRUCT_ASSIGN_MACRO(qef_cb->error, error);
...
scs_terminate(SCD_SCB *scb )
{
	...
	*/
	...
	status = qef_call(QEC_END_SESSION, ( PTR ) qef_cb);
	if (status)
	{
	    STRUCT_ASSIGN_MACRO(qef_cb->error, error);
	    sc0e_0_put(error.err_code, 0);
    ...
    */
    ...
    if ((Sc_main_cb->sc_avgrows > MAXI4)
	     ...
	     DB_ERROR	*error )
{
    ...
    */
    if ((dm_ccb = (DMC_CB *) scb->scb_sscb.sscb_dmccb) != (DMC_CB *)NULL)
    {
    ...
    */
    if ((qe_ccb = (QEF_RCB *) scb->scb_sscb.sscb_qeccb) != (QEF_RCB *)NULL)
    {
	qe_ccb->qef_asize = 1;
    ...
    */
    if ((gw_ccb = (GW_RCB *) scb->scb_sscb.sscb_gwccb) != (GW_RCB *)NULL)
    {
	gw_ccb = (GW_RCB *) scb->scb_sscb.sscb_gwccb;
               ...
               DB_ERROR *error )
{
    ...
    char			tempcmptlvl[DU_MAXDBCMPTLVL];
    SXF_ACCESS			access;
    DB_STATUS			local_status;
    i4			is_dbdb;
    SXF_RCB			*sxf_cb;
    ...
    */
	...
	for (;;)
	{
	    ...
	    */
		...
		status = qef_call(QEU_OPEN, ( PTR ) &qeu);
		if (status)
		{
		...
		status = qef_call(QEU_CLOSE, ( PTR ) &qeu);
		if (status)
		{
		    sc0m_deallocate(0, &tmptupmem);
		    break;
		...
		*/
		if (MEcmp(dmc->dmc_dbcmptlvl, DU_4DBV70, DU_MAXDBCMPTLVL) >= 0)
		{
	...
	if (status)
	{
	    ...
	    if (opened)
	    {
		(VOID) qef_call(QEU_CLOSE, ( PTR ) &qeu);
	    }
	    if (xact)
	    {
	...
	*/
    ...
    status = qef_call(QEC_ALTER, ( PTR ) &qef_cb);
    if (status)
    {
    ...
    */
    ...
    {
	MEmove(STlength(DU_CUR_DBCMPTLVL), (PTR)DU_CUR_DBCMPTLVL, ' ',
	       DU_MAXDBCMPTLVL, (PTR)tempcmptlvl);

	if (MEcmp((PTR)dbt.du_dbcmptlvl, (PTR)tempcmptlvl,
		  DU_MAXDBCMPTLVL) ||
