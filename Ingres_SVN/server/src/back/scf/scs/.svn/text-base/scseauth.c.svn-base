...
**
**	    scs_check_extern_password    - Check an external password
**
**  History:
**      15-mar-94 (robf)
**          Created.
...
**	    replace nat and longnat with i4
**	04-Oct-2000 (jenjo02)
**	    Removed CS_find_scb() prototype, now correctly 
**	    typed in cs.h.
**	21-feb-2001 (somsa01)
**	    Sync'ed up scrl_l_reserved and scrl_owner with the proper types.
*/
/*
...
)
{
	...
	*/
	...
	/*
	** Build the format parms
	*/
	fo_parms.gca_buffer = work_buff;
	fo_parms.gca_b_length = RQ_BUFF_SIZE;
	cl_stat = IIGCa_call(GCA_FORMAT,
	...
	{
		sc0e_0_put(status, 0);
		sc0e_0_put(E_SC0358_EXTPWD_GCA_FORMAT, 0);
		status = E_DB_ERROR;
		break;
	}
