...
scc_error( i4  operation, SCF_CB * scf_cb, SCD_SCB *scb )
{
    ...
    {
	scf_cb->scf_error.err_code = E_SC000C_BUFFER_LENGTH;
	ret_val = E_DB_ERROR;
    }
    else if ((scb->scb_sscb.sscb_need_facilities & (1 << DB_GCF_ID)) == 0)
    {
    ...
    {
	scf_cb->scf_error.err_code = E_SC000D_BUFFER_ADDR;
	ret_val = E_DB_ERROR;
    }
    else
    {
	...
	if (Sc_main_cb->sc_gchdr_size)	/* if not running standalone */
	{
	    ...
	    {
		sc0e_0_put(status, 0);
		scf_cb->scf_error.err_code = status;
		return(E_DB_ERROR);
	    }
	    ...
	    ebuf->scg_msize = sizeof(GCA_ER_DATA) - 
					...
					+ sizeof(i4)
		    + scf_cb->scf_len_union.scf_blength;
	    fparms.gca_buffer = ebuf->scg_marea = (PTR) 
		((char *) ebuf + sizeof(SCC_GCMSG));
	    fparms.gca_b_length = sizeof(GCA_ER_DATA)
		    + Sc_main_cb->sc_gchdr_size
		    + scf_cb->scf_len_union.scf_blength;
		...
		scf_cb->scf_error.err_code = ret_val;
		sc0m_deallocate(0, (PTR *)&ebuf);
#ifdef VMS
                if (ast_enabled) sys$setast(1);
#endif
		return(status);
	    }
            else if (fparms.gca_status != E_GC0000_OK)
	    {
		sc0e_0_put(fparms.gca_status, 0);
		scf_cb->scf_error.err_code = fparms.gca_status;
		sc0m_deallocate(0, (PTR *)&ebuf);
#ifdef VMS
                if (ast_enabled) sys$setast(1);
#endif
	    ...
	    emsg = (GCA_ER_DATA *) fparms.gca_data_area;
	...
	if (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_LOG_ERROR, 0, 0))
	{
	    char    *ops;
...
scc_trace( SCF_CB *scf_cb, SCD_SCB *scb )
{
	...
	if (Sc_main_cb->sc_gchdr_size  &&	/* if not running standalone */
	    ...
	    !ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_LOG_TRACE, 0, 0))
	{
	    status = sc0m_allocate(0,
			    (i4)(sizeof(SCC_GCMSG)
				+ scf_cb->scf_len_union.scf_blength
	    ...
	    fparms.gca_buffer = tbuf->scg_marea = (PTR) 
		((char *) tbuf + sizeof(SCC_GCMSG));
	    fparms.gca_b_length = Sc_main_cb->sc_gchdr_size
		    + scf_cb->scf_len_union.scf_blength;
	    status = IIGCa_call(GCA_FORMAT,
				(GCA_PARMLIST *)&fparms,
				GCA_SYNC_FLAG,
	    ...
	    (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_LOG_ERROR, 0, 0) ||
	      ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_LOG_TRACE, 0, 0)) )
	{
...
scc_send( SCD_SCB *scb, i4  sync )
{
    ...
    */
    ...
    {
	sc0e_0_put(last_error, &scb->scb_cscb.cscb_gco.gca_os_status);
        return(last_error);
    }
	...
	cmsg_next = cmsg->scg_next;
        ...
        if (cmsg_next == cmsg)
        {
            ...
            sc0e_0_put(E_SC023B_INCONSISTENT_MSG_QUEUE, 0);
    ...
    */
	...
	switch (cmsg->scg_mtype)
	{
		...
		*/
		sc0e_put(E_SC0255_INCOMPLETE_SEND, 0, 2,
...
scc_recv( SCD_SCB *scb, i4  sync )
{
    ...
    */
    if (scb->scb_sscb.sscb_astate == SCS_ASIGNORE)
    {
	sc0e_0_put(E_SC0275_IGNORE_ASTATE, 0);
    ...
    */
    ...
    else if (scb->scb_cscb.cscb_gci.gca_status != E_SC1000_PROCESSED)
    {
#ifdef VMS
        if (ast_enabled) sys$setast(1);
#endif
    ...
    scb->scb_cscb.cscb_gci.gca_association_id =	
	scb->scb_cscb.cscb_assoc_id;
    scb->scb_cscb.cscb_gci.gca_flow_type_indicator =
	GCA_NORMAL;
    scb->scb_cscb.cscb_gci.gca_buffer = 
	scb->scb_cscb.cscb_inbuffer;
    scb->scb_cscb.cscb_gci.gca_b_length = 
	    scb->scb_cscb.cscb_comm_size;
    scb->scb_cscb.cscb_gci.gca_descriptor = NULL; /* EJLFIX: Fix for copies */
    status = IIGCa_call(GCA_RECEIVE,
		    (GCA_PARMLIST *)&scb->scb_cscb.cscb_gci,
		    (sync ? GCA_SYNC_FLAG : 0),
		    (PTR)&scb->cs_scb,
		    -1,
		    &error);
...
scc_fa_notify( SCD_SCB *scb )
{
	...
	outbuf = (char *)(message->scg_marea) + Sc_main_cb->sc_gchdr_size;
...
scc_relay( SCF_CB *scf_cb, SCD_SCB *scb )
{
	...
	relay_msg = (GCA_IT_PARMS *)scf_cb->scf_ptr_union.scf_buffer;
	...
	*/
	...
	if (Sc_main_cb->sc_gchdr_size)	/* if not running standalone */
	{
	    ...
	    pbuf->scg_msize = sizeof(GCA_PROMPT_DATA) -
					...
					+ sizeof(i4)
				    + mesglen;
	    fparms.gca_buffer = pbuf->scg_marea = (PTR) 
		((char *) pbuf + sizeof(SCC_GCMSG));
	    fparms.gca_b_length = sizeof(GCA_PROMPT_DATA)
		    + Sc_main_cb->sc_gchdr_size
		    + mesglen;
	    status = IIGCa_call(GCA_FORMAT,
				(GCA_PARMLIST *)&fparms,
	    ...
	    pdata = (GCA_PROMPT_DATA *) fparms.gca_data_area;
