...
{				/* print the output for the states */
    ...
    SIfprintf(fileptr, "GLOBALDEF READONLY\tyytabelem %sexca[] ={\n", prefix);
    if (Filespecs[YYEXCA].filegiven)
    {
	SIfprintf(ftable, "extern WSCREADONLY yytabelem %sexca[];\n", prefix);
    }
        ...
        n;
{
    ...
    **	front.
    */
    SIfprintf(fileptr, "GLOBALDEF READONLY\tyytabelem %s%s[]={\n",
	prefix, Filespecs[s].optname + 2);
    if (Filespecs[s].filegiven)
    {
	SIfprintf(ftable, "extern WSCREADONLY yytabelem %s%s[];\n",
	    prefix, Filespecs[s].optname + 2);
    }
