	...
	PSS_SESBLK *sess_cb)
{
    ...
    */
	...
	/* count the number of parameters in the procparmlist,
	** and get the record width by finding the last param and
	** adding its length to the its offset in the record
	*/
	for (listptr = sess_cb->pss_procparmlist;
	     listptr->dt_next != (QEF_DATA *) NULL;
	     listptr = listptr->dt_next)
	    proctuple.db_parameterCount++;	/* count the parameters */

	proctuple.db_parameterCount++;		/* count the last parameter */
