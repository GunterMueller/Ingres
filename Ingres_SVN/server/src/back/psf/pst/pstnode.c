	...
	i4		    flags)
{
	...
	*/
	...
	if (~flags & PSS_NOALLOC)
	{
	    ...
	    */
		...
		(i4)ME_ALIGN_MACRO(sizeof(PST_QNODE) - sizeof(PST_SYMVALUE) +
		vallen, DB_ALIGN_SZ) + datalen,
		(PTR *) newnode, err_blk);
	}
	...
	*/
	...
	switch (type)
	{
		...
		symbol->pst_value.pst_s_op.pst_flags = 0; /* initialize
				    ** mask field */
	...
	*/
	...
	if (left && left->pst_sym.pst_dataval.db_collID >= 0)
	    symbol->pst_dataval.db_collID = left->pst_sym.pst_dataval.db_collID;
	else if (right && right->pst_sym.pst_dataval.db_collID >= 0)
	    symbol->pst_dataval.db_collID = right->pst_sym.pst_dataval.db_collID;
	else symbol->pst_dataval.db_collID = -1;
	if (datavalue != (DB_ANYTYPE *) NULL && datalen > 0)
	{
	    ...
	    */
	    symbol->pst_dataval.db_data = (data_val == NULL) ? 
	     ME_ALIGN_MACRO( ((char *) &symbol->pst_value) + vallen,DB_ALIGN_SZ)
				   : data_val;
	    MEcopy((char *) datavalue, datalen,
		(char *) symbol->pst_dataval.db_data);
	}
	else
	{
	   ...
	   )
	{
	    ...
	    symbol->pst_dataval.db_length = sizeof(bool);
	    symbol->pst_dataval.db_collID = -1;
