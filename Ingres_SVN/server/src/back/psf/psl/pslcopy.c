	...
	DB_ERROR	*err_blk)
{
	...
	*/
        ...
        if ( check_missing )
        {
            ...
            */
                 ...
                 cpdom_desc = cpdom_desc->cp_next)
            {
                ...
                if (STbcompare(cpdom_desc->cp_domname, DB_MAXNAME,
            ...
            if (!cpdom_desc)
            {
                ...
                */
                ...
                if ((att_entry->att_flags & DMT_F_NDEFAULT)
		    ...
		    ** Make sure that the value will be aligned.
				...
				(PTR *) &mcatt, err_blk);
                  ...
                  if ( def_node->pst_sym.pst_type == PST_COP)
                  {
                      ...
                      status = psl_proc_func(sess_cb, opid,(PST_QNODE *) NULL,
                               &def_node, err_blk);
	...
	{
	    /*
	    ** Expand external length if necessary
	    */
	    if( sqlda->gca_col_desc[i].gca_attdbv.db_length>
		att_entry->att_width)
	    {
		QEU_CPATTINFO *cpatt;
		...
		*/
		...
		cpattcur=cpatt;
	        ...
	        ext_offset+= sqlda->gca_col_desc[i].gca_attdbv.db_length-
				att_entry->att_width;
	    }	
	    sqlda->gca_tsize += sqlda->gca_col_desc[i].gca_attdbv.db_length;
	}
	...
	** into the native column value holder.
    ...
    if(qe_copy->qeu_att_info)
    {
	/*
	** Allocate a workspace tuple for QEF.
	*/
	status = psf_malloc(sess_cb, &sess_cb->pss_ostream, 
	     qe_copy->qeu_ext_length,		
	    (PTR *) &qe_copy->qeu_tmptuple, err_blk);
	if (status != E_DB_OK)
	    return(status);
    }
	...
	*/
	if (sqlda->gca_tsize < qe_copy->qeu_tup_length)
	{
	    (VOID) psf_error(E_PS0C05_BAD_ADF_STATUS,
		adf_scb->adf_errcb.ad_errcode, PSF_INTERR, &err_code,
		err_blk, 0);
	...
	DB_ERROR	*err_blk)
{
    ...
    */
	...
	if (att_entry->att_flags & DMT_F_HIDDEN)
	    continue;
	...
	if (dt_bits & (AD_NOEXPORT | AD_CONDEXPORT))
	{
		...
		return(status);
	     ...
	     if (att_entry->att_flags & DMT_F_PERIPHERAL)
	     {
		    cpdom_desc->cp_length = ADE_LEN_UNKNOWN;
	     }
	     else
