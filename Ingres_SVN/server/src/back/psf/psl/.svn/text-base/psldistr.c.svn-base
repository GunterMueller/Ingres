	...
	DB_ERROR	*err_blk)
{
    ...
    */
    ...
    status = psf_malloc(sess_cb, &sess_cb->pss_ostream, res_data_value->db_length,
		    &result, err_blk);
    if (status != E_DB_OK)
	return (status);
    ...
    */
	...
	case DB_CHR_TYPE:
	{
	    ...
	    */
	    ...
	    status = psf_malloc(sess_cb, &sess_cb->pss_ostream, res_len + DB_CNTSIZE,
				&result, err_blk);
	    if (status != E_DB_OK)
		return (status);
	...
	case DB_VCH_TYPE:
	{
	    ...
	    status = psf_malloc(sess_cb, &sess_cb->pss_ostream, origtxt_len,
	...
	case DB_INT_TYPE:
	{
	    ...
	    ** room left
	    */
	    status = psf_malloc(sess_cb, &sess_cb->pss_ostream, 12,
				(PTR *)&cconst.pst_origtxt, err_blk);
	    if (status != E_DB_OK)
		return (status);
