...
psl_partdef_partlist(PSS_SESBLK *sess_cb, PSQ_CB *psq_cb, PSS_YYVARS *yyvarsp)
{
    ...
    DMU_ATTR_ENTRY *dmu_att;		/* DMU style attribute descriptor */
    DMU_CB *dmucb;			/* DMF dmu instructions */
	...
	case PSQ_CREATE:
	    ...
	    */
	    dmu_att = *( (DMU_ATTR_ENTRY **) dmucb->dmu_attr_array.ptr_address + att_number - 1);
	...
	PSS_YYVARS *yyvarsp, char *pname, bool allow_ii)
{
    ...
    */
    if ( ! is_ii_name)
    {
	/* First trip thru, check current dimension, which may not have
	** dimension-nparts counted up yet.
	*/
	nnames = yyvarsp->names_cur;
	dim_ptr = &yyvarsp->part_def->dimension[yyvarsp->part_def->ndims - 1];
	for (;;) {
	...
	char *colname, i2 *col_typep)
{
    ...
    DMU_ATTR_ENTRY **attrs;		/* Pointer to DMU attr-pointer array */
    DMU_CB *dmucb;			/* DMF dmu instructions */
    i4 i;
    i4 ncols;
    PST_QNODE *qry_node;		/* Scan select query tree */
