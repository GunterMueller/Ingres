...
**                              DMU_ATTR_ENTRY if newtbl is FALSE)
...
{	
    ...
    */
    ...
    status = psf_malloc(sess_cb, &sess_cb->pss_ostream, 
			num_cols * sizeof(DMU_ATTR_ENTRY *), 
			(PTR *) &cons_att_array, err_blk);
    if (DB_FAILURE_MACRO(status))
	return (status);

    MEfill(num_cols * sizeof(DMU_ATTR_ENTRY *),
    ...
    **  DMT_ATT_ENTRYs instead of DMU_ATTR_ENTRY.  But that's OK because
...
psl_ver_ref_types(
		  ...
		  DB_ERROR       *err_blk)
{
    DMU_ATTR_ENTRY *cur_col;        /* used if newtbl == FALSE */
    ...
    */
	...
	if ((cur_col == (DMU_ATTR_ENTRY *) NULL) 
	...
	if ((newtbl) || (qmode == PSQ_ATBL_ADD_COLUMN))
	{
	    /* have new table and DMU_ATTR_ENTRY structure 
	     */
	...
	*/
	...
	if (newtbl || (qmode == PSQ_ATBL_ADD_COLUMN) )
	{
	    ...
	    */
	    ...
	    status = psl_p_telem(sess_cb, psq_cb,
			&cons->pss_check_cond->pss_tree, NULL, NULL, (i4)0);
	    if (DB_FAILURE_MACRO(status))
		return(status);
	...
	for (;;)  /* give us something to break out of */
	{
	    ...
	    */
		    ...
		    && (col_id != cons_col_id))
		{
		    ...
		    i4	    length;
		    DMU_ATTR_ENTRY  **attrs =
			(DMU_ATTR_ENTRY **) dmu_cb->dmu_attr_array.ptr_address;
