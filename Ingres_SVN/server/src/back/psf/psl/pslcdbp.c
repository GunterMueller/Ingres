		     ...
		     DB_ERROR	*err_blk)
{
    ...
    status = psf_malloc(sess_cb, &sess_cb->pss_tstream, 
			num_params * sizeof(DB_PROCEDURE_PARAMETER), 
			(PTR *) &pptuple_arr, err_blk);
    if (DB_FAILURE_MACRO(status))
	return (status);
    
    MEfill(num_params * sizeof(DB_PROCEDURE_PARAMETER), 
	   (u_char) 0, (PTR) pptuple_arr);

    /* allocate space for the QEF_DATA pointers
     */
    status = psf_malloc(sess_cb, &sess_cb->pss_tstream, 
			num_params * sizeof(QEF_DATA), 
			(PTR *) &qef_data_arr, err_blk);
    if (DB_FAILURE_MACRO(status))
	return (status);
    
    MEfill(num_params * sizeof(QEF_DATA), (u_char) 0, (PTR) qef_data_arr);
	...
	** before linking to the 'next' pointer
	*/
	if (i == num_params)
	    qef_dptr->dt_next = (QEF_DATA *) NULL;
	else
	       ...
	       DB_ERROR		*err_blk)
{
    ...
    */
	...
	att_array[i].att_flags  = parm->pss_default ? 0 : DMT_F_NDEFAULT;
