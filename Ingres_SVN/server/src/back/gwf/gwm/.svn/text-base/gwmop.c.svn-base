...
static void GM_str_to_uns(char *a, u_i4 *u);
	      ...
	      i4  *tup_used )
{
	...
	case DB_INT_TYPE:	/* 30 'i' */
	    if ( (cl_stat = CVal( src, &ival )) != OK )
	    {
		if( cl_stat == CV_OVERFLOW )
		{
		    GM_str_to_uns( src, (u_i4 *)&ival );
		}
		else
		{
	    ...
	    if( dmt_att->att_width >= 4 )
	    {
		val_i4 = ival;
		lsrc = (PTR) &val_i4;
		lsrclen = sizeof(val_i4);
	    }
	    else if( dmt_att->att_width >= 2 )
	    {
		if( ival > MAXI2 )
		    ival = MAXI2;
		else if( ival < MINI2 )
