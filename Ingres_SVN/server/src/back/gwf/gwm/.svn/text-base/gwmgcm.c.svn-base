...
GM_prep_conn( GM_CONN_BLK *conn, STATUS *cl_stat )
{
    ...
    *cl_stat = OK;
	 ...
	 ** Invoke GCA_FORMAT
	 */
	fo_parms.gca_buffer = conn->conn_buffer;
	fo_parms.gca_b_length = conn->conn_buffer_size;
	...
	*cl_stat = IIGCa_call( GCA_FORMAT,
	...
	if ( *cl_stat != OK )
	{
	    /* "GWM Internal error:  Error from GCA format" */
	    GM_error( E_GW8041_GCA_FORMAT );
	    GM_error( *cl_stat );
	    db_stat = E_DB_ERROR;
	}
	else
	{
		...
		conn->conn_data_last = fo_parms.gca_data_area;
	
	    conn->conn_data_length = fo_parms.gca_d_length;
	}
    } 
