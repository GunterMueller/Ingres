...
)
{
    ...
    II_VMS_ITEM_LIST_3	access_itmlst[2];	/* $access_check itemlist */
    i4		    access_type;	/* read or write access desired */
    i4		    access_retlen;      /* $access_check returned length */
    char            *partial_name;
    i4	            is_remote = 0;
	...
	*/
	...
	len = STzapblank((char *)fnam_buf, (char *)fnam_buf);
	    ...
	    */
            ...
            access_itmlst[0].buflen = 4;
            access_itmlst[0].itmcod   = CHP$_ACCESS;
            access_itmlst[0].bufadr = &access_type;
            access_itmlst[0].reslen = &access_retlen;
            access_itmlst[1].buflen = 0;
            access_itmlst[1].itmcod = 0;
            access_itmlst[1].bufadr = 0;
            access_itmlst[1].reslen = 0;
...
)
{
    ...
    i4		    ulecode;
    i4		    msglen;
    FUNC_EXTERN DB_STATUS   ule_doformat();
	...
	case E_GW5000_RMS_OPEN_ERROR:
		...
		default:
		    ...
		    */
		    uleret = ule_doformat(error_code, (CL_ERR_DESC *)0, uletype, 
    ...
    {
	/*
	** An error occurred in ule_doformat. At least try to report that
	** error...
	*/
	_VOID_ ule_doformat( ulecode, (CL_ERR_DESC *)0, ULE_LOG, 
			    (DB_SQLSTATE *) NULL, (char *)0, (i4)0,
			    &msglen, &ulecode, 0 );
    }
...
**	    Created.
**	29-oct-92 (andre)
**	    change 4-th arg to ule_doformat() to (DB_SQLSTATE *) NULL since we
**	    never use the value anyway
*/
static VOID
...
)
{
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    ...
	    _VOID_ ule_doformat( scf_cb.scf_error.err_code, (CL_ERR_DESC *)0,
	...
	*/
	_VOID_ ule_doformat( status, (CL_ERR_DESC *)0, ULE_LOG,
			    (DB_SQLSTATE *)NULL, (char *)0, (i4)0,
			    (i4 *)0, &local_error, 0);
    }
...
**	ule_doformat, then send it to the frontend using scc_error().
**
** Inputs:
	...
	if (inglkey)
	{
		...
		*/
		...
		if (status != E_DB_OK)
		{
		    ...
		    */
		    _VOID_ ule_doformat(E_GW5011_CVT_KEY_TO_RMS_ERROR,
