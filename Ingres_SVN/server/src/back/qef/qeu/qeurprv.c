...
char		*rolename)
{
	...
	qeu.qeu_klen 	= 0;       
	qeu.qeu_key 	= NULL;
	qeu.qeu_qual 	= (PTR)qeu_qrolegrant;
	qeu.qeu_qarg 	= (PTR)qual_parms;
	qef_data.dt_next = 0;
	...
	qual_parms[0]	= (PTR)qef_cb;
	qual_parms[1]	= (PTR)grantee;
	qual_parms[2]	= (PTR)&audit;
	qual_parms[3]   = (PTR)rolename;
...
**	    rolename		    rolename
**	rolegrant_tuple		The current tuple being considered
**
** Outputs:
**	None.
**
...
DB_ROLEGRANT	*rgr_tuple)
{
    ...
    if ( qual_parms == NULL || rgr_tuple == NULL)	/* Bad parm; abort! */

	return(DMR_F_ABORT);

    qef_cb	= (QEF_CB *)qual_parms[0];
    grantee	= (char *)qual_parms[1];
    audit	= (i4 *)qual_parms[2];
    rolename	= (char *)qual_parms[3];
