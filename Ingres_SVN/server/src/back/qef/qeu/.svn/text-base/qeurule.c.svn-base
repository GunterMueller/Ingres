...
**
**  Locals:
**	qeu_qrule_by_name  	- Qualify rule by name.
**
**  History:
** 	21-mar-89 (neil)
...
**	Note that we cannot use the "key" fields based on the rule and owner
**	name as DMF requires that the "key" fields are really keys and iirule
**	has the table id as the key.  In this case we use the qualification
**	function (qeu_qrule_by_name) which will be called indirectly.
**
...
bool		from_drop_rule)
{
	...
	** Positioning is done via qeu_qrule_by_name.  If DROP TABLE then fetch
...

/*{
** Name: qeu_qrule_by_name - Qualify a rule by rule name and owner.
**
** External call:     status = (*qeu_qual)(qeu_qarg, &current_tuple);
** Internal QEF call: status = qeu_qrule_by_name(&search_tuple, &current_tuple);
**
** Description:
**	This routine is provided to allow iirule to be accessed by name.
**	For extraction performance reasons iirule is hashed on table ids 
**	but various operations require that we look up the rule by name
**	(rule deletion and insertion).  This routine is called indirectly
**	from within DMF and passed the original rule tuple we are searching
**	for and the current (next) tuple.
**
** Inputs:
**	search_tup		Name-qualified tuple we're looking for:
**		.dbr_name	Rule name.
**		.dbr_owner	Rule owner.
**	cur_tup			Current tuple on page.
**	
** Outputs:
**	Returns:
**	    DMR_F_RETURN	This is our tuple - return it for processing.
**	    DMR_F_NEXT		This tuple is not the one we're looking for.
**	    DMR_F_ABORT		Consistency check - NULL arguments.  This
**				will cause the caller (DMF) to abort the
**				current scan.
**	Exceptions:
**	    none
**
...
DB_IIRULE	*cur_tup)
{
    ...
    if (   (MEcmp((PTR)&search_tup->dbr_name, (PTR)&cur_tup->dbr_name,
		  sizeof(DB_OWN_NAME)) == 0)
        && (MEcmp((PTR)&search_tup->dbr_owner, (PTR)&cur_tup->dbr_owner,
		  sizeof(DB_NAME)) == 0)
       )
    {
	return (DMR_F_RETURN);			/* It's ours */
    }
    else
    {
	return (DMR_F_NEXT);			/* It's not */
    }
} /* qeu_qrule_by_name */
