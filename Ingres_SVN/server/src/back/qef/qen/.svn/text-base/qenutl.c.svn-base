...
i4		function )
{
    ...
    * get the first tuple. */
    dmrcb = hold->hold_dmr_cb;
    dmrcb->dmr_flags_mask = 0;
    status = qen_sort(sortnode, rcb, dsh, dmrcb, function );
    if (status) return(status);
...
QEE_DSH    *dsh )
{
    ...
    if (status = dmf_call(DMT_OPEN, dmtcb))
    {
	dsh->dsh_error.err_code = dmtcb->error.err_code;
	return(status);
    }
...
)
{
	...
	if ( !( tempTable->tt_statusFlags & TT_CREATED ) )
	{
	    ...
	    */
	    ...
	    /* move the control block into the list of open tables in the
...
)
{
	    ...
	    (dmt->dmt_record_access_id != NULL))
	{
	    ...
	    /* remove the control block from the list of open tables in the
	    ** data segment header (DSH)
	    */

	    if (dmt->q_prev == NULL )
	        dsh->dsh_open_dmtcbs = (DMT_CB *)dmt->q_next;
	    else
	        ((DMT_CB *)dmt->q_prev)->q_next = dmt->q_next;
	    if (dmt->q_next != NULL)
		((DMT_CB *)dmt->q_next)->q_prev = dmt->q_prev;
