	...
	*/
	...
	for ( ; ; )	/* loop 'til no more fit */
	{
	    ...
	    */
	    ...
	    status = qen_hash_close(hbase, 
		(reverse) ? &hpptr->hsp_pfile : &hpptr->hsp_bfile, CLOSE_LEAVE);
	    if (status != E_DB_OK) 
			    return(status);
...

{
    ...
    while (hfptr = hbase->hsh_openfiles)
    {
	DB_STATUS lcl_status;
	lcl_status = qen_hash_close(hbase, &hfptr, CLOSE_DESTROY);
