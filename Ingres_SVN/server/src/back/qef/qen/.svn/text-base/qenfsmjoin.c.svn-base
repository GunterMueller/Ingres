...
i4		function )
{
    ...
    ** of the query tree do we continue executing the function. */
	...
	if (in_node->qen_type == QE_SORT)
	    status = qen_sort(in_node, qef_rcb, dsh, NULL, MID_OPEN);
			/* so much for the myth of encapsulation */
	else status = (*in_node->qen_func)(in_node, qef_rcb, dsh, MID_OPEN);
	qen_status->node_status_flags |= QEN1_NODE_OPEN;
	...
	if (!(qen_status->node_status_flags & QEN8_NODE_CLOSED))
	{
	    ...
	    */
	    ...
	    if (in_node->qen_type == QE_SORT)
		status = qen_sort(in_node, qef_rcb, dsh, NULL, FUNC_CLOSE);
	    else status = (*in_node->qen_func)(in_node, qef_rcb, dsh, FUNC_CLOSE);
			...
			if (node->qen_flags & QEN_PART_SEPARATE)
			{
			    if (in_node->qen_type == QE_SORT)
			        status = (*in_node->qen_func)(in_node, qef_rcb, 
						dsh, NULL, FUNC_EOGROUP);
			    else status = (*in_node->qen_func)(in_node, qef_rcb, 
						dsh, FUNC_EOGROUP);
