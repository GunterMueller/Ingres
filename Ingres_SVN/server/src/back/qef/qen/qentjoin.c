...
i4		    function)
{
    ...
    ** of the query tree do we continue executing the function. */
    ...
    if (ojoin != NULL)
    {
	/* Only left joins supported.  A right t-join would require something
	** equivalent to a bitmap with potentially one bit for every row
	** in the inner table.
 	*/
    ...
    */
	...
	qen_status->node_status = QEN1_EXECUTED;  /* show done */
    }
