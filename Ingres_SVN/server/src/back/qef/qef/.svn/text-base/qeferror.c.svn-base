...

VOID
qef_error(int errorno, int  detail,
          int err_type, int *err_code,
          DB_ERROR *err_blk, int num_parms,
          ...)
{
    QEF_ERR_MAP		*err_map;
    i4             uletype;
    ...
    for( i = 0;  i < num_parms ; i++ )  
      {
        ...
        param_value[i] = (int *) va_arg( ap, int * );
      }
    va_end( ap );
	...
	*/
	...
	qef_cb = (QEF_CB*)NULL;
    	...
    	if (errorno == E_QE0015_NO_MORE_ROWS
	    || errorno == E_QE000B_ADF_INTERNAL_FAILURE
	    || errorno == E_QE0025_USER_ERROR)
	{
	    *err_code = 0L;
	...
	** with procname and line number, if so. */
	...
	if (errorno == 0L)
	    errorno = E_QE0018_BAD_PARAM_IN_CB;

	/* Map all errors to QEF and user's errors */

	err_map = (QEF_ERR_MAP *)&Qef_err_map[0];
	if ((errorno >> 16) == 0)		/* if user error code */
	{
	    user_code = errorno;		/* set user code directly */
	    qef_code = E_QE0025_USER_ERROR;
	    uletype = 0L;			/* do not log this error */
	}
	
	/* find user error code if a QEF error */
	else if ((errorno >> 16) == DB_QEF_ID )	/* if QEF error code */
	{
	    qef_code = errorno;			/* set QEF code directly */
	    while (err_map->qef_code != 0L && err_map->qef_code != errorno)
		++err_map;			/* search user code */

	    if (err_map->qef_code == errorno)	/* if found */
	    {
		if (err_map->user_code == -1L)	/* set user's code */
		    user_code = qef_code;
		else
		    user_code = (i4)(err_map->user_code); 
	    ...
	    {
		user_code = 0L;			/* no user message */
		uletype = ULE_LOG;		/* must log message */
	    }
	}
	
	else if ((errorno >> 16) > DB_MAX_FACILITY)  /* if bad error code */
	{
	    qef_code = E_QE0018_BAD_PARAM_IN_CB;
	    user_code = 0L;
	}
	else					/* error from other facility */
	{
	    while (err_map->qef_code != 0L && err_map->err_no != errorno)
		++err_map;			/* search for QEF code */

	    if (err_map->err_no == errorno)	/* if found */
	    {
		if (err_map->qef_code == -1L)	/* set qef_code */
		    qef_code = errorno;
		else
		    qef_code = err_map->qef_code;

		if (err_map->user_code == -1L)	/* set user's code */
		{
		    if (qef_code != E_QE0122_ALREADY_REPORTED)
			user_code = qef_code;
		}
		else
		{
	    ...
	    {
		/* unexpected error */              
		qef_code = E_QE009C_UNKNOWN_ERROR;
		user_code = 0L;	    /* no user message */
	...
	** the setting of err_eflag */
	...
	else
	{
	    ...
	    dsh = (QEE_DSH *)NULL;
	    qef_code = E_QE0019_NON_INTERNAL_FAILURE;
...
/* added error message from INGRES 6.4 -- bug #62708 (harpa06) */
		...
		*/
		user_code = E_QE0002_INTERNAL_ERROR;
		break;

	    case E_QE0025_USER_ERROR:
		if ( user_code == E_US14E4_5348_NON_FAST_COPY )
		{
		    /*
	...
	*/
	if (uletype == ULE_LOG)
	{
	    ...
	    */
	    if (errorno != qef_code)
	    {
		status = ule_format(errorno, 0L, uletype, (DB_SQLSTATE *) NULL,
		    errbuf, (i4) sizeof(errbuf), &msglen, &ulecode,
		    num_parms, param_size[0], param_value[0], 
	    ...
	    status = ule_format(qef_code, 0L, uletype, &sqlstate, errbuf,
	...
	** set iierrornumber variables */
	...
	if (user_code)
	{
		...
		if (qp->qp_oerrorno_offset != -1)
		{
		    ...
		    *iierrorno = (i4) user_code;
		}
	    }

	    /* get the message text */
	    status = ule_doformat(user_code, 0L, 0L, &sqlstate, errbuf,
		    (i4) sizeof(errbuf), &msglen, &ulecode, num_parms, 
                               param_size[0], param_value[0],
                               param_size[1], param_value[1],
                               param_size[2], param_value[2],
	    ...
	    scf_cb.scf_nbr_union.scf_local_error = user_code;
	...
	*err_code = 0L;
    ...
    /* 
    ** Before returning, save the originating errorno and resultant 
    ...
    */
    if ( qef_cb )
    {
	qef_cb->qef_error_errorno = errorno;
	qef_cb->qef_error_qef_code = qef_code;
    }
	...
	DB_ERROR	   *errptr)
{
	...
	{
	    errptr->err_code = E_AD2005_BAD_DTLEN;
	    if (adf_errcb->ad_errcode != E_AD2005_BAD_DTLEN)
		errptr->err_code = E_QE0023_INVALID_QUERY;
	...
	{
	    errptr->err_code = E_QE0022_QUERY_ABORTED;
	    ret_status = E_DB_ERROR;
	}
	else
	{
	    ...
	    errptr->err_code = E_QE000B_ADF_INTERNAL_FAILURE;
	    ret_status = E_DB_ERROR;
	}
    }
    else
	...
	errptr->err_code = E_QE0025_USER_ERROR;
    }
...

{
    ...
    ** the user in finding the problem. */
	...
	status = ule_format(E_QE00C0_PROC_STMTNO, 0L, ULE_LOG,
