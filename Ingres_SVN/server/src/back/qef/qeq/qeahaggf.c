...

{
	...
	** chunk linkage leaves chunks aligned on 8 byte boundaries. */
    ...
    /* Init. hash partition table entries */ 
    for (i = 0; i < haptr->hshag_pcount; i++)
    {
	QEN_HASH_PART	*hpptr = &haptr->hshag_currlink->hsl_partition[i];

	hpptr->hsp_bfile = (QEN_HASH_FILE *) NULL;
	hpptr->hsp_offset = 0;
	hpptr->hsp_brcount = 0;
	hpptr->hsp_currbn = 0;
	hpptr->hsp_reclevel = 0;
	hpptr->hsp_flags = 0;
    }
...

{
    ...
    *(hstp->hbufpp) = (PTR)hstp->hashbufp;
	...
	TRdisplay("%@ hashagg thread %d LOAD1 loaded %d rows, %d chunks\n",
		dsh->dsh_threadno, haptr->hshag_rcount, hstp->chunk_count+1);
	for (i=0; i<haptr->hshag_pcount; i++)
	{
