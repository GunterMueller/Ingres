...
QEF_RCB		    *qef_rcb )
{
    ...
    if (status = qeq_dsh(qef_rcb, -1, &dsh, (bool)FALSE, (i4)-1))
	return (status);
...
i4		mode )
{
	...
	*/
	...
	{
	    /* Search for the DSH of the specified QP */
	    status = qeq_dsh(qef_rcb, 0, &dsh, (bool)FALSE, (i4) -1);
	    if (DB_FAILURE_MACRO(status))
	    {
	...
	*/
	...
	if (!(act_state == DSH_CT_INITIAL && status))
	{
	    ...
	    case QEA_LOAD:
		status = qea_load(act, qef_rcb, dsh,
			(act_reset) ? FUNC_RESET : NO_FUNC);
		break;
		...
		if (status == E_DB_OK)
		{
		    ...
		    /* We have successfully called a new QP.		    */
                    ...
                    if (act->qhd_obj.qhd_callproc.ahd_procparams != NULL
		      && act->qhd_obj.qhd_callproc.ahd_procparams->qen_mask &
                                                              QEN_CONSTRAINT)
    ...
    ** table) get caught in OPF (opc). Only rules have to be checked here. */
	...
	do
	{
	    ...
	    */
	    if (in_progress)
		break;
	    if (qef_cb->qef_open_count > 0 && qef_cb->qef_stat != QEF_MSTRAN)
	    {
...
PTR	control )
{
	...
	node_ncost_calls = qen_status->node_nbcost;
	qeq_nccalls(qef_cb, node, &below_ncost_calls);
	collect_cost_cpu = 
	    ((node_ncost_calls + (below_ncost_calls * 2)) 
		* dsh->dsh_cpuoverhead);
...
/* FIXME */
	...
	{
	    TRformat(uld_tr_callback, (i4 *)0,
		trbuf, sizeof(trbuf), "at%8d",
		qen_status->node_rcount);
	    uld_prnode(control, trbuf);
	    TRformat(uld_tr_callback, (i4 *)0,
		trbuf, sizeof(trbuf), "et%8d", 
		node->qen_est_tuples);
	    uld_prnode(control, trbuf);
	    if ( qen_status->node_pcount > 0 )
	    {
		TRformat(uld_tr_callback, (i4 *)0,
		    trbuf, sizeof(trbuf), "ap%8d",
	...
	uld_prnode(control, "          ");
	TRformat(uld_tr_callback, (i4 *)0,
		trbuf, sizeof(trbuf), "ad%8d", 
		(i4) (qen_status->node_dio));
	uld_prnode(control, trbuf);
	TRformat(uld_tr_callback, (i4 *)0,
		trbuf, sizeof(trbuf), "ed%8d",
		node->qen_dio_estimate);
	uld_prnode(control, trbuf);

	uld_prnode(control, "          ");
	TRformat(uld_tr_callback, (i4 *)0,
		trbuf, sizeof(trbuf), "ac%8d",
		(i4) (qen_status->node_cpu - collect_cost_cpu));
	uld_prnode(control, trbuf);
	TRformat(uld_tr_callback, (i4 *)0,
		trbuf, sizeof(trbuf), "ec%8d",
		node->qen_cpu_estimate);
	uld_prnode(control, trbuf);
	uld_prnode(control, "          ");
	TRformat(uld_tr_callback, (i4 *)0,
		trbuf, sizeof(trbuf), "et%8d",
	    (i4) qen_status->node_wc);
	uld_prnode(control, trbuf);
    }
    uld_prnode(control, "----------");
}
...
i4	    *ncost_calls )
{
    ...
    *ncost_calls = 0;
    ...
    {
     case QE_TJOIN:
	qeq_nccalls(qef_cb, node->node_qen.qen_tjoin.tjoin_out, &outer_calls);
	break;

     case QE_KJOIN:
	qeq_nccalls(qef_cb, node->node_qen.qen_kjoin.kjoin_out, &outer_calls);
	break;

     case QE_HJOIN:
	qeq_nccalls(qef_cb, node->node_qen.qen_hjoin.hjn_out, &outer_calls);
	qeq_nccalls(qef_cb, node->node_qen.qen_hjoin.hjn_inner, &inner_calls);
	break;
	...
	break;
     ...
     case QE_EXCHANGE:
	qeq_nccalls(qef_cb, node->node_qen.qen_exch.exch_out, &outer_calls);
	break;

     case QE_ISJOIN:
     case QE_FSMJOIN:
     case QE_CPJOIN:
	qeq_nccalls(qef_cb, node->node_qen.qen_sjoin.sjn_out, &outer_calls);
	qeq_nccalls(qef_cb, node->node_qen.qen_sjoin.sjn_inner, &inner_calls);
	break;

     case QE_SEJOIN:
	qeq_nccalls(qef_cb, node->node_qen.qen_sejoin.sejn_out, &outer_calls);
	qeq_nccalls(qef_cb, node->node_qen.qen_sejoin.sejn_inner, &inner_calls);
	break;

     case QE_SORT:
	qeq_nccalls(qef_cb, node->node_qen.qen_sort.sort_out, &outer_calls);
	break;

     case QE_TSORT:
	qeq_nccalls(qef_cb, node->node_qen.qen_tsort.tsort_out, &outer_calls);
	break;
	...
	{
	    qeq_nccalls(qef_cb, ahd->qhd_obj.qhd_qep.ahd_qep, &ahd_calls);
    ...
    if (ncost_calls != NULL && qen_status != NULL)
    {
...
bool	    release )
{
	...
	if (DB_SUCCESS_MACRO(c_status))
	{
		...
		{
		    status = qen_bcost_begin(dsh, &tstat, NULL);
		    if (DB_FAILURE_MACRO(status))
		    {
			return(status);
		    }
		...
		if (ult_check_macro(&qef_cb->qef_trace, 90, &val1, &val2))
		{
		    ...
		    ** form, so we use uld_prtree (i4urally). Also, we do this
