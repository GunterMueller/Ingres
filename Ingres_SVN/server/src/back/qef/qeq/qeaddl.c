...
( DMT_ATT_ENTRY ***) &(dsh->dsh_row[ details->qci_referredTableAttributeArray]);
    ...
    for ( ; ; )		/* code block to break out of */
    {
	    ...
	    ** where we can modify it */
	    ...
	    if ( ( integrityTuple.dbi_consschema.db_tab_base == ( u_i4 ) NULL )
	      && ( integrityTuple.dbi_consschema.db_tab_index == ( u_i4 ) NULL )
	       )
	    {
	...
	QEF_RCB             *qef_rcb)
{
	...
	*/
	...
	intqeu.qeu_mask = 0;
	intqeu.qeu_qual = intqeu.qeu_f_qual = 0;
	intqeu.qeu_qarg = intqeu.qeu_f_qarg  = 0;
	...
	*/
	    ...
	    && ~drop_integ->ahd_flags & AHD_DROP_CONS_CASCADE)
	{
	    ...
	    */
	    dep_qeu->qeu_qual = ( PTR ) qea_dep_cons;
	    dep_qeu->qeu_qarg = NULL;
...
{
    DB_STATUS		status = E_DB_OK;
    DMT_CB		*dmt_cb, **head_dmtcb, *next_dmtcb, *previous_dmtcb;
    i4		err;

    head_dmtcb = &dsh->dsh_open_dmtcbs;
    for (dmt_cb = *head_dmtcb; dmt_cb; 
	...
	/* unchain this control block from the list of open tables */

	next_dmtcb = ( DMT_CB * ) dmt_cb->q_next;
	previous_dmtcb = ( DMT_CB * ) dmt_cb->q_prev;

	if ( *head_dmtcb == dmt_cb )
	{   *head_dmtcb = next_dmtcb; }

	if ( previous_dmtcb != ( DMT_CB * ) NULL )
	{   previous_dmtcb->q_next = ( PTR ) next_dmtcb; }

	if ( next_dmtcb != ( DMT_CB * ) NULL )
	{   next_dmtcb->q_prev = ( PTR ) previous_dmtcb; }
...
**	dependent on it - this function will spare us the expence of looping
**	through IIDBDEPENDS tuples in qea_d_integ()
**
** Input:
...
**
** Returns:
**	DMR_F_RETURN if this tuple describes dependence of a constraint on some
**	other object; DMR_F_NEXT otherwise
**
** History:
**	16-mar-93 (andre)
**	    written
...
qea_dep_cons(
	...
	DB_IIDBDEPENDS	*dep_tuple)
{
    return((dep_tuple->dbv_dtype == DB_CONS) ? DMR_F_RETURN : DMR_F_NEXT);
}

/*
