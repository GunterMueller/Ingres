...
QEE_DSH		*dsh )
{
	...
	*/
	...
	STRUCT_ASSIGN_MACRO(*(DMU_CB*)qea_act->qhd_obj.qhd_dmu.ahd_cb, dmu_tcb);
	    ...
	    ** this DBU call.
	    ** *FIXME* I don't think this will work in a || query,
	    ** fortunately it's unlikely that opc will compile a modify in
	    ** a parallel query.  Destroy/index/relocate are bogus, currently
	    ** never happens embedded in a query.
	    */
		...
		{
		    if ((status = dmf_call(DMT_CLOSE, dmt_cb)) != E_DB_OK)
		    {
			qef_error(dmt_cb->error.err_code, 0L, status,
			    &error, &dsh->dsh_error, 0);
			return (status);
		    }
		    /* Clear dmt_record_access_id */
		    dmt_cb->dmt_record_access_id = (PTR)NULL;
		    /* remove dmt_cb from queue of open tables */
		    if (dmt_cb->q_prev != (PTR)NULL)
