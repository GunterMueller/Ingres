...
dmu_pindex(DMU_CB    *dmu_cbs)
{
    ...
    i4			numberOfIndices, k, tot_size;
    DB_STATUS		status;
    DB_ERROR		*errcb_p;


    /* Count how many control block was passed */
    for ( numberOfIndices = 1, dmu = dmu_cbs; dmu->q_next; 
	  dmu = ( DMU_CB * ) dmu->q_next)
	numberOfIndices++;
    if (numberOfIndices == 1)
    {
	status = dmu_index(dmu_cbs);
	return(status);
    }

    tot_size = sizeof(DM2U_INDEX_CB) * numberOfIndices; 
    status = dm0m_allocate(tot_size, 0, 
			   (i4)DM2U_IND_CB, (i4)DM2U_IND_ASCII_ID, 
			   (char *)dmu_cbs, (DM_OBJECT **)&indx_cbs, &error);
    if (status != E_DB_OK)
    {
	ule_format(error, 0, ULE_LOG, NULL, 0, 0, 0, &local_error, 0);
	dmu->error.err_code = local_error;
	return(E_DB_ERROR);
    }

    error = 0;

    for (k = 0, indx_cb = indx_cbs, dmu = dmu_cbs; 
	 k < numberOfIndices; 
	 k++, indx_cb++, dmu = ( DMU_CB * ) dmu->q_next)
    {
	indx_cb->indxcb_tab_name = &table_name;
	indx_cb->indxcb_tab_owner = &table_owner;

	if ( status = dmuIndexSetup(dmu, (PTR)indx_cb) )
	{
    ...
    for (k = 0, indx_cb = indx_cbs, dmu = dmu_cbs; k < numberOfIndices; 
	  k++, indx_cb++, dmu = ( DMU_CB * ) dmu->q_next)
    {
