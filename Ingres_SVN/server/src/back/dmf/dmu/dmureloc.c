...
dmu_relocate(DMU_CB    *dmu_cb)
{
	...
	*/
    ...
    {
	dmu->error.err_code = E_DM0072_NO_LOCATION;
	if (dmu->dmu_location.data_address && dmu->dmu_location.data_in_size)
	    dmu->error.err_code = E_DM001F_LOCATION_LIST_ERROR;
	return (E_DB_ERROR);
    }
	...
	*/
	...
	status = dm2t_control(odcb->odcb_dcb_ptr, dmu->dmu_tbl_id.db_tab_base,
		    xcb->xcb_lk_id, LK_X, (i4)0, timeout, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	status = dm2t_close(relocate_rcb, DM2T_PURGE, &error);
	if (status != E_DB_OK)
	    break;
    ...
    {
	if (relocate_rcb)
	    status = dm2t_close(relocate_rcb, DM2T_NOPURGE, &local_error);
	dmu->error.err_code = error;
	return(E_DB_ERROR);
    }
	...
	{
	    status = dmpe_relocate(dmu, &error);
	}
    }
