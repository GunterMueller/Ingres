...
dmu_destroy(DMU_CB    *dmu_cb)
{
    ...
    */
	...
	&has_extensions, &error);

    if ((status == E_DB_OK) && has_extensions)
    {
	status = dmpe_destroy(dmu, &error);
    }
	...
	ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char * )0,
	    (i4)0, (i4 *)0, &local_error, 0);
	dmu->error.err_code = error = E_DM009D_BAD_TABLE_DESTROY;
    }
...
i4	    *err_code)
{
    ...
    */
	...
	status = dm2t_fix_tcb(odcb->odcb_dcb_ptr->dcb_id, table_id, &tran_id,
	    ...
	    odcb->odcb_dcb_ptr, &tcb, err_code);
	if (status)
	    return(status);
	...
	local_status = dm2t_unfix_tcb(&tcb, (i4)0, lock_list, log_id, err_code);
