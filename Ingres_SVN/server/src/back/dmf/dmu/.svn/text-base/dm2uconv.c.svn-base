    ...
    i4	*err_code )
{
	...
	*/
	status = dm2t_open( dcb, tbl_id, DM2T_X, DM2T_UDIRECT, DM2T_A_MODIFY,
			    ...
			    &rcb, (DML_SCB *)0, err_code );
	if ( status != E_DB_OK )
	{
		if (*err_code == E_DM0067_READONLY_TABLE_ERR)
		{
                	status = E_DB_OK;
		}
	}
	...
	*/
	status = dm1p_convert_table_to_65( t, err_code );
	if ( status != E_DB_OK )
	    break;
	...
	*/
	...
	{
	    status = dm2u_conv_etab(rcb, dcb, xcb, err_code);
	}
	...
	*/
	...
	{
	    status = dm1u_verify(rcb, xcb, DM1U_VERIFY | DM1U_KPERPAGE, err_code);
	}
    }
    ...
    */
	...
	{
	    i4		lerr_code;
	    DB_STATUS		lstatus = E_DB_OK;

	    lstatus = dm2t_close( rcb, DM2T_NOPURGE, &lerr_code );
	    if ( lstatus != E_DB_OK )
	        dm2u_conv_log_error( E_DM92CD_DM2U_CONVERT_ERR, &lerr_code);
	}
    }
    ...
    */
    dm2u_conv_log_error( E_DM92CD_DM2U_CONVERT_ERR, err_code );
...
i4		    *err_code)
{
    ...
    */
	...
	lstatus = dm2r_release_rcb(&rcb, &lerr_code);
	if ( lstatus != E_DB_OK )
	    dm2u_conv_log_error(E_DM9026_REL_UPDATE_ERR, &lerr_code);
    }

    dm2u_conv_log_error(E_DM9026_REL_UPDATE_ERR, err_code);
	...
	i4                  *err_code)
{
	...
	status = dm2r_position(iietab_rcb, DM2R_QUAL, qual_list, (i4)1, (i4)0,
		     (i4)0, (DM_TID *)0, err_code);
	if (status && (*err_code == E_DM0074_NOT_POSITIONED ||
			*err_code == E_DM008E_ERROR_POSITIONING))
	{
	    status = dm2r_position(iietab_rcb, DM2R_ALL, 0, 0, 0, 0, 0, 
			err_code);
	    if (status)
		break;
	}
	...
	*/
	status = dmpe_create_extension(xcb, &t->tcb_rel, DM_COMPAT_PGSIZE,
		...
		att_id, t->tcb_atts_ptr[att_id].type, &etab_record, err_code);
    ...
    local_status = dm2t_close(iietab_rcb, 0, err_code);
