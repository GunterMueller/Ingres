...
i4         *err_code)
{
    ...
    */
	...
	if (!has_vers)
	{
	   error = E_DM0169_ALTER_TABLE_SUPP;
	   break;
	}
	...
	*/
	...
	{
	    status = si_drop_column_check(t, attr_entry, cascade, &error);
	    if (status)
		break;
	}
	...
	*/
	if (operation == DMU_C_ADD_ALTER)
	{
	   ...
	   {
		error = E_DM0185_MAX_COL_EXCEEDED;
		status = E_DB_ERROR;
		break;
	   }
           ...
           {
              error = E_DM0186_MAX_TUPLEN_EXCEEDED;
	      status = E_DB_ERROR;
              break;
           }
	}
        ...
        */
	    ...
	    if (MEcmp((char *)&t->tcb_rel.relcmptlvl, DMF_S3_VERSION,
		      sizeof(t->tcb_rel.relcmptlvl)) < 0)
            {
	       error = E_DM00A5_ATBL_UNSUPPORTED;
	       break;
            }
			     ...
			     hold_attintl_id = attrrecord.attintl_id;
		          ...
		          if ((attrrecord.attid > hold_attid) &&
                   ...
                   if (status == E_DB_ERROR && error == E_DM0055_NONEXT)
                   {
                      ...
                      attrrecord.attfmp = attr_entry[0]->attr_prec;
                      attrrecord.attkey = 0;
                      attrrecord.attflag = attr_entry[0]->attr_flags;
		...
		if (dt_bits & AD_PERIPHERAL)
		{
	      ...
	      /* search the attribute to alter */
		    ...
		    /* Check if the column is a key in the table */
                    ...
                    {
                        error = E_DM019C_ACOL_KEY_NOT_ALLOWED;
			...
			STcopy (t->tcb_atts_ptr[i].name.db_att_name, colname);
                        ...
                        error = E_DM019B_INVALID_ALTCOL_PARAM;
                        ule_format(error, (CL_SYS_ERR *)0, ULE_LOG,
                                           NULL, (char *)0, (i4)0,
                                           (i4 *)0, &local_error, 3,
		    ...
		    */
			...
			{
                            error = E_DM01A0_INVALID_ALTCOL_DEFAULT;
                            ule_format(error, (CL_SYS_ERR *)0, ULE_LOG,
                                           NULL, (char *)0, (i4)0,
                                           (i4 *)0, &local_error, 0);
                            log_err = error;
	      ...
	      for (;;)
		    ...
		    { 
			    ...
			    STRUCT_ASSIGN_MACRO(attr_entry[0]->attr_name,
                            ...
                            attrrecord_tmp.attfmp = attr_entry[0]->attr_prec;
                            attrrecord_tmp.attkey = attrrecord.attkey; 
                            attrrecord_tmp.attflag = attr_entry[0]->attr_flags;
                            COPY_DEFAULT_ID( attr_entry[0]->attr_defaultID,
	    ...
	    ** iidistcol att_number values. */
		...
		status = pt_adddrop_adjust(xcb, dcb, rel_rcb, &key_desc[0], 
			&relrecord, db_lockmode, dropped_col_attid, &error);
		if (status != E_DB_OK)
		    break;
	    }
    ...
    */
    ...
    purge_mode = ( (error == E_DB_OK) ? DM2T_PURGE : DM2T_NOPURGE);

    if (rel_rcb)
    {
        local_status = dm2t_close(rel_rcb, purge_mode, &local_error);
        if (local_status)
        {
            if (error == E_DB_OK)
                error = local_error;
    ...
    if (attr_rcb)
    {
        local_status = dm2t_close(attr_rcb, purge_mode, &local_error);
        if (local_status)
        {
            if (error == E_DB_OK)
                error = local_error;
    ...
    if (idx_rcb)
    {
        local_status = dm2t_close(idx_rcb, purge_mode, &local_error);
        if (local_status)
        {
            if (error == E_DB_OK)
                error = local_error;
    ...
    if (r)
    {
        local_status = dm2t_close(r, purge_mode, &local_error);
        if (local_status)
        {
            if (error == E_DB_OK)
                error = local_error;
...
**
** Outputs:
**	err_code			Set if column is in any indexes.
**
** Returns:
...
i4		    *err_code)
{
    ...
    {
	if (MEcmp(&t->tcb_data_atts[i]->name,
		    &attribute[0]->attr_name,
		    sizeof(attribute[0]->attr_name)) == 0)
	{
	    if (t->tcb_data_atts[i]->key)
	    {
		*err_code = E_DM00A6_ATBL_COL_INDEX_KEY;
		return (E_DB_ERROR);
	    }
	}
    }
    ...
    */
	...
	{
	    if (MEcmp(&it->tcb_data_atts[i]->name,
			&attribute[0]->attr_name,
			sizeof(attribute[0]->attr_name)) == 0)
	    {
		*err_code = E_DM00A7_ATBL_COL_INDEX_SEC;
		return (E_DB_ERROR);
	    }
	}
    }
...
i4		    *err_code)
{
	...
	*/
	if (*base_rcb)
	{
	    ...
	    */
	    ...
	    status = dm2t_close(*base_rcb, DM2T_PURGE, err_code);
	    if (status != E_DB_OK)
		break;
	    *base_rcb = 0;
	}
	...
	*err_code = E_DM9101_UPDATE_CATALOGS_ERROR; /* need new msg */
    }
    return (E_DB_ERROR);
}

