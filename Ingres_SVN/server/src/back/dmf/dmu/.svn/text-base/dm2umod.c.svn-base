...
DB_ERROR        *errcb)
{
    ...
    i4               	log_id;
    i4		     	pgtype_flags;
    i4		     	*err_code = &(errcb->err_code);
    i4		     	*err_data = &(errcb->err_data);
    i4               	comp_growth;   /* Worst case growth of compression */
    DMPP_ACC_PLV     	*plv = NULL;
    DMPP_ACC_TLV     	*tlv = NULL;
    i4			sources;
    i4			targets;
    DM2U_SPCB		*sp;
    DM2U_TPCB		*tp;
    ...
    *mcb->mcb_tup_info = 0;
	...
	if (!mcb->mcb_temporary)
	{
	    ...
	    status = dm2t_control(dcb, mcb->mcb_tbl_id->db_tab_base, TableLockList, 
			    control_lock_mode, (i4)0, timeout, err_code);
	    if (status != E_DB_OK)
		return(E_DB_ERROR);
	}
    }
    ...
    ** for core catalogs.
    */
    if (mcb->mcb_tbl_id->db_tab_base <= DM_SCONCUR_MAX)
	lk_mode = DM2T_IX;
	...
	*/
	...
	if ( mcb->mcb_tbl_id->db_tab_index & DB_TAB_PARTITION_MASK )
	{
	    ...
	    status = dm2t_open(dcb, &master_tabid, lk_mode,
			...
			TableLockList, 0, 0,
			mcb->mcb_db_lockmode,
			tran_id, &timestamp, &master_rcb, NULL, &error);
	    if (status != E_DB_OK)
		break;
	}
	status = dm2t_open(dcb, mcb->mcb_tbl_id, lk_mode, 
	    ...
	    timeout, (i4)20, (i4)0, log_id, 
            ...
            mcb->mcb_db_lockmode, 
	    tran_id, &timestamp, &rcb, (DML_SCB *)0, &error);
	if (status != E_DB_OK) 
	    break;
	r = rcb;
	t = r->rcb_tcb_ptr;
	...
	*/
	    ...
	    (input_rcb && (input_rcb->rcb_tcb_ptr->tcb_rel.relstat & TCB_INDEX)))
	{
        ...
        */
	    ...
	    !mcb->mcb_temporary )
        {
            status = dm2t_xlock_tcb(dcb->dcb_id, t, r, err_code);
	...
	*/
	if ((t->tcb_rel.relstat & TCB_GATEWAY) &&
	    (table_access_mode == DM2T_A_SYNC_CLOSE))
	{
	    ule_format(W_DM9A11_GATEWAY_PATCH, (CL_SYS_ERR *)0, ULE_LOG , NULL,
	            (char * )0, 0L, (i4 *)0, &local_error, 2, 
	...
	*/
	...
	{
	    status = dm2t_close(r, DM2T_NOPURGE, &error);
	...
	*/
	if ( mcb->mcb_temporary && !mcb->mcb_clustered )
	{
	    ...
	    {
		error =  E_DM0183_CANT_MOD_TEMP_TABLE;
		break;
	    }
	...
	{
	    error = E_DM0200_MOD_TEMP_TABLE_KEY; 
	    break;
	}
	...
	*/
	if ( gateway || view )
	{
	    if (truncate || reorg || mcb->mcb_merge 
			...
			(mcb->mcb_mod_options2 & DM2U_2_TO_TABLE_PRIORITY)
				||
			(mcb->mcb_mod_options2 & DM2U_2_PERSISTS_OVER_MODIFIES)
				||
			(mcb->mcb_mod_options2 & DM2U_2_NOPERSIST_OVER_MODIFIES)
				||
		...
		if ( gateway )
		    error = E_DM0137_GATEWAY_ACCESS_ERROR;
		else
		    error = E_DM009F_ILLEGAL_OPERATION;
	...
	*/
	...
	{
	    status = dm2t_close(r, DM2T_NOPURGE, &error);
	    error = E_DM005E_CANT_UPDATE_SYSCAT;
	    r = 0;
	    break;
	}
	...
	*/
		...
		*/
		...
		for ( i = 0; status == E_DB_OK && i < mcb->mcb_nparts; i++ )
		{
			...
			pr->rcb_xcb_ptr = xcb;
                	if ((dcb->dcb_status & DCB_S_ROLLFORWARD) == 0)
       		        ...
       		        status = dm1u_verify(pr, xcb, mcb->mcb_verify, &error);
            ...
            ** can be monitored. */
            ...
            status = dm1u_verify(r, xcb, mcb->mcb_verify, &error);
            if (status != E_DB_OK)
                break;  
	    ...
	    if (master_rcb != NULL)
	    {
		status = dm2t_close(master_rcb, 0, &error);
		if (status != E_DB_OK)
		    break;
		master_rcb = NULL;
	    }
...

	...
	*/
	if ( mcb->mcb_mod_options2 & DM2U_2_ADD_EXTEND )
	{
		...
		if ( mcb->mcb_extend )
		    status = dm1p_add_extend(r, mcb->mcb_extend, DM1P_ADD_NEW, &error);
		if ( status != E_DB_OK )
		    break;
	    }
...

	...
	*/
	...
	{
	    mcb->mcb_unique = TRUE;
	    if ( mcb->mcb_compressed )
		mcb->mcb_index_compressed = TRUE;
	    mcb->mcb_compressed = FALSE;
	}
	...
	else if (t->tcb_rel.relstat2 & TCB2_EXTENSION)
	...
	** pick a new one
	*/
	if (mcb->mcb_page_size == 0 && (mcb->mcb_flagsmask & DMU_RETINTO) && mcb->mcb_compressed &&
	    t->tcb_rel.relwid + t->tcb_comp_atts_count >
	    dm2u_maxreclen(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize)) 
	{
	    i4 pgsize;
	    i4 pgtype;
	    i4 found = FALSE;
		...
		status = dm1c_getpgtype(pgsize, pgtype_flags,
		    t->tcb_rel.relwid + t->tcb_comp_atts_count, &pgtype);

		if (status == E_DB_OK &&
		    (t->tcb_rel.relwid + t->tcb_comp_atts_count
		    <= dm2u_maxreclen(pgtype, pgsize)) &&
	...
	else
	{
		...
		status = dm1c_getpgtype(mcb->mcb_page_size, pgtype_flags,
		    mcb->mcb_compressed ? t->tcb_rel.relwid + t->tcb_comp_atts_count : 
		    t->tcb_rel.relwid, &mcb->mcb_page_type) ;
		if (status != E_DB_OK)
		{
		    error = E_DM000E_BAD_CHAR_VALUE;
		    break;
		}
	    }
	}
	...
	** rows spanning pages)
	*/
	if (mcb->mcb_page_type == TCB_PG_V5 && !duplicates)
	{
	    error = E_DM0159_NODUPLICATES;
	    break;
	}
	...
	*/
	...
	{
	    error = E_DM005E_CANT_UPDATE_SYSCAT;
	    if ((mcb->mcb_structure != TCB_HASH) || (mcb->mcb_compressed != 0))
		error = E_DM005A_CANT_MOD_CORE_STRUCT;
	    ule_format(error, 0, ULE_LOG, NULL, 0, 0, 0,
		    &local_error, 0);
	    status = dm2t_close(r, DM2T_NOPURGE, &local_error);
	    r = 0;
	    break;
	}		
	...
	**
	** Perform the  the READONLY or NOREADONLY option. 
	** Note:
	** We log a dm0l_alter record so that a rollback operation
	** will cause the table to be purged. Changing a tables
	** READONLY status will eventaully be supported via
	** ALTER TABLE.
	*/
	if ( ( mcb->mcb_mod_options2 & DM2U_2_READONLY ) ||
	     ( mcb->mcb_mod_options2 & DM2U_2_NOREADONLY ) )
	{
	    /*
	    ** Only allowed on Master/base tables,
	    ** and non-system catalog tables.
	    */
	    if ( syscat || t->tcb_rel.relstat & TCB_INDEX ||
		 t->tcb_rel.relstat2 & TCB2_PARTITION )
	    {
		error = E_DM0068_MODIFY_READONLY_ERR;
		status = dm2t_close(r, DM2T_NOPURGE, &local_error);
		r = 0;
		break;
	    }
	    ...
	    /*
	    ** Log the alter, this will cause the TCB to be purged
	    ** in the case of rollback;
	    */
	    if (logging)
	    {
	        status = dm0l_alter( log_id, 0, mcb->mcb_tbl_id, 
	    ...
	    if ( mcb->mcb_mod_options2 & DM2U_2_READONLY )
		m->mx_readonly = MXCB_SET_READONLY;
	    else
		m->mx_readonly = MXCB_UNSET_READONLY;


	    /*
	    ** Update the catalog entry for this table
	    */
	    status = dm2u_readonly_upd_cats(m, journal, &error);
	    if (status != E_DB_OK)
		break;
	    ...
	    ** Purge the table
	    */
            status = dm2t_close(r, DM2T_PURGE, &error);
            if (status != E_DB_OK)
                break;
...

	/*
	** MODIFY TO LOG_INCONSISTENT, LOG_CONSISTENT,
	** PHYS_CONSISTENT, PHYS_INCONSISTENT, TBL_RECOVERY_ALLOWED,
	** TBL_RECOVERY_DISALLOWED, TO Table Priority  option. 
	*/
	if ( (mcb->mcb_mod_options2 & DM2U_2_LOG_INCONSISTENT) ||
	     (mcb->mcb_mod_options2 & DM2U_2_LOG_CONSISTENT) ||
	     (mcb->mcb_mod_options2 & DM2U_2_PHYS_INCONSISTENT) ||
	     (mcb->mcb_mod_options2 & DM2U_2_PHYS_CONSISTENT) ||
	     (mcb->mcb_mod_options2 & DM2U_2_TBL_RECOVERY_DISALLOWED) ||
	...
	{
	    /*
	    ** Only allowed on non-system catalog
	    ** tables and non-partitions.
	    */
	    if ( syscat || t->tcb_rel.relstat2 & TCB2_PARTITION )
	    {
		error = E_DM0160_MODIFY_ALTER_STATUS_ERR;
		status = dm2t_close(r, DM2T_NOPURGE, &local_error);
		r = 0;
		break;
	    }
	    ...
	    */
	    if (logging)
	    {
		if (mcb->mcb_mod_options2 & 
		    (DM2U_2_LOG_INCONSISTENT | DM2U_2_LOG_CONSISTENT))
		    status = dm0l_alter( log_id, 0, mcb->mcb_tbl_id, 
			             &table_name, &owner_name, 
				     0, DMT_C_LOG_CONSISTENT,
				     (LG_LSN *)0, &lsn, &error);
		else if (mcb->mcb_mod_options2 & 
		    (DM2U_2_PHYS_INCONSISTENT | DM2U_2_PHYS_CONSISTENT))
		    status = dm0l_alter( log_id, 0, mcb->mcb_tbl_id, 
			             &table_name, &owner_name, 
				     0, DMT_C_PHYS_CONSISTENT,
				     (LG_LSN *)0, &lsn, &error);
		else 
		    /* Default of table recovery allowed record log */
		    status = dm0l_alter( log_id, 0, mcb->mcb_tbl_id, 
			             &table_name, &owner_name, 
				     0, DMT_C_TBL_RECOVERY_ALLOWED,
				     (LG_LSN *)0, &lsn, &error);
	    ...
	    m->mx_table_id.db_tab_base  = mcb->mcb_tbl_id->db_tab_base;
	    m->mx_table_id.db_tab_index = mcb->mcb_tbl_id->db_tab_index;
	    m->mx_new_relstat2		= mcb->mcb_mod_options2;
	    ...
	    ** Update the catalog entry for this table
	    */
	    status = dm2u_alterstatus_upd_cats(m, journal, &error);
	    if (status != E_DB_OK)
		break;
	...
	if (sconcur)
	{
		...
		*/
		if (logging)
		{
			...
			(DB_TAB_ID *)0, bsf_lsn, 0, 0, (LG_LSN *)0, &lsn, &error);
		    if (status != E_DB_OK)
		    {
			break;
		    }
		...
		status = dm2u_sysmod(dcb, xcb, mcb->mcb_tbl_id, mcb->mcb_structure, 
		    ...
		    mcb->mcb_allocation, mcb->mcb_extend, mcb->mcb_tup_info, mcb->mcb_reltups, &error);
		if (status != E_DB_OK)
		{
		    if (error > E_DM_INTERNAL)
		    {
			ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
			    (char *)0, (i4)0, (i4 *)0, &local_error, 
			    0);
			error = E_DM00D1_BAD_SYSCAT_MOD;
		    }
		    break;
		}
		return (E_DB_OK);
	...
	{	
	    ...
	    */
		...
		for ( i = 0; status == E_DB_OK && i < mcb->mcb_nparts; i++ )
		{
			...
			** in parallel...
			*/
			status = dm1bmmerge(pr, mcb->mcb_i_fill, mcb->mcb_l_fill, &error);
		...
		** can be monitored. */
		...
		status = dm1bmmerge(r, mcb->mcb_i_fill, mcb->mcb_l_fill, &error);
	    }
...

	...
	*/
	...
	else
	{
        ...
        dm1c_getaccessors(mcb->mcb_compressed, r->rcb_tcb_ptr->tcb_rel.relpgtype,
                              mcb->mcb_page_size, &plv, &tlv);
        comp_growth = (*tlv->dmpp_compexpand)(t->tcb_data_atts,
                              t->tcb_rel.relatts);
	...
	status = dm0m_allocate(sizeof(DM2U_MXCB) +
	    ...
	    ((t->tcb_rel.relatts + 1) * sizeof(DB_ATTS *)) + /* mx_data_atts */
	...
	sp = (DM2U_SPCB*)((char*)m->mx_att_map +
			  (t->tcb_rel.relatts + 1) * sizeof(i4));
	/* Ensure proper alignment of SPCBs */
	sp = (DM2U_SPCB*)ME_ALIGN_MACRO(sp, sizeof(ALIGN_RESTRICT));
	...
	m->mx_compressed = mcb->mcb_compressed;
	...
	m->mx_kwidth = 0;
	m->mx_comp_katts_count = t->tcb_comp_katts_count;
	m->mx_rcb = r;
	m->mx_xcb = xcb;
	m->mx_lk_id = lk_id;
	m->mx_log_id = log_id;
	...
	** FIXME **** this is wrong for partitions!  give each partition
	** either some value based on the old partition fraction if not
	** repartitioning, or based on 1/#partitions if repartitioning.
	*/
	if (mcb->mcb_structure == TCB_HASH)
	{
	    if (dcb->dcb_status & DCB_S_ROLLFORWARD)
...
#if 0 /* FIXME (see Karl's note above) */
				m->mx_reltups, 
#else
	...
	*err_code = E_DB_OK;
	...
	if ((reorg == 0) && (t->tcb_rel.relstat & TCB_INDEX))
	{
	    ...
	    */
	    ...
	    if ( mcb->mcb_index_compressed )
		compressed_katts_count =
		    (*t->tcb_acc_tlv->dmppxcompexpand)
			(t->tcb_data_atts, t->tcb_rel.relatts); 
	    else
		compressed_katts_count = 0;
		...
		if (i < mcb->mcb_kcount)
		{
		    ...
		    if (status)
		    {
			error = E_DMF012_ADT_FAIL;
			break;
		    }
		    if (dt_bits & (AD_NOSORT | AD_NOKEY))
		    {
			error = E_DM0066_BAD_KEY_TYPE;
			break;
		    }
	    ...
	    */
	    ...
	    {
		error = E_DM001C_BAD_KEY_SEQUENCE;
		break;
	    }
	...
	else if (reorg == 0)
	{
		...
		for (j = 1; j <= t->tcb_rel.relatts; j++)
		{
			...
			*/
			...
			if (mcb->mcb_key[i]->key_order == DM2U_DESCENDING)
			{
			    ...
			    {
				error = E_DM007C_BAD_KEY_DIRECTION;
				break;
			    }
			}
			next_cmp++;
	    ...
	    */
		...
		if ( mcb->mcb_clustered )
		    ...
		    compressed_katts_count =
			(*t->tcb_acc_tlv->dmppxcompexpand)
			(m->mx_b_key_atts, mcb->mcb_kcount); 
	    }
	    else
		compressed_katts_count = 0;
	...
	/*
	** Build array of pointers to attributes in the data row, for use if
	** compression is needed.
	*/
	for (i = 0; i < t->tcb_rel.relatts; i++)
	{
	    m->mx_data_atts[i] = &t->tcb_atts_ptr[i + 1];
	}
	...
	if (reorg == 0 )
	{ 
	    if ((t->tcb_rel.relstat & TCB_INDEX) && mcb->mcb_structure == TCB_HEAP)
	    {
		error = E_DM001C_BAD_KEY_SEQUENCE;
		break;
	    }
	    ...
	    */
	    ...
	    if (m->mx_width + (mcb->mcb_compressed ? comp_growth : 0) >
		    dm2u_maxreclen(m->mx_page_type, m->mx_page_size)
		    && DMPP_VPT_PAGE_HAS_SEGMENTS(m->mx_page_type) == 0)
	    {
		error = E_DM0110_COMP_BAD_KEY_LENGTH;
		*err_data = m->mx_width + t->tcb_comp_atts_count;
		break;
	    }
	    if (mcb->mcb_structure == TCB_ISAM)
	    {
		if (m->mx_kwidth > 
		...
		{
		    error = E_DM010F_ISAM_BAD_KEY_LENGTH;
		    *err_data = m->mx_kwidth;
		    break;
		}	 
		if (m->mx_kwidth > 
		    (dm2u_maxreclen(m->mx_page_type, m->mx_page_size) + 
	    ...
	    if (mcb->mcb_structure == TCB_BTREE)
	    {
		i4	max_leaflen;
		i4	adj_leaflen = m->mx_kwidth +
			(mcb->mcb_index_compressed ? compressed_katts_count : 0);
...
*/
		...
		"Bad keylen (%dK V%d) (leaflen %d atts %d klen %d) max (%d,%d)\n",
			...
			max_leaflen,
			DM1B_KEYLENGTH);
		    error = E_DM007D_BTREE_BAD_KEY_LENGTH;
		    *err_data = adj_leaflen;
		...
		*/
		...
		if (mcb->mcb_index_compressed)
		    m->mx_comp_katts_count = compressed_katts_count;
	    }
	...
	if (! gateway)
	{
	    ...
	    */
		...
		if (!((dcb->dcb_bm_served == DCB_MULTIPLE) && 
		    ...
		    DMPP_VPT_PAGE_HAS_SEGMENTS(t->tcb_rel.relpgtype) == 0 &&
		...
		{
		    error = E_DM9CB3_ONLINE_MOD_RESTRICT;
		    status = E_DB_ERROR;
		    if (DMZ_SRT_MACRO(13))
		    {
		...
		if ( status = dm2u_check_for_interrupt(xcb, &error) )
		    break;
		...
		if (m->mx_inmemory)
		{
		    ...
		    */
			...
			tp->tpcb_loc_list, tp->tpcb_loc_count, &error );
		    if (status != E_DB_OK)
			break;
		    temp_cleanup = TRUE;
		}
		else
		{
		    ...
		    */
			...
			mcb->mcb_page_size,
			DM2F_ALLOC, DM2F_MOD, &tp->tpcb_tabid,
			tp->tpcb_name_id, tp->tpcb_name_gen,
			(DMP_TABLE_IO *)0, db_sync_flag, &error);
		    if (status != E_DB_OK)
			break;
...

	...
	*/
	if (mcb->mcb_omcb)
	{
		...
		status = logModifyData(log_id, dm0l_jnlflag,
			mcb->mcb_new_part_def, mcb->mcb_new_partdef_size,
			mcb->mcb_partitions, mcb->mcb_nparts, &error);
		if (status != E_DB_OK)
		    break;

		dm0l_structure = mcb->mcb_structure;

		status = dm0l_modify(log_id, dm0l_jnlflag,
				mcb->mcb_tbl_id, &table_name, &owner_name, 
				loc_count, orig_locnArray, dm0l_structure, 
	...
	{
	    if (reorg)
		status = dm2u_reorg_table(m, &error);
	    else if (!online_modify)
	    {
		/* Load the table from the sorter. */
		status = dm2u_load_table(m, &error);

		/* rollforward online modify */
		if ((dcb->dcb_status & DCB_S_ROLLFORWARD) && 
		    (error = E_DM9CB1_RNL_LSN_MISMATCH) &&
		    (status == E_DB_INFO) &&
		    (mcb->mcb_omcb))
		{
		    /* save mxcb context */
		    octx->octx_mxcb = (PTR)m;
		    errcb->err_code = error;	
		    return(E_DB_INFO);
		}
	    }
	    if ( status )
	...
	else
	{
	    ...
	    ** Unfortunately, the dm2u_udpate_catalogs call below (that
	...
	*/
	if (reorg == 0)
	{
	    if ((mcb->mcb_structure == TCB_HEAP) && (mcb->mcb_kcount > 0))
	    {
		for ( tp = m->mx_tpcb_next; tp; tp = tp->tpcb_next )
	...
	*/
	if ((logging) && (!already_logged))
	{
	    ...
	    status = dm0l_modify(log_id, dm0l_jnlflag,
		mcb->mcb_tbl_id, &table_name, &owner_name, 
		loc_count, orig_locnArray,
		dm0l_structure, relstat, mcb->mcb_min_pages, mcb->mcb_max_pages, 
		mcb->mcb_i_fill, mcb->mcb_d_fill, mcb->mcb_l_fill, m->mx_newtup_cnt, buckets, 
		mod_key, key_order,
		mcb->mcb_allocation, mcb->mcb_extend, m->mx_page_type, m->mx_page_size,
		(online_modify ? DM0L_ONLINE : 0), (DB_TAB_ID *)0, 
	...
	if (mcb->mcb_temporary)
	{
	    status = update_temp_table_tcb(m, &error);
	    temp_cleanup = FALSE;
	}
	else
	{
	    ...
	    ** reference on it.
	    */
	    status = dm2u_update_catalogs(m, journal, &error);
	}
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	return( rebuild_persistent(dcb, xcb, mcb->mcb_tbl_id, 
			mcb->mcb_dmu, online_modify, &online_tabid, err_code) );
    }
	...
	*/
	...
	{
	    status = dm2t_close(m->mx_buildrcb, DM2T_NOPURGE, err_code);
	    if (status != E_DB_OK)
	       ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, &local_error, 0);
	}
    ...
    */
    ...
    if (master_rcb != NULL)
    {
	status = dm2t_close(master_rcb, DM2T_NOPURGE, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
...
**	    memory for DM2T_ATTR_ENTRY stuff based on tcb_relatts, which
...
i4		*err_code)  
{
    ...
    DM2T_ATTR_ENTRY	*AttEntry;
    DM2T_ATTR_ENTRY     *AttlPtr;
    ...
    */
	...
	status = dm2d_release_user_tcbs(dcb, xcb->xcb_lk_id,
			xcb->xcb_log_id, DM2T_TOSS, err_code);
	if (status != E_DB_OK)
	    return(status);
    }
    ...
    */
	...
	status = dm2t_open(dcb, tbl_id, DM2T_IX, 
	    ...
	    &xcb->xcb_tran_id, &timestamp, &rcb, (DML_SCB *)0, &error);
	if (status != E_DB_OK)
	    break;
	r = rcb;
	t = r->rcb_tcb_ptr;
	...
	*/
	...
	status = dm0m_allocate(sizeof(DMP_MISC) + (NumAtts *
			    (sizeof(DM2T_ATTR_ENTRY*) + sizeof(DM2T_ATTR_ENTRY))),
			    DM0M_ZERO, (i4)MISC_CB, (i4)MISC_ASCII_ID,
			    (char*)NULL, (DM_OBJECT**)&AttMem, &error);
	if ( status )
	    break;

	AttList = (DM2T_ATTR_ENTRY**)((char*)AttMem + sizeof(DMP_MISC));
	AttMem->misc_data = (char*)AttList;
	AttEntry = (DM2T_ATTR_ENTRY*)((char*)AttList + 
			(NumAtts * sizeof(DM2T_ATTR_ENTRY*)));
	...
	{	
	    ...
	    AttlPtr->attr_prec  = att_ptr->precision;	    
	    AttlPtr->attr_flags = att_ptr->flag;	    
	...
	*/
	...
	status = 
	    dm2u_create(dcb, xcb, &newtab_name, &owner, &location, (i4)1,
		    ...
		    (setrelstat | DM2U_SYSMOD), relstat2,
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    error = errcb.err_code;
	    if (error != E_DM0101_SET_JOURNAL_ON)
		break;
	    status = E_DB_OK;
	    error = 0;
	}
	...
	*/
	...
	if (status == E_DB_ERROR && error != E_DM0055_NONEXT)
	    break;
	...
	if (status != E_DB_OK)
	    break;
	r = 0;
	status = dm2t_close(nr, DM2T_PURGE, &error);
	if (status != E_DB_OK)
	    break;
	nr = 0;
	...
	*/
	...
	if (status != E_DB_OK)
	    break;
    	status = dm2t_close(r_upt, purge_mode, &error);
	if (status != E_DB_OK)
	    break;
	r_upt = 0;
	...
	*/
	...
	status = dm0c_open(dcb,DM0C_PARTIAL, xcb->xcb_lk_id, &config, &error);
	if (status != E_DB_OK)
	    break;
	config_open = TRUE;
	...
	status = dm0c_close(config, DM0C_UPDATE | DM0C_LEAVE_OPEN, &error);
	if (status != E_DB_OK)
	    break;
	...
	for (;;)
	{
	    ...
	    */
		...
		status = dm2f_close_file(t->tcb_table_io.tbio_location_array,
			t->tcb_table_io.tbio_loc_count, (DM_OBJECT *)f, &error); 
		if (status != E_DB_OK)
		    break;
		file_closed = TRUE;
	    }
	    ...
	    */
		...
		status = dm0l_sm1_rename(xcb->xcb_log_id, dm0l_jnlflag,
		    ...
		    oldpgtype, newpgtype, oldpgsize, newpgsize, &error); 
		if (status != E_DB_OK)
		{
		    break;
		}
		...
		&new1tabname, filelength, &dcb->dcb_name, &error);
	    if (status != E_DB_OK)
	    {
		break;
	    }
		...
		&new2tabname, filelength, &dcb->dcb_name, &error);
	    if (status != E_DB_OK)
	    {
		break;
	    }
		...
		db_sync_flag, (DM_OBJECT *)f, &error);
	    if (status != E_DB_OK)
	    {
		break;
	    }
		...
		*/
		...
		&old2tabname, filelength, &dcb->dcb_name, &error);
	    if (status != E_DB_OK)
	    {
		break;
	    }
	    ...
	    */
	    ...
	    /* Update the set of accessors (may have changes) */
	    dm1c_getaccessors(t->tcb_rel.relcomptype,
			      newpgtype,
			      newpgsize,
			      &t->tcb_acc_plv,
			      &t->tcb_acc_tlv);
	...
	*/
	...
	status = dm0c_close(config, DM0C_UPDATE | DM0C_COPY, &error);
	if (status != E_DB_OK)
	    break;
	config_open = FALSE;
	...
	status = dm2u_destroy(dcb, xcb, &ntbl_id, DM2T_X,
	    DM2U_MODIFY_REQUEST, 0, 0, 0, 0, &error);
	if (status != E_DB_OK)
    	    break;
	...
	*/
	...
	if (dev_rcb)
	{
	    status = dm2t_close(dev_rcb, DM2T_NOPURGE, &error);
	    if (status != E_DB_OK)
		break;
	    dev_rcb = 0;
	}
	...
	*/
	...
	local_status = dm2f_open_file(xcb->xcb_lk_id, xcb->xcb_log_id,
	    ...
	    db_sync_flag, (DM_OBJECT *)f, &local_error);
    ...
    if (config_open)
	local_status = dm0c_close(config, DM0C_UPDATE, &local_error);
    if (r)
	local_status = dm2t_close(r, DM2T_NOPURGE, &local_error);
    if (nr)
	local_status = dm2t_close(nr, DM2T_NOPURGE, &local_error);
    if (dev_rcb)
	local_status = dm2t_close(dev_rcb, DM2T_NOPURGE, &local_error);
...
i4		*err_code)
{
	...
	*/
	...
	status = dm0m_allocate(
			(i4)(t->tcb_rel.relpgsize * DI_EXTENT_SIZE) 
				+ sizeof(DMP_MISC),
			DM0M_ZERO, 
			(i4)MISC_CB,
	...
	*/
	...
	status = dm2f_open_file(sp->spcb_lk_id, sp->spcb_log_id,
				...
				db_sync_flag,
				(DM_OBJECT *)sp->spcb_rcb, err_code);
	if ( status )
	    break;
	...
	*/
	...
	if ( local_status )
	{
	    ule_format(local_err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
    ...
    {
	(VOID)dm2t_close(sp->spcb_rcb, DM2T_PURGE, &local_err_code);
	sp->spcb_rcb = (DMP_RCB*)NULL;
    }
...
i4             	    *err_code)
{
	...
	/* Session temp table locks are always on the SCB lock list */
	...
	*/
	status = dm2t_fix_tcb(m->mx_dcb->dcb_id, &m->mx_table_id, 
			    ...
			    m->mx_dcb, &old_tcb, err_code);
	if (status != E_DB_OK)
	{
	    status = E_DB_ERROR;
	    *err_code = E_DM9101_UPDATE_CATALOGS_ERROR;
	    break;
	}
	t = old_tcb;
	...
	*/
	...
	status = dm0m_allocate(t->tcb_rel.relatts * sizeof(DM2T_ATTR_ENTRY) +
			       t->tcb_rel.relatts * sizeof(DM2T_ATTR_ENTRY *) +
							    sizeof(DMP_MISC),
		    DM0M_ZERO, (i4)MISC_CB, (i4)MISC_ASCII_ID,
		    (char *)0, (DM_OBJECT **)&attr_cb, err_code);
	if (status)
	    break;
	attr_array = (DM2T_ATTR_ENTRY **)((char *)attr_cb + sizeof(DMP_MISC));
	attr_cb->misc_data = (char *)attr_array;
	attrs = (DM2T_ATTR_ENTRY *)((char *)attr_array +
		    (t->tcb_rel.relatts * sizeof(DM2T_ATTR_ENTRY *)));
	...
	status = dm0m_allocate(mct->mct_keys * sizeof(DM2T_KEY_ENTRY) +
			       mct->mct_keys * sizeof(DM2T_KEY_ENTRY *) +
							    sizeof(DMP_MISC),
		    DM0M_ZERO, (i4)MISC_CB, (i4)MISC_ASCII_ID,
		    (char *)0, (DM_OBJECT **)&key_cb, err_code);
	if (status)
	    break;
	...
	status = dm0m_allocate(tp->tpcb_loc_count * sizeof(DB_LOC_NAME) +
						    sizeof(DMP_MISC),
		    DM0M_ZERO, (i4)MISC_CB, (i4)MISC_ASCII_ID,
		    (char *)0, (DM_OBJECT **)&loc_cb, err_code);
	if (status)
	    break;
	...
	*/
	for ( i = 0; i < t->tcb_rel.relatts; i++ )
	{
	    ...
	    attrs[i].attr_prec = t->tcb_atts_ptr[i+1].precision;
	    attrs[i].attr_flags = t->tcb_atts_ptr[i+1].flag;
	...
	*/
	...
	if (m->mx_reorg == 0)
	{
		...
		relation.relcomptype = TCB_C_STANDARD;
	    }
	    else
	...
	*/
	status = dm2t_destroy_temp_tcb((t->tcb_rel.relstat & TCB_INDEX)
					? 0 : TableLockList,
				    m->mx_log_id, m->mx_dcb, 
				    &m->mx_table_id, (i4)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	if (m->mx_inmemory)
	{
	    ...
	    */
	    ...
	    status = dm2t_reserve_tcb(build_tcb, m->mx_dcb,
				    DM2T_TMPMOD_WAIT, XCBLockList, err_code);
	    if (status != E_DB_OK)
		break;
	...
	if (m->mx_inmemory)
	{
	    ...
	    ** destroy it.
	    */
	    status = dm0p_reparent_pages(build_tcb, t, err_code);
	    if (status != E_DB_OK)
		break;
	    ...
	    status = dm2t_destroy_temp_tcb(XCBLockList, m->mx_log_id, 
			    m->mx_dcb, &build_table_id, (i4)0, err_code);
	    if (status != E_DB_OK)
		break;
	}
    ...
    {
	status = dm2t_unfix_tcb(&old_tcb, (i4)0, TableLockList,
				m->mx_log_id, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
...
i4             *err_code)
{
    ...
    ** if (m->mx_rcb->rcb_tcb_ptr->tcb_rel.relstat2 & TCB2_EXTENSION)
...
i4		*err_code)
{
    ...
    */
	...
	{
	    status = E_DB_ERROR;
	    *err_code = E_DM0191_RAW_LOCATION_MISSING;
	    break;
	}
    }
...
i4		*err_code)
{
	    ...
	    (dcb->dcb_ext->ext_entry[i].flags & DCB_DATA))
	{
		...
		{
		    *err_code = E_DM0189_RAW_LOCATION_INUSE; 
		    return(E_DB_ERROR);
		}
		status = dm2u_raw_location_free(dcb, m->mx_xcb,
				&loc->loc_name, err_code);
		if (status != E_DB_OK)
		    return(status);
	...
	i4 *err_code)
{
    ...
    */
    if (part_def != NULL)
	status = logPartDef(log_id, flags, part_def, def_size, err_code);
	...
	i4 *err_code)
{
    ...
    */
    ...
    total_size = (i4)ME_ALIGN_MACRO((PTR)total_size, sizeof(ALIGN_RESTRICT));
    ...
    status = dm0m_allocate(MAX_RAWDATA_SIZE + sizeof(DMP_MISC), 0,
		...
		NULL, (DM_OBJECT **) &holding_area, err_code);
    if (DB_FAILURE_MACRO(status))
	return (status);
    ...
    *(i4 *)cur_posn = ppchar_entries;
	...
	if (outppc->loc_array.data_address != NULL)
	{
	    ...
	    */
	    ...
	    if (i < num_locns)
	    {
		outppc->loc_array.data_address = (PTR) locn_offset[i];
	    }
	    else
	    {
		/* Didn't find it.  Assume it goes out at the end.  */
		outppc->loc_array.data_address = (PTR) untracked_offset;
...
i4		    *err_code)
{
    ...
    */
	...
	status = dm0l_bsf(m->mx_log_id, dm0l_flag, &t->tcb_rel.reltid, 
	    &table_name, &table_owner,  (LG_LSN *)0, &m->mx_bsf_lsn, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	if (t->tcb_rel.relstat & TCB_SYS_MAINTAINED)
	    setrelstat |= DM2U_SYS_MAINTAINED;
	...
	status = dm0m_allocate(sizeof (DMP_MISC) + 
		(i4)(t->tcb_rel.relatts * sizeof (DM2T_ATTR_ENTRY)) + 
		(i4)(t->tcb_rel.relatts * sizeof(DM2T_ATTR_ENTRY *)) + 
		(i4)(t->tcb_rel.relatts * sizeof(DM2U_KEY_ENTRY)) + 
		(i4)(t->tcb_rel.relatts * sizeof(DM2U_KEY_ENTRY *)),
		0, (i4)MISC_CB, (i4)MISC_ASCII_ID, (char *)0, 
		(DM_OBJECT **)&misc_cb, err_code);
	if (status != E_DB_OK)
	    break;

	attrs = (DM2T_ATTR_ENTRY *)(misc_cb + 1);
	misc_cb->misc_data = (char*)attrs;
	attr_ptrs = (DM2T_ATTR_ENTRY **)(attrs + (t->tcb_rel.relatts));
	...
	status = dm2u_create(dcb, xcb, &newtab_name, &owner,
		    ...
		    (DM_DATA *)NULL, (i4)0, (i4)0,
		    (DMU_FROM_PATH_ENTRY *)NULL, (DM_DATA *)0, 
		    0, 0, (f8*)NULL, 0, &dmu_part_def, 0, &errcb);
	...
	else
	{
	    ...
	    dm2u_index_cb.indxcb_compressed = m->mx_compressed;
	    ...
	    dm2u_index_cb.indxcb_kcount = attr_count; /* kcount = #atts */
	    dm2u_index_cb.indxcb_key = (DM2U_KEY_ENTRY **)key_ptrs;
	    dm2u_index_cb.indxcb_acount = kcount;
	    ...
	    dm2u_index_cb.indxcb_errcb = &errcb;
	    status = dm2u_index(&dm2u_index_cb);

	    if (status != E_DB_OK)
	    {
		*err_code = errcb.err_code;
		break;
	    }
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    *err_code = errcb.err_code;
	    if (errcb.err_code != E_DM0101_SET_JOURNAL_ON)
		break;
	...
	if (partitions)
	{
           ...
           status = dm0m_allocate(sizeof (DMP_MISC) +
                        ...
                        DM0M_ZERO, (i4)MISC_CB, (i4)MISC_ASCII_ID,
                        (char *)0, (DM_OBJECT **)&pp_misc_cb, err_code);
            if (status != E_DB_OK)
                break;
	...
	/* create $online partitions */
		...
		status = dm0m_allocate(sizeof(DMP_MISC) + (sp->spcb_loc_count *
                        ...
                        (DM_OBJECT **)&loc_misc_cb, err_code);
		...
		status = dm2u_create(dcb, xcb, 
		    (DB_TAB_NAME *)newparttab_name, &owner,
                    ...
                    (DMU_FROM_PATH_ENTRY *)NULL, (DM_DATA *)0,
                    0, 0, (f8*)NULL, 0, 
		    &dmu_part_def, sp->spcb_partno, &errcb);
	...
	status = dm2t_open(dcb, &t->tcb_rel.reltid, DM2T_N,
        ...
        status = dm0m_allocate(sizeof (DMP_MISC) +
                ...
                DM0M_ZERO, (i4)MISC_CB, (i4)MISC_ASCII_ID,
                (char *)0, (DM_OBJECT **)&omisc_cb, err_code);
        if (status != E_DB_OK)
            break;
	...
	for ( i = 0; i < m->mx_spcb_count; i++)
	{
		...
		status = dm2t_open(dcb, 
		    ...
		    &(osrc->osrc_rnl_rcb), (DML_SCB *)0, err_code);
		if (status != E_DB_OK)
		    break;
		osrc->osrc_master_rnl_rcb = readnolock_rcb;
		osrc->osrc_partno = i;
	    ...
	    status = init_readnolock(m, osrc, err_code);
	    if (status != E_DB_OK)
		break;
	...
	mcb->mcb_compressed = m->mx_compressed;
	...
	if (readnolock_rcb)
	{
	    status = dm2t_close(readnolock_rcb, DM2T_NOPURGE, err_code);
	    if (status != E_DB_OK)
		break;
	...
	if (index_cbs)
	{
		...
		if (status != E_DB_OK)
		     *err_code = index_cbs->error.err_code;
	    }
	    else
	    {
				    ...
				    ix_dmu = (DMU_CB *)ix_dmu->q_next)
		{
		    ...
		    if (status != E_DB_OK)
		    {
			*err_code = ix_dmu->error.err_code;
			break;
		    }
		}
	    }
	...
	if ( get_online_rel(m, modtemp_tabid, index_cbs,
		o_partitions, db_lockmode, err_code) )
	    break;
	...
	status = dm2t_control(dcb, m->mx_table_id.db_tab_base,
		xcb->xcb_lk_id, LK_X, (i4)0, timeout, err_code);
	if (status != E_DB_OK)
	    break;
	...
	status = dm2t_open(dcb, &m->mx_table_id, DM2T_X, 
	    ...
	    xcb->xcb_lk_id, (i4)0, (i4)0, 
	    db_lockmode, 
	    &xcb->xcb_tran_id, &timestamp, &xrcb, (DML_SCB *)0, err_code);

	if (status != E_DB_OK)
	    break;

	status = dm2t_close(xrcb, DM2T_NOPURGE, err_code);
	xrcb = (DMP_RCB *)0;
	...
	status = dm0l_esf(m->mx_log_id, dm0l_flag, &t->tcb_rel.reltid, 
	    &table_name, &table_owner, (LG_LSN *)0, &m->mx_esf_lsn, err_code);
	if (status != E_DB_OK)
	    break;

	status = check_archiver(dcb, xcb, &table_name, &m->mx_esf_lsn,err_code);
	if (status != E_DB_OK)
	    break;
	...
	status = dm2t_open(dcb, modtemp_tabid, DM2T_X, 
	    ...
	    &timestamp, &opcb->opcb_master_rcb, (DML_SCB *)0, err_code);
	...
	status = do_sidefile(dcb, xcb, &table_name, &table_owner, m,
			&omcb, db_lockmode, tup_info, err_code);
	...
	for ( opcb = omcb.omcb_opcb_next; opcb; opcb = opcb->opcb_next)
	{
		...
		if (dup_tabid.db_tab_base)
		{
		    ...
		    */
		    status = do_online_dupcheck(dcb, xcb, m, opcb->opcb_rcb,
				&dup_tabid, db_lockmode, err_code);
		    if (status)
			break;
		}
	    }
	    ...
	    {
		status = dm2t_close(opcb->opcb_rcb, DM2T_PURGE, err_code);
		if (status != E_DB_OK)
		{
	...
	if (index_cbs)
	{
		...
		status = dm2t_close(ix_rcb, DM2T_NOPURGE, err_code);
		if (status)
		    break;
		ix_rcb = (DMP_RCB *)0;
		...
		status = dm2u_destroy(dcb, xcb, &ix_dmu->dmu_idx_id, DM2T_X,
			DM2U_MODIFY_REQUEST, 0, 0, 0, 0, err_code);
    ...
    if (readnolock_rcb)
    {
	local_status = dm2t_close(readnolock_rcb, DM2T_NOPURGE, &local_error);
	if (local_status != E_DB_OK)
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
                        (char *)0, (i4)0, (i4 *)0, &local_error, 0);
    }
    ...
    if (xrcb)
	local_status = dm2t_close(xrcb, DM2T_NOPURGE, &local_error);
	    ...
	    (*err_code != E_DM004D_LOCK_TIMER_EXPIRED))
	{
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		    (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM9CA6_ONLINE_MODIFY;
	}
    }
...
i4		    *err_code)
{
	...
	{
	    status = dmu_pindex(index_cbs);
	    dmu->error.err_code = index_cbs->error.err_code;
	    dmu->error.err_data = index_cbs->error.err_data;
	}
	else
	{
		...
		status = dmu_index(ix_dmu);
		dmu->error.err_code = ix_dmu->error.err_code;
		dmu->error.err_data = ix_dmu->error.err_data;
		if (status != E_DB_OK)
		    break;
	    }
	}
...
i4		    *err_code)
{
	...
	*/
    ...
    if (status != E_DB_OK)
    {
	*err_code = E_DM9CA5_ONLINE_ARCHIVER;
...
i4		    *err_code)
{
	...
	status = dm0j_open(DM0J_MERGE, (char *)&dcb->dcb_jnl->physical, 
		...
		&m->mx_table_id, (DM0J_CTX **)&jnl, err_code);
	...
	*/
	...
	else
	{
		...
		mem_needed += (position_tcb->tcb_rel.relnparts * 
			...
			(i4)MISC_ASCII_ID, (char *)0, 
			(DM_OBJECT **)&pos_misc_cb, err_code);
            if (status)
                break;
	    ...
	    if (status != E_DB_OK)
	    {
		TRdisplay("ONLINE MODIFY: %~t Can't open %~t (%d)\n",
		    sizeof(DB_TAB_NAME), table_name->db_tab_name,
		    sizeof(DB_TAB_NAME), position_tcb->tcb_rel.relid.db_tab_name,
		    *err_code);
		break;
	...
	*/
	      ...
	      rec_cnt++, comprec = FALSE, redo = redo2 = FALSE)
	{
	    ...
	    if (status = dm2u_check_for_interrupt(m->mx_xcb, err_code))
		break;
  ...
  dmd_log(FALSE, (PTR)record, dcb->dcb_jnl_block_size);
		...
		if (comptype)
		{
		    dm1c_get_tlv(comptype, &tlv);
		    status = (*tlv->dmpp_uncompress)(src_rcb->rcb_proj_data_atts,
				(i4)src_tcb->tcb_rel.relatts, log_row, origrec,
				record_size, &uncompressed_length, comptype, 
				(DM_TID *)0, &src_rcb->rcb_tupbuf, row_version,
				src_rcb->rcb_adf_cb);
		    log_row = origrec;
		}
		...
		if (record->flags & DM0L_CLR)
		{
		    ...
		    */
		    ...
		    {
			if (DMZ_SRT_MACRO(13))
			    TRdisplay("ONLINE MODIFY: %~t find_compensated failed complsn=(%d,%d)\n",
				sizeof(DB_TAB_NAME), table_name->db_tab_name,
		...
		if (status == E_DB_OK)
		{
		    status = dm2r_delete(opcb_list[i].opcb_rcb, &tid, 
			    dm2r_flag | DM2R_REDO_SF, err_code);
		    opcb_list[i].opcb_tupcnt_adjust--;
		    (*tup_info)--;
		}
		else if (*err_code == E_DM0055_NONEXT)
		{
		...
		*/
		...
		if (record->flags & DM0L_CLR)
		{
		    ...
		    ** locate compensated log rec to extract after image delta
		    ...
		    status = find_compensated(m, record, wrksp, &ndata_len, 
					&nrec_size, &diff_offset, err_code);
		    if (status)
		    {
			if (DMZ_SRT_MACRO(13))
			    TRdisplay("ONLINE MODIFY: %~t find_compensated failed complsn=(%d,%d)\n",
		...
		new_row = new_row_buf;
		    ...
		    status = (*tlv->dmpp_uncompress)(src_rcb->rcb_proj_data_atts,
				(i4)src_tcb->tcb_rel.relatts, log_row, origrec,  
				record_size, &uncompressed_length, comptype, (DM_TID *)0, 
				&src_rcb->rcb_tupbuf, rep->rep_orow_version,
				src_rcb->rcb_adf_cb);
		    old_row = origrec;

		    status = (*tlv->dmpp_uncompress)(src_rcb->rcb_proj_data_atts,
				(i4)src_tcb->tcb_rel.relatts, new_row_buf, newrec,
				record_size, &uncompressed_length, comptype, (DM_TID *)0,
		    ...
		    if (status == E_DB_OK)
  {
		...
		if (redo2)
		{
			...
			status = dm2r_put(opcb_list[i].opcb_rcb, 
			    DM2R_DUPLICATES | DM2R_REDO_SF, new_row, err_code);
		    }
		}
	    }
	}
    ...
    if (jnl)
    {
	local_status = dm0j_close(&jnl, &local_error);

	if (DMZ_SRT_MACRO(13))
	    TRdisplay("ONLINE MODIFY: %~t Close sidefile status %d\n",
		sizeof(DB_TAB_NAME), table_name->db_tab_name, local_status);
	if (local_status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
	...
	if (src_tcb->tcb_rel.relnparts)
	{
	    local_status = dm0m_allocate(sizeof(DMP_MISC) +
			...
			DM0M_ZERO, (i4)MISC_CB, (i4)MISC_ASCII_ID,
                        (char *)0, (DM_OBJECT **)&fpsrc_misc_cb, &local_error);
	    if (local_status != E_DB_OK)
	    {
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
                    (char *)0, (i4)0, (i4 *)0, &local_error, 0);
	...
	*/
	...
	if (status == E_DB_OK)
	{
		...
		local_status = dm0p_unfix_page(rcb, DM0P_UNFIX, 
				&rcb->rcb_data_page_ptr, &local_error);
		if (local_status)
		    status = local_status;
	    }

	    if (dm0p_fix_page(rcb, cand->freepg_no, DM0P_READ,
			&rcb->rcb_data_page_ptr, &local_error) == E_DB_OK)
	    {
		DMPP_PAGE	*pg = rcb->rcb_data_page_ptr;
...
for pg = %d  pgstat=0x%x\n",
	    ...
	    (*err_code != E_DM010C_TRAN_ABORTED))
	{
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		(i4)0, (i4 *)0, err_code, 0);
	    ule_format(E_DM9CA2_SIDEFILE_TABLE, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 1,
		sizeof(DB_TAB_NAME), table_name->db_tab_name);
	    *err_code = E_DM9CA3_SIDEFILE;
	}
    }
...
i4		*err_code)
{
	...
	while(1)
	{
	    ...
	    */
		...
		adt_tupcmp((ADF_CB *)base_rcb->rcb_adf_cb, (i4)t->tcb_rel.relatts, 
	...
	status = dm2r_get(base_rcb, tid, DM2R_BYTID, (char *)rowbuf, err_code);
...
i4		*err_code)
{
	...
	status = dm1p_lastused(r, (u_i4)DM1P_LASTUSED_DATA,
		    &lastused_pageno, (DMPP_PAGE **)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	status = dm0m_allocate( size, DM0M_ZERO, (i4) MISC_CB, 
	    (i4) MISC_ASCII_ID, (char *) 0, 
	    (DM_OBJECT **) &rnl->rnl_xmap, err_code);
	if ( status != E_DB_OK )
	    break;
	...
	status = dm0m_allocate( size, DM0M_ZERO, (i4) MISC_CB, 
	    (i4) MISC_ASCII_ID, (char *) 0, 
	    (DM_OBJECT **) &rnl->rnl_xlsn, err_code);
	if ( status != E_DB_OK )
	    break;
...
rnl_lsn for pg %d log_lsn %d\n",
	    ...
	    *err_code = E_DM9CB0_TEST_REDO_FAIL;
	    status = E_DB_ERROR;
	}
    }
...
i4              *err_code)
{
	...
	status = dm2t_open(dcb, dupchk_tabid, DM2T_N, DM2T_UDIRECT, 
		...
		xcb->xcb_lk_id, (i4)0, (i4)0, db_lockmode, &xcb->xcb_tran_id, 
		&timestamp, &dupchk_rcb, (DML_SCB *)0, err_code);
	...
	status = dm2r_position(dupchk_rcb, DM2R_ALL, (DM2R_KEY_DESC *)0,
                (i4)0, (i4)0, (i4)0,(DM_TID *)0, err_code);
	...
	for (; cmpstatus == E_DB_OK && status == E_DB_OK;)   
	{
	    ...
	    */
	    status = dm2r_get(dupchk_rcb, &dupchk_tid, DM2R_GETNEXT, key, 
			err_code); 
	    if (status == E_DB_ERROR && *err_code == E_DM0055_NONEXT)
	    {
		...
		key_desc[i].attr_operator = DM2R_EQ;
                key_desc[i].attr_value = &key[dupchk_tcb->tcb_data_atts[i]->offset];
	    }

	    status = dm2r_position(r, DM2R_QUAL, key_desc, keys_given,
			(i4)0, (i4)0, (DM_TID *)NULL, err_code);
...
lowtid=(%d,%d) currenttid=(%d,%d)\n",
		    ...
		    {
			ule_format(adf_cb->adf_errcb.ad_errcode, (CL_SYS_ERR *)0, 
	...
	*err_code = E_DM9CAF_ONLINE_DUPCHECK;
    }

    if (dupchk_rcb)
    {
        localstat = dm2t_close(dupchk_rcb, DM2T_NOPURGE, &localerr);
	if (localstat)
	{
	    /* FIX ME - ule_format */
	}
    ...
    if (localstat)
    {
	ule_format(dmt_cb.error.err_code, 0, ULE_LOG, NULL, 0, 0, 
			0, err_code, 0);
	status = localstat;
    }
	...
	TRdisplay("%@ do_online_dupcheck: deleted %~t (%d,%d)\n",
		sizeof(DB_TEMP_TAB_NAME), DB_TEMP_TAB_NAME,
		dupchk_tabid->db_tab_base, dupchk_tabid->db_tab_index);
...
i4		*err_code)
{
	...
	for (;;)
	{
	    status = dm0j_read(jnl, (PTR *)&record, &rec_len, err_code);
            if (status != E_DB_OK)
            {
                if (*err_code == E_DM0055_NONEXT)
                {
    ...
    {
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM9CAE_FIND_COMPENSATED;
    }
    return(status);
}
...
i4		*err_code)
{
    ...
    status = dm0m_allocate( size, DM0M_ZERO, (i4) MISC_CB,
			(i4) MISC_ASCII_ID, (char *) 0,
			(DM_OBJECT **) &rnl->rnl_btree_xmap, err_code);
    if ( status == E_DB_OK )
    {
...
i4		*err_code)
{
    ...
    */
    if ( sp && m->mx_part_def )
    {
	ADF_CB	*adf_cb = (ADF_CB *)(m->mx_rcb->rcb_adf_cb);
	u_i2	i;
	...
	if (status = adt_whichpart_no(adf_cb, m->mx_part_def, 
			log_row, (u_i2 *)&i))
	    *err_code = adf_cb->adf_errcb.ad_errcode;
	*partno = (i4)i;
    }
    return(status);
}
...
i4			*err_code)
{
	...
	status = dm2u_create(dcb, xcb, (DB_TAB_NAME *)newparttab_name, 
		...
		(DM_DATA *)NULL, (i4)0, (i4)0,
		(DMU_FROM_PATH_ENTRY *)NULL, (DM_DATA *)0,
		0, 0, (f8*)NULL, 0, dmu_part_def, partno, &errcb);
    ...
    for (;;)
    {
        ...
        for  ( tp = m->mx_tpcb_next; tp; tp = tp->tpcb_next)
        {
            ...
            */
            ...
            {
                status = dm2t_unfix_tcb(&pt, DM2T_VERIFY,
                        xcb->xcb_lk_id, m->mx_log_id, err_code);
                if (status != E_DB_OK)
                    break;
            }
            pt = (DMP_TCB *)0;
        ...
        status = dm2t_close(online_rcb, DM2T_NOPURGE, err_code);
        if (status != E_DB_OK)
            break;
    ...
    if (online_rcb)
    {
        if (pt && (pt != online_rcb->rcb_tcb_ptr))
            local_status = dm2t_unfix_tcb(&pt, DM2T_VERIFY,
                        xcb->xcb_lk_id, m->mx_log_id, &local_error);
