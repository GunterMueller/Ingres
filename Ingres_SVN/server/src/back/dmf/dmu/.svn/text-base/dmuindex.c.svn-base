...
dmu_index(DMU_CB    *dmu_cb)
{
	...
	*/
	...
	{
	    ule_format(dmu->error.err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char * )0,
		(i4)0, (i4 *)0, &local_error, 0);
	    dmu->error.err_code = E_DM0092_ERROR_INDEXING_TABLE;
	}
...
PTR		ix_cb)
{
    ...
    err_data = &dmu->error.err_data;
    err_code = &dmu->error.err_code;
	...
	*/
	...
	for (i = 0; i < chr_count; i++)
	{
		...
		switch (indx_cb->indxcb_structure)
		{
			...
			break;
		    default:
		        *err_code = E_DM000E_BAD_CHAR_VALUE;
		}
		break;
	    ...
	    case DMU_COMPRESSED:
		indx_cb->indxcb_compressed = (chr[i].char_value == DMU_C_ON);
		break;
	...
	indx_cb->indxcb_kcount = dmu->dmu_key_array.ptr_in_count;
	if ((indx_cb->indxcb_kcount == 0) || (indx_cb->indxcb_key == NULL))
	{
	    *err_code = E_DM002A_BAD_PARAMETER;
	    break;
	}
	...
	indx_cb->indxcb_acount = dmu->dmu_attr_array.ptr_in_count;
	for (i = 0; i < indx_cb->indxcb_acount; i++)
	{
	    ...
	    dmu->dmu_location.data_in_size >= sizeof(DB_LOC_NAME))
	{
		...
		*err_code = E_DM0071_LOCATIONS_TOO_MANY;
		...
		for (j = 0; j < i; j++)
		{
		    ...
		    {
			*err_code = E_DM001E_DUP_LOCATION_NAME;
			*err_data = i;
			break;
		    }	    	    
		}
	...
	else
	{
		...
		*err_code = E_DM001F_LOCATION_LIST_ERROR;
	}
