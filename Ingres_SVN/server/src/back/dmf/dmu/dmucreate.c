...
**
**         <dmu_attr_array> entries are of type DMU_ATTR_ENTRY and 
...
**         attr_flags                       Must be zero or DMU_F_NDEFAULT.
**
...
dmu_create(DMU_CB        *dmu_cb)
{
    ...
    i4		    gateway = 0;
    i4		    gwupdate = 0;
    i4		    notUserDroppable = 0;
    i4		    statementLevelUnique = 0;
    i4		    persistsOverModifies = 0;
    i4		    systemGenerated = 0;
    i4		    supportsConstraint = 0;
    i4		    notUnique = 0;
    i4		    rowSecAudit = 0;
    i4	    	    allocation = 0;
    i4	    	    extend = 0;
		...
		dmu->dmu_char_array.data_in_size)
	{
		...
		case DMU_STRUCTURE:
			...
			dmu->error.err_code = E_DM000E_BAD_CHAR_VALUE;
			return (E_DB_ERROR);
		    }		    
		...
		case DMU_PAGE_SIZE:
		    ...
		    {
			dmu->error.err_data = i;
			dmu->error.err_code = E_DM0157_NO_BMCACHE_BUFFERS;
			return (E_DB_ERROR);
		    }		    
		    else
		    {
		...
		case DMU_SYS_MAINTAINED:
		    sys_maintained = (char_entry[i].char_value == DMU_C_ON);
		    break;
		...
		case DMU_JOURNALED:
		    journaled = char_entry[i].char_value == DMU_C_ON;
		    break;
		...
		case DMU_NOT_DROPPABLE:
		    notUserDroppable = (char_entry[i].char_value == DMU_C_ON);
		    break;

		case DMU_STATEMENT_LEVEL_UNIQUE:
		    statementLevelUnique = (char_entry[i].char_value == DMU_C_ON);
		    break;

		case DMU_PERSISTS_OVER_MODIFIES:
		    persistsOverModifies = (char_entry[i].char_value == DMU_C_ON);
		    break;

		case DMU_SYSTEM_GENERATED:
		    systemGenerated = (char_entry[i].char_value == DMU_C_ON);
		    break;

		case DMU_SUPPORTS_CONSTRAINT:
		    supportsConstraint = (char_entry[i].char_value == DMU_C_ON);
		    break;

		case DMU_NOT_UNIQUE:
		    notUnique = (char_entry[i].char_value == DMU_C_ON);
		    break;

		case DMU_ROW_SEC_AUDIT:
		    rowSecAudit = (char_entry[i].char_value == DMU_C_ON);
		    break;
		...
		case DMU_GLOBAL_INDEX:
		    GlobalIndex = char_entry[i].char_value == DMU_C_ON;
		    break;
		...
		default:
		    dmu->error.err_data = i;
		    dmu->error.err_code = E_DM000D_BAD_CHAR_ID;
		    return (E_DB_ERROR);
		}
	    }

	    if ((temporary || index || extension) && dmu->dmu_part_def != NULL)
		/* bad parameter error already set up */
		break;
	    ...
	    {
		/* recovery isn't currently supported for temporary tables. */
		for (i = 0; i < char_count; i++)
		{
		    switch (char_entry[i].char_id)
		    {
	...
	if ((dmu->dmu_table_name.db_tab_name[0] == 'i' &&
	    dmu->dmu_table_name.db_tab_name[1] == 'i') ||
	    (relstat & (DM2U_CATALOG | DM2U_CATALOG)))
	...
	** or will work only with page type V5
	** look for a bigger page size that will hold the row
	*/
	if ((status != E_DB_OK || page_type == TCB_PG_V5)
		&& used_default_page_size)
	{
	    ...
	    */
		...
		if (status != E_DB_OK || pgtype == TCB_PG_V5)
		    continue;
	...
	** rows spanning pages)
	*/
	if (page_type == TCB_PG_V5 && !duplicates && !view)
	{
	    ...
	    )
	{
	    ...
	    {
		error = E_DM0071_LOCATIONS_TOO_MANY;
		break;
	    }
	    bad_loc = FALSE;
	    error = E_DM001E_DUP_LOCATION_NAME;
    ...
    */
    ...
    if (journaled)
	relstat |= DM2U_JOURNAL;

    if (duplicates)
	relstat |= DM2U_DUPLICATES;
    if (sys_maintained)
	relstat |= DM2U_SYS_MAINTAINED;
    if (gateway)
	relstat |= DM2U_GATEWAY;
    if (gwupdate)
	relstat |= DM2U_GW_UPDT;
    ...
    */
    if (dmu->dmu_flags_mask & DMU_VGRANT_OK)
	relstat |= DM2U_VGRANT_OK;
    if (extension)
	relstat |= DM2U_TABLE_EXTENSION;
	...
	status = create_temporary_table(odcb, xcb,
			...
			relstat, structure, ntab_width, attr_count, 
			(DM2T_ATTR_ENTRY **)attr_entry, db_lockmode,
			allocation, extend, page_size, recovery,
			&dmu->dmu_qry_id, tbl_pri,
			&dmu->error);
    }
    else
    ...
    error = dmu->error.err_code;
    if (error > E_DM_INTERNAL)
    {
	ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char * )0,
	    (i4)0, (i4 *)0, &local_error, 0);
	dmu->error.err_code = error = E_DM0077_BAD_TABLE_CREATE;
    }
	...
	*/
	...
	if (status != E_DB_OK)
	{
		...
		ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char * )0,
		    (i4)0, (i4 *)0, &local_error, 0);
		dmu->error.err_code = E_DM0077_BAD_TABLE_CREATE;
		error = dmu->error.err_code;
	    }
	...
	if (dmu->dmu_gw_id == GW_IMA)
	{
	    ...
	    */
		...
		db_lockmode,
		&xcb->xcb_tran_id,
		&timestamp, &this_rcb, NULL, &error);

	    if (status == E_DB_OK)
	    {
	        status = dm2t_close(this_rcb, 0, &error);
	    }
	    if (status != E_DB_OK)
	    {
	        dmu->error.err_code = error;
...
**      qry_id                          Pointer to query id for view.
...
DB_ERROR	    *errcb)
{
    ...
    */
    ...
    characteristics[0].char_value =
		    (table_flags & DM2U_DUPLICATES) ? DMT_C_ON : DMT_C_OFF;
    ...
    characteristics[3].char_value =
	    	    (table_flags & DM2U_HAS_EXTENSIONS) ? DMT_C_ON : DMT_C_OFF;
    ...
    characteristics[6].char_value =
                  (table_flags & DM2U_SYS_MAINTAINED) ? DMT_C_ON : DMT_C_OFF;
	...
	STRUCT_ASSIGN_MACRO(dmt_cb.error, *errcb);
