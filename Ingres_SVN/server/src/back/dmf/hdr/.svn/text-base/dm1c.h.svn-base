...
FUNC_EXTERN VOID	dm1c_getaccessors(
				    i4  		relcomptype,
				    i4			relpgtype,
				    i4		relpgsize,
				    DMPP_ACC_PLV	**plv,
...
FUNC_EXTERN VOID	dm1c_badtid(DMP_RCB *rcb, DM_TID *tid);
...
**
** Description:
**          This section describes the two vectors that contain accessor
**        functions:-
**               1) the page level vector
**               2) the tuple level vector
...
/*
** Name: DMPP_ACC_TLV -- Tuple level vector accessor functions.
**
** Description:
**   This structure contains a list of access functions for Tuple Manipulation.
**   The 'tcb_acc_tlv' field in the table control block points to one of
**   these structures.
*/

struct _DMPP_ACC_TLV {
	DB_STATUS (*dmpp_compress)(
			DB_ATTS	        **atts_array,
			i4         atts_count,
			char            *rec,
			i4         rec_size,
