...
struct _DM0P_BSTAT
{
    ...
    u_i4	    bs_fix[BMCB_PTYPES+1];    /* Number of fix calls. */
    u_i4	    bs_hit[BMCB_PTYPES+1];    /* Page found is cache. */
    u_i4	    bs_iowait[BMCB_PTYPES+1]; /* Stall for I/O completion. */
    u_i4	    bs_mwait[BMCB_PTYPES+1];  /* Waits for page mutex. */
    u_i4       	    bs_fcwait[BMCB_PTYPES+1]; /* Stall for FC completion. */
    u_i4	    bs_check[BMCB_PTYPES+1];  /* Cached page still valid. */
    u_i4	    bs_refresh[BMCB_PTYPES+1];/* Cached page was invalid. */
    u_i4	    bs_unfix[BMCB_PTYPES+1];  /* Number of unfix calls. */
    u_i4	    bs_dirty[BMCB_PTYPES+1];  /* Dirty page unfixed. */
    u_i4	    bs_force[BMCB_PTYPES+1];  /* Force page for reuse */
    u_i4	    bs_toss[BMCB_PTYPES+1];   /* Toss page from cache */
    u_i4	    bs_reads[BMCB_PTYPES+1];  /* Number of reads. */
    u_i4	    bs_writes[BMCB_PTYPES+1]; /* Number of writes. */
    u_i4       	    bs_syncwr[BMCB_PTYPES+1]; /* single sync write counter */
    u_i4       	    bs_reclaim[BMCB_PTYPES+1];/* free buffer reclamations */
    u_i4       	    bs_replace[BMCB_PTYPES+1];/* Toss page to make room */
    u_i4	    bs_greads[BMCB_PTYPES+1]; /* Number of group reads. */
    u_i4	    bs_gwrites[BMCB_PTYPES+1];/* Number of group writes. */
    u_i4       	    bs_gsyncwr;             /* group sync write counter */
    u_i4	    bs_fwait;		    /* Number of free buffer waiters. */
    u_i4       	    bs_gwait;               /* Stall for busy group. */
    ...
    u_i4	    bs_wb_flushed;	    /* Number of WB pages flushed this cycle */
    u_i4	    bs_wb_gflushed;	    /* Number of WB groups flushed this cycle */
    u_i4	    bs_gw_pages;	    /* Pages written by GatherWrite */
    u_i4	    bs_gw_io;	    	    /* Physical I/O rqd for bs_gw_pages */
};

/*}
...

/*}
** Name: DMP_BM_COMMON - Buffer Manager Common Data.
**
** Description:
