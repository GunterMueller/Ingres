...
# define DCB_SEC_ID_INCREMENT 100
    ...
    char	    dcb_dbcmptlvl[4];	   /* DB Major version No */
...
*/
#define FCB_STATES "OPEN,IO,FLUSH,DEFERRED,MATERIALIZING,NOTINIT"
...
#define                 RCB_ASCII_ID        CV_C_CONST_MACRO('#', 'R', 'C', 'B')
    ...
    DMP_MISC	    *rcb_proj_misc;	    /* used for projection */
    DB_ATTS	    *rcb_proj_atts_ptr;     /* used for projection */
    DB_ATTS         **rcb_proj_data_atts;   /* used for projection */
    i4		    rcb_proj_relwid;        /* used for projection */
...
#define RCB_TMP_TID_MAX 20
    ...
    i4         	    rcb_seq_number;         /* Unique sequence number per
                                            ...
                                            ** transaction. */
    DMPP_PAGE	    *rcb_data_page_ptr;     /* Pointer to a saved fixed data
                                            ** page. */ 
    DMPP_PAGE	    *rcb_other_page_ptr;    /* Pointer to a saved fixed page. */
...
#define                 RCB_INTERNAL        1L
    ...
    BITFLD	    rcb_siAgents:1;	    /* Indexes are updated
					    ** by SI agents */
    BITFLD	    rcb_bits_free:25;	    /* reserved for future use */
...
#define                  RCB_K_ROW	   3L
    ...
    i4         	    rcb_lk_count;           /* Current number of page locks
                                            ** held. */
    i4         	    rcb_lk_mode;            /* Current lock mode. */
#define                  RCB_K_N            LK_N
                                            /* Means No Lock. */
...
#define			RCB_A_MODIFY	    6L
    ...
    i4         	    rcb_k_type;             /* Lock type specified at open
                                            ...
                                            ** either RCB_LK_TABLE or 
                                            ** RCB_LK_PAGE. */
    i4         	    rcb_k_mode;             /* Lock mode specified at open
                                            ** and not modified by lower
                                            ** level routines.  Must be
                                            ** RCB_LK_N, RCB_LK_IS, RCB_LK_IX,
                                            ** RCB_LK_S, or RCB_LK_X. */
...
#define			RCB_PREFET_AVAILABLE    0x00004000L	
					    ...
					    */
#define                 RCB_FLUSHED             0x00008000L             
                                            /*
                                            ** (ICL phil.p)
                                            ** Indicates that this thread has 
                                            ** flushed the cache of dirty
                                            ** pages, to guarantee consistent 
                                            ** data from modify and smart_disk
                                            ** access.
                                            */
#define                 RCB_RECOVER             0x00010000L            
                                            /* 
                                            ** (ICL phil.p)
                                            ** Indicates that the Exclusive   
                                            ** TCB lock - used to force
                                            ** consistency for smart_disk
                                            ** and modify - must be tidied
                                            ** if transaction abort occurs.
                                            */
...
LRESTART,ABORTING,200,IN_MINI,800,PREFET_PEND,PREFET_REQ,PREFET_AVAIL,\
FLUSHED,RECOVER,CSRR_LOCK,TABLE_HAD_RECS,DPAGE_HAD_RECS,LPAGE_HAD_RECS,\
...
#define                 RCB_P_GTE           3L
    ...
    DM_TID          rcb_fetchtid;           /* This is the identifier of the
					    ...
					    ** for cursors. Cleared by 
                                            ** get next. */
    i4              (*rcb_f_qual)();        /* This is the function needed to
                                            ** fully qualify the record 
                                            ** retrieved, to eliminate calls
                                            ** to DMF. */
					    /* WARNING! WARNING! WARNING!
					    ** The qualification function
					    ** return values below, must
					    ** match those specified in 
					    ** backhdr!hdr!dmrcb.h */
#define			RCB_F_NEXT	    1L
					    /* Row does not qualify, proceed
					    ** to the next row.*/
#define			RCB_F_RETURN	    2L
					    /* Row qualified */
#define			RCB_F_ABORT	    3L
					    /* A serious error was encountered,
					    ** abort the query */
#define			RCB_F_USER_ERROR    4L
					    /* A user error was encountered,
					    ** do not report the error and
					    ** return E_DM0148_USER_QUAL_ERROR
					    ** to QEF*/
    i4              *rcb_f_arg;             /* The argument list needed by 
                                            ** the qualifying function. */
...
#define			RCB_RAHEAD_INTR     0x04L /* for readahead rcbs only */	
    ...
    i4	    	    rcb_compress;	    /* Gives table compression type
					    ** This can be TCB_C_NONE for
					    ** uncompressed otherwise a
					    ** copy of relcomptype for the
					    ** relation. */
    PTR		    rcb_gateway_rsb;	    /* Gateway record control block -
					    ** used if this is an open gateway
					    ** table. */
    PTR		    rcb_adf_cb;		    /* Pointer to session's ADF cb.  */

					    /* For reposition to CURRENT pos  */
    DM_TID          rcb_repos_bid;          /* BTREE bid                      */
    LG_LSN          rcb_repos_lsn;          /* BTREE leaf lsn                 */
    DM_PAGENO       rcb_repos_nextleaf;
    i4         	    rcb_repos_clean_count;  /* BTREE leaf clean count         */
    char            *rcb_repos_key_ptr;     /* BTREE leaf key                 */
    DM_TID          rcb_repos_tid;          /* BTREE leaf entry tid           */


					    /* For reposition to FETCHED row  */
    LG_LSN          rcb_fet_lsn;            /* BTREE fetch lsn                */
    DM_PAGENO       rcb_fet_nextleaf;
    i4         	    rcb_fet_clean_count;    /* BTREE fetch clean count        */
    char            *rcb_fet_key_ptr;       /* BTREE fetched key              */
    DM_TID          rcb_fet_tidp;           /* BTREE fetched data tid (tidp)  */

					    /* For reposition to STARTING pos */
    LG_LSN          rcb_pos_lsn;            /* BTREE position lsn             */
    DM_PAGENO       rcb_pos_nextleaf;
    i4         	    rcb_pos_clean_count;    /* BTREE position clean count     */

    DM_TID          rcb_alloc_bid;          /* BTREE reserved entry bid       */
    LG_LSN          rcb_alloc_lsn;          /* BTREE reserved entry lsn       */
    DM_PAGENO       rcb_alloc_nextleaf;   
    i4         	    rcb_alloc_clean_count;  /* BTREE reserved clean count     */
    DM_TID          rcb_alloc_tidp;         /* BTREE reserved tidp            */
...
READONLY,FASTCOMMIT,MULTIPLE,RCP_RECOVERY,ROLLDB_RECOVERY,NO_GROUPREAD,DMCM"
    ...
    BITFLD		tbio_bits_free:27;	/* Reserved for future use. */
...
**                     |----- End of Fixed Portion -------| 
**                     | area containing an one byte      |
**                     | tcb_att_key entry for each       |
**                     | attribute.                       |
**                     |----------------------------------|
**                     | area containing a 24 byte        |
**                     | tcb_atts entry for each          |
**                     | attribute.                       |
**                     |----------------------------------|
**		       | area containing a list of        |
**		       | pointers to the data attributes. |
**                     | area containing a 8 byte         |
**                     | for each additional location     |
**                     | for a table                      |
**                     +----------------------------------+
**		       | list head for table extensions   |
**                     +----------------------------------+
**		       | area containing a list of        |
**		       | pointers to the key  attributes. |
**                     +----------------------------------+
...
#define                 TCB_REPTAB          1L
    BITFLD          tcb_bits_free:21;       /* Reserved for future use. */

    i2              tcb_index_type;         /* Used to indicate if any of 
                                            ** indices are unique. */
#define                 TCB_S_NONUNIQUE     0L
#define                 TCB_S_UNIQUE	    1L
    ...
    DB_ATTS         *tcb_atts_ptr;          /* Pointer to  variable portion of
					    ...
					    ** of them.
					    */
    DB_ATTS	    **tcb_data_atts;	    /* Pointer to variable portion of
					    ** this control block which contains
					    ** a list of pointers to attribute
					    ** descriptors used to make up
					    ** entire data record. The Nth
					    ** attribute in the tcb_atts_ptr
					    ** list is pointed to by the N-1st
					    ** pointer in tcb_data_atts (i.e.,
    ...
    DB_ATTS         **tcb_key_atts;         /* Pointer to variable portion of
                                            ** this control block which contains
                                            ** a list of pointers to attribute
                                            ** descriptors used to make up key.
                                            */
    ...
    /* Attribute info for compression: */
    i4              tcb_comp_atts_count;    /* Count of number of C & CHAR
                                            ** attributes in a table, used 
                                            ** for compressed records. */
    i4              tcb_comp_katts_count;   /* Adjustment count of worst-case
                                            ** compression overhead for columns
                                            ** which are in the index, used by
					    ** index compression routines to
					    ** compute worst-case maximum key
					    ** width. */

    /* Attribute info for LEAF pages: */
    DB_ATTS         **tcb_leafkeys;	    /* Array of LEAF key attribute ptrs */
    DB_ATTS         **tcb_leafatts;	    /* Array of LEAF attribute ptrs, 
					    ** key and non-key */
    i4              tcb_leafattcnt;	    /* Number of key and non-key 
					    ** LEAF atrributes. */
    ...
    DB_ATTS         **tcb_ixatts;	    /* Array of INDEX attribute ptrs */
    i4              tcb_ixattcnt;	    /* Number of INDEX atrributes. */
    ...
    DB_ATTS	    **tcb_rngatts;	    /* LEAF range atts */
    i4		    tcb_rngklen;	    /* LEAF range klen */
    i4		    tcb_rngattcnt;	    /* LEAF range att count */
...
#define			TCB_TAB_GATEWAY	    14	/* Gateway table */
    ...
    DMPP_ACC_TLV    *tcb_acc_tlv;	    /* MPF Tuple accessor routines */
...
/* 0x40 bit was freed, reused by TCB_CLUSTERED, now changed back to TCB_VBASE */
...
#define 		TCB_C_OLD	    2
#define			TCB_C_ISAMINDEX	    6 /* for put accessor interface,
...
#define SKIP_DELETED_ROW_MACRO(r, b, low_tran, addr_page_lsn)		\
	(rcb->rcb_lk_type != RCB_K_ROW					\
	|| !low_tran 							\
	|| low_tran == r->rcb_tran_id.db_low_tran			\
	|| !(LSN_GTE(addr_page_lsn, &r->rcb_oldest_lsn)) ? TRUE :	\
	dmxe_xn_active(low_tran) ? FALSE : TRUE)

#define DUPCHECK_NEED_ROW_LOCK_MACRO(r, b, low_tran, addr_page_lsn)	\
	(rcb->rcb_lk_type != RCB_K_ROW					\
	|| !low_tran 							\
	|| low_tran == r->rcb_tran_id.db_low_tran			\
	|| !(LSN_GTE(addr_page_lsn, &r->rcb_oldest_lsn)) ? FALSE :	\
	dmxe_xn_active(low_tran) ? TRUE : FALSE)
