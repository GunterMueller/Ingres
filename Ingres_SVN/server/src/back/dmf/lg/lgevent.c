...
CL_ERR_DESC	    *sys_err;
{
    ...
    status = LG_event(flag, (LG_ID *) &lx_id, event_mask, &lxb,
	...
	*/
	if (status == E_CS0008_INTERRUPTED)
	{
	    ...
	    if (lg_status = LG_cancel(flag, (LG_ID *) &lx_id))
		status = lg_status;
...
CL_ERR_DESC	    *sys_err)
{
    ...
    {
	ule_format(E_DMA451_LG_EVENT_BAD_ID, (CL_ERR_DESC *)0,
    ...
    {
	ule_format(E_DMA452_LG_EVENT_BAD_XACT, (CL_ERR_DESC *)0,
...
i4		    status)
{
    ...
    SIZE_TYPE	    end_offset;
    i4		    resumed = 0;
    i4		    waiting_for_buffers = (lwq == &lgd->lgd_wait_buffer);
	...
	/*
	** If resuming for free buffers, only resume
	** as many threads as there are free buffers.
	**
	** This avoids a Thundering Herd of threads
	** being resumed only to put themselves right
	** back on this queue when they find no free
	** buffers.
	*/
	if ( waiting_for_buffers && ++resumed >= lfb->lfb_fq_count )
	{
    ...
    LG_ID	    *lx_id)
{
    ...
    if (lx_id->id_id == 0 || (i4)lx_id->id_id > lgd->lgd_lxbb_count)
    {
	ule_format(E_DMA453_LG_CANCEL_BAD_ID, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
		    0, lx_id->id_id, 0, lgd->lgd_lxbb_count);
	return (LG_BADPARAM);
    }
	...
	lxb->lxb_id.id_instance != lx_id->id_instance)
    {
	ule_format(E_DMA454_LG_CANCEL_BAD_XACT, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
		    0, lxb->lxb_type, 0, lxb->lxb_id.id_instance,
		    0, lx_id->id_instance);
	return (LG_BADPARAM);
