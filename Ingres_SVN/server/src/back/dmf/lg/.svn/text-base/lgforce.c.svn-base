...
CL_ERR_DESC	    *sys_err)
{
    ...
    do 
    {
	status = LG_force(flag, (LG_ID *) &lx_id, lsn, &lfb,
...
CL_ERR_DESC	    *sys_err)
{
    ...
    {
	ule_format(E_DMA418_LGFORCE_BAD_ID, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &loc_err_code, 2,
    ...
    {
	ule_format(E_DMA419_LGFORCE_BAD_XACT, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &loc_err_code, 3,
    ...
    */
	    ...
	    (lxb->lxb_status & LXB_SHARED) == 0 )
	{
	    ule_format(E_DMA494_LG_INVALID_SHR_TXN, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &loc_err_code, 6,
			0, lxb->lxb_type,
			0, LXB_TYPE,
			0, *(u_i4*)&lxb->lxb_id,
			0, lxb->lxb_status,
...
LG_write_headers( LG_ID *lx_id, register LFB *lfb, STATUS *async_status )
{
	...
	*/
	...
	if (lxb)
	{
	    ...
	    LG_suspend(lxb, &lgd->lgd_wait_buffer, async_status);
