...
CL_ERR_DESC	    *sys_err)
{
	...
	{
	    ule_format(E_DMA41D_LGSHOW_STAT_SIZE, (CL_ERR_DESC *)0,
	...
	*length = sizeof(lgd->lgd_stat);
	...
	{
	    ule_format(E_DMA41E_LGSHOW_HDR_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
	...
	*length = sizeof(LG_HEADER);
	...
	{
	    ule_format(E_DMA41F_LGSHOW_LGLA_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
	...
	*(LG_LA *)item = lgd->lgd_local_lfb.lfb_archive_window_prevcp;
	...
	if (l_item < sizeof(i4))
	{
	    ule_format(E_DMA420_LGSHOW_STS_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
	...
	*(i4 *)item = lgd->lgd_status;
	...
	if (l_item < sizeof(i4))
	{
	    ule_format(E_DMA421_LGSHOW_BCNT_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
			0, l_item, 0, sizeof(i4));
	    return (LG_BADPARAM);
	}
	...
	*/
	if (l_item < sizeof(i4))
	{
	    ule_format(E_DMA422_LGSHOW_NODEID_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
			0, l_item, 0, sizeof(i4));
	    return (LG_BADPARAM);
	}
	*(i4 *)item = CXnode_number(NULL);
	...
	{
	    ule_format(E_DMA423_LGSHOW_DB_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
	...
	*/
	...
	db = (LG_DATABASE *)item;
	MECOPY_CONST_MACRO((PTR)&db->db_id, sizeof(db_id), (PTR)&db_id);
	...
	{
	    ule_format(E_DMA425_LGSHOW_BAD_DB, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 4,
	...
	*length = 0;
	...
	MECOPY_CONST_MACRO(item, sizeof(db_id), (PTR)&db_id);
	db = (LG_DATABASE *)item;

	ldbb_table = (SIZE_TYPE *)LGK_PTR_FROM_OFFSET(lgd->lgd_ldbb_table);
	ldb  = (LDB *)LGK_PTR_FROM_OFFSET(ldbb_table[db_id.id_id]);
	
	if (ldb->ldb_id.id_id == db_id.id_id && 
	...
	*length -= i * sizeof (LG_TRANSACTION);
	...
	{
	    ule_format(E_DMA427_LGSHOW_XACT_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
	...
	*length = 0;
	...
	{
	    ule_format(E_DMA428_LGSHOW_BAD_ID, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
	...
	*length = 0;
	tr = (LG_TRANSACTION *) item;
	MECOPY_VAR_MACRO((char *) &tr->tr_id, sizeof(LG_ID), (char *) &lg_id);

	/*	Check the lg_id. */

	if (lg_id.id_id == 0 || (i4) lg_id.id_id > lgd->lgd_lxbb_count)
	{
	    ule_format(E_DMA428_LGSHOW_BAD_ID, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
			0, lg_id.id_id, 0, lgd->lgd_lxbb_count,
			0, "LG_S_FORCE_ABORT");
	    return (LG_BADPARAM);
	}

	lxbb_table = (SIZE_TYPE *)LGK_PTR_FROM_OFFSET(lgd->lgd_lxbb_table);
	lxb = (LXB *)LGK_PTR_FROM_OFFSET(lxbb_table[lg_id.id_id]);
	...
	*length -= i * sizeof (LG_PROCESS);
	...
	{
	    ule_format(E_DMA429_LGSHOW_PROC_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
	...
	*length = 0;
	...
	MECOPY_VAR_MACRO((PTR)item, sizeof(lpb_id),(PTR) &lpb_id);
	pr = (LG_PROCESS *) item;

	lpbb_table = (SIZE_TYPE *)LGK_PTR_FROM_OFFSET(lgd->lgd_lpbb_table);
	lpb  = (LPB *)LGK_PTR_FROM_OFFSET(lpbb_table[lpb_id.id_id]);
	
	if (lpb->lpb_type == LPB_TYPE &&
	    lpb->lpb_id.id_id == lpb_id.id_id && 
		lpb->lpb_id.id_instance == lpb_id.id_instance)
	{
	    pr->pr_id = *(i4 *)&lpb->lpb_id;
	    pr->pr_pid = lpb->lpb_pid;
	    pr->pr_bmid = 0;
	...
	*length = sizeof(LG_LA);
	...
	if (l_item != sizeof(i4))
	{
	    ule_format(E_DMA42A_LGSHOW_LDBS_SIZE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
			0, l_item, 0, sizeof(i4));
	    return (LG_BADPARAM);
	}
	...
	*length = 0;
	...
	lxbb_table = (SIZE_TYPE *)LGK_PTR_FROM_OFFSET(lgd->lgd_lxbb_table);
	nlxb = (LXB *)LGK_PTR_FROM_OFFSET(lxbb_table[lg_id.id_id]);
	     ...
	     lxb_offset != end_offset; lxb_offset = lxb->lxb_next)
	{
	    ...
	    if (lxb->lxb_type == LXB_TYPE &&
	        DB_DIS_TRAN_ID_EQL_MACRO(lxb->lxb_dis_tran_id ,
					    tr->tr_dis_id) &&
		(lxb->lxb_id.id_id != lg_id.id_id ||
		    lxb->lxb_id.id_instance != lg_id.id_instance)
		)
	    {
		lpd = (LPD *)LGK_PTR_FROM_OFFSET(lxb->lxb_lpd);
		ldb = (LDB *)LGK_PTR_FROM_OFFSET(lpd->lpd_ldb);
	...
	*length = sizeof(LG_MUTEX);
	...
	if ((CSs_semaphore(CS_INIT_SEM_STATS, &lgd->lgd_wait_buffer.lwq_mutex, 
				lstat, sizeof(*lstat))) == OK)
	{
	    mstat->count++;
	    lstat++;
	...
	*/
	...
	if (lg_id.id_id == 0 || (i4)lg_id.id_id > lgd->lgd_lpdb_count)
	{
	    ule_format(E_DMA424_LGSHOW_BAD_ID, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
			0, lg_id.id_id, 0, lgd->lgd_lpdb_count,
			0, (flag == LG_S_XA_PREPARE) 	 ? "LG_S_XA_PREPARE" :
			   (flag == LG_S_XA_COMMIT)  	 ? "LG_S_XA_COMMIT"  :
			   (flag == LG_S_XA_COMMIT_1PC)  ? "LG_S_XA_COMMIT_1PC"  :
			   "LG_S_XA_ROLLBACK");
	...
	lpdb_table = (SIZE_TYPE *)LGK_PTR_FROM_OFFSET(lgd->lgd_lpdb_table);
	lpd = (LPD*)LGK_PTR_FROM_OFFSET(lpdb_table[lg_id.id_id]);

	if (lpd->lpd_type != LPD_TYPE ||
	    lpd->lpd_id.id_instance != lg_id.id_instance ||
...
format_lbb(LBB *lbb, LG_BUFFER *buf)
{
	...
	buf->buf_assigned_owner = *(i4 *)&lxb->lxb_id;	
    }
...
LG_DATABASE	*db)
{
    db->db_id = *(i4 *)&ldb->ldb_id;
    db->db_database_id = *(i4 *)&ldb->ldb_database_id;
...
LG_TRANSACTION	*tr)
{
    ...
    tr->tr_id = *(u_i4 *)&lxb->lxb_id;
    tr->tr_db_id = *(i4 *)&ldb->ldb_id;
    tr->tr_pr_id = *(i4 *)&lpb->lpb_id;
    tr->tr_sess_id = *(CS_SID *)&lxb->lxb_cpid.sid;
    tr->tr_lpd_id = *(i4 *)&lpd->lpd_id;
	     ...
	     lxb->lxb_shared_lxb )
	{
	    ...
	    tr->tr_shared_id = *(u_i4*)&slxb->lxb_id;
	}
    }
...
i4	    data)
{
    ...
    ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, &err_code, 1, 0, flag);
    ule_format(E_DM900E_BAD_LOG_END, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &err_code, 1, 0, data);
