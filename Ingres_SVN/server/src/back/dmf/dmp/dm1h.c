...
#define             NO_SPACE_FOUND  0xffffffffL
    ...
    *err_code = E_DB_OK;
	...
	if (s != E_DB_OK)
		...
		datapage = *data = savepage;
                if (s != E_DB_OK)
		{
		    /* Also unfix the saved page */
		    (void) dm0p_unfix_page(r, DM0P_UNFIX, data, err_code);
                    return (s);
		}
		datapage = *data;
            }   
	...
	*/
	...
	if (*data == savepage) savepage=NULL;
    ...
    if (savepage)
        s = dm0p_unfix_page(r, DM0P_UNFIX, &savepage, &local_err_code);
