...
*/
static VOID	log_error(DB_STATUS status, DB_STATUS *err_code);

/*{
...
i4         *err_code)
{
	...
	status = dm1xreserve(mct, mct->mct_buckets, err_code);
	if (status == E_DB_OK)
	{
	    /*  Get the first page. */
    ...
    if (status != E_DB_OK)
    {
	log_error(E_DM924E_DM1H_BBEGIN, err_code);
	return (status);
    }
...
i4         *err_code)
{
    ...
    do
    {
        ...
        status = dm1hbput(mct, mct->mct_buckets, 0, 0, 0, err_code);
        if ( status != E_DB_OK )
	    break;
    ...
    */
    ...
    log_error(E_DM924F_DM1H_BEND, err_code); 
...
i4         *err_code)
{
	...
	{
	    log_error(E_DM9250_DM1H_BPUT, err_code);
	    return (status);
	}
    }
