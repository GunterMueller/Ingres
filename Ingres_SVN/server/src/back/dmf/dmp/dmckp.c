    ...
    i4	*err_code )
{
    ...
    */
    ...
    if (s == NULL || *s == EOS)
    {
        status = NMloc(UTILITY, FILENAME, "cktmpl.def", 
    ...
    */
    ...
    if ( status != OK )
    {
	ule_format( status, (CL_ERR_DESC *)0, ULE_LOG, NULL, 0, 0, 0,
	            err_code, 0);
        ule_format( E_DM1155_CPP_TMPL_MISSING, (CL_ERR_DESC *)0,
    ...
    char	*command )
{
    ...
    if ( cl_status != OK )
    {
	ule_format(cl_status, &sys_err, ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	return (E_DB_ERROR);
    }
