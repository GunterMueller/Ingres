...
static VOID	log_di_error(
			    STATUS	    di_retcode,
			    DB_STATUS	    error,
			    CL_SYS_ERR	    *sys_err,
			    DMP_DCB	    *dcb);

static VOID	log_di_dump_error(
			    STATUS	    di_retcode,
			    DB_STATUS	    error,
			    CL_SYS_ERR	    *sys_err,
			    DM0C_CNF	    *cnf,
...
i4		    *err_code)
{
	...
	*/
	...
	if (status != OK)
	{
	    log_di_error(status, E_DM9006_BAD_FILE_WRITE, &sys_err, 
                                  cnf->cnf_dcb);
	    *err_code = E_DM923B_CONFIG_CLOSE_ERROR;
	    return (E_DB_ERROR);
	}
    }
    ...
    */
    ...
    {
	ule_format(E_DM928D_NO_DUMP_LOCATION, (CL_SYS_ERR *)NULL,
	...
	*/
	...
	for (;;)
	{
	    ...
	    {
		status = dm2t_reclaim_tcb(cnf->cnf_lock_list, (i4)0, err_code);
		if (status == E_DB_OK)
		    continue;
...
#define			    RETURN_INFO	    0x04
	...
	*err_code = E_DM923A_CONFIG_OPEN_ERROR;
    ...
    /* Zero error code so we can tell if it has been set when break on error. */
    *err_code = 0;
	...
	for (;;)
	{
		...
		*err_code = E_DM923A_CONFIG_OPEN_ERROR;
                ...
                if (open_flag <= DM2F_A_WRITE)
		{
                ...
                {
                    *err_code = E_DM012A_CONFIG_NOT_FOUND;
                    if (flags & DM0C_TRYRFC)
                        state |= RETURN_INFO;
                }
                break;
	...
	*/
	...
	{
	    ule_format(E_DM928E_CONFIG_FORMAT_ERROR, 0,
	...
	*/
	...
	{
	    ule_format(E_DM9079_CONFIG_NAME_MISMATCH, 0,
	...
	if  (dsc_cnf_version != CNF_VCURRENT) 	    
	{
	    ...
	    */
		...
		while (dsc_cnf_version != CNF_VCURRENT)
		{
		    ...
		    {
			case CNF_V2 : 
			    status = dm0c_convert_to_CNF_V3(cnf, err_code); 
			    odesc = (DM0C_ODSC *)cnf->cnf_dsc;
			    dsc_cnf_version = odesc->dsc_cnf_version;
			    break;
			case CNF_V3 :
			    status = dm0c_convert_to_CNF_V4(cnf, err_code);
			    dsc_cnf_version = cnf->cnf_dsc->dsc_cnf_version;
			    break;
			case CNF_V4 :
			    status = dm0c_convert_to_CNF_V5(cnf, err_code);
			    dsc_cnf_version = cnf->cnf_dsc->dsc_cnf_version;
			    break;
			case CNF_V5 : 
			    status = dm0c_convert_to_CNF_V6(cnf, err_code); 
			    dsc_cnf_version = cnf->cnf_dsc->dsc_cnf_version;
			    break;
			default :
			    {
				...
				ule_format(E_DM928B_CONFIG_NOT_COMPATABLE, 0,
				    ...
				    &err_parm3, sizeof(i4), 
				    &err_parm4);
				*err_code = E_DM0113_DB_INCOMPATABLE;
				status = E_DB_ERROR;
			    }
			    break;
		    }
	...
	*/
	if ((flags & DM0C_PARTIAL) == 0)
		...
		*/
	    ...
	    {
		ule_format(E_DM923C_CONFIG_FORMAT_ERROR, 0, 
                           ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		break;
	    }
	}
	...
	*/
	    ...
	    && ! desc->dsc_status & DSC_VALID)
	{
	    ...
	    *(i4 *)desc->dsc_dbcmptlvl = *(i4 *)rdcnf->cnf_dsc->dsc_dbcmptlvl;
	    ...
	    status = dm0c_close(rdcnf, 0, err_code);
	}
...
#define             ID_SIZE             32
	...
	case DSC_TABLE_INCONST:
	    ...
	    break;
	case DSC_INCR_RFP_INCONS:
	    MEcopy ("INCREMENTAL RFP ", ID_SIZE, (PTR )id);
	    break;
    ...
    ule_format (E_DM9347_CONFIG_FILE_PATCHED, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0,
...
i4		    *err_code)
{
    ...
    */
	...
	if (status == DI_EXCEED_LIMIT)
	    ...
	    *err_code = E_DM928A_CONFIG_EXTEND_ERROR;
	return (E_DB_ERROR);
    }
...

/*{
** Name: log_di_error	- Log DI error and return CNF error.
**
** Description:
...
DMP_DCB		    *dcb)
{
    ...
    if (di_retcode)
	ule_format(di_retcode, sys_err, ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
...

/*{
** Name: log_di_dump_error	- Log DI error and return CNF error.
**
** Description:
...
i4		    *err_code)
{
    ...
    */
    ...
    {
	ule_format(E_DM004A_INTERNAL_ERROR, (CL_SYS_ERR *)NULL, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM928C_CONFIG_CONVERT_ERROR;
	return (E_DB_ERROR);
    }
    ...
    */
	...
	{
	    ule_format(*err_code, (CL_SYS_ERR *)NULL, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM928C_CONFIG_CONVERT_ERROR;
	}
	return (E_DB_ERROR);
    ...
    */
    ...
    {
	ule_format(E_DM936E_DM0C_GETENV, (CL_SYS_ERR *)NULL, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM936E_DM0C_GETENV;
        return (E_DB_ERROR);
    }

    STncpy( db_name, cnf->cnf_dcb->dcb_name.db_db_name, DB_MAXNAME );
    db_name[ DB_MAXNAME ] = '\0';
    STtrmwhite(db_name);
    status = LOingpath(ING_DMPDIR, db_name, LO_DMP, &loc);
    if (!status)
	LOtos(&loc, &path_str);
    if (status || *path_str == NULL)
    {
	ule_format(E_DM936F_DM0C_LOINGPATH, (CL_SYS_ERR *)NULL, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM936F_DM0C_LOINGPATH;
        return (E_DB_ERROR);
    }
...
i4		    *err_code)
{
    ...
    */
    ...
    MEcopy(old_desc->dsc_dbcmptlvl, 4, new_desc->dsc_dbcmptlvl);
    ...
    MEfill(DB_MAXNAME, ' ', (PTR)&new_desc->dsc_name);
    MEcopy(old_desc->dsc_name, DB_OLDMAXNAME, (PTR)&new_desc->dsc_name);

    MEfill(DB_MAXNAME, ' ', (PTR)&new_desc->dsc_owner);
    MEcopy(old_desc->dsc_owner, DB_OLDMAXNAME, (PTR)&new_desc->dsc_owner);

    MEfill(DB_MAXNAME, ' ', (PTR)&new_desc->dsc_collation);
    MEcopy (old_desc->dsc_collation, DB_OLDMAXNAME, new_desc->dsc_collation);
    ...
    */
	...
	MEfill(DB_MAXNAME, ' ', (PTR)&new_ext->ext_location.logical);
	...
	*/
	   ...
	   )
	{
	    ...
	    MEcopy("ii_work", sizeof("ii_work")-1,
		    new_ext->ext_location.logical.db_loc_name);
