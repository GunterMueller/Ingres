...
i4		    *err_code)
{
	...
	status = log_error(j, E_DM902A_BAD_JNL_CLOSE, &sys_err, err_code);
    }
    return(status);
}

...
**
** Inputs:
**      flag;                           Must be zero or DM0J_MERGE
...
i4		    *err_code)
{
    ...
    status = ule_format(E_DM9037_BAD_JNL_CREATE, &sys_err, 
    ...
    *err_code = E_DM9281_DM0J_CREATE_ERROR;
    return (E_DB_ERROR);
}

/*{
...
i4		    *err_code)
{
    ...
    log_error(0, E_DM9032_BAD_JNL_DELETE, &sys_err, err_code);
    return (E_DB_ERROR);
}

/*{
...
i4		    *err_code)
{
    ...
    /* Consistency check: DM0J_MRZ must be same as LG_MAX_RSZ */
    if (DM0J_MRZ != LG_MAX_RSZ)
    {
	TRdisplay("Consistency check for max jnl and log record sizes FAILED\n");
	return (E_DB_ERROR);
    }
    ...
    do
    {
	/*  Allocate a DM0J control block. */

	status = dm0m_allocate(sizeof(DM0J_CTX) + bksz + DM0J_MRZ,
	    0, DM0J_CB, DM0J_ASCII_ID, (char *)&dmf_svcb, (DM_OBJECT **) jctx, err_code);
	if (status != E_DB_OK)
	    break;
	...
	j->dm0j_page = (char *)j->dm0j_record + DM0J_MRZ;
	...
	if (status != OK)
	{
		...
		*err_code = E_DM9034_BAD_JNL_NOT_FOUND;
		...
		ule_format(status, (CL_ERR_DESC *)&sys_err, ULE_LOG , NULL, 
			(char * )0, 0L, (i4 *)0, 
			err_code, 0);
		*err_code = E_DM902C_BAD_JNL_OPEN;
	    }
...
i4		    *err_code)
{
    ...
    */
    ...
    {
	status = dm0j_backread(j, record, l_record, err_code);
	return (status);
    }
	...
	if (status != OK)
	{
		...
		*err_code = E_DM0055_NONEXT;
	    ...
	    return (log_error(j, E_DM902D_BAD_JNL_READ, &sys_err, err_code));
	}
	...
	return (log_error(j, E_DM9030_BAD_JNL_FORMAT, 0, err_code));
    ...
    if (length > DM0J_MRZ)
    {
	ule_format(E_DM9C06_JNL_BLOCK_INFO, (CL_ERR_DESC *)0, ULE_LOG , NULL, 
			(char * )0, 0L, (i4 *)0, 
			err_code, 5,
			0, j->dm0j_sequence,
			0, j->dm0j_bytes_left,
	...
	return (log_error(j, E_DM9031_BAD_JNL_LENGTH, 0, err_code));
    }
...
i4		    *err_code)
{
	...
	*/
    ...
    return (log_error(j, E_DM902E_BAD_JNL_WRITE, &sys_err, err_code));
}

/*{
...
i4		    *err_code)
{
	...
	*sequence = j->dm0j_sequence;
    ...
    return (log_error(j, E_DM902F_BAD_JNL_UPDATE, &sys_err, err_code));
}

/*{
...
i4		    *err_code)
{
    ...
    return (log_error(j, E_DM9035_BAD_JNL_TRUNCATE, &sys_err, err_code));
}

/*{
** Name: log_error	- DM0J error logging routine.
**
** Description:
...
DB_STATUS	    *err_code)
{
	...
	ule_format(E_DM9C00_JNL_FILE_INFO, sys_err, ULE_LOG, NULL, 0, 0, 0,
			err_code, 5,
			sizeof(file), &file,
	...
	status = ule_format(dmf_error, sys_err, ULE_LOG, NULL, 0, 0, 0, 
	    err_code, 3, 
	...
	{
	    *err_code = map_error[i].error_jnl;
	    break;
	}
    ...
    i4	*err_code)
{
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ...
	    /* Create the file */
	    status = get_next_jnl(cnf, &cur_jnl, err_code);
	    if (status != E_DB_OK)
		return  (status);
	...
	*/
	...
	status = dm0j_close(&cur_jnl, err_code);
	if (status != E_DB_OK)
	{
	     log_error(cur_jnl, E_DM902A_BAD_JNL_CLOSE, (CL_ERR_DESC *)0, 
		err_code);
	     return(status);
	}
	...
	*/
	...
	rec_array_size = numnodes * DM0J_MRZ;
    	...
    	for ( node = 0; node < numnodes; node++ )
	{
		...
		*err_code = E_DB_OK;
		while ( ! done )
		{
			...
			(DB_TAB_ID *)0, &node_array[i].cur_jnl, err_code);
		    if (status != E_DB_OK)
		    {
			log_error((DM0J_CTX *)0, E_DM902C_BAD_JNL_OPEN, 
				(CL_ERR_DESC *)0, err_code);
...

	...
	*/
	...
	status = dm0j_update(cur_jnl, &sequence, err_code);
	if (status != E_DB_OK)
	{
	    log_error(cur_jnl, E_DM902A_BAD_JNL_CLOSE, (CL_ERR_DESC *)0, 
		err_code);
    ...
    */
	...
	local_status = dm0j_close(&cur_jnl, &local_err_code);
    }

    for (i = 0; i < DM_CNODE_MAX; i++)
    {
	if (node_array[i].cur_jnl != 0)
	    local_status = dm0j_close(&node_array[i].cur_jnl, &local_err_code);
    }
    if (created == TRUE)
	local_status = dm0j_delete(DM0J_MERGE, (char *)jpath, 
			   l_jpath, fil_seq, (DB_TAB_ID *)0, &local_err_code);
	...
	*/
	...
	status = dm0j_create(DM0J_MERGE, jpath, l_jpath,
		...
		config->cnf_jnl->jnl_bksz, config->cnf_jnl->jnl_blkcnt, 
                -1, (DB_TAB_ID *)0, err_code);

	/*  Handle error creating journal file. */

	if (status != OK)
	{
	    log_error(0, E_DM902B_BAD_JNL_CREATE, &sys_err, err_code);
	    return(E_DB_ERROR);
	}
	...
	if (status != E_DB_OK)
	{
	    log_error(0, E_DM902C_BAD_JNL_OPEN, &sys_err, err_code);
	    return(E_DB_ERROR);
	}
...
i4		    *err_code)
{
    ...
    */
    amount_copied = 0;
    buf_ptr = j->dm0j_record + DM0J_MRZ;
	...
	*/
	amount = j->dm0j_bytes_left;
	if ((amount_copied + amount) > DM0J_MRZ)
	{
	    ule_format(E_DM9C06_JNL_BLOCK_INFO, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 5,
		0, j->dm0j_sequence, 0, j->dm0j_bytes_left,
		0, (char *)j->dm0j_next_byte - (char *)j->dm0j_page,
		0, amount_copied + amount, 0, DM0J_MRZ);
	    return (log_error(j, E_DM9031_BAD_JNL_LENGTH, 0, err_code));
	}
	...
	*/
    ...
    if (status != OK)
    {
	return (log_error(j, E_DM902D_BAD_JNL_READ, (CL_ERR_DESC*)0, err_code));
    }
