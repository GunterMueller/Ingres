    ...
    i4		    *err_code)
{
    ...
    adjusted_size = (size + (dm_obj_align - 1)) & ~(dm_obj_align - 1);
    ...
    */
	...
	if ( status )
	{
	    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, &loc_err_code, 0);
	    *err_code = E_DM923D_DM0M_NOMORE;
	    if ( class & DM0M_LONGTERM )
...
#ifdef xDEBUG
    TRdisplay("dm0m_allocate: Allocated %d (%d) bytes at %x for %.4s/%x (%x)\n",
		size, adjusted_size, obj, (char *)&tag, tag, owner);
#endif
    ...
    i4		*err_code)
{
    ...
    */
	...
	if (status != OK)
	{
	    ...
	    {
		ule_format(scf_cb.scf_error.err_code, 0, ULE_LOG, NULL, 0, 0, 0, 
		    &loc_err_code, 0);
		ule_format(E_DM9425_MEMORY_ALLOCATE, 0, ULE_LOG, NULL, 0, 0, 0, 
		    &loc_err_code, 1, sizeof(adjusted_size), &adjusted_size);
		*err_code = E_DM9429_EXPAND_POOL;
		return (E_DB_ERROR);
	    }
	}
...
#ifdef xDEBUG
    TRdisplay("dm0m_deallocate: Freeing %d bytes at %x for %.4s/%x (%x)\n",
	    obj->obj_size, obj, (char *)&obj->obj_tag, obj->obj_tag,  obj->obj_owner);
    dm0m_verify(DM0M_POOL_CHECK);
#endif
...
*/
DB_STATUS
dm0m_destroy(DML_SCB *scb, i4 *err_code)
{
    DM_SVCB	        *svcb = dmf_svcb;
    DM_OBJECT		*pool;
...
dm0m_pgalloc(i4 page_size)
{
    ...
    if ( (mem = MEreqmem(0, page_size, FALSE, &cl_stat)) == NULL )
	ule_format(E_DM9425_MEMORY_ALLOCATE, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, (char *)0, (i4)0, (i4 *)0, &err_code, 1,
		0, page_size);
    return(mem);
...
dm0m_pad_error(DM_OBJECT *obj, char *padchar)
{
	...
	"DMF pad error %x (%x %x %x %d %d) for (%.4s/%x)\n",
