...
i4         *err_code)
{
    STATUS          status;
    i4		    num_alloc;
    STATUS	    local_err_code;
    
    /*	Allocate build memory. */

    status = dm1xstart(mct, DM1X_OVFL, err_code);
    if (status != E_DB_OK)
	return (status);
	...
	*/
	...
	mct->mct_startmain = 0;
	status = dm1xreserve(mct, mct->mct_kperpage, err_code);
	if (status != E_DB_OK)
	    break;

	/*  Get the first page. */

	status = dm1xnewpage(mct, -1, &mct->mct_curdata, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    log_error(E_DM9251_DM1I_BADD, err_code);
    return (status);
}

/*{
...
i4         *err_code)
{
	...
	*/
	...
	status = dm1xnewpage(mct, 0, &mct->mct_curovfl, err_code);
	if (status != E_DB_OK)
	    break;
	    ...
	    */
		...
		if (seg_hdr_ptr && seg_hdr_ptr->seg_next)
		{
			...
			status = dm1x_get_segs(mct, seg_hdr_ptr, rec,
			    mct->mct_segbuf, err_code);
			rec = mct->mct_segbuf;
		    }
		    else
		    {
			status = dm1x_get_segs(mct, seg_hdr_ptr, rec, record, err_code);
			rec = record;
		    }
		}
		...
		if (status == E_DB_OK)
		{
		    ...
		    ** and not the TCB.
		    */
			status = (*mct->mct_acc_tlv->dmpp_uncompress)(
			    mct->mct_data_atts, (i4)mct->mct_relatts,
			    rec, record, record_size, &uncompressed_length, 
			    mct->mct_compressed, &tid,
	...
	*/
		...
		mct->mct_curovfl), level++)
	{
		...
		** Now put key on the current index page.  If there is no room,
		** go to next index page.
		** Even if (DMPP_VPT_PAGE_HAS_SEGMENTS(mct->mct_page_type))
	...
	*/
	...
	if (start_free < end_free)
	{
		...
		mct->mct_startmain + mct->mct_kperpage - 1, err_code);
	    if (status != E_DB_OK)
		break;
	}
    ...
    (VOID) dm1xfinish(mct, DM1X_CLEANUP, err_code);
    log_error(E_DM9252_DM1I_BEND, err_code);
    return (status);
}

/*{
...
i4         *err_code)
{
	...
	*/
	status = (*mct->mct_acc_tlv->dmpp_compress)(mct->mct_data_atts, 
		mct->mct_relatts, record, record_size, rec, &rec_size);
	if (status != E_DB_OK)
	{
    ...
    while ((status = dm1xbput(mct, mct->mct_curdata, rec, rec_size,
			mct->mct_compressed, mct->mct_db_fill, 0,
	...
	if (dup == 0)
	{
	    ...
	    */
		...
		status = dm1xnextpage(mct, &mct->mct_startmain, err_code);
		if (status != E_DB_OK)
		    break;
		...
		if (status == E_DB_OK)
		    status = dm1xreserve(mct,
			mct->mct_startmain + mct->mct_kperpage, err_code);
		if (status == E_DB_OK)
	...
	*/
	if (status == E_DB_WARN)
	{
	    ...
	    */
	    status = dm1xnextpage( mct, &next_page, err_code );
	    if ( status != E_DB_OK )
		break;
    ...
    if (status != E_DB_OK)
	log_error(E_DM9253_DM1I_BOTTOM, err_code);
    return (status); 
}

static VOID
