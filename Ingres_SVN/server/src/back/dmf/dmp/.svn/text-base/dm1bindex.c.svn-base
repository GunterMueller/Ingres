    ...
    i4             *err_code)
{
    ...
    **
    ** If TCB_PG_V1, duplicates on overflow pages
    **
    ...
    */
	...
	{
	    next = DM1B_VPT_GET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype, leafpage);
	}
	else
	{
	    next = DM1B_VPT_GET_BT_NEXTPAGE_MACRO(t->tcb_rel.relpgtype, leafpage);
	    if (next == NULL)
		break;
	...
	s = dm0p_fix_page(r, next, fix_action, leaf, err_code);
	if (s != E_DB_OK)
	    break;
	leafpage = ((DM1B_INDEX *)*leaf);
    ...
    i4	    *err_code)
{
	...
	*/
	...
	{
	    ule_format(E_DM9C1C_BTDELBYPASS_WARN, (CL_ERR_DESC*)0, ULE_LOG, 
	...
	*/
	...
	{
	    ule_format(E_DM9C1D_BXBADKEY, (CL_ERR_DESC*)0, ULE_LOG, NULL,
		...
		0, (key - (char *)b));
	    dmd_prindex(rcb, b, 1);
	    ule_format(E_DM93B7_BXDELETE_ERROR, (CL_ERR_DESC*)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM93B7_BXDELETE_ERROR;
	    return (E_DB_ERROR);
	}
	...
	*/
	...
	{
	    status = dm0p_before_image_check(r, (DMPP_PAGE *) b, err_code);
	    if (status != E_DB_OK)
	        break;
	}
	...
	*/
	if (mutex_required)
	{
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&b);
	    mutex_done = TRUE;
	}
	...
	*/
	if (log_required)
	{
	    ...
	    */
		...
		&delbid, child,
		&deltid, key_len, key, (LG_LSN *) 0, &lsn, 
		delpartno, t->tcb_ixklen, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	status = dm1cxdel(tbio->tbio_page_type, tbio->tbio_page_size, b,
		    ...
		    &r->rcb_tran_id, r->rcb_seq_number, dmcx_flag, childkey);
	...
	*/
	if (mutex_required)
	    dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&b);
    ...
    i4            *err_code)
{
	...
	*state = E_DM0056_NOPART;
    ...
    lfpage = ((DM1B_INDEX *)*leaf);
    dupkey = NULL; 

    /* Make sure this is really leaf with overflow chain. */

    if (DM1B_V1_GET_PAGE_OVFL_MACRO(lfpage) != NULL)
    {
	/*
	** If leaf page has entries, use it for getting key.
	*/

        if (DM1B_V1_GET_BT_KIDS_MACRO(lfpage) != 0)
        {
	    dupkey = dupspace;
	    klen = t->tcb_klen;
	    s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, lfpage,
			t->tcb_acc_tlv, t->tcb_leafatts, t->tcb_leafattcnt,
			(DM_LINE_IDX)0, &dupkey, (DM_TID *)0,  (i4*)0,
			&klen,
			DM1B_INDEX_COMPRESSION(r), NULL);

	    if (s != E_DB_OK)
	    {
		dm1cxlog_error( E_DM93E3_BAD_INDEX_GET, r, lfpage,
		    t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, (DM_LINE_IDX)0);
		*err_code = E_DM93B0_BXDUPKEY_ERROR;
		return (s);
	    }
	    if (dupkey != dupspace)
        ...
        else
        {
            ...
            lfpageno = DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, lfpage);

            /* Follow chain to get first page with entry. */

            tempbid->tid_tid.tid_page = DM1B_V1_GET_PAGE_PAGE_MACRO(lfpage); 
	    ...
	    ** fixed in 'leaf'.
	    */
            s = dm1bxchain(r, leaf, tempbid, DM1C_NEXT, (char *)0, 
			    &buf_locked, &local_err_code); 
	    lfpage = ((DM1B_INDEX *)*leaf); 
            if (s == E_DB_OK)
            {
		dupkey = dupspace;
		...
		s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, lfpage,
			...
			(DM_TID *)0,  (i4*)0,
			&klen, DM1B_INDEX_COMPRESSION(r), NULL);
	    ...
	    */
	    if ((lfpage == NULL) || 
		(lfpageno != DM1B_V1_GET_PAGE_PAGE_MACRO(lfpage)))
	    {
		if (lfpage != NULL)
		{
		    s = dm0p_unfix_page (r, DM0P_UNFIX, leaf,  err_code);
		...
		if (r->rcb_access_mode == RCB_A_READ)
		    fix_action = DM0P_READ;
		s = dm0p_fix_page(r, lfpageno, fix_action, leaf, err_code);
		if (s != E_DB_OK)
		    return (s);
	    }		    
            if (savestatus != E_DB_OK && local_err_code != E_DM0055_NONEXT)
	    {
...
**	sections of the DM1B_INDEX structure do NOT have any reliable contents
    ...
    i4		*err_code)
{
	...
	*err_code = E_DM93B8_BXFORMAT_ERROR;
    }
    return (s);
}

...
**					when rcb is NULL, tcb must be
**					supplied.
**	atts_array			Pointer to list of pointers to
**					attribute information for the
**					columns in the key entry.
**	atts_count			Number of entries in atts_array.
**	compression_type		Index compression information, one of:
**					    DM1CX_UNCOMPRESSED
**					    DM1CX_COMPRESSED
    ...
    i4	    *err_code)
{
	...
	*err_code = E_DM93B9_BXINSERT_ERROR;
	return (E_DB_ERROR);
    }
    ...
    DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(page_type, b, page_lsn);
    page_clean_count = DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(page_type, b);
    ...
    */
	...
	if (!index_update && tcb->tcb_rel.relpgtype != TCB_PG_V1 &&
	    (row_locking(r) || tcb->tcb_extended))
		update_mode |= DM1C_ROWLK;
    ...
    */
	...
	if ( status = dm1b_AllocKeyBuf(klen, compressed_key,
				    &Ckey, &AllocCkey, err_code) )
	    return(status);
	status = dm1cxcomp_rec(tcb, atts_array, atts_count, key, klen,
				Ckey, &crec_size );
	if (status != E_DB_OK)
	{
	    /* Discard any allocated key buffer */
    ...
    */
	    ...
	    (r->rcb_alloc_clean_count != DM1B_CC_INVALID))
	{
	    ...
	    */
		...
		get_status = dm1cxget(page_type, page_size, b,
		    tcb->tcb_acc_tlv, atts_array, atts_count,
		    child, &tmpkey, (DM_TID *)&tmptid, &tmppartno,
		    &tmpkey_len, compression_type, &row_low_tran );
		...
		tmptid.tid_i4 != r->rcb_alloc_tidp.tid_i4)
	    {
		status = E_DB_ERROR;
	    }
	    else if (row_locking(r) 
	...
	else
	{
	    ...
	    **
	    ** (Non unique primary btree with page type != TCB_PG_V1,
	    ...
	    */
		...
		status = dm1bxsearch(r, b, DM1C_OPTIM, DM1C_EXACT, save_key,
		    r->rcb_tcb_ptr->tcb_keys, &localtid, &localpartno,
		    &check_pos, NULL, err_code);
		if (DB_FAILURE_MACRO(status) && (*err_code == E_DM0056_NOPART))
		{
		    status = E_DB_OK;
		    *err_code = 0;
		}
	...
	if (status == E_DB_ERROR || compare != DM1B_SAME)
	{
		...
		compare);
	    ule_format(E_DM9C1E_BXKEY_ALLOC, (CL_ERR_DESC*)0, ULE_LOG, NULL,
	...
	*/
	if (logging_needed)
	{
	    ...
	    */
		...
		&bid, child, tid, klen, key, (LG_LSN *) 0, &lsn, 
		tid_partno, tcb->tcb_ixklen, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	status = dm1cxput( page_type, page_size, b,
		    compression_type, update_mode, tran_id,
    ...
    */
    ...
    if ((*err_code == 0) || (*err_code > E_DM_INTERNAL))
    {
	if (*err_code)
	{
	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
    ...
    i4	    *err_code)
{
    ...
    DMP_DCB		*d = t->tcb_dcb_ptr;
    DMP_TABLE_IO	*tbio = &t->tcb_table_io;
    DM1B_INDEX		*par = parent;
    DM1B_INDEX		*cur = current;
    DM1B_INDEX		*sib = sibling;
    LG_LSN		lsn;
    DM_TID		curtid;
    i4			curpartno;
    DM_TID		infinity; 
	...
	*/
	if (d->dcb_status & DCB_S_BACKUP)
	{
	    s = dm0p_before_image_check(r, (DMPP_PAGE *) cur, err_code);
	    if (s != E_DB_OK)
	        break;
	    s = dm0p_before_image_check(r, (DMPP_PAGE *) sib, err_code);
	    if (s != E_DB_OK)
	        break;
	}
	...
	*/
	...
	if (r->rcb_logging & RCB_LOGGING)
	{
	    ...
	    */
	    ...
	    pages_mutexed = TRUE;
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&cur);
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&sib);
	...
	if ((mode == DM1B_SIBJOIN) && (leaf_join))
	{
	    ...
	    */
	    ...
	    s = dm1cxreplace(tbio->tbio_page_type, tbio->tbio_page_size, cur,
			    ...
			    (i4)0, DM1B_RRANGE, key_ptr, key_len, 
			    &infinity, (i4)0);
	    if (s != E_DB_OK)
	    {
		dm1cxlog_error( E_DM93E6_BAD_INDEX_REPLACE, (DMP_RCB *)0, cur,
	...
	*/
	if (r->rcb_logging & RCB_LOGGING)
	{
	    if (pages_mutexed)
	    {
		dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&cur);
		dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&sib);
		pages_mutexed = FALSE;
	    }
	...
	*/
	if (mode == DM1B_SIBJOIN)
	{
	    s = dm1bxdelete(r, par, (DM_LINE_IDX) (pos+1), 
    ...
    i4     *err_code)
{
    ...
    DMP_DCB	    *d = t->tcb_dcb_ptr;
    DMP_TABLE_IO    *tbio = &t->tcb_table_io;
    DMPP_PAGE	    *newroot = (DMPP_PAGE *)0; 
    DM1B_INDEX	    *newpage, *rootpage; 
    DM_TID	    son; 
    DM_PAGENO	    avail; 
    DB_STATUS	    s = E_DB_OK;
    STATUS	    cl_status;
	...
	*/
	...
	s = dm1p_getfree(r, alloc_flag, &newroot, err_code);
	if (s != E_DB_OK)
	    break;
	...
	*/
	if (d->dcb_status & DCB_S_BACKUP)
	{
	    s = dm0p_before_image_check(r, (DMPP_PAGE *) rootpage, err_code);
	    if (s != E_DB_OK)
	        break;
	    s = dm0p_before_image_check(r, (DMPP_PAGE *) newpage, err_code);
	    if (s != E_DB_OK)
	        break;
	}
	...
	if (r->rcb_logging & RCB_LOGGING)
	{
	    ...
	    */
		...
		(VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &sys_err,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
			err_code, 1, 0, r->rcb_log_id);
		*err_code = E_DM93BD_BXNEWROOT_ERROR;
		break;
	    }
	    ...
	    */
	    pages_mutexed = TRUE;
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&rootpage);
	...
	*/
	...
	while (DM1B_VPT_GET_BT_KIDS_MACRO(tbio->tbio_page_type, rootpage) > 0)
	{
	    ...
	    s = dm1cxdel(tbio->tbio_page_type, tbio->tbio_page_size, rootpage,
		    DM1C_DIRECT, compression_type,
		    (DB_TRAN_ID *) 0, (i4) 0, (i4)DM1CX_RECLAIM, child);
	    if (s != E_DB_OK)
	    {
	...
	*/
	pages_mutexed = FALSE;
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&rootpage);
	...
	if (DMZ_CRH_MACRO(18) || DMZ_CRH_MACRO(22) || DMZ_CRH_MACRO(22))
	{
	    ...
	    if (DMZ_CRH_MACRO(23))
	    {
		(VOID) dm0p_unfix_page(r, DM0P_FORCE, root, err_code);
		TRdisplay("Old Root page Forced.\n");
	    }
	    if (DMZ_CRH_MACRO(18))
	    {
		(VOID) dm0p_unfix_page(r, DM0P_FORCE, root, err_code);
		(VOID) dm0p_unfix_page(r, DM0P_FORCE, &newroot, err_code);
	...
	*/
	if (r->rcb_logging & RCB_LOGGING)
	{
	    s = dm0l_em(r, &prev_lsn, &lsn, err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	** by the caller.
	*/
	s = dm0p_unfix_page(r, DM0P_RELEASE, &newroot, err_code);
	if (s != E_DB_OK)
	    break;
    ...
    */
	...
	if (s != E_DB_OK)
	{
	    ule_format(local_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
    ...
    i4			*err_code)
{
	...
	*partno = t->tcb_partno;
	...
	{
	    ule_format(E_DM93BA_BXSEARCH_BADPARAM, (CL_ERR_DESC *)0,
	...
	s = binary_search(r, page, direction, key, keyno, pos, err_code);
	if (s != E_DB_OK)
	{
	...
	*/
		...
		(*pos)++)
	{
	    ...
	    s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, page,
			t->tcb_acc_tlv, atts_array, atts_count, *pos, 
			&keypos, (DM_TID *)&buftid, partno,
			&keylen, compression_type, &row_low_tran);
    ...
    i4     	*err_code)
{
    ...
    DMPP_PAGE	    *sibling = (DMPP_PAGE *)0;
    DMPP_PAGE	    *data = (DMPP_PAGE *)0;
    ...
    */
    ...
    KeyBufSize = (i4)ME_ALIGN_MACRO(split_keylen, sizeof(i4));
	...
	*/
	s = division(r, curpage, &split, keyno, err_code);
	if (s != E_DB_OK)
	    break;
	...
	** the new insert key.
	*/
	if (split == 0 && (t->tcb_rel.relpgtype == TCB_PG_V1))
	{
	    s = get_dup_key(rcb, curpage, KeyBuf, err_code);
	    if (s != E_DB_OK)
		break;
		...
		ule_format(E_DMF012_ADT_FAIL, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
		*err_code = E_DM93B2_BXSPLIT_ERROR;
		break;
	    }
	    if (compare > DM1B_SAME)
	    {
	...
	*/
	if (desc_key == NULL)
	{
	    ...
	    s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, curpage,
			t->tcb_acc_tlv, atts_array, atts_count, split - 1, 
	...
	if (leaf_split)
	{
		...
		*/
		s = adt_kkcmp(adf_cb, (i4)keyno, keyatts, key, keyptr, 
	        ...
	        if (s != E_DB_OK)
	        {
		    ule_format(E_DMF012_ADT_FAIL, (CL_SYS_ERR *)0, ULE_LOG, 
	...
	*/
	...
	s = dm1p_getfree(r, alloc_flag, &sibling, err_code);
	if (s != E_DB_OK)
	    break;
	...
	*/
	...
	{
	    s = dm1p_getfree(r, DM1P_MINI_TRAN, &data, err_code);
	    if (s != E_DB_OK)
		break;
	...
	*/
	if (d->dcb_status & DCB_S_BACKUP)
	{
	    s = dm0p_before_image_check(r, (DMPP_PAGE *) sibpage, err_code);
	    if (s != E_DB_OK)
	        break;
	    s = dm0p_before_image_check(r, (DMPP_PAGE *) curpage, err_code);
	    if (s != E_DB_OK)
	        break;
	    if (data)
	    {
	...
	*/
	...
	if (r->rcb_logging & RCB_LOGGING)
	{
	    ...
	    */
	    pages_mutexed = TRUE;
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&sibpage);
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&curpage);
	    if (data)
		...
		t->tcb_kperpage,
		compression_type, t->tcb_rel.relloccount,
	...
	*/
	if (leaf_split)
	{
	    if (t->tcb_rngatts == t->tcb_leafatts)
	    {
	    ...
	    */
		...
		resbid.tid_tid.tid_page = 
		    DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, respage);
		...
		*/
		s = dm1bxsearch(r, respage, DM1C_OPTIM, DM1C_EXACT, key,
			keys_given, &localtid, &localpartno,
			&resline, NULL, err_code);

		if (s == E_DB_ERROR)
		{
		    if (*err_code != E_DM0056_NOPART)
		    {
			dm1cxlog_error(E_DM93C5_BXSPLIT_RESERVE, r, respage, 
		...
		*/
		if (dm1cxhas_room(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
			respage, DM1B_INDEX_COMPRESSION(r), 
			(i4)100 /* fill factor */,
			t->tcb_kperleaf, 
			(t->tcb_klen + t->tcb_comp_katts_count + TidSize)) == TRUE)
		{
	...
	*/
	critical_section = FALSE;
	pages_mutexed = FALSE;
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&sibpage);
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&curpage);
	if (data)
	...
	*/
	...
	{
	    if (DM1B_VPT_GET_PAGE_OVFL_MACRO(tbio->tbio_page_type, curpage))
		s = update_overflow_chain(r, curpage, err_code);
	    else if (DM1B_VPT_GET_PAGE_OVFL_MACRO(tbio->tbio_page_type, sibpage))
		s = update_overflow_chain(r, sibpage, err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	*/
	...
	s = dm1bxinsert(r, t, t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
		t->tcb_ixatts, t->tcb_ixattcnt, 
		compression_type, t->tcb_ixklen, parpage,
		index_desc_key, &bid, (i4)0,
		pos, (i4) TRUE, (i4) TRUE, err_code);
	if (s != E_DB_OK)
	    break;
	...
	if (DMZ_CRH_MACRO(19) || DMZ_CRH_MACRO(20) || DMZ_CRH_MACRO(21))
	{
	    ...
	    if (DMZ_CRH_MACRO(20))
	    {
		(VOID) dm0p_unfix_page(r, DM0P_FORCE, current, err_code);
		(VOID) dm0p_unfix_page(r, DM0P_FORCE, &sibling, err_code);
		...
		(VOID) dm0p_unfix_page(r, DM0P_FORCE, current, err_code);
		(VOID) dm0p_unfix_page(r, DM0P_FORCE, &sibling, err_code);
		(VOID) dm0p_unfix_page(r, DM0P_FORCE, &parent, err_code);
	...
	** RCB update is required only when a leaf is split
	...
	if (leaf_split)
	{
	    DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, 
			    *current, cur_lsn);
	    cur_clean_count = 
	      DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype, *current);

	    DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, 
			    sibling, sib_lsn);
	    sib_clean_count = 
	      DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype, sibling);

	    s = dm1b_rcb_update(r, &bid, &newbid, split, DM1C_MSPLIT,
			(i4)0, (DM_TID *)0, (DM_TID *)0, (char *)0, err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	*/
	if (leaf_split && row_locking(r))
	{
	    if (!buf_locked)
	    {
		*err_code = E_DM93C8_BXLOCKERROR_ERROR;
		s = E_DB_ERROR;
	    }
	    else
	    {
		s = dm1bx_lk_convert(r, insert_direction, *current, sibling, 
			&sib_lkid, &data_lkid, err_code);
	    }
	...
	*/
	if (insert_direction == DM1B_LEFT)
	{
	    s = dm0p_unfix_page(r, unfix_action, &sibling, err_code);
	    if (s != E_DB_OK)
		break;
	}
	else
	{
	    if (*buf_locked)
	    {
		dm0p_lock_buf_macro(tbio, r->rcb_lk_id, &sibling);
		dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, current);
	    }
	    s = dm0p_unfix_page(r, unfix_action, current, err_code);
	    if (s != E_DB_OK)
	    {
		dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, &sibling);
		*buf_locked = FALSE;
		break;
	    }
	    *current = sibling;
...
i4		*err_code)
{
	...
	*/
	status = dm1p_getfree(r, dm1p_flag, &ovfl_page, err_code);
	if (status != E_DB_OK)
	    break;
	ovfl = (DM1B_INDEX *) ovfl_page;

	/* Inform buffer manager of new page's type */
	dm0p_pagetype(tbio, ovfl_page, r->rcb_log_id, DMPP_CHAIN);
	...
	*/
	if (d->dcb_status & DCB_S_BACKUP)
	{
	    status = dm0p_before_image_check(r, (DMPP_PAGE *) ovfl, err_code);
	    if (status != E_DB_OK)
	        break;
	    status = dm0p_before_image_check(r, (DMPP_PAGE *) leaf, err_code);
	    if (status != E_DB_OK)
	        break;
	}
	...
	*/
	if (r->rcb_logging)
	{
		...
		(VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &sys_err,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
			err_code, 1, 0, r->rcb_log_id);
		*err_code = E_DM9C21_BXOVFL_ALLOC;
		break;
	    }
	    ...
	    */
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&ovfl);
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&leaf);
	    pages_mutexed = TRUE;
	    ...
	    */
	    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(tbio->tbio_page_type, leaf, lsn);
	}
	else
	{
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&ovfl);
	...
	** Overflow pages for TCB_PG_V1 only, so we don't format
	** the page with key information.
	*/
	status = dm1bxformat(tbio->tbio_page_type, tbio->tbio_page_size,
		    r, t, ovfl, t->tcb_kperleaf, t->tcb_klen,
		    (DB_ATTS **)0, 0, (DB_ATTS **)0, 0,
		    DM1B_VPT_GET_PAGE_PAGE_MACRO(tbio->tbio_page_type, ovfl),
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ...
	    *err_code = E_DM9C1F_BXOVFL_ALLOC;
	    break;
	}
	...
	*/
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&ovfl);
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&leaf);
	pages_mutexed = FALSE;
	critical_section = FALSE;
    ...
    */
    if (critical_section)
    {
	ule_format(E_DM9C1F_BXOVFL_ALLOC, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
...
i4		*err_code)
{
    ...
    DM1B_INDEX		*ovfl = (DM1B_INDEX *) *ovfl_ptr;
    DM1B_INDEX		*prev_ovfl;
    DM_PAGENO		nextpg;
    char		*lrange_key;
    char		*rrange_key;
    i4		lrange_keylen;
	...
	*/
	...
	while (nextpg > 0)
	{
	    status = dm0p_fix_page(r, nextpg, DM0P_WRITE, &prev_page, err_code);
	    if (status != E_DB_OK)
		break;
	    ...
	    ** we are deallocating.
	    */
	    if (DMPP_VPT_GET_PAGE_OVFL_MACRO(tbio->tbio_page_type, prev_page) == 
		DM1B_VPT_GET_PAGE_PAGE_MACRO(tbio->tbio_page_type, ovfl))
		break;
	    ...
	    */
	    nextpg = DMPP_VPT_GET_PAGE_OVFL_MACRO(tbio->tbio_page_type, prev_page);
	    status = dm0p_unfix_page(r, DM0P_UNFIX, &prev_page, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	if (prev_page == NULL)
	{
	    ule_format(E_DM9C22_BXOVFL_INCONSIST, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, err_code, 2, 
		0, DM1B_VPT_GET_PAGE_PAGE_MACRO(tbio->tbio_page_type, ovfl), 0, 
		DM1B_VPT_GET_PAGE_MAIN_MACRO(tbio->tbio_page_type, ovfl));
	    *err_code = E_DM9C23_BXOVFL_FREE;
	    break;
	}

	prev_ovfl = (DM1B_INDEX *) prev_page;
	...
	*/
	if (d->dcb_status & DCB_S_BACKUP)
	{
	    ...
	    if (status != E_DB_OK)
	        break;
	    status = dm0p_before_image_check(r, (DMPP_PAGE *) prev_ovfl, 
						err_code);
	    if (status != E_DB_OK)
	        break;
	}
	...
	*/
	if (r->rcb_logging)
	{
	    ...
	    */
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&ovfl);
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&prev_ovfl);
	    pages_mutexed = TRUE;
	...
	*/
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&ovfl);
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&prev_ovfl);
	pages_mutexed = FALSE;
	critical_section = FALSE;
	...
	*/
	status = dm1b_rcb_update(r, &bid, &newbid, 0, DM1C_MDEALLOC,
			(i4)0,
			(DM_TID *)0, (DM_TID *)0, (char *)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = dm1p_putfree(r, ovfl_ptr, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
    if (critical_section)
    {
	ule_format(E_DM9C23_BXOVFL_FREE, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	dmd_check(E_DM9C20_CRITICAL_SECTION);
    }
    ...
    if (prev_page)
    {
	status = dm0p_unfix_page(r, DM0P_UNFIX, &prev_page, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
    ...
    i4	 *err_code)
{
    ...
    */
    ...
    if ( s = dm1b_AllocKeyBuf((is_index) ? t->tcb_ixklen : t->tcb_klen,
				key_buf, &KeyBuf, &AllocKbuf, err_code) )
	return(s);
	...
	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, page,
		     t->tcb_acc_tlv, atts_array, atts_count, middle,
		     &key_ptr, (DM_TID *)0, (i4*)0,
		     &keylen, 
    ...
    i4			*err_code)
{
    ...
    */
    ...
    KeyBufSize = (i4)ME_ALIGN_MACRO(keylen, sizeof(i4));

    if ( s = dm1b_AllocKeyBuf(3 * KeyBufSize , AllKeys,
				&MidKeyBuf, &AllocKbuf, err_code) )
	return(s);
    ...
    */
	...
	s = get_range_entries(r, b, range_keys, range_lens,
		    infinities, err_code);
	if (s != E_DB_OK)
	    *err_code = E_DM93B4_BXDIVISION_ERROR;
    ...
    */
	...
	keylen = is_index ? t->tcb_ixklen : t->tcb_klen;
	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, b,
		    t->tcb_acc_tlv, atts_array, atts_count, right,
		    &rkey, (DM_TID *)0, (i4*)0,
		    &keylen, compression_type, NULL);
	...
	keylen = is_index ? t->tcb_ixklen : t->tcb_klen;
	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, b,
			t->tcb_acc_tlv, atts_array, atts_count, left,
			&lkey, (DM_TID *)0, (i4*)0,
			&keylen, compression_type, NULL);
    ...
    i4		*err_code)
{
	...
	if (infinity[DM1B_RIGHT] == FALSE)
	{
	    s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, cur,
			...
			t->tcb_rngatts, t->tcb_rngattcnt, DM1B_RRANGE,
			&key_ptr, (DM_TID *)0, (i4*)0,
			&range_len[DM1B_RIGHT], compression_type, NULL);
	    if (s != E_DB_OK)
	    {
		dm1cxlog_error( E_DM93E3_BAD_INDEX_GET, r, cur, 
		    t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, DM1B_RRANGE);
    ...
    i4			*err_code)
{
    ...
    /*
    ** Set up att/key arrays, current page is always a leaf
    */
    rngatts = t->tcb_rngatts;
    rngattcnt = t->tcb_rngattcnt;
    ...
    */
    if ( compression_type != DM1CX_UNCOMPRESSED )
    {
	KeyBufSize = (i4)ME_ALIGN_MACRO(t->tcb_rngklen, sizeof(i4));
	...
	*/
	...
	if (compression_type != DM1CX_UNCOMPRESSED)
	{
		...
		s = dm1cxcomp_rec(t, rngatts, rngattcnt, range[DM1B_LEFT],
				    range_len[DM1B_LEFT],
		...
		s = dm1cxcomp_rec(t,  rngatts, rngattcnt, range[DM1B_RIGHT],
		...
		s = dm1cxcomp_rec(t, rngatts, rngattcnt, midkey, midkey_len,
    ...
    i4	    *err_code)
{
	...
	*/
	...
	s = dm0p_fix_page(r, nextovfl, DM0P_WRITE, &ovfl, err_code);
	if (s != E_DB_OK)
	    break;

	ovflpage = ((DM1B_INDEX *)ovfl); 
	...
	*/
	if (d->dcb_status & DCB_S_BACKUP)
	{
	    s = dm0p_before_image_check(r, ovfl, err_code);
	    if (s != E_DB_OK)
	        break;
	}
	...
	*/
	if (r->rcb_logging)
	{
		...
		(VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &sys_err,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
			err_code, 1, 0, r->rcb_log_id);
		*err_code = E_DM93C4_BXUPDOVFL_ERROR;
		break;
	    }
	    ...
	    */
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, &ovfl);
	    ovfl_mutexed = TRUE;
	...
	*/
	...
	/* Release the semaphore on the page. */

	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, &ovfl);
	ovfl_mutexed = FALSE;
	critical_section = FALSE;

	s = dm0p_unfix_page(r, DM0P_WRITE, &ovfl, err_code);
	if (s != E_DB_OK)
	    break;
    ...
    */
    if (critical_section)
    {
	ule_format(E_DM93C4_BXUPDOVFL_ERROR, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	dmd_check(E_DM9C20_CRITICAL_SECTION);
    }
    ...
    if (ovfl)
    {
	s = dm0p_unfix_page(r, DM0P_UNFIX, &ovfl, &local_err_code);
	if (s != E_DB_OK)
	{
	    ule_format(local_err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
    ...
    i4             *err_code)
{
    DMP_RCB		*r = rcb;
    DMP_TCB	        *t = r->rcb_tcb_ptr;
    DM1B_INDEX          *leafpage = current; 
    DM1B_INDEX          *temppage = 0; 
    DB_STATUS           s = E_DB_OK;
	...
	*err_code = E_DM9C24_BXGETDUP_KEY;
	return (E_DB_ERROR);
    }
    ...
    while (leafpage != NULL)
    {
	if (DM1B_V1_GET_BT_KIDS_MACRO(leafpage))
	{
	    /*
	    ...
	    */
		...
		dm1cxlog_error(E_DM93E3_BAD_INDEX_GET, r, leafpage,
		    t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, (DM_LINE_IDX)0);
		*err_code = E_DM93B6_BXLFDIV_ERROR;
		break;
	    }
	...
	*/
	next_pageno = DM1B_V1_GET_PAGE_OVFL_MACRO(leafpage);

	/*
	** Unfix the current chain page - but be careful not to unfix the
	** caller's leaf page.
	*/
	if (leafpage == temppage)
	{
	    s = dm0p_unfix_page(r, DM0P_UNFIX, (DMPP_PAGE **) &temppage, 
		err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	*/
	if (next_pageno != 0)
	{
	    s = dm0p_fix_page(r, next_pageno, DM0P_WRITE, 
		(DMPP_PAGE **) &temppage, err_code);
	    if (s != E_DB_OK)
		break;
	}
    ...
    */
    if (temppage != NULL)
    {
	local_status = dm0p_unfix_page(r, DM0P_UNFIX, (DMPP_PAGE **)&temppage,
					&local_error);
    ...
    i4             *err_code)
{
    ...
    */
    if ((row_locking(r) || (t->tcb_extended && r->rcb_lk_type == RCB_K_PAGE))
    ...
    */
    if (!r->rcb_logging &&
	(row_locking(r) || DMPP_VPT_PAGE_HAS_SEGMENTS(t->tcb_rel.relpgtype)))
	return (E_DB_OK);
    ...
    */
    ...
    if (status != E_DB_OK)
	*err_code = E_DM93C7_BXCLEAN_ERROR;
    ...
    */
	...
	status = dm1b_rcb_update(r, &localbid, (DM_TID *)0, (i4)0,
		    DM1C_MCLEAN, (i4)TRUE, (DM_TID *)0, (DM_TID *)0, 
		    (char *)0, err_code);
    }
    ...
    i4         	    *err_code)
{
	...
	*/
	s = dm1cxdel(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leaf,
			update_mode, DM1B_INDEX_COMPRESSION(r),
			&r->rcb_tran_id, r->rcb_seq_number, dmcx_flag, child);
	if (s != E_DB_OK)
	{
...
i4   *err_code)
{
    ...
    s = E_DB_OK;
    sib_page = DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, sibling);
    data_page = DM1B_VPT_GET_BT_DATA_MACRO(t->tcb_rel.relpgtype, sibling);
    ...
    */
    local_status = dm1r_lk_convert(r, sib_page, sib_lkid, &local_err); 
    if (local_status != E_DB_OK)
    {
    ...
    */
    ...
    {
	local_status = dm1r_lk_convert(r, data_page, data_lkid, &local_err);
	if (local_status != E_DB_OK)
	{
	    ule_format(local_err, 0, ULE_LOG, NULL, 0, 0, 0, &local_err, 0);
    ...
    */
	...
	curlockkey.lk_key4 = 
		DM1B_VPT_GET_PAGE_PAGE_MACRO(tbio->tbio_page_type, current);
