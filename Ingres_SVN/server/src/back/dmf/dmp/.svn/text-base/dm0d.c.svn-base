    ...
    i4	*err_code )
{
    ...
    return (log_error(j, E_DM9353_BAD_DMP_CLOSE, &sys_err, err_code));
}

/*{
    ...
    i4	*err_code )
{
    ...
    status = ule_format(E_DM9354_BAD_DMP_CREATE, &sys_err, 
    ...
    *err_code = E_DM935F_DM0D_CREATE_ERROR;
    return (E_DB_ERROR);
}

/*{
    ...
    i4	*err_code )
{
    ...
    log_error(0, E_DM935B_BAD_DMP_DELETE, &sys_err, err_code);
    return (E_DB_ERROR);
}

/*{
    ...
    i4	*err_code )
{
	...
	if (status != OK)
	{
		...
		*err_code = E_DM9352_BAD_DMP_NOT_FOUND;
		...
		ule_format(status, (CL_ERR_DESC *)&sys_err, ULE_LOG , NULL, 
			(char * )0, 0L, (i4 *)0, 
			err_code, 0);
		*err_code = E_DM9355_BAD_DMP_OPEN;
	    }
	    break;
	}
	...
	*/
	...
	if (*err_code != E_DM9352_BAD_DMP_NOT_FOUND)
	    log_error(*jctx, *err_code, &sys_err, &local_err_code);
    }
...
i4		    *err_code)
{
    ...
    */
	...
	if (status != OK)
	{
	    ...
	    return (log_error(j, E_DM9356_BAD_DMP_READ, &sys_err, err_code));
	}
    ...
    */
	...
	if (length > DM0D_MRZ)
	{
	    return (log_error(j, E_DM935A_BAD_DMP_LENGTH, 0, err_code));
	}
	...
	*/
	...
	if (j->dm0d_sequence < 0)
	{
	    ule_format(E_DM0055_NONEXT, (CL_ERR_DESC *)&sys_err, ULE_LOG, NULL, 
    ...
    if (status != OK)
    {
	return (log_error(j, E_DM9356_BAD_DMP_READ, (CL_ERR_DESC*)0, err_code));
    }
    ...
    i4	*err_code )
{
    ...
    {
	status = dm0d_backread(j, record, l_record, err_code);
	return (status);
    }
	...
	return (log_error(j, E_DM9359_BAD_DMP_FORMAT, 0, err_code));
    ...
    i4	*err_code )
{
	...
	*/
    ...
    return (log_error(j, E_DM9357_BAD_DMP_WRITE, &sys_err, err_code));
}

/*{
    ...
    i4	*err_code )
{
	...
	*sequence = j->dm0d_sequence;
    ...
    return (log_error(j, E_DM9358_BAD_DMP_UPDATE, &sys_err, err_code));
}

/*{
    ...
    i4	*err_code )
{
    ...
    return (log_error(j, E_DM935D_BAD_DMP_TRUNCATE, &sys_err, err_code));
}

/*{
** Name: log_error	- DM0D error logging routine.
**
** Description:
    ...
    i4	    *err_code )
{
    ...
    i4		i;
    DB_STATUS		status;
    READONLY static	struct _map_error
	{
	    i4	    error_dm0d;
	    i4	    error_dmp;
	}   map_error[] =
    ...
    if (j)
    {
	status = ule_format(dmf_error, sys_err, ULE_LOG, NULL,
	...
	{
	    *err_code = map_error[i].error_dmp;
	    break;
	}
    }
    ...
    i4	*err_code )
{
	...
	*/
	...
	if (status != OK)
	{
	    log_error(0, E_DM9354_BAD_DMP_CREATE, &sys_err, err_code);
	    return(E_DB_ERROR);
	}
	...
	status = dm0d_open(DM0D_MERGE, &dpath->name[0], l_dpath, 
		    ...
		    1, DM0D_M_WRITE, -1, DM0D_FORWARD, cur_dmp, err_code);
	if (status != E_DB_OK)
	{
	    log_error(0, E_DM9355_BAD_DMP_OPEN, &sys_err, err_code);
	    return(E_DB_ERROR);
	}
    ...
    i4	*err_code )
{
	...
	*/
	...
	if (status != OK)
	{
	    ...
	    ule_format(E_DM9004_BAD_FILE_OPEN, &sys_err, ULE_LOG, NULL,
	    ...
	    {
		ule_format(E_DM9393_DM0D_NO_DUMP_CONFIG, &sys_err, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	    }
	    break;
	}
	...
	*/
	...
	ule_format(E_DM9001_BAD_FILE_CLOSE, &sys_err, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 4,
		sizeof(DB_DB_NAME), dcb->dcb_name, 11, "NOT_A_TABLE",
		dcb->dcb_dmp->phys_length, dcb->dcb_dmp->physical.name,
		(i4)STlength(filename), filename);
	...
	*err_code = E_DM9392_DM0D_CONFIG_RESTORE;
    ...
    i4	*err_code )
{
	...
	*/
	...
	ule_format(E_DM9003_BAD_FILE_DELETE, &sys_err, ULE_LOG, NULL,
		...
		dcb->dcb_dmp->phys_length, dcb->dcb_dmp->physical.name,
		(i4)STlength(filename), filename);
	*err_code = E_DM9394_DM0D_CONFIG_DELETE;
	return (E_DB_ERROR);
    }
...
**	attempt to process any node dump files.  For the non-cluster case,
**	this routine creates an empty dump file, if no files have been
    ...
    i4	*err_code)
{
	...
	*/
	...
	if (status)
	{
	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);
	    *err_code = E_DM923A_CONFIG_OPEN_ERROR;
	    return (status);
	}
    }
    ...
    status = get_next_dmp(cnf, &cur_dmp, config->cnf_dmp->dmp_count -1, err_code);
    if (status != E_DB_OK)
	return  (status);
	...
	** return.  We must close the dump file we opened and if we created
	** a dump file, then we must update the config file to show the
	** newly-created dump file.
	*/

	if (process_ptr != (PTR)0)
	{
	...
	status = dm0d_close(&cur_dmp, err_code);
	if (status != E_DB_OK)
	{
	     log_error(cur_dmp, E_DM9353_BAD_DMP_CLOSE, (CL_ERR_DESC *)0, 
		err_code);
	     return(status);
	}
	...
	*/
	...
	status = dm0m_allocate(rec_array_size + sizeof(DMP_MISC),
	    (i4)0, (i4)MISC_CB, (i4)MISC_ASCII_ID, (char *)0, 
	    (DM_OBJECT **)&rec_cb, err_code);
	if (status != OK)
	{
	    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	    *err_code = E_DM9329_DM0J_M_ALLOCATE;
	    break;
	}
	...
	*/
	...
	if (cs.index == 0)
	{
	    *err_code = E_DM932B_DM0J_M_NO_FILES;  
	    break;
	}	
	...
	*err_code = E_DB_OK;
	...
	for (i = 0; i < cs.index; i++)
	{
	    ...
	    */
		...
		{
		    *err_code = E_DM932C_DM0J_M_BAD_NODE_JNL;
		    status = E_DB_ERROR;
		    break;
		}
    	...
    	for ( node = 0; node < numnodes; node++ )
	{
		...
		*err_code = E_DB_OK;
		while ( ! done )
		{
			...
			&node_array[i].cur_dmp, err_code);
		    if (status != E_DB_OK)
		    {
			log_error((DM0D_CTX *)0, E_DM9355_BAD_DMP_OPEN, 
				(CL_ERR_DESC *)0, err_code);
			break;
		    }

		    status = dm0d_read(node_array[i].cur_dmp, 
					&record, &l_record, err_code);
		    if (status != E_DB_OK)
		    {
			...
			else
			{
			    ...
			    *err_code = E_DM932D_DM0J_M_BAD_READ;
			    break;
			}
		    }
...

	...
	*/
	...
	if (process_ptr != (PTR)0)
	{
	    ...
	    */
	    return (dm0c_close(config, 0, err_code));
	}
	...
	*/
	...
	status = dm0d_update(cur_dmp, &sequence, err_code);
	if (status != E_DB_OK)
	{
	    log_error(cur_dmp, E_DM9353_BAD_DMP_CLOSE, (CL_ERR_DESC *)0, 
		err_code);
    ...
    */
	...
	local_status = dm0d_close(&cur_dmp, &local_err_code);
    }

    for (i = 0; i < DM_CNODE_MAX; i++)
    {
	if (node_array[i].cur_dmp != 0)
	    local_status = dm0d_close(&node_array[i].cur_dmp, &local_err_code);
    }
    if (created == TRUE)
	local_status = dm0d_delete(DM0D_MERGE, (char *)dpath, 
			   l_dpath, fil_seq, &local_err_code);
