...

{
    ...
    */
	...
	r->rcb_tcb_ptr->tcb_rel.relspec == TCB_ISAM ||
	...
	** without a row lock
	** Disallow reclaim space by same transaction compression type = HIGH
	...
	*/
	if (row_locking(r) || t->tcb_sysrel || t->tcb_extended)
	{
	...
	/*
	** When row locking...we cannot reorg space reserved for same tranid
	** We can only use it if it is exactly the size we need
	*/
	if (row_locking(r) && row_low_tran == r->rcb_tran_id.db_low_tran)
	    continue;

	/*
	** Mutex the page while updating it.
	*/
	del_tid.tid_tid.tid_page = DMPP_V2_GET_PAGE_PAGE_MACRO(data);
	del_tid.tid_tid.tid_line = i;
	dm0p_mutex(&t->tcb_table_io, (i4)0, r->rcb_lk_id, &data);
	(*t->tcb_acc_plv->dmpp_delete)(t->tcb_rel.relpgtype, 
	    t->tcb_rel.relpgsize, data, (i4)DM1C_DIRECT,
	    (i4)TRUE, (DB_TRAN_ID *)0, &del_tid, del_recsize);
	dm0p_unmutex(&t->tcb_table_io, (i4)0, r->rcb_lk_id, &data);
    }
    ...
    */
    ...
    /* If we've cleaned all deleted records.. check to see if we have room */
    if ((DMPP_V2_GET_PAGE_STAT_MACRO(data) & DMPP_CLEAN) == 0)
	return(dm1cnv2_allocate(data, r, record, orig_rec_size, tid, line_start));
    else
	return(E_DB_INFO);
}

			...
			DMP_RCB		*rcb)
{
	...
	/* Row previously deleted, check if committed */
	if (row_locking(rcb) || t->tcb_sysrel || t->tcb_extended)
	{
	    tup_hdr = ((char*)data + dmppn_v2_get_offset_macro(lp, (i4)i));
...
**      record_type                     Value indicating type of record on
**                                      page. Not used in this accessor.
** Outputs:
			...
			DMPP_SEG_HDR	*seg_hdr)
{
	...
	*record = (char *)page + offset + 
	...
	if (row_version && Dmpp_pgtype[page_type].dmpp_has_versions)
	{
	    u_i2 ver;
	    tup_hdr = ((char *)page + offset);
	    MECOPY_CONST_2_MACRO(
		(PTR)tup_hdr + Dmpp_pgtype[page_type].dmpp_vernum_offset, &ver);
	    *row_version = ver;
	}
...
**                                      needed.
**	 record_type			Indicates record type. E.g. is an ISAM 
**					index record or not. (Not used in
**					this page format).
			...
			DMPP_SEG_HDR	*seg_hdr)
{
    ...
    if (record_type != TCB_C_ISAMINDEX)
	record_size += Dmpp_pgtype[page_type].dmpp_tuphdr_size;
    ...
    */
    if ((nextlno >= DM_TIDEOF) && (record_type != TCB_C_ISAMINDEX))
    {
	return (E_DB_WARN);
    }
...
**                                      bytes.
**       record                         Pointer to record.
**	 record_type			Indicates record type. Not used for
**					this page type.
** Outputs:
**      
**	Returns:
