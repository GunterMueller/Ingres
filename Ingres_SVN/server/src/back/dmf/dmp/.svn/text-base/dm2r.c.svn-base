    ...
    i4	*err_code )
{
	...
	**     count of the load operation.  Loads of more than 16 new pages
	**     will run in standard recreate/rebuild mode to facilitate
	**     recovery (cuts down on recovery work and allows space to be
	**     returned to the system after a failed load).  Loads with a
	**     row count estimate of less than 16 pages are run in
	**     run in nonrebuild/nonrecreate mode, to avoid the file
	**     renaming overhead.  A zero estimate is probably "don't know"
	**     and to avoid mistakes we run these in rebuild/recreate mode.
	**
	*/
	...
	*/
	if ((mct->mct_structure == TCB_HEAP) &&
	    ((flag & DM2R_EMPTY) == 0
	     || (estimated_pages > 0 && estimated_pages <= 16)) )
	{
	    mct->mct_recreate = FALSE;
