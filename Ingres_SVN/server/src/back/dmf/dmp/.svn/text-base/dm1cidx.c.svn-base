...
static	i4	    lowest_offset(i4 page_type, i4 page_size, DM1B_INDEX *b);
static	i4	    print(PTR arg, i4 count, char *buffer);
static	VOID	    page_dump(i4 page_type, i4 page_size, DM1B_INDEX *page);
static	VOID	    clear(DMP_RCB *r, DM1B_INDEX *page);
static	VOID	    dm1cxlength(i4 page_type, i4 page_size, DM1B_INDEX *b,
...
*/
DB_STATUS
dm1cxallocate(i4 page_type, i4 page_size, DM1B_INDEX *b,
...
**
** Description:
**	This routine compresses a Btree index entry. Currently, the only
**	compression type supported is the identical compression algorithm used
**	by data pages, so we simply call dmppxcompress and let it do the work.
**	In the future we may/will support more compression types.
**
**	Note that this routine takes a (DB_ATTS **) rather than a (DB_ATTS *)
**	and hence calls dmppxcompress accessor rather than dm1c_comp_rec().
**
** Inputs:
...
*/
DB_STATUS
dm1cxcomp_rec(DMP_TCB *t, DB_ATTS **atts_array, i4 atts_count,
		char *rec, i4 rec_size, char *crec, i4 *crec_size)
{
    char	    buffer[132];
    DB_STATUS	    status;

    status = (*t->tcb_acc_tlv->dmppxcompress)(atts_array, atts_count, 
...
*/
DB_STATUS
dm1cxdel(i4 page_type, i4 page_size, DM1B_INDEX *b,
		i4 update_mode, i4 compression_type,
		...
		i4 flags, i4 child) 
{
    ...
    if (DMZ_AM_MACRO(12))
    {
	if (b == (DM1B_INDEX *)0 || (i4)child < DM1B_LRANGE ||
...
*/
DB_STATUS
xdelete(i4 page_type, i4 page_size, DM1B_INDEX *b, i4 compression_type,i4 child)
{
    i4	    	    pos;
    DM_LINEID	    old_btseq;
    i4	    	    entry_length;
...
*/
DB_STATUS
dm1cxformat(i4 page_type, i4 page_size, DM1B_INDEX *b,
...
i4		Clustered)
{
	...
	*/
    ...
    DM1B_VPT_SET_PAGE_MAIN_MACRO(page_type, b, NULL); 
    DM1B_VPT_SET_PAGE_OVFL_MACRO(page_type, b, NULL); 
    DM1B_VPT_SET_BT_DATA_MACRO(page_type, b, NULL); 
...
**
**	Note that this routine takes a (DB_ATTS **) rather than a (DB_ATTS *)
**	and calls dmppxuncompress accessor rather than dm1c_uncomp_rec.
**
** Inputs:
**	t			    TCB for access to accessors.
...
*/
DB_STATUS
dm1cxget(i4 page_type, i4 page_size, DM1B_INDEX *b,
	    DMPP_ACC_TLV *tlv, 
	    DB_ATTS **atts_array, i4 atts_count, i4 child,
	    char **entry, DM_TID *tidp, i4 *partnop,
	    i4 *entry_size, i4 compression_type,
	    u_i4 *row_low_tran)

{
    ...
    if (DMZ_AM_MACRO(12))
    {
	if (b == (DM1B_INDEX *)0 || entry_size == (i4 *)0 ||
	...
	*/
	local_status = (*tlv->dmppxuncompress)(atts_array, atts_count, 
			key_ptr, *entry, (*entry_size), &uncompress_size, 
			(i4)0, (DM_TID *)0, (char *)0, (i4)0);
...
*/
bool
dm1cxhas_room(i4 page_type, i4 page_size, DM1B_INDEX *b,
i4 compression_type,
i4 fill_factor,
i4 kperpage,
i4 entry_size)
...
*/
static VOID
dm1cxlength(i4 page_type, i4 page_size, DM1B_INDEX *b,
    i4 child, i4 *entry_size)
{
    DM_LINEID  	    *lp;
    i4	    	    i,j;
...
*/
VOID
dm1cx_klen(i4 page_type, i4 page_size, DM1B_INDEX *b,
...

/*
** Name: dm1cxlog_error		- log an error encountered on an index page
**
** Description:
...
*/
VOID
dm1cxlog_error(i4 errorno, DMP_RCB *rcb, 
		DM1B_INDEX *page, i4 page_type, i4 page_size, i4 child)
{
    char		*db_name = DB_NOTAVAIL_NAME;
    char		*tbl_name = DB_NOTTAB_NAME;
    char		*own_name = DB_OWNUNKNOWN_NAME;
    i4		page_number = -1;
    i4		err_code;
    i4		page_stat = 0;
    i4		kids = -1;
    ...
    ule_format(errorno, (CL_ERR_DESC *)0, ULE_LOG, NULL, 0, 0, 0, &err_code,
		8,
...
*/
DB_STATUS
dm1cxlshift(i4 page_type, i4 page_size, DM1B_INDEX *cur, DM1B_INDEX *sib,
...
*/
i4
dm1cxmax_length(DMP_RCB *r, DM1B_INDEX *b)
{
	...
	return ((t->tcb_ixklen + t->tcb_comp_katts_count + TidSize));
    else
	return ((t->tcb_klen + t->tcb_comp_katts_count + TidSize));
}

/*
...
*/
VOID
dm1cxmidpoint(i4 page_type, i4 page_size, DM1B_INDEX *b,
i4 compression_type,
i4 *split_point)
{
    i4	avail;
...
*/
DB_STATUS
dm1cxput(i4 page_type, i4 page_size, DM1B_INDEX *b,
...
*/
DB_STATUS
dm1cxrclaim(i4 page_type, i4 page_size, DM1B_INDEX *b, i4 child)
{
    i4	del_offset;
    i4	low_off;
    char	*start_pt;
...
*/
VOID
dm1cxrecptr(i4 page_type, i4 page_size, DM1B_INDEX *b, i4 child, char **key)
{
...
*/
DB_STATUS
dm1cxreplace(i4 page_type, i4 page_size, DM1B_INDEX *page,
		...
		DM_TID *tidp, i4 partno)
{
    ...
    if (DMZ_AM_MACRO(12))
    {
	if (page == (DM1B_INDEX *)0 || (i4)position < DM1B_LRANGE ||
...
*/
DB_STATUS
dm1cxrshift(i4 page_type, i4 page_size, DM1B_INDEX *current, DM1B_INDEX *sibling,
...
*/
DB_STATUS
dm1cxtput(i4 page_type, i4 page_size, DM1B_INDEX *b, i4 child, 
DM_TID *tidp,
i4	partno)
{
    char	buffer [132];
...
**
**	Currently, we calculate this value on demand. Since the DM1B_INDEX
...
*/
static	i4
lowest_offset(i4 page_type, i4 page_size, DM1B_INDEX *b)
{
    i4	    i, kids;
    u_i4    min_offset;
    u_i4    this_offset;
...
*/
static VOID
page_dump(i4 page_type, i4 page_size, DM1B_INDEX *page)
{
    i4  i;
...
*/
DB_STATUS
dm1cxclean(i4 page_type, i4 page_size, DM1B_INDEX *b,
		i4 compression_type,
		i4 update_mode)
{
    i4		    pos;
    ...
    for (pos = 0; (pos < DM1B_VPT_GET_BT_KIDS_MACRO(page_type, b)); )
    {
	/* Test for deleted row */
	if (dmpp_vpt_test_free_macro(page_type,
		DM1B_VPT_BT_SEQUENCE_MACRO(page_type, b), 
...
*/
i4
dm1cxkeycount(i4 page_type, i4 page_size, DM1B_INDEX *b) 
{
    i4	child;
    i4     count = 0;
		...
		i4		child)
{
    ...
    */
	...
	case TCB_PG_V2:
	{
	    ...
	    MECOPY_CONST_4_MACRO(
		(PTR)tup_hdr + Dmpp_pgtype[TCB_PG_V2].dmpp_lowtran_offset,
		&low_tran);
	    if (low_tran == r->rcb_tran_id.db_low_tran)
	    {
		MECOPY_CONST_4_MACRO(
		    (PTR)tup_hdr + Dmpp_pgtype[TCB_PG_V2].dmpp_hightran_offset,
		    &high_tran);
		MECOPY_CONST_4_MACRO(
		    (PTR)tup_hdr + Dmpp_pgtype[TCB_PG_V2].dmpp_seqnum_offset,
		    &seq_num);
		...
		i4 *err_code)
{
    ...
    */
    ...
    else if ((page_type == TCB_PG_V3 || page_type == TCB_PG_V4 || page_type == TCB_PG_V5)
	    && (row_locking(r) || t->tcb_sysrel || t->tcb_extended))
    {
	tup_hdr = dm1b_vpt_entry_macro(page_type, b, (i4)pos);
	MECOPY_CONST_4_MACRO(&r->rcb_tran_id.db_low_tran,
		(PTR)tup_hdr + Dmpp_pgtype[page_type].dmpp_lowtran_offset);

	/* V3, V4, V5 pages keep new tids in rcb when row locking */
	status = defer_add_new(r, tid, err_code);
	return (status);
    }
    /* else fall thru and do clear operation and set new bit */
