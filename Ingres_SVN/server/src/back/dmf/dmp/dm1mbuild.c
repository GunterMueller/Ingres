	...
	i4         *err_code)
{
    ...
    */
    status = dm1mxformat(mct->mct_page_type, mct->mct_page_size, 
	    ...
	    DM1B_PSPRIG,
	    mct->mct_index_comp, mct->mct_acc_plv, err_code);
    if (status != E_DB_OK)
    {
	(VOID) dm1xfinish(mct, DM1X_CLEANUP, &local_err_code);
	log_error(E_DM925A_DM1B_BEGIN, err_code);
    ...
    status = dm1cxallocate(mct->mct_page_type, mct->mct_page_size, 
			...
			(DB_TRAN_ID *)0, (i4)0, 0, (i4)DM1B_TID_S);
    if (status != E_DB_OK)
    {
	...
	i4         *err_code)
{
	...
	*/
	...
	if ( status == E_DB_OK )
	{
		...
		status = finish_parent( mct, EndKey, err_code );
	...
	*/
	status = dm1xfinish(mct, DM1X_COMPLETE, err_code);
    }
	...
	i4         *err_code)
{
    ...
    */
    ...
    has_room = dm1cxhas_room(mct->mct_page_type, mct->mct_page_size,
			...
			mct->mct_index_maxlen - mct->mct_hilbertsize);
    if ( has_room )
    {
	/*
	...
	*/
	...
	status = 
	    dm1mxinsert((DMP_RCB *)0, t, mct->mct_page_type, mct->mct_page_size,
			    mct->mct_index_atts, mct->mct_index_attcount,
			    mct->mct_index_comp, leaf_klen - DM1B_TID_S,
	...
	i4		*err_code)
{
	...
	*err_code = E_DM93C0_BBPUT_ERROR;
    ...
    status = dm1mxinsert((DMP_RCB *)0, t, mct->mct_page_type,mct->mct_page_size,
			    mct->mct_index_atts, mct->mct_index_attcount,
			    mct->mct_index_comp,
			    mct->mct_klen - DM1B_TID_S, mct->mct_curindex,
			    indexkey, &temptid, 0,
	...
	i4		*err_code)
{
    ...
    DM_TID	tid;
    DMP_TCB     *t = mct->mct_oldrcb->rcb_tcb_ptr;
    ADF_CB	*adf_cb = (ADF_CB *)mct->mct_oldrcb->rcb_adf_cb;
    i4     leaf_klen;
	...
	char		*endkey)
{
	...
	*/
	...
	status = dm1cxput(mct->mct_page_type, mct->mct_page_size, page,
		    mct->mct_index_comp, DM1C_DIRECT, (DB_TRAN_ID *)0,
		    (i4)0, DM1B_RRANGE, endkey, key_len, 
	...
	*/
	...
	status = dm1cxget(mct->mct_page_type, mct->mct_page_size,
			page, t->tcb_acc_tlv, 
			mct->mct_index_atts, mct->mct_index_attcount,
			DM1B_LRANGE, &key_ptr, (DM_TID *)0, (i4*)0,
			&key_len, mct->mct_index_comp, NULL);
	if (status != E_DB_OK)
	{
	...
	for (kid = 0; kid < lastkid && status == E_DB_OK; kid++)
	{
	    ...
	    */
	    ...
	    status = dm1cxget(mct->mct_page_type, mct->mct_page_size,
			...
			(DM_TID *)0, (i4*)0,
			&endkey_len, mct->mct_index_comp, NULL);
	    if (status != E_DB_OK)
	    {
	    	dm1cxlog_error( E_DM93E3_BAD_INDEX_GET,
				(DMP_RCB *)mct->mct_oldrcb, mct->mct_curleaf,
