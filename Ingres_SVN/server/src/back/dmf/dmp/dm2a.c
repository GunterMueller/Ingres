...
i4     	*err_code )
{
    ...
    if ((! agglims) && ! excb)
    {
    	*err_code = E_DM0165_AGG_BAD_OPTS;
    	return (E_DB_ERROR);
    }

    /* Step 2.	Compile a list of actions needed to process this request */
    status = build_sagg_actions(rcb, agglims, act, &nacts, &countloc, err_code);
	...
	adf_cb->adf_year_cutoff = qef_adf_cb->adf_year_cutoff;
        adf_cb->adf_errcb.ad_ebuflen = qef_adf_cb->adf_errcb.ad_ebuflen;
        adf_cb->adf_errcb.ad_errmsgp = qef_adf_cb->adf_errcb.ad_errmsgp;
	...
	for (rec_cnt = 0; ;rec_cnt++)
	{
	    ...
	    status = (*act[a].scanner)(r, feedback, act[a].direction, rec, err_code);
	    ...
	    {
		local_status = dm2r_unfix_pages(r, &local_err);
		if (local_status != E_DB_OK)
		{
		    ...
		    {
			ule_format(local_err, (CL_SYS_ERR *)0,
		...
		/* Use QEF's exception handler while in ADE */
		if (EXdeclare(qef_exhandler, &ex_context) != OK)
		{
		    EXdelete();	/* restore DMF's exception handler */
		    status = E_DB_ERROR;
		    *err_code = E_DM0167_AGG_ADE_EXCEPTION;
		    break;
	    	}
		status = ade_execute_cx(r->rcb_adf_cb, excb);
		EXdelete();	/* restore DMF's exception handler */

		if (status != E_DB_OK)
	    	{
		  *err_code = E_DM0166_AGG_ADE_FAILED;
	...
	{
	    i4	local_err2;
            ADF_CB	*adf_cb1, *adf_cb2; 

            adf_cb1 = (ADF_CB*)rcb->rcb_adf_cb;
            adf_cb2 = (ADF_CB*)act[a].rcb->rcb_adf_cb;

	    adf_cb1->adf_warncb.ad_intdiv_cnt += 
	    adf_cb2->adf_warncb.ad_intdiv_cnt;

	    adf_cb1->adf_warncb.ad_intovf_cnt += 
	    adf_cb2->adf_warncb.ad_intovf_cnt;

	    adf_cb1->adf_warncb.ad_fltdiv_cnt += 
	    adf_cb2->adf_warncb.ad_fltdiv_cnt;

	    adf_cb1->adf_warncb.ad_fltovf_cnt += 
	    adf_cb2->adf_warncb.ad_fltovf_cnt;

	    adf_cb1->adf_warncb.ad_fltund_cnt += 
	    adf_cb2->adf_warncb.ad_fltund_cnt;

	    adf_cb1->adf_warncb.ad_mnydiv_cnt += 
	    adf_cb2->adf_warncb.ad_mnydiv_cnt;

	    adf_cb1->adf_warncb.ad_noprint_cnt += 
	    adf_cb2->adf_warncb.ad_noprint_cnt;

	    adf_cb1->adf_warncb.ad_textnull_cnt += 
	    adf_cb2->adf_warncb.ad_textnull_cnt;

	    adf_cb1->adf_warncb.ad_agabsdate_cnt += 
	    adf_cb2->adf_warncb.ad_agabsdate_cnt;

	    adf_cb1->adf_warncb.ad_decdiv_cnt += 
	    adf_cb2->adf_warncb.ad_decdiv_cnt;

	    adf_cb1->adf_warncb.ad_decovf_cnt += 
	    adf_cb2->adf_warncb.ad_decovf_cnt;

	    if (act[a].rcb->rcb_rnl_cb)
		dm0m_deallocate((DM_OBJECT **)&act[a].rcb->rcb_rnl_cb);
	    local_status = dm2r_release_rcb(&act[a].rcb, &local_err);
	    if (local_status)
	    {
		ule_format(local_err, (CL_SYS_ERR *)0, ULE_LOG, 
			(i4 *)NULL, (char *)0, (i4)0, (i4 *)0, 
			&local_err2, (i4)0);
	    }
	}
	...
	if (status != E_DB_OK)
	{
		...
		*err_code = 0;
	    ...
	    else if (*err_code != E_DM0074_NOT_POSITIONED &&
	...
	*/
	...
	if (agglims & DM2A_AGLIM_COUNT)
	{
	    ...
	    */
	    if (agglims != DM2A_AGLIM_COUNT)
	    {
		if ((status = dm2r_rcb_allocate(t, rcb, &rcb->rcb_tran_id,
		...
		if ((status = dm2r_position(act[na].rcb, DM2R_ALL, NULL, 0,
	...
	if (agglims & DM2A_AGLIM_MAX)   
	{
	    ...
	    */
	    if ((status = dm2r_rcb_allocate(t, rcb, &rcb->rcb_tran_id,
		rcb->rcb_lk_id, rcb->rcb_log_id, &arcb, err_code)) != E_DB_OK)
		break;
            /* b84245 - simple aggregates do not require WRITE mode */
		...
		if (DMPP_VPT_PAGE_HAS_SEGMENTS(t->tcb_rel.relpgtype))
	    	...
	    	status = dm0m_allocate(
			    ...
			    (DM_OBJECT **)&arcb->rcb_rnl_cb, err_code);
		if (status != E_DB_OK)
		    break;
	    ...
	    */
		...
		if ((status = dm2r_position(act[na].rcb, DM2R_LAST, NULL, 0,
		    rcb->rcb_f_qual, rcb->rcb_f_arg, (DM_TID *)0, err_code))
		    != E_DB_OK)
			break;
	    }
	    if (t->tcb_table_type == TCB_BTREE)
		...
		*/
	  	...
	  	if (rcb->rcb_hl_given)
		{
		    ...
		    if ((status = dm1b_search(arcb, rcb->rcb_hl_ptr, 
			rcb->rcb_hl_given, DM1C_RANGE, DM1C_NEXT,
                    	&arcb->rcb_other_page_ptr, &arcb->rcb_lowtid,
                    	&arcb->rcb_currenttid, err_code))!= E_DB_OK)
			    break;
    ...
    i4	*ercode )
{
	...
	*/
    	s = dm2r_get(r, &localtid, opflag, record, ercode);
    	if (s == E_DB_OK || *ercode != E_DM0055_NONEXT)
	    return (s);
	...
	*/
	...
	{
	    s = dm0p_fix_page(r, pagno, fix_action, &data, ercode); 
	    if (s != E_DB_OK)
		return (s);
		...
		data) & (DMPP_DATA | DMPP_OVFL)) == DMPP_DATA)
		break;	/* got the previous main page */

	    if ((s = dm0p_unfix_page(r, DM0P_UNFIX, &data, ercode)) != E_DB_OK)
		return (s);
	}
