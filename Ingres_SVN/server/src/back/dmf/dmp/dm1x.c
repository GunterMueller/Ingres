...
static	    DB_STATUS	allocate(DM2U_M_CONTEXT *mct, i4 pageno,
				    DMPP_PAGE **page, i4 *err_code);
static      VOID	dm1x_log_error(i4 status, i4 *err_code);
static	    DB_STATUS	read_one(DM2U_M_CONTEXT *mct, i4 pageno,
				    DMPP_PAGE **page, i4 *err_code);
static	    DB_STATUS	read_group(DM2U_M_CONTEXT *mct, i4 pageno,
				    MCT_BUFFER *group, i4 *err_code);
static	    DB_STATUS	write_one(DM2U_M_CONTEXT *mct, DMPP_PAGE *page,
				    i4 *err_code);
...
DB_STATUS	    *err_code)
{
    ...
    i4		    num_alloc;
    i4		    add_alloc = 0;
    i4		    block_size = dmf_svcb->svcb_mwrite_blocks;
    ...
    */
    ...
    if (DMPP_VPT_PAGE_HAS_SEGMENTS(mct->mct_page_type))
    {
	/* need dtsave */
	if ((flag & DM1X_SAVE) == 0)
	    add_alloc += 1;
    ...
    do
    {
        status = dm0m_allocate((SIZE_TYPE)ME_ALIGN_MACRO(
			DI_RAWIO_ALIGN +
			(num_alloc * m->mct_page_size
			    + sizeof(DMP_MISC)), 
			DI_RAWIO_ALIGN),
    	                (i4)0, (i4)MISC_CB, 
			(i4)MISC_ASCII_ID, (char *)mct,
    	                (DM_OBJECT **)(&m->mct_buffer), err_code);
	if (status == E_DB_ERROR)
	{
	    ...
	    */
	    ...
	    status = dm0m_allocate((SIZE_TYPE)ME_ALIGN_MACRO(
			...
			(char *)m, (DM_OBJECT **)(&m->mct_buffer), err_code);
	}
	...
	*/
	...
	m->mct_curseg = 0;
    	...
    	m->mct_data.mct_pages = (DMPP_PAGE *)
			ME_ALIGN_MACRO((char *)(m->mct_buffer) + 
					sizeof(DMP_MISC),
					DI_RAWIO_ALIGN);
    	((DMP_MISC *)(m->mct_buffer))->misc_data = 
					(char *) m->mct_data.mct_pages;

    	if ( flag & DM1X_KEEPDATA )
    	{
	    m->mct_last_dpage = m->mct_data.mct_pages;
	    m->mct_data.mct_pages = (DMPP_PAGE *)
				((char *)m->mct_last_dpage + m->mct_page_size);
    	}

    	if (flag & DM1X_SAVE)
        {
	    m->mct_dtsave = m->mct_data.mct_pages;
	    m->mct_lfsave = (DM1B_INDEX *)
			    ((char *)m->mct_dtsave + m->mct_page_size);
	    m->mct_nxsave = (DM1B_INDEX *)
			    ((char *)m->mct_lfsave + m->mct_page_size);
	    m->mct_data.mct_pages = (DMPP_PAGE *)
			    ((char *)m->mct_nxsave + m->mct_page_size);
    	}
	...
	if (DMPP_VPT_PAGE_HAS_SEGMENTS(mct->mct_page_type))
	{
	    ...
	    m->mct_ovsave = m->mct_data.mct_pages;
	    m->mct_data.mct_pages = (DMPP_PAGE *)
		    ((char *)m->mct_ovsave + m->mct_page_size);
	}

        if (flag & DM1X_OVFL)
	    m->mct_ovfl.mct_pages = (DMPP_PAGE *)
		((char *)m->mct_data.mct_pages+(block_size*m->mct_page_size));
	...
	/* 
        ** If this is an existing table, see how many pages currently exist
	    ...
	    if (m->mct_inmemory)
	    {
		loc = m->mct_buildrcb->rcb_tcb_ptr->tcb_table_io.tbio_location_array;
		loc_count = m->mct_buildrcb->rcb_tcb_ptr->tcb_table_io.tbio_loc_count;
	    }
	    else
	    {
		loc = m->mct_location;
		...
		** from disc.
		*/
	        if (m->mct_buildrcb->rcb_tcb_ptr->tcb_temporary == TCB_TEMPORARY)
	    	{
		    status = dm2f_flush_file(
			loc, loc_count,
			&m->mct_buildrcb->rcb_tcb_ptr->tcb_rel.relid,
			&m->mct_buildrcb->rcb_tcb_ptr->tcb_dcb_ptr->dcb_name,
			err_code );
		    if ( status != E_DB_OK )
			break;
	        }
	    ...
	    m->mct_buildrcb->rcb_tcb_ptr->tcb_table_io.tbio_lalloc = page;
    ...
    if (status != E_DB_OK)
    {
	dm1x_log_error(E_DM92E0_DM1X_START, err_code);
	if ( m->mct_buffer )
	{
...
i4		    *err_code)
{
	...
	*/
	status = write_group(m, &m->mct_data, err_code);
	if ( status != E_DB_OK )
	    break;
	...
	*/
	...
	{
	    status = write_group(m, &m->mct_ovfl, err_code);
	    if ( status != OK )
		break;
	}
	...
	*/
	...
	{
	    status = write_one(m, m->mct_curdata, err_code);
	    if ( status != E_DB_OK )
	       break;
	}
	...
	{
	    status = write_one(m, m->mct_curovfl, err_code);
	    if ( status != E_DB_OK )
	       break;
	}
	...
	{
	    status = write_one(m, (DMPP_PAGE *)m->mct_curleaf, err_code);
	    if ( status != OK )
		break;
	}
	...
	{
	    status = write_one(m, (DMPP_PAGE *)m->mct_curindex, err_code);
	    if ( status != E_DB_OK )
		break;
	}
	...
	{
	    status = write_one(m, m->mct_last_dpage, err_code );
	    if ( status != E_DB_OK )
		break;
	}
	...
	*/
	status = dm1x_build_SMS( m, err_code );
	if ( status != E_DB_OK )
	    break;
	...
	*/
	...
	/*
	** If caller has requested that all allocated disk space 
	** and data be guaranted to be on disk then do what is
	** necessary
	*/
	if ( mct->mct_guarantee_on_disk )
	{
	    /*
    ...
    if (status != E_DB_OK)
	dm1x_log_error(E_DM92E1_DM1X_FINISH, err_code);
...
i4		    *err_code)
{
	     ...
	     *newpage == (DMPP_PAGE *)m->mct_nxsave))
	{
	    ...
	    status = write_one(m, *newpage, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	...
	status = allocate(m, page, newpage, err_code);
	if (status != E_DB_OK)
	    break;
	...
	dm1x_log_error(E_DM92E3_DM1X_NEWPAGE, err_code);
    return (status);
}

/*{
...
i4		    *err_code)
{
	    ...
	    *page == (DMPP_PAGE *)m->mct_lfsave)
	{
	    ...
	    status = write_one(m, *page, err_code);
	    if (status != E_DB_OK)
		return (status);
	}
	*page = (DMPP_PAGE *)
	...
	*page = (DMPP_PAGE *)
    ...
    {
	return (read_one(m, pageno, page, err_code));
    }
	...
	group = &m->mct_ovfl;
    status = read_group(m, pageno, group, err_code);
    if (status != E_DB_OK)
    {
	dm1x_log_error(E_DM92E2_DM1X_READPAGE, err_code);
	return (status);
    }
...

/*{
** Name: dm1xheader - Place header at current point in file.
**
** Description:
**	This routine will allocates enough memory to support the
**	build operation.  It can handle multiple output files.
...
i4		    *err_code)
{
    ...
    m->mct_fhdr_pageno = m->mct_relpages;
    if (reserve)
    {
...
i4		    *err_code)
{
    ...
    dm1x_log_error(E_DM92E4_DM1X_RESERVE, err_code);
    return (status);
}

/*{
...
i4		    *err_code)
{
	...
	** error
	*/
	*err_code = E_DM92E5_DM1X_ALLOCATE;
	status = E_DB_ERROR;
    }
    ...
    */
    ...
    {
	status 	  = E_DB_ERROR;
	*err_code = E_DM0138_TOO_MANY_PAGES;
	return (status);		/* Don't need to log this one */
    }

    if (status != E_DB_OK)
    {
	dm1x_log_error(E_DM92E5_DM1X_ALLOCATE, err_code);
	return (status);
    }
...
DB_STATUS	    *err_code)
{
	...
	if (m->mct_curdata == save_buffer)
	    status = write_one(m, save_buffer, err_code);
    }
	...
	if (m->mct_curindex == (DM1B_INDEX *)save_buffer)
	    status = write_one(m, save_buffer, err_code);
    }
    else if (page == (DMPP_PAGE **)&m->mct_curleaf)
    {
	save_buffer = (DMPP_PAGE *)m->mct_lfsave;
	if (m->mct_curleaf == (DM1B_INDEX *)save_buffer)
	    status = write_one(m, (DMPP_PAGE *)m->mct_lfsave, err_code);
    }
    if (status != E_DB_OK)
	return (status);
	...
	if (status == E_DB_OK)
	{
	    MEcopy((PTR)buffer_page, m->mct_page_size,
		    (PTR)save_buffer);
	    status = dm0p_unfix_page(m->mct_buildrcb, DM0P_UNFIX, &buffer_page,
		    err_code);
	}
    }
    else
	...
	status = dm2f_read_file(m->mct_location, m->mct_loc_count,  
	    ...
	    &t->tcb_rel.relid,
	    &t->tcb_dcb_ptr->dcb_name,
	    &num_read, (i4)pageno, (char *)save_buffer, err_code);
	if (status == E_DB_OK)
	{
    ...
    if (status != E_DB_OK)
    {
	dm1x_log_error(E_DM92E8_DM1X_READ_ONE, err_code);
	return (status);
    }
...
DB_STATUS	    *err_code)
{
	...
	if (status == E_DB_OK)
	{
	    dm0p_mutex(&t->tcb_table_io, (i4)0,
		    m->mct_buildrcb->rcb_lk_id, &buffer_page);
	    MEcopy((PTR)page, m->mct_page_size, (PTR)buffer_page);
	    DMPP_VPT_SET_PAGE_STAT_MACRO(m->mct_page_type, buffer_page, 
					DMPP_MODIFY);
	    dm0p_unmutex(&t->tcb_table_io, 
		    (i4)0, m->mct_buildrcb->rcb_lk_id, &buffer_page);
	    status = dm0p_unfix_page(m->mct_buildrcb, DM0P_UNFIX, &buffer_page,
		    err_code);
	}
    }
    ...
    if (status != E_DB_OK )
    {
    	dm1x_log_error(E_DM92E7_DM1X_WRITE_ONE, err_code);
    	return (status);
    }
...
i4		    *err_code)
{
    ...
    status = write_group(m, group, err_code);
    if (status != E_DB_OK)
	return (status);
    ...
    */
	...
	status = dm2f_read_file(m->mct_location, m->mct_loc_count,  
	    ...
	    &t->tcb_rel.relid,
	    &t->tcb_dcb_ptr->dcb_name,
	    &num_read, (i4)pageno, (char *)group->mct_pages, err_code);
    ...
    dm1x_log_error(E_DM92EA_DM1X_READ_GROUP, err_code);
    return (status);
}

/*{
...
i4		    *err_code)
{
    ...
    */
	...
	if (m->mct_inmemory)
	{
		...
		*/
	        t->tcb_table_io.tbio_lpageno = 
		      DMPP_VPT_GET_PAGE_PAGE_MACRO(m->mct_page_type, buffer_page);

		dm0p_mutex(&t->tcb_table_io, (i4)0,
		    m->mct_buildrcb->rcb_lk_id, &buffer_page);

	    	MEcopy(((char *)group->mct_pages+(i*m->mct_page_size)),
			m->mct_page_size, (char *)buffer_page);
    ...
    if (status != E_DB_OK)
	dm1x_log_error(E_DM92F0_DM1X_WRITE_GROUP, err_code);
...

/*{
** Name: dm1x_log_error - call uleformat to log an error
**
** Description:
**
** Inputs:
...
DB_STATUS	    *err_code)
{
    ...
    */
    ...
    dm1x_log_error( E_DM92E9_DM1X_BUILD_SMS, err_code );
    return( status );
}

/*{
...
i4		    *err_code)
{
	...
	status = dm0m_allocate((SIZE_TYPE)ME_ALIGN_MACRO(
			DI_RAWIO_ALIGN +
			(2 * m->mct_page_size + sizeof(DMP_MISC)),
			DI_RAWIO_ALIGN),
		(i4)0, (i4)MISC_CB, (i4)MISC_ASCII_ID,
	...
	fhdr_ptr = (DM1P_FHDR *)
			ME_ALIGN_MACRO((char *)misc_pages + 
					sizeof(DMP_MISC),
					DI_RAWIO_ALIGN);
	...
	*/
	...
	while ( pages_to_mark )
	{
		...
		if ( status == E_DB_INFO )
		{
		    ...
		    */
		    status = write_one(m, (DMPP_PAGE *)fmap_ptr, err_code);
		    if (status != E_DB_OK)
	    	        break;
	...
	** Write the FHDR
	*/
	status = write_one(m, (DMPP_PAGE *)fhdr_ptr, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
	...
	if ( read_fmap )
	    s = write_one(m, (DMPP_PAGE *)fmap_ptr, &lerr_code);

	if ( read_fhdr )
	    s = write_one(m, (DMPP_PAGE *)fhdr_ptr, &lerr_code);
    }

    dm1x_log_error(E_DM92E6_DM1X_FREE, err_code);
    return (status);
}
...
i4		*err_code)
{
    ...
    if (DMPP_VPT_PAGE_HAS_SEGMENTS(mct->mct_page_type) == 0 ||
	record_type == TCB_C_ISAMINDEX)
    {
    ...
    */
    if (orig_size > max)
    {
	status = dm1xnextpage(mct, &nextpage, err_code);
	if (status != E_DB_OK)
	{
	    dm1x_log_error(E_DM924D_DM1S_BPUT, err_code);
	    return (status);
	}
   ...
   status = (*mct->mct_acc_plv->dmpp_load)(mct->mct_page_type,
	...
	for (seg = record + first_seg_size; ;seg += max)
	{
	    ...
	    status = dm1xnewpage(mct, 0, &mct->mct_curseg, err_code);
	    if (status != E_DB_OK)
	    {
		dm1x_log_error(E_DM924D_DM1S_BPUT, err_code);
		return (status);
...
i4		*err_code)
{
	...
	status = (*mct->mct_acc_plv->dmpp_get)(mct->mct_page_type,
	    ...
	    &record_size, &seg, NULL, NULL, &seg_hdr);
	if (status != E_DB_OK)
	{
	    TRdisplay("DMPP_VPT_PAGE_HAS_SEGMENTS error FIX ME\n");
	    break;
	}
