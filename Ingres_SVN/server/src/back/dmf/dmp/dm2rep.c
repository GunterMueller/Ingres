	...
	i4		*err_code)
{
    ...
    */
    ...
    {
	*err_code = E_DM9553_REP_NO_SHADOW;
	...
	case DM2REP_DELETE:
	    ...
	    */
		...
		&shad_rec, record, &prio, &cdds_no, err_code);
	    if (status != E_DB_OK)
		break;
	    /*
	    ** build and insert archive records
	    */
	    status = build_archive(dcb, rcb, arch_rcb, shad_rec, err_code);
	    if (status != E_DB_OK)
		break;
	    /*
	    ...
	    ** record(s) in table
	    */
	    status = update_shadow(shad_rcb, shad_tcb, rcb, tcb, shad_rec, err_code);
	    if (status != E_DB_OK)
		break;
	    /*
	    ...
	    */
		...
		int_date, prio, cdds_no, err_code);
	...
	default:
	    /* bad option */
	    *err_code = E_DM9554_REP_BAD_OPERATION;
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            	(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            	err_code, (i4)0);
	    return(E_DB_ERROR);
    ...
    */
    ...
    if (status != E_DB_OK && *err_code == E_DM0046_DUPLICATE_RECORD)
    {
	*err_code = E_DM9581_REP_SHADOW_DUPLICATE;
	...
	*err_code = E_DM0046_DUPLICATE_RECORD;
    ...
    else if (status != E_DB_OK && *err_code > E_DM_INTERNAL)
    {
	ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 0);
	/* return deadlock errors to DMF caller */
	if (*err_code == E_DM9042_PAGE_DEADLOCK ||
            *err_code == E_DM9044_ESCALATE_DEADLOCK ||
            *err_code == E_DM9045_TABLE_DEADLOCK ||
            *err_code == E_DM905A_ROW_DEADLOCK)
	{
	    *err_code = E_DM9572_REP_CAPTURE_FAIL;
	    ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 0);
	...
	i4			*err_code)
{
    ...
    for (;;) /* error checking */
    {
	    ...
	    *err_code = E_DM9555_REP_NOMEM;
        ...
        {
	    if (base_tcb->tcb_data_atts[i]->rep_key_seq == 0)
		continue;
            key_seq = base_tcb->tcb_data_atts[i]->rep_key_seq - 1;
	    base_key[key_seq].attr_operator = DM2R_EQ;
	    base_key[key_seq].attr_number = 
                base_tcb->tcb_data_atts[i]->rep_key_seq;
	    /*
	    ...
	    */
	    if (trans_type == DM2REP_DELETE || trans_type == DM2REP_UPDATE)
		    ...
		    base_tcb->tcb_data_atts[i]->offset;
	    else
	...
	*shad_head = shad_rec;
	...
	for (num_recs = 0;;)
	{
	    status = dm2r_get(shad_idx_rcb, &shad_idx_tid, DM2R_GETNEXT, 
	    	shad_idx_rec, err_code);
	    if (status != E_DB_OK)
	    {
		if (*err_code == E_DM0055_NONEXT && num_recs == 0)
		{
	    ...
	    */
	    ...
	    {
		*err_code = E_DM9557_REP_NO_SHAD_MEM;
	    	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            	    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            	    err_code, (i4)0);
		status = E_DB_ERROR;
	...
	i4			*err_code)
{
    ...
    ADF_CB		*adf_scb = (ADF_CB *)base_rcb->rcb_adf_cb;
    i4			size = 0;
    i4			rep_size = 0;
    STATUS		cl_stat;
    i4			i, j;
    ...
    */
    ...
    {
	*err_code = E_DM9558_REP_NO_ARCH_MEM;
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            err_code, (i4)0);
	return(E_DB_ERROR);
    ...
    */
	...
	if (base_tcb->tcb_atts_ptr[i].replicated)
	{
		...
		*err_code = E_DM956F_REP_NO_ARCH_COL;
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            	    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            	    err_code, (i4)2, 0, 
		...
		*/
		...
		if (status > E_DB_INFO)
		{
		    ...
		    *err_code = E_DM9576_REP_CPN_CONVERT;
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
	...
	** causes a bus error.
	...
	if (arch_tcb->tcb_rel.relstat2 & TCB2_HAS_EXTENSIONS)
	    status = dm2r_put(arch_rcb, DM2R_DUPLICATES, archive_rec,
		err_code);
	else
    	    status = dm2r_put(arch_rcb, DM2R_NODUPLICATES, archive_rec, 
	    	err_code);
    }
	...
	i4			*err_code)
{
	...
	*err_code = E_DM9559_REP_NO_IN_ARCH;
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
    ...
    */
	...
	MEcopy((char *)&in_archive, sizeof(in_archive), 
	    ...
	    shad_rec->shad_rec, (char *)0, err_code);
	if (status != E_DB_OK)
	    break;
    }
	...
	i4			*err_code)
{
    ...
    ADF_CB		*cmp_scb = (ADF_CB *)base_rcb->rcb_adf_cb;
	...
	*/
	if (base_tcb->tcb_data_atts[i]->replicated == FALSE ||
	    base_tcb->tcb_data_atts[i]->rep_key_seq == 0)
	    continue;
	/*
	...
	*/
	...
	{
	    if (MEcmp(shad_tcb->tcb_data_atts[j]->name.db_att_name, 
		base_tcb->tcb_data_atts[i]->name.db_att_name, DB_MAXNAME) == 0)
		break;
	}
	if (j == shad_tcb->tcb_rel.relatts)
    	{
	    /* index attribute not found in shadow table */
	    *err_code = E_DM956D_REP_NO_SHAD_KEY_COL;
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
                (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
	...
	if (base_tcb->tcb_data_atts[i]->length != 
	    shad_tcb->tcb_data_atts[j]->length)
	{
	    *err_code = E_DM956E_REP_BAD_KEY_SIZE;
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
                (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
                err_code, (i4)4, 0, 
	...
	*/
	if (trans_type == DM2REP_DELETE)
	    MEcopy(base_rcb->rcb_record_ptr + 
		base_tcb->tcb_data_atts[i]->offset,
		base_tcb->tcb_data_atts[i]->length, 
		shad_rec + shad_tcb->tcb_data_atts[j]->offset);
	else
	    MEcopy(record + base_tcb->tcb_data_atts[i]->offset,
	    	base_tcb->tcb_data_atts[i]->length, 
		shad_rec + shad_tcb->tcb_data_atts[j]->offset);
	size += base_tcb->tcb_data_atts[i]->length;
    }
    /* now the fixed part */
    ...
    */
    ...
    if (cl_stat != OK)
    {
	*err_code = E_DM955D_REP_BAD_DATE_CVT;
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            err_code, (i4)0);
	MEfree(shad_rec);
    ...
    */
	...
	if (trans_type == DM2REP_DELETE)
	    ...
	    status = cdds_lookup(base_rcb, record, cdds_no, err_code);
	if (status != E_DB_OK)
	{
            MEfree(shad_rec);
	    return(status);
    ...
    */
	...
	if (trans_type == DM2REP_DELETE)
	    ...
	    status = check_priority(base_rcb, record, prio, err_code);
	if (status != E_DB_OK)
	{
            MEfree(shad_rec);
	    return(status);
	...
	*/
	    ...
	    shad_curr = shad_curr->next_rec)
	{
		...
		dbname[DB_MAXNAME] = 0;
	    	*err_code = W_DM9579_REP_MULTI_SHADOW;
	    	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
		...
		MEcopy(base_tcb->tcb_rel.relid.db_tab_name, 
		    DB_MAXNAME, tabname);
		tabname[DB_MAXNAME] = 0;
		*err_code = E_DM957A_REP_BAD_DATE_CMP;
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            	    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            	    err_code, (i4)2, 
		    sizeof(base_rcb->rcb_tran_id.db_low_tran),
	...
	*err_code = E_DM955A_REP_BAD_SHADOW;
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            err_code, (i4)0);
        MEfree(shad_rec);
    ...
    */
    status = dm2r_put(shad_rcb, (i4)0, shad_rec, err_code);
...

{
    ...
    */
    if (input_q_rcb == NULL)
    {
	*err_code = E_DM955E_REP_NO_INPUT_QUEUE;
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            err_code, (i4)0);
	return(E_DB_ERROR);
    ...
    */
    ...
    {
	*err_code = E_DM955F_REP_BAD_INPUT_QUEUE;
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            err_code, (i4)0);
	return(E_DB_ERROR);
    ...
    */
    status = dm2r_put(input_q_rcb, (i4)0, input_q_rec, err_code);
	...
	bool		open_db)
{
    ...
    */
    ...
    for (tries = 1;;tries++) /* error handling */
    {
	    ...
	    &log_id, &tran_id, &lock_id, (DB_DIS_TRAN_ID *)NULL, err_code);
    	if (status != E_DB_OK)
	    break;
    	tx_started = TRUE;
	...
	if (in_q_rcb == NULL || in_q_rcb == (DMP_RCB *)-1)
	{
	        ...
	        (i4)0, &tran_id, &timestamp, &input_q_rcb, (DML_SCB *)0, 
	        err_code);
    	    if (status != E_DB_OK)
	        break;
	}
	else
	{
	    ...
	    (i4)0, log_id, lock_id, (i4)0, (i4)0,
	    (i4)0, &tran_id, &timestamp, &dd_paths_rcb, (DML_SCB *)0,
	    err_code);
        if (status != E_DB_OK)
	    break;
	/* open dd_db_cdds */
	    ...
	    DM2T_UDIRECT, DM2T_A_READ, (i4)0, rep_maxlocks,
	    (i4)0, log_id, lock_id, (i4)0, (i4)0,
	    (i4)0, &tran_id, &timestamp, &dd_db_cdds_rcb, (DML_SCB *)0,
	    err_code);
        if (status != E_DB_OK)
	    break;
	/* open the distribution queue */
	    ...
	    DM2T_UDIRECT, DM2T_A_WRITE, (i4)0, rep_maxlocks,
	    (i4)0, log_id, lock_id, (i4)0, (i4)0,
	    (i4)0, &tran_id, &timestamp, &dist_q_rcb, (DML_SCB *)0,
	    err_code);
        if (status != E_DB_OK)
	    break;
	/*
	...
	*/
	...
	if (dcb->dcb_bad_cdds == NULL) /* not checked before */
	{
		...
		dd_db_cdds_rec, dd_paths_rec, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	if (tx_id == -1)
	{
            status = dm2r_position(input_q_rcb, DM2R_ALL, (DM2R_KEY_DESC *)0, 
		(i4)0, 0, (i4 *)0, &tid, err_code);
            if (status != E_DB_OK)
	        break;
	}
	else
	{
	    ...
	    if (status != E_DB_OK)
	    {
	        if (*err_code == E_DM0055_NONEXT) /* out of records */
	        {
		    status = E_DB_OK;
		    break;
	        }
		...
		for (i = 0; recovery_txq[i].tx_id != 0; i++)
		{
			...
			*/
			...
			if (status != E_DB_OK)
			{
				...
				dbname);
			    dbname[DB_MAXNAME] = 0;
			    *err_code = E_DM957B_REP_BAD_DATE;
			    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
		 ...
		 */
		 ...
		 status = dm2r_position(dd_db_cdds_rcb, DM2R_QUAL, 
		     dd_db_cdds_key, (i4)2, 0, 
		     (i4 *)0, &tid, err_code);
		 if (status != E_DB_OK)
		 {
	            if (*err_code == E_DM0055_NONEXT) /* no records*/
	            {
			*err_code = W_DM9562_REP_NO_DD_DB_CDDS;
			ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			    (i4 *)0, err_code, (i4)3, 0, 
		 ...
		 */
		 ...
		 {
		     status = E_DB_ERROR;
		     *err_code = E_DM9560_REP_BAD_DIST_QUEUE;
	    	     ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            		err_code, (i4)0);
		     break;
		 ...
		 */
		 if (trans_time == (HRSYSTIME *)-1 && !mdwarn)
		 {
		     ...
		     dbname[DB_MAXNAME] = 0;
		     *err_code = W_DM956A_REP_MANUAL_DIST;
	    	     ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
		 ...
		 ** add record
		 */
		 status = dm2r_put(dist_q_rcb, (i4)0, dist_q_rec, 
		     err_code);
		 if (status != E_DB_OK)
		     break;
		/*
	    ...
	    */
	    status = dm2r_delete(input_q_rcb, &tid, DM2R_BYPOSITION, 
		err_code);
	    if (status != E_DB_OK)
		break;
	...
	if (status == E_DB_OK && trans_time == (HRSYSTIME *)-1)
	{
		...
		*/
		status = dm2r_get(dist_q_rcb, &recovery_tids[tidq_curr].tid,
		    DM2R_BYTID, dist_q_rec, err_code);
		if (status != E_DB_OK)
		{
		    char	dbname[DB_MAXNAME + 1];
			...
			dbname);
		    dbname[DB_MAXNAME] = 0;
		    *err_code = E_DM957C_REP_GET_ERROR;
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, 
			(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
			err_code, (i4)1, STtrmwhite(dbname), dbname);
		    break;
		...
		*/
		...
		if (no_prev_entry)
		{
			...
			dbname);
		    dbname[DB_MAXNAME] = 0;
		    *err_code = E_DM957D_REP_NO_DISTQ;
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, 
			(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
			err_code, (i4)2, 
			sizeof(recovery_tids[tidq_curr].tx_id), 
		...
		*/
		status = dm2r_replace(dist_q_rcb, &recovery_tids[tidq_curr].tid,
		    DM2R_BYTID, dist_q_rec, (char *)0, err_code);
		if (status != E_DB_OK)
		{
		    char	dbname[DB_MAXNAME + 1];
			...
			dbname);
		    dbname[DB_MAXNAME] = 0;
		    *err_code = E_DM957E_REP_DISTQ_UPDATE;
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, 
			(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
			err_code, (i4)2, 
			sizeof(recovery_tids[tidq_curr].tx_id), 
	...
	** check for deadlock
	*/
	if (status != E_DB_OK && (*err_code == E_DM9042_PAGE_DEADLOCK ||
	    *err_code == E_DM9044_ESCALATE_DEADLOCK ||
	    *err_code == E_DM9045_TABLE_DEADLOCK ||
	    *err_code == E_DM905A_ROW_DEADLOCK ||
	    *err_code == E_DM0042_DEADLOCK))
	{
		...
		{
		    status = dm2t_close(input_q_rcb, (i4)0, &local_err);
	    	    if (status != E_DB_OK)
	            {
			*err_code = E_DM9577_REP_DEADLOCK_ROLLBACK;
	        	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            	    	    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			    (i4 *)0, err_code, (i4)0);
			break;
    		...
    		if(dd_paths_rcb)
		{
		    status = dm2t_close(dd_paths_rcb, (i4)0, &local_err);
	    	    if (status != E_DB_OK)
	            {
			*err_code = E_DM9577_REP_DEADLOCK_ROLLBACK;
	        	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
    		...
    		if (dd_db_cdds_rcb)
		{
		    status = dm2t_close(dd_db_cdds_rcb, (i4)0, &local_err);
	    	    if (status != E_DB_OK)
	            {
			*err_code = E_DM9577_REP_DEADLOCK_ROLLBACK;
	        	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
    		...
    		if (dist_q_rcb)
		{
		    status = dm2t_close(dist_q_rcb, (i4)0, &local_err);
	    	    if (status != E_DB_OK)
	            {
			*err_code = E_DM9577_REP_DEADLOCK_ROLLBACK;
	        	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
		...
		*err_code = 0;
		/*
		** Deadlock occurred with hijacked call, so let the
		** other process (which didn't deadlock) finish this off
		*/
		if (open_db)
		{
		    status = E_DB_WARN;
	    ...
	    {
		*err_code = W_DM9578_REP_QMAN_DEADLOCK;
	        ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            	    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            	    err_code, (i4)0);
		status = E_DB_WARN;
    ...
    /* check for deadlock */
    if (status > E_DB_WARN &&
	(*err_code == E_DM9042_PAGE_DEADLOCK ||
	*err_code == E_DM9044_ESCALATE_DEADLOCK ||
	*err_code == E_DM9045_TABLE_DEADLOCK ||
	*err_code == E_DM905A_ROW_DEADLOCK ||
	*err_code == E_DM0042_DEADLOCK))
    ...
    */
    /* dd_input_queue */
    if (input_q_rcb && !iq_passed)
	local_stat = dm2t_close(input_q_rcb, (i4)0, &local_err);

    /* dd_paths */
    if(dd_paths_rcb)
	local_stat = dm2t_close(dd_paths_rcb, (i4)0, &local_err);

    /* dd_db_cdds */
    if (dd_db_cdds_rcb)
	local_stat = dm2t_close(dd_db_cdds_rcb, (i4)0, &local_err);

    /* dd_distrib_queue */
    if (dist_q_rcb)
	local_stat = dm2t_close(dist_q_rcb, (i4)0, &local_err);
	...
	local_stat = dmxe_abort(NULL, dcb, &tran_id, log_id, lock_id, 
	    dmxe_flags, (i4)0, NULL, &local_err);
	...
	i4		*err_code)
{
	...
	*err_code = E_DM9555_REP_NOMEM;
    ...
    for (;;) /* error checking */
    {
    	status = dm2r_position(prio_rcb, DM2R_ALL, &key, (i4)0, 0, 0, 
	    (DM_TID *)0, err_code);
    	if (status != E_DB_OK)
	    break;
	...
	{
	    matched_values = 0;
	    status = dm2r_get(prio_rcb, &tid, DM2R_GETNEXT, prio_rec, err_code);
	    if (status != E_DB_OK)
	    {
		if (*err_code == E_DM0055_NONEXT) /* out of records */
		{
	    ...
	    */
		...
		for (j = 1; j <= base_tcb->tcb_rel.relatts; j++)
		{
			    ...
			    base_tcb->tcb_atts_ptr[j].length)
			{
			    ...
			    *err_code = E_DM9565_REP_BAD_PRIO_LOOKUP;
			    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		        (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			        (i4 *)0, err_code, (i4)5, 
			    ...
			    prio_tcb->tcb_atts_ptr[i].type)
			{
			    ...
			    prio_fld[DB_MAXNAME] = prio_name[DB_MAXNAME] = 
				base_name[DB_MAXNAME] = EOS;
			    *err_code = E_DM9575_REP_BAD_PRIO_TYPE;
			    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		        (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			        (i4 *)0, err_code, (i4)3,
				STtrmwhite(prio_fld), prio_fld,
		...
		{
		    *err_code = E_DM9566_REP_NO_PRIO_MATCH;
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			(i4 *)0, err_code, (i4)3, DB_MAXNAME,
	...
	i4		*err_code)
{
	...
	*err_code = E_DM9555_REP_NOMEM;
    ...
    for (;;) /* error checking */
    {
    	status = dm2r_position(cdds_rcb, DM2R_ALL, &key, (i4)0, 0, 0, 
	    (DM_TID *)0, err_code);
    	if (status != E_DB_OK)
	    break;
	...
	{
	    matched_values = 0;
	    status = dm2r_get(cdds_rcb, &tid, DM2R_GETNEXT, cdds_rec, err_code);
	    if (status != E_DB_OK)
	    {
		if (*err_code == E_DM0055_NONEXT) /* out of records */
		{
	    ...
	    */
		...
		for (j = 1; j <= base_tcb->tcb_rel.relatts; j++)
		{
			    ...
			    base_tcb->tcb_atts_ptr[j].length)
			{
				...
				base_name[DB_MAXNAME] = EOS;
			    *err_code = E_DM956B_REP_BAD_CDDS_LOOKUP;
			    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		        (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			        (i4 *)0, err_code, (i4)5, 
			    ...
			    cdds_tcb->tcb_atts_ptr[i].type)
			{
			    ...
			    cdds_fld[DB_MAXNAME] = cdds_name[DB_MAXNAME] = 
				base_name[DB_MAXNAME] = EOS;
			    *err_code = E_DM9574_REP_BAD_CDDS_TYPE;
			    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		        (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			        (i4 *)0, err_code, (i4)3,
				STtrmwhite(cdds_fld), cdds_fld,
		...
		if (no_match) /* no matchig fields in the base table */
		{
		    *err_code = E_DM956C_REP_NO_CDDS_MATCH;
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			(i4 *)0, err_code, (i4)3, DB_MAXNAME,
	...
	i4		*err_code)
{
    ...
    */
    status = dm2r_position(cdds_rcb, DM2R_ALL, (DM2R_KEY_DESC *)0, (i4)2,
	0, (i4 *)0, &tid, err_code);
    if (status != E_DB_OK)
    {
	if (*err_code == E_DM0055_NONEXT) /* no records*/
    ...
    for (;;) /* error handling */
    {
	    ...
	    status = dm2r_get(cdds_rcb, &tid, DM2R_GETNEXT, cdds_rec, err_code);
            if (status != E_DB_OK)
            {
	        if (*err_code == E_DM0055_NONEXT) /* no more records*/
	        {
	            status = E_DB_OK;
		    break;
	        }
	...
	*/
	for (i = 0; cdds_list[i] != -1; i++)
	{
		...
		/* check the path */
		    ...
		    (i4)2, 0, (i4 *)0, &tid, err_code);
	    	if (status != E_DB_OK)
                {
	            if (*err_code == E_DM0055_NONEXT) /* no records*/
	            {
			*err_code = W_DM9571_REP_DISABLE_CDDS;
