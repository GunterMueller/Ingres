...
lgk_get_config(RCONF *rconf, CL_ERR_DESC *sys_err, i4  wordy)
{
    ...
    */
	...
	if (status)
	{
	    ule_format(status, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		    (char *)0, 0L, 
		    (i4 *)0, &err_code, 0);
	    ule_format(E_DMA808_LGK_NONNUM_PARM, (CL_ERR_DESC *)0, ULE_LOG,
    ...
    if (rconf->bcnt < 0)
    {
	ule_format(E_DMA804_LGK_NEGATIVE_PARM, (CL_ERR_DESC *)0, ULE_LOG,
    ...
    */
    ...
    {
	ule_format(E_DMA806_LGK_PARM_RANGE_ERR, (CL_ERR_DESC *)0, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, &err_code, 5,
		    0, parm_name,
		    0, pm_str,
		    0, rconf->cpcnt,
		    0, 1, 
    ...
    {
	ule_format(E_DMA807_LGK_BLKSIZE_ERR, (CL_ERR_DESC *)0, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, &err_code, 3,
		    0, parm_name,
		    0, pm_str,
		    0, rconf->bsize);
	...
	*/
    ...
    /* Only allowed with single-partition log files */
    if ( rconf->nparts == 1 )
    {
	...
	if (status == OK)
	{
	    ...
	    {
		ule_format(E_DMA809_LGK_NO_ON_OFF_PARM, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 2,
			0, parm_name,
			0, pm_str);
		return (E_DMA805_LGK_CONFIG_ERROR);
...
**	totsize				ptr to i4 for total size
**
** Outputs:
**	none.
**
...
SIZE_TYPE	*totsize)
{
    ...
    if (total > MAXI4) overflowed = TRUE;
    ...
    */
    ...
    /* round total to next nearest 8k boundary */
    if ( MAX_SIZE_TYPE < (total + 8192) )
	overflowed = TRUE;
    oldtotal = total;
    total = (((total + 8192) / 8192) * 8192);
    ...
    /* check for overflow */
    if (overflowed) total = MAX_SIZE_TYPE;
