...
READONLY static	struct MEMORY_INFO
{
	...
	{
	    { 256 * 1024, 128 * 1024, 8 * 1024, 0 },
	    { 1024 * 1024, 256 * 1024, 16 * 1024, 0 },
	    { 4 * 1024 * 1024, 640 * 1024, 32 * 1024, 0 },
	    { 10 * 1024 * 1024, 1024 * 1024, 32 * 1024, 0 },
	    { 50 * 1024 * 1024, 2048 * 1024, 32 * 1024, 0 },
	    { 500 * 1024 * 1024, 4096 * 1024, 32 * 1024, 0 }
	};
#define MI_COUNT 6

...
i4            *err_code)
{
	...
	if (svcb->svcb_stnthreads && svcb->svcb_strows <= records &&
		    !(flags & DMSE_CHILD) &&	/* must be sort pool, enuf */
		    !DMZ_SRT_MACRO(3))		/* rows, not a child
						    ** thread and not dm503 */
	{
	    DM_STECB	*step;
	    DMS_SRT_CB	*s;
	    SCF_CB		scfcb;
		...
		sort_cb, collation, ucollation, err_code);
	    if (status != E_DB_OK) 
		return(status);
		...
		*/
		status = dm0m_allocate(mem + sizeof(DMP_MISC), (i4)0,
			...
			(char *)s, (DM_OBJECT **)&step->ste_misc, err_code);
		if ( status == E_DB_OK )
		{
	    ...
	    status = dmse_end(s, err_code); /* free to re-start as serial */
	    if (status != E_DB_OK) 
		return(status);
...
i4            *err_code)
{
	...
	*err_code = E_DM013F_TOO_MANY_LOCS;
    ...
    awidth = (i4)ME_ALIGN_MACRO(width, sizeof(PTR));
    ...
    */
    ...
    memory = (i4)ME_ALIGN_MACRO(sizeof(DMS_SRT_CB),sizeof(ALIGN_RESTRICT));
    memory += (att_count + 2) *
		  (i4)ME_ALIGN_MACRO(sizeof(DB_CMP_LIST),sizeof(ALIGN_RESTRICT));
    memory += sr_entries *
            (i4)ME_ALIGN_MACRO(sizeof(SR_IO),sizeof(ALIGN_RESTRICT));
	    
    if (flags & (DMSE_ELIMINATE_DUPS | DMSE_CHECK_KEYS))
	memory += awidth;

    status = dm0m_allocate(memory, (i4)0, (i4)SORT_CB,
	(i4)SORT_ASCII_ID, (char *)0, (DM_OBJECT **)sort_cb, err_code);
    if (status != E_DB_OK)
    {
	*err_code = E_DM9704_SORT_ALLMEM_ERROR;
	return (E_DB_ERROR);
    }
    s = *sort_cb;
...
/* TRdisplay("Sort CB: %p, %p, %d\n", s, &s->srt_nthreads, memory); */
    ...
    */
    ...
    if (bytes < dmf_svcb->svcb_int_sort_size)
    {
	if (records < 100)
    ...
    */
    ...
    s->srt_ratts = (DB_CMP_LIST *)((char *)s +
		 (int)ME_ALIGN_MACRO(sizeof(DMS_SRT_CB),sizeof(ALIGN_RESTRICT)));
    s->srt_sr_files = (SR_IO *)((char *)s->srt_ratts + ((att_count + 2) *
		 (int)ME_ALIGN_MACRO(sizeof(DB_CMP_LIST),sizeof(ALIGN_RESTRICT))));
    s->srt_duplicate = (char *)s->srt_sr_files + (sr_entries *
		  (int)ME_ALIGN_MACRO(sizeof(SR_IO),sizeof(ALIGN_RESTRICT)));
...
/* TRdisplay("Heap: %p, %p, %p, %d\n", s->srt_misc, record, heap,
    ...
    */
	...
	for (i = 1; i <= att_count; i++)
	{
	    ...
	    if (status)
	    {
		ule_format(s->srt_adfcb.adf_errcb.ad_errcode, 0,
    ...
    s->srt_ratts[0].cmp_offset = -(sizeof(i4));
	...
	*/
    ...
    {
	if (DMZ_SRT_MACRO(1))
	    TRdisplay("%x DMSE_BEGIN: Records = %d Width = %d Atts = %d Heap = %d Memory = %d\n",
		s->srt_sid,
...
i4		    *err_code)
{
	...
	return(dmse_put_record_serial(sort_cb, record, err_code));
    ...
    *err_code = sort_cb->srt_perr_code;
...
i4		    *err_code)
{
		...
		(i = ++s->srt_next_record) > s->srt_last_record)
	{
	    ...
	    ** to this run or the next or if it's a droppable duplicate. */
	    ...
	    {
	        ule_format(s->srt_adfcb.adf_errcb.ad_errcode, 
				0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	        *err_code = E_DM970B_SORT_PUT;
		return (E_DB_ERROR);
	    }
    ...
    *err_code = E_DM9706_SORT_PROTOCOL_ERROR;
    return (E_DB_ERROR);
}
	...
	i4		*err_code)
{
	 ...
	 if (heap[j] != NULL && heap[j+1] != NULL)
	{
	    ...
	    ** replace it. Otherwise, promote j. */
		...
		status = new_root(s, j+1, err_code);
		if (status != E_DB_OK) 
		    return(status);
		else 
		    continue;
...
i4            *err_code)
{
    ...
    if (!(sort_cb->srt_state & SRT_PARALLEL))
	return(dmse_input_end_serial(sort_cb, err_code));

    if (DMZ_SRT_MACRO(1))
	TRdisplay("%x DMSE_INPUT: Records = %d Runs = %d\n", 
	    sort_cb->srt_sid,
...
i4            *err_code)
{
    ...
    ** always reheapifying to replace root with next smallest. */
    ...
    {
	if (DMZ_SRT_MACRO(1))
	TRdisplay("%x DMSE_INPUT: Records = %d Runs = %d Merge_order = %d Passes = %d\n", 
	    s->srt_sid,
...
i4            *err_code)
{
    ...
    if (!(sort_cb->srt_state & SRT_PARALLEL))
	return(dmse_get_record_serial(sort_cb, record, err_code));
...
i4            *err_code)
{
    ...
    if ((s->srt_state & SRT_HEAP_BECAME_FULL) == 0)
	return (get_in_memory_record( sort_cb, record, err_code ));
	...
	*record = ((DMS_MERGE_DESC *)heap[1])->record;
    ...
    if (s->srt_state & SRT_PUT)
	s->srt_err_code = E_DM9706_SORT_PROTOCOL_ERROR;

    *err_code = s->srt_err_code;
    if (*err_code > E_DM_INTERNAL)
    {
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM970A_SORT_GET;
    }
    return (E_DB_ERROR);
}

...
i4            *err_code)
{
    ...
    ** sort environment. */
    return(dmse_end_serial(sort_cb, err_code));
}

/*{
...
i4            *err_code)
{
    ...
    *err_code = E_DB_OK;
    ...
    {
	if (DMZ_SRT_MACRO(1))
	    TRdisplay("%x DMSE_END  : Records = %d Reads = %d Writes = %d Comparisons = %d\n",
		s->srt_sid,
	...
	{
	    ule_format(E_DM9701_SR_CLOSE_ERROR, &sys_err,
		ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	    *err_code = E_DM9708_SORT_END;
	}
...
DMS_SRT_CB        *s)
{
	...
	if ( s->srt_pstatus == E_DB_OK )
	{
	    ...
	    s->srt_perr_code = E_DM0065_USER_INTR;

	    /* *** Eschew annoying and unhelpful messages...
	    ** The interrupt could have been an error propagating thru
	    ** a partitioned modify...
	    ** ule_format(s->srt_perr_code, 0, ULE_LOG, NULL, 
	    **		0, 0, 0, &s->srt_perr_code, 0);
	    ** ule_format(E_DM9715_PSORT_PFAIL, 0, ULE_LOG, NULL, 
	    **		0, 0, 0, &s->srt_perr_code, 0);
	    */
	}
    }
...
i4		    passes)
{
    ...
    */
    ...
    {
	if (s->srt_err_code == 0)
	    s->srt_err_code = E_DM970C_SORT_DOMERGE;
	return (E_DB_ERROR);
    }
	...
	*/
	if (s->srt_err_code > E_DM_INTERNAL)
	{
	    ule_format(s->srt_err_code, 0, ULE_LOG, NULL, 0, 0, 0,
					    &s->srt_err_code, 0);
	    s->srt_err_code = E_DM970C_SORT_DOMERGE;
	}
    }
    ...
    {
	ule_format(E_DM9701_SR_CLOSE_ERROR, &sys_err,
	    ULE_LOG, NULL, 0, 0, 0, &s->srt_err_code, 0); 
	s->srt_err_code = E_DM970C_SORT_DOMERGE;
	status = E_DB_ERROR;
    }
...
DMS_MERGE_DESC	   *merge_desc)
{
	...
	for (;;)
	{
	    ...
	    m->sort_cb->srt_err_code = E_DM0065_USER_INTR;
	    return (E_DB_ERROR);
	}

	/*	Handle error reporting. */

	ule_format(E_DM9702_SR_READ_ERROR, &sys_err, ULE_LOG, NULL, 0, 0, 0,
	    &m->sort_cb->srt_err_code, 0);
	m->sort_cb->srt_err_code = E_DM970E_SORT_READ_RECORD;
	return (E_DB_ERROR);
    }
}

...
char               *record)
{
	...
	if (status != E_DB_OK)
	{
		...
		ule_format(s->srt_err_code, 0, ULE_LOG, NULL, 0, 0, 0,
		    &s->srt_err_code, 0);
		s->srt_err_code = E_DM970D_SORT_WRITE_RECORD;
	    }
	    return (E_DB_ERROR);
	}
    }
	...
	*/
	if ((s->srt_state & SRT_OUTPUT) == 0)
	{
	    ...
	    {
		if (s->srt_err_code == 0)
		    s->srt_err_code = E_DM970F_SORT_OUTPUT_RUN;
		return (E_DB_ERROR);
	    }
	...
	if (stat != OK)
	{
	    ...
	    {
		ule_format(E_DM9703_SR_WRITE_ERROR, &sys_err, ULE_LOG, NULL,
		    0, 0, 0, &s->srt_err_code, 0);
		s->srt_err_code = E_DM0112_RESOURCE_QUOTA_EXCEED;
		return (E_DB_ERROR);
	    }
	    break;
	}
        s->srt_err_code = E_DM0065_USER_INTR;
        return (E_DB_ERROR);
    }

    /*	Handle write error reporting. */

    ule_format(E_DM9703_SR_WRITE_ERROR, &sys_err, 
                   ULE_LOG, NULL, 0, 0, 0, &s->srt_err_code, 0);
    s->srt_err_code = E_DM970D_SORT_WRITE_RECORD;
    return (E_DB_ERROR);
}
...
i4            flag)
{
	...
	if (s->srt_o_blkno != s->srt_orun_desc.next_block)
	{
		...
		if (stat == SR_EXCEED_LIMIT)
		{
		    ule_format(E_DM9703_SR_WRITE_ERROR, &sys_err, ULE_LOG, NULL, 0, 0, 0,
			&s->srt_err_code, 0);
		    s->srt_err_code = E_DM0112_RESOURCE_QUOTA_EXCEED;
		    return (E_DB_ERROR);
		}
...
i4            flag)
{
	...
	if (s->srt_orun_desc.next_block)
	{
	    ...
	    if (stat != OK)
	    {
		ule_format(E_DM9702_SR_READ_ERROR, &sys_err, 
                        ULE_LOG, NULL, 0, 0, 0, &s->srt_err_code, 0);
		s->srt_err_code = E_DM9710_SORT_INPUT_RUN;
		return (E_DB_ERROR);
	    }
	}
	else
...
i4            *err_code)
{
	...
	*record = (char *)heap[1];
	...
	for (i = 1, j = 2; j <= s->srt_next_record; )
	{
	    ...
	    heap[i] = heap[j];
	    if (heap[j] == NULL) break;		/* end of branch */
	    heap[j] = NULL;
	    i = j;
	    j += j;
	}
	...
	*/
	...
	for (retry_cnt = 0;;)
	{
	    ...
	    */
	    if (stat == SR_EXCEED_LIMIT)
	    {
		status = dm2t_reclaim_tcb(s->srt_lk_id, s->srt_log_id, &err_code);
		if (status != E_DB_OK)
		{
	...
	if (stat != OK)
	{
	    ule_format(E_DM9711_SR_OPEN_ERROR, &sys_err, ULE_LOG, NULL, 0, 0,
			0, &err_code, 2,
			s->srt_array_loc[next_bit].phys_length,
			s->srt_array_loc[next_bit].physical.name,
		...
		s->srt_err_code = 0;	/* let caller set the error code */
...
**	err			CL_SYS_ERR if SR call failed
**
** Outputs:
**	Returns:
**	    OK			Ok.
...
CL_SYS_ERR	    *err)
{
	...
	if (stat)
	{
	    ...
	    */
	    ...
	    ule_format(E_DM9712_SR_READ_ERROR, err, ULE_LOG, NULL, 0, 0,
			0, &err_code, 2,
			s->srt_array_loc[next_bit].phys_length,
	...
	if (stat)
	{
	    ...
	    ule_format(E_DM9713_SR_WRITE_ERROR, err, ULE_LOG, NULL, 0, 0,
			0, &err_code, 2,
			s->srt_array_loc[next_bit].phys_length,
			s->srt_array_loc[next_bit].physical.name, 0, rbn);
	}
...
i4		in_or_out)
{
    ...
    */
	...
	if (stat != OK)
	{
	    ule_format(E_DM9700_SR_OPEN_ERROR, &sys_err, ULE_LOG, NULL, 0, 0,
			0, &err_code, 0);
	    if (stat == SR_EXCEED_LIMIT)
		s->srt_err_code = E_DM0112_RESOURCE_QUOTA_EXCEED;
	    else
