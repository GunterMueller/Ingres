...
i4	    *err_code)
{
	...
	*/
	if(tcb->tcb_rel.relstat2&TCB_ROW_AUDIT)
	{
		...
		{
			status=E_DB_ERROR;
			*err_code=E_DM910E_ROW_SEC_AUDIT_ERROR;
			break;
		}
		...
		*/
		...
		else
		{
			...
			** Log ADF error.
			*/
			(VOID) ule_format(adf_cb->adf_errcb.ad_errcode, 0, ULE_LOG, NULL,
			    (char *)0, 0L, (i4 *)0, &local_error, 
			    0);
			*err_code=E_DM910E_ROW_SEC_AUDIT_ERROR;
			break;
		    }
	...
	if(c2server)
	{
		...
		*/
		...
		if(status!=E_DB_OK)
		{
			...
			*/
			(VOID) ule_format(sxfrcb.sxf_error.err_code, 0, ULE_LOG, NULL,
			    (char *)0, 0L, (i4 *)0, &local_error, 0);
			(VOID) ule_format(E_DM910E_ROW_SEC_AUDIT_ERROR, 0, 
				ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, 
				&local_error, 0);
			*err_code = E_DM0126_ERROR_WRITING_SECAUDIT;
			break;
		}
	}
	break;
