...
i4		reserved_space)
{
        ...
        if ( DMZ_MACRO(scb->scb_trace, 12) )
	{
	    ...
	    TRformat(print, 0, buffer, sizeof(buffer) - 1,
	"%8* LSN=(%d,%d)(%x%x), COMP_LSN=(%d,%d)(%x%x), DBID=0x%x, XID=0x%x%x\n", 
...
BMINI,EMINI,OLD_CREATE,OLD_DESTRY,OLD_MODIFY,OLD_RELOCATE,OLD_INDEX,BALLOC,\
DBEALLOC,CALLOC,\
SAVEPOINT,ABORTSAVE,OLD_FRENAM,OLD_FCREAT,CONFIG,OPENDB,RAWDATA,P1,BPEXTEND,BCP,\
OLD_SM1,OLD_SM2,LOCATION,SETBOF,JNLEOF,ARCHIVE,LPS,OLD_ALTER,\
OLD_LOAD,ECP,SBACKUP,EBACKUP,OLD_CRDB,OLD_DMU,OLD_FSWAP,TEST,OLD_MODIFY,INDEX,\
SM0PURGE,OLD_ASSOC,\
OLD_ALLOC,OLD_DEALL,OLD_EXTEND,OLD_OFLOW,,,,,,60,\
,,,,,,,,,70,,,,,,,,,,80,,,,,,,,,,90,,,,,,,,,,100,\
...
BTFREE,BTUPDTOVFL,DISASSOC,166,167,168,BTREE_PUT,BTREE_DEL,BSF,ESF,RNL_LSN,UNEXTEND,BTINFO,176,177,178",
	    log_hdr->type, log_hdr->length, reserved_space,
    "JNL,READONLY,MASTER,BTREE,SPECIAL,NOTDB,ADDONLY,MINI,FASTCOMMIT,NONREDO,\
CKP,JON,WILLING_COMMIT,CLUSTER,PRETEND,NOLOGGING,DMP,NONJNLTAB,CLR,PLOCK", 
	    log_hdr->flags);
    }
...
DB_DB_NAME   *database_name)
{
	...
	if ( timeout == DMC_C_NOWAIT )
	    request_flag |= LK_NOWAIT;

	if (lock_key->lk_type == LK_PAGE)
	    TRformat(format_routine, 0, buffer, sizeof(buffer) - 1,
		"    %s:   PAGE  %4v Mode: %3w Timeout: %4d Key: (%~t,%~t,%d.%d)\n",
		rtype,
		"NOWT,CONV,PHYS,STAT,ESCT,LOCL,NODK,TEMP,NOIN,PROP,INTR,DEAD,QUTM,,,MTHD", request_flag,
		"N,IS,IX,S,SIX,U,X", lock_mode, timeout,
		sizeof(*database_name),database_name,
		sizeof(*table_name), table_name,
	    ...
	    TRformat(format_routine, 0, buffer, sizeof(buffer) - 1,
		"    %s:   TABLE %4v Mode: %3w Timeout: %4d Key: (%~t,%~t)\n",
		rtype,
		"NOWT,CONV,PHYS,STAT,ESCT,LOCL,NODK,TEMP,NOIN,PROP,INTR,DEAD,QUTM,,,MTHD", request_flag,
		"N,IS,IX,S,SIX,U,X", lock_mode, timeout,
		sizeof(*database_name),database_name,
    ...
    "    %s:   SV_PAGE %v Mode: %3w Key: (DB=%x,TABLE=[%d,%d],PAGE=%d)\n",
	    ...
	    TRformat(format_routine, 0, buffer, sizeof(buffer) - 1,
	    "    %s:   ROW   %4v Mode: %3w Timeout: %4d Key: (%~t,%~t,%d.%d,%d)\n",
		rtype,
		"NOWT,CONV,PHYS,STAT,ESCT,LOCL,NODK,TEMP,NOIN,PROP,INTR,DEAD,QUTM,,,MTHD", request_flag,
		"N,IS,IX,S,SIX,U,X", lock_mode, timeout,
		sizeof(*database_name),database_name,
	    ...
	    TRformat(format_routine, 0, buffer, sizeof(buffer) - 1,
       "    %s:   VALUE %4v Mode: %3w Timeout: %4d Key: (%~t,%~t,%d,%d,%d)\n",
		rtype,
		"NOWT,CONV,PHYS,STAT,ESCT,LOCL,NODK,TEMP,NOIN,PROP,INTR,DEAD,QUTM,,,MTHD", request_flag,
		"N,IS,IX,S,SIX,U,X", lock_mode, timeout,
		sizeof(*database_name),database_name,
