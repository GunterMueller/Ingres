...
DMM_CB    *dmm_cb)
{
    ...
    {
	err_code = E_DM0072_NO_LOCATION;
    }

    if ( dmm->error.err_code = err_code )
	return (E_DB_ERROR);
        ...
        if (status == DI_DIRNOTFOUND)
	{
	    ...
	    */
	    destroy_loc_list(dmm, &err_code);
            return (E_DB_OK);
	}
	ule_format(status, &sys_err, ULE_LOG, NULL,
			0, 0, 0, &err_code, 0);
	dmm->error.err_code = E_DM011A_ERROR_DB_DIR;
	return (E_DB_ERROR);
    }
	...
	if (status == LK_BUSY)
	{
	    dmm->error.err_code = E_DM004C_LOCK_RESOURCE_BUSY;
	    return (E_DB_ERROR);
	}
	...
	ule_format(E_DM901C_BAD_LOCK_REQUEST, &sys_err, ULE_LOG, NULL,
	    0, 0, 0, &err_code, 2, 0, LK_X, 0, xcb->xcb_lk_id);
	dmm->error.err_code = E_DM011C_ERROR_DB_DESTROY;
	return (E_DB_ERROR);
    }
    ...
    */
    ...
    open_status = dm0c_open(&dcb, cnf_flag, xcb->xcb_lk_id, &cnf, &open_err_code);
    if (open_status)
    {
	...
	*/
    ...
    if (open_status != E_DB_OK && open_err_code != E_DM012A_CONFIG_NOT_FOUND)
    {
	ule_format(open_err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
			0, 0, 0, &err_code, 0);
    }
    if ((open_status != E_DB_OK && open_err_code == E_DM012A_CONFIG_NOT_FOUND)
	 || status == E_DB_OK)
...
CL_SYS_ERR	*sys_err)
{
    ...
    ule_format(delete_info->error, (CL_ERR_DESC *)sys_err, ULE_LOG, NULL,
			0, 0, 0, &err_code, 0);
