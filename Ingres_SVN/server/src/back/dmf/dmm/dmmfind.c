...
i4		*error)
{
    ...
    ADF_CB		*adf_cb = (ADF_CB *)rcb->rcb_adf_cb;
	...
	for (i=0; i<tcb->tcb_rel.relatts; i++)
	{
		...
		*/
		...
		{
		    ule_format(adf_cb->adf_errcb.ad_errcode, 0, ULE_MESSAGE, 
			       (DB_SQLSTATE *) NULL,
			       adf_cb->adf_errcb.ad_errmsgp, 
			       adf_cb->adf_errcb.ad_emsglen, 0, error, 0);
	...
	if (ret_stat != E_DB_OK)
	{
	    ...
	    {
		ule_format(*error, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		    &err_code, 0);
		*error =  E_DM919D_BAD_IIPHYS_DATABASE;
	    }
	    ret_stat = E_DB_ERROR;
...
DMM_CB    *dmm_cb)
{
    ...
    LOGDB_CB	    logdb_cb;	    /* Control block to pass parameters to
				    ** routine Log_db() */
    i4	    err_code;	    /* place for uleformat to return
	...
	*/
	ret_stat = open_dbtbl(odcb->odcb_dcb_ptr, &logdb_cb, &error);

	if (ret_stat != E_DB_OK)
	{
	    ule_format(error, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		&err_code, 0);
	    error = E_DM0144_NONFATAL_FINDBS_ERROR;
	    break;
	}	
	ret_stat = open_exttbl(odcb->odcb_dcb_ptr, &logdb_cb, &error);
	if (ret_stat != E_DB_OK)
	{
	    ule_format(error, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		&err_code, 0);
	...
	*/
	...
	{
	    status = dmxe_writebt(xcb, FALSE, &error);
	    if (status != E_DB_OK)
	    {
		...
		** so processing was aborted */
		if (logdb_cb.list_stat != E_DM0000_OK)
		{
		    ...
		    */
		    ...
		    {
			ule_format (logdb_cb.list_stat, 0, ULE_LOG,
				    (DB_SQLSTATE *) NULL,
				    0, 0, 0, &error, 4,
				    0, logdb_cb.dir_name, 0, logdb_cb.loc_name,
				    0, logdb_cb.path_name, 
				    DB_MAXNAME, logdb_cb.filename);
			error = E_DM0145_FATAL_FINDBS_ERROR;
			ret_stat = E_DB_FATAL;
		    }
	    ...
	    {
		/* unantisipated DIlistdir error.  Report as warning */
		ret_stat = E_DB_WARN;
		error = logdb_cb.list_stat;
	    }
	}
    ...
    */
	...
	if (logdb_cb.db_rcb)
	{
	    locstat = dm2t_close(logdb_cb.db_rcb, DM2T_NOPURGE, &locerror);
	...
	if (logdb_cb.ext_rcb)
	{
	    locstat = dm2t_close(logdb_cb.ext_rcb, DM2T_NOPURGE, &locerror);
	...
	if (error > E_DM_INTERNAL)
	{
	    ule_format( error, 0, ULE_LOG , (DB_SQLSTATE *) NULL, 
		(char * )0, 0L, (i4 *)0, 
		&loc_error, 0);
	    error = E_DM0144_NONFATAL_FINDBS_ERROR;
	}
	if (error == E_DM0144_NONFATAL_FINDBS_ERROR)
	...
	dmm->error.err_code = E_DM0000_OK;
...
PTR         p12val;
{
    ...
    *error = E_DM0000_OK;
    buf = &cbuf[0];
    stat = ule_format(msg_id, NULL, ULE_LOOKUP, (DB_SQLSTATE *) NULL, buf,
...
i4	    *error)
{
	...
	** iitemp_locations file */
	...
	locstat = dm2t_close(rcb, DM2T_NOPURGE, &locerror);
	rcb = (DMP_RCB *) NULL;
	if (locstat != E_DB_OK)
	{
...
char	    *name)
{
    ...
    ** and translate to uname */
	...
	if (ret_stat != E_DB_OK)
	{
	    ule_format(error, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		&err_code, 0);
	    ule_format (E_DM9199_BADOPEN_TEMPCODEMAP, 0, ULE_LOG,
		(DB_SQLSTATE *) NULL, 0, 0, 0, &locerr, 0);
	    break;
	}
	...
	** iitemp_codemap file */
	...
	if (ret_stat != E_DB_OK)
	{
	    ...
	    */
		...
		ule_format(adf_cb->adf_errcb.ad_errcode, 0, ULE_MESSAGE, 
			   (DB_SQLSTATE *) NULL,  adf_cb->adf_errcb.ad_errmsgp, 
			   adf_cb->adf_errcb.ad_emsglen, 0, &err_code, 0);
		ule_format(E_DM919A_BADREAD_TEMPCODEMAP, 0, ULE_LOG,
	...
	ret_stat = dm2t_close(rcb, DM2T_NOPURGE, &error);
	rcb = (DMP_RCB *) NULL;
	if (ret_stat != E_DB_OK)
	{
	    logdb_cb->list_stat = E_DM5410_CLOSE_ERROR;
...
i4			*error)
{
	...
	*error = E_DM0000_OK;
	...
	if (locstat != E_DB_OK)
	{
	    ule_format(*error, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		&err_code, 0);
	    *error = E_DM9191_BADOPEN_IIPHYSDB;
	    break;
	}
	...
	locstat = dm0m_allocate( sizeof(DMP_MISC) + 
				...
				(char *) 0, (DM_OBJECT **) &alloc_ptr, error);
	if (locstat != E_DB_OK)
	{
	    ule_format(*error, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		&err_code, 0);
	    *error = E_DM9188_DMM_ALLOC_ERROR;
	    break;
	}
		...
		*/
		if ( adf_cb->adf_errcb.ad_emsglen)
		{
		    ule_format(adf_cb->adf_errcb.ad_errcode, 0, ULE_MESSAGE,
			       ...
			       adf_cb->adf_errcb.ad_errmsgp, 
			       adf_cb->adf_errcb.ad_emsglen, 0, &err_code, 0);
		    *error = E_DM9193_BAD_PHYSDB_TUPLE;
		}
		else
		if	(adf_cb->adf_errcb.ad_errcode)
		{
	...
	*/
	...
	/* DBCMPTLVL -- this guy is tricky.  He is treated like a char[4]
	**		    in the data, but for some reason the DMF structure
	**		    was changed to an I4.  We must treat the data the
	**		    way it looks (ie, DB_CHA_TYPE), but must get it
	**		    from a pointer to an i4 
	*/
	dv[DBCMPTLVL].db_length = sizeof(iidatabase->du_dbcmptlvl);
	dv[DBCMPTLVL].db_datatype = (DB_DT_ID) DB_CHA_TYPE;
	dv[DBCMPTLVL].db_data = (PTR) &iidatabase->du_dbcmptlvl[0];
...
i4			*error)
{
	...
	*error = E_DM0000_OK;
	...
	if (locstat != E_DB_OK)
	{
	    ule_format(*error, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		&err_code, 0);
	    *error = E_DM9192_BADOPEN_IIPHYSEXT;
	    break;
	}	    
		...
		*/
		if ( adf_cb->adf_errcb.ad_emsglen)
		{
		    ule_format(adf_cb->adf_errcb.ad_errcode, 0, ULE_MESSAGE,
			       ...
			       adf_cb->adf_errcb.ad_errmsgp, 
			       adf_cb->adf_errcb.ad_emsglen, 0, &err_code, 0);
		    *error = E_DM9194_BAD_PHYSEXT_TUPLE;
		}
		else
		if	(adf_cb->adf_errcb.ad_errcode)
		{
...
i4	    *ret_err)
{
    ...
    *ret_err = E_DM0000_OK;
	...
	if (error == E_DM5410_CLOSE_ERROR)
	    ...
	    (VOID) ule_format(error, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		&error2, 0);
	...
	/* dbcmptlvl */
    STcopy (DU_CUR_DBCMPTLVL, temp);
    MEfill(sizeof(iidatabase->du_dbcmptlvl), ' ', iidatabase->du_dbcmptlvl);
    MEcopy (temp, STlength(temp), iidatabase->du_dbcmptlvl);
	/* dbcmptminor */
...
i4	    *ret_err)
{
	...
	*/
	...
	if (di_stat != OK)
	{
	    ule_format(E_DM9007_BAD_FILE_SENSE, 0, ULE_LOG,
	...
	*/
	...
	ret_stat = dm0m_allocate( size, DM0M_ZERO, 
				(i4) MISC_CB, (i4) MISC_ASCII_ID,
				(char *) 0, (DM_OBJECT **) &alloc_ptr, ret_err);
	if (ret_stat != E_DB_OK)
	{
	    ule_format(*ret_err, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0, 0,
		&error, 0);
	...
	MEcopy(desc->dsc_dbcmptlvl, sizeof(desc->dsc_dbcmptlvl),
		iidatabase->du_dbcmptlvl);
	...
	{    
		...
		** information into iiphys_extend */
		...
		if (ret_stat != E_DB_OK)
		{
		    ...
		    ** internal err */
		    ule_format(*ret_err, 0, ULE_LOG, (DB_SQLSTATE *) NULL, 0, 0,
			0, &error, 0);
