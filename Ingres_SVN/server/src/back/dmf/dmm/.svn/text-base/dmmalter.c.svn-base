...
DMM_CB    *dmm_cb)
{
    ...
    */
    ...
    status = dm2d_alter_db(&dm2d_alter_info, &err_code);

    if (status == E_DB_OK)
	    return (E_DB_OK);

    if ((err_code == E_DM012A_CONFIG_NOT_FOUND) &&
	((dmm->dmm_db_access & DU_DESTROYDB) != 0))
    {
	return (E_DB_OK);
    }
	...
	err_code = E_DM011D_ERROR_DB_ALTER;
    }
...
DMM_CB    *dmm_cb)
{
    ...
    *retval = 0;
    status = dm0c_open(&dcb, 0, xcb->xcb_lk_id, &cnf, &err_code);
    if (status == E_DB_OK)
    {
	...
	*/
	...
	{
	    MEcopy( (PTR) cnf->cnf_dsc->dsc_dbcmptlvl, 
		    sizeof(cnf->cnf_dsc->dsc_dbcmptlvl), 
		    (PTR) retval);
	    MEcopy( (PTR) cnf->cnf_dsc->dsc_dbcmptlvl,
		     sizeof(cnf->cnf_dsc->dsc_dbcmptlvl),
		    (PTR) dmm->dmm_cmptlvl);
	...
	{   
	    ...
	    if (dmm->dmm_cmptlvl[0] )
		MEcopy( (PTR)dmm->dmm_cmptlvl, sizeof(dmm->dmm_cmptlvl),
			(PTR)cnf->cnf_dsc->dsc_dbcmptlvl);
	    if (dmm->dmm_dbcmptminor)
		cnf->cnf_dsc->dsc_1dbcmptminor = dmm->dmm_dbcmptminor;
	    if (dmm->dmm_db_access)
		cnf->cnf_dsc->dsc_dbaccess = dmm->dmm_db_access;
	    if (dmm->dmm_db_service)
    ...
    ule_format(status, 0, ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
    dmm->error.err_code = E_DM011D_ERROR_DB_ALTER;
    return (E_DB_ERROR);
}
