...
DMM_CB        *dmm_cb)
{
	...
	if ( (dmm->dmm_table_name.db_tab_name[0] == NULL) ||
	     (dmm->dmm_owner.db_own_name[0] == NULL) )
	{
	    error = E_DM918E_SPECIFY_LIST_TABLE;
	    break;
	}
	if (dmm->dmm_att_name.db_att_name[0] == NULL)
	{
	    error = E_DM918C_SPECIFY_LIST_COLNAME;
	    break;
	}
...
i4			*err_code)
{
	...
	*err_code = E_DM918B_DMM_SYSCAT_ERROR;
	...
	for (;;)
	{
	    ...
	    */
	    ...
	    if (locstat != E_DB_OK && error != E_DM0055_NONEXT)
		break;
	    if (locstat == E_DB_OK)
	    {
	...
	if (locstat != E_DB_OK)
	{
		...
		*err_code = E_DM918A_LISTTAB_NONEXISTENT;
		error = E_DM918A_LISTTAB_NONEXISTENT;
	    break;
	}

	/* Close the secondary index table, no longer needed. */
	locstat = dm2t_close(r_idx, DM2T_NOPURGE, &error);
	r_idx = NULL;
	if (locstat != E_DB_OK)
	    break; 
	...
	*/
	...
	locstat = dm2t_open(dcb, &table_id, DM2T_IX,
			DM2T_UDIRECT, DM2T_A_READ, (i4)0, (i4)20, 
			0, 0,dmf_svcb->svcb_lock_list, (i4)0,(i4)0, 
			0,&tran_id, &timestamp, &rel, (DML_SCB *)0, &error);
	if (locstat != E_DB_OK)
	    break;
	...
	locstat = dm2t_close(rel, DM2T_NOPURGE, &error);
	rel = NULL;
	if (locstat != E_DB_OK)
	    break; 
	...
	*/
	...
	if (locstat != E_DB_OK)
	{
	    error = E_DM9189_DMM_LISTTAB_OPEN_ERR;
	    break;
	}
	...
	*/
	...
	locstat = dm0m_allocate( sizeof(DMP_MISC) + 
				...
				(char *) 0, (DM_OBJECT **) &tuple, &error);
	if (locstat != E_DB_OK)
	{
	    *err_code = E_DM9188_DMM_ALLOC_ERROR;
	    break;
	}
	...
	*/
	adf_cb.adf_errcb.ad_ebuflen = NULL;
	...
	*/
	...
	for (i=1, col_id = NULL; i<= tcb->tcb_rel.relatts; i++)
	{
		...
		*/
		...
		{
		    ule_format(adf_cb.adf_errcb.ad_errcode, 0, ULE_MESSAGE, NULL, 
			       adf_cb.adf_errcb.ad_errmsgp, 
			       adf_cb.adf_errcb.ad_emsglen, 0, &error, 0);
		    *err_code = E_DM9182_CANT_MAKE_LIST_TUPLE;
		}
		else
	...
	{
	    locstat = E_DB_ERROR;
	    error = E_DM9187_INVALID_LIST_TBL_FMT;
	    break;
	}
	...
	*/
	...
	{
	    error = E_DM0140_EMPTY_DIRECTORY;
		...
		*/
		if (error == E_DM0140_EMPTY_DIRECTORY)
		{
		...
		*/
		    locstat = E_DB_ERROR;
		    *err_code = E_DM9186_LISTDIR_NONEXISTENT;
		    break;
	...
	*/
	...
	locstat = dm2t_close(rcb, DM2T_NOPURGE, &error);
	...
	*/
	if ( error > E_DM_INTERNAL)
	{
	    ...
	    *err_code = E_DM9184_DMMLIST_FAILED;
	...
	if (r_idx)
	{
	    tempstat = dm2t_close(r_idx, DM2T_NOPURGE, &error);
	}
	if (rel)
	{
	    tempstat = dm2t_close(rel, DM2T_NOPURGE, &error);
	}
	if (rcb)
	{
	    tempstat = dm2t_close(rcb, DM2T_NOPURGE, &error);
	}
...
CL_ERR_DESC  *sys_err)	    /* CL error status */
{
	...
	*/
	...
	if (locstat != E_DB_OK)
	{
	    ...
	    */
		...
		ule_format(adf_cb->adf_errcb.ad_errcode, 0, ULE_MESSAGE, NULL, 
			   adf_cb->adf_errcb.ad_errmsgp, 
			   adf_cb->adf_errcb.ad_emsglen, 0, err, 0);
	...
	*/
	...
	locstat = dm2r_put(rcb, DM2R_NODUPLICATES, tuple->misc_data, err);
	if (locstat != E_DB_OK)
	{
	    /* generate error message that name could not be cohersed into table,
	    ** then return with nonzero status to DIlistxxx().
	    */
	    if ( *err > E_DM_INTERNAL)
	    {
		ule_format(*err, 0, ULE_LOG, NULL, 0, 0, 0, err, 0);
