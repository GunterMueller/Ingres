...
DMM_CB    *dmm_cb)
{
	...
	if ( loc_list[0]->loc_raw_pct )
	{
	    ...
	    {
		err_code = E_DM0197_INVALID_RAW_USAGE;
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	if ( status = dmm_loc_validate(loc_list[0]->loc_type, 
		    ...
		    area, &l_area, db_dir, &l_db_dir, &loc_flags, &err_code) )
	{
	    /*
	    ...
	    */
	    if ( err_code == E_DM0080_BAD_LOCATION_AREA &&
		 loc_list[0]->loc_raw_pct &&
		 (dmm->dmm_flags_mask & DMM_EXTEND_LOC_AREA) == 0 )
	    {
		err_code = E_DM0198_RUN_MKRAWAREA;
		status = E_DB_WARN;
	    }
	    break;
	}
	...
	*/
	...
	else if ( loc_list[0]->loc_raw_pct )
	{
	    ...
	    err_code = E_DM0196_AREA_NOT_RAW;
	    status = E_DB_ERROR;
	    break;
	}
	...
	if ( dmm->dmm_flags_mask & DMM_EXTEND_LOC_AREA )
	{
	    ...
	    */
	    ...
	    {
		err_code = E_DM0080_BAD_LOCATION_AREA;
		status = E_DB_ERROR;
	    }
	    else
	    {
...
STATUS		*err_code)
{
    ...
    *err_code = E_DB_OK;
	...
	case DMM_L_DATA:
	    ...
	    break;
	default:
	    ule_format(E_DM919E_DMM_LOCV_BADPARAM, (CL_ERR_DESC *)0,
	...
	if ( *flags & DMM_EXTEND_LOC_AREA && *err_code == E_DB_OK &&
	     (loc_flags & DCB_RAW) == 0 )
	{
		...
		{
		    if (type == DMM_L_CKP)
			ule_format(E_DM919F_DMM_LOCV_CKPEXISTS, (CL_ERR_DESC *)0,
				    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		    else
			ule_format(E_DM91A0_DMM_LOCV_DMPEXISTS, (CL_ERR_DESC *)0,
				    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		    *err_code = E_DM0124_DB_EXISTS;
		}
		else if (status != CK_DIRNOTFOUND)
		{
		    ule_format(status, (CL_ERR_DESC *)&sys_err,
				    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		    ule_format(E_DM91A3_DMM_LOCV_CKLISTFILE, (CL_ERR_DESC *)0,
				    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		    *err_code = E_DM011A_ERROR_DB_DIR;
		}
		break;
		...
		*/
		...
		if (status != DI_ENDFILE)
		{
		    ule_format(status, (CL_ERR_DESC *)&sys_err,
				    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		    ule_format(E_DM91A8_DMM_LOCV_DILISTDIR, (CL_ERR_DESC *)0,
		...
		{
		    ule_format(E_DM91A2_DMM_LOCV_JNLEXISTS, (CL_ERR_DESC *)0,
				    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		    *err_code = E_DM0124_DB_EXISTS;
		}
		else if (status != JF_DIR_NOT_FOUND)
		{
		    ule_format(status, (CL_ERR_DESC *)&sys_err,
				    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		    ule_format(E_DM91A6_DMM_LOCV_JFLISTFILE, (CL_ERR_DESC *)0,
				    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		    *err_code = E_DM011A_ERROR_DB_DIR;
		}
		break;
