...
DMM_CB        *dmm_cb)
{
	...
	if ( (dmm->dmm_db_location.data_in_size ==0) ||
	     ...
	     (*fil == '\0') || (*path == '\0'))
	{
	    error = E_DM9181_SPECIFY_DEL_FILE;
	    break;
	}
	...
	*/
	...
	status = dmm_do_del(path, (u_i4) dmm->dmm_db_location.data_in_size,
		  fil, (u_i4) dmm->dmm_filename.data_in_size, &error);
	break;	    
...
static READONLY char *core_files[]=
{
...
static READONLY char *dbms_files[]=
{
	/*iitree            */      "aaaaaabf.t",
...
dmm_del_config(DMM_CB *dmm_cb)
{
    ...
    {
    	dmm->error.err_code = E_DM9190_BAD_FILE_DELETE;
    	status = E_DB_ERROR;
    }
    else
    {
