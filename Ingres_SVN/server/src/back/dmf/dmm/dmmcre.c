...
DMM_CB    *dmm_cb)
{
	...
	{
	    error = E_DM0072_NO_LOCATION;
	}
    }

    if ( dmm->error.err_code = error )
	return (E_DB_ERROR);
...
i4		*err_code)
{
    ...
    */
    ...
    if (status != E_DB_OK)
    {
	*err_code = E_DM0103_TUPLE_TOO_WIDE;
	return(status);
    }
	...
	loc_dir, &l_loc_dir, db_dir, &l_db_dir, &loc_flags, err_code);
    if (status != E_DB_OK)
	return (status);
    ...
    {
	*err_code = E_DM0199_INVALID_ROOT_LOCATION;
	return(E_DB_ERROR);
    }
    ...
    */
    if (access & DU_RDONLY)
    {
       ...
       */
           ...
           {
               *err_code = E_DM0192_CREATEDB_RONLY_NOTEXIST;
               return (E_DB_ERROR);
           }
       }
	rdloc_flags = DMM_EXTEND_LOC_AREA;
		...
		rddb_dir, &l_rddb_dir, &rdloc_flags, err_code);
    	if (status != E_DB_OK)
	    return (status);
    }
	...
	*/
	...
	status = dm0m_allocate((DM0C_PGS_HEADER*DM_PG_SIZE) + sizeof(DMP_MISC),
	    ...
	    (char *)NULL, (DM_OBJECT **)&cnf, err_code);
	if (status != E_DB_OK)
	{
	...
	MEmove(STlength(DU_CUR_DBCMPTLVL), DU_CUR_DBCMPTLVL, ' ',
		sizeof(desc->dsc_dbcmptlvl), &desc->dsc_dbcmptlvl[0]);
	...
	for (i = 0; i < loc_count; i++)
	{
		...
		if (status != OK)
		{
			...
			l_wloc_dir, wloc_dir,
			db_length, db_buffer);
		    *err_code = E_DM011B_ERROR_DB_CREATE;
		    break;
		}
	    }
	...
	*eof = 0;
    ...
    if (status == E_DB_OK)
	    ...
	    status = create_db(xcb, cnf, db_name, db_id, access,
	    	...
	    	l_rddb_dir, (DM_PATH *)rddb_dir, page_type, page_size, err_code);
	else
	    status = create_db(xcb, cnf, db_name, db_id, access,
	    	service, location_name, l_loc_dir, (DM_PATH *)loc_dir, 
	    	l_db_dir, (DM_PATH *)db_dir, page_type, page_size, err_code);
...
i4		*err_code)
{
	...
	dbstatus = dm0l_crdb(x->xcb_log_id, (i4)0,
		...
		path_length, path_name, (LG_LSN *)0, err_code);
	if (dbstatus != E_DB_OK)
	{
	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
			0, 0, 0, err_code, 0);
    ...
    */
    ...
    if (status != OK)
    {
	ule_format(status, &sys_err, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM011A_ERROR_DB_DIR;
	return (E_DB_ERROR);
    }
	...
	if ( status != OK )
	{
	    ule_format(status, &sys_err, ULE_LOG,0,0,0,0,err_code,0);
	    ule_format(E_DM9337_DM2F_CREATE_ERROR, &sys_err, ULE_LOG, NULL,
		       0, 0, 0, err_code, 0);
	    break;
	}
	...
	{
	    ule_format(status, &sys_err, ULE_LOG,0,0,0,0,err_code,0);
            ule_format( E_DM9004_BAD_FILE_OPEN, &sys_err, ULE_LOG, 
			NULL, 0, 0, 0, err_code, 4,
	...
	{
	    ule_format(status, &sys_err, ULE_LOG,0,0,0,0,err_code,0);
            ule_format( E_DM9000_BAD_FILE_ALLOCATE, &sys_err, ULE_LOG, 
			NULL, 0, 0, 0, err_code, 4,
			sizeof(DB_DB_NAME), db_name,
			11, "config file",
			path_length, path_name->name,
	...
	{
	    ule_format(status, &sys_err, ULE_LOG,0,0,0,0,err_code,0);
            ule_format( E_DM9008_BAD_FILE_FLUSH, &sys_err, ULE_LOG, 
			NULL, 0, 0, 0, err_code, 4,
			sizeof(DB_DB_NAME), db_name,
			11, "config file",
			path_length, path_name->name,
	...
	{
	    ule_format(status, &sys_err, ULE_LOG,0,0,0,0,err_code,0);
            ule_format( E_DM9006_BAD_FILE_WRITE, &sys_err, ULE_LOG, 
	...
	{
	    ule_format(status, &sys_err, ULE_LOG,0,0,0,0,err_code,0);
            ule_format( E_DM9001_BAD_FILE_CLOSE, &sys_err, ULE_LOG, 
			NULL, 0, 0, 0, err_code, 4,
			sizeof(DB_DB_NAME), db_name,
			11, "config file",
			path_length, path_name->name,
    ...
    */
	...
	if (config_open)
	{
	    ...
	    if ( lstatus != OK )
	    {
		ule_format(lstatus, &sys_err, ULE_LOG,0,0,0,0,err_code,0);
		ule_format( E_DM9001_BAD_FILE_CLOSE, &sys_err, ULE_LOG, 
			    NULL, 0, 0, 0, err_code, 4,
			    sizeof(DB_DB_NAME), db_name,
			    11, "config file",
	...
	*err_code = E_DM011B_ERROR_DB_CREATE;
	...
	dbstatus = build_file(db_name, 
	    ...
	    sizeof(DMP_RINDEX),
	    DM_sizeof_core_rindex, err_code); 
    if (dbstatus == E_DB_OK)
	dbstatus = build_file(db_name, 
...
i4		*err_code)
{
	...
	*/
    	dm1c_getaccessors(TCB_C_DEFAULT, page_type, 
		    page_size, &local_plv, &local_tlv);
	...
	*/
	...
	if ( status != OK )
	{
	    ule_format(status, &sys_err, ULE_LOG,0,0,0,0,&lerr_code,0);
            ule_format(E_DM9337_DM2F_CREATE_ERROR, &sys_err, ULE_LOG, NULL, 
		   0, 0, 0, &lerr_code, 0);
	    break;
	}
	...
	for (r = records; --count >= 0; r += record_size)
	{
	    ...
	    */
            while ((db_status =
			    ...
			    tuple, record_size,
			    TCB_C_NONE, 0, 0, (u_i2)0, (DMPP_SEG_HDR *)0))
                        != E_DB_OK || count == 0)
	    {
		/*
		...
		*/
		if ( ( page_number + 1 ) > pages_allocated )
		{
		    ...
		    if ( status != OK )
		    {
	                ule_format( status, &sys_err, ULE_LOG,0,0,0,0,
				    &lerr_code,0);
			ule_format( E_DM9000_BAD_FILE_ALLOCATE, &sys_err, 
				    ULE_LOG, NULL, 0, 0, 0, &lerr_code, 4,
				    sizeof(DB_DB_NAME), db_name,
				    sizeof(DB_TAB_NAME), table_name,
				    path_length, path_name->name,
				    sizeof(file_name), &file_name);
	...
	*/
	if ( fhdr_pageno != DM_TBL_DEFAULT_FHDR_PAGENO)
	{
	    db_status = dmm_setup_tables_relfhdr(
			...
			path_name, path_length, page_type, page_size,
			local_plv, table_name, db_name, err_code);
	    if ( db_status != E_DB_OK )
	    {
		/*
	...
	*/
	...
	if ( status != OK )
	{
	    ule_format(status, &sys_err, ULE_LOG,0,0,0,0,&lerr_code,0);
			...
			path_length, path_name->name,
			sizeof(file_name), &file_name);
            ule_format(E_DM92CE_DM2F_GUARANTEE_ERR, &sys_err, ULE_LOG, NULL, 
		   0, 0, 0, &lerr_code, 0);
	    break;
	}
    ...
    */
	...
	if ( close_status != OK )
	{
	    ule_format(close_status, &sys_err, ULE_LOG,0,0,0,0,&lerr_code,0);
            ule_format( E_DM9001_BAD_FILE_CLOSE, &sys_err, ULE_LOG, 
			NULL, 0, 0, 0, &lerr_code, 4,
			sizeof(DB_DB_NAME), db_name,
			sizeof(DB_TAB_NAME), table_name,
    ...
    */
    ...
    ule_format(E_DM9058_BAD_FILE_BUILD, &sys_err, ULE_LOG, NULL, 0, 0, 0,
	err_code, 4,
	sizeof(DB_DB_NAME), db_name,
	sizeof(DB_TAB_NAME), table_name,
	path_length, path_name->name,
    ...
    i4	 *err_code )
{
    ...
    */
	...
	if ( lstatus != OK )
	{
	    ule_format(lstatus, &sys_err, ULE_LOG,0,0,0,0,&lerr_code,0);
	    ule_format( E_DM9001_BAD_FILE_CLOSE, &sys_err, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, &lerr_code, 4,
			sizeof(DB_DB_NAME), dbname,
			10, "iirelation",
    ...
    {
	*err_code = E_DM92FC_DMM_SETUP_TBL_RELFHDR;
    }

    ule_format( E_DM92FC_DMM_SETUP_TBL_RELFHDR, (CL_SYS_ERR *)0, 
