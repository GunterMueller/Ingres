...
lk_cdt_stats( i4 (*output_fcn)(PTR, i4, char *), char *command )
{
    ...
    TRformat( output_fcn, (PTR)1, buf, sizeof(buf), 
		"lkcdtstats: Reads %d Messages %d Retries %d Sleeps %d Wakes %d MaxHops %d(%d) Ld %d Dd (%d,%d)\n",
		    ...
		    lkd->lkd_stat.deadlock, cdt_eoc_dead, cdt_lg_dead );
...
LKdist_deadlock_thread( DMC_CB *dmc_cb )
{
    ...
    */
	...
	for ( i = 0; i < msgcount; i++ )
	{
	    ...
	    /* For debugging... */
	    if ( in_msg->o_node == lkd->lkd_csid && in_msg->msg_hops > cdt_max_hops )
		cdt_max_hops = in_msg->msg_hops;


	    LK_DLOCK_TRACE("\n%@ %d LKdist_deadlock_thread: (r%d,m%d,h%d) %d,%d,%d,%d [%d of %d] %s\n\t"
		    "Continue gbl srch to %d lkblks o_node %d (%s)\n\t"
		    "o_llb=%x o_lkb_id=%x request %d o_dlm %x\n\t"
		    "o_key %s\n\t"
	    ...
	    /* Check for too many hops. If tracing, lower the bar */
	    if ( in_msg->o_node == lkd->lkd_csid
		 && in_msg->msg_hops > 16
		 && (Lk_cluster_deadlock_debug != TRACE_OFF || in_msg->msg_hops > 128) )
	    {
		cdt_futile++;
		...
		*/
		if ( in_msg->o_lkb_id && in_msg->o_lkb_id <= lkd->lkd_sbk_count )
		{
			...
			if ( (LK_mutex(SEM_EXCL, &o_rsb->rsb_mutex)) == OK )
			{
			    ...
			    **     that the original llb_stamp still matches the LLB.
			    */

			    /* Same lock request and transaction? */
			    if ( o_rsb->rsb_type != RSB_TYPE || o_lkb->lkb_type != LKB_TYPE ||
				 o_llb->llb_type != LLB_TYPE ||
				 o_llb->llb_stamp != in_msg->o_stamp ||
			...
			*/
			      ...
			      && CXdlm_mark_as_posted(&o_lkb->lkb_cx_req) )
			{
				...
				*(u_i4*)&o_llb->llb_id,
			    ...
			    o_lkb->lkb_lkreq.blocking_cpid.pid = in_msg->o_blk_pid;
			    o_lkb->lkb_lkreq.blocking_cpid.sid = in_msg->o_blk_sid;
			    o_lkb->lkb_lkreq.blocking_mode = in_msg->o_blk_gtmode;

			    /* Deadlocks in the RCP should never happen */
			    if ( o_lkb->lkb_lkreq.cpid.pid == LGK_my_pid )
			    {
			    ...
			    LKcp_resume(&o_lkb->lkb_lkreq.cpid);
			}
			else
			{
			    /*
		...
		**
		** If this o_lkb's cycle has come full circle without
		** deadlocking, then abandon this msg.
		*/
		if ( o_lkb->lkb_attribute & LKB_END_OF_DL_CYCLE )
		{
		...
		*/
		...
		/* Stamp the RSB to let the local thread know we have it */
	    ...
	    */
		...
		for ( rsb = (RSB *) rsb_hash_bucket; rsb->rsb_q_next; )
		{
			...
			*/
			...
			/*
			** If we've mutexed o_rsb and this resource's RSB
			** mutex is held by the local deadlock thread,
			** then break to avoid a potential mutex deadlock
			** and retry this same message.
			*/
			if ( o_rsb && rsb->rsb_dlock_depth & RSB_MUTEXED_LOCL )
			{
		...
		while ( lkb_offset != end_offset && !must_retry )
		{
		    ...
		    */
			...
			if ( dead_lkb )
			{
				...
				/* Save information about blocker */
				dead_lkb->lkb_lkreq.blocking_cpid.pid =
				     in_msg->o_blk_pid;
				dead_lkb->lkb_lkreq.blocking_cpid.sid =
				     in_msg->o_blk_sid;
				dead_lkb->lkb_lkreq.blocking_mode =
				     in_msg->o_blk_gtmode;
				...
				if ( dead_lkb->lkb_lkreq.cpid.pid == LGK_my_pid )
				{
				    TRdisplay("%@ %d LKdist_deadlock_thread: unexpected RCP deadlock\n\t"
					"%s\n",
					__LINE__,
				...
				*/
				...
				/* Resume the unfortunate */
				LKcp_resume(&dead_lkb->lkb_lkreq.cpid);
			    }
			    else
			    {
		...
		*/
		...
		{
		    /* 
		    ** LKMR_MUTEX_CONFLICT or LKMR_RECHECK
		    **
		    ** (we will (should) never see LKMR_RECHECK)
		    **
	...
	LK_flush_dsm_context( LK_cont_dsmc, &MsgsWritten );
	...
	*/
	...
	else
	{
	    ...
	    */
	    if ( lkd->lkd_status & LKD_PURGE_DLOCK_LIST )
	    {
...
i4 count, u_i4 *writes )
{
    ...
    LK_DLOCK_TRACE("%@ %d LK_flush_dsm_context_write: call CXppp_write count %d\n",
	  __LINE__, count);
...
LK_flush_dsm_context( LK_DSM_CONTEXT *dsmc, u_i4 *writes )
{
	...
	for ( node = 0; node < CX_MAX_NODES; node++ )
	{
		...
		if ( lk_dsm_ctx_priv->lk_dsm_ctx_head <
		     lk_dsm_ctx_priv->lk_dsm_ctx_tail)
		{
		    LK_DLOCK_TRACE("%@ %d LK_flush_dsm_context: call _write head %d"
			" tail %d\n", __LINE__, LK_DSM_CONTEXT_NELEM,
			lk_dsm_ctx_priv->lk_dsm_ctx_tail);
		...
		{
		    LK_DLOCK_TRACE("%@ %d LK_flush_dsm_context: call _write head %d"
			" tail %d\n", __LINE__, lk_dsm_ctx_priv->lk_dsm_ctx_head,
			lk_dsm_ctx_priv->lk_dsm_ctx_tail);
