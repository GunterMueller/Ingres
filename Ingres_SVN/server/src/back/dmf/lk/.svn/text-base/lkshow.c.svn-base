...
CL_ERR_DESC	    *sys_err)
{
	...
	{
	    ule_format(E_CL1043_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0, ULE_LOG,
	...
	*info_result = 0;
    ...
    case LK_S_RESOURCE:
	{
	    ...
	    {
		ule_format(E_CL1044_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0,
			ULE_LOG,
		...
		if (rsb->rsb_type == RSB_TYPE)
		{
		    ...
		    */
		    ...
		    if (rsb->rsb_grant_q_next)
		    {
			    ...
			    lkbi->lkb_pid = lkb->lkb_lkreq.cpid.pid;
			    lkbi->lkb_sid = lkb->lkb_lkreq.cpid.sid;
    ...
    case LK_S_LOCKS:
	{
	    ...
	    if (count < 0 || (count /=  sizeof(LK_LKB_INFO)) < 0)
	    {
		ule_format(E_CL1045_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0,
			ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 1,
			0, count);
		return (LK_BADPARAM);
    ...
    case LK_S_LLB_INFO:
	{
	    ...
	    {
		ule_format(E_DMA030_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0,
	    ...
	    {
		ule_format(E_DMA031_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
			&err_code, 5,
	    ...
	    {
		ule_format(E_DMA032_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
			&err_code, 3,
    ...
    case LK_S_REL_TRAN_LOCKS:
	{
		...
		{
		    ule_format(E_DMA033_LK_SHOW_NO_REL_LIST, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
			&err_code, 2,
			0, llb->llb_id.id_id,
	    ...
	    */
		...
		{
		    ule_format(E_DMA034_LK_SHOW_NO_SHR_LIST, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
			&err_code, 2,
			0, llb->llb_id.id_id,
			0, "LK_S_TRAN_LOCKS/LK_S_REL_TRAN_LOCKS");
	...
	{
	    ule_format(E_CL1046_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0,
			ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 2,
	...
	*/
	...
	lkbi->lkb_key[5] = rsb->rsb_name.lk_key5;
	lkbi->lkb_key[6] = rsb->rsb_name.lk_key6;
	lkbi->lkb_dlm_lkvalue[0] = (i4) lkb->lkb_cback.cback_arg;
	lkbi->lkb_pid = lkb->lkb_lkreq.cpid.pid;
	lkbi->lkb_sid = lkb->lkb_lkreq.cpid.sid;
	...
	*/
	...
	if (llb_id->id_id == 0 || (i4)llb_id->id_id > lkd->lkd_lbk_count)
	{
	    ule_format(E_DMA030_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0, ULE_LOG,
	...
	if (llb->llb_type != LLB_TYPE ||
	    llb->llb_id.id_instance != llb_id->id_instance)
	{
	    ule_format(E_DMA031_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0, ULE_LOG, 
		NULL, (char *)0, (i4)0, (i4 *)0, &err_code, 5,
		0, llb->llb_type, 0, LLB_TYPE, 0,
		0, *(u_i4*)&llb->llb_id, 0, *(u_i4*)&llb_id,
		0, "LK_S_LIST");
	    return (LK_BADPARAM);
	}
	...
	*/
	...
	if (info_size < sizeof(LK_LLB_INFO))
	{
	    ule_format(E_CL1045_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0, ULE_LOG,
	...
	*info_result = count * sizeof(LK_LKB_INFO);
	...
	if ((CSs_semaphore(CS_INIT_SEM_STATS, &lkd->lkd_ew_q_mutex, 
	...
	if ((CSs_semaphore(CS_INIT_SEM_STATS, &lkd->lkd_dw_q_mutex, 
					lstat, sizeof(*lstat))) == OK)
	{
	    mstat->count++;
	    lstat++;
	...
	if ((CSs_semaphore(CS_INIT_SEM_STATS, &lkd->lkd_lbk_mutex, 
					lstat, sizeof(*lstat))) == OK)
	{
	    mstat->count++;
	    lstat++;
	...
	if ((CSs_semaphore(CS_INIT_SEM_STATS, &lkd->lkd_sbk_mutex, 
					lstat, sizeof(*lstat))) == OK)
	{
	    mstat->count++;
	    lstat++;
	...
	if ((CSs_semaphore(CS_INIT_SEM_STATS, &lkd->lkd_rbk_mutex, 
					lstat, sizeof(*lstat))) == OK)
	{
	    mstat->count++;
	    lstat++;
	...
	if ((CSs_semaphore(CS_INIT_SEM_STATS, &lkd->lkd_llb_q_mutex, 
					lstat, sizeof(*lstat))) == OK)
	{
	    mstat->count++;
	    lstat++;
	...
	if ((CSs_semaphore(CS_INIT_SEM_STATS, &lkd->lkd_stall_q_mutex, 
					lstat, sizeof(*lstat))) == OK)
	{
	    mstat->count++;
	    lstat++;
	...
	{
	    llb = (LLB *)LGK_PTR_FROM_OFFSET(lbk_table[i]);
	    if (status = CSs_semaphore(CS_ROLL_SEM_STATS, &llb->llb_mutex,
					lstat, sizeof(*lstat))) 
		break;
	}
	if (status == OK)
	...
	{
	    rsb = (RSB *)LGK_PTR_FROM_OFFSET(rbk_table[i]);
	    if (status = CSs_semaphore(CS_ROLL_SEM_STATS, &rsb->rsb_mutex,
					lstat, sizeof(*lstat)))
		break;
	}
	if (status == OK)
	...
	{
	    lkh = (LKH *) &lkh_table[i];
	    if (status = CSs_semaphore(CS_ROLL_SEM_STATS, &lkh->lkh_mutex, 
					lstat, sizeof(*lstat)))
		break;
	}
	if (status == OK)
	...
	{
	    rsh = (RSH *) &rsh_table[i];
	    if (status = CSs_semaphore(CS_ROLL_SEM_STATS, &rsh->rsh_mutex, 
					lstat, sizeof(*lstat)))
		break;
	}
	if (status == OK)
	...
	ule_format(E_CL1047_LK_SHOW_BAD_PARAM, (CL_ERR_DESC *)0,
			ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 1,
			0, flag);
...
fill_in_stat_block( LK_STAT *stat, i4  info_size, u_i4 *info_result, LKD *lkd )
{
    ...
    stat->request = lkd->lkd_stat.request_new;
    stat->re_request = lkd->lkd_stat.request_convert;
    stat->convert = lkd->lkd_stat.convert;
    stat->release = lkd->lkd_stat.release;
    stat->escalate = lkd->lkd_stat.release_partial;
    stat->destroy = lkd->lkd_stat.release_all;
    stat->wait = lkd->lkd_stat.wait;
    stat->con_wait = lkd->lkd_stat.convert_wait;
    stat->con_search = lkd->lkd_stat.convert_search;
    stat->con_dead = lkd->lkd_stat.convert_deadlock;
    stat->dead_search = lkd->lkd_stat.deadlock_search;
    stat->deadlock = lkd->lkd_stat.deadlock;
...
LKkey_to_string( LK_LOCK_KEY *key, char *buffer )
{
    ...
    *bp++ = ',';
    ...
    case LK_BM_PAGE:
    case LK_PH_PAGE:
	STprintf(bp,"DB=%x,TABLE=[%d,%d],PAGE=%d",
	...
	*/
    ...
    case LK_ROW:
	STprintf(bp,"DB=%x,TABLE=[%d,%d],PAGE=%d,LINE=%d", 
	    key->lk_key1, key->lk_key2,
	    (key->lk_key2 < 0)
		? key->lk_key3
		: key->lk_key3 & ~DB_TAB_PARTITION_MASK,
