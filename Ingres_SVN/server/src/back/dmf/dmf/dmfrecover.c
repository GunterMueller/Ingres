...
**          -rcp_ignore_table (without prompt) 
...
static DB_STATUS	rcp_archive(DMP_LCTX *lctx, i4 *err_code);
...
i4		*err_code)
{
	...
	*/
	status = dm0p_force_pages(rcp->rcp_lock_list, &rcp_tran_id,
			    (i4)0, DM0P_TOSS_IGNORE, err_code);
	if (status != E_DB_OK)
	{
...
i4		*err_code)
{
	...
	*/
	...
	status = rcp_archive(lctx, err_code);
...
i4		*err_code)
{
	...
	*/
	...
	if (rdb->rdb_status & RDB_TABLE_RECOVERY_ERROR)
	{
	   ...
	   if (rtbl && (rtbl->rtbl_flag & RTBL_IGNORE_TABLE))
	   {
#ifdef xDEBUG
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    ...
	    status = add_invalid_tbl(rcp, rdb, rtbl, lctx, record, err_code);
	    if (status != E_DB_OK)
	    {
...
i4		*err_code)
{
    ...
    */
	...
	TRdisplay("RCP unfixing tbio for %~t\n",
		sizeof(DB_TAB_NAME), tabname.db_tab_name);
	loc_status = dmve_unfix_tabio(dmve, &tbio, DMVE_INVALID, &loc_error);
    }

    /* 
    ** For redo/undo errors on a secondary index, add to invalid table list
    ** If this is a user table, let the dba decide if
    ** II_DMFRCP_STOP_ON_INCONS_DB=N
    ** II_DMFRCP_PROMPT_ON_INCONS_DB=Y
    ** FIX ME for cluster
    */
    ...
    {
	/* This is not an index, should we prompt? */
	if (Db_incons_crash || !Db_incons_prompt || rcp->rcp_cluster_enabled)
	    return (E_DB_ERROR);

	TRdisplay(
	"WARNING The RCP encountered an error and is waiting for input.\n");
	TRdisplay("Valid input is:\n");
	TRdisplay("rcpconfig -rcp_ignore_db=%~t\n",
		DB_MAXNAME, &rdb->rdb_name);
	TRdisplay("OR\n");
	TRdisplay("rcpconfig -rcp_ignore_table=%~t\n",
		DB_MAXNAME, tabname.db_tab_name);
	TRdisplay("OR\n");
	TRdisplay("rcpconfig -rcp_ignore_lsn=%d,%d\n",
		record->lsn.lsn_high, record->lsn.lsn_low);
	TRdisplay("OR\n");
			...
			sizeof(recover_set), &sys_err))
	{
	    ule_format(status, (CL_ERR_DESC *)&sys_err, ULE_LOG , NULL, 
		    (char * )0, 0L, (i4 *)0, err_code, 0);
	    (VOID) ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, 
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 1,
		    0, LG_A_INIT_RECOVER);
	}
	...
	if (recover_show.lg_flag == LG_A_RCP_IGNORE_DB)
	{
	    TRdisplay("RCP wakeup by rcpconfig -rcp_ignore_db=%~t\n",
		DB_MAXNAME, &rdb->rdb_name);
	    return (E_DB_ERROR);
	}
	else if (recover_show.lg_flag == LG_A_RCP_IGNORE_TABLE)
	{
	    TRdisplay("RCP wakeup by rcpconfig -rcp_ignore_table\n",
		DB_MAXNAME, tabname.db_tab_name);
	    rtbl_flag = RTBL_IGNORE_TABLE;
	...
	{
	    TRdisplay("RCP wakeup by rcpconfig -rcp_ignore_lsn\n",
		record->lsn.lsn_high, record->lsn.lsn_low);
	    rtbl_flag = 0;
	    /* Fall through and add this table to invalid tables list */
	}
...
*/
static DB_STATUS
rcp_archive(DMP_LCTX *lctx, i4 *err_code)
{
    ACB			*acb;
    ACB			*a;
    ...
    */
    status = dma_prepare(DMA_RCP, &acb, lctx, err_code);
    if (status != E_DB_OK)
    {
	...
	*/
	status = dma_offline_context(a, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	...
	*/
    ...
    complete_status = dma_complete(&a, &complete_err_code);
    if (complete_status != E_DB_OK)
    {
	ule_format(complete_err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &complete_err_code, 0);
