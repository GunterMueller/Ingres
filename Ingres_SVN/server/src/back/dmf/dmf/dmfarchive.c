...
static DB_STATUS mark_fully_closed( ACB *acb, DBCB *dbcb, i4 *err_code);
...
i4	    *err_code)
{
	...
	status = dm0l_allocate(DM0L_ARCHIVER, 0, 0, 0, 0, (DMP_LCTX **)0,
				err_code);
	if (status != E_DB_OK)
	    return (error_translate(E_DM9812_ARCH_OPEN_LOG, *err_code, 
		err_code));
    }
    if (lctx == 0)
	...
	if (status)
	{
		...
		0, LG_A_HEADER);
	    return (error_translate(E_DM9812_ARCH_OPEN_LOG,
		E_DM9810_ARCH_LOG_BEGIN, err_code));
	}
    ...
    */
	...
	0, (DM_OBJECT **)acb, err_code);

    if (status != E_DB_OK)
	return (error_translate(E_DM9812_ARCH_OPEN_LOG, *err_code, err_code));
    a = *acb;
    ...
    */
	...
	MEcopy((PTR)DB_ARCHIVER_NAME, sizeof(DB_OWN_NAME), (PTR) &user_name);
	...
	if (status != OK)
	{
		...
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    return (error_translate(E_DM9812_ARCH_OPEN_LOG,
		E_DM9814_ARCH_LOCK_CREATE, err_code));
	}
...
i4	*err_code)
{
    ...
    */
	...
	if (cl_status != OK)
	{
		...
		(char *)0, (i4)0, (i4 *)0, err_code, 1,
		0, a->acb_s_lxid);
	    return (error_translate(E_DM9813_ARCH_CLOSE_LOG,
		E_DM9811_ARCH_LOG_END, err_code));
	}
	...
	if (cl_status != OK)
	{
		...
		0, a->acb_db_id);
	    return (error_translate(E_DM9813_ARCH_CLOSE_LOG,
		E_DM980F_ARCH_LOG_REMOVE, err_code));
	}
	...
	*/
	{
	int clear_lctx_ptr = (dmf_svcb->svcb_lctx_ptr == a->acb_lctx);
	status = dm0l_deallocate(&a->acb_lctx, err_code);
	if(clear_lctx_ptr)
	    dmf_svcb->svcb_lctx_ptr = a->acb_lctx; /* synchronize fields */
	}
	if (status != E_DB_OK)
	    return (error_translate(E_DM9813_ARCH_CLOSE_LOG, *err_code, 
		err_code));
    }
...
i4	*err_code)
{
	...
	*/
	...
	if (cl_status != OK)
	{
	    ule_format(cl_status, &sys_err, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    ule_format(E_DM9010_BAD_LOG_FORCE, &sys_err, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 1,
		0, a->acb_s_lxid);
	    lgerror = TRUE;
	    status = E_DB_ERROR;
	...
	*/
	...
	while (status == E_DB_OK)
	{
		...
		*err_code = E_DM9842_ARCH_SOC;
		...
		(LG_LA *)&db.db_sbackup, &dbcb, err_code);
	    if (status != E_DB_OK)
		break;
	    db_count++;
	    ...
	    */
		...
		ule_format(E_DM9845_ARCH_WINDOW, &sys_err, ULE_LOG, NULL,
	...
	*/
	     ...
	     (LGA_GT(&a->acb_start_addr, &a->acb_eof_addr))))
	{
	    ...
	    ule_format(E_DM9846_ARCH_WINDOW, &sys_err, ULE_LOG, NULL,
    ...
    */
	...
	ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP, 
...
i4	    *err_code)
{
	...
	*/
	status = dm0l_position(DM0L_FLGA, a->acb_s_lxid,
				&a->acb_start_addr, a->acb_lctx, err_code);
	if (status != E_DB_OK)
	{
	    /*
...
i4	    *err_code)
{
	...
	*/
	...
	ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP,
	    (DB_SQLSTATE *)NULL, a->acb_errtext, sizeof(a->acb_errtext),
	    &a->acb_errltext, &local_err_code, 0);

	return (error_translate(E_DM9843_ARCH_COPY, *err_code, err_code));
    }
    ...
    */
	...
	status = dm0l_read(a->acb_lctx, a->acb_s_lxid, &record, &la, err_code);
	if (status != E_DB_OK)
	{
	    if (*err_code == E_DM9238_LOG_READ_EOF)
	    {
	...
	*/
	if (a->acb_test_flag & ACB_T_BADREC)
	{
	    ...
	    */
		...
		(DB_SQLSTATE *)NULL, a->acb_errtext, sizeof(a->acb_errtext),
		&a->acb_errltext, &local_err_code, 0);
	    *err_code = E_DM9840_ARCH_BAD_DBID;
	    break;
	}
	...
	*/
	...
	"%@ CSP DMA_COPY: Internal error, DBID %d not previously cataloged\n",
	    ...
	    */
		...
		&odb->o_dbowner, &odb->o_root, odb->o_l_root,
		&zero_la, &zero_la, &zero_la, &zero_la, &zero_la,
		&db, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	{
	    status = dma_alloc_sidefile(a, db, record, err_code);
	    /*
	    if (status != E_DB_OK)
		break;
	    */
	...
	{
	    status = dma_close_sidefile(a, db, record, err_code);
	    /*
	    if (status != E_DB_OK)
		break;
	    */
		...
		record->type == DM0LREP || record->type == DM0LDEALLOC)
	{
	    if (a->acb_sfq_count)
	    {
		status = dma_filter_sidefile(a, db, record, &sf, err_code);
		if (sf && status == E_DB_OK)
		{
		    status = dma_write_sidefile(a, db, record, sf, err_code);
	...
	*/
	if (record->type == DM0LSBACKUP)
	{
	    ...
	    */
	    status = next_journal(a, db, header, record, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	if (record->flags & DM0L_JOURNAL) 
	{
	    ...
	    */
		...
		status = E_DB_ERROR;
		*err_code = E_DM9284_DM0J_WRITE_ERROR;
	    }
		...
		*/
		...
		ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP,
		    (DB_SQLSTATE *)NULL, a->acb_errtext, sizeof(a->acb_errtext),
		    &a->acb_errltext, &local_err_code, 3, 
...
i4	    *err_code)
{
	...
	*/
	status = dm0c_open(&db->dbcb_dcb, 0, a->acb_lk_id, &config, err_code);
	if (status != E_DB_OK)
	{
	    /*
	...
	*/
	if (config->cnf_dsc->dsc_status & DSC_DISABLE_JOURNAL)
	{
	    ...
	    ule_format(E_DM983D_ARCH_JNL_DISABLE, (CL_ERR_DESC *)0,
		ULE_LOG, NULL, (char *)0, (i4)0,
		(i4 *)0, err_code, 1,
		sizeof(db->dbcb_dcb.dcb_name.db_db_name),
		db->dbcb_dcb.dcb_name.db_db_name);

	    status = dm0c_close(config, 0, err_code);
	    if (status != E_DB_OK)
	    {
		/*
		...
		*/
		...
		STRUCT_ASSIGN_MACRO(db->dbcb_dcb.dcb_name, 
		    a->acb_errdb);
		ule_format(a->acb_error_code, (CL_ERR_DESC *)0, 
		...
		return (error_translate(E_DM9842_ARCH_SOC,
		    *err_code, err_code));
	    }
	    continue;
	}
	...
	*/
	if (LGA_GT(&db->dbcb_prev_jnl_la, &a->acb_eof_addr))
	{
		...
		db->dbcb_prev_jnl_la.la_offset);
	    if (config != 0)
		status = dm0c_close(config, 0, &error);
	    ...
	    */
	    ...
	    return (error_translate(E_DM9842_ARCH_SOC,
				    E_DM9838_ARCH_DB_BAD_LA, err_code));
	}
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    ...
	    return (error_translate(E_DM9842_ARCH_SOC, *err_code, err_code));
	}       	
	...
	*/
	if (db->dbcb_status & DBCB_JNL)
	{
	    status = init_journal(a, db, err_code);
	    if (status != E_DB_OK)
		return (error_translate(E_DM9842_ARCH_SOC, *err_code, 
		    err_code));
	}
...
i4	    *err_code)
{
	...
	*/
	if (dbcb->dbcb_jrec_cnt > 0)
	{
		...
		*/
		...
		status = error_translate(E_DM9844_ARCH_EOC, 
						*err_code, err_code);
		return (status);
	    }
	}
	...
	*/
	...
	{
	    status = dm0j_close((DM0J_CTX **)&dbcb->dbcb_dm0j, err_code);
	    if (status != E_DB_OK)
	    {
		/*
		** Fill in exit error information for archiver exit handler.
		*/
		...
		return (error_translate(E_DM9844_ARCH_EOC, *err_code, err_code));
	    }
	}
	...
	*/
	if (dbcb->dbcb_drec_cnt > 0)
	{
		...
		*/
		...
		return (error_translate(E_DM9844_ARCH_EOC, *err_code, 
		    err_code));
	    }

	    status = dm0d_close((DM0D_CTX **)&dbcb->dbcb_dm0d, err_code);
	    if (status != E_DB_OK)
	    {
		/*
		** Fill in exit error information for archiver exit handler.
    ...
    */
	...
	if (cl_status != OK)
	{
	    ...
	    */
	    ...
	    return (error_translate(E_DM9844_ARCH_EOC,
		E_DM980D_ARCH_LOG_ALTER, err_code));
	}
	...
	*/
	...
	if (cl_status != OK)
	{
	    ...
	    */
	    ...
	    return (error_translate(E_DM9844_ARCH_EOC,
		E_DM980B_ARCH_LOG_FORCE, err_code));
	}
    }
...
i4	    	*err_code)
{
	...
	(char *)a, (DM_OBJECT **)dbcb, err_code);
    if (status != E_DB_OK)
    {
	...
	*/
	...
	ule_format(E_DM9427_DM0M_MEMORY_ALLOCATE, (CL_ERR_DESC *)0, ULE_LOG,
	    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, &error,
...
ACB	    	*acb)
{
    ...
    ** Deallocate DBCB control blocks.
    */
    st = acb->acb_db_state.stq_next;
    while (st != (ACP_QUEUE *) &acb->acb_db_state.stq_next)
    {
        dbcb = (DBCB *)((char *)st - CL_OFFSETOF(DBCB, dbcb_state));

	if ((acb->acb_status & ACB_CSP) != 0)
	{
	    if (mark_fully_closed(acb, dbcb, &err_code))
	    {
		ule_format(err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			    (char *)0, (i4)0, (i4 *)0, &err_code, 0);
...
*/
static DB_STATUS
mark_fully_closed( ACB *acb, DBCB *dbcb, i4 *err_code)
{
    DMP_DCB	    local_dcb;
    DM0C_CNF	    *cnf = 0;
    DB_STATUS	    status = E_DB_OK;
	...
	*/
	open_mask = 1 << acb->acb_lctx->lctx_node_id;
...
i4	    *err_code)
{
	...
	*/
	...
	ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP,
	    (DB_SQLSTATE *)NULL, a->acb_errtext, 
	    sizeof(a->acb_errtext), &a->acb_errltext, 
	    &local_err_code, 1, DB_MAXNAME, 
	    db->dbcb_dcb.dcb_name.db_db_name);

	return (error_translate(E_DM9835_ARCH_NEW_JNL_ERROR, 
		*err_code, err_code));
    }
    ...
    */
	...
	status = dm0j_open(0, (char *)&db->dbcb_j_path, db->dbcb_lj_path, 
			...
			(DM0J_CTX **)&db->dbcb_dm0j, err_code);
	if (status == E_DB_OK && fil_seq <= 0)
	{
	    ...
	    status = E_DB_ERROR;
	    *err_code = E_DM983A_ARCH_OPEN_JNL_ZERO;
	}
    }
    else 
    {
	...
	*/
	...
	status = E_DB_ERROR;
	*err_code = E_DM9034_BAD_JNL_NOT_FOUND;
	...
	if (*err_code != E_DM9034_BAD_JNL_NOT_FOUND)	
	{
	    ...
	    */
	    ...
	    ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP,
		(DB_SQLSTATE *)NULL, a->acb_errtext, sizeof(a->acb_errtext),
		&a->acb_errltext, &local_err_code, 2,
		0, jnlerr_db, 0, jnlerr_dir);

	    status = dm0c_close(config, 0, &local_err_code);
	...
	*/
	...
	if (config->cnf_jnl->jnl_fil_seq > 0)	    
	{
	    ...
	    */
	    ...
	    status = dm0j_create(0, (char *)&db->dbcb_j_path, 
			...
			blk_cnt, 
			a->acb_node_id, (DB_TAB_ID *)0, err_code);
	}
	else
	{
	    ...
	    TRdisplay("%@ ACP INIT_JOURNAL: Can't create journal file %d.\n",
			config->cnf_jnl->jnl_fil_seq);
	    status = E_DB_ERROR;
	    *err_code = E_DM9839_ARCH_JNL_SEQ_ZERO;
	}
	...
	if (status != OK)
	{
	    ...
	    */
	    ...
	    STtrmwhite(jnlerr_dir);
	    STRUCT_ASSIGN_MACRO(db->dbcb_dcb.dcb_name, a->acb_errdb);
	    if (*err_code == E_DM9839_ARCH_JNL_SEQ_ZERO)
	    {
		a->acb_error_code = E_DM9854_ACP_JNL_ACCESS_EXIT;
		ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP,
		    (DB_SQLSTATE *)NULL, a->acb_errtext,
		    sizeof(a->acb_errtext), &a->acb_errltext,
		    &local_err_code, 2, 0, jnlerr_db, 0, jnlerr_dir);
	    }
	    else
	    {
		a->acb_error_code = E_DM9852_ACP_JNL_WRITE_EXIT;
		ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP,
		    (DB_SQLSTATE *)NULL, a->acb_errtext,
		    sizeof(a->acb_errtext), &a->acb_errltext,
		    &local_err_code, 3,  0, jnlerr_db, 0, jnlerr_dir, 0,
		    ERx("create"));
	    }
	...
	*/
	...
	status = dm0j_open(0, (char *)&db->dbcb_j_path, db->dbcb_lj_path, 
	    ...
	    blk_seq + 1, DM0J_M_WRITE, a->acb_node_id, 
	    DM0J_FORWARD, 
	    (DB_TAB_ID *)0, (DM0J_CTX **)&db->dbcb_dm0j, err_code);
	if (status != E_DB_OK)
	{
    ...
    status = dm0c_close(config, DM0C_UPDATE, err_code);
    if (status != E_DB_OK)
    {
	/*
	** Fill in exit error information for archiver exit handler.
	*/
	...
	ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP, 
	    (DB_SQLSTATE *)NULL, a->acb_errtext, sizeof(a->acb_errtext), 
	    &a->acb_errltext, &local_err_code, 1, DB_MAXNAME, 
	    db->dbcb_dcb.dcb_name.db_db_name);
...
i4	    *err_code)
{
    ...
    ule_format(old_error, (CL_ERR_DESC *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
        (char *)0, (i4)0, (i4 *)0, err_code, 0);
...
i4	    *err_code)
{
	...
	*/
	...
	return (error_translate(E_DM9825_ARCH_INIT_DUMP,
                                     *err_code, err_code));
    }
    ...
    */
	...
	status = dm0d_open(0, (char *)&db->dbcb_d_path, db->dbcb_ld_path, 
	    ...
	    (DM0D_CTX **)&db->dbcb_dm0d, err_code);
    }
    else 
    {
	/*
	...
	*/
	...
	status = E_DB_ERROR;
	*err_code = E_DM9352_BAD_DMP_NOT_FOUND;
    }
	...
	if (*err_code != E_DM9352_BAD_DMP_NOT_FOUND)	
	{
	    ...
	    */
	    ...
	    ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP, 
		(DB_SQLSTATE *)NULL, a->acb_errtext, sizeof(a->acb_errtext), 
		&a->acb_errltext, &local_err_code, 1,
		DB_MAXNAME, db->dbcb_dcb.dcb_name.db_db_name);
	...
	*/
	...
	return (error_translate(E_DM9825_ARCH_INIT_DUMP, *err_code, err_code));
    }
...
i4		*err_code)
{
	...
	case   DM0LBI:
	    ...
	    ** internal counters.
	    */
	    status = log_2_dump(a, db, header, record, la, err_code);
	    if (status != E_DB_OK)
		return (status);
	...
	*/
	...
	case   DM0LSM2CONFIG:
	    ...
	    */
	    ...
	    return (error_translate(E_DM9826_ARCH_COPY_DUMP,
		E_DM9829_ARCH_BAD_LOG_TYPE, err_code));
...
i4	    *err_code)
{
    ...
    */
    ...
    {
	status = init_dump(a, db, db->dbcb_d_filseq, 
	    db->dbcb_d_blkseq, err_code);
	if (status != E_DB_OK)
	{
	    ...
	    return (error_translate(E_DM9827_ARCH_LOG2DUMP, 
		*err_code, err_code));
	}
    }

    status = dm0d_write(
       (DM0D_CTX *)db->dbcb_dm0d, (PTR)record, *(i4 *)record, err_code);
    ...
    */
	...
	status = E_DB_ERROR;
	*err_code = E_DM9362_DM0D_WRITE_ERROR;
    }
	...
	*/
	...
	return (error_translate(E_DM9827_ARCH_LOG2DUMP, *err_code, err_code));
    }
    return (E_DB_OK);
    ...
    */
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    ...
	    ule_format(a->acb_error_code, (CL_ERR_DESC *)0, ULE_LOOKUP, 
		(DB_SQLSTATE *)NULL, a->acb_errtext, sizeof(a->acb_errtext), 
		&a->acb_errltext, &err_code, 1, 0, ERx("DB-UKNOWN"));

	    return (error_translate(E_DM981F_ARCH_CK_SBACKUP,
		E_DM9809_ARCH_LOG_SHOW, &err_code));
	}
	if (length == 0)
	    return (BKDB_INVALID);
...
i4	    *err_code)
{
	...
	*/
	...
	{
	    ule_format(E_DM9833_ARCH_BACKUP_FAILURE, &sys_err, ULE_LOG, 
...
i4		*err_code)
{
    ...
    */
    status = new_jnl_entry(a, &db->dbcb_dcb,
			(DM0L_SBACKUP *)record, err_code); /* BUG 66473 */
    if (DB_FAILURE_MACRO(status))
	return (error_translate(E_DM9831_ARCH_JWRITE, *err_code, err_code));
...
*/
    ...
    */
    ...
    status = dm0j_update((DM0J_CTX *)db->dbcb_dm0j, &new_seq, err_code);
    if (status != E_DB_OK)
    {
	/*
	** Fill in exit error information for archiver exit handler.
    ...
    */
    status = dm0j_close((DM0J_CTX **)&db->dbcb_dm0j, err_code);
    if (status != E_DB_OK)
    {
	/*
	** Fill in exit error information for archiver exit handler.
...
i4		*err_code)
{
    ...
    */
    status = dm0c_open(dcb, DM0C_PARTIAL, a->acb_lk_id, &config, err_code);
    if (status != E_DB_OK)
    {
	/*
	** Fill in exit error information for archiver exit handler.
...

/*{
** Name: dma_alloc_online_context		- Build a new SIDEFILE
**
** Description:
...
i4	    	*err_code)
{
	...
	*/
    ...
    status = dm0j_create(0, (char *)&db->dbcb_j_path, 
		...
		a->acb_node_id, &log_rec->bsf_tbl_id, err_code);
	...
	TRdisplay(
		...
		sizeof(DB_TAB_NAME), &log_rec->bsf_name, *err_code); 
	ule_format(E_DM9CA8_SIDEFILE_CREATE, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 1,
	...
	TRdisplay(
		...
		sizeof(DB_OWN_NAME), &log_rec->bsf_owner,
		sizeof(DB_TAB_NAME), &log_rec->bsf_name, *err_code);
        ule_format(E_DM9CA9_SIDEFILE_OPEN, (CL_SYS_ERR *)0, ULE_LOG, NULL,
                (char *)0, (i4)0, (i4 *)0, err_code, 1,
                sizeof(DB_TAB_NAME), &log_rec->bsf_name);
	return (E_DB_ERROR);
    }
...
i4	    	*err_code)
{
	...
	*(i4 *)record, err_code);
	...
	TRdisplay(
		...
		sizeof(DB_TAB_NAME), &sf->sf_bsf.bsf_name, *err_code);
	sf->sf_status = status;
	sf->sf_error = *err_code;
    }
    else
    {
...
i4	    	*err_code)
{
    ...
    if (!sf)
    {
	ule_format(E_DM9CAA_ARCH_SFCB_NOTFOUND, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 2,
		sizeof(u_i4), log_rec->esf_tbl_id.db_tab_base,
		sizeof(u_i4), log_rec->esf_tbl_id.db_tab_index);
	ule_format(E_DM9CAC_SIDEFILE_CLOSE, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 1,
    ...
    {
	ule_format(sf->sf_error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &sf->sf_error, 0);
	ule_format(E_DM9CAB_ARCH_BAD_SIDEFILE, (CL_SYS_ERR *)0, ULE_LOG, NULL,
    ...
    {
	local_status = dm0j_close((DM0J_CTX **)&sf->sf_dm0j, &local_error);
...
i4	    	*err_code)
{
    ...
    */
    ...
    if (sf->sf_status != E_DB_OK)
    {
		...
		sizeof(i4), sf->sf_flags);
	ule_format(E_DM9CAD_ARCH_FILTER_SIDEFILE, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	return (sf->sf_status);
    }
...

{
    ...
    TRdisplay("%@ Write eof for database %~t to journal %d, jnlctx %x\n", 
	sizeof(DB_DB_NAME), &db->dbcb_dcb.dcb_name, jnl_seq, openjnl);
    ...
    */
    ...
    if (openjnl)
    {
	status = dm0j_write(openjnl, (PTR)&rec, *(i4 *)&rec, &error);
	return (E_DB_OK);
    }
	...
	status = dm0j_write(jnl, (PTR)&rec, *(i4 *)&rec, &error);
	if (status)
	    break;

	status = dm0j_update(jnl, &jnl_sequence, &error);
	if (status)
	    break;

	status = dm0j_close(&jnl, &error);
	if (status)
	    break;
	...
	sizeof(DB_DB_NAME), &db->dbcb_dcb.dcb_name, jnl_seq, openjnl, error);
