...
DB_STATUS   *err_code)
{
	...
	*/
	status = dm0c_open(dcb, 0, dmf_svcb->svcb_lock_list, &cnf, err_code);
	if (status != E_DB_OK)
	    break;
	config_open = TRUE;
	...
	*/
	if (jsx->jsx_status & JSX_JDISABLE)
	{
	    ...
	    */
	    status = disable_jnl(jsx, dcb, cnf, err_code);
	    if (status != E_DB_OK)
		break;
	}
	else if (jsx->jsx_status1 & JSX_JNL_BLK_SIZE)
	{
	    ...
	    */
	    status = alter_jnl_bksz(jsx, dcb, cnf, err_code);
	    if (status != E_DB_OK)
		break;
	}
	else if (jsx->jsx_status1 & JSX_JNL_NUM_BLOCKS)
	{
	    ...
	    */
	    status = alter_jnl_maxcnt(jsx, dcb, cnf, err_code);
	    if (status != E_DB_OK)
		break;
	}
	else if (jsx->jsx_status1 & JSX_JNL_INIT_BLOCKS)
	{
	    ...
	    */
	    status = alter_jnl_blkcnt(jsx, dcb, cnf, err_code);
	    if (status != E_DB_OK)
		break;
	}
	else if (jsx->jsx_status & JSX_ALTDB_CKP_DELETE)
	{
	    ...
	    */
	    status = delete_oldest_ckp(jsx, dcb, &cnf, err_code);
	    if (status != E_DB_OK)
		break;
	}
         ...
         else if (jsx->jsx_status2 & JSX2_ALTDB_CKP_INVALID)
        {
            ...
            */
            status = delete_invalid_ckp(jsx, dcb, &cnf, err_code, FALSE);
            if (status != E_DB_OK)
               break;
        }
        ...
        else if (jsx->jsx_status2 & (JSX2_ALTDB_UNICODEC | JSX2_ALTDB_UNICODED))
        {
            ...
            status = convert_unicode(jsx, dcb, cnf, err_code);
		 ...
		 || (jsx->jsx_status2 & JSX2_OCKP))
        {
            ...
            status = set_pmode(jsx, dcb, cnf, err_code);
            if (DB_FAILURE_MACRO(status))
                break;
        }
	else if (jsx->jsx_status2 & (JSX2_READ_ONLY | JSX2_READ_WRITE))
	{
	    ...
	    status = set_accmode(jsx, dcb, cnf, err_code);	   
            if (DB_FAILURE_MACRO(status))
                break;
	}
	else if (jsx->jsx_status2 & (JSX2_SWITCH_JOURNAL))
	{
		...
		status = switch_journal(jsx, dcb, &cnf, err_code);	   
		if (DB_FAILURE_MACRO(status))
		    break;
	...
	*/
    ...
    if ((config_open) && (cnf))
    {
	local_status = dm0c_close(cnf, 0, &local_error);
	if (local_status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)NULL, ULE_LOG,
    ...
    */
	...
	{
	    ule_format(*err_code, (CL_SYS_ERR *)NULL, ULE_LOG,
		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
		&local_error, 0);

	    *err_code = E_DM1400_ALT_ERROR_ALTERING_DB;
	}
    }
...
DB_STATUS   *err_code)
{
    ...
    */
    ...
    {
	*err_code = E_DM1403_ALT_JNL_NOT_ENABLED;
	return (E_DB_WARN);
    }

    if ((cnf->cnf_dsc->dsc_status & DSC_JOURNAL) == 0)
    {
	*err_code = E_DM1402_ALT_DB_NOT_JOURNALED;
	return (E_DB_WARN);
    }
    ...
    */
    ...
    ule_format(E_DM1401_ALT_JOURNAL_DISABLED, (CL_SYS_ERR *)NULL, ULE_LOG,
	(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
...
DB_STATUS   *err_code)
{
	...
	*/
	TRformat(dmf_put_line, 0, local_buffer, sizeof(local_buffer),
	    ...
	    sizeof(dcb->dcb_name), &dcb->dcb_name);
	*err_code = E_DM1041_JSP_JNL_BKSZ_ERR;
	return (E_DB_WARN);
    }
    ...
    */
    ...
    ule_format(E_DM103E_JSP_JNL_BKSZ_CHGD, (CL_SYS_ERR *)NULL, ULE_LOG,
	(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
...
DB_STATUS   *err_code)
{
	...
	*err_code = E_DM1040_JSP_JNL_SIZE_ERR;
	return (E_DB_WARN);
    }
    ...
    */
    ...
    ule_format(E_DM103D_JSP_JNL_SIZE_CHGD, (CL_SYS_ERR *)NULL, ULE_LOG,
	(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
...
DB_STATUS   *err_code)
{
	...
	*err_code = E_DM1042_JSP_JNL_BKCNT_ERR;
	return (E_DB_WARN);
    }
    ...
    */
    ...
    ule_format(E_DM103F_JSP_JNL_BKCNT_CHGD, (CL_SYS_ERR *)NULL, ULE_LOG,
	(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
...
DB_STATUS   *err_code)
{
	...
	*/
	if (most_recent_valid_ckp != -1)
	{
	  status = jsp_file_maint(jsx, dcb, &cnf, 
		cnf->cnf_jnl->jnl_history[0].ckp_sequence, err_code, TRUE);
	  if (status != E_DB_OK)
	  {
	      all_ok = FALSE;
	  } else
	  {
	      ...
	      */
              status = delete_invalid_ckp(jsx, dcb, &cnf, err_code, TRUE);
	  }
    ...
    */
	...
	if (db_open)
	{
		...
		DM2D_IGNORE_CACHE_PROTOCOL, &local_error);
	}

	if (config_open)
	{
	    (VOID)dm0c_close(*config, 0, &local_error);
	    *config = 0;
	}
...
bool        only_at_start)
{
    ...
    */
	     ...
	     this_seq++ )
	{
	    ...
	    if (!found_seq)
	    {
		for (i = 0; i < dmp_count-1; i++)
		{
		    if (cnf->cnf_dmp->dmp_history[i].ckp_sequence == this_seq)
		    {
			found_seq = TRUE;
			ckp_status = cnf->cnf_dmp->dmp_history[i].ckp_dmp_valid;
		    }
		}
	    }
	    if (found_seq)
		...
		} else
		{
		    ...
		    ** we are not deleting completely any more)
	 	    */
		    found_one = TRUE;
		    status = jsp_file_maint(jsx, dcb, &cnf,this_seq,err_code, 
    ...
    i4     *err_code)
{
    ...
    ** Get the record
    */
      status = dm2r_get(rcb, &tid, DM2R_GETNEXT, (PTR) &database, err_code);
      if (DB_FAILURE_MACRO(status))
          break;
	...
	if(cnf->cnf_dsc->dsc_dbservice & DU_UTYPES_ALLOWED)  
	{
	  /* Can't do Unicode C/D conversion on db that's already Unicode. */
          ...
          ule_format(E_DM1408_ALT_ALREADY_UNICODE, (CL_SYS_ERR *)0, ULE_LOOKUP,
          ...
          ule_format(E_DM1408_ALT_ALREADY_UNICODE, (CL_SYS_ERR *)NULL, ULE_LOG,
    ...
    */
    ...
    {
      local_status = dm2t_close(rcb, 0, err_code);
    ...
    if (tran_started)
    {
      ...
      /* rollback the changes */
          local_status = dmxe_abort((DML_ODCB *)NULL, iidcb, &tran_id,
              log_id, lock_id, dmxe_flag, 0, 0, &local_error);
      }
      else
      {
      /* commit the changes */
    ...
    if(status == E_DB_OK && cnf->cnf_dsc->dsc_dbservice == DU_UTYPES_ALLOWED)  
    {
        ...
        ule_format(E_DM1407_ALT_CON_UNICODE, (CL_SYS_ERR *)0, ULE_LOOKUP,
            (DB_SQLSTATE *)NULL, error_buffer, ER_MAX_LEN, &error_length,
            &local_error, 2, 0, line_buffer);
        ...
        ule_format(E_DM1407_ALT_CON_UNICODE, (CL_SYS_ERR *)NULL, ULE_LOG,
            (DB_SQLSTATE *)0, (char *)0, (i4)0, (i4 *)0,
            &local_error, 2, 0, line_buffer);
    }
    ...
    i4     *err_code)
{
    ...
    */
    if (MEcmp((PTR)&dcb->dcb_name, (PTR)DB_DBDB_NAME,
       sizeof(DB_DB_NAME))==0)
    {
      *err_code = E_DM1519_PMODE_IIDBDB_ERR;
      return (E_DB_ERROR);
    }
    ...
    */
      ...
      {
          if (*err_code == E_DM004C_LOCK_RESOURCE_BUSY)
              *err_code = E_DM1520_PMODE_DB_BUSY;
          break;
      }
      db_locked = TRUE;
    ...
    i4     *err_code)
{
    ...
    ** to complete.
    */
    status = dm0c_open(dcb, 0, dmf_svcb->svcb_lock_list, config, &cl_error);
    ...
    */
	...
	{
	    (VOID)dm2d_close_db(dcb, dmf_svcb->svcb_lock_list, 
		DM2D_IGNORE_CACHE_PROTOCOL, &cl_error);
	}
