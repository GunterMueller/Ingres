          ...
          i4            buffsize)
{
    ...
    */
	...
	{
	    *err_code = E_DM161B_FLOAD_DB_BUSY;
	    return (E_DB_INFO);
	}
    ...
    is_journaled = (dcb->dcb_status & DCB_S_JOURNAL) != 0;
	...
	** start a transaction
	*/
	flags = DMXE_DELAYBT | DMXE_INTERNAL;
	if (is_journaled)
	    flags |= DMXE_JOURNAL;
	status = dmxe_begin(DMXE_WRITE, flags, dcb, dcb->dcb_log_id,
	...
	*/
	...
	status = dm2t_open(dcb, &tab_id, flag, DM2T_UDIRECT, DM2T_A_WRITE,
	    ...
	    dml_xcb.xcb_lk_id, 0, DMC_C_SERIALIZABLE, DM2T_S, &tran_id,
	...
	*/
	...
	/* Error Contition 2: table is not partitioned but user spedify a partition_no */
	if (!(tab_tcb->tcb_rel.relstat & TCB_IS_PARTITIONED)&&(-1!=partition_no))
	{
	...
	*/
	status = dmxe_writebt(&dml_xcb, FALSE, err_code);
	if (status != E_DB_OK)
	{
	...
	*/ 
	...
	if (NULL!=master_rcb)
	{
	    ...
	    tab_rcb = NULL;
	    status = dm2t_close(master_rcb, DM2T_NOPURGE, err_code);
	    if (status != E_DB_OK)
	    {
		ule_format(E_DM1605_FLOAD_CLOSE_TABLE,(CL_SYS_ERR *)0,ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
    ...
    */
	...
	if(NULL!=master_rcb)
	{
	    local_stat = dm2t_close(master_rcb, DM2T_NOPURGE, &local_err);
	    if (local_stat != E_DB_OK)
	    {
		ule_format(local_err, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	            (char *)0, (i4)0, (i4 *)0, &local_err, 0);
