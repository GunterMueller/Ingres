	...
	*/
	if (jnl_processing)
	{
	    ...
	    */
		...
		status = rfp_jnlswitch(jsx, dcb, err_code);
	    }
	}
...
DB_STATUS	    *err_code)
{
    ...
    */
    ...
    status = config_handler(open_flag, dcb, &cnf, err_code);
	...
	*/
	...
	if (jsx->jsx_status2 & JSX2_INCR_JNL)
	{
		...
		** rollforward. Change dsc_status in addition to
		** dsc_inconst_code which can get overwritten by other errors
		*/
		...
		if (cnf->cnf_dsc->dsc_inconst_code == DSC_INCR_RFP_INCONS)
		{
		    status = dm0c_mk_consistnt(cnf);
		    cnf->cnf_dsc->dsc_inconst_code = 0;
		}
	    }
	}
...
DB_STATUS           *err_code)
{
	...
	*/
	ule_format(E_DM1373_RFP_INCR_ARG, (CL_SYS_ERR *)0, ULE_LOG, NULL,
                        error_buffer, ER_MAX_LEN, &error_length,
                        err_code, 1, 20, "-c +j (before +c -j)");
        *err_code = E_DM1356_RFP_INVALID_ARGUMENT;
	return (E_DB_ERROR);
    }
...
DB_STATUS	    *err_code)
{
	...
	*/
	status = rfp_get_table_info (rfp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	local_status = dm2t_unfix_tcb( &master_tcb, 
		    (DM2T_PURGE | DM2T_NORELUPDAT),
		     dmf_svcb->svcb_lock_list, log_id, &local_error );
    }
...
DB_STATUS           *err_code)
{
	...
	*/
	...
	status = dm0j_open(0,
			...
			(DB_TAB_ID *)0, &jnl2, err_code);

	if (status == E_DB_OK)
	{
	    blk_seq = jnl2->dm0j_sequence - 1;
	    status = dm0j_close(&jnl2, err_code);
	}
	else
	{
	    (VOID) dm0j_close(&jnl, err_code);
	...
	if (rfp->rfp_status & RFP_NOROLLBACK)
	{
	    status = validate_incr_jnl(jsx, rfp, dcb, dmve, i, err_code);
	    if (status)
	    {
		(VOID) dm0j_close(&jnl, err_code);
		jnl = 0;
	...
	*new_jnl_cnt += 1;
	...
	if (incr_jnl)
	{
		...
		status = config_handler(UPDATE_CONFIG, dcb, &cnf, &local_error);
...
DB_STATUS	*err_code)
{
	...
	{
	    /*	Read backwards this journal file. */
	    status = dm0j_read(ljnl, (PTR *)&record, &l_record, err_code);
	    if (status)
		break;
	...
	status = dm0j_close(&ljnl, err_code);
	if (status)
	    break;
    ...
    ule_format(E_DM1377_RFP_INCR_JNL_WARNING, (CL_ERR_DESC *)0, ULE_LOG,
		NULL, error_buffer, ER_MAX_LEN, &error_length, err_code, 2,
		sizeof(dcb->dcb_name), dcb->dcb_name.db_db_name, 
		0, jnl_seq);
    ...
    if (ljnl)
	status = dm0j_close(&ljnl, &local_error);
...
DB_STATUS           *err_code)
{
	...
	status = dmf_jnlswitch(jsx, dcb, err_code);
	if (status)
	    dmf_write_msg(*err_code);
	...
	local_status = dm0l_closedb(dcb->dcb_log_id, err_code);
	dcb->dcb_log_id = 0;
	if (local_status)
	{
	    dmf_write_msg(*err_code);
...
i4	*err_code)
{
	...
	if (cl_status != OK)
	{
	    ule_format(cl_status, &sys_err, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    ule_format(E_DM900F_BAD_LOG_EVENT, &sys_err, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, err_code,
	...
	{
	    ule_format(E_DM1144_CPP_DEAD_ARCHIVER, &sys_err, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM1145_CPP_DEAD_ARCHIVER;
	    break;
	}
