...
**
**	where logfile name is optional. 
...
LDMP_CB	    *ldmp)
{
    ...
    */
	...
	if ((argv[1][0] != '-') && (argv[1][0] != '+') && (argv[1][0] != '#'))
	{
	    ...
	    ldmp->ldmp_l_filename = STlength(argv[1]);
	    STmove(argv[1], ' ', sizeof(ldmp->ldmp_filename),
		    ldmp->ldmp_filename);
	...
	*/
	...
	status = dm0l_allocate (dm0l_flags, 0, 0, 
                                    ldmp->ldmp_filename,
				    ldmp->ldmp_l_filename,
				    (DMP_LCTX **)0,
				    (i4 *)&sys_err);
    }
    else
	...
	status = dm0l_allocate(0, 0, 0, 0, 0, (DMP_LCTX **)0,
				(i4 *)&sys_err);
    }
    if (status != E_DB_OK)
    {
    ...
    MEcopy((PTR)DB_LOGDUMP_NAME, DB_MAXNAME, (PTR) &add_info.ad_dbname);
    ...
    MEcopy((PTR)DB_LOGDUMP_NAME, sizeof(DB_OWN_NAME), (PTR) &user_name);
	...
	*/
	...
	status = dm0l_position(DM0L_FLGA, lx_id, &la, lctx, &err_code);
    }
    else
	status = dm0l_position(DM0L_FIRST, lx_id, 0, lctx, &err_code);
    ...
    {
	status = dm0l_read(
	    lctx, lx_id, (DM0L_HEADER **)&record, &la, &err_code);
			      ...
			      && err_code ==  E_DM9238_LOG_READ_EOF)
	{
	    ...
	    */
	    ...
	    status = dm0l_position(DM0L_LAST, lx_id, 0, lctx,
		&err_code);
	    if (status != E_DB_OK)
	    {
	    ...
	    */
	    ...
	    if (err_code == E_DM9238_LOG_READ_EOF && count)
	    {
		status = E_DB_OK;
...
PTR	    record)
{
	...
	case DM0LOLDINDEX:
	    table_name = ((DM0L_OLD_INDEX *)h)->dui_name.db_tab_name;
	    owner_name = ((DM0L_OLD_INDEX *)h)->dui_owner.db_own_name;
	    break;
