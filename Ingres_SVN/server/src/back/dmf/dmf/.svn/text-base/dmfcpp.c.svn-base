...
i4	    *err_code)
{
	...
	*err_code = E_DM1108_CPP_OPEN_DB;
	...
	if ( !(jsx->jsx_status & JSX_CKPXL) && CXcluster_enabled() )
	{
	    ...
	    ** completes.
	    */
	    status = GetClusterStatusLock(jsx, err_code);
	...
	*/
	...
	if ((jsx->jsx_status & JSX_CKPXL) == 0)
	{
		...
		if ( status == E_DB_OK )
		{
		    status = ReleaseClusterStatusLock(jsx, err_code);
	...
	if ( ClusterStatusLocked )
	    (VOID)ReleaseClusterStatusLock(jsx, &local_err_code);
...
DB_STATUS	    *err_code)
{
	...
	*/
	status = cpp_get_table_info (cpp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	if ( !(jsx->jsx_status1 & JSX1_CKPT_DB) &&
		...
		*/
		...
		{
		    loc_status = dm0p_alloc_pgarray(reltup.relpgsize, err_code);
