...
DB_STATUS	    *err_code)
{
    ...
    */
	...
	JOURNAL_DISABLED,NOLOGGING,,,,,,,,INCR_RFP",
...
WILL_COMMIT_ERR,NOLOGGING_ERROR,NOLOGGING_OPENDB,,RFP_FAIL,INDEX_INCONSISTENT,\
TABLE_INCONSISTENT,INCREMENTAL_RFP",
...
DB_STATUS       *err_code)
{
	...
	*/
	...
	if (status != E_DB_OK)
	{
		...
		0L, (i4 *)0, &local_err, 0);
	    *err_code = E_DM1365_RFP_CHKPT_LST_OPEN;
	    break;
	}
    	TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
	...
	if (local_status != E_DB_OK)
	{
	    ...
	    {
		status = local_status;
            	*err_code = E_DM1367_RFP_CHKPT_LST_CLOSE;
	    }
	}
    } while (FALSE);
    ...
    {
	ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char * )0, 0L, (i4 *)0, &local_err, 1,
		sizeof(DM_FILE), tblctx.filename.name);
	*err_code = E_DM1059_JSP_INFODB_ERR;
    }
    return(status);
}
