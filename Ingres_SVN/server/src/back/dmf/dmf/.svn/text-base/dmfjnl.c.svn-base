...
i4		*err_code)
{
    ...
    char		*cptr;
    struct				/* iidbcapabilities structure */
    {
	char 		cap_capability[DB_MAXNAME];
	char		cap_value[DB_MAXNAME];
    } iidbc;
...
#define			UPPER_DBCAP_NAME "IIDBCAPABILITIES"
	...
	*/
	...
	status = dm2t_open(dcb, &table_id, DM2T_IS,   
	    ...
	    0, &tran_id, &timestamp, &r_idx, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	{
	    if (*err_code == E_DM0055_NONEXT)
		*err_code = E_DM918A_LISTTAB_NONEXISTENT; /* XXX */
	    break;
	}
	...
	*/
	...
	status = dm2t_open(dcb, &table_id, DM2T_IS,    
	    DM2T_UDIRECT, DM2T_A_READ, (i4)0, (i4)20, 
	    0, 0, dmf_svcb->svcb_lock_list, (i4)0,(i4)0, 
	    0, &tran_id, &timestamp, &rel, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	status = dm2t_open(dcb, &table_id, DM2T_IS,  DM2T_UDIRECT, DM2T_A_READ,
	    ...
	    &timestamp, &dbc, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	status = dm2t_close(dbc, DM2T_NOPURGE, err_code);
	if (status != E_DB_OK)
	    break; 
    ...
    */
    if (r_idx)
	loc_status = dm2t_close(r_idx, DM2T_NOPURGE, &loc_err); 

    if (rel)
	loc_status = dm2t_close(rel, DM2T_NOPURGE, &loc_err);

    if (dbc)
	loc_status = dm2t_close(dbc, DM2T_NOPURGE, &loc_err); 
    ...
    *err_code = E_DM104A_JSP_DELIM_SETUP_ERR;
...
bool		fully_delete)
{
    ...
    */
	...
	if (first_jnl && fully_delete)
	{
		...
		if (status != E_DB_OK)
		{
		    dmf_write_msg(E_DM1110_CPP_DELETE_JNL);
		    *err_code = E_DM1112_CPP_JNLDMPCKP_NODEL;
		    return (E_DB_ERROR);
		}
	    }
	}
	...
	*/
	...
	for (i = 1;; i++)
	{
	    ...
	    if (status != OK)
	    {
		dmf_write_msg(E_DM1111_CPP_DELETE_CKP);
		*err_code = E_DM1112_CPP_JNLDMPCKP_NODEL;
		return (E_DB_ERROR);
	    }
	...
	*/
	status = dm0d_d_config(dcb, cnf, ckp_seq, err_code);
	if (DB_FAILURE_MACRO(status))
	{
	    dmf_write_msg(E_DM1134_CPP_DELETE_DMP);
	    *err_code = E_DM1112_CPP_JNLDMPCKP_NODEL;
	    return (E_DB_ERROR);
	}
	status = E_DB_OK;
...

	{
		...
		ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
	    	...
	    	dmf_write_msg(E_DM116B_CPP_CHKPT_DEL_ERROR);
	    	*err_code = E_DM1112_CPP_JNLDMPCKP_NODEL;
	    	return (E_DB_ERROR);
	    }
	}
    ...
    */
	...
	if (first_dmp)
	{
		...
		status = dm0d_delete(0, (char *)&dcb->dcb_dmp->physical,
		    dcb->dcb_dmp->phys_length, first_dmp, err_code);
		if (status != E_DB_OK)
		{
		    dmf_write_msg(E_DM1134_CPP_DELETE_DMP);
		    *err_code = E_DM1112_CPP_JNLDMPCKP_NODEL;
    ...
    */
	...
	ule_format(E_DM1405_ALT_CKP_DELETED, (CL_SYS_ERR *)0, ULE_LOOKUP, 
	...
	ule_format(E_DM1405_ALT_CKP_DELETED, (CL_SYS_ERR *)NULL, ULE_LOG,
    ...
    do 
    {
		...
		*err_code = E_DM1017_JSP_II_DATABASE;
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    error_buffer, ER_MAX_LEN, &error_length, err_code, 0);
	    ...
	    */
	    status = dm2d_open_db(iidcb, access_mode, lock_mode,
			dmf_svcb->svcb_lock_list, iidb_flag, err_code);
...
i4		*err_code)
{
    ...
    ule_format(cl_status, &sys_err, ULE_LOG, NULL, (char *)0, 0L, 
		(i4 *)0, &local_err, 0);

    *err_code = E_DM1058_JSP_TBL_LST_ERR;
    return(E_DB_ERROR);
}
...
i4		*err_code)
{
    ...
    cl_status = DIopen(d, t->physical.name, t->phys_length, file.name, 
		sizeof(file), t->blksz, DI_IO_WRITE, DI_SYNC_MASK, &sys_err);
