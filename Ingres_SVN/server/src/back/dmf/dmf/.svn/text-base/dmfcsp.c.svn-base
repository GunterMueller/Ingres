...
csp_restart(bool *local_log_recovered, i4 *errcode)
{
	...
	*errcode = csp_release_recovery_lock();
	...
	TRformat(opc_send, 0, buffer, sizeof(buffer), 
	         "Ingres/CSP restart recovery completed on node %s.",
		 CXhost_name());
	status = E_DB_OK;
	break;
    }
...
csp_node_fail(i4 *err_code)
{
    ...
    TRformat(opc_send, 0, buffer, sizeof(buffer), 
	     "Ingres/CSP performing node failure recovery on node %s.",
	     CXhost_name());
	      ...
	      i++ )
	{
	    ...
	    TRformat(opc_send, 0, buffer, sizeof(buffer), 
		     "Ingres/CSP recovering for failing node %~t on node %s",
	...
	*/
	if (csp_global.csp_master_node == csp_global.csp_self_node)
	{
		...
		TRformat(opc_send, 0, buffer, sizeof(buffer), 
		         "Ingres/CSP recovering dbms classes for failing "
                         "node %~t on node %s", cx_node_info->cx_node_name_l,
		         cx_node_info->cx_node_name, CXhost_name());
	...
	TRformat(opc_send, 0, buffer, sizeof(buffer), 
	         "Ingres/CSP node failure recovery complete on node %s.",
		 CXhost_name());
	status = OK;
	break;
    }
...
try_lk_resume()
{
	...
	if ( !csp->csp_node_failure  && csp->csp_dbms_rcvr_cnt == 0)
	{
	    ...
	    */
	    ...
	    TRformat(opc_send, 0, buffer, sizeof(buffer), 
		     "Ingres/CSP normal Lock processing resumed on node %s.",
		     CXhost_name());
	}
	else
	{
