...
dmfacp(VOID)
{
    ...
    */
    ...
    {
	EXdelete();
	ule_format(E_DM904A_FATAL_EXCEPTION, (CL_ERR_DESC *)0, ULE_LOG, 
	...
	*/
	...
	ule_format(exit_error, (CL_ERR_DESC *)0, ULE_LOOKUP,
	    (DB_SQLSTATE *)NULL, exit_text, sizeof(exit_text), &exit_msg_len,
	    &err_code, 0); 
	acp_exit_handler(exit_error, exit_text, exit_msg_len, (char *)0);
	PCexit(FAIL);
    }
    ...
    if ( ret_status != OK)
    {
	ule_format(E_UL0016_CHAR_INIT, (CL_ERR_DESC *)&sys_err, ULE_LOG , 
...
#define ACP_FILENAME_LEN ((sizeof(ACP_LOGFILE_NAME))-1)
	...
	qualname = filename;
        if( (status = NMloc(FILES, FILENAME, qualname, &loc)) != OK)
        {
    ...
    TRdisplay(" \n%@ ACP STARTUP.\n");
    ule_format( E_SC0129_SERVER_UP, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		(i4)0, (i4 *)0, &err_code, 2, STlength(Version),
    ...
    */
    status = dma_prepare(DMA_ARCHIVER, &acb, (DMP_LCTX *)0, &err_code);
    if (status != E_DB_OK)
    {
    ...
    */
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    ...
	    err_code = E_DM9815_ARCH_SHUTDOWN;
	    break;
	}
...
PURGEDB,,,,,,,ECPDONE", event);
	     ...
	     (event & LG_E_PURGEDB) )       
	{
	    ...
	    ** the logging system.
	    */
	    status = dma_online_context(a, &err_code);
	    if (status != E_DB_OK)
	    {
    ...
    */
    ...
    if (err_code != E_DM9815_ARCH_SHUTDOWN)
	acp_restart = 1;
	...
	** unhappily. err_code contains the reason for the unsuccessful
	...
	*/
    ...
    status = dma_complete(&a, &err_code);
    if (status != E_DB_OK)
    {
	ule_format(err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &err_code, 0);
    ...
    ule_format(E_DM9815_ARCH_SHUTDOWN, &sys_err, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4)0, &err_code, 0);
...
char	    *dbname)
{
    ...
    */
    ule_format(0, (CL_ERR_DESC *)0, ULE_MESSAGE, (DB_SQLSTATE *)NULL,
	    exit_message, exit_msg_len, (i4 *)0, &local_error, 0);
	...
	*/
	stat = PCcmdline((LOCATION *) NULL, command_buf, (i4) NULL, 
    ...
    */
	...
	if (stat)
	{
	    ule_format(stat, cl_err_desc, ULE_LOG, (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
	}

	if (error)
	{
	    ule_format(error, (CL_ERR_DESC *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
	}

	ule_format(E_DM983B_ARCH_EXIT_ERROR, (CL_ERR_DESC *)0, ULE_LOG, 
	    (DB_SQLSTATE *)0, (char *)0, (i4)0, (i4 *)0, &local_error,
	    0);
    }
}
