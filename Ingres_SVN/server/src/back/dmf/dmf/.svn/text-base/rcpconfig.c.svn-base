...
"     rcpconfig%s -rcp_ignore_db=dbname\n",
"     rcpconfig%s -rcp_ignore_table=tablename\n",
"     rcpconfig%s -rcp_ignore_lsn=lsnhigh,lsnlow\n",
"     rcpconfig%s -rcp_stop_lsn=lsnhigh,lsnlow\n",
0	/* trailing 0 marks end of the message */
};

/*{
...
char		    *argv[])
{
    ...
    */
	...
	ule_format(status, (CL_ERR_DESC *) NULL, ULE_LOOKUP, 
    ...
    */
				...
				sizeof(shutdown_action), &sys_err))
	{
	    ule_format(status, &sys_err, ULE_LOG, NULL, (char *)0, 0L,
			(i4 *)0, &err_code, 0);
	    (VOID) ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, &err_code, 1, 0, LG_A_SHUTDOWN);
	...
	*/
     ...
     ** the log file.
     */
     MEcopy((PTR)DB_RCPCONFIG_NAME, sizeof(DB_OWN_NAME), (PTR) &user_name);
	...
	(VOID) ule_format(E_DM900C_BAD_LOG_BEGIN, &sys_err, ULE_LOG, NULL,
	...
	return (FAIL);
     ...
     if (status = LGerase(&lg_id, lx_id, header.lgh_count, block_size,
	...
	else
	{
	    _VOID_ ule_format(status, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &err_code, 0);
	    (VOID) ule_format(E_DM9018_BAD_LOG_ERASE, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &err_code, 1,
		0, "Check II_LOG_FILE");
...
LG_LXID		lx_id)
{
    ...
    */
	...
	(VOID) ule_format(status, (CL_ERR_DESC *)&sys_err, ULE_LOG , NULL, 
		    (char *)0, 0L, (i4 *)0, &err_code, 0);
	(VOID) ule_format(E_DM904E_BAD_LOG_COPY, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &err_code, 0);
...
i4		logs_to_erase)
{
    ...
    *block_size = rconf.bsize;
	...
	(VOID) ule_format(status, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &err_code, 0);
	(VOID) ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &err_code, 1, 0, LG_A_BLKS);
	...
	(VOID) ule_format(status, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &err_code, 0);
	(VOID) ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &err_code, 1, 0, LG_A_LDBS);
	...
	*/
	...
	(VOID) ule_format(E_DM9012_BAD_LOG_OPEN, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &err_code, 0);
	...
	return (OK);
     ...
     MEcopy((PTR)DB_RCPCONFIG_NAME, DB_MAXNAME, (PTR) &add_info.ad_dbname);
    ...
    */
	...
	"The -node option is only allowed from an Ingres cluster member\n");
		...
		*/
		...
		if ( MEcmp(&argv[i][1], "rcp_ignore_db=", 14) == 0)
		{
		    ...
		    MEmove(STlength(&argv[i][15]), &argv[i][15], ' ', 
			sizeof(recov_struct->lg_dbname),
			&recov_struct->lg_dbname[0]);
		}
		else if ( MEcmp(&argv[i][1], "rcp_ignore_table=", 17) == 0)
		{
		    rcf_action = RCF_IGNORE_TABLE;
		    ...
		    MEmove(STlength(&argv[i][18]), &argv[i][18], ' ', 
			sizeof(recov_struct->lg_tabname),
			&recov_struct->lg_tabname[0]);
		}
		else if ( MEcmp(&argv[i][1], "rcp_ignore_lsn=", 15) == 0)
		{
		    rcf_action = RCF_IGNORE_LSN;

		    status = parse_lsn(&argv[i][16], &recov_struct->lg_lsn);
		    if (status)
			invalid_args = TRUE;
		}
...
primary_currently_enabled(void)
{
    ...
    i4		one_page;
    char	buffer[2048];
    char	file[LG_MAX_FILE][MAX_LOC];
    char	path[LG_MAX_FILE][MAX_LOC];
