...
# endif
	...
	*/
	...
	/*
	** Enable/disable asynchronous writes during load/modify/index
	** recovery.
	*/
	status = dmf_setup_optim();
	if (status != E_DB_OK)
	{
	    *err_code = E_DM9411_DMF_INIT;
	    return (status);
	}
	...
	** manager will check and write checksums to all database pages.
	...
	}	
	{
	    ...
	    status = sca_add_datatypes((PTR)0,
					dmf_adf_svcb,
	...
	if (status != OK)
	{
	    ...
	    ule_format(E_DM00D0_LOCK_MANAGER_ERROR, &sys_err, ULE_LOG, NULL, 
		(char * )0, 0L, (i4 *)0, err_code, 0);
	    *err_code = E_DM9411_DMF_INIT;
	    return(E_DB_ERROR);
	}
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ule_format(sxf_rcb.sxf_error.err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	    *err_code = E_DM1030_JSP_SXF_INIT_ERR;
	    break;
	}
	...
	if (status != E_DB_OK)
	{
	    ule_format(status,0, ULE_LOG, NULL, 0,
			0, 0, err_code, 0);
	    break;
	}
	return (E_DB_OK);
	...
	*err_code = E_DM00AB_CANT_MAP_SHARED_MEM;
    else
#endif
	...
	i4 ustat)
{
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    (VOID) ule_format(sxf_rcb.sxf_error.err_code, NULL, ULE_LOG, 
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
	    (VOID) ule_format(E_DM1031_JSP_SXF_SES_INIT, NULL, ULE_LOG, 
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
	}
	return status;
}
...
dmf_sxf_end_session()
{
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ule_format(sxf_rcb.sxf_error.err_code, 0, ULE_LOG, NULL, 0, 0, 0, 
			&err_code, 0);
	    (VOID) ule_format(E_DM1033_JSP_SXF_SES_FLUSH, NULL, ULE_LOG, 
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
	...
	if (status != E_DB_OK)
	{
	    ule_format(sxf_rcb.sxf_error.err_code, 0, ULE_LOG, NULL, 0, 0, 0, 
			&err_code, 0);
	    (VOID) ule_format(E_DM1032_JSP_SXF_SES_TERM, NULL, ULE_LOG, 
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
	}
	return status;
}
...
dmf_sxf_terminate(i4 *err_code)
{
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ule_format(sxf_rcb.sxf_error.err_code, 0, ULE_LOG, NULL, 0, 0, 0, 
				&local_err, 0);
	     *err_code = E_DM0083_ERROR_STOPPING_SERVER;
	}
	dmf_sxf_svcb=NULL;
	return status;
}
...

/*
** Name: dmf_setup_optim() - Setup svcb_load_optim
**
** Description:
**	This routine sets up the svcb svcb_load_optim field from
**	config.dat, from the "ii.$.rcp.load_optim" parameter.
**
** Inputs:
**
** Outputs:
...
dmf_setup_optim(VOID)
{
    ...
    /*
    ** Enable/disable asynchronous writes during load/modify/index
    ** recovery.  Default is on.
    */
    dmf_svcb->svcb_load_optim = TRUE;

    status = PMget("ii.$.rcp.load_optim", &pm_str);
    if (status == E_DB_OK)
    {
	if (STcasecmp(pm_str, "OFF" ) == 0)
	    dmf_svcb->svcb_load_optim = FALSE;
    }
...
PTR		    control_block;
{
    ...
    */
	...
	case DMT_CLOSE:
	    ...
	    status=dm2t_close(rcb, 0,  &cb->error.err_code);
	    break;
	...
	if(status!=E_DB_OK)
	{
		ule_format(E_DM1055_JSP_SXF_DMF_ERR, NULL, ULE_LOG, NULL,
