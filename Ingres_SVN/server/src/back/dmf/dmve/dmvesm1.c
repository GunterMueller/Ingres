...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	case DMVE_UNDO:
	    ...
	    */
		...
		status = dm0l_sm1_rename(dmve->dmve_log_id, dm0l_flags, 
		    ...
		    log_rec->sm1_oldpgsize, log_rec->sm1_newpgsize, &error);
		if (status != E_DB_OK)
		{
		...
		*/
		status = dm2t_open(dcb, tbl_id, DM2T_IX, DM2T_UDIRECT,
				    ...
				    &timestamp, &dev_rcb, (DML_SCB *)0, &error);
		if (status)
		    break;
	    ...
	    */
		...
		status = dm2f_close_file(t->tcb_table_io.tbio_location_array,
                        t->tcb_table_io.tbio_loc_count, (DM_OBJECT *)t, &error);
		if (status != OK)
		{
		    break;
		}
	    ...
	    {
		oldexist = TRUE;
		status = dm2f_close_file(&loc,loc_count, (DM_OBJECT *)t,&error);
	    }
	    ...
	    status = dm2f_open_file(dmve->dmve_lk_id, dmve->dmve_log_id,
		&loc, loc_count, 
		(i4)DM_PG_SIZE, DM2F_E_READ, db_sync_flag, 0, &error);
	    if (status != OK && error == E_DM9291_DM2F_FILENOTFOUND)
	    {
		tempexist = FALSE;
	    }
	    else if (status != OK)
	    ...
	    if (newexist == FALSE)
	    {
			...
			filelength, &dmve->dmve_dcb_ptr->dcb_name, &sys_err);
		    if (status != OK)
		    {
			break;
		    }
	    ...
	    if (oldexist == FALSE)
	    {
			...
			&dmve->dmve_dcb_ptr->dcb_name, &sys_err);
		    if (status != OK)
		    {
			break;
		    }
		...
		DM2F_A_WRITE, db_sync_flag, 0, &error);
	    if (status != OK)
	    {   
		break;
	    }
	    ...
	    */
		...
		status = dm0c_close(config, DM0C_UPDATE, &error);
		if (status != E_DB_OK)
		    break;
	    }
	...
	case DMVE_DO:
	    ...
	    if (status)
		error = dmve->dmve_error.err_code;
	    break;    
	...
	dmve->dmve_error.err_code = E_DM9611_DMVE_SM1RENAME;
    }
    else
	dmve->dmve_error.err_code = error;
    return(status);
}

/*
...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	if (rcb)
	{
	    ...
	    ** If iidevices was opened, close it:
	    */
	    status = dm2t_close(rcb, DM2T_NOPURGE, &error);
	    if (status != E_DB_OK)
	    {
		rcb = 0;
		break;
    ...
    */
    if (rcb)
	_VOID_ dm2t_close(rcb, DM2T_NOPURGE, &local_error);
