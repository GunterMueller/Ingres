...
DMVE_CB		*dmve_cb)
{
    ...
    DMPP_PAGE		*old_page_ptr = NULL;
    DMPP_PAGE		*new_page_ptr = NULL;
    DB_STATUS		status = E_DB_OK;
    DB_STATUS		tmp_status;
    DMPP_ACC_PLV	*loc_plv;
    DMPP_ACC_TLV	*loc_tlv;
    LK_LKID		lockid;
    LK_LKID		oldpage_lockid;
    ...
    bool		opage_recover;
    bool		npage_recover;
    bool		physical_oldpage_lock = FALSE;
    ...
    MEfill(sizeof(LK_LKID), 0, &oldpage_lockid);
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->rep_tbl_id, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->rep_pg_type, 
		log_rec->rep_page_size, &loc_plv, &loc_tlv);
	...
	*/
	...
	else if (log_rec->rep_header.flags & DM0L_ROW_LOCK)
	{
	...
	*/
	if ((dcb->dcb_status & DCB_S_EXCLUSIVE) == 0)
	{
	    ...
	    */
	    status = dm2t_lock_table(dcb, &log_rec->rep_tbl_id, DM2T_IX,
		...
		*/
		if (log_rec->rep_header.flags & DM0L_PHYS_LOCK)
		    lock_action = LK_PHYSICAL;
                ...
                if (log_rec->rep_header.flags & DM0L_ROW_LOCK)
		    lock_type = LK_ROW;
		else
		    lock_type = LK_PAGE;
		...
		*/
		if (opage_recover)
		{
			...
			&log_rec->rep_tbl_id,
			log_rec->rep_otid.tid_tid.tid_page, LK_PAGE,
			lock_type == LK_PAGE ? LK_X : LK_IX,
			lock_action, (i4)0,
			tbio->tbio_relid, &dmve->dmve_tran_id,
			&oldpage_lockid, (i4 *)0, (LK_VALUE *)0, &error);
		    if (status != E_DB_OK)
			break;

		    if (lock_action == LK_PHYSICAL)
			physical_oldpage_lock = TRUE;
		}

		/*
		** Lock the page on which the new row resides if its not
		** the same page as we just locked above.  Bypass the lock
		** request if the location is not being recovered.
		*/
		if (npage_recover)
		{
		    status = dm0p_lock_page(dmve->dmve_lk_id, dcb, 
			&log_rec->rep_tbl_id,
			log_rec->rep_ntid.tid_tid.tid_page, LK_PAGE,
			lock_type == LK_PAGE ? LK_X : LK_IX,
			lock_action, (i4)0,
			tbio->tbio_relid, &dmve->dmve_tran_id,
			&newpage_lockid, (i4 *)0, (LK_VALUE *)0, &error);
		    if (status != E_DB_OK)
			break;
	...
	*/
	...
	if (opage_recover)
	{
	    status = dmve_cachefix_page(dmve, log_lsn,
					tbio, log_rec->rep_otid.tid_tid.tid_page,
					fix_action, loc_plv,
					&old_page_ptr, &error);
	    if (status != E_DB_OK)
		break;
	...
	{
	    status = dmve_cachefix_page(dmve, log_lsn,
					tbio, log_rec->rep_ntid.tid_tid.tid_page,
					fix_action, loc_plv,
					&new_page_ptr, &error);
	    if (status != E_DB_OK)
		break;
	}
	old_page = old_page_ptr;
	new_page = new_page_ptr;

        if (lock_type == LK_ROW)
	{
	    dm0p_lock_buf_macro(tbio, dmve->dmve_lk_id, &old_page);
	    buf_locked = TRUE;
	}
	...
	*/
	if (DMZ_ASY_MACRO(15))
	{
		...
		old_page, loc_plv, "OldDATA");
	    dmve_trace_page_info(log_rec->rep_pg_type, log_rec->rep_page_size,
		new_page, loc_plv, "NewDATA");
	}
	...
	*/
	...
	case DMVE_REDO:
            if (old_page && LSN_GTE(
		DMPP_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->rep_pg_type, old_page),
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			0, DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->rep_pg_type,
			    old_page), 
			0, DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->rep_pg_type,
			    old_page),
			0, DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->rep_pg_type,
			    old_page), 
			0, DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->rep_pg_type,
			    old_page),
		...
		old_page = NULL;
            ...
            if (new_page && LSN_GTE(
		DMPP_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->rep_pg_type, new_page),
		log_lsn) && ((dmve->dmve_flags & DMVE_ROLLDB_BOPT) == 0))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
	...
	*/
	...
	status = dm0m_allocate(mem_needed, (i4)0, (i4)MISC_CB,
	    (i4)MISC_ASCII_ID, (char *)0, &mem_ptr, &error);
	if (status != OK)
	{
	...
	*/
	...
	case DMVE_DO:
	case DMVE_REDO:
	    status = dmv_rerep(dmve, tbio, old_page, new_page, log_rec, 
	    oldrow_buf, newrow_buf, loc_plv, lock_type, lock_action, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_unrep(dmve, tbio, old_page, new_page, log_rec, 
	    oldrow_buf, newrow_buf, loc_plv, lock_type, lock_action, &error);
	    break;
	}
    ...
    if (buf_locked == TRUE)
	dm0p_unlock_buf_macro(tbio, dmve->dmve_lk_id, &old_page);
    ...
    */
	...
	tmp_status = dm0p_uncache_fix(tbio, DM0P_UNFIX, dmve->dmve_lk_id, 
		dmve->dmve_log_id, &dmve->dmve_tran_id, &old_page_ptr, 
		&loc_error);
    ...
    {
	tmp_status = dm0p_uncache_fix(tbio, DM0P_UNFIX, dmve->dmve_lk_id, 
		dmve->dmve_log_id, &dmve->dmve_tran_id, &new_page_ptr, 
		&loc_error);
	if (tmp_status != E_DB_OK)
	{
	    ule_format(loc_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
    ...
    ** then release them.
    */
    if (physical_oldpage_lock)
    {
        DB_STATUS	tmp_status;

	tmp_status = dm0p_unlock_page(dmve->dmve_lk_id, dcb, 
	    &log_rec->rep_tbl_id, log_rec->rep_otid.tid_tid.tid_page, 
	    LK_PAGE, tbio->tbio_relid, &dmve->dmve_tran_id, &oldpage_lockid, 
	...
	tmp_status = dm0p_unlock_page(dmve->dmve_lk_id, dcb, 
	    &log_rec->rep_tbl_id, log_rec->rep_ntid.tid_tid.tid_page, 
	    LK_PAGE, tbio->tbio_relid, &dmve->dmve_tran_id, &newpage_lockid, 
            (LK_VALUE *)0, &loc_error);
	if (tmp_status != E_DB_OK)
	{
	    ule_format(loc_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
    ...
    */
    ...
    if (status != E_DB_OK)
	dmve->dmve_error.err_code = E_DM960F_DMVE_REP;
...
i4             *err_code)
{
    ...
    */
    if (old_page)
    {
	status = (*plv->dmpp_get)(log_rec->rep_pg_type, log_rec->rep_page_size,
	    old_page, &log_rec->rep_otid, &record_size, &record, NULL, NULL, NULL);
	if (status != E_DB_OK)
	{
	    if ((dmve->dmve_action == DMVE_DO) &&
	    ...
	    *err_code = E_DM9637_REDO_REP;
	    return(E_DB_ERROR);
	}

	(*plv->dmpp_reclen)(log_rec->rep_pg_type, log_rec->rep_page_size,
	...
	*/
	    ...
	    (MEcmp((PTR)old_row, (PTR)record, compare_size) != 0))
	{
	    ...
	    dmve_trace_page_contents(log_rec->rep_pg_type, 
			log_rec->rep_page_size, old_page, "DATA");
	    ule_format(E_DM9637_REDO_REP, (CL_ERR_DESC *)0, ULE_LOG, NULL,
    ...
    ** the new record.
    */
    if (newrow_page && log_rec->rep_pg_type != TCB_PG_V1)
    {
	status = (*plv->dmpp_reallocate)(log_rec->rep_pg_type, 
	    log_rec->rep_page_size, newrow_page,
	    log_rec->rep_ntid.tid_tid.tid_line, log_rec->rep_nrec_size);
	if (status != E_DB_OK)
	{
	    status = dmve_clean(newrow_page, plv, tabio, 
			dmve->dmve_lk_id, lock_type, lock_action, err_code);
	    status = (*plv->dmpp_reallocate)(log_rec->rep_pg_type, 
		log_rec->rep_page_size, newrow_page,
		log_rec->rep_ntid.tid_tid.tid_line, log_rec->rep_nrec_size);
	}
	...
	*/
	if (index_count_changed || index_bit_changed)
	{
	    ...
	    */
	    ...
	    for (;;)
	    {
		rfp_lookup_action(dmve, &dmve->dmve_tran_id, (LG_LSN *)0, 
			    RFP_REC_NEXT, &action);
		if (action == NULL)
		    break;
    ...
    */
    if (old_page)
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, &old_page);
    if (new_page)
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, &new_page);
    ...
    */
	...
	if (old_page)
	{
	    (*plv->dmpp_delete)(log_rec->rep_pg_type, log_rec->rep_page_size,
	    ...
	    DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rep_pg_type, old_page, 
		*log_lsn);
	    DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->rep_pg_type, old_page, 
		DMPP_MODIFY);
	}
    }
    ...
    if (newrow_page)
    {
	(*plv->dmpp_put)(log_rec->rep_pg_type, log_rec->rep_page_size,
	    newrow_page, update_mode, &dmve->dmve_tran_id, &log_rec->rep_ntid,
	    log_rec->rep_nrec_size, new_row, 0, log_rec->rep_nrow_version,
	    (DMPP_SEG_HDR *)0);

	/* Write the LSN of the Rep log record to the updated page. */
	DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rep_pg_type, newrow_page, 
	    *log_lsn);
	DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->rep_pg_type, newrow_page, 
	    DMPP_MODIFY);
    }


    if (old_page)
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, &old_page);
    if (new_page)
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, &new_page);
...
i4             *err_code)
{
    ...
    */ 
    ...
    if (lock_type == LK_ROW || (log_rec->rep_header.flags & DM0L_PHYS_LOCK))
	update_mode = (DM1C_DIRECT | DM1C_ROWLK);
    else
	update_mode = DM1C_DIRECT;
    ...
    */
	...
	{
	    ule_format(E_DM966C_DMVE_TUPLE_MISMATCH, (CL_ERR_DESC *)0, 
		ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
		err_code, 8, 
		sizeof(DB_DB_NAME), tabio->tbio_dbname->db_db_name,
		log_rec->rep_tab_size, &log_rec->rep_vbuf[0],
	    ...
	    dmve_trace_page_contents(log_rec->rep_pg_type, 
			log_rec->rep_page_size, newrow_page, "DATA");
	    ule_format(E_DM9636_UNDO_REP, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0); 
	}
    }
    ...
    */
    if (old_page && log_rec->rep_pg_type != TCB_PG_V1) 
    {
	status = (*plv->dmpp_reallocate)(log_rec->rep_pg_type, 
	...
	{
	    status = dmve_clean(old_page, plv, tabio, 
			dmve->dmve_lk_id, lock_type, lock_action, err_code);
	    status = (*plv->dmpp_reallocate)(log_rec->rep_pg_type, 
		log_rec->rep_page_size, old_page,
		log_rec->rep_otid.tid_tid.tid_line, log_rec->rep_orec_size);
    ...
    */
	...
	{
	    (*plv->dmpp_delete)(log_rec->rep_pg_type, log_rec->rep_page_size,
		newrow_page, update_mode, reclaim_space, &dmve->dmve_tran_id,
	    ...
	    if (dmve->dmve_logging)
		DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rep_pg_type, 
		    newrow_page, lsn);
	}
    }
    ...
    {
	(*plv->dmpp_put)(log_rec->rep_pg_type, log_rec->rep_page_size, 
	    old_page, update_mode, &dmve->dmve_tran_id, &log_rec->rep_otid,
	    log_rec->rep_orec_size, old_row, 0, log_rec->rep_orow_version,
	    (DMPP_SEG_HDR *)0);

	DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->rep_pg_type, old_page, DMPP_MODIFY);

	if (dmve->dmve_logging)
	    DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rep_pg_type, old_page, lsn);
    }
