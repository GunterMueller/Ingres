...
DMVE_CB		*dmve_cb)
{
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->fmap_tblid, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->fmap_pg_type, 
		log_rec->fmap_page_size, &loc_plv, &loc_tlv);
	...
	*/
	...
	if (fmap_page_recover)
	{
	    ...
	    if (dmve->dmve_action != DMVE_UNDO)
	    {
		dm0p_pagetype(tbio, (DMPP_PAGE *)fmap_page, 
	...
	*/
	...
	case DMVE_REDO:
	    if (fhdr && LSN_GTE(
		DM1P_VPT_ADDR_FHDR_PG_LOGADDR_MACRO(log_rec->fmap_pg_type, fhdr),
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->fmap_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_PAGE_STAT_MACRO(log_rec->fmap_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_LOGADDR_HI_MACRO(log_rec->fmap_pg_type,
			    fhdr), 0, 
			DM1P_VPT_GET_FHDR_LOGADDR_LOW_MACRO(log_rec->fmap_pg_type,
	    ...
	    if (fmap && LSN_GTE(
		DM1P_VPT_ADDR_FMAP_PG_LOGADDR_MACRO(log_rec->fmap_pg_type, fmap),
		log_lsn) && ((dmve->dmve_flags & DMVE_ROLLDB_BOPT) == 0))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			sizeof(*tbio->tbio_relid), tbio->tbio_relid,
			sizeof(*tbio->tbio_relowner), tbio->tbio_relowner, 0,
			DM1P_VPT_GET_FMAP_PAGE_PAGE_MACRO(log_rec->fmap_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_PAGE_STAT_MACRO(log_rec->fmap_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_LOGADDR_HI_MACRO(log_rec->fmap_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_LOGADDR_LOW_MACRO(log_rec->fmap_pg_type,
	...
	case DMVE_UNDO:
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    0, DM1P_VPT_GET_FMAP_PAGE_PAGE_MACRO(log_rec->fmap_pg_type, 
			fmap),
		    0, DM1P_VPT_GET_FMAP_PAGE_STAT_MACRO(log_rec->fmap_pg_type,
			fmap),
		    0, DM1P_VPT_GET_FMAP_LOGADDR_HI_MACRO(log_rec->fmap_pg_type,
			fmap), 
		    0, DM1P_VPT_GET_FMAP_LOGADDR_LOW_MACRO(log_rec->fmap_pg_type,
			fmap),
	...
	*/
	...
	case DMVE_DO:
	case DMVE_REDO:
	    status = dmv_refmap(dmve, tbio, fhdr, fmap, log_rec, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_unfmap(dmve, tbio, fhdr, fmap, log_rec, loc_plv, 
		&error);
	    break;
	}
    ...
    */
    ...
    if (status != E_DB_OK)
	dmve->dmve_error.err_code = E_DM9641_DMVE_FMAP;
...
i4             *err_code)
{
    ...
    */
    if (fhdr) 
    {
	if (DM1P_VPT_GET_FHDR_COUNT_MACRO(log_rec->fmap_pg_type, fhdr) != 
	    log_rec->fmap_map_index)
	{
	    ule_format(E_DM9676_DMVE_FMAP_FHDR_STATE, (CL_ERR_DESC *)0, ULE_LOG,
		...
		0, DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->fmap_pg_type, fhdr),
		0, DM1P_VPT_GET_FHDR_COUNT_MACRO(log_rec->fmap_pg_type, fhdr), 
		0, log_rec->fmap_map_index);
	    dmd_log(1, log_rec, 4096);
	    ule_format(E_DM9643_REDO_FMAP, (CL_ERR_DESC *)0, ULE_LOG, NULL,
    ...
    */
	...
	DM1P_VPT_INCR_FHDR_COUNT_MACRO(log_rec->fmap_pg_type, fhdr);
	VPS_MAP_FROM_NUMBER_MACRO(log_rec->fmap_pg_type, 
	    (DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->fmap_pg_type, fhdr, 
	    log_rec->fmap_map_index)), log_rec->fmap_fmap_pageno);
	VPS_SET_FREE_HINT_MACRO(log_rec->fmap_pg_type, 
	    (DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->fmap_pg_type, fhdr, 
	    log_rec->fmap_map_index)));

	if ((log_rec->fmap_last_used / 
		DM1P_FSEG_MACRO(log_rec->fmap_pg_type, log_rec->fmap_page_size))
	    >= log_rec->fmap_map_index)
	    DM1P_VPT_SET_FHDR_HWMAP_MACRO(log_rec->fmap_pg_type, fhdr, 
		(log_rec->fmap_map_index + 1));

	DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(log_rec->fmap_pg_type,fhdr,DMPP_MODIFY);
	DM1P_VPT_SET_FHDR_PG_LOGADDR_MACRO(log_rec->fmap_pg_type, fhdr, *log_lsn);
    ...
    */
	...
	dm1p_fminit(fmap, log_rec->fmap_pg_type, log_rec->fmap_page_size);
	DM1P_VPT_SET_FMAP_SEQUENCE_MACRO(log_rec->fmap_pg_type, fmap, 
	    log_rec->fmap_map_index);
	...
	*/
	...
	DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(log_rec->fmap_pg_type,fmap,DMPP_MODIFY);
	DM1P_VPT_SET_FMAP_PG_LOGADDR_MACRO(log_rec->fmap_pg_type, fmap, *log_lsn);
...
i4             *err_code)
{
    ...
    */
	    ...
	    log_rec->fmap_map_index + 1)
	{
	    ...
	    dmd_log(1, log_rec, 4096);
	    ule_format(E_DM9642_UNDO_FMAP, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	}
    }
    ...
    if (fmap)
    {
	if (DM1P_VPT_GET_FMAP_SEQUENCE_MACRO(log_rec->fmap_pg_type, fmap) != 
	    log_rec->fmap_map_index)
	{
	    ule_format(E_DM9677_DMVE_FMAP_FMAP_STATE, (CL_ERR_DESC *)0, ULE_LOG,
		NULL, (char *)0, (i4)0, (i4 *)0, err_code, 6, 
		sizeof(DB_DB_NAME), tabio->tbio_dbname->db_db_name,
		sizeof(DB_TAB_NAME), &log_rec->fmap_tblname,
		sizeof(DB_OWN_NAME), &log_rec->fmap_tblowner,
		0, DM1P_VPT_GET_FMAP_PAGE_PAGE_MACRO(log_rec->fmap_pg_type, fmap),
		0, DM1P_VPT_GET_FMAP_SEQUENCE_MACRO(log_rec->fmap_pg_type, fmap),
		0, log_rec->fmap_map_index);
	    dmd_log(1, log_rec, 4096);
	    ule_format(E_DM9642_UNDO_FMAP, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
    ...
    */
    ...
    {
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
	DM1P_VPT_DECR_FHDR_COUNT_MACRO(log_rec->fmap_pg_type, fhdr);
	VPS_CLEAR_FREE_HINT_MACRO(log_rec->fmap_pg_type, 
	    (DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->fmap_pg_type,
	...
	DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(log_rec->fmap_pg_type,fhdr,DMPP_MODIFY);
	if (dmve->dmve_logging)
	    DM1P_VPT_SET_FHDR_PG_LOGADDR_MACRO(log_rec->fmap_pg_type, fhdr, lsn);
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
    }
    ...
    */
    ...
    {
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fmap);
	(*loc_plv->dmpp_format)(log_rec->fmap_pg_type, log_rec->fmap_page_size,
				(DMPP_PAGE *) fmap,
	...
	DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(log_rec->fmap_pg_type,fmap,DMPP_MODIFY);
	if (dmve->dmve_logging)
	    DM1P_VPT_SET_FMAP_PG_LOGADDR_MACRO(log_rec->fmap_pg_type, fmap, lsn);
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fmap);
    }
