...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	case DMVE_UNDO:
	    ...
	    */
		...
		status = dm0l_sm2_config(dmve->dmve_log_id, dm0l_flags, 
		    ...
		    &log_rec->sm2_newidx, log_lsn, &lsn, &error);
		if (status != E_DB_OK)
		{
	...
	ule_format(error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, &error, 0);
	dmve->dmve_error.err_code = E_DM9612_DMVE_SM2CONFIG;
    }
    else
...
)
{
	...
	*/
	...
	status = dm0c_open( dcb, DM0C_PARTIAL, lock_list, &config, &error);
	if (status)
	    break;
	...
	*/
		...
		oldrel->relpgtype != newrel->relpgtype)
	{
	    ...
	    */
	    ...
	    dm1c_getaccessors(newrel->relcomptype,
				newrel->relpgtype,
				newrel->relpgsize,
				&t->tcb_acc_plv,
				&t->tcb_acc_tlv);
	...
	if (tbl_id->db_tab_base == DM_B_RELATION_TAB_ID)
	{
		...
		*/
		...
		/* Update the set of accessors (may have changes) */
		dm1c_getaccessors(newidx->relcomptype,
				    newidx->relpgtype,
				    newidx->relpgsize,
				    &t->tcb_iq_next->tcb_acc_plv,
				    &t->tcb_iq_next->tcb_acc_tlv);
	    }
    ...
    */
    ...
    if (config)
	_VOID_ dm0c_close(config, DM0C_UPDATE, &local_error);
