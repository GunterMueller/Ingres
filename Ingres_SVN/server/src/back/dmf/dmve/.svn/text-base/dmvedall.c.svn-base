...
DMVE_CB		*dmve_cb)
{
	...
	*/
	if ((log_rec->dall_header.type != DM0LDEALLOC) ||
	    (log_rec->dall_header.length != 
                (sizeof(DM0L_DEALLOC) -
                        (DB_MAXNAME - log_rec->dall_tab_size) -
                        (DB_MAXNAME - log_rec->dall_own_size))))
	{
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->dall_tblid, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->dall_pg_type, 
		log_rec->dall_page_size, &loc_plv, &loc_tlv);
	...
	*/
	...
	case DMVE_REDO:
            if (fhdr && LSN_GTE(
		DM1P_VPT_ADDR_FHDR_PG_LOGADDR_MACRO(log_rec->dall_pg_type, fhdr),
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->dall_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_PAGE_STAT_MACRO(log_rec->dall_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_LOGADDR_HI_MACRO(log_rec->dall_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_LOGADDR_LOW_MACRO(log_rec->dall_pg_type,
		...
		fhdr = NULL;
            ...
            if (fmap && LSN_GTE(
		DM1P_VPT_ADDR_FMAP_PG_LOGADDR_MACRO(log_rec->dall_pg_type, fmap),
		log_lsn) && ((dmve->dmve_flags & DMVE_ROLLDB_BOPT) == 0))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			sizeof(*tbio->tbio_relid), tbio->tbio_relid,
			sizeof(*tbio->tbio_relowner), tbio->tbio_relowner, 0,
			DM1P_VPT_GET_FMAP_PAGE_PAGE_MACRO(log_rec->dall_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_PAGE_STAT_MACRO(log_rec->dall_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_LOGADDR_HI_MACRO(log_rec->dall_pg_type,
			    fmap), 0, 
			DM1P_VPT_GET_FMAP_LOGADDR_LOW_MACRO(log_rec->dall_pg_type,
		...
		fmap = NULL;
            ...
            if (free && LSN_GTE(
		DMPP_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->dall_pg_type, free),
		log_lsn) && ((dmve->dmve_flags & DMVE_ROLLDB_BOPT) == 0))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			sizeof(*tbio->tbio_relid), tbio->tbio_relid,
			sizeof(*tbio->tbio_relowner), tbio->tbio_relowner, 0, 
			DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->dall_pg_type, free),
			0, DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->dall_pg_type, 
			    free),
			0, DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->dall_pg_type,
			    free), 
			0, DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->dall_pg_type,
			    free),
	...
	case DMVE_UNDO:
		...
		(LSN_LT(DM1P_VPT_ADDR_FHDR_PG_LOGADDR_MACRO(log_rec->dall_pg_type,
		fhdr), log_lsn)))
	    {
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
		    ...
		    0, DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->dall_pg_type,
			fhdr),
		    0, DM1P_VPT_GET_FHDR_PAGE_STAT_MACRO(log_rec->dall_pg_type,
			fhdr),
		    0, DM1P_VPT_GET_FHDR_LOGADDR_HI_MACRO(log_rec->dall_pg_type,
			fhdr), 
		    0, DM1P_VPT_GET_FHDR_LOGADDR_LOW_MACRO(log_rec->dall_pg_type,
			fhdr),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DM1P_VPT_GET_FMAP_PAGE_PAGE_MACRO(log_rec->dall_pg_type,
			fmap),
		    0, DM1P_VPT_GET_FMAP_PAGE_STAT_MACRO(log_rec->dall_pg_type,
			fmap),
		    0, DM1P_VPT_GET_FMAP_LOGADDR_HI_MACRO(log_rec->dall_pg_type,
			fmap), 
		    0, DM1P_VPT_GET_FMAP_LOGADDR_LOW_MACRO(log_rec->dall_pg_type,
			fmap),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		error = E_DM9666_PAGE_LSN_MISMATCH;
		status = E_DB_ERROR;
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->dall_pg_type, free),
		    0, DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->dall_pg_type, free),
		    0, DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->dall_pg_type, 
			free), 
		    0, DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->dall_pg_type,
			free),
	...
	*/
	...
	case DMVE_DO:
	case DMVE_REDO:
	    status = dmv_redealloc(dmve, tbio, fhdr, fmap, free, log_rec, 
		loc_plv, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_undealloc(dmve, tbio, fhdr, fmap, free, log_rec, 
		loc_plv, &error);
	    break;
	}
    ...
    */
    ...
    if (status != E_DB_OK)
	dmve->dmve_error.err_code = E_DM9626_DMVE_DEALLOC;
...
i4             *err_code)
{
    ...
    */
    ...
    free_bit = log_rec->dall_free_pageno % 
		DM1P_FSEG_MACRO(log_rec->dall_pg_type, log_rec->dall_page_size);
    ...
    */
	...
	DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->dall_pg_type, fmap), free_bit)))
    {
    ...
    */
	...
	if (log_rec->dall_fhdr_hint)
	    VPS_SET_FREE_HINT_MACRO(log_rec->dall_pg_type, 
		(DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->dall_pg_type, fhdr, 
		log_rec->dall_map_index)));

	DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(log_rec->dall_pg_type,fhdr,DMPP_MODIFY);
	DM1P_VPT_SET_FHDR_PG_LOGADDR_MACRO(log_rec->dall_pg_type, fhdr, *log_lsn);
    ...
    */
	...
	BITMAP_SET_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->dall_pg_type, fmap),
	    free_bit);

	if (free_bit < 
	    (i4)DM1P_VPT_GET_FMAP_FIRSTBIT_MACRO(log_rec->dall_pg_type, fmap))
	    DM1P_VPT_SET_FMAP_FIRSTBIT_MACRO(log_rec->dall_pg_type,fmap,free_bit);

	DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(log_rec->dall_pg_type,fmap,DMPP_MODIFY);
	DM1P_VPT_SET_FMAP_PG_LOGADDR_MACRO(log_rec->dall_pg_type, fmap, *log_lsn);
    ...
    */
	...
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, &free);
        (*loc_plv->dmpp_format)(log_rec->dall_pg_type, log_rec->dall_page_size,
				free, log_rec->dall_free_pageno, DMPP_FREE,
				DM1C_ZERO_FILL);

        DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->dall_pg_type, free, DMPP_MODIFY);
	DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->dall_pg_type, free, *log_lsn);
	DMPP_VPT_SET_PAGE_TRAN_ID_MACRO(log_rec->dall_pg_type, free, 
	    dmve->dmve_tran_id);
...
i4             *err_code)
{
    ...
    */
    ...
    if (fmap && (BITMAP_TST_MACRO(
	DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->dall_pg_type, fmap), free_bit) == 0))
    {
    ...
    */
	...
	{
	    first_bit_index = 
		(i4)DM1P_VPT_GET_FMAP_FIRSTBIT_MACRO(log_rec->dall_pg_type, 
							    fmap)/DM1P_BTSZ;
            last_bit_index = 
		DM1P_MBIT_MACRO(log_rec->dall_pg_type, log_rec->dall_page_size) - 1;
            if (DM1P_VPT_GET_FMAP_LASTBIT_MACRO(log_rec->dall_pg_type, fmap) <
		DM1P_FSEG_MACRO(log_rec->dall_pg_type, log_rec->dall_page_size))
	    {
                last_bit_index = 
		(i4)DM1P_VPT_GET_FMAP_LASTBIT_MACRO(log_rec->dall_pg_type, fmap)
		    		/ DM1P_BTSZ;
	    }
	    ...
	    fhdr_freehint_update = TRUE;
            ...
            {
                if ((DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->dall_pg_type, fmap)[i]))
                {
		    /*
		    ...
		    */
			...
			DM1P_FSEG_MACRO(log_rec->dall_pg_type, log_rec->dall_page_size)))
			||
			((i4)(DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->dall_pg_type,
						fmap)[i]) !=
			(free_bit % DM1P_FSEG_MACRO(log_rec->dall_pg_type, log_rec->dall_page_size))))
		    {
    ...
    */
	...
	if (dmve->dmve_logging)
	{
		...
		*err_code = E_DM9626_DMVE_DEALLOC;
		return(E_DB_ERROR);
	    }
	}
    }
    ...
    */
	...
	{
	    dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
	    VPS_CLEAR_FREE_HINT_MACRO(log_rec->dall_pg_type, 
		(DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->dall_pg_type, fhdr, 
		log_rec->dall_map_index)));
	    DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(log_rec->dall_pg_type, fhdr,
		DMPP_MODIFY);

	    if (dmve->dmve_logging)
		DM1P_VPT_SET_FHDR_PG_LOGADDR_MACRO(log_rec->dall_pg_type, fhdr,
		    lsn);
	    dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
	}
    }
    ...
    */
    ...
    {
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fmap);
	BITMAP_CLR_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->dall_pg_type, fmap),
	    free_bit);
	DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(log_rec->dall_pg_type,fmap,DMPP_MODIFY);

	if (dmve->dmve_logging)
	    DM1P_VPT_SET_FMAP_PG_LOGADDR_MACRO(log_rec->dall_pg_type, fmap, lsn);
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fmap);
    }
    ...
    */
	...
	DMPP_VPT_REC_PAGE_LOG_ADDR_MACRO(log_rec->dall_pg_type, save_lsn, free);
	...
	DMPP_VPT_SET_PAGE_TRAN_ID_MACRO(log_rec->dall_pg_type, free, 
	    dmve->dmve_tran_id);
        DMPP_VPT_SET_PAGE_SEQNO_MACRO(log_rec->dall_pg_type, free, 0);
        DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->dall_pg_type, free, DMPP_MODIFY);
	...
	if (dmve->dmve_logging)
        {
	    DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->dall_pg_type, free, lsn);
        }
        else
        {
            DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->dall_pg_type, free, 
							save_lsn);
        }
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, &free);
    }
