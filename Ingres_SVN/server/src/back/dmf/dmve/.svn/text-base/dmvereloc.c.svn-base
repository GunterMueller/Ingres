...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	{
	case DMVE_UNDO:
	    status = dmv_unreloc(dmve, &error);
	    break;

	case DMVE_DO:
	    status = dmv_rereloc(dmve,  &error);
	    break;
	}
    ...
    dmve->dmve_error.err_code = E_DM960E_DMVE_RELOC;
...
i4		*err_code)
{
	...
	*/
	...
	status = dm0m_allocate(allocate_size, DM0M_ZERO, (i4)MISC_CB,
	    (i4)MISC_ASCII_ID, (char *)0, (DM_OBJECT **)&loc_cb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	status = dm2f_build_fcb(dmve->dmve_lk_id, dmve->dmve_log_id,
				...
				0, 0, (DMP_TABLE_IO *)0, db_sync_flag, err_code);
	if (status != OK)
	    break;
	oldfcb_alloc = TRUE;
	...
	*/
	status = dm2f_open_file(dmve->dmve_lk_id, dmve->dmve_log_id,
			    ...
			    db_sync_flag,
			    (DM_OBJECT *)dmve->dmve_dcb_ptr, err_code);
	if (status != OK)
	    break;
	oldfile_open = TRUE;
	...
	status = dm2f_flush_file(&newloc_info, loc_count, &log_rec->dur_name,
			    &dmve->dmve_dcb_ptr->dcb_name, err_code);
	if (status != OK)
	    break;
	...
	*/
	for (i = 0; i <= endpage; i += page_count)
	{
	    ...
	    */
		...
		&page_count, (i4)i, (char *)page_array, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	...
	status = dm2f_release_fcb(dmve->dmve_lk_id, dmve->dmve_log_id,
			     &oldloc_info, loc_count, DM2F_ALLOC, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	if (db_sync_flag == DI_USER_SYNC_MASK)
	{
		...
		&log_rec->dur_name, &dmve->dmve_dcb_ptr->dcb_name, err_code);
	    if (status != E_DB_OK)
		break;
	}
    ...
    */
    ...
    {
	status = dm2f_close_file(&oldloc_info, loc_count, 
			     (DM_OBJECT *)dmve->dmve_dcb_ptr, &local_error);
	if (status != E_DB_OK)
	{
    ...
    {
	status = dm2f_release_fcb(dmve->dmve_lk_id, dmve->dmve_log_id,
			     &newloc_info, loc_count, DM2F_ALLOC, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
    ...
    {
	status = dm2f_release_fcb(dmve->dmve_lk_id, dmve->dmve_log_id,
			     &oldloc_info, loc_count, DM2F_ALLOC, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
    ...
    *err_code = E_DM9686_REDO_RELOCATE;
	...
	*/
	     ...
	     ((log_rec->dur_header.flags & DM0L_CLR) == 0))
	{
	    ...
	    status = dm0l_relocate(dmve->dmve_log_id, dm0l_flags, 
			...
			&log_rec->dur_file, log_lsn, &lsn, err_code);
	    if (status != E_DB_OK) {
                /*
                 * Bug56702: return logfull indication.
                 */
    ...
    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
	(char *)0, (i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM9687_UNDO_RELOCATE;
