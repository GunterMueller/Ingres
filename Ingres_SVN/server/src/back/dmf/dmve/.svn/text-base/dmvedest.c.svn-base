...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	{
	case DMVE_DO:
	    status = dmv_redestroy(dmve, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_undestroy(dmve, &error);
	    break;
	}
    ...
    dmve->dmve_error.err_code = E_DM9608_DMVE_DESTROY;
...
i4		*err_code)
{
    ...
    *err_code = E_DM967E_REDO_DESTROY;
...
i4		*err_code)
{
	...
	*/
	    ...
	    ((log_rec->dud_header.flags & DM0L_CLR) == 0))
	{
		...
		&log_rec->dud_location[0], log_lsn, &lsn, err_code);
	    if (status != E_DB_OK) {
		/*
		 * Bug56702: return logfull indication.
		 */
    ...
    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
	(char *)0, (i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM967F_UNDO_DESTROY;
