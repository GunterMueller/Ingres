...
DMVE_CB		*dmve_cb)
{
    ...
    {
	ule_format(E_DM9601_DMVE_BAD_PARAMETER, 0, ULE_LOG, NULL, 0, 0, 0, 
		    &error, 0);
	dmve->dmve_error.err_code = E_DM9619_DMVE_JNLEOF;
	return (E_DB_ERROR);
    }
    ...
    status = dm0c_open(&dcb, 0, dmve->dmve_lk_id, &cnf, &error);
    if (status == E_DB_OK)
    {
	config = cnf;
