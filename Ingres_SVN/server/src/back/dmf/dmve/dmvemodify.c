...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	{
	case DMVE_DO:
	    status = dmv_remodify(dmve, log_rec, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_unmodify(dmve, log_rec, &error);
	    break;
	}
    ...
    */
    ...
    dmve->dmve_error.err_code = E_DM960C_DMVE_MODIFY;
...
i4		*err_code)
{
	...
	*/
	status = dm0m_allocate(sizeof(DMP_MISC) +
	    ...
	    (char *)0, (DM_OBJECT **)&misc, err_code);
	if (status != E_DB_OK)
	    break;
	...
	{
	    keyptrs = (DM2U_KEY_ENTRY **)0;
	    keyentry = (DM2U_KEY_ENTRY *)0;
	}
	else
	{
	...
	*/
	    ...
	    (log_rec->dum_name.db_tab_name[0] == '$') && octx)
	{
	    ...
	    status = dm2t_open(dmve->dmve_dcb_ptr, &log_rec->dum_rnl_tbl_id, 
		    ...
		    &timestamp, &rnl_rcb, (DML_SCB *)0, err_code);
	    if (status)
	    	break;
		    ...
		    */
		    status = dm0m_allocate(sizeof(DMP_MISC) + 
				sizeof(DM2U_OSRC_CB), DM0M_ZERO,
                                MISC_CB, MISC_ASCII_ID, (char *)0, 
				(DM_OBJECT **)&osrc_misc_cb, err_code);
		    if (status)
			break;
		...
		osrc->osrc_partno = i;
		if (osrc_created)
		    osrc->osrc_rnl_rcb->rcb_rnl_online = (DMP_RNL_ONLINE *)0;
		else
	...
	mcb->mcb_compressed = (log_rec->dum_status & TCB_COMPRESSED) ? TRUE : FALSE;
    ...
    */
    ...
    if ((log_rec->dum_flag & DM0L_START_ONLINE_MODIFY) &&
	(status == E_DB_INFO) &&
	(errcb.err_code == E_DM9CB1_RNL_LSN_MISMATCH))
    {
	...
	*/
	     ...
	     ((log_rec->dum_header.flags & DM0L_CLR) == 0))
	{
	    ...
	    status = dm0l_modify(dmve->dmve_log_id, dm0l_flags, 
			...
			log_rec->dum_bsf_lsn,
			log_rec->dum_name_id,
			log_rec->dum_name_gen, log_lsn, &lsn, err_code);
	    if (status != E_DB_OK) {
                /*
                 * Bug56702: return logfull indication.
                 */
