...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	{
	case DMVE_DO:
	    status = dmv_recreate(dmve, log_rec, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_uncreate(dmve, log_rec, &error);
	    break;
	}
    ...
    */
    ...
    dmve->dmve_error.err_code = E_DM9606_DMVE_CREATE;
...
i4		*err_code)
{
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->duc_page_type, 
		log_rec->duc_page_size, &loc_plv, &loc_tlv);
	...
	*/
	status = dm2f_build_fcb(dmve->dmve_lk_id, dmve->dmve_log_id,
			...
			&log_rec->duc_tbl_id, 0, 0, (DMP_TABLE_IO *)0, 
			db_sync_flag, err_code);
	if (status != E_DB_OK)
	    break;
	built_fcbs = TRUE;
	...
	*/
	status = dm1s_empty_table(dcb, &log_rec->duc_tbl_id,
			...
			log_rec->duc_loc_count, loc_cnf, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	status = dm2f_release_fcb(dmve->dmve_lk_id, dmve->dmve_log_id, loc_cnf,
			log_rec->duc_loc_count, DM2F_ALLOC, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
    ...
    {
	status = dm2f_release_fcb(dmve->dmve_lk_id, dmve->dmve_log_id, loc_cnf,
			log_rec->duc_loc_count, DM2F_ALLOC, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
