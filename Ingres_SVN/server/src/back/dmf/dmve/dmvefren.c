...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	{
	case DMVE_DO:
	    status = dmv_rerename(dmve, log_rec, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_unrename(dmve, log_rec, &error);
	    break;
	}
...
i4 	*err_code)
{
    ...
    *err_code = E_DM9681_UNDO_FRENAME;
...
i4 	*err_code)
{
	...
	*/
    ...
    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
	(char *)0, (i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM9680_REDO_FRENAME;
...
DM_FILE		*filename) 
{
	...
	*/
	...
	status = dm2f_build_fcb(dmve->dmve_lk_id, dmve->dmve_log_id,
				...
				DM2F_TAB, &dummy_tabid, 0, 0, (DMP_TABLE_IO *)0,
				db_sync_flag, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = dm2f_open_file(dmve->dmve_lk_id, dmve->dmve_log_id, &loc_entry,
	    1, (i4)DM_PG_SIZE, DM2F_E_READ, 0, (DM_OBJECT *)dcb, &error);
	if (status != E_DB_OK)
	{
	    if (error != E_DM9291_DM2F_FILENOTFOUND)
		break;
	...
	{
	    status = dm2f_close_file(&loc_entry, 1, (DM_OBJECT *)dcb, &error);
	    if (status != E_DB_OK)
		break;
	}
