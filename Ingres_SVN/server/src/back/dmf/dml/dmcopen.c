...
DMC_CB    *dmc_cb)
{
	...
	status = dm0m_allocate((i4)sizeof(DML_ODCB), 0, 
            (i4)ODCB_CB,
	    (i4)ODCB_ASCII_ID, (char *)scb, (DM_OBJECT **)&odcb, &error);
	if (status != E_DB_OK)
	{
	...
	status = dm2d_open_db(odcb->odcb_dcb_ptr, odcb->odcb_access_mode,
	    odcb->odcb_lk_mode, scb->scb_lock_list, flag_mask, &error);
	if (status != E_DB_OK)
	{
	...
	*/
	...
	if (!(dcb->dcb_status & DCB_S_JOURNAL) &&
		...
		!= 0) &&
	    (MEcmp((PTR)scb->scb_user.db_own_name, (PTR)DB_AUDITTHREAD_NAME,
                sizeof(DB_OWN_NAME))
		!= 0) )
        {
            ule_format(W_DM5422_IIDBDB_NOT_JOURNALED, (CL_SYS_ERR *)0, ULE_LOG,
