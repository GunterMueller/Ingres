...

{
	...
	switch (op_code)
	{
	    ...
	    */
	    ...
	    default:
		pop_cb->pop_error.err_code = E_AD7005_ADP_BAD_POP;
		break;
	}
    } while (0);
#ifdef	xDEBUG
	...
	{
	    pop_cb->pop_error.err_code = E_AD7010_ADP_USER_INTR;
	    status = E_DB_ERROR;
	}
	else if (pop_cb->pop_error.err_code != E_AD7001_ADP_NONEXT)
	{
	    ule_format(pop_cb->pop_error.err_code, 0, ULE_LOG,
					NULL, (char *) 0, 0L, (i4 *) 0,
					&err_code, 0);
	    pop_cb->pop_error.err_code = error_override;
	    ule_format(pop_cb->pop_error.err_code, 0, ULE_LOG,
					NULL, (char *) 0, 0L, (i4 *) 0,
					&err_code, 0);
	}
...

{
    ...
    */
	...
	pop_cb->pop_error.err_code = E_AD700C_ADP_BAD_POP_UA;
	return (E_DB_ERROR);
    }
			...
			*/
			if (load_blob)
			{
			    ...
			    */
				...
				{
				    status = dmpe_start_load(rcb, &err_code);
				    if (status == E_DB_OK)
				    {
				...
				pcb->pcb_dmrcb->error.err_data = 0;
				status = E_DB_ERROR;
			    }
			}
			else
...
# endif
			...
			*/
			...
			if (pcb->pcb_base_dmtcb.dmt_record_access_id)
			{
				...
				pcb->pcb_dmtcb->error.err_code
					 = pcb->pcb_base_dmtcb.error.err_code;
			}
			if (status || local_stat)
			{
			    ule_format(pcb->pcb_dmtcb->error.err_code, 0,
					    ULE_LOG, NULL, (char *) 0, 0L,
					    (i4 *) 0, &err_code, 0);
			    ule_format(E_DM9B05_DMPE_PUT_CLOSE_TABLE, 0,
					    ULE_LOG, NULL, (char *) 0, 0L,
					    (i4 *) 0, &err_code, 0);
			    STRUCT_ASSIGN_MACRO(pcb->pcb_dmtcb->error,
							pop_cb->pop_error);
			    if (status == E_DB_OK)
				status = local_stat;
			    break;
		...
		else
		{
		    ...
		    {
			STRUCT_ASSIGN_MACRO(pcb->pcb_dmrcb->error,
						    pop_cb->pop_error);
		    }
		    pcb->pcb_dmtcb->dmt_flags_mask = 0;
		    ...
		    if (close_status)
		    {
			ule_format(pcb->pcb_dmtcb->error.err_code, 0, ULE_LOG,
					NULL, (char *) 0, 0L, (i4 *) 0,
					&err_code, 0);
			ule_format(E_DM9B05_DMPE_PUT_CLOSE_TABLE, 0 , ULE_LOG,
					NULL, (char *) 0, 0L, (i4 *) 0,
					&err_code, 0);
			STRUCT_ASSIGN_MACRO(pcb->pcb_dmtcb->error,
						    pop_cb->pop_error);
		...
		*/
		...
		/* Lock temporary ET's of any flavor against the session
		** lock list (why are we locking them at all?)
		*/
		if (pcb->pcb_dmtcb->dmt_id.db_tab_base < 0)
		    pcb->pcb_dmtcb->dmt_flags_mask |= DMT_SESSION_TEMP;
...

{
	...
	if (!pcb->pcb_dmtcb->dmt_record_access_id)
	{
	    ...
	    */
	    ...
	    /* Lock temporary etabs (any flavor) against the session list */
	    if (pcb->pcb_dmtcb->dmt_id.db_tab_base < 0)
		pcb->pcb_dmtcb->dmt_flags_mask |= DMT_SESSION_TEMP;
	    ...
	    if (status)
	    {
		STRUCT_ASSIGN_MACRO(pcb->pcb_dmtcb->error, pop_cb->pop_error);
		break;
	    }
	    pcb->pcb_dmrcb->dmr_access_id =
		    pcb->pcb_dmtcb->dmt_record_access_id;
	...
	do
	{
		...
		else if (pcb->pcb_dmrcb->error.err_code == E_DM0055_NONEXT)
		{
		    ...
		    */
			...
			if ((pop_cb->pop_continuation & ADP_C_RANDOM_MASK) == 0)
			    ule_format(E_DM9B0B_DMPE_LENGTH_MISMATCH, 0,
				ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
				0, p->per_length1, 0, p->per_length0);
			pop_cb->pop_error.err_code = E_DM9B01_GET_DMPE_ERROR;
		    }
		    break;
		}
		STRUCT_ASSIGN_MACRO(pcb->pcb_dmrcb->error, pop_cb->pop_error);
		break;
	    }
	    if (repositioned == 1)
	    {
	...
	if (pop_cb->pop_error.err_code != E_AD7001_ADP_NONEXT)
	{
		...
		pcb->pcb_dmtcb->dmt_record_access_id = 0;
		if (local_status)
		    ule_format( pcb->pcb_dmtcb->error.err_code,
...
dmpe_delete(ADP_POP_CB     *pop_cb )
{
    ...
    MEfill(sizeof(adf_cb), 0, &adf_cb);
    pop_cb->pop_error.err_code = E_DB_OK;
	...
	for (;status == E_DB_OK;)
	{
		    ...
		    etab_rcb->rcb_tcb_ptr->tcb_rel.relspec == DB_HEAP_STORE)
		{
		    ...
		    status = dmpe_destroy_heap_etab(base_tcb, cpn->cpn_etab_id,&err_code);
	...
	{
	    ule_format(my_pop_cb.pop_error.err_code, 0 , ULE_LOG,
			    NULL, (char *) 0, 0L, (i4 *) 0,
			    &err_code, 0);
	    pop_cb->pop_error.err_code = E_AD7008_ADP_DELETE_ERROR;
	}
	else
	{
	    status = E_DB_OK;
...
dmpe_move(ADP_POP_CB *pop_cb, i4  load_blob, bool cleanup_source)
{
	...
	*/
	...
	status = dm0m_allocate(sizeof(DM_OBJECT)
			+ (int)ME_ALIGN_MACRO(under_dv.db_length, sizeof(PTR))
			+ (2 * sizeof(ADP_POP_CB)),
			(i4) 0,
			(i4) PCB_CB,
			(i4) PCB_ASCII_ID,
	...
	for (done = FALSE;!done;)
	{
		...
		{
		    get_pop->pop_error.err_code = E_AD0000_OK;
	           ...
	           || (put_pop->pop_error.err_code)))
	{
		...
		if (get_pop->pop_error.err_code != E_DM0065_USER_INTR)
		    ule_format(get_pop->pop_error.err_code, 0 , ULE_LOG,
			    NULL, (char *) 0, 0L, (i4 *) 0,
			    &err_code, 0);
		...
		if (put_pop->pop_error.err_code != E_DM0065_USER_INTR)
		    ule_format(put_pop->pop_error.err_code, 0 , ULE_LOG,
			    NULL, (char *) 0, 0L, (i4 *) 0,
			    &err_code, 0);
...

{
	...
	*/
				...
				old_cpn->cpn_log_key.tlk_low_id))
	{
	    ...
	    pop_cb->pop_error.err_code = E_DM0154_DUPLICATE_ROW_NOTUPD;
	    return(E_DB_OK);
	}
    }
	...
	*/
	...
	if (!old_null)
	{
	    ...
	    if (status)
	    {
		pop_cb->pop_error.err_code = put_pop->pop_error.err_code;
		break;
	    }
	}
	else
	...
	else
	{
	    if (get_pop->pop_error.err_code != E_AD7001_ADP_NONEXT)
	    {
		pop_cb->pop_error.err_code = get_pop->pop_error.err_code;
	    }
	    if (put_pop->pop_error.err_code)
	    {
		pop_cb->pop_error.err_code = put_pop->pop_error.err_code;
...

{
	...
	*error = pop_cb.pop_error.err_code;
	...
	pop_cb.pop_error.err_code = *error = dmu_cb.error.err_code;
    }
    ...
    *error = pop_cb.pop_error.err_code;
	...
	else
	{
	    ...
	    */
		...
		{
		    status = dm2t_close(rcb, DM2T_NOPURGE, &local_error);
		    rcb = 0;
		    status = E_DB_WARN;
               ...
               DMPE_PCB  **pcb_ptr )
{
	...
	*/
	...
	{
	    pop_cb->pop_error.err_code = E_DM9B08_DMPE_NEED_TRAN;
	    status = E_DB_ERROR;
	    break;
	}
	    ...
	    */
		...
		if (pcb->pcb_fblk.adf_r_dv.db_data)
		{
		    ...
		    */
			...
			status = dmxe_writebt(xcb, journal,
			    &pop_cb->pop_error.err_code);
			if (status != E_DB_OK)
			{
			    ...
			    E_DM9380_TAB_KEY_OVERFLOW)
			{
			    ...
			    */
			    ule_format(pop_cb->pop_error.err_code, 
				(CL_SYS_ERR *) 0, ULE_LOG, 
				NULL, (char *) 0, (i4) 0, (i4 *) 0, 
				&pop_cb->pop_error.err_code, 3, 
				sizeof(DB_TAB_NAME), 
		...
		ADP_POP_CB         *pop_cb)
{
	...
	pop_cb->pop_error.err_code = E_AD700B_ADP_BAD_POP_CB;
		...
		&& (op_code != ADP_FREE_LOCATOR))
	{
	    ...
	    */
	    pop_cb->pop_error.err_code = E_AD700D_ADP_NO_COUPON;
...

{
	    ...
	    &&	(pop_cb->pop_error.err_code != E_DM0055_NONEXT))
	{
	    ule_format(pop_cb->pop_error.err_code, 0 , ULE_LOG,
			NULL, (char *) 0, 0L, (i4 *) 0,
			&err_code, 0);
	    pop_cb->pop_error.err_code = E_DM9B06_DMPE_BAD_CAT_SCAN;
	}
	...
	u_i4 *new_etab)
{
	...
	*/
	if (att_id == 0)
	{
	    ...
	    pop_cb->pop_error.err_code = E_DM002A_BAD_PARAMETER;
	    status = E_DB_ERROR;
	    break;
	}
	...
	status = dmpe_create_extension(xcb, &t->tcb_rel,
		...
		&etab_record, &err_code);
	if (status != E_DB_OK)
	    pop_cb->pop_error.err_code = err_code;
	...
	*new_etab = etab_record.etab_extension;
	...
	if (pop_cb->pop_error.err_code != E_DM0065_USER_INTR)
	{
	    ule_format(pop_cb->pop_error.err_code, 0 , ULE_LOG,
			NULL, (char *) 0, 0L, (i4 *) 0,
			&err_code, 0);
	    pop_cb->pop_error.err_code = E_DM9B07_DMPE_ADD_EXTENSION;
	}
    }
    pcb->pcb_dmrcb->dmr_data.data_address = (char *) pcb->pcb_record;
    pcb->pcb_dmrcb->dmr_data.data_in_size = sizeof(DMPE_RECORD);
...
#define             DMPE_ATT_COUNT      5
    ...
    DMU_ATTR_ENTRY	*atts[DMPE_ATT_COUNT];
    DMU_ATTR_ENTRY	att_ents[DMPE_ATT_COUNT];
    DMU_KEY_ENTRY	key_ents[DMPE_KEY_COUNT];
    DMU_KEY_ENTRY	*keys[DMPE_KEY_COUNT];
	...
	*/
	...
	dmucb->dmu_attr_array.ptr_size = sizeof(DMU_ATTR_ENTRY);
	...
	*/
	if (etab_tbl_status & TCB_JON)
	{
	    ...
	    if (status)
	    {
		*err_code = jon_dmt_cb.error.err_code;
		break;
	    }
	}
	...
	if (local_status)
	{
	    ule_format(dmtcb.error.err_code, 0 , ULE_LOG,
			NULL, (char *) 0, 0L, (i4 *) 0, &local_error, 0);
	    status = local_status;
	}
    }
	...
	if (*err_code != E_DM0065_USER_INTR)
	{
	    ule_format(*err_code, 0 , ULE_LOG,
			NULL, (char *) 0, 0L, (i4 *) 0, &local_error, 0);
	    *err_code = E_DM9B07_DMPE_ADD_EXTENSION;
	}
    }
    if (misc_cb)
...
#define             DMPE_ATT_COUNT      5
	...
	*/
	...
	dmtcb->dmt_attr_array.ptr_size = sizeof(DMU_ATTR_ENTRY);
	...
	if (status)
	{
	    STRUCT_ASSIGN_MACRO(dmtcb->error, pop_cb->pop_error);
	    break;
	}
	...
	*new_temp_etab = dmtcb->dmt_id.db_tab_base;
	...
	if (pop_cb->pop_error.err_code != E_DM0065_USER_INTR)
	{
	    ule_format(pop_cb->pop_error.err_code, 0 , ULE_LOG,
			NULL, (char *) 0, 0L, (i4 *) 0,
			&err_code, 0);
	    pop_cb->pop_error.err_code = E_DM9B09_DMPE_TEMP;
	}
    }
    return(status);
...

{
	...
	if (open)
	{
		...
		ule_format(dmt_cb->error.err_code, 0 , ULE_LOG,
			NULL, (char *) 0, 0L, (i4 *) 0,
			&err_code, 0);
	    }
	}
...
*/
DB_STATUS
dmpe_free_temps(DML_ODCB *odcb, i4 *err_code)
{
    CS_SID		sid;
    ...
    */
	  ...
	  && (xccb->xccb_blob_temp_flags & BLOB_HAS_LOCATOR) == 0)
	{
	    ...
	    */
	    status = dmt_destroy_temp(parent_scb, xccb, &error);
	    if (status != E_DB_OK && error != E_DM0054_NONEXISTENT_TABLE)
	    {
		/* If it didn't work, log the problem */
		ule_format(error, NULL, ULE_LOG, NULL,
				NULL, 0, NULL, &local_error, 0);
		*err_code = E_DM9B09_DMPE_TEMP;
	    }
...
DB_STATUS	*error)
{
	...
	rcb->rcb_tupbuf = dm0m_tballoc((tcb->tcb_rel.relwid +
					     tcb->tcb_comp_atts_count));
    if (rcb->rcb_tupbuf == NULL)
    {
	*error = E_DM923D_DM0M_NOMORE;
	return (E_DB_ERROR);
    }
		...
		i4       *err_code)
{
	...
	*etab_extension = etab_record.etab_extension;
    ...
    {
	*err_code = E_DM9B06_DMPE_BAD_CAT_SCAN;
	ule_format( *err_code,
		(CL_ERR_DESC *)0, ULE_LOG,
		NULL, (char *) 0, 0L, (i4 *) 0,
		err_code, 0);
	...
	if (!r->rcb_bulk_misc)
	{
	    ...
	    */
	    status = dm0m_allocate( size, (DM0M_ZERO | DM0M_LONGTERM), 
			    ...
			    (DM_OBJECT **) &r->rcb_bulk_misc, &error);
	    if ( status != E_DB_OK )
	    {
		ule_format(error, 0, ULE_LOG , NULL, (char * )0, 0L,
			(i4 *)0, &error, 0);
		dmr_cb->error.err_code = E_DM9B02_PUT_DMPE_ERROR;
		break;
	    }
	...
	*/
	...
	if (status == E_DB_INFO)
	{
	    ...
	    */
		...
		&record_size, (PTR *)&first_rec, NULL, NULL, (DMPP_SEG_HDR *)0);
	    if (status != E_DB_OK)
	    {
		dmr_cb->error.err_code = E_DMF011_DM1R_BAD_TID; 
		break;
	    }
	    ...
	    */
				...
				fix_tid.tid_tid.tid_line++)
		{
			...
			&record_size, (PTR *)&fix_rec, NULL, NULL, (DMPP_SEG_HDR *)0);
			...
			&r->rcb_tran_id, &fix_tid, sizeof(DMPE_RECORD),
			(char *)fix_rec, r->rcb_compress, 
		...
		status = (*t->tcb_acc_plv->dmpp_get)(t->tcb_rel.relpgtype,
		    t->tcb_rel.relpgsize, page, &first_tid, 
		    &record_size, (PTR *)&first_rec, NULL, NULL,
		    (DMPP_SEG_HDR *)0);
	    }
	...
	(*t->tcb_acc_plv->dmpp_put)(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,	
		page, DM1C_DIRECT, &r->rcb_tran_id, &put_tid,
	...
	if (last_seg)
	{
	    ...
	    */
				...
				fix_tid.tid_tid.tid_line++)
		{
		    status = (*t->tcb_acc_plv->dmpp_get)(t->tcb_rel.relpgtype,
			t->tcb_rel.relpgsize, page, &fix_tid, 
			&record_size, (PTR *)&fix_rec, NULL, NULL,
			(DMPP_SEG_HDR *)0);
		...
		*/
		(*t->tcb_acc_plv->dmpp_delete)(t->tcb_rel.relpgtype, 
		    ...
		    &r->rcb_tran_id, &put_tid, sizeof(DMPE_RECORD));
		r->rcb_bulk_cnt--;
		break;
	    }
	}
    ...
    if (status != E_DB_OK)
    {
	i4		err = dmr_cb->error.err_code;

	ule_format(err, 0, ULE_LOG , NULL, (char * )0, 0L,
		(i4 *)0, &err, 0);
    }
    return (status);
}
...
i4		*err_code)
{
    ...
    status = dmpe_allocate(&pop_cb, &pcb);
    *err_code = pop_cb.pop_error.err_code;
    if (status)
	return(status);
	...
	if (status == E_DB_OK)
	{
	    ...
	    pop_cb.pop_error.err_code = *err_code = dmt_cb.error.err_code;
	}
    }
    if (status == E_DB_WARN && pop_cb.pop_error.err_code == E_DM0055_NONEXT)
    {
...
dmpe_check_table_lock(i4 opcode, DMT_CB *dmtcb, DMP_TCB *base_tcb)
{
    ...
    */
    ...
    /* Check RCB's on the base table, if any are using
    ** table level locking, do likewise on the etab
    ** FIXME probably should be a bit more clever here
    */
    dm0s_mlock(&base_tcb->tcb_mutex);
    base_rcb = base_tcb->tcb_rq_next;
    while (base_rcb != (DMP_RCB *)&base_tcb->tcb_rq_next)
    {
	if (base_rcb->rcb_lk_type == RCB_K_TABLE)
	{
	    if (opcode == ADP_GET)
		dmtcb->dmt_lock_mode = DMT_S;
	    else
	...
	base_rcb = base_rcb->rcb_q_next;
    }
    dm0s_munlock(&base_tcb->tcb_mutex);
...
dmpe_cpn_to_locator(ADP_POP_CB	*pop_cb)
{
    ...
    */
	...
	if (!found)
	{
	    pop_cb->pop_error.err_code = E_AD700E_ADP_BAD_COUPON;
	    return (E_DB_ERROR);
	}
    }
	...
	status = dm0m_allocate(sizeof(DMPE_LLOC_CXT) +
	    ...
	    (DM_OBJECT **)&lloc_cxt, &pop_cb->pop_error.err_code);
	if (status != E_DB_OK)
	{
	    return (E_DB_ERROR);
	}
	...
	loc_p->per_length0 = cpn_p->per_length0;
	loc_p->per_length1 = cpn_p->per_length1;
	loc_p->per_value.val_locator = locator;
	if (xccb)
	{
	...
	status = dmpe_create_locator_temp(scb, &err_code);
	if (status)
	{
	    pop_cb->pop_error.err_code = err_code;
	    return (status);
    ...
    loc_stat = dm2r_unfix_pages(loc_rcb, &loc_err);
...
#define             DMPE_LOC_ATTS      1
    ...
    DMU_ATTR_ENTRY	*atts[DMPE_LOC_ATTS];
    DMU_ATTR_ENTRY	att_ents[DMPE_LOC_ATTS];
    DB_TAB_TIMESTAMP	timestamp;
    DMP_RCB		*loc_rcb;
	...
	MEmove(25, (PTR) "<LOCATOR_TEMPORARY_TABLE>", ' ',
		sizeof(dmtcb.dmt_table), (PTR) dmtcb.dmt_table.db_tab_name);
...
dmpe_locator_to_cpn(ADP_POP_CB	*pop_cb)
{
    ...
    if (!lloc_cxt)
    {
	pop_cb->pop_error.err_code = E_DM9B08_DMPE_NEED_TRAN; /* FIX ME */
	return (E_DB_WARN);
    }

    /* Extract locator from pop */
    locator = loc_p->per_value.val_locator;
    ...
    */
    ...
    if (status)
    {
	pop_cb->pop_error.err_code = dmrcb.error.err_code;
...
dmpe_free_locator(ADP_POP_CB *pop_cb)
{
    ...
    */
    ...
    if (loc_dv)
	locator = ((ADP_PERIPHERAL *)loc_dv->db_data)->per_value.val_locator;
    else
	locator = 0;
...
#ifdef xDEBUG
    if ( !(input_scb = GET_DML_SCB(session_id)) ||
	  dm0m_check((DM_OBJECT *)input_scb, (i4)SCB_CB) ||
		input_scb != thread_scb)
    {
        TRdisplay("dmpe_free_locator input_scb %x thread_scb %x\n",
	input_scb, thread_scb);
    }
#endif
    ...
    */
	  ...
	  (locator == 0 && (xccb->xccb_blob_temp_flags & BLOB_LOCATOR_TBL)))
	{
	    ...
	    ** a valid transaction which we may not have.
	    */
	    status = dmt_destroy_temp(thread_scb, xccb, &error);
	    if (status != E_DB_OK && error != E_DM0054_NONEXISTENT_TABLE)
	    {
		/* If it didn't work, log the problem */
		ule_format(error, NULL, ULE_LOG, NULL,
