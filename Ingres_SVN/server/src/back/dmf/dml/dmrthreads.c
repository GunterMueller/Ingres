...
DMR_CB		*dmr)
{
    ...
    DMR_RTB	*rtb;
    DB_STATUS	status;
    i4		*err_code = &dmr->error.err_code;
	...
	*/
	if ( dmrThrAttach(r, (DMP_TCB*)NULL, TRUE, err_code) )
	{
    ...
    */
	...
	case DMR_REPLACE:
	    ...
	    */
	    if ( !rtb && dmrThrAttach(r, (DMP_TCB*)NULL, FALSE, err_code) )
		break;
		...
		*/
		if ( status = dmrThrAttach(r, it, TRUE, err_code) )
		    break;
	    }
...
**	and error code (rtb_err_code) to the caller.
**
** Inputs:
...
**	err_code	and the corresponding error code.
**
...
SCF_FTX	*ftx)
{
	...
	if ( rtb->rtb_status == E_DB_OK )
	{
	    ...
	    rtb->rtb_err_code = E_DM004A_INTERNAL_ERROR;
	}
	/*
	...
	*/
	     ...
	     seq != rtb->rtb_dmr_seq )
	{
		...
		** bad status */
		...
		dmr->error.err_code = rtb->rtb_err_code;;
	...
	else if ( !r && rtl.rtl_rcb )
	{
		...
		if ( lg_status == LG_EXCEED_LIMIT )
		    rtb->rtb_err_code = E_DM0062_TRAN_QUOTA_EXCEEDED;
		else if ( lk_status == LK_NOLOCKS )
		    rtb->rtb_err_code = E_DM004B_LOCK_QUOTA_EXCEEDED;
		else
		    rtb->rtb_err_code = E_DM9500_DMXE_BEGIN;
	    }
