		...
		else if ( xccb->xccb_is_session && dmt->dmt_id.db_tab_index )
		{
		    ...
		    */
		    status = dm2t_destroy_temp_tcb(xcb->xcb_scb_ptr->scb_lock_list,
				xcb->xcb_log_id, xccb->xccb_t_dcb, 
				&dmt->dmt_id, 0, &error);
		}
		else
		{
		    ...
		    */
		    status = dmt_destroy_temp(xcb->xcb_scb_ptr,xccb, &error);
		    if (status == E_DB_OK)
		    {
...
**	to be in a transaction.  Luckily the log_id that's passed to
**	various lower levels is bogus and unused for temp tables.
**	(And dm2t_destroy_temp_tcb really shouldn't be asking for it.)
**
...
DB_STATUS
dmt_destroy_temp(DML_SCB *scb, DML_XCCB *xccb, i4 *error)
{
	...
	*/
    ...
    status = dm2t_destroy_temp_tcb(lockid, 0 /* irrelevant */,
