...
dmc_repq(DMC_CB		*dmc_cb)
{
    ...
    */
    ...
    cl_stat = LKevent(LK_E_SET | LK_E_CROSS_PROCESS | LK_E_INTERRUPT, 
        scb->scb_lock_list, &lk_event, &sys_err); 
    if (cl_stat == LK_INTERRUPT) /* terminate thread */
   	    return(status);
    else if (cl_stat != OK)
    {
        ule_format(cl_stat, &sys_err, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &toss_err, 0);
        ule_format(E_DM904B_BAD_LOCK_EVENT, &sys_err, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &toss_err, 3,
	    0, LK_E_SET, 0, REP_READQ, 0, scb->scb_lock_list);
        status = E_DB_ERROR;
        return(status);
    }
	...
	*/
	...
	for (i = Dmc_rep->queue_start, last = FALSE;;i++)
	{
	    ...
	    */
		...
		dbname[DB_MAXNAME] = 0;
		err_code = E_DM9564_REP_SCF_DBUSERADD;
		ule_format(err_code, (CL_SYS_ERR *)0, ULE_LOG,
		    NULL, (char *)0, (i4)0, (i4 *)0,
		    &toss_err, (i4)2, STtrmwhite(dbname), dbname,
		...
		*/
		...
		if (status == E_DB_WARN) /* last user in db */
		    status = dm2d_del_db(next_dcb, &err_code);
		if (status != E_DB_OK)
		    break;
	    ...
	    */
	    ...
	    if (status != E_DB_OK)
	    {
		err_code = E_DM9568_REP_DBEVENT_ERROR;
		ule_format(err_code, (CL_SYS_ERR *)0, ULE_LOG,
		    NULL, (char *)0, (i4)0, (i4 *)0,
		    &toss_err, (i4)0);
		break;
	    ...
	    */
		...
		/*
		** signal threads waiting on queue entries
		*/
		lk_event.type_high = REP_FULLQ;
		lk_event.value = REP_FULLQ_VAL;
		...
		if (cl_stat != OK)
		{
			...
			NULL, (char *)0, (i4)0, (i4 *)0, &toss_err,
	...
	*/
	...
	else if (cl_stat != OK)
	{
	    ...
	    ule_format(E_DM904B_BAD_LOCK_EVENT, &sys_err, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &toss_err, 3,
		0, LK_E_WAIT, 0, REP_READQ, 0, scb->scb_lock_list);
	    status = E_DB_ERROR;
	    break;
	}
	cl_stat = LKevent(LK_E_SET | LK_E_CROSS_PROCESS | LK_E_INTERRUPT, 
