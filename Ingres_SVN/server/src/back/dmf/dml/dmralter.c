...
DMR_CB  *dmr_cb)
{
	     ...
	     entry_number++)
	{
		...
		*/
		...
		break;
	      ...
	      {
		/*
		** Alter lock strategy of open table.
		*/
		DMT_CB		dmt_cb;
		    ...
		    DMC_C_SYSTEM, DMC_C_SYSTEM);
                ...
                if (lock_mode == DM2T_N)
                {
                    if (!(dcb->dcb_status & DCB_S_PRODUCTION))
                    {
                        status = dm2t_control(dcb, table_id->db_tab_base, 
                            lock_id, LK_S, (i4)0, timeout, &error);
                        if (status != E_DB_OK)
                            break;
                    }
                }
                else
                {
                    status = dm2t_lock_table(dcb, table_id, lock_mode, lock_id,
                        timeout, &grant_mode, &lockid, &error);
		    if (status != E_DB_OK)
		        break;
		}
		...
		** into rcb_repos*
		*/
		rcb->rcb_repos_clean_count = DM1B_CC_REPOSITION;
		rcb->rcb_repos_bid.tid_i4 = rcb->rcb_lowtid.tid_i4;
		...
		*/
		...
		rcb->rcb_fet_clean_count = DM1B_CC_REPOSITION;
		/*
		...
		*/
		if ((tcb->tcb_rel.relstat & TCB_INDEX) == 0)
		    rcb->rcb_repos_tid.tid_i4 = rcb->rcb_currenttid.tid_i4;
	...
	ule_format( error, 0, ULE_LOG , NULL, (char * )0, 0L, (i4 *)0, 
	    &error, 0);
	error = E_DM0139_ERROR_ALTERING_RCB;
    }

    dmr->error.err_code = error;
    dmr->error.err_data = entry_number;
    return (status);
}
