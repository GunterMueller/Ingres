...
static VOID PutTxnCvalue( DML_XCB	  *xcb,
			  DMS_SEQ_ENTRY*dms_seq,
			  DML_SEQ	  *s,
			  PTR	  	  value,
			  i4		  *error);

static VOID GetTxnCvalue( DML_XCB	  *xcb,
			  DMS_SEQ_ENTRY*dms_seq,
			  DML_SEQ	  *s,
			  PTR	  	  value,
			  i4		  *error);
	...
	*/
	...
	for ( dms_seq; count; dms_seq++, count-- )
	{
		...
		else if ( cl_status == LK_TIMEOUT )
		{
		    ...
		    error = E_DM004D_LOCK_TIMER_EXPIRED;
		}
		else if ( cl_status == LK_UBUSY )
		{
		    ...
		    ** is on in this SCB */
			...
			{
			    ule_format(E_DM9D0C_SEQUENCE_LOCK_BUSY, 
		...
		else
		{
		    ...
		    ule_format(lk_error, &sys_err, ULE_LOG , NULL,
			(char * )0, 0L, (i4 *)0,
			&local_error, 2, 0, lock_mode, 
			0, xcb->xcb_lk_id);
		    error = E_DM9D07_SEQUENCE_LOCK_ERROR;
		}
		ule_format(lk_error, &sys_err, ULE_LOG , NULL,
			(char * )0, 0L, (i4 *)0,
			&local_error, 3, 
	...
	if ( !lk_error )
	{
	    ule_format(error, 0 ,ULE_LOG, NULL, 0, 0, 0, &local_error, 0);
	    error = E_DM9D05_SEQUENCE_OPEN_FAILURE;
	    if ( dms_seq )
...
PTR        dms_cb)
{
	...
	**  that can be updated. */
    ...
    for ( dms_seq; error == 0 && count; dms_seq++, count-- )
    {
	...
	error = E_DM9D09_SEQUENCE_CLOSE_FAILURE;
	if ( dms_seq )
	    ule_format( error, &sys_err, ULE_LOG , NULL,
		    (char * )0, 0L, (i4 *)0,
		    &local_error, 2, 
...
PTR        dms_cb)
{
	...
	*/
	      ...
	      (value = dms_seq->seq_value.data_address) )
	{
		...
		/* CURRENT VALUE: */
		GetTxnCvalue(xcb, dms_seq, s, value, &error);
	    else
	    {
		/* NEXT VALUE: */
		...
		*/
		...
		else if ( error == 0 )
		{
			...
			if ( s->seq_next.intval > s->seq_max.intval ||
			     ...
			     s->seq_cache < 0 ) /* b112605 */
			{
			    ...
			    {
				/* Boundary breach */
				error = E_DM9D01_SEQUENCE_EXCEEDED;
				break;
			    }
		...
		if ( error == E_DM9D00_IISEQUENCE_NOT_FOUND )
		{
		...
		if ( error == 0 )
		    PutTxnCvalue(xcb, dms_seq, s, value, &error);
	    }
	}
	else
	    error = E_DM002A_BAD_PARAMETER;
    ...
    if ( dms->error.err_code = error )
    {
	/* These are "expected" and handled by QEF */
	if ( error != E_DM9D01_SEQUENCE_EXCEEDED
	  && error != E_DM9D00_IISEQUENCE_NOT_FOUND )
	{
	    ule_format(error, 0 ,ULE_LOG, NULL, 0, 0, 0, &local_error, 0);
	    error = E_DM9D0A_SEQUENCE_ERROR;
	}
	...
	if ( txn_state == DMS_TRAN_COMMIT )
	{
	    ...
	    */
	    if ( s->seq_flags & SEQ_UPDATED && s->seq_id &&
		 s->seq_cache > 0 &&	 /* b112605 */
		(status = dms_update_iiseq(xcb, s, err_code)) )
	    {
		break;
	    }
	}
...
i4		*error)
{
	...
	*error = E_DM9D02_SEQUENCE_NOT_FOUND;
