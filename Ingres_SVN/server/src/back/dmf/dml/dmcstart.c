...
DB_VPT_SIZEOF_TUPLE_HDR(TCB_PG_V4), DMPP_VPT_SIZEOF_TUPLE_HDR_MACRO(TCB_PG_V4));
	...
	DB_VPT_PAGE_OVERHEAD_MACRO(TCB_PG_V4) != DMPPN_VPT_OVERHEAD_MACRO(TCB_PG_V4))
    {
	dmc->error.err_code = E_DM007F_ERROR_STARTING_SERVER;
	return(E_DB_ERROR);
    }
	...
	err_code = E_DM006F_SERVER_ACTIVE;
	if (dmf_svcb)
	    break;
	err_code = E_DM000C_BAD_CB_TYPE;
	if (dmc->dmc_op_type != DMC_SERVER_OP)
	    break;
	err_code = E_DM001A_BAD_FLAG;
	...
	err_code = E_DM002E_BAD_SERVER_NAME;
...
# endif
		...
		case DMC_C_DEF_PAGE_SIZE:
		    ...
		    {
                	ule_format(E_DM0184_PAGE_SIZE_NOT_CONF, 
				(CL_ERR_DESC *)0, ULE_LOG , NULL, (char * )0, 
				0L, (i4 *)0, &err_code, 0);
	...
	if (star_server == 0)
	{
	    ...
	    */
	    ...
	    if ( status != E_DB_OK )
	    {
		ule_format(E_DM91A7_BAD_CATALOG_TEMPLATES, (CL_ERR_DESC *)0, 
	...
	svcb->svcb_lock_list = NULL;
	svcb->svcb_ss_lock_list = NULL;
	...
	if (config_v1)
	    svcb->svcb_config_pgtypes |= SVCB_CONFIG_V1;
	if (config_v2)
	    svcb->svcb_config_pgtypes |= SVCB_CONFIG_V2;
	if (config_v3)
	    svcb->svcb_config_pgtypes |= SVCB_CONFIG_V3;
	if (config_v4)
	    svcb->svcb_config_pgtypes |= SVCB_CONFIG_V4;
	if (config_v5)
	    svcb->svcb_config_pgtypes |= SVCB_CONFIG_V5;
	...
	if (stat != E_DB_OK)
	{
	    ule_format(stat, 0, ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	...
	*/
	...
	if ((dmc->dmc_flags_mask & DMC_RECOVERY) == 0)
	{
		...
		ule_format(E_DM901E_BAD_LOCK_INIT, (CL_ERR_DESC *)0,
			ULE_LOG, NULL,
			(char * )0, 0L, (i4 *)0, &err_code, 0);
		err_code = E_DM007F_ERROR_STARTING_SERVER;
		break;
	    ...
	    */
		...
		ule_format(status, &sys_err, ULE_LOG, NULL,
			(char * )0, 0L, (i4 *)0, &err_code, 0);
		ule_format(E_DM9011_BAD_LOG_INIT, (CL_ERR_DESC *)0,
			ULE_LOG, NULL,
			(char * )0, 0L, (i4 *)0, &err_code, 0);
		err_code = E_DM007F_ERROR_STARTING_SERVER;
		break;
		...
		ule_format(status, &sys_err, ULE_LOG, NULL,
			(char * )0, 0L, (i4 *)0, &err_code, 0);
		ule_format(E_DM900B_BAD_LOG_ALTER, (CL_ERR_DESC *)0,
			ULE_LOG, NULL,
	    ...
	    if ((lgd_status & 1) == 0)
	    {
		ule_format(E_DM9C0A_RSRV_NOT_ONLINE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL,
			(char * )0, 0L, (i4 *)0, &err_code, 0);
		status = E_DB_ERROR;
		err_code = E_DM007F_ERROR_STARTING_SERVER;
...
#           define DAWDLE_TIME        5 /* Seconds to wait for lock */
	    ...
	    MEfill(sizeof(LK_LKID), 0, &lockid);
			...
			*/
			ule_format(E_DM006B_SERVER_CLASS_ONCE, &sys_err,
			    ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0,
		    ...
		    */
			...
			if ( lock_value.lk_low != local_node_number )
			    ...
			    ule_format(E_DM006C_SERVER_CLASS_ELSEWHERE,
				&sys_err, ULE_LOG, NULL, (char *)0,
				(i4)0, (i4 *)0, &err_code, 1,
				0, svr_class_name);
	...
	*/
	...
	if ((dmc->dmc_flags_mask & DMC_RECOVERY) != 0)
	{
	    status = dmf_udt_lkinit(svcb->svcb_lock_list, (i4)TRUE, &err_code);
	    if (status)
		break;
	}
	...
	if ((major_id_index != -1) || (minor_id_index != -1))
	{
        ...
        ** running DMCM.
        */
        status = dmcm_lkinit(svcb->svcb_lock_list, c_dmcm,
	...
	/*
	** Setup load optimization config.
	*/
	status = dmf_setup_optim();
	if (status != E_DB_OK)
	{
	    err_code = E_DM007F_ERROR_STARTING_SERVER;
	    break;
	...
	*/
	{
		...
		*/
		...
		c_gbuffers, c_gcount, c_writebehind, c_wbstart, c_wbend, c_cache_flags,
                c_dbcache_size, c_tblcache_size, c_cache_name, &err_code); 
	    if (status != E_DB_OK)
		break;
	...
	*/
	...
	status = dm0m_allocate(sizeof(DMP_HCB) + 
	    ...
	    (DM_OBJECT **)&svcb->svcb_hcb_ptr, &err_code);
	if (status != E_DB_OK)
	{
	    break;
	}
	...
	*/
	...
	if (status != OK)
	{
	    ule_format(scf_cb.scf_error.err_code,
	...
	*/
	...
	if (c_numrahead)
	{
	    ...
	    (DM_OBJECT **)&svcb->svcb_prefetch_hdr, &err_code);
	    if (status != E_DB_OK)
	    	break;
	...
	*/
	status = dmf_gwf_init(&gateway_present, &gw_xacts, &err_code);
	if (status != E_DB_OK)
	    break;
	...
	if (gc_numticks_changed)
	{
	    ...
	    if (stat != OK)
	    {
		ule_format(stat, &sys_err, ULE_LOG, NULL, (char *)0,
			    (i4)0, (i4 *)0, &err_code, 0);
		ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err,
			    ULE_LOG, NULL, (char *)0,
			    (i4)0, (i4 *)0, &err_code,
    ...
    if (bufmgr_allocated && dm0p_deallocate(&local_err_code))
	ule_format(local_err_code, 0, ULE_LOG, NULL, 0, 0, 0,
		    &local_err_code, 0);
    if (log_allocated && dm0l_deallocate(&svcb->svcb_lctx_ptr, &local_err_code))
	ule_format(local_err_code, 0, ULE_LOG, NULL, 0, 0, 0,
		    &local_err_code, 0);
    dmc->error.err_code = err_code;
    ...
    */
	    ...
	    (CMcmpnocase(c3, "F") == 0))
	{
	    ...
	    */
	    ...
	    ule_format(E_DM9056_CHECKSUM_DISABLE, (CL_ERR_DESC *)0, 
		 ...
		 (CMcmpnocase(c2, "N") != 0))
	{
	    ...
	    */
	    ule_format(E_DM9057_PM_CHKSM_INVALID, (CL_ERR_DESC *)0, 
		ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, &err_code, 
		1, STlength(SystemCfgPrefix), SystemCfgPrefix);
	}
    }
...
#define NON_DMCM_LOCK_VALUE  1
    ...
    if ((status != OK) && (status != LK_VAL_NOTVALID))
    {
        ...
        ule_format(E_DM901C_BAD_LOCK_REQUEST, &sys_err, ULE_LOG, NULL,
            (char *)0, (i4)0, (i4 *)0, err_code, 2,
	    0, mode, 0, lock_list);
	*err_code = E_DM0172_DMCM_LOCK_FAIL;
        return(E_DB_ERROR);
    }
    else
    {
	...
	if ( !create || status == E_DB_ERROR )
	{
           ...
           if (lstatus != OK)
           {
               ule_format(lstatus, &sys_err, ULE_LOG, NULL,
                   (char * )0, (i4)0, (i4 *)0, err_code, 0);
		   ...
		   *err_code = E_DM0172_DMCM_LOCK_FAIL;
           ...
           if (dmcm_fail)
           {
              *err_code = E_DM0171_BAD_CACHE_PROTOCOL;
           }
       }
    } 
    return(status);
