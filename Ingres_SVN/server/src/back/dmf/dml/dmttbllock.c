...
DMT_CB   *dmt_cb)
{
    ...
    else
    {
       ...
       for(dmt_cb->dmt_record_count = 0; (status == E_DB_OK) && !dmt_cb->dmt_record_count;)
       {
          ...
          for (i = 0; i < (i4) (length / sizeof(LK_LKB_INFO)); i++)
          {
             ...
             if ( (lkb->lkb_key[0] == LK_TABLE) &&
                  ...
                  (lkb->lkb_key[2] == (i4) dmt_cb->dmt_id.db_tab_base) &&
                  (lkb->lkb_key[3] == (i4) dmt_cb->dmt_id.db_tab_index) )
             {
                /* This is the LKB for the specified table */

                dmt_cb->dmt_record_count = 1;
                dmt_cb->dmt_lock_mode = lkb->lkb_grant_mode;
...
DMT_CB   *dmt_cb)
{
   ...
   { 
      ...
      if (status != OK)
      {
         ule_format(status, &sys_err, ULE_LOG, NULL,
                    (char *)0, (i4)0, (i4 *)0, &ule_error, 0);
  

         error = E_DM901B_BAD_LOCK_RELEASE;
         ret = E_DB_ERROR;
      } 
   }
   ...
   if (ret != E_DB_OK)
   {
      if (error > E_DM_INTERNAL)
      {
         ule_format(dmt->error.err_code, 0, ULE_LOG, NULL,
