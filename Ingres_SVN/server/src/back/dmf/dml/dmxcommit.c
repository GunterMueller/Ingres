...
DMX_CB    *dmx_cb)
{
	...
	** been done by dmx_secure().
	    ...
	    */
		...
		if (Dmc_rep)
		{
		    local_error = W_DM9561_REP_TXQ_FULL;
		...
		status = dm2rep_qman(xcb->xcb_odcb_ptr->odcb_dcb_ptr,
		    ...
		    &curtime, xcb->xcb_rep_input_q, &error, FALSE);
		if (status != E_DB_OK)
		{
		    if (error > E_DM004A_INTERNAL_ERROR)
		    {
		        ule_format( error, 0, ULE_LOG , NULL, 
				    (char * )0, 0L, (i4 *)0, 
				    &local_error, 0);
		        error = E_DM957F_REP_DISTRIB_ERROR;
		    }
		...
		lk_event.value = REP_READQ_VAL;
		    ...
		    &lk_event, &sys_err);
                if (cl_stat != OK)
                {
                    ule_format(cl_stat, &sys_err, ULE_LOG, NULL,
	    ...
	    */
	    ...
	    {
                status = dm2t_close(xcb->xcb_rep_input_q, (i4)0, &error);
                if (status != E_DB_OK)
                    break;
	    }
        }
	...
	if ( (xcb->xcb_seq || xcb->xcb_cseq) &&
	    (status = dms_end_tran(DMS_TRAN_COMMIT, xcb, &error)) )
	{
	    break;
	}
	...
	*/
	...
	while (xcb->xcb_rq_next != (DMP_RCB*) &xcb->xcb_rq_next)
	{
	    ...
	    status = dm2t_close(rcb, (i4)0, &error);
	    if (status != E_DB_OK)
	    {
		if (error != E_DM004A_INTERNAL_ERROR)
		{
		    ule_format( error, 0, ULE_LOG , NULL, 
				    (char * )0, 0L, (i4 *)0, 
				    &local_error, 0);
		    error = E_DM0095_ERROR_COMMITING_TRAN;
		}
	    
		dmx->error.err_code = error;
		xcb->xcb_state |= XCB_TRANABORT;
		return (status);
	    }
	}
	...
	*/
	...
	{
	    status = dmf_gwx_commit(scb, &error);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	if ( dmx->dmx_option & DMX_XA_END_SUCCESS )
	{
		...
		if (xccb->xccb_operation == XCCB_F_DELETE)
		{
			...
			ule_format( cl_stat, 0, ULE_LOG , NULL, 
				(char * )0, 0L, (i4 *)0, &error, 0);
			ule_format( E_DM0095_ERROR_COMMITING_TRAN,
				0, ULE_LOG , NULL, 
				(char * )0, 0L, (i4 *)0, &error, 0);
		    }
	...
	else
	{
	    ...
	    status = dmxe_commit(&xcb->xcb_tran_id, xcb->xcb_log_id, 
			 xcb->xcb_lk_id, commit_flags, &ctime, &error);
	...
	*/
	...
	while (xcb->xcb_cq_next != (DML_XCCB*) &xcb->xcb_cq_next)
	{
		...
		** xccb. Set to zero. */
		...
		if (local_status)
		{
		    ule_format( local_error, 0, ULE_LOG , NULL, 
		...
		*/
	    ...
	    if (status != E_DB_OK)
	    {
                ...
                ule_format(E_DM901B_BAD_LOCK_RELEASE, &sys_err, ULE_LOG, NULL,
                        (char *)0, (i4)0, (i4 *)0, &error, 3,
                        0, LK_E_CLR, 0, REP_READQ, 0, xcb->xcb_lk_id);
	        dmx->error.err_code = E_DM9501_DMXE_COMMIT;
                xcb->xcb_state |= XCB_TRANABORT;
	        return (E_DB_ERROR);
	    }
	}
...
i4	   *error)
{
	...
	ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, error, 1, 0, LG_S_STRANSACTION);
	return (E_DB_ERROR);
    }
	...
	*/
	...
	{
	    ule_format(E_DM00AC_LOGFULL_COMMIT, 0, ULE_LOG , (DB_SQLSTATE*)NULL, 
		(char * )0, 0L, (i4 *)0, 
		&local_error, 6,
