...
DMC_CB    *dmc_cb)
{
	...
	*/
	     ...
	     scb->scb_lock_list )
	{
	    ...
	    cl_status = LKrelease(LK_ALL, scb->scb_lock_list, (LK_LKID *)0,
		(LK_LOCK_KEY *)0, (LK_VALUE *)0, &clerror);
	    if (cl_status != OK)
	    {
		ule_format(cl_status, (CL_SYS_ERR *)&clerror, 
		    ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
		    &local_error, 0);
		ule_format( E_DM901B_BAD_LOCK_RELEASE, &clerror, ULE_LOG, NULL,
	            (char *)0, (i4)0, (i4 *)0, &local_error, 1,
		    0, scb->scb_lock_list);
		error = E_DM0106_ERROR_ENDING_SESSION;
		break;
	    }
	}
	/* Clean up mutexes.
...
i4		*err_code;
{
	...
	while (xcb->xcb_rq_next != (DMP_RCB*) &xcb->xcb_rq_next)
	{
		    ...
		    (error != E_DM004A_INTERNAL_ERROR))
		{
			...
			(char * )0, 0L, (i4 *)0, 
			&local_error, 0);
		    *err_code = E_DM0106_ERROR_ENDING_SESSION;
		}
		else
		    *err_code = error;
		return (E_DB_FATAL);
		...
		*/
		...
		status = dm2t_destroy_temp_tcb(scb->scb_lock_list, 
			    xcb->xcb_log_id, dcb, &tbl_id, 0, &error);
		if (status != E_DB_OK &&
	...
	*/
	...
	status = dmxe_pass_abort(xcb->xcb_log_id, xcb->xcb_lk_id, 
		    &xcb->xcb_tran_id, odcb->odcb_dcb_ptr->dcb_id, err_code);
	if (status)
	{
