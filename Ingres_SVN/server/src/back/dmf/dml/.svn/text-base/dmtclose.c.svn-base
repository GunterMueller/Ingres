...
**          .dmt_flags_mask                 Must be zero or DMT_CLOSE_FORCE.
...
DMT_CB  *dmt_cb)
{
    ...
    status = E_DB_ERROR;
    if ((dmt->dmt_flags_mask & ~(DMT_CONSTRAINT | DMT_SESSION_TEMP)) == 0)
    {
	...
	if (dm0m_check((DM_OBJECT *)rcb, (i4)RCB_CB) == E_DB_OK)
	{
		...
		*/
		if (rcb->rcb_tcb_ptr->tcb_rel.relstat2 & TCB2_HAS_EXTENSIONS)
		{
			...
			*/
			if ((next_rcb->rcb_tcb_ptr->tcb_rel.relstat2 &
			    TCB2_EXTENSION)
			      ...
			      rcb->rcb_tcb_ptr->tcb_rel.reltid.db_tab_base)
			{
			    ...
			    status = dm2t_close(next_rcb, (i4)0, &error);
			    if (status != E_DB_OK)
				internal_error = 1;
			}
			if (internal_error)
	...
	ule_format(error, 0, ULE_LOG, NULL, (char * )0, 0L, (i4 *)0, 
	    &local_error, 0);
	error = E_DM0090_ERROR_CLOSING_TABLE;
    }
