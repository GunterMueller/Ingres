...
DMC_CB	    *dmc_cb)
{
    ...
    i4	    error_code = E_DM0164_READ_AHEAD_FAILURE;
    struct  _racontext	racontext;
    STATUS	    stat;
    i4	    error;
    CL_SYS_ERR	    sys_err;
    ...
    ** the LG_NOTDB flag.
    */
    MEcopy((PTR)DB_BREADAHEAD_NAME, sizeof(add_info.ad_dbname),
    ...
    if (stat != OK)
    {
	ule_format(stat, (CL_SYS_ERR *)&sys_err, 
	    ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, &error, 0);
	...
	if (stat == LG_EXCEED_LIMIT)
	    ...
	    dmc_cb->error.err_code = E_DM0164_READ_AHEAD_FAILURE;
	return (E_DB_ERROR);
    }
	...
	** require backout.
	*/
	MEcopy((PTR)DB_BREADAHEAD_NAME, sizeof(DB_OWN_NAME), 
							(PTR) &user_name);
	...
	for (;;)
	{
		...
		*/
		...
		else
		{
		    ...
		    {
			ule_format(error, 0, ULE_LOG, NULL, (char *)0, 
			    (i4)0, (i4 *)0, &error, 0);
			error_code = E_DM0164_READ_AHEAD_FAILURE;
		    }
		    break;
		};   
	    }
		...
		if ((pf->pfr_status & (PFR_WAITED_ON | PFR_STALE_REQ)) == 0)
		{
		    ...
		    dm0s_munlock(&dm_prefet->pfh_mutex);
			    ...
			    /* read tuples sec indx entries */
		    	    status = prefet_secidx(&racontext, pf, 
				curr_ix, &error_code);  
		    	    break;
			...
			case PFR_PREALLOC_SECIDX:
			    ...
			    */
			    status = prealloc_secidx(&racontext, pf,
				curr_ix, &error_code);
			    break;

			case PFR_PREFET_PAGE:
			    /* read in the requested page */
			    status = prefet_page(&racontext, pf, &error_code);
			    break;
    ...
    */
	...
	if (stat != OK)
	{
		...
		(char *)0, (i4)0, (i4 *)0, &error, 1, 0, lock_list);
	    status = E_DB_ERROR;
	}
	have_locklist = FALSE;
    }
...
i4		*err_code)  
{
	...
	*/
	    ...
	    racontext->locklist, racontext->log_id, &ir, err_code);
    	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	case TCB_ISAM:
		...
		DM1C_FIND, DM1C_EXACT, &ir->rcb_lowtid, err_code);
	    break;
	...
	local_status = dm2r_return_rcb(&ir, err_code);
	...
	if (*err_code > E_DM_INTERNAL)
	{
	    ...
	    *err_code = E_DM0164_READ_AHEAD_FAILURE;
	}
    ...
    */
    if (ir)
	local_status = dm2r_return_rcb(&ir, &local_err_code);
...
i4		*err_code)  
{
	...
	*/
	...
	{
	    if (*err_code == E_DM0056_NOPART || *err_code == E_DM0065_USER_INTR)
	    {
...
i4		*err_code)  
{
    ...
    status = dm0p_fix_page(r, pf->t.dp.pageno,
    	DM0P_NOREADAHEAD, &page, err_code);
