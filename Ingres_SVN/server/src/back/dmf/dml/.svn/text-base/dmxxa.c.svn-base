...
DMX_CB    *dmx)
{
    ...
    LG_TRANSACTION	tr;
    i4			length;
    i4			*err_code = &dmx->error.err_code;
	...
	*/
	...
	else 
	{
		...
		*err_code = E_DM0061_TRAN_NOT_IN_PROGRESS;
		...
		*err_code = E_DM0064_USER_ABORT;
		...
		if ( stat || length == 0 )
		{
			...
			(char *)0, 0L, (i4 *)0, err_code, 1, 0, LG_S_ATRANSACTION);
		    *err_code = E_DM012E_ERROR_XA_START;
		}
		else
		{
			...
			*err_code = E_DM011F_INVALID_TXN_STATE;
		    else
		    {
	...
	else 
	{
		...
		*err_code = E_DM0120_DIS_TRAN_UNKNOWN;
		...
		if ( stat || length == 0 )
		{
		    ...
		    ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, err_code, 1, 0, LG_S_ATRANSACTION);
		    *err_code = E_DM012F_ERROR_XA_END;
		}
		else
		{
		    /* Must match on supplied Distributed tran id */
...
DMX_CB    *dmx)
{
    ...
    *err_code = 0;
	...
	status = dmxe_xa_prepare(&dmx->dmx_dis_tran_id, dcb, err_code);
    }
    else
    {
...
DMX_CB    *dmx)
{
    ...
    {
	dcb = (DMP_DCB *)odcb->odcb_db_id;
	status = dmxe_xa_rollback(&dmx->dmx_dis_tran_id, dcb, err_code);
    }
    else
    {
