...
**              .attr_prec                  Precision of attribute.
**              .attr_collID		    Collation ID of attribute.
**              .attr_flags                 Must be zero.
**
...
DMT_CB  *dmt_cb)
{
	...
	attr_entry = (DMT_ATTR_ENTRY **)dmt->dmt_attr_array.ptr_address;
            ...
            (dmt->dmt_attr_array.ptr_size == sizeof(DMT_ATTR_ENTRY)))
	{
		...
		*/
		...
		if (attr_entry[i]->attr_flags & ~DMT_F_LEGAL_ATTR_BITS)
		{
		...
		dmt->dmt_char_array.data_in_size)
	{
		...
		case DMT_C_PAGE_SIZE:
		    ...
		    {
			dmt->error.err_data = i;
			dmt->error.err_code = E_DM000D_BAD_CHAR_ID;
			return (E_DB_ERROR);
		    }
		    break;
	...
	if ((dmt->dmt_flags_mask & DMT_DBMS_REQUEST) == 0)
	{
	    ...
	    */
	    ...
	    status = dm2t_lookup_tabid(dcb, (DB_TAB_NAME *)&dmt->dmt_table,
			(DB_OWN_NAME *)&dmt->dmt_owner, &temp_table_id, &error);
	    if (status == E_DB_OK)
	    {
    ...
    "NOWAIT,FORCE_CLOSE,LOAD,DBMS_REQ,SHOW_STAT,TIMESTAMP,SESSION_TEMP,RECOVERY,LO_TEMP,CONSTRAINT", dmt->dmt_flags_mask);
	    ...
	    */
		...
		ule_format( error, 0, ULE_LOG , NULL, 
			(char * )0, 0L, (i4 *)0, 
			&local_error, 0);
		ule_format( E_DM901F_BAD_TABLE_CREATE, 0, ULE_LOG , NULL, 
			(char * )0, 0L, (i4 *)0, 
...
# endif
	    ...
	    status = dm2t_temp_build_tcb(dcb, &relrecord.reltid, 
			    ...
			    (DM2T_ATTR_ENTRY **)attr_entry, attr_count,
