...
DB_ERROR    *dmf_err)
{
    ...
    i4	    error_code = E_DM0117_WRITE_BEHIND;
    i4	    wbcount = 0;
    i4	    wait_time = 0;
    i4	    base_time = 0;
	...
	** the LG_NOTDB flag.
	*/
	MEcopy((PTR)DB_WRITEBEHIND_NAME, sizeof(add_info.ad_dbname),
	...
	if (stat != OK)
	{
		...
		dmf_err->err_code = E_DM0062_TRAN_QUOTA_EXCEEDED;
	    else
		dmf_err->err_code = E_DM0117_WRITE_BEHIND;
	    status = E_DB_ERROR;
	}
	else
	    lg_added = TRUE;
	...
	** require backout.
	*/
	MEcopy((PTR)DB_WRITETHREAD_NAME, sizeof(DB_OWN_NAME), 
							(PTR) &user_name);
	...
	*/
	...
	if (stat != OK)
	{
	    ule_format(stat, (CL_SYS_ERR *)&sys_err, 
		ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, &error, 0);
	    ule_format(E_DM901A_BAD_LOCK_CREATE, &sys_err, ULE_LOG, NULL, (char *)0,
		(i4)0, (i4 *)0, &error, 0);
	    if (stat == LK_NOLOCKS)
	...
	*/
	do
	{
		...
		** signal.
		*/
		status = dm0p_wbflush_wait(cfa, lock_list, &error);
		if (status != E_DB_OK)
		{
		    /*
		    ...
		    */
			...
			if (error > E_DM_INTERNAL)
			{
				...
				(i4 *)0, &error, 0);
			    error_code = E_DM0117_WRITE_BEHIND;
			}
			break;
		    }
		}
	    ...
	    */
	    ...
	    if (stat != OK)
	    {
		ule_format(stat, (CL_SYS_ERR *)&sys_err, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, &error_code, 0);
		ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, &error_code, 1, 
		    0, LG_S_LGSTS);
		error_code = E_DM0117_WRITE_BEHIND;
		status = E_DB_ERROR;
		break;
...
DMC_CB	    *dmc_cb)
{
    ...
    i4		    have_transaction = FALSE;
    DB_STATUS	    status = E_DB_OK;
    i4	    error_code = E_DM0163_WRITE_ALONG_FAILURE;
    i4	    wbcount = 0;
    i4	    base_time = 0;
    i4	    flush_time, new_time;
    i4	    length;
...
#define NUM_DUTY (sizeof(duties)/sizeof (i4))
    ...
    ** the LG_NOTDB flag.
    */
    MEcopy((PTR)DB_BWRITALONG_NAME, sizeof(add_info.ad_dbname),
	(PTR) &add_info.ad_dbname);
    MEcopy((PTR)DB_INGRES_NAME, sizeof(add_info.ad_dbowner),
	(PTR) &add_info.ad_dbowner);
    MEcopy((PTR)"None", 4, (PTR) &add_info.ad_root);
	...
	if (stat == LG_EXCEED_LIMIT)
	    ...
	    dmc_cb->error.err_code = E_DM0163_WRITE_ALONG_FAILURE;
	return (E_DB_ERROR);
    }
	...
	** require backout.
	*/
	MEcopy((PTR)DB_BWRITALONG_NAME, sizeof(DB_OWN_NAME), 
							(PTR) &user_name);
	stat = LGbegin(LG_NOPROTECT, lg_dbid, &tran_id, &lx_id,
	    sizeof(DB_OWN_NAME), user_name.db_own_name,
	    (DB_DIS_TRAN_ID*)NULL, &sys_err);
	...
	for (;;)
	{
		...
		*/
		...
		if (stat != E_CS0009_TIMEOUT)
		{
		    ule_format(stat, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, &error_code, 0);
		    error_code = E_DM0163_WRITE_ALONG_FAILURE;
		    status = E_DB_ERROR;
		    break;
		}
	    }
