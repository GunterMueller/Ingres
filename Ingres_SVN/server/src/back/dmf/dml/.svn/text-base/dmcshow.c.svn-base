...
DMC_CB    *dmc_cb)
{
    ...
    for(;;)
    {
	   chr = (DMC_CHAR_ENTRY*) dmc_cb->dmc_char_array.data_address;
	   chr_count = 
		dmc_cb->dmc_char_array.data_out_size / sizeof(DMC_CHAR_ENTRY);

	   /*
	   ** char aray must contain at least one entry, unless
	   ** getting session info
	   */
	   if ((!chr || !chr_count) && dmc_cb->dmc_op_type!=DMC_SESSION_OP)
	   {
		error=E_DM002A_BAD_PARAMETER;
	   }
	   switch(dmc_cb->dmc_op_type)
	   {
	   case DMC_SERVER_OP:
		/*
		...
		*/
		...
		if (   dmc_cb->dmc_flags_mask & ~valid_flags
			|| !(dmc_cb->dmc_flags_mask & valid_flags))
		{
			error = E_DM009F_ILLEGAL_OPERATION;
			break;
		}

		if ( (scb = GET_DML_SCB(dmc_cb->dmc_session_id)) == 0 || 
		...
		odcb = (DML_ODCB *)dmc_cb->dmc_db_id;
		...
		if (dmc_cb->dmc_flags_mask & DMC_JOURNAL)
		{
		...
		*/
		...
		case DMC_XACT_ID:
			...
			*/
			if (scb->scb_x_ref_count == 0)
				dmc_cb->dmc_xact_id=NULL;
			else
				dmc_cb->dmc_xact_id= (PTR)scb->scb_x_next;
			break;
		...
		case DMC_SESS_LLID:
			...
			*/
			dmc_cb->dmc_scb_lkid = (PTR)&scb->scb_lock_list;
			break;
