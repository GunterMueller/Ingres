...
DMT_CB   *dmt_cb)
{
    ...
    for (;;)
    {
        if ((dmt->dmt_flags_mask & ~(DMT_DBMS_REQUEST | DMT_SHOW_STAT | 
	       ...
	       DMT_LK_ONLN_NEEDED | DMT_FORCE_NOLOCK)) == 0)
	{
	    ...
	    */
		...
		if (dm0m_check((DM_OBJECT *)xcb, (i4)XCB_CB) == E_DB_OK)
		{
			...
			error = E_DM0061_TRAN_NOT_IN_PROGRESS;
		}
		else
	...
	if (dmt->dmt_lock_mode < DMT_RIX || dmt->dmt_lock_mode > DMT_X ||
	...
	**
	** SERIALIZABLE lock protocols during constraint processing
	...
	*/
	if ((t->tcb_rel.relstat & TCB_VIEW) ||
        ...
        {
	    if (t->tcb_rel.relstat & TCB_VIEW)
		error = E_DM009E_CANT_OPEN_VIEW;
	    else
		error = E_DM0052_COMPRESSION_CONFLICT;
	    break;
	}
	...
	*/
	...
	local_status = dm2t_close(rcb, 0, &local_error);
	if (local_status != E_DB_OK)
	{
	    ule_format(local_error, 0, ULE_LOG, NULL, (char * )0, 0L, (i4 *)0, 
		    &local_error, 0);
	}
    }
    if (error > E_DM_INTERNAL)
    {
	ule_format(error, 0, ULE_LOG, NULL, (char * )0, 0L, (i4 *)0, 
	    &local_error, 0);
	error = E_DM008F_ERROR_OPENING_TABLE;
    }
...
i4              *total_pages)
{
    ...
    i4         lock_mode = dmt_cb->dmt_lock_mode;
    i4	    i;
	...
	case DMT_C_LOCKMODE:
		...
		*iso_level = DMC_C_READ_UNCOMMITTED; 
            ...
            /* Set lock_level value according to lock_mode
            ** to override session level setting.
            */
            if (lock_mode >= DMT_S)
                *lock_level = DMC_C_TABLE;
            else if (lock_mode <= DMT_RIS)
...
i4         total_pages)
{
    ...
    */
	...
	(lock_mode == DM2T_IS || lock_mode == DM2T_IX))
    {
	lock_level = DMC_C_ROW;
    }
    ...
    */
    ...
    /*
    ** Set lockmode according to the lock level chosen.
    */
    if (lock_level == DMC_C_TABLE)
    {
        if (lock_mode == DMT_RIX || lock_mode == DMT_IX || lock_mode == DMT_X)
	    lock_mode = DM2T_X;
        else 
            lock_mode = DM2T_S;
    }
    ...
    */
    ...
    if (iso_level == DMC_C_READ_UNCOMMITTED || iso_level == DMC_C_READ_COMMITTED)
    {
	...
	*/
	if (dmt->dmt_id.db_tab_base <= DM_SCONCUR_MAX
	  || dmt->dmt_id.db_tab_base == DM_B_SEQ_TAB_ID)
	    iso_level = DMC_C_SERIALIZABLE;
        else if (dmt->dmt_id.db_tab_base <= DM_SCATALOG_MAX)
	    iso_level = DMC_C_REPEATABLE_READ;
    }
    ...
    *lock_mode_pt  = lock_mode;
	...
	{
	    ule_format(E_DM9065_READNOLOCK_READUNCOMMITTED, (CL_SYS_ERR *)0,
