...
DMR_CB  *dmr_cb)
{
	...
	if (dm0m_check((DM_OBJECT *)rcb, (i4)RCB_CB) == E_DB_OK)
	{
	    ...
	    */
                             ...
                             >= rcb->rcb_tcb_ptr->tcb_rel.relwid)
		{
			...
			** Start a mini-xact
			*/
			if ((status = dm0l_bm(rcb, &bm_lsn, &error)) != E_DB_OK)
				break;
		    }
		    ...
		    {
			local_status = dm2r_unfix_pages(rcb, &local_error);
			...
			dmr->error.err_code = E_DM0000_OK;
			if (rcb->rcb_val_logkey & (RCB_TABKEY | RCB_OBJKEY))
			{
			    ...
			    ** Finish mini-xact
			    */
			    if ((status = dm0l_em(rcb, &bm_lsn, &lsn, &error)) 
					!= E_DB_OK)
			    {
				break;
			    }
    ...
    if (error == E_DM0022_BAD_MASTER_OP ||
	...
	*(rcb->rcb_uiptr) &= ~SCB_USER_INTR;
	...
	(error == E_DM9245_DM1S_ALLOCATE || error == E_DM925E_DM1B_ALLOCATE))
    {
	error = E_DM0138_TOO_MANY_PAGES;
    }
	...
	ule_format(E_DM904D_ERROR_PUTTING_RECORD, 0, ULE_LOG, NULL,
        ...
        if (error == E_DM9247_DM1H_ALLOCATE) 
	    error = E_DM0112_RESOURCE_QUOTA_EXCEED;
	else
    ...
    return(dmrThrError(rcb, status, &error));
}
