...
#define RQF_GCBFSIZE	1024
    ...
    GCA_SD_PARMS	rql_send;			/* gca send struct */
    GCA_FO_PARMS	rql_format;			/* format write */
    char	    	rql_read_buff[RQF_GCBFSIZE];    /* gca read buffer */
    GCA_RV_PARMS	rql_rv;				/* receive struct */
    GCA_IT_PARMS	rql_it;				/* interpret struct */
...
FUNC_EXTERN VOID rqu_end_all_assoc(	RQR_CB		*rcb, 
					RQL_ASSOC	*assoc);
FUNC_EXTERN VOID rqu_format_write(	GCA_FO_PARMS	*format, 
					char		*buffer,
					i4		buffer_size, 
					i4		assoc_id,
					GCA_SD_PARMS	*send);
FUNC_EXTERN VOID rqu_fmt_read(		RQL_ASSOC	*assoc);
