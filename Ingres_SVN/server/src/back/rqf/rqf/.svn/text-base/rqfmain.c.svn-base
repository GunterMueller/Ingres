	...
	RQR_CB	    *rqr_cb)
{
  ...
  if (rqr_cb->rqr_error.err_code == E_RQ0000_OK &&
      rqu_get_reply( assoc, -1 ) == GCA_S_BTRAN)
    {
      ...
      MEcopy((PTR)assoc->rql_it.gca_data_area, sizeof(GCA_TX_DATA),
	...
	RQR_CB	    *rqr_cb)
{
  ...
  if (status == E_RQ0000_OK)
  {
	...
	case GCA_RESPONSE:
	{
	    GCA_RE_DATA	*response = (GCA_RE_DATA *)assoc->rql_it.gca_data_area;
...
rqf_td_get(
	...
	RQL_ASSOC   *assoc)
{
    GCA_TD_DATA	    *td_data = (GCA_TD_DATA *)assoc->rql_it.gca_data_area,
    ...
    */
    ...
    assoc->rql_b_ptr = (char *)assoc->rql_it.gca_data_area;
    assoc->rql_b_remained = (u_i2)assoc->rql_it.gca_d_length;
	...
	RQR_CB	    *rqr_cb)
{
    ...
    */
	...
	switch (rqu_get_reply(assoc, -1))
	{
		...
		{
		    response = (GCA_RE_DATA *)assoc->rql_it.gca_data_area;
		    rqr_cb->rqr_tupcount = response->gca_rowcount;
		}
		break;
	    case GCA_RESPONSE:
	...
	RQR_CB	*rcb)
{
	...
	IIGCa_call(GCA_INTERPRET, (GCA_PARMLIST *)interpret, GCA_SYNC_FLAG, 
		 (PTR)0, (i4)-1, &status);

	/* unconditional trace */
	rqu_dump_gca_msg(assoc);

	(VOID)rqu_gcmsg_type(GCA_RECEIVE, interpret->gca_message_type,
				assoc);

	if (interpret->gca_message_type == GCA_RESPONSE)
	{
	    GCA_RE_DATA *re = (GCA_RE_DATA *) interpret->gca_data_area;
    ...
    {
	GCA_SD_PARMS	send;
	GCA_IT_PARMS	*it = (GCA_IT_PARMS *)dc->sdc_client_msg;
        ...
        send.gca_buffer = it->gca_buffer;
        send.gca_message_type = it->gca_message_type;
        send.gca_msg_length = it->gca_d_length;
        send.gca_end_of_data = it->gca_end_of_data;
	...
	RQR_CB	*rcb)
{
    ...
    */
	...
	/*
        ** get the node name
        */
        (VOID)STlpolycat( (i4) 3, sizeof(log_name)-1,
			"II_GCN", installation, "_LCL_VNODE", &log_name[0]);
        NMgtAt( log_name, &vnode);
        if ( vnode == (char *)NULL || *vnode == EOS )
        {
	    RQTRACE(rqs,1)(RQFMT(rqs,
		"RQF:rqf_startup. II_GCNxx_LCL_VNODE is not defined\n"));
	...
	RQR_CB	    *src_cb)
{
    ...
    */
    if ((dest_assoc->rql_it.gca_message_type != GCA_C_FROM) &&
        (dest_assoc->rql_it.gca_message_type != GCA1_C_FROM))
    {
	...
	reply_type = dest_assoc->rql_it.gca_message_type;
    ...
    */
    ack = (GCA_AK_DATA *)dest_assoc->rql_format.gca_data_area;
    ...
    */
    dest_assoc->rql_send.gca_buffer = src_assoc->rql_rv.gca_buffer;
    ...
    */
    ...
    dest_assoc->rql_send.gca_descriptor = (PTR)td_data;
	...
	*/
	status = rqu_send(dest_assoc, 
			  src_assoc->rql_it.gca_d_length,
			  src_assoc->rql_it.gca_end_of_data,
			  GCA_CDATA);
	/* 
    ...
    */
    else if (src_assoc->rql_it.gca_message_type != GCA_RESPONSE )
    {
    ...
    */
    dest_assoc->rql_send.gca_buffer = dest_assoc->rql_write_buff;
    if (status != E_RQ0000_OK)
    {
	...
	PTR             *td_data)
{
    ...
    it = (GCA_IT_PARMS  *)&assoc->rql_it;
    ptr1 = ptr2 = (PTR)NULL;
    size = 0;
    msg_type = it->gca_message_type;
	...
	ulm.ulm_flags = ULM_PRIVATE_STREAM | ULM_OPEN_AND_PALLOC;
    	ulm.ulm_psize = size + it->gca_d_length;
	...
	*/
	ptr2 = ptr1 + size;
	MEcopy( (PTR)it->gca_data_area, it->gca_d_length, (PTR)ptr2);
	size += it->gca_d_length;
    } while (it->gca_end_of_data == FALSE && 
             (rqu_get_reply(assoc, -1) == msg_type) );

    if (it->gca_message_type != msg_type)
    {
        if (ptr1 != (PTR)NULL)
        {
            ulm.ulm_streamid = strm1;
