...
qso_trans_or_define( QSF_RCB *qsf_rb )
{
	...
	*err_code = E_QS0000_OK;
	...
	while ( obj = qso_find(alias_objid, &master, &hash) )
	{
	    ...
	    */
	    status = qso_wait_for(&qp_obj, &master, err_code);
	...
	else
	{
	    ...
	    */
	    ...
	    status = qsf_attach_inst(*alias_objid);
    ...
    *err_code = E_QS0000_OK;
	    ...
	    && obj->qso_moprev == (QSO_OBJ_HDR *) NULL)
	{
	    ...
	    */
		...
		stat = qsf_detach_inst(master->qsmo_aliasid);
		if (stat)
		{
#ifdef xDEBUG
...
i4		*err_code )
{
    ...
    *err_code = E_QS0000_OK;
    ...
    /* if (the QP is in the middle of being built) then wait for
    ** it to be finished.
    */
    while (obj->qso_lk_state == QSO_EXLOCK || obj->qso_root == (PTR)NULL)
    {
...
i4		*err_code )
{
	...
	*err_code = E_QS0016_BAD_LOCKSTATE;
	...
	if ((new_obj->qso_status & (QSO_DEFERRED_DESTROY | QSO_LRU_DESTROY)) == 0)
	{
	    ...
	    /* Wait for any QP's in the process of being built. */
	    status = qso_wait_for(&new_obj, master, err_code);
	    if (status != E_DB_OK)
		return(status);
