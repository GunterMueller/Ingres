    ...
    i4 	*err_code)
{
	...
	*/
	...
	else if ((cl_status == OK) || (cl_status == LK_VAL_NOTVALID) )
	{
 	    ...
 	    */
 	    LK_VALUE	lkval;
 	    MEfill(sizeof(LK_VALUE),NULL,(PTR)&lkval);
	    /*
    ...
    i4	*err_code)
{
	...
	*/
	CSp_semaphore(TRUE, &Sxap_cb->sxap_semaphore);
	got_semaphore=TRUE;
	...
	*max_file = Sxap_cb->sxap_max_file;
    ...
    if (got_semaphore)
	CSv_semaphore(&Sxap_cb->sxap_semaphore);
    if (*err_code != E_SX0000_OK)
    {
	_VOID_ ule_format(*err_code, NULL,
		ULE_LOG, NULL, NULL, 0L, NULL, &local_err, 0);
    ...
    i4	*err_code)
{
    ...
    for(;;)
    {
	    ...
	    **	- If on a cluster:
	    **    - If the new stamp matches the shared buff stamp, then
	    **	    some other server on this node changed files, so
	    **	    just need to re-open the new file for this server.
	    **    - If the server stamp matches shared buff stamp, then
	    **      file changed on some other node, so flush this then
	    **	    change to a new file.
	    **	  - Otherwise internal protocol error.
	    **
	    **  - If NOT on a cluster:
	    **    - If the new stamp matches the shared buff stamp, then
	    **	    some other server on this node changed files, so
	    ...
	    **
	    **	In a cluster, check to see if the server stamp matches 
	    **  shared buff stamp, then file changed on some other node, 
	    **  so change to the new file, flushing first.
	    **  If not the same, something went wrong.
	    */
	    if ( !(CXcluster_enabled()) ||
		  sxap_stamp_cmp(svr_stamp, buff_stamp) )
	    {
    ...
    */
    ...
    for(;;)
    {
		...
		*/
		...
		i4    newpage;
		/*
		**	Check if need to reinitialize the buffer
		*/
	        buff_stamp= &Sxap_cb->sxap_shm->shm_stamp;
		svr_stamp=  &Sxap_cb->sxap_stamp;
		if (!sxap_stamp_cmp(buff_stamp, svr_stamp))
		{
			/*
...
/*
** The following ifdef has been put in because of inconsistencies
** between the VMS CL and the UNIX CL. In VMS the variable PC_NOWAIT
** is defined, on UNIX the variable PC_NO_WAIT is defined.
*/
# ifdef PC_NOWAIT
		stat=PCcmdline((LOCATION *)NULL, command_buf, PC_NOWAIT, 
				 (LOCATION *)NULL, &clerr);
# else
