...

/*{
** Name: ascf_init_dbg	- initialize server for module (other than scf) testing
**
** Description:
...
# endif /* NT_GENERIC */
    ...
    if (status)
    {
	sc0e_put(E_SC0124_SERVER_INITIATE, 0, 0);
	if (status & E_SC_MASK)
	{
	    sc0e_put(E_SC0204_MEMORY_ALLOC, 0, 0);
	    sc0e_put(status, 0, 0);
	}
	else
	{
	    sc0e_put(E_SC0200_SEM_INIT, 0, 0);
