...
$	if f$parse(d) .eqs. "" then goto contloop
$	l=f$len(d)
$	d=f$edit(d, "lowercase")
$	ext=""
$	if f$locate("_vms", d) .ne. l
$	then	ext="vms"
$	else	if f$locate("_unix", d) .ne. l
$	then	ext="unix"
$	else	if f$locate("_win", d) .ne. l
$	then	ext="win"
$	endif
$	endif
$	endif
...
$ endif
$!
$ ipath=f$trnlnm("ING_SRC")-".]"+"]"
$ dsk=f$parse(ipath,,,"device")-":"
$ if f$trnlnm(dsk) .nes. ""
$ then	if (f$trnlnm(dsk)-":")+":" .nes. (dsk+":") then dsk=f$trnlnm(dsk)-":"
$ endif
$ ingsrcpath=f$edit(dsk+":"+f$parse(ipath,,, "directory"), "upcase")
$ dpath=f$edit(f$trnl(f$parse(d,,,"device")-":")+f$parse(d,,,"directory"), -
	"upcase")
$ if f$locate("[000000]", dpath) .ne. f$len(dpath) then dpath=dpath-"[000000]"
$ if f$locate(".][", dpath) .ne. f$len(dpath) then dpath=dpath-"]["
$ if f$locate(".]", dpath) .eq. f$len(dpath)-2 then dpath=dpath-".]"+"]"
$ if dpath .eqs. ingsrcpath	! ING_SRC is a simple case
$ then	dir="ING_SRC"
$	p1=dir
$ else	dir=dpath
$	dir=dir-"]["
$	plen=f$len(ingsrcpath)-1
$	if f$extr(0, plen, dir) .eqs. f$extr(0, plen, ingsrcpath)
$	then	dir=f$edit(f$extr(plen+1,f$len(dir),dir)-"]","lowercase")
$	endif
$!
...
$	p1=f$extr(0, f$loc(".", dir), dir)
$	rdir=dir-p1-"."
$	p2=f$extr(0, f$loc(".", rdir), rdir)
$	rdir=rdir-p2-"."
$	p3=f$extr(0, f$loc(".", rdir), rdir)
$	rdir=rdir-p3-"."
$	p4=f$extr(0, f$loc(".", rdir), rdir)
$ endif
$!
$ GROUP="''p1'"
