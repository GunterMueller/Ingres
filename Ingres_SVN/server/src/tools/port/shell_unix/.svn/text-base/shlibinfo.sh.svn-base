...
eval `egrep "VERS[ 	]*=|SLSFX[ 	]*=|MWSLSFX[	 ]*=|XERCVERS[	  ]*=" \
...
case "$config" in
	...
	use_kerberos=true
	use_shared_libs=true
	shlink_cmd="gcc -dynamiclib"
	archout=""
	# archout="-arch i386 -arch ppc"
	# Disabled until we can build universal binaries
	shlink_opts="-framework System -undefined dynamic_lookup -dead_strip"
...
export use_shared_libs use_shared_svr_libs use_archive_svr_lib shlink_cmd shlinkHB_cmd shlink_opts shlinkHB_opts krblink_cmd krblink_opts
