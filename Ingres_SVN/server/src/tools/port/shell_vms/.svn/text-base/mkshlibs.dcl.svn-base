...
$ libdir="ING_SRC:[''dir']"
$ optdir=F$PARSE("''libdir'",,,"DEVICE")+F$PARSE("''libdir'",,,"DIRECTORY")
$ if "''optdir'" .EQS. "" THEN GOTO err_exit
$ set default 'libdir'
$ pipe sed "-e s/INGLIB:/ING_BUILD:[lib]/g" 'libname'.lot > 'optdir''libname'.opt
$ on warning then goto err_exit
$ link 'LINKFLAGS' /share/section_binding/replace/notraceback -
	/map='optdir''libname'.map/full /symb=ING_BUILD:[debug]'libname'.stb -
	/exec='exec' 'optdir''libname'.opt/options
$ del 'libname'.opt;*
$ set def 'orig_wd'
$ exit
$err_exit:
