...
**	the iidbdb).
**
**	This routine builds an initial purgelist by creating the header for
**	a list of files for each directory.  The list of files will be filled
**	in by subsequent processing.
**
**
** Inputs:
**      duve_cb                         verifydb control block
...
**
** Side Effects:
**	    If table/view 'iipurge' exists, it is destroyed.
**
** History:
**      28-Feb-1989 (teg)
**          initial creation
...
DUVE_CB		*duve_cb;
{
  ...
  i4 l_status;
  char *parea;
/* # line 398 "duvepurg.sc" */	/* host code */
    /* 
    ** allocate memory to hold array of directory information 
    */
/* # line 401 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)"select count(lname)from iiex\
tend");
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
...
/* # line 402 "duvepurg.sc" */	/* host code */
    size = sizeof(DU_DIRLST) * cnt;
    duve_cb->duve_dirs = (DUVE_DIRLST *) MEreqmem( NULL, size,
						    TRUE, &mem_stat);
    ...
    */
	...
	STcopy(duve_cb->duve_list->du_dblist[i].du_db, dbname);
        parea = NULL;
/* # line 419 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select dbdev from iidatabase where name=");
    IIputdomio((short *)0,1,32,0,dbname);
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,DB_MAXNAME+1-1,dbdev);
    } /* IInextget */
    IIsqFlush((char *)0,0);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 420 "duvepurg.sc" */	/* host code */
	len = STzapblank(dbdev,dbdev);
/* # line 421 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select d.name, e.lname, l.area, d.own, l.status, e.status from iidata\
base d, iiextend e, iilocations l where d.name=e.dname and l.lname=e.l\
name and e.dname=");
    IIputdomio((short *)0,1,32,0,dbname);
    IIwritio(0,(short *)0,1,32,0,(char *)"order by d.name, l.area");
    IIretinit((char *)0,0);
    if (IIerrtest() != 0) goto IIrtE1;
IIrtB1:
    while (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,DB_MAXNAME+1-1,db);
      IIgetdomio((short *)0,1,32,DB_MAXNAME+1-1,loc);
      IIgetdomio((short *)0,1,32,DB_AREA_MAX+1-1,area);
      IIgetdomio((short *)0,1,32,DB_MAXNAME+1-1,dba);
      IIgetdomio((short *)0,1,30,4,&loc_type);
      IIgetdomio((short *)0,1,30,4,&stat);
      if (IIerrtest() != 0) goto IIrtB1;
/* # line 433 "duvepurg.sc" */	/* host code */
	    j = duve_cb->duve_pcnt;
...
DUVE_CB		*duve_cb;
{
  ...
  char fipsbuf[6];
  i4 numcnt;
/* # line 559 "duvepurg.sc" */	/* host code */
	/*
	** decide if this installation is FIPS or not 
	*/
/* # line 562 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)"select dbmsinfo('db_name_cas\
e')");
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
...
/* # line 563 "duvepurg.sc" */	/* host code */
	if (STcompare(fipsbuf, ERx("UPPER")) == DU_IDENTICAL)
	    isfipsid = TRUE;
/* # line 566 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
...
/* # line 567 "duvepurg.sc" */	/* host code */
	if (STcompare(fipsbuf, ERx("UPPER")) == DU_IDENTICAL)
	    isfipsuser = TRUE;
	...
	** from
	*/
	for (i=NULL; i<	MAXLISTNAME; i++)
	{
	    (VOID) STcopy (lst_names[i], tblnam);
	    if (isfipsid)
		CVupper(tblnam);
/* # line 583 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select count(relid)from iirelation where relid=");
    IIputdomio((short *)0,1,32,0,tblnam);
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,30,4,&numcnt);
    } /* IInextget */
    IIsqFlush((char *)0,0);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 585 "duvepurg.sc" */	/* host code */
	    if (numcnt == NULL)
		break;
	}
	if (i == MAXLISTNAME)
...
/* # line 618 "duvepurg.sc" */	/* host code */
...
/* # line 630 "duvepurg.sc" */	/* host code */
	/* 
	** create the QEF internal procedure to list files and the one to
	** destroy files
	*/
/* # line 635 "duvepurg.sc" */	/* drop procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(1,(char *)"iiQEF_list_file");
    IIwritio(0,(short *)0,1,32,0,(char *)"drop procedure iiQEF_list_fi\
le");
    IIsyncup((char *)0,0);
  }
/* # line 636 "duvepurg.sc" */	/* drop procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(1,(char *)"iiQEF_drop_file");
    IIwritio(0,(short *)0,1,32,0,(char *)"drop procedure iiQEF_drop_fi\
le");
    IIsyncup((char *)0,0);
  }
/* # line 639 "duvepurg.sc" */	/* create procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(0,(char *)"iiQEF_list_file");
    IIwritio(0,(short *)0,1,32,0,(char *)
"create procedure iiQEF_list_file(");
    IIputctrl(0);
    IIwritio(0,(short *)0,1,32,0,(char *)
...
/* # line 649 "duvepurg.sc" */	/* create procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(0,(char *)"iiQEF_drop_file");
    IIwritio(0,(short *)0,1,32,0,(char *)
"create procedure iiQEF_drop_file(");
    IIputctrl(0);
    IIwritio(0,(short *)0,1,32,0,(char *)
"                        directory char(256) not null not default,");
    IIputctrl(0);
...
/* # line 657 "duvepurg.sc" */	/* host code */
	/*
	** create cursor statement-- cursor name will be "temp_cursor"
	*/
	cmd = STpolycat(2, "select filnam, locnam, area from ",*list_tbl, buf);
/* # line 661 "duvepurg.sc" */	/* prepare */
  {
    IIsqInit(&sqlca);
    IIsqPrepare(0,(char *)"s1",(char *)0,0,cmd);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 664 "duvepurg.sc" */	/* host code */
	return ( (DU_STATUS) DUVE_YES);		
}
/*{
...
/* # line 761 "duvepurg.sc" */	/* host code */
    if (STcompare(ERx("UPPER"), fipsbuf) == DU_IDENTICAL)
	(VOID) STcopy (ERx("FILNAM"), colnam);
    else
    	(VOID) STcopy (ERx("filnam"), colnam);
/* # line 766 "duvepurg.sc" */	/* execute procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(2,(char *)"iiQEF_list_file");
    IILQprvProcValio((char *)"directory",0,(short *)0,1,32,0,dbms_path);
    IILQprvProcValio((char *)"tab",0,(short *)0,1,32,0,tabnam);
    IILQprvProcValio((char *)"owner",0,(short *)0,1,32,0,ownnam);
    IILQprvProcValio((char *)"col",0,(short *)0,1,32,0,colnam);
    while (IILQprsProcStatus(0) != 0) {
      if (sqlca.sqlcode < 0) 
        Clean_Up();
    } /* IILQprsProcStatus */
    IIeqiqio((short *)0,1,30,4,&retcnt,(char *)"iiret");
  }
/* # line 772 "duvepurg.sc" */	/* host code */
...
/* # line 780 "duvepurg.sc" */	/* host code */
    }
    return ( (DU_STATUS) DUVE_YES);
}
/*{
...
/* # line 859 "duvepurg.sc" */	/* host code */
...
/* # line 867 "duvepurg.sc" */	/* host code */
    ...
    */
/* # line 871 "duvepurg.sc" */	/* open */
  {
    IIsqInit(&sqlca);
    IIcsOpen((char *)"temp_cursor",6229,27366);
    IIwritio(0,(short *)0,1,32,0,(char *)"s1");
    IIcsQuery((char *)"temp_cursor",6229,27366);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 873 "duvepurg.sc" */	/* host code */
    for (;;)	    /* loop to delete temp files */
    {
/* # line 875 "duvepurg.sc" */	/* fetch */
  {
    IIsqInit(&sqlca);
    if (IIcsRetScroll((char *)"temp_cursor",6229,27366,-1,-1) != 0) {
      IIcsGetio((short *)0,1,32,DI_FILENAME_MAX +1-1,filnam);
      IIcsGetio((short *)0,1,32,DB_MAXNAME +1-1,locnam);
      IIcsGetio((short *)0,1,32,DI_PATH_MAX + 1-1,dbms_path);
      IIcsERetrieve();
    } /* IIcsRetrieve */
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 876 "duvepurg.sc" */	/* host code */
        if (sqlca.sqlcode == DU_SQL_NONE)
        {       
/* # line 878 "duvepurg.sc" */	/* close */
  {
    IIsqInit(&sqlca);
    IIcsClose((char *)"temp_cursor",6229,27366);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 879 "duvepurg.sc" */	/* host code */
            break;
        }
...
/* # line 892 "duvepurg.sc" */	/* execute procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(2,(char *)"iiQEF_drop_file");
    IILQprvProcValio((char *)"directory",0,(short *)0,1,32,0,dbms_path);
    IILQprvProcValio((char *)"file",0,(short *)0,1,32,0,filnam);
    while (IILQprsProcStatus(0) != 0) {
      if (sqlca.sqlcode < 0) 
        Clean_Up();
    } /* IILQprsProcStatus */
  }
/* # line 894 "duvepurg.sc" */	/* host code */
    ...
    */
/* # line 907 "duvepurg.sc" */	/* drop procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(1,(char *)"iiQEF_list_file");
    IIwritio(0,(short *)0,1,32,0,(char *)"drop procedure iiQEF_list_fi\
...
/* # line 908 "duvepurg.sc" */	/* drop procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(1,(char *)"iiQEF_drop_file");
    IIwritio(0,(short *)0,1,32,0,(char *)"drop procedure iiQEF_drop_fi\
...
/* # line 909 "duvepurg.sc" */	/* host code */
    cmd = STpolycat (2, "drop table ",list_tbl,buf);
/* # line 910 "duvepurg.sc" */	/* execute */
  {
    IIsqInit(&sqlca);
    IIsqExImmed(cmd);
    IIsyncup((char *)0,0);
...
/* # line 912 "duvepurg.sc" */	/* host code */
    /* last but not least, if dmf left a work (ie temp) table named iirtemp
    ** laying around, get rid of it. */
/* # line 914 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select count(relid)from iirelation where relid='iirtemp'");
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,30,4,&cnt);
    } /* IInextget */
...
/* # line 916 "duvepurg.sc" */	/* host code */
    if (cnt)
    {
/* # line 919 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select relowner from iirelation where relid='iirtemp'");
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,DB_MAXNAME +1-1,ownnam);
    } /* IInextget */
    IIsqFlush((char *)0,0);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 922 "duvepurg.sc" */	/* host code */
	cmd = STpolycat( (i4) 3,"drop '", ownnam,"'.iirtemp",buf);
/* # line 923 "duvepurg.sc" */	/* execute */
  {
    IIsqInit(&sqlca);
    IIsqExImmed(cmd);
    IIsyncup((char *)0,0);
...
/* # line 924 "duvepurg.sc" */	/* host code */
    }
    return ( (DU_STATUS) DUVE_YES);
}
/*{
...
DUVE_CB		    *duve_cb;
{
  ...
  u_i4 cnt;
    ...
    i4	    i;
    current_time = TMsecs();
/* # line 985 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select count(relsave)from iirelation where relsave is not null and(re\
lsave<");
    IIputdomio((short *)0,1,30,4,&current_time);
    IIwritio(0,(short *)0,1,32,0,(char *)"and relsave>0)");
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,30,4,&cnt);
    } /* IInextget */
...
/* # line 989 "duvepurg.sc" */	/* host code */
    if (cnt == 0)
	return((DU_STATUS)DUVE_YES);
    expired_list = 0;
    size = cnt * sizeof(EXPIRE_LIST);
    expired_list = (EXPIRE_LIST *) MEreqmem( (u_i4)0, size, TRUE, &mem_stat);
    if ( (mem_stat != OK) || (expired_list == 0) )
    {
	...
	*/
    ...
    i = 0;
/* # line 1004 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select relid, relowner from iirelation where relsave is not null and(\
relsave<");
    IIputdomio((short *)0,1,30,4,&current_time);
    IIwritio(0,(short *)0,1,32,0,(char *)"and relsave>0)");
    IIretinit((char *)0,0);
...
IIrtB4:
    while (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,DB_MAXNAME +1-1,tabnam);
      IIgetdomio((short *)0,1,32,DB_MAXNAME +1-1,ownnam);
      if (IIerrtest() != 0) goto IIrtB4;
/* # line 1009 "duvepurg.sc" */	/* host code */
...
/* # line 1033 "duvepurg.sc" */	/* host code */
...
/* # line 1048 "duvepurg.sc" */	/* host code */
	    }
...
DUVE_CB		    *duve_cb;
{
  ...
  i4 relsave, current_time;
    u_i4	    len;
    current_time = TMsecs();
/* # line 1113 "duvepurg.sc" */	/* open */
  {
    IIsqInit(&sqlca);
    IIcsOpen((char *)"r_cursor",6230,29027);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select relid, relowner, relsave from iirelation where relsave is not \
null and(relsave<");
    IIputdomio((short *)0,1,30,4,&current_time);
    IIwritio(0,(short *)0,1,32,0,(char *)"and relsave>0)");
    IIcsQuery((char *)"r_cursor",6230,29027);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 1115 "duvepurg.sc" */	/* host code */
    for(;;)
    {
/* # line 1118 "duvepurg.sc" */	/* fetch */
  {
    IIsqInit(&sqlca);
    if (IIcsRetScroll((char *)"r_cursor",6230,29027,-1,-1) != 0) {
      IIcsGetio((short *)0,1,32,DB_MAXNAME +1-1,tabnam);
      IIcsGetio((short *)0,1,32,DB_MAXNAME +1-1,ownnam);
      IIcsGetio((short *)0,1,30,4,&relsave);
      IIcsERetrieve();
    } /* IIcsRetrieve */
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 1119 "duvepurg.sc" */	/* host code */
	if (sqlca.sqlcode == DU_SQL_NONE)
	{
/* # line 1121 "duvepurg.sc" */	/* close */
  {
    IIsqInit(&sqlca);
    IIcsClose((char *)"r_cursor",6230,29027);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
  }
/* # line 1122 "duvepurg.sc" */	/* host code */
	    break;
	}
	/* this relation is expired 
	**  it may be a base table, an index table or a view 
...
/* # line 1141 "duvepurg.sc" */	/* host code */
	    if (duve_cb->duve_msg->du_ingerr)
	    {
		/* the server generated an error during this drop
		** statement -- probably caused because tablename is an SQL
...
/* # line 1233 "duvepurg.sc" */	/* host code */
...
/* # line 1252 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select reltid, relstat from iirelation where relid=");
    IIputdomio((short *)0,1,32,0,tbl_name);
    IIwritio(0,(short *)0,1,32,0,(char *)"and relowner=");
    IIputdomio((short *)0,1,32,0,owner_name);
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,30,4,&table_id);
      IIgetdomio((short *)0,1,30,4,&relstat);
    } /* IInextget */
    IIsqFlush((char *)0,0);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
...
/* # line 1255 "duvepurg.sc" */	/* host code */
...
/* # line 1290 "duvepurg.sc" */	/* host code */
    }
    /*
    ** create the internal procedure to do the patch table operation
    */
/* # line 1296 "duvepurg.sc" */	/* drop procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(1,(char *)"iiQEF_patch_table");
    IIwritio(0,(short *)0,1,32,0,(char *)"drop procedure iiQEF_patch_t\
able");
    IIsyncup((char *)0,0);
    if (sqlca.sqlcode == 700) 
      duve_msg();
  }
/* # line 1298 "duvepurg.sc" */	/* create procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(0,(char *)"iiQEF_patch_table");
    IIwritio(0,(short *)0,1,32,0,(char *)
...
/* # line 1307 "duvepurg.sc" */	/* host code */
    /* 
    **	enter a loop to delete all secondary indices assocated with the table 
    */
/* # line 1313 "duvepurg.sc" */	/* open */
  {
    IIsqInit(&sqlca);
    IIcsOpen((char *)"rel_cursor",6231,24755);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select relid, relowner, relstat2 from iirelation where reltid=");
    IIputdomio((short *)0,1,30,4,&table_id);
    IIwritio(0,(short *)0,1,32,0,(char *)"and reltidx!=0");
    IIcsQuery((char *)"rel_cursor",6231,24755);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
    else if (sqlca.sqlcode == 700) 
      duve_msg();
  }
/* # line 1315 "duvepurg.sc" */	/* host code */
    for ( ;; )
    {
	/*
	** get entry from iirelation
	*/
/* # line 1320 "duvepurg.sc" */	/* fetch */
  {
    IIsqInit(&sqlca);
    if (IIcsRetScroll((char *)"rel_cursor",6231,24755,-1,-1) != 0) {
      IIcsGetio((short *)0,1,32,DB_MAXNAME+1-1,index_name);
      IIcsGetio((short *)0,1,32,DB_MAXNAME+1-1,index_owner);
      IIcsGetio((short *)0,1,30,4,&relstat2);
      IIcsERetrieve();
    } /* IIcsRetrieve */
    if (sqlca.sqlcode < 0) 
      Clean_Up();
...
/* # line 1321 "duvepurg.sc" */	/* host code */
	if (sqlca.sqlcode == DU_SQL_NONE)
	{	
/* # line 1323 "duvepurg.sc" */	/* close */
  {
    IIsqInit(&sqlca);
    IIcsClose((char *)"rel_cursor",6231,24755);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
    else if (sqlca.sqlcode == 700) 
      duve_msg();
  }
/* # line 1324 "duvepurg.sc" */	/* host code */
	    break;
        }
...
/* # line 1349 "duvepurg.sc" */	/* host code */
    ...
    if (has_constraint)
    {
    	...
    	*/
/* # line 1367 "duvepurg.sc" */	/* open */
  {
    IIsqInit(&sqlca);
    IIcsOpen((char *)"rel_cons",6232,12903);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select constraint_name, constraint_type from iiconstraints where tabl\
e_name=");
    IIputdomio((short *)0,1,32,0,tbl_name);
    IIwritio(0,(short *)0,1,32,0,(char *)"and schema_name=");
    IIputdomio((short *)0,1,32,0,owner_name);
    IIcsQuery((char *)"rel_cons",6232,12903);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
    else if (sqlca.sqlcode == 700) 
      duve_msg();
  }
/* # line 1369 "duvepurg.sc" */	/* host code */
    	for ( ;; )
    	{
	    /*
	    ** get entry from iirelation
	    */
/* # line 1374 "duvepurg.sc" */	/* fetch */
  {
    IIsqInit(&sqlca);
    if (IIcsRetScroll((char *)"rel_cons",6232,12903,-1,-1) != 0) {
      IIcsGetio((short *)0,1,32,DB_MAXNAME+1-1,index_name);
      IIcsGetio((short *)0,1,32,1,cons_type);
      IIcsERetrieve();
    } /* IIcsRetrieve */
    if (sqlca.sqlcode < 0) 
      Clean_Up();
...
/* # line 1375 "duvepurg.sc" */	/* host code */
	    if (sqlca.sqlcode == DU_SQL_NONE)
	    {	
/* # line 1377 "duvepurg.sc" */	/* close */
  {
    IIsqInit(&sqlca);
    IIcsClose((char *)"rel_cons",6232,12903);
    if (sqlca.sqlcode < 0) 
      Clean_Up();
    else if (sqlca.sqlcode == 700) 
      duve_msg();
  }
/* # line 1378 "duvepurg.sc" */	/* host code */
	        break;
       	    }
	    switch (cons_type[0])
	    {
...
/* # line 1418 "duvepurg.sc" */	/* host code */
		switch (cons_type[0])
		{
		    case 'U' : 
	        	duve_talk (DUVE_MODESENS, duve_cb, 
    ...
    */
/* # line 1457 "duvepurg.sc" */	/* execute procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(2,(char *)"iiQEF_patch_table");
    IILQprvProcValio((char *)"table_id",0,(short *)0,1,30,4,&table_id);
    IILQprvProcValio((char *)"mode",0,(short *)0,1,32,0,mode);
    IILQprvProcValio((char *)"verbose",0,(short *)0,1,32,0,verbose);
    while (IILQprsProcStatus(0) != 0) {
      if (sqlca.sqlcode == 700) 
        duve_msg();
    } /* IILQprsProcStatus */
  }
/* # line 1460 "duvepurg.sc" */	/* host code */
    if (sqlca.sqlcode < 0)
	Clean_Up();
/* # line 1464 "duvepurg.sc" */	/* host code */
    ...
    */
/* # line 1471 "duvepurg.sc" */	/* update */
  {
    IIsqInit(&sqlca);
    IIwritio(0,(short *)0,1,32,0,(char *)
"update iiattribute set attkdom=0 where attkdom>0 and attrelid=");
    IIputdomio((short *)0,1,30,4,&table_id);
...
/* # line 1474 "duvepurg.sc" */	/* host code */
...
char		*tablename;
{
  ...
  char *cmd, buf[CMDBUFSIZE];
  char verbose[10];
/* # line 1535 "duvepurg.sc" */	/* host code */
    STcopy( tablename, tbl_name);
    if (duve_cb->duve_user[0]=='\0')
	STcopy(duve_cb->duveowner, owner_name);
    else
...
/* # line 1548 "duvepurg.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select reltid, reltidx, relspec, relstat from iirelation where relid=");
    IIputdomio((short *)0,1,32,0,tbl_name);
    IIwritio(0,(short *)0,1,32,0,(char *)"and relowner=");
    IIputdomio((short *)0,1,32,0,owner_name);
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,30,4,&table_id);
      IIgetdomio((short *)0,1,30,4,&index_id);
      IIgetdomio((short *)0,1,30,4,&relspec);
      IIgetdomio((short *)0,1,30,4,&relstat);
    } /* IInextget */
    IIsqFlush((char *)0,0);
    if (sqlca.sqlcode < 0) 
...
/* # line 1551 "duvepurg.sc" */	/* host code */
    if (sqlca.sqlcode == DU_SQL_NONE)
    {       
    ...
    */
/* # line 1571 "duvepurg.sc" */	/* drop procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(1,(char *)"iiQEF_check_table");
    IIwritio(0,(short *)0,1,32,0,(char *)"drop procedure iiQEF_check_t\
able");
    IIsyncup((char *)0,0);
    if (sqlca.sqlcode == 700) 
      duve_msg();
  }
/* # line 1573 "duvepurg.sc" */	/* create procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(0,(char *)"iiQEF_check_table");
    IIwritio(0,(short *)0,1,32,0,(char *)
...
/* # line 1582 "duvepurg.sc" */	/* host code */
    /* set verbose flag appropriately */
    if (duve_cb->duve_verbose)
	STcopy (VERBOSE, verbose);
    else
    ...
    */
/* # line 1593 "duvepurg.sc" */	/* execute procedure */
  {
    IIsqInit(&sqlca);
    IILQpriProcInit(2,(char *)"iiQEF_check_table");
    IILQprvProcValio((char *)"table_id",0,(short *)0,1,30,4,&table_id);
    IILQprvProcValio((char *)"index_id",0,(short *)0,1,30,4,&index_id);
    IILQprvProcValio((char *)"verbose",0,(short *)0,1,32,0,verbose);
    while (IILQprsProcStatus(0) != 0) {
      if (sqlca.sqlcode == 700) 
        duve_msg();
...
/* # line 1596 "duvepurg.sc" */	/* host code */
    if (sqlca.sqlcode < 0)
	Clean_Up();
/* # line 1600 "duvepurg.sc" */	/* host code */
    /* close the DB without aborting */
    duve_close(duve_cb, FALSE);
    return ( (DU_STATUS) DUVE_YES);
}
...
duve_msg()
{
/* # line 1610 "duvepurg.sc" */	/* host code */
    char	*msg_ptr;
    char	underscore = '_';
  ...
  char msg[2048];
  int msgnum;
/* # line 1620 "duvepurg.sc" */	/* inquire_ingres */
  {
    IILQisInqSqlio((short *)0,1,30,4,&msgnum,68);
    IILQisInqSqlio((short *)0,1,32,2047,msg,69);
  }
/* # line 1622 "duvepurg.sc" */	/* host code */
