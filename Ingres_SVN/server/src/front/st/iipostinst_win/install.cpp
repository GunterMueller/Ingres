...
{
	if (!theInstall.m_bWait4PostInstall)
		::SendMessage(HWND_BROADCAST, WM_SETTINGCHANGE, 0L, (LPARAM)"Environment");
	else
		::PostMessage(HWND_BROADCAST, WM_SETTINGCHANGE, 0L, (LPARAM)"Environment");
    return 0;
}
	...
	*/
	...
	else if (dbms && dbms->m_selected)
	{
	    ...
	    */
		...
		/*
		** New config.dat value for Ingres 2006 Release 2, introduces ANSIDATE
		** date_type ANSIDATE scenario is handled automatically by dbms.crs
		** this only takes care of non-default behavior.
		*/
...
CInstallation::SetTemp()
{
    ...
    m_temp.Format("%s", (LPCSTR)temp_path);
}
void
CInstallation::CreateConfigDir()
{
    CString config_path;
    char *path;
    int bret;
    ...
    if (path != NULL)
    {
	config_path.Format("%s\\AppData", (LPCSTR)path);
	if (GetFileAttributes(config_path) != -1)
	{
	      config_path.Format("%s\\AppData\\Local\\Ingres\\files", (LPCSTR)path);
	      if (GetFileAttributes(config_path) == -1)
	      {
 		   bret = CreateDirectoryRecursively(config_path.GetBuffer(MAX_PATH));
	
		   if (!bret)
		   {	
			config_path.Format("%s\\AppData\\Local\\Ingres", (LPCSTR)path);
			if (GetFileAttributes(config_path) == -1)
...
CInstallation::ThreadPostInstallation()
{
    ...
    */
    ...
    /*ConfigKey.Format("ii.%s.ingstart.*.jdbc", (LPCSTR)m_computerName);
    if (Local_PMget(ConfigKey, ConfigKeyValue))
	...
	Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd);
    /*cmd.Format("ii.%s.ingstart.*.jdbc 0", (LPCSTR)m_computerName);
	Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd);
	*/
...
CInstallation::NetPostInstallation()
{
    ...
    */
	...
	if (!bTCPIP && !bWINTCP)
	{
	    cmd.Format("ii.%s.gcc.*.wintcp.status ON", m_computerName);
	    Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd, FALSE);
	    cmd.Format("ii.%s.gcb.*.wintcp.status ON", m_computerName);
	    Exec(m_installPath + "\\ingres\\utility\\iisetres.exe", cmd, FALSE);
...
CInstallation::SetODBCEnvironment()
{
    ...
    if(!RegCreateKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\ODBC\\ODBCINST.INI\\Ingres 3.0", 0, NULL, 
    ...
    if(!RegCreateKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\ODBC\\ODBCINST.INI\\ODBC Drivers", 0, NULL, 
		REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwDisposition))
    {
	RegSetValueEx(hKey, "Ingres 3.0", 0, REG_SZ, (CONST BYTE *)"Installed", strlen("Installed")+1);
	RegCloseKey(hKey);
    }

    WritePrivateProfileString("ODBC 32 bit Drivers", "Ingres 3.0 (32 bit)", "Installed", szODBCINIFileName);
    WritePrivateProfileString("Ingres 3.0 (32 bit)", "Driver", szODBCDriverFileName, szODBCINIFileName);
    WritePrivateProfileString("Ingres 3.0 (32 bit)", "Setup", szODBCSetupFileName, szODBCINIFileName);
    WritePrivateProfileString("Ingres 3.0 (32 bit)", "32Bit", "1", szODBCINIFileName);
