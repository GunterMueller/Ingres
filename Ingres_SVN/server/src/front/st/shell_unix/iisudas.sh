...
$BATCH || prompt "Do you want to continue this setup procedure?" y  || exit 1
      ...
      if (PROG1PRFX)genres $CONFIG_HOST $(PRODLOC)/(PROD2NAME)/files/das.rfm ; then
...
# Create Ingres JDBC driver properties file.
if ( $(PRODLOC)/(PROD2NAME)/bin/(PROG1PRFX)jdbcprop </dev/null >/dev/null 2>&1 ) >/dev/null 2>&1
then
{
...
(PROD1NAME) Data Access Server has been successfully set up in this installation. Please adjust the startup count and check the listen address with the (PROG0PRFX)cbf utility.
