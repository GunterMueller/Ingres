...
# original config.dat and keep the nfs backup
   if [ -f $(PRODLOC)/(PROD2NAME)/files/config.nfs ] ; then
      cp -p $(PRODLOC)/(PROD2NAME)/files/config.nfs \
            $(PRODLOC)/(PROD2NAME)/files/config.dat
   else
      cp -p $(PRODLOC)/(PROD2NAME)/files/config.dat \
            $(PRODLOC)/(PROD2NAME)/files/config.nfs
   fi
fi
...
!
  ...
  if [ "$WRITE_RESPONSE" = "false" ] ; then
   if [ "$SETUP" != "defaults" ] ; then
      ...
      NET_RFM_FILE=$(PRODLOC)/(PROD2NAME)/files/net.rfm
...
!
  ...
  if [ "$WRITE_RESPONSE" = "false" ] ; then
      ...
      SERVER_ID=`eval (PROG3PRFX)_ADMIN=$(PRODLOC)/(PROD2NAME)/files \
...
!
      ...
      (PROG3PRFX)_TEMPORARY=`eval (PROG3PRFX)_ADMIN=$(PRODLOC)/(PROD2NAME)/files \
         (PROG2PRFX)prenv (PROG3PRFX)_TEMPORARY`
      (PROG5PRFX)_EDIT=`eval (PROG3PRFX)_ADMIN=$(PRODLOC)/(PROD2NAME)/files (PROG2PRFX)prenv (PROG5PRFX)_EDIT`
...
!
   ...
   VALUE=`eval (PROG3PRFX)_ADMIN=$(PRODLOC)/(PROD2NAME)/files \
...
!
      ...
      TZ_RULE_MAP=$(PRODLOC)/(PROD2NAME)/files/net.rfm
      if $BATCH ; then
	 if [ "$READ_RESPONSE" = "true" ] ; then
            (PROG3PRFX)_TIMEZONE_NAME=`iiread_response (PROG3PRFX)_TIMEZONE_NAME $RESINFILE`
         fi
         [ -z "$(PROG3PRFX)_TIMEZONE_NAME" ] && 
		(PROG3PRFX)_TIMEZONE_NAME=`eval (PROG3PRFX)_ADMIN=$(PRODLOC)/(PROD2NAME)/files \
...
if [ "$WRITE_RESPONSE" = "false" ] ; then
   if [ "$(PROG3PRFX)_CONFIG_LOCAL" ] ; then
      cp -p $(PRODLOC)/(PROD2NAME)/files/config.dat $CLIENT_ADMIN/config.dat
      cp -p $CLIENT_ADMIN/config.dat $CLIENT_ADMIN/config.$CONFIG_HOST
      mv -f $(PRODLOC)/(PROD2NAME)/files/config.nfs \
            $(PRODLOC)/(PROD2NAME)/files/config.dat
   fi
fi #end of WRITE_RESPONSE mode
   $BATCH || pause
...
The name server failed to start.  See the (PROD1NAME) error log:
	...
	$(PRODLOC)/(PROD2NAME)/files/errlog.log
...
if $NFS_CLIENT ; then
   SERVER_(PROG3PRFX)_INSTALLATION=`eval (PROG3PRFX)_ADMIN=$(PRODLOC)/(PROD2NAME)/files \
      (PROG2PRFX)prenv (PROG3PRFX)_INSTALLATION`
