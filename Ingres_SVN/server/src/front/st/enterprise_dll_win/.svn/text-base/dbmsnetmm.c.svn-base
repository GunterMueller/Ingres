...
ingres_start_ingres_service (MSIHANDLE ihnd)
{
	...
	dwSize=sizeof(tchValue);
	if(MsiGetTargetPath(ihnd, "INSTALLDIR.96721983_F981_11D4_819B_00C04F189633", tchValue, &dwSize)!=ERROR_SUCCESS)
		return ERROR_INSTALL_FAILURE;
	if(tchValue[strlen(tchValue)-1]=='\\')
		tchValue[strlen(tchValue)-1]='\0';
...
ingres_set_shortcuts(MSIHANDLE hInstall)
{
    ...
    *szIngAllUsers=*szIngShortcuts=*szId=*szORFolder=*szLang=0;
    ...
    if (GetFileAttributes(szBuf)!=-1)
	VdbaInstalled=TRUE;
    sprintf(szBuf, "%s\\ingres\\files\\english\\Bookshelf.pdf", szII_SYSTEM);
    if (GetFileAttributes(szBuf)!=-1)
	DocInstalled=TRUE;
	...
	sprintf(szTarget, "%s\\perfmon.exe", szSystemFolder);
	...
	sprintf(szTarget, "%s\\ingres\\files\\english\\Bookshelf.pdf", szII_SYSTEM);
	CreateOneShortcut(szFolder, "Ingres 2006 Bookshelf", szTarget, "",
	                  "", szWorkDir, "Ingres 2006 Bookshelf", 0, 0);
    }
	...
	CreateOneShortcut(szFolder, "Ingres 2006 Getting Started", szTarget, "",
	                  "", szWorkDir, "Ingres 2006 Getting Started", 0, 0);

	sprintf(szTarget, "%s\\ingres\\files\\english\\ing_Linux_GetStart_E.pdf", szII_SYSTEM);
	CreateOneShortcut(szFolder, "Ingres 2006 Getting Started for Linux", szTarget, "",
	                  "", szWorkDir, "Ingres 2006 Getting Started for Linux", 0, 0);

	sprintf(szTarget, "%s\\ingres\\files\\english\\ing_Connectivity_E.pdf", szII_SYSTEM);
	CreateOneShortcut(szFolder, "Ingres 2006 Connectivity Guide", szTarget, "",
	                  "", szWorkDir, "Ingres 2006 Connectivity Guide", 0, 0);

	sprintf(szTarget, "%s\\ingres\\files\\english\\ing_Admin_E.pdf", szII_SYSTEM);
	CreateOneShortcut(szFolder, "Ingres 2006 System Administrator Guide", szTarget, "",
	                  "", szWorkDir, "Ingres 2006 System Administrator Guide", 0, 0);

	sprintf(szTarget, "%s\\ingres\\files\\english\\ing_DB_Admin_E.pdf", szII_SYSTEM);
	CreateOneShortcut(szFolder, "Ingres 2006 Database Administrator Guide", szTarget, "",
	                  "", szWorkDir, "Ingres 2006 Database Administrator Guide", 0, 0);

	sprintf(szTarget, "%s\\ingres\\files\\english\\ing_SQL_Ref_E.pdf", szII_SYSTEM);
	CreateOneShortcut(szFolder, "Ingres 2006 SQL Reference Guide", szTarget, "",
	                  "", szWorkDir, "Ingres 2006 SQL Reference Guide", 0, 0);

	sprintf(szTarget, "%s\\ingres\\files\\english\\ing_Command_E.pdf", szII_SYSTEM);
	CreateOneShortcut(szFolder, "Ingres 2006 Command Reference Guide", szTarget, "",
	                  "", szWorkDir, "Ingres 2006 Command Reference Guide", 0, 0);
    }
...
{
    DWORD dwSize;
    char szIngAllUsers[2], szId[3], szII_SYSTEM[MAX_PATH];
    char szFolder[MAX_PATH], szTarget[MAX_PATH], szArgs[1024], szIconFile[MAX_PATH];
    char szSystemFolder[MAX_PATH];
...
ingres_create_rspfile(MSIHANDLE hInstall)
{
    ...
    cchValue = sizeof(szBuf)/sizeof(TCHAR);
	ZeroMemory(szBuf, sizeof(TCHAR)*MAX_PATH);
	MsiGetProperty(hInstall, TEXT("INGRES_DOC"), szBuf, &cchValue);
    fprintf(fp, "II_LOCATION_DOCUMENTATION=\"%s\"\n\n", szBuf);
...
SetOneEnvironmentVariable(char *szEnv, char *szValue)
{
	...
	if (!RegQueryValueEx(hKey, szEnv, NULL, &dwType,(BYTE *)szTemp, &dwSize))
	{
		...
		strcat(szBuf, ";");
		strcat(szBuf, szTemp);
		RegSetValueEx(hKey, szEnv, NULL, dwType, (BYTE *)szBuf, strlen(szBuf)+1);
	    }
	}
	else
	{
	    RegSetValueEx(hKey, szEnv, NULL, REG_EXPAND_SZ, (BYTE *)szValue, strlen(szValue)+1);
	}
	RegCloseKey(hKey);
    }
}
