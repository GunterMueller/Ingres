...
SetODBCEnvironment(BOOL setup_IngresODBC, BOOL setup_IngresODBCReadOnly, BOOL bRollback)
{
	...
	*szII_SYSTEM=0;
	...
	if(!RegCreateKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\ODBC\\ODBCINST.INI\\Ingres 2006", 0, NULL, 
		REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwDisposition))
	{
		...
		if (RegQueryValueEx(hKey, "UsageCount", NULL, &dwType, (CONST BYTE *)&dwUsageCount, &dwSize))
			dwUsageCount = 1;
		else
		{
	...
	if(!RegCreateKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\ODBC\\ODBCINST.INI\\ODBC Drivers", 0, NULL, 
		REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwDisposition))
	{
		RegSetValueEx(hKey, "Ingres 2006", 0, REG_SZ, (CONST BYTE *)"Installed", strlen("Installed")+1);
		RegCloseKey(hKey);
	}

	WritePrivateProfileString("ODBC 32 bit Drivers", "Ingres 2006 (32 bit)", "Installed", szODBCINIFileName);
	WritePrivateProfileString("Ingres 2006 (32 bit)", "Driver", szODBCDriverFileName, szODBCINIFileName);
	WritePrivateProfileString("Ingres 2006 (32 bit)", "Setup", szODBCSetupFileName, szODBCINIFileName);
	WritePrivateProfileString("Ingres 2006 (32 bit)", "32Bit", "1", szODBCINIFileName);
...
ingres_remove_odbc_files (MSIHANDLE ihnd)
{
    ...
    HINSTANCE	hLib;
    char	cur_id[3], id[3], ii_system[2048], szDriver[2048];
    ...
    */
	...
	if (pSQLRemoveDriver) 
	    pSQLRemoveDriver("Ingres 2006", TRUE, &dwSize);
    }
