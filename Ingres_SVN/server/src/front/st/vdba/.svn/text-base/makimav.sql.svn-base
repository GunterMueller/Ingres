/*
** Table registrations for the IMA Standard Schema
** This contains the registrations for OpenVMS only
**
## History:
##
##	30-Apr-95 (johna)
...
##	04-Dec-1997 (kinte01)
##	    Created a VMS version from makimau.sql. 
##      14-Jan-98 (fanra01)
...
##	15-May-98 (kinte01)
##	    Put in changes from above integration plan that I missed
##	    earlier.
##	03-aug-98 (kinte01)
##	    Cross integrate changes from Unix change 436237
##          18-Jun-98 (fanra01)
...
##	05-aug-2003 (devjo01)
##	    Remove registration of columns lfb_last_lsn, lfb_last_lsn_high,
##	    lfb_last_lsn_low as these fields no longer exist as of change
##	    433417 submitted back on 12-dec-1997. (b110663)
##  28-Feb-2005 (fanra01)
##      Bug 113976
...
register table ima_gcc_info (
    ...
    is 'SERVER',
  inbound_max	      varchar(10) not null not default 
    is 'exp.gcf.gcc.ib_max',
	inbound_current	  varchar(10) not null not default 
    is 'exp.gcf.gcc.ib_conn_count',
	outbound_max	    varchar(10) not null not default 
    is 'exp.gcf.gcc.ob_max',
	outbound_current	varchar(10) not null not default 
...
register table ima_memory_info (
        ...
        bytes_used integer4 not null not default
                is 'exp.clf.vms.me.num.bytes_used',
        free_pages integer4 not null not default
                is 'exp.clf.vms.me.num.free_pages',
        get_pages integer4 not null not default
                is 'exp.clf.vms.me.num.get_pages',
        pages_used integer4 not null not default
                is 'exp.clf.vms.me.num.pages_used'
) as import from 'tables'
with dbms = ima,
structure = sortkeyed,
key = (server);
...
register table ima_block_info (
  ...
  blk_num_sessions integer4 not null not default is
    'exp.clf.vms.cs.srv_block.num_sessions',
  blk_max_sessions integer4 not null not default is
    'exp.clf.vms.cs.srv_block.max_sessions',
  blk_num_active integer4 not null not default is
    'exp.clf.vms.cs.srv_block.num_active',
  blk_max_active integer4 not null not default is
    'exp.clf.vms.cs.srv_block.max_active'
)
as import from 'tables'
