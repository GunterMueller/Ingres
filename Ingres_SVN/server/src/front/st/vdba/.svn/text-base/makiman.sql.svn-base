/*
** Table registrations for the IMA Standard Schema - NT only
**
## History:
##
##	30-Apr-95 (johna)
...
##      04-Nov-97 (fanra01)
##              Modified comment prefix to enable removal during make.
##              Modified ima_server_sessions to use exp.scf.scs.scb_self for
##              session_id.
##      26-Nov-1997 (kinte01)
...
##	04-Dec-1997 (kinte01)
##	    Reversed previous change as this file is NT specific
##      14-Jan-98 (fanra01)
...
##      16-Feb-98 (fanra01)
##              Modified the ima_server_sessions table to use
##              exp.scf.scs.scb_index for session id.
##      19-Oct-98 (fanra01)
##              Add exp.scf.scd.start.name for the server flavour to
##              ima_dbms_server_parameters.
##      04-Nov-98 (wonst02)
##              Added page_size integer4 not null not default
##              is 'exp.dmf.dm0p.bm_pgsize' to ima_dmf_cache_stats.
##      06-Nov-98 (fanra01)
##              Add IMA objects for icesvr.
...
##      12-Mar-1999 (fanra01)
##          Add ice_trace objects for enabling logging and tracing.
##      22-Apr-1999 (wonst02)
##          Remove numusereventsamples & numsyseventsamples from cssampler_stats.
##	23-aug-1999 (somsa01)
...
##	13-oct-2003 (somsa01)
##	    Added usage of scb_self.
##  26-Apr-2004 (fanra01)
##      Sir 111718
##      Add table registrations for instance version and configuration.
##  20-Jul-2004 (noifr01)
##      Sir 111718
...
##  01-Jun-2005 (fanra01)
##      SIR 114614
##      Add bldlevel and system fields to ima_version registration.
##  30-Aug-2005 (fanra01)
##      Bug 115124
...
register table ima_dbms_server_status (
	server varchar(64) not null not default is
		'SERVER',
	average_rows integer4 not null not default is
		'exp.scf.scd.avgrows',
...
register table ima_memory_info (
	...
	bytes_used integer4 not null not default
		is 'exp.clf.nt.me.num.bytes_used',
	free_pages integer4 not null not default
		is 'exp.clf.nt.me.num.free_pages',
	get_pages integer4 not null not default
		is 'exp.clf.nt.me.num.get_pages',
	pages_used integer4 not null not default
		is 'exp.clf.nt.me.num.pages_used'
) as import from 'tables'
with dbms = ima,
structure = sortkeyed,
key = (server);
...
begin
    ...
    */
    /*
    ** At this time the format of the string returned for servid has escaped
    ** backslash characters and the test of server = :servid will fail.
    ** This procedure is included for completeness until a solution is
    ** forthcoming.
    */
...
drop table ice_trace;
\p\g
register table ice_trace (
    server          varchar(64) not null not default is
        'SERVER',
    thread          integer     not null not default is
        'exp.ascf.asct.thread',
    entry           integer     not null not default is
        'exp.ascf.asct.entry',
    state           varchar(30) not null not default is
        'exp.ascf.asct.state',
    maxflush        integer     not null not default is
        'exp.ascf.asct.maxflush',
    trigger         integer     not null not default is
        'exp.ascf.asct.trigger',
    startp          integer     not null not default is
        'exp.ascf.asct.startp',
    endp            integer     not null not default is
        'exp.ascf.asct.endp',
    threshold       integer     not null not default is
        'exp.ascf.asct.threshold',
    logfile         varchar(30) not null not default is
        'exp.ascf.asct.logfile',
    mask            varchar(30) not null not default is
        'exp.ascf.asct.tracemask',
    trc_start       integer     not null not default is
        'exp.ascf.asct.trcstart',
    trc_suspend     integer     not null not default is
        'exp.ascf.asct.trcsuspend',
    trc_flush       integer     not null not default is
        'exp.ascf.asct.trcflush',
    conf_logsize    integer     not null not default is
        'exp.ascf.asct.trclogsize',
    conf_msgsize    integer     not null not default is
        'exp.ascf.asct.trcmsgsize',
    conf_threshold  integer     not null not default is
        'exp.ascf.asct.trcthreshold',
    conf_timeout    integer     not null not default is
        'exp.ascf.asct.trctimeout'
)
as import from 'tables'
with dbms = IMA,
structure = sortkeyed,
