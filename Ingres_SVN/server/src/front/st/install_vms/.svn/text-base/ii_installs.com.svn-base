$ GOTO beginning !		II_INSTALLS.COM
	...
	Richard Troy, Ingres Corp.			March 23, 1993
...
$ IF F$LOCATE(cmd,"REPLACE").EQ.0 THEN sub="replace"
$ IF F$LOCATE(cmd,"SAVE").EQ.0 THEN sub="save"
$ IF F$LOCATE(cmd,"SEARCH").EQ.0 THEN sub="search_file"
$ IF F$LOCATE(cmd,"SHOW").EQ.0 THEN sub="display"
$ IF F$LOCATE(cmd,"SORT").EQ.0 THEN sub="sort_file"
...
Verbs:		ADD		Create a new history entry. Help available.
		...
		REPLACE		Perform a REMOVE and ADD (in place) together
		SAVE		Keep any changes made
		SHOW		Displays all recorded history
		SEARCH		Locate and display specific entries
		SORT		Organizes by location and date
...
$ replace:
$ sort_file:
$ say "%INGRES-I-CMDNOTHERE, Command not here yet!"
$ RETURN
...
$ cmd_list="DISPLAY,SEARCH,REPLACE,ADD,REMOVE,USE,SAVE,SORT,HELP,EXIT,QUIT"
