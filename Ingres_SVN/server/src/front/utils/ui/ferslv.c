...
**	the first FE_MAXNAME bytes (blank padded), and the resolved object name is
**	placed into the second FE_MAXNAME bytes (blank padded), and reflects its
...
**	is assumed that FE_MAXNAME will always be at least that size.
**	
  ...
  char *owner;
{
			...
			FE_RSLV_NAME	w_frn;
			FE_REL_INFO		relinfo;
			char	owner_buf[(FE_MAXNAME + 1)];
  char resolved_name[((2 * FE_MAXNAME) + 1)];
...
*/
		...
		{
			MEfill((2 * FE_MAXNAME),' ',&resolved_name[0]);
			STcopy(relinfo.owner,&resolved_name[0]);
			*(&resolved_name[0] + STlength(relinfo.owner)) = ' ';
			STcopy(relinfo.name,&resolved_name[FE_MAXNAME]);
			*(&resolved_name[FE_MAXNAME] + STlength(relinfo.name)) = ' ';
			*(&resolved_name[0] + (2 * FE_MAXNAME)) = EOS;
		}
	}
	else
	{
...
*/
/* # line 322 "ferslv.sc" */	/* select */
  {
    IIsqInit((char *)0);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)"select resolve_table(");
    ...
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,((2 * FE_MAXNAME) + 1)-1,resolved_name);
    } /* IInextget */
    IIsqFlush((char *)0,0);
  }
/* # line 324 "ferslv.sc" */	/* host code */
	}
/*
...
*/
	...
	** it was moved.
	*/
	i_ptr = &resolved_name[FE_MAXNAME];
	save_end_char = *i_ptr;
	*i_ptr = EOS;
	if  (resolved_name[(FE_MAXNAME - 1)] != ' ')
	{
	...
	*/
	*i_ptr = save_end_char;
	i_ptr = &resolved_name[FE_MAXNAME];	/* Trim the name component	*/
...
*/
	...
	*/
	...
	if  (frn_ptr->is_syn)
	{
                ...
                bool    norml = TRUE;
		name_ptr = &resolved_name[FE_MAXNAME];
	...
	{
		STcopy(&resolved_name[FE_MAXNAME],r_ptr);
	}
	return(TRUE);
}
/*
...
**				least (FE_MAXNAME + 1) bytes, and is_nrml is properly set.
...
**				Assumed to point to a buffer of (FE_MAXNAME + 1) length and
