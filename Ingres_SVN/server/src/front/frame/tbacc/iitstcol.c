...
i4
IItcolset(colname, variable, type, len, data)
char		*colname;			/* table column name	 */
i4		variable;			/* set if isn't constant */
i4		type;
i4		len;
char		*data;				/* can be const or var	 */
{
...
/* VARARGS5 */
i4
IItcoputio(colname, ind, variable, type, len, data)
char		*colname;	/* table column name */
i2		*ind;		/* null indicator */
i4		variable;	/* set if isn't constant */
i4		type;
i4		len;
PTR		data;		/* can be const or var */
{
	...
	*/
	...
	edv.ed_type = type;
	edv.ed_length = len;
	edv.ed_data = (variable || type == DB_CHR_TYPE) ? data : ((PTR) &data );
	edv.ed_null = ind;
