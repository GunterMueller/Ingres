...
TBSTRUCT	*tb;
{
	...
	char			*bufptr;	/* virtual buffer ptr */
	...
	*/
	...
	for (i = 0, cd = rd->r_coldesc; i < rd->r_ncols; i++, cd++)
	{
		...
		*/
		...
		if ( (afe_pad( cb, (PTR) bufptr, cd->c_dbv.db_datatype, &pad) ) )
		{
			/* error */
			return( NULL );
		}
		...
		** valid since bufptr is initialised to NULL before this
		** for loop and then offsets are added to it.
		*/
		cd->c_offset = (i4) bufptr;	/* offset for col data */

		bufptr += cd->c_dbv.db_length;

		/*
		**  Get aligned offset for query operator data value
		*/

		if ( (afe_pad(cb, (PTR) bufptr, op_dbv.db_datatype, &pad) != OK ) )
		{
			/* error */
			return( NULL );
		}
		...
		** for loop and then offsets are added to it.
		*/
		cd->c_qryoffset = (i4) bufptr;

		bufptr += op_dbv.db_length;

		/*
		**  Get aligned offset for change bit.
		*/
		if ((afe_pad(cb, (PTR) bufptr, DB_INT_TYPE, &pad) != OK))
		{
			return(NULL);
		}
		bufptr += pad;
		...
		** for loop and then offsets are added to it.
		*/
		cd->c_chgoffset = (i4) bufptr;
		bufptr += sizeof(i4);	/* Size of flag region. */
