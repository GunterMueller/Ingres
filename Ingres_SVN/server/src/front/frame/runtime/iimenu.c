...
FUNC_EXTERN	i4	IIavchk();
FUNC_EXTERN	i4	IIFRgtoGetTOact();
...
IIactmenu(strvar, exp, val, intrp)
char	*strvar;
char	*exp;
i4	val;
i4	intrp;
...
IInactmenu(strvar, exp, val, act, intrp)
char	*strvar;
char	*exp;
i4	val;
i4	act;
...
IInmuact(strvar, exp, val, act, intrp)
char	*strvar;
char	*exp;
i4	val;
i4	act;
...
i4
IIaddact( acttag, flist, actcode, enactcode, fldhdr )
i2		acttag;
FLD_SAVE	**flist;
i4		actcode;
FLDHDR		*fldhdr;
...
*/
void
IIFRmcMuChg(rfrm, menu_name, to_state)
RUNFRM	*rfrm;
char	*menu_name;
i4	to_state;
{
	...
	for (i = 0; i < maxitems; i++, ct++)
	{
		if (STequal(ct->ct_name, menu_name))
		{
			...
			*/
			...
			/* Check through to see if there are any function
			** keys defined to have the same interrupt value
			** as this menu item.  If there is one, then it
			** has been defined as part of the menu item definition
			** and should be activated/deactivated at the same
			** time (b117462) kibro01
			*/
			for (frsknum = 0;
				(frsknum < MAX_FRS_KEYS) &&
					(IIstkfrm->fdfrskey != NULL);
				frsknum++)
			{
				FRSKY *frskey = &(IIstkfrm->fdfrskey[frsknum]);
				if (frskey && frskey->frs_int == ct->ct_enum)
				{
					frskey->frs_val=to_state?FRS_YES:FRS_NO;
					frskey->frs_act=to_state?FRS_YES:FRS_NO;
					IIresfrsk();
				}
			}
			return;
...
*/
i4
IIRTinqmu(rfrm, mu_name, rtcode, data)
RUNFRM	*rfrm;
char	*mu_name;
i4	rtcode;
i4	*data;
...
*/
i4
IIRTsetmu(rfrm, mu_name, rtcode, data)
RUNFRM	*rfrm;
char	*mu_name;
i4	rtcode;
i4	*data;
