...
typedef struct tagLOBFILEMGT
{
	...
	long nReadCount;        /* The amount of bytes read from the buffer file 'pFileBuffer'*/
	long nWriteCount;       /* The amount of bytes written to the buffer file 'pFileBuffer'*/
...
static long nCopyRec = 0;
static long nCbCopyBlock = 0;
...
int CIRCULARBUFFER_Read (CIRCULARBUFFER* pObj, long nSize,long* lRead, char* pBuffer)
{
	long v, n2Read = 0;
	long k = nSize;
	char* pSource = NULL;
...
void CIRCULARBUFFER_Write (CIRCULARBUFFER* pObj, long nSize, char* pBuffer)
{
	long v, n2Write = 0;
	long k = nSize;
	char* pDest = NULL;
...
int LOBFILEMGT_Read  (LOBFILEMGT* pLobMgt, long nCount, long* lRead, char* pBuffer)
{
	int lRes = CIRCULARBUFFER_Read (&(pLobMgt->circularBuff), nCount, lRead, pBuffer);
	return lRes;
}

void LOBFILEMGT_Write (LOBFILEMGT* pLobMgt, long nCount, char* pBuffer, long* dummy)
{
	CIRCULARBUFFER_Write (&(pLobMgt->circularBuff), nCount, pBuffer);
}

void LOBFILEMGT_SIfseek(LOBFILEMGT* pLobMgt, long nOffset, long nMode)
{
}
...
*/
long LOBFILEMGT_FillBuffer (LOBFILEMGT* pLobMgt, char* lpRow, long nlpRowBufferSize)
{
	STATUS   st;
	LOCATION lf;
	int  nLenSpec, nExit;
...
#endif
	long dummy = 0;
	long lByteWindow = nlpRowBufferSize;
	FIELDSTRUCT* pFieldStruct = &(g_dataLoaderStruct.fields);
...
void CMD_SetReadSize(COMMANDLINE* pCmdStructure, long lReadSize)
{
...
long CMD_GetReadSize(COMMANDLINE* pCmdStructure)
{
...
void DTSTRUCT_SetDiscardMax(DTSTRUCT* pDtStructure, long lDiscard)
{
...
long  DTSTRUCT_GetDiscardMax(DTSTRUCT* pDtStructure){return pDtStructure->lDiscardMax;}
			...
			if (iQuotedAndCopied==0) {
				if (stricmp (strTeminator, "WHITESPACE") != 0)
				{
					...
					if (pFound)
					{
						...
						for (i=0; i<nSepLen; i++)
							CMnext (pFound);
						STlcopy (pch, szField, SI_MAX_TXT_REC-1);
						*pCur = pFound;
					}
					else
					if (pch)
...
static char bufcache[SI_MAX_TXT_REC];
...
{
	STATUS st, sr;
	char szRecBuffer[SI_MAX_TXT_REC+1];
	char szRecBuffer1[SI_MAX_TXT_REC+1];
	char* strLine = NULL;
#if defined (NT_GENERIC)
	char szOpenMode[] = "rb";
#else
	...
	*/
	...
	if (pInfile)
	{
		...
		st = SIfopen (&lf, szOpenMode, SI_TXT, SI_MAX_TXT_REC, &f);
		if (st == OK)
		{
			FIELDSTRUCT* pField = &(pDataStructure->fields);
			/*
			...
			*/
			...
			long lFix = DTSTRUCT_GetFixLength(pDataStructure, pDataStructure->nCurrent);
			/*
...
*/
long GetRecordLine (long* rowLength, char* lpRow, long* returnByte)
{
	int   nAddedNL  = 0;
	int   nFinished = 0;
	char* strRecord = NULL;
	...
	*/
	...
	{
		long lRequest = *rowLength;
		long lReturnBytes = LOBFILEMGT_FillBuffer (pLobfileManageMent, lpRow, lRequest);
		*returnByte = 0;
		if (lReturnBytes == -1)
		{
			g_nCriticalError = 1;
...
** Writer thread:
*/
static long lCopiedRows = 0;
static long lTotalCopiedRows = 0;
int StartTransferDataThread(DTSTRUCT* pDataStructure)
{
		...
		*/
		if (strBadFile && access(strBadFile, 0) != -1)
		{
			...
			if (!tmpFile)
			{
				...
				*/ 
				st = SIfopen (&lf, "w", SI_TXT, SI_MAX_TXT_REC, &tmpFile);
			}
			...
			** in the Compatlib documentation
			*/ 
			st = SIfopen (&lf, "r", SI_TXT, SI_MAX_TXT_REC, &f1);
			if (st == OK && tmpFile && f1)
			{
				int nDumy =0;
				while (1)
				{
					st = SIread (f1, SI_MAX_TXT_REC, &lRead, szRecBuffer);
	...
	*/
	if (tmpFile && strBadFile)
	{
		...
		** in the Compatlib documentation
		*/ 
		st = SIfopen (&lf, "r", SI_TXT, SI_MAX_TXT_REC, &tmpFile);
		if (st == OK && tmpFile)
		{
			LOfroms (PATH&FILENAME, strBadFile, &lf);
			/*
			...
			** in the Compatlib documentation
			*/ 
			st = SIfopen (&lf, "w", SI_TXT, SI_MAX_TXT_REC, &f);
			if (st == OK)
			{
				while (1)
				{
					st = SIread (tmpFile, SI_MAX_TXT_REC, &lRead, szRecBuffer);
					if (st == ENDFILE && lRead == 0)
						break;
