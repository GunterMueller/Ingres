...
long  g_lReadSize = READSIZE_DEF;
long  g_lRecordRead = 0;
int   g_nGetRecordLineIvoked = 0;
long  g_lCpuStarted = 0;
long  g_lCpuEnded = 0;
	...
	*/
	...
	if (szCommandLineString[0])
	{
		...
		*/
		...
		st = SIfopen (&lf, "w", SI_TXT, SI_MAX_TXT_REC, &g_fLogFile);
		if (st != OK)
		{
			g_fLogFile = NULL;
	...
	{
		double ds = 0.0, d1;
		long lms = g_lCpuEnded - g_lCpuStarted;
		while (lms >=1000)
		{
			lms -= 1000;
			ds += 1.0;
