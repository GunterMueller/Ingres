...
int StartReplicServer (int hdl,  LPREPLICSERVERDATAMIN lpReplicServerDta )
{
	...
	if ( strVName.CompareNoCase(strLoc) == 0)
	{
		...
		if (!IsServiceInstalled ((LPCTSTR)strReplicServiceName)) 
		{
			...
			int iansw = AfxMessageBox (msg, MB_YESNO|MB_ICONQUESTION);
			if (iansw == IDNO)
				return RES_ERR;
			CString Cmd;
			...
			if (!IsServiceRunning(strIngresServiceName)) {
				...
				if (iansw == IDYES) {
					if (!SetServicePassword ((LPCTSTR) strIngresServiceName, ServiceParms)) {
						//msg.Format("Failure in user/password setup for the Ingres service");
						AfxMessageBox (IDS_E_USER_PASSWORD);
						return RES_ERR;
					}
				}
				return RES_ERR;
