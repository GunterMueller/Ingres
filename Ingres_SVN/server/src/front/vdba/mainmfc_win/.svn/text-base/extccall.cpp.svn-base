...
extern "C" LPTSTR CreateSelectStmWizard(HWND hwndOwner, LPTSTR lpszNode, LPTSTR lpszDatabase)
{
	USES_CONVERSION;
	...
	UCHAR szBuffer[200];
	char* node = T2A(lpszNode);
	szBuffer[0] = '\0';

	if (GetGWClassNameFromString((LPUCHAR)node, szBuffer))
	{
		if (szBuffer[0])
			wcscpy (iiparam.wczServerClass, T2W((LPTSTR)szBuffer));
		RemoveGWNameFromString((LPUCHAR)node);
	}

	szBuffer[0] = '\0';
	if(GetConnectUserFromString((LPUCHAR)node, szBuffer))
	{
		if (szBuffer[0])
			wcscpy (iiparam.wczUser, T2W((LPTSTR)szBuffer));
		RemoveConnectUserFromString((LPUCHAR)node);
	}

	wcscpy (iiparam.wczNode, T2W(node));
	wcscpy (iiparam.wczDatabase, T2W(lpszDatabase));
	wcscpy (iiparam.wczSessionDescription, L"Ingres Visual DBA invokes SQL Assistant");

	if (IsStarDatabase(GetCurMdiNodeHandle(), (LPUCHAR)lpszDatabase))
...
extern "C" LPTSTR CreateSQLWizard (HWND hwndOwner, LPTSTR lpszNode, LPTSTR lpszDatabase)
{
	USES_CONVERSION;
	SQLASSISTANTSTRUCT iiparam;
	memset(&iiparam, 0, sizeof(iiparam));
...
extern "C" LPTSTR CreateSearchCondWizard(HWND hwndOwner, LPTSTR lpszNode, LPTSTR lpszDatabase, TOBJECTLIST* pTableList, TOBJECTLIST* pListCol)
{
	USES_CONVERSION;
	SQLASSISTANTSTRUCT iiparam;
	memset(&iiparam, 0, sizeof(iiparam));
