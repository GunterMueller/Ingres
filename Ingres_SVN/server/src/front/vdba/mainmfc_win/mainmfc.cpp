...
//
// The 3 lines bellow are inserted instead of including "libguids.h" in ..\inc
...
BOOL CMainMfcApp::InitInstance()
{
  ...
  if (cmdParse.IsUnicenterDriven()) {
	...
	*/
	...
	UINT nIDSErrorIISystem = IDS_MSG_II_SYSTEM_NOT_DEFINED;
	CString strSvriia = _T("svriia.dll");
#if defined (MAINWIN)
	#if defined(hpb_us5)
	strSvriia = _T("libsvriia.sl");
	#else
	strSvriia = _T("libsvriia.so");
	#endif
#endif
#if defined (EDBC)
	nIDSErrorIISystem = IDS_MSG_EDBC_ROOT_NOT_DEFINED;
#endif
	...
	if (!LIBGUIDS_IsCLSIDRegistered(cstrCLSID_IMPxEXPxASSISTANCT))
	{
		...
		case 2:
			...
			strMsg.LoadString (nIDSErrorIISystem);
...
void CMainMfcApp::OnAppAbout()
{
	...
	if (bOK)
	{
		...
		else
		{
			CString strTitle;
			CString strAbout;
			/* parse "II 2.6/0301 (int.wnt/86)" style string from gv.h */
			/* to generate the version string and year */
			...
			int year = COPYRIGHT_YEAR;
			/* BUG #113271 / ISSUE #13751480 -- The string GV_VER does not contain 
			* information about Year anymore
			CString strTmp =strVer.Mid(3,1);
			if (strTmp.Compare(_T("/"))==0) {
				strTmp = strVer.Mid(7,2);
				int year2= atoi((char*)(LPCTSTR)strTmp);
				if (year2>0)
					year = 2000+ year2;
			}
			*/
