...
CxDlgCreateAlterDatabase::CxDlgCreateAlterDatabase(CWnd* pParent /*=NULL*/)
	: CDialog(CxDlgCreateAlterDatabase::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CxDlgCreateAlterDatabase, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_CHECK_UNICODE, OnBnClickedCheckUnicode)
	ON_BN_CLICKED(IDC_CHECK_UNICODE2, OnBnClickedCheckUnicode2)
END_MESSAGE_MAP()
...
BOOL CxDlgCreateAlterDatabase::OnInitDialog() 
{
	...
	if (!TabTitleOne.LoadString(IDS_TAB_TITLE_ONE) ||//"Primary"
		!TabTitleTwo.LoadString(IDS_TAB_TITLE_TWO))  //"Extend"
		return TRUE;
	...
	if (!OccupyProdutsControl() || !OccupyLocationsControl())
	{
		ASSERT(NULL);
		EndDialog (-1);
		return TRUE;
			...
			for (i=0; i<nCount; i++)
			{
				...
				switch (dwtype)
				{
					...
					case PROD_INGRES_DBD :
						if (m_StructDatabaseInfo->szCatOIToolsAvailables[TOOLS_INGRESDBD]=='Y')
							bCheck = TRUE;
						break;
				}
				if (bCheck) {
	...
	{
		m_ctrlStaticCoordName.ShowWindow(SW_HIDE);
		m_ctrlCoordName.ShowWindow(SW_HIDE);
...
void CxDlgCreateAlterDatabase::OnOK() 
{
	...
	else
	{
		...
		BOOL newPublicStatus  = m_ctrlPublic.GetCheck()?TRUE:FALSE;
		if	( newPublicStatus == oldPrivateStatus )
		{
			ChangeGlobalAccessStatus(newPublicStatus);
		}
		int nIndex = m_CtrlCatalogsPageSize.GetCurSel();
		if ( nIndex != LB_ERR )
		{
			long lCurPgSize,lOldPgSize;
			lCurPgSize = m_CtrlCatalogsPageSize.GetItemData(nIndex);
...
BOOL CxDlgCreateAlterDatabase::OccupyProdutsControl()
{
	int i=0;
	LPCTLDATA lpdata;
...
void CxDlgCreateAlterDatabase::OnDistributed() 
{
	...
	else
	{
		...
		m_ctrlCoordName.ShowWindow(SW_HIDE);
		CButton*  cbPrivate = (CButton*)GetDlgItem (IDC_PRIVATE);
		cbPrivate->ShowWindow(SW_SHOW);
	}
}

void CxDlgCreateAlterDatabase::OnGenerateFrontEnd() 
{
...
void CxDlgCreateAlterDatabase::UpdateCoordName()
{
	CString csBufDBname,csBuffSet;
	m_ctrlDatabaseName.GetWindowText( csBufDBname );
...
void CxDlgCreateAlterDatabase::FillStructureWithCtrlInfo()
{
	...
	if ( m_ctrlLanguage.GetWindowTextLength() != 0)
	{
		CString csTempo;
		m_ctrlLanguage.GetWindowText( csTempo );
		lstrcpy ((char *)m_StructDatabaseInfo->Language, csTempo);
	}

	// Star
	if (m_ctrlDistributed.GetCheck() == 1)
	{
		CString csTempo;
		m_StructDatabaseInfo->bDistributed = TRUE;
		m_ctrlCoordName.GetWindowText(csTempo);
	...
	if (m_ctrlGenerateFrontEnd.GetCheck() == 1)
	...
	if ( nCount != 0 )
	{
		...
		{
			if (m_CheckFrontEndList.GetCheck (i))
			{
	...
	// Catalogs Page Size
	nIndex = m_CtrlCatalogsPageSize.GetCurSel();
...
void CxDlgCreateAlterDatabase::ExecuteRemoteCommand()
{
	...
	if (m_ctrlGenerateFrontEnd.GetCheck() == 1)
	{
		...
		for (i=0; i<nCount; i++)
		{
			if (m_CheckFrontEndList.IsItemEnabled(i) && 
				m_CheckFrontEndList.GetCheck(i) == 1)
			{
				char szName[MAXOBJECTNAME];
				DWORD dwtype = m_CheckFrontEndList.GetItemData(i);
				if (bFirst)
...
void CxDlgCreateAlterDatabase::OnReadOnly() 
{
	if ( m_cbReadOnly.GetCheck() == BST_CHECKED )
	{
		...
		m_ctrlGenerateFrontEnd.EnableWindow(FALSE);
		OnGenerateFrontEnd();
		m_pDlgcreateDBLocation->m_ctrlLocDatabase.EnableWindow(TRUE);
		m_pDlgcreateDBLocation->m_ctrlLocWork.EnableWindow(FALSE);
		m_pDlgcreateDBLocation->m_ctrlLocWork.SetCurSel(0);
		m_pDlgcreateDBLocation->m_ctrlLocJournal.EnableWindow(FALSE);
		m_pDlgcreateDBLocation->m_ctrlLocJournal.SetCurSel(0);
		m_pDlgcreateDBLocation->m_ctrlLocCheckPoint.EnableWindow(FALSE);
		m_pDlgcreateDBLocation->m_ctrlLocCheckPoint.SetCurSel(0);
		m_pDlgcreateDBLocation->m_ctrlLocDump.EnableWindow(FALSE);
		m_pDlgcreateDBLocation->m_ctrlLocDump.SetCurSel(0);
	...
	else
	{
		...
		OnGenerateFrontEnd();
		m_pDlgcreateDBLocation->m_ctrlLocDatabase.EnableWindow(TRUE);
		m_pDlgcreateDBLocation->m_ctrlLocWork.EnableWindow(TRUE);
		m_pDlgcreateDBLocation->m_ctrlLocJournal.EnableWindow(TRUE);
		m_pDlgcreateDBLocation->m_ctrlLocCheckPoint.EnableWindow(TRUE);
		m_pDlgcreateDBLocation->m_ctrlLocDump.EnableWindow(TRUE);
...
void CxDlgCreateAlterDatabase::InitPageSize (LONG lPage_size)
{
	int i, index,nMax;
...
void CxDlgCreateAlterDatabase::OnBnClickedCheckUnicode()
{
	// TODO: Add your control notification handler code here
	if (m_ctrlUnicodeEnable.GetCheck() == BST_CHECKED)
		m_ctrlUnicodeEnable2.EnableWindow(FALSE);
	if (m_ctrlUnicodeEnable.GetCheck() == BST_UNCHECKED)
		m_ctrlUnicodeEnable2.EnableWindow(TRUE);
}

void CxDlgCreateAlterDatabase::OnBnClickedCheckUnicode2()
{
	// TODO: Add your control notification handler code here
	if (m_ctrlUnicodeEnable2.GetCheck() == BST_CHECKED)
		m_ctrlUnicodeEnable.EnableWindow(FALSE);
	if (m_ctrlUnicodeEnable2.GetCheck() == BST_UNCHECKED)
