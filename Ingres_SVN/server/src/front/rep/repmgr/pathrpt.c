...
s where local_db=1");
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,64,db);
    } /* IInextget */
    IIsqFlush((char *)0,0);
  }
/* # line 99 "pathrpt.sc" */	/* host code */
	if (RPdb_error_check(DBEC_SINGLE_ROW, &errinfo))
	{
		IIUGerr(E_RM0030_Err_retrieve_db_name, UG_ERR_ERROR, 0);
		goto rpt_err;
	}
/* # line 105 "pathrpt.sc" */	/* select */
  {
    IIsqInit(&sqlca);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select c.cdds_no, s.database_no, l.database_no, t.database_no, TRIM(s\
.vnode_name) +'::' +TRIM(s.database_name), TRIM(l.vnode_name) +'::' +T\
RIM(l.database_name), TRIM(t.vnode_name) +'::' +TRIM(t.database_name),\
 c.cdds_name from dd_databases s, dd_databases");
    IIwritio(0,(short *)0,1,32,0,(char *)
" l, dd_databases t, dd_cdds c, dd_paths p where p.cdds_no=c.cdds_no a\
nd p.sourcedb=s.database_no and p.localdb=l.database_no and p.targetdb\
=t.database_no order by 1, 2, 3, 4");
    IIretinit((char *)0,0);
    if (IIerrtest() != 0) goto IIrtE1;
IIrtB1:
    while (IInextget() != 0) {
      IIgetdomio((short *)0,1,30,2,&cdds_no);
      IIgetdomio((short *)0,1,30,2,&source_no);
      IIgetdomio((short *)0,1,30,2,&local_no);
      IIgetdomio((short *)0,1,30,2,&target_no);
      IIgetdomio((short *)0,1,32,64,source_db);
      IIgetdomio((short *)0,1,32,64,local_db);
      IIgetdomio((short *)0,1,32,64,target_db);
      IIgetdomio((short *)0,1,32,DB_MAXNAME+1-1,cdds_name);
      if (IIerrtest() != 0) goto IIrtB1;
/* # line 123 "pathrpt.sc" */	/* host code */
		if (first)
...
/* # line 168 "pathrpt.sc" */	/* commit */
  {
    IIsqInit(&sqlca);
    IIxact(3);
  }
/* # line 169 "pathrpt.sc" */	/* host code */
	SIclose(report_fp);
	return (OK);
rpt_err:
/* # line 174 "pathrpt.sc" */	/* host code */
	status = -sqlca.sqlcode;
/* # line 175 "pathrpt.sc" */	/* rollback */
  {
    IIsqInit(&sqlca);
    IIxact(2);
  }
/* # line 176 "pathrpt.sc" */	/* host code */
	SIclose(report_fp);
	return (status);
}
