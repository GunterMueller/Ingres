...
/* # line 95 "repevent.sc" */	/* execute */
  {
    IIsqInit(&sqlca);
    IIsqExImmed(stmt);
    IIsyncup((char *)0,0);
  }
/* # line 96 "repevent.sc" */	/* host code */
...
/* # line 107 "repevent.sc" */	/* execute */
  {
    IIsqInit(&sqlca);
    IIsqExImmed(stmt);
    IIsyncup((char *)0,0);
  }
/* # line 108 "repevent.sc" */	/* host code */
			RPdb_error_check(0, &errinfo);
			if (errinfo.errorno)
				return (FAIL);
		}
...
drop_events()
{
  ...
  char stmt[128];
	...
	for (i = 0; i < DIM(general_events); ++i)
	{
		...
		STcat(stmt, general_events[i]);
/* # line 146 "repevent.sc" */	/* execute */
  {
    IIsqInit(&sqlca);
    IIsqExImmed(stmt);
    IIsyncup((char *)0,0);
  }
/* # line 147 "repevent.sc" */	/* host code */
		RPdb_error_check(0, &errinfo);
	...
	for (i = 0; i < DIM(per_server_events); ++i)
	{
		for (j = 1; j <= MAX_SERVERS; ++j)
		{
			STprintf(stmt, ERx("DROP DBEVENT %s%d"),
				per_server_events[i], j);
/* # line 158 "repevent.sc" */	/* execute */
  {
    IIsqInit(&sqlca);
    IIsqExImmed(stmt);
    IIsyncup((char *)0,0);
  }
/* # line 159 "repevent.sc" */	/* host code */
			RPdb_error_check(0, &errinfo);
			if (errinfo.errorno && errinfo.errorno != GE_NOT_FOUND)
				return (FAIL);
		}
