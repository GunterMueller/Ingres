...
static		char	dc_lname[FE_MAXNAME+1]	ZERO_FILL;  /* which language,
							    ** fortran, 
							    ** cobol, etc */
...
void	dc_c(), dc_fortran(), dc_pascal(), dc_pl1(), dc_cobol(), dc_ada(), 
	dc_basic();
...
dc_dumpdesc()
{
    ...
    char	 alias_column[FE_MAXNAME + 1];
    char	*name_ptr;
    i4	adflength;	
    DB_DT_ID	datatype;
...
char	*lang;
{
	...
	{ERx("pl1"),		2,	L_ALL,	dc_pl1},
	{ERx("pli"),		2,	L_ALL,	dc_pl1},
	{(char *)0,		0,	0,	0}
    };
...
** Defines:	dc_ada, dc_basic, dc_c, dc_cobol, dc_fortran, dc_pascal, dc_pl1
...
char	*str;
{
	...
	return;
      case LgenSTRUCT:
	/* Structure head */
	SIfprintf( df, ERx("%sdcl 1 %s,\n"), dc_pp, dc_strucname );
	for (i = 0, da = dc_attab; i < dc_attcnt; i++, da++)
	{
	    SIfprintf( df, ERx("%s\t2 %s\t"), dc_pp, da->atname );
	    switch (da->atadftype)
	    {
	      case DB_INT_TYPE:
		SIfprintf( df, ERx("fixed binary") );
		if (da->atexsize == sizeof(i4))
		    SIfprintf( df, ERx("(31)") );
		else if (da->atexsize == sizeof(i2))
		    SIfprintf( df, ERx("(15)") );
		...
		break;
	      ...
	      case DB_LVCH_TYPE:
	      case DB_LBIT_TYPE:
		SIfprintf( df, ERx("character(%d) varying"), da->atexsize );
		break;
	      case DB_MNY_TYPE:
		SIfprintf( df, ERx("float binary") );
		SIfprintf( df, ERx("(53)") );
		...
		prec = (i4) DB_P_DECODE_MACRO((i2) da->atexsize);
		scale = (i4) DB_S_DECODE_MACRO((i2) da->atexsize);
		SIfprintf( df, ERx("fixed decimal(%d,%d)"), prec, scale);
		break;
