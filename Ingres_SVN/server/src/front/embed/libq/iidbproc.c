...
static VOID             IIproc_send( II_LBQ_CB *, IIDB_PROC_ID * );
...
char	*proc_name;
{
    ...
    */
    ...
    if ( gcatype == GCA_INVPROC  &&
	 IIlbqcb->ii_lq_gca->cgc_version >= GCA_PROTOCOL_LEVEL_60 )
    {
      /* new, improved message type */
      gcatype = GCA1_INVPROC;
    }
    else
    {
      /* reset to old message type */
      procs[IIPROC_EXEC].gcatype = GCA_INVPROC;
    }
    ...
    if (IIlbqcb->ii_lq_gca->cgc_version < GCA_PROTOCOL_LEVEL_60 )
    {
        ...
        */
        ...
        CVlower(name_buf);
        MEmove(name_len, name_buf, ' ', GCA_MAXNAME, proc_id.gca_name);
    ...
    { /* new improved protocol */
        ...
        IIUGgci_getcid(proc_name, &ownproc, TRUE);
        ...
        */
        ...
        proc_p = IIproc_find( IIlbqcb, &ownproc );
        if (proc_p == NULL)
        {
            /* new procedure */
            proc_p = IIproc_add( IIlbqcb, &ownproc );
        }
        IIproc_send( IIlbqcb, proc_p );
    }
...
PTR		addr2;		/* Only used for f8 by value */
{
    ...
    IIputdomio(indaddr, isref, type, length, addr1, addr2);
} /* IILQprvProcValio */

/*{
...
# ifdef II_DEBUG
    ...
    */
      ...
      if (IIdebug)
      {
    	if (STbcompare(IIlbqcb->ii_lq_curproc->gca_id_proc.gca_name, 
...
**      do then return a pointer to it.  
**
...
/*{
+*  Name: IIproc_send - Send a DB-procedure id to the DBMS.
**
**  Description:
**      Given a DB-procedure id (probably found via IIproc_find or IIproc_add)
**      send the id to the DBMS in the order described in the definition of
**      IIDB_PROC_ID.
**
**  Inputs:
**	IIlbqcb		Current session control block.
**      proc_p - IIDB_PROC_ID - DB-procedure id.
**
**  Outputs:
**      Returns:
**          Nothing
...
static VOID
IIproc_send( II_LBQ_CB *IIlbqcb, IIDB_PROC_ID *proc_p )
{
    DB_DATA_VALUE       dbv;

    IIqid_send ( IIlbqcb, &proc_p->gca_id_proc );   /* send the GCA_ID part */

    II_DB_SCAL_MACRO(dbv, DB_CHA_TYPE,
                     STlength(proc_p->gca_proc_own),
                     proc_p->gca_proc_own);
    IIcgc_put_msg(IIlbqcb->ii_lq_gca, FALSE, IICGC_VNAME, &dbv);
