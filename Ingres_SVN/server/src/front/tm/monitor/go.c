...
FUNC_EXTERN struct qbuf *q_ropen();
FUNC_EXTERN char	macgetch();
	    ...
	    */
                ...
                if (Tm_qrb->error == 0 || Tm_qrb->severity == 0) 
		{
		    /*** If \suppress was done, skip row output ***/
	    	    ...
	    	    if ((Tm_suppress == TRUE) && (Tm_qrb->step == 3))
	    ...
	    if (Tm_qrb->endtrans)
	    {
/* # line 228 "go.sc" */	/* commit */
  {
    IIsqInit((char *)0);
    IIxact(3);
  }
/* # line 229 "go.sc" */	/* host code */
	    }
	    qrrelease(Tm_qrb);
	    Tm_qrb = NULL;
	}
