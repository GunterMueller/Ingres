...
FUNC_EXTERN	bool	qrtokcompare();
FUNC_EXTERN	bool	qrtokend();
...
register QRB *qrb;
{
	...
	DB_STATUS   rval;
	char	    type[FE_MAXNAME+1];	    /* DB object type:e.g, tbl,view */
        qrb->norowmsg = FALSE;          /* Bug 4816 */
	/*
	** The following printf format must match the current value
	** for FE_MAXNAME for both 'name' and 'owner'.
	*/
	qrprintf(qrb, ERx("%-*s %-*s %-s\n"),
		(i4)(FE_MAXNAME + 2), ERget(F_QR0053_name),
		(i4)(FE_MAXNAME + 2), ERget(F_QR0054_owner), ERget(F_QR0055_type));
	qrprintf(qrb, ERx("\n"));
	/*
