...
PTR	arg;
{
  ...
  i2 flags_ind;
	...
	*/
	...
	IIUIbeginXaction();
/* # line 477 "iamdmchk.sc" */	/* repeat select */
  {
    IIsexec();
    while (IInexec() == 0) {
      IIsqInit((char *)0);
      IIexExec(1,(char *)"iamdmchk1",6610,8171);
      ...
      if (IInexec() == 0) {
        IIsqInit((char *)0);
        IIsqMods(1);
        IIexDefine(1,(char *)"iamdmchk1",6610,8171);
        IIwritio(0,(short *)0,1,32,0,(char *)
"select abf_flags from ii_objects o, ii_abfobjects a where o.object_id\
=a.object_id and a.object_id=");
        ...
        IIputdomio((short *)0,1,30,4,&applid);
        IIexDefine(0,(char *)"iamdmchk1",6610,8171);
        if (IIerrtest() == 0) {
          IIsexec();
        } /* IIerrtest */
      } /* IInexec */
    ...
    if (IInextget() != 0) {
      IIgetdomio(&flags_ind,1,30,4,&flags);
    } /* IInextget */
    IIsqFlush((char *)0,0);
  }
/* # line 485 "iamdmchk.sc" */	/* host code */
	/* 
	** Quit now if :
	** 1. The select failed,
...
/* # line 504 "iamdmchk.sc" */	/* update */
  {
    IIsqInit((char *)0);
    IIwritio(0,(short *)0,1,32,0,(char *)
"update ii_abfobjects set abf_flags=");
...
/* # line 508 "iamdmchk.sc" */	/* host code */
	iiuicw0_CatWriteOff();
	status = FEinqerr();
	IIUIendXaction();		
	return OK;
