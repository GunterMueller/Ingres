	...
	*/
	...
	if ( SIfopen(&loc, ERx("r"), SI_VAR, 4096,
       ...
       if(STcompare(IIUIscl_StdCatLevel(), UI_LEVEL_902) >=0)
              numofbytes = FE_MAXNAME + 1 + sizeof(i8) + 2*sizeof(i4);
       else
              numofbytes = FE_MAXNAME + 1 + 3*sizeof(i4);
			...
			bytcnt <= numofbytes )
	{
		...
		char			rvar[FE_MAXNAME + 1];
                i4                      dataSize;
		MEcopy(infoptr, FE_MAXNAME + 1, rvar);
		STtrmwhite(rvar);
                if(STcompare(IIUIscl_StdCatLevel(), UI_LEVEL_902) >=0)
                        dataSize = sizeof(i8);
                else 
                        dataSize = sizeof(i4);
		MEcopy((PTR) (infoptr+(FE_MAXNAME+1)), dataSize,
			(PTR) &rectid);
		MEcopy((PTR) (infoptr+(FE_MAXNAME+1)+dataSize),
			sizeof(i4), (PTR) &recstat
		);
		MEcopy((PTR) (infoptr+(FE_MAXNAME+1)+dataSize+sizeof(i4)),
			sizeof(i4), (PTR) &recfirst
		);
#ifdef DEBUG
	if (mq_debug)
	{
		...
		{
			tabl = qdef->mqtabs[i];
			if ( STequal(tabl->mqrangevar, rvar) )
			{
		...
		if (tfound)
		{
					...
					) == SUCCEED )
			{
				...
				if ( status != OK )
				{
					...
					if ( p == 's' || p == 'c' )
					{
/* # line 298 "wrtupd.qsc" */	/* message */
  {
    IImessage(ERget(S_QF00C6_WritingData));
  }
/* # line 299 "wrtupd.qsc" */	/* host code */
						skip = 1;
					}
					else if ( p == 'q' )
					{
...
*/
	if ( ( onetbl || mqnummasters+mqnumdetails == 1) && !tblfield )
	{
		...
		{
/* # line 387 "wrtupd.qsc" */	/* message */
  {
    IImessage(ERget(S_QF00CA_ChangesSaved));
  }
/* # line 388 "wrtupd.qsc" */	/* sleep */
  {
    IIsleep(3);
  }
/* # line 389 "wrtupd.qsc" */	/* host code */
		}
	}
	else if (goterr)
	{
	...
	{
/* # line 397 "wrtupd.qsc" */	/* message */
  {
    IImessage(ERget(S_QF00CA_ChangesSaved));
  }
/* # line 398 "wrtupd.qsc" */	/* sleep */
  {
    IIsleep(3);
  }
/* # line 399 "wrtupd.qsc" */	/* host code */
	}
	return TRUE;
restartupd:
/*
...
/* # line 421 "wrtupd.sc" */	/* rollback */
  {
    IIsqInit((char *)0);
    IIxact(2);
  }
/* # line 422 "wrtupd.sc" */	/* host code */
		return 'q';
	}
	else for (;;)
    {
      char bufr[36];
/* # line 434 "wrtupd.qsc" */	/* prompt */
      {
        IIprmptio(0,
        ERget(qdef_type == 0 ? S_QF00CE_SkipCurrentMasterQ
						: S_QF00CF_ContinueProcessQ),(short *)0,1,32,35,bufr);
      }
/* # line 438 "wrtupd.qsc" */	/* host code */
		CVlower(bufr);
