...
char	**argv;
{
    ...
    */
	...
	while (FEutaget(ERx("colname"), colcnt, FARG_FAIL, &rarg, &pos) == OK)
	  ...
	  if (xfcnvtabname(rarg.dat.name, cvtabname, FE_MAXNAME) == OK)
	  {
...
/* # line 358 "ctoumain.sc" */	/* host code */
    }
    if (automodify_on)
    {
    ...
    xfcrscript(IIUIdbdata()->suser, progname, portable,
