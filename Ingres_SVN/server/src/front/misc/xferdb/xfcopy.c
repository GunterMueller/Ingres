...
xftables(i4 *tcount, i4 output_flags, i4 output_flags2, XF_TABINFO **list_ptr)
{
    ...
    */
    ...
    if (output_flags2 & XF_CONVTOUNI)
    {
      ...
      xfindexes(output_flags, (output_flags2 | XF_NOCREATE), tlist);
    }
	...
	*/
    ...
    if (!output_flags || ((output_flags & XF_INDEXES_ONLY )
        || (output_flags & XF_PRINT_TOTAL)
        || (output_flags2 & XF_CONVTOUNI )) )
    xfindexes(output_flags, output_flags2, tlist);
...
writecreate(XF_TABINFO	*tp, i4 output_flags)
{
    ...
    */
    ...
    if (tp->pagesize > XF_COMPAT_PGSIZE)
        writewith(ERx("page_size"), tp->pagesize, &firstmodatt, Xf_in);
...
static char	Last_owner[FE_MAXNAME + 1] = ERx("");
