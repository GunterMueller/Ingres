...
i4	output_flags2;
{
    ...
    */
    ...
    if (t->pagesize > XF_COMPAT_PGSIZE)
  ...
  XF_TABINFO *tp;
{
  ...
  char year_buf[AFE_DATESIZE + 1];
	i4 year;
        /* 
	** convert date into MM DD YYYY format to produce a "SAVE UNTIL" 
	** statement.  Make the server do the work for us.  This is OK,
	** since I believe that SAVE UNTILs are very rare.
	*/
/* # line 471 "xfphys.sc" */	/* select */
  {
    IIsqInit((char *)0);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select text(date_part('month', date(gmt_timestamp(");
    IIputdomio((short *)0,1,30,4,&tp->expire_date);
    IIwritio(0,(short *)0,1,32,0,(char *)
")))) +' ' +text(date_part('day', date(gmt_timestamp(");
    IIputdomio((short *)0,1,30,4,&tp->expire_date);
    IIwritio(0,(short *)0,1,32,0,(char *)")))) +' '");
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,AFE_DATESIZE + 1-1,date_buf);
    } /* IInextget */
    IIsqFlush((char *)0,0);
  }
/* # line 478 "xfphys.sc" */	/* repeated select */
  {
    IIsexec();
    while (IInexec() == 0) {
      IIsqInit((char *)0);
      IIexExec(1,(char *)"xfphys1",7151,7421);
      IIputdomio((short *)0,1,30,4,&tp->expire_date);
      IIretinit((char *)0,0);
      if (IInexec() == 0) {
        IIsqInit((char *)0);
        IIsqMods(1);
        IIexDefine(1,(char *)"xfphys1",7151,7421);
        IIwritio(0,(short *)0,1,32,0,(char *)
"select text(date_part('year', date(gmt_timestamp(");
        IIwritio(0,(short *)0,1,32,0,(char *)" $0=");
        IIputdomio((short *)0,1,30,4,&tp->expire_date);
        IIwritio(0,(short *)0,1,32,0,(char *)"))))");
        IIexDefine(0,(char *)"xfphys1",7151,7421);
        if (IIerrtest() == 0) {
          IIsexec();
        } /* IIerrtest */
      } /* IInexec */
    ...
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,AFE_DATESIZE + 1-1,year_buf);
    } /* IInextget */
    IIsqFlush((char *)0,0);
  }
/* # line 482 "xfphys.sc" */	/* host code */
	CVan(year_buf,&year);
	if (year < 1970)
	    CVna(year+100,year_buf);
	STcat(date_buf,year_buf);
...
XF_TABINFO	*tlist)
{
    ...
    int		gopending = 0;
    i4          rngvars = 0;
    if ((ilist = xffillindex(0, tlist)) == NULL)
	return;
...
i4 output_flags;
{
    ...
    if (ip->pagesize > XF_COMPAT_PGSIZE)
	writewith(ERx("page_size"), ip->pagesize, &firstmodatt, Xf_in);
...
ame)=");
    IIputdomio((short *)0,1,32,0,table);
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
...
static bool xf_dbreplicated()
{
  char tabname[FE_MAXNAME + 1];
  bool result = FALSE;
/* # line 956 "xfphys.sc" */	/* select */
  {
    IIsqInit((char *)0);
    IIsqMods(1);
    IIwritio(0,(short *)0,1,32,0,(char *)
"select TRIM(relid)from iirelation where LOWERCASE(relid)='dd_support_\
tables'");
    IIretinit((char *)0,0);
    if (IInextget() != 0) {
      IIgetdomio((short *)0,1,32,FE_MAXNAME + 1-1,tabname);
