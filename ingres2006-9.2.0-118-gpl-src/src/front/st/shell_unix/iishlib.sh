...
touch_files()
{
   ...
   [ -f $(PRODLOC)/(PROD2NAME)/files/symbol.tbl ] ||
   {
      touch $(PRODLOC)/(PROD2NAME)/files/symbol.tbl ||
      {
         cat << !

Unable to create $(PRODLOC)/(PROD2NAME)/files/symbol.tbl.  Exiting... 
...
!
   ...
   [ -f $(PROG3PRFX)_CONFIG/config.dat ] ||
   {
      touch $(PROG3PRFX)_CONFIG/config.dat 2>/dev/null ||
      {
         cat << !

Unable to create $(PROG3PRFX)_CONFIG/config.dat.  Exiting...

!
         exit 1
      }
      chmod 644 $(PROG3PRFX)_CONFIG/config.dat
...
#	number assignments (IDSLST).  NEW_NICKNAMES cleared.
#
gen_nodes_lists()
{
...
#=[ get_nickname ]=============================================
#
#	Prompt user for a nickname (shorthand NODE alias)
#
#	Params:	$1 = RAD number or blank if regular cluster or non-cluster.
#
#	Data:	II_CLUSTER_ID - Used to set default nickname
#		II_INSTALLATION - Used to set default nickname
#		NEW_NICKNAMES - Check for conflict with uncommited
#			        names.
#	Outputs:
#		NICKNAME
#
get_nickname()
{
    node_name_name="`node_name $1`"
    echo "Enter nickname for $node_name_name, or hit return to accept default."
    while :
    do
	NICKNAME=`iigetres ii.${node_name_name}.config.nickname`
	[ -z "$NICKNAME" ] && 
	{
	    if [ -n "$II_CLUSTER_ID" ] ; then
		# For clusters, default nickname is 'iiNODEnn'.
		NICKNAME=`echo "${II_INSTALLATION}NODE0${II_CLUSTER_ID}" | \
		   sed 's/0\([0-9][0-9]\)/\1/'`
	    else
		# For normal Ingres, default nickname is truncated box name.
		NICKNAME=`echo $CONFIG_HOST | sed 's/^\([^_]\{1,8\}\).*$/\1/' |
		   tr '[a-z]' '[A-Z]'`
	    fi
	}
	while :
	do
	    ...
	    if [ -z "$junk" ]
	    then
		junk="`echo $answer | tr '[a-z]' '[A-Z]'`"
		if [ -z "`echo $junk | sed 's/^[A-Z][A-Z0-9_]\{0,7\}//'`" ]
		then
		    NICKNAME="$junk"
		    break
		fi
...
Nickname must be no longer than eight (8) characters long, start with a letter, 
and may contain only letters, numbers, and the underscore character.

!
	done

	# Make sure there are no conflicts with existing names
	if in_set "$NEW_NICKNAMES" $NICKNAME
	then
	    cat << !
"$NICKNAME" conflicts with previously entered nickname.

Please enter a unique nickname.
!
	    continue
	fi
	if in_set "$NODESLST" $NICKNAME
	then
	    [ "$NICKNAME" = "$node_name_name" ] ||
	    {
		cat << !
"$NICKNAME" conflicts with an existing node name.
...
!
	...
	if grep -i "config.nickname[ 	]*${NICKNAME}" \
	    $(PROG3PRFX)_CONFIG/config.dat >/dev/null 2>&1
	then
	    cat << !
"$NICKNAME" conflicts with an existing nickname.
...
!
	...
	NEW_NICKNAMES="${NEW_NICKNAMES} ${NICKNAME}"
	return 0
    done
}
...
log_config_change_banner()
{
    echo "$1" | box ${2} >> $(PRODLOC)/(PROD2NAME)/files/config.log
    cat << ! >> $(PRODLOC)/(PROD2NAME)/files/config.log
host:		`uname -n`
user:		${WHOAMI}
terminal:	`tty | sed 's/^.*\///'`
date:		`date`
...
lock_config_dat()
{
    if [ -f $(PRODLOC)/(PROD2NAME)/files/config.lck ]
    then
	error_box << !
$(PRODLOC)/(PROD2NAME)/files/config.lck exists.  Other users may be configuring this Ingres instance.

`cat $(PRODLOC)/(PROD2NAME)/files/config.lck | expand`
!
	return 1
    else
	cat << ! >  $(PRODLOC)/(PROD2NAME)/files/config.lck
application:			${1}
host:				`uname -n`
pid:				$$
terminal			`tty | sed 's/^.*\///'`
...
unlock_config_dat()
{
    [ -f $(PRODLOC)/(PROD2NAME)/files/config.lck ] && \
     rm -f $(PRODLOC)/(PROD2NAME)/files/config.lck
}
...
if [ "$WRITE_RESPONSE" = "false" ] || [ "$IGNORE_RESPONSE" = "true" ] ; then
   rm -f $(PRODLOC)/(PROD2NAME)/files/config.lck /tmp/*.$$ 1>/dev/null 2>/dev/null
   trap : 0
fi
   exit 0
}
...
{
   # back it up
   if cp $(PROG3PRFX)_CONFIG/config.dat $(PROG3PRFX)_CONFIG/config.bck ; then
      grep -v (PROG4PRFX).\\\*.setup $(PROG3PRFX)_CONFIG/config.dat >/tmp/config.dat
      cp /tmp/config.dat $(PROG3PRFX)_CONFIG/config.dat && rm $(PROG3PRFX)_CONFIG/config.bck
      rm /tmp/config.dat
      chmod 644 $(PROG3PRFX)_CONFIG/config.dat
   else
      cat << !
