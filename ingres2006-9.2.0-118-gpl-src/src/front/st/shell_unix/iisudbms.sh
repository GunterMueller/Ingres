...
update_parameters()
{
    ...
    x=`iigetres "ii.$CONFIG_HOST.dbms.*.system_lock_level"`
    if [ -z "$x" ] ; then
	$DOIT iiinitres system_lock_level $II_CONFIG/dbms.rfm
    fi
    x=`iigetres "ii.$CONFIG_HOST.dbms.*.di_zero_bufsize"`
    if [ -z "$x" ] ; then
	$DOIT iiinitres di_zero_bufsize $II_CONFIG/dbms.rfm
    fi
    x=`iigetres "ii.$CONFIG_HOST.dbms.*.direct_io"`
    if [ -z "$x" ] ; then
	$DOIT iiinitres direct_io $II_CONFIG/dbms.rfm
    fi
    x=`iigetres "ii.$CONFIG_HOST.dbms.*.fd_affinity"`
    if [ -z "$x" ] ; then
	$DOIT iiinitres fd_affinity $II_CONFIG/dbms.rfm
    fi
...
!
	...
	rm -f $II_CONFIG/sudbms$$.crs
	cat >$II_CONFIG/sudbms$$.crs <<'!EOF!'
...
!EOF!
	...
	rm -f /tmp/rfm$$ $II_CONFIG/sudbms$$.crs
...
!
	DBMS_RFM_FILE=$II_SYSTEM/ingres/files/dbms.rfm
...
!
	[ -f $II_SYSTEM/ingres/files/config.dat ] && \
	 cat $II_SYSTEM/ingres/files/config.dat  >> $DEBUG_LOG_FILE
	cat << ! >> $DEBUG_LOG_FILE

--- protect.dat generated by dry run ---
!
	[ -f $II_SYSTEM/ingres/files/protect.dat ] && \
	 cat $II_SYSTEM/ingres/files/protect.dat  >> $DEBUG_LOG_FILE
	...
	do
	    if [ -f /tmp/$f.$$ ] ; then
		$DOIT rm -f $II_CONFIG/$f
		$DOIT cp /tmp/$f.$$ $II_CONFIG/$f
		[ -s $II_CONFIG/$f ] || rm -f $II_CONFIG/$f >/dev/null 2>&1
	    fi
	done
    fi

    $DOIT rm -f $II_SYSTEM/ingres/files/config.lck /tmp/*.$$ >/dev/null 2>&1
...
then	# Set up for creating response file
	...
	[ -f $II_CONFIG/$f ] &&
	{
	    cp $II_CONFIG/$f /tmp/$f.$$ >/dev/null 2>&1 || \
	    {
		cat << !
...
!
    ...
    then
	$DEBUG_FAKE_NEW && \
	 rm $II_CONFIG/config.dat $II_CONFIG/symbol.tbl
    else
	mkresponse_msg
    fi
else
...
FROM64=
if [ ! -f $II_CONFIG/config.dat -a -f $II_CONFIG/rundbms.opt ] ; then
    FROM64=Y
fi
export FROM64
...
NEW_NICKNAMES=""	# List of new nicknames added in this run.
...
then
	...
	then
	    ...
	    EXISTING_RAD_IDS=`grep -i "R[1-9][0-9]*_${HOST_GREP}\.config\.numa\.rad" $II_SYSTEM/ingres/files/config.dat 2>/dev/null | \
	     cut -d':' -f2`
...
!
	...
	#
	#   Get nodes and node nicknames for each RAD
	#
	node_number_instuctions
        
	cat << !
Ingres allows you to specify an alternate name to be used in place
of the host machine name when logging information to the error log,
and as a node name argument to certain Ingres utilities.

This is particularly useful with NUMA Ingres clusters, since
you will often need to specify the target RAD to be operated
on, and a short easy to remember node name alias (nickname)
is very handy. 
...
!
		...
		if $WRITE_RESPONSE ; then
		    check_response_write "NICKNAME${rad}" $NICKNAME replace
		else
		    $DOIT iisetres "ii.`node_name $rad`.config.nickname" \
		     $NICKNAME
		fi
	    }
...
if $WRITE_RESPONSE ; then
   ...
   $DOIT iisetres ii.$CONFIG_HOST.gcn.local_vnode "$HOSTNAME"
   ...
   [ -z "$SERVER_HOST" ]  &&
   {
      ...
      [ -n "$SERVER_ID" ] && 
      {
         ...
         SERVER_HOST=`eval II_ADMIN=$II_SYSTEM/ingres/files iipmhost`
      }
   }
...
!
	...
	do
	    ...
	    k=`expr $k + 1`
	    echo \
"$rad `nth_element \"$NODESLST\" $k` `nth_element \"$IDSLST\" $k` `nth_element \"$NEW_NICKNAMES\" $j`" | \
	    awk '{ printf( "%3d    %-24.24s %-8.8s%8d\n", $1, $2, $4, $3 ) }'
	done
    else
	[ -n "$II_CLUSTER_ID" ] && cat << !
...
Ingres cluster membership ID:
	...
	$II_CLUSTER_ID
!
	NEW_NICKNAMES="`echo $NEW_NICKNAMES | sed 's/^ //'`"
	[ -n "$NEW_NICKNAMES" ] && cat << !
Node alias (Nickname):
...
!
   ...
   DATA=`eval II_ADMIN=$II_SYSTEM/ingres/files ingprenv II_DATABASE`
...
if $WRITE_RESPONSE ; then
   ...
   $DOIT ingsetenv II_GCN${II_INSTALLATION}_LCL_VNODE $HOSTNAME 
fi

$NUMA_CLUSTER ||
{
    #
    # Get nickname (node alias) if not a NUMA cluster setup
    #
    if $BATCH
    then	
	if $READ_RESPONSE
	then	# Can only get nickname if have a response file if batch.
	    nickname=`iiread_response NICKNAME $RESINFILE`
	    [ "$nickname" ] && \
	     $DOIT iisetres ii.$CONFIG_HOST.config.nickname $nickname
	fi
    else
	cat << !
...
!
	prompt "Do you want to specify a machine nickname?" y && get_nickname &&
	{
	    if $WRITE_RESPONSE ; then
		check_response_write "NICKNAME" $NICKNAME replace
	    else
		$DOIT iisetres ii.$CONFIG_HOST.config.nickname $NICKNAME
	    fi
	}
...
# ("DATA" set earlier on.)
CKP=`eval II_ADMIN=$II_SYSTEM/ingres/files ingprenv II_CHECKPOINT`
JNL=`eval II_ADMIN=$II_SYSTEM/ingres/files ingprenv II_JOURNAL`
DMP=`eval II_ADMIN=$II_SYSTEM/ingres/files ingprenv II_DUMP`
WORK=`eval II_ADMIN=$II_SYSTEM/ingres/files ingprenv II_WORK`
...
#
# set permissions on zoneinfo directories
#
[ -d $II_CONFIG/zoneinfo ] &&
{
   chmod 775 $II_CONFIG/zoneinfo/*
}
[ -d $II_CONFIG/zoneinfo/lp64 ] &&
{
   chmod 775 $II_CONFIG/zoneinfo/lp64/*
}
...
!
   ...
   TZ_RULE_MAP=$II_SYSTEM/ingres/files/dbms.rfm
   if $BATCH ; then
      if $READ_RESPONSE ; then
...
export "II_CHARSET$II_INSTALLATION"="$VALUE"
...
if $WRITE_RESPONSE ; then 
   ...
   #
   [ -f $II_SYSTEM/ingres/files/startup.dst ] &&
      [ ! -r $II_SYSTEM/ingres/files/startup ] &&
   { 
      $DOIT cp $II_SYSTEM/ingres/files/startup.dst $II_SYSTEM/ingres/files/startup
   }
   [ -f $II_SYSTEM/ingres/files/startsql.dst ] &&
      [ ! -r $II_SYSTEM/ingres/files/startsql ] &&
   {
      $DOIT cp $II_SYSTEM/ingres/files/startsql.dst $II_SYSTEM/ingres/files/startsql
   }
...
!
	...
	if $UPGRADE_DBS ; then
	    COMMAND="upgradedb -all >>$II_SYSTEM/ingres/files/upgradedb.log 2>&1"
	    COMMAND2="upgradedb iidbdb >>$II_SYSTEM/ingres/files/upgradedb.log 2>&1"
	else
	    COMMAND="upgradedb iidbdb >>$II_SYSTEM/ingres/files/upgradedb.log 2>&1"
	fi
	rm -f $II_SYSTEM/ingres/files/upgradedb.log
...
!
		...
		UPG25=`grep 'config.dbms.ii25' $II_CONFIG/config.dat \
			| awk '{print $2}'`
		UPG20=`grep 'config.dbms.ii20' $II_CONFIG/config.dat \
			| awk '{print $2}'`
...
!
	if $DOIT $II_SYSTEM/ingres/bin/createdb -S iidbdb ; then
	    # FIXME - sleep to stop DB lock race condition
	    sleep 1
	    $DOIT sysmod +w iidbdb
...
!
	$DOIT sql -u${DIX}\$ingres imadb < \
		  $II_SYSTEM/ingres/vdba/makimau.sql \
		  > $II_SYSTEM/ingres/vdba/mkimau.out 2>&1
...
CREATE_DEMODB=NO
DEMODB_ERROR=''
if [ -d "$II_SYSTEM/ingres/data/demodb" ] && [ $UPGRADE_DBS="false" ] ; then
    cat << !
You have chosen not to upgrade the user databases at this time. As such
demodb cannot be reload until the database has been upgraded using the
'upgradedb' utility.
...
if [ "$WRITE_RESPONSE" = 'false' ] ; then
    ...
    if [ "$CREATE_DEMODB" = "YES" ] &&
        [ ! -d "$II_SYSTEM/ingres/data/default/demodb" ] ; then
	cat << !
Creating demodb...
...
!
	$DOIT createdb -i demodb ||
	{
	    ...
	    NONFATAL_ERROR=Y
	    cat << ! | error_box
An error occured creating database 'demodb'. As this database is used by some of the Ingres demonstration applications they may not function correctly until this issue has been resolved.
...
!
	[ -f "$II_SYSTEM/ingres/demo/data/drop.sql" ] &&
	    [ -f "$II_SYSTEM/ingres/demo/data/copy.in" ] &&
	    ( $DOIT cd "$II_SYSTEM/ingres/demo/data" ; \
		$DOIT sql demodb < drop.sql > copyin.log ;
		$DOIT sql demodb < copy.in >> copyin.log ) ||
		{
		    DEMODB_ERROR=Y
		    NONFATAL_ERROR=Y
		    cat << ! | error_box
An error occured loading 'demodb'. As this database is used by some of the Ingres demonstration applications they may not function correctly until this issue has been resolved.
