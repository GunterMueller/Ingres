...
cd $(PRODLOC)/(PROD2NAME)/files/memory/$MEMDIR
...
for f in `ls $(PRODLOC)/(PROD2NAME)/files/memory/$MEMDIR`
do
    [ -f $(PRODLOC)/(PROD2NAME)/files/memory/$MEMDIR/$f ] && \
     rm -f $(PRODLOC)/(PROD2NAME)/files/memory/$MEMDIR/$f
done
}
