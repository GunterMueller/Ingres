...
check_dest_file()
{
   ...
   ext=`expr $name : '\(.*\)${ext_tar}.Z'`
   if [ "$ext" = "" ] ; then
       if [ "$dest_compressed" = "true" ] ; then
           echo Invalid dest_saveset file name.
           echo A .Z extension may only be used in conjunction with ${ext_tar}
           echo E.g. filename${ext_tar}.Z
           fail_exit
       fi
   fi
   ...
   ext=`expr $name : '\(.*\).tar'`
   if [ "$ext" = "" ] ; then
       dest_saveset=$dest_saveset$ext_tar
       dest_tar=true
   else
       dest_tar=true
   fi
...
unpack()
{
   ...
   if [ "$source_base_format" = "true" ] ; then
       ...
       #             the file name on ls -l is the 9th entry of output.
       #             All directories other than install contain a
       #             files called install.pax.Z
       subdir_list=`ls -l | awk '{print $9}'`
       ...
       do
           ...
           if [ "$subdir" != "install" ] ; then
               cd $subdir
               uncompress ${subinstfile}${ext_Z}
               tar -xf $subinstfile 1> $log_dir/xtar_$subdir.log 2>&1
               for needfile in `cat $log_dir/$filelist`
               do
                   # We'll bundle only the files needed although
                   # we may need the whole subset for an install
                   if [ -f $needfile ] ; then  
                       need_pkg=true
                   fi
               done
       ...
       if [ "$source_compressed" = "true" ] ; then
           zcat $source_saveset | tar -xf - -I $log_dir/$filelist \
                > $log_dir/xtar_src.log 2>&1
       else
           tar -xf $source_saveset -I $log_dir/$filelist  \
                > $log_dir/xtar_src.log 2>&1
       fi
