...
# rename the package in the spec
    ...
    sed -e "s/^Name: ${pkg}/&\-$2/" \
	-e "s/^PreReq: ${PRODNAME}[a-zA-Z_2,-]*/&-$2/" \
	-e "s/^Provides: ${PRODNAME}[a-zA-Z_2,-]*/&-$2/" \
	-e "s/^Obsoletes: ca-ingres[a-zA-Z_2,-]*/&-$2/" \
...
# package an rpm archive
    $RPMBLD -bb --sign --target $RPMARCH --define "_rpmdir $WORKDIR " --buildroot $WORKDIR $WORKDIR$RPMSPEC >& /dev/null
    rc=$?
    if (($rc)); then
