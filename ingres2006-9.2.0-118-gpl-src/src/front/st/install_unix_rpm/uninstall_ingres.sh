...
# installation ID is specified the ingres2006 group of packages will be
# removed
#
remove_installation()
{
	...
	case "$1" in
	  [A-Z][A-Z]|\
          [A-Z][0-9])
			pkglist=`rpm -qa | egrep "ingres2006.*${1}" |\
					grep -v license`
			# If we're cleaning out everything get II_SYSTEM
			$clean && ii_sys=`get_prefix ingres2006-$1`
			;;
	   ingres2006)
			pkglist=`rpm -qa | grep ingres2006 | \
			grep -v [[:upper:]] | egrep -v "license|documentation"`
			# If we're cleaning out everything get II_SYSTEM
			$clean && ii_sys=`get_prefix ingres2006`
			;;
	     *)
...
EOF
    ...
    # Get RPM package list ignoring ingres2006-license
    echo "Generating list of installations..."
    rpm -qa | grep -v license | \
	grep "ingres2006-\([[:upper:]]\|[[:digit:]]\.[[:digit:]]\)" |\
	 sort > $instlst
   ...
   [3-9]\.[0-9]\.[0-9])
			remove_installation ingres2006
			;;
		     *)
...
if $rm_all ; then
    ...
    # Remove standard (not renamed) ingres2006 packages
	remove_installation ingres2006
fi
