:
##
##  Copyright (c) 2004, 2006 Ingres Corperation. All rights reserved.
##
...
export INGLIC_RPM=ingres2006-license-${inglic}-${INGVER}-${build}.${RPMARCH}.rpm
...
if [ -f $INGLIC_RPM_LOC ] ; then
    ...
    echo Cannot locate ingres2006-license-${inglic} RPM package
    echo "Aborting.."
    exit 4
fi
...
read -p "Do you accept this license agreement? (y or n)" ans
    case "\$ans" in
      ...
      yes|YES)
        ...
        n|N|\
       NO|no)
                echo "You must answer 'yes' to continue with the Ingres 2006 product install"
                exit 1
                ;;
           *)
