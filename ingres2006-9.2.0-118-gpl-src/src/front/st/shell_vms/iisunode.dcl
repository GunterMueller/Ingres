...
$	IF "''parm'" - "-batch" .NES. "''parm'"
$       THEN
$          'self'BATCH	   	== true
$          GOTO _CHECK_PARM
$       ENDIF
$	IF parm - "-vbatch" .NES. parm
$       THEN
$	   'self'BATCH		==true
$          GOTO _CHECK_PARM
$       ENDIF
$	IF parm - "-mkresponse" .NES. parm
$       THEN
$	   'self'BATCH	  	== false
...
$	   WRITE_RESPONSE = 'self'READ_RESPONSE
$          'self'RESINFILE == parm 
...
$    ENDIF
$!
$    warn_box "All other Ingres nodes in this Ingres cluster must be shutdown in order to run this procedure."
$!
$    IF BATCH .EQ. false
$    THEN
$	PIPE confirm_intent
...
$    nname = 'self'NICKNAME
$    message_box "''CONFIG_HOST' will be set up as node ''II_CLUSTER_ID' with nickname ''nname'." 
$!
...
$! 
$!   # Add new CNA to all nodes.
$    CNA           = "''IIAffinityClass'''clusterid2'"
$    CNA_server    = "''IIAffinityServer'''clusterid2'" 
$!
$    cna_id = 0
$    nlst = 'self'NODESLST
$    nodes_lst = F$Edit("''nlst' ", "UPCASE")
$    info_box "NODESLST = ''nodes_lst'"
$! 
$_CNA_NEXT:
$   node = F$ELEMENT(cna_id," ",nodes_lst)
$   warnings = 0
$   IF node .NES. "" .AND. node .NES. "''CONFIG_HOST'"
$   THEN
$!     # Skip if already exists
$      PIPE SEARCH/NOWARN/NOLOG/NOOUTPUT II_CONFIG:config.dat "ii.''node'.dbms.''CNA'." ; ss = $SEVERITY > NL:
$!   
$   info_box "Adding Affinity classes to  nodes:" - "''node' EXISTS=''ss'" 
$      IF ss .NE. 1
$      THEN 
$!
$	  info_box "Add new CNA as a copy of ''first_node':" -
	  	      "Node       = ''node'" -
		      "CNA        = ''CNA'"
$         info_box "Adding Affinity classes to  node: ''node'" 
$ 	  iiappend_class "''self'" "''first_node'" "config.dat"  "''node'"  "''CNA'" 
$         if iishlib_err .ne. ii_status_ok
$	  then
$            warnings = warnings + 1
$         endif 
$!
$	  if iishlib_err .ne. ii_status_error
$         then
$    	     info_box "Adding Affinity classes to  nodes: ''node'" 
$  	     iiappend_class "''self'" "''first_node'" "protect.dat" "''node'"  "''CNA'"
$            if iishlib_err .ne. ii_status_ok
$            then
$               warnings = warnings + 1
$            endif 
$!
$	     iisetres "ii.''node'.ingstart.''CNA'.dbms" 		  0
$	     iisetres "ii.''node'.dbms.''CNA'.cache_name" 	  "''CNA'"
$	     iisetres "ii.''node'.dbms.''CNA'.cache_sharing"  	  "OFF"
$	     iisetres "ii.''node'.dbms.''CNA'.class_node_affinity"  "ON"
$	     iisetres "ii.''node'.dbms.''CNA'.server_class"   	  "''CNA_server'"
$	     iisetres "ii.''node'.ingstart.''CNA'.dbms" 0
$!
$         endif 
$!
$         nobox "Affinity class """"''CNA'"""" added to configuration """"''node'"""""
$         if warnings .gt. 0
$	  then
$            warn_box    "During the adding of the Affinity class ''CNA' to configuration for """"''node'"""", problems were identified." -
              " " -
	      "Please check the logfile for more detailed information."
$	     message_box " " -
"The setup of this node will be completed, but the Class Node Affinity configurations may not be fully defined. this can be performed manually, or the node can be removed using """"iisunode -remove""""" 
$	  endif
$      ENDIF
$   ENDIF
$   IF node .NES. ""
...
$!
$   PURGE/NOLOG/KEEP=1  II_CONFIG:*'P1'* 
