...
$   get_nickname        :== 'callsh' _get_nickname 
...
$!       # the response file.
$        check_response_write "CLUSTER_NODE_ID''numa_rad'" "GETRES" -
...
$!
$!------------------------------------------------------------------------
$ _get_nickname: SUBROUTINE
$!
$   self 	 = "''p1'"
$   bch  	 = F$INTEGER(P2)
$   iishlib_err == ii_status_ok
$!
$   node_name_name="''CONFIG_HOST'"
$!
$   getressym 'self' "''node_name_name'" "config.nickname"
$   nick_name = F$EDIT(iishlib_rv1,"UPCASE")
$!
$   if "''nick_name'" .EQS. ""
$   then
$      if "''II_CLUSTER_ID'" .NES. "" .AND. F$TYPE(II_CLUSTER_ID) .EQS. "INTEGER"
$      then
$         ii_clu_id2 = F$FAO("!2ZL",'II_CLUSTER_ID')
$!        # For clusters, default nickname is 'iiNODEnn'.
$         nick_name="''II_INSTALLATION'NODE''ii_clu_id2'" 
$      else
$!        # For normal Ingres, default nickname is truncated box name.
$         nick_name=F$FAO("!8AS","''CONFIG_HOST'")
$      endif
$      nick_name=F$EDIT(nick_name,"UPCASE")
$   endif
$!
$   if bch .EQ. true 
$   then 
$      if nick_name .NES. ""
$      then
$         GOTO _GET_NICKNAME_BATCH_OK
$      else
$         GOTO _GET_NICKNAME_BATCH_ERROR
$      endif
$   endif
$!
$ ON CONTROL_C then GOTO _GET_NICKNAME_FAIL
$ ON CONTROL_Y then GOTO _GET_NICKNAME_FAIL
$!  Interactive mode - issue prompt 
$!
$_NICKNAME_GET:
$   nobox " " "Enter nickname for ''node_name_name', or hit return to accept default." " "
$!
$   INQUIRE/PUNCT resp " Nickname [''nick_name'] "
$   resp = F$EDIT (resp,"TRIM")
$   if resp .EQS. "" 
$   then
$      resp = nick_name
$   endif
$!
$!  Re-check value for resp after assigning local nick_name
$   if resp .EQS. "" 
$   then
$      GOTO _NICKNAME_GET
$   endif
$!
$   resp1 = F$EXTRACT(0,1,resp)
$   if F$LENGTH(resp) .GT. 8 .OR. F$TYPE(resp1) .EQS. "INTEGER" .OR. -
      resp1 .EQS. "_"
$   then
$      nobox " " "Nickname must be no longer than eight (8) characters" -
             "long, start with a letter, and may contain only letters," -
             "numbers, and the underscore character." " "
$      GOTO _NICKNAME_GET
$   endif
$!
$!  define the local nick_name for checking.
$   nick_name = resp
$!
$!  # Make sure there are no conflicts with existing names
$   nnick = 'self'NEW_NICKNAMES
$   in_set "''nnick'" "''nick_name'"
$   if iishlib_rv1 .NE. 0
$   then
$      error_box """""''nick_name'"""" conflicts with previously entered nickname." " " -
                 "Please enter a unique nickname."
$      GOTO _NICKNAME_GET
$   endif
$!
$!  if we find an entry in_set will return OK.  this is an error as
$!  far as this routine is concerned, since we want a new value.
$!
$   nlst   	= 'self'NODESLST
$   ilst   	= 'self'IDSLST
$   info_box "Current list of nodes"  -
	...
	"IDSLST     =''ilst'" 
$   nlst = 'self'NODESLST
$   in_set "''nlst'" "''nick_name'"
$   if iishlib_err .EQ. ii_status_ok
$   then
$      if "''nick_name'" .NES. "''node_name_name'"
$      then
$	  error_box """""''nick_name'"""" conflicts with an existing" -
                    "node name." " " -
                    "Please enter a unique nickname."
$         GOTO _NICKNAME_GET
$      endif
$      PIPE SEARCH/MATCH=AND II_CONFIG:config.dat "config.nickname", "''nick_name'" ; SS = $SEVERITY > NL:
$      if SS .EQ. 1
$      then
$         nobox " " """""''nick_name'"""" conflicts with an existing nickname." " " -
		"Please enter a unique nickname."
$         GOTO _NICKNAME_GET
$      endif
$!
...
$   endif
$!
$_GET_NICKNAME_BATCH_OK:
$!
$   'self'NICKNAME == nick_name
$   'self'NEW_NICKNAMES=='self'NEW_NICKNAMES + " " + 'self'NICKNAME
$!
$   nick_name = 'self'NICKNAME  
$   info_box "Validated Nickname - OK" "NICKNAME==''nick_name'"
$   iishlib_err == ii_status_ok
$   goto _GET_NICKNAME_EXIT
$!
$_GET_NICKNAME_BATCH_ERROR:
$!
$   iishlib_err == ii_status_warning
$   goto _GET_NICKNAME_EXIT
$!
