...
ReadInitFile()
{
    ...
    FILE    *fptr, *fptr2;
    CString loc = CString(getenv("II_SYSTEM")) +
    ...
    */
    RegOpenKeyEx (
	HKEY_LOCAL_MACHINE,
	"SYSTEM\\CurrentControlSet\\Services\\oiPfCtrs\\Performance",
	0L,
	KEY_READ,
	&hKeyDriverPerf);
    size = sizeof (DWORD);
    ...
    */
    loc = CString(getenv("II_SYSTEM")) + CString("\\ingres\\files\\oipfpers.ini");
    if (GetFileAttributes(loc) != -1)
    {
