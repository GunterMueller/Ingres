...
** History:
**	12-sep-95 (fauma01)				
**	    Program finalized for OpenIngres 1.2/00 NT 
**	    and CA-INSTALL 3.48 for Windows NT.
**	20-nov-95 (tutto01)
**	    Added code to handle system dll's. Also cleaned up a bit for
**	    80 column display.
**	02-dec-95 (tutto01)
**	    Modified for use with 7 products instead of 6.  Added NUM_PRODUCTS
**	    constant to simplify adding/removing products in the future.
**	25-jan-96 (tutto01)
**	    Hide various files on the install CD to give a cleaner
**	    appearance, and prevent people from trying to run things
**	    such as Ingres' CICHKCAP.EXE or the CA-Installer directly!
**	25-feb-96 (tutto01)
**	    Generalized to work for products other than Ingres.  Done for the
**	    Desktop 1.2 project.  Also added the option of declaring a null
**	    string identifying the icon for a given file.  This implies an
**	    icon embedded in the file/executable.
**	11-mar-96 (tutto01)
**	    Fixed invalid string compare to work with new formatting of
**	    the ingresnt.mas file.
**	26-mar-96 (tutto01)
**	    CA-Installer doesn't deal well with hidden files.  Therefore,
**	    files hidden to give the install CD a cleaner look should not
**	    include CA-Installer files(ones that are copied to CA-APPSW)
**	09-may-96 (tutto01)
**	    The current process did not easily handle additions to the 
**	    Windows system directory.  Now this can be done by putting
**	    files in a "WINSYS" subdirectory, such as:  A \WINSYS\ XYZ.DLL 2
**	    This would cause XYZ.DLL to be copied into the Windows system
**	    directory during the installation of product "A".
**      06-jun-96 (somsa01)
**              Added Replicator as an additional product, and made appropriate
**              changes.
**	13-jun-96 (somsa01)
**		Backed out most of previous changes; just changed NUM_PRODUCTS
**		to 8 (Replicator).
**	22-jul-96 (somsa01)
**		Incremented NUM_PRODUCTS by one for ICE. Also took out hiding
**		of .HLP files; they cannot be found by WinHelp() in the
**		installer.
**	06-jan-97 (somsa01)
**	    Incremented NUM_PRODUCTS to 10 for OpenROAD.
**	14-jan-97 (somsa01)
**	    Added DLLs to root of CD needed for checking if an existing Ingres
**	    installation is up and running.
**	17-jan-97 (somsa01)
**	    Changed names of new DLLs for installer.
**	24-jan-97 (somsa01)
**	    Changed caption name format during running of CA-Installer.
**	30-jan-97 (somsa01)
**	    Incremented NUM_PRODUCTS to 15 for Partner software companies. Also
**	    took out hiding of ".00*" files; if they are hidden, when the
**	    CA-Installer puts these files back together, the result is a
**	    HIDDEN file (we don't want this).
**	10-may-97 (mcgem01)
**	    Change product name to OpenIngres.
**	29-jun-97 (mcgem01)
**	    Add two new development partner demos.
**	
*/

# define	NUM_PRODUCTS	17
...
readmaster()
{
	...
	char 	*mkdir, disk_prod = '\0', prev_path[50][MAXSTRING],
			mline[MAXSTRING] = "\0";
	int		i, span_file, new_prod, disk_num = 0, disk_total_num = -1,
