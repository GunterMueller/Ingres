	...
	switch( CRtype( idx ) )
	{
		...
		case CR_INTEGER:
			...
			if (value != NULL && *value != EOS) 
			{
				...
				else if (CVal( value, &number ) == OK)
				{
					flag = INT_VALUE; /* integer */
					stnum = number;
				}
				else if (  (CVal( value, &number ) == CV_OVERFLOW) 
				        || (CVal( value, &number ) == CV_UNDERFLOW) )
					flag = OUT_OF_RANGE; /* integer overflow, underflow */
				else if (CVaf(value, '.',&ftemp) == OK ) 
					flag = FLOAT_VALUE; /* floating point no */
				else flag = NOT_A_NUMBER; /* neither */
			}
...
CRcompute( i4  idx, char **result )
{
	...
	switch( expr_type )
	{
		case CR_INTEGER:
			/*
			** Currently dmf_memory is not limited to 2Gig on 
			** 64-bit platforms, whereas other derived memory- 
			** related params are. 
			** to be revisited with SIZE_TYPE proj.
			** addition of SIZE_TYPE attribute type? 
			*/
			if (*(f8 *)val > MAXI4)
			{
			    if ( (PMmNumElem(pm_context, CRsymbol(idx)) != 6) ||
				 ( (PMmNumElem(pm_context, CRsymbol(idx)) == 6) &&
					(STcompare(PMmGetElem(pm_context, 5,
                                	CRsymbol(idx)), ERx("dmf_memory")) != 0) ) )
				*(f8 *)val = MAXI4;
			}
