	...
	char tmp_buf[BIG_ENOUGH];
        ...
        */
        ...
        if (lgk_mem_ready)
        {
           ...
           */
           MEsmdestroy("LGLKDATA.MEM", &err_code);
...
# ifdef VMS
    ...
    */
	...
	while ( SIopen( &mutex_loc , ERx("r"), &mutex_fp ) == OK )
	{
		...
		*/
		status = sys$getjpiw( 0, &otherpid, NULL, &itmlist,
				      NULL, NULL, 0 );
		if ( status & 0x1 )
		{
...
# define PARAM_MASK_INSVCMGR	(1<<5)  /* -insvcmgr argument */
	char *argp, **argh, *argvalp, *component_arg, *pnodename;
	i4    nodenum, rad;
	u_i4  mask;
...
# ifdef NT_GENERIC
		if (Is_w95)
		{
			...
			STprintf( tmp_buf, ERx( "\nUsage: %s [ -iigcn | -dmfrcp | -dmfacp | -client | -rmcmd | -icesvr |\n\t-{iidbms|iigcc|iigcb|iistar|oracle|informix|mssql|sybase|db2udb}[=name] ]\n\n"),argv[0] );
			SIfprintf( stderr, tmp_buf);
# else
# ifdef VMS
...
# ifdef VMS
	...
	{
		II_VMS_ITEM_LIST_3 item_list[ 2 ];
		STATUS osstatus;
		...
		item_list[ 0 ].buflen = STlength( value );
		item_list[ 0 ].itmcod = LNM$_STRING; 
		item_list[ 0 ].bufadr = value; 
		item_list[ 0 ].reslen = NULL; 
		item_list[ 1 ].buflen = 0;
		item_list[ 1 ].itmcod = 0;
...
add_to_node_mask( char *host, char *nodename, u_i4 *nodemask, u_i4 *remotemask )
{
	...
	if ( ( NULL == nodename ) ||
	     ...
	     ( pni->cx_nick_name_l &&
	       0 == STbcompare( nodename, 0,
			  pni->cx_nick_name, pni->cx_nick_name_l, TRUE ) ) )
	{
