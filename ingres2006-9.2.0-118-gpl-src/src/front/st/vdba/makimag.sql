...
##	    Reconciled all other generic non-ICE registrations with UNIX
##	    versions.
##	16-Mar-2005 (thaju02)
...
\p\g
drop view ima_server_sessions_extra;
\p\g
create view ima_server_sessions_extra 
            ( server, session_id, session_state,
              session_state_num , session_mask ,
              session_wait_reason ) as 
       select server , session_id ,'n/a',0,'n/a','n/a' 
...
register table ima_dmf_buffer_manager_stats (
    ...
    mutex_wait_count integer4 not null not default
        is 'exp.dmf.dm0p.lbmc_mwait',
    cache_lock_reclaims integer4 not null not default
        is 'exp.dmf.dm0p.lbmc_lockreclaim',
    wb_flush integer4 not null not default
        is 'exp.dmf.dm0p.lbmc_wbflush'
)
as import from 'tables'
with dbms = IMA,
structure = sortkeyed,
...
** IMA_BLOCK_INFO 
*/
drop view ima_block_info;
\p\g

create view ima_block_info 
            ( server, blk_num_sessions, blk_max_sessions,
              blk_num_active, blk_max_active ) as 
       select server , -1 , -1 , -1 , -1  
