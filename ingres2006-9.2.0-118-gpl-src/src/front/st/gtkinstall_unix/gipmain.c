...
# define ERROR_NOT_ROOT "Error: Must be run as root\n"
	...
	*binptr = '\0';
    ...
    /*
    ** Use RPM to query the packages contained in the saveset and
    ** and the ones already installed 
    */
    rc = query_rpm_package_info( argc, argv, &inst_state, &inst_count );
 
    if ( rc != OK )
    {
# define ERROR_RPM_PKG_QUERY_FAIL "Failed to query RPM package info\n"
	SIprintf( ERROR_RPM_PKG_QUERY_FAIL );
	return( rc );
    }
...
generate_command_line( char **cmd_line )
{
	...
	*/
	...
	if ( pkgs_to_install & PKG_DOC && 
		~inst_state & UM_RENAME ) /* no doc on renamed instance */
	{
...
add_pkglst_to_rmv_command_line( char **cmd_line, PKGLST pkglist )
{
    ...
    */
    renamed = is_renamed( selected_instance->pkg_basename );
    i = 1;
