...
get_new_pkg_hdr( Header *file_hdr)
{
    ...
    {
	rpmError( RPMERR_OPEN, "Failed to open %s\n%s\n",
...
find_files_to_install( void )
{
	...
	if ( LOisdir( &entloc, &flag ) != OK )
	{
	    ...
	    if ( CHECK_PKGNAME_VALID( pkgbuf, sufptr ) ) /* valid pkg ? */
	    {
		/* add package to list */
...
gip_findAuxLoc( instance *inst )
{
    ...
    /* set II_CONFIG so we can query config.dat */
    STprintf( pathbuf, "%s/ingres/files", inst->inst_loc );
    gip_setIIConfig( pathbuf );
