...
ingres_addsummary(MSIHANDLE hInstall)
{
	...
	char *path;
	int length, counter, bDualLog;
	u_char *point, *point2;
	HANDLE	handle;
	char	IngresDBMS[] = {"Ingres Database Server and Tools"};
	char	IngresNet[]  = {"Ingres Networking"};
	...
	char	*component_names[] = {"Ingres Database Server and Tools", "Ingres Networking", "Ingres Replicator", 
	...
	char	date_type_alias[] = {"Date Type Alias"};
	...
	while((point=strstr(txt_buf, title))!=NULL) 
	{
		...
		if (strcmp(setup_type, "Custom") == 0 || strcmp(setup_type, "Complete") == 0)
		{
			...
			if (strcmp(setup_type, "Custom") == 0)
				*setup_type='\0';		
			//Special case to take care of Vision package
			if (!MsiGetFeatureState(hInstall, "IngresVision", &iInstalled, &iAction) 
				&& (iAction == INSTALLSTATE_LOCAL)) 
			{
				if ((point=strstr(txt_buf, IngresVision)) != NULL)
				{
					point = point+sizeof(IngresVision);
...
ingres_install_documentation(MSIHANDLE hInstall)
{
	...
	*buf=0; dwSize=sizeof(buf);
	...
	sprintf(guid, "{FB1D9396-8AAD-4D2E-8224-CA425D9A3C3E}"); //current documentation product code
	...
	else
	{
		dwError=GetLastError();
	     if (!bUpgrade)
	          sprintf(cmd, "MsiExec.exe /i \"%s\\files\\documentation\\Ingres 2006 Documentation.msi\" /qb INGRESCORPFOLDER=\"%s\"", buf, installdir);  
	     else
		  sprintf(cmd, "MsiExec.exe /i \"%s\\files\\documentation\\Ingres 2006 Documentation.msi\" /qb REINSTALL=ALL REINSTALLMODE=vomus", buf);   
	}
...
ingres_install_dotnet(MSIHANDLE hInstall)
{
	...
	*buf=0; dwSize=sizeof(buf);
	...
	sprintf(guid,"{DF54A6FE-D2AD-4569-9A16-71601216D8C9}"); //current product code for .NET Data Provider
	sprintf(cmd2, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\%s", guid);
	//if product code for documentation is already in the registry then this is an upgrade scenario.
	//Otherwise fresh install.
	if (!RegOpenKeyEx(HKEY_LOCAL_MACHINE, cmd2, 0, KEY_QUERY_VALUE, &hRegKey))
	...
	if ((handle = FindFirstFile(cmd, &wfd)) != INVALID_HANDLE_VALUE)
	{
	     ...
	     if (!bUpgrade)
		 sprintf(cmd, "MsiExec.exe /i \"%s\\files\\dotnet\\Ingres .NET Data Provider 2.0.msi\" /qn INGRESCORPFOLDER=\"%s\"", buf, installdir);  
	     else
	     sprintf(cmd, "MsiExec.exe /i \"%s\\files\\dotnet\\Ingres .NET Data Provider 2.0.msi\" /qn REINSTALL=ALL REINSTALLMODE=vamus", buf);  
