...
ingres_remove_shortcuts(MSIHANDLE hInstall) 
{
	...
	sprintf(pathBuf, "%s\\Ingres\\Ingres 2006 Documentation", szPath);
...
ingres_install_adobe_reader(MSIHANDLE hInstall)
{
	...
	if (!IsCurrentAcrobatAlreadyInstalled())
	{
		...
		MsiGetProperty(hInstall, "INSTALLDIR", szPath, &dwSize);  
		...
		{
		    if (AskUserYN(hInstall, "The Ingres on-line documentation is shipped as a series of PDF documents.  You will need an Adobe Acrobat Reader in order to view these files.  Would you like to go to Adobe website to install the Adobe Acrobat Reader Version 7.0?"))
			//InstallAcrobat(szPath);
			OpenPageForAcrobat();
		}
		else
		{
...
ingres_start_bookshelf(MSIHANDLE ihnd)
{
	...
	if (_access(szPath, 00) == 0)
	{
		...
		{	
			...
			if (rc == ERROR_SUCCESS)
			{
				sprintf(szBuf, "\"%s\\%s\" \"%s\\Bookshelf.pdf\"", acrord, subKey, szPath);
				...
				{
					sprintf(szBuf, "Failed to execute \"%s\\Bookshelf.pdf\" with error %d", szPath, GetLastError());
