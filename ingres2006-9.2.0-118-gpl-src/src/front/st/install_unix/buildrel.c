...
**	
**	-rpm	Generate file attribute lists for RPM specfiles.
**
...
#define SPATIAL_MANIFEST   ERx("relspatial.dat")
GLOBALDEF char *releaseDir;
GLOBALDEF char *rpmBuildroot;
...
main(i4 argc, char *argv[])
{
	...
	if( audit && force )
	{
        ...
        /* generate spatial object library release */ 
        if( (!rpm)  &&  spatial )     
           STcopy(  SPATIAL_MANIFEST, releaseManifest) ;
	...
	else
	{
        ...
        /* get II_RELEASE_DIR, API_RELEASE_DIR or SOL_RELEASE_DIR */
	if( do_remote_api || spatial ) 
          {
           if ( do_remote_api )
            {
            ...
            {
             /* it must be doing a spatial release on unix */
             NMgtAt( ERx( "SOL_RELEASE_DIR" ), &releaseDir );
             if (!releaseDir || !*releaseDir)
              {
                SIfprintf(stderr,ERx("\nSOL_RELEASE_DIR must be defined.\n"));
                PCexit( FAIL );
              }
            }
          }
	...
	if( (!releaseDir || !*releaseDir) && !audit && !rpm && !xml )
	{
	...
	*/
	...
	if( audit ) 
	{
		    ...
		    if( cp != NULL && *cp != EOS )
		    {
                        ingbuild = MEreqmem(0, (STlength(cp) + 1), 
                                TRUE, &rtn);
                        STlcopy(cp, ingbuild, STlength(cp));
		    }
                    else
                    {
		        SIfprintf( stderr, ERx( "\nING_BUILD must be defined when using the -p option.\n\n" ) );
		        PCexit( FAIL );
                    }
                }
	...
	if( rpm )
	{
		NMgtAt( ERx( "II_RPM_BUILDROOT" ), &rpmBuildroot );
		if ( !rpmBuildroot || !*rpmBuildroot )
		{
            SIfprintf(stderr, 
...
# if defined(LNX)
                STpolycat( 9,
                        ...
                        ERx( " sleep 1" ),
                        cmd );
# elif defined(r64_us5) || defined(rs4_us5)
                 STpolycat( 7,
                        ERx( " cd " ),
                        tmpdir,
                        ERx( " ; " ),
