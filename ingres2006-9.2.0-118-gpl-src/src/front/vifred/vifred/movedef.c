...
i4	shift;		/* only used for shifts */
{
	...
	*/
				...
				y++, adj = adj->lt_next)
	{
		...
		if (chkSide(overnd, ps->ps_endx+2, TRUE, shift) == FALSE)
			return (FALSE);
	}
	return (TRUE);
}
...
i4	howfar;
{
	...
	while (list != ps)
	{
		...
		*/
		...
		nd = ndonPos(list);
		if (chkSide(nd, list->ps_begx + howfar, FALSE,howfar) == FALSE)
			return (FALSE);
	...
	*/
	while (lt != NULL)
	{
		...
		*/
		...
		exChange(&ps->ps_oldx, &ps->ps_begx);
		exChange(&ps->ps_oldy, &ps->ps_begy);
		ps->ps_mask = code;
		insPos(ps, FALSE);
	}
}
