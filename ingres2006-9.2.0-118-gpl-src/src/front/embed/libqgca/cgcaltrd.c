...
**
**	if (usr_rows = 0) then
**	    cgc_arows = (IICGC_ADEF_BYTES - IICGC_ADEF_BYTES % GCA buffer size)
**			/ tupsize;
**	else if (usr_rows * tupsize) > (IICGC_AMAX_BYTES * GCA buffer size) then
**	    cgc_arows = (IICGC_AMAX_BYTES - IICGC_AMAX_BYTES % GCA buffer size)
**			/ tupsize;
**	else
**	    cgc_arows = usr_rows;
**	endif;
...
IICGC_ALTRD	**astream;
{
    ...
    *astream = NULL;
    ...
    /* Figure out the number of rows - add 1 for dynamically calculated rows */
    cga->cgc_aflags = flags;
    cga->cgc_atupsize = tup_size;
    gc_buf_size = cgc->cgc_write_msg.cgc_d_length;
    if (usr_pfrows <= 0)
    {
	cga->cgc_arows = (IICGC_ADEF_BYTES - IICGC_ADEF_BYTES % gc_buf_size)
			/ tup_size;
	if (cga->cgc_arows < 1)
	    cga->cgc_arows = 1;
	row_why = ERx("default");
    }
    else if ((usr_pfrows * tup_size) > IICGC_AMAX_BYTES)
    {
	cga->cgc_arows = (IICGC_AMAX_BYTES - IICGC_AMAX_BYTES % gc_buf_size)
			/ tup_size;
	row_why = ERx("user exceeded maximum");
    }
    else
    {
	cga->cgc_arows = usr_pfrows;
	row_why = ERx("user count");
    }
