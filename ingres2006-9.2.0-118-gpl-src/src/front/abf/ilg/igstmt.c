...
IL	result;
{
	...
	*/
	...
	il->il_op1 = (IL)expr->il_size;
	il->il_op2 = (IL)expr->il_last;
	il_offset += expr->il_size + 3;
	...
	for (;;)
	{
		...
		*code++ = il->il_op;
		switch (operator)
		{
		  ...
		  case IL_EXPR:	
			...
			*code++ = il->il_op2;
			MEcopy((PTR) il->il_expr, 
				(u_i2) il->il_op1*sizeof(IL), (PTR) code);
			code += il->il_op1;
			break;

		  case IL_IF:
			*code++ = il->il_op1;
			*code++ = getsid(il, opr_il);
			break;
		  ...
		  case IL_QRYROWCHK:
			*code++ = il->il_op1;
			*code++ = (il->il_sid == NULL) ? 0 : getsid(il, opr_il);
			break;
