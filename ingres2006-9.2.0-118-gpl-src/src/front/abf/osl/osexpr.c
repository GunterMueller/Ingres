...
bool		negation;
{
	...
	{ /* apply a NOT node */
		...
		else if ( expr->n_token == RELOP || expr->n_token == NULLOP )
...
register OSNODE	*expr;
{
	switch (expr->n_token)
	{
	  ...
	  case NULLOP:
		/* Note:  NULLOP is a unary relational operator */
	  case UNARYOP:
...
bool		result;
{
	switch (expr->n_token)
	{
	  ...
	  case RELOP:
		...
		{
			/* Note:  All RELOPs are binary */
			if ( !result )
			{
				if ( expr->n_lhstemp == NULL ||
						expr->n_rhstemp == NULL )
	  ...
	  case NULLOP:
		...
		{
			instr_gen( expr->n_fiid, 0, 1, expr->n_lhstemp,
					(OSSYM *)NULL
			);
		}
		break;
