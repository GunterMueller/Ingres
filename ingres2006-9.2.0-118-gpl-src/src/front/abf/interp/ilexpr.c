...
IL	*stmt;
{
	...
	** will be negative if it is larger than MAX_IL (this assumes two's-
	** complement integers.)
	*/
	ilexpr = (ILREF *)( stmt + IIILtab[*stmt&ILM_MASK].il_stmtsize );
	if ((*stmt&ILM_MASK) == IL_LEXPR)
	{
		result_dbv = IIOFdgDbdvGet( (i4)ILgetOperand(stmt, 1) );
		lastinstr_offset = (i4) ILgetOperand(stmt, 3);
	}
	else
	{
		lastinstr_offset = (i4) ILgetOperand(stmt, 2);
	}

	if ( lastinstr_offset < 0 )
		lastinstr_offset += (MAX_IL + 1) * 2;	/* 2's complement */
