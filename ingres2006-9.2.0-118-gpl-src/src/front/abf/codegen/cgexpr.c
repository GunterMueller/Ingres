...
register IL	*stmt;
{
	...
	IICGobbBlockBeg();
		...
		ilsize = ILgetOperand( stmt, ((*stmt&ILM_MASK) == IL_LEXPR) ? 2 : 1 );
		ilptr = ((*stmt&ILM_MASK) == IL_LEXPR) ? 4 : 3;
		for ( i = ilsize ; --i >= 0 ; )
		{
		...
		** negative if it is larger than MAX_IL (this assumes two's-
		** complement integers.)  Correct it in this case . . .
		*/
		last_ref = ILgetOperand(stmt, ((*stmt&ILM_MASK) == IL_LEXPR) ? 3 : 2);
		if ( last_ref < 0 )
			last_ref += (MAX_IL + 1) * 2;	/* 2's complement */
