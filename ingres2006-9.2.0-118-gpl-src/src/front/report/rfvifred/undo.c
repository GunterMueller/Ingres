...
GLOBALDEF i4	vfoldundo = 0;		/* added for fix to BUG 1588 (dkh). */
GLOBALDEF bool	vfinundo = FALSE;	/* added for fix to BUG 1587 (dkh). */
...
# endif
	...
	*/
	...
	moveBack(unCode, 0);
	exChange(&unEndFrame, &endFrame);
	exChange(&unendxFrm, &endxFrm);
...
# endif
	else if (unCom == edDATA)
	{
		...
		type = FDgettype(fd);
		FDToSmlfd(&oldFd, ps);
		exChange(&type->ftlength, &unWdth);
		exChange(&type->ftfmtstr, &unFmtStr);
		exChange(&type->ftfmt, &unFmt);
...
/*
** exchange the two gizmos they must be nats 
*/
exChange(g1, g2)
reg	i4	*g1,
		*g2;
