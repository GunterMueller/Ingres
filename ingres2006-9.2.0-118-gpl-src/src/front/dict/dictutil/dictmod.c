...
FUNC_EXTERN bool        cat_exists(char *catname, char *dummy);
/* I don't know why 'dummy' is needed, BUT this now matches what is in 
   DCTCOMMON.C  - (huffman)*/
...
static READONLY char _Pgm[]	= ERx("modifyfe");
static READONLY char _Empty[]	= ERx("");
static READONLY char _Wait[]	= ERx("+w");
static READONLY char _Nowait[]	= ERx("-w");
...
char	**argv;
{
    ...
    if (IIDDcseCheckStartupErrs(rval, dbname, _Pgm, F_DD0005_MODIFY) != OK)
