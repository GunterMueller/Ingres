...
if [ "$VERS" = "su4_us5" -o "$VERS" = "a64_sol" ] ;  then
   ...
   su $ING_USER $SU_FLAGS "/usr/bin/env SHLIB_PATH=$II_SYSTEM/ingres/lib LD_LIBRARY_PATH=$II_SYSTEM/ingres/lib/lp64 ipcclean >/dev/null"
else
...
if $DUAL_LOG ; then
   ...
   then
      ...
      if [ -f $II_LOG_FILE_PATH -o -c $II_LOG_FILE_PATH ] && \
   ...
   then
      ...
      if [ -f $II_DUAL_LOG_PATH -o -c $II_DUAL_LOG_PATH ] && \
...
if [ "$VERS" = "su4_us5" -o "$VERS" = "a64_sol" ] ; then
  ...
  su $ING_USER $SU_FLAGS "/usr/bin/env SHLIB_PATH=$II_SYSTEM/ingres/lib LD_LIBRARY_PATH=$II_SYSTEM/ingres/lib/lp64 iimklog $DUAL_ARG -erase >/dev/null"
else
