...
#   and nickname because sadly these routines update the config.dat
...
Existing $VERSION Ingres instance "$II_INSTALLATION" on host $CONFIG_HOST will be converted to the initial node of an Ingres Cluster using node number $II_CLUSTER_ID and nickname $NICKNAME.
...
#   Establish trap to undo a partial conversion   
    ...
    # FIX-ME, FIXME. lock_limit should really be recalculated using CRS, 
    ...
    VALUE="`iigetres ii.${CONFIG_HOST}.rcp.lock.lock_limit`"
    LOCK_LIMIT="100000"
    [ ${VALUE}0 -ge ${LOCK_LIMIT}0 ] || \
    {
	 iisetres +p "ii.${CONFIG_HOST}.rcp.lock.lock_limit" $LOCK_LIMIT
	 [ -n "$VALUE" ] && VALUE=" from $VALUE"
	 wrap_lines << !

Maximum number of locks increased to $LOCK_LIMIT$VALUE.
!
	# Make sure we can support amount of shared memory required.
	syscheck ||
	{
	    error_box << !
...
#   Modify config.dat
    ...
    [ -n "$NICKNAME" ] && iisetres ii.$CONFIG_HOST.config.nickname "$NICKNAME"
