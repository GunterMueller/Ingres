	...
	{   
	   ...
	   */
	   if (argv[parmno][1] != 'u' &&
		...
		break;
	    case 'a':	    /* optional user specification */
		...
		break;
	    case 'l':	    /* optional user specification */
		...
		break;
	    case 'n':	    /* debug flag - should be 1st parameter */
		duve_cb->duve_skiplog = TRUE;
		break;
	    case 't':	    /* supress stripping of timestamp from dmf msgs*/
...
DUVE_CB            *duve_cb;
{
    ...
    len = STtrmwhite( searchloc->db_loc_name );
    ...
    {
	if ( STcompare (searchloc->db_loc_name,
	     duve_cb->duve_locs->du_locname[i].duloc.db_loc_name)!=DU_IDENTICAL)
	    continue;
...
DUVE_CB *duve_cb;		    /* verifydb control block */
{
	...
	PTR	    mem_ptr;
	    ...
	    names = (char **) MEreqmem( (u_i4)0, size, TRUE, &recmem_stat);
