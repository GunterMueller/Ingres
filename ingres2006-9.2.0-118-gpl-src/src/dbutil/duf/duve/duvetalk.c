...
DUVE_CB		*duve_cb;
{
        ...
        if (duve_cb->duve_altlog)
	{
	    ...
	    duve_cb->duve_msg->du_clerr = NMloc(FILES, FILENAME,
    ...
    */
	...
	if (!(loinf.li_type & LO_IS_FILE))
	{
	    ...
	    */
	    STlcopy(duve_cb->duve_log.du_floc.path, temppath, MAX_LOC);
	    err_param[0].er_size  = (i4) 0; 
    ...
    ** exist and append to it if it does exist */
    if ((duve_cb->duve_msg->du_clerr = SIopen(&duve_cb->duve_log.du_floc, "a", 
	    &duve_cb->duve_log.du_file)
	) != OK
       )
	return(duve_error(duve_cb->duve_msg, TRUE,
			  E_DU5023_CANT_OPEN_LOG_FILE, 0, err_param));
