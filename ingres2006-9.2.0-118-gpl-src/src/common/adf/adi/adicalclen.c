/*
** Copyright (c) 1986 - 1999, Ingres Corporation
*/
...
# endif
{
    ...
    if( adi_lenspec->adi_lncompute == ADI_LEN_EXTERN ||
	adi_lenspec->adi_lncompute == ADI_LEN_EXTERN_COUNTVEC ||
	...
	if( status == E_DB_OK)
	{
		...
		*/
	    ...
	    else if (adi_lenspec->adi_lncompute == ADI_LEN_EXTERN_COUNTVEC)
	    {
    ...
    */
	   ...
	   abs(adi_dv[1]->db_datatype) == DB_INT_TYPE)
	{
	    ...
	    if (fi_desc->adi_fiopid == ADI_NVCHAR_OP)
	    	rlen *= sizeof(UCS2);
		...
		fi_desc->adi_fiopid == ADI_NVCHAR_OP)
		  rlen += DB_CNTSIZE;
	    /* Only use the second argument if the length is valid */
	    if( rlen > 0 && rlen < adi_dvr->db_length)
		adi_dvr->db_length = rlen;
	}
    }
