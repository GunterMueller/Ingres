...
DB_DATA_VALUE      *adc_dvhg)
{
	...
	if ((adf_scb->adf_utf8_flag & AD_UTF8_ENABLED) &&
		adc_dvfrom->db_datatype == DB_CHA_TYPE)
	    goto UTF8prep;
	...
	*/
	...
	if ((adf_scb->adf_utf8_flag & AD_UTF8_ENABLED) &&
		(adc_dvfrom->db_datatype == DB_VCH_TYPE ||
		 adc_dvfrom->db_datatype == DB_TXT_TYPE))
	    goto UTF8prep;
	...
	break;
     ...
     case DB_NVCHR_TYPE:
     {
	   ...
	   UCS2		cw[DB_MAX_HIST_LENGTH+DB_CNTSIZE/sizeof(UCS2)];
	   i2		res_len, save_len;

	   res_dv.db_data = (char *)&cw[0];
	   res_dv.db_datatype = DB_VBYTE_TYPE;
	   res_dv.db_length = DB_MAX_HIST_LENGTH;
	   res_dv.db_prec = 0;
	   res_dv.db_collID = -1;
	   ...
	   if (adc_dvfrom->db_datatype == DB_NVCHR_TYPE)
	   {
		...
		for (i = ((DB_NVCHR_STRING *)adc_dvfrom->db_data)->count;
		    i > 0 && ((DB_NVCHR_STRING *)adc_dvfrom->db_data)->
			element_array[i-1] == U_BLANK; i--);
	   ...
	   /* Reset length of NVARCHAR. */
	   if (adc_dvfrom->db_datatype == DB_NVCHR_TYPE)
		((DB_NVCHR_STRING *)adc_dvfrom->db_data)->count = save_len;

	   /* Copy result to adc_dvhg & zero fill. */
	   MEfill(adc_dvhg->db_length, 0, adc_dvhg->db_data);
	   res_len = ((DB_TEXT_STRING *)res_dv.db_data)->db_t_count -
			sizeof(UCS2);	/* 1st element is always 0 */
	   if (res_len > adc_dvhg->db_length)
		res_len = adc_dvhg->db_length;
	   MEcopy((char *)&((DB_NVCHR_STRING *)res_dv.db_data)->
			element_array[1], res_len, adc_dvhg->db_data);
...
UTF8prep:
    ...
    ** UTF8 to UCS2, then get collation weight (as for Unicode types). */
	...
	UCS2		uval[(DB_MAX_HIST_LENGTH+DB_CNTSIZE)/sizeof(UCS2)];
	UCS2		cw[(DB_MAX_HIST_LENGTH+DB_CNTSIZE)/sizeof(UCS2)];
	i4		res_len, i;

	utf8_dv.db_data = (char *)&uval[0];
	utf8_dv.db_datatype = DB_NVCHR_TYPE;
	utf8_dv.db_length = DB_MAX_HIST_LENGTH;
	utf8_dv.db_prec = 0;
	utf8_dv.db_collID = -1;

	if (db_stat = adu_nvchr_fromutf8(adf_scb, adc_dvfrom, &utf8_dv, -1))
	    return(db_stat);
	...
	/* Strip trailing blanks from resulting NVARCHAR. */
	for (i = ((DB_NVCHR_STRING *)utf8_dv.db_data)->count;
		    i > 0 && ((DB_NVCHR_STRING *)utf8_dv.db_data)->
			element_array[i-1] == U_BLANK; i--);
	((DB_NVCHR_STRING *)utf8_dv.db_data)->count = i;
