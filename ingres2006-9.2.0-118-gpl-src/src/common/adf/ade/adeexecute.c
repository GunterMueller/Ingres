...
# endif
{
    i4			cx_value = TRUE;
    i4			pm_quel;
    u_char		esc_char;	/* used to hold the esc char for LIKE */
    UCS2		uesc_char;	/* ditto for Unicode */
    u_char		*esc_ptr = NULL;   /* start with no esc char for LIKE */
    UCS2		*uesc_ptr = NULL;  /* ditto for Unicode */
    UCS2		uesc_null = U_NULL;
    i4			*dcmp;
    i4			cmp;
    i4			i;
    i4                  rangekey_flag;
	...
	*/
	...
	else
	{
		...
		*/
		...
		if (oprP[i]->opr_base == lbase)
		{
			...
			*/
			...
			if (oprP[i]->opr_len > MAXI2)
			    oprP[i]->opr_len = MAXI2;
			if (lbase_size < 0)
	  ...
	  /* ----------------------- */
          ...
          case ADE_NOT:
	    if      (cx_value == TRUE) cx_value = FALSE;
	    else if (cx_value == FALSE) cx_value = TRUE;
	    break;
	  ...
	  case ADE_ANDL:	/* labelled version of AND */
	    ...
	    ** form (classic Ingres). As such, cx_value being FALSE means 
...
/* {@fix_me@} */
	  ...
	  case ADE_ORL:		/* labelled OR */
	    /* Labelled OR takes branch if cx_value is TRUE, else keep going */
	    if (cx_value == TRUE)
	    {
		next_instr_offset = (i4)data[0];
	  ...
	  case ADE_BRANCHF:	/* branch if false (for case function) */
	    ...
	    if (cx_value == FALSE || cx_value == UNKNOWN)
	    {
		next_instr_offset = (i4)data[0];
		if (next_instr_offset > last_instr_offset)
		    next_instr_offset = ADE_END_OFFSET_LIST;
	  ...
	  case ADE_UNORM:
		...
		if ((dv[0].db_datatype == DB_NVCHR_TYPE) || 
		    ...
		    db_stat = adu_unorm(adf_scb, &dv[0], &dv[1], 0);
		else
	  ...
	  case ADE_AVGQ:
		...
		** do the division. */
		...
		break;
	      ...
	      case DB_DTE_TYPE:
            	...
            	adu_6to_dtntrnl (&dv[1], &dnag1);
            	adu_6to_dtntrnl (&dv[2], &dnag2);
		...
		** the result is also the stupid null date. */
	 	if (!(((AD_AGGDATE *)data[1])->tot_empty))
		{
			...
			((AD_AGGDATE *)data[1])->tot_secs) / (f8)count;
		    ...
		    ** the rest. */
		    ...
		    dnag1.dn_time = (i4)(f1 - (days * AD_40DTE_SECPERDAY));
		    if (dnag1.dn_time)
	  ...
	  case ADE_ESC_CHAR:
	    ...
	    esc_char = *(u_char *)data[0];
	    esc_ptr  = &esc_char;
	    uesc_char = (UCS2)*(u_char *)data[0];
	    uesc_ptr = &uesc_char;
	    break;
	  ...
	  case ADE_KEYBLD:
	    ...
	    if (kblk.adc_isescape = (esc_ptr != NULL))
		kblk.adc_escape = *esc_ptr;
		  ...
		  ** code is ADE_COMPARE fast path. */
		  ...
		  /* Setup operands */
	          ...
	          if (oprP[0]->opr_dt == DB_VCH_TYPE)
	          {
	              ln1 = *(u_i2*)c1;
	              c1 += 2;
		      if (ln1 < 0  ||  ln1 > adf_scb->adf_maxstring)
	          ...
	          if (oprP[1]->opr_dt == DB_VCH_TYPE)
	          {
	              ln2 = *(u_i2*)c2;
	              c2 += 2;
		      if (ln2 < 0  ||  ln2 > adf_scb->adf_maxstring)
			  return (adu_error(adf_scb, E_AD1014_BAD_VALUE_FOR_DT, 0));
	          }
	          else
	              ln2 = oprP[1]->opr_len;
	 ...
	 case ADE_PNAME:
	 {
	  ...
	  case ADFI_464_AVG_DATE:
	    ...
	    ** old aggregation functions. */
		...
		((AD_AGGDATE *)data[0])->tot_secs += dnag2.dn_time;
	    break;
	...
	/* copy one less byte if datatype nullable */
	  ...
	  case ADFI_465_SUM_DATE:
	    ...
	    ** normalized. */
		...
		i4	day1, day2;
		day2 = dnag2.dn_day;
		day1 = dnag1.dn_day + day2;
	    }	
	    if (i & AD_DN_TIMESPEC) 
				dnag1.dn_time += dnag2.dn_time;
...
strCompare:
	    ...
	    ad0_opr2dv(base_array, oprP[0], &dv[1]);
	    ad0_opr2dv(base_array, oprP[1], &dv[2]);
            if (db_stat = adu_like(adf_scb, &dv[1], &dv[2], esc_ptr, &cmp))
		return(db_stat);
            cx_value = (cmp == 0);
	    esc_ptr = NULL;	/* turn off esc char if there was one */
            break;
	    ...
	    ad0_opr2dv(base_array, oprP[0], &dv[1]);
	    ad0_opr2dv(base_array, oprP[1], &dv[2]);
	    if (db_stat = adu_ulike(adf_scb, &dv[1], &dv[2],
			(pm_quel || uesc_ptr) ? uesc_ptr : &uesc_null, &cmp))
		return(db_stat);
            cx_value = (cmp == 0);
	    uesc_ptr = NULL;	/* turn off esc char if there was one */
            break;
	  ...
	  case ADFI_1314_SIGN_DEC:
	    esc_char = 0x0c;		/* borrow esc_char for the compare */
