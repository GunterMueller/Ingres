...
ADF_AG_OLAP	*olapag_struct)
{
    ...
    {
	if (((u_char *)dv_y->db_data)[dv_y->db_length-1] != 0) return;
    }
    if (dv_x->db_datatype < 0)
    {
	if (((u_char *)dv_x->db_data)[dv_x->db_length-1] != 0) return;
    }
    /* Increment value counter. */
    olapag_struct->adf_agcnt++;

    /* Load x, y data values. */
    if (dv_y->db_length <= 5) locyf8 = *(f4 *)dv_y->db_data;
    else locyf8 = *(f8 *)dv_y->db_data;
    if (dv_x->db_length <= 5) locxf8 = *(f4 *)dv_x->db_data;
    else locxf8 = *(f8 *)dv_x->db_data;
...
DB_DATA_VALUE	*dv_result)
{
    ...
    f8	sumx, sumx2, sumy, sumy2, sumxy;
	...
	*(f8 *)dv_result->db_data = sqrt(MHipow((count*sumxy - sumx*sumy), 2) / 
...

{
	...
	*(f8 *)dv_result->db_data = (sumxy - sumx*sumy/count) / count;
    }
...

{
	...
	*(f8 *)dv_result->db_data = (sumxy - sumx*sumy/count) / (count-1);
    }
...

{
	...
	*(f8 *)dv_result->db_data = olapag_struct->adf_agfrsv[0] / count;
    }
...

{
	...
	*(f8 *)dv_result->db_data = olapag_struct->adf_agfrsv[1] / count;
    }
...

{
    ...
    /* regr_count(y, x) simply re4turns the row count. */

    *(i4 *)dv_result->db_data = olapag_struct->adf_agcnt;
...
DB_DATA_VALUE	*dv_result)
{
	...
	*(f8 *)dv_result->db_data = (sumy*sumx2 - sumx*sumxy) / 
		(count*sumx2 - sumx*sumx);
    }
...
DB_DATA_VALUE	*dv_result)
{
	...
	*/
	...
	if (count*sumy2 == sumy*sumy) *(f8 *)dv_result->db_data = 1.0;
	else
	    *(f8 *)dv_result->db_data = MHipow((count*sumxy - sumx*sumy), 2) / 
		((count*sumx2 - sumx*sumx) * (count*sumy2 - sumy*sumy));
    }
...
DB_DATA_VALUE	*dv_result)
{
	...
	*(f8 *)dv_result->db_data = (count*sumxy - sumx*sumy) / 
		(count*sumx2 - sumx*sumx);
    }
...
DB_DATA_VALUE	*dv_result)
{
	...
	*(f8 *)dv_result->db_data = sumx2 - sumx*sumx/count;
    }
...
DB_DATA_VALUE	*dv_result)
{
	...
	*(f8 *)dv_result->db_data = sumxy - sumx*sumy/count;
    }
...
DB_DATA_VALUE	*dv_result)
{
	...
	*(f8 *)dv_result->db_data = sumy2 - sumy*sumy/count;
    }
