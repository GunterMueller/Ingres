...
# endif /* xDEBUG */
static i4 month_hours[] = {0, 0, 744, 1440, 2184, 2904, 3648, 4368,
...
DB_DATA_VALUE	    *date_dv2)
{
    ...
    curnd.dn_time	= cur_tv.tm_hour * AD_39DTE_ISECPERHOUR +
	...
	{
	  /* Convert to UTC */
	  dn->dn_time -= TMtz_search (adf_scb->adf_tzcb, TM_TIMETYPE_LOCAL, 
          ...
          if (abs(date_dv2->db_datatype) == DB_TSWO_TYPE)
	  {
            ...
            dn->dn_tzminute = 0;
	  	   ...
	  	   (abs(date_dv2->db_datatype) == DB_TME_TYPE) ||
	           (abs(date_dv2->db_datatype) == DB_TSTMP_TYPE))
	  { /* convert to UTC */
	    dn->dn_time -= ((dn->dn_tzhour * 3600) + (dn->dn_tzminute * 60));
            dn->dn_tzhour = 0;
            dn->dn_tzminute = 0;
            adu_1normldate(dn);
	  }
...
#define D_VAL_ARRAY_SIZE	16
    ...
    */
	...
	/* In this loop we look at most-common cases first.  At least,
	** most common in frontend catalogs.
	*/
...
#       endif
	...
	switch (*p)
	{
	  ...
	  case 'g':
	    ...
	    s = dsizp[ip - *dvalp];
	    /* Truncate the nanosecond values beyond 9 digits */
	    if (s > 9)
		s = 9;
	    dv->dv_nanosecond = *ip++;
...
#define	    AD_TV_NORMMON		   1
	...
	ctv_init = TRUE;
        ...
        dn->dn_time	= cur_tv.tm_hour * AD_39DTE_ISECPERHOUR +
			  cur_tv.tm_min * AD_10DTE_ISECPERMIN +
			  cur_tv.tm_sec;
...
#       endif
	...
	if (status & DV_AMPM)
	{
            ...
            if (datetype == DV_ABSOLUTE || datetype == DV_OFFSET)
            {
                ...
                if (dv->dv_hour == 12)
                     msec -= (dv->dv_hour * AD_39DTE_ISECPERHOUR);

                msec += (dv->dv_ampm * AD_39DTE_ISECPERHOUR); 
	    }
	    else
	    {
...
#       ifdef xDEBUG
        if (ult_check_macro(&Adf_globs->Adf_trvect, ADF_004_DATE_SUPPORT,
		&dum1, &dum2))
            TRdisplay("date: after seconds we have %d msecs\n", msec);
#       endif
	...
	** Note: For time/timestamp with/without timezone the msec is 
	** adjusted as follows:
	...
	*/
        ...
        if (datetype != DV_LENGTH)
	{
	    	    ...
	    	    (dn->dn_dttype == DB_TSW_TYPE))
		{
	   	    ...
	   	    (dn->dn_dttype != DB_TSWO_TYPE))
                    msec -= ((i4) dv->dv_timezone * AD_10DTE_ISECPERMIN);
	    }
	    else if ((dn->dn_dttype != DB_TMWO_TYPE) && 
			(dn->dn_dttype != DB_TMW_TYPE) && 
	   		(dn->dn_dttype != DB_TSWO_TYPE) && 
	    		(dn->dn_dttype != DB_TSW_TYPE))
	    {
		msec -= TMtz_search(adf_scb->adf_tzcb, TM_TIMETYPE_LOCAL, 
			     adu_5time_of_date(dn));
#   ifdef xDEBUG
...
DB_DATA_VALUE	    *str_dv)
{
    ...
    */
    ...
    local_offset.dn_time = TMtz_search(adf_scb->adf_tzcb, 
...
#       ifdef xDEBUG
		...
		&dum1, &dum2))
			...
			(i4)nd->dn_year);
	    TRdisplay("month = %d, day = %d, time = %d.%d, status = %d\n",
			(i4)nd->dn_month,
			(i4)((I1_CHECK_MACRO(nd->dn_day))),
			nd->dn_time, nd->dn_nsecond, nd->dn_status);
#       endif
...
#       endif
		  ...
		  nd->dn_dttype == DB_TMWO_TYPE)
	{
	    ...
	    for (cnt = 9; cnt > date_dv->db_prec; cnt--)
	      ms /= 10;
	    ...
	    if ( nd->dn_dttype != DB_TMW_TYPE) 
	    {
	      if (date_dv->db_prec != 0)
	      {
                ...
                STprintf(temp, formatbuf, hr, mn, sc, ms);
	      }
	      else
                STprintf(temp, "%02d:%02d:%02d",
                        hr, mn, sc);
	...
	do
	{
		 ...
		 if (timeval < 0)
	 	 ...
	 	 if (timeval < 0)	
			...
			abs(nd->dn_year), abs(nd->dn_month) );
                   ...
                   STprintf(temp, "%d-%02d", 
			nd->dn_year, nd->dn_month * timeval);
		}
	    }
		...
		hr = nd->dn_time;
		  ...
		  if ((timeval < 0) || (nd->dn_day < 0))
			   ...
			   abs(mn), abs(sc), abs(ms));
		  else
                     STprintf(temp, "%d %02d:%02d:%02d", 
			   nd->dn_day, hr, mn, sc, ms);
	       } 
	    }
	    else 
	    {
		...
		if (hr != 0  ||  (mn == 0  &&  sc == 0  &&  ms == 0))
		{
		    _VOID_ CVla((i4)(hr * timeval), tmpbuffer);
		    STpolycat((i4)4,
				temp,
				(need_space ? " " : ""),
				tmpbuffer,
		...
		if (mn != 0)
		{
		    _VOID_ CVla((i4)(mn * timeval), tmpbuffer);
		    STpolycat((i4)4,
				temp,
				(need_space ? " " : ""),
				tmpbuffer,
		...
		if (sc != 0)
		{
		    _VOID_ CVla((i4)(sc * timeval), tmpbuffer);
		    STpolycat((i4)4,
				temp,
				(need_space ? " " : ""),
				tmpbuffer,
		...
		if (ms != 0)
		{
		    _VOID_ CVla((i4)(ms * timeval), tmpbuffer);
		    STpolycat((i4)4,
				temp,
				(need_space ? " " : ""),
				tmpbuffer,
...
**	    dn_time in seconds and fraction seconds part in dn_nseconds.
**	07-nov-2006 (gupsh01)
	...
	*/
	...
	if (nd->dn_status & AD_DN_TIMESPEC)
	{
	    ...
	    */
	    ...
	    {
	        nd->dn_time += TMtz_search(adf_scb->adf_tzcb, 
			     TM_TIMETYPE_GMT, 
...
DB_DATA_VALUE	*hg_dv)
{
	...
	** A 24 hour discontinuity once per year isn't too bad! */
	...
	if (local_datep.dn_time > 0)
	    mapval += local_datep.dn_time / AD_39DTE_ISECPERHOUR ;
	else if (local_datep.dn_time < 0)
	{
	    f8	f8val;
	    char	fstr[DBL_DIG + 4];
	    i1	i;

	    for (i = DBL_DIG - 1, STcopy("0.", fstr); i--;)
		STcat(fstr, "9");

	    CVaf(fstr, '.', &f8val);

	    mapval += (i4)(((f8)local_datep.dn_time / AD_39DTE_ISECPERHOUR) - f8val);
	}
	break;
	...
	** milliseconds in value. */
	mapval = (local_datep.dn_day * AD_40DTE_SECPERDAY + local_datep.dn_time)
			* 1000 + (local_datep.dn_nsecond / 1000000);
	break;
...
#define  AD_400_IDAYPER400  (1+4*AD_100_IDAYPER100) /* # days in quad century */
	...
	*/
    ...
    datep->dn_time = 0;             /* initialize time = 00:00:00*/
...
DB_DATA_VALUE  *result)
{
	...
	if (datep->dn_status == AD_DN_NULL)
	{
	    ...
	    * (f8)AD_40DTE_SECPERDAY
        ...
        case 'I':
            *res = nsecs / AD_43DTE_SECPERMIN;
            break;

        case 'H':
            *res = nsecs / AD_44DTE_SECPERHOUR;
            break;

        case 'D':
            *res = nsecs / AD_40DTE_SECPERDAY;
            break;

        case 'M':
            *res = nsecs / (AD_40DTE_SECPERDAY * AD_13DTE_DAYPERMONTH);
            break;
        ...
        case 'W':
            *res = nsecs / (AD_40DTE_SECPERDAY * AD_12DTE_DAYPERWEEK);
            break;

        case 'Y':
            *res = nsecs / (AD_40DTE_SECPERDAY * AD_14DTE_DAYPERYEAR);
            break;
        default:
...
DB_DATA_VALUE       *date_result)
{
    ...
    */
    ...
    if (datep->dn_status == AD_DN_NULL)
    {
	/* If the empty date, just return an empty date */
	return (E_DB_OK);
    }
    ...
    switch (dateclass)
    {
      ...
      case 'I':
	    ...
	    */
	    ...
	    datep->dn_time -= (datep->dn_time % AD_10DTE_ISECPERMIN);
        }
        break;
      case 'H':
        /* truncate hours */
	    ...
	    */
	    adu_1normldate(datep);
	    datep->dn_time -= (datep->dn_time % AD_39DTE_ISECPERHOUR);
        }
        break;
      case 'D':
        /* truncate days */
	    ...
	    if (tzadjust)
	    {
	      datep->dn_time += TMtz_search(adf_scb->adf_tzcb, 
			      TM_TIMETYPE_GMT, 
...
DB_DATA_VALUE       *result)
{
	...
	*/
	...
	switch (dateclass)
	{
	  case 'S':
	    /* extract seconds */
	    component = datep->dn_time % AD_10DTE_ISECPERMIN;
	    break;

	  case 'I':
	    /* extract minutes */
	    component= (datep->dn_time/AD_10DTE_ISECPERMIN) % AD_11DTE_IMINPERHOUR;
	    break;

	  case 'H':
	    /* extract hours */
	    component = (datep->dn_time/AD_11DTE_IMINPERHOUR/
						AD_10DTE_ISECPERMIN);
	    break;
	     ...
	     i4		*error)
{
	...
	*error = E_AD5063_BAD_SYSTIME;
	...
	time->tv_nsec >= 0 && time->tv_nsec < 1000000000)
    {
...
DB_DATA_VALUE   *date_dv)
{
    ...
    */
	...
	{ 
	    /* This character is not allowed in an ANSI date format
	    ** There may be issues with Date coercion from Ingres
	    ** to ANSI date type. In which case this may be
	    ** an Issue.
	    */
	    /* This is a delimiter, legal in an empty date. */
	    *datespec++ = chr;
