...
#   ifdef xDEBUG
    if (ult_check_macro(&Adf_globs->Adf_trvect, ADF_006_NORMDATE, &dum1, &dum2))
    {
        ...
        TRdisplay("normldate:and time=%d.%d, status=%d\n",
            d->dn_time, d->dn_nsecond, d->dn_status);
    }
#   endif
    ...
    if (d->dn_time < 0 || d->dn_time >= AD_41DTE_ISECPERDAY)
    {
	t = d->dn_time / AD_41DTE_ISECPERDAY;
	days += t;
	d->dn_time -= (t * AD_41DTE_ISECPERDAY);
	/* if time is still negative, then bring it positive */
	if (d->dn_time < 0)
	{
	    days--;
	    d->dn_time += AD_41DTE_ISECPERDAY;
	}
    }
...
AD_NEWDTNTRNL   *d)
{
    ...
    d->dn_time   = time = flt_dn_time + (flt_dn_time >= 0.0 ? 0.5 : -0.5);
...
i4		*cmp)
{
    ...
    */
    ...
    else if(!(d1->dn_status & AD_DN_ABSOLUTE && d2->dn_status & AD_DN_ABSOLUTE))
    {
        ...
        else        /* both dates are intervals */
        {
            ...
            if (!(tmp_val = days1 - days2))
                tmp_val = d1->dn_time - d2->dn_time;
        }
    }
    else
    {
	if ((d1->dn_dttype == DB_TMW_TYPE) || (d1->dn_dttype == DB_TSW_TYPE))
		d1->dn_time -= (d1->dn_tzhour * 3600 + d1->dn_tzminute * 60);

	if ((d2->dn_dttype == DB_TMW_TYPE) || (d2->dn_dttype == DB_TSW_TYPE))
		d2->dn_time -= (d2->dn_tzhour * 3600 + d2->dn_tzminute * 60);
	     ...
	     (d1->dn_dttype == DB_TSTMP_TYPE)) 
	{
		...
		d2->dn_time = -(TMtz_search(adf_scb->adf_tzcb, 
					     TM_TIMETYPE_LOCAL, 
		...
		d1->dn_time = -(TMtz_search(adf_scb->adf_tzcb, 
					     TM_TIMETYPE_LOCAL, 
...
#define DAYS_SINCE_1970 719163
    ...
    */
    ...
    maxval = MAXI4 - (d->dn_time)/60;
    minval = MINI4 + (d->dn_time)/60;

    /* if( sec > (MAXI4 - (d->dn_time)/60)) */
    if (sec > maxval)
	sec = MAXI4;
    if (sec < minval)
    /* else if( sec < (MINI4 - (d->dn_time)/60)) */
	sec = MINI4;
    else
	sec = sec*60 + d->dn_time;
...

{
	...
	outval->dn_time	= ing_date.dn_time/1000;
	outval->dn_nsecond = 
		(ing_date.dn_time % 1000) * AD_38DTE_NSPERMS;
	...
	outval->dn_time = outval->dn_nsecond = 
		outval->dn_tzhour = outval->dn_tzminute = 0;
	...
	outval->dn_time = atime.dn_time;
	outval->dn_nsecond = atime.dn_nsecond;
	outval->dn_status = AD_DN_ABSOLUTE; 
	if (outval->dn_time || outval->dn_nsecond)
	{
	...
	outval->dn_time = atimestamp.dn_time;
	...
	outval->dn_day = outval->dn_time = outval->dn_nsecond = 
		outval->dn_tzhour = outval->dn_tzminute = 0;
...

{
	...
	ing_date.dn_time = inval->dn_time * 1000 + 
					inval->dn_nsecond/1000000;
	...
	*/
	...
	atimestamp.dn_time = inval->dn_time;
