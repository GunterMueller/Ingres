...
ADF_AG_STRUCT	*ag_struct)
{
    ...
    f8			*imsecs	  = &ag_struct->adf_agfrsv[0];
    AD_NEWDTNTRNL	datenext_data;
    AD_NEWDTNTRNL	*datenext = &datenext_data;
    register i1		status;
    
    MEfill ((u_i2)sizeof(datenext), NULLCHAR, (PTR) datenext);
    adu_6to_dtntrnl (dv_next, datenext);
	...
	*imonths += datenext->dn_month;
	...
	/* a u_i4.  If this gets casted to an f8 and added to imsecs, we    */
	/* will get weird results.  (This case arises when we have negative */
	/* days in a date interval.)					    */
	*imsecs += (i4)(I1_CHECK_MACRO(datenext->dn_day)) * AD_40DTE_SECPERDAY;
    }

    if (status & AD_DN_TIMESPEC)
	*imsecs += (f8)datenext->dn_time;
...
ADF_AG_STRUCT	*ag_struct)
{
	...
	ag_struct->adf_agcnt++;
        ...
        MEfill ((u_i2)sizeof(datemin), NULLCHAR, (PTR) &datemin);
        adu_6to_dtntrnl (curmin, &datemin);
...
ADF_AG_STRUCT	*ag_struct)
{
	...
	ag_struct->adf_agcnt++;
        ...
        MEfill ((u_i2)sizeof(datemax), NULLCHAR, (PTR) &datemax);
        adu_6to_dtntrnl (curmax, &datemax);
...
DB_DATA_VALUE	*dv_result)
{
    ...
    i4			*imonths  = &ag_struct->adf_agirsv[2];
    f8			*imsecs	  = &ag_struct->adf_agfrsv[0];
    AD_NEWDTNTRNL	resdata;
    AD_NEWDTNTRNL	*res = &resdata;
    i4			mremain;
    i4			days;
    f8			ftemp;

    MEfill ((u_i2)sizeof(res), NULLCHAR, (PTR) res);
    adu_6to_dtntrnl (dv_result, res);
	...
	adu_7from_dtntrnl (dv_result, res);
	return(E_DB_OK);
    }
	...
	*istat |= AD_DN_MONTHSPEC;
    ...
    ftemp = (mremain * AD_13DTE_DAYPERMONTH * AD_40DTE_SECPERDAY + *imsecs)
	    / (f8)ag_struct->adf_agcnt;

    /* we have to partially normalize now because the imsecs f8 might not   */
    /* fit into an i4 so we can't just put it into res->dn_time		    */
    
    days = (i4)(ftemp / AD_40DTE_SECPERDAY);
    if (days != 0)
	*istat |= AD_DN_DAYSPEC;
	
    res->dn_time = (i4)(ftemp - (days * AD_40DTE_SECPERDAY));
    if (res->dn_time != 0)
...
DB_DATA_VALUE	*dv_result)
{
    ...
    /* we have to partially normalize now because the imsecs f8 might not   */
    /* fit into an i4 so we can't just put it into res->dn_time		    */
    days	    = (i4)(*imsecs / AD_40DTE_SECPERDAY);
    res->dn_time    = (i4)(*imsecs - (days * AD_40DTE_SECPERDAY));
