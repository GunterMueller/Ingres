...
static char	*coll_array[] = {" ", "unicode ", "unicode_case_insensitive ",
...
DB_DATA_VALUE      *rdv)
{
    ...
    */
	...
	{
	    *cptr++ = '(';
	    CVna((i4)DB_P_DECODE_MACRO(dv1->db_prec), cptr);
	    cptr += STlength(cptr);
	    *cptr++ = ',';
	    *cptr++ = ' ';
	    CVna((i4)DB_S_DECODE_MACRO(dv1->db_prec), cptr);
	    cptr += STlength(cptr);
	    *cptr++ = ')';
	}
	else
	...
	STcopy(" collate ", cptr);
	cptr += 9;
	STcopy((char *) coll_array[dv1->db_collID], cptr);
	cptr += STlength((char *) coll_array[dv1->db_collID]);
    }
	...
	tmp_dv.db_length--;
	db_stat = adu_movestring(adf_scb, (u_char *)buf, cptr-buf, &tmp_dv);
    }
    else
    {
	db_stat = adu_movestring(adf_scb, (u_char *)buf, cptr-buf, rdv);
    }
