...
i4                  *rcmp)
{
    ...
    */
    ...
    while (( db_status == 0) &&
	((pop_cb1.pop_error.err_code != E_AD7001_ADP_NONEXT) && 
	 (pop_cb2.pop_error.err_code != E_AD7001_ADP_NONEXT)));
...
i4		*rcmp)
{
	...
	return(adu_nvchr_utf8comp(adf_scb, dv1, dv2, rcmp));
	...
	if (use_quel_pm)
	{
	    *rcmp = ad0_1lexc_pm(p1, endp1, p2, endp2,
				 blank_pad, ignore_blanks);
	}
	else
	{
...
i4		    *rcmp)
{
    ...
    while (n && pptr < endp)
    {
	if (!bignore || !CMspace(pptr) && *pptr != NULLCHAR)
	    n--;
	CMnext(pptr);
    }
