       ...
       i2	collID)
{
    ...
    return( MakeCE(adf_scb, bst, est, (u_i2*)0, listp, opts, 0) );
}

/*
       ...
       i4	collID)
{
    ...
    return( MakeCE(adf_scb, bst, est, esc, listp, opts, 0) );
}

/*
...
**	listp	- points to address into which return list addr is placed.
**		If ret list addr is NULL, MakeCE allocates memory for list
**		and requires caller to free it.
**	cecount	- count of elements in CE array (if pre-allocated).
**
** Outputs:
**	listp     - points to the produced list. The caller
**		    is responsible for MEfree-ing the list.
	...
	i4	cecount)
{
    ...
    /* *listp = (u_i2*)NULL;    removed because it's init'ed in all
    **				callers but adu_ucollweight() which
    **				passes pre-allocated addr. 
    */
    ...
    ** of zero.
    */
    for ( sptr = bst, num_sch = 0, num_ces = 0;
	  sptr < est;
	  sptr++, num_ces++ )
    {
    ...
    {
	num_sch = 0;
	sizesk = (sizeof(u_i2) * ((num_ces * max_levels) + 1))
	    /* for level separator */ + ((max_levels - 1) * sizeof(u_i2));
    }
    ...
    */
    ...
    /* Get memory for sort key list, if wanted */
    if ( opts & Sortkeys )
    {
	if ((sk_mem = (char *)*listp) == NULL)
	    sk_mem = MEreqmem(0, sizesk, TRUE, &status);
    ...
    */
	...
	if ( opts & LikeCE && cetable[*sptr].comb_class == 0 )
	{
		...
		{
		    /* Quel form or SQL and not an escaped character */
		    switch ( *sptr )
		    {
			case '-':
	...
	*/
    ...
    celist[0] = (((char*)ce - (char*)celist) / sizeof(*ce)) - 1;
	...
	for ( k = 1, l = 0; l < max_levels; l++ )
	{
	    /* NOTE: tot_levels controls loop because ignored levels
			 ...
			 DB_DATA_VALUE	*dst_dv)
{
   ...
   if ( src_dv->db_datatype != DB_NCHR_TYPE && 
    ...
    dst = (u_i2 *) &((DB_TEXT_STRING *)dst_dv->db_data)->db_t_text;
    dstlen = dst_dv->db_length - DB_CNTSIZE;
    ...
    db_stat = MakeCE(adf_scb, bst, est, (u_i2*)0, &dst, opts, 
			(dst_dv->db_length-2 /2));

    if (db_stat == E_DB_OK)
    {
	/* Reset result length from CE count and set CE count to 0. */
	dstlen = ((DB_NVCHR_STRING *)dst_dv->db_data)->element_array[0];
#ifndef BYTE_SWAP
	for (i = 1; i <= dstlen; i++)
	{
	    /* For byte swapped platforms, flip each u_i2 since result of
	    ** function is char array, not u_i2 array. */
	    UCS2	workelement;
	    char	*workp = (char *)&workelement;
	    char	x;

	    workelement = ((DB_NVCHR_STRING *)dst_dv->db_data)->element_array[i];
	    x = workp[0];
	    workp[0] = workp[1];
	    workp[1] = x;
	    ((DB_NVCHR_STRING *)dst_dv->db_data)->element_array[i] = workelement;
	}
#endif
	((DB_TEXT_STRING *)dst_dv->db_data)->db_t_count = 2*(dstlen+1);
	((DB_NVCHR_STRING *)dst_dv->db_data)->element_array[0] = 0x0000;
    }
