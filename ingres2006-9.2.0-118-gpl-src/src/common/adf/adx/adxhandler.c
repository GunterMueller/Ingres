...
# else
DB_STATUS
adx_handler( adf_scb, adx_exstruct)
ADF_CB             *adf_scb;
EX_ARGS            *adx_exstruct;
# endif
{
	...
	*/
	...
	case EXDECDIV:
	    ...
	    if (!hard && adf_scb->adf_exmathopt == ADX_IGN_MATHEX)
	    {
		adf_scb->adf_errcb.ad_errcode = E_AD0001_EX_IGN_CONT;
		return (E_DB_OK);
	    }
	    else if (!hard && adf_scb->adf_exmathopt == ADX_WRN_MATHEX)
	    {
	    ...
	    else if (hard || adf_scb->adf_exmathopt == ADX_ERR_MATHEX)
	    {
		switch (adx_exstruct->exarg_num)
		{
		  case EXFLTOVF:
	...
	case MH_BADARG:	/* Bad arg to MH routine ... always generate an error */
