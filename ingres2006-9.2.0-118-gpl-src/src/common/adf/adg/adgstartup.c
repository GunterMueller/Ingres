...
adg_srv_size()
{
	...
	sz_coerctab = (	Adi_fis_size / sizeof(ADI_FI_DESC)
			- ADZ_COERCION_TY_FIIDX + 227) * sizeof(ADI_COERC_ENT);
				/* added 210 entries for dtfamily hack */
...
/*  scb->adf_owner = ?????  -- Don't set owner; ADF doesn't own this struct */
    ...
    */ 
    ...
    if (CMget_attr(&isdouble, &isUTF8) == OK)  
    {
        if (isdouble)
...
-- old largest was %d.\n",
	...
	num_cos += 227;			/* replicate hack of adg_srv_size(). */
...
"Ad0_dtinit: Invalid underlying type (%d./%<%x) for dtd_id (%d./%<%x)\n\
    ...
    "Ad0_dtinit: Invalid name <%t> for datatype number %d. (zero based)\n",
	    ...
	    {
		TRdisplay(
		    "Ad0_dtinit: Adding datatype %t (%d./%<%x) into table\n",
    ...
    {
	STRUCT_ASSIGN_MACRO(zeromask, dt->adi_dtcoerce);
...
"Ad0_dtinit:  Function instance id %d./%<%x specifies an unknown\n\
 parameter type id %d.(%<%x).\n",
	...
	BTset((i4) ADI_DT_MAP_MACRO(hold_into), (char *) &dt->adi_dtcoerce);
