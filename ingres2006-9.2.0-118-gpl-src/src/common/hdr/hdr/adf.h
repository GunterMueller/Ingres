...
typedef struct _ADI_WARN
{
    ...
    i4		    ad_1rsvd_cnt;	/* <RESERVED> */
#define		    AD_1RSVD_EXEDBP	0x1
}   ADI_WARN;
...
**      a math exception in the adx_handler() routine.  These 3 options
**      are:  Ignore, Warn, or Error.
**
** History:
**      19-feb-86 (thurston)
**          Initial creation.
...
#define                 ADI_COPY_COERCION   7
    ...
    i2		    adi_fiflags;	/* Function instance flags.  The
...
#define			ADI_F4096_LEGACY 4096 /* Legacy FI
					** This FI is a placeholded for legacy
					** function definitions. The resolver
					** will never choose this for execution.
					*/
...
#define                 ADC_KALLMATCH   7   /* The value given will match    */
    ...
    DB_DATA_VALUE    adc_hikey;		/* If adc_tykey = ADC_KHIGHKEY, this
                                        ...
                                        ** undefined.
                                        */
    u_char	    adc_escape;		/* adc_escape is the escape char that
					** the user has specified for the LIKE
					** (or NOT LIKE) operator.  adc_escape
					** has valid info in it *ONLY* if
					** adc_isescape == TRUE.
                                        */
    i1		    adc_isescape;	/* If adc_isescape is TRUE, then
...
typedef struct _ADF_FN_BLK
{
    ADI_FI_ID       adf_fi_id;          /* Function instance id for the
                                        ...
                                        */
    DB_DATA_VALUE   adf_r_dv;           /* Result data value.
					*/
    DB_DATA_VALUE   adf_1_dv;           /* First operand.
                                        */
    DB_DATA_VALUE   adf_2_dv;           /* Second operand.
                                        */
    DB_DATA_VALUE   adf_3_dv;           /* Third operand.
                                        */
    DB_DATA_VALUE   adf_4_dv;           /* Fourth operand.
                                        */
    u_char	    adf_escape;		/* If .adf_isescape is TRUE, then this
					** this is the `escape char'.  As with
					** .adf_isescape, this is only used for
					** LIKE and NOT LIKE comparison
					** function instances, and will be
					** ignored for all others.
					*/
    i1		    adf_isescape;	/* TRUE if there is an `escape char'.
					** This is only used for LIKE and
					** NOT LIKE comparison function
					** instances, and will be ignored for
					** all others.
					*/
}   ADF_FN_BLK;
...
FUNC_EXTERN VOID adu_7from_dtntrnl(
		              DB_DATA_VALUE   *outdv,
        		      AD_NEWDTNTRNL   *inval);

FUNC_EXTERN VOID adu_6to_dtntrnl(
        		      DB_DATA_VALUE   *indv,
