	...
	DB_ERROR	*error)
{
    ...
    ** check parms
    */
    error->err_code = E_CUF_OK;
    if (num_bufs < 1)
    {
	erlog = error->err_code = E_CU0101_BAD_BUFFER_COUNT;
	status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0, 0,
	...
	if (cut_rcb[i] == NULL)
	{
	    erlog = error->err_code = E_CU0102_NULL_BUFFER;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0,
	...
	{
	    erlog = error->err_code = E_CU0103_BAD_BUFFER_USE;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0,
	...
	{
	    erlog = error->err_code = E_CU0104_BAD_CELLS;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0,
    ...
    */
	...
	do
	{
	    ...
	    erlog = error->err_code = E_CU0105_NO_MEM;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0, 0,
	...
	if (cl_stat != OK)
	{
	    erlog = error->err_code = E_CU0106_INIT_SEM;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0, 0,
	...
	if (cl_stat != OK)
	{
	    erlog = error->err_code = E_CU0107_INIT_READ_COND;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0, 0,
		&erlog, 1, sizeof(cut_rcb[i]->cut_buf_name),
		cut_rcb[i]->cut_buf_name);
	    status = E_DB_ERROR;
	...
	if (cl_stat != OK)
	{
	    erlog = error->err_code = E_CU0108_INIT_WRITE_COND;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0, 0,
		&erlog, 1, sizeof(cut_rcb[i]->cut_buf_name),
		cut_rcb[i]->cut_buf_name);
	    status = E_DB_ERROR;
	...
	if (cl_stat != OK)
	{
	    erlog = error->err_code = E_CU0109_INIT_EVENT_COND;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0, 0,
		&erlog, 1, sizeof(cut_rcb[i]->cut_buf_name),
		cut_rcb[i]->cut_buf_name);
	    status = E_DB_ERROR;
	...
	if (cl_stat != OK)
	{
	    erlog = error->err_code = E_CU010A_NAME_COND;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0, 0,
		&erlog, 1, sizeof(cut_rcb[i]->cut_buf_name),
		cut_rcb[i]->cut_buf_name);
	    status = E_DB_ERROR;
	...
	DB_ERROR	*error)
{
    ...
    */
	...
	do
	{
		...
		CSv_semaphore(&cur_buf->cut_buf_sem);
		CSv_semaphore(&Buf_list_sem);
		error->err_code = W_CU0301_THREAD_ATTACHED;
		return(E_DB_WARN);
	    }
	...
	DB_ERROR	*error)
{
    ...
    */
    ...
    if (!cur_thread)
    {
	error->err_code = W_CU0302_THREAD_NOT_ATTACHED;
	CSv_semaphore(&cur_buf->cut_buf_sem);
	CSv_semaphore(&Buf_list_sem);
	return(E_DB_WARN);
    }
	...
	{
	    error->err_code = E_CU0201_BAD_THREAD_STATE;
	    CSv_semaphore(&cur_buf->cut_buf_sem);
	    CSv_semaphore(&Buf_list_sem);
	    return(E_DB_ERROR);
	}
		...
		cur_buf->cut_buf_info.cut_buf_head )
	{
	    ...
	    if (num_readers <= 1)
	    {
		error->err_code = E_CU0202_READ_CELLS;
		CSv_semaphore(&cur_buf->cut_buf_sem);
		CSv_semaphore(&Buf_list_sem);
		return(E_DB_ERROR);
	    }
	...
	DB_ERROR	*error)
{
	...
	case CUT_INFO_BUF:
	{
		...
		cut_rcb->cut_buf_handle)
    	    {
		CSv_semaphore(&cur_buf->cut_buf_sem);
		erlog = error->err_code = E_CU010C_INCONSISTENT_BUFFER;
		status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0,
	...
	case CUT_ATT_BUFS:
	{
	    ...
	    if (cl_stat != OK)
	    {
		erlog = error->err_code = E_CU010B_CANT_GET_SEM;
		status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0,
	    ...
	    if (Buf_list == NULL)
	    {
		error->err_code = W_CU0304_NO_BUFFERS;
		CSv_semaphore(&Buf_list_sem);
		return(E_DB_WARN);
	    }
	    if (cut_rcb == NULL)
		...
		if (cur_buf == Buf_list)
		{
		    error->err_code = I_CU0401_LAST_BUFFER;
		    CSv_semaphore(&Buf_list_sem);
		    return(E_DB_INFO);
		}
	        /*
	        ...
	        */
	        ...
	        {
		    CSv_semaphore(&Buf_list_sem);
		    erlog = error->err_code = E_CU010D_BUF_NOT_IN_LIST;
		    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 
	...
	case CUT_INFO_THREAD:
	{
	    ...
	    {
		error->err_code = E_CU010C_INCONSISTENT_BUFFER;
		return(E_DB_ERROR);
	    }
	    cl_stat = CSp_semaphore(TRUE, &cur_buf->cut_buf_sem);
	    if (cl_stat != OK)
	...
	DB_ERROR	*error)
{
    ...
    */
    if (cur_buf->cut_buf_info.cut_num_threads == 0)
    {
	erlog = error->err_code = E_CU0118_BAD_BUFFER;
	status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 
	    0, 0, 0, &erlog, 0); 
	return(E_DB_ERROR);
    }
    ...
    {
	error->err_code = erlog = E_CU010E_NO_CELLS;
	status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
	    0, 0, 0, &erlog, 0);
	return(E_DB_ERROR);
    }
    ...
    if (output_buf == NULL || (*output_buf == NULL &&
				!(flags & CUT_RW_DIRECT)) )
    {
	error->err_code = erlog = E_CU010F_NO_OUTPUT_BUF;
	status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
	    0, 0, 0, &erlog, 0);
	return(E_DB_ERROR);
    }
    ...
    */
	...
	{
	    CSv_semaphore(&cur_buf->cut_buf_sem);
	    error->err_code = erlog = E_CU0111_NOT_ATTACHED;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
	...
	*/
   	  ...
   	  (flags & (CUT_RW_SYNC | CUT_RW_WAIT)) )
	{
	    ...
	    */
	    if (! ((cur_buf->cut_buf_writers == 0 && flags & CUT_RW_SYNC)
	           || (cur_buf->cut_buf_writers > 0 && flags & CUT_RW_WAIT)) )
		break;
		...
		** cut_signal_status(), return async status */
		...
		my_thread->cut_thread_flush = FALSE;
		CSv_semaphore(&cur_buf->cut_buf_sem);
		error->err_code = erlog = E_CU0112_BAD_WRITE_CONDITION;
		status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
			0, 0, 0, &erlog, 1, 
			sizeof(cur_buf->cut_ircb.cut_buf_name),
			cur_buf->cut_ircb.cut_buf_name);
	...
	DB_ERROR	*error)
{
    ...
    */
    ...
    if (input_buf == NULL || (*input_buf == NULL &&
				!(flags & CUT_RW_DIRECT)) )
    {
	error->err_code = erlog = E_CU0013_NO_INPUT_BUF;
	status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
	    0, 0, 0, &erlog, 0);
	return(E_DB_ERROR);
    }
	...
	*/
	...
	{
	    CSv_semaphore(&cur_buf->cut_buf_sem);
	    error->err_code = E_CU0111_NOT_ATTACHED;
	    return(E_DB_ERROR);
	}
	/* Return thread handle so next time it'll be right */
	cut_rcb->cut_thr_handle = (PTR)my_thread;
    ...
    */
	...
	if ( free_cells == 0 )
	{
		...
		*/
		...
		{
		    error->err_code = erlog = E_CU0114_BAD_READ_CONDITION;
		    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
			0, 0, 0, &erlog, 1, 
			sizeof(cur_buf->cut_ircb.cut_buf_name),
			cur_buf->cut_ircb.cut_buf_name);
	...
	DB_ERROR	*error)
{
    ...
    */
    ...
    {
	error->err_code = erlog = E_CU0115_INVALID_STATUS;
	local_stat = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
	...
	{ 
		...
		*/
		if ( status > cur_thread->cut_tcb->cut_thread_status )
		{
		    ...
		    **
		    ** But must hold cut_tcb_sem to keep the
		    ** thread from disappearing from underneath us.
		    */
		    CSv_semaphore(&cur_buf->cut_buf_sem);
	...
	DB_ERROR	*error)
{
    ...
    */
    ...
    {
	error->err_code = erlog = E_CU0116_BAD_EVENT;
	status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
    ...
    */
    ...
    {
	error->err_code = W_CU0309_TOO_FEW_THREADS;
	return (E_DB_WARN);
    }
    if (cut_rcb == NULL || cut_rcb->cut_buf_handle == NULL)
    {
	...
	*/
	while (cur_buf->cut_sync_satisfied)
	{
		...
		** cut_signal_status(), return async status */
	    ...
	    {
		error->err_code = erlog = E_CU0117_BAD_EVENT_CONDITION;
		status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0,
		    0, 0, 0, &erlog, 1, sizeof(cur_buf->cut_ircb.cut_buf_name),
		    cur_buf->cut_ircb.cut_buf_name);
		CSv_semaphore(&cur_buf->cut_buf_sem);
	   ...
	   DB_ERROR		*error)
{
    ...
    ** Buffer not in list 
    */
    erlog = error->err_code = E_CU0119_BUF_NOT_IN_LIST;
    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 
		...
		DB_ERROR	*error)
{
	...
	if (thread_found)
	{
	    ...
	    {
		error->err_code = I_CU0402_THREAD_INITIALIZED;
		status = E_DB_INFO;
	    }
		...
		DB_ERROR	*error)
{
	...
	if (tcb_found)
	{
		...
		{
		    error->err_code = E_CU0203_TERM_ATTACHED;
		    CSv_semaphore(&my_tcb->cut_tcb_sem);
		    return (E_DB_ERROR);
		}
	    }
	  ...
	  DB_ERROR	*error)
{
    ...
    */
	...
	default:
	{
	    erlog = error->err_code = E_CU011A_BAD_BLOCK_TYPE;
	    status = (*log_error)(erlog, (CL_SYS_ERR *)0, ULE_LOG, 0, 0, 0,
		0, &erlog, 0);
	    return (E_DB_ERROR);
	}
