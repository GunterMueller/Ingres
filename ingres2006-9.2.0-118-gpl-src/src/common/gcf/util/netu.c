...
static i4	gcn_line_get( char *, char *, i4, char *, i4  );
...
#ifdef WIN32
#define DEF_NET_WARE    "wintcp"        /* default network software for
                                           this product platform */
...
#define         MAXTOKEN        8
    ...
    */
	...
	SIprintf("%s> ", argv[0]);
	SIflush(stdout);
	if ( gcn_getrec(line,(i4) MAXLINE) != OK)
...
gcn_u_q( char *uid, char *cs_id )
{
	...
	if(gcn_line_get("   Quiesce Communication Server: (Yes/No)", "", FALSE,
			 line,FALSE) == EOF)
	    return FAIL;
...
gcn_u_s( char *uid, char *cs_id )
{
	...
	SIprintf("   Comm_Server_Id = %s \n", cs_id);
	if(gcn_line_get("   Stop Communication Server: (Yes/No)","", FALSE,
			 line,FALSE) == EOF)
	    return FAIL;
	STzapblank(line,line);
	if(line[0] != 'y' && line [0] != 'Y')
...
gcn_u_a( char *uid )
{
    ...
    char	euser[64];
    char	encrp_pwd[128];
    char	password1[64], password2[64];
  ...
  p_g_loop:
    if(gcn_line_get("\tEnter Private or Global ","P", FALSE, p_g, FALSE ) == EOF)
	continue;
    ...
    if(opcode == GCN_ADD)
    {
      if(gcn_line_get("       Enter the remote Password","",TRUE,
			 password1, TRUE ) == EOF)
  	continue;
      if(gcn_line_get("       Repeat the remote Password","",TRUE,
			 password2, TRUE ) == EOF)
  	continue;
...
static i4
gcn_line_get( char *prompt, 
	      char *def_line, i4  wild_flag, char *in_line, i4  noecho ) 
{
	char    line[MAXLINE];
	char	ret = '\n';
	char	*p;
...
/* This is somewhat unusuall but does force the SIflush to work */
		...
		if( noecho )
			s = gcn_qgetrec( line, (i4)MAXLINE );
		else
			s = gcn_getrec( line, (i4)MAXLINE );
...
gcn_opcode_n( char *menu_prompt, i4  *opcode, bool *merge )
{
   ...
   *merge = FALSE;
   ...
   for(;;)
   {
    if( gcn_line_get(prompt, "", FALSE, line, FALSE ) == EOF)
...
gcn_u_o( char *uid )
{
    ...
    char    		type[ MAXLINE ];
    char    		v_node[ MAXLINE ];
    char    		attr_name[ MAXLINE ];
    char    		attr_value[ MAXLINE ];
    char    		value[ GCN_VAL_MAX_LEN + 1 ];
    char    		p_g[ 64 ];
    STATUS		status = OK;
    bool		merge;
...
p_g_loopa:
	if( gcn_line_get( "\tEnter Private or Global ","P", FALSE,
						p_g, FALSE ) == EOF )
	    continue;
        if(STncasecmp(p_g, "public",STlen(p_g)) 
        && STncasecmp(p_g, "private",STlen(p_g) )
