...
{
    QUEUE 	q;
    char	listen_addr[ GCN_VAL_MAX_LEN + 1 ];
    char	gcn_type[ GCN_TYP_MAX_LEN + 1];
} GCN_BCHK_QUE;
	...
	*/
	for( tup = tupvec; tup < tupvec + tupcount; tuptotal++, tup++ )
	{
	    ...
	    */
	    ...
	    bq = (GCN_BCHK_QUE *)MEreqmem( 0, sizeof( *bq ), TRUE, NULL );
	    ...
	    */
	    ...
	    STcopy( nq->gcn_type, bq->gcn_type );
...
)
{
    ...
    */
    STcopy(((GCN_BCHK_QUE *)bchk_q->q_next)->listen_addr, grcb->catl.tupv[0]);
    grcb->catl.host[0] = "";
