...
{
    if ( trace.enabled() )  
	trace.log( title + ".getMajorVersion(): " + DRV_MAJOR_VERSION );
...
{
    if ( trace.enabled() )  
	trace.log( title + ".getMinorVersion(): " + DRV_MINOR_VERSION );
...
{
    String  host = null;
    int     start = full_protocol_id.length();	// Skip the protocol
    int	    db, attr; 
...
getDatabase( String url )
{
	...
	*/
    ...
    /*
    ** Step over '/' prefix (if be present).
    */
...
{
    int attr, start = full_protocol_id.length();
