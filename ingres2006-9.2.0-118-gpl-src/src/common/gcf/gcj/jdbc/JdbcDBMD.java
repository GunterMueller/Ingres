    ...
    implements DatabaseMetaData, DrvConst, DbmsConst, GcfErr
{
    ...
    private final static String  NO		= new String("NO");
    private final static String  YES		= new String("YES");
...
getDriverVersion() 
    throws SQLException
{
    String version = DRV_MAJOR_VERSION + "." + DRV_MINOR_VERSION;
    if ( trace.enabled() )  
...
{
    if ( trace.enabled() )  
	trace.log( title + ".getSQLStateType(): " + sqlStateSQL99 );
    ...
    throws SQLException
{
    ...
    String esc = conn.dbCaps.getDbCap( DBMS_DBCAP_ESCAPE );

    if ( esc != null  &&  (esc.charAt(0) == 'N'  ||  esc.charAt(0) == 'n') )
	result = false;
...
{
    String  result = "\\"; 
    String  escape  = conn.dbCaps.getDbCap( DBMS_DBCAP_ESCAPE_CHAR );
    ...
    throws SQLException
{
    ...
    if ( std_catl_lvl >= DBMS_SQL_LEVEL_OI10 )
        qry_text = "select schema_name from iischema order by schema_name";
    else
        qry_text = "select table_owner from iitables union " + 
		   "select table_owner from iiviews";
    ...
    throws SQLException
{
    ...
    whereClause += makePattern( schemaPattern, whereClause.length(), "procedure_owner", false );
    whereClause += makePattern( procedureNamePattern, whereClause.length(), "procedure_name", false );
    ...
    {
	Statement stmt = conn.jdbc.createStatement();
	rs = new MDprocedureRS( conn, rsmd, (JdbcRslt)stmt.executeQuery(qry_txt) );
    }
    catch( SQLException ex )
    {
    ...
    throws SQLException
{
    ...
    whereClause += makePattern( schemaPattern, whereClause.length(), columnName, false );

    columnName = "procedure_name";
    if ( ! conn.is_ingres )  columnName = "proc_name";
    whereClause += makePattern( procedureNamePattern, whereClause.length(), columnName, false );

    whereClause += makePattern( columnNamePattern, whereClause.length(), "param_name", false );
    ...
    {
	Statement stmt = conn.jdbc.createStatement();
	rs = new MDprocedureColRS( conn, rsmd, (JdbcRslt)stmt.executeQuery(qry_txt) );
    ...
    throws SQLException
{
    ...
    whereClause += makePattern( schemaPattern, whereClause.length(), "table_owner", false );

    whereClause += makePattern( tableNamePattern, whereClause.length(), "table_name", false );
    ...
    throws SQLException
{
    ...
    whereClause += makePattern( schemaPattern, whereClause.length(), "object_owner", false );

    whereClause += makePattern( tableNamePattern, whereClause.length(), "object_name", false );
	...
	*/
    ...
    {
	Statement stmt = conn.jdbc.createStatement();
	rs = new MDgetTablePrivRS( conn, rsmd, (JdbcRslt)stmt.executeQuery(qry_txt) );
    ...
    throws SQLException
{
    ...
    */
    ...
    whereClause += makePattern( schemaPattern, whereClause.length(), "table_owner", likeNotSupported );

    whereClause += makePattern( tableNamePattern, whereClause.length(), "table_name", likeNotSupported );

    whereClause += makePattern( columnNamePattern, whereClause.length(), "column_name", likeNotSupported );
    ...
    throws SQLException
{
    ...
    */
	...
	Statement stmt = conn.jdbc.createStatement();
	rs = new MDgetColPrivRS( conn, rsmd, (JdbcRslt)stmt.executeQuery(qry_txt) );
    }
    catch( SQLException ex )
    {
	if ( trace.enabled() )
    ...
    throws SQLException
{
    ...
    */
	...
	try
        {
	    PreparedStatement stmt = conn.jdbc.prepareStatement(qry_txt);
	    int i =1;
	    if( schema != null )
    ...
    throws SQLException
{
    ...
    try
    {
	PreparedStatement stmt = conn.jdbc.prepareStatement( query );
	int i = 1;
...
private static final SqlData	OpenSqlTypeValues[][] =
{
    ...
    {	// Double
	S_DBL,	T_DBL,	I7,	NULL,	NULL,	S_PREC,	T_NULL,	S0,	T_PRDB,
    ...
    {	// Ingres Date
	S_IDAT,	T_TS,	I0,	QUOTE,	QUOTE,	NULL,	T_NULL,	S0,	T_PRDB,
...
private static final SqlData	sqlTypeValues[][] =
{
    ...
    {	// Double
	S_DBL,	T_DBL,	I15,	NULL,	NULL,	S_PREC,	T_NULL,	S0,	T_PRDB,
    ...
    {	// ANSI Date
	S_ADAT,	T_DATE,	I0,	QUOTE,	QUOTE,	NULL,	T_NULL,	S0,	T_PRDB,
	S1,	S0,	S0,	S_ADAT,	S0,	S0,	I0,	I0,	I10
    },
    {	// ANSI Time
	S_TIME,	T_TIME,	I0,	QUOTE,	QUOTE,	NULL,	T_NULL,	S0,	T_PRDB,
	S1,	S0,	S0,	S_TIME,	S0,	S0,	I0,	I0,	I10
    },
    {	// ANSI Timestamp
	S_TS,	T_TS,	I0,	QUOTE,	QUOTE,	NULL,	T_NULL,	S0,	T_PRDB,
    ...
    throws SQLException
{
    ...
    if ( conn.sqlLevel > 0 ) 
        return( new RsltData( conn, rsmd, sqlTypeValues ) );
    else
        return( new RsltData( conn, rsmd, OpenSqlTypeValues ) );
    ...
    throws SQLException
{
    ...
    if ( trace.enabled() )  
	trace.log( title + ".makePattern(" + pattern + ", " + whereClauseLength + ", " + columnName + ", " + likeNotSupported + ") - dbmsType=" + dbmsType ) ;
	...
	if ( whereClauseLength == 0 )
	    ...
	    whereClause = " and "; 
	whereClause += trim_fn + columnName + trim_fn_end + operator + "'" + pattern + "'";
    }


    if ( trace.enabled() )  trace.log( title + ".makePattern(): " + pattern + "," + whereClause ) ;
    return( whereClause );
} // makePattern
    ...
    throws SqlEx
{
    ...
    case Types.TIME:		return( 21 );
    case Types.TIMESTAMP:	return( 32 );
