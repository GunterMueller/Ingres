...
protected
MsgIn( String target, byte msg_cp[][], TraceLog log )
    throws SqlEx
{
    super( target, msg_cp[0], log );
    title = "MsgIn[" + connID() + "]";
    msg_cp[0] = null;	// No output yet.

    try 
    { 
	in = new InBuff( socket.getInputStream(), 
			 connID(), 1 << DAM_TL_PKT_MIN, log ); 
    }
    catch( Exception ex )
    {
	if ( trace.enabled( 1 ) )  
	...
	*/
	while( in.avail() >= 2 )
	{
		...
		*/
	    ...
	    case DAM_TL_CP_MSG :
		msg_cp[0] = new byte[ param_len ];
		if ( in.readBytes( msg_cp[0], 0, param_len ) != param_len )
		    throw SqlEx.get( ERR_GC4002_PROTOCOL_ERR );		
		break;
	...
	if ( in.avail() > 0 )  throw SqlEx.get( ERR_GC4002_PROTOCOL_ERR );
    ...
    */
    if ( msg_cp[0] == null )  msg_cp[0] = new byte[0];
    return;
} // MsgIn
    ...
    */
	...
	if ( param_id == DAM_TL_DP_ERR  &&  in.avail() >= param_len ) 
	{
	    ...
	    */
	    ...
	    throw new SqlEx( sqlEx.getMessage(), sqlEx.getSQLState(), error );
	}
    }
