/*
** Copyright (c) 2003, 2004 Ingres Corporation.
*/
...
rqst_show_sessions( QUEUE *rsltlst, PTR sid, u_i2 cmd, i4 max_len )
{
    ...
    */
    ...
    info = (GCC_SESS_INFO *)MEreqmem( 0, count * sizeof( GCC_SESS_INFO ),
                                      TRUE, NULL);
    ...
    */
    if ( ! info  ||  ! (count = gcc_sess_info( (PTR)info, info_flag )) )
        return( max_len );
...
gcc_sess_info( PTR info, i4 flag ) 
{
	...
	if( ( ccb = IIGCc_global->gcc_tbl_ccb[ cei ] )  !=  NULL )
	{
		...
		sess->is_system = is_system;
	        STcopy(ccb->ccb_hdr.username, sess->user_id);
                if( sess->is_system )
                {
		    TRdisplay("Found system sessions in gcc_sess_info\n");
                    ...
                    if( ccb->ccb_hdr.trg_addr.n_sel[0] )
                        STcopy(tl_states[ccb->ccb_tce.t_state], sess->state );
                    else
                        STcopy(al_states[ccb->ccb_aae.a_state], sess->state );
	        }
		else
	        {
		    TRdisplay("Found user sessions in gcc_sess_info\n");
	            STcopy(ccb->ccb_hdr.target, sess->target_db);
	            if( ccb->ccb_hdr.flags & CCB_IB_CONN )
		        STcopy("INBOUND", sess->state );
	            if( ccb->ccb_hdr.flags & CCB_OB_CONN )
		        STcopy("OUTBOUND", sess->state );
    		    if ( ccb->ccb_hdr.trg_addr.n_sel[0] )  	
    		    {
