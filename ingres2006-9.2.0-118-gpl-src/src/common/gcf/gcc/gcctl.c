...
# endif /* GCXDEBUG */
	...
	else		/* Successful Listen */
	{
	    ...
	    if ( ! p_list->function_parms.listen.node_id )
		ccb->ccb_hdr.flags &= ~CCB_RMT_HOST;
	    else
	    {
		ccb->ccb_hdr.flags |= CCB_RMT_HOST;
		STcopy( p_list->function_parms.listen.node_id,
			ccb->ccb_hdr.sechost );
	    }
...
# endif /* GCXDEBUG */
	...
	*/
	mech_count = ( ccb->ccb_hdr.emode == GCC_ENCRYPT_OFF )
		     ? 0 : gcc_gcs_mechanisms( ccb->ccb_hdr.emech, mech_ids );
