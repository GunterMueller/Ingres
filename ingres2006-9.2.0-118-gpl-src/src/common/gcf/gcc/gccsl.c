	...
	*/
	if ( ccb->ccb_hdr.flags & CCB_PWD_ENCRYPT )
	{
	    ...
	    */
	    ...
	    ccb->ccb_hdr.sectyp = 0;
	    ccb->ccb_hdr.seclen = 0;
			...
			*/
			...
			while( parm < end_parm )
			{
				...
				case SL_USER_XPI :
				    if ( xud_pci.pgiu.li )
				    {
					GCgets( parm, min( xud_pci.pgiu.li, 
							   sizeof(user_key) ),
						user_key );
					user_key[ sizeof(user_key) - 1 ] = EOS;
					GCngets( user_key, min(xud_pci.pgiu.li,
						 sizeof(ccb->ccb_hdr.username)),
				...
				case SL_SECL_XPI :
				    if ( xud_pci.pgiu.li )
				    {
					GCngets( parm, min( xud_pci.pgiu.li, 
						 sizeof(ccb->ccb_hdr.seclab) ),
						 ccb->ccb_hdr.seclab );
					ccb->ccb_hdr.seclen = GCC_L_SECLABEL;
				    }
				    break;
	    ...
	    */
	    ...
	    {
		char 	buf[ GCC_L_ENCODE + 1 ];

		passwd_len = min( passwd_len, GCC_L_ENCODE );
	    	gcc_decode( passwd_ptr, passwd_len, user_key, 
			    (ccb->ccb_hdr.mask_len > 0) 
			    ? ccb->ccb_hdr.mask : NULL, buf );
		passwd_len = min( STlength( buf ), 
				  sizeof( ccb->ccb_hdr.password ) - 1 );
	    	GCngets( buf, passwd_len, ccb->ccb_hdr.password );
	        ccb->ccb_hdr.password[ passwd_len + 1 ] = EOS;
	    }
	...
	*/
	if ( ccb->ccb_hdr.flags & CCB_PWD_ENCRYPT )
	{
	    ...
	    */
	    ...
	    /* password is encrypted using username as the key. 
	    ** convert the username to NTS form before using it as key.
	    ** Also convert the password to NTS form before encrypting it.
	    */
	    pci += GCaddn1( SL_USER_XPI, pci );
	    len  = GCnadds( ccb->ccb_hdr.username,
			    STlength( ccb->ccb_hdr.username ) + 1, user_key );
	    pci += GCaddn1( len, pci );
	    (VOID) MEcopy( (PTR) user_key, len, (PTR) pci );
	    pci += len;

	    if ( ccb->ccb_hdr.password[0] != EOS )
	    {
	    	pci += GCaddn1( SL_PASS_XPI, pci );
	    	GCnadds( ccb->ccb_hdr.password, 
	    		    STlength( ccb->ccb_hdr.password ) + 1, buf );
	    	len = gcc_encode( buf, user_key, (ccb->ccb_hdr.mask_len > 0) 
						 ? ccb->ccb_hdr.mask : NULL,
						 passwd_ebuf );
	    	pci += GCaddn1( len, pci );
	    	(VOID)MEcopy( (PTR)passwd_ebuf, len, (PTR)pci );
	    	pci += len;
	    }

	    if ( ccb->ccb_hdr.seclen )
	    {
		pci += GCaddn1( SL_SECT_XPI, pci );
		pci += GCaddn1( 2, pci );
		pci += GCaddn2( ccb->ccb_hdr.sectyp, pci );

		pci += GCaddn1( SL_SECL_XPI, pci );
		pci += GCaddn1( ccb->ccb_hdr.seclen, pci );
		pci += GCnadds(ccb->ccb_hdr.seclab, ccb->ccb_hdr.seclen, pci);
	    }
