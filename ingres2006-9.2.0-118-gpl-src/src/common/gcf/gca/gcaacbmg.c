...
gca_del_acb( i4  assoc_id )
{
    ...
    */
    ...
    {
	char assoc_buff[16];
	MOulongout( 0, (u_i4)assoc_id, sizeof(assoc_buff), assoc_buff );
    ...
    /* These next three are allocated with STalloc, which uses MEreqmem;
    ** they must be freed with MEfree, nevermind what gca-free calls.
    */
    if ( acb->user_id )  MEfree( (PTR)acb->user_id );
    if ( acb->client_id )  MEfree( (PTR)acb->client_id );
