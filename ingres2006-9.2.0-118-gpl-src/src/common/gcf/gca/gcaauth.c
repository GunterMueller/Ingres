...
**	gca_cnvrt_old_peer	Process original peer info.
**	gca_frmt_old_peer	Generate original peer info.
**
** History:
**	29-May-97 (gordy)
**	    Created.
...
gca_seclab( GCA_ACB *acb, GCA_PEER_INFO *peer, GCA_RQ_PARMS *parms )
{
	...
	sldata.version = 1;
	sldata.flags = 0;
	MEcopy( parms->gca_sec_label, GC_L_SECLABEL, (PTR)&sldata.sec_label );
...
)
{
    ...
    MEfill( sizeof( peer->gca_user_id ), '\0', (PTR)peer->gca_user_id );
    ...
    */
    ...
    {
	STcopy( parms->gca_rem_username, peer->gca_user_id );
	...
	if ( parms->gca_user_name  &&  *parms->gca_user_name )
	    STcopy( parms->gca_user_name, peer->gca_user_id );
...

/*
** Name: gca_cnvrt_old_peer
**
** Description:
**	Convert GCA_PEER_INFO at version 5 or less to version 6.
**
** Input:
**	acb		Association Control Block.
**	length		Length of peer info.
**	buffer		Buffer containing peer info.
**
** Output:
**	peer		Version 6 peer info.
**
** Returns:
**	STATUS		OK or error code.
**
...
STATUS
gca_cnvrt_old_peer( GCA_ACB *acb, i4  length, PTR buffer, GCA_PEER_INFO *peer )
{
    GCA_OLD_PEER	old_peer;
    STATUS		status = OK;

    if ( length != sizeof( GCA_OLD_PEER ) )  return( E_GC000A_INT_PROT_LVL );

    /*
    ** Convert old peer info.
    */
    MEcopy( buffer, length, (PTR)&old_peer );
    peer->gca_length = sizeof( GCA_PEER_INFO );
    peer->gca_version = old_peer.gca_version;
    peer->gca_id = GCA_PEER_ID;
    peer->gca_flags = old_peer.gca_flags;
    peer->gca_partner_protocol = old_peer.gca_partner_protocol;
    peer->gca_status = old_peer.gca_status;
    MEcopy( (PTR)old_peer.gca_user_id,
	    min( sizeof( old_peer.gca_user_id ), sizeof( peer->gca_user_id ) ),
	    (PTR)peer->gca_user_id );
    ...
    ** protocol.
    */
    if ( old_peer.gca_password[ 0 ] != EOS )
    {
	GCS_PWD_PARM	pwd_parm;

	pwd_parm.user = old_peer.gca_user_id;
	pwd_parm.password = old_peer.gca_password;
	pwd_parm.length = length;
	pwd_parm.buffer = buffer;
	...
	** process.
	*/
	MEfill( sizeof( peer->gca_user_id ), EOS, (PTR)peer->gca_user_id );
	usr_parm.length = length;
	usr_parm.buffer = buffer;
    ...
    if ( status == OK )
    {
	status = gca_aux_element( acb, GCA_ID_AUTH, length, buffer );
	peer->gca_aux_count++;
    }
    ...
    ** Convert aux data.
    */
    if ( status == OK  &&  old_peer.gca_aux_data.type_aux_data )
    {
	if ( old_peer.gca_aux_data.type_aux_data == GCA_ID_RMT_ADDR )
	    status = gca_aux_element( acb, GCA_ID_RMT_ADDR,
				      sizeof( GCA_RMT_ADDR ), 
				      (PTR)&old_peer.gca_aux_data.rmt_addr);
	else
	    status = gca_aux_element( acb, old_peer.gca_aux_data.type_aux_data, 
				      0, NULL );
...

/*
** Name: gca_frmt_old_peer
**
** Description:
**	Formats an version 5 or lower peer info structure from a
**	version 6 peer info structure.  It is assumed that the
**	old peer info is only sent server to client, so username,
**	password and aux data are not processed.
**
** Input:
**	peer		Version 6 peer info.
**
** Output:
**	buffer		Buffer to receive peer info.
**
** Returns:
**	i4		Length of output.
**
...
i4
gca_frmt_old_peer( GCA_PEER_INFO *peer, PTR buffer )
{
    GCA_OLD_PEER old_peer;

    MEfill( sizeof( old_peer ), EOS, (PTR)&old_peer );
    old_peer.gca_partner_protocol = peer->gca_partner_protocol;
    old_peer.gca_version = GCA_IPC_PROTOCOL_5;
    old_peer.gca_status = peer->gca_status;
    old_peer.gca_flags = peer->gca_flags;

    MEcopy( (PTR)&old_peer, sizeof( old_peer ), buffer );

    return( sizeof( old_peer ) );
}
