...
static VOID gca_purge( GCA_SVC_PARMS *svc_parms );
...
# define GC_DELPASSWD	143	/* call GCpasswd_delete_vnode_info */
...
static char *gca_names[] = 
{
    ...
    "GC_DISCONN", "GC_RELEASE", "GC_DELPASSWD",
};
...
top:
	...
	*/
	...
	/*
	** Adjust our size advice to a multiple of the GCA CL size.
	*/
    ...
    case SA_BUFFERS:
	/*
	** Allocate buffers large enough to hold 
	** FASTSELECT data and a message header.
	*/
	acb->size_advise = 
	acb->recv[ GCA_NORMAL ].bufsiz = 
	acb->send[ GCA_NORMAL ].bufsiz = GCA_FS_MAX_DATA + sizeof(GCA_MSG_HDR);
	...
	**
	** The peer info structure starts with two i4s.
	** At version 6 and greater, the first element is the
	** peer info size.  The peer info version is always 
	** the second element.
	*/
	if ( (rsm->bufend - rsm->bufptr) < (sizeof( i4 ) * 2) )
	{
	    branch = TRUE;
	    break;
	}

	MEcopy( (PTR)rsm->bufptr, sizeof( i4 ) * 2, (PTR)peerinfo );

	/*
	** Older peer verions require special conversion.
	*/
	if ( peerinfo->gca_version < GCA_IPC_PROTOCOL_6 )
	{
	    if ( (rsm->bufend - rsm->bufptr) < sizeof( GCA_OLD_PEER ) )
	    {
		branch = TRUE;
		break;
	    }
	    ...
	    svc_parms->gc_parms.status = 
		gca_cnvrt_old_peer( acb, sizeof( GCA_OLD_PEER ), 
				    (PTR)rsm->bufptr, peerinfo );
	    rsm->bufptr += sizeof( GCA_OLD_PEER );

	    if ( rsm->bufptr >= rsm->bufend )		/* Reset the buffer */
		rsm->bufptr = rsm->bufend = rsm->buffer;
	    break;
	}

	if ( (rsm->bufend - rsm->bufptr) < sizeof( GCA_PEER_INFO ) )
	{
	    branch = TRUE;
	    break;
	}
	...
	/*
	** Save and validate the peer info.  Note that
	** we only save what we can handle, but then
	** remove from the buffer the actual peer info
	** size to allow for future extensions.
	*/
	MEcopy((PTR)rsm->bufptr, sizeof(GCA_PEER_INFO), (PTR)peerinfo);

	if ( peerinfo->gca_id != GCA_PEER_ID )
	{
	    svc_parms->gc_parms.status = E_GC000A_INT_PROT_LVL;
	    rsm->bufptr = rsm->bufend = rsm->buffer;
	    break;
	}

	if ( (rsm->bufend - rsm->bufptr) < peerinfo->gca_length )
	{
	    branch = TRUE;
	    break;
	}
	...
	{
	    GCA_AUX_DATA	hdr;
	    char		*ptr = rsm->bufptr + peerinfo->gca_length;
	    i4  		count;

	    /*
	    ** We don't currently have a loop for reading
	    ** the aux data, so it must all be present
	    ** before processing begins.
	    */
	    for( 
		 count = peerinfo->gca_aux_count;
		 count  &&  (ptr + sizeof(hdr)) <= rsm->bufend; 
		 count-- 
	       )
	    {
		MEcopy( (PTR)ptr, sizeof( hdr ), (PTR)&hdr );
		ptr += hdr.len_aux_data;
	    }

	    if ( count  ||  ptr > rsm->bufend )
	    {
		branch = TRUE;
		break;
	    }
	    ...
	    /*
	    ** Now we can save the entire aux data.
	    */
	    rsm->bufptr += peerinfo->gca_length;
	    svc_parms->gc_parms.status = gca_append_aux(acb, ptr - rsm->bufptr,
							(PTR)rsm->bufptr);
	    rsm->bufptr = ptr;
    ...
    case SA_SEND_PEER:
	/*
	** Prepare to send the peer info.
	*/
	peersend->gca_length = sizeof( GCA_PEER_INFO );
	peersend->gca_id = GCA_PEER_ID;
	peersend->gca_version = GCA_IPC_PROTOCOL;
	...
	*/
	...
	if ( peerinfo->gca_version < GCA_IPC_PROTOCOL_6 )
	    ssm->bufend = ssm->bufptr + gca_frmt_old_peer( peersend, 
							   ssm->bufptr );
	else
	{
	    ssm->bufend = ssm->bufptr + sizeof( GCA_PEER_INFO );
	    MEcopy( (PTR)peersend, sizeof( GCA_PEER_INFO ), (PTR)ssm->bufptr );

	    if ( acb->gca_aux_len )
	    {
		/*
		** Currently, no forseen combination of AUX
		** data will overflow the GCA_FS_MAX_DATA
		** size of our buffers.  This may need to
		** be addressed in the future.
		*/
		MEcopy( (PTR)acb->gca_aux_data, 
			acb->gca_aux_len, (PTR)ssm->bufend );
		ssm->bufend += acb->gca_aux_len;
	    }
	}
	break;
	...
	*/
	...
	MEfill(sizeof(peersend->gca_user_id), EOS, (PTR)peersend->gca_user_id);
	...
	*/
	{
	    ...
	    */
	    if ( ! peerinfo->gca_user_id[ 0 ] )
		STcopy( svc_parms->gc_parms.user_name, peerinfo->gca_user_id );
	    ...
	    */
		...
		i4			len = aux->len_aux_data - 
					      sizeof( GCA_AUX_DATA );

		peerinfo->gca_flags |= GCA_LS_SECLABEL;
		MEcopy( (PTR)((char *)aux + sizeof( GCA_AUX_DATA )), 
	...
	*/
	acb->inbound = Yes;
	acb->user_id = STalloc( peerinfo->gca_user_id );
	acb->client_id = STalloc( svc_parms->gc_parms.user_name );
	acb->partner_id = STalloc(svc_parms->gc_parms.access_point_identifier);
	...
	{
	    char buff[16];
	    MOulongout( 0, (u_i4)acb->assoc_id, sizeof(buff), buff );
	...
	*/
	{
		...
		STcpy( conn_buff, RQ_PARMS_MACRO->gca_partner_name );
	...
	*/
	...
	if ( acb->partner_id )  MEfree( (PTR)acb->partner_id );
	if ( ! svc_parms->gc_parms.partner_host  ||
	     ! *svc_parms->gc_parms.partner_host )
	    acb->partner_id = STalloc( svc_parms->gc_parms.partner_name );
	else
	{
	    char id[(GCN_VAL_MAX_LEN * 2) + 2];
	    STpolycat( 3, svc_parms->gc_parms.partner_host, ":",
			svc_parms->gc_parms.partner_name, id );
	    acb->partner_id = STalloc( id );
	}
	...
	*/
	...
	{
	    MEfill( sizeof( peersend->gca_user_id ), 
		    '\0', (PTR)peersend->gca_user_id );
	    STcopy( rqcb->ns.lcl_user, peersend->gca_user_id );
	...
	*/
	acb->inbound = No;
	acb->user_id = STalloc( *peersend->gca_user_id 
				? peersend->gca_user_id : acb->client_id );
	GCusername( peersend->gca_user_id, sizeof( peersend->gca_user_id ) );
	acb->client_id = STalloc( peersend->gca_user_id );
	...
	*/
	...
	GCA_DEBUG_MACRO(2)
	    ( "%04d   GCA_SEND msg=%s snd_len=%d eod=%d flow=%s\n",
	      (i4)(acb ? acb->assoc_id : -1), 
...
gca_purge( GCA_SVC_PARMS *svc_parms )
{
	...
	( "%04d   GCA_PURGE %s %s\n",
	  ...
	  (acb->flags.send_active) ? "deferred" : "" );

    /*
    ** Don't call GCpurge if a normal GCsend is outstanding.  
    */
    if ( acb->flags.send_active )  return;
