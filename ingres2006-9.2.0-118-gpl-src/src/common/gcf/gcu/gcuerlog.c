...
gcu_erinit( char *host, char *server, char *addr )
{
    PID proc_id;
    char fmt[20];
    char node_name[128]; /* qwikfix must be CX_MAX_NODE_NAME_LEN */
    ...
    /* translate node name to nickname for clusters */
...
)
{
    ...
    {
#ifdef LP64
	STprintf( buf, "%-8.8s::[%-18.18s, %-10s, %016.16x]: ",
		gca_er_host, gca_er_servid, gca_er_pid, sessid );
#else
	STprintf( buf, "%-8.8s::[%-18.18s, %-10s, %08.8x]: ",
		gca_er_host, gca_er_servid, gca_er_pid, sessid );
#endif
