...
{
    GCS_OBJ_HDR	hdr;
    char	user[ GC_L_USER_ID + 1 ];
    char	ubuff[ GCS_MAX_BUF + 1 ];
    char	ebuff[ GCS_MAX_BUF + 1 ];
    ...
    ** Encode components.
    */
    ulen = gcs_encode( user, IIgcs_global->host, ubuff ) + 1;
    elen = gcs_gen_exp( expiration_time, user, ebuff ) + 1;
...
gcs_val_usr( GCS_VALID_PARM *valid, GCS_OBJ_HDR *hdr )
{
    ...
    */
    ...
    /*
    ** Validate user ID.  Checking encoded length assures
    ** that decoded length will fit in buffers.
    */
    if ( 
	 obj_len != (ulen + (hdr->obj_ver >= GCS_OBJ_V1 ? elen + 2 : 0))  ||
         ulen >= GCS_MAX_BUF  ||  *(uptr + ulen - 1) != EOS 
       )
    {
	GCS_TRACE( 1 )( "GCS %s: invalid user ID length: %d, %d, %d\n", 
    ...
    */
    if ( elen )
    {
	if ( elen >= GCS_MAX_BUF  ||  *(eptr + elen - 1) != EOS )
	{
...
gcs_pwd_auth( GCS_PWD_PARM *pwd_parm )
{
    ...
    if ( STlength( pwd_parm->password ) > GCS_MAX_STR )
    {
	GCS_TRACE( 1 )( "GCS %s: password too long: %d bytes\n",
			mech_info.mech_name, STlength( pwd_parm->password ) );
	return( E_GC1010_INSUFFICIENT_BUFFER );
    }
    ...
    */
	...
	plen = gcs_encode( pwd_parm->password, IIgcs_global->host, pbuff ) + 1;
    else
    {
	char buff1[ GCS_MAX_BUF + 1 ];

	version = GCS_OBJ_V2;
	plen = gcs_encode( pwd_parm->password, pwd_parm->user, buff1 );

	if ( plen > GCS_MAX_STR )
	{
	    GCS_TRACE( 1 )( "GCS %s: encoded password too long: %d bytes\n",
			    mech_info.mech_name, STlength(pwd_parm->password) );
	    return( E_GC1010_INSUFFICIENT_BUFFER );
	}

	plen = gcs_encode( buff1, IIgcs_global->host, pbuff ) + 1;
	elen = gcs_gen_exp( expiration_time, pwd_parm->user, ebuff ) + 1;
    }
    ...
    */
    ...
    {
	MEcopy( (PTR)pbuff, plen, (PTR)ptr );		/* Password */
	pwd_parm->length = sizeof( hdr ) + plen;
    }
    else
    {
	*ptr++ = plen;					/* Password */
	MEcopy( (PTR)pbuff, plen, (PTR)ptr );
	ptr += plen;
...
gcs_val_pwd( GCS_VALID_PARM *valid, GCS_OBJ_HDR *hdr )
{
    ...
    CL_ERR_DESC		sys_err;
    char		password[ GCS_MAX_STR + 1 ];
    char		buff[ GCS_MAX_STR + 1 ];
    u_i1		*pptr, *eptr;
    u_i2		obj_len = GCS_GET_I2( hdr->obj_len );
    u_i2		plen = obj_len;
    u_i2		elen = 0;
    ...
    */
    ...
    /*
    ** Validate password.  Checking encoded length assures
    ** that decoded length will fit in buffers.
    */
    if ( 
         obj_len != (plen + (hdr->obj_ver >= GCS_OBJ_V2 ? elen + 2 : 0))  ||
	 plen > GCS_MAX_BUF  ||  *(pptr + plen - 1) != EOS 
       )
    {
	...
	gcs_decode( (char *)pptr, IIgcs_global->host, password );
    else
    {
	gcs_decode( (char *)pptr, IIgcs_global->host, buff );
	gcs_decode( buff, valid->alias, password );
    }
    ...
    if ( status == FAIL )
	status = GCusrpwd( valid->alias, password, &sys_err );
	...
	*/
	if ( status == E_GC000B_RMT_LOGIN_FAIL  &&
	     ! MEcmp( password, GCN_AUTH_TICKET_MAG, GCN_L_AUTH_TICKET_MAG ) )
	{
...
gcs_srv_auth( GCS_SRV_PARM *srv_parm )
{
 ...
 host %s, server %s, expire %d\n",
    ...
    if ( STlength( srv_parm->server ) > GCS_MAX_STR )
    {
	GCS_TRACE( 1 )( "GCS %s: server ID too long: %d\n",
			mech_info.mech_name, STlength( srv_parm->server ) );
	return( E_GC1012_INVALID_PARM_VALUE );
    }

    if ( STlength( srv_parm->alias ) > GCS_MAX_STR )
    {
	GCS_TRACE( 1 )( "GCS %s: alias name too long: %d\n",
			mech_info.mech_name, STlength( srv_parm->alias ) );
	return( E_GC1012_INVALID_PARM_VALUE );
    }
    ...
    ** Encode components.
    */
    slen = gcs_encode( srv_parm->server, IIgcs_global->host, sbuff ) + 1;
    ulen = gcs_encode( srv_parm->alias, obj.key.key, ubuff ) + 1;
    elen = gcs_gen_exp( expiration_time, srv_parm->alias, ebuff ) + 1;
...
gcs_val_srv( GCS_VALID_PARM *valid, GCS_OBJ_HDR *hdr )
{
    ...
    */
    ...
    /*
    ** Validate server ID.  Checking encoded length assures
    ** that decoded length will fit in buffers.
    */
    if (
	 obj_len != ( slen + ulen + 
	 	      (hdr->obj_ver >= GCS_OBJ_V1 ? elen + 3 : 2) )  ||
         slen >= GCS_MAX_BUF  ||  *(sptr + slen - 1) != EOS 
       )
    {
    ...
    */
	...
	return( E_GC1009_INVALID_SERVER );
    }
    ...
    ** Validate user alias.
    */
    if ( ulen >= GCS_MAX_BUF  ||  *(uptr + ulen - 1) != EOS )
    {
...
**	buffer should be at least GCS_MAX_STR in length.
**
** Input:
