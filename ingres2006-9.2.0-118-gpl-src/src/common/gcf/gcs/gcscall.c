...
gcs_init( GCS_INIT_PARM	*init_parm )
{
    ...
    STcopy( PMhost(), IIgcs_global->config_host );
    GChostname( IIgcs_global->host, sizeof( IIgcs_global->host ) );
    GCusername( IIgcs_global->user, sizeof( IIgcs_global->user ) );
...
gcs_term( VOID )
{
    ...
    */
	...
	MOulongout( 0, (u_i4)mech_id, sizeof( str ), str );
...
gcs_init_mechs( VOID )
{
    ...
    */
    ...
    if ( gcs_load_mechs() != OK )  status = E_GC1006_SEC_MECH_FAIL;
    ...
    */
    ...
    stat = gcs_check_caps( IIgcs_global->user_mech, GCS_CAP_USR_AUTH );
	...
	( "GCS init: user authentication mechanism: %s %s %s\n",
	  ...
	  (stat == E_GC1005_SEC_MECH_DISABLED) ? "(disabled)" : 
	  (stat == E_GC1003_GCS_OP_UNSUPPORTED) ? "(unsupported)" : "",
    ...
    stat = gcs_check_caps( IIgcs_global->password_mech, GCS_CAP_PWD_AUTH );
    ...
    GCS_TRACE( 2 )
	( "GCS init: password authentication mechanism: %s %s\n",
    ...
    stat = gcs_check_caps( IIgcs_global->server_mech, GCS_CAP_SRV_AUTH );
    ...
    GCS_TRACE( 2 )
	( "GCS init: server authentication mechanism: %s %s\n",
    ...
    gcs_config_mech( str, IIgcs_global->install_mech,
				   &IIgcs_global->remote_mech );
    stat = gcs_check_caps( IIgcs_global->remote_mech, GCS_CAP_REM_AUTH );
    ...
    GCS_TRACE( 2 )
	( "GCS init: remote authentication mechanism: %s %s\n",
...
gcs_load_mechs( VOID )
{
    ...
    char	*ptr, *path, str[ 128 ];
    char	*names[ GCS_MAX_DYNAMIC ];
    char	nbuff[ GCS_MAX_DYNAMIC * 33 ];
    char	pbuff[ MAX_LOC + 1 ];
    char	module[ 33 ], entry[ 33 ];
    i4		i, cnt;
...
gcs_mech_init( char *name, MECH_FUNC func, PTR hndl )
{
    ...
    */
    ...
    MOulongout( 0, (u_i4)info->mech_id, sizeof( str ), str );
