...
typedef struct _GCA_OLD_PEER GCA_OLD_PEER;
...
# define	GC_L_PROTOCOL_ID    16
# define	GC_L_NODE_ID	    64
# define	GC_L_PORT_ID	    64
# define	GC_L_PARTNER_ID	    64
...
struct _GCA_GLOBAL
{
    ...
    char		gca_uid[GC_L_USER_ID];		    /* User ID */
...
{
    char	partner_id[GC_L_PARTNER_ID];
    char	protocol_id[GC_L_PROTOCOL_ID];
    char	node_id[GC_L_NODE_ID];
    char	port_id[GC_L_PORT_ID];
    char	user_id[GC_L_USER_ID];
...
**
** Description:
**	Original peer info structure.  Used for backward compatibility.
**
** History:
**      14-May-87 (berrow)
**          Initial structure implementation
...
struct _GCA_OLD_PEER
{
    ...
    i4	    gca_old1[2];
    i4		    gca_old2;
    char	    gca_user_id[ GC_L_USER_ID ];
    char	    gca_password[ GC_L_PASSWORD ];
    u_i4	    gca_flags;
...
**
** Description:
**	Peer information initially exchaned on an association.
**
...
# define GCA_IPC_PROTOCOL_6	6	/* New PEER_INFO structure */

# define GCA_IPC_PROTOCOL	GCA_IPC_PROTOCOL_6
...
#define	GCA_PEER_ID	0x47434150	/* 'GCAP' (ascii big-endian) */
    ...
    char	gca_user_id[ GC_L_USER_ID ];	/* User alias */
...
# define        GC_L_SECLABEL       140 

    char	sec_label[GC_L_SECLABEL]; /*Partner label */
};
...
FUNC_EXTERN STATUS	gca_cnvrt_old_peer( GCA_ACB *, 
					    i4, PTR, GCA_PEER_INFO * );
FUNC_EXTERN i4		gca_frmt_old_peer( GCA_PEER_INFO *, PTR );
