...
#define	QRY_HAVE_DATA	0x8000	/* Paremeter data received */
	...
	u_i2	req_id;
	char	rqst_id0[ GCD_NAME_MAX + 1 ];
	char	rqst_id1[ GCD_NAME_MAX + 1 ];
    } rqst;
...
#define	GCD_STMT_AUTO_CLOSE	0x20	/* Close statment at EOD */
    ...
    char	crsr_name[ GCD_NAME_MAX + 1 ];	/* Cursor name (if cursor) */
...
typedef struct
{
    PTR         sess_id;
    i4          assoc_id;
    char        user_id[ GCD_NAME_MAX + 1 ];
    char        state[ GCD_NAME_MAX +1 ];
    char        database[ GCD_NAME_MAX +1 ];
    bool	is_system;
    bool	isLocal;
    bool	autocommit;
    char        srv_class[ GCD_NAME_MAX + 1 ];
    char        gca_addr[ GCD_NAME_MAX + 1 ];
