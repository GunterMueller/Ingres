...
# define    GCC_L_TARGET	64	/* Length of a logical db name */
# define    GCC_L_UNAME		32	/* Length of a username */
# define    GCC_L_PWD		64	/* Length of a password */
# define    GCC_L_EMECH		32	/* Length of encry mech name */
# define    GCC_L_EMODE		16	/* encryption option */
# define    GCC_L_SECLABEL	140	/* Length of a security label */
...
# define    GCC_L_ENCODE	128	/* Length of encoded string */
...
#define	GCC_REG_MASTER_STR	"master"
    ...
    char	    gcc_installation[4];    /* Installation of Comm Server */
    char	    gcc_host[ GCC_L_NODE ]; /* Host name */
...
# define GCC_OB_DEFAULT		9999    /* Default max # outbound conn's */
    ...
    char	    gcc_ib_mech[GCC_L_EMECH];	/* Inbound encryption mech */
    i4		    gcc_ob_mode;		/* Outbound encryption mode */
    char	    gcc_ob_mech[GCC_L_EMECH];	/* Outbound encryption mech */
...
/* 
** gca protocol was defined as a u_i4 at 6.1 and 6.2. It was set but never
** used. At application protocol level 6.3, it is both set and used.
** At 6.5 we usurp the first two octets to transport the GCA association flags.
*/
    u_i2	    gcc_gca_flags;
...
# define	CCB_RMT_HOST	    0x0010	/* Got remote host on listen */
# define	CCB_PWD_ENCRYPT     0x0020	/* Password encryption */
	...
	N_ADDR		trg_addr;		/* Request/Listen Address */
    	...
    	char		target[GCC_L_TARGET];	/* Target DBMS */
	char		username[GCC_L_UNAME];	/* User ID */
	char		password[GCC_L_PWD];	/* Password */
	i4		sectyp;			/* Security label type */
	i4		seclen;			/* Length of security label */
	char		seclab[GCC_L_SECLABEL];	/* Security label */
	char		sechost[GCC_L_NODE];	/* Remote host for sec label */
	...
	char		emech[ GCC_L_EMECH ];   /* Encryption Mechanism */
...
# define        GCCAL_SECLABEL          0x100   /* Security label passed */
...
# ifdef DOUBLEBYTE
	QUEUE		   mde_q[2];		/* queue of mdes held while */
						/* converting char types */
	i4		   split;		/* whether a char type is */
						/* split across multiple mdes */
# endif /* DOUBLEBYTE
...
typedef struct
{
    PTR         sess_id;
    i4          assoc_id;
    char        user_id[ GCC_L_UNAME + 1 ];
    char        state[ GCC_L_UNAME +1 ];
    char        target_db[ GCC_L_TARGET +1 ];
