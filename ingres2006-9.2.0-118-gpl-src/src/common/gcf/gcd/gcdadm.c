...
static void             rqst_sys_stats( GCD_CCB * , char *, char * );
...
adm_rqst_cb( PTR ptr )
{
	...
	if ( GCD_global.adm_sess_count == 0)  gca_sm( 0 );
	return;
    }
...
gcd_adm_session( i4 aid, PTR gca_cb, char *user )
{
    ...
    sess_cb->aid = aid;
    STlcopy( user, sess_cb->user_id, sizeof(sess_cb->user_id) - 1 );
...
rqst_show_pool( QUEUE *rsltlst, PTR sid, u_i2 cmd, i4 max_len )
{
    ...
    info = (GCD_SESS_INFO *)MEreqmem( 0, count * sizeof( GCD_SESS_INFO ),
                                      TRUE, NULL);
    ...
    */
    if ( ! info  ||  ! (count = gcd_pool_info( (PTR)info, info_flag )) )
        return;
...
rqst_show_sessions( QUEUE *rsltlst, PTR sid, u_i2 cmd, i4 max_len )
{
    ...
    ** Now load and display active session info.
    */
    if ( ! info  ||  ! (count = gcd_sess_info( (PTR)info, info_flag )) )
        return( max_len );
...
gcd_sess_info( PTR info, i4 flag ) 
{
	...
	switch( flag )
        ...
        if( selected )
        {
		 ...
		 if( cib->username )
                     STcopy( cib->username, sess->user_id);
		 if( cib->database )
                     STcopy( cib->database, sess->user_id);
             }
...
rqst_show_server( GCD_SESS_CB *sess_cb, PTR scb )
{
    ...
    STprintf( sess_cb->msg_buff,"     Current:    %d ", 
    		GCD_global.cib_active - GCD_global.pool_cnt ); 
    max_len = build_rsltlst( &sess_cb->rsltlst_q, sess_cb->msg_buff, max_len );

    if ( ( GCD_global.cib_max == -1 ) ||  (GCD_global.cib_max == 0 )) 
    	STprintf( sess_cb->msg_buff,"     Limit:      %s ", "none"); 
    else 
    	STprintf( sess_cb->msg_buff,
	    "     Limit:      %d ", GCD_global.cib_max); 
    max_len = build_rsltlst( &sess_cb->rsltlst_q, sess_cb->msg_buff, max_len );
...
static void
rqst_sys_stats( GCD_CCB * ccb, char *srv_class, char *gca_addr )
{
    i4          perms = 0;
