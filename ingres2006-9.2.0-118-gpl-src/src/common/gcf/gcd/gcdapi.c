...
gcd_api_init( void )
{
    ...
    init.in_timeout = -1;
    init.in_version = IIAPI_VERSION_6;
	...
	GCD_global.api_env = init.in_envHandle;
	break;
	...
	set.se_envHandle = GCD_global.api_env;
...
**
** Description:
**	Terminate the GCD API interface.
**
** Input:
**	None.
**
...
gcd_api_term( void )
{
    if ( GCD_global.api_env )
    {
	IIAPI_RELENVPARM	rel;
	IIAPI_TERMPARM		term;

	rel.re_envHandle = GCD_global.api_env;
	GCD_global.api_env = NULL;
