...
**
** Description:
**	Returns an indication that the Service Provider
**	services the provided request.  Called for first
**	data packet received on a connection to determine
**	which TL Service will service the connection.
**
**	If accepted, the RCB is consumed.  If not accepted,
**	the RCB is not changed.
**
**	If the Service Provider would normally accept the
**	connection but an error occurs, the provider still
**	indicates that the connection is accepted and
**	initiates whatever actions are necessary to abort
**	the connection.  This must ultimately result in
...
dmtl_accept( GCC_SRVC *gcc_service, GCD_RCB *rcb )
{
    ...
    */
    ...
    /*
    ** We will accept this connection from GCC,
    ...
    */
	...
	else
	{
	    ...
	    */
	    status = (*gcd_dam_service.init)( &dmtl_msg_service, ccb, 
					       &msg_parms, &msg_len );
	    if ( status == OK )  ccb->tl.msg_service = &gcd_dam_service;
	}
	...
	** Subsequent packets must use our packet ID which
	** the protocol level.
	*/
	gcd_init_rcb( rcb );
...
dmtl_data( GCD_RCB *rcb )
{
	...
	*/
    ...
    default : 
	if ( GCD_global.gcd_trace_level >= 1 )
	    TRdisplay( "%4d    GCD unknown DMTL packet type: 0x%x\n",
		       ccb->id, pkt_type );
	break;
    }
