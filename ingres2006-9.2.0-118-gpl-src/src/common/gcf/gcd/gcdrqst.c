...
gcd_msg_request( GCD_CCB *ccb )
{
    ...
    char		schema_name[ GCD_NAME_MAX + 1 ];
    char		proc_name[ GCD_NAME_MAX + 1 ];
    char		db_name[ GCD_NAME_MAX + 1 ];
    char		info_id[ GCD_NAME_MAX + 1 ];
    ...
    */
	...
	case MSG_RQP_SCHEMA_NAME :
	    if ( qp.val_len >= sizeof( schema_name )  ||
		 ! gcd_get_bytes( ccb, qp.val_len, (u_i1 *)schema_name ) )
		break;
	...
	case MSG_RQP_PROC_NAME :
	    if ( qp.val_len >= sizeof( proc_name )  ||
		 ! gcd_get_bytes(ccb, qp.val_len, (u_i1 *)proc_name) )
		break;
	...
	case MSG_RQP_INFO_ID :
	    if ( qp.val_len >= sizeof( info_id )  ||
		 ! gcd_get_bytes(ccb, qp.val_len, (u_i1 *)info_id) )
		break;
	    
	    info_id[ qp.val_len ] = EOS;
	...
	case MSG_RQP_DB_NAME :
	    if ( qp.val_len >= sizeof( db_name )  ||
		 ! gcd_get_bytes(ccb, qp.val_len, (u_i1 *)db_name) )
		break;
	...
	*/
	...
	STcopy( proc_name, ccb->rqst.rqst_id1 );
	...
	*/
	if ( p_rec & RQP_SCHEMA )  
	{
	    ...
	    */
	    ...
	    STcopy( schema_name, ccb->rqst.rqst_id0 );
	}
	else
	{
	    ccb->rqst.rqst_id0[0] = EOS;
	...
	if ( ! gcd_expand_qtxt(ccb, STlength(qry_info) + info_id_len, FALSE) )
	{
	...
	pcb->data.query.type = IIAPI_QT_QUERY;
	pcb->data.query.params = FALSE;
	STprintf( ccb->qry.qtxt, qry_info, info_id );
	...
	if ( ! gcd_expand_qtxt(ccb, STlength(qry_xids) + db_name_len, FALSE) )
	{
	    status = E_GC4808_NO_MEMORY;
	    break;
	}
	...
	*/
	if ( ! pcb->data.data.row_cnt )  
	    ...
	    extract_string( (IIAPI_DESCRIPTOR *)pcb->scb->column.desc, 
			    ...
			    ccb->rqst.rqst_id0, sizeof( ccb->rqst.rqst_id0 ) );
	/*
	** End of data.
	*/
	scb = pcb->scb;
    ...
    case REQ_INFO_5 :		/* End of query */
    {
	/*
	** Build and send the response messages.
	*/
	u_i2 len = STlength( ccb->rqst.rqst_id0 );

	gcd_msg_begin( ccb, &pcb->rcb, MSG_DATA, 0 );
	gcd_put_i1( &pcb->rcb, 1 );	/* Not NULL */
	gcd_put_i2( &pcb->rcb, len );
	gcd_put_bytes( &pcb->rcb, len, (u_i1 *)ccb->rqst.rqst_id0 );
...
proc_owner( GCD_PCB *pcb )
{
    ...
    */
	...
	extract_string( &desc[1], &data[1], name, sizeof( name ) );
	ccb->cib->usr = STalloc( name );
	extract_string( &desc[2], &data[2], name, sizeof( name ) );
	ccb->cib->dba = STalloc( name );
	extract_string( &desc[3], &data[3], name, sizeof( name ) );
    ...
    {
	char	owner[ GCD_NAME_MAX + 1 ];
	extract_string( &desc[0], &data[i], owner, sizeof( owner ) );
	...
	case 4 :
	    ...
	    {
		STcopy( owner, ccb->rqst.rqst_id0 );
		level = 3;
	    	found = TRUE;
	    }
...
save_pname( GCD_PCB *pcb )
{
	...
	if ( ! (name = (RQST_PNAME *)
			MEreqmem( 0, sizeof( RQST_PNAME ), FALSE, NULL )) )
	{
	    if ( GCD_global.gcd_trace_level >= 1 )
	...
	if ( column_length( &desc[1], &data[i+1] ) >= sizeof( name->name ) )
	{
	    if ( GCD_global.gcd_trace_level >= 1 )
		TRdisplay( "%4d    GCD Invalid parameter name length!\n", 
			   ccb->id );
	    continue;
	}

	extract_string(&desc[1], &data[i+1], name->name, sizeof(name->name));
...
save_pinfo( GCD_PCB *pcb )
{
	...
	if ( ! (info = (RQST_PINFO *)
			MEreqmem( 0, sizeof( RQST_PINFO ), FALSE, NULL )) )
	{
	    if ( GCD_global.gcd_trace_level >= 1 )
	...
	if ( column_length( &desc[1], &data[i+1] ) >= sizeof( info->name ) )
	{
	    if ( GCD_global.gcd_trace_level >= 1 )
		TRdisplay( "%4d    GCD Invalid info name length!\n", ccb->id );
	    continue;
	}
	
	extract_string(&desc[1], &data[i+1], info->name, sizeof(info->name));
