...
static	GCC_SRVC	gcc_service = { gcc_xoff, gcc_send, gcc_disc };
...
gcc_get_service( GCD_RCB *rcb )
{
    ...
    */
    ...
    {
	/*
	** There is a possible race condition if the 
	** TL Service Provider accepts but immediately 
	** aborts the connection because of an error 
	** detected in accept().  This could result
	** in a call to gcc_disc() prior to accept()
	** returning.  We cannot distinguish between
	** a successful accept and and accept with an
	** error.
	**
	** The service is dropped in gcc_disc(), so we
	** should not save the service after calling
	** accept.  The service is saved prior to the 
	** call and cleared if not accepted.
	*/
	ccb->gcc.tl_service = GCD_global.tl_services[ i ];
...
gcc_sm( GCD_RCB *rcb )
{
	...
	case TLA_OCMP :		/* GCC_OPEN completed */
	{
	    ...
	    */
	    ...
	    if ( rcb->gcc.p_list.function_parms.open.lsn_port )
		STcopy(rcb->gcc.p_list.function_parms.open.lsn_port, pib->port);
	    else
	...
	case TLA_LCMP :		/* GCC_LISTEN completed */
	{
	    ...
	    */
	    ...
	    /*
	    ** Expose new connection.
	    */
	    MOulongout( 0, (u_i4)ccb->id, sizeof( buff ), buff );
	    MOattach( MO_INSTANCE_VAR, GCD_MIB_CONNECTION, buff, (PTR)ccb );

	    GCD_global.connections++;
