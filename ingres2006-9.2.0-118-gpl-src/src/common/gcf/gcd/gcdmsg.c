...
**	gcd_msg_notify().  If the current request has completed,
...
build_msg_list( GCD_RCB *rcb )
{
	...
	*/
	...
	if ( GCD_global.gcd_trace_level >= 2 )
	    TRdisplay( "%4d    GCD received message %s length %d%s%s\n", 
...
process_messages( GCD_CCB *ccb )
{
    ...
    ** message is flushed (gcd_msg_flush() clears 
    ** GCD_MSG_FLUSH) or no more messages are queued.
    */
    while( ccb->msg.flags & GCD_MSG_FLUSH  &&  scan_msg_list( ccb ) )
	gcd_msg_flush( ccb );

    if ( ccb->msg.flags & GCD_MSG_FLUSH  ||  ! scan_msg_list( ccb ) )
    {
	/*
...
scan_msg_list( GCD_CCB *ccb )
{
	...
	*/
	...
	if ( ccb->msg.msg_flags & MSG_HDR_EOD )
	{
	    if ( GCD_global.gcd_trace_level >= 4 )
	    ...
	    ** the queue.
	    */
	    if ( (QUEUE *)next == &ccb->msg.msg_q  &&  ccb->msg.tl_service )
...
**
** Description:
**	Send a MSG_RESULT message to the client.  The
...
gcd_send_done( GCD_CCB *ccb, GCD_RCB **rcb_ptr, GCD_PCB *pcb )
{
    ...
    */
    ...
    gcd_send_result( ccb, rcb_ptr, pcb, TRUE );
    return;
}
...
**	result parameters may also be provided.  The request 
**	control block may not be used after this call if the 
**	flush parameter is TRUE.
**
** Input:
**	ccb	Connection control block.
...
gcd_send_trace( GCD_CCB *ccb, u_i2 len, char *msg )
{
	...
	*/ 
	if ( (trc_len + 1) > RCB_AVAIL( rcb ) )
	    ...
	    ** is complete (see gcd_msg_end()).  In either
	    ** case, the RCB can't be used for the current
	    ** trace element.
	    */
...
**	using gcd_msg_end() and may be split using gcd_msg_split().  
**
...
gcd_msg_end( GCD_RCB **rcb_ptr, bool flush )
{
    ...
    */
    gcd_hdr_upd( rcb, MSG_HDR_EOD | (flush ? MSG_HDR_EOG : 0) );
    ccb = rcb->ccb;
    ...
    */
    if ( flush  ||  ! QUEUE_EMPTY( &ccb->msg.info_q ) )
    {
	/*
	** Send the message.  The RCB is simply freed if the
	** TL service has been terminated.
...
gcd_hdr_upd( GCD_RCB *rcb, u_i1 flags )
{
    ...
    if ( GCD_global.gcd_trace_level >= 2 )
	TRdisplay( "%4d    GCD sending message %s: length %d%s%s\n", 
