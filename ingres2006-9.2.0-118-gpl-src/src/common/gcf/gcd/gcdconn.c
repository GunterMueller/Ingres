...
gcd_msg_connect( GCD_CCB *ccb )
{
	...
	*/
	if ( 
	     GCD_global.cib_max <= 0  ||
	     GCD_global.cib_active <= GCD_global.cib_max  ||  
	     gcd_pool_flush( GCD_global.cib_active - GCD_global.cib_max ) 
	   )
	{
	...
	*/
	if ( GCD_global.gcd_trace_level >= 1 )
	    TRdisplay( "%4d    GCD client limit reached: %d of %d\n",
		       ccb_id, GCD_global.cib_active, GCD_global.cib_max );

	status = E_GC480E_CLIENT_MAX;
	break;
    }
...
read_parms( GCD_CCB *ccb, DAM_ML_PM *cp, CONN_PARM *parms, u_i2 *count )
{
	...
	*/
	...
	if ( STequal( str_val, "multinational" ) )
	    *(II_LONG *)val = (II_LONG)DB_MLTI_DFMT;
	else  if ( STequal( str_val, "multinational4") )
	    *(II_LONG *)val = (II_LONG)DB_MLT4_DFMT;
	else  if ( STequal( str_val, "finland" )  ||  
		   STequal( str_val, "sweden" ) )
	    *(II_LONG *)val = (II_LONG)DB_FIN_DFMT;
	else  if ( STequal( str_val, "iso" ) )
	    *(II_LONG *)val = (II_LONG)DB_ISO_DFMT;
	else  if ( STequal( str_val, "iso4" ) )
	    *(II_LONG *)val = (II_LONG)DB_ISO4_DFMT;
	else  if ( STequal( str_val, "us" ) )
	    *(II_LONG *)val = (II_LONG)DB_US_DFMT;
	else  if ( STequal( str_val, "german" ) )
	    *(II_LONG *)val = (II_LONG)DB_GERM_DFMT;
	else  if ( STequal( str_val, "ymd" ) )
	    *(II_LONG *)val = (II_LONG)DB_YMD_DFMT;
	else  if ( STequal( str_val, "dmy" ) )
	    *(II_LONG *)val = (II_LONG)DB_DMY_DFMT;
	else  if ( STequal( str_val, "mdy" ) )
	    *(II_LONG *)val = (II_LONG)DB_MDY_DFMT;
	else
	{
	    if ( GCD_global.gcd_trace_level >= 1 )
