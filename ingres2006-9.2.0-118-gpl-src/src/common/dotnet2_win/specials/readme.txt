...
Strong-named assemblies must be signed with a Strong Name Key (SNK).  In order to preserve security, Ingres Corporation does not distribute an SNK file.  If you wish to build the Ingres .NET from source then you must create an SNK before the first build.  To create an SNK, locate the Microsoft .NET Strong Name Tool “sn.exe”. The “sn.exe” tool is usually in the "bin" directory of Visual Studio .NET.
For example:
...
The assemblyinfo file in each of the projects for the four assemblies will reference the keypair.snk file in this directory to sign the assembly.  All three assemblies must be signed with the same keypair.snk file. 
