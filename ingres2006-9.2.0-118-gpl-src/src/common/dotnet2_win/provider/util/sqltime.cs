		...
		*/
		...
		get(TimeZone tz)
		{
			...
			switch (dbms_type)
			{
				case DBMS_TYPE_TIME:
					{
						if (tz == null) tz = TimeZone.CurrentTimeZone;
						now = DateTime.Now;
						...
						*/
						...
						else
						{
							...
							*/
							daylightTime = tz.GetDaylightChanges(now.Year);
							int dst = (int)((daylightTime.Delta.Ticks /
								TimeSpan.TicksPerMillisecond) / 60000);
							time = SqlDates.parseTime(value, SqlDates.getTZ(-dst));
						}
				...
				case DBMS_TYPE_TMTZ:
					...
					*/
					...
					{
						daylightTime = tz.GetDaylightChanges(now.Year);
						tz = SqlDates.getTZ(timezone);
						long dstTicks = daylightTime.Delta.Ticks /
							TimeSpan.TicksPerMillisecond / 60000;
						long tzTicks = tz.GetUtcOffset(now).Ticks;
						long diffTicks = tzTicks - dstTicks;
						int diffMilliseconds =
							(int)((tzTicks - diffTicks) / TimeSpan.TicksPerMillisecond);
						time = SqlDates.parseTime(value,
							SqlDates.getTZ(diffMilliseconds / 60000));
					}
					break;
