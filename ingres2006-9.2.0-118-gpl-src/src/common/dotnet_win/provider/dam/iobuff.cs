...
namespace Ca.Ingres.ProviderInternals
{
	...
	*/
	...
	internal class IoBuff : TlConst
	{
		...
		** I/O streams.
		*/
		private System.IO.Stream inputStream  = null; // Input data stream.
		private System.IO.Stream outputStream = null; // Output data stream.
		...
		**	    Separated tracing interface and implementation.*/
		...
		/// buffer size to initialize the input buffer.
		/// </summary>
		/// <param name="ioStream">The associated input/output stream.</param>
		/// <param name="fileAccess">Read or Write to indicate direction of stream.
		/// </param>
		protected internal IoBuff(System.IO.Stream     ioStream,
		                          System.IO.FileAccess fileAccess)
		{
			trace = new Tracing(DAM_NL_TRACE_ID);

			if (fileAccess == System.IO.FileAccess.Write)
			{
				title = "IoBuff[out]";
				this.outputStream = ioStream;
			}
			else // (fileAccess == System.IO.FileAccess.Read)
			{
				title = "IoBuff[in]";
