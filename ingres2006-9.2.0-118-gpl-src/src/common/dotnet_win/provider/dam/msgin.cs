...
namespace Ca.Ingres.ProviderInternals
{
	...
	*/
		...
		MsgIn : MsgOut
	{
		...
		**
		** Description:
		**	Class constructor.  Initializes the input buffer.  Receives 
		**	the DAM-TL Connection Confirm packet.  DAM-ML connection 
		**	parameters are returned
		**
		** Input:
		**	target		Hostname and port.
		**	msg_cp		DAM-ML connection parameters.
		**	char_set	Character set/encoding (may be null).
		**	log		ITrace log.
		**
		** Output:
		**	msg_cp		DAM-ML connection parameters.
		...
		*/
		...
		protected
			MsgIn( String target, ref byte[] msg_cp, CharSet char_set ) :
			base(target, msg_cp)
		{
			title = "MsgIn[" + ConnID + "]";
			msg_cp = null;	// No output yet.

			try 
			{ 
				inBuff = new InBuff( socket.getInputStream(), 
			...
			try
			{
				...
				*/
				...
				/*
				** Connection character encoding: either explicitly provided
				** or mapped from connection character set ID or name.
				*/
				this.CharSet = (char_set != null) ? char_set : getCharSet(cs_id, cs);
			}
			catch( SqlEx ex )
			{
				if ( trace.enabled( 1 ) )  
			...
			*/
			...
			if ( trace.enabled( 3 ) )
			{
				...
				trace.write( "        " + this.CharSet );
			}

			/*
			** If no DAM-ML connection parameters were received, 
			** return a zero length array.
			*/
			if ( msg_cp == null )  msg_cp = new byte[0];
			return;
		} // MsgIn
			...
			getCharSet( int id, String name )
		{
			...
			{ 
				...
				*/
				if ( id != 0 )
					...
					{ 
						...
						if ( trace.enabled( 5 ) )
						{
							trace.write( title + ": mapped Charset ID " + id +
								": "  );
							trace.write( "        " + charset );
						}
					}
				...
				*/
				if ( charset == null )
					if ( name == null )
						charset = CharSet.getCharSet( null );	// Default encoding.
					else
					{
						charset = CharSet.getCharSet( name ); 
		
						if ( trace.enabled( 5 ) )
							trace.write( title + ": mapped Charset '" + name + 
								"': " + charset );
					}
			}
			catch( Exception )
			{
				if ( trace.enabled( 1 ) )
					trace.write( title + ": incompatible character set: " + name );
			...
			if ( inBuff.avail() >= 2 )
			{
				...
				if ( param_id == DAM_TL_DP_ERR  &&  inBuff.avail() >= param_len ) 
				{
					...
					*/
					...
					throw SqlEx.get( sqlEx.Message, sqlEx.getSQLState(), error );
				}
			}
		...
		**	data value (the IByteArray limit()).
		**
		** Input:
		**	None.
		**
		...
		**	SqlChar data value (the IByteArray limit()).
		**
		** Input:
		**	None.
		**
		...
		**	of the SqlNChar data value (the ICharArray limit()).
		**
		** Input:
		**	None.
		**
