...
namespace Ca.Ingres.ProviderInternals
{
	...
	**	method in turn calls the default handler methods to read and 
	**	process specific message types: readDesc(), readData(), 
	**	readError() and readDone().  There is also a default handler 
	**	method for procedure return values: setProcResult().  These
	**	methods are expected to be overridden by sub-classes which
	**	need to provide non-default processing.
	**
	**  Constants:
	**
	...
	**	rslt_val_rows		Query update count.
	...
	*/
	...
	internal class DrvObj : MsgConst
	{
		...
		*/
		...
		protected internal int    rslt_val_rows  = -1;
		...
		**
		**	Date/time internal values are GMT as are Ingres dates.
		...
		*/
		...
		protected bool
			timeValuesInGMT()
		{
			return( conn.use_gmt_tz  ||  conn.ingres_tz );
		} // timeValuesInGMT
		...
		**	    Replaced repository with formatters for GMT and local timezone.*/
		...
		protected internal virtual DateFormat ConnDateFormat
		{
			get
			{
				return (conn.use_gmt_tz?
				            df_gmt:
				            df_lcl);
			}
		...
		*/
		...
		protected internal virtual SqlEx readError()
		{
			...
			if (type == MSG_ET_ERR)
			{
				...
				if (trace.enabled(1))
					trace.write(tr_id + ": Received error" + " '" + sqlState +
						"' 0x" + System.Convert.ToString(status, 16) +
						" -- " + message);
			}
			else
			{
				if (trace.enabled((type == MSG_ET_MSG)?3:1))
					trace.write(tr_id + ": Received " +
						(type == MSG_ET_MSG?"message":"warning") +
						" '" + sqlState + "' 0x" +
						System.Convert.ToString(status, 16) +
						" -- " + message);
		...
		*/
			...
			while (msg.moreData())
			{
				...
				switch (param_id)
				{
					case MSG_RP_ROWCOUNT: 
						switch (param_len)
						{
							case 1:  rslt_val_rows = msg.readByte(); break;
							case 2:  rslt_val_rows = msg.readShort(); break;
							case 4:  rslt_val_rows = msg.readInt(); break;
							default: 
								if (trace.enabled(1))
									trace.write(tr_id +
					...
					switch (param_len)
					{
						case 1:  ProcResult = msg.readByte();  break;
						case 2:  ProcResult = msg.readShort(); break;
						case 4:  ProcResult = msg.readInt();   break;
						default: 
							if (trace.enabled(1))
					...
					default: 
						if (trace.enabled(1))
							trace.write(tr_id + ": Invalid result param ID " + param_id);
						throw SqlEx.get( ERR_GC4002_PROTOCOL_ERR );
		...
		/*
		** Name: ProcResult property
		**
		** Description:
		**	Set the rslt_val_proc and returnProc class variables.
		**
		** Input:
		...
		*/
		...
		protected internal virtual int ProcResult
		{
			set
			{
				if (trace.enabled(3))
					trace.write(tr_id + ".setProcResult(" + value + ")");
