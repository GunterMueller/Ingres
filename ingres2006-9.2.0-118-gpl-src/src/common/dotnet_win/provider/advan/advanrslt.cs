...
namespace Ca.Ingres.ProviderInternals
{
	...
	**	TIMESTAMP	IngresDate
	**
	**	BIT, FLOAT, NUMERIC, CHAR, and BINARY are treated the same as
	**	BOOLEAN, DOUBLE, DECIMAL, VARCHAR and VARBINARY, respectively.
	**	DATE and TIME are not handled directly as values, but may be
	**	represented by an IngresDate object.
	**
	...
	**  Protected Methods:
	**
	**	getSqlData		Generic column data access.
	**	getColByName		Determine column index from column name.
...
*/
		...
		*/
		...
		{ updateObject( getColByName( name ), value, -1 ); }

		public void 
			updateObject( String name, Object value, int scale ) 
		{ updateObject( getColByName( name ), value, scale ); }
		...
		*/
			...
			getBoolean( int index )
		{
			...
			SqlData	data = getSqlData( index );
			bool	value = isNull ? false : data.getBoolean();
		...
		*/
			...
			getByte( int index )
		{
			...
			SqlData	data = getSqlData( index );
			byte 	value = isNull ? (byte)0 : data.getByte();
		...
		*/
			...
			getShort( int index )
		{
			...
			SqlData	data = getSqlData( index );
			short 	value = isNull ? (short)0 : data.getShort();
		...
		*/
			...
			getInt( int index )
		{
			...
			SqlData	data = getSqlData( index );
			int 	value = isNull ? 0 : data.getInt();
		...
		*/
			...
			getLong( int index )
		{
			...
			SqlData	data = getSqlData( index );
			long	value = isNull ? 0L : data.getLong();
		...
		*/
			...
			getFloat( int index )
		{
			...
			SqlData	data = getSqlData( index );
			float	value = isNull ? 0.0F : data.getFloat();
		...
		*/
			...
			getDouble( int index )
		{
			...
			SqlData	data = getSqlData( index );
			double	value = isNull ? 0.0 : data.getDouble();
		...
		*/
			...
			getDecimal( int index, int scale )
		{
			...
			SqlData	data = getSqlData( index );
			Decimal	value = isNull ? Decimal.MinValue : data.getDecimal();
		...
		*/
			...
			getString( int index )
		{
			...
			{
				value = (rs_max_len > 0 ) ? data.getString( rs_max_len )
					: data.getString();
				/*
		...
		*/
			...
			getBytes( int index )
		{
			...
			SqlData	data = getSqlData( index );
			byte[]	value = isNull ? null :
				((rs_max_len > 0) ? data.getBytes( rs_max_len )
				: data.getBytes());
		...
		*/
			...
			getDate( int index )
		{
			...
			SqlData	data = getSqlData( index );
			DateTime	value = isNull ? DateTime.MinValue : 
				data.getDate( null );
		...
		*/
			...
			getTime( int index )
		{
			...
			SqlData 	data = getSqlData( index );
			TimeSpan	value = isNull ? TimeSpan.MinValue : 
				data.getTime( null );
		...
		*/
			...
			getBinaryStream( int index )
		{
			...
			SqlData	data = getSqlData( index );
			InputStream value = isNull ? null : data.getBinaryStream();
		...
		*/
			...
			getAsciiStream( int index )
		{
			...
			SqlData	data = getSqlData( index );
			InputStream value = isNull ? null : data.getAsciiStream();
		...
		*/
			...
			getUnicodeStream( int index )
		{
			...
			SqlData	data = getSqlData( index );
			InputStream value = isNull ? null : data.getUnicodeStream();
		...
		*/
			...
			getCharacterStream( int index )
		{
			...
			SqlData	data = getSqlData( index );
			Reader	value = isNull ? null : data.getCharacterStream();
		...
		*/
			...
			getObject( int index )
		{
			...
			Object	value = isNull ? null :
				((rs_max_len > 0 ) ? data.getObject( rs_max_len )
				: data.getObject() );
		...
		*/
		...
		{ return( getBoolean( getColByName( name ) ) ); } 

		public byte
			getByte( String name )
		{ return( getByte( getColByName( name ) ) ); } 

		public short
			getShort( String name )
		{ return( getShort( getColByName( name ) ) ); }

		public int
			getInt( String name )
		{ return( getInt( getColByName( name ) ) ); }

		public long
			getLong( String name )
		{ return( getLong( getColByName( name ) ) ); }

		public float
			getFloat( String name )
		{ return( getFloat( getColByName( name ) ) ); }

		public double
			getDouble( String name )
		{ return( getDouble( getColByName( name ) ) ); }

		public Decimal
			getDecimal( String name, int scale )
		{ return( getDecimal( getColByName( name ), scale ) ); }
		...
		{ return( getString( getColByName( name ) ) ); }

		public byte[]
			getBytes( String name )
		{ return( getBytes( getColByName( name ) ) ); }

		public DateTime
			getDate( String name )
		{ return( getDate( getColByName( name ) ) ); }

		public TimeSpan
			getTime( String name )
		{ return( getTime( getColByName( name ) ) ); }

		public DateTime
			getTimestamp( String name )
		{ return( getTimestamp( getColByName( name ) ) ); }

		public InputStream
			getBinaryStream( String name )
		{ return( getBinaryStream( getColByName( name ) ) ); }

		public InputStream
			getAsciiStream( String name )
		{ return( getAsciiStream( getColByName( name ) ) ); }

		public InputStream
			getUnicodeStream( String name )
		{ return( getUnicodeStream( getColByName( name ) ) ); }

		public Reader
			getCharacterStream( String name )
		{ return( getCharacterStream( getColByName( name ) ) ); }

		public Array
			getArray( String name )
		{ return( getArray( getColByName( name ) ) ); }

		public Object 
			getObject( String name )
		{ return( getObject( getColByName( name ) ) ); }
		...
		**
		** Input:
		**	index		External column index (1 based).
		**
		** Output:
		**	None.
		**
		...
		*/
			...
			getSqlData( int index )
		{
			...
			{
				warnings = null;
				SqlData data = columns[ columnMap( index ) ];
				isNull = data.isNull();

				if ( ! isNull  &&  data.isTruncated() )  
					setWarning( DataTruncation( rsmd.desc[index].name, index, false,
						true, data.getDataSize(), data.getTruncSize() ) );
				return( data );
			}
		} // getSqlData
		...
		*/
			...
			columnMap( int index )
		{
			...
			if ( index < 0  ||  index >= rsmd.count  ||
				pos_status != POS_ON_ROW  ||  (row_status & ROW_DELETED) != 0 )
		...
		*/
		...
		private Exception DataTruncation(
			string colName,
			int colOrdinal,
			bool parm,
			bool reading)
