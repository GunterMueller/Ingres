	...
	public sealed class MetaData
	{
		...
		*/
		...
		private void BuildTableColumnInfoForSELECTStatement(string cmdText)
		{
			...
			upperList.Add(";");  // add a few tokens at end to avoid 
			upperList.Add(";");  // worrying about overflow on lookahead
			upperList.Add(";");
			upperList.Add(";");
			upperList.Add(";");
			upperList.Add(";");
			...
			int i = 1;  // index into the SELECT stmt token list
			...
			while (i < CmdTokenList.Count)  // loop through tables list
			{
				...
				break;   // end of table list reached
			}  // end while loop through tables list

			this._columns = Columns;
			this._tables  = Tables;

			if (!token.Equals("ORDER"))   // if not "ORDER BY", return good
				return;
			token = (string)upperList[++i];
			if (!token.Equals("BY"))   // if not "ORDER BY", return good
				return;
		...
		*/
		...
		/// Scan the SQL statment and build tokens.
		/// </summary>
		internal static ArrayList ScanSqlStatement(string cmdText)
		{
		...
		*/
		...
		public static string WrapInQuotesIfEmbeddedSpaces(string identifier)
		{
			...
			{
				if (c == '"')
					sb.Append('"');  // double up on the quote
				sb.Append(c);
			}
