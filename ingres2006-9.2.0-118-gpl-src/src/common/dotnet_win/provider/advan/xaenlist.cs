...
namespace Ca.Ingres.ProviderInternals
{
	...
	*/
	...
	/// <summary>
	/// Represents an enlistment in DTC.
	/// </summary>
	...
	internal sealed class DTCEnlistment : ITransactionResourceAsync
	{
		...
		{
			debugging = false;  // for developer debugging output Console
		...
		*/
		...
		/// <param name="isExplicitTransaction">Indicator for manual or
		/// automatic transaction.</param>
			...
			bool         isExplicitTransaction)
		{
			...
			trace.write(3, title + ": Enlist(isExplicitTransaction=" +
				isExplicitTransaction.ToString() +")");
			if (debugging)
				Console.WriteLine("Enlist(" +
					isExplicitTransaction.ToString() +") begins...");
			...
			*/
			...
			contains the DSN and the name of the client DLL:
			   1) LoadLibrary the client dll (Ca.Ingres.Support.dll which will
			      drag in the referenced Ca.Ingres.ProviderInternals from the GAC).
			   ...
			   6) Write a record to the DTCXATM.LOG file indicating that a new
			     ...
			     the DSN name, the name of our RM (Ca.Ingres.Support.dll), and 
			...
			*/
			...
			// Locate the Ca.Ingres.Support.dll that holds our XASwitch
			// support.  The dll should be in the GAC so use a type from
			// the Ca.Ingres.Support assembly, qualified by the same
			// Version and PublicKeyToken that the currently executing
			// Ca.Ingres.Support assembly is using.
			...
			if (index != -1)
				...
				version = ", Version=1.1.0.0, Culture=neutral" +
				          ", PublicKeyToken=1234567890123456";

			Type type = Type.GetType(
				"Ca.Ingres.Support.XAProvider" +
				", Ca.Ingres.Support" + version);
			// BTW, type will be null if Ca.Ingres.Support is not properly
			// in the GAC or has a different version number from
			// Ca.Ingres.Client.  A NullReferenceException would follow.
			...
			identifying the global transaction identifier.
			...
			*/
			...
			advanConnect.msg.LockConnection(); // wait until EnlistWithRM is done
			advanConnect.msg.UnlockConnection();

				advanConnect.startTransaction(new AdvanXID(xaXIDstruct));
			}
			catch
			{
				// release our connection to MS DTC and
		...
		*/
		...
		/// Transaction Coordinator (MSDTC), and release our cookie to it.
		/// </summary>
		public void Delist(bool ReleaseConnection)
		{
			try
			{
			trace.write(3, title + ": Delist(ReleaseConnection=" +
				ReleaseConnection.ToString() +")");
			if (debugging)
				Console.WriteLine("Delist()...");
			...
			if (trace.enabled(4))
			{
				if (ReleaseConnection)
					trace.write(title + ": Delist's " +
						"MSDTCRequest.DELIST starting");
				else
					trace.write(title + ": Delist's " +
						"MSDTCRequest.DELISTandRELEASECONECTION starting");
			}

			if (debugging  &&  ReleaseConnection)
				Console.WriteLine("Delist's MSDTCRequest.DELIST starting");
			if (debugging  &&  !ReleaseConnection)
				Console.WriteLine("Delist's MSDTCRequest." +
				                  "DELISTandRELEASECONECTION starting");

			// release pooled connection if requested
			if (!StartMSDTCRequest(ReleaseConnection ?
				MSDTCRequest.DELISTandRELEASECONECTION :
				MSDTCRequest.DELIST))
			{	// if MSDTC worker thread is down then release it ourself.
				if (ReleaseConnection) // release pooled connection if requested
					// put the connection back in pool or close it
					AdvanConnectionPoolManager.Put(advanConnect);
			}

			trace.write(3, title + ": Delist() complete");
			if (debugging)
				Console.WriteLine("Delist() exiting...");
			}
		...
		*/
			...
			int  fSinglePhase)
		{
			...
			if (debugging)
				Console.WriteLine("PrepareRequest called");
		...
		*/
			...
			IntPtr /*BOID*/ pNewUOW)
		{
			...
			if (debugging)
				Console.WriteLine("CommitRequest called");
		...
		*/
			...
			IntPtr /*BOID*/  pNewUOW)
		{
			...
			if (debugging)
				Console.WriteLine("AbortRequest called");
		...
		void ITransactionResourceAsync.TMDown()
		{
			...
			if (debugging)
				Console.WriteLine("TMDown called");
			try
			{
		...
		bool StartMSDTCRequest(MSDTCRequest request)
		{
			lock(MSDTCRequestQueue)
			{
					...
					txResourceAsyncThread.IsAlive == false)
				{
					...
					txResourceAsyncThread.ApartmentState = ApartmentState.STA;
		...
		*/
		...
		void TxResourceAsyncThreadMethod()
		{
			...
			bool ReleaseConnection  = false;  // release conn back to pool?
			...
			while(true)
			{
				...
				lock(MSDTCRequestQueue)
				{
					...
					trace.write(3, title + ": Asynch thread processing " +
						request.ToString() + "...");
					if (debugging)
						Console.WriteLine("Asynch thread processing " +
							request.ToString() + "...");

					if (request == MSDTCRequest.DELISTandRELEASECONECTION)
					{
						ReleaseConnection  = true;
						request = MSDTCRequest.DELIST;
					}
				}  // end lock
				...
				// if tried and failed to ENLIST then close down thread
				if (request == MSDTCRequest.ENLIST  &&  this.RMCookie == 0)
					ShutdownThread = true;
			...
			// put the connection back in pool or close it
			if (ReleaseConnection)  // if requested by Delist(true)
				AdvanConnectionPoolManager.Put(advanConnect);
		...
		void TxResourceAsyncThreadProcessRequest(MSDTCRequest request)
		{
			...
			switch(request)
			{
				case MSDTCRequest.ENLIST:
					try
					{
						trace.write(4, title + ": Enlisting with RM...");
						if (debugging)
							Console.WriteLine("Enlisting with RM...");
						...
						trace.write(4, title + ": Enlisted with RM successfully");
						if (debugging)
							Console.WriteLine("Enlisted with RM");
					}
					catch (Exception ex)
					{
				...
				case MSDTCRequest.DELISTandRELEASECONECTION:
					break;
				...
				case MSDTCRequest.PREPARE:
				{
					...
					try
					{
						...
						trace.write(3, title +
							...
							hr.ToString("X8") + ")");
						if (debugging)
							Console.WriteLine("PrepareRequestDone called");
				...
				case MSDTCRequest.COMMIT:
				{
					...
					try
					{
						...
						trace.write(3, title +
							...
							hr.ToString("X8") + ")");
						if (debugging)
							Console.WriteLine("CommitRequestDone called");
						XaHelperSinglePipe_ReleaseRMCookie(ref this.RMCookie);
					}
					catch (Exception ex)
					{
				...
				case MSDTCRequest.ABORT:
				{
					...
					try
					{
						...
						trace.write(3, title +
							...
							hr.ToString("X8") + ")");
						if (debugging)
							Console.WriteLine("AbortRequestDone called");
						XaHelperSinglePipe_ReleaseRMCookie(ref this.RMCookie);
					}
					catch (Exception ex)
					{
		...
		*/
		...
		private uint PrepareTx()
		{
			try
			{
				advanConnect.prepareTransaction();
				return S_OK;
			}
			catch (Exception)
			{
		...
		*/
			...
			int       fNormal)
		{
			...
			if (debugging)
				Console.WriteLine("ReleaseRMCookie");
			try
			{
