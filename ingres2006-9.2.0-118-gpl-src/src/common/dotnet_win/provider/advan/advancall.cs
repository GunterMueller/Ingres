...
namespace Ca.Ingres.ProviderInternals
{
	...
	**	setBigDecimal		Send parameter as a Decimal value.
	...
	*/
		...
		AdvanCall : AdvanPrep
	{
		...
		*/
		...
		private ProviderType[]	opType =	// Output param type
			new ProviderType[0];
		...
		*/
						...
						DrvConst.CLOSE_CURSORS_AT_COMMIT )
		{
			...
			*/
			paramSet.setDefaultFlags( ParamSet.PS_FLG_PROC_PARAM );
			...
			*/
			try
			{
				SqlParse parser = new SqlParse( sql, timeLiteralsInGMT() );
				...
				for( int param = 0; param < param_cnt; param++ )
				{
					switch( procInfo.getParamType( param ) )
					{
						...
						default :			/* Literal input parameters */
						{
							...
							ProviderType sqlType = ParamSet.getSqlType( value );
							paramSet.setObject( param, value, 
								sqlType, -1, useAltStorage( sqlType ) );
						}
							break;
					}
		...
		*/
			...
			addBatch()
		{
			...
			lock( batch ) 
			{ batch.Add( hasByRefParam ? null : paramSet.clone() ); }
		...
		*/
			...
			registerOutParameter( int paramIndex, ProviderType sqlType, int scale )
		{
			...
			{
				/*
				** Flag the parameter as an output parameter and set 
				** the output parameter map.  The actual mapping will 
				** be done after all parameters have been registered,
				** so for now zero is used to indicate a mappable 
				** parameter.
				*/
				int index = paramMap( paramIndex );
				paramSet.setFlags( index, ParamSet.PS_FLG_PROC_BYREF );
				opMap[ index ] = 0;
				...
				/*
				** Output parameters will be coerced to the desired type 
				** via the result-set getXXX methods, so we ignore the 
				** registered type.  However, since Ingres BYREF parameters 
				** require an input value, we use the registered type to 
				** provide a NULL value if no other input is provided.
				*/
				if ( ! paramSet.isSet( index ) )  
					paramSet.setObject( index, null, 
						sqlType, scale, useAltStorage( sqlType ) );
			}
		...
		{ registerOutParameter( index, type, 0 );
		} // registerOutParameter
		...
		*/
			...
			registerOutParameter( String parameterName, ProviderType sqlType, int scale )
		{
			...
			*/
			if ( ! paramSet.isSet( index ) ) 
				paramSet.setObject(index, null, sqlType, scale, useAltStorage(sqlType));
			return;
		} // registerOutParameter
		...
		*/
		...
		{ registerOutParameter( name, type, 0 ); }

		internal void
			registerOutParameter( int index, ProviderType type, String tn )
		{ registerOutParameter( index, type, 0 ); }
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setNull('" + parameterName + 
										"'," + sqlType + ",'" + typeName + "')" );

			paramSet.setObject( findParam( parameterName ), null, 
		...
		*/
			...
			setBoolean( String parameterName, bool value )
		{
			...
			paramSet.set( findParam( parameterName ), 
				ProviderType.Boolean, value, false );
			return;
		} // setBoolean
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setByte('" + 
										parameterName + "', " + value + " )" );
    
			paramSet.set( findParam( parameterName ), 
				ProviderType.TinyInt, value, false );
			return;
		} // setByte
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setShort('" + 
										parameterName + "', " + value + " )" );
    
			paramSet.set( findParam( parameterName ), 
				ProviderType.SmallInt, value, false );
			return;
		} // setShort
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + " setInt('" + 
										parameterName + "', " + value + " )" );

			paramSet.set( findParam( parameterName ), 
				ProviderType.Integer, value, false );
			return;
		} // setInt
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setLong('" + 
										parameterName + "', " + value + " )" );

			paramSet.set( findParam( parameterName ), 
				ProviderType.BigInt, value, false );
			return;
		} // setLong
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setFloat('" + 
										parameterName + "', " + value + " )" );

			paramSet.set( findParam( parameterName ), 
				ProviderType.Real, value, false );
			return;
		} // setFloat
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setDouble('" + 
										parameterName + "', " + value + " )" );

			paramSet.set( findParam( parameterName ), 
				ProviderType.Double, value, false );
			return;
		} // setDouble
		...
		*/
		...
		public void
			setBigDecimal( String parameterName, Decimal value )
		{
			if ( trace.enabled() )  trace.log( title + ".setBigDecimal('" + 
										parameterName + "', " + value + " )" );

			paramSet.set( findParam( parameterName ),
				ProviderType.Decimal, value, false );
			return;
		} // setBigDecimal
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setString('" + 
										parameterName + "', " + value + " )" );

			if ( useAltStorage( ProviderType.VarChar ) )
				paramSet.setObject( findParam( parameterName ), 
					value, ProviderType.VarChar, -1, true);
			else
				paramSet.set( findParam( parameterName ),
					ProviderType.VarChar, value, false );
			return;
		} // setString
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setBytes('" + 
										parameterName + "', " + value + " )" );

			paramSet.set( findParam( parameterName ),
				ProviderType.VarBinary, value, false );
			return;
		} // setBytes
		...
		*/
			...
			setDate( String parameterName, Date value, Calendar cal )
		{
			...
			if ( value != null )
			{
				...
				** be in the client TZ.  Calendars can be applied in both
				** cases to get the actual date in the desired TZ.  Other-
				** wise, the date in the local TZ is used.
				*/
				if ( cal != null )
					str = SqlDates.formatDate( value, cal.getTimeZone() );
				else
					str = SqlDates.formatDate( value, false );
			}

			paramSet.set( findParam( parameterName ), ProviderType.DATE, str, false );
			return;
		} // setDate
		...
		*/
			...
			setTime( String parameterName, Time value, Calendar cal )
		{
			...
			{
				/*
				** The time is stored in GMT.  Calendars are not applied to
				** Ingres times since they are also stored in GMT.  Gateway
				** times are assumed to be in the client TZ, so calendars
				** can be applied to store values for other timezones.
				*/
				if ( cal != null  &&  ! conn.use_gmt_tz )
				{
					/*
					** First retrieve the time in the desired TZ.
					*/
					str = SqlDates.formatTime( value, cal.getTimeZone() );
				...
				** .NET and Ingres.  Due to these problems, format the time
				** as a timestamp to ensure consistent processing.
				*/
				str = SqlDates.formatTimestamp( value, timeValuesInGMT() );
			}

			paramSet.set( findParam( parameterName ), ProviderType.TIME, str, false );
			return;
		} // setTime
#endif
		...
		*/
			...
			setTimestamp( String parameterName, DateTime value )
		{
			...
			if ( trace.enabled() )  trace.log( title + ".setTimestamp('" + 
										parameterName + "', " + value + " )" );
			/*
			** The timestamp is stored in GMT.  Gateway
			** timestamps are assumed to be in the client TZ.
			*/
			/*
			...
			*/
			...
			paramSet.set( findParam( parameterName ),
				ProviderType.DateTime, str, false );
			return;
		} // setTimestamp
#endif
		...
		*/
			...
			setBinaryStream( String parameterName, InputStream stream, int length )
		{
			...
			paramSet.set(findParam(parameterName),
				ProviderType.LongVarBinary, stream, false);
			return;
		} // setBinaryStream
		...
		*/
			...
			setCharacterStream( String parameterName, Reader reader, int length )
		{
			...
			paramSet.set( findParam( parameterName ), 
				ProviderType.LongVarChar, reader, useAltStorage(ProviderType.LongVarChar) );
			return;
		} // setCharacterStream
		...
		*/
		...
		{
			if ( trace.enabled() )  trace.log( title + ".setObject('" + 
										parameterName + "', " + sqlType + ", " + scale + " )" );
    
			if ( sqlType == ProviderType.Other )
				paramSet.setObject( findParam( parameterName ), value, 
					ParamSet.getSqlType( value ), scale, true );
			else
				paramSet.setObject( findParam( parameterName ), value, 
					sqlType, scale, useAltStorage( sqlType ) );
			return;
		...
		*/
		...
		{ setObject( parameterName, value, ParamSet.getSqlType( value ), -1 ); }
		...
		*/
			...
			getDate( String parameterName )
		{
			...
			Date value = resultRow.getDate( resultMap( parameterName ) );
			null_param = resultRow.wasNull();
		...
		*/
			...
			getTime( String parameterName )
		{
			...
			Time value = resultRow.getTime( resultMap( parameterName ) );
			null_param = resultRow.wasNull();
				...
				*/
				...
				switch( opType[ opType.Length - 1 ] )
				{
					...
					case ProviderType.TinyInt :
						obj = (SByte)rslt_val_proc;
						break;

					case ProviderType.SmallInt :
						obj = (Int16)rslt_val_proc;
						break;

					case ProviderType.Integer :
						obj = (Int32)rslt_val_proc;
						break;

					case ProviderType.BigInt :
						obj = (Int64)rslt_val_proc;
						break;
			...
			getObject( AdvanRslt results, int index )
		{
			...
			switch( opType[ param ] )
			{
				...
				case ProviderType.DateTime :
					obj = results.getTimestamp( index );	break;
		...
		*/
		...
		private int
			resultMap( int index )
		{
			if ( index < 0  ||  index >= dpMap.Length  ||  
				dpMap[ index ] < 0  ||  opMap[ dpMap[ index ] ] < 0 )
				throw SqlEx.get( ERR_GC4011_INDEX_RANGE );
			...
			for( int param = 0; param < dpMap.Length; param++ )
				if ( dpMap[ param ] >= 0 )		// Dynamic parameter.
				{
					...
					if ( paramName != null  &&
						...
						ToInvariantUpper(paramName) ) )
					{
						if ( opMap[ index ] < 0 )	// Not registered.
							throw SqlEx.get( ERR_GC4012_INVALID_COLUMN_NAME );
						return( opMap[ index ] );
					}
				}
