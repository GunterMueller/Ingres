		...
		static Tracing()  // Class Initializer
		{
			try
			{
				...
				if ((KeyTraceLogFilename =
					ConfigurationSettings.AppSettings.Get(KEY_TRACE_LOG)) != null)
					open_log(KeyTraceLogFilename);
				
				if ((KeyTraceTime =
					ConfigurationSettings.AppSettings.Get(KEY_TRACE_TIME)) != null)
				{
		...
		**	    Handle security exceptions thrown by getProperty().*/
		...
		public Tracing(String id)
		{
			try
			{
				...
				{
					if (ConfigurationSettings.AppSettings != null  &&
						(level = ConfigurationSettings.AppSettings.Get(
							KEY_TRACE + "." + id)) != null)
					{
