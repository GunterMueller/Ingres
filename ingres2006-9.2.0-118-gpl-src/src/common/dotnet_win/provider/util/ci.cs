...
namespace Ca.Ingres.Utility
{
	...
	**
	** Description:
	**	Implements the encryption class used by the provider.
	**
	** History:
	**	14-Jul-99 (gordy)
	**	    Created.
	...
	*/
	...
	internal class CI
	{
		...
		*/
		internal const int CRYPT_SIZE = 8;
		
		private const int BITSPERBYTE = 8;
		private const int ITER_NUM = 8;
		private const int BITS_IN_CRYPT_BLOCK = (CRYPT_SIZE * BITSPERBYTE);
		
		/*
		** Constants used in CI but not formally defined.
		*/
		private const int KS_BITS = 48;
		
		internal const int KS_SIZE = (ITER_NUM * KS_BITS);
		...
		private static byte[] FP = new byte[]{
		...
		*/
		private static byte[] PC1_C = new byte[]{
			57, 49, 41, 33, 25, 17,  9,
			 1, 58, 50, 42, 34, 26, 18,
			10,  2, 59, 51, 43, 35, 27,
			19, 11,  3, 60, 52, 44, 36};
		
		private static byte[] PC1_D = new byte[]{
		...
		private static byte[] shifts = new byte[]{
		...
		*/
		private static byte[] PC2_C = new byte[]{
			14, 17, 11, 24, 1,  5,
			 3, 28, 15, 6, 21, 10,
			23, 19, 12, 4, 26,  8,
			16, 7, 27, 20, 13,  2};
		
		private static byte[] PC2_D = new byte[]{
			41, 52, 31, 37, 47, 55,
			30, 40, 51, 45, 33, 48,
			44, 49, 39, 56, 34, 53,
			46, 42, 50, 36, 29, 32};
		...
		*/
		...
		/*
		** This buffer is used by encode() and requires
		** synchronization.
		*/
		private static byte[] bit_map = new byte[CRYPT_SIZE * BITSPERBYTE];
		...
		**	    Created.*/
		...
		private static int expand(byte[] ate_bytes, int offset, byte[] b_map)
		{
			int accum = 0;
			 ...
			 for (int i = 0; i < CRYPT_SIZE; i++)
			{
				...
				{
					b_map[accum++] = (byte) ((c >> ((BITSPERBYTE - 1) - j)) & 0x01);
				}
			}
		...
		*/
		...
		private static int shrink(byte[] b_map, byte[] ate_bytes, int offset)
		{
			int accum = 0;
			...
			{
				byte c = 0;
				unchecked  // make sure overflows don't raise exception
				{          // and to allow wraps
					for (int j = 0; j < BITSPERBYTE; j++)
						c = (byte)((byte)(c << 1) | (byte)(b_map[accum++]));
				
					ate_bytes[offset++] = (byte)c;
				}
			}
		...
		**	    Created.*/
		...
		internal static void  set(byte[] key_str, byte[] KS)
		{
			int i, j, k;
				...
				*/
				 for (i = 0; i < ITER_NUM; i++)
				{
					 ...
					 for (j = 0; j < (KS_BITS / 2); j++)
					{
						...
						KS[(i * KS_BITS) + j + (KS_BITS / 2)] = D[PC2_D[j] - D.Length - 1];
					}
				}
			} // synchronized
...
//		private static void  encrypt(
		private static void  toString(  // obfuscate name
			byte[] KS,
			bool decode_flag,
			byte[] block)
		{
				 ...
				 for (ii = 0; ii < ITER_NUM; ii++)
				{
					...
					*/
					 for (j = 0; j < preS.Length; j++)
						preS[j] = (byte) (LR[(LR.Length / 2) + e[j] - 1] ^ KS[(i * KS_BITS) + j]);
					...
					*/
					 for (j = 0; j < (LR.Length / 2); j++)
						LR[(LR.Length / 2) + j] = (byte) (LR[0 + j] ^ f[P[j] - 1]);
...
//		internal static void  encode(
		internal static void  toString(  // obfuscate name
			byte[] plain_text,
			int p_offset,
			int size,
			byte[] ks,
			...
			lock(bit_map)
			{
				while (p_offset < end)
				{
					...
					*/
					...
					// encrypt using obfuscated encrypt name method
//					encrypt( ks, false, bit_map); // encrypt the bit map
					toString(ks, false, bit_map); // encrypt the bit map
