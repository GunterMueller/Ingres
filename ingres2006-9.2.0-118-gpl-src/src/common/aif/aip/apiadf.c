...
IIapi_initADF( II_VOID )
{
    ...
    r_stat = adg_startup( IIapi_static->api_adf_cb, adf_size, &adf_tab_dbms, 0, 0);
	...
	*/
	...
	if ( STequal( ptr, "multinational" ) )
	    IIapi_static->api_dfmt = DB_MLTI_DFMT;
	else  if ( STequal( ptr, "multinational4") )
	    IIapi_static->api_dfmt = DB_MLT4_DFMT;
	else  if ( STequal( ptr, "finland" )  ||  STequal( ptr, "sweden" ) )
	    IIapi_static->api_dfmt = DB_FIN_DFMT;
	else  if ( STequal( ptr, "iso" ) )
	    IIapi_static->api_dfmt = DB_ISO_DFMT;
	else  if ( STequal( ptr, "iso4" ) )
	    IIapi_static->api_dfmt = DB_ISO4_DFMT;
	else  if ( STequal( ptr, "us" ) )
	    IIapi_static->api_dfmt = DB_US_DFMT;
	else  if ( STequal( ptr, "german" ) )
	    IIapi_static->api_dfmt = DB_GERM_DFMT;
	else  if ( STequal( ptr, "ymd" ) )
	    IIapi_static->api_dfmt = DB_YMD_DFMT;
	else  if ( STequal( ptr, "dmy" ) )
	    IIapi_static->api_dfmt = DB_DMY_DFMT;
	else  if ( STequal( ptr, "mdy" ) )
	    IIapi_static->api_dfmt = DB_MDY_DFMT;
	else
	{
	    IIAPI_TRACE( IIAPI_TR_ERROR )
