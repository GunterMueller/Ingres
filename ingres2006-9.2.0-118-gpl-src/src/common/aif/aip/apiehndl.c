...
II_EXTERN II_VOID
IIapi_createDbevCB( IIAPI_CONNHNDL *connHndl, IIAPI_GCA_BUFF *gcaBuff )
{
    GCA_RV_PARMS	*rcvParm = (GCA_RV_PARMS *)gcaBuff->gca_parm;
    IIAPI_ENVHNDL	*envHndl = connHndl->ch_envHndl;
    GCD_EV_DATA		*gca_data;
    GCD_DATA_VALUE	*data, *dataEnd;
    IIAPI_DBEVCB	*dbevCB;
    STATUS		status;
    II_LONG		i;
    ...
    dataEnd = (GCD_DATA_VALUE *)(gcaBuff->data + gcaBuff->data_len);

    if ( connHndl->ch_flags & IIAPI_CH_MSG_CONTINUED )
    {
	data = (GCD_DATA_VALUE *)gcaBuff->data;
	dbevCB = (IIAPI_DBEVCB *)connHndl->ch_dbevCBList.q_next;
    }
    else
    {
	gca_data = (GCD_EV_DATA *)gcaBuff->data;

	if ( ! ( dbevCB = (IIAPI_DBEVCB *)
		 MEreqmem( 0, sizeof(IIAPI_DBEVCB), TRUE, &status ) ) )
	{
	    IIAPI_TRACE( IIAPI_TR_FATAL )
		("IIapi_createDbevCB: can't alloc database event control block\n");
	    goto dbevCBAllocFail;
	}
	...
	*/
	...
	if ( ! (dbevCB->ev_name = 
		MEreqmem(0, gca_data->gca_evname.gca_l_name+1, TRUE, &status)) )
	{
	    IIAPI_TRACE( IIAPI_TR_FATAL )
		( "IIapi_createDbevCB: can't allocate database event name\n" );
	    goto dbevNameAllocFail;
	}

	MEcopy( gca_data->gca_evname.gca_name, 
		gca_data->gca_evname.gca_l_name, dbevCB->ev_name );
	dbevCB->ev_name[ gca_data->gca_evname.gca_l_name ] = '\0';

	if (! (dbevCB->ev_owner = 
	       MEreqmem(0, gca_data->gca_evowner.gca_l_name+1, TRUE, &status)))
	{
	    IIAPI_TRACE( IIAPI_TR_FATAL )
		( "IIapi_createDbevCB: can't allocate database event owner\n" );
	    goto dbevOwnerAllocFail;
	}

	MEcopy( gca_data->gca_evowner.gca_name, 
		gca_data->gca_evowner.gca_l_name, dbevCB->ev_owner );
	dbevCB->ev_owner[ gca_data->gca_evowner.gca_l_name ] = '\0';

	if ( ! (dbevCB->ev_database = 
		MEreqmem(0, gca_data->gca_evdb.gca_l_name + 1, TRUE, &status)) )
	{
	    IIAPI_TRACE( IIAPI_TR_FATAL )
		("IIapi_createDbevCB: can't allocate database event DB name\n");
	    goto dbevDatabaseAllocFail;
	}

	MEcopy( gca_data->gca_evdb.gca_name, 
		gca_data->gca_evdb.gca_l_name, dbevCB->ev_database );
	dbevCB->ev_database[ gca_data->gca_evdb.gca_l_name ] = '\0';

	if ( gca_data->gca_evtime.gca_type != IIAPI_DTE_TYPE )
	{
	    dbevCB->ev_time.dv_null = TRUE;
	    dbevCB->ev_time.dv_length = 0;
	    dbevCB->ev_time.dv_value = NULL;
	}
	else  if ( ! ( dbevCB->ev_time.dv_value = 
		       MEreqmem( 0, (u_i2)gca_data->gca_evtime.gca_l_value, 
				 TRUE, &status ) ) )
	{
	    IIAPI_TRACE( IIAPI_TR_FATAL )
		( "IIapi_createDbevCB: can't allocate database event time\n" );
	    goto dbevTimeAllocFail;
	}
	else
	{
	    MEcopy( gca_data->gca_evtime.gca_value,
		    gca_data->gca_evtime.gca_l_value, 
		    dbevCB->ev_time.dv_value );
	    dbevCB->ev_time.dv_null = FALSE;
	    dbevCB->ev_time.dv_length = (u_i2)gca_data->gca_evtime.gca_l_value;
	}

	dbevCB->ev_count = 0;

	if ( ! gca_data->gca_l_evvalue )
	{
	...
	else
	{
	    if ( ! ( dbevCB->ev_descriptor = (IIAPI_DESCRIPTOR *)
		 MEreqmem( 0, 
			   sizeof(IIAPI_DESCRIPTOR) * gca_data->gca_l_evvalue,
			   TRUE, &status ) ) )
	    {
		IIAPI_TRACE( IIAPI_TR_FATAL )
		    ("IIapi_createDbevCB: can't allocate database event descriptor\n");
		goto dbevDescAllocFail;
	    }

	    if ( ! ( dbevCB->ev_data = (IIAPI_DATAVALUE *)
		 MEreqmem( 0, 
			   sizeof(IIAPI_DATAVALUE) * gca_data->gca_l_evvalue,
			   TRUE, &status ) ) )
	    {
		IIAPI_TRACE( IIAPI_TR_FATAL )
		    ("IIapi_createDbevCB: can't allocate database event data\n");
		goto dbevDataAllocFail;
	    }
	}

	data = &gca_data->gca_evvalue[ 0 ];

	IIAPI_TRACE( IIAPI_TR_VERBOSE )
	    ( "IIapi_createDbevCB: dbevCB %p created\n", dbevCB );
    }

    for( ; data < dataEnd; data++, dbevCB->ev_count++ )
    {
	IIapi_cnvtGDV2Descr( envHndl, (IIAPI_DT_ID)data->gca_type,
			     data->gca_l_value,
			     data->gca_precision,
			     0, NULL, 
			     &dbevCB->ev_descriptor[ dbevCB->ev_count ] );

	if ( ! (dbevCB->ev_data[ dbevCB->ev_count ].dv_value = 
		MEreqmem(0, 
			 dbevCB->ev_descriptor[ dbevCB->ev_count ].ds_length, 
			 FALSE, &status)) )
	{
	    IIAPI_TRACE( IIAPI_TR_FATAL )
		( "IIapi_createDbevCB: can't allocate database event value\n" );
	    goto dbevValueAllocFail;
	}

	IIapi_cnvtGDV2DataValue( &dbevCB->ev_descriptor[ dbevCB->ev_count ],
				 data->gca_value,
				 &dbevCB->ev_data[ dbevCB->ev_count ] );
    }
 ...
 dbevDataAllocFail:
    ...
    if ( dbevCB->ev_descriptor )
	MEfree( (II_PTR)dbevCB->ev_descriptor );

 dbevDescAllocFail:

    if ( dbevCB->ev_time.dv_value )
	MEfree( (II_PTR)dbevCB->ev_time.dv_value );

 dbevTimeAllocFail:

    MEfree( (II_PTR)dbevCB->ev_database );

 dbevDatabaseAllocFail:

    MEfree( (II_PTR)dbevCB->ev_owner );

 dbevOwnerAllocFail:

    MEfree( (II_PTR)dbevCB->ev_name );
