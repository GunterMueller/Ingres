...
static IIAPI_STATUS IIapi_buildPrmpt( IIAPI_GCA_BUFF *gcaBuff, 
					IIAPI_PROMPTPARM *prmptParm );
...
# define SQL_CA_HALT	((IIAPI_ACTION) 32)	/* Halt SM execution */
...
static SM_TRANSITION smt_list[] =
{
    ...
    */
    ...
    { IIAPI_EV_ERROR_RCVD,	SQL_CS_CONN,	SQL_CS_CONN,	SQL_CAS_15 },
    { IIAPI_EV_ERROR_RCVD,	SQL_CS_WCR,	SQL_CS_WCR,	SQL_CAS_15 },
    { IIAPI_EV_ERROR_RCVD,	SQL_CS_WMR,	SQL_CS_WMR,	SQL_CAS_15 },
    { IIAPI_EV_ERROR_RCVD,	SQL_CS_WXA,	SQL_CS_WXA,	SQL_CAS_15 },
...
)
{
	...
	case SQL_CA_RECV :
	    ...
	    */
	    status = IIapi_rcvNormalGCA( ev_hndl, NULL, (II_LONG)(-1) );
	    if ( status != IIAPI_ST_SUCCESS )  success = FALSE;
	    break;
	...
	case SQL_CA_RCVT :
	    ...
	    */
	    status = IIapi_rcvNormalGCA( sm_hndl, NULL, 
	...
	case SQL_CA_SMDA :
	    ...
	    */
	    if ( ! ( parmNmem = IIapi_createGCAMdassoc( connHndl ) ) )
	    {
	    ...
	    status = IIapi_sndGCA( ev_hndl, parmNmem, NULL );
	...
	case SQL_CA_RPMT :
	{
	    ...
	    */
	    ...
	    prmptParm = ( IIAPI_PROMPTPARM * )connHndl->ch_prmptParm; 
	    prmptParm->pd_envHandle = 
		    (envHndl == IIapi_defaultEnvHndl()) ? NULL : envHndl;
	    prmptParm->pd_connHandle = connHndl; 
	    status = IIapi_buildPrmpt( (IIAPI_GCA_BUFF *)parmBlock, prmptParm );
	    if( status != IIAPI_ST_SUCCESS )
	    {
		success = FALSE;
		break;
	...
	case SQL_CA_SPRY:
	{
	    ...
	    */
	    ...
	    if( ! (parmNmem = IIapi_createGCAPrreply( connHndl )) )
	    {
		status = IIAPI_ST_OUT_OF_MEMORY ;
		success = FALSE;
		break;
	    ...
	    MEfree( (PTR) prmptParm->pd_message );
	    MEfree( (PTR) prmptParm->pd_reply );
	...
	case SQL_CA_REVE :
	    ...
	    */
	    IIapi_createDbevCB( connHndl, (IIAPI_GCA_BUFF *)parmBlock );
	    break;
	...
	case SQL_CA_OTRM :
	{
	    ...
	    */
		...
		traceParm.tr_length = gcaBuff->data_len;

		traceParm.tr_message = MEreqmem( 0, traceParm.tr_length + 1,
						 TRUE, NULL );
		...
		{
		    MEcopy( gcaBuff->data, 
			    traceParm.tr_length, traceParm.tr_message );
	...
	case SQL_CA_SXAP :
	{
	    ...
	    */
	    ...
	    if ( ! (parmNmem = IIapi_createGCAxa( GCA_XA_PREPARE, 
	...
	case SQL_CA_SXAC :
	{
	    ...
	    */
	    ...
	    if ( ! (parmNmem = IIapi_createGCAxa( GCA_XA_COMMIT, 
	...
	case SQL_CA_SXAR :
	{
	    ...
	    */
	    ...
	    if ( ! (parmNmem = IIapi_createGCAxa( GCA_XA_ROLLBACK, 
	...
	case SQL_CA_RXAR :
	{
	    ...
	    */
	    ...
	    GCD_RE_DATA    *rspData = (GCD_RE_DATA *)gcaBuff->data;

	    /*
	    ** If no XA error is returned, generate generic XA error.
	    */
	    if ( ! (rspData->gca_rqstatus & GCA_XA_ERROR_MASK)  ||
	         ! rspData->gca_errd5 )  
		rspData->gca_errd5 = IIAPI_XAER_RMERR;

	    if ( ! IIapi_xaError( ev_hndl, rspData->gca_errd5 ) )
	    {
		status = IIAPI_ST_OUT_OF_MEMORY;
		success = FALSE;
	    }
	...
	case SQL_CA_RRSP :
	{
	    ...
	    */
	    if ( 
	         rspData->gca_rqstatus & GCA_XA_ERROR_MASK  &&
	         rspData->gca_errd5  && 
	         ! IIapi_xaError( ev_hndl, rspData->gca_errd5 ) 
	       )
	    {
		status = IIAPI_ST_OUT_OF_MEMORY;
		success = FALSE;
...
/*
** Name: IIapi_buildPrmpt
**
** Description:
**    This function builds prompt data from GCA_PROMPT message.
**
** Input:
**   gcaBuff		GCA buffer containing GCA_PROMPT message.
**   prmptParm		Prompt parameter Block.
**
** Output:
**   None.
**
...
static IIAPI_STATUS 
IIapi_buildPrmpt( IIAPI_GCA_BUFF *gcaBuff, 
		 IIAPI_PROMPTPARM *prmptParm )
{
    GCD_PROMPT_DATA     *prData = (GCD_PROMPT_DATA *)gcaBuff->data;
    STATUS		status;

    if( prData->gca_prflags & GCA_PR_NOECHO )
	prmptParm->pd_flags |= IIAPI_PR_NOECHO;

    if( prData->gca_prflags & GCA_PR_TIMEOUT )
	prmptParm->pd_flags |= IIAPI_PR_TIMEOUT;

    prmptParm->pd_timeout     = prData->gca_prtimeout;
    prmptParm->pd_max_reply   = prData->gca_prmaxlen;

    if( prData->gca_l_prmesg )
    {
	prmptParm->pd_msg_len = prData->gca_prmesg[0].gca_l_value;
	prmptParm->pd_message = (II_CHAR *)MEreqmem(0, 
				    	    prmptParm->pd_msg_len + 1,
				    	    TRUE, &status );
	if( status != OK )
	   return( IIAPI_ST_OUT_OF_MEMORY );
	MEcopy( prData->gca_prmesg[0].gca_value, prmptParm->pd_msg_len,
		prmptParm->pd_message );  
	prData->gca_prmesg[0].gca_value[ prmptParm->pd_msg_len ] = EOS;
	prmptParm->pd_reply = (II_CHAR *)MEreqmem(0, 
				    	    prData->gca_prmaxlen + 1,
				    	    TRUE, &status );
	if( status != OK )
	{
	   MEfree( (PTR) prmptParm->pd_message );
