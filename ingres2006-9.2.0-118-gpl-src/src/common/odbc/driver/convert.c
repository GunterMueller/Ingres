...
static SQLTYPEATTR INGRES_DATE_TYPE_TABLE[] = 
{
...
*/
    ...
    { SQL_NUMERIC,       SQL_C_NUMERIC,             0,
        ...
        8,     TRUE, 0 },
    { SQL_BIT,           SQL_C_BIT,                 0,
        IIAPI_BYTE_TYPE, 1,         1,      sizeof(UCHAR),             
        1,     TRUE, 0 },
    ...
    UWORD       cbData)
{
    ...
    */
    ...
    status = CVal8(szInt, &intValue); /* convert char szInt to int intValue */
    if (status != OK)
        return (CVT_OUT_OF_RANGE);
    ...
    if (c)    /* if fractional part, it better zeros */
       {
        intTemp = 0;
        status = CVal8(c, &intTemp); /* convert fractional part to int */
        if (intTemp != 0)           /* if fraction != 0 */
            rc = CVT_TRUNCATION;    /*    return (TRUNCATION) */
       }
    ...
    for (i = 0; i < TYPE_NUM; i++)
        if (fType == TYPE_TABLE[i].fCType)
        {
                ...
                TYPE_TABLE[i].fVerboseType == SQL_DATETIME)
            {
                ...
                TYPE_TABLE[i].fVerboseType = fType;
...
LPSQLTYPEATTR CvtGetSqlType(
    SWORD fType,  UDWORD fAPILevel)
{
    UWORD      i;

    for (i = 0; i < TYPE_NUM; i++)
        if (fType == TYPE_TABLE[i].fSqlType)
        {
            if (TYPE_TABLE[i].fVerboseType == SQL_INTERVAL ||
                TYPE_TABLE[i].fVerboseType == SQL_DATETIME)
            {
...
{
    u_i2 len = (u_i2)(srcDataType == IIAPI_DEC_TYPE && dstDataType == 
         IIAPI_CHA_TYPE ? pipd->Precision + 3 : pipd->Precision);
    ...
    fp.fd_srcDesc.ds_length     = (II_UINT2) pipd->Precision;
    ...
    fp.fd_srcValue.dv_length    = (II_UINT2) pipd->Precision;
    ...
    fp.fd_dstDesc.ds_precision  = (II_UINT2) len;
...
# else
    ...
    cv.cv_srcDesc.ds_dataType   = srcDataType; 
    cv.cv_srcDesc.ds_nullable   = FALSE;  
    cv.cv_srcDesc.ds_length     = (II_UINT2) pipd->Precision;
    cv.cv_srcDesc.ds_precision  = (II_UINT2) pipd->Precision;
    cv.cv_srcDesc.ds_scale      = pipd->Scale;
    cv.cv_srcDesc.ds_columnType = IIAPI_COL_TUPLE;
    cv.cv_srcDesc.ds_columnName = NULL;  
    cv.cv_srcValue.dv_null      = FALSE; 
    cv.cv_srcValue.dv_length    = (II_UINT2) pipd->Precision;
    cv.cv_srcValue.dv_value     = pSource; 
          
    cv.cv_dstDesc.ds_dataType   = dstDataType; 
    cv.cv_dstDesc.ds_nullable   = FALSE;
    cv.cv_dstDesc.ds_length     = (II_UINT2) len;
    cv.cv_dstDesc.ds_precision  = (II_UINT2) len;
    ...
    cv.cv_dstValue.dv_length    = (II_UINT2) len;
