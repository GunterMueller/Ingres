    ...
    SQLHSTMT  hstmt)
{
    ...
    /*
    **  If a previous SQLFreeStmt() call invoked a commit, re-prepare the
    **  statement.
    */

    if (!(pstmt->fStatus & STMT_PREPARED))
    {
        rc = SQLPrepare_InternalCall(hstmt, pstmt->szSqlStr, pstmt->cbSqlStr, 
            TRUE);
    }
    if (!SQL_SUCCEEDED(rc))
        return rc;
    ...
    */
    else
    {
        ...
        szAPISqlStr = MEreqmem((u_i2)0, cbSqlStr +
           CountLiteralItems(cbSqlStr, szSqlStr, pstmt->fAPILevel) + 8,
                              TRUE, NULL);
        if (szAPISqlStr == NULL)
        {
            UnlockStmt (pstmt);
                ...
                fSqlType == SQL_WLONGVARCHAR)
            {
                if (pstmt->fCommand != CMD_EXECPROC)  /* and not execute procedure */
                {
                    ...
                    if (fSqlType == SQL_LONGVARBINARY)
                       if (fCType == SQL_C_CHAR)       /* if CHAR->BINARY conversion */
                           OctetLength /= 2;           /*     adjust expected length */
                       else if (fCType == SQL_C_WCHAR) /* if WCHAR->BINARY conversion*/
                           OctetLength /= sizeof(SQLWCHAR); /*adjust expected length */
    ...
    SQLINTEGER      cbValue)
{
    ...
    */
	...
	case SQL_VARCHAR:
	case SQL_WVARCHAR:
        if ((papd->cbPartOffset) < (SQLUINTEGER)pipd->OctetLength)
            pstmt->moreData = TRUE;
        else
    ...
    LPDESCFLD pipd)
{
    ...
    */
    ...
    ptype = CvtGetSqlType (fSqlType, pstmt->fAPILevel);
    cb = ptype->cbLength;
    if (cb) return cb;
    ...
    BOOL    offsetData ) /* Whether to offset char data */
{
    ...
    UDWORD    fAPILevel = pstmt->fAPILevel;

    TRACE_INTL(pstmt,"PutParm");
    /*
    **  Validations that are new or moved with ODBC 2.01:
    **
    **  1. We don't do output parms.
    **  2. Check that we can convert convert C type to ODBC SQL type:
    */
 ...
 /* if a procedure call is to just use the default value of the parm, just return */
	...
	ptype = CvtGetCType(fCType, pstmt->fAPILevel);
    ...
    */
    ...
    case SQL_C_CHAR:  /* and fall-through from SQL_C_WCHAR */
        ...
        case SQL_DECIMAL:
        {
            ...
            if (!CvtApiTypes( rgbData, (PTR)szTemp, 
    ...
    case SQL_C_NUMERIC:
    {
        ...
        CHAR decimal_char = pstmt->pdbcOwner->fOptions & 
            OPT_DECIMALPT ? ',' : '.';
        ...
        case SQL_INTEGER:
        {
            ...
            */
            if ((wb = STindex(workBuff, pstmt->pdbcOwner->fOptions & 
                OPT_DECIMALPT ? "," : "." , 0)) != NULL) 
            { 
                char *t;
		...
		case SQL_BIGINT:
        {
            ...
            */
            ...
            if ((rc = CVal8(workBuff, &int64Value)) == OK)
                rc = PutInt64 (rgbData, int64Value, fSqlType, FALSE);
            break;
        }
...
/*          _gcvt (floatValue, 17, szValue);  */
    ...
    case SQL_C_TYPE_DATE:
	    ...
	    ** blank out the trailing part of pipd->DataPtr.
        ...
        case SQL_TYPE_DATE:
            rc = CvtDateChar ((DATE_STRUCT     *)rgbValue, rgbData, cbData, pstmt->fAPILevel);
            if (rc != CVT_SUCCESS)   break;
            ...
            if (rc == CVT_SUCCESS)
               {
                if (pstmt->fAPILevel < IIAPI_LEVEL_4)
                   ...
                   /* convert ODBC date format (yyyy-mm-dd hh:mm:ss)
                           to Ingres US format (yyyy_mm_dd hh:mm:ss) */ 
                /* if not INGRES */
                if (!isServerClassINGRES(pstmt->pdbcOwner) || pstmt->fAPILevel > IIAPI_LEVEL_3)
                      rc = CvtCharToIngresDateTime(rgbData, pipd, pstmt->fAPILevel);
                                         /* convert to ts struct */
               }
            break;
    ...
    case SQL_C_TYPE_TIME:
        ...
        rc = CvtEditTime ((ISO_TIME_STRUCT     *)rgbValue, pstmt->fAPILevel);
        if (rc != CVT_SUCCESS) break;

            rc = CvtTimeChar ((ISO_TIME_STRUCT *)rgbValue, rgbData, cbData, pstmt->fAPILevel);
    ...
    case SQL_C_TYPE_TIMESTAMP:
        ...
        case SQL_LONGVARCHAR:
            ...
            rc = CvtEditTimestamp ((TIMESTAMP_STRUCT     *)rgbValue,pstmt->fAPILevel);
            if (rc != CVT_SUCCESS) break;
            rc = CvtTimestampChar (
                    (TIMESTAMP_STRUCT     *)rgbValue,
                    rgbData, cbData, (UWORD)pipd->Precision, pstmt->fAPILevel);
        ...
        case SQL_TYPE_DATE:
            ...
            rc = CvtDateChar (&dateValue, rgbData, cbData, pstmt->fAPILevel);
            if (rc != CVT_SUCCESS)   break;
                if (pstmt->fAPILevel < IIAPI_LEVEL_4)
                    ODBCDateToIngresUS(rgbData);
                  /* convert ODBC date format (yyyy-mm-dd hh:mm:ss)
        ...
        case SQL_TYPE_TIME:
            ...
            rc = CvtTimeChar (&timeValue, rgbData, cbData, pstmt->fAPILevel);
            break;
