...
#define QST_DESCRIBE     4      /* Describe a statement */   
#define QST_FETCH        5      /* Fetch rows */
#define QST_PROC         6      /* Call a DB procedure */
#define QST_PUTSEG       7      /* Put a segment */
#define QST_GETSEG       8      /* Get a segment */
#define QST_DESCRIBE_INPUT 9      /* Describe parameters */
	...
	if (p)
		{
		    ...
		    CMcpychar("\0",p1);    /* null term the schemaname) */
...
II_INT2 *descrCount)
{
    ...
    */
    ...
    if (!dsCount)
        return (TRUE);
    /*
    ** Move descriptor info into the Implementation Row Descriptor
    */
...
# endif
        if (ds->ds_columnName)
            STlcopy (ds->ds_columnName, pird->Name, sizeof(pird->Name)-1);
        else
           {
            STcopy ("col", pird->Name);
            STprintf (pird->Name+3, "%d", (i4)(i+1));
           }
        pird->Nullable    = ds->ds_nullable?SQL_NULLABLE:SQL_NO_NULLS;
        pird->fIngApiType = ds->ds_dataType;
        pird->Length      =       /* data length plus prefix length in bytes */
        pird->OctetLength =     ds_length;
        pird->Precision   = ds->ds_precision;
        pird->Scale       = ds->ds_scale;
...
**
** Description:
**       get parmeter info from descriptor 
**
** On entry: 
**
** On exit:
...
**      
*/
static BOOL odbc_getInput( LPSTMT pstmt)
{
    LPDESC    pIRD = pstmt->pIRD;   /* Implementation Row Descriptor */
    LPDESC    pIPD = pstmt->pIPD;   /* Implementation Parameter Descriptor */
    LPDESCFLD pird;                 /* IRD row field */
    LPDESCFLD pipd;                 /* IPD row field */
    int i;

    TRACE_INTL (pstmt, "odbc_getInput");

    pIPD->Count = pIRD->Count;

    ResetDescriptorFields(pIPD, 0, pIPD->Count);  /* clear all desc fields */
    /*
    **  Ensure that IPD row descriptor is large enough.
    **  If too few then re-allocate.
    */
    if (pstmt->pIPD->CountAllocated < pIPD->Count)
    {
        if (ReallocDescriptor(pIRD, pIPD->Count) == NULL)
        { 
            ErrState (SQL_HY001, pstmt, F_OD0001_IDS_BIND_ARRAY);
                return FALSE;
        }
    ...
    pird = pIRD->pcol + 1;
    pipd = pIPD->pcol + 1;
    for (i = 0; i < pIPD->Count; pird++, pipd++, i++)
    {
        switch (pird->fIngApiType)
        {
            case IIAPI_CHR_TYPE:
            case IIAPI_CHA_TYPE:
                pipd->ConciseType = SQL_CHAR;
                break;

            case IIAPI_VCH_TYPE:
                pipd->ConciseType = SQL_VARCHAR;
                break;

            case IIAPI_LVCH_TYPE:
                pipd->ConciseType = SQL_LONGVARCHAR;
                break;

            case IIAPI_BYTE_TYPE:
                pipd->ConciseType = SQL_BINARY;
                break;

            case IIAPI_VBYTE_TYPE:
                pipd->ConciseType = SQL_VARBINARY;
                break;

            case IIAPI_LBYTE_TYPE:
                pipd->ConciseType = SQL_LONGVARBINARY;
                break;

            case IIAPI_NCHA_TYPE:
                pipd->ConciseType = SQL_WCHAR;
                break;

            case IIAPI_NVCH_TYPE:
                pipd->ConciseType = SQL_WVARCHAR;
                break;

            case IIAPI_LNVCH_TYPE:
                pipd->ConciseType = SQL_WLONGVARCHAR;
                break;
            ...
            case IIAPI_INT_TYPE:
                pipd->ConciseType = SQL_INTEGER;
                break;

            case IIAPI_FLT_TYPE:
                pipd->ConciseType = SQL_FLOAT;
                break;

            case IIAPI_MNY_TYPE:
                pipd->ConciseType = SQL_DECIMAL;
                break;
            ...
            case IIAPI_TMWO_TYPE:
            case IIAPI_TMTZ_TYPE:
                pipd->ConciseType = SQL_TYPE_TIME;
                break;
 
            case IIAPI_INTYM_TYPE:
                pipd->ConciseType = SQL_INTERVAL_YEAR_TO_MONTH;
                break;

            case IIAPI_INTDS_TYPE:
                pipd->ConciseType = SQL_INTERVAL_DAY_TO_SECOND;
                break;

            case IIAPI_DATE_TYPE:
                pipd->ConciseType = SQL_TYPE_DATE;
                break;
            ...
            /*
            ** Should never get here, but punt just in case.
            */
	    default:
	        pipd->ConciseType = SQL_CHAR;
	        break;
        }
        pipd->OctetLength = pird->OctetLength;
        pipd->Scale = pird->Scale;
        pipd->Nullable = pird->Nullable;
    }
    return(TRUE);
}
...
static BOOL odbc_setDescr( LPSTMT pstmt, IIAPI_QUERYTYPE apiQueryType )
{
    ...
    if (PrepareParms(pstmt) != SQL_SUCCESS)
        return FALSE;
...
/*
*/
static BOOL odbc_ExecProcSetDescr( LPSTMT pstmt, LPDESC pprocIPD)
{
	IIAPI_SETDESCRPARM setDescrParm = {0};
	II_INT2            i;
	BOOL            rc = TRUE;
	...
	IIAPI_DESCRIPTOR  *psd;
        ...
        TRACE_INTL(pstmt,"odbc_ExecProcSetDescr");
	setDescrParm.sd_genParm.gp_callback = NULL;
	setDescrParm.sd_genParm.gp_closure  = NULL;
	setDescrParm.sd_stmtHandle          = pstmt->stmtHandle;
	setDescrParm.sd_descriptorCount     = pprocIPD->Count;

	psd = setDescrParm.sd_descriptor = (IIAPI_DESCRIPTOR*)
	    odbc_malloc( setDescrParm.sd_descriptorCount * sizeof(IIAPI_DESCRIPTOR));

	pprocipd=pprocIPD->pcol+1;          /* pprocipd->1st descriptor field after bookmark */

	for (i=0;  i < pprocIPD->Count; i++, pprocipd++, psd++)
	   {
	    SQLSMALLINT  ParameterType = pprocipd->ParameterType;
	    ...
	    else if (ParameterType == SQL_PARAM_TYPE_PROCNAME)
	              psd->ds_columnType = IIAPI_COL_SVCPARM;
	    else if (ParameterType == SQL_PARAM_INPUT_OUTPUT)
	              psd->ds_columnType = IIAPI_COL_PROCBYREFPARM;
	    else if (ParameterType == SQL_PARAM_TYPE_PROCGTTPARM)
	    ...
	    psd->ds_dataType   = pprocipd->fIngApiType;
	    psd->ds_nullable   =(pprocipd->Nullable==SQL_NULLABLE)?TRUE:FALSE;
	    psd->ds_length     = (II_UINT2)pprocipd->OctetLength;

	    if (pprocipd->fIngApiType == IIAPI_DEC_TYPE  ||
	        pprocipd->fIngApiType == IIAPI_MNY_TYPE)
	          psd->ds_precision  = (II_INT2)pprocipd->Precision;
	    else  psd->ds_precision  =          0;

	    if (pprocipd->fIngApiType == IIAPI_DEC_TYPE)
	          psd->ds_scale      =          pprocipd->Scale;
	    else  psd->ds_scale      =          0;

	    psd->ds_columnName = (*pprocipd->Name)? pprocipd->Name : NULL;
	   }  /* end for loop through parm descriptors */
...
/*
*/
static BOOL odbc_ExecProcPutParms(LPSTMT pstmt, LPDESC pprocIPD)
{
	...
	u_i2               savelen;
        ...
        TRACE_INTL(pstmt,"odbc_ExecProcPutParms");

	putParmParm.pp_genParm.gp_callback = NULL;
	putParmParm.pp_genParm.gp_closure  = NULL;
	putParmParm.pp_stmtHandle          = pstmt->stmtHandle;
	putParmParm.pp_parmCount           = pprocIPD->Count;

	ppd= putParmParm.pp_parmData = (IIAPI_DATAVALUE*)
	       odbc_malloc( putParmParm.pp_parmCount * sizeof(IIAPI_DATAVALUE) );

	pprocipd=pprocIPD->pcol+1;          /* pprocipd->1st descriptor field after bookmark */

	for (i=0;  i < pprocIPD->Count; i++, pprocipd++, ppd++)
	   {
	    SQLSMALLINT  ParameterType = pprocipd->ParameterType;
	    ...
	    if (pprocipd->Nullable == SQL_NULLABLE)
	       {
	        rgbNull = (i4*)(pprocipd->DataPtr + pprocipd->cbNullOffset);
	        if (*rgbNull == SQL_NULL_DATA /* -1 */)
	             ppd->dv_null = TRUE;   
	        else ppd->dv_null = FALSE;  
	       }

	    rgbData     =           pprocipd->DataPtr + pprocipd->cbDataOffset;
	    rgbDataui2  =    (u_i2*)rgbData;   /* rgbDataui2 -> 2 byte length */
	    OctetLength = (II_UINT2)pprocipd->OctetLength;

	    if (pprocipd->fIngApiType == IIAPI_VCH_TYPE   ||
# ifdef IIAPI_NVCH_TYPE
			pprocipd->fIngApiType == IIAPI_VBYTE_TYPE ||
	        pprocipd->fIngApiType == IIAPI_NVCH_TYPE)
# else
	        pprocipd->fIngApiType == IIAPI_VBYTE_TYPE)
# endif
	       {
	        OctetLength = *rgbDataui2 + 2;  /* data length + 2 byte length prefix*/
# ifdef IIAPI_NVCH_TYPE
	        if (pprocipd->fIngApiType == IIAPI_NVCH_TYPE)  /* NVCHAR prefix has */
	            *rgbDataui2 = *rgbDataui2 / sizeof(SQLWCHAR);  /* char length */
# endif
	       }
	    else
	    if (pprocipd->fIngApiType == IIAPI_LVCH_TYPE  ||
# ifdef IIAPI_LNVCH_TYPE
	        pprocipd->fIngApiType == IIAPI_LBYTE_TYPE ||
	        pprocipd->fIngApiType == IIAPI_LNVCH_TYPE)
# else
	        pprocipd->fIngApiType == IIAPI_LBYTE_TYPE)
# endif
	       {  /* Ingres does does not do blobs as proc parms but keep starting code for future */
	        OctetLength = (*(u_i4*)rgbData) + 2;  /* data length + 2 byte segment length prefix*/
	        rgbData    += 2;                      /* rgbData -> 2 byte segment length */
	        if (OctetLength > MAGIC_1980)
	            OctetLength = MAGIC_1980;
	        savelen = *rgbDataui2;                /* save the original length */
	        *rgbDataui2 = (u_i2)(OctetLength - 2);        /* set the segment length */
	       }

	    ppd->dv_value  =           rgbData;
	    ppd->dv_length = (II_UINT2)OctetLength;
...
# else
	    pprocipd->fIngApiType == IIAPI_LBYTE_TYPE)
# endif
	    *rgbDataui2 = savelen;               /* restore the original length */
    ...
    */
    ...
    for(;;)
    {
        ...
        */
        if (!plb)
            MoveData(pstmt, pard, pird);  
        else   /* else move data to long buffer (plb->pLongData)*/
        {
            ...
            */
            if (plb && (plb->pLongData)) 
            {
                plb->cbData =
                    ...
                    GetAllSegments(pstmt, pard, pird, plb);
                    /* 
...
static BOOL odbc_prepare(SESS* pSession, LPSTMT pstmt)
{
    ...
    STprintf (szPrepareStr,"Prepare s%05.5d from ", pstmt->iPreparedStmtID);
...
static BOOL odbc_describe(SESS* pSession, LPSTMT pstmt)
{
    ...
    odbc_getDescr(pstmt, TRUE, 0);  /* get the API descriptor and description*/
...
)
{
	...
	CHAR    *szRole;
	CHAR    szSupportIIDECIMAL[LEN_IIDECIMAL+1];
	CHAR    *szDecimal = ".\0";
    ...
    */
    ...
    case SQLCONNECT:
        ...
        pDSN = pBuf;                        /* -> DSN name */
        *szSupportIIDECIMAL = 'N';          /* default to SupportIIDECIMAL=N*/
        ...
        if (pDSN)  /* if DSN=, not DRIVER= */
           {
			 ...
			 if (!(pdbc->fOptionsSet & OPT_ALLOW_DBPROC_UPDATE)) /* if not already set */
                ...
                if (!(pdbc->fOptionsSet & OPT_CONVERTINT8TOINT4)) /* if not already set */
                {SQLGetPrivateProfileString (pDSN,"ConvertInt8ToInt4","",szWithOption, 
                                            6,ODBC_INI);
                if (*szWithOption=='Y' ||  *szWithOption=='y') /* if option is set to yes */
                {pdbc->fOptions |= OPT_CONVERTINT8TOINT4; /* set the option */
                }
				}
		...
		rc=SetConnectParam(pSqb, IIAPI_CP_CENTURY_BOUNDARY,
		                  &templong);
 		if (*szSupportIIDECIMAL =='Y') /* if supporting II_DECIMAL */
		{
		    szDecimal = ProcessII_DECIMAL();
                    if (!STbcompare(szDecimal,0,",\0",0,TRUE))
...
# ifdef IIAPI_VERSION_2
                {
                    ...
                    se.se_paramValue = szDecimal;
                    IIapi_setEnvParam(&se);
   		    rc = SetConnectParam(pSqb, IIAPI_CP_DECIMAL_CHAR, 
                        szDecimal );
                }
# else 
		rc = SetConnectParam(pSqb, IIAPI_CP_DECIMAL_CHAR, szDecimal );
  ...
  */ 
	...
	qry_cb->apiQueryType = apiQueryType;
	    ...
	    if (pstmt->fCommand == CMD_SELECT)
	        ...
	        qry_cb->apiQueryType = apiQueryType;
                qry_cb->sequence = QRY_QUERY;
                qry_cb->sequence_type = QST_DESCRIBE;
                pSqb->pSqlca->SQLCODE = odbc_query_sm(qry_cb);
            }
...
RETCODE odbc_query_sm( QRY_CB *qry_cb )
{
    ...
    case QRY_QUERY :	
        ...
        if (sequence_type == QST_DESCRIBE)
        { 
            if (!odbc_describe(pSession, pstmt))
            {
               rc = SQL_ERROR;
               sequence = QRY_CLOSE;
               break;
    ...
    case QRY_PUTP :
	    ...
	    odbc_putParms(pstmt,apiQueryType, serviceParms);
	...
	if (!odbc_getDescr(pstmt, TRUE, &dsCount))
        {
            rc = SQL_ERROR;
            sequence = QRY_CLOSE;
            break;
