...
int  INSTAPI SQLGetPrivateProfileString( LPCSTR, LPCSTR, LPCSTR, 
    LPSTR, int, LPCSTR);
...
#define isOld6xRelease(pdbc)                  ((pdbc)->fRelease < fReleaseRELEASE10)
    ...
    SQLUINTEGER     cQueryTimeout;          /* Phony statement option support. */
    SQLUINTEGER     cRowsetSize;            /* Phoney statement option support. */
    SQLUINTEGER     cBindType;              /* Phoney statement option support. */
    SQLUINTEGER     cKeysetSize;            /* Phoney statement option support. */
...
#define MAX_PSID    256 /* max. number of Prepared Stmt IDs in alloc bitmap */
    ...
    CHAR        szCatTable[40];         /* Catalog table name override. */
    CHAR        szDriver[LEN_DRIVER_NAME + 1]; /* Driver name. */
/*  CHAR        szTransDLL[129];           Translation DLL name. */
    CHAR        szCacheOwner[37];       /* SQLTables cached szTableOwner parm. */
    CHAR        szCacheTable[37];       /* SQLTables cached table parm. */
    ...
    CHAR        szUID[33];              /* Connect user id. */
    CHAR        szPWD[33];              /* Connect password. */
    CHAR        szDBMS_PWD[33];         /* Connect DBMS password. */
    ...
    UWORD       max_schema_name_length; /* SQL_MAX_SCHEMA_NAME_LEN (aka SQL_MAX_OWNER_NAME_LEN) */
...
#define OPT_not_used_0008   0x0008      /* not used */
...
#define OPT_DBCS            0x0080      /* DBCS support enabled */
...
#define OPT_NOCAID               0x8000 /* Don't honor CAID options (until registered). */
...
typedef struct tSTMT
{
    ...
    SQLUINTEGER     cQueryTimeout;          /* Phoney statement option support. */
    SQLUINTEGER     cRowsetSize;            /* Phoney statement option support. */
    SQLUINTEGER     cKeysetSize;            /* Phoney statement option support. */
    UWORD       cRetrieveData;          /* Phoney statement option support. */
    ...
    CHAR        szProcName  [OBJECT_NAME_SIZE+1];  /* call proc's name   */
    CHAR        szProcSchema[OBJECT_NAME_SIZE+1];  /* call proc's schema */
    CHAR        reserved[SQLCA_FUDGE];  /* Allow for extra bytes from MF. */
...
void    GetSQLMaxLen(LPSTMT, SWORD fSqlType, UCHAR     * p);
...
RETCODE ParseCommand (LPSTMT, CHAR     * szSqlStr, SDWORD cbSqlStr);
