    ...
    LPSTMT    *ppstmt)
{
    ...
    */
    ...
    pstmt->cRowsetSize   = pdbc->cRowsetSize;
    pstmt->cKeysetSize   = pdbc->cKeysetSize;
    ...
    SQLINTEGER    *pcbValue)    /* StrLen_or_IndPtr */
{
    ...
    */
    ...
    case SQL_WLONGVARCHAR:
        ...
        /* 
        ** Thin gateways do not support long data types. The gateway hangs on 
        ** getqueryinfo. So we have to put a check here as a workaround. 
        */
        if (!isServerClassINGRES(pdbc))
        {
            ErrState (SQL_HY000, pstmt, F_OD0075_IDS_ERR_TYPE_NONE);
            UnlockStmt (pstmt);
            return SQL_ERROR;
        }
    ...
    case SQL_NUMERIC:
            ...
            /* if not Ingres or is old Ingres then use float instead */
        if (!isServerClassINGRES(pdbc)  ||  isOld6xRelease(pdbc))
    ...
    SQLINTEGER* pcbValue)
{
    ...
    szAPISqlStr = MEreqmem(0, cbSqlStr + 300, TRUE, NULL);
    ...
    rc = ParseCommand (pstmt, szAPISqlStr, cbAPISqlStr);
    ...
    BOOL          rePrepared)
{
    ...
    */
    ...
    szAPISqlStr = MEreqmem(0, cbSqlStr + 
         CountLiteralItems(cbSqlStr, (CHAR*)szSqlStr, pstmt->fAPILevel) + 8,
                           TRUE, NULL);
    if (szAPISqlStr == NULL)
    {
        UnlockStmt (pstmt);
    ...
    /* rc = ParseCommand (pstmt, szSqlStr, cbSqlStr); */
    ...
    LPSTMT  pstmt)
{
    ...
    for (i = 0;
         (UWORD)i < pstmt->icolParmLast;  /* i < count of parameter markers */
         i++, papd++, pipd++)
    {
    ...
    SDWORD      * cbAPISqlStr)
{
    ...
    while (*pFrom != EOS  && length)
    {
        ...
        if (CMcmpcase(pFrom,"{") && CMcmpcase(pFrom,"}"))
        {
            CMcpychar(pFrom,pTo); 
			CMnext(pTo);  
        }
    ...
    SDWORD      cbSqlStr)
{
    ...
    **  to space (' ') characters:
    */
    pstmt->szSqlStr = MEreqmem(0, cb + 1001, TRUE, NULL); /* add for escape substitutions*/
    ...
    */
    if (!(pstmt->fOptions & OPT_NOSCAN))
    {
        ...
        while (p)
        {
            ...
            if (!ParseConvert (pstmt,p,&pNext,isaParameterFALSE, pstmt->fAPILevel))
            {
                /*
...
*/
static void ConvertDateTs (LPDBC pdbc,
...
#define sqlToScalarTblMax sizeof(sqlToScalarTbl) / sizeof(sqlToScalarTbl[0])
    ...
    */
    ...
    if (*p=='\'')
        ...
        ** from a blank date. We should treat {d'9999-12-31'}
        ** or {ts'9999-12-31'} value as a special case value
        ** and blank out this value if it's used as a literal 
        ** of date(d) or timestamp(ts) type when used
        ** in a SQL statement
        */
        ...
        {
            for (len = 0, p_temp = p; *p_temp != '\'' && *p_temp != '}' && *p_temp != '\0' && len < DATE_MAX_LEN; len++, p_temp++);
            MEfill(len, ' ', p );
            break;
        }
    
        /* If the literal is in a parameter then always convert to yyyy_mm_dd format.
        ** Else if literal is in a command string then
        **    if server is Ingres or Ingres-based server (VSAM, IMS, RMS)
        **       then convert to yyyy_mm_dd format.
        **    else leave in yyyy-mm-dd format for now.
        */
    if (pstmt->fAPILevel < IIAPI_LEVEL_4 && (isaParameter || isServerClassAnIngresEngine(pdbc))  &&
        p[4]=='-'  &&  p[7]=='-') /* if yyyy-mm-dd format (should be) convert*/
                                  /* to yyyy_mm_dd format */
       {p[4]= '_';
        p[7]= '_';
       }
    ...
    if (token==TOK_ESC_DATE)
        /* if a parameter     then date is in yyyy_mm_dd format.
        ** else
        ** if a Ingres server then date is in yyyy_mm_dd format.
        ** if a DB2    server then date is in yyyy-mm-dd format so that DB2 native date
        **    fields that are returned by other products as char(10) will match 'yyyy-mm-dd'.
        */
       break;
        ...
        else if (isServerClassOPINGDT(pdbc)  ||  isServerClassORACLE(pdbc)) 
        {
            ...
            */
                ...
                STbcompare(p,6,"WHERE(",6,TRUE)==0)     /* "WHERE" clause present */
            {
                ...
                p1 = pEV+count;
                CMcpychar("\0",p1);        /* make sure "everything" string terminated*/
                CMnext(p);
                ...
                CMcpychar("\0",p);   /* null terminated the target string     */
        ...
        {          /* if "--(*", peek ahead to see if escape sequence */
            ...
            {            /* p1 ->                     "),PRODUCT(ODBC)" */
                ...
                {          /* p1 ->                                 ")" */
                    ...
                    CMcpychar("\0",p1);    /* null term the escape sequence */
...
DWORD CountLiteralItems(SQLINTEGER cbSqlStr, CHAR * p, UDWORD fAPILevel)
{
    ...
    while(cbSqlStr-- > 0  && CMcmpcase(p,"\0"))
    {
        if(!CMcmpcase(p,"?")) 
            count +=3;
