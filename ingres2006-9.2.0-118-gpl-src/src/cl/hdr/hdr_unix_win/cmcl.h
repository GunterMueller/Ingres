...
*/
# define        CM_UTF8LEAD(s)    ((*(s)&0xFF) & 0xC0)

/*
** CM_UTF8FOLLOW - Checks whether a following byte of a UTF8 character.
*/
# define        CM_UTF8FOLLOW(s)  (((*(s)&0xFF) & 0x80) && !(((*(s)&0xFF) & 0xC0)))
...
# define CMGETUTF8	((char)CMgetUTF8())
...
GLOBALDLLREF	char	*CM_CaseTab;
GLOBALDLLREF	char	CM_isUTF8;
...
# define CMprint(str)	(CMdbl1st(str) ? cmdbl2nd((str)+1) : \
...
# define CMwhite(str) ((CMGETUTF8 && CM_UTF8MULTI(str)) ? ((CMGETATTR(str)&CM_A_SPACE) ? TRUE : FALSE) : (!(CMGETATTR(str)&CM_A_SPACE) ? FALSE :\
	(CMGETATTR(str)&(CM_A_SPACE|CM_A_DBL1|CM_A_DBL2))==CM_A_SPACE ? TRUE :\
	cmdblspace(str)))
...
# define CMspace(str)	(*(str) == ' ' || cmdblspace(str))
...
*/
# ifdef	DOUBLEBYTE
# define CMtolower(src, dst) ((CMGETUTF8 && CM_UTF8MULTI(src)) ? (cmu_getutf8_tolower((u_char *)src, CMUTF8cnt(src), (u_char *)dst, 0)) : (CMdbl1st(src) ? (*(dst) = *(src), *((dst)+1) = *((src)+1)) : (*(dst) = cmtolower(src))))
# else
...
/*
# ifdef	DOUBLEBYTE
# define CMtolower(src, dst)	(CMdbl1st(src) ? (*(dst) = *(src), *((dst)+1) = *((src)+1)) : (*(dst) = cmtolower(src)))
# else
# define CMtolower(src, dst)	(*(dst) = cmtolower(src))
# endif
*/
...
# ifdef	DOUBLEBYTE
# define CMtoupper(src, dst) ((CMGETUTF8 && CM_UTF8MULTI(src)) ? (cmu_getutf8_toupper((u_char *)src, CMUTF8cnt(src), (u_char *)dst, 0)) : (CMdbl1st(src) ? (*(dst) = *(src), *((dst)+1) = *((src)+1)) : (*(dst) = cmtoupper(src))))
# else
...
# ifdef	DOUBLEBYTE
# define CMbyteinc(count, str)	((CMGETUTF8) ? (count += CMUTF8cnt(str)) : (CMdbl1st(str) ? (count) += 2 : ++(count)))
# else
...
# ifdef	DOUBLEBYTE
# define CMbytedec(count, str)	((CMGETUTF8) ? (count -= CMUTF8cnt(str)) : (CMdbl1st(str) ? (count) -= 2 : --(count)))
# else
...
# ifdef DOUBLEBYTE
# define CMbytecnt(ptr)	((CMGETUTF8) ? (CMUTF8cnt(ptr)) : (CMdbl1st(ptr) ? 2 : 1))
# else
...
# ifdef	DOUBLEBYTE
# define CMnext(str)	((CMGETUTF8) ? ((str) += (CMUTF8cnt(str))) : ((CMdbl1st(str)) ? ((str) += 2) : (++(str))) )
# else
...
# ifdef	DOUBLEBYTE
# define CMprev(str, startpos)	(CMGETUTF8) ? (CMuprev(str, startpos)) : ((cmkcheck((u_char *)((str)-1), (u_char *)startpos) == CM_A_DBL2) ? (str) -= 2 : --(str))
# else
...
# ifdef	DOUBLEBYTE
# define CMcmpnocase(str1, str2) (CMdbl1st(str1) ? CMcmpcase(str1, str2) : \
...
# ifdef	DOUBLEBYTE
# define CMcmpcase(str1, str2)	(CMdbl1st(str1) ? (((i4)(CM_DEREF(str1)) == (i4)(CM_DEREF(str2))) ? \
...
#  define   CMcopy(source, copy_len, dest)				    \
	(cmicopy((source), (copy_len), (dest)))		/* Uses for loop */
#else
...
*/
#if defined(NT_GENERIC) 
#define CMwcstombs(wcstr, mbstr, mbsize) (WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK, wcstr, -1, mbstr, mbsize, NULL, NULL)>0)
#else /*NT_GENERIC*/
#define CMwcstombs(wcstr, mbstr, mbsize) (wcstombs(mbstr, (const wchar_t *)wcstr, mbsize)>1)
#endif 
...
**      wcstr      pointer to wide character string
**      mbstr     pointer to multibyte character string
**      mbsize  number of multibyte characters to convert
**
** Output:
**      bool    
**
...
*/
#if defined(NT_GENERIC) 
#define CMmbstowcs(mbstr, wcstr, mbsize) (MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, mbstr, mbsize, wcstr, mbsize)>0)
#else /*NT_GENERIC*/
#define CMmbstowcs(mbstr, wcstr, mbsize) (mbstowcs( wcstr, mbstr, mbsize)>1)
#endif 
