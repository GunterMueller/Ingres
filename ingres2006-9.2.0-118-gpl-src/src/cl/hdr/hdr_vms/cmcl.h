...
# define CMGETATTR(s)	CMGETATTRTAB[CM_DEREF(s)]
...
/*	define local macros	*/
# define cmdbl2nd(s)	(CMGETATTR(s) & CM_A_DBL2)
...
# ifdef	DOUBLEBYTE
# define CMdbl1st(str)	(CMGETATTR(str) & CM_A_DBL1)
# else
/* Allow optimizer to cut out redundant DOUBLEBYTE code */
...
# define CMwhite(str) (!(CMGETATTR(str)&CM_A_SPACE) ? FALSE :\
...
# ifdef	DOUBLEBYTE
# define CMcpychar(src, dst)	(CMdbl1st(src) ? (*(dst) = *(src), *((dst)+1) = *((src)+1)) : (*(dst) = *(src)))
# else
...
# ifdef	DOUBLEBYTE
# define CMcpyinc(src, dst)	(CMdbl1st(src) ? (*(dst)++ = *(src)++, *(dst)++ = *(src)++) : (*(dst)++ = *(src)++))
# else
...
# ifdef	DOUBLEBYTE
# define CMtoupper(src, dst)	(CMdbl1st(src) ? (*(dst) = *(src), *((dst)+1) = *((src)+1)) : (*(dst) = cmtoupper(src)))
# else
...
# ifdef	DOUBLEBYTE
# define CMbyteinc(count, str)	(CMdbl1st(str) ? (count) += 2 : ++(count))
# else
...
# ifdef	DOUBLEBYTE
# define CMbytedec(count, str)	(CMdbl1st(str) ? (count) -= 2 : --(count))
# else
...
# ifdef	DOUBLEBYTE
# define CMprev(str, startpos)	((cmkcheck((str)-1, startpos) == CM_A_DBL2) ? (str) -= 2 : --(str))
# else
