/*
**	Copyright (c) 1982, 2003 Ingres Corporation
**	All rights reserved.
*/
...
static II_VMS_EF_NUMBER		meflag;
static int			pid;
static II_VMS_EF_NUMBER		eflag;
...
{	
	...
	*/
	...
	{ /* open mail boxes and start sub-process */
		...
		{ /* initialize descriptors */
			...
			if (!((rval = lib$get_ef(&eflag)) & 01))
				goto errreturn;
			if (!((rval = lib$get_ef(&meflag)) & 01))
				goto errreturn;
		...
		if ((rval = mailbox(&status, &statchan)))
			goto errreturn;
		if (!((rval = sys$setimr(eflag, deltim, PCitimer, timerid, 0))
				& 01))
			goto errreturn;
		/*
		** BUG 3695
	...
	*/
	...
	if (!((rval = sys$cantim(timerid, 0)) & 01))
		goto errreturn;
	if (!((rval = sys$setimr(eflag, deltim, PCitimer, timerid, 0)) & 01))
		goto errreturn;
...
PCitimer()
{
	...
	if (incom)
	{
		sys$setimr(eflag, deltim, PCitimer, timerid, 0);
	}
	else
	{
		PCcleanup();
...
int	*size)
{
	...
	IOSB	iosb;
        ...
        if (!((rval = sys$qiow(meflag, chan, IO$_READVBLK, &iosb, 0, 0, buf, *size, 0, 0, 0, 0)) & 01))
		return rval;
...
int     size)
{
	...
	if ( size > 0 )
	{
	        if (!((rval = sys$qiow(meflag, chan, IO$_WRITEVBLK|IO$M_NOW, &iosb, 0, 0, buf, size, 0, 0, 0, 0)) & 01))
        	        return rval;
 	}
       return OK;
}
...
{
	II_VMS_ITEM_LIST_3	itemlist[3];
	STATUS	PCstatus;
	int	modeval;

	
	itemlist[0].buflen = sizeof(modeval);
	itemlist[0].itmcod = JPI$_MODE;
	itemlist[0].bufadr = &modeval;
	itemlist[0].reslen = NULL;

	itemlist[1].buflen = MAX_LOC;
	itemlist[1].itmcod = JPI$_USERNAME;
	itemlist[1].bufadr = us;
	itemlist[1].reslen = NULL;

	itemlist[2].buflen = NULL;
	itemlist[2].itmcod = NULL;
	itemlist[2].bufadr = NULL;
	itemlist[2].reslen = NULL;
