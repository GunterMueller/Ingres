...
i4 *uperms;			/* Returned permissions */
{
	...
	ITEM P_AccLst[NO_ITEMS];	/* Itemlist for SYS$CHKPRO */

	/*
	**	Set up the itemlist.
	*/
	P_AccLst[ACCESS].i_buflen = sizeof(int);
	P_AccLst[ACCESS].i_code = CHP$_ACCESS;
	P_AccLst[ACCESS].i_bufadr = (int *)&access_type;
	P_AccLst[ACCESS].i_retlenadr = NULL;

	P_AccLst[ACMODE].i_buflen = sizeof(char);
	P_AccLst[ACMODE].i_code = CHP$_ACMODE;
	P_AccLst[ACMODE].i_bufadr = (int *)&user_mode;
	P_AccLst[ACMODE].i_retlenadr = NULL;

	P_AccLst[MODE].i_buflen = sizeof(char);
	P_AccLst[MODE].i_code = CHP$_MODE;
	P_AccLst[MODE].i_bufadr = (int *)&user_mode;
	P_AccLst[MODE].i_retlenadr = NULL;

	P_AccLst[MODES].i_buflen = 2*sizeof(int);
	P_AccLst[MODES].i_code = CHP$_MODES;
	P_AccLst[MODES].i_bufadr = (int *)user_modes;
	P_AccLst[MODES].i_retlenadr = NULL;

	P_AccLst[OWNER].i_buflen = sizeof(int);
	P_AccLst[OWNER].i_code = CHP$_OWNER;
	P_AccLst[OWNER].i_bufadr = (int *)&xabp->xab$l_uic;
	P_AccLst[OWNER].i_retlenadr = NULL;

	P_AccLst[PROT].i_buflen = sizeof(short);
	P_AccLst[PROT].i_code = CHP$_PROT;
	P_AccLst[PROT].i_bufadr = (int *)&xabp->xab$w_pro;
	P_AccLst[PROT].i_retlenadr = NULL;

	if (xabp->xab$w_acllen > 0)
	{
	 	P_AccLst[ACL].i_code = CHP$_ACL;
 	 	P_AccLst[ACL].i_buflen = xabp->xab$w_acllen;
	 	P_AccLst[ACL].i_bufadr = (int *)xabp->xab$l_aclbuf;
	 	P_AccLst[ACL].i_retlenadr = NULL;

		P_AccLst[END].i_code = CHP$_END;
 	 	P_AccLst[END].i_buflen = 0;
	}
	else
	{
	 	P_AccLst[ACL].i_code = CHP$_END;
 	 	P_AccLst[ACL].i_buflen = 0;
	}
