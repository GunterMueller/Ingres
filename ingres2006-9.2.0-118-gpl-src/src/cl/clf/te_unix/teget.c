...
# else
# if defined(i64_aix)
    /* FIX_ME: sigsetmask takes a 32-bit value, the mask contains
    ** and array of 64-bit values on this platform, is there another
    ** function call we can use? Cast for now
    */
    (void)sigsetmask((int)scp->sc_mask.ss_set[1]);
# else /* i64_aix */
...
# if defined(dol_us5) || defined(sqs_ptx) || defined(dg8_us5) \
  ...
  || defined(hpb_us5) || defined(i64_aix) || defined(LNX) \
  || defined(rs4_us5) || defined(mg5_osx)
	{
	sigset_t mask;
	sigemptyset(&mask);
