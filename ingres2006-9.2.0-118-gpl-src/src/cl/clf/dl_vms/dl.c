/*
**    Copyright (c) 1991, 2002 Ingres Corporation
*/
#include <compat.h>
#include <gl.h>
#include <cm.h>
...
**  none
*/
static STATUS DLconstructobjname(dlmodname, tmpbuf, tmplocp, errp)
    char *dlmodname;
    char tmpbuf[];
    LOCATION *tmplocp;
    CL_ERR_DESC	*errp;
...
static STATUS DLconstructobjloc(dlmodname, tmplocp, errp)
    char *dlmodname;
    LOCATION *tmplocp;
    CL_ERR_DESC	*errp;
{
...
#else	/* CL_PROTOTYPED */
STATUS
DLprepare(vers,	dlmodname, syms, handle, errp)
char *vers;
char *dlmodname;
char *syms[];
PTR *handle;
...
#endif	/* CL_PROTOTYPED */
{
    ...
    */
    if ( syms[0] != NULL && ! STcmp(syms[0],((DLhandle *)handle)->sym))
    {
        *addr = ((DLhandle *)handle)->func;
        return(OK);
    }
    else  if  ( DLosprepare(NULL, &syms, errp, (DLhandle *)handle ) != OK )
    {
