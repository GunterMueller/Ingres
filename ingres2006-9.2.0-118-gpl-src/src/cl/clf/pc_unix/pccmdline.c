...
# ifdef LNX
static READONLY char	def_interpreter[] =	"/bin/bash";
# else
static READONLY char	def_interpreter[] =	"/bin/sh";
# endif
...
CL_ERR_DESC     *err_code;
{
    ...
    *ap++ = interp;
    ...
    {
	SIfprintf(stderr, cmdline != NULL ? cmdline : interp);
	SIflush(stderr);
    }
    else
	debug = NULL;	/* so we don't have to keep checking EOS */
...
#endif
	*ap++ =
	    debug != NULL ? STprintf(newcmdline, "adb %s", cmdline) : cmdline;
    }
