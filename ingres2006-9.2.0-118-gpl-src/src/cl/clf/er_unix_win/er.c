...
# else /* NT_GENERIC */
# if defined(OS_THREADS_USED) && ( defined(rs4_us5) || defined(ris_u64) ) 
	syslog_r( LOG_ALERT|LOG_ERR, message );
# endif /* OS_THREADS_USED && ( rs4_us5 || ris_u64 ) */
...
# endif /* NT_GENERIC */
    ...
    {
        i4 msglen = 0;
        char  node_name[128]; /* quick fix, must be CX_MAX_NODE_NAME_LEN */
        ...
        */
        TRformat( NULL, 0, tmp_buf, ER_MAX_LEN - 1,
            "%8.8t::[INGSYSLOG         , 00000000]: %@ ", STlength(node_name),
            node_name );
...
CL_ERR_DESC	   *err_code;
{
    ...
    if (er_ifi == -2)
    {
	if (NMloc(FILES, FILENAME, "errlog.log", &loc) != OK)
	{
	    er_ifi = -1;
	    return(ER_BADSEND);
	}
