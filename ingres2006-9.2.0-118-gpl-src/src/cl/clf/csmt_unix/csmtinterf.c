...
#if (defined(xCL_077_BSD_MMAP) || defined(rs4_us5) || defined(ris_u64)) && !defined(LNX)
    {
	PTR		    ex_stack;
	SIZE_TYPE	    npages;
...
# ifndef USE_BLOCKING_IO  /* use CLpoll */
        if (mask & CS_BIO_MASK )
	{
	    STATUS status=0;
	    int admin_counter = 0;
	    ...
	    while ( scb->cs_state == CS_EVENT_WAIT &&
		...
		if ((scb->cs_self == CS_ADDER_ID) && (status != E_TIMEOUT) && (to_cnt))
		   admin_connect = 1;
	    }
...
# endif /* ! USE_BLOCKING_IO */

	{
	    ...
	    while ( scb->cs_state == CS_EVENT_WAIT &&
		    ...
		    !((scb->cs_mask & CS_IRCV_MASK) && (mask & CS_INTERRUPT_MASK)) &&
	            scb->cs_evcb->event_state == EV_WAIT_MASK )
	    {
...
# if defined(rs4_us5) || defined(axp_osf) || defined(sgi_us5) || \
     defined(hpb_us5) || defined(ris_u64) || defined(i64_aix) || \
...
# if defined(rs4_us5) || defined(axp_osf) || defined(sgi_us5) || \
     defined(ris_u64) || defined(LNX) || defined(i64_aix) || \
...
# endif  /* rs4_us5 || axp_osf */
			...
			*/
			TMet(&time_now);
			ts.tv_sec  = time_now.TM_secs + unint_tmo;
			ts.tv_nsec = time_now.TM_msecs * 1000000;
...
# endif  /* rs4_us5 || axp_osf */
			{
			    ...
			    if (scb->cs_state != CS_EVENT_WAIT ||
					(scb->cs_mask & (CS_TO_MASK | CS_EDONE_MASK | CS_IRCV_MASK)) ||
					scb->cs_evcb->event_state != EV_WAIT_MASK)
				    TRdisplay("SUSP:No signal but state changed in %d secs cs_mask:%v cs_state:%w ev:%v\n",
					unint_tmo,
			    ...
			    status = OK;
					...
					scb->cs_state == CS_EVENT_WAIT &&
					!(scb->cs_mask & (CS_TO_MASK | CS_EDONE_MASK)) &&
					scb->cs_evcb->event_state == EV_WAIT_MASK)
			{
			    TRdisplay("SUSP:Signalled but with no state change (%d)\n", unint_tmo);
			}
			unint_tmo += unint_tmo/2; /* Increase the timeout by 1.5 */
		    }
# else /* xCSBIOPEEK */
		    status = CS_cond_wait( &scb->cs_evcb->event_cond,
					   &scb->cs_evcb->event_sem );
# endif /* xCSBIOPEEK */
		}
	    }
	...
	else if ( scb->cs_mask & CS_EDONE_MASK ||
...
**	occur after the CScancel() call is made).
**
** Inputs:
**	ecb				Pointer to a system specific event
**					control block.	Can be zero if not used.
...
# if !defined(xCL_NO_STACK_CACHING) && !defined(hp8_us5) && \
   !defined(hpb_us5) && !defined(hp2_us5)
...
int  pos;
{
   ...
   if (scb)
   {
      /* Performing a "dirty" read of the cs_mask. As this function will be called
      ** by the current thread, and its this thread that will clear the mask bit
      ** being checked. CSMTcancelled() takes the cs_access_sem for the scb before
      ** clearing the bit.
      */
      if (scb->cs_mask & CS_EDONE_MASK)
      {
         TRdisplay("[%x, %x] %@ EDONE bit set prior to call from %s Pos %d",
                  Cs_srv_block.cs_pid, scb, descrip, pos);
      }
