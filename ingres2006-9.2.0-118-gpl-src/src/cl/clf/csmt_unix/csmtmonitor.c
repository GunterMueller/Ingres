...
FUNC_EXTERN STATUS      CS_null_handler();
FUNC_EXTERN CS_SCB     *CS_verify_sid();
FUNC_EXTERN i4		CSmon_try_registered_diags(
			 i4 (*)(i4, i4, char *), char * );
...
*/
STATUS
CSMTmonitor(mode, temp_scb, nmode, command, powerful, output_fcn)
i4		   mode;
CS_SCB		   *temp_scb;
i4		   *nmode;
char		   *command;
...
i4		   (*output_fcn)();
{
    ...
    *nmode = CS_EXCHANGE;
	...
	case CS_INPUT:
		...
		{
		    TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
			"Mutex at %p: Name: %s, EXCL owner: (tid: %d, pid: %d)\n ",
				mutex,
		    mutex->cs_sem_name[0] ? mutex->cs_sem_name : "(UN-NAMED)",
#if defined(axp_osf)
                                mutex->cs_thread_id->__sequence,
#else
				*(i4*)&mutex->cs_thread_id,
#endif
				mutex->cs_pid);
		}
		...
		{
		    TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
			"Mutex at %p: Name: %s, SHARE owner: (tid: %d, pid: %d), count: %d\n",
				mutex,
		    mutex->cs_sem_name[0] ? mutex->cs_sem_name : "(UN-NAMED)",
#if defined(axp_osf)
                                mutex->cs_thread_id->__sequence,
		...
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		"    Shared: %8d Collisions: %8d Hwm: %4d \n      Excl: %8d Collisions: %8d \n",
...
Water\t%d\r\n\r\nrdy mask %x state: %x\r\nidle quantums %d./%d. (%d.%%)",
		...
		buffer += STlength(buffer);
		STprintf(buffer, "                     Spins: %8d.     Yields: %8d.\n",
    ...
    i4	    powerful)
{
	...
	if (stats)
	{
		...
		an_scb->cs_self,
#if defined(axp_osf)
                an_scb->cs_thread_id->__sequence,
#else
		*(i4*)&an_scb->cs_thread_id,
#endif
		an_scb->cs_username,
		sbuf);
	}
	else switch (an_scb->cs_state)
	{
	    ...
	    case CS_UWAIT:
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		    "Session %p:%d (%24s) cs_state: %w cs_mask: %v",
			    an_scb->cs_self,
#if defined(axp_osf)
                            an_scb->cs_thread_id->__sequence,
#else
		...
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		    "Session %p:%d (%24s) cs_state: <invalid> %x",
		    an_scb->cs_self,
#if defined(axp_osf)
                    an_scb->cs_thread_id->__sequence,
#else
	...
	if (formatted)
	{
	    ...
	    {
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		    "Session %p:%d is a monitor task, owner is pid %d",
			an_scb->cs_self,
#if defined(axp_osf)
                        an_scb->cs_thread_id->__sequence,
#else
	    ...
	    {
		TRformat(output_fcn, 1, buf, sizeof(buf) - 1,
		"Session %p:%d is an internal task; no info available",
			an_scb->cs_self,
#if defined(axp_osf)
                        an_scb->cs_thread_id->__sequence);
#else
...
# if defined(DCE_THREADS)
	CS_create_thread( Cs_srv_block.cs_stksize, /* stack size */
                          ...
                          &status );
# elif defined(hp8_us5) || defined(hpb_us5) || defined(hp2_us5)
	CS_create_thread( Cs_srv_block.cs_stksize, /* stack size */
			  NULL,                    /* stack addr */
			  CSMT_sampler,		   /* init routine */
			  CsSamplerBlkPtr,	   /* parm */
...
LKCallbackThread,LKDeadlockThread,SamplerThread,SortThread,FactotumThread,LicenseThread,Invalid",
    ...
    */	
	...
	TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
	    "Top Mutexes Analysis:", 0L);

	TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
	    "Mutex Name                       ID        User  Fast  Write   Log  Group  Sort  Other", 0L);
    ...
    */	
	...
	for (k = 0; k < NUMTOP - 1; k++)
	{
	    ...
	    (*Cs_srv_block.cs_format_lkkey)(&CsSamplerBlkPtr->Lock[i].lk_key,
	      lockname);
	    TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
		"%64s  %5.1f  %5.1f",
    ...
    */
	...
	TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
	"          DIOR  DIOW  LIOR  LIOW  BIOR  BIOW   Log  Lock LGEvnt LKEvnt", 0L);
...
# endif /* xCL_094_TLS_EXISTS */
    ...
    TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
	    "getrusage() measurements of this process since sampling started:", 0L);

    TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
    "    UTIME    STIME  MAXRSS   IDRSS  MINFLT  MAJFLT   NSWAP INBLOCK OUBLOCK", 0L);
    TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
    ...
    TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
    "   MSGSND   MSGRCV   NSIGS   NVCSW  NIVCSW", 0L);
    TRformat(output_fcn, (i4 *) 1, buf, sizeof(buf)-1,
    " %8d %8d%8d%8d%8d",
