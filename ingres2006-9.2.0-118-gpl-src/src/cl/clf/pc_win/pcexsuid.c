...
PCexec_suid(char *cmdbuf)
{
    ...
    */
    ...
    STprintf(ServiceName, "%s_Database_%s", SYSTEM_SERVICE_NAME,
	     tchII_INSTALLATION );
    if ( (OpIngSvcHandle = OpenService(schSCManager, ServiceName,
				       SERVICE_USER_DEFINED_CONTROL)) == NULL)
    {
	error_exit(GetLastError());
	return(FAIL);
    }
	...
	*/
	...
	for (;;)
	{
	    if ( ((!ReadFile(OutFile, OutBuf, sizeof(OutBuf), &BytesRead, NULL)
		  || (BytesRead == 0)) &&
		  (!ProcHandle || (GetExitCodeProcess(ProcHandle, &ExitCode) &&
	...
	if (PCadjust_SeDebugPrivilege(FALSE, &err_code) == FAIL)
	{
	   error_exit(err_code.errnum);
	   return (FAIL);
	}
