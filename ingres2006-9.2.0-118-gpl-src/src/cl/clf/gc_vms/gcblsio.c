...
{
    char *trace;
    II_VMS_ITEM_LIST_3	syi_list[4] = {  { 4, SYI$_MAXBUF, (PTR)&maxwbuf, 0 },
    ...
    sys$getsyiw(0, 0, 0, syi_list, 0, 0, 0);
...
comdone( )
{
    ...
    sys$qiow( 0, comfd, IO$_SETMODE, 0, 0, 0, &ttmode_initial, 12, 0, 0, 0, 0 );
}

/*
** Name: setto
...
int bdrt, par;
{
    ...
    sys$cancel( comfd );
    status = sys$qiow( 0,		/* set mode event flag  */
