...
STATUS
GC_tcp6_set_socket( char *buf, SOCKADDRIN6 *s)
{
    i4 port;

    /* set defaults */

    s->SIN6$B_FAMILY = AF_INET6;

    /*
    ** Set wildcard address for listens.
    */
    MEfill ( sizeof(s->SIN6$R_ADDR_OVERLAY.SIN6$T_ADDR), 0, 
        &s->SIN6$R_ADDR_OVERLAY.SIN6$T_ADDR );
    ...
    {
        CVan( buf, &port );
            s->SIN6$W_PORT = htons( (u_short)port );
    }
    else
        return (BS_NOPORT_ERR);
