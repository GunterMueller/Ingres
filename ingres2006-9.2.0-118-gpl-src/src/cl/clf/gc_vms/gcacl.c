...
** puts the IOXB (QIO-level control block) into GCsynch_ioxb; GCsync
** waits for the I/O's event flag and then examines GCsynch_ioxb to
** drive the completions of sync operations.  GCsynch_ioxb is an array 
...
GLOBALDEF i4 GCC_lsn_chan = 0;          /* GCC Listen chan */

/* VMS event flags */

static  i4	    GCdv_ef;		/* event flag for sys$getdvi */
static  i4	    GCio_ef;		/* event flag for sys$qio */
static  i4	    GCtm_ef;		/* event flag for SYNCH timer */
static  i4	    GCzm_ef;		/* event flag for zombie check tmr */

# define GC_EF_MASK ( 1 << GCio_ef % 32 | 1 << GCtm_ef % 32 )

static  IOXB	    *GCsynch_ioxb[2];	/* current SYNCH I/Os */
...
static	i4	    GCzomb_tmrset;	/* Zombie checking timer set */

static  char	    GC_instcode[ 3 ];	/* Installation code */

static  i4	    GCttrace = 0;	/* Timer trace level */
...
VOID  	    (*free)();
{
    ...
    status = lib$get_ef(&GCdv_ef);
    status = lib$get_ef(&GCio_ef);
    status = lib$get_ef(&GCtm_ef);
    status = lib$get_ef(&GCzm_ef);
...
SVC_PARMS          *svc_parms;
{
    ...
    if( VMS_FAIL( sys$getjpiw( 0, 0, 0, jpiget, 0, 0, 0 ) ) )
	pid = 0;
...
SVC_PARMS	*svc_parms;
{
    ...
    if( !( asipc = GCinitasipc() ) )
    {
	GCTRACE(1)( "GCrequest> (GCinitasipc), memory allocation failed\n" );
    ...
    if( VMS_FAIL(status) )
    {
	GCTRACE(1)( "GCrequest> (GCmbx_assign), expected partner not available\n" );
    ...
    */
    ...
    {
	GCTRACE(1)( "GCrequest> (GCmbx_detect), partner not available\n" );
    	svc_parms->sys_err->error = 0;
	goto complete;
    }
    ...
    if( VMS_FAIL(status) )
    {
	GCTRACE(1)( "GCrequest> (GCmbx_client), Unable to create partner assication\n" );
...
SVC_PARMS          *svc_parms;
{
	...
	*/
	...
	i4 	msg_count = 0;
	II_VMS_ITEM_LIST_3	dvi_list[] = {
			    {4, DVI$_DEVDEPEND, (PTR)&msg_count, 0},
			    {0, 0, 0, 0} };

	qiostat = sys$getdviw(GCdv_ef, a_chan->chan, 0, dvi_list, 0, 0, 0, 0);
	...
	GCTRACE(2)( "GCreceive asipc=%x msg_count=%d\n", asipc, msg_count );
...
# define	    UAI$_ENCRYPT	0x16
# define	    UAI$_FLAGS		0x23
# define	    UAI$_PWD		0x12
# define	    UAI$_SALT		0x15
# define            UAI$_EXPIRATION     0x19 
...
# else /* ALPHA */
    ...
    */
    ...
    sys_err->error = sys$getsyiw(0, 0, 0, syi_list, 0, 0, 0);
...
CL_ERR_DESC	*sys_err;
{
    ...
    II_VMS_ITEM_LIST_3		lnm_item_list[2];
    ...
    */
	...
	break;
      ...
      case GC_SET_NSID:
      {
      	lnm_item_list[0].itmcod = LNM$_STRING;
	lnm_item_list[0].buflen = (short) length ;
	lnm_item_list[0].reslen = &ret_leng;
	lnm_item_list[0].bufadr = gca_listen_id;
	lnm_item_list[1].itmcod = 0;
	lnm_item_list[1].buflen = 0;
...
GChostname(char *ourhost, i4  hostlen)
{
	...
	char *nodename = MEreqmem(0,8,0,NULL);

    II_VMS_ITEM_LIST_3	itmlst[]
        =
        {
                {8, SYI$_NODENAME, nodename, 0},
                {0, 0, 0, 0}
        };
    ...
    qiostat = sys$getsyiw(0, 0, 0, itmlst, 0, 0, 0);
...
GCinitasipc()
{
    ...
    */
    ...
    MEfill( sizeof(ASSOC_IPC), NULL, (PTR)asipc );
...
char	*convtime;
{
    ...
    GCTRACE(2)( "GCtim_strtoms %s -> %d ms\n", convtime, millis );
...
GCsync(i4 timeout)
{
    ...
    {
	GCTRACE(0)( "GCsync: sys$wflor failed\n" );
    ...
    ** GCio_ef is set by sys$qio completion (from GCqio_exec).
    ...
    */
    ...
    GCTRACE(2)( "GCsync wflor io=%d timer=%d\n", io_done, tm_done );
    GCX_TRACER_2_MACRO( "GCsync>", 0, "wflor io=%d timer=%d", io_done, tm_done);

    /* Look for any sync IOXB's - drive their completions. */

    for( i = 0; i < 2; i++)
    {
	if( !( ioxb = GCsynch_ioxb[i] ) )
	    continue;

	GCTRACE(3)( "GCsync ioxb=%x [%d]\n", ioxb, i );
	GCX_TRACER_1_MACRO( "GCsync>", ioxb, "ioxb #%d", i );

	/* If timer expired, cancel I/O with GCqio_timeout.  */

	if( ioxb->ioxb$l_tmo > 0 && tm_done )
	    GCqio_timeout( ioxb );

	/* If the I/O completed, process with GCqio_complete & clear. */
...
i4	sync;
{
    ...
    GCTRACE(1)( "GCqio_exec ioxb=%x func=%x tmo=%d sync=%d\n",
	ioxb, func, tmo, sync );
    GCX_TRACER_3_MACRO( "GCqio_exec>", ioxb, "func=%x tmo=%d sync=%d",
	    func, tmo, sync );
	...
	*/
	...
	status = sys$setimr(
	    GCtm_ef,			    			/* efn */
	    bintime,  	    					/* daytim */
    	    sync ? NULL : GCqio_timeout,   			/* astadr */
	    ioxb, 0);			    			/* reqidt */
    ...
    status = sys$qio(
    	  ...
    	  func,				    	/* func */
    	  ioxb,				    	/* ioxb */
    	  sync ? NULL : GCqio_complete,		/* astadr */
    	  sync ? NULL : ioxb,			/* astprm */
    	  va,				    	/* p1 = VA */
    	  len,			    	      	/* p2 = size */
    ...
    */
    ...
    /* For SYNC calls, stash ioxb for GCsync to find. */
	...
	else if( !GCsynch_ioxb[1] )
	{
	    GCsynch_ioxb[1] = ioxb;
	}
	else
	{
	    GCTRACE(0)( "GCqio_exec: too many synch I/O's.\n");
	    GCX_TRACER_1_MACRO( "GCqio_exec>", 0, "too many synch I/O's.", 0 );
	    lib$signal(SS$_DEBUG);
	}
    }
...
error:
    ...
    GCTRACE(1)( "GCqio_exec ioxb=%x fail VMS status %x\n", ioxb, status );
...
IOXB	*ioxb;
{
    GCTRACE(2)( "GCqio_timeout ioxb=%x %s cancel QIO\n",
...
IOXB	*ioxb;
{
    GCTRACE(2)( "GCqio_complete ioxb=%x w_status %x w_can_status %x\n",
		ioxb, ioxb->ioxb$w_status, ioxb->ioxb$w_can_status );
...
SVC_PARMS       *svc_parms;
{
    ...
    */
    ...
    GCTRACE(2)( "GCqio_status ioxb=%x status %x VMS status %x\n",
	ioxb, svc_parms->status, svc_parms->sys_err->error );
...
ASSOC_IPC *asipc;
{
    ...
    for( i = 0; i < CHANS; i++ )
	if( asipc->chan[i].ioxb.ioxb$w_status != GC_CH_WS_IDLE )
	    io_count++;

    GCTRACE(2)( "GCqio_pending asipc=%x pending %d\n", asipc, io_count );
...
GCqio_zombie()
{
    ...
    GCTRACE(2)( "GCqio_zombie started by %s\n",
    ...
    */
    ...
    {
	ASSOC_IPC 	*asipc = (ASSOC_IPC *)asipc_qhdr;
	IOXB		*ioxbs[ CHANS ];
	i4		count;
	...
	*/
	...
	if( GCmbx_detect( asipc->chan[RN].chan ) < 2 )
	{
	    ...
	    GCTRACE(1)( "GCqio_zombie asipc=%x dead\n", asipc );
    ...
    */
    ...
    GCTRACE(2)( "GCqio_zombie done I/O's pending = %d\n", pending_io );
	...
	/* XXX what to do if the timer can't be started? */
...
ASSOC_IPC	   *asipc;
{
    ...
    */
	...
	GCTRACE(2)( "GCmbx_assoc: asipc=%x chan %d: %s\n",
	    asipc, i, asipc->chan[ i ].name );
...
i4		channel;
{
    ...
    if( VMS_FAIL( sys$getdviw(GCdv_ef, channel, 0, dvi_list, 0, 0, 0, 0) ) )
	return -1;
...
char	*invoker;
{
    ...
    */
    ...
    {
        status = sys$getjpiw(0, &sender_pid, 0, jpiget, 0, 0, 0);
    ...
    */
    ...
    for (i = 0; i < TRY_COUNT; i++)
    {
        status = sys$getjpiw(0, &parent_id, 0, jpiget2, 0, 0, 0);
        if (VMS_SUSPENDED(status))
        {
	   PCsleep(1000);
	   continue;
    ...
    */
	...
	status = sys$getdviw(GCdv_ef, 0, &term_desc, dviget2,	0, 0, 0, 0);
	if (VMS_FAIL(status))
	    return status;
