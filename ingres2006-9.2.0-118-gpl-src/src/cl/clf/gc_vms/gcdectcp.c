...
FUNC_EXTERN  STATUS     GC_tcp6_set_socket( char *buf, SOCKADDRIN6 *s);
FUNC_EXTERN  STATUS     GC_tcp6_addr( char *buf, struct addrinfo **addrinfo );

GLOBALREF i4 GCC_lsn_chan;                      /* IIGCC listen chan */
...
BS_PARMS	*bsp;
{
	   ...
	   ( status = lbcb->iosb.iosb$w_status ) != SS$_NORMAL )
	{
        ...
        GCC_lsn_chan = lbcb->chan;   /* for use during IIGCC shutdown */
...
BS_PARMS	*bsp;
{
	...
	VMS_STATUS              status;
	SOCKADDRIN6	        sockaddrin6, *s = &sockaddrin6;
        struct sockchar         sck_parm;
	   ...
	   ( status = lbcb->iosb.iosb$w_status ) != SS$_NORMAL )
	{
	    ...
	    bsp->syserr->error = status;
            if( bsp->status != OK )
            {
                TCP_TRACE(1)("tcp6_listen: %p failure from sys$qio(), status=%x\n",status);
	        goto complete;
            }
	}
...
BS_PARMS	*bsp;
{
    ...
    II_VMS_ITEM_LIST_3 peer_itemlst;     /* client socket address item-list  */ 

    u_i4 socket_addrlen;             /* returned length of server socket */ 
    struct sockaddr_in socket_addr;  /* server socket address structure  */ 
    II_VMS_ITEM_LIST_3 socket_itemlst;   /* server socket address item-list  */ 
    ...
    */ 
    ...
    peer_itemlst.buflen = sizeof( peer_addr ); 
    peer_itemlst.itmcod = INET$C_SOCK_NAME;
    peer_itemlst.bufadr = &peer_addr; 
    peer_itemlst.reslen = &peer_addrlen; 

    socket_itemlst.buflen = sizeof( socket_addr ); 
    socket_itemlst.itmcod = INET$C_SOCK_NAME; 
    socket_itemlst.bufadr = &socket_addr; 
    socket_itemlst.reslen = &socket_addrlen; 
...
BS_PARMS	*bsp;     
{
	...
	if( status != SS$_NORMAL )
	{
            TCP_TRACE(1)("tcp6_accept: %p failure from sys$qio(), status=%x\n",
                status);
	    bsp->status = BS_ACCEPT_ERR;
	    bsp->syserr->error = status;
	    goto complete;
...
BS_PARMS	*bsp;
{
    ...
    u_i4 peer_addrlen;               /* returned length of client socket */ 
    II_VMS_ITEM_LIST_3 peer_itemlst;     /* client socket address item-list  */ 
    SOCKADDRIN6 peer_addr, socket_addr; 
    u_i4 socket_addrlen;             /* returned length of server socket */ 
    II_VMS_ITEM_LIST_3 socket_itemlst;   /* server socket address item-list  */ 
    ...
    */
    ...
    status = getnameinfo( (struct sockaddr *)&peer_addr, 
        sizeof(peer_addr), node, sizeof(node), NULL, 0, 
    ...
    */
    if (IN6_ARE_ADDR_EQUAL(
        (struct in6_addr *)&peer_addr.SIN6$R_ADDR_OVERLAY.SIN6$B_SA6_ADDR, 
        (struct in6_addr *)&socket_addr.SIN6$R_ADDR_OVERLAY.SIN6$B_SA6_ADDR))
        bsp->is_remote = FALSE;
...
BS_PARMS	*bsp;
{
	...
	/* translate address */
	if( GC_tcp6_addr( bsp->buf, &bcb->addrinfo ) != OK )
	{
	    bsp->status = BS_SOCK_ERR;
	    bsp->syserr->error = status;
	    goto complete;
        ...
        for( i=0; bcb->addrinfo; bcb->addrinfo = bcb->addrinfo->ai_next, i++ )
        {
            /*
            ** Attempt to connect on the next ipv6 or ipv4 address presented 
            ** on the address list.
            */
            if (bcb->addrinfo->ai_family != AF_INET6 && 
                bcb->addrinfo->ai_family != AF_INET)
                continue;
            tcp6_connect(bsp);
        }
...
BS_PARMS	*bsp;
{
	...
	if( status != SS$_NORMAL )
	{
            TCP_TRACE(3)("tcp6_connect: %p failure from sys$qiow(), status=%x\n",
                status);
	    bsp->status = BS_SOCK_ERR;
	    bsp->syserr->error = status;
	    return;
	...
	if( ( status = bcb->siosb.iosb$w_status ) != SS$_NORMAL )
	{
            TCP_TRACE(2)("tcp6_connect: %p failure from sys$qiow(), IOSB status=%x\n",
                status);
        ...
        TCP_TRACE(3)("tcp6_connect(1): flags 0x%x, protocol %d, type %d, next addr %p, addr %p\n",
            bcb->addrinfo->ai_flags, bcb->addrinfo->ai_protocol,
            bcb->addrinfo->ai_socktype, bcb->addrinfo->ai_next, bcb->addrinfo);

        TCP_TRACE(2)("tcp6_connect: Initiated connection to host: %s, port: %d\n",
                bcb->addrinfo->ai_canonname, 
                htons(((SOCKADDRIN6 *)bcb->addrinfo->ai_addr)->SIN6$W_PORT));

        s = (SOCKADDRIN6 *)bcb->addrinfo->ai_addr;
        ...
        if( status != SS$_NORMAL )
        {
            TCP_TRACE(3)("tcp6_connect(3): %p failure from sys$qiow(), status=%x\n", 
status);
            bsp->status = BS_CONNECT_ERR;
            bsp->syserr->error = status;
            return;
        ...
        if( ( status = bcb->siosb.iosb$w_status ) != SS$_NORMAL)
        {
            TCP_TRACE(2)("tcp6_connect(4): %p failure from sys$qiow(), IOSB status=%x\n", status);
