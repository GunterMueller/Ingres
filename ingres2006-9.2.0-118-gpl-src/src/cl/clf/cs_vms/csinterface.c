...
FUNC_EXTERN STATUS  CSterminate();      /* Terminate the server */
FUNC_EXTERN STATUS  CSalter();          /* Alter the server characteristics */
FUNC_EXTERN STATUS  CSdispatch();       /* Begin dispatching */
FUNC_EXTERN STATUS  CSremove();         /* Remove a thread from the system */
FUNC_EXTERN void    CSdump_statistics();/* Dump server statistics to log */
...
#endif
FUNC_EXTERN void    i_EX_initfunc();	/* Init the EX facility */
...
static	i4			Cs_numprocessors = 0;
static	i4			Cs_desched_usec_sleep = 10000;
static	i4			Cs_max_sem_loops = 250;
static  i4                     Cs_sleep_ratio = 1;

static void	CSset_exsp();		/* set current exception stack ptr */
...
GLOBALDEF MO_CLASS_DEF CS_int_classes[] =
{
  ...
  { 0, "exp.clf.vms.cs.srv_block.wait_stats.bio_time",
	sizeof( Cs_srv_block.cs_wtstatistics.cs_bio_time ),
	MO_READ, 0,
	CL_OFFSETOF( CS_SYSTEM, cs_wtstatistics.cs_bio_time ),
	MOintget, MOnoset, (PTR)&Cs_srv_block, MOcdata_index },

  { 0, "exp.clf.vms.cs.srv_block.wait_stats.bio_waits",
	sizeof( Cs_srv_block.cs_wtstatistics.cs_bio_waits ),
	MO_READ, 0,
	CL_OFFSETOF( CS_SYSTEM, cs_wtstatistics.cs_bio_waits ),
	MOintget, MOnoset, (PTR)&Cs_srv_block, MOcdata_index },
  ...
  { 0, "exp.clf.vms.cs.srv_block.wait_stats.bio_done",
	sizeof( Cs_srv_block.cs_wtstatistics.cs_bio_done ),
	MO_READ, 0,
	CL_OFFSETOF( CS_SYSTEM, cs_wtstatistics.cs_bio_done ),
	MOintget, MOnoset, (PTR)&Cs_srv_block, MOcdata_index },

  { 0, "exp.clf.vms.cs.srv_block.wait_stats.dio_time",
	sizeof( Cs_srv_block.cs_wtstatistics.cs_dio_time ),
	MO_READ, 0,
	CL_OFFSETOF( CS_SYSTEM, cs_wtstatistics.cs_dio_time ),
	MOintget, MOnoset, (PTR)&Cs_srv_block, MOcdata_index },

  { 0, "exp.clf.vms.cs.srv_block.wait_stats.dio_waits",
	sizeof( Cs_srv_block.cs_wtstatistics.cs_dio_waits ),
	MO_READ, 0,
	CL_OFFSETOF( CS_SYSTEM, cs_wtstatistics.cs_dio_waits ),
	MOintget, MOnoset, (PTR)&Cs_srv_block, MOcdata_index },
  ...
  { 0, "exp.clf.vms.cs.srv_block.wait_stats.dio_done",
	sizeof( Cs_srv_block.cs_wtstatistics.cs_dio_done ),
	MO_READ, 0,
	CL_OFFSETOF( CS_SYSTEM, cs_wtstatistics.cs_dio_done ),
	MOintget, MOnoset, (PTR)&Cs_srv_block, MOcdata_index },
...
CSinitiate(i4 *argc, char ***argv, CS_CB *ccb)
{
    ...
    char		*sem_parms;
    II_VMS_ITEM_LIST_3	syidef[] =
			{
			    { sizeof(Cs_numprocessors), SYI$_AVAILCPU_CNT,
							&Cs_numprocessors, 0},
			    { 0, 0, 0, 0}
			};
    ...
    **	    II_NUM_OF_PROCESSORS	- controls Cs_numprocessors
    ...
    **                                    -5 - 5 switch / 1 sleep
    */
    NMgtAt("II_NUM_OF_PROCESSORS", &sem_parms);
    if (sem_parms && *sem_parms)
	(void)CVal(sem_parms, &Cs_numprocessors);
    NMgtAt("II_MAX_SEM_LOOPS", &sem_parms);
    if (sem_parms && *sem_parms)
	(void)CVal(sem_parms, &Cs_max_sem_loops);
    NMgtAt("II_DESCHED_USEC_SLEEP", &sem_parms);
    if (sem_parms && *sem_parms)
    	(void)CVal(sem_parms, &Cs_desched_usec_sleep);
    NMgtAt("II_SLEEP_RATIO", &sem_parms);
    if (sem_parms && *sem_parms)
        (void)CVal(sem_parms, &Cs_sleep_ratio);
    if (Cs_numprocessors <= 0)
	Cs_numprocessors = 1;
    if (Cs_max_sem_loops <= 0)
	Cs_max_sem_loops = 250;
...
*/
  ...
  Cs_srv_block.cs_rdy_que[Cs_admin_scb.csa_scb.cs_priority]->cs_rw_q.cs_q_prev;
	...
	*/
	...
	status = sys$getjpiw(0, 0, 0, jpiget, 0, 0, 0);
	if ((status & 1) == 0)
	    sys$exit(status);
	...
	if( Cs_srv_block.cs_define )
	{
	    ...
	    itmlst[0].buflen = STlength( csib.cs_name );
	    itmlst[0].bufadr = csib.cs_name;
...
PTR		   ecb;
{
	...
	{
	    scb->cs_dior++;
	    Cs_srv_block.cs_wtstatistics.cs_dio_done++;
	}
	...
	{
	    scb->cs_bior++;
	    Cs_srv_block.cs_wtstatistics.cs_bio_done++;
	}
	...
	*/
	if (mask & CS_DIO_MASK)
	    Cs_srv_block.cs_wtstatistics.cs_dio_waits++;
	else if (mask & CS_BIO_MASK)
	    Cs_srv_block.cs_wtstatistics.cs_bio_waits++;
        else if (mask & CS_GWFIO_MASK)
...
CS_SID                sid;
{
	...
	if (scb)
	{
		...
		*/
		...
		{
		    Cs_srv_block.cs_wtstatistics.cs_bio_time +=
			Cs_srv_block.cs_quantums - scb->cs_ppid;
		}
		else if (scb->cs_memory & CS_DIO_MASK)
		{
		    Cs_srv_block.cs_wtstatistics.cs_dio_time +=
			Cs_srv_block.cs_quantums - scb->cs_ppid;
		}
        ...
        */
		 ...
		 ** the exact priority of the blocking thread, or
		 ...
		 { /* kept in_kernel prior to !exclusive test */
			 scb->cs_mask |= CS_SMUTEX_MASK;
                 ...
                 scb->cs_inkernel = 0;
                 Cs_srv_block.cs_inkernel = 0;
                 if ( !exclusive ) scb->cs_mask |= CS_SMUTEX_MASK;
                 scb->cs_sync_obj = (i4)sem;
                 if (Cs_srv_block.cs_async)
                    CS_move_async(scb);
		 ...
		 /* moved the setting of the Cs_srv_block.cs_inkernel above setting
		 ** the cs_mask, changes to the cs_mask should be protected
		 ** Added the  scb->cs_inkernel protection
		 */
                 Cs_srv_block.cs_inkernel = 1;
		 ...
		 */
		 retune_priority = TRUE;
           ...
           */
           if ( trace_count++ > Cs_max_sem_loops)
           {
        ...
        " Sem Wait: pid %x ses %x, spid %x sses %x adr %x t %d s %d %0.30s ",
        Cs_srv_block.cs_pid, scb, sem->cs_pid, sem->cs_owner, sem, sem->cs_type,
        sem->cs_test_set, sem->cs_sem_name);
                    ERlog(err_buffer, STlength(err_buffer), &error);
...
*/
STATUS
CSadd_thread(priority, crb, thread_type, thread_id, error)
i4                priority;
PTR                crb;
i4                thread_type;
CS_SID             *thread_id;
...
*/
STATUS
CSstatistics(timer_block, server_stats)
...
i4		server_stats;
{
    ...
    */
	...
	timer_block->stat_dio = Cs_srv_block.cs_wtstatistics.cs_dio_done;
	timer_block->stat_bio = Cs_srv_block.cs_wtstatistics.cs_bio_done;
...
*/
STATUS
CSaltr_session(session_id, option, item)
CS_SID		session_id;
i4		option;
PTR		item;
{
...
i4	    (*print_fcn)(char *, ...))
{
    ...
    STprintf(buffer, "          --------  ----------  ---------  ---------  ---------\n");
    (*print_fcn)(buffer);
    STprintf(buffer, "    BIO: %8d.   %8d.  %8d.  %8d.  %8d.\n",
		    Cs_srv_block.cs_wtstatistics.cs_bio_done,
		    Cs_srv_block.cs_wtstatistics.cs_bio_waits,
		    (Cs_srv_block.cs_wtstatistics.cs_bio_waits ?
			(Cs_srv_block.cs_wtstatistics.cs_bio_time / 
			    Cs_srv_block.cs_wtstatistics.cs_bio_waits) : 0),
		    Cs_srv_block.cs_wtstatistics.cs_bio_done - 
			Cs_srv_block.cs_wtstatistics.cs_bio_waits,
		    Cs_srv_block.cs_wtstatistics.cs_bio_time);
    (*print_fcn)(buffer);
    STprintf(buffer, "    DIO: %8d.   %8d.  %8d.  %8d.  %8d.\n",
		    Cs_srv_block.cs_wtstatistics.cs_dio_done,
		    Cs_srv_block.cs_wtstatistics.cs_dio_waits,
		    (Cs_srv_block.cs_wtstatistics.cs_dio_waits ?
			(Cs_srv_block.cs_wtstatistics.cs_dio_time / 
			    Cs_srv_block.cs_wtstatistics.cs_dio_waits) : 0),
		    Cs_srv_block.cs_wtstatistics.cs_dio_done - 
			Cs_srv_block.cs_wtstatistics.cs_dio_waits,
		    Cs_srv_block.cs_wtstatistics.cs_dio_time);
    (*print_fcn)(buffer);
...
CSswitch()
{
    if (CS_lastquant > ALLOWED_QUANTUM_COUNT)	/* defined in cscl.h */
    {
	CS_ssprsm( FALSE );
    }
}
...
CSms_thread_nap(i4	ms)
{
	...
	*/
	status = sys$setimr(0, delta_timeout,
			    CS_ms_timeout, scb, 0);
	if ((status & 1) == 0)
	    CS_ms_timeout(scb);
