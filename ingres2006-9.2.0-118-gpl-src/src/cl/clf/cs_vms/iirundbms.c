/*
** Copyright 1993, 1987, 2003 by Ingres Corporation 
** All rights reserved.
*/
...
main( int argc, char **argv )
{
	...
	*/
	...
	server_type = argv[1];
        ...
        /* Bug 68559 - Are we starting a GCC ? */
	...
	server_flavor = argv[2];
        ...
        /* Bug 68559 - Need to precede server_flavor with "-instance="
        **             if we're starting a GCC.
        */
        gcc_instance = gcc;
    }
	...
	*/
	...
	if ( status == SS$_WASSET )
        ...
        */
        ...
        {
            if ( (STbcompare( server_type, 6, "oracle", 6, TRUE ) == 0) ||
                      (STbcompare( server_type, 3, "rdb", 3, TRUE ) == 0) )
	...
	{
            /* Bug 68559 - If a GCC private server has been specified
            **             then preceed the servor flavor with "-instance".
            */
            STprintf(buf, start_commands[i], server_type,
                     (gcc_instance) ? "-instance=" : "", server_flavor);
	}
	else
	    STcopy(start_commands[i], buf );
	...
	status = sys$qiow( MAILBOX_EFN, chan, IO$_WRITEVBLK|IO$M_NOW, &iosb,
