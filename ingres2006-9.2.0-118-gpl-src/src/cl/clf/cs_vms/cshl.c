...
FUNC_EXTERN STATUS  CSp_semaphore();	/* perform a "P" operation on a semaphore */
FUNC_EXTERN STATUS  CSv_semaphore();	/* perform a "V" operation on a semaphore */
FUNC_EXTERN bool
...
GLOBALDEF II_VMS_ITEM_LIST_3     Cs_cpu_jpi[] = {
...
i4		   priority;
{
	...
	*/
    ...
    Cs_srv_block.cs_current = new_scb;
    if (new_scb == (CS_SCB *)&Cs_idle_scb)
    {
    ...
    */
	...
	new_scb->cs_rw_q.cs_q_next = Cs_srv_block.cs_rdy_que[i];
	new_scb->cs_rw_q.cs_q_prev =
...
i4                mode;
{
    ...
    status = sys$setimr(0, Cs_srv_block.cs_bquantum, CS_quantum, CS_IDLE_ID, 0);
    if (status & 1)
	return(OK);
...
CS_toq_scan()
{
    ...
    */
	...
	if (status & 1)
	{
	    ...
	    if (status & 1)
	    {
		Cs_cpu_jpi[0].bufadr = &cpu_used;
		status = sys$getjpiw(0, 0, 0, Cs_cpu_jpi, 0, 0, 0);
		if ( status & 1)
		{
		    /*
...
CS_SCB		*scb;
{
	...
	itmlst[0].buflen = sizeof(acc_rec);
	itmlst[0].bufadr = &acc_rec;
	status = sys$sndjbcw(0, SJC$_WRITE_ACCOUNTING, 0, itmlst, &iosb, 0, 0);
    }
...
CS_last_chance(EX_ARGS *sig_vec)
{
	...
	** Vax/VMS when CS_restart() (an alteri4e entry to CS_ssprsm()
		  ...
		  i4      verbose)
{
	...
	*/
	lib$get_curr_invo_context(&stack_context);
    }
    ...
    */
    while ((stack_depth-- > 0) && lib$get_prev_invo_context(&stack_context) )
    {
