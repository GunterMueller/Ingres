...
**
**          MHpkadd_sim() - Add two packed decimals via software (this
**			    routine is executed only via calls from MHpkadd;
**			    it is not an external CL interface)
**          MHpksub_sim() - Subtract two packed decimals via software (this
**			    routine is executed only via calls from MHpksub;
**			    it is not an external CL interface)
**          MHpkmul_sim() - Multiply two packed decimals via software (this
**			    routine is executed only via calls from MHpkmul;
**			    it is not an external CL interface)
**          MHpkdiv_sim() - Divide two packed decimals via software (this
**			    routine is executed only via calls from MHpkdiv;
**			    it is not an external CL interface)
**
...
**	    calculate acutal boundary for truncating digits. Unix change
**	    number 431041
**      19-may-99 (kinte01)
...
/*
[@function_reference@]...
[@function_external@] [@data_type@] {@declarator@}#21$; {@comment@}#41#
...

/*{
** Name: MHpkadd_sim() - Add two packed decimals.
**
** Description:
**	This routine is called by MHpkadd which is written in MACRO.  The
...
**      15-nov-2004 (huazh01)
**          change the prototypes of MHpkadd_sim() / MHpksub_sim() / 
**          MHpkdiv_sim() / MHpkmul_sim(). 
**          This fixes b113059, INGSRV2964. 
**	14-feb-2007 (dougi)
**	    Replaced hard constant 31 with CL_MAX_DECPREC.
*/
 
VOID
MHpkadd_sim(pk1, p1, s1, pk2, p2, s2, pkout, pout, sout)
PTR         pk1;
i4         p1;
i4         s1;
PTR         pk2;
...
i4         *sout;
{
	...
	char	pktmp[(CL_MAX_DECPREC+1)/2];
	    ...
	    MEcopy(pk1, (u_i2)(p1 / 2 + 1), (PTR)pktmp);
            ...
            MHpksub_sim(pk2, p2, s2, pktmp, p1, s1, pkout, pout, sout);
            return;
        }
        else
        {
	    MEcopy(pk2, (u_i2)(p2 / 2 + 1), (PTR)pktmp);
            ...
            MHpksub_sim(pk1, p1, s1, pktmp, p2, s2, pkout, pout, sout);
            return;
        }
    }
...

/*{
** Name: MHpksub_sim() - Subtract two packed decimals.
**
** Description:
**	This routine is called by MHpksub which is written in MACRO.  The
...
VOID
MHpksub_sim(pk1, p1, s1, pk2, p2, s2, pkout, pout, sout)
PTR         pk1;
i4         p1;
i4         s1;
PTR         pk2;
...
i4         *sout;
{
	...
	char	pktmp[(CL_MAX_DECPREC+1)/2];
	    ...
	    MEcopy(pk2, (u_i2)(p2 / 2 + 1), (PTR)pktmp);
            ...
            MHpkadd_sim(pk1, p1, s1, pktmp, p2, s2, pkout, pout, sout);
            return;
        }
        else
        {
...

/*{
** Name: MHpkmul_sim() - Multiply two packed decimals.
**
** Description:
**	This routine is called by MHpkmul which is written in MACRO.  The
...
**	01-sep-89 (jrb)
**	    Made this into a subordinate routine to be called from MHpkmul when
...
**          This fixes b113059, INGSRV2964.
**	31-jan-2007 (kibro01)
**	    Backed out the equivalent of change 431041 (original bug 83402)
**	    which caused a precision of (p1+p2)%31 as opposed to documented
**	    behaviour of min(p1+p2,31).
**	14-feb-2007 (dougi)
**	    Replaced hard constant 31 with CL_MAX_DECPREC.
*/

VOID
MHpkmul_sim(pk1, p1, s1, pk2, p2, s2, pkout, pout, sout)
PTR         pk1;
i4         p1;
i4         s1;
PTR         pk2;
...

/*{
** Name: MHpkdiv_sim() - Divide two packed decimals.
**
** Description:
**	This routine is called by MHpkdiv which is written in MACRO.  The
**	sequence is: try using the VAX instruction to divide; if that
**	overflows, use this code; if that overflows then it's really an
**	overflow.
**
** Inputs:
...
**	    Initial creation and coding.
**	01-sep-89 (jrb)
**	    Made this into a subordinate routine to be called from MHpkdiv when
...
VOID
MHpkdiv_sim(pk1, p1, s1, pk2, p2, s2, pkout, pout, sout)
PTR         pk1;
i4         p1;
i4         s1;
PTR         pk2;
