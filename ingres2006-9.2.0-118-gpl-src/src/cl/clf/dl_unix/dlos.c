...
#include	"dlint.h"
    ...
    defined(sqs_ptx) || defined(ris_u64) || defined(rs4_us5) || \
    defined(nc4_us5) || defined(rmx_us5) || defined(sgi_us5) || \
    defined(sos_us5) || defined(usl_us5) || defined(i64_aix) || \
    defined(hp2_us5) || defined(LNX) || defined(a64_sol) || \
    defined(mg5_osx)
#include <dlfcn.h>
#endif
...
#endif	/* hp3 */
#if (defined(hp8_us5) || defined(hp8_bls)) && !defined(hp2_us5)
...
iiDLsymfunc( void *handle, char *symname, void **retaddr )
{
    ...
    defined(dg8_us5) || defined(su4_us5) || defined(usl_us5) || \
    defined(dgi_us5) || defined(sui_us5) || defined(axp_osf) || \
    defined(sqs_ptx) || defined(ris_u64) || defined(rs4_us5) || \
    defined(nc4_us5) || defined(rmx_us5) || defined(sgi_us5) || \
    defined(sos_us5) || defined(i64_aix) || defined(hp2_us5) || \
    defined(a64_sol) || defined(LNX) || defined(mg5_osx)
...
*/
    ...
    defined(sgi_us5) || defined(i64_aix) || defined(hp2_us5) || \
    defined(a64_sol) || defined(LNX) || defined(mg5_osx)
	{
		/*
...
#	if defined(en8_us5)
/*
** NOTE NOTE NOTE This Encore-specific code hasn't been tested yet!
** NOTE NOTE NOTE It worked on W4GL05, and I'm leaving it in, but you'll
** NOTE NOTE NOTE find that it won't compile [yet].
** NOTE NOTE NOTE If you fill in the "exename" and the "dl_libs", away you
** NOTE NOTE NOTE should go!
**			- jab 1/14/92
*/
	{
		...
		if (inited == FALSE)
		{
			if (initsyms(localareap->dl_exename) == 0)
			{
				SETCLERR(errp, DL_OSLOAD_FAIL, 0);
				return(DL_OSLOAD_FAIL);
			}
		...
		if ((ret = dynload(shlibname, &loadaddr,
			(long)0, localareap->dl_libs)) < 0)
		{
			SETCLERR(errp, DL_OSLOAD_FAIL, 0);
			return(DL_OSLOAD_FAIL);
		}
...
#	if defined(ris_us5)
	{
		...
		DLdebug("Trying to load %s\n", shlibname);
		cookie = localareap->dl_cookie =
			(void *)load(shlibname, /* flags = */	(unsigned int)1,
			(char *)NULL);
/*
** The cookie returned should be the R/S6000 "entry function" that was
** specified when this thing was created.
*/
		if (cookie == NULL)
		{
			SETCLERR(errp,	DL_OSLOAD_FAIL, 0);
			if (DLdebugset == 1)
			{
				...
				buf[0] = "execerror";
				buf[1] = shlibname;
				if (loadquery(L_GETMESSAGES, &buf[2], sizeof(buf)-8) == -1)
					perror("loadquery");
				else
				{
					int pid;
					if ((pid = fork()) == 0)
					{
						execvp("/usr/sbin/execerror", buf);
						exit(1);
					} else if (pid == -1)
					{
						DLdebug("Cannot fork to show error!\n");
					} else {
		...
		lookupfcn = (int (*)()) cookie;
		haslookup = TRUE;

		DLdebug("Cookie was 0x%p\n", lookupfcn);
		if (loadbind(0, (void *)lookupfcn, (void *)main) == -1)
		{
			perror("loadbind");
			SETCLERR(errp,	DL_OSLOAD_FAIL, 0);
			return(DL_OSLOAD_FAIL, 0);
		}
	} /* the block of the ris_us5 code */
#endif /* ris_us5 */
/*
