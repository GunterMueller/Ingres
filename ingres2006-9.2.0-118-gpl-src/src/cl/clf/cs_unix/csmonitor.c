...
Water\t%d\r\n\r\nrdy mask %x state: %x\r\nidle quantums %d./%d. (%d.%%)",
		...
		buffer += STlength(buffer);
                ...
                STprintf(buffer, "    Multi-Process single-processor redispatches: %8d.\r\n",
                            Cs_srv_block.cs_smstatistics.cs_smsp_count);
                STprintf(buffer, "    Multi-Process multi-processor redispatches: %8d.\r\n",
                            Cs_srv_block.cs_smstatistics.cs_smmp_count);
                buffer += STlength(buffer);
                STprintf(buffer, "    Multi-Process non-server naps: %8d.\r\n",
