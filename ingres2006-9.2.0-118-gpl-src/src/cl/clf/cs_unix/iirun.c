...
# endif
	...
	/*
	** Why is this dependent on xCL_086_SETPGRP_0_ARGS?!?!
	** Linux kernel complains whe you set SIGCHLD to be ignored
	** and then call wait(). Call the handler() on Linux.
	*/
#if !defined(xCL_086_SETPGRP_0_ARGS) && !defined(ris_u64) \
    && !defined(rs4_us5) || defined(LNX)
    	EXsetsig( SIGCHLD, handler ); /* BSD */
#else
    	EXsetsig( SIGCHLD, SIG_IGN ); /* SYSV */
#endif
