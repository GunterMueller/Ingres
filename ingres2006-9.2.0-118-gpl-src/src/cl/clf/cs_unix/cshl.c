...
# if defined(sur_u42) || defined(sco_us5) || defined(sqs_ptx) || \
...
/*
NO_OPTIM = su4_us5 su4_u42 dr6_us5 su4_cmw i64_aix
*/
...
FUNC_EXTERN VOID    CS_dump_stack();	/* dump stack trace of a session */
...
# if defined(su4_u42) || defined(su4_cmw) || defined(su4_us5) || \
     defined(dr6_us5)
static VOID    	    CS_su4_dump_stack();/* sun4 support for CS_dump_stack */
# endif /* su4_u42 su4_cmw su4_us5 dr6_us5 */
#ifdef axp_osf
  VOID CS_axp_osf_dump_stack(); /* Digital UNIX support for CS_dump_stack */
#endif /* axp_osf */

# if defined(hp8_us5) && !defined(HPUX_DO_NOT_INCLUDE)
...
# if defined(sun_u42) || defined(su4_u42) || \
     defined(dr6_us5) || defined(hp3_us5) || \
     defined(su4_us5) || defined(su4_cmw)
# define gotit
    if (scb &&
...
# if defined(sur_u42) || defined(sco_us5) || defined(ds3_ulx) || \
...
# define gotit
    if (scb && scb->cs_registers[CS_SP] <=
			(long)scb->cs_stk_area + sizeof(CS_STK_CB) + 64)
# endif /* defined(ris_us5) || ( defined(rs4_us5) && !defined(ADD_ON64)) */
...
# if defined(i64_aix) || defined(i64_lnx) || defined(a64_lnx) || \
...
# if defined(sco_us5) || \
    (defined(int_lnx) && defined(OS_THREADS_USED)) || \
    (defined(int_rpl) && defined(OS_THREADS_USED)) || \
...
#if defined(su4_u42) || defined(sun_u42) || defined(dr6_us5) || \
    defined(su4_us5) || \
    defined(usl_us5) || defined(ris_us5) || defined(su4_cmw) || \
    defined(axp_osf) || defined(rs4_us5) || defined(ris_u64) || \
...
#ifdef	notdef
    gen_Psem(ME_page_sem);
    return (MEfree_pages((PTR)stk_hdr, stk_hdr->cs_size/ME_MPAGESIZE,
			 &errcode));
    gen_Vsem(ME_page_sem);
#endif
}
...
**
** Description:
**	Dummy context switch function, only needed for to satisfy linker
**	for Mac OSX as we don't support INTERNAL_THREADS. 
**
** History:
**	Jun-2005 (hanje04)
**	    Created.
...
** Description:
**	Dummy context switch function, only needed for to satisfy linker
**	for PowerPC Linux as we don't support INTERNAL_THREADS. 
**
** History:
**	26-Mar-2007 (hanje04)
**	    Created.
...
CS_SCB		*scb;
{
	...
	if (Cs_srv_block.cs_mask & CS_ACCNTING_MASK)
	    ...
	    if (NMloc(FILES, FILENAME, "iiacct", &loc) != OK)
	    {
	    	ac_ifi = -4;
...
# endif /* su4_u42 su4_us5 su4_cmw dr6_us5 */

# if defined(hp8_us5) && !defined(HPUX_DO_NOT_INCLUDE) && !defined(i64_hpu)
...
# endif /* ris_u64 || ( defined(rs4_us5) && defined(ADD_ON64) )  */
...
# ifdef LP64
# define EXCEPT_FMT "%-8.8s::[%-16.16s, %016.16lx]: pid %d:"
# define SYMBUF_FMT "[%016lx]"
# define STATIC_TRACE_FMT "%d: %016lx %s <static function, no info available>"
# define STACK_TRACE_FMT "%d: %016lx %s(%016lx %016lx %016lx %016lx)"
# else
# define EXCEPT_FMT "%-8.8s::[%-16.16s, %08.8x]: pid %d:"
# define SYMBUF_FMT "[%08x]"
# define STATIC_TRACE_FMT "%d: %08x %s <static function, no info available>"
# define STACK_TRACE_FMT "%d: %08x %s(%08x %08x %08x %08x)"
# endif
VOID
