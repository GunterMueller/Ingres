...
**
**  Output:
**	CL_SYS_ERR *err_code	error code structure
**	Return:
**	    STATUS  OK
**		    ER_BADOPEN
**
...
CL_SYS_ERR *err_code;
{
    ...
    */
	...
	if (	(status = SIread(*file,(i4)size,&rcount,pbuf)) != OK
	    &&	status != ENDFILE
	   )
	{
	    err_code->error = status;
	...
	{
	    /*
	    ** if the count that read and the count that had read are
	    ** differrence, return error code.  (Say what?)
	    */
	    if (size != rcount)
	    {
		ret_stat = ER_BADREAD;
...
CL_SYS_ERR	*err_code;
{
    ...
    if ((i4)errnum < 0 || (i4)errnum >= sys_nerr)
	return (ER_BADPARAM);
    
    len = STlength(msgp = sys_errlist[errnum]);
    if (len > msg_desc->desc_length - 1)
    {
    	/* copy the message to msg descriptor */
	(VOID)STlcopy(msgp,msg_desc->desc_value,msg_desc->desc_length - 1);
	return (ER_TRUNCATED);
    }
    /* copy the message to msg descriptor */
    (VOID)STlcopy(msgp,msg_desc->desc_value,len);
