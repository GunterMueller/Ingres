...
GLOBALDEF i4		Di_direct_io = FALSE;
...
** Di_zero_buffer will be aligned to an ME_MPAGESIZE boundary.
*/
...
GLOBALDEF CSSL_CB	*Di_slave;	/* slave control block */
...
# ifdef OS_THREADS_USED
        ...
        if ( Cs_srv_block.cs_max_active > 0 )
	{
	    ...
	    */
		...
		{
/* Use FD-per-thread on Solaris only.  Turn it on for Linux once we get
** epoll installed and can remove the dratted 1024-fd limit.  Might be
** useful for other Unixes, but needs testing.  (schka24 Sep-2005)
*/
#if !defined(su4_us5) && !defined(su9_us5) && !defined(a64_sol)
			TRdisplay("%@ Dilru_init: FD Thread affinity is not (yet) supported on this platform\n");
#endif
		    if ( Di_async_io )
...
# endif /* OS_THREADS_USED */
	...
	/*
	** If not a pseudo-server, tell if directio is on or off.
	** DIRECTIO is useful both in the servers and slaves.
	** The slaves, if any, are told to use DIRECTIO via the
	** file properties sent from the server.
	*/
	Di_direct_io = FALSE;
        if ( Cs_srv_block.cs_max_active > 0 )
	{
	    status = PMget("ii.$.$.$.direct_io",&str);
		...
		{
		    Di_direct_io = TRUE;
		    TRdisplay("%@ DIlru_init: direct_io is ON\n");
		}
		else
		    TRdisplay("%@ DIlru_init: direct_io is OFF\n");
	    }
	}
	...
	else
	{
	    ...
	    */
		...
		if ( status == OK )
		{
		    ...
		    if (bufsize < 32 * 1024)
			bufsize = DI_ZERO_BUFFER_SIZE;	/* Silent lower limit */
		}
		status = OK;		/* Ignore "no such param" */
		TRdisplay("DIlru_init: using zero-buffer size %d\n",bufsize);
	    }
	    bufsize += ME_MPAGESIZE;
	    Di_zero_buffer = MEreqmem(0, bufsize, TRUE, &me_status);
	    ...
	    Di_zero_bufsize = bufsize - ME_MPAGESIZE;
	} /* if no slaves */
    ...
    CL_ERR_DESC	*err_code)
{
	...
	*/
	...
	/*
	** If DIRECTIO hint but direct_io is not configured,
	** reset the hint (had to wait for DIlru_init() to resolve
	** Di_direct_io setting).
	*/
	if ( FPROP_DIRECT_REQ_CHECK(f->io_fprop) && !Di_direct_io )
	    FPROP_DIRECT_REQ_RESET(f->io_fprop);
