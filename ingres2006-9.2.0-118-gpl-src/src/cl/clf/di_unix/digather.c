...
DIgather_write( i4 op, char *gio_p, DI_IO *f, i4 *n, i4 page, char *buf, 
  VOID (*evcomp)(), PTR closure, i4 *uqueued, u_i4 *gw_pages, u_i4 *io_count, 
...
do_writev( DI_TGIO * tgio, CL_ERR_DESC *err_code )
{
	...
	*/
...
#ifdef DEBUG_THIS_PUPPY
	{
	    ...
	    for (x = 0; x <= i; x++)
	    {
	TRdisplay("%@ %x do_writev: %t todo %d fd %d i=%d iovec[%d] pages %d-%d base %x len %d\n",
		    tgio->tgio_scb->cs_self, 
		    f->io_l_filename, f->io_filename,
		    tgio->tgio_queued - j,
		    diop->di_fd, i,x, offset / f->io_bytes_per_page, 
		    ((offset + iov[x].iov_len) / f->io_bytes_per_page) - 1,
...
#ifdef DEBUG_THIS_PUPPY
    TRdisplay("%@ %x do_writev: %d write requests completed using %d(%d) writev, %d(%d) write\n",
		tgio->tgio_scb->cs_self, 
		tgio->tgio_queued, 
