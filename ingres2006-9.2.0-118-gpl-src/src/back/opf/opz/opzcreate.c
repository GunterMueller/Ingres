...
{	/* found a var node so create a joinop attribute and relabel node */
	...
	found = FALSE;
	for (selectp = subquery->ops_vars.opv_base->opv_rt[joinopvar]->
		opv_subselect;
	...
	{	/* traverse list of subselects referenced in subquery and
	    ** check if a joinop attribute has already been assigned */
		...
		{	/* assign a new joinop attribute to this node */
		    ...
		    if ((   (subquery->ops_vars.opv_base->opv_rt[joinopvar]
				->opv_grv->opv_created == OPS_FAGG)
			    ||
			    (subquery->ops_vars.opv_base->opv_rt[joinopvar]
				->opv_grv->opv_created == OPS_RFAGG)
			)
			&&
			(subquery->ops_vars.opv_base->opv_rt[joinopvar]
				->opv_grv->opv_gquery->ops_mask & OPS_VOPT)
			)
		    {
#if 0
			subquery->ops_mask |= OPS_VFIXUP;   /* extra traversal
					** required to resolve equal op nodes since
					** datatypes have changed */
#endif
			opz_varfixup(subquery->ops_vars.opv_base->opv_rt[joinopvar]
				->opv_grv->opv_gquery, nodep);
		    }
		    selectp->opv_atno = opz_addatts(subquery, joinopvar,
    ...
    */
	...
	case PST_UOP:
	{
		...
		!(  subquery->ops_global->ops_cb->ops_server->opg_ifnull
		    ...
		    nodep->pst_sym.pst_value.pst_s_op.pst_opno
		    &&
		    subquery->ops_oj.opl_cojid != OPL_NOOUTER
		))
