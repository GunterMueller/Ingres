	...
	OPS_STATE	   *global)
{
	...
	** a cartesean product is expected for the result of the aggregate */
	...
	if (subquery->ops_agg.opa_projection)
	{
		...
		{
		    if (varno == OPV_NOGVAR)
			varno = qnodep->pst_right->pst_sym.pst_value.pst_s_var.pst_vno;
	...
	for (covar = -1; (covar = BTnext((i4)covar, (char *)localmap, 
	    (i4)BITS_IN(*localmap)))>=0;)
	{
	    ...
	    for (localvar= covar; (localvar = BTnext((i4)localvar, (char *)localmap, 
		(i4)BITS_IN(*localmap)))>=0;)
	    {
		RDR_INFO	    *local_rdfp;
...
{
    OPS_SUBQUERY	*usq, *psq, *fsq, *msq, *sq1;
	...
	OPS_STATE          *global)
{
    ...
    */
	...
	subquery->ops_gentry =
	subquery->ops_result = qheader->pst_restab.pst_resvno; /* use parser 
                                            ** result variable
...

{
	...
	*isunion = TRUE;	/* tell folks it's a union query */
	if (usq->ops_sqtype != OPS_VIEW) 
	    return(FALSE);	/* last must be VIEW (as far as I know */
	subquery = usq;		/* reset subquery to end of union for 
				** recursive call */
    }

    if (subquery->ops_next && subquery->ops_next->ops_sqtype != OPS_MAIN)
     if (!opa_suckdriver(global, subquery->ops_next, mainsq, FALSE, isunion)) 
	...
	*mainsq = subquery->ops_next; /* save for extra suck call */
	...
	(subquery->ops_agg.opa_concurrent && *isunion) ||
...

{
	...
	** additional restreictions to promote. */
	if (gotvars && subquery->ops_sqtype != OPS_PROJECTION &&
		subquery->ops_sqtype != OPS_UNION &&
		subquery->ops_sqtype != OPS_VIEW &&
...

{
	...
	** from the same table is. */
	if ((vno = n1->pst_sym.pst_value.pst_s_var.pst_vno) == 
						subquery->ops_gentry)
	...
	/* Last check - don't do it if predicate involves a "generated"
...

{
    ...
    ** VAR array as an equivalent node. */
	...
	if ((*nodep)->pst_left) 
	    opa_suckeqc(global, aggvar, vararray, 
				resdomp, &(*nodep)->pst_left, firstcall);
				/* recurse on left */
	if ((*nodep)->pst_right) 
	    opa_suckeqc(global, aggvar, vararray, 
				resdomp, &(*nodep)->pst_right, firstcall);
	...
	** var to the array anchored by pst_rsno of resdom. */
	...
	if (firstcall && var1->pst_sym.pst_value.pst_s_var.pst_vno == aggvar)
	 for (n1 = resdomp, i = 0; resdomp && i < 32 &&
	...
	** it's eligible for copying. */
	...
	if ((*nodep)->pst_sym.pst_value.pst_s_var.pst_vno == aggvar)
	 if (global->ops_qheader->pst_rngvar_count > aggvar &&
		(global->ops_qheader->pst_rangetab[aggvar] == NULL ||
		BTcount((char *)&global->ops_qheader->pst_rangetab[aggvar]
		  ->pst_inner_rel, (i4)BITS_IN(PST_J_MASK)) != 0)) 
		return(FALSE);		/* don't promote restrs on inners */
	 else for (n1 = resdomp; n1 && n1->pst_sym.pst_type == PST_RESDOM;
		n1 = n1->pst_left)	/* loop over RESDOMs */
	 {
	    ...
	    ** of how the VAR node is found. */
		...
		if (subquery->ops_sqtype == OPS_VIEW && subquery->ops_union)
		    vsq = subquery;
		else if (subquery->ops_sqtype == OPS_UNION)
		{
		    for (vsq = global->ops_subquery; vsq; vsq = vsq->ops_next)
			if (vsq->ops_sqtype == OPS_VIEW && vsq->ops_union)
			{
			    for (usq = vsq->ops_union;usq; usq = usq->ops_union)
				if (usq == subquery)
				{
				    found_view = TRUE;
				    break;
		...
		*gotvar = TRUE;
		...
		n1->pst_sym.pst_value.pst_s_rsdm.pst_rsno ==
		(*nodep)->pst_sym.pst_value.pst_s_var.pst_atno.db_att_id)
	    {	/* Can't copy back an agg or binary op  - it isn't computed 'til
...
# ifdef OPT_F001_DUMP_QTREE
    ...
    {
	opu_qtprint( qheader->pst_qtree );
        /* print query tree given the root */
    }
# endif
