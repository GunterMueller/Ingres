...
static DB_DATA_VALUE	tid_dv = {NULL, DB_TID8_LENGTH, DB_INT_TYPE, 0};
    ...
    ** recurses on the left side of ANDs and iterates on the right. */
	...
	switch ((*nodep)->pst_sym.pst_type) {
	  ...
	  case PST_NOT:
		...
		** be transformed, and do it. */
		...
		nodep1->pst_sym.pst_value.pst_s_op.pst_opno ==
			global->ops_cb->ops_server->opg_eq
		&&
		nodep1->pst_sym.pst_value.pst_s_op.pst_opmeta == PST_ANY_SUBSELECT
		&&
		nodep1->pst_sym.pst_value.pst_s_op.pst_joinid == PST_NOJOIN
		&&
	  ...
	  case PST_BOP:
		...
		&&
		(*nodep)->pst_sym.pst_value.pst_s_op.pst_opmeta == PST_ALL_SUBSELECT
		&&
...

{
    ...
    *gotone = FALSE;			/* init */
    ...
    for (outer_root = header->pst_qtree; 
	...
	** (joinids) when only one is required. */
	...
	if (outer_root->pst_sym.pst_value.pst_s_root.pst_qlang == DB_SQL)
	 for (resdom = outer_root->pst_left;
	  ...
	  if (resdom->pst_right && 
		resdom->pst_right->pst_sym.pst_type == PST_AGHEAD)
	  {
	    opa_subsel_search(global, &resdom->pst_right->pst_right, 
		header, outer_root, resdom->pst_right, gotone, firstag);
	    firstag = FALSE;
	  }
    }
