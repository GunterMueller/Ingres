            ...
            {   /* the assumption is that if an aggregate has been substituted
                ...
                ** tuples */
                    ...
                    (subquery->ops_root->pst_right->pst_sym.pst_type == PST_QLEND))
                {
                    ...
                    /* 76337 - joinid has not been inited, mark it as such */
        ...
        PST_QNODE          *root)
{
	...
	for (nodep = resdomp->pst_right; nodep; nodep = nodep->pst_left)
	{
		...
		(nodep->pst_sym.pst_value.pst_s_op.pst_opno ==
			subquery->ops_global->ops_cb->ops_server->opg_ifnull))
	    {
...
{   /* an SQL query may have a variable in the from list
    ...
    ** in the from list here */
        ...
        )
    {
        ...
        else
        {
            ...
            (VOID) opv_parser(global, from_grv, OPS_MAIN, TRUE, TRUE, TRUE); /* init
                                    ** the global range table info
                                    */
...
#if 0
    if (global->ops_astate.ops_uview)
    {   /* place union views into context of parent node */
        OPS_SUBQUERY    **sq_pp;
        for (sq_pp = &global->ops_astate.ops_uview;
           *sq_pp; sq_pp = &(*sq_pp)->ops_next)
           ;
        *sq_pp = gstate->opa_uvlist;
        gstate->opa_uvlist = global->ops_astate.ops_uview;
    ...
    if (BTcount((char *)&new_subquery->ops_agg.opa_views, OPV_MAXVAR))
    {
        ...
        BTor(OPV_MAXVAR, (char *)&new_subquery->ops_agg.opa_views,
                        (char *)&global->ops_astate.opa_allviews);
                                    ...
                                    ** so far */
        MEcopy((char *)&new_subquery->ops_agg.opa_views, sizeof(tempmap),
                                (char *)&tempmap);
        BTnot(OPV_MAXVAR, (char *)&tempmap);
        BTand(OPV_MAXVAR, (char *)&tempmap, (char *)from_map);
    ...
    */
                                        ...
                                        ** non-flattened query */
    {
        ...
        MEcopy((char *)&new_subquery->ops_aggmap, sizeof(tempmap), 
                        (char *)&tempmap);
        BTnot(OPV_MAXVAR, (char *)&tempmap);
        if (new_subquery->ops_root->pst_sym.pst_value.pst_s_root.pst_qlang == DB_SQL)
        {
            ...
            BTor(OPV_MAXVAR, (char *)&new_subquery->ops_agg.opa_views, (char *)&fromlist);
                                        ...
                                        ** in query tree */
            BTand(OPV_MAXVAR, (char *)&tempmap, (char *)&fromlist);
        ...
        else
        {
            ...
            BTor(OPV_MAXVAR, (char *)&new_subquery->ops_root->pst_sym.pst_value.
                        pst_s_root.pst_rvrm, (char *)&fromlist);
            BTor(OPV_MAXVAR, (char *)&new_subquery->ops_agg.opa_rview, (char *)&fromlist);
                                        /* due to linkage requirements of
                                        ** of quel aggregates, all views should be
                                        ** querymoded */
        }

        BTand(OPV_MAXVAR, (char *)&tempmap, 
    ...
    {   /* remove duplicated outer join indicators from parent, which is caused
        ...
        ** aggregate and parent from list */
        OPL_PARSER      *pinnerp;
        OPL_PARSER      *pouterp;
        BTnot((i4)BITS_IN(temp_ojmask), (char *)&temp_ojmask);
        BTnot((i4)BITS_IN(temp_ijmask), (char *)&temp_ijmask);
        ...
        {
            BTand((i4)BITS_IN(temp_ojmask), (char *)&temp_ojmask,
                (char *)&pouterp->opl_parser[from_grv]);
            BTand((i4)BITS_IN(temp_ijmask), (char *)&temp_ijmask,
	...
	** subquery queries */
    ...
    {   /* if this is an SQL node and not the main query then
        ...
        */
                ...
                outer = outer->ops_agg.opa_father)
            {
                ...
                BTor((i4)(BITS_IN(tempcorelated)),
                    ...
                    (char *)&tempcorelated); /* part of b40059 fix,
                                        ...
                                        */
                BTnot((i4)BITS_IN(tempcorelated), 
                    (char *)&tempcorelated); /* bitmap of all
                                        ** variables which could be
                                        ** corelated */
                BTand((i4)BITS_IN(corelated), (char *)&tempcorelated,
        ...
        if (((sqtype == OPS_RFAGG) || (sqtype == OPS_FAGG))
            &&
        ...
        PST_QNODE          *nodep)
{
    ...
    DB_ERROR        error; /* error code from resolution routine
                            */
    res_status = pst_resolve((PSF_SESSCB_PTR) NULL, /* "parser session
    ...
    if (res_status != E_DB_OK)
        opx_verror( res_status, E_OP0685_RESOLVE, error.err_data);
}
#if 0

/*{
...

/*{
** Name: opa_flatten    - flatten = ANY or EXISTS subselect
**
** Description:
...
** conflicts which the consistency checks made */
    ...
    }
    {
        ...
        opa_grtable(gstate, subselect, OPS_DREMOVE, FALSE, &viewmap,
            ...
            ** which are visible to the parent */
            ...
            if (qnode->pst_right)
                opa_cosubquery(global, &scope, qnode->pst_right); /* set the
                                        ...
                                        ** be resolved */
            MEcopy ((PTR)&qnode->pst_sym.pst_value.pst_s_root.pst_tvrm,
                sizeof(temp), (PTR)&temp); /* Invert the from list to
                                        ** reset all variables which are
                                        ** resolved at this scope level
                                        */
            BTnot((i4)(BITS_IN(temp)), (char *)&temp);
            BTand((i4)(BITS_IN(scope)), (char *)&temp, (char *)&scope);
                                        ...
                                        */ )
{
...
#endif
    {
        ...
        ** be simulated, each has different processing */
        ...
        if ((   exists_ptr->pst_left->pst_sym.pst_value.pst_s_root.pst_mask1 
                &
                PST_1CORR_VAR_FLAG_VALID
            )
            &&
            (   (   (   exists_ptr->pst_left->pst_sym.pst_value.pst_s_root.pst_mask1
                        & 
            ...
            &&
            !(exists_ptr->pst_sym.pst_value.pst_s_root.pst_mask1 & PST_5HAVING)
        )
        opx_error(E_OP0A91_CORELATED);  /* PSF's corelation test should match OPF's,
                                ** eventually this test can be eliminated */
    }
#if 0
    else
                                            ...
                                            ** for distributed */
    {
        ...
        {   /* place PST_QLEND on the subselect */
            ...
            ** written */
            ...
            exists_ptr->pst_sym.pst_value.pst_s_op.pst_opmeta = PST_ANY_SUBSELECT;
        }
        else
        {
...
# endif
...

    {
        ...
        */
        ...
        if (opmeta == PST_ANY_SUBSELECT)
            tree->pst_sym.pst_value.pst_s_op.pst_opmeta 
                = PST_ALL_SUBSELECT;
        else if (opmeta == PST_ALL_SUBSELECT)
            tree->pst_sym.pst_value.pst_s_op.pst_opmeta 
                = PST_ANY_SUBSELECT;
    }
}
...
*/
        ...
        gstate->opa_gflatten)
    {
        ...
        {
	/* ONE_SUBSELECT should NOT be flattened as it subverts the cardinality
	** check required for singleton subselects. I had move this case to
	** default to avoid the flattening, but that triggers "scope" errors
	** elsewhere (particularly with aggregate subselects), so the code
	** is changed back until we can figure it out. */
        case PST_ONE_SUBSELECT:	/* don't flatten scalar sq's */
