...
**
** Description:
**	 Reformats no longer necessary since the parent will contain this
**       information.
**      
**       General:
**      
	...
	bool               keyed_only)
{
	...
	*/
    ...
    if ((prcost = opn_cost(subquery, dio, cpu))  >= subquery->ops_cost)
	return;			    /* do not do anything if the cost is larger
...
# endif
	...
	** by TRUE multi-attribute processing */
	...
	if (distributed)
	{
		...
		if (!useful 
		    ||
		    opd_bestplan(subquery, cop, subquery->ops_msort.opo_topsort, 
	...
	else if (root_flag)
	{
		...
		** avoid a sort node */
		...
		if (opn_checksort(subquery, &subquery->ops_bestco, subquery->ops_cost,
			    ...
			    cop,            (OPL_BMOJ *)NULL ))
		    return;		/* return if added cost of sort node, or creating
