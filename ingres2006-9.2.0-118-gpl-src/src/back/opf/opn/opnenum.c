...
static bool
rule_halloween(OPS_SUBQUERY       *subquery);
	...
	OPS_SUBQUERY       *subquery)
{
    ...
    */
	...
	(   (subquery->ops_duplicates == OPS_DREMOVE)
		...
		||
		(   (subquery->ops_vars.opv_base->opv_rt[0]->opv_grv->opv_created != OPS_FAGG)
		    &&
		    (subquery->ops_vars.opv_base->opv_rt[0]->opv_grv->opv_created != OPS_RFAGG)
	...
	** expressions */
    ...
    {
	dupsort |= rdupsort;
	subquery->ops_msort.opo_topsort = TRUE;
	...
	if (possible && order_by)
	{
		...
		** convert this to a non-sorted query */
		...
		{   /* do not need sort node if order by without
		    ** a distinct is used */
		    subquery->ops_msort.opo_mask &= (~OPO_MTOPSORT);
		    subquery->ops_msort.opo_topsort = FALSE;
		}
	    }
	    else
	    {
	...
	OPS_SUBQUERY       *subquery)
{
	...
	** (kibro01) b118289
	*/
        storage_type = subquery->ops_vars.opv_base->opv_rt[0]->opv_grv->
				opv_relation->rdr_rel->tbl_storage_type;
    }
...
onceMorewithFeeling:
	...
	*/
    ...
    if ((global->ops_cb->ops_override & PST_HINT_NOGREEDY)
	||
	...
	!(global->ops_cb->ops_override & PST_HINT_GREEDY)))
     sigstat = opn_tree(subquery);      /* Star or no op247 or < 5 base tables
					** or < 10 tables/indexes:
					** do old enumeration */
     else 
     {
	sigstat = opn_newenum(subquery, first);  /* otherwise do new enumeration */
	if (sigstat == OPN_SIGEXIT)
	    sigstat = opn_tree(subquery);  /* new technique failed - try old */
     }
...

/*{
** Name: rule_halloween	- halloween processing for (delete) rules
**
** Description:
**      This routine tries to limit the cases where we need to add
**      a prefetch to the query plan for the halloween problem.
**      Non-system rules following the delete could potentially cause 
...
** Outputs:
**	Returns:
**	    TRUE			Potential halloween problem
**          FALSE			NO halloween problem with rules
**	Exceptions:
**	    none
**
** Side Effects:
