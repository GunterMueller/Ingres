...

{
    ...
    ** that passes subsequent heuristic tests. */
    ...
    /* Simple test allows CP joins as long as there are 6 or fewer entries.
    ** Otherwise, queries with mutually exclusive (joinwise) subsets of tables
    ** may not find a plan. This could be more finely tuned, but is done
    ** this way for simplicity. To improve the solution would likely require 
    ** returning a flag to the caller so we can remember the state we're in 
    ** (and the tests to apply) while we're in here. By the time we're down
    ** to 6 entries in the range table, we ought to pick a good plan anyway. */
    if (n <= 6)
	skipCPs = FALSE;
    else skipCPs = TRUE;
	...
	**  - var doesn't join other vars in combination but does join 
	**    PRIMARY vars outside of combination.
	...
	** to test final condition (but avoided if CPs are ok). */
	if (skipCPs)
	 for (i = 0; i < k && !failure; i++)
	{
	    MEcopy((char *)&evarp[emap[cvector[i]]].opn_varmap, sizeof(OPV_BMVARS),
		(char *)&workmap);
	    BTnot(maxvars, (char *)&workmap);
	    BTand(maxvars, (char *)&combmap, (char *)&workmap);
	    BTand(maxvars, (char *)&evarp[emap[cvector[i]]].opn_joinable,
		(char *)&workmap);	/* vars in current comb this one joins to */
	    if (BTnext(-1, (char *)&workmap, maxvars) >= 0)
		continue;		/* joins to combination */
	    MEcopy((char *)&restmap, sizeof(OPV_BMVARS), (char *)&workmap);
	    BTand(maxvars, (char *)&evarp[emap[cvector[i]]].opn_joinable,
		(char *)&workmap);	/* vars outside curr comb this one joins to */
	    if ((j = BTnext(-1, (char *)&workmap, maxvars)) >= 0 &&
			j < subquery->ops_vars.opv_prv)
		failure = TRUE;		/* joins to a primary in the rest,
					** but not combination */
	}
	if (failure)
	    continue;
...
/*
	BTor(subquery->ops_vars.opv_prv, (char *)&evarp[iemap[i]].opn_varmap, 
			(char *)&primset);
*/
    ...
    ** set. */
	...
	/* If this is last loop (la_count == ert_degree), we must verify that
	** 2 of the entries don't conflict - for example, a replacement index
	** and its matching base table. In that case, combinations may only
	** be of la_count-1, so we simply decerment la_count. */
	if (la_count >= ert_degree)
	{
	    OPV_VARS	*varp;
	    OPV_IVARS	baseno = -1;
	    OPV_IVARS	ixno = -1;
	    ...
	    for (i = 0; i < la_count; i++)
	     ...
	     {
		varp = subquery->ops_vars.opv_base->opv_rt[evarp[i].
							u.v.opn_varnum];
		if (varp->opv_mask & OPV_CINDEX)
		    ixno = varp->opv_index.opv_poffset;
		else if (varp->opv_mask & OPV_CPLACEMENT)
		    baseno = varp->opv_index.opv_poffset;
		else baseno = evarp[i].u.v.opn_varnum;

		if (baseno == ixno && baseno != -1)
		{
		    la_count--;
		    break;
		}
	...
	** set. */
			...
			la_count, &firstcomb))
	{
	    ...
	    ** each into the eqcmap. */
		...
		/* See if permutation covers OJs set of inners & outers. */
		if (!BTsubset((char *)ojp->opl_onclause, (char *)&pvarmap, varcount))
		    continue;
	...
	** really big trouble! */
	...
	if (subquery->ops_bestfragco == (OPO_CO *) NULL)
	{
		...
		** Failure may be the result of opn memory management quirks as
		** yet undebugged. */
		first_noplan = FALSE;
	...
	else if (((pvarcount < 0) ? subquery->ops_vars.opv_prv : pvarcount) <= 
				(fvarcount-ert_degree+la_count))
	    doneCO = opn_fullplan(subquery, &primset);
						/* not last iteration, but 
						** bestplan is still complete */
	if (doneCO && final)
	{
	...
	** saved. */
	threeleaf = (doneCO || subquery->ops_bestwholetree);
	...
	** condensed range table. */
	...
	for (i = -1; (i = BTnext(i, (char *)&ixmap, varcount)) >= 0; )
	{
	    ...
	    ** range table map. */
		...
		/* Following test is removed. I'm not sure why it was there
		** and it causes OP0287's in the presence of OJ indexes
		if (!OJsubset)		** only decr when main sq */
		    ert_degree--;
		evarcount--;
		break;
	     }
	...
	TRformat(opt_scc, (i4 *)global,
	    (char *)&global->ops_trace.opt_trformat[0],
	    (i4)sizeof(global->ops_trace.opt_trformat),
	    "EVAR dump at degree:%d - end of fragment\n\n", ert_degree);
	opt_evp(evarp, (ert_degree > 0) ? ert_degree-1 : evarcount);
    }
	...
	i4		   enumcode)
{
    ...
    bool	ojanal = FALSE, done, gotone, fullboth;
    ...
    ** initialize entries from subquery range table. */
    ...
    la_count = subquery->ops_lacount;		/* number of tables in jtree */
    ...
    ** the final plan. If no OJs to precompile, just execute once. */
    do {
	OPV_BMVARS ivmap, not_ivmap, workmap;
	JDESC	*jptr;
	...
	else
	{
		...
		for (i = 0, j = -1; (j = BTnext(j, (char *)&jptr->ojivmap,
			varcount)) >= 0; i++)
		    iemap[i] = j;
		/* Save ivmap (and inverse) for later. */
		MEcopy((char *)&jptr->ojivmap, sizeof(OPV_BMVARS),
				(char *)&ivmap);
		MEcopy((char *)&jptr->ojivmap, sizeof(OPV_BMVARS),
				(char *)&not_ivmap);
		BTnot(varcount, (char *)&not_ivmap);
	...
	** entries contain indexes to the modified evar array. */
	if (gotone)
	{
		...
		if ((jptr = &jdesc[i])->ojdrop)
		    continue;
		else if (BTsubset((char *)&ivmap, (char *)&jptr->ojivmap,
			varcount))
		{
		    /* This join covers the one just compiled - remove
		    ** inner join inputs from this join and (later) replace
		    ** by composite result. */
		    BTand(varcount, (char *)&not_ivmap,
					(char *)&jptr->ojivmap);
		    setres = TRUE;
		}
		/* Now look at each remaining entry in ojivmap and 
		** re-map it using oemap. */
		MEfill(sizeof(OPV_BMVARS), 0, (char *)&workmap);
		for (j = -1; 
		    (j = BTnext(j, (char *)&jptr->ojivmap, ecount)) >= 0; )
		{
		    for (k = 0; k < ecount; k++)
		     if (oemap[k] == j)
			break;
		    ...
		    ** extinguished by the same loop. */
		    BTclear(j, (char *)&jptr->ojivmap);
		    BTset(k, (char *)&workmap);
		}
		BTor(varcount, (char *)&workmap, 
					(char *)&jptr->ojivmap);

		if (setres)
		    BTset(rsltix, (char *)&jptr->ojivmap);
	    }
	}
