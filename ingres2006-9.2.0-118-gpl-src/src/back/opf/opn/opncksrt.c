	...
	OPL_BMOJ	   *filtermap)
{
	...
	** to deferred update semantics */
	...
	if ((   !(subquery->ops_msort.opo_mask & OPO_MRDUPS)
		...
		!(*sortrequired)
#ifdef OPL_OJFILTER
/* enable when filter processing is supported in OPC */
		||
		(oj_compatible = (subquery->ops_oj.opl_smask & OPL_OJSPLIT)!=0)
#endif
	    )
	    && 
	    ...
	    !(subquery->ops_global->ops_cb->ops_smask & OPS_MDISTRIBUTED))
	{
	    ...
	    *sortrequired = TRUE;
		...
		if (result == jordering) 
		{   if (oj_compatible)
		    {	/* need to check if outer join filters are needed */
		    }
		    else
		    {
			*sortrequired = FALSE;	    /* sort node can be avoided */
			/* need to check if outer join filters are needed and
			** can be satisfied by this ordering also */
		    }
		}
		else if (!single)
...
/* enable when filter processing is supported in OPC */
    ...
    {	/* need to verify that outer join duplicate handling is
	** correct, and generate a sort node if not */
	if (!BTsubset((char *)&subquery->ops_oj.opl_filterneeded,
	    (char *)filtermap,(i4)subquery->ops_oj.opl_lv))
	{   /* all outer joins which require filter checks have
	    ** been made so no further sort is needed */
	    BTxor((i4)subquery->ops_oj.opl_lv,
		(char *)&subquery->ops_oj.opl_filterneeded,
		(char *)filtermap);
	    if (*sortrequired)
	    {	/* check if filter sort is compatible with existing
		** required sort node */
	    }
	    *sortrequired = TRUE;
	}
	else
	    MEfill(sizeof(*filtermap), (u_char)0, (PTR)filtermap);
    }
#endif
