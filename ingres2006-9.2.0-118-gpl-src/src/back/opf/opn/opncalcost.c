...

/*{
** Name: opl_filterplacement	- check vars for filter
**
** Description:
**      Check if the placement of variables on the outer CO subtree are 
**      placed such that a filter can be used at this join node.
**	The outer relations must outer to all inner relations for this
**	outer join.  Also, no sorts which change the order of the tuples
**	are allowed since this affects the grouping for the filter. 
**
** Inputs:
**      subquery                        ptr to subquery being analyzed
**      cop                             ptr to current co node
**      ojp                             outer join descriptor
**
** Outputs:
**      inner_found                     TRUE if subtree contains an
**					inner
**
**	Returns:
**	    TRUE if relation placement will allow a filter to correct
**	    duplicate semantics for the outer join
**	Exceptions:
**
** Side Effects:
**
	...
	OPL_OUTER	    *ojp)
{
    ...
    if (cop->opo_sjpr == DB_ORIG)
    {
	if (BTtest((i4)cop->opo_union.opo_orig, (char *)ojp->opl_maxojmap))
	    *inner_found = TRUE;
	else
	    *outer_found = TRUE;
	retval = TRUE;
    }
    else if (opl_filterplacement(subquery, cop->opo_outer, inner_found, outer_found, ojp))
    {
	if (cop->opo_osort && *inner_found)
	    ;			/* return since sort node will change grouping
				** of tuples */
	else if (cop->opo_inner)
	{
	    bool	temp_inner;
	    bool	temp_outer;
	    temp_inner = FALSE;
	    temp_outer = FALSE;
	    if (opl_filterplacement(subquery, cop->opo_inner, &temp_inner, &temp_outer, ojp))
	    {
		if (*inner_found && temp_outer)
		    return (FALSE);	/* an outer relation occurs
				** positionally as an inner to an inner
				** relation, which prevents using a
				** filter */
		*inner_found |= temp_inner;
		*outer_found |= temp_outer;
		retval = TRUE;
	    }
	}
	else
...

/*{
** Name: opl_filtercheck	- check if filter condition satisfied
**
** Description:
**      The routine will check if a filter condition is satisfied.  
**	1) relations are placed such that appropriate tids are grouped
**	allowing the filter to be executed
**	2) the ordering on the outer used by the join can be used to 
**	order the tids in the outer relations so that the filter can
**	be executed
**
** Inputs:
[@PARAM_DESCR@]...
...
** History:
**      10-sep-93 (ed)
**          initial creation to correct outer join secondary index
**	    performance problem.
[@history_template@]...
*/
static VOID
...
{
    OPL_IOUTER	    maxoj;
    OPL_BMOJ	    needed;	/* map of outer joins which still 
				** need filtering */
    OPL_IOUTER	    ojid;	/* current outer join id being
				** analyzed */
    OPE_IEQCLS	    maxeqcls;
    OPV_IVARS	    maxvar;
    OPV_RT	    *vbase;
    ...
    /* determine which, if any, outer joins require filter actions
    ** at this node */
    MEcopy((PTR)&newcop->opo_outer->opo_maps->opo_filter,
	sizeof(*filtermap), (PTR)filtermap); /* get maps of filters
				** which have been completed from
				** left and right children */
    BTor((i4)maxoj, (char *)&newcop->opo_inner->opo_maps->opo_filter,
	(char *)filtermap);	/* map of outer joins which have had
				** filter actions satisfied */
    MEcopy((PTR)filtermap, sizeof(needed), (PTR)&needed);
    BTnot((i4)maxoj, (char *)&needed);
    BTand((i4)maxoj, (char *)&subquery->ops_oj.opl_filterneeded,
	(char *)&needed);	/* mask out outer joins which may
				** require a filter action */
    BTand((i4)maxoj, (char *)&np->opn_ojevalmap, 
	(char *)&needed);	/* opn_ojevalmap contains those 
				** outer joins which have been
				** totally evaluated within this
				** subtree
				** -needed - now contains outer joins
				** which have filter actions which
				** still need to be satisfied */
    for (ojid = -1; (ojid = BTnext((i4)ojid, (char *)&needed,
	(i4)maxoj))>=0;)
    {   /* for each outer join in the needed list determine if
	** filter requirements can be satisfied */
	OPL_OUTER	    *ojp;
	ojp = subquery->ops_oj.opl_base->opl_ojt[ojid];
	if (ojid == newcop->opo_union.opo_ojid)
	{   /* since the outer join is evaluated at this node
	    ** check if the outer relations are appropriately
	    ** in the outer tree
	    ** - only need to perform this check once at the 
	    ** final node at which this outer join is evaluated 
	    ** - if all relations of the tree are placed in 
	    ** infix order and all outer relations preceed all
	    ** inner relations for the outer join then all outer
	    ** tids will be grouped so a filter can be placed on
	    ** top of this node
	    */
	    bool	inner_found;
	    bool	outer_found;
	    ...
	    inner_found = FALSE;
	    outer_found = FALSE;
	    if (!BTtest((i4)ojid, (char *)&np->opn_ojinnermap)
		    /* since outer join id only appears once there
		    ** is not any duplicate handling problem, this
		    ** probably means the secondary index was not
		    ** used */
		||
		opl_filterplacement(subquery, newcop->opo_outer,
		    &inner_found, &outer_found, ojp))
	    {	
		BTset((i4)ojid, (char *)filtermap);
		continue;
	    }
	}
	if (newcop->opo_ordeqc >= 0)
	{   /* if an ordering is available check that the
	    ** tid attributes of the outer relations can be
	    ** ordered prior to the tid attributes of the
	    ** inner relations */
	    OPO_SORT		*sortp;	/* sort descriptor of 
				    ** ordering at this node */
	    bool		first_time; /* used to save an
				    ** MEcopy of eqcmaps */
	    OPV_IVARS		varno; /* current variable being
				    ** analyzed in the outer
				    ** subtree */
	    OPE_BMEQCLS		outereqcmap;

	    if (newcop->opo_ordeqc >= maxeqcls)
		sortp =  subquery->ops_msort.opo_base->
		    opo_stable[newcop->opo_ordeqc - maxeqcls];
	    else
		sortp = NULL;
	    /* check that all outer tid equivalence classes
	    ** are available in the ordering */
	    first_time = FALSE;
	    for (varno = -1; (varno = BTnext((i4)varno,
		(char *)&orlp->opn_relmap, (i4)maxvar))>=0;)
	    {
		if (!BTtest((i4)varno, (char *)ojp->opl_maxojmap))
		{   /* an outer relation found so tideqc should
		    ** be in the sort ordering */
		    OPO_COMAPS	    *maps;
		    maps = &vbase->opv_rt[varno]->opv_maps;
		    if (first_time)
		    {
			MEcopy((PTR)&maps->opo_eqcmap, sizeof(outereqcmap),
			    (PTR)&outereqcmap);
			first_time = FALSE;
		    }
		    else
			BTor((i4)maxeqcls, (char *)&maps->opo_eqcmap,
			    (char *)&outereqcmap);
		}	
	    }
	    BTand((i4)maxeqcls, (char *)&newcop->opo_outer->opo_maps->opo_eqcmap,
		(char *)&outereqcmap);	/* only interested in the outer
					** eqcls available at the immediate
					** child,... this must include the
					** tid eqcls since these are in
					** non-printing resdoms in target list*/
	    if ((   sortp
		    &&
		    !first_time
		    &&
		    BTsubset((char *)&outereqcmap, (char *)sortp->opo_bmeqcls,
			(i4)maxeqcls) /* all the referenced
					** outer equivalence classes need
					** to be in the ordering */
		)
		||
		(   !sortp
		    &&
		    BTtest((i4)newcop->opo_ordeqc, (char *)&outereqcmap)
		    &&
		    (BTcount((char *)&outereqcmap, (i4)maxeqcls) == 1)
		))
	    {   /* all outer equivalence classes are available
		** in the ordering, so traverse the suborderings
		** in the sortp ordering and determine if all
		** outer equivalence classes preceed all inner
		** equivalence classes */
		OPO_ISORT	    eqlist[2];
		OPO_EQLIST	    tempeqlist;
		OPO_ISORT	    *newp; /* ptr to new more restrictive
					** ordering used to support filter */
		OPO_ISORT	    *currentp; /* ptr to suborder component
					** of available ordering */
		OPE_IEQCLS	    maxsize; /* number of elements in 
					** tempeqlist */
		bool		    ordering_found;

		ordering_found = FALSE;
		if (sortp->opo_eqlist)
		    currentp = &sortp->opo_eqlist->opo_eqorder[0];
		else
		{
		    eqlist[0] = newcop->opo_ordeqc;
		    eqlist[1] = OPE_NOEQCLS;
		    currentp = &eqlist[0];
		}
		newp = &tempeqlist.opo_eqorder[0];
		maxsize = 0;
		for (;(*currentp) != OPE_NOEQCLS;
		    *(newp++) = *(currentp++))
		{   /* remove equivalence classes in ordering from
		    ** the map of outer equivalence classes until all
		    ** are eliminated or and inner equivalence class
		    ** is found... if all are eliminated then the
		    ** ordering can be used for a filter */
		    if (maxsize++ >= DB_MAXKEYS)
			break;
		    if (*currentp < maxeqcls)
		    {
			if (BTtest((i4)*currentp, (char *)&outereqcmap))
			    BTclear((i4)*currentp, (char *)&outereqcmap);
			else
			{
			    ordering_found = BTnext((i4)-1, (char *)&outereqcmap,
				(i4)maxeqcls)>=0;
			    break;
			}
		    }
		    ...
		    {
			OPE_BMEQCLS	    *subordering;
			subordering = subquery->ops_msort.opo_base->
			    opo_stable[*currentp - maxeqcls]->opo_bmeqcls;
			if (BTsubset((char *)subordering,
			    (char *)&outereqcmap, (i4)maxeqcls))
			    BTxor((i4)maxeqcls, (char *)subordering,
				(char *)&outereqcmap); /* remove outer eqcls
						** from map */
			else
			{
			    if (BTnext((i4)-1, (char *)&outereqcmap,
				(i4)maxeqcls) < 0)
				ordering_found = TRUE; /* previous loop removed
						    ** all outer eqcls */
			    else if (BTsubset((char *)&outereqcmap,
				(char *)subordering, (i4)maxeqcls))
			    {
				/* 2 new orderings need to be created to
				** split this subordering */
				*(newp++) = opo_fordering(subquery, &outereqcmap);
						    /* portion of ordering which
						    ** belongs to outer relations */
				BTxor((i4)maxeqcls, (char *)subordering,
				    (char *)&outereqcmap); /* remaining eqcls are from
						    ** inner relations only */
				if (maxsize++ >= DB_MAXKEYS)
				    break;
				*(newp++) = opo_fordering(subquery, &outereqcmap);
				currentp++;
				for (;maxsize++ <= DB_MAXKEYS;)
				{   /* copy remainder of ordering */
				    if (*currentp == OPE_NOEQCLS)
				    {
					*newp = OPE_NOEQCLS;
					ordering_found = TRUE;
					newcop->opo_ordeqc = opo_cordering(subquery, 
					    &tempeqlist, TRUE); /* replace ordering
						    ** with modified ordering so that
		...
		if (ordering_found)
		    BTset((i4)ojid, (char *)filtermap); /* ordering can be used
						    ** to support filter */
	    }
	}
    }
...

/*{
** Name: opl_cosearch	- search for better CO node
**
** Description:
**      Given the cost and ordering of a potential CO node, determine
**	if there is a subtree which is as good or better than the
**	potential CO node.  Given that the CO node uses the same
**	relations, calculates the same oj filters, and provides the
*	same output ordering for the parent.
**
** Inputs:
**      subquery                        ptr to subquery being analyzed
**      pcost                           cost of CO node to be added
**      subtp                           list of subtrees which contain
**					comparable CO trees
**      cartprod                        TRUE if this potential CO tree
**					is created with a cart prod
**      aggmap                          map of aggregates evaluated at
**					this potential CO node
**      new_ordeqc                      ordering available at output of
**					this potential CO tree
**      newstorage                      storage structure available at
**					this potential CO tree
**	trysorts			TRUE - if it a sort node
**					with this potential CO tree would
**					be useful.
**
** Outputs:
**      insertpp                        insertion point of potential CO
**					tree in saved work area
**      trysorts                        TRUE if this CO tree should be
**					considered for a sort node for
**					aggregate evaluation
**
**	Returns:
**	    TRUE - if no comparable CO can be found in the CO list
**	Exceptions:
**	    none
**
** Side Effects:
...
opl_cosearch(
	OPS_SUBQUERY       *subquery,
	OPN_SUBTREE	   *sbtp,
	OPN_SUBTREE	   **insertpp,
	OPO_COST	   pcost,
	bool		   cartprod,
	OPL_BMOJ	   *filtermap,
	OPO_ISORT	   new_ordeqc,
	OPO_STORAGE	   newstorage)
{
    /* determine if building a new CO node would be beneficial, i.e. is there
    ** a cheaper CO node which already exists */
    OPN_SUBTREE	*subtreep;	/* used to traverse related subtrees
				** which have comparable CO lists */
    bool	tooexpensive;	/* TRUE - if plan should not be added
				** since it is too expensive */    
    OPL_IOUTER	maxoj;		/* maximum number of outer joins */

    /* search existing list of CO nodes to determine if this ordering
    ** and cost for this set of aggregates is evaluated */
    subtreep = sbtp;		/* search thru list of subtrees which
				** evaluate different aggregates but 
				** otherwise have the same structure */
    maxoj = subquery->ops_oj.opl_lv; /* max number of outer joins */
    tooexpensive = FALSE;
    *insertpp = NULL;
    do 
    {
	OPO_CO             *cop;	/* current cost ordering subtree
					** being analyzed */
	bool		    ojsubset;	/* TRUE - if the candidate CO has
					** an oj subset of the list
					** of CO nodes in subtreep */
	bool		    ojsuper;	/* TRUE - if the candidate CO has
					** an oj superset of the list
					** of CO nodes in subtreep */
	ojsubset = BTsubset((char *)filtermap, (char *)&subtreep->opn_eqsp->opn_maps.opo_filter,
	    (i4)maxoj);
	ojsuper = BTsubset((char *)&subtreep->opn_eqsp->opn_maps.opo_filter, (char *)filtermap, 
	    (i4)maxoj);
	
	if (ojsubset  || ojsuper )
	{
	    bool	equal;  /* TRUE, if this is the
				** correct subtree for aggregate evaluation
				** - it has exactly the correct
				** aggregates available so
				** if a cheaper ordering exists then
				** this plan should not be used */
	    equal = ojsubset && ojsuper ;
	    if (equal)
		*insertpp = subtreep;
	    /* FIXME - if the CO list is empty then this may imply that
	    ** all CO nodes in this subtree are too expensive to use
	    ** which may imply that this CO node is too expensive for
	    ** use, or that sort nodes would be too expensive */
	    for (cop = subtreep->opn_coforw;
		 cop != (OPO_CO *) subtreep;
		 cop = cop->opo_coforw)
	    {
		if (cop->opo_sjpr == DB_ORIG)
		    continue;	/* orig nodes do not have costs */
		if (cartprod)
		{
		    ...
		    {
			tooexpensive = TRUE;
			break;              /* prefer joins over cart prods,
		...
		if ((cop->opo_storage == newstorage)
		    &&
		    (cop->opo_ordeqc == new_ordeqc) /* FIXME - ordering may
		...
		{   /* found a cost ordering with the same storage structure
		    ...
		    */
		    if (pcost >= opn_cost(subquery, cop->opo_cost.opo_dio,
					  cop->opo_cost.opo_cpu)
		       )
		    {
			if (ojsubset)
			    tooexpensive = TRUE; /* the insertion candidate
					** has outer joins which are a subset
					** of this list, but an element of this
					** list is cheaper, so the candidate
					** should not be inserted and the
					** search should be terminated */
		    }
		    else 
		    {
			if (ojsuper)
			    cop->opo_deleteflag = TRUE; /* the insertion 
					** candidate has a super set of the
					** outer joins being evaluated in this
					** list and is cheaper so delete the
					** this element since it cannot be part
					** of the best plan */
		    }
		    break;		/* there should be only one cop for each
					** unique ordering in each subtree list
					*/
		}
	    }
	}
	subtreep = subtreep->opn_aggnext;
    } while (!tooexpensive && (subtreep != sbtp));
    return(!tooexpensive);
}

/*{
** Name: opl_ojfilter	- place co in list if best so far
**
** Description:
**      Find the correct list with outer join filter requirements
**	satisfied and place in this list if subplan is better than
**	all current subplans with same ordering and filter requirements 
**
** Inputs:
[@PARAM_DESCR@]...
**
...
**
**	Returns:
**	    TRUE if plan processed, i.e. inserted into list
**	Exceptions:
**	    none
**
** Side Effects:
	...
	OPO_CO		    *newcop)
{
    ...
    cartprod = newcop->opo_sjeqc == OPE_NOEQCLS;
    if (opl_cosearch(subquery, sbtp, &insertp, pcost, cartprod, filtermap, 
	    newcop->opo_ordeqc, newcop->opo_storage))
    {	/* CO node should be inserted since it has a unique set of filters
	** being evaluated */
	if (!insertp)
	{   /* if no insertion point exists then a new OPN_SUBTREE structure
	    ** needs to be created and inserted */
	    if ((   (sbtp->opn_coforw == (OPO_CO *)sbtp)
		    ||
		    (!sbtp->opn_coforw)
		)
		&&
		(   (eqcp->opn_subtree == sbtp)
		    ||
		    (!eqcp->opn_subtree)
		)
		&&
	    ...
	    {	/* current list is empty so change it so it can be used for
		** this filter */
		MEcopy((PTR)filtermap, sizeof(eqcp->opn_maps.opo_filter),
		    (PTR)&eqcp->opn_maps.opo_filter);
		insertp = sbtp;
	    }
	    else
	    {
		...
		sel = OPN_0PC;
		rlmpp = &rlsp;
		rv = opn_srchst (subquery, 0,/* number of leaves = 0 indicates
					    ** special lookup in which rlmp
					    ** parameter is known but that
					    ** OPN_EQS parameter needs to be
					    ** searched */
		    &eqcp->opn_maps.opo_eqcmap, /* equivalence classes required */
		    (OPV_BMVARS *)NULL,	    /* map of relations not relevant
					    ** for this search */
		    &sbtp->opn_relasgn[0], 
		    &sbtp->opn_structure[0],
		    filtermap,		    /* map of filters which have
					    ** been evaluated in this
					    ** subtree */
		    &subtp, &eqclp, rlmpp, &icheck);
					    /* if this subtree has already been
					    ** evaluated return the subtp, eqclp
					    ** pointers
					    */
		opn_ncommon (subquery, np, rlmpp, &eqclp, &subtp, &eqcp->opn_maps.opo_eqcmap, rv,
		    np->opn_nleaves, &blocks, selapplied, sel, &tups, filtermap); 
					/* common routine
					** for processing leaf and non-leaf
					** node */
		/* place into linked list of OPN_SUBTREE structures with the same
		** tree shape and relation assignment but different filter
		** evaluation */
		insertp = subtp;
		insertp->opn_aggnext = sbtp->opn_aggnext;
		sbtp->opn_aggnext = insertp;	/* place into linked list
						    ** of equivalent subtree structures */
	    }
	}
	opn_coinsert (insertp, newcop);	    /* insert the cost ordering into
...

{
	...
	*/
	...
	for (byresp = subquery->ops_byexpr; byresp && 
	    byresp->pst_sym.pst_type == PST_RESDOM; byresp = byresp->pst_left)
	{
	    byvarp = byresp->pst_right;
	    ghistp = (OPH_HISTOGRAM **) NULL;
	    if (byvarp && byvarp->pst_sym.pst_type == PST_VAR)
	    {
	    ...
	    if (ghistp && (*ghistp)->oph_prodarea != OPH_UNIFORM)
	    {
	...
	OPN_RLS		   *rlp)
{
    ...
    OPL_BMOJ		filtermap;	    /* map of outer joins which have
					    ** filter requirements satisfied */
    OPE_IEQCLS		maxeqcls;
...
#ifdef OPL_OJFILTER
    if (subquery->ops_oj.opl_smask & OPL_OJSPLIT)
    {	/* check if a outer join filter conditions are satisfied 
	** given the current CO tree */
	opl_filtercheck(subquery, newcop, sjp->np, sjp->outer.rlsp, &filtermap); /* this
				** routine will adjust the opo_ordeqc if a filter
				** condition is satisfied, and will also mark the
				** outer joins which have been satisfied */
    }
#endif
	...
	if (global->ops_cb->ops_smask & OPS_MDISTRIBUTED)
	{
	    ...
	    ** site is required */
	    ...
	    if (subquery->ops_msort.opo_topsort)
	    {
		if (sjp->existence_only)
		{
	    ...
	    else if (opd_bestplan(subquery, newcop, subquery->ops_msort.opo_topsort, 
	...
	{   /* local optimization only ... distributed optimization already processed */
	    ...
	    ** if they aren't at least 10 times better than the alternative. */
	    ...
	    if (subquery->ops_msort.opo_topsort || deferredsort)
	...
	*/
	...
	{   /* local optimization has only one cost per CO node instead of
	    ** an array */
	    ...
	    if (!(subquery->ops_oj.opl_smask & OPL_OJSPLIT)
		||
		!opl_ojfilter(subquery, jcost, sjp->np, sjp->join.rlsp, 
		    sjp->join.eqsp, sjp->join.sbtp, &filtermap, newcop))
#endif
	    {
    ...
    ** we'll use it to modify the agg output estimates. */
    ...
    if (rlp != NULL) subquery->ops_besthisto = rlp->opn_histogram;
...
#   endif
	...
	i4	    first;
	i4	    second;
	/* timeout if tracing indicates that plan ID desired has been reached */
	if (global->ops_cb->ops_check)
	{
	    if (opt_strace( global->ops_cb, OPT_F060_QEP))
	    {	/* print subtree of plan, which was found to be possibly useful */
		global->ops_trace.opt_conode = newcop;
		uld_prtree((PTR)newcop,	    /* root of CO tree to print */
		    opt_coprint,            /* routine to print a node */
	...
	bool		   kjonly)
{
    ...
    OPN_SUBTREE         *aggosbtp;          /* used to traverse outer aggregate
                                            ** list */
    OPS_STATE           *global;
    bool		filters_needed;	    /* TRUE if filters are needed for
					    ** outer join duplicate semantics */
    OPL_IOUTER		maxoj;
    i4		pagesize = -1;
    i4		maxtupsize = -1;
    bool		spjoin = FALSE;
    ...
    global = subquery->ops_global;
    maxoj = subquery->ops_oj.opl_lv;
    filters_needed = (subquery->ops_oj.opl_smask & OPL_OJSPLIT) != 0;
	...
	** product */
    ...
    {
        if (filters_needed)
        {   /* merge outer joins which have filter requirements satisfied
	    ** by children */
            MEcopy((PTR)&aggisbtp->opn_eqsp->opn_maps.opo_filter,
                sizeof(sjstate.filter),
                (PTR)&sjstate.filter);
            BTor((i4)maxoj, (char *)&aggosbtp->opn_eqsp->opn_maps.opo_filter,
                (char *)&sjstate.filter);
        }
	...
	*/
	obase = subquery->ops_msort.opo_base; /* ptr to sort descriptors */
	if (const_bm = subquery->ops_bfs.opb_bfeqc)
	{
	...
	{   /* look at every cost ordering in the cost ordering set of the 
		...
		** do its calculations correctly.  bug 1501 */
                     ...
                     innercop = innercop->opo_coforw)
		{
		    ...
		    /* if implicit tid then only use DB_ORIG co cause this is
		    ** the only one which has the implicit tid available. if
		    ** not implicit then only use the co if either it is the
		    ** minimum cost one (excluding the original), it is ordered
		    ** on the joining eqclass, or it is the original. notice
		    ** that imincop (and omincop) will not point to an
		    ** original co (opo_sjpr != DB_ORIG).
		    */
		    while (TRUE)
