	...
	bool		   *icheck)
{
    ...
    *icheck = FALSE;			/* indicates that if subtree is not
    ...
    if (filtermap)
	rlp = *rlmp;			/* relation structure already found
					** - need to find the descriptor which
					** has filtermap defined */
    else
	...
	if (filtermap
	    ||
	    (	(rlp->opn_relmap.opv_bm[0] == rmap->opv_bm[0])
		&&
	...
	{   /* if found the correct map then search eqs list */
	    ...
	    *rlmp = rlp;		/* return the OPN_RLS struct found */
		...
		if ((eqp->opn_maps.opo_eqcmap.ope_bmeqcls[0] == eqr->ope_bmeqcls[0])
		    ...
		    (	!filtermap
			||
			!MEcmp((PTR)&eqp->opn_maps.opo_filter, (PTR)filtermap,
			    sizeof(*filtermap))
		    ))
