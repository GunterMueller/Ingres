		...
		** used */
		...
		{   /* set when a cartesean product is required in order
		    ** to fully use the key */
		    ...
		    subquery->ops_mask |= OPS_CCARTPROD;
					/* not clear what this is for. There
					** are no other refs to the flag */
		}
		else
		    first_time = TRUE;
	...
	{   /* look at placement of each outer join */
		...
		** must have been evaluated in another part of tree */
		...
		!BTsubset((char *)outerp->opl_onclause, (char *)&prim_bothmaps,
								    (i4)maxvar)
		&&
		MEcmp((PTR)outerp->opl_onclause, (PTR)&prim_bothmaps,
						sizeof(prim_bothmaps)))
	    {
		OPZ_IATTS	ojattr;
		...
		{
		    if (!(BTtest((i4)subquery->ops_attrs.opz_base->
			opz_attnums[ojattr]->opz_equcls, (char *)&nodep->
			opn_child[0]->opn_eqm)
			&&
			BTtest((i4)subquery->ops_attrs.opz_base->
			opz_attnums[ojattr]->opz_equcls, (char *)&nodep->
			opn_child[1]->opn_eqm)))
			return(FALSE);
		}
...
#ifndef OPL_OJFILTER
/* disallow query plans in which filter processing is needed until
** OPC implements filters */
...
** OPC implements filters */
			...
			return(FALSE);	/* outer join is evaluated at 2 nodes
				    ** and not a tid join so that filters are needed
				    ** to maintain correct tuple counts */
#endif
		    break;
		}
	        } /* end switch */
...
*/			else if (!opl_ojverify(subquery, outerp->opl_ojattr, innerchildp, outerp))
	...
	** a leaf or... this must be a binary tree containing both tables */
    ...
    if ((!subquery->ops_vars.opv_cart_prod && !subquery->ops_oj.opl_base)
