...
{
    PST_QNODE	*cnst, *cnst1;
    int		i;

    /* Just loop over array entries, cleaning up as we go. */
    for (i = 0; i < opj_orstatp->array_size; i++)
    {
	for (cnst = opj_orstatp->const_array[i]; cnst != NULL;)
	{
	    cnst1 = cnst->pst_left;		/* save next node addr */
	    cnst->pst_left = NULL;
	    cnst->pst_right = NULL;
	    cnst = cnst1;
	}
...

{
    ...
    ** the const_array. */
    ...
    {
	/* Insert i'th IN-list constant into corresponding list in
	** const_array. */
	if (MEcmp((PTR)opj_orstatp->const_array[invar]->pst_sym.pst_dataval.
		db_data, (PTR)opj_orstatp->in_array[i]->pst_sym.pst_dataval.
		db_data, opj_orstatp->in_array[i]->pst_sym.pst_dataval.
		db_length) != 0) 
	 BTclear((i4)invar, (PTR)&opj_orstatp->alldups);
					/* first reset dups flag, if ok */
	opj_orstatp->in_array[i]->pst_left = opj_orstatp->const_array[invar];
	opj_orstatp->const_array[invar] = opj_orstatp->in_array[i];
					/* link 'em up/down */
...

{
    ...
    ** definition. */
    ...
    MEcopy("$memory_res_table ", sizeof(DB_TAB_NAME), 
		(char *)&rdr->rdr_rel->tbl_name);
    MEcopy("$internal ", sizeof(DB_OWN_NAME), 
		(char *)&rdr->rdr_rel->tbl_owner);
