	...
	bool               valid_ordeqc)
{
	...
	** the same joinid */
	...
	/* b113725, INGSRV3116 */
	if (outerp &&
            (cop->opo_variant.opo_local->opo_ojid >= 0) &&
            (outerp->opo_variant.opo_local->opo_jtype == OPO_SJTID) &&
            (!outerp->opo_variant.opo_local->opo_ojoin)
           )
        {
            outerp->opo_variant.opo_local->opo_ojoin =
            ...
            MEcopy((PTR)&cop->opo_maps->opo_eqcmap, 
               sizeof(*outerp->opo_variant.opo_local->opo_ojoin),
               (PTR)outerp->opo_variant.opo_local->opo_ojoin);

            BTor((i4)subquery->ops_eclass.ope_ev, 
                 (char *)&cop->opo_inner->opo_maps->opo_eqcmap,
                 (char *)outerp->opo_variant.opo_local->opo_ojoin);
        }
     }
    ...
    */
	...
	{   /* this is a join node, so mark the appropriate type of join
	    ** for OPC */
	    ...
	    if (opj_usehashjoin(subquery) &&
		subquery->ops_sqtype != OPS_FAGG && /* these assume join o/p 
						** is sorted */
		(subquery->ops_sqtype != OPS_RFAGG ||
		 subquery->ops_bestco->opo_sjpr == DB_RSORT) &&
		(!(subquery->ops_msort.opo_mask & OPO_MTOPSORT) || /* these
						** assure we're not subverting
						** an "order by" interesting
						** order */
		 subquery->ops_bestco->opo_sjpr == DB_RSORT) &&
		!(cop->opo_odups) && !(cop->opo_idups) &&
		(jtype == OPO_SJPSM ||
		(jtype == OPO_SJFSM &&		/* replace FSM which sorts
						** either source or either
						** is also hash join */
		((cop->opo_isort || cop->opo_inner->opo_sjpr == DB_SJ &&
		cop->opo_inner->opo_variant.opo_local->opo_jtype == OPO_SJHASH) ||
		(cop->opo_osort || cop->opo_outer->opo_sjpr == DB_SJ &&
		cop->opo_outer->opo_variant.opo_local->opo_jtype == OPO_SJHASH)))) &&
		opj_ckhasheqc(subquery, cop->opo_sjeqc) &&
						/* no nulljoins or joins on 
						** UDT or date columns */
		(!(cop->opo_existence) || cop->opo_union.opo_ojid == OPL_NOOUTER))
						/* can't have CO and OJ in same
						** hash join (for now) */
	    {
	    ...
	    ** is high (i.e., the kjoin will benefit from presorting). */
		...
		cop->opo_sjeqc >= maxeqcls &&
		subquery->ops_global->ops_qheader->pst_firstn >= 1 &&
...

/*{
** Name: OPJ_GROUPORD	- check underlying plan for ordering which
**	matches grouping column(s) of function aggregate.
**
** Description:
**      This function determines whether the ordering of the underlying
**	CO tree is consistent with the grouping columns of the function
**	aggregates being computed by the current subquery. The ordering
**	need only be a permutation of the grouping columns.
**
** Inputs:
**	sq	- Subquery state info.  
**
...
**
**	Returns:
**	    TRUE - if the CO-tree ordering is consistent with the 
**	    grouping column(s) or at least one grouping column is 
**	    incompatible with hash techniques - so don't use hash agg.
**	    FALSE - if the ordering is not sufficient - so use hash agg.
**	Exceptions:
**	    none
**
** Side Effects:
...

{
    ...
    ** by list. Do hash key length test at same time. */
    ...
    MEfill(sizeof(byeqcmap), (u_char)0x00, (PTR)&byeqcmap);
	...
	if (varp == NULL || varp->pst_sym.pst_type != PST_VAR ||
	    abs(varp->pst_sym.pst_dataval.db_datatype) >= DB_DT_ILLEGAL ||
	    abs(varp->pst_sym.pst_dataval.db_datatype) == DB_CHR_TYPE ||
	    abs(varp->pst_sym.pst_dataval.db_datatype) == DB_NCHR_TYPE ||
	    abs(varp->pst_sym.pst_dataval.db_datatype) == DB_NVCHR_TYPE ||
	    (sq->ops_global->ops_adfcb->adf_collation != NULL &&
	    (abs(varp->pst_sym.pst_dataval.db_datatype) == DB_CHA_TYPE ||
	    abs(varp->pst_sym.pst_dataval.db_datatype) == DB_VCH_TYPE)))
		return(TRUE);		/* all by-exprs must be simple
					** column references with one
					** of the intrinsic (but not C
					** or Unicode) types */

	hashlen += varp->pst_sym.pst_dataval.db_length;
				/* cumulative hash key length */
	if (hashlen > DB_MAXSTRING)
	    return(TRUE);	/* assure hash key isn't too big */
    ...
    ** ordering on the CO-tree. */
    ...
    if (cop == (OPO_CO *) NULL || cop->opo_ordeqc < 0) return(FALSE);

    /* Check for single attr ordering and if it matches by list. */
    if (sq->ops_eclass.ope_ev > cop->opo_ordeqc)	/* single att */
     if (bycount == 1 && eqcno == cop->opo_ordeqc) return(TRUE);
					/* 1 by expr and eqc's match */
     else return(FALSE);		/* single att doesn't match */
    ...
    ** covered by the high order subset of ordering. */
	...
	if ((eqcno = sortlist->opo_eqorder[i]) == OPE_NOEQCLS)
		return(FALSE);		/* more byexprs than ord cols */
	if (!BTtest(sortlist->opo_eqorder[i], (PTR)&byeqcmap))
		return(FALSE);		/* high order ord col NOT in 
					** by list */
    }
...
fixmerfagg: 
	...
	** on outer which will require only the first tuple to be read */
	...
	opa_union(subquery);	    /* relabel union subqueries once
					...
					*/
    /* Check for conversion to hash aggregation. */
    if (opj_usehashjoin(subquery) &&
	subquery->ops_sqtype == OPS_RFAGG && 
	subquery->ops_duplicates != OPS_DREMOVE && /* no agg distincts */
	(*nextpp == NULL || (*nextpp)->ops_duplicates != OPS_DREMOVE ||
	 (*nextpp)->ops_root->pst_sym.pst_value.pst_s_root.pst_qlang == DB_SQL) &&
					/* owner sq also allows dups (to
					** avoid more Quel oddities) */
	!opj_groupord(subquery))	/* group cols not covered by table
					** key structure (so sort not
					** needed anyway) */
    {
	OPV_GRV	*gvarp;

	subquery->ops_sqtype = OPS_HFAGG;
					/* ordering is NOT sufficient to
					** avoid hash aggregation */
	if (subquery->ops_gentry >= 0)
	{
	    /* Show new sqtype in corresponding range var. */
	    gvarp = global->ops_rangetab.opv_base->opv_grv[subquery->ops_gentry];
	    gvarp->opv_created = OPS_HFAGG;
	}
	if (subquery->ops_bestco->opo_sjpr == DB_RSORT)
	{
	    /* If top sort, drop it. Hash agg doesn't use sort. */

	    subquery->ops_bestco = subquery->ops_bestco->opo_outer;
	}
    }
    return (nextpp);
}
