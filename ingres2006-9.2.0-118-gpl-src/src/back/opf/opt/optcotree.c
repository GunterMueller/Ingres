...
static char *ntype_array[] = {"PST_AND", "PST_OR", "PST_UOP", "PST_BOP", 
           "PST_AOP", "PST_COP", "PST_CONST", "PST_RESDOM", "PST_VAR",
           "PST_ROOT", "PST_QLEND", "PST_TREE", "PST_BYHEAD", "PST_AGHEAD",
           "PST_SORT", "PST_CURVAL", "PST_NOT", "PST_SUBSEL", "PST_RULEVAR",
	   "PST_TAB", "PST_MOP", "PST_OPERAND", "PST_CASEOP", "PST_WHLIST",
	   "PST_WHOP", "PST_SEQOP", "PST_GSET", "PST_GBCR", "PST_GCL"};

static char *mtype_array[] = {" ","PSQ_RETRIEVE","PSQ_RETINTO","PSQ_APPEND",
           "PSQ_REPLACE","PSQ_DELETE","PSQ_COPY","PSQ_DISTCREATE",
           "PSQ_DESTROY","PSQ_HELP","PSQ_INDEX","PSQ_MODIFY","PSQ_PRINT",
           "PSQ_RANGE","PSQ_SAVE","PSQ_DEFCURS"};
	...
	char               *msg_buffer)
{
	...
	char	    *basep;
	char	    *temp_ptr;
	msg_buffer[length]=0;		    /* NULL terminate the string */
	basep = msg_buffer;
	temp_ptr = msg_buffer;
...
opt_attnums()
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"JOINOP ATTRIBUTES: base=%d, number of attributes=%d\n\n",
...
opt_fall()
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"FUNCTION ATTRIBUTES: base=%d, number of attributes=%d\n\n",
	...
	OPB_BFKEYINFO      *bfinfop)
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"OPB_BFKEYINFO: next=%d, eqc=%d, used=%d, sarg=",
	bfinfop->opb_next,
        bfinfop->opb_eqc,
        bfinfop->opb_used);
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	 "    list=%d , data", bfinfop->opb_keylist);
	...
	OPB_BFKEYINFO      *bfinfop)
{
    ...
    opscb = ops_getcb();            /* get the session control block
                                    ** from the SCF */
    global = opscb->ops_state;      /* ptr to global state variable */
    if (!bfinfop->opb_global)
    {
	TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	 "OPB_BFGLOBAL: addr=%d, NULL", bfinfop);
	return;
    }
    TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	...
	"OPB_BFGLOBAL: addr=%d, single-eqc=%d, mult-eqc=%d, range-eqc=%d\n\n" ,
	bfinfop,
	bfinfop->opb_global->opb_single,
	bfinfop->opb_global->opb_multiple,
	bfinfop->opb_global->opb_range);
    TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"    min=");    
    if (bfinfop->opb_global->opb_pmin)
	opt_element(bfinfop->opb_global->opb_pmin->opb_keyvalue, bfinfop->opb_bfdt);
    else
	TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	...
	(i4)sizeof(global->ops_trace.opt_trformat),
	", max=");    
    if (bfinfop->opb_global->opb_pmax)
	opt_element(bfinfop->opb_global->opb_pmax->opb_keyvalue, bfinfop->opb_bfdt);
    else
	TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	...
	OPB_IBF            bfi)
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	 "    qual=%d, bfkeyinfo=%d, querytree=%p, ojid=%d, mask=%%x%p\n\n",
	bfp->opb_truequal,
	bfp->opb_keys,
	...
	bool		    corelation)
{
        ...
        if (grv_ptr->opv_relation)
	{
	    ...
	    if (tabnamesize >= sizeof(OPT_NAME))
	...
	if (rdf_infop)
	{
		...
		{   /* if the names are not equal then print the underlying
		    ** LDB or base table name */
		    if (tabnamesize >= sizeof(tablename))
...
opt_rdall()
{
	...
	(i4)sizeof(global->ops_trace.opt_trformat),
	"GLOBAL VAR=%d, ", grv);
	    opt_rdf(grvp->opv_relation); /* dump RDF info for this relation */
	}
	TRformat(opt_scc, (i4 *)global,
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	...
	i4                sqno)
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"    father=%d, concurrent=%d, byhead=%d\n\n",
	subquery->ops_agg.opa_father,
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"    aop=%d, redundant=%d, nestcnt=%d, attlast=%d\n\n",
...
opt_ojstuff(OPS_SUBQUERY  *sq1)
{
    ...
    opt_newline(global);
       ...
       TRformat(opt_scc, (i4 *)global,
       	   (char *)&global->ops_trace.opt_trformat[0],
   	   (i4)sizeof(global->ops_trace.opt_trformat),
   "OPL_OUTER: entry=%d, id=%d, gid=%d, node=%d, type=%d, translate=%d, mask=%d\n",
...
opt_rall(bool	histdump)
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"OPV_RANGEV: prv=%d, rv=%d, base=%d, cartprod?=%d, ",
	...
	OPV_IGVARS         grv)
{
	...
	(i4)sizeof(global->ops_trace.opt_trformat),
        ...
        "OPV_GRV: grv:%d, relp:%p, parser rv:%d, lrv:%d",
	grv,
        grvp->opv_relation,
        grvp->opv_qrt,
        global->ops_rangetab.opv_lrvbase[grv]);
    opt_newline(global);
    opt_rdf(grvp->opv_relation);	    /* print RDF information */
...
opt_eall()
{
	...
	(char *)&global->ops_trace.opt_trformat[0],
	(i4)sizeof(global->ops_trace.opt_trformat),
	"OPE_EQCLIST: maxeqcls = %d, ptr base= %d, ",
	maxeqcls,
...
opt_fjtree()
{
    ...
    */
    uld_prtree((PTR)global->ops_estate.opn_sroot,/* root of Join tree to print */
...
{
    char	buffer[128];
    char	*leader = "QUERY PLAN ";
    char	*format = "%d,%d%s%s%s";

    if ( withStats == TRUE )
    {
	STpolycat( 3, leader, format, trailer, buffer );
	TRformat(opt_scc, (i4 *)NULL,
		(char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		buffer,
    ...
    {
	STpolycat( 2, leader, trailer, buffer );
	TRformat(opt_scc, (i4 *)NULL,
		(char *)&global->ops_trace.opt_trformat[0],
		(i4)sizeof(global->ops_trace.opt_trformat),
		buffer );
	...
	bool		withStats)
{
	...
	case OPS_MAIN:
	    opt_printPlanType( global, withStats, " of main query\n\n",
	     (i4)planid, (i4)queryid, (char *)timeout,
    ...
    ** Neither does UNIX (ejl)
    */
    uld_prtree((PTR)cop,			/* root of CO tree to print */
...
OPS_SUBQUERY	    **already_done;
{
	...
	if (i < MAX_ALREADY_DONE)
	{
        ...
        TRformat(opt_scc, (i4 *)NULL, global->ops_cstate.opc_prbuf,
          OPT_PBLEN, "  ops_agg.opa_concurrent: 0x%p, .opa_graft: 0x%p, .opa_father: 0x%p\n\n",
             ...
             char            *temp)
{
   ...
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_opno: %d\n",indentstr,onode->pst_opno);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_opinst: %d\n",indentstr,onode->pst_opinst);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_distinct: %d\n",indentstr,onode->pst_distinct);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_retnull: %d\n",indentstr,onode->pst_retnull);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_oplcnvrtid: %d\n",indentstr,onode->pst_oplcnvrtid);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_oprcnvrtid: %d\n",indentstr,onode->pst_oprcnvrtid);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
   ...
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n  %sadi_numargs: %d\n",indentstr,onode->pst_fdesc->adi_numargs);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n  %sadi_npre_instr: %d\n",indentstr,onode->pst_fdesc->adi_npre_instr);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
   ...
   if (onode->pst_isescape == PST_HAS_ESCAPE)
	 TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	    "\n%spst_escape: 0x%x\n",indentstr,onode->pst_escape);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_joinid: %d\n",indentstr,onode->pst_joinid);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_flags: %d\n",indentstr,onode->pst_flags);
                ...
                char            *temp)
{
   ...
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_ntargno: %d\n",indentstr,rnode->pst_ntargno);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_ttargtype: %d\n",indentstr,rnode->pst_ttargtype);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_rsupdt: %d\n",indentstr,rnode->pst_rsupdt);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_rsflags: %d\n",indentstr,rnode->pst_rsflags);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
              ...
              char            *temp)
{
   ...
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_srvno: %d\n", indentstr, snode->pst_srvno);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_srasc: %d\n", indentstr, snode->pst_srasc);
               ...
               char            *temp)
{
   ...
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_tparmtype: %d\n", indentstr, cnode->pst_tparmtype);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_parm_no: %d\n", indentstr, cnode->pst_parm_no);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_pmspec: %d\n", indentstr, cnode->pst_pmspec);
   TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\n%spst_cqlang: %d\n", indentstr, cnode->pst_cqlang);
   if (cnode->pst_origtxt != NULL)     /* if there's real data ... */
	TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	    "\n%s pst_origtxt: addr 0x%x, value 0x%x\n",indentstr,
	...
	bool		   printviews)
{
    ...
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\npst_resjour: %s\n",
	 header->pst_restab.pst_resjour ? "TRUE" : "FALSE");
    TRformat(opt_scc, (i4 *)NULL, temp, OPT_PBLEN,
	"\npst_resdup: %s\n",
