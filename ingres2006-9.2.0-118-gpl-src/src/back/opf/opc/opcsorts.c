...
opc_escatts(
	...
	OPE_BMEQCLS	*seqcmp,
	OPC_EQ		*ceq,
	DMT_ATTR_ENTRY	***patts,    /* ptr to a ptr to an array of ptrs to atts */
	i4		*pacount );
		...
		OPC_NODE    *cnode)
{
	...
	** on grouping columns! */
	...
	else if (st->tsort_dups == DMR_NODUPLICATES)
	{
	    /* Skip past the columns that were supplemented due to
	    ** being needed for an insert but which should not be
	    ** included in the sort for DISTINCT. Although the added
	    ** data would usually be constant, users could specify a
	    ** value of 'now' for a date field and could then cause
	    ** extra rows to be added where they should not. b116230
	    */
	    while (sortcols &&
		sortcols->pst_sym.pst_type == PST_RESDOM &&
		sortcols->pst_sym.pst_value.pst_s_rsdm.pst_rsflags&PST_RS_DEFAULTED)
	    {
		sccount--;
		sortcols = sortcols->pst_left;
	    }
	}
	...
	i4		*pacount )
{
    ...
    *pacount = ntargs;
    atts = ( DMT_ATTR_ENTRY **) opu_qsfmem(global, 
			DB_ALIGN_MACRO(ntargs * sizeof(DMT_ATTR_ENTRY *)) +
			ntargs * sizeof(DMT_ATTR_ENTRY) );
    att = (DMT_ATTR_ENTRY *) ME_ALIGN_MACRO((atts + ntargs), DB_ALIGN_SZ);
	...
	i4		*pacount )
{
    ...
    *pacount = BTcount((char *)seqcmp, (i4)subqry->ops_eclass.ope_ev);
    atts = (DMT_ATTR_ENTRY **) opu_qsfmem(global, 
			DB_ALIGN_MACRO((*pacount) * sizeof(DMT_ATTR_ENTRY *)) +
			(*pacount) * sizeof(DMT_ATTR_ENTRY) );
    att = (DMT_ATTR_ENTRY *) ME_ALIGN_MACRO( (atts + *pacount), DB_ALIGN_SZ);
	...
	i4		dups )
{
    ...
    *pcmplist = cmp;
	    ...
	    )
	{
	    ...
	    {
		if (opc_sortable(global, (DB_DT_ID)
			resdom->pst_sym.pst_dataval.db_datatype) == FALSE)
	...
	PST_QNODE	*top_resdom )
{
	...
	*/
	...
	opc_adbase(global, &cadf, QEN_ROW, prowno, 0, 0);
