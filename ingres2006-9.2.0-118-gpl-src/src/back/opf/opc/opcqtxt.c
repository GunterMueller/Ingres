...
typedef struct _QTX_STATE
{
    ...
    bool	    qtx_faboolfact;	/* expression-LIKE in boolean factors */
	...
	QEQ_D1_QRY  *qptr )
{
	    ...
	    )
	{
	    ...
	    agg_count += 1;
	    total_size += 
		(i4)ME_ALIGN_MACRO(aop_node->pst_sym.pst_dataval.db_length, 
...
/* may need to send back a consistency check error */
    ...
    */
	...
	/*
	**  See if this co node has any expression-LIKE boolean
	**  factors associated with it.  If so, the expressions
	**  will have to be materialized into temp table
	**  columns before the LIKE can be evaluated.
	*/

	if ( conode->opo_variant.opo_local->opo_fmask & OPO_FALIKE)
	    txstate->qtx_faboolfact = TRUE;
	...
	*/
    ...
    if (txstate->qtx_faboolfact)
    {
	    /*  
	    **  Create a temp to materialize expressions for LIKE boolean
	    **  factors.  These can only occur in queries against views,
	    **  where, when the view is expanded, a column name can expand
	    **  to be an expression - and SQL does not allow the object
	    **  of LIKE to be an expression.  The INGRES LDB handles this 
	    **  as a special case.
	    **  
	    */
	    ...
	    txstate->qtx_qthd.opq_q16_expr_like = OPQ_1ELIKE;
	    ...
	    opc_crsel( txstate, qptr, res_temp, txstate->qtx_qthd.opq_q5_src_ldb, (bool) FALSE, (bool) FALSE );	    

	    opc_tbflg_ch( txstate, conode->opo_variant.opo_local->opo_bmvars, res_temp );
	    txstate->qtx_where = NULL;
	    ...
	    */
	    ...
	    txstate->qtx_qthd.opq_q16_expr_like = OPQ_2ELIKE;
    }
 ...
 not sure why this check is needed? it always should be an update
			 ...
			 actionhdr->ahd_atype != QEA_D7_OPN)
		{
		    ...
		    actionhdr->qhd_obj.qhd_d2_get.qeq_g1_ldb_p 
					= txstate->qtx_qthd.opq_q5_src_ldb;
		    actionhdr->qhd_obj.qhd_d2_get.qeq_g1_firstn = 
			global->ops_qheader->pst_firstn;
		}
