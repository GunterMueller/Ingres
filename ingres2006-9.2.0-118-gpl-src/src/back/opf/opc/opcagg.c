		...
		QEF_QEP	    *ahd_qepp)
{
	...
	*/
    ...
    opc_iaggtarg_init(global, cnode, aghead, &cadf, &hcadf, &ocadf, &tginfo, 
		...
		oflwbase, &bycount, qn, ahd_qepp->u1.s2.ahd_aghcmp,
		subexlistp);
    ahd_qepp->u1.s2.ahd_bycount = bycount;
	...
	PST_QNODE   **subexlistpp )
{
    ...
    *bycount = 0;
    ...
    {
	aggbufsz = sizeof(i4) + sizeof(PTR);
	align = aggbufsz % sizeof(PTR);
	if (align != 0)
		aggbufsz += sizeof(PTR) - align;
					/* leave space for hash no, chain */
    ...
    *oflwwork = *workbase;
    opc_adbase(global, cadf, QEN_ROW, *workbase, 0, 0);
	...
	** It should be same size as aggregation CX. */
	...
	/* Compute rounded up overflow buffer size. */
	align = oflwsize % sizeof(ALIGN_RESTRICT);
	if (align != 0) oflwsize += (sizeof(ALIGN_RESTRICT) - align);

	/* Allocate overflow buffer. */
	opc_ptrow(global, oflwbase, oflwsize);
	opc_adbase(global, ocadf, QEN_ROW, *oflwbase, 0, 0);
	*oflwbase = ocadf->opc_row_base[*oflwbase];
	opc_adbase(global, ocadf, QEN_ROW, *oflwwork, 0, 0);
	...
	** grouping expressions and computes hash number. */
	...
	opc_adbase(global, hcadf, QEN_ROW, *hashbase, 0, 0);
	...
	PST_QNODE   *subexlistp)
{
	...
	** (if OPS_HFAGG) run hash function. */
	keystart = &((QEF_HASH_DUP *)0)->hash_key[0] - (char *)0;
	offset = keystart;
	if (subqry->ops_sqtype == OPS_HFAGG)
	{
	...
	/* If hash aggregate, add the hash function on the end. */
	if (subqry->ops_sqtype == OPS_HFAGG)
	{
	    ...
	    */
	    ...
	    /* Generate "HASH" against the concatenated byvals */
	    ops[1].opr_len = offset - keystart;
	    ops[1].opr_dt = DB_BYTE_TYPE;
	    ops[1].opr_offset = keystart;
	    ops[1].opr_base = bybase;
	    ops[1].opr_prec = 0;
	    ops[0].opr_len = 5;
	    ...
	    opc_adinstr(global, byadf, 807 /* ADFI_807_HASH_ALL */, 
				ADE_SMAIN, 2, ops, 1);
	    opc_adend(global, byadf);	/* and it's done */
	...
	PST_QNODE   *subexlistp)
{
    ...
    {
	offset = sizeof(i4) + sizeof(PTR);	/* find start of byvals */
	align = offset % sizeof(PTR);
	if (align != 0)
		offset += sizeof(PTR) - align;
	...
	i4	    outbase)
{
	...
	if (subqry->ops_sqtype == OPS_SAGG)
	{
		...
		opc_adbase(global, cadf, QEN_PARM, pno, 0, 0);
	...
	** is null (leaving the empty value result). */
		...
		|| opno == ADE_AVG))
	{
		...
		if (f8tof4)
		{
		    ...
		    */
		    ...
		    opc_adtransform(global, cadf, &ops[1], &saveop, ADE_SFINIT, FALSE);
		    STRUCT_ASSIGN_MACRO(saveop, ops[1]);
		    f8tof4 = FALSE;
		}
	    }
	...
	if (f8tof4)
	    opc_adtransform(global, cadf, &ops[1], &ops[0], ADE_SFINIT, FALSE);
