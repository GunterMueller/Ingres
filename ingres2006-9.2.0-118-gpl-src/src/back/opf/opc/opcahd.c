...
/* Partition copy without texts, names, etc */
static VOID
opc_partdef_copy(
	OPS_STATE	*global,
	DB_PART_DEF	*pdefp,
		...
		QEF_AHD	    **proj_ahd)
{
	    ...
	    )
	{
	    ...
	    */
	    journaled = global->ops_qheader->pst_restab.pst_resjour;
		...
		*/
		...
		if (storage_type == DB_HASH_STORE || journaled == TRUE)
		{
	...
	QEF_AHD	    **proj_ahd )
{
	...
	*proj_ahd = ahd;
	...
	switch (sq->ops_mode)
	{
	 ...
	 case PSQ_DGTT_AS_SELECT:
	    ...
	    */
	    ...
	    if (global->ops_qheader->pst_restab.pst_resjour == TRUE)
	    {
		ahd->ahd_atype = QEA_APP;
	    }
	    else
    ...
    if (sq->ops_bfs.opb_qfalse == TRUE &&
        !(global->ops_qheader->pst_rptqry == TRUE &&
	...
	ahd->ahd_atype == QEA_LOAD))
	ahd->qhd_obj.qhd_qep.ahd_firstn = global->ops_qheader->pst_firstn;
    else ahd->qhd_obj.qhd_qep.ahd_firstn = 0;
    ...
    */
	...
	opc_ptcb(global, &ahd->qhd_obj.qhd_qep.ahd_scroll->ahd_rssplix, 0);
	ahd->qhd_obj.qhd_qep.ahd_scroll->ahd_rsrsize = qp->qp_res_row_sz;
	ahd->qhd_obj.qhd_qep.ahd_scroll->ahd_rskscnt = 0;
	ahd->qhd_obj.qhd_qep.ahd_scroll->ahd_rskattr = (DMT_ATTR_ENTRY *) NULL;
	...
	att = (DMT_ATTR_ENTRY *)opu_qsfmem(global, sizeof (DMT_ATTR_ENTRY));
	ahd->qhd_obj.qhd_qep.ahd_scroll->ahd_rsattr = att;
	STmove ("Resultsetcol", ' ', sizeof(DB_ATT_NAME), 
			att->attr_name.db_att_name);
	att->attr_type = DB_CHA_TYPE;
	att->attr_precision = 0;
	att->attr_collID = -1;
	att->attr_flags = 0;
	SET_CANON_DEF_ID(att->attr_defaultID, DB_DEF_NOT_DEFAULT);
	att->attr_default = NULL;

	/* If this is also a KEYSET cursor, call function to add
	** extra stuff for KEYSET processing. */
	if (global->ops_qheader->pst_mask1 & PST_KEYSET)
	    opc_keyset_build(global, ahd, &cnode);

	att->attr_size = ahd->qhd_obj.qhd_qep.ahd_scroll->ahd_rsrsize;
...

{
    ...
    memsize = kcnt * sizeof(DMT_ATTR_ENTRY) + 2 * sizeof(i4);
    attp = (DMT_ATTR_ENTRY *)opu_qsfmem(global, memsize);
    koff1 = (i4 *)&attp[kcnt];
    koff2 = &koff1[kcnt];
...
**
** Description:
**      This routine performs some constency checks on targets lists. We 
	...
	i4	    index_compressed)
{
	...
	*/
	...
	if (qtree->pst_restab.pst_resjour == TRUE)
	{
	    chars[char_array_idx].char_value = DMU_C_ON;
	}
	else
	...
	*/
	...
	dmucb->dmu_attr_array.ptr_address = 
		    (PTR) opu_qsfmem(global, (i4)(sizeof (DMU_ATTR_ENTRY *) * 
					dmucb->dmu_attr_array.ptr_in_count));
	attrs = (DMU_ATTR_ENTRY **) dmucb->dmu_attr_array.ptr_address;
	    ...
	    )
	{
	    ...
	    if (EQUAL_CANON_DEF_ID(att->att_defaultID,
				   DB_DEF_NOT_DEFAULT))
	    {
		att->att_flags |= DMT_F_NDEFAULT;
	    }

	    if (att->att_type > 0)
	    {
		att->att_flags |= DMT_F_KNOWN_NOT_NULLABLE;
	    }
	    /*
		...
		*/
		...
		if (bits & AD_PERIPHERAL)
		{
		    ...
		    att->att_flags |= DMT_F_PERIPHERAL;
		}
	    }
		...
		*/
		...
		attrs[resdom->pst_sym.pst_value.pst_s_rsdm.pst_rsno
		    - hidden_atts - 1] = 
		(DMU_ATTR_ENTRY *) opu_qsfmem(global, sizeof (DMU_ATTR_ENTRY));
	    ...
	    ** partitioning definitions. */
	    if (sq->ops_mode == PSQ_RETINTO)
	     for (i = 0; i < psf_dmucb->dmu_part_def->ndims; i++)
	      for (j = 0; j < (wdimp = &psf_dmucb->dmu_part_def->dimension[i])
				->ncols; j++)
	       wdimp->part_list[j].row_offset = rel->rdr_attr[wdimp->
					part_list[j].att_number]->att_offset;
	...
	OPV_IGVARS  gresvno )
{
    ...
    */
    ...
    dmtcb->dmt_attr_array.ptr_size = sizeof (DMT_ATTR_ENTRY);
    dmtcb->dmt_attr_array.ptr_address = 
		(PTR) opu_qsfmem(global, (i4)(sizeof (DMT_ATTR_ENTRY *) * 
				    dmtcb->dmt_attr_array.ptr_in_count));
    attrs = (DMT_ATTR_ENTRY **) dmtcb->dmt_attr_array.ptr_address;
	...
	attr = attrs[tsoffsets->opc_sortno - 1] = 
		(DMT_ATTR_ENTRY *) opu_qsfmem(global, sizeof (DMT_ATTR_ENTRY));
	...
	OPS_STATE   *global )
{
	...
	*/
    opc_adbase(global, &cadf, QEN_OUT, ADE_ZBASE, 0, 0);

    opc_crupcurs(global, root->pst_left, &cadf, &opdummy, rel);
	...
	opc_crupcurs(global, root->pst_right, &cadf, &opdummy, rel);
	opc_adend(global, &cadf);
    }
		...
		QEF_AHD		**pahd)
{
    ...
    */
    ...
    opc_crupcurs(global, 
	    global->ops_statement->pst_specific.pst_if.pst_condition,
	    &cadf, &opdummy, global->ops_cstate.opc_rdfdesc);
    opc_adend(global, &cadf);
		...
		QEF_AHD		**pahd)
{
	...
	opc_adalloc_begin(global, &cadf, &ahd->qhd_obj.qhd_return.ahd_rtn,
			ninstr, nops, nconst, szconst, max_base, OPC_STACK_MEM);
	opc_adbase(global, &cadf, QEN_OUT, 0, 0, 0);
		...
		QEF_AHD		**pahd)
{
	...
	*/
	...
	opc_crupcurs(global, root, &cadf, &opdummy,
...
**
**	FIXME FIXME above errors will leak RDF memory because rdf info
**	blocks fetched here aren't unfixed.  Ideally arrange to go thru
**	unfix cleanup before opx-error'ing.
**
** Side Effects:
**	    [@description_or_none@]
**
    ...
    PST_TAB_NODE *tabnode)
{
    ...
    */
	...
	opx_2perror(E_OP08AF_GTTPARM_MISMATCH, (PTR)&gtt_name, 
		(PTR)&proc_name);		/* mismatch */
    ...
    i4		*p_offset_list[])
{
    ...
    */
    ...
    attr_list = *p_attr_list = (DMT_ATTR_ENTRY **)
	opu_qsfmem(global, sizeof(DMT_ATTR_ENTRY *) * tuples_to_fill);
	...
	*/
	...
	)
	{
	    ...
	    if (tuples_to_fill-- == 0)
		opx_error(E_OP08A2_EXCEPTION);
	    attr = attr_list[param_tup->dbpp_number-1] = (DMT_ATTR_ENTRY *)
		opu_qsfmem(global, sizeof(DMT_ATTR_ENTRY));
		...
		attr->attr_flags |= DMT_F_KNOWN_NOT_NULLABLE;
	    if (param_tup->dbpp_flags & DBPP_NDEFAULT)
		attr->attr_flags |= DMT_F_NDEFAULT;
    ...
    QEF_AHD		**pahd)
{
	...
	*pahd = (QEF_AHD *) NULL;
    ...
    opc_adbase(global, &cadf, QEN_ROW,
	    ahd->qhd_obj.qhd_proc_insert.ahd_proc_temptable->ttb_tuple, 0, 0);
    ...
    QEF_AHD	*proc_insert_ahd)
{
    ...
    if (ahd->qhd_obj.qhd_callproc.ahd_procparams)
    {
	if (ptuple->db_mask[0] & DBP_CONS)
            ahd->qhd_obj.qhd_callproc.ahd_procparams->qen_mask |= 
							QEN_CONSTRAINT;
	else
            ahd->qhd_obj.qhd_callproc.ahd_procparams->qen_mask &= 
							~QEN_CONSTRAINT;
		...
		QEU_CB		**target )
{
    ...
    *target = qeu_cb;
    ...
    qeu_cb->qeu_qual = ( PTR ) NULL;
    qeu_cb->qeu_qarg = ( PTR ) NULL;
		...
		QEF_AHD		**new_ahd )
{
    ...
    */
	...
	rdf_rb->rdr_l_querytext = psf_crt_view->pst_view_text->db_t_count;
	rdf_rb->rdr_querytext   = (PTR) psf_crt_view->pst_view_text->db_t_text;
		...
		DM_PTR		*target )
{
    ...
    DMU_ATTR_ENTRY	*sourceAttr;
    DMU_ATTR_ENTRY	*targetAttr;
	      ...
	      i < objectCount; i++ )
	{
	    ...
	    */
	    ...
	    {
		sourceAttr = ( DMU_ATTR_ENTRY * ) sourceObjectAddress[ i ];
		targetAttr = ( DMU_ATTR_ENTRY * ) targetObjectAddress[ i ];
