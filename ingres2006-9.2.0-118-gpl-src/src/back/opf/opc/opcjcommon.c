		...
		OPC_EQ		**pceq)
{
    ...
    OPO_ISORT		meqcno;
    OPO_ST		*mbase;
    OPE_IEQCLS		ord_eqcls[2];
    OPO_ISORT		*ord_eqorder;
    OPE_IEQCLS		sj_eqcls[2];
    OPO_ISORT		*sj_eqorder;
    i4			ordno;
    ...
    if (    (pcnode->opc_cco->opo_osort == TRUE && 
		pcnode->opc_cco->opo_outer == co
	    ) ||
	    (pcnode->opc_cco->opo_isort == TRUE && 
		pcnode->opc_cco->opo_inner == co
	    )
	)
    {
    ...
    *pceq = cnode.opc_ceq;
    ...
    if (co->opo_storage == DB_SORT_STORE
	|| 
	co->opo_storage == DB_BTRE_STORE
	||
	cnode.opc_invent_sort == TRUE
       )
    {
	/* figure out what this node is ordered on and and the parent is
	** joining on.
	*/
	mbase = subqry->ops_msort.opo_base;
	if (co->opo_ordeqc >= subqry->ops_eclass.ope_ev)
	{
	    meqcno = co->opo_ordeqc - subqry->ops_eclass.ope_ev;
	    ord_eqorder = 
			&mbase->opo_stable[meqcno]->opo_eqlist->opo_eqorder[0];
	}	
	else
	{
	    ord_eqcls[0] = co->opo_ordeqc;
	    ord_eqcls[1] = OPE_NOEQCLS;
	    ord_eqorder = &ord_eqcls[0];
	}

	if (pco->opo_sjeqc >= subqry->ops_eclass.ope_ev)
	{
	    meqcno = pco->opo_sjeqc - subqry->ops_eclass.ope_ev;
	    sj_eqorder = &mbase->opo_stable[meqcno]->opo_eqlist->opo_eqorder[0];
	}	
	else
	{
	    sj_eqcls[0] = pco->opo_sjeqc;
	    sj_eqcls[1] = OPE_NOEQCLS;
	    sj_eqorder = &sj_eqcls[0];
	}

	/*
	** If the list of eqcs that this node is ordered on is a superset
	** of the eqcs that the parent is joined on then this node
	** is sorted for the parent, otherwise it isn't
	*/
	for (ordno = 0; 
		sj_eqorder[ordno] != OPE_NOEQCLS;
		ordno += 1
	    )
	{
	    if (ord_eqorder[ordno] != sj_eqorder[ordno])
	    {
		break;
	    }
	}

	if (sj_eqorder[ordno] == OPE_NOEQCLS && ordno != 0)
	{
	    *sorted = TRUE;
	}
	else
		...
		i4		*tidatt)
{
    ...
    */
	...
	opc_gtqual(global, cnode, qnode->qen_type, &outer_eqcmp, &inner_eqcmp,
	    cnode->opc_cco->opo_variant.opo_local->opo_bmbf,
	    &t1_tree, &qual_eqcmp);

	opc_gtqual(global, cnode, (i4)QE_TJOIN,
	    &cnode->opc_cco->opo_inner->opo_maps->opo_eqcmap, (OPE_BMEQCLS *)NULL,
	...
	*/
	...
	if (keqcmp != NULL)
	{
	    MEcopy((PTR)keqcmp, sizeof (noteqcmp), (PTR)&noteqcmp);
	    BTnot((i4)BITS_IN(noteqcmp), (char *)&noteqcmp);
	    BTand((i4)BITS_IN(nkeqcmp), (char *)&noteqcmp, (char *)&nkeqcmp);
	}

	/* eliminate unnecessary equijoin tests for TJOIN 2ndary index tuples */
	if (qnode->qen_type == QE_TJOIN)
	{
	    BTnot((i4)BITS_IN(tjeqcmp), (char *)&tjeqcmp);
	    BTand((i4)BITS_IN(tjeqcmp), (char *)&tjeqcmp, (char *)&nkeqcmp);
	}
	...
	*/
	if ( OJequijoins != NULL )
	{
	    MEcopy((PTR)OJequijoins, sizeof (noteqcmp), (PTR)&noteqcmp);
	    BTnot((i4)BITS_IN(noteqcmp), (char *)&noteqcmp);
	    BTand((i4)BITS_IN(nkeqcmp), (char *)&noteqcmp, (char *)&nkeqcmp);
	}
		...
		i4		*tidatt)
{
    ...
    ** should be evaluated at this node.
    */
    opc_gtiqual(global, cnode, &inner_eqcmp,
	cnode->opc_cco->opo_inner->opo_variant.opo_local->opo_bmbf,
	&comp_tree, &qual_eqcmp);
...

{
    ...
    ** key buffer. */
    ...
    opc_adbase(global, &key_cadf, QEN_ROW, keyrow, 0, 0);
    kj->node_qen.qen_kjoin.kjoin_key->qen_output = keyrow;
    opc_adbase(global, &kcomp_cadf, QEN_ROW, keyrow, 0, 0);
    ...
    opc_adbase(global, &key_cadf, QEN_ROW, oceq[oeqc].opc_dshrow, 0, 0);
    opc_adbase(global, &kcomp_cadf, QEN_ROW, oceq[oeqc].opc_dshrow, 0, 0);
    ...
    ** structure to define the Rtree probe */
    ...
    kj->node_qen.qen_kjoin.kjoin_kys = qekey;  /* and attach to kjoin */
		...
		OPE_BMEQCLS	*keymap)
{
    ...
    bool                rmsflag = (rel->rdr_rel->tbl_relgwid == DMGW_RMS);
	...
	** effectively equivalent to OR's. */
	...
	*/
    ...
    opc_adbase(global, &key_cadf, QEN_ROW, keyrow, 0, 0);
    opc_adbase(global, &key_cadf, QEN_KEY, 0, 0, 0);
		...
		*/
		    ...
		    )
		{
			...
			*/
		    ...
		    /* If this operand has an escape char, then compile an
		    ** instruction to let ADF know.
		    */
		    if (qop->pst_sym.pst_value.pst_s_op.pst_isescape == 
								PST_HAS_ESCAPE
			)
		    {
			...
			escape_dv.db_data = 
				    &qop->pst_sym.pst_value.pst_s_op.pst_escape;
			opc_adconst(global, &key_cadf, &escape_dv, 
							&escape_ops[0],
							    DB_SQL, ADE_SMAIN);
		...
		OPE_BMEQCLS	*eqcmp1)
{
	...
	*/
	...
	MEcopy((PTR)eqcmp0, sizeof(tmp), (PTR)&tmp);
	BTnot((i4)BITS_IN(tmp), (char *)&tmp);
	BTand((i4)BITS_IN(*eqcmp1), (char *)&tmp, (char *)eqcmp1);
    }
    else
    {
	/*
...
)
{
	    ...
	    )
	{
	    ...
	    */
		...
		cnode->opc_ceq[eqcno].opc_dshrow, 0, 0);
	    adeops[0].opr_base = 
		cadf.opc_row_base[cnode->opc_ceq[eqcno].opc_dshrow];
	}
	...
	if (sceq[i].opc_eqavailable == TRUE)
	{
		...
		*/
		opc_adbase(global, &cadf, QEN_ROW, 
		    cnode->opc_ceq[i].opc_dshrow, 0, 0);
		adeops[0].opr_base = 
			cadf.opc_row_base[cnode->opc_ceq[i].opc_dshrow];
	    }
		...
		** parm from the inner side of the outer join. */
		    ...
		    )
		{
		    ...
		    ** to this CX then add it now.
		    */
		    opc_adbase(global, &cadf, QEN_ROW, sceq[i].opc_dshrow, 0, 0);
		    adeops[1].opr_base = 
		...
		OPE_BMEQCLS	*eqcmp)
{
	...
	BTnot((i4)BITS_IN(tmp), (char *)&tmp);
	BTand((i4)BITS_IN(*eqcmp), (char *)&tmp, (char *)eqcmp);
    }
		...
		OPC_EQ		*child_ceq)
{
	...
	*/
		...
		)
	{
	    ...
	    */
	    opc_adbase(global, &cadf, QEN_ROW, 
				    child_ceq[eqcno].opc_dshrow, 0, 0);
	    adeops[1].opr_base = 
			cadf.opc_row_base[child_ceq[eqcno].opc_dshrow];
	}
...
/*	    opc_speqcmp(global, cnode, rightChild, &LNULLspecialEQCSto0,
	    ...
	    */
		...
		MEcopy((PTR)&LNULLspecialEQCSto0,
			sizeof( LNULLspecialEQCSto0 ), ( PTR ) &tmp );
		BTnot((i4)BITS_IN(tmp), (char *)&tmp);
		BTand((i4)BITS_IN(LNULLempeqcmp), (char *)&tmp,
			(char *)&LNULLempeqcmp);

		MEcopy((PTR)&LNULLspecialEQCSto1,
			sizeof( LNULLspecialEQCSto1 ), ( PTR ) &tmp );
		BTnot((i4)BITS_IN(tmp), (char *)&tmp);
		BTand((i4)BITS_IN(LNULLempeqcmp), (char *)&tmp,
			(char *)&LNULLempeqcmp);
	    }
...
/*	    opc_speqcmp(global, cnode, leftChild, &RNULLspecialEQCSto0,
	    ...
	    */
		...
		MEcopy((PTR)&RNULLspecialEQCSto0,
			sizeof(RNULLspecialEQCSto0), (PTR)&tmp);
		BTnot((i4)BITS_IN(tmp), (char *)&tmp);
		BTand((i4)BITS_IN(RNULLempeqcmp), (char *)&tmp,
			(char *)&RNULLempeqcmp);

		MEcopy((PTR)&RNULLspecialEQCSto1,
			sizeof(RNULLspecialEQCSto1), (PTR)&tmp);
		BTnot((i4)BITS_IN(tmp), (char *)&tmp);
		BTand((i4)BITS_IN(RNULLempeqcmp), (char *)&tmp,
			(char *)&RNULLempeqcmp);
	    }
...
)
{
    ...
    attr_list = (DMT_ATTR_ENTRY **)
	opu_qsfmem(global, sizeof(DMT_ATTR_ENTRY *) * TID_SPLIT );

    buf[0] = 'a';
    for ( i = 0; i < TID_SPLIT; i++ )
    {
	attr = attr_list[ i ] = ( DMT_ATTR_ENTRY * )
	...
	*/
	    ...
	    ( cnode->opc_cco->opo_union.opo_ojid != OPL_NOOUTER))
	{
	    ...
	    )
	    {
	        ...
	        */
	        opc_adbase(global, &cadf, QEN_ROW, 
				oceq[ specialEqcno ].opc_dshrow, 0, 0);
	        adeops[0].opr_base = 
		    cadf.opc_row_base[oceq[ specialEqcno ].opc_dshrow];
	    }
