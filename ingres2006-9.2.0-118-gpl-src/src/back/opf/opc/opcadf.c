		...
		i4	    mem_flag )
{
    ...
    */
    opc_adbase(global, cadf, QEN_ROW, OPC_CTEMP, 0, 0);
}

/*{
	...
	*/
	...
	else if (global->ops_adfcb->adf_errcb.ad_errcode == E_AD5505_UNALIGNED)
	{
		...
		*/
		...
		/* align the operand */
		opc_adinstr(global, cadf, ADE_MECOPY, seg, 2, alops, 1);
		...
		** operand to aligned memory.
                */
                ...
                /* bug 116999: align alops[0] and copy ops[unaligned]
                ** into alops[0]
                */ 
		...
		global->ops_cstate.opc_temprow->opc_olen += 
                ...
                /* bug 116999: copy the data into the aligned space   
                ** by doing a MEcopy().
                */
                opc_adinstr(global, cadf, ADE_MECOPY, seg, 2, alops, 1);
		...
		/* bug 116999: now swap alops[0] with alops[1] and copy the 
                ** results to the unaligned area.
                */
                STRUCT_ASSIGN_MACRO(alops[0], tempop);
                STRUCT_ASSIGN_MACRO(alops[1], alops[0]);
                STRUCT_ASSIGN_MACRO(tempop, alops[1]);
	...
	i4	    baseno )
{
	...
	break;
     ...
     case QEN_ADF_AG:
	cadf->opc_adf_ag_base = baseno + ADE_ZBASE;
	array = QEN_ROW;
	opc_bsmap(global, cadf, &array, &index, baseno);
	break;
    ...
    ** reduction in the amount of data to be moved around. */
	...
	for (i = 0; i < qadf->qen_sz_base; i++)
		...
		break;
	      ...
	      case QEN_PARM:
		opc_ptrow(global, &new_base[i].u.qen_parm_row, 0);
		basep[i] = new_base[i].u.qen_parm_row;
		break;
		...
		PTR		data[])
{
	...
	if (global->ops_adfcb->adf_errcb.ad_errcode == E_AD2022_UNKNOWN_LEN)
	{
	    ...
	    ** to reflect this.
	    */
	    opc_adbase(global, cadf, QEN_VTEMP, 0, 0, 0);
		...
		PST_QNODE	    *qop)
{
    ...
    if (qop->pst_sym.pst_value.pst_s_op.pst_isescape == PST_HAS_ESCAPE)
    {
	keyblk.adc_escape = qop->pst_sym.pst_value.pst_s_op.pst_escape;
	keyblk.adc_isescape = TRUE;
    }
    else
	keyblk.adc_isescape = FALSE;
		...
		DB_DATA_VALUE	*rdv)
{
    ...
    */
    if (abs(rdv->db_datatype) == DB_DEC_TYPE)
    {
