		...
		OPS_STATE          *global)
{
    ...
    */
    ...
    {
        if (global->ops_qheader->pst_rptqry == TRUE)
	{
    ...
    ** the flag will compile for local CX bases. */
    ...
    /* TP op195 reverts to local CX bases. */
		...
		OPS_STATE          *global)
{
    ...
    */
	...
	/* QEF. */
	if (global->ops_inAfterRules)
	    opc_resolveif(global, global->ops_firstAfterRule, 
		(PST_STATEMENT *)NULL, (PST_STATEMENT *)NULL);
	else opc_resolveif(global, global->ops_firstBeforeRule, 
		(PST_STATEMENT *)NULL, (PST_STATEMENT *)NULL);
	...
	*/
	opc_pst = (OPC_PST_STATEMENT *)global->ops_trigger->pst_opf;
	if (global->ops_inAfterRules)
	    ...
	    ~global->ops_cstate.opc_flags & OPC_STARVIEW)
	{
		...
		opc_resolveif(global, global->ops_procedure->pst_stmts, 
			     (PST_STATEMENT *) NULL, (PST_STATEMENT *) NULL);
	...
	PST_STATEMENT	*last_stmt )
{
	...
	*/
	if (cur_stmt->pst_type == PST_FOR_TYPE)
	{
	    ...
	    cur_ahd->qhd_obj.qhd_qep.ahd_qep = next_ahd->qhd_obj.qhd_qep.ahd_qep;
	    next_ahd->qhd_obj.qhd_qep.ahd_qepflag |= AHD_FORGET;
	}
