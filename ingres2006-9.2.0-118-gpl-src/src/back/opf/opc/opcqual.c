	...
	** no Unicode normalization is required. */
	...
	if (resop != NULL)
	{
		...
		*/
		if (resop->opr_offset == OPR_NO_OFFSET)
		{
		    opc_adtransform(global, cadf, &ops[0], resop, segment, (bool)TRUE);
		    if (unorm &&
			!((abs(resop->opr_dt) == DB_NCHR_TYPE && 
			...
			** opc_adtransform). */
		    ...
		    opc_adtransform(global, cadf, &ops[0], resop, oldseg, (bool)FALSE);
	    }
	}
	global->ops_cstate.opc_qp->qp_status |= QEQP_NEED_TRANSACTION;
	break;
