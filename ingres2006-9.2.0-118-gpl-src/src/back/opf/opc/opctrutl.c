...
typedef struct _ULD_TTCB
{
    ...
    i4		    uld_nnodes;
    bool	    uld_ssvar;		/* special indicator for subselect */
...
static NULL_COERCE	nc_array[] =
	{{{NULL, 2,  (DB_DT_ID)-DB_BYTE_TYPE,  0}, "byte"},
	 {{NULL, 2,  (DB_DT_ID)-DB_CHR_TYPE,   0}, "c"},
	 {{NULL, 2,  (DB_DT_ID)-DB_CHA_TYPE,   0}, "char"},
	 {{NULL, 13, (DB_DT_ID)-DB_DTE_TYPE,   0}, "ingdate"},
	 {{NULL, 5, (DB_DT_ID)-DB_ADTE_TYPE,   0}, 
						  "ansidate"},
	 {{NULL, 11, (DB_DT_ID)-DB_TMWO_TYPE,   0}, 
				    "time without time zone "},
	 {{NULL, 11, (DB_DT_ID)-DB_TMW_TYPE,   0}, 
				    "time with time zone"},
	 {{NULL, 11, (DB_DT_ID)-DB_TME_TYPE,   0}, 
				    "time with local time zone"},
	 {{NULL, 15, (DB_DT_ID)-DB_TSWO_TYPE,   0}, 
				    "timestamp without time zone"},
	 {{NULL, 15, (DB_DT_ID)-DB_TSW_TYPE,   0}, 
				    "timestamp with time zone"},
	 {{NULL, 15, (DB_DT_ID)-DB_TSTMP_TYPE,   0}, 
				"timestamp with local time zone"},
	 {{NULL, 4, (DB_DT_ID)-DB_INYM_TYPE,   0}, 
				"interval year to month"},
	 {{NULL, 13, (DB_DT_ID)-DB_INDS_TYPE,   0}, 
				"interval day to second"},
	 {{NULL, 5,  (DB_DT_ID)-DB_FLT_TYPE,   0}, "float4"},
	 {{NULL, 3,  (DB_DT_ID)-DB_INT_TYPE,   0}, "int2"},
	 {{NULL, 9,  (DB_DT_ID)-DB_MNY_TYPE,   0}, "money"},
	 {{NULL, 3,  (DB_DT_ID)-DB_VCH_TYPE,   0}, "varchar"},
	 {{NULL, 4,  (DB_DT_ID)-DB_VBYTE_TYPE, 0}, "varbyte"}};
#define NCA_SIZE 9		/* keep in sync with array */
	...
	PST_QNODE	*varp )
{
    ...
    if(!ttcbptr->uld_ssvar)	/* not a subselect var */
    {
	opc_varnode(ttcbptr,
    ...
    {
	ttcbptr->uld_ssvar = (bool)FALSE;	/* reset indicator */
	textcb = (OPC_TTCB *) ttcbptr->handle;
	subquery = textcb->subquery;
	global = (OPS_STATE *)subquery->ops_global;
	attp = subquery->ops_attrs.opz_base->opz_attnums[
	    varp->pst_sym.pst_value.pst_s_var.pst_atno.db_att_id];
	...
	PST_QNODE	*constp )
{
	...
	opcu_intprintf(ttcbptr, (i4)( constp->pst_sym.pst_value.pst_s_cnst.pst_parm_no - 1));
	opcu_printf(ttcbptr, " ");
	...
	PST_QNODE          *qnode )
{
	...
	** 1 = ANY (select any(*)..... form generated below    */
	...
	if(qnode->pst_sym.pst_value.pst_s_op.pst_opmeta != PST_NOMETA)
	{
	    ttcbptr->uld_ssvar = (bool)TRUE;	/* set indicator for VAR */
	    if(qnode->pst_sym.pst_value.pst_s_op.pst_opmeta == PST_ANY_SUBSELECT)
	    {
		opcu_printf(ttcbptr, " ANY ");
	    }
	    if(qnode->pst_sym.pst_value.pst_s_op.pst_opmeta == PST_ALL_SUBSELECT)
	    {
	...
	if (qnode->pst_sym.pst_value.pst_s_op.pst_isescape == PST_HAS_ESCAPE)
	{
	    char escape_str[2];
	    escape_str[0] = qnode->pst_sym.pst_value.pst_s_op.pst_escape;
	    escape_str[1] = EOS;
	...
	PST_QNODE          **qtreepp )
{
	...
	*qtreepp = bfp->opb_qnode;
	...
	{
	    /*
	    **  This boolean factor is an expression-LIKE
	    */

	    if (handle->qryhdr->opq_q16_expr_like == OPQ_2ELIKE )
	    {
		/*
		**  A temp table has been created to materialize expression
		**  columns that are objects of a LIKE operator - so all
		**  conjuncts and boolean factors EXCEPT the LIKE operator(s)
		**  were applied when the temp was created.  Only the LIKE
		**  on expressions need to be evaluated now, and they will be
		**  evaluated against the temp table column containing the
		**  expression result.
		*/
	    ...
	    {
		/*
		**  We are in the process of creating the temp table to materialize
		**  expression columns that are objects of LIKE, so skip the
		**  expression-LIKE boolean factors in the where clause for now.
		*/
