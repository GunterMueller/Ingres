...

{
    ...
    if (attcount <= level + 1) nolower = TRUE;
	...
	** Loop over values in current BF (representing ORs). */
	...
	for (klistp = bfkeyp->opb_keylist, firstval = TRUE; 
		klistp; klistp = klistp->opb_next, firstval = FALSE)
	{
	    ...
	    */
		...
		nextklistp = klistp->opb_next;
		if (nextklistp)
		 if ((reverse = ((klistp->opb_opno == (ADI_OP_ID) opg_cb->opg_lt ||
		     klistp->opb_opno == (ADI_OP_ID) opg_cb->opg_le) &&
		    (nextklistp->opb_opno == (ADI_OP_ID) opg_cb->opg_gt ||
		     nextklistp->opb_opno == (ADI_OP_ID) opg_cb->opg_ge))) ||
		    ((klistp->opb_opno == (ADI_OP_ID) opg_cb->opg_gt ||
		     klistp->opb_opno == (ADI_OP_ID) opg_cb->opg_ge) &&
		    (nextklistp->opb_opno == (ADI_OP_ID) opg_cb->opg_lt ||
		     nextklistp->opb_opno == (ADI_OP_ID) opg_cb->opg_le)))
		{
		    /* A "between" predicate is found - next value is
		    ** closing boundary. */
		    between = TRUE;
		    MEcopy((PTR)nextklistp->opb_hvalue, attrlen, key1addr);
		    newvalp->opb_opno = nextklistp->opb_opno;
		    fill1len = totallen - (offset + attrlen);
		}
		...
		if (!between) 
		{
			...
			klistp->opb_opno == (ADI_OP_ID) opg_cb->opg_le);
		    if (reverse) newvalp->opb_opno = (ADI_OP_ID)opg_cb->opg_ge;
		    else newvalp->opb_opno = (ADI_OP_ID)opg_cb->opg_le;
		    key1addr = key1addr - attrlen;
		    fill1len = totallen - offset;
		}
		
		/* Fill rest of key buffer with 0's or f's depending on
		** whether it's lower or upper bound. */
		...
		if (reverse)
		{
		    ...
		    for (wvalp = histp->oph_composite.oph_vallistp;
		...
		valp->opb_operator = ADC_KRANGEKEY;
		newvalp->opb_operator = ADC_KRANGEKEY;
		if (nextklistp) klistp = nextklistp;
	    }
...
involved
	...
	{   /* this should never be called */
	    ...
	    (PTR)DB_TEMP_TAB_NAME, (sizeof(DB_TEMP_TAB_NAME)-1)) !=0))
    {
	...
	*/
	...
	/* If the info block does change then we could have an RDF memory
	** leak on our hands. Issue a TRdisplay to provide a clue.
	*/
        if(rdf_info != rdf_cb->rdf_info_blk)
	{
            TRdisplay("oph_catalog() - rdf_info_blk changed for %s. Old value = %p, New value = %p\n",
              ((ttab_hist) ? "grv_ptr->opv_ttmodel" : "grv_ptr->opv_relation"),
	      rdf_info, rdf_cb->rdf_info_blk);
	}
...

{
	...
	** mapped by key columns. */
	for (i = 0; i < varptr->opv_mbf.opb_count; i++)
	{
	    i4	j;
