	...
	for (j = i+1; j < subquery->ops_vars.opv_prv; j++)
	{
		...
		** columns. */
		...
		for (m = 0, nogood = FALSE; m < pdim1p->ncols && !nogood; m++)
		{
		    ...
		    /* Check if matching column was found and types
		    ** are the same and amongst the supported types
		    ** (Unicode, C and UDTs are NOT supported). */
		    len1 = (pcol1p->type > 0) ? pcol1p->length :
							pcol1p->length-1;
		    len2 = (pcol2p->type > 0) ? pcol2p->length :
							pcol2p->length-1;
		    if (n < 0 || abs(pcol1p->type) != abs(pcol2p->type) ||
			len1 != len2 || pcol1p->precision != pcol2p->precision ||
			abs(pcol1p->type) == DB_CHR_TYPE ||
			abs(pcol1p->type) == DB_NCHR_TYPE ||
			abs(pcol1p->type) == DB_NVCHR_TYPE ||
			abs(pcol1p->type) >= DB_DT_ILLEGAL)
		    {
			nogood = TRUE;
			break;
		    }
		...
		if (pdim1p->distrule != DBDS_RULE_HASH &&
			!(adt_partbreak_compare(pdim1p, pdim2p)))
		...
		** descriptions and add them to the 2 variables. */
		...
		subquery->ops_global->ops_gmask |= OPS_PCJOINS;
					/* note pc join potential */
