	...
	bool		   explicit_index)
{
    ...
    */
	...
	BTcount((char *)&subquery->ops_global->ops_qheader->pst_rangetab[
		    basevarp->opv_gvar]->pst_inner_rel.pst_j_mask, OPV_MAXVAR))
    {
	if (!index_sub)
	{
	    OPL_OUTER	*ojdesc;
	    ...
	    ** or full. If so, a TID join with this index isn't possible. */
	    ...
	    jmask = &subquery->ops_global->ops_qheader->
		pst_rangetab[basevarp->opv_gvar]->pst_inner_rel;

	    for (gojid = -1; (gojid = BTnext(gojid, (char *)jmask,
		(i4)BITS_IN(*jmask))) > -1;)
	    {
    ...
    ** secondary, there may be no point in adding it to the range table. */
    primcsec = TRUE;
    if (prim_keys && prim_keys->key_count >= mbfkl.opb_bfcount)
    {
	i4	i;
	for (i = 0; i < mbfkl.opb_bfcount && primcsec; i++)
