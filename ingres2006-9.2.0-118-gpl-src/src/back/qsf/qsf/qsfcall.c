...
qsf_call( i4 qsf_operation, QSF_RCB *qsf_rb )
{
    ...
    qsf_rb->qsf_error.err_data = 0;
    qsf_rb->qsf_error.err_code = 0;
    ...
    */
    ...
    {
	qsf_rb->qsf_error.err_code = E_QS0017_BAD_REQUEST_BLOCK;
    }
    else if ( (qsf_operation < QSF_MIN_OPCODE)
	      || (qsf_operation > QSF_MAX_OPCODE)
	    )
    {
	qsf_rb->qsf_error.err_code = E_QS0003_OP_CODE;
    }
    else if ( Qsr_scb == NULL )
    {
	if ( qsf_operation != QSR_STARTUP )
	    qsf_rb->qsf_error.err_code = E_QS001A_NOT_STARTED;
    }
	...
	break;
      ...
      default:
	qsf_rb->qsf_error.err_code = E_QS9999_INTERNAL_ERROR;
	status = E_DB_FATAL;
	break;
    }
