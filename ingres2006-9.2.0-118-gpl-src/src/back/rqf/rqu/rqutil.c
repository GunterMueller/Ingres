...
**	rqu_format_write - pre-format association write buffer
**	rqu_format_read	- set association for receive and interpret
...
** Inputs:
**
**	it	a interpreted message block
**
** Outputs:
**          none
**        
	...
	GCA_IT_PARMS	*it)
{
  ...
  scf_cb.scf_len_union.scf_blength = 
    it->gca_d_length + (it->gca_data_area-it->gca_buffer);
  scf_cb.scf_ptr_union.scf_buffer = (PTR)it;
  scf_cb.scf_session = DB_NOSESSION;	    /* print to current session */
	...
	GCA_ER_DATA	*gca_error_blk)
{
    ...
    */
	...
	status = ule_format(scf_cb.scf_nbr_union.scf_local_error, NULL,
			    ...
			    (i4)NULL, (PTR)NULL, (i4)NULL, (PTR)NULL,
			    (i4)NULL, (PTR)NULL, (i4)NULL, (PTR)NULL,
			    (i4)NULL, (PTR)NULL, (i4)NULL, (PTR)NULL,
			    (i4)NULL, (PTR)NULL, (i4)NULL, (PTR)NULL,
			    (i4)NULL, (PTR)NULL, (i4)NULL, (PTR)NULL,
...
rqu_send(
	...
	i4		msg_type)
{
    GCA_SD_PARMS    *send = &assoc->rql_send;
    STATUS	    status;
    RQS_CB	    *rqs = (RQS_CB *)assoc->rql_rqr_cb->rqr_session;
...
rqu_release_assoc(
	RQL_ASSOC	*assoc)
{
    GCA_ER_DATA		*er_data = (GCA_ER_DATA *)assoc->rql_format.gca_data_area;
...

/*{
** Name: rqu_format_write	- pre-format association write buffer
**
** Description:
**      pre-format association write buffer for future write for an
**	association.  SD_PARMS are also initialized.
** Inputs:
**
	...
	GCA_SD_PARMS	*send)
{
  ...
  format->gca_buffer = buffer;
  format->gca_b_length = buffer_size;

  IIGCa_call(GCA_FORMAT, (GCA_PARMLIST *)format, GCA_SYNC_FLAG, 
	     (PTR)0, (i4)-1, &status);

  /* SD_PARMS constants */
  send->gca_association_id = assoc_id;
  send->gca_buffer = buffer;
  send->gca_descriptor = NULL;
}

/*{
** Name: rqu_format_read	- set association for receive and interpret
**
** Description:
**      pre-format GCA write buffer for future write for an
**	association.  
** Inputs:
**
**	assoc	-	association token
	...
	RQL_ASSOC	*assoc)
{
  ...
  GCA_IT_PARMS	*it = &assoc->rql_it;
  RQS_CB	*rqs = (RQS_CB *)assoc->rql_rqr_cb->rqr_session;

  if (! RQF_DBG_LVL(rqs,1))
     return;

  switch (it->gca_message_type)
  {
    case GCA_ERROR:
    case GCA_REJECT:
    case GCA_RELEASE:
    {
	GCA_ER_DATA	*release = (GCA_ER_DATA *)it->gca_data_area;
...
{
    GCA_SD_PARMS	*send = &assoc->rql_send;
    GCA_FO_PARMS	*format = &assoc->rql_format;
    ...
    do
    {
        space_avail = format->gca_d_length - send->gca_msg_length;
        space_ptr   = format->gca_data_area + send->gca_msg_length;
	...
	*/
	...
	if ((len > space_avail) && (len <= format->gca_d_length))
	    amt = 0;
	...
	DD_DX_ID	*restart_dx_id)
{
	...
	*/
	        ...
	        != E_GC0000_OK)
	{
	    ...
	    */
		...
		disassociate.gca_association_id = gca_req.gca_assoc_id;
	        /* Ignore status */
	        (VOID)IIGCa_call(GCA_DISASSOC, (GCA_PARMLIST *)&disassociate,
	...
	rqu_format_write(&assoc->rql_format, assoc->rql_write_buff,
			(i4)sizeof(assoc->rql_write_buff),
			assoc->rql_assoc_id, &assoc->rql_send);
	rqu_fmt_read(assoc);

	/* Check for association request acceptance */
	assoc->rql_it.gca_data_area = NULL;
	assoc->rql_status = RQL_OK;
	assoc->rql_id_error = 0;
	assoc->rql_comm.rqc_rxtx = RQC_IDDLE;
	assoc->rql_comm.rqc_status = RQC_OK;
	...
	{
	    if (assoc->rql_it.gca_data_area)
	    {
	        GCA_ER_DATA     *er;
	        er = (GCA_ER_DATA *)assoc->rql_it.gca_data_area;
	...
	else
	{
	    ...
	    rqu_format_write(&new_assoc->rql_format, new_assoc->rql_write_buff,
			     (i4)sizeof(new_assoc->rql_write_buff),
			     new_assoc->rql_assoc_id, &new_assoc->rql_send);
	    rqu_fmt_read(new_assoc);
	...
	bool		ingres_b)
{
  ...
  parms = (GCA_SESSION_PARMS *)assoc->rql_format.gca_data_area;
	...
	i4		timeout)
{
    ...
    GCA_IT_PARMS	*interpret = &assoc->rql_it;
    STATUS		status;
    i4			n_errs = 0;
    i4			iack_count = 0;	
    RQS_CB	        *rqs = (RQS_CB *)assoc->rql_rqr_cb->rqr_session;
  ...
  */
  ...
  do
  {
	...
	if (timeout == 0)	/* peeking read */
	{
	    ...
	    */
	    IIGCa_call(GCA_RECEIVE, (GCA_PARMLIST *)receive, GCA_SYNC_FLAG, 
	...
	interpret->gca_buffer = (PTR)receive->gca_buffer;
	(VOID)IIGCa_call(GCA_INTERPRET, (GCA_PARMLIST *)interpret, 
			 GCA_SYNC_FLAG, (PTR)0, (i4)-1, &status);

	if (status != E_GC0000_OK)
	{
	     RQTRACE(rqs,1)(RQFMT(rqs,
				"RQF:Assn %d. "), assoc->rql_assoc_id);
	    (VOID)rqu_dbg_error( assoc, status );
	}

	/* log the GCA receipt */
	(VOID)rqu_gcmsg_type(GCA_RECEIVE, interpret->gca_message_type, assoc);

	switch(interpret->gca_message_type)
	{
	    case GCA_RELEASE:
	    case GCA_REJECT:
	    case GCA_ERROR:
	    	...
	    	*/
		rqu_gca_error( assoc, (GCA_ER_DATA *)interpret->gca_data_area );
		if (assoc->rql_status == RQL_OK )
		{
		    GCA_ER_DATA	*er = (GCA_ER_DATA *)interpret->gca_data_area;
		    /*
		...
		if (   (interpret->gca_message_type == GCA_RELEASE) 
		    || (interpret->gca_message_type == GCA_REJECT) )
		{
		    assoc->rql_turned_around = TRUE;
		    assoc->rql_status = RQL_TERMINATED;
		    assoc->rql_comm.rqc_status = RQC_ASSOC_TERMINATED;
	    ...
	    */
	    ...
	    case GCA_REFUSE:
	    case GCA_DONE:
		re =  (GCA_RE_DATA *)assoc->rql_it.gca_data_area;
	...
	"RQF:Assn %d. PROTOCOL VIOLATION, too many GCA_IACKs.\n"), 
							assoc->rql_assoc_id);
		    return(interpret->gca_message_type);
		}
		break;
	    case GCA_RETPROC:
	...
	*/
	      ...
	      interpret->gca_message_type == GCA_ERROR 
		|| 
	      interpret->gca_message_type == GCA_TRACE 
		||
	      interpret->gca_message_type == GCA_IACK
		||
	      interpret->gca_message_type == GCA_EVENT
	    ); 
	...
	i4		pagesize)
{
    ...
    char		qbuffer[100];/* buffer used to construct create query */
    ...
    ** query execution.
    */
    if (src_assoc->rql_it.gca_message_type != GCA_TDESCR)
    {
	/*
	...
	*/
    ...
    src_assoc->rql_b_ptr = src_assoc->rql_it.gca_data_area;
    src_assoc->rql_b_remained = src_assoc->rql_it.gca_d_length;
	...
	rqu_putformat(&col_desc, lang, qbuffer);
	       ...
	       i4               *eo_tuple)
{
	...
	*eo_tuple = TRUE;
	...
	switch (reply = rqu_get_reply(assoc, -1))
	{
		...
		assoc->rql_b_ptr = assoc->rql_it.gca_data_area;
		assoc->rql_b_remained = assoc->rql_it.gca_d_length;
		if (eo_tuple && los_present)
		    *eo_tuple = assoc->rql_it.gca_end_of_data;
...
rqu_getqid(
	...
	DB_CURSOR_ID	*qid)
{
  GCA_ID	*gca_qid = (GCA_ID *)assoc->rql_it.gca_data_area;
  RQS_CB	*rqs = (RQS_CB *)assoc->rql_rqr_cb->rqr_session;
...
{
    DB_ERRTYPE   status = E_RQ0000_OK;
    GCA_RP_DATA  *rp  = (GCA_RP_DATA *)assoc->rql_it.gca_data_area;
    RQS_CB	 *rqs = (RQS_CB *)assoc->rql_rqr_cb->rqr_session;
	...
	DB_DATA_VALUE	*col)
{
    ...
    if (! 
	 (
	  assoc->rql_it.gca_message_type == GCA_TDESCR
	   ||
	  assoc->rql_it.gca_message_type == GCA_C_FROM
	   ||
	  assoc->rql_it.gca_message_type == GCA_C_INTO
	 )
       )
    {
	...
	RQL_ASSOC	*assoc)
{
  ...
  /*
    ...
    query execution.
  */
  if (assoc->rql_it.gca_message_type != GCA_TDESCR)
	...
	i4	    interrupt_type)
{
    ...
    attention = (GCA_AT_DATA *)assoc->rql_format.gca_data_area;
	...
	if ( rx_status != E_DB_OK )
	{
        ...
        interpret->gca_buffer = receive->gca_buffer;
        (VOID)IIGCa_call(GCA_INTERPRET, (GCA_PARMLIST *)interpret, GCA_SYNC_FLAG, 
                    (PTR)0, (i4)-1, &status);
        if (status != E_GC0000_OK)
        {
	    RQTRACE(rqs,1)(RQFMT(rqs,
        ...
        /* log the GCA receipt */
        rqu_gcmsg_type(GCA_RECEIVE, interpret->gca_message_type, assoc);
    ...
    if ( (interpret->gca_message_type == GCA_IACK) && (assoc->rql_comm.rqc_status == RQC_OK) )
    {
	STATUS  rv;
	...
	*/
    ...
    else if ( (interpret->gca_message_type != GCA_IACK) && (assoc->rql_comm.rqc_status == RQC_OK) )
    {
	RQTRACE(rqs,1)(RQFMT(rqs,
	...
	RQL_ASSOC *assoc)
{
	...
	** synchrounous call 
	*/
	(VOID)IIGCa_call(req, (GCA_PARMLIST *)msg, (i4)GCA_SYNC_FLAG, 
		(PTR)0, (i4)-1, &status);
	return(status);
    }
    else
	...
	*/
	do
	{
		...
		(VOID)IIGCa_call(req, (GCA_PARMLIST *)msg, (i4)gca_indicator, 
	...
	RQTRACE(rqs,2)(RQFMT(rqs,
			"Req = %d, CSsuspend = %x, IIGCa_call = %p,"),
		        req, rv, *poll_addr );
	RQTRACE(rqs,2)(RQFMT(rqs,
	...
	RQL_ASSOC *assoc)
{
    ...
    */
#if 0
    (VOID)IIGCa_call(req, (GCA_PARMLIST *)msg, GCA_NO_ASYNC_EXIT, 
		 (PTR)&assoc->rql_scb, (i4)-1, &status);
#else
    (VOID)IIGCa_call(req, (GCA_PARMLIST *)msg, GCA_NO_ASYNC_EXIT, 
		 (PTR)assoc->rql_scb, (i4)-1, &status);
...
#define		RQF_MAX_RETRY	5
    ...
    */
    ...
    {
        RQTRACE(rqs,2)(RQFMT(rqs,
		   "RQF:Assn %d. IIGCa_call already exists, req=%d(%x).\n"),
		    assoc->rql_assoc_id, req, req);

    }
    else
    {
	(VOID)IIGCa_call(req, (GCA_PARMLIST *)msg, (i4)0, 
		 (PTR)assoc->rql_scb, (i4)-1, &status);
    }
	...
	RQL_ASSOC    *assoc)
{
    ...
    itp = (dcp != (SCC_SDC_CB *)NULL) ? 
	  (GCA_IT_PARMS *)dcp->sdc_client_msg : (GCA_IT_PARMS *)NULL;

    if ((rqr == (RQR_CB *)NULL) || (rqs == (RQS_CB *)NULL) ||
	(dcp == (SCC_SDC_CB *)NULL) || (itp == (GCA_IT_PARMS *)NULL))
	return;
...
**		.sdc_it		    - GCA_IT_PARMS if interrupted
**        
** Returns:
**      DB_ERRTYPE
	...
	RQR_CB  *rcb)
{
    ...
    else if ( receive->gca_status == E_GC0000_OK )
    {
       ...
       dc->sdc_msg_type = interpret->gca_message_type;
       dc->sdc_ldb_msg = (PTR)interpret->gca_buffer;
       dc->sdc_ldb_status = SDC_INTERRUPTED;
       dc->sdc_it = (PTR)interpret;
       return (E_DB_OK);
    }
    else
    {
