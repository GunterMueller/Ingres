          ...
          i4            buffsize)
{
    ...
    DMP_EXT		*ext;
    SCF_CB              scf_cb;
    DMP_RCB		*tab_rcb = NULL;
    DMP_TCB		*tab_tcb = NULL;
    DM_MDATA		*load_data;
    char		*table;
    char		*inputfile;
    ...
    *err_code = E_DB_OK;
    ...
    {
	ule_format(E_DM1612_FLOAD_BADARGS, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
        *err_code = E_DM1612_FLOAD_BADARGS;
	return (E_DB_ERROR);
    }
    ...
    */
    ...
    /*
    ** Init DML transaction control block and queue it to the session (above)
    */
    MEfill(sizeof(DML_XCB), 0, &dml_xcb);
    dml_xcb.xcb_x_type = XCB_UPDATE;
    dml_xcb.xcb_state = 0;
    dml_xcb.xcb_flags = 0;
    dml_xcb.xcb_seq_no = 0;
    dml_xcb.xcb_length = sizeof(DML_XCB);
    dml_xcb.xcb_type = XCB_CB;
    dml_xcb.xcb_ascii_id = XCB_ASCII_ID;
    dml_xcb.xcb_odcb_ptr = &dml_odcb;
    dml_xcb.xcb_rq_next = (DMP_RCB *)&dml_xcb.xcb_rq_next;
    dml_xcb.xcb_rq_prev = (DMP_RCB *)&dml_xcb.xcb_rq_next;
    dml_xcb.xcb_sq_next = (DML_SPCB *)&dml_xcb.xcb_sq_next;
    dml_xcb.xcb_sq_prev = (DML_SPCB *)&dml_xcb.xcb_sq_next;
    dml_xcb.xcb_cq_next = (DML_XCCB *)&dml_xcb.xcb_cq_next;
    dml_xcb.xcb_cq_prev = (DML_XCCB *)&dml_xcb.xcb_cq_next;
    dml_xcb.xcb_seq  = (DML_SEQ*)NULL;
    dml_xcb.xcb_cseq = (DML_CSEQ*)NULL;
    /* queue to session */
    dml_xcb.xcb_q_next = dml_scb.scb_x_next;
    dml_xcb.xcb_q_prev = (DML_XCB *)&dml_scb.scb_x_next;
    dml_scb.scb_x_next->xcb_q_prev = &dml_xcb;
    dml_scb.scb_x_next = &dml_xcb;
    dml_xcb.xcb_scb_ptr = &dml_scb;
    ...
    */
    {
        ...
        if (status != E_DB_OK)
        {
            ule_format(adf_scb.adf_errcb.ad_errcode,
                0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
            *err_code = E_DM007F_ERROR_STARTING_SERVER;
            return(E_DB_ERROR);
        }
    ...
    */
    ...
    else
    {
        ...
        if (cl_stat != OK)
        {
            ule_format(E_DM1600_FLOAD_OPEN_FILE, (CL_SYS_ERR *)0, ULE_LOG, NULL,
                (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM1600_FLOAD_OPEN_FILE;
            return (E_DB_ERROR);
        }
    }
    file_open = TRUE;
    ...
    */
    ...
    status = dm2d_open_db(dcb, DM2D_A_WRITE, DM2D_IX,
                          dmf_svcb->svcb_lock_list, 0, 
                          err_code);
    if (status != E_DB_OK)
    {
        ule_format(E_DM1601_FLOAD_OPEN_DB, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
        *err_code = E_DM1601_FLOAD_OPEN_DB; 
        return (E_DB_ERROR);
    }
    dbopen = TRUE;
	...
	*/
	    ...
	    (char *)&dml_scb, (DM_OBJECT **)&dml_loc_masks, err_code);
        if (status != E_DB_OK)
        {
            ule_format(E_DM160D_FLOAD_NOMEM, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	        (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM160D_FLOAD_NOMEM; 
            break;
        }
	...
	dml_loc_masks->locm_d_allow =
        ...
        /*
        ** Init SCF for blob functioanlity
        */
        ...
        if (status != E_DB_OK)
	{
            ule_format(E_DM1618_FLOAD_INIT_SCF, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	        (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM1618_FLOAD_INIT_SCF; 
	    break;
	}
	...
	** start a transaction
	*/
	status = dmxe_begin(DMXE_WRITE, DMXE_INTERNAL, dcb, dcb->dcb_log_id,
	    &jsx->jsx_username, dmf_svcb->svcb_lock_list, &log_id, &tran_id,
	    &lock_id, (DB_DIS_TRAN_ID *)NULL, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(E_DM1602_FLOAD_START_XACT,(CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM1602_FLOAD_START_XACT;
	    break;
	}
	open_xact = TRUE;
	dml_scb.scb_x_ref_count = 1;
	dml_xcb.xcb_x_type = XCB_UPDATE;
	dml_xcb.xcb_flags = XCB_NOLOGGING;
	dml_xcb.xcb_log_id = log_id;
	dml_xcb.xcb_lk_id = lock_id;
	STRUCT_ASSIGN_MACRO(tran_id, dml_xcb.xcb_tran_id);
	/* Lock id needs to be in SCB too, for temp etab holders */
	dml_scb.scb_lock_list = lock_id;
	/*
	** get table ID from name
	*/
    	MEcopy(table, DB_MAXNAME, tab_name.db_tab_name);

	status = dml_get_tabid(dcb, &tab_name, &tab_owner, &tab_id, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(E_DM1603_FLOAD_SHOW, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM1603_FLOAD_SHOW;
	    break;
	}
	/*
	** open the table
	*/
	...
	status = dm2t_open(dcb, &tab_id, DM2T_X, DM2T_UDIRECT, DM2T_A_WRITE,
	    (i4)0, (i4)20, (i4)0, dml_xcb.xcb_log_id, 
	    dml_xcb.xcb_lk_id, 0, 0, 0, &tran_id,
	    &timestamp, &tab_rcb, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(E_DM1604_FLOAD_OPEN_TABLE,(CL_SYS_ERR *)0, ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM1604_FLOAD_OPEN_TABLE;
	    break;
	}
	table_open = TRUE;
	/* queue RCB to XCB */
	tab_rcb->rcb_xcb_ptr = &dml_xcb;
	tab_rcb->rcb_xq_next = dml_xcb.xcb_rq_next;
	tab_rcb->rcb_xq_prev = (DMP_RCB *)&dml_xcb.xcb_rq_next;
	dml_xcb.xcb_rq_next->rcb_q_prev = (DMP_RCB *)&tab_rcb->rcb_xq_next;
	dml_xcb.xcb_rq_next = (DMP_RCB *)&tab_rcb->rcb_xq_next;
	/*
	** set nologging
	*/
	tab_rcb->rcb_logging = 0;
	/*
	** get the TCB, this has already been fixed by the table open
	** but we need a handle to it for the load.
	*/
	status = dm2t_fix_tcb(dcb->dcb_id, &tab_id, &tran_id, 
	    dml_xcb.xcb_lk_id, dml_xcb.xcb_log_id, DM2T_NOPARTIAL, dcb,
	    &tab_tcb, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(E_DM160B_FLOAD_NO_TCB, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM160B_FLOAD_NO_TCB;
	    break;
	}
	tcb_fixed = TRUE;
	/*
	...
	*/
	...
	{
	    ule_format(E_DM1615_FLOAD_BAD_TABLE_FORMAT, (CL_SYS_ERR *)0, 
		ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
		err_code, 0);
            *err_code = E_DM1615_FLOAD_BAD_TABLE_FORMAT;
	    break;
	}

	/* ***** TEMP UNTIL WE HAVE PARTITIONED LOAD DONE */
	if (tab_tcb->tcb_rel.relstat & TCB_IS_PARTITIONED)
	{
	...
	status = dm2r_position(tab_rcb, DM2R_ALL, (DM2R_KEY_DESC *) 0,
	    ...
	    if (status != E_DB_OK)
	    {
	        ule_format(E_DM1617_FLOAD_TABLE_ERROR, (CL_SYS_ERR *)0, 
		    ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
		    err_code, 0);
            	*err_code = E_DM1617_FLOAD_TABLE_ERROR;
	    	break;
	    }
	    ...
	    status = dm2r_get(tab_rcb, &tid, DM2R_GETNEXT, rec_buf, err_code);
	...
	if (status == E_DB_OK)
	{
	    ...
	    {
	    	ule_format(E_DM1616_FLOAD_TABLE_NOT_EMPTY, (CL_SYS_ERR *)0, 
		    ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
		    err_code, 0);
                *err_code = E_DM1616_FLOAD_TABLE_NOT_EMPTY;
	        (VOID)MEfree(rec_buf);
	        break;
	    }
	    else
	...
	*/
	...
	status = dm2r_load(tab_rcb, tab_tcb, DM2R_L_BEGIN | DM2R_L_FLOAD, 
	    flags, &rows_read, 
	    NULL, nrows, NULL, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(E_DM160F_FLOAD_LOAD_ERR, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM160F_FLOAD_LOAD_ERR;
	    break;
	}
        /*
	...
	*/
        ...
        while (!done)	
	{
		...
		if (record_offset > 0)
		{
		    ...
		    {
	    		ule_format(E_DM1619_FLOAD_BAD_READ, (CL_SYS_ERR *)0, 
			    ULE_LOG, NULL, (char *)0, (i4)0, 
			    (i4 *)0, err_code, 0);
            		*err_code = E_DM1619_FLOAD_BAD_READ;
			status = E_DB_ERROR;
			break;
		    }
		}
		status = handle_peripheral(readbuffer + record_offset, 
		    fdesc, tab_tcb, att_num, err_code);
		...
		for (i = att_num + 1; i <= tab_tcb->tcb_rel.relatts; i++)
		{
			...
			*/
			if (record_offset < tab_tcb->tcb_atts_ptr[i].offset)
			{
			    ...
			    {
	    		        ule_format(E_DM1619_FLOAD_BAD_READ, 
		...
		*/
	    ...
	    else /* standard read, no peripherals */
	    {
		    ...
		    load_data, nrows, NULL, err_code);
	        if (status != E_DB_OK)
	        {
	    	    ule_format(E_DM160F_FLOAD_LOAD_ERR, (CL_SYS_ERR *)0,
                               ULE_LOG, NULL,
	      	        (char *)0, (i4)0, (i4 *)0, err_code, 0);
                    *err_code = E_DM160F_FLOAD_LOAD_ERR;
	    	    break;
	...
	*/
	status = dm2r_load(tab_rcb, tab_tcb, DM2R_L_END | DM2R_L_FLOAD, 0, 
	    &rows_read, NULL, 0, NULL, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(E_DM160F_FLOAD_LOAD_ERR, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, err_code, 0);
	...
	** close the table
	*/ 
	status = dm2t_close(tab_rcb, DM2T_NOPURGE, err_code); 
	if (status != E_DB_OK) 
	{ 
	    ule_format(E_DM1605_FLOAD_CLOSE_TABLE,(CL_SYS_ERR *)0,ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM1605_FLOAD_CLOSE_TABLE;
	    break;
	}
	table_open = FALSE;
	/*
	** unfix the tcb and toss it from the cache
	*/
	status = dm2t_unfix_tcb(&tab_tcb, (DM2T_PURGE | DM2T_NORELUPDAT),
	    dml_xcb.xcb_lk_id, dml_xcb.xcb_log_id, err_code);
	tcb_fixed = FALSE;
	if (status != E_DB_OK)
	{ 
	    ule_format(E_DM1611_FLOAD_UNFIX_TCB, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM1611_FLOAD_UNFIX_TCB;
	    break;
	}
	...
	*/
	if (have_peripherals)
	    (void) dmpe_free_temps(&dml_odcb, &local_err);

	/*
	** end the transaction
	*/
	status = dmxe_commit(&tran_id, dml_xcb.xcb_log_id,dml_xcb.xcb_lk_id,
				0, &timestamp, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(E_DM1606_FLOAD_COMMIT_XACT,(CL_SYS_ERR *)0,ULE_LOG, NULL,
	    	(char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM1606_FLOAD_COMMIT_XACT;
	    break;
	}
	/* Avoid changing the message if not blobs... */
	if (have_peripherals)
	    SIprintf("Row size   : %d\nRows loaded: %d\nTotal bytes (excluding variable length data): %d\n",
                 (i4)row_size, rows_done, row_size * rows_done);
	else
	    SIprintf("Row size   : %d\nRows loaded: %d\nTotal bytes: %d\n",
                 (i4)row_size, rows_done, row_size * rows_done);
	open_xact = FALSE;
        ...
        ** (Copied, with mods, from dmxcommit.c by musro02
        */

        while (dml_xcb.xcb_cq_next != (DML_XCCB*) &dml_xcb.xcb_cq_next)
        {
            /* Get pend XCCB */

            dcb = dml_xcb.xcb_odcb_ptr->odcb_dcb_ptr;
            xccb = dml_xcb.xcb_cq_next;
            ...
            if (xccb->xccb_operation == XCCB_F_DELETE)
            {
                ...
                sizeof(DM_PATH), (char *)&l_ext.physical);
                l_ext.phys_length = xccb->xccb_p_len;
                ...
                loc.loc_fcb = &fcb;
                /* 
                ** Location id is not important since file name is
                ** in the xccb. Set it to zero. 
                */
                loc.loc_id = 0;
                loc.loc_status = 0;    /* not really a valid loc */
                status = dm2f_delete_file(dml_scb.scb_lock_list,
                        dml_xcb.xcb_log_id, &loc, loc_count, 
                        (DM_OBJECT *)dcb, (i4)0, err_code);
                if (status)
                {
                    ule_format( *err_code, 0, ULE_LOG , NULL,
                        (char * )0, 0L, (i4 *)0, err_code, 0);
                    ule_format(E_DM0095_ERROR_COMMITING_TRAN, (CL_SYS_ERR *)0,
                        ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
                }
            }
            dm0m_deallocate((DM_OBJECT **)&xccb);
        }  /* end while */
        /*  end of code copied from dmx_commit() */
	open_xact = FALSE;
	/*
	** close the database
	*/
	status = dm2d_close_db(dcb, dmf_svcb->svcb_lock_list,
	    (DM2D_NLK | DM2D_NLG), err_code);
	if (status != E_DB_OK)
	{
	    ule_format(E_DM1607_FLOAD_CLOSE_DB, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    	(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM1607_FLOAD_CLOSE_DB;
	    break;
	}
	dbopen = FALSE;
	/*
	...
	*/
	...
	if (cl_stat != OK)
	{
	    ule_format(E_DM1608_FLOAD_CLOSE_FILE, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, (char *)0, (i4)0, (i4 *)0, NULL, 0);
            *err_code = E_DM1608_FLOAD_CLOSE_FILE;
	    break;
	}
	/*
	** memory cleanup
    ...
    */
    if (table_open)
    {
	local_stat = dm2t_close(tab_rcb, DM2T_NOPURGE, &local_err); 
	if (local_stat != E_DB_OK) 
	{ 
	    ule_format(local_err, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, &local_err, 0);
	    ule_format(E_DM1605_FLOAD_CLOSE_TABLE, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, (char *)0, (i4)0, (i4 *)0, NULL, 0);
            *err_code = E_DM1605_FLOAD_CLOSE_TABLE;
	}
    }
    if (tcb_fixed)
    {
	local_stat = dm2t_unfix_tcb(&tab_tcb, (DM2T_PURGE | DM2T_NORELUPDAT),
	    dml_xcb.xcb_lk_id, dml_xcb.xcb_log_id, &local_err);
	if (local_stat != E_DB_OK)
	{ 
	    ule_format(local_err, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	      (char *)0, (i4)0, (i4 *)0, &local_err, 0);
	    ule_format(E_DM1611_FLOAD_UNFIX_TCB, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, (char *)0, (i4)0, (i4 *)0, NULL, 0);
            *err_code = E_DM1611_FLOAD_UNFIX_TCB;
	}
    }
    if (open_xact)
    {
	local_stat = dmxe_abort(NULL, dcb, &tran_id, dml_xcb.xcb_log_id, 
			dml_xcb.xcb_lk_id, 0, 0, NULL, &local_err);
	if (local_stat != E_DB_OK)
	{
	    ule_format(local_err, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &local_err, 0);
	    ule_format(E_DM1609_FLOAD_ABORT_XACT, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, (char *)0, (i4)0, (i4 *)0, NULL, 0);
            *err_code = E_DM1609_FLOAD_ABORT_XACT;
	}
    }
    if (dbopen)
    {
	local_stat = dm2d_close_db(dcb, dmf_svcb->svcb_lock_list, 
	    (DM2D_NLK | DM2D_NLG), &local_err);
	if (local_stat != E_DB_OK)
	{
	    ule_format(local_err, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &local_err, 0);
	    ule_format(E_DM1607_FLOAD_CLOSE_DB, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, NULL, 0);
            *err_code = E_DM1607_FLOAD_CLOSE_DB;
	}
    }
    if (file_open)
		...
		i4		*err_code)
{
	...
	*/
	...
	status = dm2t_open(dcb, &catalog_id, DM2T_IS, DM2T_UDIRECT,
	    ...
	    &timestamp, &r_idx, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	    break;
	/*
	...
	** close iirel_idx
	*/
	status = dm2t_close(r_idx, DM2T_NOPURGE, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	tran_id.db_high_tran = NULL;
	tran_id.db_low_tran = NULL;

	status = dm2t_open(dcb, &catalog_id, DM2T_IS, DM2T_UDIRECT, 
	    DM2T_A_READ, (i4)0, (i4)20, 0, 0, 
	    dmf_svcb->svcb_lock_list, (i4)0, (i4)0, 0, &tran_id, 
	    &timestamp, &rel, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	status = dm2t_close(rel, DM2T_NOPURGE, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
    if (r_idx)
	local_stat = dm2t_close(r_idx, DM2T_NOPURGE, &local_err);

    if (rel)
	local_stat = dm2t_close(rel, DM2T_NOPURGE, &local_err);

    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
	(i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM160C_FLOAD_NO_TABLE;
		...
		i4		*err_code)
{
	...
	*/
	...
	if (cl_stat != OK || bytes_read != 2)
	{
	    ...
	    ule_format(E_DM1619_FLOAD_BAD_READ, (CL_SYS_ERR *)0, ULE_LOG, 
	...
	** to overrite memory and loop
	*/
	if (seg_length > MAX_SEG_SIZE || seg_length < 0)
	{
	    /* We've read an invalid length! */
	    ule_format(E_DM1619_FLOAD_BAD_READ, (CL_SYS_ERR *)0, ULE_LOG, 
			NULL, (char *)0, (i4)0, (i4 *)0, err_code, 0);
	...
	nextseg_length = seg_length;
		...
		** to overrite memory and loop
		*/
		if (nextseg_length > MAX_SEG_SIZE || nextseg_length < 0)
		{
		    /* We've read an invalid length! */
		    ule_format(E_DM1619_FLOAD_BAD_READ, (CL_SYS_ERR *)0, ULE_LOG, 
				NULL, (char *)0, (i4)0, (i4 *)0, err_code, 0);
	    ...
	    */
	    ...
	    {
	    	ule_format(E_DM161A_FLOAD_COUPONIFY_ERROR, (CL_SYS_ERR *)0, 
		    ULE_LOG, NULL, (char *)0, (i4)0, 
		    (i4 *)0, err_code, 0);
            	*err_code = E_DM161A_FLOAD_COUPONIFY_ERROR;
		status = E_DB_ERROR;
		break;
	    }
	    else
