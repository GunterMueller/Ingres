...
*/
static	DB_STATUS   rcp_recover(i4 *err_code);

static	VOID	    rcp_shutdown(VOID);

static	DB_STATUS   count_opens(i4 open_database);

static  VOID        rcp_commit(VOID);    /* Manually commit a transaction. */

static	DB_STATUS   dmr_error_translate(
				i4		new_error,
				i4		old_error);

DB_STATUS   dmr_log_error(
				DB_STATUS           err_code,
				CL_SYS_ERR	    *syserr,
				i4		    p,
				i4		    ret_err_code);
...
dmr_rcp_init(LG_LGID	*recovery_log_id)
{
    ...
    /* Create filename. Non cluster is DMFRCP.LOG, VAXcluster
    ** is DMFRCP.LOG_NODE, where NODE is CX_MAX_NODE_NAME_LEN+1
    ...
    */
	...
	CXnickname_from_node_name(node_name, node_name);
    }    
    else
    ...
    {
	ule_format(status, (CL_ERR_DESC *)0, ULE_LOG, NULL, (char *)0, 0L, 
	    (i4 *)0, &err_code, 0);
    }
    else
    {
	...
	*/
    ...
    /*
    ** Check if we should crash rather than marking a 
    ** database inconsistent
    */

    Db_incons_crash = FALSE;
    NMgtAt( ERx("II_DMFRCP_STOP_ON_INCONS_DB"), &env);
    if ( env && *env)
    {
	if ( CMcmpnocase(env, "Y") == 0)
	{
	    TRdisplay("%@RCP II_DMFRCP_STOP_ON_INCONS_DB enabled.\n");
	    Db_incons_crash = TRUE;
	}
    }
	...
	if (status != OK)
	{
	    (VOID) ule_format( status, (CL_ERR_DESC *)&sys_err, ULE_LOG, NULL,
				0, 0, 0, &err_code, 0 );
	    break;
	}
	...
	*/
	...
	if (status)
	{
	    ...
	    ule_format(E_DM9401_RCP_LOGINIT, 0, ULE_LOG, NULL, 
		    (char *)0, 0L, (i4 *)0, &err_code, 0);
	    break;
	}
	...
	{
	    (VOID) ule_format(E_DM901E_BAD_LOCK_INIT, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &err_code, 0);
	...
	*/
			    ...
			    rconf.bsize, &sys_err)) != OK)
	{
	    ...
	    _VOID_ ule_format(E_DM9012_BAD_LOG_OPEN, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &err_code, 0);

	    _VOID_ ule_format(E_DM9401_RCP_LOGINIT, 0, ULE_LOG, NULL, 0, 0, 0,
		    &err_code, 0);
	    break;
	} 
	/*
	...
	*recovery_log_id = rcp_lg_id;
	...
	MEcopy((PTR)DB_RECOVERY_NAME, DB_MAXNAME, (PTR) &add_info.ad_dbname);
	    ...
	    &rcp_db_id, &sys_err))
	{
	    ...
	    (char *)0, 0L, (i4 *)0, &err_code, 4, 0, rcp_lg_id,
	    0, "$recovery",
	    0, "$ingres",
	    0, "None");

	    ule_format(E_DM9401_RCP_LOGINIT, 0, ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	    break;
	}
	...
	** called.
	*/
	MEcopy((PTR)DB_RECOVERY_NAME, sizeof(DB_OWN_NAME), (PTR) &user_name);
...
dmr_log_init(void)
{
	...
	*/
	...
	if (dm0l_bcp(rcp_lx_id, &bcp_la, &bcp_lsn, &err_code))
	{
	    status = dmr_error_translate(E_DM9401_RCP_LOGINIT, err_code);
	    break;
	}

	if (dm0l_ecp(rcp_lx_id, &bcp_la, &bcp_lsn, &err_code))
	{
	    status = dmr_error_translate(E_DM9401_RCP_LOGINIT, err_code);
	    break;
	}
	...
	TRdisplay("\n%@ RCP Dual logging is %w.\n",
		    "NOT enabled,ENABLED", rconf.dual_logging);
...
rcp_recover(i4 *err_code)
{
	...
	*/
	status = dm0l_force(rcp_lx_id, (LG_LSN *)0, &lsn, &error);
	if (status != E_DB_OK)
	    break;
    ...
    */
    ...
    for (;;)
    {
	status = dmr_online_context(rcp, &error);
	if (status != E_DB_OK)
	    break;
	...
	status = release_orphan_locks(rcp, &error);
	if (status != E_DB_OK)
	    break;
...
LG_LXID		lx_id)
{
	...
	*/
	...
	if (cl_status)
	{
		...
		(char *)0, (i4)0, (i4 *)0, &error, 1, 0, mask);
	    break;
	}
	...
	*/
	if ((event & LG_E_RECOVER) && flag != LG_NOWAIT)
	{
	    ...
	    */
	    if (cl_status = LGalter(LG_A_RCPRECOVER,
				    (PTR)&false, sizeof(false), &sys_err))
	    {
		ule_format(cl_status, &sys_err, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, &error, 0);
		ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, NULL, 
	...
	*/
	...
	{
	    status = dm0l_bcp(real_lx_id, &bcp_la, &bcp_lsn, &error);
	    if (status != E_DB_OK)
		break;
	}
	if ((event & LG_E_ECP) && flag != LG_NOWAIT)
	{
	    status = dm0l_ecp(real_lx_id, &bcp_la, &bcp_lsn, &error);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	...
	if (status)
	{
	    ule_format(error, &sys_err, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &error, 0);
	}
	ule_format(E_DM940F_RCP_CHKEVENT, &sys_err, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, &error, 0);
...
rcp_shutdown(VOID)
{
    ...
    */
    ...
    if ((cl_status == OK) && (lgd_status & 0x1000) && recovery_done)
	clean_shutdown = TRUE;
    else
	clean_shutdown = FALSE;
	...
	*/
	status = dm0l_force(rcp_lx_id, (LG_LSN *)0, (LG_LSN *)0, 
			    &err_code);
	if (DB_FAILURE_MACRO(status))
	{
	    status = dmr_error_translate(E_DM9410_RCP_SHUTDOWN, err_code);
	    clean_shutdown = FALSE;
	}
	...
	*/
	if (clean_shutdown)
	{
	    status = dm0l_bcp(rcp_lx_id, &bcp_la, &bcp_lsn, &err_code);
 
	    if (DB_SUCCESS_MACRO(status))
		status = dm0l_ecp(rcp_lx_id, &bcp_la, &bcp_lsn, &err_code);
    ...
    */
	...
	TRdisplay("%@ RCP SHUTDOWN completed normally.\n");
	ule_format( E_SC0128_SERVER_DOWN, (CL_SYS_ERR *)0, ULE_LOG, NULL,
    ...
    {
	ule_format(E_DM9424_RCP_HARD_SHUTDOWN, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, &err_code, 0);
	TRdisplay("%@ RCP Shutdown completed.\n");
	TRdisplay("%@ RCP Recovery will be performed after system restart.\n");
	ule_format( E_SC0127_SERVER_TERMINATE, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, &err_code, 0);
    }
...
i4	    open_database)
{
	...
	if (open_database == 0)
	{
	    ...
	    */
		...
		ule_format(E_DM9413_RCP_COUNT_OPENS, 0, ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		continue;
	    }
	...
	if (local_status != E_DB_OK)
	    status = dmr_error_translate(E_DM9413_RCP_COUNT_OPENS, err_code);
	if (open_database == 0)
	{
		...
		** better mark it inconsistent. 
		...
		if ( Db_incons_crash )
		{
		    ...
		    ule_format(E_DM942A_RCP_DBINCON_FAIL, (CL_SYS_ERR *)0,
...
rcp_commit(VOID)
{
	...
	*/
	...
	if (cl_status != OK)
	{
		...
		0, LG_A_LXB_OWNER);
	    ule_format(E_DM9422_RCP_COMMIT, (CL_SYS_ERR *)0, ULE_LOG, 
	...
	if (LKcreate_list((i4)LK_RECOVER, 
		    ...
		    &sys_err))
	{
	    (VOID) ule_format(E_DM901A_BAD_LOCK_CREATE, &sys_err, 
                    ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, &err_code, 0);

	    ule_format(E_DM9422_RCP_COMMIT, 0, ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	    break;
	}
	...
	if (dm0l_et(tr.tr_id, et_flags, DM0L_COMMIT, 
		(DB_TRAN_ID *)&tr.tr_eid, tr.tr_db_id, &ctime, &err_code))
	{
	    status = dmr_error_translate(E_DM9422_RCP_COMMIT, err_code);
	    break;
	}
    ...
    */
	...
	for (rlp = rcp->rcp_lph[i]; rlp != NULL; rlp = rlp->rlp_next)
	{
	    ...
	    */
		...
		ule_format( E_DM940E_RCP_RCPRECOVER, (CL_SYS_ERR *)0, ULE_LOG, 
...

/*{
** Name: dmr_error_translate	- Convert the DMF error to RCP error code.
**
** Description:
...
i4		old_error)
{
    ...
    ule_format(old_error, 0, ULE_LOG, NULL, 0, 0, 0, &local_err_code, 0);
    ule_format(new_error, 0, ULE_LOG, NULL, 0, 0, 0, &local_err_code, 0);
    return(E_DB_ERROR);
}

/*{
** Name: dmr_log_error	- Log the CL error message, convert the error
**			  code to the proper RCP error code.
**
** Description:
...
i4		    ret_err_code)
{
    ...
    ule_format(err_code, syserr, ULE_LOG, NULL,
	(char *)0, 0L, (i4 *)0, &local_err_code, 1, 0, p);

    ule_format(ret_err_code, 0, ULE_LOG, NULL, 0, 0, 0, &local_err_code, 0);
