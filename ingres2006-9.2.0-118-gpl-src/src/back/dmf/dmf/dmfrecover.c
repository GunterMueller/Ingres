...
bool		*recovery_needed)
{
    ...
    */
    ...
    if (cl_status = LGshow(LG_A_HEADER,
			    ...
			    &sys_err))
    {
	_VOID_ ule_format(cl_status, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &error, 0);
	_VOID_ ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &error, 1,
		0, LG_A_HEADER);
	_VOID_ ule_format(E_DM9401_RCP_LOGINIT, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &error, 0);
	return (E_DB_ERROR);
    }
    ...
    if (status != E_DB_OK)
    {
	ule_format(E_DM943C_DMR_RECOVER, (CL_SYS_ERR *)0, ULE_LOG, NULL,
    ...
    */
    ...
    {
	if (status = dmr_get_cp(rcp, (i4)0, &error))
	{
    ...
    *recovery_needed = (header->lgh_status != LGH_OK);
	...
	_VOID_ ule_format(cl_status, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, &error, 0);
	_VOID_ ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, &error, 1,
	    0, LG_A_NODELOG_HEADER);
	_VOID_ ule_format(E_DM9405_RCP_GETCP, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &error, 0);
	return (E_DB_ERROR);
    }
	...
	_VOID_ ule_format(cl_status, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, &error, 0);
	_VOID_ ule_format(E_DM9010_BAD_LOG_FORCE, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, &error, 1,
	    0, lctx->lctx_lxid);
	_VOID_ ule_format(E_DM9405_RCP_GETCP, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &error, 0);
    ...
    */
    ...
    for (;;)
    {
	status = dmr_offline_context(rcp, node_id, &error);
	if (status != E_DB_OK)
	    break;

	status = dmr_analysis_pass(rcp, node_id, &error);
	if (status != E_DB_OK)
	    break;
	...
	status = dmr_init_recovery(rcp, &error);
	if (status != E_DB_OK)
	    break;

	status = dmr_redo_pass(rcp, node_id, &error);
	if (status != E_DB_OK)
	    break;

	status = dmr_undo_pass(rcp, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	{
	    status = dmr_update_bof(rcp, node_id, &error);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	...
	status = dmr_complete_recovery(rcp, 1, &error);
	if (status != E_DB_OK)
	    break;
...
i4	    num_nodes)
{
    ...
    */
	...
	ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &error, 0);
	ule_format(E_DM9407_RCP_RECOVER, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &error, 0);
	return(E_DB_ERROR);
    }
    ...
    */
    rcp = *result_rcp;
    rcp->rcp_db_count = 0;
    rcp->rcp_rdb_map = (RDB**) ((char *)rcp + sizeof(RCP));
    rcp->rcp_txh = 
      (RTX**) ((char *)rcp->rcp_rdb_map + (RCP_DB_HASH + 1) * sizeof(RDB *));
    rcp->rcp_lph = (RLP**) ((char *)rcp->rcp_txh + RCP_TX_HASH * sizeof(RTX *));
    ...
    for (i = 0; i < RCP_TX_HASH; i++)
	rcp->rcp_txh[i] = 0;
    for (i = 0; i <= RCP_DB_HASH; i++)
	rcp->rcp_rdb_map[i] = 0;
...
i4		*err_code)
{
	...
	*/
	...
	if (db.db_l_buffer < (2 * DB_MAXNAME + path_len + 8))
	{
	    ...
	    *err_code = E_DM940E_RCP_RCPRECOVER;
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	...
	if (rlp == 0)
	{
		...
		*err_code = E_DM940E_RCP_RCPRECOVER;
	    ...
	    status = add_rlp(rcp, tr.tr_pr_id, lpb.pr_pid, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	status = add_rtx(rcp, lctx, &tr.tr_eid, (LK_LLID)tr.tr_lock_id, tr.tr_id, 
		    ...
		    (LG_LA *) &tr.tr_cp, rdb, tr.tr_user_name, err_code);
	if (status != E_DB_OK)
	    break;
...
i4		*err_code)
{
    ...
    */
    status = dm0l_position(DM0L_FLGA, lctx->lctx_lxid,
			    &lctx->lctx_cp, lctx, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM9408_RCP_P0;
	return (E_DB_ERROR);
    }
    ...
    */
    for (;;)
    {
	status = dm0l_read(lctx, lctx->lctx_lxid, &record, &la, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
	...
	TRdisplay("%@ RCP-P0: Found non CP log record at <%d,%d,%d>\n",
	    ...
	    la.la_block,
	    la.la_offset);
	*err_code = E_DM942E_RCP_INCOMPLETE_CP;
	status = E_DB_ERROR;
    }
...
i4		*err_code)
{
    ...
    */
    ...
    status = dmr_preinit_recovery(rcp, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
    ...
    */
	...
	status = check_recovery(rcp, rdb, err_code);
	if (status != E_DB_OK)
	{
	    /*
	    ...
	    */
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    ule_format(E_DM9430_RCP_CHECK_REC_ERROR, (CL_SYS_ERR *)0, ULE_LOG,
	...
	for (;;)
	{
		...
		*/
		...
		if (cl_status != OK)
		{
		    ule_format(cl_status, &sys_err, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, err_code, 0);
		    ule_format(E_DM900A_BAD_LOG_DBADD, &sys_err, ULE_LOG, NULL,
		...
		*/
		    ...
		    (RCP_R_CSP_ONLINE != rcp->rcp_action))
		{
			...
			ule_format(cl_status, &sys_err, ULE_LOG, NULL,
			    (char *)0, (i4)0, (i4 *)0, err_code, 0);
			ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, 
	    ...
	    */
	    ...
	    status = dm2d_add_db(flags, &dm2mode, 
		     ...
		     (DM2D_LOC_ENTRY *)&location, &rdb->rdb_dcb, 0L, err_code);
	    if (status != E_DB_OK)
		break;
	    ...
	    */
		...
		rcp->rcp_lock_list, flags, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	if (status != E_DB_OK)
	{
	    ule_format(E_DM9431_RCP_OPENDB_ERROR, &sys_err, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 1,
		sizeof(rdb->rdb_name), &rdb->rdb_name);
	    mark_invalid(rcp, rdb, DSC_OPENDB_INCONSIST);
	    status = E_DB_OK;
	...
	*/
	if (status != E_DB_OK)
	{
	    ule_format(E_DM9432_RCP_OPENTX_ERROR, &sys_err, ULE_LOG, NULL,
    ...
    */
    ...
    dmr_check_event(LG_NOWAIT, rcp->rcp_lctx[0]->lctx_lxid);
...
i4		*err_code)
{
    ...
    */
	...
	{
	    ule_format(E_DM9433_RCP_INCOMP_TX_ERROR, &sys_err, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 3,
		sizeof(tx->rtx_rdb->rdb_name), &tx->rtx_rdb->rdb_name,
		0, tx->rtx_tran_id.db_high_tran, 
		0, tx->rtx_tran_id.db_low_tran);
	    ...
	    if ( rcp->rcp_state != RCP_LG_GONE ) {
	    	...
	    	*/
			...
			if (cl_status != OK)
			{
				...
				(char *)0, (i4)0, (i4 *)0, err_code, 1, 
				0, LG_A_REASSOC);
		   	 *err_code = E_DM9434_DMR_COMPLETE_ERROR;
		   	 status = E_DB_ERROR;
		   	 break;
			}
			continue;
	...
	*/
	    ...
	    st = db->rdb_state.stq_next)
	{
	    ...
	    */
		...
		loc_status = mark_tbl_incons(rcp, db, &loc_err);
	    }
	    ...
	    */
	    ...
	    {
		status = close_config(rcp, db, err_code);
		if (status != E_DB_OK)
		{
		    /*
		...
		*/
		...
		status = dm2d_del_db(db->rdb_dcb, err_code);
		if (status != E_DB_OK)
		    break;
	    ...
	    */
		...
		if (cl_status != OK)
		{
		    ule_format(cl_status, &sys_err, ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);
		    ule_format(E_DM9016_BAD_LOG_REMOVE, &sys_err, ULE_LOG, 
			NULL, (char *)0, 0L, (i4 *)0, err_code, 1,
			0, db->rdb_dbopen_id);
		    *err_code = E_DM9434_DMR_COMPLETE_ERROR;
		    break;
	...
	*/
	...
	if ( incons_error || incons_pc )
	{
	    ...
	    */
	    if ( incons_error || Db_incons_crash )
	    {
	    ...
	    */
	    if (Db_incons_crash)
	    {
		*err_code = E_DM942A_RCP_DBINCON_FAIL;
		break;
	    }
	}
...
i4		*err_code)
{
	...
	*/
	if (LGA_GTE(&prev_la, &lctx->lctx_eof))
	{
		...
		*/
		...
		if (status)
		{
			...
			(char *)0, (i4)0, (i4 *)0, err_code, 0);
		    ule_format(E_DM900B_BAD_LOG_ALTER, (CL_ERR_DESC *)&sys_err,
			 ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
			err_code, 1, 0, LG_A_LFLSN);
		    status = E_DB_OK;
		}
	    }
	    break;
	...
	if (status != E_DB_OK)
	{
	    ...
	    ** analysis pass.
	    */
	    if (*err_code == E_DM9238_LOG_READ_EOF)
	    {
		/*
		** If offline recovery, tell the logging system the lsn 
		** the last record in the file.
		*/
		if (rcp->rcp_action != RCP_R_ONLINE)
		{
		    ...
		    if (status)
		    {
			ule_format(status, (CL_ERR_DESC *)&sys_err, ULE_LOG, 
			    NULL, (char *)0, (i4)0, (i4 *)0, 
			    err_code, 0);
			ule_format(E_DM900B_BAD_LOG_ALTER, 
	...
	case DM0LBT:
	    ...
	    */
		...
		status = add_rtx(rcp, lctx, tran_id, (LK_LLID)0, 0, &la, &la, 
		    &lctx->lctx_cp, rdb, btran->bt_name.db_own_name, err_code);
		if (status != E_DB_OK)
		    break;
	    }
	...
	case DM0LOPENDB:
	    ...
	    */
	    ...
	    status = add_rdb(rcp, lctx, dbid, &open->o_dbname, 
			...
			0, &la, &record->lsn, recover_flags, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	if (record->flags & DM0L_NONREDO)
	{
		...
		if (rcp->rcp_verbose)
		{
			...
			la.la_block,
			la.la_offset);
		    TRdisplay("\t    Table (%d,%d) LSN (%d,%d) operation %d.\n",
		...
		*/
		...
		if (rdmu == 0)
		{
		    status = add_nonredo(rcp, dbid, &tabid, &lsn, err_code);
		    if (status)
			break;
    ...
    *err_code = E_DM9435_DMR_ANALYSIS_PASS;
    return (E_DB_ERROR);
}

/*{
...
i4		*err_code)
{
	...
	*/
	...
	{
	    dmr_check_event(LG_NOWAIT, lctx->lctx_lxid);
	}
	...
	*/
	...
	if (rdb->rdb_status & RDB_TABLE_RECOVERY_ERROR)
	{
	   ...
	   if (rtbl)
	   {
		...
		TRdisplay("\tREDO SKIPPED due to INVALID table status.\n");
#endif
		rtbl->redo_cnt++;
		continue;
	   }
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    ...
	    status = add_invalid_tbl(rcp, rdb, record, err_code);
	    if (status != E_DB_OK)
	    {
		ule_format(E_DM943D_RCP_DBREDO_ERROR, (CL_SYS_ERR *)0, ULE_LOG, 
...
i4		*err_code)
{
    ...
    ** contain this field.
    */
    status = read_first_log_record(rcp, err_code);
	...
	*/
	...
	{
	    status = read_next_log_record(rcp, rtx, err_code);
	    if (status)
		break;
	}
	read_a_log_record = 1;
	...
	*/
	    ...
	    (prev_lsn.lsn_high != 0))
	{
	    ...
	    TRdisplay(
	    "\tCurrent LA:<%d,%d,%d> LSN:(%d,%d), Previous LSN:(%d,%d)\n",
		la.la_sequence, la.la_block,
		la.la_offset,
		rtx->rtx_last_lsn.lsn_high, rtx->rtx_last_lsn.lsn_low,
		prev_lsn.lsn_low, prev_lsn.lsn_low);
	    *err_code = E_DM9437_RCP_P3;
	    status = E_DB_ERROR;
	    break;
	}
	...
	if (record->type == DM0LP1)
	{
		...
		*/
		...
		if (rtx->rtx_recover_type == RTX_WILLING_COMMIT)
		{
			...
			status = recover_willing_commit(rcp, rtx, err_code);
		    if (status == E_DB_OK)
		    {
			rtx->rtx_status |= RTX_WC_FINISHED;
			rcp->rcp_tx_count--;
		    }
		    else if ( ! Db_incons_crash)
		    {
			/*
			** XXXX error message.
			*/
	...
	*/
	status = apply_undo(rcp, rtx, record, &la, err_code);

#ifdef xDEBUG
	/* DEBUG: if an index, force an error! */
	if (tabid.db_tab_index > 100 && tabid.db_tab_index % 5 == 0)
	{
	    *err_code = E_DM9639_DMVE_UNDO;
	    status = E_DB_ERROR;
	}
#endif
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	     ...
	     */
	    if ( *err_code == E_DM9070_LOG_LOGFULL ) {
		...
		/* If possible, mark table or index invalid for redo/undo */
		status = add_invalid_tbl(rcp, rtx->rtx_rdb, record, err_code);
		if (status != E_DB_OK)
		{
		    ule_format(E_DM943E_RCP_DBUNDO_ERROR, (CL_SYS_ERR *)0, ULE_LOG, 
	...
	*/
	...
	{
	    dmr_check_event(LG_NOWAIT, rtx->rtx_lctx->lctx_lxid);
	}
    }
...
i4		*err_code)
{
	...
	*/
	...
	ule_format(E_DM9432_RCP_OPENTX_ERROR, &sys_err, ULE_LOG, NULL,
	    ...
	    0, tx->rtx_tran_id.db_high_tran, 
	    0, tx->rtx_tran_id.db_low_tran);
	*err_code = E_DM9443_DMR_PREINIT_RECOVER;
	return (E_DB_ERROR);
    }
	...
	for (rlp = rcp->rcp_lph[i]; rlp != NULL; rlp = rlp->rlp_next)
	{
		...
		*/
		...
		for (;;)
		{
			...
			ule_format(cl_status, &sys_err, ULE_LOG, NULL,
			    (char *)0, (i4)0, (i4 *)0, err_code, 0);
			ule_format(E_DM901D_BAD_LOCK_SHOW, &sys_err, ULE_LOG, 
			    NULL, (char *)0, (i4)0, (i4 *)0, err_code,
			    1, 0, lli);
			status = E_DB_ERROR;
			break;
	...
	*/
	    ...
	    && (rdb->rdb_status & RDB_REDO_PARTIAL))
	{
		...
		*/
	    ...
	    {
		ule_format(E_DM9446_RCP_NO_SVDB_LOCK, (CL_SYS_ERR *)0, ULE_LOG,
		    NULL, (char *)0, (i4)0, (i4 *)0, err_code, 1,
		    sizeof(rdb->rdb_name), &rdb->rdb_name);
		status = E_DB_ERROR;
		break;
    ...
    if (status != E_DB_OK) 
    {
	ule_format(E_DM9447_RCP_SVDB_LOCK_ERROR, &sys_err, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM9443_DMR_PREINIT_RECOVER;
	return (E_DB_ERROR);
    }
...
i4		*err_code)
{
    ...
    *err_code = E_DM9436_DMR_UPDATE_BOF;
    return (E_DB_ERROR);
}

/*{
...
i4		*err_code)
{
	...
	if (status = dm0l_position(DM0L_LAST, lctx->lctx_lxid,
		(LG_LA *)&(header->lgh_end), lctx, err_code))
	    break;
	...
	for (;;)
	{
	    ...
	    if (status != E_DB_OK && *err_code != E_DM9238_LOG_READ_EOF)
		break;
	    ...
	    } while (*err_code != E_DM9238_LOG_READ_EOF);
	...
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	ule_format(E_DM9405_RCP_GETCP, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);

	header->lgh_status = LGH_BAD;
	*err_code = E_DM9405_RCP_GETCP;
	status = E_DB_ERROR;
    }
    else
    {
...
i4		*err_code)
{
	...
	*/
	else if ( ! database_open)
	{
	    ...
	    */
	    if (unexpected_xacts)
	    {
		ule_format(E_DM943F_RCP_TX_WO_DB, (CL_SYS_ERR *)0, ULE_LOG, 
	...
	*/
	status = dm0c_close(cnf, (i4) 0, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	(char *)0, (i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM942F_RCP_CHECK_RECOVERY;
...
i4		*err_code)
{
	...
	*/
	if (rdb->rdb_status & (RDB_INVALID | RDB_TABLE_RECOVERY_ERROR))
	{
	    ...
	    {
		ule_format(E_DM943B_RCP_DBINCONSISTENT, (CL_SYS_ERR *)0,  
	    ...
	    {
		ule_format(E_DM944A_RCP_DBINCONSISTENT, (CL_SYS_ERR *)0,  
		ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, err_code, 2,
		sizeof(DB_DB_NAME), &rdb->rdb_name,
		sizeof(DB_OWN_NAME), &rdb->rdb_owner);
	    }
    ...
    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	(char *)0, (i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM9438_RCP_CLOSE_CONFIG;
    return (E_DB_ERROR);
}

/*{
...
i4		*err_code)
{
    ...
    */
	 ...
	 */
	if (dmve.dmve_error.err_data == E_DM9070_LOG_LOGFULL )
		*err_code = E_DM9070_LOG_LOGFULL;
	else
		*err_code = E_DM943A_APPLY_UNDO;
    }
    return (save_status);
}

...
i4		*err_code)
{
	...
	*/
	...
	status = read_next_log_record(rcp, tx, err_code);
	if (status)
	{
...
*/
static DB_STATUS
read_next_log_record(RCP *rcp, RTX *rtx, i4 *err_code)
{
    DB_STATUS		status = E_DB_OK;
    LG_LA		la;
    LG_LSN		lsn;
	...
	*/
	...
	status = dm0l_read(lctx, rtx->rtx_id, &record, &la, err_code);
	if (status != E_DB_OK)
	{
	    /* ERROR PROCESSING - error message - dump xacts - give reason */
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	...
	if (p1->p1_flag == P1_LAST)
	{
	    ...
	    */
		...
		ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 1,
		    0, LG_A_CONTEXT);
		*err_code = E_DM9421_RECOVER_WILLING_COMMIT;
		status = E_DB_ERROR;
		break;
	    }
		...
		*err_code = E_DM9421_RECOVER_WILLING_COMMIT;
		...
		ule_format(cl_status, &sys_err, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
		*err_code = E_DM901A_BAD_LOCK_CREATE;
		status = E_DB_ERROR;
		break;
	    }		    
...
i4		*err_code)
{
    ...
    */
    status = dm0m_allocate(sizeof(RDB), 0, RDB_CB, RDB_ASCII_ID, 
			(char *)rcp, (DM_OBJECT **)&rdb, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM9408_RCP_P0; /* XXXX */
    }
...
i4		*err_code)
{
    ...
    */
    status = dm0m_allocate(sizeof(RTX), 0, RTX_CB, RTX_ASCII_ID, 
			(char *)rcp, (DM_OBJECT **)&rtx, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
...
RDMU		**return_dmu)
{
    ...
    *return_dmu = NULL;
	    ...
	    (rdmu->tabid.db_tab_base == tabid->db_tab_base))
	{
	    ...
	    ** Return a base table DMU for a request on a secondary index
	    ** (because a non-redo operation on a base table also invalidates
	    ** any previously logged operations on the secondary indexes as
	    ** well), but do not return a secondary index DMU for a request
	    ** on a base table.
	    **
	    ** Note that entries on the Non-Redo list are sorted by LSN
	    ** so if both a matching base table DMU and a matching secondary 
	    ** index DMU exist, the one with the earliest LSN will be returned.
	    */
...
i4		*err_code)
{
    ...
    */
    status = dm0m_allocate(sizeof(RDMU), 0, RDMU_CB, RDMU_ASCII_ID, 
			(char *)rcp, (DM_OBJECT **)&rdmu, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
...
i4		*err_code)
{
    ...
    */
    status = dm0m_allocate(sizeof(RLP), 0, RLP_CB, RLP_ASCII_ID, 
			(char *)rcp, (DM_OBJECT **)&rlp, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
...

/*{
** Name: extract_tabid	-  extract the table id from the log record
**
** Description:
**	This routine extracts the table id out of the log record context.
**	If the log record is not in the list of log records relevent to
**	REDO processing, then a table id of (0,0) is returned.
**
** Inputs:
**	rcp			Recovery Context
**
** Outputs:
**	tabid				    table id referred to in log rec
**	Returns:
**	    none
**	Exceptions:
**	    none
...
**	    Added DM0LOVFL log record for REDO of File Extend operations.
**	14-dec-1992 (rogerk)
**	    Reduced Logging Project: Added new log records.
**	14-oct-93 (jrb)
**          Added support for DM0LEXTALTER.
**      28-may-1998 (stial01)
**          extract_tabid() Added case DM0LSM1RENAME
**	29-Apr-2004 (gorvi01)
**		Added case for DM0LDELLOCATION used for unextenddb.
*/
static VOID
extract_tabid(
DM0L_HEADER *record,
DB_TAB_ID   *tabid)
{
    ...
    {
    case DM0LCREATE:
	STRUCT_ASSIGN_MACRO(((DM0L_CREATE *)record)->duc_tbl_id, *tabid); 
	break;

    case DM0LMODIFY:
	STRUCT_ASSIGN_MACRO(((DM0L_MODIFY *)record)->dum_tbl_id, *tabid); 
	break;

    case DM0LINDEX:
	STRUCT_ASSIGN_MACRO(((DM0L_INDEX *)record)->dui_tbl_id, *tabid);
	break;

    case DM0LRELOCATE:
	STRUCT_ASSIGN_MACRO(((DM0L_RELOCATE *)record)->dur_tbl_id, *tabid);
	break;

    case DM0LDESTROY:
	STRUCT_ASSIGN_MACRO(((DM0L_DESTROY *)record)->dud_tbl_id, *tabid);
	break;

    case DM0LLOAD:
	STRUCT_ASSIGN_MACRO(((DM0L_LOAD*)record)->dul_tbl_id, *tabid);
	break;

    case DM0LAI:
	STRUCT_ASSIGN_MACRO(((DM0L_REP*)record)->rep_tbl_id, *tabid);
	break;

    case DM0LPUT:
	STRUCT_ASSIGN_MACRO(((DM0L_PUT*)record)->put_tbl_id, *tabid);
	break;
    
    case DM0LDEL:
	STRUCT_ASSIGN_MACRO(((DM0L_DEL*)record)->del_tbl_id, *tabid);
	break;
    ...
    case DM0LSM1RENAME:
	STRUCT_ASSIGN_MACRO(((DM0L_SM1_RENAME*)record)->sm1_tbl_id, *tabid);

    case DM0LASSOC:
	STRUCT_ASSIGN_MACRO(((DM0L_ASSOC*)record)->ass_tbl_id, *tabid);
	break;

    case DM0LALLOC:
	STRUCT_ASSIGN_MACRO(((DM0L_ALLOC*)record)->all_tblid, *tabid);
	break;

    case DM0LDEALLOC:
	STRUCT_ASSIGN_MACRO(((DM0L_DEALLOC*)record)->dall_tblid, *tabid);
	break;

    case DM0LEXTEND:
	STRUCT_ASSIGN_MACRO(((DM0L_EXTEND*)record)->ext_tblid, *tabid);
	break;

    case DM0LALTER:
	STRUCT_ASSIGN_MACRO(((DM0L_ALTER*)record)->dua_tbl_id, *tabid);
	break;

    case DM0LOVFL:
	STRUCT_ASSIGN_MACRO(((DM0L_OVFL*)record)->ovf_tbl_id, *tabid);
	break;

    case DM0LNOFULL:
	STRUCT_ASSIGN_MACRO(((DM0L_NOFULL*)record)->nofull_tbl_id, *tabid);
	break;

    case DM0LFMAP:
	STRUCT_ASSIGN_MACRO(((DM0L_FMAP*)record)->fmap_tblid, *tabid);
	break;

    case DM0LBTPUT:
	STRUCT_ASSIGN_MACRO(((DM0L_BTPUT*)record)->btp_tbl_id, *tabid);
	break;

    case DM0LBTDEL:
	STRUCT_ASSIGN_MACRO(((DM0L_BTDEL*)record)->btd_tbl_id, *tabid);
	break;

    case DM0LBTSPLIT:
	STRUCT_ASSIGN_MACRO(((DM0L_BTSPLIT*)record)->spl_tbl_id, *tabid);
	break;

    case DM0LBTOVFL:
	STRUCT_ASSIGN_MACRO(((DM0L_BTOVFL*)record)->bto_tbl_id, *tabid);
	break;

    case DM0LBTFREE:
	STRUCT_ASSIGN_MACRO(((DM0L_BTFREE*)record)->btf_tbl_id, *tabid);
	break;

    case DM0LBTUPDOVFL:
	STRUCT_ASSIGN_MACRO(((DM0L_BTUPDOVFL*)record)->btu_tbl_id, *tabid);
	break;

    case DM0LRTPUT:
	STRUCT_ASSIGN_MACRO(((DM0L_RTPUT*)record)->rtp_tbl_id, *tabid);
	break;

    case DM0LRTDEL:
	STRUCT_ASSIGN_MACRO(((DM0L_RTDEL*)record)->rtd_tbl_id, *tabid);
	break;

    case DM0LRTREP:
	STRUCT_ASSIGN_MACRO(((DM0L_RTREP*)record)->rtr_tbl_id, *tabid);
	break;

    case DM0LDISASSOC:
	STRUCT_ASSIGN_MACRO(((DM0L_DISASSOC*)record)->dis_tbl_id, *tabid);
	break;

    case DM0LDMU:
	STRUCT_ASSIGN_MACRO(((DM0L_DMU*)record)->dmu_tabid, *tabid);
	break;
...
RCP		*rcp)
{
	...
	TRdisplay("\t    DB %x, Table (%d, %d) \tat LSN (%d,%d)\n",
...
i4	node_id)
{
    ...
    */
	...
	status = dm0l_read(lctx, lctx->lctx_lxid, &record, &la, &error);
	if (status != E_DB_OK)
	    break;
...
RTX		    *rtx)
{
    ...
    status = dm0l_position(DM0L_BY_LGA, rtx->rtx_id, 
		(LG_LA *)&rtx->rtx_orig_last_lga, lctx, &error);
    if (status != E_DB_OK)
    {
	ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &error, 0);
	ule_format(E_DM940C_RCP_ABORT, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &error, 0);
	return;
    }
	...
	status = dm0l_read(lctx, rtx->rtx_id, &record, &la, &error);
	if (status != E_DB_OK)
	{
	    ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &error, 0);
...
i4		logfile_blocksize)
{
    ...
    TRdisplay("(LA=(%d,%d,%d),PREV_LA=(%d,%d,%d),LSN=(%d,%d))\n", 
	la->la_sequence, 
	la->la_block,
	la->la_offset,
...
i4		*err_code)
{
    ...
    if (Db_incons_crash && rcp->rcp_action != RCP_R_STARTUP)
	return (E_DB_ERROR);
    ...
    /* Right now we can only mark secondary indices invalid */
    if (tabid.db_tab_index <= 0)
	return (E_DB_ERROR);

    TRdisplay("%@ RCP-ERROR processing %~t,%~t\n",
    ...
    */
    status = dm0m_allocate(sizeof(RTBL), 0, RTBL_CB, RTBL_ASCII_ID, 
			(char *)rcp, (DM_OBJECT **)&rtbl, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
...
i4		*err_code)
{
	...
	status = dm2t_open(dcb, &table_id, DM2T_IX, DM2T_UDIRECT, DM2T_A_WRITE,
		...
		&stamp, &rel_rcb, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	    ...
	    st = rtbl->rtbl_state.stq_next)
	{
		...
		*/
		...
		ule_format(E_DM944B_RCP_IDXINCONS, (CL_SYS_ERR *)0,  
		    ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, err_code, 2,
    ...
    if (rel_rcb)
    {
	status = dm2t_close(rel_rcb, 0, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
