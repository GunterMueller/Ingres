...
DB_STATUS   *err_code)
{
    ...
    {
        EXdelete();
        dmf_write_msg(E_DM904A_FATAL_EXCEPTION);
        PCexit(FAIL);
    }
    ...
    status = atp_prepare(&atp, jsx, dcb, err_code);
    if (status != E_DB_OK)
    {
        EXdelete();
	return (status);
    ...
    */
    ...
    {
        EXdelete();
	*err_code = E_DM1209_ATP_CNF_OPEN;
	return (status);
    }
    ...
    {
        EXdelete();
	*err_code = E_DM120A_ATP_CNF_CLOSE;
	return (status);
    }
    ...
    */
    ...
    {
	status = atp_archive_wait(jsx, dcb, err_code);
	if (status != E_DB_OK)
	{
            EXdelete();
	    if (cnf)
		(VOID)dm0c_close(cnf, 0, &local_err_code);
	    return (status);
	}
		...
		(jsx->jsx_status2 & JSX2_ABORT_TRANS))
	{
            ...
            if ( (check_flag = atp.atp_jsx->jsx_status & (JSX_BTIME | JSX_START_LSN) )  )
            {
               ...
               for (i = atp.atp_begin_jnl + 1; check_flag && (i < atp.atp_l_jnl); i++)
               {
                  ...
                  for (status = dm0j_open(DM0J_MERGE, (char *)&dcb->dcb_jnl->physical, 
                       ...
                       (status == E_DB_OK) &&
                       ((status = dm0j_read(jnl, &record, &l_record, err_code)) == E_DB_OK) ; )
                  {
		...
		*/
		...
		if (status != E_DB_OK)
		{
		   ...
		   {
		      dmf_write_msg(*err_code);
		      *err_code = E_DM1202_ATP_JNL_OPEN;
		   }
		   break;
		}
		...
		for (;;)
		{
		    ...
		    status = dm0j_read(jnl, &record, &l_record, err_code);
		    if (status != E_DB_OK)
		    {
			if (*err_code == E_DM0055_NONEXT)
			    status = E_DB_OK;

			*err_code = E_DM1203_ATP_JNL_READ;
			break;
		    }
	    ...
	    */
		...
		status = move_tx_to_atx(&atp, err_code); 
            ...
            */
            if (atp.atp_btime_open_et)
            {
               ...
               for (i = atp.atp_begin_jnl -1; i && i > atp.atp_f_jnl; i--)
               {
                  ...
                  if (*err_code != E_DM1203_ATP_JNL_READ)
                  {
                     break;
                  }
	...
	*/ 
	if (status == E_DB_ERROR) 
	{
	    *err_code = E_DM9055_AUDITDB_PRESCAN_FAIL;
	    break;
	}
    ...
    */
    local_status = complete(&atp, &local_err_code);
    (VOID) dm2d_close_db(dcb, dmf_svcb->svcb_lock_list,
		DM2D_IGNORE_CACHE_PROTOCOL, &local_err_code);
...
DB_STATUS   *err_code)
{
       ...
       case DM0LABORTSAVE:
          ...
          */
          if (!(atp->atp_jsx->jsx_status2 & JSX2_ABORT_TRANS)) 
              return add_abortsave(atp, record, err_code);
          else
              return E_DB_OK;
    ...
    */
	...
	if (h->type == DM0LBT)
	{
	   ...
	   if (tx == 0)
	   {
	       dmf_write_msg(E_DM1206_ATP_TOO_MANY_TX);
	       return (E_DB_OK);
	   }
	...
	*/
        ...
        {
            return add_abort(atp, &tmp_tran_id, err_code);
        }
	     ...
	     (audit_tx >= 0) )
	{
	    ...
	    return add_btime(atp, &tmp_tran_id, tx ? FALSE:TRUE, err_code);
	}
    }
...
# define	    ATP_OP_SIZE    sizeof(audit_record.audit_hdr.operation)
	...
	*/
	...
	else if (type == DM0LREP)
	{
	    ...
	    */
	    ...
	    {
		*err_code = E_DM1218_ATP_OROW_COMPRESS;
		return (E_DB_ERROR);
	    }
	    ...
	    */
		...
		ule_format(E_DM9425_MEMORY_ALLOCATE, (CL_SYS_ERR *)0, ULE_LOG,
		    ...
		    0, nrec_size);
		*err_code = E_DM1211_ATP_MEM_ALLOCATE;
		return (E_DB_ERROR);
	    }
	...
	if (status != OK)
	{
		*err_code = E_DM1201_ATP_WRITING_AUDIT;
		return (E_DB_ERROR);
	}
    ...
    */
	...
	case DM0LABORTSAVE:
		STprintf(line_buffer, "  AbortSave  : Transaction Id %08x%08x ",
				...
				if(atp->atp_jsx->jsx_status2 & JSX2_INTERNAL_DATA)
		{
			STprintf(line_buffer, "  SAVEPOINT LSN=(%d,%d) SAVEPOINT ID=%d ",
	...
	case DM0LPUT:
		...
		if(atp->atp_jsx->jsx_status2 & JSX2_INTERNAL_DATA)
		{
			STprintf(line_buffer, "	LSN=(%d,%d) TID %d",
	...
	case DM0LREP:
		...
		nrec_ptr = &((DM0L_REP *)record)->rep_vbuf[table_size + 
			...
			if(atp->atp_jsx->jsx_status2 & JSX2_INTERNAL_DATA)
			{
				STprintf(line_buffer, "  LSN=(%d,%d) OLD TID %d, NEW TID %d",
					((DM0L_HEADER *)record)->lsn.lsn_high,
					((DM0L_HEADER *)record)->lsn.lsn_low,
 ...
 Id (%d,%d) Table [%*s,%*s]",
            ((DM0L_HEADER *)record)->tran_id.db_high_tran,
            ((DM0L_HEADER *)record)->tran_id.db_low_tran,
            ((DM0L_OLD_INDEX *)record)->dui_idx_tbl_id.db_tab_base,
            ((DM0L_OLD_INDEX *)record)->dui_idx_tbl_id.db_tab_index,
            sizeof(DB_OWN_NAME),&((DM0L_OLD_INDEX *)record)->dui_name,
            sizeof(DB_OWN_NAME), &((DM0L_OLD_INDEX *)record)->dui_owner);
        write_line(atp, line_buffer, STlength(line_buffer));
        break;
 ...
 Type %d", 
	...
	if((atp->atp_jsx->jsx_status2 & JSX2_INTERNAL_DATA) &&
		( type != DM0LPUT) && (type != DM0LREP) && (type != DM0LDEL))
	{
		STprintf(line_buffer, "  LSN=(%d,%d)", 
			((DM0L_HEADER *)record)->lsn.lsn_high,
			((DM0L_HEADER *)record)->lsn.lsn_low);
		write_line(atp, line_buffer, STlength(line_buffer));
	}
...
i4	    *err_code)
{
	...
	if (h->type != DM0LBT)
	{
		...
		*/
		if ((h->type != DM0LSBACKUP) && (h->type != DM0LEBACKUP))
		    dmf_write_msg(E_DM1205_ATP_NO_BT_FOUND);
	...
	*/
	...
	status = display_record(atp, (PTR)h, tx, err_code);
	if (status != E_DB_OK)
	    return(E_DB_ERROR);
    }
...
PTR	    record)
{
    ...
    case DM0LOLDINDEX:
	table_name = (char *)&((DM0L_OLD_INDEX *)h)->dui_name.db_tab_name;
	owner_name = (char *)&((DM0L_OLD_INDEX *)h)->dui_owner.db_own_name;
	break;
...
DB_STATUS   *err_code)
{
    ...
    */
	...
	dmf_svcb->svcb_lock_list,
        DM2D_IGNORE_CACHE_PROTOCOL, err_code);
    if (status != E_DB_OK)
    {
	if (*err_code == E_DM004C_LOCK_RESOURCE_BUSY)
	{
	    *err_code = E_DM120E_ATP_DB_BUSY;
	    return (E_DB_INFO);
	}
	if (*err_code == E_DM0053_NONEXISTENT_DB)
	    *err_code = E_DM1207_ATP_NON_DB;
 	else if (jsx->jsx_status1 & JSX_ATP_INCONS)
	{
	    ...
	    */
	    status = dm2d_open_db(dcb, DM2D_A_READ, DM2D_IS, 
		dmf_svcb->svcb_lock_list, 
	 	DM2D_IGNORE_CACHE_PROTOCOL | DM2D_NOCK, err_code);
	    if (status != E_DB_OK)
		*err_code = E_DM1208_ATP_OPEN_DB;
	}
	if (status != E_DB_OK)
	    return (E_DB_ERROR);
    }
    ...
    */
    status = jsp_get_case(dcb, jsx, err_code);
    if (status != E_DB_OK)
	return (E_DB_ERROR);
	...
	** Make sure that the database is journaled. 
        ...
        */
        ...
        {
            status = E_DB_ERROR;
            *err_code = E_DM1200_ATP_DB_NOT_JNL;
            break;
        }
        ...
        */
        if (cnf->cnf_dsc->dsc_status & DSC_DISABLE_JOURNAL)
        {
            ...
            */
            ule_format(E_DM93A1_AUDITDB_JNLDISABLE, (CL_ERR_DESC *)0,
	...
	*/
	...
	if (jsx->jsx_status & JSX_CKP_SELECTED)
	{
	    ...
	    */
	    ...
	    {
		ule_format(E_DM1219_ATP_INV_CKP, (CL_ERR_DESC *) NULL, 
		    ULE_LOOKUP, (DB_SQLSTATE *) NULL, error_buffer, 
		    ER_MAX_LEN, &error_length, err_code, 0);
		SIprintf("%s\n", error_buffer);

		*err_code = E_DM1004_JSP_BAD_ARGUMENT;
		break;
	    }
	...
	else
	{
		...
		ule_format(E_DM1025_JSP_BAD_DATE_RANGE, (CL_ERR_DESC *) NULL,
		    ULE_LOOKUP, (DB_SQLSTATE *) NULL, error_buffer, ER_MAX_LEN, 
		    &error_length, err_code, 2, TM_SIZE_STAMP, bgn_time, 
	...
	*/
	...
	for (i = 0; i < ATP_TD_CNT; i++)
	{
	    atp->atp_td[i].table_id.db_tab_base = 0;
	    atp->atp_td[i].att_array = 0;
	    atp->atp_td[i].att_ptr_list = 0;
	}
	...
	*/
	...
	status = dm0m_allocate(mem_needed, (i4)0, (i4)MISC_CB, 
	    (i4)MISC_ASCII_ID, (char *)0, (DM_OBJECT**)&mem_ptr, err_code);
	if (status != OK)
	{
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	...
	*/
	if (jsx->jsx_status & (JSX_AUDITFILE | JSX_FNAME_LIST))
	{
	    ...
	    *err_code = E_DM120B_ATP_CATALOG_ERROR;
		...
		&timestamp, &r_idx, (DML_SCB *)0, &error);
	    if (status != E_DB_OK)
		break;
		...
		*/		
		...
		for (;;)
		{
		    ...
		    */
		    status = dm2r_get(r_idx, &tid, DM2R_GETNEXT, 
			    (char *)&rindexrecord, &error);
		    if (status != E_DB_OK && error != E_DM0055_NONEXT)
			break;
		    if (status == E_DB_OK)
		    {
		...
		if (status != E_DB_OK)
		{
			...
			STncpy( error_buffer,
			    jsx->jsx_tbl_list[i].tbl_name.db_tab_name, 
			    DB_MAXNAME);
			error_buffer[ DB_MAXNAME ] = '\0';
			STtrmwhite(error_buffer);
			ule_format(E_DM1215_ATP_TBL_NONEXIST,
			    (CL_SYS_ERR *)0, ULE_LOOKUP, (DB_SQLSTATE *)0, 
			    error_buffer, ER_MAX_LEN, &error_length, err_code, 
			    1, 0, error_buffer);

			SIprintf("%s\n", error_buffer);
			*err_code = E_DM120F_ATP_TBL_NOTFOUND;
		    }
		    break;
		}
		...
		*/
		...
		else 
		{
		    ...
		    */
		    ...
		    {
			*err_code = E_DM1217_ATP_DLM_FNAME;
			status = E_DB_ERROR;
			break;
		    }
		    ...
		    if (status != E_DB_OK)
		    {
			*err_code = E_DM1210_ATP_BAD_TRL_CREATE;
			break;
		    }
		}
	    ...
	    */
	    status = dm2t_close( rel, DM2T_NOPURGE, &error);
	    rel = NULL;
	    if (status != E_DB_OK)
		break; 
	    status = dm2t_close( r_idx, DM2T_NOPURGE, &error);
	    r_idx = NULL;
	    if (status != E_DB_OK)
		break; 
	}
	...
	*err_code = E_DM120C_ATP_ADF_ERROR;
	{
	...
	*/
	{
	    ...
	    if (status != E_DB_OK)
	    {
		ule_format( adf_cb.adf_errcb.ad_errcode, 0, ULE_LOG,
			    NULL, 0, 0, 0, err_code, 0 );
		status = E_DM120C_ATP_ADF_ERROR;
		break;
	    }
    ...
    */
    ...
    if ( rel )
	dm2t_close(rel, DM2T_NOPURGE, &error);
    if ( r_idx )
	dm2t_close(r_idx, DM2T_NOPURGE, &error);

    if (*err_code == E_DM1200_ATP_DB_NOT_JNL)
	return(E_DB_INFO);
...
PTR         jrecord)
{
    ...
    */
	...
	{
	    dmf_write_msg(E_DM1212_ATP_DISPLAY);
	    return;
	}
    }
    ...
    */
    ...
    {
	/*
	** Get table level accessor list to find the uncompress routine.
	*/
	dm1c_get_tlv(comptype, &tlv);

	status = (*tlv->dmpp_uncompress)(td->att_ptr_list, td->att_count,
	    r, atp->atp_uncompbuf, td->row_width, &uncompressed_length,
	    comptype, &localtid, NULL, td->row_version, (PTR)&adf_cb);  
	if (DB_FAILURE_MACRO(status))
	{
		...
		td->row_width, uncompressed_length, &adf_cb);
	    dmf_write_msg(E_DM1212_ATP_DISPLAY);
	    return;
	}
	...
	*/
	...
	if(convert_row == TRUE)
	{
            s = dm1r_cvt_row(td->att_ptr_list, td->att_count, r, 
	...
	*/
	if (DB_FAILURE_MACRO(status))
	{
	    ...
	    if (error != E_AD1012_NULL_TO_NONNULL)
	    {
		dmf_write_msg(error);
		ule_format(E_DM1213_ATP_COLUMN_DISPLAY, (CL_SYS_ERR *)0, 
...
DB_TAB_ID   *table_id)
{
	...
	**************************************************************/
	...
	status = dm2r_rcb_allocate(atp->atp_dcb->dcb_rel_tcb_ptr, (DMP_RCB *)0,
	    &tran_id, dmf_svcb->svcb_lock_list, 0, &rel_rcb, &error);
	if (status != E_DB_OK)
	    break;
	...
	for (;;)
	{
	    ...
	    */
	    ...
	    if ((rel_record.reltid.db_tab_base != table_id->db_tab_base) ||
	    	(rel_record.reltid.db_tab_index != table_id->db_tab_index))
		continue;

	    td->relstat2 = rel_record.relstat2;
	    td->compression = rel_record.relcomptype;
	    td->row_width = rel_record.relwid;
	    td->att_count = rel_record.relatts;
	    td->lobj_att_count = 0;
	...
	*/
	...
	status = dm2r_release_rcb(&rel_rcb, &error);
	if (status != E_DB_OK)
	    break;
	...
	** it is indexed from 1 to att_count.  The att_ptr_list array
	** is indexed 0 to att_count-1.
	*/
	mem_needed = sizeof(DMP_MISC) + 
		((td->att_count + 1) * sizeof(DB_ATTS)) + 
		(td->att_count * sizeof(DB_ATTS *));

	status = dm0m_allocate(mem_needed, (i4)0, (i4)MISC_CB, 
	    (i4)MISC_ASCII_ID, (char *)0, (DM_OBJECT**)&mem_ptr, &error);
	if (DB_FAILURE_MACRO(status))
	{
	...
	td->att_ptr_list = (DB_ATTS **) ((char *)td->att_array +
				((td->att_count + 1) * sizeof(DB_ATTS)));
	...
	**************************************************************/
	...
	status = dm2r_rcb_allocate(atp->atp_dcb->dcb_att_tcb_ptr, (DMP_RCB *)0,
	    &tran_id, dmf_svcb->svcb_lock_list, 0, &attr_rcb, &error);
	if (status != E_DB_OK)
	    break;
	...
	for (;;)
	{
	    ...
	    */
	    attr_count++;
	    if ((attrrecord.attid > td->att_count) || (attrrecord.attid <= 0))
	    {
		ule_format(E_DM00A8_BAD_ATT_ENTRY,
		    (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0, (i4)0, 
		    (i4 *)0, &error, (i4)5, 0, attrrecord.attid,
	    ...
	    else if (attr_count > td->att_count)
	    {
		ule_format(E_DM00A9_BAD_ATT_COUNT,
		    (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0, (i4)0, 
		    (i4 *)0, &error, (i4)5, 0, attr_count,
		    sizeof(DB_OWN_NAME), rel_record.relowner.db_own_name,
		    sizeof(DB_TAB_NAME), rel_record.relid.db_tab_name,
		    sizeof(DB_DB_NAME), atp->atp_dcb->dcb_name.db_db_name,
		    0, td->att_count);      
	...
	status = dm2r_release_rcb(&attr_rcb, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	if ((attr_count == 0) || (attr_count != td->att_count))
	    break;
	...
	*/
	...
	td->table_id.db_tab_index = table_id->db_tab_index;
    ...
    */
	...
	if (error)
	    dmf_write_msg(error);
	dmf_write_msg(E_DM120D_ATP_DESCRIBE);
    }
...
i4	    bytes_used)
{
    ...
    */
    ...
    {
	dmf_write_msg(E_DM1214_ATP_DISPLAY_OVFL);
	return (E_DB_ERROR);
    }
...
i4		*err_code)
{
    ...
    */
    if (atx == 0)
    {
	status = add_atx(atp, &h->tran_id, ATX_ABORTSAVE, &atx, err_code);
	if (status != E_DB_OK)
	    return (E_DB_ERROR);   		
    }
    ...
    */
	...
	(i4)ABS_ASCII_ID, (char *)0, (DM_OBJECT **)&ab, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
...
i4		*err_code)
{
    ...
    */
    if (atx == 0)
    {
	status = add_atx(atp, tran_id, ATX_ABORT, &atx, err_code);
	if (status != E_DB_OK)
	    return (E_DB_ERROR);   		
    }
    else
...
i4		*err_code)
{
    ...
    */
    status = dm0m_allocate(sizeof(ATP_ATX), 0, DM_ATX_CB, ATX_ASCII_ID,
		(char *)atp, (DM_OBJECT **)&atx, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
...
*/
static DB_STATUS
add_btime( atp, tran_id, missing_bt, err_code)
DMF_ATP    *atp;
DB_TRAN_ID *tran_id;
i4         missing_bt;
DB_STATUS  *err_code;
...
i4		*err_code)
{
    ...
    */
    status = dm0j_open( DM0J_MERGE, (char *)&atp->atp_dcb->dcb_jnl->physical, 
			...
			DM0J_FORWARD, (DB_TAB_ID *)0, &jnl_etab, err_code);
    if (status != E_DB_OK)
    {
	if (*err_code == E_DM9034_BAD_JNL_NOT_FOUND &&
	...
	*/
	status = dm0j_read(jnl_etab, &record, &l_record, err_code);

	if ( status == E_DB_OK )
	    status = lobj_redeem((PTR)atp, record, err_code);

	if ( status )
	    (VOID) dm0j_close(&jnl_etab, &local_err_code);
    }
...
i4	*err_code)
{
    ...
    if (!(atp->lobj_found_start) && !(td->relstat2 & TCB2_EXTENSION))
	return(E_DB_OK);
        ...
        if (td)
        {
            for (i = 1; i <= td->att_count; i++)
            {
                ...
                status = dm0m_allocate(td->att_array[i].length + sizeof(DMP_MISC),
                    (i4)0, (i4)MISC_CB,
                    (i4)MISC_ASCII_ID, (char *)0, (DM_OBJECT**)&datavalue_obj, err_code);
	...
	datavalue_obj->misc_data = datavalue_ptr;
        ...
        */
        ...
        if (DB_FAILURE_MACRO(status))
        {
            ...
            error = adf_cb.adf_errcb.ad_errcode;
            dmf_write_msg(error);
            ule_format(E_DM121A_ATP_LSN_NO_ALL, (CL_SYS_ERR *)0,
                ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0,
                &error, 1, 0, 0);
            length = 0;
        }
        ...
        */
        ...
        {
            status = E_DB_ERROR;
            ule_format(E_DM121B_ATP_NOEMPTY_TUPLE, (CL_SYS_ERR *)0,
            ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, &error, 1, 0, 0);
        }
return status;
}
