...
i4         *err_code)
{
    ...
    */
	...
	if ((MEcmp((PTR)&dcb->dcb_name, (PTR)DB_DBDB_NAME,
		sizeof(DB_DB_NAME)) == 0) )
	{
	    *err_code = E_DM1518_RELOC_IIDBDB_ERR;
	    return (E_DB_ERROR);
	}
    }
    ...
    */
	...
	dmf_write_msg(E_DM150A_RELOC_MEM_ALLOC);
	return (E_DB_ERROR);
    }
    else
    {
	...
	do
	{
            ...
            iidb_flag = 0;  /* FIX ME */
            status = open_iidbdb(&iidcb, DM2D_A_WRITE, DM2D_IX,
                                iidb_flag, err_code);
	    ...
	    */
		...
		dmf_svcb->svcb_lock_list, db_flag, err_code);
	    ...
	    *err_code = E_DM1507_RELOC_ERR;
        ...
        else if (log_id)
        {
            ...
            close_status = dmxe_abort((DML_ODCB *)0, iidcb, &tran_id,
                        log_id, lock_id, dmxe_flag, 0,0, err_code);
	...
	if (open_cnf)
	{
	    close_status = config_handler(CLOSEKEEP_CONFIG, dcb, &cnf, 
				err_code);
	    if (close_status != E_DB_OK)
	    {
		/* error formatted in config_handler */
...
i4         *err_code)
{
	...
	** the database must be journal disabled
	...
	{
	    dmf_write_msg(E_DM1505_RELOC_DISABLE_JNL);
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	if (usage == DCB_WORK)
	{
	    ...
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(E_DM1503_RELOC_NEWLOC_NOTFOUND);
		break;
	    }
	    ...
	    */
	    ...
	    {
		status = E_DB_ERROR;
		dmf_write_msg(E_DM1506_RELOC_INVL_USAGE);
		break;
	    }
	}
	...
	*/
	if (!oldloc)
	{
	    status = find_dcb_ext(jsx, dcb, oldname, usage,
				&oldloc, err_code);
	    if (status != E_DB_OK)
	    {
		*err_code = E_DM150C_RELOC_LOC_CONFIG_ERR;
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	...
	*/
	status = get_iilocation(jsx, iidcb, loc_rcb, &jsx_newloc->loc_name,
				&location, log_id, tran_id, lock_id, err_code);
	if (status != E_DB_OK)
	{
	    dmf_write_msg(E_DM1503_RELOC_NEWLOC_NOTFOUND);
	    break;
	...
	{
	    status = E_DB_ERROR;
	    dmf_write_msg(E_DM1500_RELOC_OLDLOC_EQ_NEW);
	    break;
	}
	...
	*/ 
	if (usage != DCB_WORK)
	{
	    ...
	    {
		status = E_DB_ERROR;
		dmf_write_msg(E_DM1501_RELOC_CP_FI_ERR);
		break;
	    }
	}
	...
	*/
	if (usage != DCB_WORK)
	{
	    ...
	    {
		status = E_DB_ERROR;
		dmf_write_msg(E_DM1502_RELOC_DEL_FI_ERR);
	    }
...
DB_STATUS           *err_code)
{
	...
	status = dm2t_open(dcb, &iidatabase, DM2T_IX, DM2T_UDIRECT,
		...
		0, 0, DM2T_IX, tran_id, &stamp, &rcb, (DML_SCB *)0, err_code);
    else
	status = dm2t_open(dcb, &iidatabase, DM2T_IS, DM2T_UDIRECT,
		DM2T_A_READ, 0, 20, 0, log_id, lock_id, 
		0, 0, 0, tran_id, &stamp, &rcb, (DML_SCB *)0, err_code);

    if (status != E_DB_OK)
    {
	dmf_write_msg(*err_code);
	dmf_write_msg(E_DM100F_JSP_OPEN_IIDB);
	return (E_DB_ERROR);
    }

    status = dm2r_position(rcb, DM2R_QUAL, key_list, 1, 0, 0, &tid, err_code);
    if (status == E_DB_OK)
    {
	...
	if (status == E_DB_OK && new_locname)
	{
	    ...
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(E_DM1504_RELOC_UPDT_IIDB_ERR);
	    }
	}	
    }
...
DB_STATUS           *err_code)
{
	...
	{
	    dmf_write_msg(*err_code);
	    dmf_write_msg(E_DM1018_JSP_II_LOCATION);
	    return (E_DB_ERROR); 
	}
    }
	...
	status = dm2r_get(rcb, &tid, DM2R_GETNEXT,
			(char *)location, err_code);
	if (status != E_DB_OK)
	   *err_code = E_DM1023_JSP_NO_LOC_INFO;
...
CL_SYS_ERR    *sys_error)
{
    ...
    */
	...
	ule_format(E_DM9002_BAD_FILE_CREATE, &sys_err, ULE_LOG,
		NULL, error_buffer, ER_MAX_LEN, &error_length, &sys_err, 5, 
	...
	for (; ;)
	{
		...
		pagesize, DI_IO_WRITE, 0, &sys_err);

	    if (status == DI_EXCEED_LIMIT)
	    {
#ifdef xxx /* FIX ME */
		status = dm2t_reclaim_tcb(cnf->cnf_lock_list, &err_code);
		if (status == E_DB_OK)
		    continue;
		if (err_code == E_DM9328_DM2T_NOFREE_TCB)
#endif
	...
	if (status != OK)
	{
	    err_code = E_DM9004_BAD_FILE_OPEN; 
	    break;
	}
	else
	    newopen = TRUE;
	...
	if (status != OK)
	{
	    err_code = E_DM9007_BAD_FILE_SENSE;
	    break;
	}
	...
	*/
	...
	for (i = 0; i <= old_endpg; i+= page_count)
	{
	    ...
	    */
	    ...
	    {
		err_code = E_DM9005_BAD_FILE_READ;
		break;
	    }
	    ...
	    if (status != OK || page_count != save_page_count)
	    {
		err_code = E_DM9006_BAD_FILE_WRITE;
		break;
	    }
	}
    ...
    if (err_code != E_DB_OK)
    {
	ule_format(err_code, &sys_err, ULE_LOG,
		NULL, 0, 0, 0, &err_code, 5, 
		sizeof (copy_info->cp_dbname), &copy_info->cp_dbname,
		11, "Not a table",
		copy_info->cp_newloc->phys_length,
		copy_info->cp_newloc->physical.name,
	...
	if (close_status != OK)
	{
	    close_code = E_DM9001_BAD_FILE_CLOSE;
	    if (status == OK)
		status = close_status;
	}
    }
    ...
    if (close_code != E_DB_OK)
    {
	ule_format(close_code, &sys_err, ULE_LOG,
		NULL, 0, 0, 0, &err_code, 5, 
		sizeof (copy_info->cp_dbname), &copy_info->cp_dbname,
		11, "Not a table",
		copy_info->cp_newloc->phys_length,
    ...
    if (status == OK && err_code == E_DB_OK && close_code == E_DB_OK)
	copy_info->cp_status = OK;
    else
...
i4         *err_code)
{
    ...
    */
    ...
    if (compare == 0)
    {
	dmf_write_msg(E_DM1508_RELOC_OLDDB_EQ_NEW);
	return (E_DB_ERROR);
    }
	...
	*/
	status = get_iidatabase(jsx, iidcb, &jsx->jsx_newdbname, &newdatabase,
			0, 0, RELOC_READ, log_id, tran_id, lock_id, err_code);
	if (status == E_DB_OK)
	{
	    status = E_DB_ERROR;
	    dmf_write_msg(E_DM0124_DB_EXISTS);
	    break;
	}
	else
	{
	...
	*/
	if (jsx->jsx_status1 & JSX_LOC_LIST)
	{
	    status = reloc_check_loc_lists(jsx, dcb, iidcb,
			log_id, tran_id, lock_id, err_code);
	    if (status != E_DB_OK)
	    {
	    ...
	    status = reloc_check_newlocs(jsx, dcb, err_code);
	    if (status != E_DB_OK)
	    {
	    ...
	    status = reloc_loc_process(jsx, dcb, 
				RELOC_PREPARE_LOCATIONS, err_code);
	...
	status = dm2t_open(iidcb, &iilocation, DM2T_IS, DM2T_UDIRECT, 
		DM2T_A_READ, 0, 20, 0, log_id, lock_id,
		0, 0, 0, tran_id, &stamp, &loc_rcb, (DML_SCB *)0, err_code);
	...
	*/
	...
	for (j = 0; j < ext->ext_count; j++)
	{
	    ...
	    */
	    ...
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(E_DM1023_JSP_NO_LOC_INFO);
		break;
	    }
	    ...
	    */
	    status = reloc_db_dircreate(jsx, copy_info, err_code);
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(E_DM011A_ERROR_DB_DIR);
		break;
	    }
	    ...
	    */
	    ...
	    {
		status = E_DB_ERROR;
		*err_code = E_DM150B_RELOC_CP_FI_ERR;
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    error_buffer, ER_MAX_LEN, &error_length, err_code, 3,
	...
	*/
	...
	status = dm2r_put(db_rcb, 0, (char *)&newdatabase, err_code);
	if (status != E_DB_OK)
	{
	    dmf_write_msg(*err_code);
	    break;
	    ...
	    */
	    ...
	    status = get_iiextend(jsx, iidcb, &dcb->dcb_name, 
			&ext->ext_entry[j].logical, ext_flags, ext_rcb, 
                        &extend, log_id, tran_id, lock_id, err_code);
	    if (status != E_DB_OK)
	    {
		*err_code = E_DM150D_RELOC_IIEXTEND_ERR;
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    error_buffer, ER_MAX_LEN, &error_length, err_code, 2,
	    ...
	    status = dm2r_put(ext_rcb, 0, (char *)&extend, err_code); 
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(*err_code);
		break;
	...
	*/
	...
	{
	    status = E_DB_ERROR;
	    dmf_write_msg(E_DM923A_CONFIG_OPEN_ERROR);
	    break;
	}
	...
	{
	    /* FIX ME */
	    dmf_write_msg(E_DM923B_CONFIG_CLOSE_ERROR);
	    break;
	}
	...
	*/
	...
	status = dm2d_open_db(newdcb, DM2D_A_WRITE, DM2D_X,
                dmf_svcb->svcb_lock_list, newdb_flag, err_code);

	if (status != E_DB_OK)
	{
	    dmf_write_msg(E_DM1512_RELOC_CANT_OPEN_NEWDB);
	    break;
	}
	...
	*/
	...
	{
	    status = reloc_loc_process(jsx, newdcb,  
				RELOC_FINISH_LOCATIONS, err_code);
	}
    ...
    if (loc_rcb)
    {
	close_status = dm2t_close(loc_rcb, 0, err_code);
	if (close_status != E_DB_OK)
	{
	    dmf_write_msg(*err_code);
	    if (close_status > status)
    ...
    if (ext_rcb)
    {
	close_status = dm2t_close(ext_rcb, 0, err_code);
	if (close_status != E_DB_OK)
	{
	    dmf_write_msg(*err_code);
	    if (close_status > status)
    ...
    if (db_rcb)
    {
	close_status = dm2t_close(db_rcb, 0, err_code);
	if (close_status != E_DB_OK)
	{
	    dmf_write_msg(*err_code);
	    if (close_status > status)
	...
	for (j = 0; j < loc_done; j++)
	{
		...
		if (di_status != DI_ENDFILE && di_status != DI_DIRNOTFOUND)
		{
		    dmf_write_msg(E_DM1509_RELOC_DEL_NEWFI_ERR);
		}
	    }
...
DB_STATUS	    *err_code)
{
	...
	*config = 0;
	...
	if (status == E_DB_OK)
	{
		...
		*err_code = E_DM150A_RELOC_MEM_ALLOC;
		break;
	    }
	...
	*config = 0;
	dmf_write_msg(E_DM150F_RELOC_CNF_OPEN);
	return (E_DB_ERROR);
	...
	dmf_write_msg(E_DM1510_RELOC_CNF_CLOSE);
	return (E_DB_ERROR);
	...
	*err_code = E_DM1510_RELOC_CNF_CLOSE;
	return (E_DB_ERROR);
...
i4		    *err_code)
{
	...
	*/
	for (i = 0; i < jsx->jsx_loc_cnt && status == E_DB_OK; i++)
	{
	    ...
	    status = find_dcb_ext(jsx, dcb, loc_name, DCB_DATA,
			&oldloc, err_code);
	    if (status == E_DB_OK)
	    {
	...
	status = dm2t_open(dcb, &table_id, lock_mode, DM2T_UDIRECT, 
		...
		log_id, lock_id,
		(i4)0, (i4)0, (i4)0, &tran_id,
		&stamp, &dev_rcb, (DML_SCB *)0, err_code);
	...
	*/
	      ...
	      )
	{
		...
		*/
		...
		{
		    status = E_DB_ERROR;
		    dmf_write_msg(E_DM1511_RELOC_NO_CORE_GATE_VIEW);
		    break;
		}

		status = reloc_validate_table(jsx, dcb, 
				&tblcb->tblcb_table_id, &relrecord, 
				rel_rcb, dev_rcb, err_code);
    ...
    if (rel_rcb)
    {
	close_status = dm2t_close(rel_rcb, 0, err_code);
	if (close_status != E_DB_OK)
	{
	    dmf_write_msg(*err_code);
	    if (close_status > status)
    ...
    if (dev_rcb)
    {
	close_status = dm2t_close(dev_rcb, 0, err_code);
	if (close_status != E_DB_OK)
	{
	    dmf_write_msg(*err_code);
	    if (close_status > status)
    ...
    /* Close the database */
    close_status = rfp_close_database(rfp, dcb, dbdb_flag, err_code);
    if (close_status != E_DB_OK)
    {
	dmf_write_msg(*err_code);
	if (close_status > status)
...
DB_STATUS           *err_code)
{
	...
	for ( ; ; )
	{
		...
		{
		    dmf_write_msg(*err_code);
		    dmf_write_msg(E_DM1513_RELOC_CANT_UPDT_IIDEVICES);
		    break;
		}
		upd_devloc = TRUE;
	    }
	...
	*/
	...
	{
	    dmf_write_msg(*err_code);
	    dmf_write_msg(E_DM1514_RELOC_CANT_UPDT_IIRELATION);
	}
    }
    ...
    */
	...
	for ( ; ; )
	{
	    ...
	    */
		...
		if (compare == 0)
		{
		    *err_code = E_DM1517_RELOC_TBL_LOC_ERR;
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			error_buffer, ER_MAX_LEN, &error_length, err_code, 2,
...
i4      *err_code)
{
	...
	status = dm2t_open(iidcb, &table_id, DM2T_IS, DM2T_UDIRECT,
		DM2T_A_READ, 0, 20, 0, log_id, lock_id,
		0, 0, 0, tran_id, &stamp, &ext_rcb, (DML_SCB *)0, err_code);
	...
	*/
	...
	for (j = 0; j < jsx->jsx_loc_cnt; j++)
	{
	    ...
	    if (status != E_DB_OK)
	    {
		*err_code = E_DM150E_RELOC_IILOCATIONS_ERR;
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			error_buffer, ER_MAX_LEN, &error_length, err_code, 2,
			sizeof (DB_LOC_NAME), loc_name, 0, 0);
		dmf_put_line(0, error_length, error_buffer);
		...
		*/
		...
		{
		    *err_code = E_DM1515_RELOC_USAGE_ERR;
		    status = E_DB_ERROR;
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			error_buffer, ER_MAX_LEN, &error_length, err_code, 3,
...
i4		    *err_code)
{
	...
	*/
	for (scan_cnt = 1; scan_cnt <= maxscans; scan_cnt++)
	{
		...
		*/
		status = reloc_validate_table(jsx, dcb, &relrecord.reltid,
			&relrecord, rel_rcb, dev_rcb, err_code);
		if (status != E_DB_OK)
		{
    ...
    if (ridx_rcb)
    {
	close_status = dm2t_close(ridx_rcb, 0, err_code);
	if (close_status != E_DB_OK)
	{
	    dmf_write_msg(*err_code);
	    if (status == E_DB_OK)
...
DB_STATUS           *err_code)
{
	...
	*/
	for (i = 0; i < new_cnt; i++)
	{
	    ...
	    {
		dmf_write_msg(E_DM1516_RELOC_DB_LOC_ERR);
		status = E_DB_ERROR;
		break;
	    }
	}
...
DMP_DCB             *dcb)
{
    ...
    {
	dmf_write_msg(err_code);
	dmf_write_msg(E_DM1304_RFP_MEM_ALLOC);
	return (E_DB_ERROR);
    }
    else
    {
