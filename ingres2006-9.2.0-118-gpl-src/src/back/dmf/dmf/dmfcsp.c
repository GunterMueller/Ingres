...
static     char csp_ckp_type_names[] = 
	      "0,INIT,STALL,SBACKUP,RESUME,CP,EBACKUP,DBACKUP,STATUS,ERROR,ABORT";
...
dmfcsp(DMC_CB *dmc_cb)
{
	...
	*/
    ...
    if ( csp_debug )
    {
	_VOID_ ule_format(E_DM9913_CSP_DEBUG_ON, (CL_ERR_DESC *)0, 
	...
	*/
	     ...
	     csp->csp_node_failure )
	{
	    ...
	    */
	    ...
	    if ( status && err_code )
	    {
		_VOID_ ule_format(err_code, (CL_ERR_DESC *)0, ULE_LOG,
...
csp_message_handler( CX_MSG *msgblk, PTR pauxdata, bool invalid )
{
	...
	ule_format(E_DM991E_CSP_BAD_BROADCAST, (CL_ERR_DESC *)0,
	...
	*/
	...
	ule_format(E_DM9910_CSP_BADMSG, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
		    &status, 0);
	break;
    }
...
csp_alert_overflow()
{
    ...
    ule_format(E_DM9912_CSP_READ_QUEUE, (CL_ERR_DESC *)0,
		ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
		&status, 0);
} /* csp_alert_overflow */
...
dmfcsp_msg_thread(DMC_CB *dmc_cb)
{
    ...
    if ( 0 != ( dmc_cb->error.err_code = 
	 CXmsg_connect( CSP_CHANNEL, csp_message_handler, NULL ) ) )
	status = E_DB_ERROR;
    return status;
} /* dmfcsp_msg_thread */
...
csp_deadman_notify( CX_REQ_CB *preq, i4 status )
{
	...
	(void)LKalter( LK_A_STALL, 0, (CL_ERR_DESC *)0 );
...
csp_initialize(bool *recovery_done)
{
	...
	*/
	...
	status = dm0m_allocate( sizeof (DMP_MISC) + CSP_ALERT_QUEUE_SIZE, 
				0, MISC_CB, MISC_ASCII_ID,
			        0, (DM_OBJECT**)&new_object, &errcode );
	if ( status ) break;
	...
	*(i4 *)(csp->csp_alert_qbuf_write) = CSP_ALERT_QUEUE_EOF;
    ...
    {
	if ( errcode )
	    _VOID_ ule_format(errcode, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &errcode, 0);
	_VOID_ ule_format(E_DM9900_CSP_INITIALIZE, (CL_ERR_DESC *)0,
		ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, &errcode, 0);
    }
...
i4		    *err_code)
{
    ...
    if (status != E_DB_OK)
    {
	ule_format(E_DM940E_RCP_RCPRECOVER, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	...
	*/
	...
	{
	    if (status = dmr_get_cp(rcp, i, err_code))
	    {
	...
	*/
	if (header->lgh_status != LGH_OK)
	{
	    ...
	    if (status != OK)
	    {
		_VOID_ ule_format(status, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, err_code, 0);
		_VOID_ ule_format(E_DM900B_BAD_LOG_ALTER, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, err_code, 1,
		    0, LG_A_NODELOG_HEADER);
		_VOID_ ule_format(E_DM9405_RCP_GETCP, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
		break;
	    }
		...
		_VOID_ ule_format(status, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, err_code, 0);
		_VOID_ ule_format(E_DM9010_BAD_LOG_FORCE, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, err_code, 1,
		    0, lctx->lctx_lxid);
		_VOID_ ule_format(E_DM9405_RCP_GETCP, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
	...
	*/
	...
	status = dmr_offline_context(rcp, recover[i], err_code);
	if (status)
	    break;
    }
    if (status)
    ...
    */
    for (i = 0; i < logs; i++)
    {
	status = dmr_analysis_pass(rcp, recover[i], err_code);
	if (status)
	    break;
    }
    if (status)
    ...
    */
    status = dmr_init_recovery(rcp, err_code);
    if (status)
	crash(*err_code, &sys_err);
    ...
    */
    for (i = 0; i < logs; i++)
    {
	status = dmr_redo_pass(rcp, recover[i], err_code);
	if (status)
	    break;
    }
    if (status)
    ...
    */
    ...
    status = dmr_undo_pass(rcp, err_code);
    if (status)
	crash(*err_code, &sys_err);

    status = dmr_complete_recovery(rcp, num_nodes, err_code);
    if (status)
	crash(*err_code, &sys_err);
...
csp_restart(bool *local_log_recovered, i4 *errcode)
{
	...
	for (;;)
	{
	    ...
	    */
		...
		if (local_status = LGalter(LG_A_NODELOG_HEADER,
				    ...
				    &sys_err))
		{
		    _VOID_ ule_format(local_status, &sys_err, ULE_LOG, NULL,
			    (char *)0, 0L, (i4 *)0, errcode, 0);
		    break;
		}
		...
		if (local_status != OK)
		{
			...
			(char *)0, 0L, (i4 *)0, errcode, 0);
		    _VOID_ ule_format(E_DM9010_BAD_LOG_FORCE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, errcode, 1,
			0, lctx->lctx_lxid);
		    break;
		}
	    }
	...
	*errcode = csp_release_recovery_lock();
	...
	TRformat(opc_send, 0, buffer, sizeof(buffer), 
...
csp_archive(DMP_LCTX *lctx, i4 *err_code)
{
    ...
    */
    status = dma_prepare(DMA_CSP, &acb, lctx, err_code);
    if (status != E_DB_OK)
    {
...
csp_node_fail(i4 *err_code)
{
	      ...
	      i++ )
	{
	    ...
	    TRformat(opc_send, 0, buffer, sizeof(buffer), 
		"INGRES/CSP Recovering for failing node %~t",
	...
	*/
	...
	for (i = 0; i < actual_nodes; i++)
	{
		...
		*err_code = 0; /* to prevent caller from logging err twice */ 
	    ...
	    local_status = dm0l_deallocate(&lctx_array[i], err_code);
	    if (local_status != OK)
		break;
	}
	...
	*/
	...
	if (local_status != OK)
	{
	    (VOID) ule_format(local_status, (CL_ERR_DESC *)0, ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);
	...
	*/
	if (csp_global.csp_master_node == csp_global.csp_self_node)
	{
		...
		TRformat(opc_send, 0, buffer, sizeof(buffer), 
		    "INGRES/CSP Recovering dbms classes for failing node %~t",
		    cx_node_info->cx_node_name_l,
		    cx_node_info->cx_node_name);
...
CSP_CX_MSG	*ckp_msg)
{
    ...
    _VOID_ TRdisplay(
    ERx("%@ CPP: %w(%d) Msg from node %d (%d)\n"),
	csp_ckp_type_names, ckp_msg->ckp.phase,
	ckp_msg->ckp.phase, 
	ckp_msg->csp_msg_node, ckp_msg->ckp.dbid);
	...
	case CKP_CSP_DBACKUP:
	    ...
	    */
		...
		TRdisplay(ERx("%@ CPP: %w(%d) Put action (%d)\n"),
		    csp_ckp_type_names, ckp_msg->ckp.phase,
		    ckp_msg->ckp.phase,
		    ckp_msg->ckp.dbid);
	...
	case CKP_CSP_ABORT:
	    ...
	    */
	    TRdisplay(ERx("%@ CPP: %w(%d) After phase %d (%d)\n"),
		    csp_ckp_type_names, ckp_msg->ckp.phase,
		    ckp_msg->ckp.phase,
...
csp_ckp_action(SCF_FTX *ftx)
{
	...
	status = online_cx_phase(ckp_msg->ckp.phase, &jsx);
        ...
        if (status != E_DB_OK)
	{
	    TRdisplay(ERx("%@ CPP: %w(%d) Error checkpointing %~t\n"),
		csp_ckp_type_names, ckp_msg->ckp.phase,
		ckp_msg->ckp.phase,
...
CSP_CX_MSG	*ckp_msg)
{
	...
	/* Local node:	 Request LK_CKP_CLUSTER status lock */
	if ( csp->csp_self_node == ckp_msg->ckp.ckp_node)
	{
	    /* Local node: Request LK_CKP_CLUSTER CKP_CSP_STATUS lock */ 
	    lockckey.lk_type = LK_CKP_CLUSTER;
	    lockckey.lk_key1 = ckp_msg->ckp.dbid;
	    ...
	    TRdisplay(ERx("%@ CPP: %w(%d) LKrequest LK_CKP_CLUSTER (%w)\n"),
		    csp_ckp_type_names, ckp_msg->ckp.phase,
		    ckp_msg->ckp.phase,
		    csp_ckp_type_names, CKP_CSP_STATUS);
	    lk_status = LKrequest(LK_PHYSICAL | LK_NOWAIT, 
		    dmf_svcb->svcb_lock_list,
		    &lockckey, LK_X, (LK_VALUE * )0, &lockid,
		    0L, &sys_err);
	    if (lk_status != OK)
	        break;

	    status = csp_ckp_lkconvert(ckp_msg, &lockid, E_DB_OK, pid);
	    if (status != E_DB_OK)
		loc_status = csp_ckp_lkrelease(ckp_msg, CKP_CSP_STATUS);
	}
	...
	if ( csp->csp_self_node != ckp_msg->ckp.ckp_node)
	{
	    ...
	    {
		lockckey.lk_key2 = pnum;
		TRdisplay(ERx("%@ CPP: %w(%d) LKrequest LK_CKP_CLUSTER (%w)\n"),
		    csp_ckp_type_names, ckp_msg->ckp.phase,
		    ckp_msg->ckp.phase,
		    csp_ckp_type_names, pnum);
		lk_status = LKrequest(LK_PHYSICAL | LK_NOWAIT, 
		    dmf_svcb->svcb_lock_list, 
    ...
    {
	TRdisplay(ERx("%@ CPP: %w(%d) Testing error handling\n"),
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase);
        status = E_DB_ERROR;
    }
...
csp_ckp_error(CSP_CX_MSG *ckp_msg)
{
    ...
    TRdisplay(ERx("%@ CPP: %w(%d) (%d) -> ERROR \n"), 
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase, ckp_msg->ckp.dbid);
...
csp_ckp_lkrelease(CSP_CX_MSG *ckp_msg, i4 pnum)
{
    ...
    TRdisplay(ERx("%@ CPP: %w(%d) LKrelease LK_CKP_CLUSTER (%w)\n"),
	csp_ckp_type_names, ckp_msg->ckp.phase,
	ckp_msg->ckp.phase, 
	csp_ckp_type_names, pnum);
    ...
    if (cl_status != OK)
    {
	TRdisplay(ERx("%@ CPP: %w(%d) LKrelease error for %w\n"),
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase, 
	    csp_ckp_type_names, pnum);
...
DMF_JSX		*jsx)
{
    ...
    */
    ...
    {
	TRdisplay(ERx("%@ CPP: %w(%d) Database %d is not known to the logging system"),
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase, ckp_msg->ckp.dbid);
	ule_format(cl_status, &sys_err, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &err_code, 0);
    ...
    */
    ...
    {
	TRdisplay(ERx("%@ CPP: %w(%d) LGshow DB buffer size mismatch (%d vs %d)\n"),
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase,
    ...
    TRdisplay(ERx("%@ CPP: %w(%d) Init context for %d, %~t\n"),
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase, ckp_msg->ckp.dbid,
    ...
    */
    ...
    if (ckp_msg->ckp.flags & CKP_CRASH)
    {
	TRdisplay(ERx("%@ CPP: %w(%d) Init testing of error handling\n"),
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase);
...
/*{
** Name: csp_ckp_lkconvert - Update LK_CKP_CLUSTER CKP_CSP_STATUS lock value
**
** Description:
**	Update LK_CKP_CLUSTER CKP_CSP_STATUS lock (value)
**
** Inputs:
**      ckp_msg				ckp message
...
i4 high_value, i4 low_value)
{
    ...
    **
    ** Local node: Convert LK_CKP_CLUSTER CKP_CSP_STATUS lock
    ** LK_CONVERT LK_X->LK_X to store LK_VALUE
    ** (high value has status, low value has ckpdb pid)
    **
    ...
    */
    ...
    TRdisplay(ERx("%@ CPP: %w(%d) LKconvert CKP_CSP_CLUSTER (%d,%d)\n"), 
	    csp_ckp_type_names, ckp_msg->ckp.phase,
...
i4			*log_id)
{
    ...
    *log_id = 0;
	...
	&dblocation->physical, dblocation->phys_length, log_id, &err_code); 

    if (status)
    {
	TRdisplay(ERx("%@ CPP: %w(%d) Add database ERROR\n"), 
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase);
	return (status);
    }

    TRdisplay(ERx("%@ CPP: %w(%d) Add database log_id %d\n"), 
	    csp_ckp_type_names, ckp_msg->ckp.phase,
    ...
    lockckey.lk_type = LK_CKP_CLUSTER;
    lockckey.lk_key1 = ckp_msg->ckp.dbid;
    lockckey.lk_key2 = CKP_CSP_INIT;
    lockckey.lk_key3 = csp->csp_self_node;
    lockckey.lk_key4 = 0;
    lockckey.lk_key5 = 0;
    lockckey.lk_key6 = 0;
    MEfill(sizeof(LK_LKID), 0, &lockid);
    ...
    TRdisplay(ERx("%@ CPP: %w(%d) LKrequest LK_CKP_CLUSTER (%w)\n"),
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase, 
	    csp_ckp_type_names, CKP_CSP_INIT);
	...
	*/
    ...
    if (status)
        loc_status = dm0l_closedb(*log_id, &err_code);
...
csp_ckp_closedb(CSP_CX_MSG *ckp_msg, i4 log_id)
{
    ...
    /* Close database */
    status = dm0l_closedb(log_id, &err_code);
    TRdisplay(ERx("%@ CPP: %w(%d) Close database log_id %d\n"), 
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase, log_id);
...
csp_ckp_logid(CSP_CX_MSG *ckp_msg, i4 *log_id)
{
    ...
    *log_id = lock_value.lk_low;
    if (*log_id == 0)
	return (E_DB_ERROR);
    TRdisplay(ERx("%@ CPP: %w(%d) Using log_id %d\n"), 
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase, *log_id);
    return (E_DB_OK);
}
...
csp_ckp_lkrelease_all(CSP_CX_MSG *ckp_msg)
{
    ...
    TRdisplay(ERx("%@ CPP: %w(%d) LKRelease LK_CKP_CLUSTER (ALL) for %d\n"),
	    csp_ckp_type_names, ckp_msg->ckp.phase,
	    ckp_msg->ckp.phase, ckp_msg->ckp.dbid);
	...
	*/
	...
	for (i = 0; i < ((i4)length / sizeof(LK_LKB_INFO)); i++)
	{
	    ...
	    if (lkb->lkb_key[0] == LK_CKP_CLUSTER 
		    && lkb->lkb_key[1] == ckp_msg->ckp.dbid)
	    {
		csp_ckp_lk = TRUE;

		if (lkb->lkb_key[2] == CKP_CSP_STATUS)
		{
		    /* Invalidate lock value before releasing */
		    lockid.lk_unique = lkb->lkb_id;
		    lockid.lk_common = lkb->lkb_rsb_id;
		    lstatus = csp_ckp_lkconvert(ckp_msg, &lockid, E_DB_ERROR, 0);
		}

		TRdisplay(ERx("%@ CPP: %w(%d) LKRelease LK_CKP_CLUSTER %w(%d)\n"),
		    csp_ckp_type_names, ckp_msg->ckp.phase,
		    ckp_msg->ckp.phase, 
		    csp_ckp_type_names, lkb->lkb_key[2], lkb->lkb_key[2]);
		...
		TRdisplay(ERx("%@ CPP: %w(%d) LKRelease LK_CKP_TXN %~t %d\n"),
		    csp_ckp_type_names, ckp_msg->ckp.phase,
		    ckp_msg->ckp.phase, 
