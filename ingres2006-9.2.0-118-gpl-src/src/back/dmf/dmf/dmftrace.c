...
**	DM414	- Turn on/off Cache Flush tracing.
**	DM420	- Dump Buffer Manager Statistics.
**	DM421	- Toss pages from buffer cache.
**	DM422	- Toss unmodified pages from buffer cache.
**	DM425	- Set table cache priority (obsolete)
...
i4		trace_point)
{
    ...
    */
	...
	xcb = scb->scb_x_next;
	(VOID) dm2t_purge_tcb(dcb, &table_id, DM2T_PURGE, xcb->xcb_log_id,
	    xcb->xcb_lk_id, &xcb->xcb_tran_id, DM2D_IX, &error);
    }
...
DB_DEBUG_CB *debug_cb)
{
	...
	*/
	...
	ule_format(E_DM9395_SYSTEM_SHUTDOWN_TEST, (CL_SYS_ERR *)0, ULE_LOG, 
      ...
      case 29:
      {
	    ...
	    */
	    ...
	    status = dm0l_test(xcb->xcb_log_id, journal_flag, test_number, 
		    0, 0, 0, 0, (char *)0, 0, (char *)0, 0, &lsn, &error);
	    break;
      }
