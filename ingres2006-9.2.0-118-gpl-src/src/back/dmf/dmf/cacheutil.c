...
cache_list(VOID)
{
    ...
    if ((inst_code == NULL) || (*inst_code == NULL))
    {
...
# endif
    ...
    */
	...
	ule_format(status1, &sys_err, ULE_LOG, NULL, (char *)0, 0L, 
	    (i4 *)0, &err_code, 0);
	cache_os_error(&sys_err);
	TRdisplay(" \n");
	return (CAC_FAIL);
...
i4		*dbms_count)
{
	...
	*/
	MEcopy((PTR)DB_CACHEUTIL_NAME, sizeof(add_info.ad_dbname),
	...
	*/
	MEcopy((PTR)DB_DCACHEUTIL_NAME, sizeof(DB_OWN_NAME),
	    (PTR) &user_name);
