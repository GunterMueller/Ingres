...
static DB_STATUS rfp_get_index_info(
    DMF_RFP             *rfp,
    DMF_JSX             *jsx,
    DMP_DCB             *dcb,
    DB_STATUS           *err_code);
...
DB_STATUS	    *err_code)
{
	...
	if (jsx->jsx_status2 & JSX2_ON_ERR_PROMPT)
	{
	    ...
	    */
	    ...
	    {
		ule_format(tr_status, &sys_err, ULE_LOG , NULL,
    ...
    "\nYou have specified -on_error_prompt, Do you want to continue Y/N: \n");
        ...
        */
        status = rfp_validate_cl_options( jsx, &rfp, dcb, err_code);
        if ( status != E_DB_OK )
            break;
	...
	*/
	status = prepare(&rfp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
		...
		|| (jsx->jsx_status1 & JSX_END_LSN))
	{
	    ...
	    */
	    ...
	    ule_format(E_DM1372_RFP_JNL_WARNING, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    error_buffer, sizeof(error_buffer), &error_length,
		    err_code, 2, STlength(line_buffer), line_buffer,
	...
	*/
	if ( ! ckp_processing)
	{
	    status = validate_nockp(jsx, dcb, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	if (ckp_processing)
	{
	    ...
	    status = rfp_ckp_processing(&rfp, jsx, dcb, err_code);
	    if (status != E_DB_OK)
		break;
	    ...
	    if ( jsx->jsx_status1 & JSX1_RECOVER_DB )
    	    {
                status = db_post_ckp_config(&rfp, jsx, dcb, err_code);
                if (status != E_DB_OK)
                    break;
	    }
        }
	...
	*/
	...
	{
	    status = reloc_tbl_process(&rfp, jsx, dcb, 
				RELOC_FINISH_TABLES, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	  ...
	  ((rfp.open_db & DB_OPENED) == 0))
	{
		...
		(DM2D_NLK | DM2D_NLG | DM2D_RFP | DM2D_SYNC_CLOSE), err_code);

	    if (status != E_DB_OK)
	    {
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
		*err_code = E_DM1300_RFP_OPEN_DB;
		break;
	    }
	...
	*/
	if (dmp_processing)
	{
	    ...
	    */
	    ...
	    status = read_dumps(jsx, &rfp, &dmve, err_code);
	...
	*/
	if (jnl_processing)
	{
	    ...
	    */
	    ...
	    status = read_journals(jsx, &rfp, &dmve, err_code);
	    if (status != E_DB_OK)
	    {
		db_valid = FALSE;
		redo_error = TRUE;
	    ...
	    */
		...
		status = rollback_open_xacts(jsx, &rfp, dcb, &dmve, err_code);
		if (status != E_DB_OK)
		{
		    db_valid = FALSE;
		    break;
	...
	*/
	...
	status = rfp_complete(&rfp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	{
	    status = dm2d_close_db(dcb, dmf_svcb->svcb_lock_list,
				   (DM2D_NLG | DM2D_NLK), err_code);
	    if (status != E_DB_OK)
	    {
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
		*err_code = E_DM1310_RFP_CLOSE_DB;
		break;
	    }
	...
	status = config_handler(OPEN_CONFIG, dcb, &cnf, &local_error);
	if (status == E_DB_OK)
	{
	    ...
	    status = config_handler(UPDTCOPY_CONFIG, dcb, &cnf, &local_error);
	    if (status == E_DB_OK)
		status = config_handler(CLOSEKEEP_CONFIG, dcb, &cnf, &local_error);
	}

	if (status != E_DB_OK)
	{
	    dmf_write_msg(local_error);
	    /* continue and try to rollback open transactions and close db */
	}
	else
	{
	    dmf_write_msg(E_DM0100_DB_INCONSISTENT);
	    TRformat(dmf_diag_put_line, 0, line_buffer, sizeof(line_buffer),
	...
	if (redo_error && rfp.rfp_tx_count && rfp.rfp_redo_err)
	{
		...
		*/
		...
		TRformat(dmf_diag_put_line, 0, line_buffer, sizeof(line_buffer),
		    "Begin rollback of open transactions with end lsn (%d,%d)\n",
		    jsx->jsx_end_lsn.lsn_high, jsx->jsx_end_lsn.lsn_low);

		init_recovery(&rfp, dcb, &dmve, DMVE_UNDO);

		status = rollback_open_xacts(jsx, &rfp, dcb, &dmve, &local_error);
		...
		if (status == E_DB_OK)
		{
		    ...
		    TRformat(dmf_diag_put_line, 0, line_buffer, sizeof(line_buffer),
		    "Only transactions completed before LSN (%d,%d) were applied.\n",
		    jsx->jsx_end_lsn.lsn_high, jsx->jsx_end_lsn.lsn_low
		    );
    ...
    */
    if ((*err_code != E_DM1315_RFP_DB_BUSY))
    {
	/* only write the msg if it hasn't been written */
	if (*err_code != 0)
	    dmf_write_msg(*err_code);
	*err_code = E_DM1301_RFP_ROLL_FORWARD;
    }
...
DB_STATUS	    *err_code)
{
	...
	*/
    ...
    status = config_handler(OPEN_CONFIG, dcb, &cnf, err_code);
    if (status == E_DB_OK)
    {
	if (jsx->jsx_status & JSX_TRACE)
	...
	*/
	...
	if (jsx->jsx_status2 & JSX2_INCR_JNL)
	{
            cnf->cnf_dsc->dsc_status &= ~DSC_VALID;
	    cnf->cnf_dsc->dsc_inconst_time = TMsecs();
	    cnf->cnf_dsc->dsc_inconst_code = DSC_INCR_RFP_INCONS;
	    incons = TRUE;
	}
	...
	** Set dsc_inconst_code to the most serious error ignored
	...
	status = config_handler(UPDTCOPY_CONFIG, dcb, &cnf, err_code);
	if (status == E_DB_OK)
	    status = config_handler(CLOSEKEEP_CONFIG, dcb, &cnf, err_code);
	if (status != E_DB_OK)
	{
	    *err_code = E_DM1313_RFP_CKP_HISTORY;
	    return (status);
	}
    ...
    {
	ule_format(E_DM9003_BAD_FILE_DELETE, &sys_err,
            ULE_LOG, NULL, 0, 0, 0, err_code, 4,
	    sizeof(dcb->dcb_name), dcb->dcb_name.db_db_name,
	    4, "None",
	    dcb->dcb_root->phys_length, &dcb->dcb_root->physical,
	    12, "aaaaaaaa.rfc");
	*err_code = E_DM1309_RFP_RFC_DELETE;
	return (E_DB_ERROR);
    }
...
DB_STATUS           *err_code)
{
    ...
    if ( jsx->jsx_status1 & JSX1_RECOVER_DB )
    {
        status = rfp_db_complete( rfp, jsx, dcb, err_code );
    }
    else if ( jsx->jsx_status & JSX_TBL_LIST )
    {
        ...
        ** consistent etc...
        */
        status = rfp_tbl_complete( rfp, jsx, dcb, err_code);
    }
...
DB_STATUS	    *err_code)
{
	...
	*/
	...
	ule_format(E_DM1346_RFP_NOLOGGING, (CL_ERR_DESC *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &loc_error, 1,
	    0, local_buffer);
    }
	...
	*/
	...
	local_buffer[ DB_MAXNAME ] = '\0';
	STtrmwhite(local_buffer);
	ule_format(E_DM1347_RFP_NOROLLBACK, (CL_ERR_DESC *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, &loc_error, 1,
	    0, local_buffer);
    }
    ...
    if ( jsx->jsx_status1 & JSX1_RECOVER_DB )
    {
        ...
        */
        status = rfp_prepare_db_recovery( rfp, jsx, dcb, err_code);
    }
    else if ( jsx->jsx_status & JSX_TBL_LIST )
    {
        /*
        ...
        */
        ...
        status = rfp_prepare_tbl_recovery( rfp, jsx, dcb, err_code);
...
DB_STATUS	    *err_code)
{
    ...
    */
	...
	for ( new_jnl_cnt = 0; ; )
	{
	    status  = rfp_read_incr_journals_db(jsx, rfp, dmve, 
		&new_jnl_cnt, err_code);
	...
	if (jsx->jsx_status & JSX_TRACE)
		...
		total, status, *err_code, new_jnl_cnt);
    }
    else if ( jsx->jsx_status1 & JSX1_RECOVER_DB )
    {
	status  = rfp_read_journals_db(jsx, rfp, dmve, err_code);
    }
    else if ( jsx->jsx_status & JSX_TBL_LIST )
    {
	status = rfp_read_journals_tbl(jsx, rfp, dmve, err_code);
    }
...
#define	PROCESS	1
    ...
    */
    status = dmckp_init( d, DMCKP_RESTORE, err_code );
    if ( status != E_DB_OK )
        return( status );
    ...
    */
    status = dmckp_begin_journal_phase( d, err_code );
    if ( status != E_DB_OK )
        return( status );
    ...
    */
	...
	status = rfp_prescan_btime_qual(jsx, rfp, dmve, err_code);
    }
	...
	phase <= PROCESS; phase++, rfp->rfp_status = save_rfp_status)
  {
	     ...
	     i++)
	{
	    ...
	    */
	    if (i == 0)
            ...
            */
            dm0j_filename( DM0J_MERGE, i, -1, &jnl_filename, (DB_TAB_ID *)0 );

            d->dmckp_jnl_file   = jnl_filename.name;
            d->dmckp_jnl_l_file = sizeof( jnl_filename );

            status = dmckp_work_journal_phase( d, err_code );
            if ( status != E_DB_OK )
                break;
	    ...
	    status = dm0j_open(DM0J_MERGE,
			...
			-1, DM0J_FORWARD, (DB_TAB_ID *)0, &jnl, err_code);
	    if (status != E_DB_OK)
	    {
		if (*err_code == E_DM9034_BAD_JNL_NOT_FOUND)
		{
		    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0,
				err_code, 0);
		    TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
		...
		status = dm0j_read(jnl, (PTR *)&record, &l_record, err_code);
		if (status != E_DB_OK)
		{
		...
		*/
		if ( (!lsn_check) && (jsx->jsx_status1 & JSX_START_LSN) )
		{
			...
			ule_format(E_DM1053_JSP_INV_START_LSN,
			    (CL_ERR_DESC *)0, ULE_LOOKUP, (DB_SQLSTATE *)NULL,
			...
			*err_code = E_DM1306_RFP_APPLY_RECORD;
			status = E_DB_ERROR;
			break;
		    }
		...
		*/
		...
		if (jsx->jsx_status2 & JSX2_INVALID_TBL_LIST)
		{
		    if (( rfp_filter_log_record( rfp, jsx, record, err_code)) == 0)
		    {
			/*
		...
		if ((jsx->jsx_status & JSX_TRACE) && (phase == PROCESS))
		    dmd_log(FALSE, (PTR)record, rfp->jnl_block_size);
                ...
                if (no_jnl)
                {
                    ...
                    ule_format(E_DM134C_RFP_INV_CHECKPOINT,
                        (CL_ERR_DESC *)0, ULE_LOOKUP, (DB_SQLSTATE *)NULL,
                        error_buffer, sizeof(error_buffer),
                        &error_length, err_code, 2,
		...
		{
		    if (phase == PRESCAN)
			status = prescan_records(rfp, dmve, (PTR)record, err_code);
		    else
		        status = process_record(rfp, dmve, (PTR)record, err_code);
		}
		jnl_done++;
		...
		if (status)
		{
		    ...
		    *err_code = E_DM1306_RFP_APPLY_RECORD;
		    status = rfp_error(rfp, rfp->rfp_jsx, rfp->rfp_cur_tblcb,
				dmve, RFP_FAILED_TABLE, status, err_code);
		}
    ...
    */
    status = dmckp_end_journal_phase( d, err_code );
    if ( status != E_DB_OK )
        return( status );
...
DB_STATUS	    *err_code)
{
    ...
    ** Call the OpenJournal begin routine
    */
    status = dmckp_begin_dump_phase( d, err_code );
    if ( status != E_DB_OK )
        return( status );
    ...
    for (i = rfp->dmp_last; i >= rfp->dmp_first; i--)
    {
        ...
        */
        ...
        status = dmckp_work_dump_phase( d, err_code );
        if ( status != E_DB_OK )
            break;
	...
	status = dm0d_open(DM0D_MERGE, 
	    ...
	    DM0D_EXTREME, DM0D_M_READ, -1, DM0D_BACKWARD, &dmp, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	    *err_code = E_DM1324_RFP_DMP_OPEN;
	    break;
	}
	...
	for (;;)
	{
	    ...
	    status = dm0d_read(dmp, (PTR *)&record, &l_record, err_code);
	    if (status != E_DB_OK)
	    {
		if (*err_code == E_DM0055_NONEXT)
		{
		...
		ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
		*err_code = E_DM1339_RFP_DMP_READ;
		break;
	    }
		...
		|| (jsx->jsx_status2 & JSX2_INVALID_TBL_LIST))
            {
                if (!( rfp_filter_log_record( rfp, jsx, record, err_code)))
                {
                    /*
                    ** Not interested so skip this record
                    */
	    ...
	    status = apply_d_record(rfp, jsx, dmve, record, err_code);

	    if (status)
	    {
		/* Check if ok to continue */
		dmf_write_msg(*err_code);
		*err_code = E_DM1325_RFP_APPLY_DRECORD;
		status = rfp_error(rfp, rfp->rfp_jsx, rfp->rfp_cur_tblcb,
			    dmve, RFP_FAILED_TABLE, status, err_code);
	    }
    ...
    */
    status = dmckp_end_dump_phase( d, err_code );
    if ( status != E_DB_OK )
        return( status );
...
DB_STATUS	    *err_code)
{
	...
	*/
	status = config_handler(OPENRFC_CONFIG, dcb, &cnf, err_code);
	if (status != E_DB_INFO)
	{
	    if (status == E_DB_OK)
	    {
		status = E_DB_ERROR;
		*err_code = E_DM1311_RFP_MUST_RESTORE;
	    }
	    break;
	}
	...
	*/
    ...
    if (*err_code != E_DM1311_RFP_MUST_RESTORE)
    {
        ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM1348_RFP_CHECK_NOCKP;
    }
...
DB_STATUS	    *err_code)
{
    ...
    */
    ...
    if (status != E_DB_OK)
    {
        ule_format(*err_code, &sys_err, ULE_LOG, NULL,
            (char *)0, (i4)0, (i4 *)0, err_code, 0);
        ule_format(E_DM133B_RFP_DUMP_CONFIG, &sys_err, ULE_LOG, NULL,
            (char *)0, (i4)0, (i4 *)0, err_code, 0);
    ...
    */
    ...
    status = config_handler(UNKEEP_CONFIG, dcb, &cnf, err_code);
    if (status == E_DB_OK)
        status = config_handler(OPEN_CONFIG, dcb, &cnf, err_code);
    if (status == E_DB_OK)
        status = config_handler(CLOSEKEEP_CONFIG, dcb, &cnf, err_code);
    ...
    */
    status = dmckp_begin_restore_db( d, dcb, err_code );
    if ( status != E_DB_OK )
        return( status );
    ...
    */
    ...
    {
        *err_code = E_DM130C_RFP_DELETE_ALL;
        return (E_DB_ERROR);
    }
    ...
    **  Get a free device
    */
    status = rfp_wait_free(jsx, d, err_code);
    if (status != OK)
    {
	ule_format(E_DM1307_RFP_RESTORE, &sys_err,
    ...
    */
    status = dmckp_pre_restore_loc( d, dcb, err_code );
    if (status != OK)
    {
	*err_code = E_DM1307_RFP_RESTORE;
	return (E_DB_ERROR);
    }
    status = dmckp_restore_loc( d, dcb, err_code );
	...
	*(ext->ext_entry[i].physical.name + ext->ext_entry[i].phys_length) = '\0';
	...
	if (!(ext->ext_entry[i].flags & DCB_RAW))
	    status = dmckp_delete_loc( d, err_code );
	/*
    ...
    */
    status = rfp_wait_all(jsx, err_code);
    if (status != OK)
    {
        *err_code = E_DM1307_RFP_RESTORE;
        return (E_DB_ERROR);
    ...
    */
    status = dmckp_end_restore_db( d, dcb, err_code );
    if ( status != E_DB_OK )
        return( status );
	...
	*/
	...
	for (i = 0; i < ext->ext_count; i++)
	{
        ...
        status = config_handler(UPDATE_CONFIG, dcb, &cnf, err_code);
        status = config_handler(CLOSEKEEP_CONFIG, dcb, &cnf, err_code);
    }
    ...
    if (status != OK)
    {
        if (status == DI_FILENOTFOUND)
        {
            ...
            *err_code = E_DM130D_RFP_BAD_RESTORE;
            return (E_DB_ERROR);
        }
        ule_format(E_DM9003_BAD_FILE_DELETE, &sys_err,
            ULE_LOG, NULL, 0, 0, 0, err_code, 4,
        ...
        *err_code = E_DM130E_RFP_ALIAS_DELETE;
        return (E_DB_ERROR);
    }
        ...
        if (status != OK)
        {
            if (status == DI_FILENOTFOUND)
            {
                ...
                if (status == E_DB_OK)
                {
                    ...
                    cnf->cnf_dsc->dsc_inconst_time = TMsecs();
                    cnf->cnf_dsc->dsc_inconst_code = DSC_RFP_FAIL;
                    status = config_handler(UPDATE_CONFIG, dcb, &cnf,
                        err_code);
                    status = config_handler(CLOSEKEEP_CONFIG, dcb, &cnf,
                        err_code);
                }
    ...
    if ((jsx->jsx_status & JSX_USEJNL) == 0 && (jsx->jsx_status & JSX_CKPXL))
    {
        ...
        if (status == E_DB_OK)
        {
            ...
            status = config_handler(UPDATE_CONFIG, dcb, &cnf, err_code);
            if (status == E_DB_OK)
                status = config_handler(CLOSE_CONFIG, dcb, &cnf, err_code);
        }
        if (status != E_DB_OK)
            return (status);
    }
...
DB_STATUS	    *err_code)
{
	...
	*/
	   ...
	   ((jsx->jsx_status & JSX_TBL_LIST) == 0))
	{
	    ...
	    */
	    ule_format(E_DM134E_RFP_LAST_TABID, (CL_SYS_ERR *)0, ULE_LOG,
		NULL, (char *)0, (i4)0, (i4 *)0, &local_error, 3,
		sizeof(DB_DB_NAME), dcb->dcb_name.db_db_name,
		0, rfp->last_tbl_id,
	...
	*/
	...
	/* RFP +c -j -incremental,  clear jnl history for last checkpoint */
	if (ckp_processing && !jnl_processing && (jsx->jsx_status2 & JSX2_INCR_JNL) )
	{
	    i4	jnl_last = cnf->cnf_jnl->jnl_count - 1;

	    if (jsx->jsx_status & JSX_TRACE)
		TRdisplay("%@ Clear incremental jnl history %d,%d for %d \n", 
		cnf->cnf_jnl->jnl_history[jnl_last].ckp_f_jnl,
		cnf->cnf_jnl->jnl_history[jnl_last].ckp_l_jnl,
		jnl_last);

	    cnf->cnf_jnl->jnl_history[jnl_last].ckp_f_jnl = 0;
	    cnf->cnf_jnl->jnl_history[jnl_last].ckp_l_jnl = 0;
	}
	...
	*err_code = E_DM004A_INTERNAL_ERROR;
	...
	if (tx == 0)
	{
	    ...
	    */
	    if (h->type == DM0LBT)
	    {
		status = add_tx(rfp, (DM0L_BT *)record, err_code); 
		if (status != E_DB_OK)
		{
		    dmf_write_msg(*err_code);
		    break;
		...
		MEcopy(DB_OWNUNKNOWN_NAME, sizeof(DB_OWN_NAME), &tmp_bt.bt_name);
		status = add_tx(rfp, &tmp_bt, err_code);
		if (status != E_DB_OK)
		{
		    dmf_write_msg(*err_code);
		    break;
	...
	status = apply_j_record(rfp, tx, dmve, (DM0L_HEADER *)record, err_code);
...
DB_STATUS	    *err_code)
{
	...
	*/
    ...
    if (rfp->rfp_jsx->jsx_status & JSX_VERBOSE)
    {
	...
	*err_code = E_DM1308_RFP_BAD_RECORD;
	status = E_DB_ERROR;
	break;
	...
	*err_code = E_DM1308_RFP_BAD_RECORD;
	...
	if (mod_rec->dum_flag & DM0L_ONLINE)
	{
	    ...
	    {
		status = rfp_osrc_close(octx, err_code);
	    }
	    delete_rawdata(tx);
	}
	else if (mod_rec->dum_flag & DM0L_START_ONLINE_MODIFY)
	{
		...
		else
		{
			...
			local_status = rfp_add_lsn_waiter(rfp, octx, err_code);
			if (local_status)
			    status = local_status;
			else
			    status = E_DB_OK;
	...
	*/
	status = gather_rawdata(tx, (DM0L_RAWDATA *) record, (i4 *) err_code);
	break;
    ...
    */
    ...
    default:
        ...
        */
        ...
        ule_format(E_DM1308_RFP_BAD_RECORD, (CL_SYS_ERR *)0, ULE_LOG, NULL,
            (char *)0, (i4)0, (i4 *)0, err_code, 0);
        status = E_DB_OK;
    }
	...
	local_status = rfp_check_lsn_waiter(rfp, record, &local_err);
    }
    ...
    {
	dmve->dmve_error.err_code = E_DM9601_DMVE_BAD_PARAMETER;
	status = E_DB_ERROR;
    }
    ...
    ** up to our caller.
    */
    if (status != E_DB_OK && *err_code != E_DM1308_RFP_BAD_RECORD)
	*err_code = dmve->dmve_error.err_code;
...
DB_STATUS	    *err_code)
{
	...
	*config = 0;
	...
	if (status == E_DB_OK)
	{
		...
		ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, &local_err_code, 0);
		*err_code = E_DM1304_RFP_MEM_ALLOC;
		break;
	    }
	...
	if (*config)
	    dm0c_close(*config, 0, err_code);
	*config = 0;
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM1302_RFP_CNF_OPEN;
	return (E_DB_ERROR);
	...
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM1303_RFP_CNF_CLOSE;
	return (E_DB_ERROR);
...
CL_SYS_ERR	    *sys_err)
{
    ...
    ule_format(E_DM9003_BAD_FILE_DELETE, sys_err, ULE_LOG, NULL, 0, 0, 0,
            &err_code, 4,
	    4, "None",
	    4, "None",
...
DB_STATUS	*err_code)
{
		...
		*/
		if (cnf->cnf_dsc->dsc_cnf_version >= CNF_V4)
		{
			...
			*/
			...
			{
			    *err_code = E_DM133C_RFP_UNUSABLE_CKP;
			    return (E_DB_ERROR);
			}
		    }
		}
	...
	*/
	ule_format(E_DM1327_RFP_INV_CHECKPOINT, 0,
		    ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM133C_RFP_UNUSABLE_CKP;
	return (E_DB_ERROR);
    }
...
DB_STATUS	    *err_code)
{
    ...
    */
    status = dm0m_allocate(sizeof(RFP_TX), 0, RFPTX_CB, RFPTX_ASCII_ID,
			(char *)rfp, (DM_OBJECT **)&tx, err_code);
    if (status != E_DB_OK)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM1320_RFP_TOO_MANY_TX;
	return (E_DB_ERROR);
    }
...
DB_STATUS	*err_code)
{
	...
	*/
	status = prepare_undo_phase(jsx, rfp, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	while (rollback_complete == FALSE)
	{
		...
		*/
		...
		{
		    status = E_DB_ERROR;
		    *err_code = E_DM133D_RFP_UNDO_INCOMPLETE;
		    break;
		}
	    ...
	    */
	    status = undo_journal_records(jsx, rfp, jnl, dcb,
					&rollback_complete, dmve, err_code);
	...
	** contexts.
	*/
	status = cleanup_undo_phase(jsx, rfp, dcb, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    if (jnl)
    {
	status = dm0j_close(&jnl, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
    ...
    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	(char *)0, (i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM133E_RFP_ROLLBACK_ERROR;
    return (E_DB_ERROR);
}

/*
...
DB_STATUS	*err_code)
{
	...
	for (tx = rfp->rfp_tx; tx != NULL; tx = tx->tx_next)
	{
	    ...
	    */
		...
		ule_format(E_DM900C_BAD_LOG_BEGIN, &sys_err, ULE_LOG, NULL,
		    ...
		    0, dcb->dcb_log_id);
		*err_code = E_DM133F_RFP_UNDO_BEGIN_XACT;
		status = E_DB_ERROR;
		break;
	    }
	...
	*/
    ...
    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	(char *)0, (i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM1340_RFP_PREPARE_UNDO;
    return (E_DB_ERROR);
}

/*
...
DB_STATUS	*err_code)
{
	...
	for (tx = rfp->rfp_tx; tx != NULL; tx = tx->tx_next)
	{
		...
		if (cl_status != OK)
		{
			...
			(char *)0, (i4)0, (i4 *)0, err_code, 1,
			0, tx->tx_id);
		    *err_code = E_DM1341_RFP_CLEANUP_UNDO;
		    status = E_DB_ERROR;
		    break;
		}
	    }
	    ...
	    {
		ule_format(E_DM1345_RFP_XACT_UNDO_FAIL,
		    (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		    (i4)0, (i4 *)0, err_code, 3,
		    sizeof(DB_DB_NAME), dcb->dcb_name.db_db_name,
	...
	*/
	...
	if (rollback_error)
	{
	    *err_code = E_DM1342_RFP_INCOMPLETE_UNDO;
	    break;
	}
...
DB_STATUS	*err_code)
{
	...
	*/
	...
	{
	    if (jsx->jsx_status & JSX_TRACE)
		TRdisplay("%@ RFP: Undo skipped LSN <%d,%d>.\n",
	...
	*/
	...
	status = apply_j_undo(jsx, rfp, dcb, tx, dmve, record, err_code);

	if (status != E_DB_OK)
	{
	    /* Check if ok to continue */
	    dmf_write_msg(*err_code);
	    *err_code = E_DM1343_RFP_JNL_UNDO;
	    status = rfp_error( rfp, rfp->rfp_jsx, rfp->rfp_cur_tblcb,
			dmve, RFP_FAILED_TABLE, status, err_code);
	}
...
DB_STATUS	*err_code)
{
    ...
    */
    ...
    if (status != E_DB_OK)
    {
	ule_format(dmve->dmve_error.err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	dmf_write_msg(E_DM1343_RFP_JNL_UNDO);
	*err_code = E_DM1344_RFP_APPLY_UNDO;
    }
...
DM0L_HEADER	    *record)
{
    ...
    */
    ...
    {
	TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
	    "%@ RFP: User supplied -start_lsn encountered.\n",
	    record->lsn.lsn_high, record->lsn.lsn_low);
	...
	*/
	if (LSN_EQ(&record->lsn, &jsx->jsx_end_lsn))
	    ...
	    TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
		"%@ RFP: User supplied -end_lsn encountered.\n",
		record->lsn.lsn_high, record->lsn.lsn_low);
    }
	...
	*/
	...
	TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
	    "%@ RFP: User supplied -e time encountered, LSN: (%d,%d).\n",
...
RFP_TX		    *tx)
{
    ...
    */
	...
	if (act_ptr->rfp_act_action == RFP_FRENAME)
	{
	    ...
	    */
	    status = dm2f_delete_file(dmf_svcb->svcb_lock_list, dmve->dmve_log_id,
			&loc, (i4) 1, (DM_OBJECT *)0, (i4)1, &error);
	    if ((status != E_DB_OK) && (error != E_DM9291_DM2F_FILENOTFOUND))
	    {
		ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, &error, 0);
		ule_format(E_DM134A_RFP_SPURIOUS_FILE, (CL_SYS_ERR *)0, ULE_LOG,
		 ...
		 (act_ptr->rfp_act_tabid.db_tab_index == 0))
	{
	    ...
	    */
	    ule_format(W_DM134D_RFP_NONJNL_TAB_DEL,
...
static DB_STATUS
gather_rawdata(RFP_TX *tx, DM0L_RAWDATA *record, i4 *err_code)
{
    ...
    */
    ...
    {
	ule_format(E_DM1323_RFP_BAD_RAWDATA, (CL_ERR_DESC *)0, ULE_LOG, NULL,
...
DB_STATUS       *err_code)
{
    ...
    if ( jsx->jsx_status1 & JSX1_RECOVER_DB )
    {
        ...
        */
        ...
        {
            ule_format(E_DM1354_RFP_INV_ARG_FOR_DB_RECOVERY,
                        (CL_SYS_ERR *)0, ULE_LOG, NULL,
                        error_buffer, ER_MAX_LEN, &error_length,
                        err_code, 1, 24, "force_logical_consistent");
        ...
        if ( jsx->jsx_status1 & JSX1_NOSEC_IND )
        {
            ule_format(E_DM1354_RFP_INV_ARG_FOR_DB_RECOVERY,
                        (CL_SYS_ERR *)0, ULE_LOG, NULL,
                        error_buffer, ER_MAX_LEN, &error_length,
                        err_code, 1, 17, "nosecondary_index");
	    dmf_put_line(0, error_length, error_buffer);
            status = E_DB_ERROR;
        }
	...
	**    +c +j
	...
	if (invalid_opt)
	{
	    ule_format(E_DM1373_RFP_INCR_ARG,
                        (CL_SYS_ERR *)0, ULE_LOG, NULL,
                        error_buffer, ER_MAX_LEN, &error_length,
                        err_code, 1, STlength(invalid_opt), invalid_opt);
	    dmf_put_line(0, error_length, error_buffer);
	    status = E_DB_ERROR;
	}
    }
    if ( status != E_DB_OK )
    {
        dmf_write_msg(E_DM1356_RFP_INVALID_ARGUMENT);
    }
    return( status );
}

...
DB_STATUS           *err_code)
{
    ...
    do
    {
	status = rfp_init_tbl_hash(rfp, err_code);
	if (status != E_DB_OK)
	    break;
        ...
        */
        ...
        {
            *err_code = E_DM130A_RFP_NO_CNFRFC;
            break;
        }
        ...
        status = rfp_prepare_validate_cnf_info( jsx, dcb, cnf, err_code);
        if ( status != E_DB_OK )
            break;
	...
	*/
	...
	{
	    *err_code = E_DM1361_RFP_NO_DB_RECOV_TBL_CKPT;
	    status = E_DB_ERROR;
	    break;
	}
	...
	for (i=0; i < cnf->cnf_dsc->dsc_ext_count; i++)
		...
		(DM_OBJECT **)&p, err_code);
        if (status != E_DB_OK)
        {
	    DB_STATUS		local_err_code;
            ule_format(*err_code, 0, ULE_LOG, 
		NULL, 0, 0, 0, &local_err_code, 0);
	    *err_code = E_DM1304_RFP_MEM_ALLOC;
            break;
        }     
	...
	** as this will try to create a journal file
        */
        if ( rfc_exists == FALSE && (jsx->jsx_status2 & JSX2_INCR_JNL) == 0)
        {
            status = rfp_prepare_drain_journals( jsx, dcb, &cnf, err_code);
            if ( status != E_DB_OK )
            {
                open_config = FALSE;
                break;
        ...
        */
        ...
        status = rfp_prepare_get_ckp_jnl_dmp_info( rfp, jsx, cnf,
                                   jnl_history_no, dmp_history_no, err_code);
        if ( status != E_DB_OK )
            break;
        ...
        */
            ...
            ((jsx->jsx_status & JSX_USECKP) != 0))
        {
            ...
            if (status != OK)
            {
                ule_format(E_DM9009_BAD_FILE_RENAME, &sys_err, ULE_LOG, NULL,
                    0, 0, 0, err_code, 4,
                    sizeof(dcb->dcb_name), dcb->dcb_name.db_db_name,
                    dcb->dcb_root->phys_length, &dcb->dcb_root->physical,
                    12, "aaaaaaaa.rfc",
                ...
                *err_code = E_DM130B_RFP_RENAME_CNF;
                return (E_DB_ERROR);
            }
    ...
    if (open_config)
    {
        ...
        lstatus = config_handler(CLOSE_CONFIG, dcb, &cnf, &lerr_code);
        if ( lstatus != E_DB_OK )
        {
            ule_format(lerr_code, 0, ULE_LOG, NULL, 0, 0, 0, &lerr_code, 0);
        }
    }
...
DB_STATUS           *err_code)
{
    ...
    */
    ...
    {
        *err_code = E_DM1316_RFP_NO_CHECKPOINT;
        return (E_DB_ERROR);
    }
    ...
    */
    ...
    else if (cnf->cnf_dsc->dsc_status & DSC_DISABLE_JOURNAL)
    {
        ...
        */
        ule_format(E_DM93A0_ROLLFORWARD_JNLDISABLE, (CL_ERR_DESC *)0,
                ULE_LOOKUP, (DB_SQLSTATE *)NULL,
                error_buffer, sizeof(error_buffer), &error_length, err_code, 1,
        ...
        ule_format( 0, (CL_ERR_DESC *)0, ULE_MESSAGE, (DB_SQLSTATE *)NULL,
                    error_buffer, error_length, &error_length, err_code, 0);
    }
                 ...
                 ((cnf->cnf_dsc->dsc_status & DSC_JOURNAL) != 0))
    {
        ...
        ** the terminal in any case.
        */
        ule_format(E_DM134F_RFP_INV_JNL, (CL_ERR_DESC *)0,
                   ULE_LOOKUP, (DB_SQLSTATE *)NULL,
                   error_buffer, sizeof(error_buffer), &error_length,
                   err_code, 1,
                   sizeof(dcb->dcb_name.db_db_name), dcb->dcb_name.db_db_name);
...
DB_STATUS	    *err_code)
{
    ...
    if ((*config)->cnf_dsc->dsc_status & DSC_JOURNAL)
    {
	TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
	...
	status = config_handler(CLOSE_CONFIG, dcb, &*config, err_code);
	if (status != E_DB_OK)
	    return( E_DB_ERROR );

	status = dm0l_drain(dcb, err_code);
	if (status == E_DB_INFO)
	{
	    consistent = FALSE;
	   /* status = E_DB_OK; */
        ...
        else if (status != E_DB_OK)
	{
	    ule_format( *err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		        (char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM1314_RFP_DRAIN_ERROR;
	    return (E_DB_ERROR);
	}

	status = config_handler(OPEN_CONFIG, dcb, &*config, err_code);
        if ( status != E_DB_OK )
	    return( E_DB_ERROR );
						...
						== 0))
	{
	    ...
	    */
	    ...
	    if (status != E_DB_OK)
	        ...
	        lstatus = config_handler( CLOSE_CONFIG, dcb, &*config, &lerr_code);
	        return( E_DB_ERROR );
            }
	}
    }
...
DB_STATUS	    *err_code)
{
	...
	*/
    ...
    "%@ RFP: Checkpoint history of journal tables indicates  that checkpoint is invalid\n");
	    *err_code = E_DM1327_RFP_INV_CHECKPOINT;
	    return (E_DB_ERROR);
	}
...
DB_STATUS	    *err_code)
{
    ...
    */
	...
	(DM_OBJECT **)&rfp->rfp_tblhcb_ptr, err_code);
    if ( status != E_DB_OK )
	return;
...
DB_STATUS	    *err_code)
{
    ...
    ** operates on only one database.
	...
	if (status != E_DB_OK)
        ...
        */
        ...
        status = rfp_open_database (rfp, jsx, dcb, open_flags, err_code);
	...
	status = rfp_close_database (rfp, dcb, open_flags, err_code);

        if (status != E_DB_OK)
        {
            ule_format(*err_code, 0, ULE_LOG, NULL,
                (char *)0, (i4)0, (i4 *)0, err_code, 0);
            *err_code = E_DM110A_CPP_CLOSE_DB;
            return (status);
        }
	...
	** is valid or not!
	*/
	status = rfp_read_tables (rfp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	{
	    dmf_write_msg (*err_code);
	    return (status);
	...
	/*
	** Determine which secondary indexes need to be rolled forward 
	...
	*/
	status = rfp_get_index_info (rfp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = rfp_get_blob_info (rfp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
 	...
 	if (status != E_DB_OK)
	{
	    ...
	    ** so display and log error message.
            */
	    *err_code = E_DM1355_RFP_NO_CORE_TBL_RECOVERY;
	    return (status);
	}
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ...
	    */
	    tmp_status = config_handler(CLOSE_CONFIG, dcb, &cnf, err_code);
	...
	*/
	if ( rfp->ckp_mode & CKP_TABLE )
	    status = rfp_recov_chkpt_tables (rfp, &dmp_physical,
						dmp_phys_length,err_code);
...
DMP_DCB		    *dcb)
{
    ...
    */
	...
	tblcb = (RFP_TBLCB *)((char *)totq - CL_OFFSETOF(RFP_TBLCB, tblcb_totq));
	    ...
	    (tblcb->tblcb_table_status & RFP_SYSTEM_CATALOG))
        {
	    /*
...
DB_STATUS           *err_code)
{
	...
	*/
	...
	if (tblcb->tblcb_table_status & RFP_VIEW_TABLE)
        {
	    /*
	    ** Table specified by the user and is a view 
	    */
	    dmf_write_msg(E_DM1358_RFP_VIEW_TBL_NO_RECV);
	    *err_code = E_DM135E_RFP_USR_INVALID_TABLE;
	    return (E_DB_ERROR);
	}

	if (tblcb->tblcb_table_status & RFP_GATEWAY_TABLE)
        {
	    /*
	    ** Table specified by the user and is a gateway table
	    */
	    dmf_write_msg(E_DM1359_RFP_GATEWAY_TBL_NO_RECV);
	    *err_code = E_DM135E_RFP_USR_INVALID_TABLE;
	    return (E_DB_ERROR);
	}
	...
	if (tblcb->tblcb_table_status & RFP_RECOV_DISALLOWED)
	    ...
	    ** for this table
	    */
	    *err_code = E_DM135F_RFP_NO_TBL_RECOV;
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		error_buffer, ER_MAX_LEN, &error_length, err_code, 1,
                DB_MAXNAME, tblcb->tblcb_table_name.db_tab_name);
	    dmf_put_line(0, error_length, error_buffer);
	    if ((tblcb->tblcb_table_status & RFP_USER_SPECIFIED) == 0)
		*err_code = E_DM1370_RFP_NO_SEC_INDEX;
	    else
	        *err_code = E_DM135E_RFP_USR_INVALID_TABLE;
	    return (E_DB_ERROR);
	}

	TMsecs_to_stamp(tblcb->tblcb_rel_ptr->relmoddate, &modify_date);
	    ...
	    >= 0)
	{
        ...
        */
		...
		((cnf->cnf_dsc->dsc_status & DSC_JOURNAL) == 0))  &&
	     (tblcb->tblcb_rel_ptr->relstamp12.db_tab_high_time >=
	  cnf->cnf_jnl->jnl_history[jnl_history_no].ckp_date.db_tab_high_time))
	    rfp->rfp_point_in_time = TRUE;
    }
	...
	if (!table_found)
	{
	    *err_code = E_DM1360_RFP_TBL_NOTFOUND;
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		error_buffer, ER_MAX_LEN, &error_length, err_code, 1,
...
DB_STATUS	    *err_code)
{
    ...
    do
    {
	status = tbllst_open(&tblctx, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, (char * )0,
		0L, (i4 *)0, &local_err, 0);
	    ule_format(E_DM1365_RFP_CHKPT_LST_OPEN, (CL_SYS_ERR *)0, ULE_LOG, 
	...
	{
	    status = tbllst_read(&tblctx, err_code);
	    if (status != E_DB_OK)
	    {
		if (status == E_DB_WARN)
		{
		...
		else
		{
			...
			(char * )0, 0L, (i4 *)0, &local_err, 0);
        	    *err_code = E_DM1366_RFP_CHKPT_LST_READ;
	...
	if (status == E_DB_OK)
	{
		...
		if ((tblcb->tblcb_table_status & 
			...
			== RFP_RECOVER_TABLE)
		{
	    	    *err_code = E_DM1364_RFP_NO_TBL_CHKPT;
	...
	local_status = tbllst_close(&tblctx, err_code);
	if (local_status != E_DB_OK)
	{
	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL,
		(char * )0, 0L, (i4 *)0, &local_err, 0);
	    ule_format(E_DM1367_RFP_CHKPT_LST_CLOSE, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, error_buffer, ER_MAX_LEN, &error_length, &local_err, 1,
		sizeof(DM_FILE), tblctx.filename.name);
	    dmf_put_line(0, error_length, error_buffer);
	    status = local_status;
    ...
    if (DB_FAILURE_MACRO(status))
	*err_code = E_DM1368_RFP_CHKPT_LST_ERROR;
    return(status);
}

/*{
** Name: rfp_error - Error encountered recoverying
**
** Description:
**
**
...
DB_STATUS	    *err_code)
{
    ...
    */
	...
	STRUCT_ASSIGN_MACRO(tblcb->tblcb_rel_ptr->relid, tabname);
	STRUCT_ASSIGN_MACRO(tblcb->tblcb_rel_ptr->relowner, ownname);
    }
	...
	if (!found)
	{
	    loc_stat = rfp_allocate_tblcb (rfp, &tblcb, &local_error);
	    if (loc_stat != E_DB_OK)
	    {
		dmf_write_msg (local_error);
		return (E_DB_ERROR);
	    ...
	    STRUCT_ASSIGN_MACRO(tabname, tblcb->tblcb_rel_ptr->relid);
	    STRUCT_ASSIGN_MACRO(ownname, tblcb->tblcb_rel_ptr->relowner);
    ...
    */
    ...
    "WARNING: Rollforward of %~t will continue, ignoring error for %~t.\n",
	    ...
	    sizeof(DB_TAB_NAME), tblcb->tblcb_rel_ptr->relid.db_tab_name);
...
DB_STATUS	    *err_code)
{
	...
	STRUCT_ASSIGN_MACRO(tblcb->tblcb_rel_ptr->relid, sconcur_name);
	STRUCT_ASSIGN_MACRO(tblcb->tblcb_rel_ptr->relowner, sconcur_own);
    }
    else if (!tblcb && rec)
    {
	/* this must be database recovery - or we would already have tblcb */
...
DB_STATUS	    *err_code )
{
    ...
    status = dm0m_allocate((i4)
		(sizeof( RFP_TBLCB ) + sizeof( DMP_RELATION )),
            ...
            (char *)rfp, (DM_OBJECT **)&tblcb, err_code);
    if (status != E_DB_OK)
    {
	dmf_write_msg (*err_code);
    ...
    tblcb->tblcb_rel_ptr = 
		(DMP_RELATION *)((char *)tblcb + sizeof( RFP_TBLCB ));
...
DB_STATUS	    *err_code)
{
	...
	*/
    ...
    status = rfp_open_database (rfp, jsx, dcb, dbdb_flag, err_code);
    ...
    */
	...
	ule_format( *err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
        dmf_write_msg(E_DM135C_RFP_CANNOT_OPEN_IIRELATION);
	return (status);
    }
	      ...
	      )
	{
		...
		*/
		...
		else if (phase == RFP_UPDATE_INVALID_TBLS_STATUS)
		{
			...
			sizeof(tblcb->tblcb_rel_ptr->relid),
			tblcb->tblcb_rel_ptr->relid.db_tab_name);
		}
		...
		*/
		...
		status = dm2r_replace( iirel_rcb, &tid, DM2R_BYPOSITION,
				     (char *)&rel_record, (char *)0, err_code);
		if ( status != E_DB_OK )
		    break;
	...
	*/
    ...
    status = dm2t_close(iirel_rcb, DM2T_NOPURGE, err_code);
    if ( status != E_DB_OK )
    {
        /*
    ...
    ** Close the database
    */
    status = rfp_close_database (rfp, dcb, dbdb_flag, err_code);
    if (status != E_DB_OK)
    {
	dmf_write_msg (*err_code);
	return (status);
...
DMF_JSX             *jsx )
{
	...
	** then skip it.
	...
	TRformat( dmf_put_line, 0, line_buffer, sizeof(line_buffer),
		       " %~t\n", DB_MAXNAME, &tblcb->tblcb_rel_ptr->relid );
...
DMF_JSX             *jsx )
{
 ...
 "Additional tables required to maintain physical consistency of the database :"
	...
	*/
	if (tblcb->tblcb_table_status & 
		(RFP_SYSTEM_CATALOG | RFP_VIEW_TABLE | RFP_GATEWAY_TABLE))
            continue;
...
DMF_JSX             *jsx )
{
	...
	** If not an invalid table then skip it
	...
	TRformat( dmf_diag_put_line, 0, line_buffer, sizeof(line_buffer),
		       ...
		       DB_MAXNAME, tblcb->tblcb_rel_ptr->relid.db_tab_name,
		       DB_MAXNAME, tblcb->tblcb_rel_ptr->relowner.db_own_name,
		       tblcb->tblcb_table_id.db_tab_base,
		       tblcb->tblcb_table_id.db_tab_index,
...
DB_STATUS           *err_code)
{
	...
	** Database level recovery
	*/
	status = rfp_db_ckp_processing( rfp, jsx, dcb, err_code );
    }
    else
    {
	/*
	...
	*/
	status = rfp_tbl_ckp_processing( rfp, jsx, dcb, err_code );
    }

    if ((status != E_DB_OK) && (*err_code == I_DM1369_RFP_NOT_DEF_TEMPLATE))
    {
	i4	l;
...
i4             *err_code)
{
	...
	status = db_restore_ckp(rfp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
...
DB_STATUS           *err_code)
{
	...
	*/
	lstatus = rfp_tbl_restore_ckp( rfp, jsx, dcb, err_code );
	if ( lstatus != E_DB_OK )
	    break;
...
DB_STATUS           *err_code)
{
	...
	*/
	...
	status = dm2t_unfix_tcb( &tcb, (DM2T_PURGE | DM2T_NORELUPDAT),
		           	 dmf_svcb->svcb_lock_list, log_id, err_code );
	if (status != E_DB_OK)
	{
	    dmf_write_msg (*err_code);
	    return (status);
	...
	** Call the checkpoint starting routine
	*/
        status = dmckp_begin_restore_tbl( d, dcb, err_code );
	if ( status != E_DB_OK )
	    break;
	...
	*/
	...
	for (i = 0; i < ext->ext_count; i++)
	{
	    ...
	    */
	    status = dmckp_begin_restore_tbls_at_loc( d, dcb, err_code );
	    if ( status != E_DB_OK )
                break;
		...
		*/
		...
		for ( j = 0; j <tbio->tbio_loc_count; j++)
		{
		    ...
		    */
		    ...
		    status = dmckp_delete_file( d, err_code );
		    if ( status != E_DB_OK )
	            {
		        /*
			** Check if ok to continue
			*/
			if ( rfp_error( rfp, jsx, tblcb, (DMVE_CB *)0,
			    RFP_FAILED_TABLE, status, err_code ) != E_DB_OK )
			{
			    /*
		    ...
		    */
			...
			if (status != OK)
			{
			    ...
			    {
				if (rfp_error(rfp,jsx,tblcb, (DMVE_CB *)0,
				    RFP_FAILED_TABLE, status, err_code) 
							!= E_DB_OK)
				{
				    status = E_DB_ERROR;
				    break;
		    ...
		    */
		    ...
		    status = dmckp_restore_file( d, dcb, err_code );
	    ...
	    */
	    status = dmckp_end_restore_tbls_at_loc( d, dcb, err_code );
	    if ( status != E_DB_OK )
                break;
	...
	** Call the checkpoint end routine
	*/
        status = dmckp_end_restore_tbl( d, dcb, err_code );
	if ( status != E_DB_OK )
	    break;
...
DB_STATUS           *err_code)
{
    ...
    ** Initialise the DMCKP_CB structure
    */
    status = dmckp_init( &dmckp, DMCKP_RESTORE, err_code );
    if ( status != E_DB_OK )
        return( status );
	...
	for (i = 0; i < ext->ext_count; i++)
	{
	    ...
	    */
	    ...
	    filename.name[7] = (loc_sequence % 10) + '0';
            ...
            status = dmckp_check_snapshot( &dmckp, err_code );
	    if ( status != E_DB_OK )
	    {
		ule_format( E_DM1318_RFP_NO_CKP_FILE, &sys_err,
			    ULE_LOG, NULL, 0, 0, 0, err_code, 3,
		            sizeof(dcb->dcb_name), dcb->dcb_name.db_db_name,
...
DB_STATUS	    *err_code)
{
	...
	*/
	apply = ( jsx->jsx_status1 & JSX1_RECOVER_DB );
	break;
...
DB_STATUS	    *err_code)
{
	...
	*/
	status = rfp_allocate_tblcb (rfp, &tblcb, err_code);
        if ( status != E_DB_OK )
        {
	    dmf_write_msg (*err_code);
	    break;
	...
	status = dm2r_get (iirel_rcb, &tid, DM2R_GETNEXT,
                           (char *)tblcb->tblcb_rel_ptr, err_code);
	...
	tblcb->tblcb_table_name = tblcb->tblcb_rel_ptr->relid;
	tblcb->tblcb_table_owner = tblcb->tblcb_rel_ptr->relowner;
	tblcb->tblcb_table_id = tblcb->tblcb_rel_ptr->reltid;
	tblcb->tblcb_tcb_ptr = dcb->dcb_rel_tcb_ptr;

        /*
        ** Check system catalog/view/gateway first
        */
        if (tblcb->tblcb_rel_ptr->relstat & TCB_CATALOG)
        {
            tblcb->tblcb_table_status |= RFP_SYSTEM_CATALOG;
        }
	if (tblcb->tblcb_rel_ptr->relstat & TCB_VIEW)
	{
	    tblcb->tblcb_table_status |= RFP_VIEW_TABLE;
	}
	if (tblcb->tblcb_rel_ptr->relstat & TCB_GATEWAY)
	{
	    tblcb->tblcb_table_status |= RFP_GATEWAY_TABLE;
	}

	if (tblcb->tblcb_rel_ptr->relstat2 & TCB2_TBL_RECOVERY_DISALLOWED)
	{
    ...
    */
    if ( iirel_rcb )
    {
        status = dm2r_release_rcb( &iirel_rcb, err_code );
        if ( status != E_DB_OK )
        {
	    dmf_write_msg (*err_code);
	    return (status);
    ...
    */
    ...
    if (status != E_DB_OK)
    {
        ...
        if (*err_code == E_DM0100_DB_INCONSISTENT)
        {
            ...
            ** database
            */
            dmf_write_msg(E_DM1357_RFP_INCONSIST_DB_NO_TBLRECV);
            return (E_DB_INFO);
        }
...
DB_STATUS       *err_code)
{
    ...
    if (rfp->open_db & DB_OPENED)
    {
        status = dm2d_close_db (dcb, dmf_svcb->svcb_lock_list, flags, err_code);
...

/*{
** Name: rfp_get_index_info - Determine sec indexes for tables being recovered
**
** Description:
...
DB_STATUS           *err_code)
{
	...
	*/
	      ...
	      )
	{
	    ...
	    if ( (tblcb->tblcb_table_id.db_tab_base ==
		  tblcb2->tblcb_table_id.db_tab_base) &&
		 (tblcb2->tblcb_table_id.db_tab_index > 0) )
	    {
		tblcb2->tblcb_table_status |= RFP_INDEX_REQUIRED;

		/* Unless -nosecondary_index specified, recover the index */
...
DB_STATUS           *err_code)
{
	...
	if (status != E_DB_OK)
	{
	    ...
	    ** the database.
	    */
	    if (*err_code == E_DM0054_NONEXISTENT_TABLE)
	    {
		status = E_DB_OK;
	    }
	    else
    ...
    */
    if (etab_rcb)
    {
	local_status = dm2t_close(etab_rcb, 0, &local_error);
	if (local_status != E_DB_OK)
	{
	    dmf_write_msg(local_error);
    ...
    ** function, if so close db
    */
    local_status = rfp_close_database (rfp, dcb, dbdb_flag, &local_error);
    ...
    DB_STATUS	*err_code )
{
    ...
    for (;;)
    {
	    ...
	    if ( (i4)jsx->jsx_device_list[i].dev_pid > 0 )
			...
			ule_format( retval, &err_desc, ULE_LOG, NULL, 0,
				    0, 0, &errcode, 0 );
			MEfree(command);
...
char		*buffer)
{
    ...
    ule_format(E_DM1371_RFP_INFO, (CL_SYS_ERR *)0, ULE_LOG,
	NULL, (char *)0, (i4)0, (i4 *)0, &error, 1, length, buffer);
...
DB_STATUS       *err_code)
{
	...
	if (header->type == DM0LBSF)
	{
	    ...
	    status = create_om_context(rfp, dmve, log_rec, err_code);
	    break;
	}
	...
	if (header->type == DM0LRNLLSN)
	{
		...
		status = dm0m_allocate(sizeof(DMP_MISC) +
				...
				(char *)0, (DM_OBJECT **)&rnl->rnl_xlsn,
				err_code);
		if (status != E_DB_OK)
		{
		    DB_STATUS	local_err_code;

		    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 
				0, &local_err_code, 0);
		    *err_code = E_DM1304_RFP_MEM_ALLOC;
		    break;
		}
...
pg %d now at lsn=(%d,%d)\n",
	...
	status = dm2u_load_table((DM2U_MXCB *)octx->octx_mxcb, err_code);
	if ((status == E_DB_INFO) && 
	    (*err_code == E_DM9CB1_RNL_LSN_MISMATCH))
	{
	    ...
	    local_status = rfp_add_lsn_waiter(rfp, octx, &local_err);
	    if (local_status)
		status = local_status;
	    else
...
i4		*err_code)
{
    ...
    {
	status = dm2t_close(osrc->osrc_rnl_rcb, DM2T_PURGE, err_code);
	if (status)
	{
	    dmf_write_msg(*err_code);
	    break;
    ...
    if (master_rcb)
    {
	status = dm2t_close(master_rcb, DM2T_PURGE, err_code);
	if (status)
	    dmf_write_msg(*err_code);
    }
...
DB_STATUS           *err_code)
{
    ...
    last_jnl = rfp->rfp_jnl_history[rfp->jnl_last].ckp_l_jnl;
    incr_jnl = last_jnl + 1;
    ...
    */
    ...
    /* Only restore open xacts for FIRST new journal */
    if (last_jnl && *new_jnl_cnt == 0)
    {
	status = restore_open_xacts(jsx, rfp, dcb, dmve, last_jnl, err_code);
    } 
	...
	*new_jnl_cnt += 1;
	...
	if (incr_jnl)
	{
	    ...
	    if (jsx->jsx_status & JSX_TRACE)
		TRdisplay("%@ RFP: Add incremental jnl %d to jnl history.\n",
			incr_jnl);
	    status = config_handler(OPEN_CONFIG, dcb, &cnf, &local_error);
	    if (status == E_DB_OK)
	    {
	    ...
	    status = save_open_xacts(jsx, rfp, dcb, dmve, incr_jnl, err_code);
	}
...
DB_STATUS	*err_code)
{
    ...
    if (jsx->jsx_status & JSX_TRACE)
	TRdisplay("%@ Saving incremental transaction context.\n");
	...
	status = dm0j_open(0,
		    ...
		    (DB_TAB_ID *)0, &btjnl, err_code);

	if (status)
	    break;

	if (jsx->jsx_status & JSX_TRACE)
	    TRdisplay("%@ Open jnl %d dm0j_sequence %d dm0j_jnl_seq %d\n",
	    last_jnl,  btjnl->dm0j_sequence, btjnl->dm0j_jnl_seq); 
	    sequence = btjnl->dm0j_sequence;

	/* Read the LAST record from this journal file. */
	status = dm0j_read(btjnl, (PTR *)&record, &l_record, err_code);
	if (status != E_DB_OK)
	{
	    if (*err_code == E_DM0055_NONEXT)
	    {
		/* empty jnl file is ok */
		l_record = 0;
		status = E_DB_OK;
	    }
	    else
	...
	{
	    STRUCT_ASSIGN_MACRO(*record, last_hdr);
	}

	status = dm0j_close(&btjnl, err_code);
	if (status)
	    break;

	/* xact context may have already been written to this jnl */
	if (l_record && last_hdr.type == DM0LBTINFO)
	{
	    if (jsx->jsx_status & JSX_TRACE)
		TRdisplay("%@ Incremental transaction context already saved %d\n",
		last_hdr.type);
	    return (E_DB_OK);
	}
	...
	status = dm0j_open(0,
		    ...
		    dmve->dmve_dcb_ptr->dcb_jnl->phys_length,
		    &dmve->dmve_dcb_ptr->dcb_name,
		    rfp->jnl_block_size, last_jnl, 0, sequence, DM0J_M_WRITE,
		    -1, DM0J_FORWARD, (DB_TAB_ID *)0, &btjnl, err_code);
	...
	if (rfp->rfp_tx)
	{
	    status = dm0j_update(btjnl, &btjnl_sequence, err_code);
	    if (status)
	    {
		TRformat(dmf_put_line, 0, line_buffer, sizeof(line_buffer),
    ...
    dmf_write_msg(E_DM1374_RFP_INCR_TXN_SAVE);

    if (btjnl)
	status = dm0j_close(&btjnl, &local_error);

    *err_code = E_DM1376_RFP_INCREMENTAL_FAIL;
...
DB_STATUS	*err_code)
{
	...
	for ( ; status == E_DB_OK; )
	{
	    ...
	    if (status != E_DB_OK)
	    {
		if (*err_code == E_DM0055_NONEXT && need_raw_cnt == 0)
		{
		    status = E_DB_OK;
		    break;
		}
		if (need_raw_cnt)
		    *err_code = E_DM1323_RFP_BAD_RAWDATA;
		break;
	    }
		...
		status = add_tx(rfp, &bti->bti_bt, err_code); 
		tx_cnt++;
		...
		status = reverse_gather_rawdata(tx, rawdata, (i4 *) err_code);
    ...
    dmf_write_msg(E_DM1375_RFP_INCR_TXN_RESTORE);
...
static DB_STATUS
reverse_gather_rawdata(RFP_TX *tx, DM0L_RAWDATA *record, i4 *err_code)
{
