...
DB_STATUS	    *err_code)
{
    ...
    status = prepare(&dtp, jsx, dcb, err_code);
    if (status != E_DB_OK)
    {
	return (status);
    }
    ...
    if (status != E_DB_OK)
    {
	*err_code = E_DM1331_DTP_CNF_OPEN;
	return (status);
    }
    ...
    if (status != E_DB_OK)
    {
	*err_code = E_DM1332_DTP_CNF_CLOSE;
	return (status);
    }
	...
	status = dm0d_open(DM0D_MERGE, (char *)&dcb->dcb_dmp->physical, 
	    ...
	    -1, DM0D_BACKWARD, &dmp, err_code);
	if (status != E_DB_OK)
	{
	    *err_code = E_DM132A_DTP_DMP_OPEN;
	    break;
	}
	...
	for (;;)
	{
	    ...
	    status = dm0d_read(dmp, &record, &l_record, err_code);
	    if (status != E_DB_OK)
	    {
		...
		*err_code = E_DM132B_DTP_DMP_READ;
		break;
	    }
    ...
    */
	...
	(VOID) dm2d_close_db(dcb, dmf_svcb->svcb_lock_list, 0, &local_err_code);
...
DB_STATUS	    *err_code)
{
	...
	if (*err_code == E_DM0053_NONEXISTENT_DB)
	    *err_code = E_DM132F_DTP_NON_DB;
	else if (*err_code == E_DM004C_LOCK_RESOURCE_BUSY)
	    *err_code = E_DM1336_DTP_DB_BUSY;
	else
	    *err_code = E_DM1330_DTP_OPEN_DB;
	return (E_DB_ERROR);
    }
...
/*
	...
	{
	    status = E_DB_ERROR;
	    *err_code = E_DM1328_DTP_DB_NOT_DMP;
	    break;
	}
*/
	...
	*err_code = E_DB_OK;
    ...
    if (*err_code == E_DM1328_DTP_DB_NOT_DMP)
	return(E_DB_INFO);
    return (E_DB_ERROR);
}
