...
# endif
    ...
    */
    ...
    if ( ret_status != OK)
    {
	ule_format(E_UL0016_CHAR_INIT, (CL_ERR_DESC *)&cl_err, ULE_LOG ,
    ...
    */
	...
	break;
    case PM_FILE_BAD:
	err_code=E_DM1039_JSP_BAD_PM_FILE;
	break;
    default:
	err_code=E_DM103A_JSP_NO_PM_FILE;
	break;
    }
    ...
    */ 
    ...
    status = parse_command_line(argc, argv, &jsx, &err_code);
    if (status != E_DB_OK)
    {
	if (jsx.jsx_status2 & JSX2_MUST_DISCONNECT_BM)
	    (void) dm0p_deallocate(&toss_err);
	...
	*/
	...
	status = search_dcb(&jsx, &err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	{
	    err_code = E_DM100E_JSP_NO_SUPERUSER;
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	...
	{
	    err_code = E_DM1011_JSP_DB_NOTFOUND;
	    break;
	...
	while (jsx.jsx_next != (DMP_DCB *)&jsx.jsx_next)
	{
		...
		status = dmfcpp(&jsx, dcb, &err_code);
		break;

	    case JSX_O_ATP:
		/*  Generate an audit trail. */

		status = dmfatp(&jsx, dcb, &err_code);
		break;

	    case JSX_O_DTP:
		/*  Generate an dump trail. */

		status = dmfdtp(&jsx, dcb, &err_code);
		break;

	    case JSX_O_RFP:
		/*  Roll forward a database. */

		status = dmfrfp(&jsx, dcb, &err_code);
		break;

	    case JSX_O_INF:
		/*  Display database information. */

		status = dmfinfo(&jsx, dcb, &err_code);
		break;
#ifdef UNIX
       	   case JSX_O_IN1:
        	/*  Display database information. DBALERT*/
 
        	status = dmfinfo1(&jsx, dcb, &err_code);
        	break;
#endif 
	    case JSX_O_ALTER:
		...
		status = dmfalter(&jsx, dcb, &err_code);
		break;
		...
		status = dmfreloc(&jsx, dcb, &err_code);
		break;

            case JSX_O_FLOAD:
                status = dmffload(&jsx, dcb, &err_code,
	...
	if (status == E_DB_OK)
	{
	    ...
	    _VOID_ dmf_sxf_terminate(&err_code);
	    PCexit(OK);
	}
	break;
    }
    ...
    {
	dmf_write_msg_with_args(err_code, 1, 0, dbname );
    }
    else if (err_code!=0)
    {
	dmf_write_msg(err_code);
...
DB_STATUS	    *err_code)
{
    ...
    *err_code = E_DB_OK;
    ...
    {
	jsx->jsx_operation = -1;
	*err_code = E_DM1002_JSP_MISS_ARGS;
	return (E_DB_ERROR);
    }
	...
	** so jsx_tbl_cnt should be 0 by default.
    ...
    {
	jsx->jsx_operation = -1;
	*err_code = E_DM1003_JSP_UNRECOGNIZED;
	return (E_DB_ERROR);
    }
    jsx->jsx_db_cnt = 0;
	...
	if (argv[i][0] == '-')
	{
		...
		*/
		else if (operator == JSX_O_RFP || operator == JSX_O_RELOC)
		{
			...
			if (status != E_DB_OK)
			     ...
			     *err_code = E_DM105C_JSP_INV_ARG_VAL;
		    else
		...
		*/
		...
		if (operator == JSX_O_RFP)
		{
		    if (cused == 'Y')
		    {
			*err_code = E_DM1005_JSP_BAD_ARG_COMBO;
			break;
		    }
		    else
			cused = 'Y';
                        ...
                        while (*dest++ = *src++)
                        {
                            if (*src == ',')
                            {
                                ...
                                if (dest >= (PTR)&jsx->jsx_fname_list[j])
                                {
                                    ...
                                    ** File name too long.
                                    */
                                    *err_code = E_DM104F_JSP_NAME_LEN;
                                    break;
                                }
...
# endif
                  ...
                  else 
                  {
                       if (argv[i][2] !=EOS)
                       {
                        if (cui_chk3_locname(&argv[i][2]) != OK)
                         {
                            *err_code = E_DM1406_ALT_INV_UCOLLATION;
                         }
                       else
                         {
		...
		if ((operator == JSX_O_CPP) || (operator == JSX_O_RFP))
                {
                    ...
                    */
                    status = parse_device_list(jsx, &argv[i][2], err_code);
                    if (status != E_DB_OK)
		    {
                        *err_code = E_DM105A_JSP_INC_ARG;
                        break;
		...
		*/
		if (operator == JSX_O_RFP)
		{
		    ...
		    {
			/* nothing really to set - this is the default anyway */
			jsx->jsx_status &=  ~JSX_NORFP_UNDO;
		    }
		    else
			*err_code = E_DM1004_JSP_BAD_ARGUMENT;
...
/* ---> Removed until implemented
 ...
 <---- */
		...
		else if (STcmp(&argv[i][1], "s" ) == 0)
		{
		    ...
		    */
		    dmf_write_msg(W_DM1038_JSP_SUPU_IGNORED);
		}
	    ...
	    case 't':
                if (operator == JSX_O_FLOAD)
                {
                    if (MEcmp(&argv[i][1], "table=", 6) == 0)
                    {
                        ...
                        */
                        status = parse_table_list(jsx, &argv[i][7], err_code);
                        if (status != E_DB_OK)
                            break;
                    }
	...
	else if (argv[i][0] == '#')
	{
		...
		*/
		if ((operator == JSX_O_RFP) || (operator == JSX_O_ATP))
		{
			...
			*/
			dmf_write_msg(E_DM1219_ATP_INV_CKP);
			*err_code = E_DM105C_JSP_INV_ARG_VAL;
		    }
		    else
		    {
	...
	*/
	...
	if (*err_code != E_DB_OK)
	{
	    ...
	    ** error codes.
	    */
	    if (*err_code == E_DM1004_JSP_BAD_ARGUMENT ||
		*err_code == E_DM1005_JSP_BAD_ARG_COMBO ||
		*err_code == E_DM105C_JSP_INV_ARG_VAL ||
		*err_code == E_DM105A_JSP_INC_ARG)
	    {
		...
		else if (*err_code == E_DM1005_JSP_BAD_ARG_COMBO)
		    new_err = E_DM103C_JSP_INCON_ARG;
		else
		    new_err = *err_code;
		

		ule_format(new_err, (CL_ERR_DESC *) NULL,
			ULE_LOOKUP, (DB_SQLSTATE *) NULL, error_buffer,
			ER_MAX_LEN, &error_length, err_code, 2,
    ...
    */
    ...
    {
	if (operator == JSX_O_ALTER)
	    *err_code = E_DM104B_JSP_ALTDB_ONE_DB;
	else if (operator == JSX_O_ATP)
	    *err_code = E_DM104C_JSP_ATP_ONE_DB;
	else if (operator == JSX_O_CPP)
	    *err_code = E_DM1050_JSP_CPP_ONE_DB;
	else if (operator == JSX_O_RELOC)
	    *err_code = E_DM1056_JSP_RELOC_ONE_DB;
	else if (operator == JSX_O_FLOAD)
	    *err_code = E_DM1057_JSP_FLOAD_ONE_DB;
	else
	    *err_code = E_DM104D_JSP_RFP_ONE_DB;

	dmf_write_msg(*err_code);
	*err_code = E_DM1001_JSP_INIT;
	return (E_DB_ERROR);
    }
	...
	*/
    ...
    status = dmf_init(
		...
		0, 0, 32, err_code, jsx->jsx_lgk_info);

    if (status != E_DB_OK)
    {
	if (*err_code == E_DM101A_JSP_CANT_CONNECT_BM)
	    dmf_write_msg_with_args(*err_code, 1,
			STlength(cache_name), cache_name);
	else
	{
	    dmf_write_msg(*err_code);
	    dmf_write_msg(E_DM1051_JSP_NO_INSTALL);
	}
    ...
    */
	...
	for (i = 0; i < jsx->jsx_db_cnt; i++)
	{
	    ...
	    status = allocate_dcb(&dbname, &dcb, err_code);
	    if (status != E_DB_OK)
		break;
    ...
    {
	*err_code = E_DM1006_JSP_TAPE_ONE_DB;
	return (E_DB_ERROR);
    }
	...
	*/
	...
	{
	    dmf_write_msg(E_DM1351_RFP_BTIME_NOFORCE);
	    *err_code = E_DM1004_JSP_BAD_ARGUMENT;
	    return (E_DB_ERROR);
	}
	...
	*/
	if ( (jsx->jsx_status1 & JSX_START_LSN) &&
	     (jsx->jsx_status1 & JSX_FORCE) == 0 )
	{
	    dmf_write_msg(E_DM1353_RFP_SLSN_NOFORCE);
	    *err_code = E_DM1004_JSP_BAD_ARGUMENT;
	    return (E_DB_ERROR);
	}
	...
	*/
	...
	{
	    dmf_write_msg(E_DM1045_JSP_FILE_REQ_TBL);
	    *err_code = E_DM1005_JSP_BAD_ARG_COMBO;
	    return(E_DB_ERROR);
	}
	...
	*/
	...
	{
	    dmf_write_msg(E_DM1044_JSP_FILE_TBL_EQ);
	    *err_code = E_DM1005_JSP_BAD_ARG_COMBO;
	    return(E_DB_ERROR);
	}
    }
    ...
    */
	...
	{
	    dmf_write_msg(E_DM1054_JSP_SLSN_GT_ELSN);
	    *err_code = E_DM1004_JSP_BAD_ARGUMENT;
	    return(E_DB_ERROR);
	}
...
DB_STATUS	    *err_code)
{
	...
	*err_code = E_DM1007_JSP_MEM_ALLOC;
	return (status);
    }
...
DB_STATUS	    *err_code)
{
	...
	*err_code = E_DM1007_JSP_MEM_ALLOC;
    ...
    {
	*err_code = E_DM1017_JSP_II_DATABASE;
	return (E_DB_ERROR);
    }
	...
	** Ok that db unopened.
 	*/
 	status = jsp_get_case((DMP_DCB *)0, jsx, err_code);
 	if (status != E_DB_OK)
	    return (E_DB_ERROR);
	...
	*/
	...
	{
	    *err_code = E_DM1024_JSP_BAD_LOC_INFO;
	    return (E_DB_ERROR);
	}
	/*
	** Start an SXF audit session.
	*/
	...
	{
	    *err_code = E_DM1031_JSP_SXF_SES_INIT;
	    return (E_DB_ERROR);
	}
	/*
	** Get terminal name
	...
	*/
	if ((jsx->jsx_status2 & JSX2_SERVER_CONTROL) == 0)
	{
		...
		local_status = dma_write_audit( SXF_E_USER,
		    ...
		    &local_error, NULL);
	    *err_code = E_DM1037_JSP_NO_PRIV_UNAME;
	    return (E_DB_ERROR);
	}
	jsx->jsx_status2 |= JSX2_SERVER_CONTROL;
    ...
    */
    ...
    status = dm2d_open_db(dcb, DM2D_A_READ, DM2D_IS,
                          dmf_svcb->svcb_lock_list, dbdb_flag, err_code);
    if (status == E_DB_OK)
    {
 	/*
	...
	*/
	...
	error = E_DM100A_JSP_OPEN_IIUSER;
	...
	*/
	...
	if (status == E_DB_OK)
	{
	    error = E_DM100B_JSP_NOT_USER;
	    status = dm2r_get(rcb, &tid, DM2R_GETNEXT, (char *)&user, err_code);
	    if (status == E_DB_OK)
	    {
		/*
		...
		*/
		...
		{
		    error = E_DM1037_JSP_NO_PRIV_UNAME;
		    status = E_DB_ERROR;
		    break;
		}
		...
		if (user.du_status & DU_USECURITY)
		{
	        ...
	        */
	        ...
	        if (status)
	        {
		    error=sxf_rcb.sxf_error.err_code;
	    	    break;
	        }
	    }
	}
	if (status == E_DB_OK && (jsx->jsx_status & JSX_UNAME))
	{
	    ...
	    */
	    error = E_DM100C_JSP_NOT_OVERUSER;
    ...
    */
    if (error == E_DM100B_JSP_NOT_USER && status != E_DB_OK)
	access |= SXF_A_FAIL;
    else
	...
	{
	    error = E_DM100D_JSP_CLOSE_IIUSER;
	    status = close_status;
	}
	rcb = 0;
    }
    ...
    */
	...
	if (local_status == E_DB_OK)
	{
	    do
	    {
			...
			sxf_rcb.sxf_auditevent = secstate.du_id;
	    	        ...
	    	        {
		            ule_format(sxf_rcb.sxf_error.err_code, 0,
				ULE_LOG, NULL, 0, 0, 0, &error, 0);
	    	            local_error = E_DM1036_JSP_ALTER_SXF;;
	...
	if (close_status != E_DB_OK && status == E_DB_OK)
	{
	    error = E_DM1035_JSP_CLOSE_SECSTATE;
	    status = close_status;
	}
	rcb = 0;
    }
	...
	*/
	...
	if (jsx->jsx_db_cnt == 0)
	{
		...
		for(;;)
		{
		    ...
		    */
		    ...
		    status = allocate_dcb(
			(DB_DB_NAME *)database.du_dbname, &ndcb, err_code);
		    if (status != E_DB_OK)
			break;
	...
	else
	{
		...
		error = E_DM1011_JSP_DB_NOTFOUND;
		...
		if (status == E_DB_OK)
		{
		    ...
		    **  A user with OPERATOR or SECURITY privilege can
			...
			if ( MEcmp((char *)&database.du_own,
			      ...
			      sizeof(DB_OWN_NAME)))
			{
			    error = E_DM1014_JSP_NOT_DB_OWNER;
			    /*
			    ...
			    */
				...
				local_status = audit_jsp_action(jsx->jsx_operation,
				    ...
				    &database.du_own, TRUE, &local_error);
			    status = E_DB_ERROR;
			    break;
			}
		    ...
		    */
		    if (database.du_dbservice & DU_1SER_DDB)
		    {
			error = E_DM1022_JSP_NO_DISTRIB_DB;
			status = E_DB_ERROR;
			break;
		    }
    ...
    */
	...
	error = E_DM1018_JSP_II_LOCATION;
...
*/
		...
		if (status == E_DB_OK)
		{
		    if (build_location(ndcb, location.du_l_area,
				    ...
				    &ndcb->dcb_location) != E_DB_OK)
		    {
			error = E_DM1024_JSP_BAD_LOC_INFO;
			status = E_DB_ERROR;
		    }
		}
		else
		{
		    error = E_DM1023_JSP_NO_LOC_INFO;
		}
	    }
	    else
	    {
...
/* ---> Removed until implemented
 ...
 <--- */
	...
	SIprintf(
	    "\t[-target_jnl_blocks=nnn] [-delete_invalid_ckp] [-n]\n");
...
# endif /* PMODE_DOCUMENTED */
    ...
    case JSX_O_FLOAD:
        SIprintf(
            "usage: fastload dbname -file=<filename> -table=<tablename>\n");
        break;
...
i4		*err_code)
{
    ...
    *trans_len = DB_MAXNAME;
	...
	CUI_ID_DLM_M, &ret_mode, &err_blk);
    if (DB_FAILURE_MACRO(status))
    {
    	*err_code = E_DM104E_JSP_NORMALIZE_ERR;
	return(E_DB_ERROR);
    }
...
i4		*err_code)
{
	...
	*/
	if (*src == '\"')
	{
	    ...
	    */
	    status = cui_idlen(src, ',', &tbl_name_len, &err_blk);
	    if (status != E_DB_OK)
	    {
		*err_code = E_DM104E_JSP_NORMALIZE_ERR;   
		break;
            ...
            */
            status = cui_idlen(src, '.', &own_name_len, &err_blk);
            if (status != E_DB_OK)
            {
                *err_code = E_DM104E_JSP_NORMALIZE_ERR;
                break;
    ...
    ** to a bad argument.
    */
    ule_format(*err_code, (CL_ERR_DESC *) NULL, ULE_LOOKUP, 
...
i4		*err_code)
{
	...
	*/
	...
	{
	    status = E_DB_ERROR;
	    *err_code = E_DM105B_JSP_NUM_LOCATIONS;
	    break;
	}
...
jsp_log_info(DMF_JSX *jsx, i4 argc, char *argv[])
{
	...
	jsx->jsx_operation == JSX_O_RELOC || jsx->jsx_operation == JSX_O_ALTER)
	    ule_format(E_DM105D_JSP_INFO, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &error, 1, STlength(buf), buf);
