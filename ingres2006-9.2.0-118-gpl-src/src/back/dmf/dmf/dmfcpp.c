...
static DB_STATUS cpp_ckpt_core_tbl(
    DMF_CPP         *cpp,
    DMF_JSX         *jsx,
    DMP_DCB         *dcb);
...
static DB_STATUS cpp_get_index_info(
    DMF_CPP             *cpp,
    DMF_JSX             *jsx,
    DMP_DCB             *dcb,
    DB_STATUS           *err_code);
...
i4	    *err_code)
{
    ...
    */
	...
	dmf_svcb->svcb_lock_list, open_flags, err_code);
	...
	*err_code = E_DM1108_CPP_OPEN_DB;
	...
	if (jsx->jsx_status & JSX_CKPXL)
	{
	    status = dm2d_open_db(dcb, DM2D_A_READ, DM2D_N,
			dmf_svcb->svcb_lock_list, DM2D_NLK, err_code);
	    if (status != E_DB_OK)
		break;
	    jsx->jsx_log_id = dcb->dcb_log_id;
	}
	...
	if (!(jsx->jsx_status & JSX_CKPXL))
	{
		...
		*err_code = E_DM116C_CPP_NO_ONLINE_CKP;
		break;
	    }
	}
	...
	status = cpp_prepare_tbl_ckpt(&cpp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	if ((jsx->jsx_status1 & JSX1_CKPT_DB) == 0)
	{
	    ...
	    */
	    status = cpp_prepare_save_tbl(&cpp, jsx, dcb, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	status = config_handler(OPEN_CONFIG, dcb, &cnf, jsx, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	{
	    *err_code = E_DM1157_CPP_DISABLE_JNL;
	    break;
	}
	...
	{  
            status = config_handler(CLOSE_CONFIG, dcb, &cnf, jsx, err_code);
            if (status != E_DB_OK)
                break;   
	...
	*/
	...
	{    
	    ...
	    */
	    ...
	    {
		*err_code = E_DM1156_CPP_NOLOGGING;
		break;
	    }

	    if ((cnf->cnf_dsc->dsc_dbaccess & DU_OPERATIVE) == 0)
	    {
		*err_code = E_DM000F_BAD_DB_ACCESS_MODE;
		break;
	    }
		...
		status = online_cxmsg_send(CKP_CSP_INIT, jsx, dcb, err_code);
		if (status != E_DB_OK)
		    break;

		/* CXmsg_send, and wait for all nodes to finish */
		status = online_cxmsg_send(CKP_CSP_STALL, jsx, dcb, err_code);
	    }
	    else
	    {
		status = online_setup(jsx, &ckp_lock_list, err_code);
	    }
	    ...
	    status = lk_ckp_db(jsx, dcb, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	    ...
	    (cnf->cnf_dsc->dsc_status & (DSC_JOURNAL | DSC_DISABLE_JOURNAL)))
	{
	    ...
	    status = cpp_wait_for_purge(jsx, dcb, err_code);
	    if  (status != E_DB_OK)
		    break;
	    ...
	    */
	    ...
	    if (status == E_DB_OK)
		while ( (status = dm0l_drain(dcb, err_code)) == E_DB_INFO && (retries) )
		{
		   /* test then re-initialize the retries counter if MAX_DELAY is reached */
		   if ((retries++ >= CPP_MSG_MAX_DELAY) && (retries = 1) )
			 ...
			 TRdisplay("%@ CPP: Recovery Server has not yet closed database: %~t complete.\n",
			  ...
			  {
				/* RCP Process is dead!!!! */
				*err_code=E_DM1170_NO_RECOVERY_SERVER ;
				break;
			   }
		      }
		      else
	...
	if (jsx->jsx_status & JSX_DRAINDB)
	{
		...
		dcb, dmf_svcb->svcb_lock_list, open_flags, err_code);
	...
	status = config_handler(CLOSEKEEP_CONFIG, dcb, &cnf, jsx, err_code);
	if (status != E_DB_OK)
	    break;
	...
	status = create_ckpjnldmp(
	    jsx, dcb, save_ckp_seq, journal_flag, dump_flag, err_code);
	if (status != E_DB_OK)
	    break;

	/* Now deallocate the extent info. */

	status = config_handler(UNKEEP_CONFIG, dcb, &cnf, jsx, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	    ...
	    (CXcluster_enabled()))
	{
	    ...
	    */
            status = cluster_merge(jsx, dcb, err_code);
            if (status != E_DB_OK)
                break;
	...
	*/
	...
	status = upd1_jnl_dmp(jsx, dcb, &cnf, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	status = config_handler(DUMP_CONFIG, dcb, &cnf, jsx, err_code);
	if (status != E_DB_OK)
	{
	    dmf_write_msg(*err_code);
	    *err_code = E_DM110B_CPP_FAILED;
	    break;
	}
	...
	*/
	...
	if ((jsx->jsx_status & JSX_CKPXL) == 0)
	{
	    ...
	    {
	        /* CXmsg_send, and wait for all nodes to finish */
	        status = online_cxmsg_send(CKP_CSP_SBACKUP, jsx, dcb, err_code);
	    }
	    else
	    {
		status = online_write_sbackup(jsx, err_code);
	    }
	    if	(status != E_DB_OK)
		    break;
            /* liibi01 Bug 59771 
	    ...
	    if (cluster_locked)
		status = online_cxmsg_send(CKP_CSP_RESUME, jsx, dcb, err_code);
	    else
		status = online_resume(jsx, &ckp_lock_list, err_code);
	    ...
	    */
	    if (cluster_locked)
	        status = online_cxmsg_send(CKP_CSP_CP, jsx, dcb, err_code);
	    else
		status = online_signal_cp(jsx, err_code);
	...
	*/
	...
	status = check_point(&cpp, jsx, dcb, &cnf, sequence, err_code);    
	if (status != E_DB_OK)
	    break;
	...
	*/
	if ((jsx->jsx_status & JSX_CKPXL) == 0)
	{
	    ...
	    {
	        /* CXmsg_send, and wait for all nodes to finish */
	        status = online_cxmsg_send(CKP_CSP_EBACKUP, jsx, dcb, err_code);
	    }
	    else
	    {
		status = online_write_ebackup(jsx, err_code);
		if (status != E_DB_OK)
		    break;

		status = online_wait_for_purge(jsx, err_code);
	    }
	...
	if (cluster_locked && (jsx->jsx_status & JSX_CKPXL) == 0)
	{
	    ...
	    status = dm0d_merge(cnf, (DMP_DCB *)0, (PTR)0,
	...
	status = upd2_jnl_dmp(jsx, dcb, &cnf, &sbackup_time, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	status = update_catalogs(jsx, dcb, &cnf, &abort_xact_lgid, err_code);
	if (status != E_DB_OK)
	   break;
	...
	*/
	...
	{
	    status = truncate_jnl_dmp(jsx, dcb, &cnf, err_code);
	    if (status != E_DB_OK)
		break;
	}
	else if (jsx->jsx_status & JSX_DESTROY)
	{
	    ...
	    */
	    status = jsp_file_maint(jsx, dcb, &cnf, 0, err_code, TRUE);
	    if (status != E_DB_OK)
		break;

	    status = config_handler(UPDATE_CONFIG, dcb, &cnf, jsx, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	...
	if ((jsx->jsx_status & JSX_CKPXL) == 0)
	{
	    ...
	    {
	        /* CXmsg_send, and wait for all nodes to finish */
	        status = online_cxmsg_send(CKP_CSP_DBACKUP, jsx, dcb, err_code);
                if (status == E_DB_OK)
                    status = online_cxmsg_send(CKP_CSP_STATUS, jsx, dcb, err_code);
	    }
	    else
	    {
		status = online_dbackup(jsx, err_code);
	    }
	    if	(status != E_DB_OK)
		break;

	    /* Release DBU operation lock */
	    status = online_finish(jsx, dcb, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	if (cnf)
	{
	    ...
	    config_handler(CLOSE_CONFIG, dcb, &cnf, jsx, &local_err_code);
	}
	...
	if (dbu_locked)
	{
		...
		*/
		...
		while (1)
		{
		    ...
		    {
			if (locstat)
			    ule_format(locstat, &locsyserr, ULE_LOG, NULL,
				(char *)0, 0L, (i4 *)0, &local_err_code, 0);
			break;
		    }
...
*/
	...
	(VOID)dm2d_close_db(
	    dcb, dmf_svcb->svcb_lock_list, open_flags, &local_err_code);
...
i4	    *err_code)
{
	...
	*/
	status = online_display_stall_message( jsx, err_code );
	if ( status != E_DB_OK )
	    break;
	...
	if (cl_status != OK)
	{
		...
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM1125_CPP_LGBGN_ERROR;
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	    ...
	    && (jsx->jsx_status1 & JSX1_STALL_TIME))
	{
	    ...
	    if (cl_status)
	    {
		ule_format(E_DM901A_BAD_LOCK_CREATE, &sys_err,
		    ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
		    err_code, 0);
		*err_code = E_DM1131_CPP_LK_FAIL;
		status = E_DB_ERROR;
		break;
	    }
	} else
	...
	*/
	    ...
	    (event & (LG_E_START_ARCHIVER | LG_E_BACKUP_FAILURE)))
	{
		...
		ule_format(E_DM900F_BAD_LOG_EVENT, &sys_err, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, err_code,
		    1, 0, event_mask);
		*err_code = E_DM1150_CPP_BACKUP_FAILURE;
	    }
		...
		*err_code = E_DM1145_CPP_DEAD_ARCHIVER;
	    ...
	    {
		ule_format(E_DM1149_CPP_BACKUP_FAILURE, &sys_err, ULE_LOG, NULL,
	...
	if (cl_status != OK)
	{
		...
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM1127_CPP_LGEND_ERROR;
	    status = E_DB_ERROR;
	    break;
	}
...
i4	    *err_code)
{
	...
	if (cl_status != OK)
	{
	    ...
	    ule_format(E_DM900C_BAD_LOG_BEGIN, &sys_err, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM1151_CPP_CPNEEDED;
	    break;
	}
...
i4	    *err_code)
{
	...
	*/
	...
	if (cl_status != E_DB_OK)
	{
		...
		(char *)0, (i4)0, (i4 *)0, err_code, 1,
		0, LG_S_DATABASE);
	    *err_code = E_DM1152_CPP_ONLINE_PURGE;
	    break;
	}
...
i4	    *err_code)
{
    ...
    {
	{
		...
		TRformat( dmf_put_line, 0, line_buffer, sizeof(line_buffer),
  	    ...
  	    lcl_err_code = E_DM116F_CPP_DRN_PURGE;
	}
...
i4	    *err_code)
{
	...
	if (status != E_DB_OK)
	{
	    *err_code = E_DM1126_CPP_DM0LSBACKUP;
	    break;
	}
	...
	{
	    TRdisplay(
	       "%@ CPP: Wrote Start Backup Log Record at Log Seq Num <%d,%d>\n",
...
i4	    *err_code)
{
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    *err_code = E_DM1129_CPP_DM0LEBACKUP;
	    break;
	}
	...
	{
	    TRdisplay(
		"%@ CPP: Wrote End Backup Log Record at Log Seq Num <%d,%d>\n",
		lsn.lsn_high, lsn.lsn_low);
	}
...
i4	    *err_code)
{
	...
	*/
...

	{
	    ...
	    status = ule_format(status, &sys_err, ULE_LOOKUP, NULL, message,
	    ...
	    status = ule_format(E_DM1114_CPP_LOCATION, 0, ULE_LOOKUP, NULL, message,
		sizeof(message), &l_message, &local_err_code, 2,
	...
	*err_code = E_DM1100_CPP_BAD_LOCATION;
	return (E_DB_ERROR);
    }
	...
	ule_format(E_DM9002_BAD_FILE_CREATE, &sys_err, 
    ...
    *config = (DM0C_CNF *)0;
	...
	if (*err_code == I_DM116D_CPP_NOT_DEF_TEMPLATE)
	{
	    i4	l; 
	    ...
	    l, tbuf, defowner);
	    *err_code = E_DM1121_CPP_BEGIN_ERROR;
        }
	...
	*/
	...
	if (*err_code == E_DM1155_CPP_TMPL_MISSING)
	{
	    ule_format( E_DM1155_CPP_TMPL_MISSING, &sys_err,
			ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	    *err_code = E_DM1155_CPP_TMPL_MISSING;
	}
	else
	{
	    ule_format(E_DM1104_CPP_CKP_ERROR, &sys_err, 
			ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	    *err_code = E_DM1121_CPP_BEGIN_ERROR;
	}
	return (E_DB_ERROR);
	...
	**  Get a free device 
	*/
	status = cpp_wait_free(jsx, d, err_code);
	if (status != OK)
	{
	    ule_format(E_DM1104_CPP_CKP_ERROR, &sys_err,
		       ULE_LOG, NULL, 0, 0, 0, err_code, 0);
 ...
 '\0';
    ...
    */
    ...
    status = cpp_wait_all(jsx, err_code);
    if (status != OK)
    {
	ule_format(E_DM1104_CPP_CKP_ERROR, &sys_err, 
		   ULE_LOG, NULL, 0, 0, 0, err_code, 0);
    ...
    */
	...
	ule_format(E_DM1104_CPP_CKP_ERROR, &sys_err, 
			ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM1122_CPP_END_ERROR;
	return (E_DB_ERROR);
    }
...
i4	    *err_code)
{
	...
	*/
    ...
    status = config_handler(UPDATE_CONFIG, dcb, config, jsx, err_code);
    if (status != E_DB_OK)
    {
	dmf_write_msg(*err_code);
	*err_code = E_DM1140_CPP_UPD1_JNLDMP;
	return (E_DB_ERROR);
    }
...

{
    ...
    */
    ...
    {
	dmf_write_msg(*err_code);
	*err_code = E_DM1135_CPP_UPD2_JNLDMP;
	return (E_DB_ERROR);
    }
...
# define TUP_REC_ALLOW	0x04
	...
	*/
	...
	status = dm2t_open(dcb, &table_id, DM2T_IX, DM2T_UDIRECT, DM2T_A_WRITE,
	    ...
	    &timestamp, &rcb, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	status = dm2r_position(rcb, DM2R_ALL, 0, 0, 0, 0, 0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	/*	Close the table. */
	status = dm2t_close(rcb, DM2T_NOPURGE, err_code);
	if (status != E_DB_OK)
	    break;
	rcb = 0;
	...
	status = dm2d_close_db(
	    dcb, dmf_svcb->svcb_lock_list, DM2D_NLK, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*config = cnf;
    ...
    dmf_write_msg(E_DM1107_CPP_CATALOG_ERROR);
    *err_code = E_DM110F_CPP_FAIL_NO_CHANGE;
	...
	*/
	...
	local_status = config_handler(OPEN_CONFIG, dcb, &cnf, jsx, &local_err);
	if (local_status != E_DB_OK)
	{
	    ule_format(local_err, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		(i4)0, (i4 *)0, &local_err, 0);
	    cnf = 0;
	}
    }
	...
	if (rcb)
	{
	    local_status = dm2t_close(rcb, DM2T_NOPURGE, &local_err);
	    if (local_status)
		ule_format(local_err, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
...
CL_SYS_ERR	    *sys_err)
{
    ...
    ule_format(E_DM9003_BAD_FILE_DELETE, sys_err, ULE_LOG, NULL,
...
i4  	    *err_code)
{
	...
	*config = 0;
	status = dm0c_open(dcb, 0, dmf_svcb->svcb_lock_list, config, err_code);
	if (status == E_DB_OK)
	{
	    cnf = *config;
		...
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
		*err_code = E_DM1109_CPP_MEM_ALLOC;
		break;
	    }
	...
	*config = 0;
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM1105_CPP_CNF_OPEN;
	return (E_DB_ERROR);
	...
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM110C_CPP_CNF_CLOSE;
	return (E_DB_ERROR);
	...
	*/
	...
	status = dm0c_close(
	    cnf, DM0C_UPDATE | DM0C_LEAVE_OPEN | DM0C_COPY, err_code);
	...
	*/
	...
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM1148_CPP_CNF_DUMP;
	return (E_DB_ERROR);
    }
    return (E_DB_ERROR);
}
...
i4  	    *err_code)
{
    ...
    if (status || *cp == EOS)
    {
	ule_format(E_DM1117_CPP_II_DATABASE, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, err_code, 1,
	    STlength(SystemProductName), SystemProductName);
	*err_code = E_DM9036_JNLCKP_CREATE_ERROR;
        return (E_DB_ERROR);
    }
    ...
    */
	...
	status = dm2d_open_db(d, DM2D_A_READ, DM2D_IS, 
                              dmf_svcb->svcb_lock_list, 0, err_code); 

    if (status == E_DB_OK)
	jsx->jsx_log_id = dcb->dcb_log_id;
    else
    {
	*err_code = E_DM1009_JSP_DBDB_OPEN;
	return (E_DB_ERROR);
    }

    status = dmf_jsp_begin_xact(DMXE_READ, 0, d, d->dcb_log_id, &dcb->dcb_db_owner,
	dmf_svcb->svcb_lock_list, &log_id, &tran_id, &lock_id, err_code);
	...
	status = dm2t_open(d, &iilocation, DM2T_IS, DM2T_UDIRECT, DM2T_A_READ,
	    ...
	    &stamp, &rcb, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	{
	    *err_code = E_DM1015_JSP_OPEN_IILOC;	
	    break;
	}
	...
	status = dm2r_position(rcb, DM2R_QUAL, key_list, 1, 0, 0, 
                               &tid, err_code);
	if (status == E_DB_OK)
	{
	    status = dm2r_get(rcb, &tid, DM2R_GETNEXT, 
				(char *)&jnl_location, err_code);
	}
	if (status != E_DB_OK)
	    break;
    ...
    if (rcb)
    {
	close_status = dm2t_close(rcb, 0, err_code);
	if (close_status != E_DB_OK && status == E_DB_OK)
	{
	    error = E_DM1013_JSP_CLOSE_IILOC;
	    status = close_status;
	}
	rcb = 0;
    }
	...
	close_status = dm2d_close_db(
	    d, dmf_svcb->svcb_lock_list, DM2D_NLK, err_code);
    else	
	close_status = dm2d_close_db(
	    d, dmf_svcb->svcb_lock_list, 0, err_code);
    if (close_status != E_DB_OK && status == E_DB_OK)
    {
	status = close_status;
    }
	...
	*err_code = error;
	...
	if (status || *cp == EOS)
	{
	    ule_format(E_DM1115_CPP_II_CHECKPOINT, (CL_SYS_ERR *)0, ULE_LOG, 
	...
	if (status != OK && status != CK_EXISTS)
	{
		...
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM9040_BAD_CKP_DIRCREATE;
	    return (E_DB_ERROR);
	}
    }
	...
	if (status || *cp == EOS)
	{
	    ule_format(E_DM1116_CPP_II_JOURNAL, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, (char *)0, (i4)0, (i4 *)0, err_code, 1,
		STlength(SystemProductName), SystemProductName);
	    *err_code = E_DM9036_JNLCKP_CREATE_ERROR;
	    return (E_DB_ERROR);
	...
	{
	    ule_format(E_DM9033_BAD_JNL_DIRCREATE, &sys_err, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, err_code, 1,
		STlength(cp), cp);
	    *err_code = E_DM9036_JNLCKP_CREATE_ERROR;
	    return (E_DB_ERROR);
    ...
    */
	...
	if (status || *cp == EOS)
	{
	    ule_format(E_DM1133_CPP_II_DUMP, 0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM9036_JNLCKP_CREATE_ERROR;
	    return (E_DB_ERROR);
	}
	...
	if (status != OK && status != JF_EXISTS)
	{
	    ule_format(E_DM935C_BAD_DMP_DIRCREATE, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, (char *)0, (i4)0, (i4 *)0, err_code, 1,
		STlength(cp), cp);
	    *err_code = E_DM9036_JNLCKP_CREATE_ERROR;
	    return (E_DB_ERROR);
...
i4  	*err_code)
{
    ...
    TRdisplay(ERx("%@ CPP: Begin cluster merge of node journal files\n"));
    ...
    */
    ...
    TRdisplay(ERx("%@ CPP: Finish cluster merge of node journal files\n"));
    ...
    *err_code = E_DM1153_CPP_CLUSTER_MERGE;
    return (E_DB_ERROR);
}

/*{
...
i4  	    *err_code)
{
	...
	*/
	for (; j <= cnf->cnf_jnl->jnl_history[i].ckp_l_jnl; j++)
	{
		...
		DM0J_FORWARD, (DB_TAB_ID *)0, &jnl, err_code);
	    if (status != E_DB_OK)
	    {
		...
		{
		    dmf_write_msg(E_DM1118_CPP_JNL_OPEN);
		    *err_code = E_DM1118_CPP_JNL_OPEN;
		    return(E_DB_ERROR);
		}
	    }
	    status = dm0j_truncate(jnl, err_code);
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(E_DM1119_CPP_JNL_TRUNCATE);
		*err_code = E_DM1119_CPP_JNL_TRUNCATE;
		return(E_DB_ERROR);
	    }
	    (VOID) dm0j_close(&jnl, err_code);
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(E_DM1120_CPP_JNL_CLOSE);
		*err_code = E_DM1120_CPP_JNL_CLOSE;
		return(E_DB_ERROR);
	    }
	    jnl = 0;
	}
	...
	*/
	...
	for (; j <= cnf->cnf_dmp->dmp_history[i].ckp_l_dmp; j++)
	{
		...
		DM0D_FORWARD, &dmp, err_code);
	    if (status != E_DB_OK)
	    {
		/* If no dump files exist, no problem */

		if (*err_code == E_DM9352_BAD_DMP_NOT_FOUND)
		{
		    return(E_DB_OK);
		}
		else
		{
		    dmf_write_msg(E_DM1141_CPP_DMP_OPEN);
		    *err_code = E_DM1141_CPP_DMP_OPEN;
		    return(E_DB_ERROR);
		}
	    }
	    status = dm0d_truncate(dmp, err_code);
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(E_DM1142_CPP_DMP_TRUNCATE);
		*err_code = E_DM1142_CPP_DMP_TRUNCATE;
		return(E_DB_ERROR);
	    }
	    (VOID) dm0d_close(&dmp, err_code);
	    if (status != E_DB_OK)
	    {
		dmf_write_msg(E_DM1143_CPP_DMP_CLOSE);
		*err_code = E_DM1143_CPP_DMP_CLOSE;
		return(E_DB_ERROR);
	    }
	}
    }
...
i4	    *err_code )
{
	...
	*/
    ...
    if (status != E_DB_OK)
    {
        ...
        if (*err_code == E_DM9017_BAD_LOG_SHOW)
        {
            ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
                (char *)0, (i4)0, (i4 *)0, err_code, 1,
                0, err_arg);
        ...
        *err_code = E_DM942D_RCP_ONLINE_CONTEXT;
        return (E_DB_ERROR);
    }
...
DB_STATUS	    *err_code)
{
	...
	*/
	    ...
	    sizeof(CPP_TBLHCB) + ( 63 * sizeof(TBL_HASH_ENTRY)),
            ...
            (DM_OBJECT **)&cpp->cpp_tblhcb_ptr, err_code);
        if ( status != E_DB_OK )
	    break;
	...
	** is valid or not!
	*/
	status = cpp_read_tables (cpp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	{
	    dmf_write_msg (*err_code);
	    return (status);
	...
	/*
	** Determine which secondary indexes need to be checkpointed 
	** for consistency:
	**
	...
	*/
	status = cpp_get_index_info (cpp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = cpp_get_blob_info (cpp, jsx, dcb, err_code);
	if (status != E_DB_OK)
	    break;

	/*
	** Table level checkpoint requires that the core DBMS catalogs
	** and the database are consistent
	*/
	status = cpp_ckpt_core_tbl (cpp, jsx, dcb);
 	...
 	if (status != E_DB_OK)
	{
	    ...
	    ** so display and log error message.
            */
	    *err_code = E_DM1161_CPP_NO_CORE_TBL_CKPT;
	    return (status);
	}
	...
	** the user are valid
	*/
	status = cpp_ckpt_invalid_tbl (cpp, jsx, dcb, err_code);
...
DB_STATUS	    *err_code)
{
	...
	*/
	status = cpp_allocate_tblcb (cpp, &tblcb, err_code);
        if ( status != E_DB_OK )
        {
	    dmf_write_msg (*err_code);
	    break;
	...
	tblcb->tblcb_tcb_ptr = dcb->dcb_rel_tcb_ptr;
        ...
        */
        ...
        {
            tblcb->tblcb_table_status |= CPP_SYSTEM_CATALOG;
        }
	if (tblcb->tblcb_rel_ptr->relstat & TCB_VIEW)
	{
	    tblcb->tblcb_table_status |= CPP_VIEW_TABLE;
	}
	if (tblcb->tblcb_rel_ptr->relstat & TCB_GATEWAY)
	{
	    tblcb->tblcb_table_status |= CPP_GATEWAY_TABLE;
	}
		...
		*/
		if (!(dm0p_has_buffers(tblcb->tblcb_rel_ptr->relpgsize)))
		{
		    loc_status = dm0p_alloc_pgarray(
					tblcb->tblcb_rel_ptr->relpgsize,
					err_code);
    ...
    */
    if ( iirel_rcb )
    {
        loc_status = dm2r_release_rcb( &iirel_rcb, err_code );
        if ( loc_status != E_DB_OK )
        {
	    dmf_write_msg (*err_code);
	    return (loc_status);
...
DB_STATUS	    *err_code )
{
    ...
    status = dm0m_allocate((i4)
		(sizeof( CPP_TBLCB ) + sizeof( DMP_RELATION )),
            (i4)DM0M_ZERO, (i4)TBL_CB, (i4)CPP_TBLCB_ASCII_ID,
            (char *)cpp, (DM_OBJECT **)&tblcb, err_code);
    if (status != E_DB_OK)
    {
	dmf_write_msg (*err_code);
    ...
    tblcb->tblcb_rel_ptr = 
		(DMP_RELATION *)((char *)tblcb + sizeof( CPP_TBLCB ));
 ...
 "Additional tables required to maintain physical consistency of the database :"
	...
	*/
	if (tblcb->tblcb_table_status & 
		(CPP_SYSTEM_CATALOG | CPP_VIEW_TABLE | CPP_GATEWAY_TABLE))
	    continue;
...
** Name:
**
** Description: cpp_ckpt_core_tbl - Returns true if checkpointing a core
**			             DBMS catalog.
**
**
** Inputs:
...
DMP_DCB		    *dcb)
{
    ...
    */
	...
	tblcb = (CPP_TBLCB *)((char *)totq - CL_OFFSETOF(CPP_TBLCB, tblcb_totq));
        ...
        if ((tblcb->tblcb_table_status & CPP_USER_SPECIFIED) &&
	    (tblcb->tblcb_table_status & CPP_SYSTEM_CATALOG))
        {
	    /*
...
DB_STATUS           *err_code)
{
    ...
    */
	...
	if ( !(jsx->jsx_status1 & JSX1_CKPT_DB) &&
	    ((tblcb->tblcb_rel_ptr->relstat2 &
			TCB2_TBL_RECOVERY_DISALLOWED) ||
	    !(tblcb->tblcb_rel_ptr->relstat & TCB_JOURNAL)) )
	{
	    *err_code = E_DM116E_CPP_TBL_CKP_DISALLOWED;
	    return(E_DB_ERROR);
	}

	if (tblcb->tblcb_table_status & CPP_VIEW_TABLE)
        {
	    /*
	    ** Table specified by the user and is a view 
	    */
	    *err_code = E_DM1162_CPP_USR_INVALID_TBL;
	    return (E_DB_ERROR);
	}

	if (tblcb->tblcb_table_status & CPP_GATEWAY_TABLE)
        {
	    /*
	...
	*/
	      ...
	      )
	{
	    ...
	    */
	    ...
	    /* Increment count of tables being checkpointed */
	    cpp_tbl_cnt++;
...

/*{
** Name: cpp_get_index_info - Determine secondary indicies for tables 
**				being checkpointed
**
** Description:
...
DB_STATUS       *err_code)
{
	...
	{
    	    status = tbllst_create(&tblctx, err_code);
	    if (status == E_DB_OK)
		break;
    	    if (status == E_DB_WARN)
		/* file exists so delete and try again */
		status = tbllst_delete(&tblctx, err_code);
	}
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, (char * )0, 
		0L, (i4 *)0, &local_err, 0);
	    *err_code = E_DM1165_CPP_CHKPT_LST_CREATE;
	    break;
	}
	...
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, (char * )0,
		0L, (i4 *)0, &local_err, 0);
	    *err_code = E_DM1166_CPP_CHKPT_LST_OPEN;
	    break;
	}
	...
	*/
	      ...
	      )
	{
		...
		/* 
		** Db level ckp 
		**	Skip system catalogs, gateway tables and views
		*/
		if (tblcb->tblcb_table_status &
			(CPP_SYSTEM_CATALOG | CPP_GATEWAY_TABLE | 
			  CPP_VIEW_TABLE))
		    continue;
	    }
	    else 
		...
		*/
	    ...
	    if (++i == TBLLST_MAX_TAB)
	    {
		status = tbllst_write(&tblctx, err_code);
		if (status != E_DB_OK)
		{
	    	    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
			(char * )0, 0L, (i4 *)0, &local_err, 0);
	    	    *err_code = E_DM1168_CPP_CHKPT_LST_WRITE;
	    	    break;
		}
		i = 0;
	    }
    ...
    if (tblctx.blkno != -1)
    {
	local_status = tbllst_close(&tblctx, &local_err);
	if (local_status != E_DB_OK)
	{
	    ule_format(local_err, (CL_ERR_DESC *)0, ULE_LOG, NULL, (char * )0,
		0L, (i4 *)0, &local_err, 0);
	    if (status != E_DB_OK)
	    {
		ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG,
		    NULL, (char * )0, 0L, (i4 *)0, &local_err, 1,
		    sizeof(tblctx.filename), tblctx.filename.name);
	    }
	    status = E_DB_ERROR;
	    *err_code = E_DM1169_CPP_CHKPT_LST_CLOSE;
	}
    }
    ...
    if (status != E_DB_OK)
    {
	    ...
	    0L, (i4 *)0, &local_err, 1, 
	    sizeof(tblctx.filename), tblctx.filename.name);
    	*err_code =  E_DM116A_CPP_CHKPT_LST_ERROR;
    }
    return(status);
}

	...
	*string               &&
    ...
    for (;;)
    {
	    ...
	    if ( (i4)jsx->jsx_device_list[i].dev_pid > 0 )
			...
			ule_format( retval, &err_desc, ULE_LOG, NULL, 0, 0,
				    0, &errcode, 0 );
			MEfree(command);
...
online_cx_phase(i4 phase, DMF_JSX *jsx)
{
    ...
    {
	case CKP_CSP_STALL:
	    status = online_setup(jsx, &jsx->jsx_lock_list, &err_code);
	    /*
	...
	case CKP_CSP_RESUME:
	    status = online_resume(jsx, &jsx->jsx_lock_list, &err_code);
	    break;
	...
	default:
	    TRdisplay(ERx("%@ CPP: Invalid phase %d\n"), phase);
	    return (E_DB_ERROR);
	    break;
    }
    ...
    {
	TRdisplay(ERx("%@ CPP: Phase %d testing error handling\n"), phase), 
	err_code = E_DM1102_CPP_FAILED;
	status = E_DB_ERROR;
    }
...
i4		*err_code)
{
	...
	TRdisplay("CXmsg_send phase %d status %d\n", phase, cl_status);
        if (cl_status != OK)
            break;
	...
	*/
        ...
        lockckey.lk_key1 = jsx->jsx_db_id;
        lockckey.lk_key2 = phase;
        lockckey.lk_key3 = 0;
        lockckey.lk_key4 = 0;
        lockckey.lk_key5 = 0;
        lockckey.lk_key6 = 0;
	...
	if (ckpdb_status != E_DB_OK)
	{
        ...
        lockckey.lk_key2 = CKP_CSP_STATUS;
        cl_status = LKrequest(LK_PHYSICAL, jsx->jsx_lock_list,
            &lockckey, LK_N, &lock_value, (LK_LKID *)0, 0L, &sys_err);
	...
	TRdisplay("After phase %d LK_CKP_CLUSTER CKP_CSP_STATUS (%d,%d)\n",
		phase, lock_value.lk_low, lock_value.lk_high);

        if (lock_value.lk_low != jsx->jsx_pid || lock_value.lk_high != 0)
        {
	    TRdisplay("LK_CKP_CLUSTER value (%d,%d) should be (%d,%d)\n",
		lock_value.lk_low,lock_value.lk_high, jsx->jsx_pid, 0);
            status = E_DB_ERROR;
            break;
        }
	...
	TRdisplay("After phase %d sending ABORT msg \n", phase);
        cl_status = CXmsg_send( CSP_CHANNEL, NULL, NULL, (PTR)&err_msg);
