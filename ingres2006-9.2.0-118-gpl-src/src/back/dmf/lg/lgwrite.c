...
static VOID		assign_logwriter(LGD *lgd, LBB *lbb, LPB *lpb);
...
CL_ERR_DESC	    *sys_err)
{
	...
	*/
	ule_format(E_DMA402_LG_WRITE_BADPARM, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
		    0, num_objects, 0, lsn, 0, objects);
	return(LG_BADPARAM);
    }

    lxbb_table = (SIZE_TYPE *)LGK_PTR_FROM_OFFSET(lgd->lgd_lxbb_table);
    lxb = (LXB *)LGK_PTR_FROM_OFFSET(lxbb_table[((LG_ID *) (&lx_id))->id_id]);
	...
	*/
	status = LG_write(&internal_flag, (LG_ID *) &lx_id, num_objects, 
			    objects, lsn, &async_status, sys_err);
...
#define MAX_MECOPY_LENGTH	65535	/* max MEcopy length */
    ...
    {
	ule_format(E_DMA403_LG_WRITE_BAD_ID, (CL_ERR_DESC *)0,
    ...
    {
	ule_format(E_DMA404_LG_WRITE_BAD_XACT, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
	...
	||
	 (handle_lxb->lxb_status & LXB_READONLY &&
	  !(handle_lxb->lxb_status & LXB_ILOG) &&
	      !(*flag & LG_ILOG)) )
    {
	ule_format(E_DMA406_LG_WRITE_BAD_STAT, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 1, 0, 
	...
	*/
	    ...
	    objects[obj_num].lgo_size <= 0)
	{
	    ...
	    (VOID)LG_unmutex(&handle_lxb->lxb_mutex);
	    ule_format(E_DMA405_LG_WRITE_BAD_RLEN, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
	...
	*/
	...
	if (lxb->lxb_status & LXB_PROTECT)
	{
	    ...
	    */
		...
		if ( handle_lxb != lxb )
		    (VOID)LG_unmutex(&handle_lxb->lxb_mutex);
		ule_format(E_DMA407_LG_WRITE_NOT_FIRST, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
    ...
    */
	...
	if ((lbb->lbb_state & LBB_CURRENT) == 0)
	{
		...
		LG_suspend(handle_lxb, &lgd->lgd_wait_buffer, async_status);
		(VOID)LG_unmutex(&lxb->lxb_mutex);
		if ( handle_lxb != lxb )
		    (VOID)LG_unmutex(&handle_lxb->lxb_mutex);
		return (LG_MUST_SLEEP);
    ...
    */
    ...
    /*
    ** Update external log record header with transaction & database info.
    **
    ...
    */
    ...
    {
	status = LGlsn_next(&dm0l_header->lsn, sys_err);
	if (status)
	{
	...
	lfb->lfb_header.lgh_last_lsn.lsn_high = dm0l_header->lsn.lsn_high;
	lfb->lfb_header.lgh_last_lsn.lsn_low = dm0l_header->lsn.lsn_low;
	...
	dm0l_header->lsn.lsn_high = lfb->lfb_header.lgh_last_lsn.lsn_high;
	dm0l_header->lsn.lsn_low  = lfb->lfb_header.lgh_last_lsn.lsn_low;
    }
    /*
    ** Return the LSN of the record being written to the caller. Also record
    ** this LSN in the caller's log record header, and save it in the LXB and
    ** LFB structures for information tracking purposes.
    */
    lsn->lsn_high = lxb->lxb_last_lsn.lsn_high = dm0l_header->lsn.lsn_high;
    lsn->lsn_low = lxb->lxb_last_lsn.lsn_low = dm0l_header->lsn.lsn_low;
    if (lbb->lbb_first_lsn.lsn_high == 0)
	lbb->lbb_first_lsn = *lsn;

    /* Record the LSN/LGA of the txn's BT record */
    if (lxb->lxb_first_lsn.lsn_high == 0)
    {
	lxb->lxb_first_lsn = *lsn;
	...
	*/
	...
	{
	    CS_breakpoint();
	    TRdisplay("%@ PANIC!! LG_allocate_lbb() failed!\n");
	    /*
	    ...
	    */
	    ...
	    LG_resume(&lgd->lgd_wait_buffer, 0);
	    return(status);
	}

	span_lbb->lbb_first_lsn = span_lbb->lbb_last_lsn = *lsn;
    ...
    *size_ptr = size_lr;
    ...
    lbh->lbh_address = lfb->lfb_header.lgh_end;
    span_lbb->lbb_last_lsn = *lsn;
    ...
    */
    if (lxb->lxb_status & LXB_JOURNAL)
    {
	ldb->ldb_j_last_la = lfb->lfb_header.lgh_end;
	lfb->lfb_archive_window_end = lfb->lfb_header.lgh_end;
    }

    if (ldb->ldb_status & LDB_BACKUP)
    {
	ldb->ldb_d_last_la = lfb->lfb_header.lgh_end;
	lfb->lfb_archive_window_end = lfb->lfb_header.lgh_end;
    }
    ...
    i4  flags)
{
    ...
    */
	...
	if (lbb->lbb_lga.la_block)
	{
	    ...
	    lbb->lbb_forced_lsn = lbb->lbb_last_lsn;
    ...
    */
	...
	if (status)
	{
	    ule_format(status, sys_err, ULE_LOG, NULL, 0, 0, 0,
				&err_code, 0);
	    ule_format(E_DMA44F_LG_WB_BLOCK_INFO, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 6,
...
CL_ERR_DESC	    *sys_err)
{
    ...
    {
	ule_format(E_DMA44C_LG_WBLOCK_BAD_ID, (CL_ERR_DESC *)0,
				ULE_LOG, NULL, 0, 0, 0,
				&err_code, 2,
				0, lx_id->id_id, 0, lgd->lgd_lxbb_count);
	return (LG_BADPARAM);
	...
	lxb->lxb_id.id_instance != lx_id->id_instance)
    {
	ule_format(E_DMA44D_LG_WBLOCK_BAD_XACT, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
    ...
    CL_ERR_DESC *sys_err)
{
    ...
    */
	...
	if (status)
	{
	    ule_format(status, sys_err, ULE_LOG, NULL, 0, 0, 0,
				&err_code, 0);
	    ule_format(E_DMA44E_LG_WBLOCK_BAD_WRITE, sys_err,
...
choose_logwriter( VOID )
{
    ...
    SIZE_TYPE		lxbq_offset;
    SIZE_TYPE		end_offset;
    register LXB	*lxb = (LXB *)0;
    LXB			*alternate_lxb = (LXB *)0;
