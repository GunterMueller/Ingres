...
LG_meminit(CL_ERR_DESC *sys_err)
{
    ...
    lgd->lgd_wait_buffer.lwq_lxbq.lxbq_next =
	lgd->lgd_wait_buffer.lwq_lxbq.lxbq_prev =
		LGK_OFFSET_FROM_PTR(&lgd->lgd_wait_buffer.lwq_lxbq);
    lgd->lgd_wait_buffer.lwq_count = 0;
    if (status = LG_imutex(&lgd->lgd_wait_buffer.lwq_mutex, "LG Wait buffer mutex"))
	return (status);
    ...
    header_lbb = (LBB *)lgkm_allocate_ext( sizeof(LBB) + 2048 + DI_RAWIO_ALIGN);
    if (header_lbb == (LBB *)0)
	...
	return (status);
    aligned_buffer = 
	    (LBH*)ME_ALIGN_MACRO((char*)header_lbb + sizeof(LBB),
					DI_RAWIO_ALIGN);
...
i4             pid;
{
	...
	if ((pid == master_lpb->lpb_pid) ||
	    (PCis_alive(master_lpb->lpb_pid) == FALSE))
	{
