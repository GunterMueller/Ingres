...
CL_ERR_DESC	    *sys_err)
{
    ...
    {
	ule_format(E_DMA48B_LGERASE_BAD_ID, (CL_ERR_DESC *)0,
    ...
    {
	ule_format(E_DMA48A_LGERASE_BAD_PROC, (CL_ERR_DESC *)0,
		...
		*/
		if (LG_logs_opened & LG_DUAL_LOG_OPENED)
		{
		    ...
		    */
		    ...
		    if (status)
		    {
			(VOID) ule_format(status, sys_err, ULE_LOG,
					NULL, 0, 0, 0,
					&err_code, 0);
			(VOID) MEfree_pages(mem,
					(LGD_MAX_BLK_SIZE / ME_MPAGESIZE),
					&me_sys_err);
			return(LG_CANTINIT);
		    }
		}
    ...
    */
	...
	case LG_ERASE_DUAL_ONLY:
	    ...
	    status = LG_write_log_headers( header, (LG_IO *)0,
				    dual_dio, sys_err);
	    break;
...
CL_SYS_ERR *sys_err )
{
    ...
    MEcopy( (PTR)header, sizeof(LG_HEADER), (PTR)buffer );
    MEcopy( (PTR)header, sizeof(LG_HEADER), (PTR)&buffer[1024] );
