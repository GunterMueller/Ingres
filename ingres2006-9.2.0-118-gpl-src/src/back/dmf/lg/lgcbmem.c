...
LG_allocate_cb( i4  type )
{
	...
	case LPB_TYPE:
		...
		{
		    ule_format(E_DMA437_NOT_ENOUGH_SBKS, (CL_ERR_DESC *)0,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
			    0, lgd->lgd_lpbb_count, 0, lgd->lgd_lpbb_size);
		    ule_format(E_DMA438_LPB_ALLOC_FAIL, (CL_ERR_DESC *)0,
	...
	case LPD_TYPE:
		    ...
		    (cb_offset = expand_list(LPD_TYPE)) == 0)
		{
		    ...
		    ule_format(E_DMA43A_LPD_ALLOC_FAIL, (CL_ERR_DESC *)0,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		    return ((PTR)NULL);
		}
	...
	case LFD_TYPE:
		    ...
		    (cb_offset = expand_list(LFD_TYPE)) == 0)
		{
		    ...
		    ule_format(E_DMA441_LFD_ALLOC_FAIL, (CL_ERR_DESC *)0,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		    return ((PTR)NULL);
		}
	...
	case LDB_TYPE:
		...
		{
		    ule_format(E_DMA43B_NOT_ENOUGH_LBKS, (CL_ERR_DESC *)0,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
			    0, lgd->lgd_ldbb_count, 0, lgd->lgd_ldbb_size);
		    ule_format(E_DMA43C_LDB_ALLOC_FAIL, (CL_ERR_DESC *)0,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		    return ((PTR)NULL);
		}
...
**
** Description:
**      This routine allocates the next LBB from the free list and initializes
**	it for writing.  If no free buffers are available or wraparound is
**	about to happen, an error is returned so that the caller can suspend.
**
...
LBB	**lbbo)
{
    ...
    */
    ...
    {
	ule_format(E_DMA481_WRITE_PAST_BOF, (CL_ERR_DESC *)0,
	    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	PCexit(FAIL);
    }
	...
	*shared_lxb = (LXB*)NULL;
	...
	do
	{
	    ...
	    */
		...
		while ( lxbq != (LXBQ *)&lgd->lgd_slxb.lxbq_next )
		{
			...
			if ( slxb->lxb_status & LXB_ABORT )
			{
			    ...
			    ule_format(E_DMA495_LG_BAD_SHR_TXN_STATE, (CL_ERR_DESC *)0,
		    ...
		    (cb_offset = expand_list(LXB_TYPE)) == 0)
		{
			...
			*shared_lxb = (LXB *)NULL;
		    ...
		    ule_format(E_DMA439_LXB_ALLOC_FAIL, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		...
		*/
		...
		{
		    LG_assign_tran_id(lfb, lxb, TRUE);
...
expand_list(i4 type)
{
	...
	case LPB_TYPE:
	    ...
	    {
		ule_format(E_DMA43D_SBK_LIMIT_REACHED, (CL_ERR_DESC *)0,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
			    0, lgd->lgd_lpbb_count, 0, lgd->lgd_lpbb_size);
		return (0);
	    }
	...
	case LFB_TYPE:
	    ...
	    {
		ule_format(E_DMA43E_LBK_LIMIT_REACHED, (CL_ERR_DESC *)0,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
			    0, lgd->lgd_lfbb_count, 0, lgd->lgd_lfbb_size);
		return (0);
	    }
    ...
    */
	...
	{
	    ule_format(E_DMA43F_LG_SHMEM_NOMORE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	    return (0);
	}
    } while (items == NULL);
...
**	Computes the next available transaction id, 
	...
	case LPD_TYPE:
	    ...
	    return (lgd->lgd_lpdb_free);
**	returns it in lxb->lxh_tran_id.
**
** Inputs:
**	lfb			- the Log File Block
**	lxb			- The LXB in need of a tran_id.
**	mutex_held		TRUE if caller holds lgd_lxbb_mutex.
**
** Outputs:
**	lxb
**		lxh_tran_id	Holds the assigned tran_id.
**
** Returns:
**	VOID
**
...
bool	mutex_held)
{
	...
	*/
	lxb->lxb_lxh.lxh_tran_id = lfb->lfb_header.lgh_tran_id;
	...
	while (lxh_offset != end_offset)
	{
	    ...
	    if ( lxb->lxb_lxh.lxh_tran_id.db_low_tran ==
		     lxh->lxh_tran_id.db_low_tran)
	    {
		break;
	    }
	}
