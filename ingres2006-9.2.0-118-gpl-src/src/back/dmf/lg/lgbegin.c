...
CL_ERR_DESC	    *sys_err)
{
    ...
    /*	Check the lg_id and db_id. */

    if (db_id->id_id == 0 || (i4)db_id->id_id > lgd->lgd_lpdb_count)
    {
	ule_format(E_DMA412_LGBEGIN_BAD_ID, (CL_ERR_DESC *)0,
    ...
    {
	ule_format(E_DMA413_LGBEGIN_BAD_DB, (CL_ERR_DESC *)0,
	...
	l_user_name == 0)
    {
	ule_format(E_DMA414_LGBEGIN_BAD_LEN, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 1, 0, l_user_name);
	return(LG_BADPARAM);
    }
    ...
    */
	...
	if ( return_status == LG_EXCEED_LIMIT )
	    ule_format(E_DMA440_LGBEGIN_NO_LXBS, (CL_ERR_DESC *)0,
...
CL_ERR_DESC	    *sys_err)
{
    ...
    LG_ID	*clx_id = (LG_ID*)&connect_lx_id;
    ...
    if ( clx_id->id_id == 0 || (i4)clx_id->id_id > lgd->lgd_lxbb_count)
    {
	ule_format(E_DMA496_LGCONNECT_BAD_ID, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
		    0, clx_id->id_id, 0, lgd->lgd_lxbb_count);
	return(LG_BADPARAM);
    }

    lxbb_table = (SIZE_TYPE *)LGK_PTR_FROM_OFFSET(lgd->lgd_lxbb_table);
    clxb = (LXB *)LGK_PTR_FROM_OFFSET(lxbb_table[clx_id->id_id]);

    if ( clxb->lxb_type != LXB_TYPE ||
	 clxb->lxb_id.id_instance != clx_id->id_instance )
    {
	ule_format(E_DMA497_LGCONNECT_BAD_ID, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 3,
		    0, clxb->lxb_type, 
		    0, clxb->lxb_id.id_instance,
		    0, clx_id->id_instance);
	return(LG_BADPARAM);
    }
	...
	{
	    ule_format(E_DMA498_LGCONNECT_NO_LXBS, (CL_ERR_DESC*)0,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	...
	** on the active hash and txn queues with the
	** SHARED LXB.
	*/
	...
	/* Copy Tran_id, tran bucket */
	slxb->lxb_lxh = clxb->lxb_lxh;
