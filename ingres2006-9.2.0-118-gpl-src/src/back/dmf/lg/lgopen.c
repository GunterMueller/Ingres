    ...
    */
	...
	if (lgd_active_log != LG_DUAL_ACTIVE)
	{
	    ...
	    */
		...
		if (test_val == 0)
		{
		    ...
		    {
			ule_format(status_primary, (CL_ERR_DESC *)sys_err,
				    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		    }
		}
		else
	...
	if (lgd_active_log != LG_PRIMARY_ACTIVE)
	{
		...
		if (test_val == 0)
		{
			...
			ule_format(status_dual, (CL_ERR_DESC *)sys_err,
				    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		}
		else
		{
	...
	if (lgd_active_log == LG_BOTH_LOGS_ACTIVE)
	{
		...
		*/
		...
		if (status == OK)
		{
			...
			{
			    ule_format(E_DMA47F_LGOPEN_SIZE_MISMATCH,
				    (CL_ERR_DESC *)0,
...
{	
	...
	*/
	...
	for (;;)
	{
		...
		{
		    if (((LG_HEADER *)buffer)->lgh_version == LGH_VERSION)
		    {
		...
		hdr_ptr = &buffer[(LOG_HEADER_IO_SIZE * 2)];

		if (LGchk_sum(hdr_ptr, sizeof(LG_HEADER)) ==
				((LG_HEADER *)hdr_ptr)->lgh_checksum)
		{
		    if (((LG_HEADER *)hdr_ptr)->lgh_version == LGH_VERSION)
		    {
			*header = *(LG_HEADER *)hdr_ptr;
			break;
		    }
		}
		if (((LG_HEADER *)hdr_ptr)->lgh_version != LGH_VERSION)
    ...
    */
	...
	if (status == OK)
	{
		...
		if (((LG_HEADER *)buffer)->lgh_version == LGH_VERSION)
		{
		    header_primary = *(LG_HEADER *)buffer;  /* struct assign */
		    break;
		}
	    }
	    if (((LG_HEADER *)buffer)->lgh_version != LGH_VERSION)
		...
		if (((LG_HEADER *)hdr_ptr)->lgh_version == LGH_VERSION)
		{
		    header_primary = *(LG_HEADER *)hdr_ptr;
		    break;
		}
	    }
	    if (((LG_HEADER *)hdr_ptr)->lgh_version != LGH_VERSION)
	...
	if (status == OK)
	{
		...
		if (((LG_HEADER *)buffer)->lgh_version == LGH_VERSION)
		{
		    header_dual = *(LG_HEADER *)buffer;  /* struct assign */
		    break;
		}
	    }
	    if (((LG_HEADER *)buffer)->lgh_version != LGH_VERSION)
		...
		if (((LG_HEADER *)hdr_ptr)->lgh_version == LGH_VERSION)
		{
		    header_dual = *(LG_HEADER *)hdr_ptr;  /* struct assign */
		    break;
		}
	    }
	    if (((LG_HEADER *)hdr_ptr)->lgh_version != LGH_VERSION)
...
CL_ERR_DESC	*sys_err)
{
	...
	ule_format(status, (CL_ERR_DESC *)sys_err,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	TRdisplay("%@ LG: Logfile header reconstruction -- no memory!\n");
	return(status);
    }
    else
    {
	...
	** ENTIRE log file, including the header, is filled with binary 0's)
	...
	for (;;)
	{
		...
		ule_format(status, (CL_ERR_DESC *)sys_err,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		status = MEfree_pages(mem, (LGD_MAX_BLK_SIZE / ME_MPAGESIZE),
				    sys_err);
		if (status)
		    ule_format(status, (CL_ERR_DESC *)sys_err,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	    ...
	    */
		...
		for (b = buffer, i = 0; i < j; i++, b += cbsize)
		{
	...
	*/
    ...
    /* free up the memory */
    status = MEfree_pages(mem, (LGD_MAX_BLK_SIZE / ME_MPAGESIZE), sys_err);
    if (status)
    {
	ule_format(status, (CL_ERR_DESC *)sys_err,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
...
CL_ERR_DESC	*sys_err)
{
    ...
    */
	...
	one_page = 1;
	status = LG_READ(lg_io, &one_page, eof_block, buffer, sys_err);
	if (status != OK)
	{
	    /*
	...
	if (((LBH *)buffer)->lbh_address.la_sequence <= seq - 1)
	    break;

	seq = ((LBH *)buffer)->lbh_address.la_sequence;
    ...
    */
    if (flag & LG_DUAL_LOG)
    {
	status = check_uneven_eof( header, seq, buffer,
	...
	if (status != OK)
	{
	    ...
	    {
		(VOID)MEfree_pages(mem, (LGD_MAX_BLK_SIZE / ME_MPAGESIZE),
				    sys_err);
		return ( status );
	    }
	}
    ...
    */
	...
	if (LGchk_sum(buffer, header->lgh_size)==((LBH *)buffer)->lbh_checksum)
	{
	    /*
	    ...
	    */
	    ...
	    if ((((LBH *)buffer)->lbh_address.la_offset) == 0) 
	    {
		TRdisplay("%@ LG: Page <%d,%d,%d> was empty or incomplete.\n",
			((LBH *)buffer)->lbh_address.la_sequence,
			((LBH *)buffer)->lbh_address.la_block,
			((LBH *)buffer)->lbh_address.la_offset);
		continue;
	    }
	    /*
	    ...
	    */
	    header->lgh_end = ((LBH *)buffer)->lbh_address;
	    break;
	}
	else
	{
	...
	(VOID)MEfree_pages(mem, (LGD_MAX_BLK_SIZE / ME_MPAGESIZE), sys_err);
	return (LG_CANTOPEN);
    }
    ...
    */
    ...
    if (((LBH *)buffer)->lbh_used == header->lgh_size && 
	(((LBH *)buffer)->lbh_address.la_offset) <
	...
	*/
	...
	((LBH *)buffer)->lbh_used = ((LBH *)buffer)->lbh_address.la_offset 
	    + sizeof(i4);

	((LBH *)buffer)->lbh_checksum = LGchk_sum(buffer, header->lgh_size);

	/*	Issue the QIO to write the block. */

	one_page = 1;
	status = LG_WRITE(lg_io, &one_page, eof_block, buffer, sys_err);
	if (status != OK)
	{
	    ule_format(status, (CL_ERR_DESC *)sys_err,
			ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	...
	if (flag & LG_DUAL_LOG)
	{
	    ...
	    one_page = 1;
	    status = LG_WRITE(dual_lg_io, &one_page, eof_block, buffer, sys_err);
	    if (status != OK)
	    {
		ule_format(status, (CL_ERR_DESC *)sys_err,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
    ...
    status = MEfree_pages(mem, (LGD_MAX_BLK_SIZE / ME_MPAGESIZE), sys_err);
    return (return_status ? return_status : status);
}

/*
...
i4	    *num_files)
{
    ...
    */
	...
	if (status)
	{
	    ule_format(status, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
	    ule_format(E_DMA479_LGOPEN_PATH_INFO, (CL_ERR_DESC *)0,
				...
				i4 *l_path)
{
    ...
    *l_file = STlength(file);
    ...
    if (LOcompose(device, path_buf, (char *)0, (char *)0, (char *)0, &tmploc)
		!= OK)
    {
