	...
	*/
	...
	if (PCis_alive(saved_pid) == FALSE)
	{
	    ...
	    */
	    ...
	    ule_format(E_DMA469_PROCESS_HAS_DIED, (CL_ERR_DESC *)0,
...
{ 
    ...
    */
    ...
    if (status)
	TRdisplay("LG_rundown: LGK_get_critical_sem returns %d\n", status);
    ...
    */
    ...
    if (status)
	TRdisplay("LG_rundown: LGK_rel_critical_sem returns %d\n", status);
...
get_critical_sems(void)
{
    ...
    */
    ...
    /*
    ** Other external entry points variously lock the lgd_ and
    ** local lfb_mutex.
    */
    (VOID)CSp_semaphore(SEM_EXCL, &lgd->lgd_mutex);
    (VOID)CSp_semaphore(SEM_EXCL, &lgd->lgd_local_lfb.lfb_mutex);
...
rel_critical_sems(void)
{
    ...
    */
    (VOID)CSv_semaphore(&lgd->lgd_local_lfb.lfb_mutex);
...
LG_disable_dual_logging(i4 active_log, register LFB *lfb)
{
	...
	*/
	...
	status = LG_write_headers( &lx_id, lfb, (STATUS *)0 );
	if (status)
	    ule_format(status, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);

	ule_format(E_DMA467_DISABLE_LOGGING, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
...
LG_check_logfull(register LFB *lfb, LXB *lxb)
{
    ...
    */
	...
	if (lgd->lgd_status & LGD_ONLINE)
	{
	    ule_format(E_DMA48E_LOGFULL_EXCEEDED, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &error, 7,
	...
	*/
	...
	{
	    ule_format(E_DMA48F_LOGFILE_FULL, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &error, 7,
		    0, lfb->lfb_header.lgh_begin.la_sequence,
		    0, lfb->lfb_header.lgh_begin.la_block,
		    0, lfb->lfb_header.lgh_begin.la_offset,
...

/*
** Name: LGidnum_from_id	    - extract the ID portion of an LG id.
**
** Description:
**	Normally, logging system identifiers are opaque i4 "handles" which
**	the LG clients should never look into. LG generates these handles and
**	the client is expected just to hand them back in at appropriate times.
**
**	Among other uses, the handles are written into log record headers, and
**	this has led to the archiver using these handles to track the database
**	which are being archived. Therefore, the archiver needs a way to
**	extract the "numeric" portion of a logging system handle. This function
**	performs that extraction.
**
** Inputs:
**	handle			    - a logging system handle
**
** Outputs:
**	None
**
** Returns:
**	number			    - the numeric portion of the handle.
**
** History:
**	17-nov-1992 (bryanp)
**	    Added LGidnum_from_id() to extract the ID portion of an LG id.
...
*/
i4
LGidnum_from_id(i4 handle)
{
    return (((LG_ID *)&handle)->id_id);
}

/*
...
bool	include_reserved)
{
    ...
    if (include_reserved)
	blocks_used += (lfb->lfb_reserved_space / lfb->lfb_header.lgh_size) + 1;
