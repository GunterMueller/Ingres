...
SCF_FTX		*ftxcb)
{
	...
	*/
	...
	if ( psort->srt_pstatus == E_DB_OK )
	{
	    ...
	    psort->srt_perr_code = err_code;
	}

	ule_format(err_code, 0, ULE_LOG, NULL, 
		    0, 0, 0, &err_code, 0);
	ule_format(E_DM9714_PSORT_CFAIL, 0, ULE_LOG, NULL, 
		    0, 0, 0, &err_code, 0);
    }
    else if (DMZ_SRT_MACRO(1))
    {
	TRdisplay("%x DMSE_CHILD: Gets = %d, Puts = %d, Rows = %d, Cwaits = %d, Pwaits = %d\n",
	    step->ste_ctid,
    ...
    */
    ...
    {
	s = (DMS_SRT_CB*)NULL;
	(VOID)dmse_end_serial(temp_s, &err_code);
    }
...

{
    ...
    ** "put" function. */
	...
	if ( step->ste_c_puts )
	{
		...
		TRdisplay("%x DMSE_TIMEC1: exec - %f, wait - %f, pw %d, pr %d, cw %d, cr %d\n",
		step->ste_ctid,
	    ...
	    status = dmse_input_end_serial(step->ste_ccb, &err_code);
		...
		if (DMZ_SRT_MACRO(1))
		{
		    ...
		    step->ste_texec += step->ste_exec;
		    step->ste_twait += step->ste_wait;
		    TRdisplay("%x DMSE_TIMEC2: exec - %f, wait - %f, texec - %f, twait - %f, pw %d, pr %d, cw %d, cr %d\n",
		    step->ste_ctid,
...
DM_STECB 	*step)
{
    ...
    */
	...
	status = dmse_get_record_serial(s, &record, &err_code);
...
DM_STECB	  *step)
{
	...
	if ( CScnd_wait(&psort->srt_cond, &psort->srt_cond_sem) )
	{
		...
		psort->srt_perr_code = E_DM0065_USER_INTR;

		ule_format(psort->srt_perr_code, 0, ULE_LOG, NULL, 
			    0, 0, 0, &psort->srt_perr_code, 0);
		ule_format(E_DM9714_PSORT_CFAIL, 0, ULE_LOG, NULL, 
			    0, 0, 0, &psort->srt_perr_code, 0);
	    }
	}
