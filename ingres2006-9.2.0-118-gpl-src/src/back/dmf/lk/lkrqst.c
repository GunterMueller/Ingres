...
static STATUS	lk_set_dlm_callback( LKB *lkb );
...
static CX_BLKI	Lk_default_blki_array[LK_DEFAULT_BLKI_COUNT];
static CX_BLKI *Lk_blki_pointer = Lk_default_blki_array;
static i4	Lk_blki_count = LK_DEFAULT_BLKI_COUNT;
# endif
...
(%x %x %x %x %x %x %x)\n", lock_key->lk_type,
		...
		*/
		...
		{
		    /* Request failed. LK_do_cancel needs llb_mutex */ 
		    (VOID)LK_mutex(SEM_EXCL, &llb->llb_mutex);
		    LK_do_cancel( &completion_handle.lkb_offset );

		    /* This will be NULL if LKB was freed */
		    lkb = (LKB*)LGK_PTR_FROM_OFFSET(completion_handle.lkb_offset);

		    if ( lkb && !completion_handle.blocking_cpid.pid )
		    {
...
LK_COMP_HANDLE 	    *completion_handle)
{
 ...
 (%x %x %x %x %x %x %x)\n", lock_key->lk_type,
    ...
    */
    lkreq.cbt_id = LK_mycbt;
    lkreq.cback = callback;
...
NODLOCK,TEMP,NONINTR", lkreq->flags, lkreq->lock_mode);
	TRdisplay("  LKD Sanity check! status=%v, rqst_id=%d\n",
	    "AST_ENABLE,CLUSTERED,CSP_RUNDOWN,IN_CSP_COMPLETN,\
CSP_RCVR_HOLD,CSP_RCVR_HELD1", lkd->lkd_status, lkreq->request_id);
	TRflush();
    }
#endif
    if (lkd->lkd_status & LKD_CSP_RUNDOWN)
    ...
    */
	...
	/*	Update LKB type flags. */
		...
		(VOID) LK_mutex(SEM_EXCL, &rsb->rsb_mutex);
            lkb->lkb_count++;
	    if ( lkreq->flags & LK_MULTITHREAD )
		(VOID) LK_unmutex(&rsb->rsb_mutex);
        }
    ...
    */
    ...
    lkb->lkb_cbt_id = lkreq->cbt_id;
    lkb->lkb_cback_fcn  = lkreq->cback;
    lkb->lkb_cback_arg = lkreq->cback_arg;

# ifdef	PARANOIA
    if ( lkb->lkb_cback_off )
    {
...
# if defined(conf_CLUSTER_BUILD)
	if ( distributed_lock )
	{
	    ...
	    status = lk_set_dlm_callback(lkb);
	    if ( status )
	    {
		/* Could not allocate CBACK structure. */
		return status;
	    }
...
CL_ERR_DESC     *sys_err)
{
    ...
    */
    new_cback = FALSE;
    /* cbt_id must always be that of lock holder's server */
    lkb->lkb_cbt_id = lkreq->cbt_id;
    if (lkreq->cback != (LKcallback) NULL)
    {
        /* If there was no callback before then inc. rsb's cback count */
        if (lkb->lkb_cback_fcn == (LKcallback) NULL)
        {
            rsb->rsb_cback_count++;
            new_cback = TRUE;
        }
...
# endif
	...
	*/
        ...
        */
        ...
        /* b87049 - Make sure to pick a different holder than the current */
        if ( (next_lkb = lkb) ==
             (LKB*)LGK_PTR_FROM_OFFSET(rsb->rsb_grant_q_next)
            && lkb->lkb_q_next !=
                 LGK_OFFSET_FROM_PTR(&rsb->rsb_grant_q_next) )
...
# endif /* CLUSTER_BUILD */
	...
	/* Sleep until woken by LK_suspend or our interval elapses */
	...
	if (lk_cluster_deadlock_debug && (++wakes % 100 == 0))
	    TRdisplay("%@ LKdeadlock_thread: woke from CSsuspend %d\n", wakes);
	...
	*/
	while (lkd->lkd_dw_prev != end_offset)
	{
	    ...
	    */
		...
		&& (lkb->lkb_attribute & LKB_DEADLOCKED) == 0 )
	    {
# if defined(conf_CLUSTER_BUILD)
		if ( lkd->lkd_status & LKD_DISTRIBUTE_DEADLOCK )
		{
...
# endif /* CLUSTER_BUILD */
		...
		if ( NULL !=
		     (victim_lkb = LK_local_deadlock(lkb, lkb, 
			  lkb->lkb_request_mode, NULL, LK_main_dsmc, 0,
			  (i4*)NULL)) )
		{
		    /*
		  ...
		  {
		    TRdisplay("%@ p%d s%x LKB %x rqstid %d state %2w"
		              " selected as DEADLOCK victim\n",
			victim_lkb->lkb_lkreq.cpid.pid, 
			victim_lkb->lkb_lkreq.cpid.sid,
			victim_lkb->lkb_id, 
			victim_lkb->lkb_lkreq.request_id,
			",GR,CV,WT", victim_lkb->lkb_state);
		    victim_llb = 
		    (LLB*)LGK_PTR_FROM_OFFSET(victim_lkb->lkb_lkbq.lkbq_llb);
...
# endif /* CLUSTER_BUILD */
	...
	/* If any deadlocks, halve the sleep time */
	if (deadlocks)
	    ms_timeout /= 2;
      ...
      {
	if (queue_len)
	    TRdisplay("%@ LKdeadlock sleeping %d, deadlocks: %d, qlen: %d\n",
		ms_timeout + delta, deadlocks, queue_len);
      }
#endif /* LKDEADLOCK_DEBUG */
    } /* for (;;) */
}
...
**					Will be NULL if LK_local_deadlock
**					is called as part of a continuation
**					of a distributed deadlock check.
**	max_request_mode		Maximum (so far) of requested mode
**					for current lock, and modes of
**					those locks ahead of it.
...
*/
LKB *
LK_local_deadlock(LKB *current_lkb, LKB *top_lkb, i4 max_request_mode,
		  ...
		  i4 *must_retry)
{
    ...
    **  max_request_mode - Maximum of lock modes of lock
    ...
    **
    ** If distributed thread (inlkdsm) and rsb_mutex is owned by or
    ** is intended to be owned by the local thread, abandon this
    ** search and return "must_retry" to LKdist_deadlock_thread.
    */
    if ( inlkdsm )
    {
	...
	{
	    if ( *must_retry == 0 )
		TRdisplay("%@ LK_local_deadlock: RSB mutex conflict, must retry msg %d\n",
			inlkdsm->msg_seq);
	    (*must_retry)++;
	    return((LKB*)NULL);
	}
	...
	*/
	if ( current_lkb->lkb_state == LKB_GRANTED ||
    ...
    ** this lock.
    */
    if ( max_request_mode != LK_X && ( inlkdsm == NULL || depth > 1 ) )
    {
	/*
	...
	**
	** Also max_request_mode passed by LKdist_deadlock_thread is also
	...
	*/
	...
	if ( rsb->rsb_converters )
	{
		...
		{
		    /* 
		    ** Only INCREASE cummulative max_request_mode.
		    ** 
		    ** max_request_mode = LK_convert_mode[max_request_mode]
		    **  [next_lkb->lkb_request_mode];
		    */
		    cover_mode = LK_convert_mode[max_request_mode]
		                                [next_lkb->lkb_request_mode];
		    if ( max_request_mode < cover_mode )
		    {
			max_request_mode = cover_mode;
		    }
		    if (max_request_mode == LK_X || next_lkb == current_lkb)
			break;
		}
	    }
	}
    ...
    ** "current_lkb" is the one and only grant/convert LKB we want 
    ** to deadlock check.
    */
    if ( inlkdsm && depth == 1 && current_lkb->lkb_state != LKB_WAITING ) 
    {
	/* Start and end with current_lkb */
	next_lock_offset = LGK_OFFSET_FROM_PTR(current_lkb);
	end_offset = current_lkb->lkb_q_next;
    }
    else
    {
	/*
	** Otherwise, start at front of grant/cvt queue, go to end if
	** current_lkb is waiter, up to current_lkb if a converter.
	*/
	...
	*/
	...
	if (((LK_compatible[next_lkb->lkb_grant_mode] >> 
			max_request_mode) & 1) == 0
	      || (inlkdsm && next_lkb->lkb_state == LKB_CONVERT
                        ...
                        max_request_mode) & 1) == 0))
	{
		...
		if ( next_lkb->lkb_lkbq.lkbq_llb ==
		    ...
		    */
		    ...
		    return(top_lkb);
	              ...
	              next_llb->llb_name[1] == inlkdsm->o_tran_id.lk_ulow &&
		      next_llb->llb_name[0] == inlkdsm->o_tran_id.lk_uhigh )
	    {
		/*
	    ...
	    */
	    ...
	    {
		if (inlkdsm && lk_cluster_deadlock_debug)
		    TRdisplay("%@ LK_local_deadlock %d:%x Skipped converter"
			" dlm %x depth %d\n", lkd->lkd_csid, next_lkb->lkb_id,
			next_lkb->lkb_cx_req.cx_lock_id, depth), TRflush();
		next_lock_offset = next_lkb->lkb_q_next;
		continue;
	    }
		...
		*/
		...
		do
		{
			...
			if (inlkdsm && lk_cluster_deadlock_debug)
			    TRdisplay("%@ LK_local_deadlock: recursing from "
				"LLB %x LKB %x state %2w mode (%d,%d) "
                                "to LLB %x LKB %x state %2w mode (%d,%d), "
				"depth %d\n", *(u_i4*)&llb->llb_id,
				current_lkb->lkb_id, ",GR,CV,WT",
				current_lkb->lkb_state,
                                current_lkb->lkb_grant_mode,
				current_lkb->lkb_request_mode,
                                *(u_i4*)&next_llb->llb_id, wait_lkb->lkb_id, 
                                ",GR,CV,WT", wait_lkb->lkb_state,
                                wait_lkb->lkb_grant_mode,
				wait_lkb->lkb_request_mode, depth), TRflush();

			if ( dead_lkb = LK_local_deadlock(wait_lkb, top_lkb,
				  wait_lkb->lkb_request_mode, inlkdsm, 
	...
	} /* if (((LK_compatible[max_request_mode] >> */
	next_lock_offset = next_lkb->lkb_q_next;
...
# if defined(conf_CLUSTER_BUILD)
    ...
    ** this for us.
    **
    ** Check for remote blockers if current_lkb is not granted
    ** (either waiting or converting).
    */
    if ( lkd->lkd_status & LKD_DISTRIBUTE_DEADLOCK
...
# endif
		...
		if ( !(LK_dup_msg(lkdsm_context, pblkinfo, inlkdsm,
				  current_lkb, top_lkb, max_request_mode)) &&
		     ( inlkdsm == NULL ||
		       CX_BLKI_WAITQ != pblkinfo->cx_blki_queue ||
		       CX_ID_EQUAL( &pblkinfo->cx_blki_lkid, \
		                    &inlkdsm->o_lockid ) ) )
		{
		    /* 
		    ** Only send the message if we've not already sent
		    ** one for this resource to this node, and this
		    ** is not a waiter, except for the special case
		    ** where the waiter is the originating lock.
		    */

		    plkdsm = LK_get_free_dsm( lkdsm_context, 
		    ...
		    plkdsm->lock_cum_mode = max_request_mode;
		    plkdsm->lock_state = current_lkb->lkb_state;
		    plkdsm->tran_id.lk_uhigh = llb->llb_name[0];
		    plkdsm->tran_id.lk_ulow = llb->llb_name[1];
		    /* Set DLM lockid of blocking lock */
		    plkdsm->lockid = pblkinfo->cx_blki_lkid;
		    if ( inlkdsm )
		    {
			/* Continuation case */
			plkdsm->o_tran_id = inlkdsm->o_tran_id;
			...
			{
			    plkdsm->o_tran_id = plkdsm->tran_id;
			    plkdsm->o_stamp = top_lkb->lkb_lkreq.request_id;
			}
			else
			{
			    /* use next_llb as scratch to get top_lkb XID */
			    next_llb = (LLB *)LGK_PTR_FROM_OFFSET( \
			     top_lkb->lkb_lkbq.lkbq_llb);
			    plkdsm->o_tran_id.lk_uhigh = next_llb->llb_name[0];
			    plkdsm->o_tran_id.lk_ulow = next_llb->llb_name[1];
			    plkdsm->o_stamp = top_lkb->lkb_lkreq.request_id;
			}
		    }
		  ...
		  if (lk_cluster_deadlock_debug)
		  {
		    TRdisplay("%@ LK_local_deadlock %8d from %s: [%d of %d]\n"
			"\tLoop over blockers %d lkblks, node %x blk node %x\n"
			"\to_tran=%x%x, tran=%x%x depth %d seq %d "
			"from LLB %x LKB %x\n\tlkb_id = %x, key = %s\n"
			"\tcum_mode %d dlm %x state %2w\n",
			plkdsm->msg_seq, 
			(inlkdsm) ? "DIST" : "LOCL", 
			cnt + 1, blkcnt,
			lkd->lkd_lbk_count, plkdsm->o_node, plkdsm->o_blk_node,
			plkdsm->o_tran_id.lk_uhigh, plkdsm->o_tran_id.lk_ulow,
			plkdsm->tran_id.lk_uhigh,   plkdsm->tran_id.lk_ulow,
			depth, 
			(inlkdsm) ? lkd->lkd_stat.gdlck_call : lkd->lkd_stat.dlock_wakeups, 
			*(u_i4*)&llb->llb_id, current_lkb->lkb_id,
			plkdsm->o_lkb_id,
			LKkey_to_string(&plkdsm->lock_key, keydispbuf), 
			plkdsm->lock_cum_mode, plkdsm->o_lockid.lk_ulow,
			",GR,CV,WT", plkdsm->lock_state ); TRflush();
		  }
#endif
		}
	    }
	...
	else if ( E_CL2C25_CX_E_DLM_NOTHELD == status )
	{
	  ...
	  if (lk_cluster_deadlock_debug)
	  {
	    TRdisplay("%@ p%d s%x LKB %x rqstid %d state %2w"
		    ...
		    ",GR,CV,WT", current_lkb->lkb_state); TRflush();
	  }
#endif /* LKDEADLOCK_DEBUG */
	    /* Do nothing */;
	}
	else if ( E_CL2C21_CX_E_DLM_DEADLOCK == status )
	{
	    ...
	    */
	    ...
	    if ( !(current_lkb->lkb_attribute & LKB_DEADLOCKED) )
	    {
	      ...
	      if (lk_cluster_deadlock_debug)
	      {
	        TRdisplay("%@ p%d s%x LKB %x rqstid %d state %2w top_lkb %p"
		      " Distributed cvt-cvt deadlock victim\n",
		    current_lkb->lkb_lkreq.cpid.pid, 
		    current_lkb->lkb_lkreq.cpid.sid,
		    current_lkb->lkb_id, 
		    current_lkb->lkb_lkreq.request_id,
		    ",GR,CV,WT", current_lkb->lkb_state, top_lkb); TRflush();
	      }
#endif /* LKDEADLOCK_DEBUG */
	        if ( lkd->lkd_status & LKD_SHOW_DEADLOCKS )
...
**	queued for a transaction + lock_key + node combination.
**
** Inputs:
**      lkdsm_context		Distributed deadlock context to use.
...
i4		max_request_mode )
{
    ...
    /* Resolve originating transaction id */
    if ( inlkdsm )
	tran_id= inlkdsm->o_tran_id;
    else
    {
	llb = (LLB*)LGK_PTR_FROM_OFFSET(top_lkb->lkb_lkbq.lkbq_llb);
	tran_id.lk_uhigh = llb->llb_name[0];
	tran_id.lk_ulow  = llb->llb_name[1];
    }
	...
	/*
	** Look for a message already sent for this o_tranid,
	** lock key combination on this node.
	*/
	if ( dsm && dsm->o_tran_id.lk_uhigh == tran_id.lk_uhigh &&
		    dsm->o_tran_id.lk_ulow  == tran_id.lk_ulow &&
...
i4                  mode)
{
    ...
    */
	...
	if (lkb->lkb_attribute & (LKB_DEADLOCKED | LKB_ENQWAIT))
	    continue;
    ...
    CL_ERR_DESC	    *sys_err)
{
	...
	*status = lkreq->status;
    ...
    CKP_TXN", lkreq->lock_key.lk_type,
	    ...
	    "N,IS,IX,S,SIX,U,X", lkreq->lock_mode,
	    lkb_id,
	    lkreq->status);
...
**	20-Mar-2007 (jonj)
**	    If syncgrant, neither llb_mutex nor rsb_mutex
**	    are held; otherwise both are.
...
lk_handle_ivb( LLB *llb, LKB *lkb, bool syncgrant, i4 waitflags, i4 timeout, i4 lineno )
{
    ...
    */
    if ( OK == CXalter( CX_A_IVB_SEEN, (PTR)1 ) )
    {
	/*
	...
	*/
	if ( (lkb->lkb_state == LKB_CONVERT && rsb->rsb_converters > 1) ||
	     (lkb->lkb_state == LKB_WAITING && rsb->rsb_waiters > 1) )
	{
	    TRdisplay("%@ lk_handle_ivb %d: (ERR ) status %x LKB %x LLB %x state %2w Gr %w Rq %w c:%d w%d KEY %s\n",
		    lineno,
		    status,
		    lkb->lkb_id,
		    *(u_i4*)&llb->llb_id,
		    ",GR,CV,WT", lkb->lkb_state,
		    "N,IS,IS,S,SIX,U,X", lkb->lkb_grant_mode,
		    "N,IS,IS,S,SIX,U,X", lkb->lkb_request_mode,
	...
	{
	    TRdisplay("%@ lk_handle_ivb %d: (undo) status %x LKB %x LLB %x state %2w Gr %w Rq %w KEY %s\n",
		    lineno,
		    status,
		    lkb->lkb_id,
		    *(u_i4*)&llb->llb_id,
	...
	*/
	...
	TRdisplay("%@ lk_handle_ivb %d: (redo) status %x LKB %x LLB %x state %2w Gr %w Rq %w KEY %s\n",
		lineno,
		status,
		lkb->lkb_id,
		*(u_i4*)&llb->llb_id,
...
lk_stall_request( LKB *lkb, i4 lineno )
{
	...
	{
	    lkb->lkb_lkreq.result_flags |= LKREQ_STALLED_DURING_REQUEST;
	    ...
	    */
	    ...
	    TRdisplay("%@ %d Stalling LKB %x LLB %x state %2w Gr %w Rq %w KEY %s, flags %v\n",
		lineno,
		lkb->lkb_id,
		*(u_i4*)&llb->llb_id,
		",GR,CV,WT", lkb->lkb_state,
		...
		"N,IS,IS,S,SIX,U,X", lkb->lkb_request_mode,
		LKkey_to_string(&rsb->rsb_name, keydispbuf),
		"UPD_VALUE,UPD_LOCKID,VALADDR_PROVIDED,CHECK_STATUS,P\
HYS,STALL_RQ,STALL_CV,DEALL_CB",
		lkb->lkb_lkreq.result_flags);
		...
		*/
		    ...
		    lkb->lkb_lkbq.lkbq_rsb == LGK_OFFSET_FROM_PTR(rsb))
		{
		    ...
		    */
		    ...
		    cback_fcn = lkb->lkb_cback_fcn;
		    if ( lkb->lkb_attribute & LKB_DISTRIBUTED_LOCK )
		    {
			if ( lkb->lkb_cback_off )
			{
			    cback =
			     (CBACK *)LGK_PTR_FROM_OFFSET(lkb->lkb_cback_off); 
			    cback_arg = cback->cback_arg;
			}
		    }
		    else
		    {
			cback_arg = lkb->lkb_cback_arg;
		    }
		    ...
		    status = LK_request(flags | (LK_PROPAGATE | LK_NOWAIT |
						LK_NODEADLOCK), 
			llb_id, new_lock_key,
			lock_mode, (LK_VALUE *)0, (LK_LKID *)0, (i4) 0,
			sys_err, &sleep_mask, cback_fcn, cback_arg,
			&completion_handle);
...
# endif /* PARANOIA */
    ...
    do
    {
	cback_off = lkb->lkb_cback_off;
    } while ( CScas4( &lkb->lkb_cback_off, cback_off, 0 ) );
	     
    if ( cback_off )
    {
	cback = (CBACK *)LGK_PTR_FROM_OFFSET(cback_off);

# ifdef	PARANOIA
	if ( CBACK_TYPE != cback->cback_type )
	{
	    TRdisplay( "%@ LK_AST_callback: bad CBACK (%p) for lkb at %p\n",
		       cback, lkb );
	    return;
	}
# endif /* PARANOIA */

	cback->cback_blocked_mode = blocked_mode;
	lk_cback_post( lkb->lkb_cbt_id, cback, 1 );
    }
}
...
*/
static STATUS
lk_set_dlm_callback( LKB *lkb )
{
    STATUS	 status;
    CBACK	*cback;

    if ( NULL != lkb->lkb_cback_fcn )
    {
	/*
	** For distributed locks, we need to pessimistically
	** pre-allocate CBACK structure, since callback is
	** triggered at the SH/AST level, and currently
	** allocating an LK CB involves taking a mutex,
	** which is forbidden to SH/AST code.
	*/
	if ( 0 == lkb->lkb_cback_off )
	{
	    status = lk_cback_alloc( lkb, &cback );
	    if ( OK != status )
	    {
		return status;
	    }
	    ...
	    /*
	    ** Now overload space which formerly held callback
	    ** argument with offset to CBACK structure.
	    */
	    lkb->lkb_cback_off = LGK_OFFSET_FROM_PTR(cback);
	}
	else
	{
	    LK_CALLBACK_TRACE( 10, &lkb->lkb_cx_req.cx_key );
	    cback = (CBACK*)LGK_PTR_FROM_OFFSET(lkb->lkb_cback_off);
# ifdef	PARANOIA
	    if ( 0 != MEcmp( (PTR)&cback->cback_key, \
			     (PTR)&lkb->lkb_cx_req.cx_key, \
			     sizeof(LK_LOCK_KEY) ) )
	    {
		LK_CALLBACK_TRACE( 99, &cback->cback_key );
	    }
# endif /*PARANOIA*/

	    cback->cback_cback = lkb->lkb_cback_fcn;
	    cback->cback_arg = lkb->lkb_cback_arg;
	}
