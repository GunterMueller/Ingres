...
CL_ERR_DESC	*sys_err)
{
	...
	status = LK_cancel(flag, lock_list_id, (SIZE_TYPE *)0,
		    (LK_COMP_HANDLE *)0, sys_err);
...
CL_ERR_DESC	    *sys_err)
{
	...
	(i4)input_list_id->id_id > lkd->lkd_lbk_count )
    {
	ule_format(E_CL1033_LK_EVENT_BAD_PARAM, (CL_ERR_DESC *)0, ULE_LOG,
    ...
    {
	ule_format(E_CL1034_LK_EVENT_BAD_PARAM, (CL_ERR_DESC *)0, ULE_LOG,
    ...
    ** same time; LK_E_SET must be separate.
    */
    if (flag & LK_E_SET)
    {
	...
	if (llb->llb_status & LLB_ESET)
	{
	    ule_format(E_CL1035_LK_EVENT_BAD_PARAM, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 2,
...

/*{
** Name: LK_resume	- Resume a lock list.
**
** Description:
**      This routine is called to mark a LLB as active and queue an 
**	AST to the process to wake it up.
**
** Inputs:
**      lkb                             The LKB to resume.
**
** Outputs:
**	Returns:
**	    VOID
...
LK_resume(LKB *lkb)
{
    ...
    */
    ...
    /* This is a bad thing to do without holding llb_mutex */
	...
	*/
    ...
    else if (lkreq->result_flags & LKREQ_VALADDR_PROVIDED)
    {
	if (rsb->rsb_invalid)
	    lkreq->status = LK_VAL_NOTVALID;
	/*
	** This value will be retrieved from the lkb when the lkrequest
	** caller is awoken.
	*/
	lkreq->value.lk_high = rsb->rsb_value[0];
	lkreq->value.lk_low = rsb->rsb_value[1];
	lkreq->value.lk_mode = lkb->lkb_grant_mode;
...
**	could not be granted immediately.  This routine fills in the llb
**	with the information needed to reschedule this lock list.  The
**	information needed to reschedule is formatted as a VMS AST control
**	block.  The resume function queues the ACB for execution which
**	will cause the target process to be activated.
**
**	The proc_llb contains information that describes the process to
**	suspend.  The acb should be filled in with information from this
**	rather than the lock list on which the lkb resides, since that list
**	may be shared by multiple processes.
**
** Inputs:
**      lkb                             The LKB to suspend.
**	proc_llb			Lock list owned by suspend process.
**
** Outputs:
**	Returns:
**	    VOID
