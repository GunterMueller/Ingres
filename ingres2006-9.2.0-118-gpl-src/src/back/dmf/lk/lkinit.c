...
LKinitialize(CL_ERR_DESC *sys_err, char *lgk_info)
{
    ...
    if (status = LGK_initialize(0, sys_err, lgk_info))
    {
    ...
    CXalter( CX_A_PROXY_OFFSET, (PTR)(i4)(&((LKB *)0)->lkb_lkreq.cpid) );
...
*/
/*{
** Name: LK_do_rundown	- Rundown LK for a process.
**
** Description:
...
LK_rundown(i4 pid)
{
	...
	if ((llb->llb_type == LLB_TYPE) && llb->llb_cpid.pid == pid)
	{
	    ...
	    */
		...
		if ( llb->llb_type == LLB_TYPE )
		{
		    ...
		    if ((status = LK_cancel_event_wait( llb, lkd, TRUE, &sys_err)))
		    {
			TRdisplay("Panic: LK_cancel_event_wait() failed. Status=%d.\n",status);
			return;
		    }
		...
		)
	{
	    ...
	    */
	    ...
	    (VOID) LK_mutex(SEM_SHARE, &rsb->rsb_mutex);
		...
		if (owning_llb->llb_cpid.pid == pid)
		{
		    ...
		    acb = llb->llb_wait->lkb_acb;
		    llb->llb_wait->lkb_acb = 0;
		    acb->acb_status = LK_RETRY;
		    vms_qast(acb);
#endif
		    /*
    ...
    {
	(VOID) LK_mutex(SEM_EXCL, &xsvdb_llb->llb_mutex);
...
free_lock_list(LLB *llb, LLB **shared_llb, LLB **related_llb)
{
	...
	*/
	...
	(VOID)LK_do_unlock(lkb, (LK_VALUE *) 0, LK_DO_UNLOCK_RUNDOWN);
    }
    ...
    */
    ...
    {
	sllb = (LLB *)
		LGK_PTR_FROM_OFFSET(llb->llb_shared_llb);
    ...
    */
	...
	(VOID) LK_mutex(SEM_EXCL, &rllb->llb_mutex);
