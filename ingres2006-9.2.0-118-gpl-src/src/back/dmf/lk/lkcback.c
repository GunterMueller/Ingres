...
**	lk_cback_alloc()	      -  Allocate & fill CBACK struct.
**	lk_cback_dealloc()	      -  Unhook & toss a pessimistically
**					 allocated CBACK struct.
...
# define LK_CBTRACE_ENTRY_MAX	1024
...
lk_callback(DMC_CB	    *dmc_cb)
{
    ...
    TRdisplay("%@ Starting LK Blocking Callback Thread for server id 0x%x\n",
	dmc_cb->dmc_id);

    /* Find the callback thread descriptor from offset */
    cbt = (CBT *)LGK_PTR_FROM_OFFSET(LK_mycbt);

    if (cbt->cbt_type != CBT_TYPE)
    {
	ule_format(E_CL106A_LK_CBACK_REG_BADCBT, (CL_ERR_DESC *)0,
    ...
    **   reawakened.
    */
    while (status == E_DB_OK && cl_status == OK)
    {
	...
	*/
	...
	{
	    /*
	    ** Unhook current CBACK list.  CBACK list is maintained
	...
	if ( !cback_offset )
	{
		...
		if (clustered)
		    cl_status = CSsuspend_for_AST(suspmask, timeout, NULL);
		else
	...
	*/
	...
	for ( ; /* something to break out off */ ; )
	{
	    ...
	    if (cback->cback_type != CBACK_TYPE)
	    {
		ule_format(E_CL1064_LK_CBACK_GET_BADCBACK, (CL_ERR_DESC *)0,
			   ULE_LOG, NULL, 0, 0, 0, &err_code, 1,
			   0, cback->cback_type);
		status = E_DB_ERROR;
		break;
	...
	*/
	while ( OK == status )
	{
	    ...
	    LK_CALLBACK_TRACE( 2, &cback->cback_key );

	    /* Invoke callback */ 
	    lkd->lkd_stat.cback_cbacks++;
	    cbt->cbt_stat.cbacks++;
	    status = (*cback->cback_cback)
			(cback->cback_blocked_mode,
			 cback->cback_arg,
			 &cback->cback_lkb_id,
			 &cback->cback_key);
		    
	    LK_CALLBACK_TRACE( 3 + status, &cback->cback_key );
	    if (status == E_DB_INFO)
	    {
	    ...
	    {
		/* Deallocate the CBACK */
		LK_deallocate_cb(CBACK_TYPE, (PTR) cback, (LLB*)NULL);
	    }
	    ...
	    */
	    ...
	    cback = (CBACK *) LGK_PTR_FROM_OFFSET(next_offset);
	    cback_offset = next_offset;
	}
    } /* end-while (main loop) */
    ...
    {
	ule_format(cl_status, (CL_ERR_DESC *)0,
...
lk_cback_init()
{
    ...
    */ 
    if ((cbt = (CBT *)LK_allocate_cb(CBT_TYPE, (LLB *)0)) == 
				(CBT *)0)
    {
...

/*
** Name: lk_cback_alloc - Allocate & initialize a CBACK.
**
** Description:
...
**      lock request. This routine allocates and initialized the CBACK
**	structure.
**
** Inputs:
**      LKB lkb                     - lock descriptor for lock needing CBACK
**	CBACK pcback		    - Ptr. to ptr. to hold new CBACK addr.
** Outputs:
**	None.
**
** Side Effects:
**      A CBACK structure is allocated and filled in.
**
** Returns:
**	STATUS
**
...
*/
STATUS 
lk_cback_alloc( LKB *lkb, CBACK **pcback)
{
    RSB		 *rsb;
    i4      	  err_code;
    CBACK	 *cback;
    ...
    /*
    ** Allocate a CBACK structure for the callback info.
    */ 
    if ((*pcback = cback = (CBACK *)LK_allocate_cb(CBACK_TYPE, (LLB *)0)) == 
		(CBACK *)0)
    {
        ule_format(E_DMA00E_LK_NO_RSBS, (CL_ERR_DESC *)0, ULE_LOG,
                   NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
    ...
    cback->cback_type = CBACK_TYPE;
    cback->cback_blocked_mode = 0;
    cback->cback_cback = lkb->lkb_cback_fcn;
    cback->cback_arg = lkb->lkb_cback_arg;
    cback->cback_lkb_id.lk_unique = lkb->lkb_id;
    cback->cback_lkb_id.lk_common = rsb->rsb_id;
    cback->cback_key.lk_type = rsb->rsb_name.lk_type;
    cback->cback_key.lk_key1 = rsb->rsb_name.lk_key1;
    cback->cback_key.lk_key2 = rsb->rsb_name.lk_key2;
    cback->cback_key.lk_key3 = rsb->rsb_name.lk_key3;
    cback->cback_key.lk_key4 = rsb->rsb_name.lk_key4;
    cback->cback_key.lk_key5 = rsb->rsb_name.lk_key5;
    cback->cback_key.lk_key6 = rsb->rsb_name.lk_key6;
 
    return(OK);
}

/*
** Name: lk_cback_dealloc - Deallocate (Free) a CBACK.
**
** Description:
**	Clustered Ingres pessimistically allocates a CBACK, just in case
**	it is needed, because it cannot do the allocation within the
**	signal handler/AST that handles the blocking notification.
**	This reserved CBACK struct is referenced by lkb_cback_off.
**
**	Since it is possible (although VERY unlikely), that a blocking
**	notification is received just as we are about to free the
**	reserved CBACK, the unhook and release needs to be done
**	atomically, to remove any chance of a freed CBACK winding up
**	on the cbt_cback_list list.
**
**	Since this operation is slightly tricky, and appears in more than 
**	one place, it has been made a sub-routine.
**
** Inputs:
**      LKB	lkb          - lock descriptor with doomed CBACK.
**
** Outputs:
**	None.
**
** Side Effects:
**	lkb_cback_off is zeroed.
**      CBACK structure is returned to available pool.
**
** Returns:
**	none.
**
...
lk_cback_dealloc( LKB *lkb )
{
	...
	cback_off = lkb->lkb_cback_off;
	      ...
	      CScas4(&lkb->lkb_cback_off, cback_off, 0) );
#else
    {
	CBT	*cbt;
    ...
    if ( cback_off )
    {
	/* Dispose of unwanted callback */
	cback = (CBACK*)LGK_PTR_FROM_OFFSET(cback_off);

	LK_CALLBACK_TRACE( 14, &cback->cback_key );
# ifdef	PARANOIA
	if ( 0 != MEcmp( (PTR)&cback->cback_key, \
			 (PTR)&lkb->lkb_cx_req.cx_key, \
			 sizeof(LK_LOCK_KEY) ) )
	{
	    LK_CALLBACK_TRACE( 95, &lkb->lkb_cx_req.cx_key );
	}
# endif /*PARANOIA*/
	LK_deallocate_cb(CBACK_TYPE, (PTR)cback, (LLB*)NULL);
    }
...
**	CBACK cback		    - Callback struct to fill out & queue.
** Outputs:
**	None.
**
** Side Effects:
...
*/
STATUS 
lk_cback_post(SIZE_TYPE cbt_id, CBACK *cback, i4 from_AST )
{
    CBT          *cbt;
    i4      	 err_code;
    i4		 triggered;
    ...
    if ( cbt_id != LK_mycbt && from_AST )
    {
	/* Trace disparity, force LK_mycbt */
	LK_CALLBACK_TRACE( 87, &cback->cback_key );
	cbt_id = LK_mycbt;
    }

    /* Check that lkb has a valid cbt descriptor */
    cbt = (CBT *) LGK_PTR_FROM_OFFSET(cbt_id);

    if (cbt->cbt_type != CBT_TYPE)
    {
	ule_format(E_CL1065_LK_CBACK_PUT_BADCBT, (CL_ERR_DESC *)0,
	...
	LK_CALLBACK_TRACE( cbt->cbt_triggered, &cback->cback_key );
...
lk_cback_put(i4 blocked_mode, LKB *lkb)
{
    ...
    */ 
	...
	cback->cback_blocked_mode = blocked_mode;
        status = lk_cback_post( lkb->lkb_cbt_id, cback, 0 );
    }
    return status;
}

...
lk_cback_fire(RSB *rsb, i4  blocked_lock_mode)
{
    ...
    {
	ule_format(E_CL1067_LK_CBACK_FIRE_BADRSB, (CL_ERR_DESC *)0,
         ...
         lkb_offset = next_lkb->lkb_q_next)
    {
        ...
        if (next_lkb->lkb_cback_fcn == (LKcallback) NULL ||
	    ...
	    */
        ...
        if (status)
        {
            ule_format(status, (CL_ERR_DESC *)0,
                       ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
            status =  E_CL1068_LK_CBACK_FIRE_FAILED;
            break;
        }
        /* Reset LKBs callback */
        next_lkb->lkb_cback_fcn = (LKcallback) NULL;
        rsb->rsb_cback_count--;
    }
    return(status);
}
