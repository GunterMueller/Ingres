...
**
**	On VMS systems, it also disables ASTs during LKmutex operations. This
**	is needed in the DMFCSP process, where LK code can run both at normal
**	user mode level and as called from user mode ASTs. In order to prevent
**	LK from re-entering itself (which it can't handle), we are experimenting
**	with disabling AST's during LK internal code operation.
**
** Inputs:
**	mode				SEM_SHARE or
**					SEM_EXCL
**	amutex				semaphore to latch in that mode
	...
	i4 	 linenum) 
{
	...
	ule_format(E_DMA009_LK_MUTEX, (CL_ERR_DESC *)0, ULE_LOG,
  ...
  Test_set: %d\n",
	      ...
	      amutex->cs_pid,
	      amutex->cs_test_set);
	    ule_format(0, (CL_ERR_DESC *)0, ULE_MESSAGE, NULL,
			xtra_msg, STlength(xtra_msg), (i4 *)0,
			&err_code, 0);
	}
#endif
#ifdef LK_MUTEX_TRACE
...
*/
STATUS
LK_unmutex_fcn(CS_SEMAPHORE *amutex,
	char	*filename,
	i4	linenum)
{
#ifdef VMS
	...
	ule_format(E_DMA00A_LK_UNMUTEX, (CL_ERR_DESC *)0, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, &err_code, 3,
		    0, sstat.name,
		    0, filename,
		    0, linenum);
...
*/
STATUS
LK_imutex(CS_SEMAPHORE *amutex, char *mutex_name)
{
    STATUS      status;
    i4	err_code;
	...
	ule_format(status, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
	ule_format(E_DMA00B_LK_IMUTEX, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
    }
    else
    {
		...
		char trace_type)
{
	...
	** printed.  Use ule_format() here to get node/server/session/time
	...
	*/
	...
	ule_format(0, (CL_ERR_DESC *)0, ULE_MESSAGE, NULL,
	    line, STlength(line), (i4 *)0, &error, 0);
	STprintf(line,
...
"%@ LKmutex_trace: failed to allocate %d bytes for trace queue; status was:\n",
		    trace_size);
		ule_format(0, (CL_ERR_DESC *)0, ULE_MESSAGE, NULL,
		    line, STlength(line), (i4 *)0, &err_code, 0);
		ule_format(status, (CL_ERR_DESC *)0, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
