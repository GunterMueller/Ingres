...
LLB *l)
{
	...
	case CBACK_TYPE:
	    LK_WHERE("LK_allocate_cb (CBACK)")
	    /*
	    ** Allocate a CBACK from the rbk pool 
	    **
	    ** CBACKs have their own free list
	    */

	    if ((status = LK_mutex(SEM_EXCL, &lkd->lkd_rbk_mutex)) == 0)
	    {
		if ((cb_offset = lkd->lkd_cback_free) == 0 &&
		    (cb_offset = expand_list(CBACK_TYPE, (LLB *)0)) == 0)
		{
		    (VOID)LK_unmutex(&lkd->lkd_rbk_mutex);
		    TRdisplay("LK_allocate_cb: expand_list failed (CBACK)\n");
		    break;
		}

		cback = (CBACK *)LGK_PTR_FROM_OFFSET(cb_offset);

		lkd->lkd_cback_free = cback->cback_q_next;
...
LK_deallocate_cb( i4 type, PTR cb, LLB *l )
{
	...
	case CBACK_TYPE:
	    LK_WHERE("LK_deallocate_cb (CBACK)")

	    /*	Deallocate the CBACK. */

	    cback = (CBACK *)cb;

	    cback->cback_q_prev = 0;
	    cback->cback_type = 0;

	    if ((status = LK_mutex(SEM_EXCL, &lkd->lkd_rbk_mutex)) == 0)
	    {
		cback->cback_q_next = lkd->lkd_cback_free;
		lkd->lkd_cback_free = LGK_OFFSET_FROM_PTR(cback);
		(VOID)LK_unmutex(&lkd->lkd_rbk_mutex);
	    }
...
LLB *l)
{
    ...
    switch (type)
    {
        case LKB_TYPE:
	     if (l && (l->llb_status & (LLB_MASTER | LLB_RECOVER)))
	     {
	         ...
	         {
		    ule_format(E_CL102F_LK_EXPAND_LIST_FAILED, (CL_ERR_DESC *)0,
        ...
        case RSB_TYPE:
	     ...
	     {
	         ule_format(E_DMA024_LK_EXPAND_LIST_FAILED, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, &err_code,
        ...
        case LLB_TYPE:
	     ...
	     {
	         ule_format(E_CL1030_LK_EXPAND_LIST_FAILED, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, &err_code,
    ...
    */
	...
	if (items == NULL && ((count >>= 1) == 0))
	{
	    ...
	    ule_format(E_CL102D_LK_EXPAND_LIST_FAILED, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, 0L, (i4 *)0, &err_code,
			3, 0, type, 0, lkd->lkd_lbk_count, 0, lkd->lkd_sbk_count);
	    return (0);
	}
    } while (items == NULL);
	...
	*/
	   ...
	   ) 
	{
	    ...
	    */
	    if ( !sem && l->llb_tick )
	    {
		sem = (CS_SEMAPHORE*)LGK_PTR_FROM_OFFSET(l->llb_tick);
		LK_unmutex(sem);
	    }
