...
static MO_CLASS_DEF LKmo_lkd_classes[] =
{
	...
	MO_SIZEOF_MEMBER(LKD, lkd_stat.request_new), MO_READ, 0,
	CL_OFFSETOF(LKD, lkd_stat.request_new),
	MOintget, MOnoset, (PTR)0, MOcdata_index
    },
    {0, "exp.dmf.lk.lkd_stat.request_convert",
	MO_SIZEOF_MEMBER(LKD, lkd_stat.request_convert), MO_READ, 0,
	CL_OFFSETOF(LKD, lkd_stat.request_convert),
	MOintget, MOnoset, (PTR)0, MOcdata_index
    },
    {0, "exp.dmf.lk.lkd_stat.convert",
	MO_SIZEOF_MEMBER(LKD, lkd_stat.convert), MO_READ, 0,
	CL_OFFSETOF(LKD, lkd_stat.convert),
	MOintget, MOnoset, (PTR)0, MOcdata_index
    },
    {0, "exp.dmf.lk.lkd_stat.release",
	MO_SIZEOF_MEMBER(LKD, lkd_stat.release), MO_READ, 0,
	CL_OFFSETOF(LKD, lkd_stat.release),
	MOintget, MOnoset, (PTR)0, MOcdata_index
    },
	...
	MO_SIZEOF_MEMBER(LKD, lkd_stat.wait), MO_READ, 0,
	CL_OFFSETOF(LKD, lkd_stat.wait),
	MOintget, MOnoset, (PTR)0, MOcdata_index
    },
    {0, "exp.dmf.lk.lkd_stat.convert_wait",
	MO_SIZEOF_MEMBER(LKD, lkd_stat.convert_wait), MO_READ, 0,
	CL_OFFSETOF(LKD, lkd_stat.convert_wait),
	MOintget, MOnoset, (PTR)0, MOcdata_index
    },
	...
	MO_SIZEOF_MEMBER(LKD, lkd_stat.convert_deadlock), MO_READ, 0,
	CL_OFFSETOF(LKD, lkd_stat.convert_deadlock),
	MOintget, MOnoset, (PTR)0, MOcdata_index
    },
	...
	MO_SIZEOF_MEMBER(LKD, lkd_stat.deadlock), MO_READ, 0,
	CL_OFFSETOF(LKD, lkd_stat.deadlock),
	MOintget, MOnoset, (PTR)0, MOcdata_index
    },
...
LK_lkb_llb_get( i4  offset, i4  size, PTR object, i4  lsbuf, char *sbuf )
{
    ...
    {
	cl_stat = MOulongout( MO_VALUE_TRUNCATED,
			     (u_i4)llb->llb_id.id_id, lsbuf, sbuf );
    }
...
LK_lkb_cx_lock_id_get( i4  offset, i4  size, PTR object, i4  lsbuf, char *sbuf )
{
    ...
    CX_EXTRACT_LOCK_ID(&lkb->lkb_cx_req.cx_lock_id, &lock_id);
...
char *userbuf )
{
    ...
    TRformat(NULL, 0, format_buf, sizeof(format_buf), "%v",
	    "AST_ENABLE,CLUSTER_SYSTEM,CSP_RUNDOWN,CSP_IN_COMPLETION",
	    lkd->lkd_status);
...
char *userbuf )
{
    ...
    TRformat(NULL, 0, format_buf, sizeof(format_buf), "%v",
	    "WAITING,NONPROTECT,ORPHANED,EWAIT,RECOVER,MASTER,ESET,EDONE,\
NOINTERRUPT,SHARED,PARENT_SHARED,ENQWAITING,GDLK_SRCH,ST_ENQ,MASTER_CSP,\
ALLOC,,XSVDB,XROW,MULTITHREAD",
	    status);
    ...
    i4  offset, i4  size, PTR object, i4  lsbuf, char *sbuf )
{
	...
	return (MOulongout( MO_VALUE_TRUNCATED,
			    (u_i4)related_llb->llb_id.id_id, lsbuf, sbuf));
    }
    else
    {
	return (MOulongout( MO_VALUE_TRUNCATED,
...
LKmo_llb_wait_id_get( i4  offset, i4  size, PTR object, i4  lsbuf, char *sbuf )
{
	...
	return (MOulongout( MO_VALUE_TRUNCATED,
			    (u_i4)wait_lkb->lkb_id, lsbuf, sbuf));
    }
    else
    {
	return (MOulongout( MO_VALUE_TRUNCATED,
...
char *userbuf )
{
    ...
    TRformat(NULL, 0, format_buf, sizeof(format_buf), "%v",
	    "PHYSICAL,TEMPORARY,CANCEL,ENQWAIT,CONCANCEL",
	    lkb->lkb_attribute);
...
char *userbuf )
{
    ...
    TRformat(NULL, 0, format_buf, sizeof(format_buf), "%w",
	    "N,IS,IX,S,SIX,U,X",  rsb->rsb_grant_mode);
...
char *userbuf )
{
    ...
    TRformat(NULL, 0, format_buf, sizeof(format_buf), "%w",
	    "N,IS,IX,S,SIX,U,X",  rsb->rsb_convert_mode);
...
char *userbuf )
{
	...
	case LK_ROW:
	    TRformat(NULL, 0, format_buf, sizeof(format_buf),
		    "KEY(%w,DB=%x,TABLE=[%d,%d],PAGE=%d,LINE=%d)",
		    lock_key_string,
		    rsb->rsb_name.lk_type,
		    rsb->rsb_name.lk_key1, rsb->rsb_name.lk_key2,
		    (rsb->rsb_name.lk_key2 < 0)
        ...
        case LK_VAL_LOCK:
            TRformat(NULL, 0, format_buf, sizeof(format_buf),
                    "KEY(%w,DB=%x,TABLE=[%d,%d],VALUE=%d,%d,%d)",
                    lock_key_string,
                    rsb->rsb_name.lk_type,
                    rsb->rsb_name.lk_key1, rsb->rsb_name.lk_key2,rsb->rsb_name.lk_key3,
