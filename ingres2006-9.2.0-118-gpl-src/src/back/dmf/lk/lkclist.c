...
CL_ERR_DESC		*sys_err)
{
	...
	{
	    ule_format(E_CL102A_LK_CREATE_BADPARAM, (CL_ERR_DESC *)0, ULE_LOG,
	...
	{
	    ule_format(E_CL102B_LK_CREATE_BADPARAM, (CL_ERR_DESC *)0, ULE_LOG,
    ...
    {
	ule_format(E_CL102C_LK_CREATE_BADPARAM, (CL_ERR_DESC *)0, ULE_LOG,
    ...
    */
    ...
    if ((llb = (LLB *)LK_allocate_cb(LLB_TYPE, (LLB *)0)) == 0)
    {
	ule_format(E_DMA011_LK_NO_LLBS, (CL_ERR_DESC *)0, ULE_LOG,
			    NULL, (char *)0, 0L, (i4 *)0, &err_code, 0);
	return (LK_NOLOCKS);
    }
    ...
    */
	    ...
	    ((unique_id->lk_uhigh == 0) && ((flags & LK_CONNECT) == 0)))
	{
	    ...
	    ule_format(E_CL1031_LK_BAD_UNIQUE_ID, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 2,
			0, unique_id,
    ...
    */
    ...
    {
	ule_format(local_status, &local_sys_err,
	...
	if (llb->llb_name[0] == next_llb->llb_name[0] && 
		...
		*lock_list_id = *(LK_LLID *)&next_llb->llb_id;
	    ...
	    ule_format(E_CL1032_LK_DUPLICATE_LOCK_ID, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 2,
    ...
    */
    ...
    {
	(VOID) LK_unmutex(&lkd->lkd_llb_q_mutex);
	ule_format(E_CL1050_LK_LOCKID_NOTFOUND, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 3,
    ...
    */
    ...
    if ( (flags & (LK_MULTITHREAD | LK_SHARED)) == LK_MULTITHREAD )
	llb->llb_status |= LLB_MULTITHREAD;

# if 0  /* s103715 */
//    /*
//    ** Originally we designed an LK_MASTER_CSP flag for the caller of
//    ** LKcreate_list to use to specify that this lock list is being used by the
//    ** CSP. However, lock lists get created all over the place, and often it's
//    ** by code which doesn't know whether it's running in the CSP or not.
//    ** Therefore, since LK *does* know whether this is the CSP process or not,
//    ** we have the following code simply treat any lock list created by the
//    ** CSP as an LLB_MASTER_CSP lock list. This means that the LK_MASTER_CSP
//    ** flag is not tremendously useful anymore, and perhaps someday it should
//    ** get removed...
//    */
//    if ( llb->llb_cpid.pid == lkd->lkd_csp_id )
//	llb->llb_status |= LLB_MASTER_CSP;
# endif /* s103715 */
	...
	*/
	...
	else
	{
		...
		if (llb->llb_name[0] == proc_next_llb->llb_name[0] && 
		    llb->llb_name[1] == proc_next_llb->llb_name[1])
		{
		    ule_format(E_CL1051_LK_DUPLICATE_LOCK_ID,
	...
	*/
	...
	shared_llb->llb_status |= LLB_SHARED;
	...
	** list.  Mark the llb as a shared reference and point it to the
...
CL_ERR_DESC		*sys_err)
{
    ...
    {   
	ule_format(E_CL106D_LK_CONNECT_BADPARAM, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 2,
    ...
    {
	ule_format(E_CL106E_LK_CONNECT_BADPARAM, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 3,
	...
	if ( (sllb = (LLB*)LK_allocate_cb(LLB_TYPE, (LLB*)0)) == 0 )
	{
	    ule_format(E_DMA011_LK_NO_LLBS, (CL_ERR_DESC*)0, ULE_LOG,
			NULL, (char*)0, 0L, (i4*)0, &err_code, 0);
	...
	*/
	...
	sllb->llb_status  = cllb->llb_status;
	...
	*/
	...
	{
	    ule_format(E_CL106F_LK_CONNECT_BADPARAM, (CL_ERR_DESC *)0, ULE_LOG,
	...
	{
	    ule_format(E_CL1070_LK_CONNECT_BADPARAM, (CL_ERR_DESC *)0, ULE_LOG,
			NULL, (char *)0, 0L, (i4 *)0, &err_code, 3,
			0, connect_to_id->id_id, 
			0, sllb->llb_type, 
			0, sllb->llb_status);
    ...
    */
    ...
    if ( sllb->llb_status & LLB_MULTITHREAD )
