...
struct _LFB
{
    i4	    	    lfb_next;           /* Next youngest transaction. */
    i4	    	    lfb_prev;		/* Next oldest transaction. */
...
#define		    LGD_II_DUAL_LOG	0x02
    ...
    SIZE_TYPE	    lfb_first_lbb;	/* 1st LLB in contiguous alloc. ext. */
...
struct _LDB
{
    SIZE_TYPE	    ldb_next;          /* First transaction in this database.*/
    SIZE_TYPE	    ldb_prev;	        /* Last transaction in this database. */
...
#define			LDB_CLOSE_WAIT	    0x00200000
    int             ldb_lxbo_count;     /* Number of ongoing xact for ckpdb */
    int		    ldb_lxb_count;	/* Number of transactions. */
    SIZE_TYPE	    ldb_lfb_offset;	/* logfile for this transaction */
...
#define		LGD_OPTIMWRITE_TRACE	0x000002L /* Trace optimized writes */
    ...
    LWQ		    lgd_wait_buffer;	/* Free buffer wait queue. */
...
FUNC_EXTERN	VOID	LG_resume(LWQ *lwq, i4 status);
