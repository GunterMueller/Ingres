...
typedef struct
{
    ...
    i4		request_id;		/* Unique identifier of request */
    /*b90388*/
    CS_CPID	    cpid;
...

/*}
** Name: LKREQ - LK input parameters block.
**
** Description:
**      This structure contains LK input parameters. It is used
**	by the VAXCluster locking system to pass information to the CSP which
**	performs the real lk_request, lk_release, and lk_cancel operations.
**
** History:
**	Summer, 1992 (bryanp)
**	    Working on the new portable logging and locking system.
...
**	    Added pid field to LKREQ structure to trace Process ID of requestor.
**	21-jun-1993 (bryanp)
**	    Added LKREQ_RCP_IS_ONLINE for CSP/RCP process linkage.
**	    Add special LKREQ message which triggers the CSP into the debugger.
**	26-jul-1993 (bryanp)
**	    Define LKREQ_STALLED_CONVERSION for use by cluster lock stalling.
**	08-sep-93 (swm)
...
**	11-sep-96 (nick)
**	    Define new result flag LKREQ_CB_DEALLOCATED which is set when
**	    the resources which were specified as 'free on fail' are
**	    deallocated.
**	22-Apr-1997 (jenjo02)
**	    Removed nick's LKREQ_CB_DEALLOCATED flag. When the LKB itself
**	    is one of the deallocated resources it cannot be viewed
...
**	    reallocated to another thread!
**      05-Nov-98 (hanal04)
**          Define new result flag LKREQ_COLLECTED which is set when
**          the LKREQ has been used to obtain the the completion
**          status. b90388. Crossed from 1.2/01.
**	02-Aug-1999 (jenjo02)
**	    Added blocking_cpid in which to stash the blocker's id
**	    until we need it.
**	20-Aug-2002 (jenjo02)
**	    Added blocking_mode.
*/
typedef struct _LKREQ
{
    i4		    function;		/* which function to request */
#define	LKREQ_REQUEST_LOCK	1
#define	LKREQ_RELEASE_LOCK	2
#define	LKREQ_CANCEL_LOCK	3
#define	LKREQ_RUNDOWN_PROC	4
#define	LKREQ_GLOBAL_DEADLOCK	5
#define	LKREQ_RCP_IS_ONLINE	6
#define	LKREQ_ENTER_DEBUGGER	7
    i4	    	    flags;		/* Various flavors of the LK request
					** operations. */
    i4		    result_flags;	/* LKREQ result information */
#define	LKREQ_UPDATE_VALUE		0x0001
#define	LKREQ_UPDATE_LOCKID		0x0002
#define	LKREQ_VALADDR_PROVIDED		0x0004
#define	LKREQ_CHECK_STATUS_FIELD	0x0008
#define	LKREQ_PHYSICAL_REQUEST		0x0010
#define	LKREQ_STALLED_DURING_REQUEST	0x0020
#define	LKREQ_STALLED_CONVERSION	0x0040
/* b90388 - Used to prevent reuse of old lock request blocks */
#define LKREQ_COLLECTED                 0x0100
    LK_LOCK_KEY	    lock_key;		/* Resource name to lock/unlock. */
    i4		    lock_mode;		/* The requested lock mode for 
					** the resource. */
    LK_VALUE	    value;
    LK_LKID	    lockid;
    i4              cbt_id;     /* Callback Thread Descriptor */
    LKcallback          cback;          /* Callback Function */ 
    PTR                 cback_arg;      /* Callback argument */
    SIZE_TYPE	    llb_offset;
    SIZE_TYPE	    lkb_offset;
    i4	    	    request_id;		/* unique request ID */
    CS_CPID    	    cpid;		/* cross-process session info */
    CS_CPID	    blocking_cpid;	/* CPID of blocking lock */
    i4	   	    blocking_mode;	/* Lock mode of blocker */
    STATUS	    status;		/* result of this lock request */
    LLBQ	    lkreq_stall_wait;
} LKREQ;

/*}
** Name: LKB - Lock Block.
...
#define LKB_ON_CBACK_LIST	0x0200  /* LKB is on callback queue */
...
#define LKB_IS_RSBS		0x8000	/* LKB is embedded in RSB */
    SIZE_TYPE       lkb_cbt_id;         /* Descriptor for callback thread */
    i4              lkb_cback_off;      /* CBACK struct offset (Distributed)*/
    LKcallback      lkb_cback_fcn;      /* Callback Function */
    PTR             lkb_cback_arg;      /* Callback argument */
    struct _LKBQ
    {
    ...
    LLBQ	    lkb_dead_wait;	/* Queue of LKBs awaiting deadlock check */
...
#define LKD_SHOW_DEADLOCKS	0x8000L /* Display deadlock info */
...
#endif
#endif
    CS_SEMAPHORE    lkd_lbk_mutex;	/* Mutex to protect LBK */
    ...
    CS_SEMAPHORE    lkd_sbk_mutex;	/* Mutex to protect SBK */
    ...
    CS_SEMAPHORE    lkd_rbk_mutex;	/* Mutex to protect RBK */
    ...
    */
    ...
    CS_SEMAPHORE    lkd_llb_q_mutex;	/* mutex to block LLB queue */
    SIZE_TYPE	    lkd_llb_next;	/* Largest LLB on queue. */
    SIZE_TYPE	    lkd_llb_prev;	/* Smallest LLB on queue. */
    CS_SEMAPHORE    lkd_dw_q_mutex;	/* mutex to latch deadlock wait queue */
    ...
    CS_SEMAPHORE    lkd_ew_q_mutex;	/* mutex to latch ew queue */
    SIZE_TYPE	    lkd_ew_next; 	/* Next LLB on event wait queue. */
    SIZE_TYPE	    lkd_ew_prev; 	/* Last LLB on event wait queue. */
    CS_SEMAPHORE    lkd_stall_q_mutex;	/* mutex to latch stall queue */
...
#define	LK_STALL_LOGICAL_LOCKS		2
    ...
    struct
    {
      ...
      u_i4	    request_convert;	/* # of request with implied conversions. */
      u_i4	    convert;		/* # of explicit convert requests. */
      u_i4	    release;		/* # of release physical locks. */
    ...
    CS_ASET	    lkd_ivb_seen;	/* DLM invalid value block has been 
					  detected, and not yet resolved. */
};

/*}
...
#define	LKH_TYPE	4
    ...
    CS_SEMAPHORE    lkh_mutex;		/* LKH semaphore */
};

/*}
...
#define LLB_CANCELLED_EVENT 0x100000L    /* LK_event() wait cancelled      */
    ...
    i4	    	    llb_search_count;	/* Last deadlock search for this list. */
    i4	    	    llb_deadlock_current;  /* LKB of previous LLB's
					** conflicting LKB. */
    i4	    	    llb_deadlock_next;	/* LKB of lock in granted queue, */
    i4		    llb_deadlock_maxrqst;   /*  Computed max request mode */
...
#define	LK_CROSS_PROCESS_EVENT  0x0001
    ...
    LLB		    *llb_next_mod;	/*
					...
					*/
    i4	    	    llb_gsearch_count;  /*
					** Last global deadlock search on this
					** lock list.
					*/
    ...
    CS_SEMAPHORE    llb_mutex;		/* LLB semaphore */
}; 

/*}
...
#define		RSB_MUTEXED_DIST	0x20000	/* Mutexed by LKdist_deadlock_thread */
    ...
    CS_SEMAPHORE    rsb_mutex;		/* RSB semaphore */
...
#define	CBT_TYPE	11
    ...
    i4         	    cbt_cback_count;    /* # of cbacks in queue */
    struct
    {
...
#if !defined(conf_CAS_ENABLED)
    CS_SEMAPHORE    cbt_mutex;          /* mutex to lock down CBT */
#endif
};

/*}
** Name: CBACK - Callback descriptor.
**
** Description:
**      Threads which discover a blocking callback needs to be run store 
**      the details in one of these structures and add it to the appropriate  
**      cbt's callback queue.
** 
** History:
**      01-aug-1996 (nanpr01 for ICL keving)
**          Created. 
...
struct _CBACK 
{ 
    i4         	    cback_q_next;       /* Next callback thread descriptor. */
    i4         	    cback_q_prev;       /* Previous */ 
    i2              cback_type;         /* Type of block. */ 
#define	CBACK_TYPE	12
    i2              cback_reserved;     /* Padding */ 

    i4              cback_blocked_mode; /* Mode of lock being blocked */ 
    LKcallback      cback_cback;        /* Function to be run */ 
    PTR             cback_arg;          /* Original argument passed in by */ 
                                        /* client */  
    LK_LKID         cback_lkb_id;       /* lockid of lock triggering on */ 
    LK_LOCK_KEY	    cback_key;		/* and its lock key */
}; 

/*}
...
FUNC_EXTERN	STATUS	LK_meminit(
                            CL_ERR_DESC *sys_err);
FUNC_EXTERN	VOID	LK_do_rundown(
			    LKREQ 	*lkreq);
...
FUNC_EXTERN     STATUS  LK_try_convert(
                            RSB **rsb, 
                            i4  new_group_mode);
FUNC_EXTERN	STATUS	LK_do_convert(
			    LKB 	*lkb, 
			    i4  	mode, 
			    i4  	flags,
			    LK_VALUE	*value, 
			    LKREQ 	*lkreq,
			    CL_ERR_DESC *sys_err);
FUNC_EXTERN	STATUS	LK_do_new_lock_request(
			    LKREQ   	*lkreq,
			    bool	release_mutex,
			    i4   	*free_res,
			    CL_ERR_DESC *sys_err);
...
FUNC_EXTERN	STATUS	LK_release_lock(
			    LKREQ 	*lkreq);
FUNC_EXTERN	STATUS	LK_cancel(
			    ...
			    CL_ERR_DESC	*sys_err);
FUNC_EXTERN	STATUS	LK_cancel_lock(
			    LKREQ 	*lkreq);
...
/* Functions in LKCSP.C: */
FUNC_EXTERN	STATUS  LK_csp_send(
			    LKREQ 	*lkreq, 
			    CL_ERR_DESC *sys_err);
FUNC_EXTERN	STATUS  LK_csp_mbx_create(
			    CL_ERR_DESC *sys_err);
FUNC_EXTERN	STATUS	LK_wakeup(
			    STATUS	result_status,
			    bool	release_mutex,
			    LLB		*llb,
			    LKREQ	*lkreq);
FUNC_EXTERN	VOID	LK_disable_asts(void);
FUNC_EXTERN	VOID	LK_enable_asts(void);
FUNC_EXTERN	STATUS	LK_csp_rcp_online(void);
...
FUNC_EXTERN     STATUS          lk_cback_alloc(
                                    LKB         *lkb,
                                    CBACK       **pcback );
FUNC_EXTERN     VOID            lk_cback_dealloc(
                                    LKB         *lkb );
...
FUNC_EXTERN	STATUS LK_flush_dsm_context( LK_DSM_CONTEXT *pcontext, u_i4 *writes );
