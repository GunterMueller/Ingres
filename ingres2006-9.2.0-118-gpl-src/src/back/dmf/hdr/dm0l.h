...
#define			DM0L_CLUSTER_REQ    	0x02000L
...
#define                 DM0L_BT_RECLAIM         0x1000000L
#define                 DM0L_BT_UNIQUE          0x2000000L
#define                 DM0L_REPLACE            0x4000000L
...
** to this list.
*/
#define DM0L_HEADER_FLAGS "JOURNAL,READONLY,MASTER,BTREE,SPECIAL,NOTDB,ADDONLY,MINI,FASTCOMMIT,NONREDO,\
CKP,JON,WILLING_COMMIT,CLUSTER,PRETEND,NOLOGGING,DUMP,NONJNLTAB,CLR,PHYSLOCK,COMP_REPL,TBL_CKPT,\
ROWLOCK,K_ROW_LOCK,BT_RECLAIM,BT_UNIQUE,REPLACE,NONJNL,TEMP_IIRELATION,ILOG,2BT"
...
#define			DM0L_SYSMOD	    14L
    ...
    char		dum_key[DB_MAX_COLS]; /* Attribute position .vs.
					    ** key position map. */
...

/*}
** Name: DM0L_OLD_INDEX - Index Table log record.
**
** Description:
**      This structure defines the format of a INDEX table log record.
**
**	As part of bugfix for B38527, this is now the OLD index record
**	format. New index log records contain the dui_reltups field.
**	Except for the dui_reltups field, the two records are identical and
**	are treated equally by auditdb, rollforwarddb, etc.
**
**	Eventually we will get rid of this log record, once we no longer need
**	to support 6.3-era journal files.
...
**          Added multiple location support for tables.
**	16-jul-1991 (bryanp)
**	    B38527: Renamed to DM0L_OLD_INDEX.
*/
typedef struct
{
    ...
    i4		dui_status;	    /* Relation status. */
    i4		dui_min;	    /* Minpages. */
    i4		dui_max;	    /* Maxpages. */
    i4		dui_ifill;	    /* Index Fill Factor. */
    i4		dui_dfill;	    /* Data Fill Factor. */
    i4		dui_lfill;	    /* Leaf Fill Factor. */
    i4		dui_acount;	    /* Key attribute count. */
    char		dui_key[DB_MAXKEYS]; /* Attribute position .vs.
					    ** key position map. */
...
#define		DM0L_ONLINE			0x1
#define		DM0L_START_ONLINE_MODIFY	0x2
...
typedef struct
{
    ...
    char		dui_key[DB_MAXIXATTS]; /* Attribute position .vs.
					    ** key position map. */
...
typedef struct
{
    ...
    i2			btp_ixklen;	/* Max size of Index key */
...
typedef struct
{
    ...
    i2			btd_ixklen;	/* Max size of Index key */
...
typedef struct
{
    ...
    i2			btf_ixklen;	/* Max Index key length */
...
FUNC_EXTERN DB_STATUS dm0l_archive(i4 *err_code);
