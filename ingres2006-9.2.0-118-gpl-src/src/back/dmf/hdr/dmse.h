...
FUNC_EXTERN DB_STATUS dmse_end(DMS_SRT_CB *sort_cb, i4 *err_code);

FUNC_EXTERN DB_STATUS dmse_get_record(DMS_SRT_CB *sort_cb,
			    char **record, i4 *err_code);

FUNC_EXTERN DB_STATUS dmse_input_end(DMS_SRT_CB *sort_cb, i4 *err_code);
...
*/
#define                 DMSE_CHECK_KEYS		0x01L
#define			DMSE_ELIMINATE_DUPS	0x02L
#define			DMSE_EXTERNAL		0x04L
						/* Row estimates were generated
						** externally, so don't trust
						** them to be accurate.
						*/ 
#define			DMSE_PARALLEL		0x08L
						/* Caller is parent thread 
						** trying to start parallel sort.
						*/
#define			DMSE_CHILD		0x10L
...
#define			SRT_NEED_CSSWITCH	0x0800L
    ...
    i4	    	    srt_err_code;	/*  Error return code from 
                                        **  internal routine. */
    ...
    i4	    	    srt_c_comps;  	/*  Number of comparisons */
    ...
    i4	    	    srt_perr_code;	/* ...and error code */
    i4	    	    srt_secs;
    i4	    	    srt_msecs;
