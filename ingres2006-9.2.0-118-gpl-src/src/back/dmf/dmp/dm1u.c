...
i4		*err_code)
{
    ...
    */
	...
	{
	case DM1U_REPAIR:
	    ule_format( E_DM910A_DM1U_VERIFY_INFO, (CL_SYS_ERR *)0,  ULE_LOG, 
			...
			0, type);
	    status = dm1u_repair( dm1u, err_code);
	    break;
	...
	case DM1U_FPATCH:
		...
		status = E_DB_OK;
	        ...
	        status = patch_physical(dm1u, &fhdr_pageno, err_code);
	    if (status == E_DB_OK)
	        status = patch_relation(dm1u, fhdr_pageno, err_code);
	    break;

	case DM1U_DEBUG:
	    if (type & DM1U_BITMAP)
		status = dm1p_dump(rcb, DM1P_D_BITMAP, err_code);
	    if ((status == E_DB_OK) && (type & DM1U_LINK))
		status = dm1p_dump(rcb, DM1P_D_PAGETYPE, err_code);
	    break;
	}
    }
...
DB_STATUS	   *err_code)
{
    ...
    */
    status = dm1p_checkhwm( rcb, err_code);
    if (status != E_DB_OK)
    {
	/*  
    ...
    */
	...
	status = dm0p_fix_page( rcb, (DM_PAGENO) page_number, 
	    		        DM0P_WRITE, &rcb->rcb_data_page_ptr, err_code);
	if (status != E_DB_OK)
	{
	    /*
		...
		*err_code = E_DM9104_ERR_CK_PATCH_TBL;
		return (E_DB_ERROR);
	    }
	} /* endif bad page read */

	page = (DMPP_PAGE *)rcb->rcb_data_page_ptr;
	...
	else 
	{
		...
		*/
		...
		status = dm0p_unfix_page( rcb, DM0P_UNFIX, 
					  &rcb->rcb_data_page_ptr,
					   err_code);
		if ( status != E_DB_OK )
		{
		    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, &error,0);
	...
	DMPP_VPT_SET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype, 
	    rcb->rcb_data_page_ptr, 0);
	DMPP_VPT_SET_PAGE_MAIN_MACRO(t->tcb_rel.relpgtype,
	    rcb->rcb_data_page_ptr, 0);
	DMPP_VPT_SET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype,
	    rcb->rcb_data_page_ptr, DMPP_MODIFY);
	if (rcb->rcb_other_page_ptr)
	{
	    DMPP_VPT_SET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype,
		rcb->rcb_other_page_ptr, DMPP_MODIFY);
	    DMPP_VPT_SET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype,
		rcb->rcb_other_page_ptr, page_number);
	...
	rcb->rcb_other_page_ptr = rcb->rcb_data_page_ptr;
	rcb->rcb_data_page_ptr = NULL;
    }
...
DB_STATUS	   *err_code)
{
	...
	tab_id.db_tab_index = DM_I_RELATION_TAB_ID;
		...
		&timestamp, &rel_rcb, (DML_SCB *)0, &error);
        if (status)
        {
                ule_format(error, 0, ULE_LOG, NULL, 0, 0, 0, &error, 0);
	...
	else
	{
		...
		*/
		relrecord.relprim = 1;
		relrecord.relmain = 1;
		relrecord.relifill = NULL;
		relrecord.reldfill = DM_F_HEAP;
		relrecord.rellfill = NULL;
		relrecord.relspec = TCB_HEAP;
		relrecord.relkeys = NULL;
		...
		status = dm2r_replace(rel_rcb, &tid, 
			DM2R_BYPOSITION, (char*)&relrecord, (char *)0, &error);
		if (status != E_DB_OK )
		{
		    ule_format(error, 0, ULE_LOG, NULL, 0, 0, 0, &error, 0);
		    dm1u_talk(dm1u,E_DM9026_REL_UPDATE_ERR, 2, 
	...
	*/
	...
	if (rel_rcb)
	{
	    loc_stat = dm2t_close(rel_rcb, DM2T_NOPURGE, &error);
	    if (loc_stat != E_DB_OK)
	    {
		*err_code = error;
	    }
    ...
    DB_STATUS	    *err_code)
{
	...
	status = dm1p_repair( rcb, dm1u, err_code);
...
DB_STATUS	    *err_code)
{
	...
	*/
	if (dm1u->dm1u_op & DM1U_BITMAP)
	{
	    ...
	    */
	    status = dm1p_verify( rcb, dm1u, err_code );
	    if ( status != E_DB_OK )
	    {
		if ( status == E_DB_INFO )
		{
	...
	*/
	...
	status = dm0m_allocate( size, DM0M_ZERO, (i4) MISC_CB, 
	    (i4) MISC_ASCII_ID, (char *) 0, 
	    (DM_OBJECT **) &dm1u->dm1u_xmap, err_code);
	if ( status != E_DB_OK )
	    break;
	...
	status = dm0m_allocate( size, DM0M_ZERO, (i4) MISC_CB, 
	    (i4) MISC_ASCII_ID, (char *) 0, 
	    (DM_OBJECT **) &dm1u->dm1u_xchain, err_code);
	if ( status != E_DB_OK )
	    break;
	...
	*/
		...
		(DM1U_LINK | DM1U_RECORD | DM1U_ATTRIBUTE | DM1U_KPERPAGE))
	{
	    ...
	    {
	    case TCB_HEAP:
		status = heap(dm1u, err_code);
		break;

	    case TCB_HASH:
		status = hash(dm1u, err_code);
		break;

	    case TCB_ISAM:
		status = isam(dm1u, err_code);
		break;

	    case TCB_BTREE:
		dm1u->dm1u_1stleaf = (DM_PAGENO) BTREE_ROOT;
		status = btree(dm1u, err_code);
		break;
	...
	{
	    status = periph_check(dm1u, err_code);
	}
...
DB_STATUS	*err_code)
{
    ...
    */
    ...
    status = main_chain(dm1u, (DM_PAGENO) 0, (DM_PAGENO) 0, err_code);
    if (status == E_DB_OK)
	status = check_orphan(dm1u, err_code);
    return (status);
}

/*{
...
**	    rcb->rcb_data_page_ptr, the reference to that page will be lost.
**
** History:
**      24-oct-89 (teg)
**          initial creation.
...
DB_STATUS	*err_code)
{
	...
	status = fix_page(dm1u, (DM_PAGENO) page_number,
	    DM0P_WRITE | DM0P_NOREADAHEAD, &rcb->rcb_data_page_ptr, err_code);
	if (status != E_DB_OK)
	    return (status);
	page = (DMPP_PAGE *)rcb->rcb_data_page_ptr;
	...
	unfix_page(dm1u, &rcb->rcb_data_page_ptr);
...
DB_STATUS	*err_code)
{
    ...
    ipage = (DMPP_PAGE *) rcb->rcb_data_page_ptr;
    ...
    if (DMPP_VPT_GET_PAGE_MAIN_MACRO(t->tcb_rel.relpgtype, ipage) == NULL)
    {
    ...
    */
	...
	if (indexlevel)
	{
	    isam_index(dm1u, idx_page, err_code);
	}
	else
	{
	    ...
	    main_chain(dm1u, idx_page, exp_main_page, err_code);
	}

	if ((u_i2)(idx_tidno + 1) < page_next_line)
	{
	    status = fix_page(dm1u, (DM_PAGENO) index_page_number,
		DM0P_READ, &rcb->rcb_data_page_ptr, err_code);
	    if (status != E_DB_OK)
		return (status);
	    ipage = (DMPP_PAGE *) rcb->rcb_data_page_ptr;
	}
...
DB_STATUS	*err_code)
{
    ...
    ipage = (DM1B_INDEX *) rcb->rcb_data_page_ptr;
    ...
    */
	...
	{
	    dm1u->dm1u_e_cnt++;
	    ule_format(W_DM5062_INDEX_KPERPAGE, 
		(CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0, (i4)0, (i4)0,
		err_code, 10, 
		sizeof(DB_TAB_NAME), t->tcb_rel.relid.db_tab_name,
		sizeof(DB_OWN_NAME), t->tcb_rel.relowner.db_own_name,
		sizeof(index_page_number), &index_page_number,
	    ...
	    unfix_page(dm1u, &rcb->rcb_data_page_ptr);
	    *err_code = E_DM926B_BUILD_TCB; /* bad tcb_kperpage */
	    return (E_DB_ERROR);
	}
    }
    ...
    **	this index page.  First, assure that offset to bid is valid */
	...
	if (offset)
	{
		...
		status = btree_leaf(dm1u, idx_page, err_code);
	    else
		status = btree_index(dm1u, idx_page, err_code);
...
**	rcb->rcb_data_page_ptr.  That page will remain fixed throughout
**	leaf overflow page processing. Each leaf overflow page will be fixed
**	to rcb->rcb_other_page_ptr, processed, then unfixed (before the next
...
DB_STATUS	*err_code)
{
    ...
    ** broken and give up on this page. */
    ...
    if (status != E_DB_OK)
	return (status);
    leafpage = (DM1B_INDEX *) rcb->rcb_data_page_ptr;
	...
	if (infinity.tid_i4 == TRUE)
	{
		...
		*/
		if (DM1B_INDEX_COMPRESSION(rcb) == DM1CX_UNCOMPRESSED)
		{
		    ...
		    {
			dm1u->dm1u_e_cnt++;
			ule_format(W_DM5063_LEAF_KPERPAGE,
			...
			unfix_page(dm1u, &rcb->rcb_data_page_ptr);
			*err_code = E_DM926B_BUILD_TCB; /* bad tcb_kperleaf */
			return (E_DB_ERROR);
		    }
		}
	    }
    ...
    */
	...
	leafpage) > 0)
    {
	if (dm1bvpt_kidoff_macro(t->tcb_rel.relpgtype, leafpage, 0) == NULL)
	{
	    dm1u->dm1u_e_cnt++;
	...
	status = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
		    ...
		    t->tcb_leafattcnt, line_zero, &keyptr,
		    (DM_TID *)0,  (i4*)0,
		    &klen, DM1B_INDEX_COMPRESSION(rcb), NULL);
	get_status = status;
	if (status == E_DB_WARN 
	...
	have_key = TRUE;
	btree_line_table(dm1u, leafpage, leaf_page_number, KeyBuf, err_code);
    }
	...
	if (status != E_DB_OK)
	    break;
	ovflpage = (DM1B_INDEX *) rcb->rcb_other_page_ptr;
	...
	*/
	    ...
	    ovflpage) > 0)
	{
	    ...
	    */
	    if ( ! have_key)
	    {
		if (dm1bvpt_kidoff_macro(t->tcb_rel.relpgtype, ovflpage, 0) == NULL)
		{
		    dm1u->dm1u_e_cnt++;
		    dm1u_talk(dm1u,W_DM5036_CANT_READ_LEAF_TID, 4,
	    ...
	    btree_line_table(dm1u, ovflpage, ovfl, KeyBuf, err_code);
	    break;
	}
	ovfl = DM1B_VPT_GET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype, 
		ovflpage);
	unfix_page (dm1u, &rcb->rcb_other_page_ptr);
...
**	fixed on exit.  That page must NOT be fixed to rcb->rcb_data_page_ptr
...
DB_STATUS	*err_code)
{
	...
	if ( (DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, page) & 
	    DMPP_LEAF) && 
	    (DM1B_VPT_GET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype, page) == NULL) )
	{
	...
	if (btree_data(dm1u, (u_i4) temptid.tid_tid.tid_page, 
	    (u_i4) temptid.tid_tid.tid_line, err_code) == FALSE)
	{
...
**
**	This routine fixes pages to dm1u_cb->dm1u_data_pg_ptr, and unfixes
...
DB_STATUS	 *err_code)
{
	...
	DM0P_WRITE | DM0P_NOREADAHEAD, &dm1u->dm1u_data_pg_ptr, err_code);
    if (status != E_DB_OK)
	return (FALSE);

    page = (DMPP_PAGE *)dm1u->dm1u_data_pg_ptr;
	...
	unfix_page(dm1u, &dm1u->dm1u_data_pg_ptr);
	return (FALSE);
    }
	...
	data_page(dm1u, (DMPP_PAGE *)dm1u->dm1u_data_pg_ptr, err_code);
    }
    ...
    if ( (*t->tcb_acc_plv->dmpp_get_offset)(t->tcb_rel.relpgtype,
		t->tcb_rel.relpgsize, page, line_num) == NULL )
	temp=FALSE;
...
**
** Side Effects:
**	    leaf pages are fixed to rcb->rcb_data_page_ptr, and then
...
DB_STATUS	*err_code)
{
    ...
    if ((leaf = dm1u->dm1u_1stleaf) == NULL)
    {
    ...
    KeyBufSize = (i4)ME_ALIGN_MACRO(t->tcb_klen, sizeof(i4));
	...
	** key on this page */
	...
	if ( DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leafpage) )
	{
		...
		stat = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
				...
				t->tcb_leafattcnt, (i4)0, &key_ptr, 
			    (DM_TID *)0, (i4*)0,
			    &klen, DM1B_INDEX_COMPRESSION(rcb),
			    NULL);
		if (stat == E_DB_WARN 
			&& (t->tcb_rel.relpgtype != TCB_PG_V1) )
		    stat = E_DB_OK;
		...
		stat = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
			    leafpage, t->tcb_acc_tlv, t->tcb_leafatts,
			    t->tcb_leafattcnt, line,
			    &key_ptr, (DM_TID *)0, (i4*)0,
			    &klen, 
			    DM1B_INDEX_COMPRESSION(rcb), NULL);
		if (stat == E_DB_WARN 
			&& (t->tcb_rel.relpgtype != TCB_PG_V1) )
		    stat = E_DB_OK;
		if (stat != E_DB_OK)
...
**	    rcb->rcb_data_page_ptr, so calling routine should not have any fixed
**	    data pages.
**
** History:
**      24-oct-89 (teg)
...
DB_STATUS	*err_code)
{
	...
	if (!CHECK_MAP_MACRO(dm1u_cb, i))
	{
	    ...
	    if (status == E_DB_OK)
	    {
		DMPP_PAGE   *page = (DMPP_PAGE *)rcb->rcb_data_page_ptr;
		...
		unfix_page(dm1u_cb, &rcb->rcb_data_page_ptr);
	    }
	    else
	    {
	...
	*/
	...
	for (i=0; i<page_next_line; i++)
	{
	    ...
	    */
		...
		&tup_width, &rec_ptr, row_ver_ptr, NULL, (DMPP_SEG_HDR *)0);
		...
		status = dm1c_get(rcb, page, &localtid, 
			(char *)&adf->dm1u_record[0], err_code);
		...
		if ((dm1u->dm1u_op & DM1U_OPMASK) != DM1U_VERIFY)
		{
		    ...
		    (*t->tcb_acc_plv->dmpp_delete)
				...
				&rcb->rcb_tran_id, &localtid, del_size);
		...
		*/
		...
		if (status == E_DB_ERROR)
		{
			...
			if ( (t->tcb_rel.relstat & TCB_UNIQUE) ||
			     ((t->tcb_rel.relstat & TCB_DUPLICATES) == NULL))
			{
			...
			else if (badcount < MAXBAD)
			{
				...
				*/
				...
				status = (*t->tcb_acc_tlv->dmpp_compress)
					(t->tcb_data_atts,
		...
		/*	    >>>>>>>>>>>>>   			*/
				...
				(*t->tcb_acc_plv->dmpp_put)
				    (t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
				    page, (i4)DM1C_DIRECT, 
				    &rcb->rcb_tran_id, &localtid, 
				    crec_len, crec, rcb->rcb_compress,
				    t->tcb_rel.relversion, 
				    (DMPP_SEG_HDR *)0); /* FIX ME */
			    }
			    else
			    {
...
DB_STATUS	   *err_code)
{
    ...
    */
    ...
    status = dm0p_fix_page(rcb, page_number, action, page, err_code);
    if (status != E_DB_OK)
    {
	i4	    bad_page = -1;
	...
	if ((dm1u->dm1u_state & DM1U_S_ORPHAN) == 0)
	{
	    ...
	    if (*err_code == E_DM9202_BM_BAD_FILE_PAGE_ADDR)
		dm1u_talk(dm1u,W_DM5001_BAD_PG_NUMBER, 4, 
		   sizeof(page_number), &page_number,
...
**	fact.  It is up to the caller to assure that *page_ptr has the address
**	of a valid DMPP_PAGE.
**
** Inputs:
**	page_num		the page number of the page being unfixed.
**	page_ptr		pointer to the variable containing a pointer to
**				    the fixed page.
**	dm1u_cb			check table control block.
**
** Outputs:
**	page_ptr		the value is *page_ptr is zeroed by dmpp_unfix_page
**
**	Returns:
**	    none.
**	Exceptions:
...
DMPP_PAGE       **page_ptr)
{
    ...
    status = dm0p_unfix_page(rcb, DM0P_UNFIX, page_ptr, &err_code);
    if (status != E_DB_OK)
    {
	DB_STATUS	error;
...
DB_STATUS	*err_code)
{
	...
	*/
	status = dm2r_rcb_allocate(dcb->dcb_rel_tcb_ptr, 
	    (DMP_RCB *)0, &rcb->rcb_tran_id, rcb->rcb_lk_id, 
	    rcb->rcb_log_id, &rel_rcb, err_code);
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, 
		err_code, 0);
	    dm1u_talk(dm1u,E_DM911C_RCB_ALLOC_ERR, 0);
	    *err_code = E_DM911C_RCB_ALLOC_ERR;
	    break;
	}
	...
	for (i = 1; i <= t->tcb_rel.relatts; i++)
	{
		...
		if (status != E_DB_OK)
		{
			...
			STlength(att_name), att_name);
		    *err_code = E_DM911B_NO_DT_INFO;
		    status = E_DB_OK;
		    continue;
		}
		/* peripheral data-type, check it out */
		status = dm2r_position(rcb, DM2R_ALL, NULL, 0, NULL, NULL, 
				       NULL, err_code);
		if (status != E_DB_OK)
		{
		    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, 
		    ...
		    *err_code = E_DM9113_PERPH_POS_ERR;
		    break;
		}
		for (;;)
		{
			...
			else
			{
			    ...
			    *err_code = E_DM9115_PERPH_GET_ERR;
			    break;
			}
		    }
			...
			dm1u_talk(dm1u,E_DM911D_NO_NULL_INFO, 2, 
			    STlength(att_name), att_name);
			*err_code = E_DM911D_NO_NULL_INFO;
			status = E_DB_OK;
			continue;
		    }
		    if (null_value)
			...
			if (etab_rcb)
			{
			    status = dm2t_close(etab_rcb, (i4)0, err_code);
			    if (status != E_DB_OK)
			    {
				ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 
				    0, err_code, 0);
			...
			if (status != E_DB_OK)
			{
			    ...
			    *err_code = E_DM9118_ETAB_OPEN_ERR;
			    break;
			}
		    }
			...
			status = dm2r_position(etab_rcb, DM2R_QUAL, qual_list, 
			    (i4)3, (i4)0, (i4)0, (DM_TID *)0, err_code);
			if (status != E_DB_OK)
			{
			    if (*err_code == E_DM0055_NONEXT)
			    {
				...
				dm1u_talk( dm1u,W_DM5060_NO_ETAB_SEGMENT, 
				    ...
				    sizeof(rcb->rcb_currenttid.tid_i4),
				    &rcb->rcb_currenttid.tid_i4);
				*err_code = W_DM5060_NO_ETAB_SEGMENT;
				...
				else
				{
					...
					sizeof(rcb->rcb_currenttid.tid_i4),
					&rcb->rcb_currenttid.tid_i4);
				    *err_code = W_DM505F_TOO_MANY_BAD_SEGS;
				    status = E_DB_OK;
				    break;
				}
			    }
				...
				dm1u_talk(dm1u,E_DM9119_ETAB_POS_ERR, 2, 
				    sizeof(relrecord.reltid.db_tab_base), 
				    &relrecord.reltid.db_tab_base);
				*err_code = E_DM9119_ETAB_POS_ERR;
				break;
			    }
			}
			status = dm2r_get(etab_rcb, &tid, DM2R_GETNEXT,
			    (char *)&etab_record, err_code);
			if (status != E_DB_OK)
			{
			    if (*err_code == E_DM0055_NONEXT)
			    {
			...
			*/
			...
			dm1u_talk( dm1u,W_DM5061_WRONG_BLOB_LENGTH, 
			    ...
			    &blob_length);
			*err_code = W_DM5061_WRONG_BLOB_LENGTH;
		    }
		}
		if (status != E_DB_OK)
		    break;
    ...
    if (rel_rcb)
	(VOID)dm2r_release_rcb(&rel_rcb, &local_err);

    if (etab_rcb)
	(VOID)dm2t_close(etab_rcb, (i4)0, &local_err);
...
PTR	    p12val)
{
    ...
    */
    ...
    status = ule_doformat(msg_id, NULL, ULE_LOOKUP, NULL, cbuf, DM1U_MSGBUF_SIZE, 
	&mlen, &err_code, count, 
