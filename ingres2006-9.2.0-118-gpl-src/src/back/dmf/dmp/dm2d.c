...
static DB_STATUS 	dm2d_convert_iirel_from_64 (
				DMP_DCB	    	*db_id,
                                i4              flag,
                                i4              lock_list,
				i4	    	*err_code );
...
**	local_tlv	format routine for page
** Outputs:
**	err_code	error code
**
**	Returns:
...

/*{
** Name: dm2d_convert_iirel_from_64 - convert iirelation from 6.4 to current
**
** Description:
**      Used indirectly by upgradedb to convert iirelation from 6.4
**	to the current format.
**	There will be more conversions coming so the config file version
**	is not updated.
**
**	The 64-to-current iirelation conversion is a size changing
**	conversion, so we rewrite a new iirelation file.  Upgradedb
**	will modify it back to a decent hash.
**
** Inputs:
...
**          initial creation
**      21-sep-92 (ed)
**          added DB_MAXNAME changes
**	04-jan-1992 (rmuth)
**	    prototyping DI showed up wrong parameter to DIrename.
**	23-July-1993 (rmuth)
**	    6.4->6.5 conversion needs to update the iirelation.relatts count
**	    for iiattribute. Added code to do this.
**	27-jul-1993 (rmuth)
...
**	    DM_sizeof_core_rel to hold this value.
**	20-sep-1993 (rogerk)
**	    Fix to updating of relatts value in iiattribute's relation row.
**	    Use correct index variable (j not i) to index into core catalog
**	    array.
**	21-apr-1994 (rmuth)
**	    b62335, whilst converting the iirelation table to 6.5 format
**	    take the opportunity to set the the relloccount. In earlier
...
**	    surpise this did happen with death-like results. 
**	06-mar-1996 (stial01 for bryanp)
**	    Pass DM_PG_SIZE as the page_size argument. Core system catalogs are
**		required for bootstrapping reasons to have this page size.
**	06-mar-1996 (stial01 for bryanp)
**	    Call dmpp_get, rather than dmpp_get_offset, to extract rows.
**	06-may-1996 (thaju02)
**	    New Page Format Support:
**		Change page header references to use macros. Pass macros
**		DM_PG_SIZE as the page size argument.
**		Pass DM_PG_SIZE to dm1c_getaccessors().
**	20-mar-1996 (ramra01)
**	    Added DMPP_TUPLE_INFO argument to the accessor routine
**      03-june-1996 (stial01)
**          Use DMPP_INIT_TUPLE_INFO_MACRO to init DMPP_TUPLE_INFO
**      28-may-1998 (stial01)
**          dm2d_convert_iirel_from_64() Support VPS system catalogs.
**      21-Jul-1999 (wanya01)
**          Add parameter flag and lock_list to dm2d_convert_iirel_from_64()
**          Do not try to lock CNF if it's already locked. (Bug 98010)
**	18-Dec-2003 (schka24)
**	    init new 2.65 table partitioning columns
**	25-Aug-2004 (schka24)
**	    rename for clarity, make sure we're building proper V7 rows.
**	    Turn off all has-stats flags, since from-64 in upgradedb is going
**	    to drop iistatistics, might as well take care of it now.
[@history_template@]...
*/
static DB_STATUS
dm2d_convert_iirel_from_64 (
    DMP_DCB	    *db_id,
    i4              flag,
    i4              lock_list,
    i4	    *err_code )
{
    ...
    i4	    temp_lock_list, status, i=0,j, local_error, local_status;
    CL_SYS_ERR	    sys_err;
    DM0C_CNF	    *config = 0;
    bool	    config_open = FALSE, tmp_open = FALSE;
    bool	    iirel_open = FALSE, iirel_idx_open = FALSE, add_status;
    DB_TAB_ID	    iirel_tab_id, iirel_idx_tab_id;
    DM_FILE	    tmp_file_name = 
			{{'i','i','r','e','l','c','v','t','.','t','m','p'}};
    DM_FILE	    iirel_file_name, iirel_idx_file_name;
    DI_IO	    tmp_di_context, iirel_di_context, iirel_idx_di_context;
    DMPP_PAGE	    tmp_page, iirel_page, iirel_idx_page;
    i4	    iirel_page_number=0, iirel_idx_page_number=0;
    i4	    read_count, write_count, line, offset;
    DMP_RELATION    iirel_record;
    DMP_OLDRELATION *iirel_oldrecord;
    DMP_RINDEX	    iirel_idx_record;
    DM_TID	    iirel_idx_tid, iirel_tid;
    DMPP_ACC_PLV    *local_plv;
    DMPP_ACC_TLV    *local_tlv;
    DM_TID	    tid;
    i4	    record_size;
	...
	*/
	...
	if (status != OK)
	{
	    ule_format(status, &sys_err, ULE_LOG, (DB_SQLSTATE *)NULL,
	...
	status = dm0c_open(dcb, flag, temp_lock_list, &config, err_code);
	...
	*/
	...
	if  ((config->cnf_dsc->dsc_status & DSC_IIREL_CONV_DONE) != 0)
	{
	    ...
	    status = dm0c_close(config, (flag & DM0C_CNF_LOCKED), err_code);
	...
	/*  Generate the filenames for iirelation and iirel_idx tables. */

	iirel_tab_id.db_tab_base = DM_B_RELATION_TAB_ID;
	iirel_tab_id.db_tab_index = DM_I_RELATION_TAB_ID;

	iirel_idx_tab_id.db_tab_base = DM_B_RIDX_TAB_ID;
	iirel_idx_tab_id.db_tab_index = DM_I_RIDX_TAB_ID;

	dm2f_filename (DM2F_TAB, &iirel_tab_id, 0, 0, 0, &iirel_file_name);
	dm2f_filename (DM2F_TAB, &iirel_idx_tab_id, 0, 0, 0, &iirel_idx_file_name);
	...
	*/ 
	...
	if (status == DI_FILENOTFOUND)
	{
		...
		(char *) &dcb->dcb_location.physical,
		dcb->dcb_location.phys_length,
		iirel_file_name.name,
		sizeof(iirel_file_name.name),	     
		tmp_file_name.name,
		sizeof(tmp_file_name.name),
		&sys_err);
	...
	*/
	...
	/* 
	** Delete iirel_idx if it exists. We might have created this file
	** in a previous invocation of upgradedb.
	*/

	status = DIdelete (	
	    &iirel_idx_di_context, 
	    (char *) &dcb->dcb_location.physical,
	    dcb->dcb_location.phys_length,
	    iirel_idx_file_name.name, 
	    sizeof(iirel_idx_file_name.name), 
	    &sys_err);
	...
	*/
	...
	/* Format the first pages of the new iirelation and iirel_idx. */

	(*local_plv->dmpp_format)(DM_COMPAT_PGTYPE, DM_COMPAT_PGSIZE, 
			&iirel_page, 0, DMPP_DATA, DM1C_ZERO_FILL);
	(*local_plv->dmpp_format)(DM_COMPAT_PGTYPE, DM_COMPAT_PGSIZE, 
			&iirel_idx_page, 0, DMPP_DATA, DM1C_ZERO_FILL);

	/* 
	** Now copy the records from the tmp iirelation to the new iirelation.
	*/
	...
	for (i=0;;)
	{
	    ...
	    while (++line < (i4)DMPP_VPT_GET_PAGE_NEXT_LINE_MACRO(TCB_PG_V1, 
		&tmp_page))
	    {
		tid.tid_tid.tid_line = line;
		local_status = (*local_plv->dmpp_get)(TCB_PG_V1, DM_PG_SIZE,
		    &tmp_page, &tid, &record_size, (char **)&iirel_oldrecord,
		    NULL, NULL, (DMPP_SEG_HDR *)0);
		...
		/* Copy the record to the new format. */

		MEfill (sizeof(iirel_record), (u_char)0, (PTR)&iirel_record);

		/* copy old info */

		iirel_record.reltid = iirel_oldrecord->reltid;
		dm2d_cname((char *)&iirel_record.relid, iirel_oldrecord->relid);
		dm2d_cname((char *)&iirel_record.relowner, iirel_oldrecord->relowner);
		iirel_record.relatts = iirel_oldrecord->relatts;               
		iirel_record.relwid = iirel_oldrecord->relwid;                
		iirel_record.relkeys = iirel_oldrecord->relkeys;		    
		iirel_record.relspec = iirel_oldrecord->relspec;               
		iirel_record.relstat = iirel_oldrecord->relstat;               
		iirel_record.reltups = iirel_oldrecord->reltups;               
		iirel_record.relpages = iirel_oldrecord->relpages;              
		iirel_record.relprim = iirel_oldrecord->relprim;               
		iirel_record.relmain = iirel_oldrecord->relmain;               
		iirel_record.relsave = iirel_oldrecord->relsave;               
		STRUCT_ASSIGN_MACRO(iirel_oldrecord->relstamp12, iirel_record.relstamp12);
		dm2d_cname((char *)&iirel_record.relloc, iirel_oldrecord->relloc);
		iirel_record.relcmptlvl = iirel_oldrecord->relcmptlvl;
		iirel_record.relcreate = iirel_oldrecord->relcreate;             
		iirel_record.relqid1 = iirel_oldrecord->relqid1;               
		iirel_record.relqid2 = iirel_oldrecord->relqid2;               
		iirel_record.relmoddate = iirel_oldrecord->relmoddate;            
		iirel_record.relidxcount = iirel_oldrecord->relidxcount;           
		iirel_record.relifill = iirel_oldrecord->relifill;              
		iirel_record.reldfill = iirel_oldrecord->reldfill;              
		iirel_record.rellfill = iirel_oldrecord->rellfill;              
		iirel_record.relmin = iirel_oldrecord->relmin;                
		iirel_record.relmax = iirel_oldrecord->relmax;                
		iirel_record.relloccount = iirel_oldrecord->relloccount;
		...
		** just in case.
		*/
		if (((iirel_oldrecord->relstat & TCB_MULTIPLE_LOC) == 0) &&
		     (iirel_oldrecord->relloccount != 1))
	        {
		    iirel_record.relloccount = 1;
		}

		iirel_record.relgwid = iirel_oldrecord->relgwid;	            
		iirel_record.relgwother = iirel_oldrecord->relgwother;            
		iirel_record.relhigh_logkey = iirel_oldrecord->relhigh_logkey;        
		iirel_record.rellow_logkey = iirel_oldrecord->rellow_logkey;         

		/* 
		** Initialize relfhdr, relextend and relalloction to default
		** values. This is required by the file extend project.
		*/
		iirel_record.relfhdr       = DM_TBL_INVALID_FHDR_PAGENO;
		iirel_record.relallocation = DM_TBL_DEFAULT_ALLOCATION;
		iirel_record.relextend     = DM_TBL_DEFAULT_EXTEND;

		/* 
		** Initialise other new 6.5 fields
		*/
		iirel_record.relstat2	   = 0;

		iirel_record.relpgtype = TCB_PG_V1;

		if ( iirel_record.relstat & TCB_COMPRESSED )
		    iirel_record.relcomptype = TCB_C_STANDARD;
		else
		    iirel_record.relcomptype = TCB_C_NONE;

		/* 
		** Initialise other new OI2.0 fields
		*/
		iirel_record.relpgsize     = DM_PG_SIZE;
		iirel_record.relversion    = 0;

		/* Clear has-stats, upgradedb will kill iistats */
		iirel_record.relstat &= ~TCB_ZOPTSTAT;

		/* 
		** Update the relwid, relprim and relatt fields for the
		** iirelation tuple.
		*/

		if ((iirel_record.reltid.db_tab_base == DM_B_RELATION_TAB_ID) &&
		    (iirel_record.reltid.db_tab_index == DM_I_RELATION_TAB_ID))
		{
		    iirel_record.relwid = sizeof(iirel_record);
		    iirel_record.relprim = 1;
		    iirel_record.relatts = DM_core_relations[0].relatts; 
		}

		/*
		** Reset relwid, relprim for the rel_idx tuple. We're changing the
		** # of hash buckets to 1.
		*/

		if ((iirel_record.reltid.db_tab_base == DM_B_RIDX_TAB_ID) &&
		    (iirel_record.reltid.db_tab_index == DM_I_RIDX_TAB_ID))
		{
		    iirel_record.relwid = sizeof(iirel_idx_record);
		    iirel_record.relprim = 1;
		}


		/* 
		** Update the relwid, relprim, relatts fields for the
		** iiattribute tuple.
		*/

		if ((iirel_record.reltid.db_tab_base == DM_B_ATTRIBUTE_TAB_ID) &&
		    (iirel_record.reltid.db_tab_index == DM_I_ATTRIBUTE_TAB_ID))
		{
		    iirel_record.relwid = sizeof(DMP_ATTRIBUTE);
		    iirel_record.relprim = 1;

		    /*
		    ** 6.5 has added some new fields to iiattribute so need to
		    ** update iirelation.relatts accordingly. Get this 
		    ** value from the global defn of the core catalogs instead
		    ** of hard code. Done this way as already people have missed
		    ** updating relatts when adding new columns but this method
		    ** automatically updates.
		    */

		    /*
		    ** Search for iiattribute entry
		    */
		    for ( j = 0; j < DM_sizeof_core_rel; j++)
		    {
			if ((DM_core_relations[j].reltid.db_tab_base == 
						    DM_B_ATTRIBUTE_TAB_ID) &&
			    (DM_core_relations[j].reltid.db_tab_index == 
						    DM_I_ATTRIBUTE_TAB_ID) )
			{
			    /*
			    ** Update relatts and finish
			    */
			    iirel_record.relatts = DM_core_relations[j].relatts;
			    break;
			}
		    }

		}
		iirel_record.reltotwid = iirel_record.relwid;
		...
		*/
		...
		status = add_record(TCB_PG_V1, DM_PG_SIZE,
		    &iirel_page,
		    (char *)&iirel_record,
		    sizeof(iirel_record),
		    &iirel_tid,
		...
		/* Store the iirel_idx tuple corresponding to this tuple. */

		MEfill (sizeof(iirel_idx_record), (u_char)0, (PTR)&iirel_idx_record);

		STRUCT_ASSIGN_MACRO(iirel_record.relid, iirel_idx_record.relname);
		STRUCT_ASSIGN_MACRO(iirel_record.relowner, iirel_idx_record.relowner);
		iirel_idx_record.tidp = iirel_tid.tid_i4;

		status = add_record(TCB_PG_V1, DM_PG_SIZE,
		    &iirel_idx_page,
		    (char *)&iirel_idx_record,
		    sizeof(iirel_idx_record),
		    &iirel_idx_tid,
		    &iirel_idx_di_context,
		    local_plv,
		    local_tlv,
	    ...
	    if (DMPP_VPT_GET_PAGE_OVFL_MACRO(TCB_PG_V1, &tmp_page) != NULL)
		i = DMPP_VPT_GET_PAGE_OVFL_MACRO(TCB_PG_V1, &tmp_page);
	    ...
	    */
	    ...
	    else if (DMPP_VPT_GET_PAGE_MAIN_MACRO(TCB_PG_V1, &tmp_page) != NULL) 
		i = DMPP_VPT_GET_PAGE_MAIN_MACRO(TCB_PG_V1, &tmp_page);

	    /* 
	    ** No more main and overflow buckets. We're done. Allocate and
	    ** write out the last iirel and iirel_idx pages and then exit
	    ** the loop.
	    */
		...
		status = DIalloc(
		    &iirel_di_context, 1, &iirel_page_number, &sys_err);

		if (status != OK)
		    break;

		status = DIflush(&iirel_di_context, &sys_err);
		...
		status = DIwrite(
		    &iirel_di_context, 
		    &write_count,
		    DMPP_VPT_GET_PAGE_PAGE_MACRO(TCB_PG_V1, &iirel_page), 
		    (char *)&iirel_page, 
		    &sys_err);
		...
		/* Allocate and flush the last iirel_idx page. */

		status = DIalloc(
		    &iirel_idx_di_context, 
		    1, 
		    &iirel_idx_page_number, 
		    &sys_err);

		if (status != OK)
		    break;

		status = DIflush(&iirel_idx_di_context, &sys_err);
		...
		status = DIwrite(
		    &iirel_idx_di_context, 
		    &write_count,
		    DMPP_VPT_GET_PAGE_PAGE_MACRO(TCB_PG_V1, &iirel_idx_page), 
		    (char *)&iirel_idx_page, 
		    &sys_err);
	...
	status = DIclose (&iirel_di_context, &sys_err);
	...
	/* Close the iirel_idx file */

	status = DIclose (&iirel_idx_di_context, &sys_err);

	if  (status != OK)
	    break;
	    
	iirel_idx_open = FALSE;
	...
	*/
	...
	/* 
	** We've converted iirelation and iirel_idx to hash structures with
	...
	*/
	...
	status = dm0c_close(config, (flag & DM0C_CNF_LOCKED) | DM0C_UPDATE, err_code);
    ...
    if (config_open == TRUE)
	local_status = dm0c_close(config, (flag & DM0C_CNF_LOCKED), &local_error);
    ...
    if (iirel_open == TRUE)
	local_status = DIclose (&iirel_di_context, &sys_err);

    if (iirel_idx_open == TRUE)
	local_status = DIclose (&iirel_idx_di_context, &sys_err);

    *err_code = E_DM93AC_CONVERT_IIREL;
    return (E_DB_ERROR);
}

/*{
	...
	*/
	if ((flag & DM2D_RECOVER) == 0)
	{
	        ...
	        status = LKcreate_list(LK_ASSIGN | LK_NONPROTECT | 
			  	LK_NOINTERRUPT, (i4)0, (LK_UNIQUE *)0, 
		    ...
		    ule_format(status, &sys_err, ULE_LOG, (DB_SQLSTATE *)NULL,
		        (char *)0, 0L, (i4 *)0, &local_error, 0);
		    ule_format( E_DM901A_BAD_LOCK_CREATE, &sys_err, ULE_LOG , 
			(DB_SQLSTATE *)NULL, (char * )0, 0L, (i4 *)0, 
                        &local_error, 0);
		    *err_code = E_DM9265_ADD_DB;
	    ...
	    /* Zero err_code so we can till if set on error */
	    *err_code = 0;

	    status = dm0c_open(dcb, cnf_flag, temp_lock_list, &cnf, err_code);
	    if (status == E_DB_OK)
	    {
		...
		*/
		...
		MEcopy(cnf->cnf_dsc->dsc_dbcmptlvl, 
			DU_MAXDBCMPTLVL, dcb->dcb_dbcmptlvl);
	...
	defined(a64_win)
		...
		{
		    ule_format (E_DM9580_CONFIG_DBSERVICE_ERROR, 
			(CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
			(char *)0, (i4)0, (i4 *)0,
			err_code, 2, 0, cnf->cnf_dsc->dsc_dbservice,
			sizeof(DB_DB_NAME), &cnf->cnf_dsc->dsc_name);
		}

		status = dm0c_close(cnf, (cnf_flag & DM0C_CNF_LOCKED), err_code);
	    }
	    if ((status != E_DB_OK) && (*err_code > E_DM_INTERNAL))
	    {
	        ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0,
			   err_code, 0);
	        *err_code = E_DM9265_ADD_DB;
	    }
		    ...
		    ule_format(status, &sys_err, ULE_LOG, (DB_SQLSTATE *)NULL,
		        (char *)0, 0L, (i4 *)0, &local_error, 0);
		    ule_format( E_DM901B_BAD_LOCK_RELEASE, &sys_err, ULE_LOG,
			(DB_SQLSTATE *)NULL, (char * )0, 0L, (i4 *)0, 
                        &local_error, 1,
	    ...
	    if (root == 0)
	    {
		if ((*err_code > E_DM_INTERNAL) || (*err_code == 0))
		{
		    if (*err_code)
			ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 0);
		    *err_code = E_DM0053_NONEXISTENT_DB;
		}
		break;
	    }
	}
	...
	if (svcb->svcb_cnt.cur_database == svcb->svcb_d_max_database)
	{
	    ...
	    *err_code = E_DM0041_DB_QUOTA_EXCEEDED;
	    break;
	}
	...
	*/
	...
	if (flag & DM2D_CVCFG)
	{
	    ...
	    */
	    ...
	    {
		status = dm2d_convert_iirel_from_64(dcb, cnf_flag, temp_lock_list, err_code);
		if (status != E_DB_OK)
		{
		    ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 0);
		    *err_code = E_DM9265_ADD_DB;
		...
		status = dm2d_convert_preV7_iirel(dcb, create_version,
			cnf_flag, temp_lock_list, err_code);
		if (status != E_DB_OK)
		{
		    ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 0);
		    *err_code = E_DM9265_ADD_DB;
	    ...
	    {
		status = dm2d_convert_preV7_iiatt(dcb, create_version,
			cnf_flag, temp_lock_list, err_code);
		if (status != E_DB_OK)
		{
		    ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 0);
    ...
    i4	*err_code )
{
	...
	*/
	if ((dcb->dcb_status & DCB_S_REPLICATE) && Dmc_rep)
	{
		...
		*/
		...
		have_mutex = FALSE;
		*err_code = W_DM9563_REP_TXQ_WORK;
		...
		if (i == Dmc_rep->queue_start)
		{
		    ...
		    */
			...
			lock_list, &lk_event, &sys_err);
                    if (cl_status != OK)
                    {
                        ule_format(cl_status, &sys_err, ULE_LOG, (DB_SQLSTATE*)NULL,
                            (char *)0, (i4)0, (i4 *)0, err_code, 0);
                        ule_format(E_DM904B_BAD_LOCK_EVENT, &sys_err, ULE_LOG,
                            (DB_SQLSTATE*)NULL, (char *)0, (i4)0, (i4 *)0, err_code,
	...
	*/
	...
	status = dm2d_release_user_tcbs(dcb, lock_list, 0, 0, err_code);
	...
	*/
	...
	while ( t->tcb_status & TCB_BUSY || t->tcb_ref_count > 1 )
	{
		...
		*/
		ule_format(E_DM9C75_DM2D_CLOSE_TCBBUSY, (CL_SYS_ERR*)0,
	...
	t->tcb_ref_count--;
	status = dm2t_release_tcb(&t, 0, lock_list, (i4)0, err_code);
	...
	*/
	if (!(flag & DM2D_NLG))
	{
	    status = dm0l_closedb(dcb->dcb_log_id, err_code);
	    if (status != E_DB_OK)
		break;
	}
    ...
    */
    (void)dm0p_dbcache(dcb->dcb_id, 0, 0, lock_list, err_code);
    ...
    */
	...
	if (dcb->dcb_ref_count <= 0)
	{
		...
		cl_status = LKrelease((i4)0, svcb->svcb_lock_list,
		    (LK_LKID *)dcb->dcb_odb_lock_id, (LK_LOCK_KEY *)0,
		    (LK_VALUE *)0, &sys_err);

		if (cl_status)
		    ule_format(cl_status, &sys_err, ULE_LOG, 
                        (DB_SQLSTATE *)NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);

		/*	Release temporary table id lock. */

		cl_status = LKrelease((i4)0, dmf_svcb->svcb_lock_list,
		    (LK_LKID *)dcb->dcb_tti_lock_id, (LK_LOCK_KEY *)0,
		    (LK_VALUE *)0, &sys_err);
		...
		if (dcb->dcb_served == DCB_SINGLE)
		    svcb_lock_list = dmf_svcb->svcb_ss_lock_list;
		else
		    svcb_lock_list = dmf_svcb->svcb_lock_list;
		...
		if (cl_status != OK)
		{
			...
			err_code, 1, 0, dmf_svcb->svcb_lock_list);
		    status = E_DB_ERROR;
		    *err_code = E_DM9266_UNLOCK_CLOSE_DB;
		    break;
		}
	    }
	...
	*/
	if ((flag & (DM2D_NLK | DM2D_NLK_SESS)) == 0)
	{
		...
		(LK_LOCK_KEY *)&db_key, (LK_VALUE *)0, &sys_err);
	    if (cl_status != OK)
	    {
		ule_format(cl_status, &sys_err, ULE_LOG, (DB_SQLSTATE *)NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);
    ...
    */
    ...
    {
	ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 
                   0, 0, 0, err_code, 0);
	*err_code = E_DM9267_CLOSE_DB;
    }
    return (E_DB_ERROR);
}
/*{
...
i4 	*err_code)
{
    ...
    /*
    ** (ICL phil.p)
    ** If db opened with DMCM, toss ALL pages.
    */
	...
	*/
	if (h->hash_q_next != (DMP_TCB *) h)
	{
	    ...
	    */
		...
		else if ( dcb->dcb_status & DCB_S_DMCM &&
			 (t->tcb_tup_adds || t->tcb_page_adds) )
		{
		    /* DM2T_KEEP: Do page/tuple count updates only */
		    (VOID) dm2t_release_tcb(&t, DM2T_KEEP,
				lock_list, log_id, err_code);
		    /* 
		    ** If for whatever that didn't take place,
		    ** we don't care; just avoid looping.
		    */
		    t->tcb_tup_adds = 0;
		    t->tcb_page_adds = 0;

		    /*
		    ** Now the TCB may be busy/referenced again
		    ** if, for example, the writebehind threads
		    ** are flushing the iirelation records.
		    ** Skip to the bottom so we run into this
		    ** TCB again and wait for it if busy/ref'd; 
		    ** next time through we'll really TOSS it
		    */
		}
		else
		{
			...
			ule_format(*err_code, (CL_SYS_ERR*)0, ULE_LOG, 
			    ...
			    (i4*)0, err_code, (i4)0);
			ule_format(E_DM9C76_DM2D_CLOSE_TCBERR, (CL_SYS_ERR*)0,
			    ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
			    (i4 *)0, err_code, (i4)6,
			    sizeof(DB_DB_NAME), dcb->dcb_name.db_db_name,
			    sizeof(DB_TAB_NAME), t->tcb_rel.relid.db_tab_name,
    ...
    i4	*err_code )
{
	...
	*err_code = E_DM003E_DB_ACCESS_CONFLICT;
	...
	i4	    lock_flag = (LK_PHYSICAL | LK_INTERRUPTABLE);
	...
	if (flag & DM2D_NOWAIT)
	    lock_flag |= LK_NOWAIT;
	cl_status = LKrequest(lock_flag, lock_list, (LK_LOCK_KEY *)&db_key,
	    lock_mode, (LK_VALUE *)0, (LK_LKID *)&dblkid, (i4)0, &sys_err);
	if (cl_status != OK)
	{
	    /*	Lock the DCB. */
		...
		*err_code = E_DM004B_LOCK_QUOTA_EXCEEDED;
	    ...
	    {
		*err_code = E_DM004E_LOCK_INTERRUPTED;
		return (E_DB_ERROR);
	    }
    ...
    */
	...
	if ((flag & (DM2D_NLK | DM2D_IGNORE_CACHE_PROTOCOL)) == 0)
	{
	    ...
	    */
	    ...
	    if ((cl_status == OK) && (dcb->dcb_served != DCB_SINGLE))
	    {
		/*
		...
		*/
		if (lock_value.lk_low == 0)
		{
		    ...
		    */
			...
			(LK_LKID *)dcb->dcb_lock_id, (i4)0, &sys_err);
		    if ((cl_status == OK) && (lock_value.lk_low == 0))
		    {
	...
	*/
	if ((flag & DM2D_NLK) == 0)
	{
		...
		cl_status = LKrequest(LK_PHYSICAL,
		    svcb->svcb_lock_list, &db_key, LK_N, (LK_VALUE *)0, 
		...
		*/
		...
		if (cl_status == LK_NOLOCKS)
		    ...
		    *err_code = E_DM9269_LOCK_OPEN_DB;
		break;
	    }
	}
	...
	*/
	loc_status = dm0c_open(dcb, cnf_flag, lock_list, &cnf, err_code);
	...
	*/
	...
	{   
	    ...
	    {	
		*err_code = E_DM93AD_IIREL_NOT_CONVERTED;
		break;
	    }
		...
		{	
		    *err_code = E_DM93F1_IIATT_NOT_CONVERTED;
		    break;
		}
	...
	*/
	...
	else if ((flag & (DM2D_RFP | DM2D_NOCK)) == 0)
	{
		...
		{
		    *err_code = E_DM0100_DB_INCONSISTENT;
		    break;
		}
	    }
		...
		*/
		...
		status = dm0c_close(cnf, DM0C_UPDATE|DM0C_LEAVE_OPEN, err_code);
		*err_code = E_DM0100_DB_INCONSISTENT;
		ule_format(E_DM9327_BAD_DB_OPEN_COUNT, 0, ULE_LOG, 
                                (DB_SQLSTATE *)NULL, 0, 0, 
				0, &local_error, 0);
		break;
	    }
	    ...
	    */
		...
		status = dm0c_close(cnf, DM0C_UPDATE|DM0C_LEAVE_OPEN, err_code);
		*err_code = E_DM0100_DB_INCONSISTENT;
		ule_format(E_DM9327_BAD_DB_OPEN_COUNT, (CL_ERR_DESC *)0, 
	...
	*/
	...
	status = dm0p_dbcache(dcb->dcb_id, 1, write_mode, lock_list, err_code);
	...
	if ( status == E_DB_WARN )
	{
	    ...
	    */
	    ...
	    (void) dm2d_release_user_tcbs(dcb, lock_list, 0, DM2T_TOSS, err_code);
	...
	*/
	if (flag & DM2D_JLK)
	{
	    ...
	    status = dm0c_close(cnf, 0, err_code);
	    if (status != E_DB_OK)
		break;
	    config = 0;
	...
	status = dm0m_allocate((i4)(sizeof(DMP_EXT) +
	    ...
	    (DM_OBJECT **)&dcb->dcb_ext, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	if (status = aducolinit(cnf->cnf_dsc->dsc_collation, dm2d_reqmem, 
				&tbl, &sys_err))
	{
	    *err_code = E_DM00A0_UNKNOWN_COLLATION;
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	...
	if (!sm_inconsistent)
	{
	    status = update_tcb(rel_tcb, lock_list, flag, err_code);
	    if (status != E_DB_OK)
		break;

	    status = update_tcb(relx_tcb, lock_list, flag, err_code);
	    if (status != E_DB_OK)
		break;

	    status = update_tcb(att_tcb, lock_list, flag, err_code);
	    if (status != E_DB_OK)
		break;

	    status = update_tcb(idx_tcb, lock_list, flag, err_code);
	    if (status != E_DB_OK)
		break;
	    ...
	    */
	    ...
	    status = update_tcbatts(rel_tcb, lock_list, flag, err_code);
	    if (status != E_DB_OK)
		break;

	    status = update_tcbatts(relx_tcb, lock_list, flag, err_code);
	    if (status != E_DB_OK)
		break;

	    status = update_tcbatts(att_tcb, lock_list, flag, err_code);
	    if (status != E_DB_OK)
		break;

	    status = update_tcbatts(idx_tcb, lock_list, flag, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	if ( flag & DM2D_CVCFG && SMS_converted_flag == FALSE )
	{
	    ...
	    */
	    status = dm0c_open(dcb, cnf_flag, lock_list, &cnf, err_code);
	    if ( status != E_DB_OK )
		break;
	    ...
	    if (cnf->cnf_dsc->dsc_c_version >= DSC_V3)
	    {
		/* Coming from 1.x or newer, just need to convert
		** iiattribute because we whacked it during the
		** physical level conversion at add time.
		*/
		status = dm2d_convert_core_table_to_65( att_tcb, lock_list, err_code);
		if ( status != E_DB_OK )
		    break;
#ifdef NOT_UNTIL_CLUSTERED_INDEXES
		/* Do the same for iiindex: */
		status = dm2d_convert_core_table_to_65( idx_tcb, lock_list, err_code);
		if ( status != E_DB_OK )
		    break;
#endif /* NOT_UNTIL_CLUSTERED_INDEXES */
	    }
	    ...
	    {
		/* 6.4 needs to convert all of the basic catalogs */
		/*
		** Convert the four core catalogs to the new format
		*/
	    ...
	    ** Close the configuration file. 
	    */
	    status = dm0c_close(cnf, DM0C_UPDATE, err_code);
	    if (status != E_DB_OK)
	         break;
	...
	*/
	...
	{
	    *err_code= E_DM92CA_SMS_CONV_NEEDED;
	    status = E_DB_ERROR;
	    break;
	}
	/*
	...
	*/
	...
	if ((dmf_svcb->svcb_status & (SVCB_RCP | SVCB_ACP | 
		...
		&& dcb->dcb_id != 1 )
	{
	    ...
	    */
		...
		if (*err_code == E_DM0055_NONEXT) /* table does not exist
					     ** do not replicate */
		{
	    ...
	    if (*err_code != E_DM0055_NONEXT)
	    {
	        ...
	        */
		    ...
		    (DM2R_KEY_DESC *)0, 1, 0, 0, 0, err_code);
	        if (status != E_DB_OK)
		    break;
	        ...
	        {
		    *err_code = E_DM9551_REP_DB_NOMEM;
        	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            	        (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
            	        err_code, (i4)0);
		    status = E_DB_ERROR;
	        ...
	        for(;;)
	        {
	            ...
	            if (status != E_DB_OK)
		    {
		        if (*err_code == E_DM0055_NONEXT) /* no local db entry,
						     ** don't replicate */
		        {
			    status = E_DB_OK;
			    break;
	        ...
	        ** {
	        **	*err_code = E_DM955B_REP_BAD_LOCAL_DB;
                **	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
                **	    (i4 *)NULL, (char *)0, (i4)0, 
		** 	    (i4 *)0, err_code, (i4)0);
	        **	status = E_DB_ERROR;
		...
		/* only replicate if we found the local db record */
	        ...
	        ** clean up
	        */
	        status = dm2t_close(repdb_rcb, 0, err_code);
	        if (status != E_DB_OK)
		    break;
	...
	*/
	if ((dcb->dcb_status & DCB_S_REPLICATE) && Dmc_rep)
	{
		...
		*/
		status = dm2rep_qman(dcb, -1, (HRSYSTIME *)-1, NULL, err_code,
			TRUE);
		/*
		...
		*/
		if (status != E_DB_OK && status != E_DB_WARN)
		{
		    ...
		    if (*err_code)
		        ule_format(*err_code, 0, ULE_LOG, 
                            (DB_SQLSTATE *)NULL, 0, 0, 0, 
		    	    &local_error, 0);
		    MEcopy(dcb->dcb_name.db_db_name, DB_MAXNAME, dbname);
		    dbname[DB_MAXNAME] = 0;
	            *err_code = E_DM9569_REP_INQ_CLEANUP;
                    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
                        (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
    ...
    */
    ...
    /* release rcb */
    if ( idx_rcb )
	status = dm2r_release_rcb(&idx_rcb, &local_error);

    if (rcb)
	status = dm2r_release_rcb(&rcb, &local_error);

    if (repdb_rcb)
	status = dm2t_close(repdb_rcb, 0, &local_error);

    /*	Close the configuration file. */

    if (config)
	status = dm0c_close(cnf, 0, &local_error);
	...
	if (idx_tcb->tcb_table_io.tbio_status & TBIO_OPEN)
	{
		...
		idx_tcb->tcb_table_io.tbio_loc_count, 0, &local_error);
	    if (status != E_DB_OK)
	    {
	...
	if (att_tcb->tcb_table_io.tbio_status & TBIO_OPEN)
	{
		...
		att_tcb->tcb_table_io.tbio_loc_count, 0, &local_error);
	    if (status != E_DB_OK)
	    {
		ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, 
                    (DB_SQLSTATE *)NULL, 
	...
	if (relx_tcb->tcb_table_io.tbio_status & TBIO_OPEN)
	{
		...
		relx_tcb->tcb_table_io.tbio_loc_count, 0, &local_error);
	    if (status != E_DB_OK)
	    {
		ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, 
                    (DB_SQLSTATE *)NULL, 
	...
	if (rel_tcb->tcb_table_io.tbio_status & TBIO_OPEN)
	{
		...
		rel_tcb->tcb_table_io.tbio_loc_count, 0, &local_error);
	    if (status != E_DB_OK)
	    {
		ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, 
                    (DB_SQLSTATE *)NULL, 
    ...
    */
    status = dm0p_dbcache(dcb->dcb_id, 0, 0, lock_list, &local_error);
    if (status != E_DB_OK)
    {
	ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, 
            (DB_SQLSTATE *)NULL, 
	...
	cl_status = LKrelease((i4)0, svcb_lock_list,
            (LK_LKID *)dcb->dcb_lock_id, (LK_LOCK_KEY *)0, &lock_value,
	    &sys_err);
	if (cl_status != OK)
	{
	    ule_format(cl_status, &sys_err, ULE_LOG, (DB_SQLSTATE *)NULL,
    ...
    if (*err_code > E_DM_INTERNAL || *err_code == E_DM0100_DB_INCONSISTENT)
    {
	/*	Log the change in error messages. */

	ule_format(*err_code, 0, ULE_LOG, (DB_SQLSTATE *)NULL, 
                   0, 0, 0, &local_error, 0);
	if (*err_code != E_DM0100_DB_INCONSISTENT)
	    *err_code = E_DM9268_OPEN_DB;
        else
        {
            ule_format (E_DM0152_DB_INCONSIST_DETAIL, (CL_ERR_DESC *)0, ULE_LOG,
                (DB_SQLSTATE *)0, (char *)0, (i4)0, (i4 *)0,
    ...
    i4		*err_code )
{
	...
	cl_status = LKrequest(LK_PHYSICAL | LK_NOWAIT, lock_list,
            (LK_LOCK_KEY *)&db_key, LK_X, (LK_VALUE *)0, (LK_LKID *)&dblkid,
            (i4)0, &sys_err);
	if (cl_status == OK)
    	{
	    /* Get the dbopen lock, used to determine if 
	...
	if (cl_status != OK) 
    	{
            ule_format(cl_status, &sys_err, ULE_LOG ,
                       (DB_SQLSTATE *)NULL, (char * )0, 0L, (i4 *)0, 
                       err_code, 1, 0, lock_list);
	    ule_format(E_DM901C_BAD_LOCK_REQUEST, (CL_ERR_DESC *) 0, ULE_LOG , 
		       (DB_SQLSTATE *)NULL, (char * )0, 0L, (i4 *)0, err_code, 
                       2, 0, LK_X, 0, lock_list );
	    *err_code = E_DM9269_LOCK_OPEN_DB;
            if (cl_status == LK_BUSY)
            {
                ule_format(*err_code, (CL_ERR_DESC *)0,
                    ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
                    &local_error, 0);
                *err_code = E_DM004C_LOCK_RESOURCE_BUSY;
            }
	...
	status = dm0c_open(dcb, 0, lock_list, &cnf, err_code);
	if (status != E_DB_OK)
	    break;
	config = cnf;
	...
	*/
	if ( loc_type & DCB_RAW )
	{
		...
		status = dm0m_allocate(
		    ...
		    (char*)dcb, (DM_OBJECT**)&raw_array, err_code);
		if ( status )
		    break;
	...
	for (i = 0; i < cnf->cnf_dsc->dsc_ext_count; i++)
	{
		...
		*raw_blocks = loc->raw_blocks;
		status = E_DB_ERROR;
		*err_code = E_DM007E_LOCATION_EXISTS;
		break;
	    }
	...
	if ( loc_type & DCB_RAW )
	{
		...
		*raw_start = 0;
		...
		ule_format(E_DM0193_RAW_LOCATION_EXTEND,
		    (CL_ERR_DESC*)0,
		    ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 4,
		    l_area, area,
		    0, raw_total_blocks -  raw_in_use,
		    DB_MAXNAME, location_name,
		    0, *raw_blocks); 
		status = E_DB_ERROR;
		*err_code = E_DM0193_RAW_LOCATION_EXTEND;
		break;
	    }
	...
	{
	    status = dm0c_extend(cnf, err_code);
	    if (status != E_DB_OK)
	    {
		*err_code = E_DM0071_LOCATIONS_TOO_MANY;	
		break;
	    }
	}
	...
	*/
	...
	cl_status = LKrequest(LK_PHYSICAL | LK_CONVERT, lock_list, 0, LK_N,
	    ...
	    ule_format(cl_status, &sys_err, ULE_LOG, (DB_SQLSTATE *)NULL,
            ...
            ule_format(E_DM901C_BAD_LOCK_REQUEST, &sys_err, ULE_LOG , 
                (DB_SQLSTATE *)NULL,
                (char * )0, 0L, (i4 *)0, err_code, 1, 0, lock_list); 
	    *err_code = E_DM9269_LOCK_OPEN_DB;
            status = E_DB_ERROR;
            break;
    	}
	...
	status = dmxe_begin(DMXE_WRITE, dmxe_flag, dcb, dcb->dcb_log_id,
	    ...
	    &tran_id, &tran_llid, (DB_DIS_TRAN_ID *)NULL, err_code);
	if (status != E_DB_OK)
	        break;
    	...
    	for (;;)
	{
	    ...
	    */
		...
		{
		    ule_format(cl_status, &sys_err,
			   ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 4,
			    db_length, db_buffer, 0, "", l_area, area, 
			    db_buffer, db_length);
		    ule_format(E_DM929D_BAD_DIR_CREATE, (CL_ERR_DESC *)0, 
			    ULE_LOG, (DB_SQLSTATE *)NULL, 0, 0, 0, err_code, 0);

		    *err_code = E_DM011E_ERROR_ADD_LOC;
		    status = E_DB_ERROR;
		    break;
		}
	    }
	    ...
	    status = dm0c_close(cnf, DM0C_UPDATE | DM0C_COPY, err_code);
	    if (status != E_DB_OK)
	        break;
	    config = 0;
	    ...
	    if (dblkid.lk_unique)
    	    {
    	   	...
    	   	{ 
    		    ule_format(cl_status, &sys_err, 
                           ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
                           (i4 *)0, err_code, 1, 0, lock_list);
                    ule_format(E_DM901B_BAD_LOCK_RELEASE, (CL_ERR_DESC *)0,
                          ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, 
                          (i4 *)0, err_code, 0);
	    	    status = E_DB_ERROR;
	    	    *err_code = E_DM9266_UNLOCK_CLOSE_DB;
    ...
    */
	...
	status = dm0c_close(cnf, 0, &local_error);
        if (status != E_DB_OK)
	    ule_format(local_error, (CL_ERR_DESC *)0,
                   ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
                   &local_error, 0);
	...
	status = dmxe_abort((DML_ODCB *)0, dcb, &tran_id, tran_lgid,
		    tran_llid, dmxe_flag,
		    0, 0, &local_error);
        if (status != E_DB_OK)
            ule_format(local_error, (CL_ERR_DESC *)0,
    ...
    {
	status = dm0l_closedb(dcb->dcb_log_id, &local_error);
        if (status != E_DB_OK)
            ule_format(local_error, (CL_ERR_DESC *)0,
                   ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
                   &local_error, 0);
    ...
    if (dcb && dcb->dcb_odb_lock_id[0]) 
    {
        ...
        cl_status = LKrelease(0, lock_list,
            ...
            (LK_VALUE *)0, &sys_err);
	if (cl_status != OK)
	    ule_format(cl_status, (CL_ERR_DESC *)&sys_err,
                   ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
    ...
    i4 		*err_code )
{
	...
	cl_status = LKrequest(LK_PHYSICAL | LK_NOWAIT, lock_list,
			(LK_LOCK_KEY *)&db_key, LK_X, (LK_VALUE *)0, 
	...
	{
	    ule_format(cl_status, &sys_err, ULE_LOG,
		(DB_SQLSTATE *)NULL, (char * )0, 0L, (i4 *)0, err_code, 1, 
                0, lock_list);
	    ule_format(E_DM901C_BAD_LOCK_REQUEST, (CL_ERR_DESC *) 0, ULE_LOG , 
		(DB_SQLSTATE *)NULL, (char * )0, 0L, (i4 *)0, err_code, 2, 0, 
                LK_X, 0, lock_list );
	    *err_code = E_DM9269_LOCK_OPEN_DB;
	    if (cl_status == LK_BUSY)
	    {
	...
	if ( loc == NULL )
	{
	    ...
	    *err_code = E_DM013D_LOC_NOT_EXTENDED;
	    status = E_DB_ERROR;
	    break;
	}
	     ...
	     LOwcard(&tmploc,NULL,NULL,NULL,&lc) == OK )
	{
	    ...
	    *err_code = E_DM019E_ERROR_FILE_EXIST;
	    status = E_DB_ERROR;
	    break;
	}
	LOwend(&lc);
	...
	*/
	...
	for (;;)
	{
		...
		*err_code = E_DM013D_LOC_NOT_EXTENDED;
	    ...
	    /* Last entry? */
	    if (i != (cnf->cnf_dsc->dsc_ext_count - 1))
	    {	    
		/* Not the last entry, shift the reminder over */
		MEcopy((char *)&cnf->cnf_ext[i+1].ext_location.logical,
			sizeof(DMP_LOC_ENTRY) *
			    (cnf->cnf_dsc->dsc_ext_count-i-1),
			(char *)loc);
		/* Point "i" to vacated last entry */
		i = cnf->cnf_dsc->dsc_ext_count-1;
	    }
    ...
    i4		*err_code )
{
	...
	if (locking)
	{
		...
		*/
		...
		ule_format(cl_status, &sys_err, ULE_LOG, (DB_SQLSTATE *)NULL, 
                    (char * )0, 0L, (i4 *)0, err_code, 1, 0, lock_list);
		ule_format(E_DM901C_BAD_LOCK_REQUEST, (CL_ERR_DESC *) 0,
		    ULE_LOG, (DB_SQLSTATE *)NULL, (char * )0, 0L, (i4 *)0, 
                    err_code, 0);
		*err_code = E_DM9269_LOCK_OPEN_DB;
		status = E_DB_ERROR;
		break;
	...
	*/
	...
	if (logging)
	{
		...
		&tran_llid, (DB_DIS_TRAN_ID *)NULL, err_code);
	    if (status != E_DB_OK)
		    break;
	}
	...
	switch (dm2d->alter_op)
	{
	  case DM2D_TAS_ALTER:
		...
		{
		    ule_format(E_DM92A2_DM2D_LAST_TBID, 0, ULE_LOG, 
                        (DB_SQLSTATE *)NULL,
			(char * )0, 0L, (i4 *)0, &local_error, 3,
	  ...
	  case DM2D_EXT_ALTER:
		...
		status = dm0l_ext_alter(tran_lgid, journal, drop_loc_type,
		    add_loc_type, dm2d->location_name, (LG_LSN *)0, err_code);
		if (status != E_DB_OK)
		    break;
	    }
		...
		*/ 
		if ((tflags & drop_loc_type) == 0)
		{
		    ...
		    */
		    ule_format(E_DM92A1_DM2D_EXT_MISMATCH, 0, ULE_LOG, 
                        (DB_SQLSTATE *)NULL,
    ...
    */
	...
	status = dm0l_closedb(dcb->dcb_log_id, &local_error);
        ...
        if (status != E_DB_OK)
            ule_format(local_error, (CL_ERR_DESC *)0, ULE_LOG, 
	...
	if (cl_status != OK)
	    ule_format(cl_status, (CL_ERR_DESC *)&sys_err, ULE_LOG, 
                (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, 1, 0,
		lock_list);
    }
    ...
    if (*err_code > E_DM_INTERNAL)
    {
	    ...
	    ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
	    &local_error, 0);
        *err_code = E_DM929F_DM2D_ALTER_DB;
    }
    ...
    i4		*err_code )
{
    ...
    status = dm2t_alloc_tcb(rel, (i4)1, dcb, tcb, err_code);
    if (status != E_DB_OK)
	return (status);
    ...
    */
	...
	t->tcb_data_atts[i] = &(t->tcb_atts_ptr[i + 1]);
    ...
    */
    ...
    if (status == E_DB_ERROR && *err_code == E_DM0114_FILE_NOT_FOUND)
    {
    ...
    i4	    *err_code )
{
	...
	*/
	...
	{
	    status = dm2r_get(
		rcb, &tid, DM2R_GETNEXT, (char *)&relrecord, err_code);
    ...
    i4	    *err_code )
{
	...
	*/
	...
	{
	    status = dm2r_get(
		rcb, &tid, DM2R_GETNEXT, (char *)&iiatt_record, err_code);
    ...
    STATUS	*status )
{
	...
	status = &locstat;
    /* Get LongTerm memory */
    *status = dm0m_allocate((i4)size + sizeof(DMP_MISC),
    ...
    i4	*err_code )
{
	...
	*/
	...
	if (cl_status != OK)
	{
	    ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG,
		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
		err_code, 1, 0, LG_S_DATABASE);

	    status = E_DB_ERROR;
	    *err_code = E_DM9500_DMXE_BEGIN;
    ...
    i4	    	*err_code )
{
	...
	*/
	status = dm2r_rcb_allocate(
			dcb->dcb_rel_tcb_ptr,
			0, &tran_id, lock_list, 0 , &rel_rcb, err_code );
	if ( status != E_DB_OK )
	    break;
	...
	*/
	...
	do
	{
	    ...
	    */
		...
		status = dm1p_convert_table_to_65( table_tcb, err_code);
		if (status != E_DB_OK )
		    break;
		...
		** close the table 
		*/
		status = dm2t_close( table_rcb, DM2T_PURGE, err_code);
		if ( status != E_DB_OK )
		    break;
    ...
    */
	...
	s = dm2t_close( table_rcb, DM2T_PURGE, &lerror);
    ...
    */
	...
	s = dm2r_release_rcb( &rel_rcb, &lerror);
    }

    dm2d_log_error( E_DM92FE_DM2D_CONV_DBMS_CATS, err_code);
    return( status );
}

/*
    ...
    i4	*err_code )
{
	...
	*/
	status = dm1p_convert_table_to_65( tcb, err_code );
	if ( status != E_DB_OK )
	    break;
	...
	*/
	...
	status = dm2u_conv_rel_update( tcb, lock_list, &tran_id, err_code );
	if ( status != E_DB_OK )
	    break;
    ...
    if ( status != E_DB_OK )
        dm2d_log_error( E_DM92FD_DM2D_CONV_CORE_TABLE, err_code);
    ...
    i4	*err_code )
{
	...
	*/
	ule_format(terr_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL, 
            (char *)0, (i4)0, (i4 *)0, &l_err_code, 0);
    }
    ...
    i4	*err_code )
{
	...
	*/
	status = dm2d_convert_core_table_to_65( rel_tcb, lock_list, err_code);
	if ( status != E_DB_OK )
	    break;
	...
	*/
	status = dm2d_convert_core_table_to_65( relx_tcb, lock_list, err_code);
    	if ( status != E_DB_OK )
	    break;
...
{
    DB_DEF_ID		*defaultID = &newTuple->attDefaultID;
...
)
{
	...
	** Database directory not present...
        */
	*err_code = E_DM9444_DMD_DIR_NOT_PRESENT;
        status = E_DB_ERROR;
    }
    return (status);
}
...

/*{
** Name: dm2d_convert_preV7_iirel - convert iirelation to V7 format
**
** Description:
    ...
    i4	    *err_code )
{
    ...
    DI_IO	    iirel_di_io;
    DMPP_PAGE	    *iirel_pg = (DMPP_PAGE *)0;
    i4	   	    io_count;
    DMP_RELATION_26 old_iirel;
    DMP_RELATION_26 *old_iirel_ptr;
    DMP_RELATION    new_iirel;
    DMP_RELATION    *old_as_new;
    DM_TID	    iirel_tid;
    DMPP_ACC_PLV    *local_plv;
    DMPP_ACC_TLV    *local_tlv;
	...
	*/
	if ((flag & DM0C_CNF_LOCKED) == 0)
	{
		...
		(i4)0, (LK_UNIQUE *)0, (LK_LLID *)&temp_lock_list, 0, &sys_err);
	...
	*/
	db_status = dm0c_open(dcb, flag, temp_lock_list, &config, err_code);
	...
	*/
	...
	dm1c_getaccessors(TCB_C_NONE, iirel_pgformat, iirel_pgsize,
			  &local_plv, &local_tlv);
	...
	for (;;)
	{
		...
		/*
		** Don't convert if this already looks like the new format;
		** new relpgsize is where old relloccount was so there's
		** little likelihood of confusion!
		*/
		old_as_new = (DMP_RELATION *)&old_iirel;
		if (old_as_new->relpgsize == 2048 || 
			old_as_new->relpgsize == 4096 ||
			old_as_new->relpgsize == 8192 ||
			old_as_new->relpgsize == 16384 ||
			old_as_new->relpgsize == 65536)
		{
		    continue;
		}
		...
		if (create_version < DSC_V6)
		{
		    if (old_iirel.relpgsize == 0)
		    {
			old_iirel.relpgsize = DM_COMPAT_PGSIZE;
			old_iirel.relpgtype = TCB_PG_V1;
		    } else if (old_iirel.relpgsize == DM_COMPAT_PGSIZE)
			old_iirel.relpgtype = TCB_PG_V1;
		    else
			old_iirel.relpgtype = TCB_PG_V2;
		    if (create_version < DSC_V4)
		    {
			old_iirel.reltotwid = old_iirel.relwid;
			old_iirel.relversion = 0;
			old_iirel.reltcpri = 0;
		    }
		}
		    
		/* Copy all fields in DMP_RELATION26 to DMP_RELATION */
		MEfill(sizeof(DMP_RELATION), '\0', &new_iirel);
		STRUCT_ASSIGN_MACRO(old_iirel.reltid, new_iirel.reltid);
		STRUCT_ASSIGN_MACRO(old_iirel.relid, new_iirel.relid);
		STRUCT_ASSIGN_MACRO(old_iirel.relowner, new_iirel.relowner);
		new_iirel.relatts = old_iirel.relatts;
		new_iirel.relwid = old_iirel.relwid;
		new_iirel.relkeys = old_iirel.relkeys;
		new_iirel.relspec = old_iirel.relspec;
		new_iirel.relstat = old_iirel.relstat;
		new_iirel.reltups = old_iirel.reltups;
		new_iirel.relpages = old_iirel.relpages;
		new_iirel.relprim = old_iirel.relprim;
		new_iirel.relmain = old_iirel.relmain;
		new_iirel.relsave = old_iirel.relsave;
		STRUCT_ASSIGN_MACRO(old_iirel.relstamp12, new_iirel.relstamp12);
		STRUCT_ASSIGN_MACRO(old_iirel.relloc, new_iirel.relloc);
		new_iirel.relcmptlvl = old_iirel.relcmptlvl;
		new_iirel.relcreate = old_iirel.relcreate;
		new_iirel.relqid1 = old_iirel.relqid1;
		new_iirel.relqid2 = old_iirel.relqid2;
		new_iirel.relmoddate = old_iirel.relmoddate;
		new_iirel.relidxcount = old_iirel.relidxcount;
		new_iirel.relifill = old_iirel.relifill;
		new_iirel.reldfill = old_iirel.reldfill;
		new_iirel.rellfill = old_iirel.rellfill;
		new_iirel.relmin = old_iirel.relmin;
		new_iirel.relmax = old_iirel.relmax;
		new_iirel.relloccount = old_iirel.relloccount;
		new_iirel.relgwid = old_iirel.relgwid;
		new_iirel.relgwother = old_iirel.relgwother;
		new_iirel.relhigh_logkey = old_iirel.relhigh_logkey;
		new_iirel.rellow_logkey = old_iirel.rellow_logkey;
		new_iirel.relfhdr = old_iirel.relfhdr;
		new_iirel.relallocation = old_iirel.relallocation;
		new_iirel.relextend = old_iirel.relextend;
		new_iirel.relcomptype = old_iirel.relcomptype;
		new_iirel.relpgtype = old_iirel.relpgtype;
		new_iirel.relstat2 = old_iirel.relstat2;
		new_iirel.relpgsize = old_iirel.relpgsize;
		new_iirel.relversion = old_iirel.relversion;
		new_iirel.reltotwid = old_iirel.reltotwid;
		new_iirel.reltcpri = old_iirel.reltcpri;

		/*
		** Now initialize new_iirel fields added for table partitioning
		...
		*/
		new_iirel.relstat &= ~TCB_IS_PARTITIONED;
		new_iirel.relstat2 &= ~(TCB2_PARTITION | TCB2_GLOBAL_INDEX);
		new_iirel.relnparts = 0;
		new_iirel.relnpartlevels = 0;

		/* Adjust relatts if this is an iirelation or iiattribute
		** relation row.
		*/
		if (new_iirel.reltid.db_tab_base == DM_B_RELATION_TAB_ID
		  && new_iirel.reltid.db_tab_index == DM_I_RELATION_TAB_ID)
		{
		    /* iirelation doesn't change width but does change number
		    ** attributes.
		    */
		    new_iirel.relatts = DM_core_relations[0].relatts;
		}
		    ...
		    */
			...
			if ((DM_core_relations[n].reltid.db_tab_base ==
			  ...
			  (DM_core_relations[n].reltid.db_tab_index ==
						DM_I_ATTRIBUTE_TAB_ID) )
			{
			    new_iirel.relatts = DM_core_relations[n].relatts;
			    new_iirel.relwid = DM_core_relations[n].relwid;
			    new_iirel.reltotwid = new_iirel.relwid;
			    break;
			}
		    }
		}
		...
		(*local_plv->dmpp_put)(iirel_pgformat, iirel_pgsize, iirel_pg,
			DM1C_DIRECT, &upgrade_tran, &tid,
			sizeof(new_iirel), (char *)&new_iirel,
			(i4)TCB_C_NONE, /* no compression */
	    ...
	    if (DMPP_VPT_GET_PAGE_OVFL_MACRO(iirel_pgformat, iirel_pg) != NULL)
		curpagenum = 
			DMPP_VPT_GET_PAGE_OVFL_MACRO(iirel_pgformat, iirel_pg);
	    /* No more overflow buckets. Go to the next hash bucket. */
	    else if (DMPP_VPT_GET_PAGE_MAIN_MACRO(iirel_pgformat, iirel_pg) != NULL)
		curpagenum = 
	...
	*/
	...
	if ((flag & DM0C_CNF_LOCKED) == 0)
	{
	    status = LKrelease(LK_ALL, temp_lock_list, (LK_LKID *)0,
			(LK_LOCK_KEY *)0, (LK_VALUE *)0, &sys_err);
...

/*{
** Name: dm2d_convert_preV7_iiatt - convert iiattribute to V8 format
**
** Description:
**      Used indirectly by upgradedb to convert iiattribute to V8 format
...
**
**	Since there have been various changes in iirelation and iiattribute
**	columns and their positions, we'll take the brute force approach
**	of writing a brand new (one-bucket hash) iiattribute.  That way
**	we can regenerate all the core catalog attribute info from the
**	builtin arrays, and upgradedb doesn't have to.
**
**	Note that the iirelation converter (either _from_64 or _preV7)
**	has anticipated this, and has set iiattribute's relprim to 1,
**	and invalidated the FHDR page number.  Remember, this rewrite
**	occurs at a very low physical level, avoiding all the usual DMF
**	buffering and table handling.
**
** Inputs:
**      db_id                           database to upgrade
**
...
**          Core catalog upgrade for 256k rows, rows spanning pages
**	25-Aug-2004 (schka24)
**	    Redo to model after old 6.4 conversion, ie rewrite iiattribute
**	    entirely.  Easier/safer than scary mucking around with core
**	    catalog attributes in upgradedb.
[@history_template@]...
*/
static DB_STATUS
dm2d_convert_preV7_iiatt(
    ...
    i4	    *err_code )
{
    ...
    char	    *old_att;
    DMP_OLDATTRIBUTE *old_64_att;
    DMP_ATTRIBUTE_1X *old_1x_att;
    DMP_ATTRIBUTE_26 *old_26_att;
    DMP_ATTRIBUTE_26 old_att_row;	/* See below, at "get" */
    DM_TID	    iiatt_tid;
    DMPP_ACC_PLV    *local_plv;
    DMPP_ACC_TLV    *local_tlv;
    DM_TID	    tid;
	...
	*/
	...
	if  ((config->cnf_dsc->dsc_status & DSC_IIATT_V7_CONV_DONE) != 0)
	{
	    /* We've already completed the conversion. */
	...
	*/
	...
	for (i=0;;)
	{
		...
		*/
		MEfill(sizeof(new_att), 0, (PTR) &new_att);
		if (create_version < DSC_V3)
		{
		    old_64_att = (DMP_OLDATTRIBUTE *) old_att;
		    if (old_64_att->attrelid.db_tab_base <= DM_B_INDEX_TAB_ID)
			continue;
		    STRUCT_ASSIGN_MACRO(old_64_att->attrelid, new_att.attrelid);
		    new_att.attid = old_64_att->attid;
		    new_att.attxtra = old_64_att->attxtra;
		    dm2d_cname((char *)&new_att.attname, old_64_att->attname);
		    new_att.attoff = old_64_att->attoff;
		    new_att.attfmt = old_64_att->attfmt;
		    new_att.attfml = old_64_att->attfml;
		    new_att.attfmp = old_64_att->attfmp;
		    new_att.attkey = old_64_att->attkey;
		    new_att.attflag = old_64_att->attflag;
		    new_att.attintl_id = old_64_att->attid;
		    makeDefaultID( &new_att, old_64_att );
		    if ( old_64_att->attfmt > 0 )
			new_att.attflag |= ATT_KNOWN_NOT_NULLABLE;
		}
		else if (create_version == DSC_V3)
		{
		    old_1x_att = (DMP_ATTRIBUTE_1X *) old_att;
		    if (old_1x_att->attrelid.db_tab_base <= DM_B_INDEX_TAB_ID)
			continue;
		    STRUCT_ASSIGN_MACRO(old_1x_att->attrelid, new_att.attrelid);
		    new_att.attid = old_1x_att->attid;
		    new_att.attxtra = old_1x_att->attxtra;
		    STRUCT_ASSIGN_MACRO(old_1x_att->attname, new_att.attname);
		    new_att.attoff = old_1x_att->attoff;
		    new_att.attfmt = old_1x_att->attfmt;
		    new_att.attfml = old_1x_att->attfml;
		    new_att.attfmp = old_1x_att->attfmp;
		    new_att.attkey = old_1x_att->attkey;
		    new_att.attflag = old_1x_att->attflag;
		    STRUCT_ASSIGN_MACRO(old_1x_att->attDefaultID, new_att.attDefaultID);
		    new_att.attintl_id = old_1x_att->attid;
		}
		else
		{
		    old_26_att = (DMP_ATTRIBUTE_26 *) old_att;
		    if (old_26_att->attrelid.db_tab_base <= DM_B_INDEX_TAB_ID)
			continue;
		    STRUCT_ASSIGN_MACRO(old_26_att->attrelid, new_att.attrelid);
		    new_att.attid = old_26_att->attid;
		    new_att.attxtra = old_26_att->attxtra;
		    STRUCT_ASSIGN_MACRO(old_26_att->attname, new_att.attname);
		    new_att.attoff = old_26_att->attoff;
		    new_att.attfmt = old_26_att->attfmt;
		    new_att.attfml = old_26_att->attfml;
		    new_att.attfmp = old_26_att->attfmp;
		    new_att.attkey = old_26_att->attkey;
		    new_att.attflag = old_26_att->attflag;
		    STRUCT_ASSIGN_MACRO(old_26_att->attDefaultID, new_att.attDefaultID);
		    new_att.attintl_id = old_26_att->attintl_id;
		    new_att.attver_added = old_26_att->attver_added;
		    new_att.attver_dropped = old_26_att->attver_dropped;
		    new_att.attval_from = old_26_att->attval_from;
		}

		/* Make security label columns into CHAR so ADF doesn't
		** barf on them.
		*/
		if (abs(new_att.attfmt) == 60 || abs(new_att.attfmt) == 61)
		{
		    i4 old_type = new_att.attfmt;

		    new_att.attfmt = DB_CHA_TYPE;
		    if (old_type < 0) new_att.attfmt = -DB_CHA_TYPE;
		    if (abs(old_type) == 61) new_att.attfml = 8;
		}
		/* No label attributes any more */
		new_att.attflag &= ~(0x4000);	/* clear SEC_LBL */
		...
		*/
	    ...
	    if (DMPP_VPT_GET_PAGE_OVFL_MACRO(iiatt_pgformat, tmp_pg) != NULL)
	    ...
	    */
	    ...
	    else if (DMPP_VPT_GET_PAGE_MAIN_MACRO(iiatt_pgformat, tmp_pg) != NULL) 
    ...
    *err_code = E_DM93F0_CONVERT_IIATT;
    return (E_DB_ERROR);
}

/*{
	...
	*/
	...
	for (i=0;;)
	{
		...
		*/
	    ...
	    if (DMPP_VPT_GET_PAGE_OVFL_MACRO(iiind_pgformat, tmp_pg) != NULL)
	    ...
	    */
	    ...
	    else if (DMPP_VPT_GET_PAGE_MAIN_MACRO(iiind_pgformat, tmp_pg) != NULL) 
