    ...
    i4		    *err_code)
{
            ...
            !(flag & DM0L_2ND_BT))
	{
	    ...
	    {
		ret_status = log_error(E_DM9017_BAD_LOG_SHOW, &error, lg_id, 0,
		    E_DM9211_DM0L_BT, err_code, status);
		return (ret_status);
	    }
		...
		if (status != E_DB_OK)
		{
			...
			lg_id, 0, E_DM9211_DM0L_BT, err_code, status);
	...
	*/
    ...
    if (status != OK)
    {
	(VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
        ...
        if (status == LK_DEADLOCK)
        {
           ...
           return( log_error(E_DM9211_DM0L_BT, &error, lg_id, 0,
                             E_DM0042_DEADLOCK, err_code, status));
        }

	return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	    E_DM9211_DM0L_BT, err_code, status));
    }
    ...
    status = LGwrite(lg_flags, lg_id, (i4)1, &lg_object, lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
    ...
    ret_status = log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	    E_DM9211_DM0L_BT, err_code, status);
    return(ret_status);
}

    ...
    i4		*err_code)
{
	...
	*/
	...
	status = LGwrite(action, lg_id, (i4)1, &lg_object, &lsn, &error);
	if (status != OK)
	{
	    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
		    E_DM9213_DM0L_ET, err_code, status));
	}
	...
	** Commit Timestamp.
	*/
	ctime->db_tab_high_time = lsn.lsn_high;
	ctime->db_tab_low_time =  lsn.lsn_low;
	...
	status = LGshow(LG_S_STAMP, (PTR)ctime, sizeof(*ctime), &length,
			&error);
	if (status)
	    _VOID_ log_error(E_DM9017_BAD_LOG_SHOW, &error, LG_S_STAMP, 0,
			E_DM9213_DM0L_ET, err_code, status);

	status = LGend(lg_id, 0, &error);
	if (status != OK)
	    return (log_error(E_DM900E_BAD_LOG_END, &error, lg_id, 0,
		    E_DM9213_DM0L_ET, err_code, status));
    }
    return (E_DB_OK);
...
i4		*err_code)
{
    ...
    status = LGwrite(0, log_id, (i4)2, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, log_id, 0,
	E_DM9214_DM0L_BI, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, log_id, 0,
	E_DM9C0B_DM0L_AI, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(0, log_id, num_objects, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, log_id, 0,
	E_DM9215_DM0L_PUT, err_code, status));
}
...
i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(0, log_id, (i4)3, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, log_id, 0,
	E_DM9C15_DM0L_NOFULL, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(0, log_id, (i4)4, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, log_id, 0,
	E_DM9216_DM0L_DEL, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(0, log_id, obj_num, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, log_id, 0,
	E_DM9217_DM0L_REP, err_code, status));
}

/*{
    ...
    i4           *err_code)
{
    ...
    */
    ...
    return(log_error(E_DM900B_BAD_LOG_ALTER, &sys_err, LG_A_BMINI, 0,
	E_DM921C_DM0L_BM, err_code, status));
}

/*{
    ...
    i4	    *err_code)
{
    ...
    */
	...
	return(log_error(E_DM900B_BAD_LOG_ALTER, &error, LG_A_EMINI, 0,
	    E_DM921D_DM0L_EM, err_code, status));
    }

    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, rcb->rcb_log_id, 0,
	E_DM921D_DM0L_EM, err_code, status));
}

/*{
    ...
    i4             *err_code)
{
    ...
    return(log_error(E_DM9017_BAD_LOG_SHOW, &sys_err, LG_S_LASTLSN, 0,
		E_DM9221_DM0L_SAVEPOINT, err_code, status));
}

/*{
    ...
    i4	    *err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9222_DM0L_ABORTSAVE, err_code, status));
}

/*{
    ...
    i4             *err_code)
{
    ...
    */
	...
	if (status != OK)
	{
		...
		lg_id, 0, E_DM9223_DM0L_FRENAME, err_code, status));
	}
    }
    ...
    status = LGwrite(LG_FORCE, lg_id, (i4)1, &lg_object, (LG_LSN*)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9223_DM0L_FRENAME, err_code, status));
}

/*{
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM9224_DM0L_FCREATE, err_code, status));
	}
    }
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9224_DM0L_FCREATE, err_code, status));
}

/*{
    ...
    i4		    *err_code)
{
    ...
    */
    ...
    if (status != OK)
    {
	(VOID) ule_format(status, &error, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	(VOID) ule_format(E_DM900A_BAD_LOG_DBADD, &error, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, err_code, 4, 0, lg_id,
    ...
    */
    ...
    MEcopy((PTR)DB_OPENDB_NAME, sizeof(DB_OWN_NAME), (PTR) &user_name);
    status = LGbegin(LG_NOPROTECT, *db_id, &tran_id, &lx_id,
		     sizeof(DB_OWN_NAME), (char *)&user_name, 
		     (DB_DIS_TRAN_ID*)NULL, &error);
    if (status != OK)
    {
	(VOID) ule_format(E_DM900C_BAD_LOG_BEGIN, &error, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, err_code, 1, 0, *db_id);
	...
	if (status)
	    (void) log_error(E_DM9016_BAD_LOG_REMOVE, &error, *db_id, 0,
		    E_DM9225_DM0L_OPENDB, &loc_error, status);
    ...
    */
    ...
    status = LGwrite((LG_FIRST | LG_FORCE | LG_NOT_RESERVED | LG_LAST), lx_id, (i4)1, &lg_object,
	&lsn, &error);
    if (status != OK)
    {
 	ret_status = log_error(E_DM9015_BAD_LOG_WRITE, &error, lx_id, 0,
	    E_DM9225_DM0L_OPENDB, err_code, status);
	status = LGend(lx_id, 0, &error);
	if (status)
	    (void) log_error(E_DM900E_BAD_LOG_END, &error, lx_id, 0,
		    E_DM9225_DM0L_OPENDB, &loc_error, status);
	status = LGremove(*db_id, &error);
	if (status)
	    (void) log_error(E_DM9016_BAD_LOG_REMOVE, &error, *db_id, 0,
    ...
    i4		    *err_code)
{
    ...
    if (status != OK)
    {
	ret_status = log_error(E_DM9016_BAD_LOG_REMOVE, &error, db_id, 0,
	    E_DM9226_DM0L_CLOSEDB, err_code, status);
    }
    return (ret_status);
}

    ...
    i4		*err_code)
{
	...
	*/
	...
	if (plog == NULL)
	{
	    (VOID) ule_format(E_DM911E_ALLOC_FAILURE, &error,
		ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
		&loc_error, 1, 0, lg_id);
	    return(log_error(E_DM911E_ALLOC_FAILURE, &error,
		lg_id, 0, E_DM9229_DM0L_CREATE, err_code, status));
	}
    ...
    */
    ...
    status = LGwrite(0, lg_id, (i4)1, &lg_object, (LG_LSN *)lsn, &error);
    if (plog != &log)
	MEfree((void*)plog);
    if (status == OK)
    {
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9229_DM0L_CREATE, err_code, status));
}

/*{
    ...
    i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(LG_NOT_RESERVED, lg_id, (i4)1, &lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C0C_DM0L_CRVERIFY, err_code, status));
}

/*{
    ...
    i4	    *err_code)
{
	...
	*/
	...
	if (plog == NULL)
	{
		...
		&loc_error, 1, 0, lg_id);
	    return(log_error(E_DM911E_ALLOC_FAILURE, &error,
		lg_id, 0, E_DM922A_DM0L_DESTROY, err_code, status));
	}
	MEcopy(&log, sizeof(DM0L_CREATE), plog);
    }
    for (i = 0; i < loc_count; i++)
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM922A_DM0L_DESTROY, err_code, status));
}

/*{
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    err_code, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM922B_DM0L_RELOCATE, err_code, status));
	}
    }
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM922B_DM0L_RELOCATE, err_code, status));
}

/*{
    ...
    i4		    *err_code)
{
    ...
    MEcopy(key_map, DB_MAX_COLS, log.dum_key);
	...
	*/
	...
	if (plog == NULL)
	{
	    (VOID) ule_format(E_DM911E_ALLOC_FAILURE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM911E_ALLOC_FAILURE, &error,
		lg_id, 0, E_DM922C_DM0L_MODIFY, err_code, status));
	}
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
    	E_DM922C_DM0L_MODIFY, err_code, status));
}

/*{
    ...
    i4		    *err_code)
{
    ...
    MEcopy(key_map, DB_MAXIXATTS, log.dui_key);
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM922D_DM0L_INDEX, err_code, status));
	}
    }
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM922D_DM0L_INDEX, err_code, status));
}

/*{
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM932F_DM0L_ALTER, err_code, status));
	}
    }
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM932F_DM0L_ALTER, err_code, status));
}

/*{
    ...
    i4	    *err_code)
{
	...
	if (status != OK)
	    return (log_error(E_DM9013_BAD_LOG_POSITION, &sys_err, lx_id, 0,
		    E_DM922E_DM0L_POSITION, err_code, status));
    }
    ...
    i4	    *err_code)
{
    ...
    status = LGread(lx_id, lctx->lctx_area, lctx->lctx_l_area,
	    (LG_RECORD **)&lctx->lctx_header, lga, &sys_err);
    if (status != OK)
    {
	if (status == LG_ENDOFILE)
	{
	    *err_code = E_DM9238_LOG_READ_EOF;
	    return (E_DB_WARN);
	}
	(VOID) ule_format(status, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, err_code, 1, 0, lx_id);

	return (log_error(E_DM9014_BAD_LOG_READ, &sys_err, lx_id, 0,
		    E_DM922F_DM0L_READ, err_code, status));
    }
    ...
    i4	    *err_code)
{
    ...
    if (status == OK)
	return (E_DB_OK);
    return (log_error(E_DM9010_BAD_LOG_FORCE, &sys_err, lg_id, 0,
	E_DM9230_DM0L_FORCE, err_code, status));
}

/*{
...
**	lg_id				Log system id.
**      name                            Log name for VAXcluster.
**      l_name                          Length of name.
**
** Outputs:
**      err_code                        Reason for error return status.
    ...
    i4             *err_code)
{
	...
	(VOID) ule_format(E_DM9011_BAD_LOG_INIT, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, err_code, 0);

	*err_code = E_DM9232_DM0L_ALLOCATE;
	return (E_DB_ERROR);
    }
    ...
    */
    ...
    {
	(VOID) ule_format(E_DM9426_LGADD_SIZE_MISMATCH, &sys_err,
	...
	*err_code = E_DM9232_DM0L_ALLOCATE;
	...
	if ((flag & DM0L_NOLOG) == 0)
	{
	    ...
	    */
		...
		ule_format(status, &sys_err, ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);
		(VOID) ule_format(E_DM9012_BAD_LOG_OPEN, &sys_err,
			ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);

		if (status == LG_NOTLOADED)
		    *err_code = E_DM0081_NO_LOGGING_SYSTEM;
		else if (status == LG_EXCEED_LIMIT)
		    *err_code = E_DM0062_TRAN_QUOTA_EXCEEDED;
		else
		    *err_code = E_DM9232_DM0L_ALLOCATE;
	...
	if (flag & DM0L_ARCHIVER)
	{
	    ...
	    MEcopy((PTR)DB_ARCHIVER_NAME, DB_MAXNAME,
	    ...
	    {
		ule_format(status, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
		(VOID) ule_format(E_DM900A_BAD_LOG_DBADD, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 4, 0, lg_id,
		...
		*err_code = E_DM9232_DM0L_ALLOCATE;
	    ...
	    MEcopy((PTR)DB_ALLOCATE_NAME, sizeof(DB_OWN_NAME),
		    (PTR) &user_name);
	    status = LGbegin(LG_NOPROTECT, db_id, &tran_id, &lx_id,
			sizeof(DB_OWN_NAME), (char *)&user_name, 
		...
		ule_format(status, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
		return (log_error(E_DM900C_BAD_LOG_BEGIN, &sys_err, db_id, 0,
			E_DM9232_DM0L_ALLOCATE, err_code, status));
	    }
		...
		return (log_error(E_DM900F_BAD_LOG_EVENT, &sys_err,
			LG_E_ONLINE | LG_E_IMM_SHUTDOWN, 0,
			E_DM9232_DM0L_ALLOCATE, err_code, status));
	    }
		...
		ule_format(status, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
		return (log_error(E_DM900E_BAD_LOG_END, &sys_err, lx_id, 0,
			E_DM9232_DM0L_ALLOCATE, err_code, status));
	    }
		...
		ule_format(status, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
		return (log_error(E_DM9016_BAD_LOG_REMOVE, &sys_err, db_id, 0,
		    E_DM9232_DM0L_ALLOCATE, err_code, status));
	    }
	}
	/*
	...
	*/
	...
	if (status != OK)
	{
	    ule_format(status, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
	    return (log_error(E_DM9017_BAD_LOG_SHOW, &sys_err, LG_A_HEADER, 0,
		    E_DM9232_DM0L_ALLOCATE, err_code, status));
	}
	size = header->lgh_size;
    }
    for (;;)
    {
	/*  Allocate the LCTX. */

	status = dm0m_allocate(sizeof(DMP_LCTX) + LG_MAX_CTX + LG_MAX_RSZ +
	    size, 0, LCTX_CB, LCTX_ASCII_ID, (char *)&dmf_svcb,
	    (DM_OBJECT **)&lctx, err_code);
	if (status != E_DB_OK)
	    break;

	lctx->lctx_area = (char *)lctx + sizeof(DMP_LCTX);
	lctx->lctx_l_area = LG_MAX_CTX + LG_MAX_RSZ + size;
	...
	if (status != OK)
	{
	    ...
	    ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 2, 0,
		    LG_A_NODEID, 0, 0);
	    *err_code = E_DM9232_DM0L_ALLOCATE;/* FIX ME: NEED A New msg */
	    break;
	}
	...
	*/
	if (flag & DM0L_RECOVER)
	{
	    ...
	    MEcopy((PTR)DB_RECOVERY_NAME, DB_MAXNAME,
			(PTR) &add_info.ad_dbname);
	    MEcopy((PTR)DB_INGRES_NAME, DB_MAXNAME,
			(PTR) &add_info.ad_dbowner);
	    MEcopy((PTR)"None", 4, (PTR) &add_info.ad_root);
		...
		(VOID) ule_format(E_DM900A_BAD_LOG_DBADD, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 4, 0, lg_id,
		    0, DB_RECOVERY_NAME, 0, DB_INGRES_NAME, 0, "None");
	...
	if (status != OK)
	{
	    ule_format(status, &sys_err, ULE_LOG,
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
	    return (log_error(E_DM900D_BAD_LOG_CLOSE, &sys_err, lg_id, 0,
		    E_DM9232_DM0L_ALLOCATE, err_code, status));
	}
	else
	{
	    (VOID) ule_format(*err_code, 0, ULE_LOG, NULL,
				(char *)0, 0L, (i4 *)0, err_code, 0);
	    return (E_DB_ERROR);
	}
    }
    ...
    i4             *err_code)
{
	...
	if (status != OK)
	{
	    ...
	    return (log_error(0, &sys_err,
		    (i4)l->lctx_lxid, 0,
		    E_DM9233_DM0L_DEALLOCATE, err_code, status));
	}
    }
    ...
    i4	    *err_code)
{
    ...
    */
    ...
    status = LGalter(LG_A_BCPDONE, (PTR)&false, sizeof(false), &sys_err);
    if (status)
    {
	dm0m_deallocate(&mem_ptr);
	return (log_error(E_DM900B_BAD_LOG_ALTER, &sys_err, LG_A_BCPDONE, 0,
			E_DM9235_DM0L_BCP, err_code, status));
    }
    ...
    {
	dm0m_deallocate(&mem_ptr);
	return (log_error(E_DM9017_BAD_LOG_SHOW, &sys_err, LG_A_EOF, 0,
		    E_DM9235_DM0L_BCP, err_code, status));
    }
	...
	{
	    dm0m_deallocate(&mem_ptr);
	    return (log_error(E_DM9017_BAD_LOG_SHOW, &sys_err, LG_N_DATABASE, 0,
			    E_DM9235_DM0L_BCP, err_code, status));
	}
	...
	if (i >= count)
	{
	    ...
	    status = LGwrite(LG_FIRST | LG_NOT_RESERVED, lg_id, (i4)1, &lg_object, &lsn,
		&sys_err);
	    if (!firstwrite)
	    {
		firstwrite = 1;
		*bcp_lsn = lsn;
	    ...
	    {
		dm0m_deallocate(&mem_ptr);
		return (log_error(E_DM9015_BAD_LOG_WRITE, &sys_err, lg_id, 0,
			    E_DM9235_DM0L_BCP, err_code, status));
	    }
    ...
    */
	...
	status = LGwrite(LG_FIRST | LG_NOT_RESERVED, lg_id, (i4)1, &lg_object, &lsn, &sys_err);
	if (!firstwrite)
        {
	    firstwrite = 1;
	    *bcp_lsn = lsn;
	...
	*bcp_lsn = lsn;
    ...
    status = LGalter(LG_A_BCPDONE, (PTR)&true, sizeof(true), &sys_err);
    if (status)
	return (log_error(E_DM900B_BAD_LOG_ALTER, &sys_err, LG_A_BCPDONE, 0,
			E_DM9235_DM0L_BCP, err_code, status));
    ...
    i4	    *err_code)
{
    ...
    status = LGalter(LG_A_ECPDONE, (PTR)&false, sizeof(false), &sys_err);
    if (status)
	return (log_error(E_DM900B_BAD_LOG_ALTER, &sys_err, LG_A_ECPDONE, 0,
			E_DM9343_DM0L_ECP, err_code, status));

    /* Get the BOF. */

    status = LGshow(LG_A_BOF, (PTR)&bof, sizeof(bof), &length, &sys_err);
    if (status)
	return (log_error(E_DM9017_BAD_LOG_SHOW, &sys_err, LG_A_BOF, 0,
			E_DM9343_DM0L_ECP, err_code, status));
    ...
    */
    ...
    if (status)
	return (log_error(E_DM900B_BAD_LOG_ALTER, &sys_err, LG_A_CPA, 0,
			E_DM9343_DM0L_ECP, err_code, status));
    ...
    status = LGalter(LG_A_ECPDONE, (PTR)&true, sizeof(true), &sys_err);
    if (status)
	return (log_error(E_DM900B_BAD_LOG_ALTER, &sys_err, LG_A_ECPDONE, 0,
			E_DM9343_DM0L_ECP, err_code, status));
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM9236_DM0L_SM1_RENAME, err_code, status));
	}
    }
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
		E_DM9236_DM0L_SM1_RENAME, err_code, status));
}

/*{
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM9237_DM0L_SM2_CONFIG, err_code, status));
	}
    }
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
		E_DM9237_DM0L_SM2_CONFIG, err_code, status));
}

/*{
    ...
    i4             *err_code)
{
    ...
    status = LGwrite(LG_NOT_RESERVED, lg_id, (i4)1, &lg_object, lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
		E_DM9C09_DM0L_SM0_CLOSEPURGE, err_code, status));
}

/*{
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM9277_DM0L_LOCATION, err_code, status));
	}
    }

    status = LGwrite(0, lg_id, (i4)1, &lg_object, &lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
		E_DM9277_DM0L_LOCATION, err_code, status));
}
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM929E_DM0L_EXT_ALTER, err_code, status));
	}
    }
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
		E_DM929E_DM0L_EXT_ALTER, err_code, status));
}

/*{
    ...
    i4	    *err_code)
{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
		E_DM9278_DM0L_SETBOF, err_code, status));
}

/*{
    ...
    i4	    *err_code)
{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9279_DM0L_JNLEOF, err_code, status));
}

/*{
...
dm0l_archive(i4 *err_code)
{
    ...
    status = LGCwrite(0, (i4)1, &lg_object, (LG_LSN *)&lsn, &error);

    if (status == OK)
	return (E_DB_OK);
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, 0, 0,
	E_DM927A_DM0L_ARCHIVE, err_code, status));
#endif
}

/*{
    ...
    i4	    *err_code)
{
    ...
    adddb.ad_dbname = dcb->dcb_name;
    adddb.ad_dbowner = dcb->dcb_db_owner;
    adddb.ad_l_root = dcb->dcb_location.phys_length;
    MEcopy((PTR) &dcb->dcb_location.physical, dcb->dcb_location.phys_length,
	(PTR) &adddb.ad_root);
	...
	** the database open, so it is potentially inconsistent. */
	...
	local_status = dm0c_close(cnf, DM0C_UPDATE, err_code);
    ...
    *err_code = E_DM927B_DM0L_STARTDRAIN;
    return (E_DB_ERROR);

}

/*{
** Name: log_error	- Log error message and return error.
**
** Description:
...
**	ret_err				area for returned error code.
...
**
** Outputs:
**      ret_err				Returned error code is set to the passed
    ...
    DB_STATUS	    stat)
{
    ...
    (VOID) ule_format(stat, syserr, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, ret_err, 0);
    ...
    if (temp)
    {
        ...
        {
            (VOID) ule_format(E_DM0152_DB_INCONSIST_DETAIL, (CL_ERR_DESC *)0,
                        ULE_LOG, NULL, (char *)0, (i4)0,
                        (i4 *)0, ret_err, 1, sizeof (db_name), &db_name);
        }
	(VOID) ule_format(err_code, syserr, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, ret_err, 1, 0, p);

	(VOID) ule_format(ret_err_code, syserr, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, ret_err, 1, 0, p);

	(VOID) ule_format(temp, syserr, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, ret_err, 1, 0, p);
	...
	*ret_err = E_DM9070_LOG_LOGFULL;
    else
    	*ret_err = ret_err_code;
    return (E_DB_ERROR);
}

/*{
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM927D_DM0L_LOAD, err_code, status));
	}
    }
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
    	E_DM927D_DM0L_LOAD, err_code, status));
}
    ...
    DB_STATUS	    *err_code)
{
    ...
    /*
    ** Prepare this transaction for WILLING COMMIT. 
    **
    ** If we're the last or only preparer, LGalter will return
    ** LG_CONTINUE, indicating we may procede with the
    ** process of writing the DM0L_SECURE records to the log.
    ** When that completes, call LGalter again to put the
    ** transaction in a WILLING COMMIT state.
    */
    ctx.lg_ctx_dis_eid = *dis_tran_id;
    ctx.lg_ctx_id = lg_id;

    status = LGalter(LG_A_PREPARE, (PTR)&ctx, sizeof(ctx), &sys_err);
    if ( status && status != LG_CONTINUE )
    {
	return(log_error(E_DM900B_BAD_LOG_ALTER, &sys_err, LG_A_PREPARE,
			    0, 	E_DM9370_DM0L_SECURE, err_code, status));
    }

    if ( status == LG_CONTINUE )
    {
	/*
	...
	*/
	status = dm0p_force_pages(lock_id, tran_id, lg_id, DM0P_TOSS, err_code);
	if ( status )
	    return(status);
	...
	if (status)
	{
	    return(log_error(E_DM9017_BAD_LOG_SHOW, &sys_err, LG_S_STRANSACTION, 0,
			E_DM9370_DM0L_SECURE, err_code, status));
	}
	...
	for (context = 0;;)
	{
	    ...
	    if (LKshow(LK_S_TRAN_LOCKS, lock_id, 0, 0, buf_size,
			    lk_buffer, (u_i4 *)&length, (u_i4 *)&context,
			    &sys_err))
	    {
		dm0m_deallocate(&mem_ptr);
		return(log_error(E_DM901D_BAD_LOCK_SHOW, &sys_err, LK_S_TRAN_LOCKS, 0,
			E_DM9370_DM0L_SECURE, err_code, (STATUS)0));
	    }
		    ...
		    lkb->lkb_grant_mode == LK_IX)
		{
			...
			if (status)
			{
			    dm0m_deallocate(&mem_ptr);
			    return(log_error(E_DM9015_BAD_LOG_WRITE, &sys_err,
				    lg_id, 0, E_DM9370_DM0L_SECURE, err_code,
				    status));
			}
			if (DMZ_SES_MACRO(11))
	...
	if (i && i < count)
	{
	    ...
	    status = LGwrite(LG_NOT_RESERVED, lg_id, (i4)1, &lg_object, &lsn, &sys_err);
	    if (status)
	    {
		dm0m_deallocate(&mem_ptr);
		return(log_error(E_DM9015_BAD_LOG_WRITE, &sys_err, lg_id, 0,
	...
	for (context = 0;;)
	{
	    ...
	    if (LKshow(LK_S_REL_TRAN_LOCKS, lock_id, 0, 0, buf_size,
			    lk_buffer, (u_i4 *)&length, (u_i4 *)&context,
			    &sys_err))
	    {
		dm0m_deallocate(&mem_ptr);
		    ...
		    lkb->lkb_grant_mode == LK_IX)
		{
			...
			status = LGwrite(LG_NOT_RESERVED, lg_id, (i4)1, &lg_object, &lsn,
					    &sys_err);
			if (status)
			{
			    dm0m_deallocate(&mem_ptr);
	...
	status = LGwrite(LG_FORCE | LG_NOT_RESERVED, lg_id, (i4)1, &lg_object, &lsn, &sys_err);
	if (status)
	{
	    dm0m_deallocate(&mem_ptr);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &sys_err, lg_id, 0,
    ...
    i4		*err_code)
{
	...
	(DB_DIS_TRAN_ID*)NULL, &error);
    if (status != OK)
        return (log_error(E_DM900C_BAD_LOG_BEGIN, &error, lg_id, 0,
                E_DM936C_DM0L_SBACKUP, err_code, status));
    ...
    */
    ...
    if (status != OK)
    {
	return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lx_id, 0,
	    E_DM936C_DM0L_SBACKUP, err_code, status));
    }
    ...
    i4		*err_code)
{
    ...
    if (status != OK)
        return (log_error(E_DM900C_BAD_LOG_BEGIN, &error, lg_id, 0,
                E_DM936D_DM0L_EBACKUP, err_code, status));
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &error,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &error,
		lg_id, 0, E_DM929A_DM0L_CRDB, err_code, status));
	}
    }

    status = LGwrite(LG_FIRST, lg_id, (i4)1, &lg_object, &lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
                E_DM929A_DM0L_CRDB, err_code, status);
}

/*{
    ...
    */
	...
	if (status != OK)
	{
	    (VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &sys_err,
		    ...
		    &loc_error, 1, 0, lg_id);
	    return(log_error(E_DM9015_BAD_LOG_WRITE, &sys_err,
		lg_id, 0, E_DM938F_DM0L_DMU, err_code, status));
	}
    }
    ...
    status = LGwrite(0, lg_id, (i4)1, &lg_object, &lsn, &sys_err);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &sys_err, lg_id, 0,
    	E_DM938F_DM0L_DMU, err_code, status));
}

/*{
    ...
    i4		    *err_code)
{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM939F_DM0L_TEST, err_code, status));
}

/*{
    ...
    i4	    *err_code)
{
    ...
    */
    ...
    status = LGwrite(0, lg_id, (i4)3, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9296_DM0L_ASSOC, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9297_DM0L_ALLOC, err_code, status));
}

/*{
...
i4         *err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9298_DM0L_DEALLOC, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9299_DM0L_EXTEND, err_code, status));
}

/*{
    ...
    i4         *err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM929B_DM0L_OVFL, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C0D_DM0L_FMAP, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(0, lg_id, num_objects, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C0E_DM0L_BTPUT, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(0, lg_id, (i4)4, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C0F_DM0L_BTDEL, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C10_DM0L_BTSPLIT, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(0, lg_id, lg_count, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C11_DM0L_BTOVFL, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C12_DM0L_BTFREE, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C13_DM0L_BTUPDOVFL, err_code, status));
}

/*{
...
i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(0, lg_id, (i4)5, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    i4	    *err_code)
{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C14_DM0L_DISASSOC, err_code, status));
}
    ...
    i4		*err_code)
{
    ...
    */
    ...
    status = LGwrite(LG_NOT_RESERVED, lg_id, (i4)2, &lg_object[0], (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
	E_DM9C90_DM0L_RAWDATA, err_code, status));
}

/*{
    ...
    i4		    *err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
    	E_DM9CA0_DM0L_BSF, err_code, status));
}
    ...
    i4		    *err_code)
{
    ...
    */
    ...
    return (log_error(E_DM9015_BAD_LOG_WRITE, &error, lg_id, 0,
    	E_DM9CA1_DM0L_ESF, err_code, status));
}
    ...
    i4		    *err_code)
{
    ...
    */
    ...
    status = LGwrite(LG_FORCE, lg_id, (i4)2, lg_object, (LG_LSN *)lsn, &error);
    if (status == OK)
    {
	if (DMZ_SES_MACRO(11))
	{
...
DB_DB_NAME *db_name_ptr)
{
	...
	{
	    ule_format ((i4)scf_cb.scf_error.err_code, (CL_ERR_DESC *)0,
