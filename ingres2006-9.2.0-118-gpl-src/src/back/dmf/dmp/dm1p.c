...
**	dm1p_setfree	    - Set a bit in bitmap.
**	dm1p_testfree	    - Test a bit in bitmap.
...
static VOID	    dm1p_log_error(
			i4	    terr_code,
			i4	    *err_code,
			DB_TAB_NAME *table_name,
			DB_DB_NAME  *dbname );

static DB_STATUS    mark_free(
			DMP_RCB	    *rcb,
			DM1P_FHDR   *free_header,
			DM1P_LOCK   *fhdr_lock,
    ...
    i4	*err_code )
{
    ...
    */
    ...
    status = fix_header(r, &fhdr, &fhdr_lock, err_code);
	...
	*/
	if ( status == E_DB_OK )
	{	
	    newpageno = DMPP_VPT_GET_PAGE_PAGE_MACRO(pgtype, *newpage);
	...
	else if ( status == E_DB_WARN )
	{
	    ...
	    */
		...
		status = relock_pages(r, &fhdr_lock, (DM1P_LOCK *)0, err_code);
	    }
	}
	...
	if (status == E_DB_OK || status == E_DB_WARN)
	{
	    ...
	    status = extend(r, fhdr, &fhdr_lock,
			    r->rcb_opt_extend, DM1P_ADD_NEW, err_code);
	    ...
	    if (status != E_DB_OK && r->rcb_opt_extend > t->tcb_rel.relextend)
	    {
    ...
    if (status != E_DB_OK)
    {
    	dm1p_log_error(E_DM92D0_DM1P_GETFREE, err_code, 
		   &t->tcb_rel.relid,
    ...
    i4	*err_code )
{
	...
	*/
	if (r->rcb_logging & RCB_LOGGING)
	{
		...
		*/
		if (fhdr_update)
		    status = dm0p_before_image_check(r, (DMPP_PAGE *) fhdr, err_code);
		if (status == E_DB_OK)
		    status = dm0p_before_image_check(r, (DMPP_PAGE *) fmap, err_code);
		if (status == E_DB_OK)
		    status = dm0p_before_image_check(r, page, err_code);
		if (status != E_DB_OK)
		    break;
	    }
	    ...
	    */
	    ...
	    /* 
	    ** Reserve log file space for alloc and its clr.  
	    ** XXX FIXME XXX
	    ** XXXXX fhdr/fmaps are forced during online recovery, XXXX
	    ** XXXXX pass the force flag to LGreserve.             XXXX
	    ** XXXXX Do we need to reserve a block for the FHDR    XXXX
	    ** XXXXX and another for the FMAP?                     XXXX
	    ** XXX FIXME XXX
	    */
	    cl_status = LGreserve(LG_RS_FORCE, rcb->rcb_log_id, 2, 
		...
		status = E_DB_ERROR;
		*err_code = E_DM92D1_DM1P_PUTFREE;
		break;
	    }
	    ...
	    */
		...
		fhdr_update, (LG_LSN *)0, &lsn, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	if (fhdr_update)
	{
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **) &fhdr);
	    ...
	    dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **) &fhdr);
	...
	*/
	dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **) &fmap);
	...
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **) &fmap);
	fmap_status = unfix_map(r, &fmap, &fmap_lock, &fhdr, &fhdr_lock, err_code);

	/*
	** Mutex and mark deallocated page as FREE.
	*/
	dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, &page);
	...
	*/
	...
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, &page);
	page_status = dm0p_unfix_page(r, DM0P_UNFIX, freepage, err_code);
    ...
    */
    if (fmap)
    {
	status = unfix_map(r, &fmap, &fmap_lock, &fhdr, &fhdr_lock, &local_error);
	if (status != E_DB_OK)
	{
    ...
    dm1p_log_error(E_DM92D1_DM1P_PUTFREE, err_code, 
		   &t->tcb_rel.relid,
		   &t->tcb_dcb_ptr->dcb_name);
    return (E_DB_ERROR);
}

/*{
** Name: dm1p_setfree  - Sets a bit on of off in the bitmap.
**
** Description:
**	This routine will forces a specific bit in the bitmap to be
**	set to ON or OFF.  This routine is typically used by the recovery
**	system to handle UNDO or REDO of bitmap operations.
**
**	Setting a bit from free to allocated will not change the fmap_firstbit
**	field.  This is just a hint to the allocation routine.
**
**	If the table is an unconverted BTREE, this routine will ignore the
**	change of page state.  This is tolerable because the BTREE has to be
**	scanned at a later time (Modify to merge) to find all free pages.
**
** Inputs:
**      rcb                             Pointer to record access context.
**	pageno				Page number to change allocation state.
**	state				New allocation state.
**					    TRUE = free.
**					    FALSE = allocated.
** Outputs:
**      err_code                        Pointer to an area to return
...
** Side Effects:
**	Allocation bitmaps are updated.
**	rcb_page_adds field is decremented/incremented.
**
** History:
**      02-feb-90 (Derek)
**          Created.
    ...
    i4	*err_code )
{
    ...
    /*	Fix the FHDR page, without automatic conversion. */
	...
	fmap_block = VPS_NUMBER_FROM_MAP_MACRO(pgtype, fhdr_map);
	...
	do
	{
	    ...
	    fmap_lock.fix_lock_mode = FMAP_UPDATE_LKMODE;
	    status = fix_map(rcb, fmap_block, &fmap, &fmap_lock,
				&fhdr, &fhdr_lock, err_code);
	    if (status != E_DB_OK)
		break;

	    if (DMZ_AM_MACRO(50))
		TRdisplay("dm1p_setfree: set %w table %~t page %d\n",
			"USED,FREE", state,
	    ...
	    if (state == TRUE)
	    {
		/*	Check if header says that bitmap has no free pages. */

		if (VPS_TEST_FREE_HINT_MACRO(pgtype, fhdr_map) == 0)
		{
		    /*  Update Hint in FHDR for this FMAP. */

		    VPS_SET_FREE_HINT_MACRO(pgtype, fhdr_map);
		    DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(pgtype, fhdr, DMPP_MODIFY);
		}

		/*	Set bit in bitmap, update lowest free bit in bitmap. */
	    ...
	    {
		/*	Turn off free bit in bitmap.	*/

		BITMAP_CLR_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(pgtype, fmap), fmap_bit);
		rcb->rcb_page_adds++;
	    }
	    DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(pgtype, fmap, DMPP_MODIFY);

	    /*  Unfix the FMAP and FHDR. */
	    status = unfix_map(rcb, &fmap, &fmap_lock, &fhdr, &fhdr_lock, err_code);
	    if ( status != E_DB_OK)
		break;
    ...
    dm1p_log_error(E_DM92D2_DM1P_SETFREE, err_code, 
		   &t->tcb_rel.relid,
		   &t->tcb_dcb_ptr->dcb_name);
    return (status);
}

/*{
** Name: dm1p_testfree - Tests whether a page is free or allocated.
**
** Description:
**	This routine tests the allocated state of a page in the bitmap.
**
** Inputs:
**      rcb                             Pointer to record access context.
**	pageno				Page number to check allocation state.
** Outputs:
    ...
    i4	*err_code )
{
    ...
    *state = TRUE;
	...
	do
	{
	    ...
	    status = fix_map(r, fmap_block, &fmap, &fmap_lock,
				&fhdr, &fhdr_lock, err_code);
	    if (status != E_DB_OK)
		break;

	    if (BITMAP_TST_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(pgtype, fmap), fmap_bit) == 0)
		*state = FALSE;

	    if (DMZ_AM_MACRO(50))
		TRdisplay("dm1p_testfree: test free table %~t page %d %w\n",
		    sizeof(t->tcb_rel.relid),
		    &t->tcb_rel.relid,
		    pageno, "NO,YES", *state);
	    ...
	    status = unfix_map(r, &fmap, &fmap_lock, &fhdr, &fhdr_lock, err_code);
	    if ( status != E_DB_OK )
	        break;
    ...
    dm1p_log_error(E_DM92D3_DM1P_TESTFREE, err_code, 
		   &t->tcb_rel.relid,
		   &t->tcb_dcb_ptr->dcb_name);
    return (status);
}
    ...
    i4	*err_code )
{
    ...
    */
    ...
    {
	dm1p_log_error(E_DM92D4_DM1P_CHECKHWM, err_code,
		       &t->tcb_rel.relid,
		       &t->tcb_dcb_ptr->dcb_name);
    }
    return (status);
    ...
    i4	*err_code )
{
	...
	*/
	...
	if (status == E_DB_OK)
	{
		...
		*/
		...
		status = unfix_map(r, &fmap, &fmap_lock, (DM1P_FHDR **)0, 
					(DM1P_LOCK *)0, err_code);
		if (status != E_DB_OK)
		    break;
	    }
	}
	...
	*/
	...
	if (status == E_DB_OK && lastused_page)
	{
	    /*
	    ...
	    */
	    status = dm0p_fix_page(r, pageno, (DM0P_WRITE | DM0P_NOWAIT), 
				lastused_page, err_code);
	    if (status != E_DB_OK)
	    {
		if (status == E_DB_ERROR && *err_code == E_DM004C_LOCK_RESOURCE_BUSY)
		{
		    ...
		    else if (*err_code == E_DM9202_BM_BAD_FILE_PAGE_ADDR ||
    ...
    } while (status == E_DB_OK && lastused_page && *lastused_page == 0);
    ...
    *page_number = pageno;
	...
	s = unfix_map(r, &fmap, &fmap_lock, &fhdr, &fhdr_lock, &local_err_code);
	if (s != E_DB_OK && status == E_DB_OK)
	    status = s;
    }
    ...
    if (DB_FAILURE_MACRO(status))
    {
	dm1p_log_error(E_DM92D5_DM1P_LASTUSED, err_code, 
		       &t->tcb_rel.relid,
		       &t->tcb_dcb_ptr->dcb_name);
    }
    return (status);
    ...
    i4	*err_code )
{
	...
	*/
	...
	status = fix_map(
		    ...
		    VPS_NUMBER_FROM_MAP_MACRO(t->tcb_rel.relpgtype,
		      ...
		      (fhdr_hwmap - 1))), 
		    &fmap, &fmap_lock, &fhdr, &fhdr_lock, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = unfix_map(r, &fmap, &fmap_lock, (DM1P_FHDR **)0, (DM1P_LOCK *)0,
				err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
        ...
        if  (flag & DM1P_D_PAGETYPE)
	{
	    ...
	    */
	    ...
	    {
	        status =  display_hash( rcb, highwater_mark, err_code);
		if ( status != E_DB_OK )
		    break;
	    }
        } 
    ...
    dm1p_log_error(E_DM92D7_DM1P_DUMP, err_code, 
		   &t->tcb_rel.relid,
		   &t->tcb_dcb_ptr->dcb_name);
    return (status);
}
...
#define	    PAGE_PER_GROUP  10
	...
	for ( page_number = 0; page_number < (highwater_mark +1); page_number++)
	{
		...
		*/
	        remap_pagetype( page, r, &page_type);
	    }
	    ...
	    *next_char++ = page_type;
	    ...
	    status = dm0p_unfix_page(r, DM0P_UNFIX, &page, err_code);
	    if ( status != E_DB_OK )
		break;
	...
	*/
    ...
    if ( status != E_DB_OK )
	dm1p_log_error( E_DM92F4_DM1P_DISPLAY_PAGETYPES, err_code, 
		       &t->tcb_rel.relid,
		       &t->tcb_dcb_ptr->dcb_name);
...
Last used pageno %d, Pages never used: %d\n",
	...
	s = unfix_map(r, &fmap, &fmap_lock, (DM1P_FHDR **)0, (DM1P_LOCK *)0,
				&local_err_code);
	if (s != E_DB_OK && status == E_DB_OK)
	    status = s;
    }

    dm1p_log_error( E_DM92F5_DM1P_DISPLAY_BITMAPS, err_code, 
		   &t->tcb_rel.relid,
		   &t->tcb_dcb_ptr->dcb_name);
    return( status );
}
    ...
    i4	*err_code )
{
	...
	*/
	      ...
	      page_number++ )
	{
	    ...
	    page_stat = DMPP_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype,
				page);
	    lsn.lsn_high = DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(r->rcb_tcb_ptr->tcb_rel.relpgsize, page);
	    lsn.lsn_low = DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(r->rcb_tcb_ptr->tcb_rel.relpgsize, page);
	...
	*/
    ...
    dm1p_log_error(E_DM92D8_DM1P_VERIFY, err_code, 
		   &t->tcb_rel.relid,
		   &t->tcb_dcb_ptr->dcb_name);
    return (status);
}
    ...
    i4	*err_code )
{
	...
	*/
	...
	dm0p_pagetype( &t->tcb_table_io, (DMPP_PAGE *)fhdr, 
		       r->rcb_log_id, DMPP_FHDR);
	dm1p_init_fhdr(fhdr, &ignore, t->tcb_rel.relpgtype, t->tcb_rel.relpgsize);

	status = dm0p_unfix_page(r, DM0P_UNFIX, (DMPP_PAGE **)&fhdr, err_code);
	if ( status != E_DB_OK )
	    break;
	...
	*/
	for ( last_used_pageno = 0; last_used_pageno <= last_alloc_pageno; 
	      last_used_pageno++)
	{
	    status = dm0p_fix_page( r, last_used_pageno, DM0P_WRITE, &page,
				    err_code);
	    if (status != E_DB_OK)
	    {
	        if (*err_code == E_DM9206_BM_BAD_PAGE_NUMBER)
	        {
		    last_used_pageno--;
		    status = E_DB_OK;
		    break;
	        }
		else if (*err_code == E_DM920C_BM_BAD_FAULT_PAGE)
		{
	...
	*/
	...
	s = dm0p_unfix_page(r, DM0P_UNFIX, (DMPP_PAGE **)&fhdr, &local_err_code);
	if (status == E_DB_OK && s != E_DB_OK)
	    status = s;
    }

    if ( status != OK )
	dm1p_log_error(E_DM93EF_DM1P_REPAIR, err_code, 
		       &t->tcb_rel.relid,
		       &t->tcb_dcb_ptr->dcb_name);
    ...
    DM_PAGESIZE	page_size )
{
    ...
    DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(pgtype, fhdr, (DMPP_FHDR | DMPP_MODIFY));
    ...
    i4	*err_code )
{
    ...
    {
	*err_code = E_DM92F2_DM1P_SINGLE_FMAP_FREE;
	return (E_DB_ERROR);
    }
	...
	*new_map = VPS_NUMBER_FROM_MAP_MACRO(pgtype,
    ...
    {
	bit = VPS_NUMBER_FROM_MAP_MACRO(pgtype, 
				DM1P_VPT_GET_FHDR_MAP_MACRO(pgtype, fhdr, i));

	if (bit >= first && bit <= last)
	{
    ...
    if (first < fmap_firstbit)
	DM1P_VPT_SET_FMAP_FIRSTBIT_MACRO(pgtype, fmap, first);

    if (fmap_hw_mark == fseg || (first - 1) > fmap_hw_mark)
	DM1P_VPT_SET_FMAP_HW_MARK_MACRO(pgtype, fmap, first - 1);

    if (fmap_lastbit == fseg || (last % fseg) > fmap_lastbit)
	DM1P_VPT_SET_FMAP_LASTBIT_MACRO(pgtype, fmap, (last % fseg));
    ...
    i4	*err_code )
{
    ...
    if (map_index > fhdr_count)
    {
	*err_code = E_DM92F6_DM1P_USED_RANGE;
	return (E_DB_ERROR);
    }
	...
	*new_map = VPS_NUMBER_FROM_MAP_MACRO(pgtype,
    ...
    for (bit = *first_used; bit <= last_used; bit++)
    {
	DM1P_VPT_CLR_FMAP_BITMAP_MACRO(pgtype, fmap, 
	    (bit % fseg) / DM1P_BTSZ,
	    (1 << ((bit % fseg) % DM1P_BTSZ)));
    ...
    if ((last_used % fseg) > fmap_firstbit)
	DM1P_VPT_SET_FMAP_FIRSTBIT_MACRO(pgtype, fmap, (last_used % fseg) + 1);

    if (fmap_hw_mark == fseg || (last_used % fseg) - 1 > fmap_hw_mark)
	DM1P_VPT_SET_FMAP_HW_MARK_MACRO(pgtype, fmap, (last_used % fseg));

    if (fmap_lastbit == fseg || (last_used % fseg) > fmap_lastbit)
	DM1P_VPT_SET_FMAP_LASTBIT_MACRO(pgtype, fmap, (last_used % fseg));
    ...
    i4     *err_code )
{
	...
	{
	    ule_format(E_DM93AB_MAP_PAGE_TO_FMAP, (CL_SYS_ERR *)0, ULE_LOG, 
		NULL, (char *)0, (i4 )0, (i4 *)0, err_code, 7,
    ...
    dm1p_log_error(E_DM92DB_DM1P_FREE, err_code, 
		   &t->tcb_rel.relid,
		   &t->tcb_dcb_ptr->dcb_name);
    return (status);
}
...
i4		*err_code )
{
    ...
    DM_PAGENO	    first_free_page;
    DM_PAGENO	    first_used_page;
    DM_PAGENO	    last_free_page;
	...
	*/
	...
	else
	{
	    status = dm2f_sense_file(tbio->tbio_location_array, 
		    ...
		    &t->tcb_dcb_ptr->dcb_name, &old_physical_pages, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	...
	if (allocate_amount > 0)
	{
	    ...
	    */
		...
		status 	  = E_DB_ERROR;
    	        ule_format( E_DM92BF_DM1P_TABLE_TOO_BIG,
	...
	*/
	if (fmap_hw_mark < fmap_lastbit)
	{
	    ...
	    ** on this fmap which must first be used for any new FMAP. 
	...
	else
	{
	    ...
	    */
	    ...
	    first_free_page = fhdr_pages + fmap_needed;
	    last_free_page = new_physical_pages - 1;
	}
	...
	*/
	if (r->rcb_logging & RCB_LOGGING)
	{
	    ...
	    */
	    status = dm0l_bm(r, &bmini_lsn, err_code);
	    if (status != E_DB_OK)
		break;
	    ...
	    ** XXXXX fhdr/fmaps are forced during online recovery, XXXX
	    ** XXXXX pass the force flag to LGreserve.             XXXX
	    ** XXXXX Do we need to reserve 2 log buffers?          XXXX
	    ** XXX FIXME XXX
	    */
	    cl_status = LGreserve(LG_RS_FORCE, r->rcb_log_id, 2, 
	...
	/*
	** Update the FMAP to reflect the newly added free pages and
	...
	*/
	    ...
	    (first_used_page / fseg) <= fmap_sequence)
	{
		...
		*/ 
		...
		if (fmap_firstbit += fmap_needed >= fseg)
		{
	...
	*/
	...
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **) &fmap);
        fmap_status = unfix_map(r, &fmap, &fmap_lock, (DM1P_FHDR **)0,
				(DM1P_LOCK *)0, err_code);
	...
	*/
	...
	/*
	** Add any new Free Space Maps to the table needed to reflect the
	** new free pages.
	*/
	...
	while (fmap_needed && fmap_status == E_DB_OK)
	{
		...
		*/
		if (d->dcb_status & DCB_S_BACKUP)
		{
		    ...
		    */
		    fmap_status = dm0p_before_image_check(r, (DMPP_PAGE *) fhdr, err_code);
		    if (fmap_status == E_DB_OK)
			fmap_status = dm0p_before_image_check(r, (DMPP_PAGE *) fmap, err_code);
		    if (fmap_status != E_DB_OK)
			break;
		}
	...
	*/
	...
	{
	    status = dm0l_em(r, &bmini_lsn, &lsn, err_code);
	    if (status != E_DB_OK)
		break;
	    if (in_mini)
    ...
    */
    ...
    dm1p_log_error(E_DM92DC_DM1P_EXTEND, err_code, 
		   &t->tcb_rel.relid,
		   &t->tcb_dcb_ptr->dcb_name);
    return (E_DB_ERROR);
}
...

/*{
** Name: mark_free - Mark page as free in bitmap.
**
** Description:
...
**	used by the convert and rebuild routines.)  The page 'pageno' is
**	marked as free in the FMAP.
**
** Inputs:
**	rcb				Pointer to RCB for this table
...
**	force				Flag indicating to force map page
**					on unfix if its modified.
**
** Outputs:
**	err_code			Pointer to error.
**                                      
    ...
    i4	*err_code )
{
    ...
    if (DMZ_AM_MACRO(50))
	TRdisplay("DM1P: mark page %d free in table %~t\n",
    		pageno,
    		sizeof(t->tcb_rel.relid),
    		&t->tcb_rel.relid);
	...
	if (fm)
	{
	    if ( force )
	    {
		status = force_fpage( rcb, (DMPP_PAGE **)fmap, err_code );
		if ( status != E_DB_OK )
		    return( status );
	    }

	    status = unfix_map(rcb, fmap, &fmap_lock, (DM1P_FHDR **)0,
				(DM1P_LOCK *)0, err_code);
	    if (status != E_DB_OK)
		return (status);
	}
    ...
    /*
    **	Turn on free bit in bitmap.
    **  Also update the nextbit field.
    **	    The firstbit field is set to the freed page if there are no
    **	    free bits on the page. (I.E. nextbit == FSEG)
    **
    **	    If a valid FMAP was pass in, the firstbit field is set to the
    **	    minimum of firstbit and the freed page.  The convert and rebuild
    **	    routines pass in a FMAP as an optimization, and it is used here
    **	    to point to the lowest free bit on the page.  This is not desirable
    **	    in the normal case because the free page will be locked until
    **	    the transaction commits.  It's better to not waste time on this
    **	    kind of page.
    */

    BITMAP_SET_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(pgtype, fm), fmap_bit);
    if (fmap_bit < (i4)DM1P_VPT_GET_FMAP_FIRSTBIT_MACRO(pgtype, fm))
	DM1P_VPT_SET_FMAP_FIRSTBIT_MACRO(pgtype, fm, fmap_bit);
    if (fmap_bit > (i4)DM1P_VPT_GET_FMAP_LASTBIT_MACRO(pgtype, fm))
	DM1P_VPT_SET_FMAP_LASTBIT_MACRO(pgtype, fm, fmap_bit);
    DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(pgtype, fm, DMPP_MODIFY);

    /*  Make FMAP hint point at first free bit set. */
        ...
        if (force)
	{
	    ...
	    s = force_fpage(rcb, (DMPP_PAGE **)fmap, &lerror);
	    if (s != E_DB_OK)
	    {
		ule_format(lerror, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, &lerror, 0);
	    }
	}
...

/*{
** Name: mark_used - Mark page as used in bitmap.
**
** Description:
**	This routine is called with the FHDR fixed.  An FMAP pointer can
**	be passed in to optimization consecutive calls (this mechanism is
**	used by the convert and rebuild routines.)  The page 'pageno' is
**	marked as used in the FMAP.
**
** Inputs:
**	rcb				Pointer to RCB for this table
**	free_header			Pointer to FHDR.
    ...
    i4	*err_code )
{
    ...
    if (DMZ_AM_MACRO(50))
	TRdisplay("DM1P: mark page %d used in table %~t\n",
    		pageno,
    		sizeof(t->tcb_rel.relid),
    		&t->tcb_rel.relid);
    ...
    BITMAP_CLR_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(pgtype, fm), fmap_bit);

    fmap_hw_mark = DM1P_VPT_GET_FMAP_HW_MARK_MACRO(pgtype, fm);

    if (fmap_hw_mark == fseg || fmap_bit > fmap_hw_mark)
    {
	DM1P_VPT_SET_FMAP_HW_MARK_MACRO(pgtype, fm, fmap_bit);

	t->tcb_rel.relpages = pageno + 1; 

	/*  Update FHDR pointer to current FMAP with highwater mark. */

	if (fmap_index >= DM1P_VPT_GET_FHDR_HWMAP_MACRO(pgtype,fhdr))
	{
	    DM1P_VPT_SET_FHDR_HWMAP_MACRO(pgtype, fhdr, fmap_index + 1);
	    DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(pgtype, fhdr, DMPP_MODIFY);
	}
    }
    DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(pgtype, fm, DMPP_MODIFY);
    ...
    i4	*err_code )
{
	...
	if (*page == 0)
	{
		...
		for (map >>= bit_index; map ; bit_index++, map >>= 1)
		{
			...
			*/
		    ...
		    else if (t->tcb_extended && r->rcb_lk_type != RCB_K_TABLE)
		    {
                        /*
                        ...
                        */
			if (DMPP_VPT_GET_PAGE_PAGE_MACRO(pgtype, *page) == pageno)
			{
			    bool	xn_active = FALSE;

			    page_low_tran = 
				DMPP_VPT_GET_TRAN_ID_LOW_MACRO(pgtype, *page);
				...
				xn_active = dmxe_xn_active(page_low_tran); 
			    }
				...
				dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, page);
				status = dm0p_unfix_page(r, DM0P_UNFIX, page, 
						err_code);
			    }
			}
		    }
			...
			*/ 
			if ( DMPP_VPT_GET_PAGE_PAGE_MACRO(pgtype, *page) == pageno &&
				...
				(*page)) == r->rcb_tran_id.db_high_tran &&
			     DMPP_VPT_GET_TRAN_ID_LOW_MACRO(pgtype,
				(*page)) == r->rcb_tran_id.db_low_tran )
			{
			    found_busy_page = TRUE;
	...
	*/
	if (fmap_lock.lock_value.lk_mode != FMAP_UPDATE_LKMODE)
	{
	    ...
	    dm0p_unmutex(tbio, (i4)DM0P_MINCONS_PG, r->rcb_lk_id, page);

	    status = relock_pages(r, &fmap_lock, (DM1P_LOCK *)0, err_code);

	    /* Remutex the page */
	    dm0p_mutex(tbio, (i4)DM0P_MINCONS_PG, r->rcb_lk_id, page);
		...
		*/
		if ((fmap_bitmap[bitmap_index] & (1 << bit_index)) == 0)
		{
		    ...
		    fmap_status = relock_pages(r, &fmap_lock, (DM1P_LOCK *)0, err_code);

		    /* Unfix/UNLOCK the no longer "free" page */
		    status = unfix_nofmt_page(r, page, pageno, err_code);
	...
	*/
	if (*map_index >= fhdr_hwmap || last_free_bit)
	{
	    ...
	    */
		...
		status = relock_pages(r, fhdr_lock, &fmap_lock, err_code);
	...
	*/
	DMPP_VPT_SET_PAGE_PAGE_MACRO(pgtype, *page, pageno);
	...
	*/
	if (r->rcb_logging & RCB_LOGGING)
	{
	    ...
	    */
		...
		(*t->tcb_acc_plv->dmpp_format)(pgtype, pgsize, *page,
	    ...
	    */
		...
		(VOID) ule_format(E_DM9054_BAD_LOG_RESERVE, &sys_err,
			    ULE_LOG, NULL, (char *)0, 0L, (i4 *)0,
			    err_code, 1, 0, r->rcb_log_id);
		*err_code = E_DM92F7_DM1P_SCAN_MAP;
		status = E_DB_ERROR;
		break;
	    }
	...
	*/
	...
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **) &fmap);
	fmap_status = unfix_map(r, &fmap, &fmap_lock, free_header, fhdr_lock, err_code);
	...
	*/
	...
	{
	   DMPP_VPT_SET_PAGE_STAT_MACRO(pgtype, *page, DMPP_MODIFY);
	}

	DMPP_VPT_SET_PAGE_TRAN_ID_MACRO(pgtype, *page, r->rcb_tran_id);
	DMPP_VPT_SET_PAGE_SEQNO_MACRO(pgtype, *page, 0);
	if (r->rcb_logging & RCB_LOGGING)
	    DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(pgtype, *page, lsn);
	...
	**
	** If row_locking, the caller will convert PHYSICAL X to IX,
    ...
    */
	...
	/* Unfix/UNLOCK the no longer "free" page */
	local_status = unfix_nofmt_page(r, page, pageno, &local_err_code);
    ...
    if (fmap)
    {
	local_status = unfix_map(r, &fmap, &fmap_lock, (DM1P_FHDR **)0,
					(DM1P_LOCK *)0, &local_err_code);
	if (local_status != E_DB_OK)
	{
	    ule_format(local_err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_err_code, 0);
    ...
    if (status == E_DB_ERROR)
    {
	dm1p_log_error( E_DM92F7_DM1P_SCAN_MAP, err_code, 
    ...
    i4	*err_code )
{
    ...
    *header = NULL;
    ...
    status = lock_page(r, fhdr_lock, t->tcb_rel.relfhdr, err_code);
	...
	if (status == E_DB_OK)
	{
	    ...
	    */
	    ...
	    {
		status = E_DB_WARN;
		*err_code = E_DM92DD_DM1P_FIX_HEADER;
	    }
	}
	else
	{
	    /* An error occurred fixing the FHDR; unlock it */
	    s = unlock_page(r, fhdr_lock, &local_err_code);
	    if (s != E_DB_OK)
	    {
		ule_format(local_err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, &local_err_code, 0);
	...
	if (*header)
	{
	    s = unfix_header(r, header, fhdr_lock, 
				(DM1P_FMAP**)0, (DM1P_LOCK*)0, &local_err_code);
	    if (s != E_DB_OK)
	    {
		ule_format(local_err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, &local_err_code, 0);
	...
	dm1p_log_error(E_DM92DD_DM1P_FIX_HEADER, err_code, 
		       &t->tcb_rel.relid,
		       &d->dcb_name);
    ...
    i4	*err_code )
{
	...
	*/
	if (r->rcb_lk_type != RCB_K_TABLE && fmap && *fmap &&
	...
	else
	{
	    ...
	    status = unlock_page(r, fhdr_lock, err_code);
	    *fhdr = (DM1P_FHDR*)NULL;
	}
    }
    else
    {
	status = E_DB_ERROR;
	*err_code = E_DM92DE_DM1P_UNFIX_HEADER;
    }

    if (status != E_DB_OK)
    {
	dm1p_log_error(E_DM92DE_DM1P_UNFIX_HEADER, err_code, 
		   &t->tcb_rel.relid,
		   &d->dcb_name);
    }
    ...
    i4	*err_code )
{
	...
	while (cl_status == OK)
	{
	    ...
	    cl_status = LKrequest(lock_flags,
			r->rcb_lk_id, (LK_LOCK_KEY *)0, lock1->new_lock_mode,
		...
		{
		    cl_status = LKrequest(lock_flags,
				r->rcb_lk_id, (LK_LOCK_KEY *)0, LK_N,
	...
	{
	    cl_status = LKrequest(lock_flags,
			r->rcb_lk_id, (LK_LOCK_KEY *)0, lock2->new_lock_mode,
			&lock2->lock_value, &lock2->lock_id,
			(i4)0, &sys_err);
	    if (cl_status == OK)
	    {
    ...
    i4	*err_code )
{
    ...
    status = dm0p_unfix_page(rcb, DM0P_FORCE, page, err_code);
    return (status);
}
/*{

    ...
    i4	    *err_code )
{
    ...
    */
	...
	status = lock_page(r, fmap_lock, fmap_pageno, err_code);
    
    if (status == E_DB_OK)
    {
	if (fmap_lock->fix_lock_mode & DM1P_FOR_SCRATCH)
	    fix_action |= DM0P_SCRATCH;
	...
	if (status == E_DB_OK)
	{
	    ...
	    */
	    ...
	    {
		status = E_DB_WARN;
		*err_code = E_DM92EE_DM1P_FIX_MAP;
	    }
	}
	else
	{
	    /* An error occurred fixing the FMAP; unlock it */
	    s = unlock_page(r, fmap_lock, &local_err_code);
	    if (s != E_DB_OK)
	    {
		ule_format(local_err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		    (char *)0, (i4)0, (i4 *)0, &local_err_code, 0);
	...
	if (*fmap)
	{
	    s = unfix_map(r, fmap, fmap_lock, (DM1P_FHDR **)0,
				(DM1P_LOCK *)0, &local_err_code);
	    if (s != E_DB_OK)
	    {
		ule_format(local_err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
	...
	dm1p_log_error(E_DM92EE_DM1P_FIX_MAP, err_code, 
		       &t->tcb_rel.relid, &d->dcb_name);
    }
    ...
    i4	*err_code )
{
    ...
    */
	...
	status = unlock_page(r, fmap_lock, err_code);
	...
	*fmap = (DM1P_FMAP*)NULL;
    ...
    {
	*err_code = E_DM92EF_DM1P_UNFIX_MAP;
	status = E_DB_ERROR;
    }

    if (status != E_DB_OK)
	dm1p_log_error(E_DM92EF_DM1P_UNFIX_MAP, err_code, 
		   &t->tcb_rel.relid, &d->dcb_name);
    return (status);
}

/*{
** Name: dm1p_log_error - Used to log trace back messages.
**
** Description:
    ...
    DB_DB_NAME	*dbname )
{
    ...
    ** back messages
    */
    if ( *err_code != E_DM0042_DEADLOCK  && 
	 *err_code != E_DM004D_LOCK_TIMER_EXPIRED )
    {
	ule_format( E_DM92CB_DM1P_ERROR_INFO, (CL_SYS_ERR *)0, ULE_LOG,NULL,
		(char *)0, (i4)0, (i4 *)0, &l_err_code, 2,
	...
	else
	{
	    ...
	    */
	    ule_format(terr_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		(i4)0, (i4 *)0, &l_err_code, 0);
	}
...
i4		  *err_code )
{
	...
	*err_code = E_DM002A_BAD_PARAMETER;
	...
	status = dm0m_allocate(
			(build_cb->SMS_page_size * 2) + sizeof(DMP_MISC),
			(i4)0,
			(i4)MISC_CB,
			(i4)MISC_ASCII_ID,
			(char *)NULL,
    ...
    */
    ...
    dm1p_log_error( E_DM92F1_DM1P_BUILD_SMS, err_code, 
	...
	*/
		...
		*/
    		status = dm0p_fix_page( 
				build_cb->SMS_inmemory_rcb, (i4)pageno,
			    	DM0P_READ, &buffer_page, err_code );
		if ( status != E_DB_OK)
		    break;

		MEcopy((PTR)buffer_page, build_cb->SMS_page_size,
			(PTR)page);

		status = dm0p_unfix_page(
				build_cb->SMS_inmemory_rcb, DM0P_UNFIX,
				&buffer_page, err_code );
		if ( status != E_DB_OK )
		    break;
	...
	else
	{
		...
		*/
    		status = dm0p_fix_page( 
				build_cb->SMS_inmemory_rcb, (i4)pageno,
			    	DM0P_SCRATCH,&buffer_page, err_code );
		if ( status != E_DB_OK)
		    break;
    ...
    if ( status != E_DB_OK )
	dm1p_log_error( E_DM92F8_DM1P_GET_PAGE_BUILD_SMS, err_code, 
		        &build_cb->SMS_table_name,
		        &build_cb->SMS_dbname );
...
i4			*err_code )
{
    ...
    do
    {
	    ...
	    */
	    status = dm0p_fix_page(
		      ...
		      page),
		      DM0P_WRITE, &buffer_page, err_code);
	    if ( status != E_DB_OK )
	        break;

	    dm0p_mutex(&build_cb->SMS_inmemory_rcb->rcb_tcb_ptr->tcb_table_io,
		        (i4)0, build_cb->SMS_inmemory_rcb->rcb_lk_id,
		        &buffer_page);

	    MEcopy((PTR)page, build_cb->SMS_page_size, (PTR)buffer_page);
	    DMPP_VPT_SET_PAGE_STAT_MACRO(build_cb->SMS_page_type, buffer_page, 
		DMPP_MODIFY);
	    ...
	    */
	    ...
	    dm0p_pagetype(
		&build_cb->SMS_inmemory_rcb->rcb_tcb_ptr->tcb_table_io,
		buffer_page, build_cb->SMS_inmemory_rcb->rcb_log_id,
		(DMPP_VPT_GET_PAGE_STAT_MACRO(build_cb->SMS_page_type, page) & mask));

	    dm0p_unmutex(&build_cb->SMS_inmemory_rcb->rcb_tcb_ptr->tcb_table_io,
			  (i4)0,
    ...
    if ( status != E_DB_OK )
	dm1p_log_error( E_DM92F9_DM1P_PUT_PAGE_BUILD_SMS, err_code, 
		        &build_cb->SMS_table_name,
		        &build_cb->SMS_dbname );
    ...
    i4	*err_code )
{
	...
	*/
	status = dm1p_build_SMS( &SMS_build_cb, err_code );
	if (status != E_DB_OK)
	    break;
	...
	*/
    ...
    if ( status != E_DB_OK )
        dm1p_log_error(E_DM92EB_DM1P_CVT_TABLE_TO_65, err_code, 
		       &tcb->tcb_rel.relid,
		       &tcb->tcb_dcb_ptr->dcb_name);
	...
	*/
	...
	r->rcb_state |= RCB_READAHEAD;
		...
		*next_char++ = page_type;
		...
		overflow_page = 
			DMPP_VPT_GET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype,page);
    ...
    i4	*err_code)
{
	...
	*/
    ...
    if ( status != E_DB_OK )
	dm1p_log_error(E_DM92D6_DM1P_REBUILD, err_code, 
		       &t->tcb_rel.relid,
		       &t->tcb_dcb_ptr->dcb_name);
    ...
    i4	*err_code )
{
	...
	** Extend the table
	*/
	status = extend(r, fhdr, &fhdr_lock, extend_amount, extend_flag, err_code);
    ...
    */
    ...
    if (DB_FAILURE_MACRO(status))
    {
	dm1p_log_error(E_DM92EC_DM1P_ADD_EXTEND, err_code, 
		       &t->tcb_rel.relid,
		       &t->tcb_dcb_ptr->dcb_name);
    }
...
i4		*err_code)
{
	...
	*/
	lk_status = dm0p_unlock_page(r->rcb_lk_id, d, &t->tcb_rel.reltid,
		...
		(LK_LKID *)0, (LK_VALUE *)0, &lk_errcode);
	if (lk_status)
