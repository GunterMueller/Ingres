    ...
    i4     *err_code)
{
    ...
    */
	...
	(t->tcb_leafattcnt == t->tcb_ixattcnt))
    {
    ...
    */
    ...
    r->rcb_alloc_bid.tid_tid.tid_page = 0;
    r->rcb_alloc_bid.tid_tid.tid_line = DM_TIDEOF;
    r->rcb_alloc_clean_count = DM1B_CC_INVALID; 
    ...
    */
    ...
    {
	leafpage = (DM1B_INDEX *)((*leaf));

	if (row_locking(r))
	{
	    dm0p_lock_buf_macro(tbio, r->rcb_lk_id, leaf);
	    buf_locked = TRUE;
	}
	...
	*/
	s = dm1b_compare_range( r, leafpage, leafkey, &correct_leaf, err_code);
	if (s != E_DB_OK)
	    break;
	...
	*/
	s = dm1bxclean(r, leafpage, err_code);
	if (s != E_DB_OK)
	    break;
	...
	**
	** Overflow pages are only possible for 2k pages, page locking
	**
	** Neither apply to CLUSTERED.
	*/
	if (DM1B_VPT_GET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype, leafpage) != NULL)
	{
	    /*
	    ...
	    */
	    state = E_DB_OK;
	    if ( s = dm1b_AllocKeyBuf(t->tcb_klen, keybuf, &tmpkey, &AllocKbuf, err_code) )
		break;
	    s = dm1bxdupkey(r, leaf, tmpkey, &state, err_code);
	    if (DB_FAILURE_MACRO(s))
		break;

	    /*
	    ** Reset local leaf pointer as dm1bxdupkey may have unfixed the
	    ** page and refixed it into a different buffer.
	    */
	    leafpage = (DM1B_INDEX *)((*leaf));
	    ...
	    */
		...
		s  = adt_kkcmp(adf_cb, t->tcb_keys, t->tcb_leafkeys, 
		     ...
		     *err_code = E_DM93D0_BALLOCATE_ERROR;
		     break;
	        }
	...
	*/
	s = dm1bxsearch(r, leafpage, DM1C_OPTIM, DM1C_EXACT, leafkey, 
		t->tcb_keys, &localtid, &localpartno, &lineno, NULL, err_code); 
	if (DB_FAILURE_MACRO(s))
	{
	    if (*err_code != E_DM0056_NOPART)
		break;
	    s = E_DB_OK;
	}
	...
	*/
	bid->tid_tid.tid_page = 
	    DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leafpage);
	...
	*/
	if (*leaf != NULL)
	{
	    if (buf_locked)
	    {
		dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, leaf);
		buf_locked = FALSE;
	    }
	    s = dm0p_unfix_page(r, DM0P_UNFIX, leaf, err_code);
	}
	...
	*/
	s = btree_search(r, leafkey, t->tcb_keys, DM1C_OPTIM, DM1C_EXACT, 
		leaf, bid, &localtid, &buf_locked, NULL, err_code); 
	if (DB_FAILURE_MACRO(s))
	{
	    /*
	    ...
	    */
	    if ((opflag == DM1C_REALLOCATE) || (*err_code != E_DM0057_NOROOM))
		break;
    ...
    */
    ...
    {
	s = dm1b_dupcheck(r, leaf, bid, record, leafkey, 
		&requalify_leaf, 
		&buf_locked, &redo_dupcheck, err_code);
	leafpage = (DM1B_INDEX *)((*leaf));
	...
	*/
	...
	{
	    s = dm1b_requalify_leaf(r, DM1C_OPTIM, leaf, 
				leafkey, t->tcb_keys, &buf_locked, err_code);
	    leafpage = (DM1B_INDEX *)((*leaf));
	    if (s != E_DB_OK)
		break;

	    if (dm1cxhas_room(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
			leafpage, compression_type,
			(i4)100 /* fill factor */,
			t->tcb_kperleaf,
			(t->tcb_klen + t->tcb_comp_katts_count
			    + DM1B_VPT_GET_BT_TIDSZ_MACRO(
			      t->tcb_rel.relpgtype, leafpage))))
	    ...
	    */
	    s = dm1bxsearch(r, leafpage, DM1C_OPTIM, DM1C_EXACT, leafkey, 
		    ...
		    &lineno, NULL, err_code);
	    if (DB_FAILURE_MACRO(s) && *err_code == E_DM0056_NOPART)
		s = E_DB_OK;
    ...
    */
	...
	if ( !tmpkey )
	     s = dm1b_AllocKeyBuf(tmpkey_len, keybuf, &tmpkey, &AllocKbuf, err_code);
	if ( s == E_DB_OK )
	    s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage, 
			    t->tcb_acc_tlv, t->tcb_leafatts, t->tcb_leafattcnt, 
			    lineno, &tmpkey, 
			    (DM_TID *)0, (i4*)0,
	...
	if ((s == E_DB_OK) && (samekey == DM1B_SAME))
	{
		...
		*/
		s = dm1bxreserve(r, leafpage, bid, leafkey, FALSE, TRUE, 
				 err_code);
		if (s != E_DB_OK)
		    dm1cxlog_error(E_DM93C6_ALLOC_RESERVE, r, leafpage,
		...
		*/
		r->rcb_alloc_bid.tid_i4 = bid->tid_i4;
		DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype,
				leafpage, r->rcb_alloc_lsn);
		r->rcb_alloc_clean_count = 
			DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype,
			leafpage);
		r->rcb_alloc_nextleaf =
	...
	*/
	...
	/*
	** For non unique primary btree (page type != TCB_PG_V1)
	** we no longer have overflow pages, we always split
	*/
	...
	*/
	...
	s = newduplicate(r, leaf, leafkey, &duplicate_insert, err_code);
	if (s != E_DB_OK)
	    break;
	...
	** duplicates exist.
	*/
	s = dm1b_find_ovfl_space(r, leafpage,
		(t->tcb_klen + t->tcb_comp_katts_count +
		 DM1B_VPT_GET_BT_TIDSZ_MACRO(t->tcb_rel.relpgtype, leafpage)),
				 bid,err_code);
	if ((s != E_DB_OK) && (*err_code != E_DM0057_NOROOM))
	    break;
	...
	** data page.
	*/
	s = dm1bxovfl_alloc(r, leafpage, bid, err_code);
	if (s != E_DB_OK)
	    break;
	...
	*/
	if ((t->tcb_rel.relpgtype != TCB_PG_V1)
	    && (r->rcb_alloc_clean_count != DM1B_CC_INVALID))
	{
	    bid->tid_i4 = r->rcb_alloc_bid.tid_i4;
	    break;
	}
	...
	*/
	if (dm1cxhas_room(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
			    ...
			    t->tcb_comp_katts_count 
			    + DM1B_VPT_GET_BT_TIDSZ_MACRO(
				t->tcb_rel.relpgtype, leafpage))) == TRUE)
	    break;
	...
	**
	*/
	if ( (t->tcb_rel.relpgsize == DM_COMPAT_PGSIZE)
	...
	dm1cxlog_error(E_DM93E9_BAD_INDEX_RSHIFT, r, leafpage, 
			t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, (i4)0);
	*err_code = E_DM934A_BTREE_NOROOM;
	s = E_DB_ERROR;
	break;
    }

    if (row_locking(r) && (r->rcb_alloc_clean_count == DM1B_CC_INVALID)
	&& DB_SUCCESS_MACRO(s))
    {
	/*
	...
	**    Mutex required = TRUE, buf should already be locked
	*/
	s = dm1bxreserve(r, leafpage, bid, leafkey, TRUE, TRUE, err_code);
	if (s != E_DB_OK)
	    dm1cxlog_error(E_DM93C6_ALLOC_RESERVE, r, leafpage, 
			t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, (i4)0);
    ...
    */
    ...
    {
	if (row_locking(r) && buf_locked == FALSE)
	{
	...
	if (t->tcb_extended && r->rcb_bulk_misc && r->rcb_bulk_cnt > 0)
	{
	    ...
	    s = bulkpage(r, leaf, data,
		    t->tcb_rel.relcomptype == TCB_C_NONE ? record : crecord,
		    need, leafkey, tid, bid, &buf_locked, err_code);
	}
	else
	{

	    s = findpage(r, leaf, data, 
		    t->tcb_rel.relcomptype == TCB_C_NONE ? record : crecord,
		    need, leafkey, tid, bid, &buf_locked, err_code);
	}
	...
	/* Note buf_locked may be FALSE after findpage */
    }
    ...
    */
    if (DB_SUCCESS_MACRO(s) && (t->tcb_rel.relpgtype == TCB_PG_V1) &&
	(DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leafpage) != 
	bid->tid_tid.tid_page))
    {
	s = dm0p_unfix_page(r, DM0P_UNFIX, leaf, err_code);
	if (DB_SUCCESS_MACRO(s))
	{
	    s = dm0p_fix_page(r, bid->tid_tid.tid_page, 
		    (DM0P_WRITE | DM0P_NOREADAHEAD), leaf, err_code);
	    leafpage = (DM1B_INDEX *)((*leaf)); 
	}
    }
    ...
    */
	...
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM925E_DM1B_ALLOCATE;
    }

    if (*leaf != NULL)
    {
	s = dm0p_unfix_page(r, DM0P_UNFIX, leaf, &local_error);
	if (DB_FAILURE_MACRO(s))
	{
    ...
    i4     *err_code)
{
    ...
    */
	...
	if (t->tcb_rel.relpgtype != TCB_PG_V1)
	{
	    (VOID) ule_format(E_DM9C29_BXV2KEY_ERROR, (CL_ERR_DESC*)0, ULE_LOG, 
		    NULL, (char *)0, 0L, (i4 *)0, err_code, 0);
	    *err_code = E_DM934B_DM1B_DUPCHECK;
	    return (E_DB_ERROR);
	}
    }
    ...
    */
    save_page = DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leafpage);
    ...
    */
	...
	if ( s = dm1b_AllocKeyBuf(t->tcb_klen, keybuf, &KeyPtr, &AllocKbuf, err_code) )
	    break;
	/*
	...
	*/
	if (ignore_tidp && (localbid.tid_tid.tid_line == 0))
	{
		...
		s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
			...
			&tmpkey, (DM_TID *)0, (i4*)0, &tmpkey_len,
			compression_type, NULL);
		if (s != E_DB_OK)
		{
		    dm1cxlog_error( E_DM93E3_BAD_INDEX_GET, r, leafpage,
			t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, DM1B_LRANGE);
		    *err_code = E_DM93D1_BDUPKEY_ERROR;
		    break;
		}
		...
		*/
		s = btree_search(r, leafkey, keys_given, DM1C_OPTIM, DM1C_EXACT,
			leaf, &localbid, &tid, buf_locked, NULL, err_code);
		if (DB_FAILURE_MACRO(s))
		{
		    if (*err_code != E_DM0057_NOROOM)
			break;
		    s = E_DB_OK;
		}
	...
	*/
	...
	if ((i4)lineno < 
	    (i4)DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leafpage))
	{
	    tmpkey = KeyPtr;
	    tmpkey_len = t->tcb_klen;
	    s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
	    ...
	    */
	    ...
	    {
		s = E_DB_OK;
		if (DM1B_SKIP_DELETED_KEY_MACRO(r, leafpage, row_low_tran))
		    break;
	    }
		...
		*err_code = E_DM93D1_BDUPKEY_ERROR;
	    ...
	    if (compare == DM1B_SAME)
	    {
		if (DM1B_DUPCHECK_NEED_ROW_LOCK_MACRO(r, leafpage,row_low_tran))
		{
		    if (*buf_locked == TRUE)
		    {
			dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, leaf);
			*buf_locked = FALSE;
		    ...
		    lock_flag = DM1R_ALLOC_TID | DM1R_LK_PKEY_PROJ;
	...
	*/
	...
	{
	    next_page = 
		DM1B_VPT_GET_BT_NEXTPAGE_MACRO(t->tcb_rel.relpgtype, leafpage);
		...
		*/
		if (DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leafpage) > 0)
		{
		    tmpkey = KeyPtr;
		    tmpkey_len = t->tcb_klen;
		    s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
			leafpage, t->tcb_acc_tlv, t->tcb_leafatts, 
			t->tcb_leafattcnt, 0, &tmpkey,
			(DM_TID *)0, (i4*)0,
			&tmpkey_len, compression_type, NULL);
    ...
    */
    ...
    {
	s = dm1badupcheck(r, leaf, &localbid, &tid, record, leafkey,
		requalify_leaf, buf_locked, redo_dupcheck, err_code);
	leafpage = (DM1B_INDEX *)((*leaf));
    }
	...
	*/
	if (DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leafpage) != 
	    save_page)
	{
	    *requalify_leaf = TRUE;
	    if (*buf_locked == TRUE)
    ...
    i4         *err_code)
{
	...
	if (leaf == NULL)
	{
	    ...
	    s = dm0p_fix_page(r, (DM_PAGENO  )bid->tid_tid.tid_page, 
                    fix_action | DM0P_NOREADAHEAD, &r->rcb_other_page_ptr,
		    err_code);
	...
	if (row_locking(r))
	{
	    dm0p_lock_buf_macro(tbio, r->rcb_lk_id, &r->rcb_other_page_ptr);
	    buf_locked = TRUE;
	}
	...
	*/ 
	...
	if (t->tcb_rel.relpgtype != TCB_PG_V1)
	{
	    s = btree_reposition(r, &r->rcb_other_page_ptr, bid,
		&r->rcb_fet_lsn, r->rcb_fet_clean_count, r->rcb_fet_nextleaf, 
		r->rcb_fet_key_ptr, tid, DM1C_BYTID, &buf_locked, err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	*/
	...
	if ( t->tcb_rel.relstat & TCB_CLUSTERED )
	{
	    ...
	    */
	    ...
	    if ( r->rcb_access_mode != RCB_A_READ &&
		    r->rcb_update_mode == RCB_U_DEFERRED &&
		    dm1cx_isnew(r, leafpage, (i4)bid->tid_tid.tid_line) )
	    {
	...
	else
	{
	    ...
	    ** not already fixed.
	    */
	    if (r->rcb_data_page_ptr && localtid.tid_tid.tid_page !=
		DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, r->rcb_data_page_ptr))
	    {
		s = dm0p_unfix_page(r, DM0P_UNFIX, 
		...
		s = dm0p_fix_page(r, (DM_PAGENO )localtid.tid_tid.tid_page, 
		    fix_action | DM0P_NOREADAHEAD, &r->rcb_data_page_ptr,
		    err_code);
		if (s != E_DB_OK)
		    break;
	    }
	    ...
	    */
	    ...
	    s = (*t->tcb_acc_plv->dmpp_get)(t->tcb_rel.relpgtype,
		    ...
		    &rec_ptr, row_ver_ptr, NULL, (DMPP_SEG_HDR *)0);
	...
	*/
	...
	else
	{
		...
		break;
	    tmpkey_len = t->tcb_klen;
	    _VOID_ dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
			t->tcb_acc_tlv, t->tcb_leafatts, t->tcb_leafattcnt, 
			bid->tid_tid.tid_line,
			&KeyPtr, (DM_TID *)0, (i4*)0,
			&tmpkey_len,
			DM1B_INDEX_COMPRESSION(r), NULL);
    ...
    if (buf_locked)
    {
	dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, &r->rcb_other_page_ptr);
    }
	...
	*err_code = E_DM925F_DM1B_GETBYBID;
    }
...
**	rcb_other_page_ptr and rcb_data_page_ptr pointers, if they are not
    ...
    i4	*err_code)
{
    ...
    DMP_TCB         *t = r->rcb_tcb_ptr;
    DMP_TABLE_IO    *tbio = &t->tcb_table_io;
    DM1B_INDEX	    *leafpage;	    /* Pointer to page of type index. */
    ...
    for (;;)
    {
	if (row_locking(r) && BaseTable && DMZ_SES_MACRO(33))
	{
	    DB_STATUS     tmp_status;
	...
	*/
	if ((r->rcb_other_page_ptr == NULL) || 
	    (DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype,
	    r->rcb_other_page_ptr) != localbid.tid_tid.tid_page))
	{
	    if (r->rcb_other_page_ptr != NULL)
	    {
		s = dm0p_unfix_page(r, DM0P_UNFIX, &r->rcb_other_page_ptr,
		    err_code);
		if (s != E_DB_OK)
		    break;
	    }
	...
	leafpage = (DM1B_INDEX *)r->rcb_other_page_ptr;
	...
	*/ 
	...
	/* Re-init local bid,leafpage in case we repositioned */
	localbid.tid_i4 = bid->tid_i4;
	leafpage = (DM1B_INDEX *)r->rcb_other_page_ptr;
	...
	** fixed into rcb_data_page_ptr and not locally.
	*/
	if ( NeedDataPage )
	{
	    if ( r->rcb_data_page_ptr == NULL || 
		(DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype,
		    r->rcb_data_page_ptr) != localtid.tid_tid.tid_page) )
	    {
		if (r->rcb_data_page_ptr != NULL)
		{
		    s = dm0p_unfix_page(r, DM0P_UNFIX, &r->rcb_data_page_ptr, 
					err_code);
		    if (s != E_DB_OK)
			break;
		}
		...
		s = dm0p_fix_page(r, (DM_PAGENO)localtid.tid_tid.tid_page, 
		    (DM0P_WRITE | DM0P_NOREADAHEAD), &r->rcb_data_page_ptr, 
		    err_code);
		if (s != E_DB_OK)
		    break;
	    }
	...
	*/
	if ( NeedDataPage )
	{
	    ...
	    */
	    s = dm1r_delete(r, &localtid, (i4)0, opflag, err_code);
	    if (s != E_DB_OK)
		break;
	    ...
	    ** data page pointer.
	    */
	    s = check_reclaim(r, &r->rcb_data_page_ptr, err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	*/
	if ((DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, leafpage) & 
			DMPP_CHAIN) && 
	    (DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leafpage)== 1))
	{
	    dm1cxrecptr(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
			localbid.tid_tid.tid_line, &duplicate_key);
	    duplicate_keylen = t->tcb_klen;
	    if (DM1B_INDEX_COMPRESSION(r) == DM1CX_COMPRESSED)
	    {
		dm1cx_klen(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
			localbid.tid_tid.tid_line, &duplicate_keylen);
	    }
	    if ( s = dm1b_AllocKeyBuf(duplicate_keylen, key_buf,
					&KeyPtr, &AllocKbuf, err_code) )
		break;
	...
	*/
	s = dm1bxdelete(r, leafpage, (DM_LINE_IDX)localbid.tid_tid.tid_line,
		(i4)TRUE, (i4)TRUE, &reclaim_space, err_code); 
	if (s != E_DB_OK)
	    break;
	...
	** causes it to be unfixed from rcb_other_page_ptr.
	**
	** Note if row locking, the entry is just marked deleted
	*/
	if ((t->tcb_rel.relpgtype == TCB_PG_V1) &&
	    (DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, leafpage) & 
				DMPP_CHAIN) && 
	    (DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leafpage) == 0))
	{
	...
	*/
	...
	{
	    s = dm1b_rcb_update(r, &localbid, (DM_TID *)0, (i4)0,
			DM1C_MDELETE,
			reclaim_space, tid, (DM_TID *)0, (char *)0, err_code);
	}
	...
	*/
	...
	if (DMZ_CRH_MACRO(2) && ( ! BaseTable))
	{
	    ...
	    (VOID)dm0p_unfix_page(r,DM0P_FORCE,&r->rcb_other_page_ptr,err_code);
	    CSterminate(CS_CRASH, &n);
	}
...
**      rcb->rcb_other_page_ptr  - A pointer to current leaf.
    ...
    i4         	    *err_code)
{
    ...
    if (r->rcb_rnl_online)
    {
	return (dm1b_online_get(r, tid, record, opflag, err_code));
    }
	    ...
	    DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leaf))
	{
	    ...
	    */
	    if (buf_locked)
	    {
		dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, 
	...
	*/
	...
	{
	    s = btree_reposition(r, &r->rcb_other_page_ptr, &r->rcb_repos_bid, 
		    &r->rcb_repos_lsn, r->rcb_repos_clean_count,
		    r->rcb_repos_nextleaf, r->rcb_repos_key_ptr,
		    &r->rcb_repos_tid, opflag, &buf_locked, err_code);
	    if (s != E_DB_OK)
		break;
	    r->rcb_lowtid.tid_i4 = r->rcb_repos_bid.tid_i4;
	}
	reposition = FALSE;
	...
	*/
	if (opflag & DM1C_GETNEXT) 
	    s = next(r, &buf_locked, low_tran_ptr, opflag, err_code);
	else
	    s = prev(r, &buf_locked, low_tran_ptr, opflag, err_code);
	...
	*/
	...
	else
	{
		...
		*/
		if (row_locking(r) && (r->rcb_data_page_ptr == NULL))
		{
		    s = dm1b_rowlk_access(r, get_status, ROW_ACC_NEWDATA, 
				NULL, &tid_to_lock, (i4)0,
				&buf_locked, &data_buf_locked, &save_lock_key,
				opflag, err_code);
		    if (s == E_DB_WARN)
			continue;
		    if (s == E_DB_ERROR)
		...
		*/
		...
		if (row_locking(r))
		{
		    dm0p_lock_buf_macro(tbio, r->rcb_lk_id, 
					&r->rcb_data_page_ptr);
		    data_buf_locked = TRUE;
		}
		...
		*/
		...
		if (s != E_DB_OK)
		{
		    ...
		    */
			...
			t->tcb_rel.relpgsize, leafpage, t->tcb_acc_tlv, 
			t->tcb_leafatts, t->tcb_leafattcnt,
			r->rcb_lowtid.tid_tid.tid_line, &KeyPtr,
			(DM_TID *)0, (i4*)0,
			&key_len, DM1B_INDEX_COMPRESSION(r), NULL);
		    if (t->tcb_rel.relpgtype != TCB_PG_V1 
		    ...
		    */
		    ...
		    {
			s = E_DB_ERROR;
			*err_code = E_DM9350_BTREE_INCONSIST;
			break;
		    }
		}
	...
	*/
	...
	if (r->rcb_f_qual)
	{
	    ...
	    ** qualification
	    */
	    if (row_locking(r) && (opflag & DM1C_PKEY_PROJECTION) == 0)
	    {
		s = dm1b_rowlk_access(r, get_status, ROW_ACC_NONKEY, low_tran_ptr,
			    &tid_to_lock, r->rcb_partno,
			    &buf_locked, &data_buf_locked, 
		...
		** be changed to return E_DM93A3_QUALIFY_ROW_ERROR
		** so that traceback messages will be given.
		*/
# ifdef USER_ERROR_HANDLED
	        *err_code = E_DM93A3_QUALIFY_ROW_ERROR;
# endif
	...
	/*
	** If row locking, this row qualifies, make sure it is locked.
	** Convert physical to logical if repeatable read
	** Unlock the buffers
	*/
	if (row_locking(r))
	{
	    s = dm1b_rowlk_access(r, get_status, ROW_ACC_RETURN, low_tran_ptr, 
			&tid_to_lock, r->rcb_partno,
			&buf_locked, &data_buf_locked, 
			&save_lock_key, opflag, err_code);
	    if (s == E_DB_WARN)
	...
	*/
	...
	{
	    TRdisplay("Row locking and leaf buffer not locked\n");
	    return (E_DB_ERROR);
	}
	...
	if (t->tcb_rel.relpgtype != TCB_PG_V1)
	{
	    ...
	    */
		...
		{
		    att = t->tcb_leafkeys[k];
		    MEcopy(r->rcb_record_ptr + att->offset, 
				att->length, 
			   r->rcb_repos_key_ptr + att->offset);
		    if ( fix_action & DM0P_WRITE )
			MEcopy(r->rcb_record_ptr + att->offset, 
				    att->length, 
			       r->rcb_fet_key_ptr + att->offset);
		}
	    }
	    else
	    ...
	    DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, 
		    r->rcb_other_page_ptr, r->rcb_repos_lsn);
	    r->rcb_repos_clean_count = 
		    DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype,
		    r->rcb_other_page_ptr);
	    r->rcb_repos_nextleaf =
		    DM1B_VPT_GET_BT_NEXTPAGE_MACRO(t->tcb_rel.relpgtype,
		    r->rcb_other_page_ptr);
	    r->rcb_repos_tid.tid_i4 = tid->tid_i4;
	    r->rcb_repos_bid.tid_i4 = r->rcb_lowtid.tid_i4;
	}
	if (buf_locked)
	    dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, &r->rcb_other_page_ptr);
	...
	if (fix_action & DM0P_WRITE)
	{
	    /* Already did this above for CLUSTERED */
	    if ( !(t->tcb_rel.relstat & TCB_CLUSTERED) )
		MEcopy((t->tcb_rel.relstat & TCB_INDEX) ? r->rcb_record_ptr : 
		    r->rcb_s_key_ptr, t->tcb_klen, r->rcb_fet_key_ptr);
	    /* Some rcb_fet* fields needed only for V2 pages */
	    if (t->tcb_rel.relpgtype != TCB_PG_V1)
	    {
		STRUCT_ASSIGN_MACRO(r->rcb_repos_lsn, r->rcb_fet_lsn);
		r->rcb_fet_nextleaf = r->rcb_repos_nextleaf;
		r->rcb_fet_clean_count = r->rcb_repos_clean_count;
	...
	*/
	if ( t->tcb_rel.relstat & (TCB_INDEX | TCB_CLUSTERED) )
	{
	    if ( t->tcb_rel.relstat & TCB_CLUSTERED )
	    {
		s = VerifyHintTidAndKey(r, &leaf, tid, r->rcb_hl_ptr, 
					record, &localtid, err_code);
		if ( s )
		    break;
	    }
	    else
		...
		*/
		...
		if (leaf == NULL)
		{
			...
			fix_action | DM0P_NOREADAHEAD, &leaf, err_code);
		    if (s != E_DB_OK)
		    {
			if (*err_code > E_DM_INTERNAL)
			    *err_code = E_DM003C_BAD_TID;
		...
		*/
		...
		s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
			    ...
			    &rec_len,
			    DM1B_INDEX_COMPRESSION(r), low_tran_ptr);
	    ...
	    */
		...
		s = dma_row_access(DMA_ACC_SELECT|DMA_ACC_BYTID, r,
			(char*)rec_ptr, (char*)NULL, &compare, err_code);
	...
	{
	    s = dm0p_fix_page(r, (DM_PAGENO )localtid.tid_tid.tid_page, 
		(fix_action | DM0P_USER_TID), &r->rcb_data_page_ptr,
		err_code);
	    if (s != E_DB_OK)
	    {
		if (*err_code > E_DM_INTERNAL)
	    ...
	    r->rcb_data_page_ptr))
	{
		...
		break;
	    s = dm0p_fix_page(r, (DM_PAGENO )localtid.tid_tid.tid_page, 
		(fix_action | DM0P_USER_TID), &r->rcb_data_page_ptr, err_code);
	    if (s != E_DB_OK)
	    {
		if (*err_code > E_DM_INTERNAL)
		    *err_code = E_DM003C_BAD_TID;
	...
	if (s == E_DB_OK)
	{
	    ...
	    */
		...
		if (s)
		{
		    ...
		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			       (char *)0, (i4)0, (i4 *)0, &local_err_code, 0);
		    *err_code = E_DM9B01_GET_DMPE_ERROR;
		    break;
		}
	    }
	...
	*err_code = E_DM9261_DM1B_GET;
    ...
    if (leaf && leaf != r->rcb_other_page_ptr)
	s = dm0p_unfix_page(r, DM0P_UNFIX, &leaf, &local_err_code);
    ...
    i4	    *err_code)
{
    ...
    DMP_TCB		*t = r->rcb_tcb_ptr;
    DMP_TABLE_IO        *tbio = &t->tcb_table_io;
    DM1B_INDEX  	*leafpage = (DM1B_INDEX *)*leaf;
    i4		n;
    DB_STATUS		s;
    bool		base_table;
    bool		NeedDataPage;
	...
	*/
	if ((leafpage == NULL) || 
	    ...
	    bid->tid_tid.tid_page))
	{
	    ule_format(E_DM9396_BT_WRONG_PAGE_FIXED, (CL_SYS_ERR *)0, ULE_LOG,
		...
		(leafpage ? DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype,
		leafpage): -1), 0, bid->tid_i4, 0, tid->tid_i4);
	    *err_code = E_DM9262_DM1B_PUT;
	    break;
	}
	...
	*/
	if ( NeedDataPage )
	{
	    s = dm1r_put(r, data, tid, record, (char*)0, record_size, 
	...
	*/ 
	if (r->rcb_alloc_clean_count != DM1B_CC_INVALID)
	{
	    s = btree_reposition(r, leaf, bid,
		    &r->rcb_alloc_lsn, r->rcb_alloc_clean_count, 
		    r->rcb_alloc_nextleaf, key,
		    &r->rcb_alloc_tidp, DM1C_BYTID, &buf_locked, err_code);
	    if (s != E_DB_OK)
		break;

	    /* Re-init local leafpage in case we repositioned */
	    leafpage = (DM1B_INDEX *)*leaf;
	}
	...
	*/
	s = dm1bxinsert(r, t, t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
		    t->tcb_leafatts, t->tcb_leafattcnt, 
		    DM1B_INDEX_COMPRESSION(r), t->tcb_klen, leafpage, 
	...
	if (opflag == DM1C_MPUT)
	{
	    s = dm1b_rcb_update(r, bid, (DM_TID *)0, (i4)0,
			DM1C_MPUT,
			(i4)0, tid, (DM_TID *)0, (char *)0, err_code);
	}
    ...
    i4	    *err_code)
{
    ...
    TidSize = DM1B_VPT_GET_BT_TIDSZ_MACRO(t->tcb_rel.relpgtype, leafpage);
	...
	s = (*t->tcb_acc_plv->dmpp_get)(t->tcb_rel.relpgtype,
		...
		&rec_ptr, NULL, NULL, (DMPP_SEG_HDR *)0);
	...
	if (line != 0)
	{
	    ...
	    */
	    s = dm1b_compare_range( r, leafpage, dmpe_key, &correct_leaf,
			err_code);
	    if (s != E_DB_OK)
		break;
	    if (correct_leaf && 
		dm1cxhas_room(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
			leafpage, compression_type, (i4)100 /* fill factor */,
			t->tcb_kperleaf, (t->tcb_klen + 
			t->tcb_comp_katts_count + TidSize)) )
	    {
		/*
		...
		*/
		s = dm1bxsearch(r, leafpage, DM1C_OPTIM, DM1C_EXACT, dmpe_key,
		...
		*/
		...
		s = btree_search(r, dmpe_key, 
			...
			leaf, bid, &search_tid, &buf_locked, NULL, err_code);
		leafpage = (DM1B_INDEX *)*leaf;
		if (s != E_DB_OK)
		    break;
		STRUCT_ASSIGN_MACRO(*bid, local_bid);
	...
	/* Insert the row */
	s = dm1b_put(rcb, leaf, data, &local_bid, &local_tid, rec_ptr,
			dmpe_key, dmpe_size, opflag, err_code);
    ...
    i4         	    *err_code)
{
    ...
    DMP_TCB		*t = r->rcb_tcb_ptr;
    DM1B_INDEX		*leaf; 
    DM1B_INDEX		*newleaf = (DM1B_INDEX *)(*newleafpage); 
    DMPP_PAGE		*data;
    DMPP_PAGE		*newdata = *newdatapage;
    DMPP_PAGE		*leafpage;
    DM_TID		datatid;
    i4			data_partno = 0;
	...
	** they are required:
	**
	**     Old leaf page - must be fixed in rcb_other_page_ptr
	**     New leaf page - must be fixed in newleafpage
	**           - unless INPLACE replace requires no index update
	**
	**     Old data page - must be fixed in rcb_data_page_ptr
	...
	*/
	leaf = (DM1B_INDEX *) r->rcb_other_page_ptr;
	data = r->rcb_data_page_ptr;
	...
	if (!inplace_update)
	{
	    ...
	    */ 
	    ...
	    {
		s = btree_reposition(r, &r->rcb_other_page_ptr, bid, 
		    &r->rcb_fet_lsn, r->rcb_fet_clean_count, 
		    r->rcb_fet_nextleaf, r->rcb_fet_key_ptr,
		    tid, DM1C_BYTID, &buf_locked, err_code);
		if (s != E_DB_OK)
		    break;
	    }
	    ...
	    /*
	    ** We don't really expect to need to reposition here
	    ** since we only do replace if page locking, in which
	    ** case we should've repositioned after allocate
	    */
	    if (save_bid.tid_i4 != bid->tid_i4)
		TRdisplay("dm1b_replace: reposition %t\n",
			    sizeof(DB_TAB_NAME), &t->tcb_rel.relid);
	}
	    ...
	    bid->tid_tid.tid_page)))
	{
		...
		sizeof(DB_TAB_NAME), &t->tcb_rel.relid,
		sizeof(DB_OWN_NAME), &t->tcb_rel.relowner,
		0, bid->tid_tid.tid_page, 0, (leaf ? 
		DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leaf) : -1), 
		0, bid->tid_i4, 0, tid->tid_i4);
	    *err_code = E_DM9263_DM1B_REPLACE;
	    break;
	}
	...
	*/
	if ( NeedDataPage )
	{
	    s = dm1r_replace(r, tid, record, record_size, newdata, newtid, 
			     nrec, nrec_size, (i4)0, 
	...
	*/
	if ( ! inplace_update || t->tcb_rel.relstat & TCB_CLUSTERED )
	{
	    ...
	    */
	    ...
	    {
		dm1cxrecptr(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leaf, 
			bid->tid_tid.tid_line, &duplicate_key);
		duplicate_keylen = t->tcb_klen;
		if (DM1B_INDEX_COMPRESSION(r) == DM1CX_COMPRESSED)
		{
		    dm1cx_klen(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
			leaf, bid->tid_tid.tid_line, &duplicate_keylen);
		}
		if ( s = dm1b_AllocKeyBuf(duplicate_keylen, key_buf,
					    &KeyPtr, &AllocKbuf, err_code) )
		    break;
	    ...
	    s = dm1bxdelete(r, leaf, (DM_LINE_IDX)bid->tid_tid.tid_line,
		    (i4)TRUE, (i4)TRUE, &reclaim_space, err_code);
	    if (s != E_DB_OK)
		break;
	    ...
	    */
	    if (DMZ_CRH_MACRO(7))
	    {
		(VOID) dm0p_unfix_page(r,DM0P_FORCE,(DMPP_PAGE*)&leaf,err_code);
	    ...
	    */
	    ...
	    s = dm1bxinsert(r, t, t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
			t->tcb_leafatts, t->tcb_leafattcnt, 
			DM1B_INDEX_COMPRESSION(r), t->tcb_klen, 
			newleaf, 
			(t->tcb_rel.relstat & TCB_CLUSTERED) ? nrec : newkey, 
			&datatid, data_partno,
			insert_position, 
			(i4)TRUE, (i4)TRUE, err_code); 
	    if (s != E_DB_OK)
		break;
	    ...
	    ** from rcb_other_page_ptr.
	    **
	    ** Note if row locking, the entry is just marked deleted
	    */
	    if ((DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, leaf) & 
		DMPP_CHAIN) && 
		(DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leaf) == 0))
	    {
		s = dm1bxfree_ovfl(r, duplicate_keylen, duplicate_key,
				&r->rcb_other_page_ptr, err_code);
		if (s != E_DB_OK)
	...
	*/
	s = dm1b_rcb_update(r, bid, newbid, (i4)0, DM1C_MREPLACE,
				reclaim_space, tid, newtid, newkey, err_code);
	if (s != E_DB_OK)
	    break;
	...
	*/
	if (DMZ_CRH_MACRO(8))
	{
	    ...
	    {
		(VOID) dm0p_unfix_page(r,DM0P_FORCE,(DMPP_PAGE*)&leaf,err_code);
		(VOID) dm0p_unfix_page(r,DM0P_FORCE,newleafpage,err_code);
	    }
    ...
    i4        *err_code)
{
	...
	*/
	status = active_rcb_update(rcb, 
		...
		del_reclaim_space, tid, newtid, newkey, err_code);
    }
    else
    {
	status = passive_rcb_update(rcb, opflag, bid_input,
			tid, newtid, newkey, err_code);
    }
    ...
    i4        *err_code)
{
	...
	*/
	if ((r->rcb_k_duration & RCB_K_READ) &&
	    (r->rcb_other_page_ptr != NULL) &&
	    (r->rcb_other_page_ptr == r->rcb_1rnl_page ||
	    r->rcb_other_page_ptr == r->rcb_2rnl_page) &&
	    (DMPP_VPT_GET_PAGE_PAGE_MACRO(r->rcb_tcb_ptr->tcb_rel.relpgtype,
	    r->rcb_other_page_ptr) == bid.tid_tid.tid_page))
	{
	     continue;
	}
	...
	{	
		...
		*/
		...
		{
		    MEcopy(newkey, r->rcb_tcb_ptr->tcb_klen, 
		    	r->rcb_fet_key_ptr);
		    r->rcb_fet_tidp.tid_i4 = newtid->tid_i4;
		    r->rcb_state |= RCB_FETCH_REQ;
		}
		break;
	    }
	    ...
	    **
	    */
	    if (r->rcb_fet_tidp.tid_i4 == tid->tid_i4)
	    {
		MEcopy(newkey, r->rcb_tcb_ptr->tcb_klen, 
		    r->rcb_fet_key_ptr);
		r->rcb_fet_tidp.tid_i4 = newtid->tid_i4;
	...
	default:
	    ...
	    *err_code = E_DM9C25_DM1B_RCB_UPDATE;
	    return (E_DB_ERROR);
	}
...
**  DM1C_MPUT:
**
**          No rcb update action for DM1C_MPUT
**
**  DM1C_MSPLIT:
**
**          Set r->rcb_state |= RCB_FETCH_REQ, 
**          If we are doing a replace, if allocating space for the new
**          tuple caused a split, we need to re-fetch tuple to be deleted
**
**  DM1C_MDELETE:
**
**         Compare:
**              (delete key, delete tid)
**         To:
**              (rcb_fet_key_ptr, rcb_fet_tidp)
**
...
**
**          Compare:
**              (replace key, replace tid)
**          To:
**              (rcb_fet_key_ptr, rcb_fet_tidp)
**
**          If another cursor has fetched the row being updated,
**          update the key and tidp saved for the fetched record.
**
**
** Inputs:
**      rcb                             Pointer to record access context
    ...
    i4        *err_code)
{
	...
	*/
	...
	switch (opflag)
	{
	    ...
	    {
		/*
		** During replace, if a SPLIT, RESERVE, CLEAN happens
		** for the leaf where the old tuple was,
		** re-fetch tuple to delete
		*/
		if (r == orig_rcb &&
		    r->rcb_fetchtid.tid_tid.tid_page == bid->tid_tid.tid_page)
		{
		    r->rcb_state |= RCB_FETCH_REQ;
		}
    ...
    i4            *err_code)
{
    ...
    status = btree_search(r, key, keys_given, mode, direction, leaf, 
	bid, tid, &buf_locked, NULL, err_code);

    /*
    ** Save page_lsn, page_clean_count at time of position
    */
    if ( (status == E_DB_OK) && (*leaf != NULL) && 
	(DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, *leaf) 
	    == bid->tid_tid.tid_page))
	
    {
	DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, *leaf,
		r->rcb_pos_lsn);
	r->rcb_pos_clean_count = 
    ...
    /* Init clean_count for CURRENT position */
    ...
    i4            	*err_code)
{
    ...
    DMP_TABLE_IO *tbio = &t->tcb_table_io;
    DMPP_PAGE    *save_sprig, *parent, *current; 
                                        /* Pointer to pages. */
    DM1B_INDEX   *parpage, *curpage, *leafpage; 
...
# endif
	...
	s = dm0p_fix_page(r, (DM_PAGENO)DM1B_ROOT_PAGE, 
	    index_fix_action, &parent, err_code);     
	if (s != E_DB_OK) 
	    break;
	...
	if (mode == DM1C_SPLIT &&
	    dm1cxhas_room(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, parpage, 
		DM1B_INDEX_COMPRESSION(r),
		(i4)100 /* fill factor */,
		t->tcb_kperpage,
	       (t->tcb_ixklen + t->tcb_comp_katts_count 
		+ DM1B_VPT_GET_BT_TIDSZ_MACRO(
		    t->tcb_rel.relpgtype, parpage)))
			== FALSE)
	{
	    /* Split the root page, make parent the newroot. */

	    s = dm1bxnewroot(r, &parent, err_code);  
	    if (s != E_DB_OK)
		break;
	    parpage = ((DM1B_INDEX *)parent); 
	}
	...
	*/
	...
	do 
	{
	    s = dm1bxsearch(r, parpage, xmode, direction, index_key, 
		keys_given, &child, &childpartno, &pos, NULL, err_code); 
	    if (s != E_DB_OK && *err_code != E_DM0056_NOPART) 
		break;
	    ...
	    */
	    ...
	    if ((DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, parpage) & 
		(DMPP_SPRIG|DMPP_INDEX)) == 0)
	    {
		dm1cxlog_error(E_DM93ED_WRONG_PAGE_TYPE, r, parpage,
				t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 0);
		*err_code = E_DMF014_DM1B_PARENT_INDEX;
		s = E_DB_ERROR;
		break;
	    }

	    at_sprig_level = 
		(DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, parpage) & 
		DMPP_SPRIG) != 0;
		...
		if (mode != DM1C_SPLIT)
		{	
		    s = dm0p_unfix_page(r, DM0P_RELEASE, &parent, err_code);
		    if (s != E_DB_OK)
			break;
		}	    
		else
	    ...
	    {
		dm1cxlog_error(E_DM93ED_WRONG_PAGE_TYPE, r, curpage,
				t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 0);
		*err_code = E_DMF013_DM1B_CURRENT_INDEX;
		s = E_DB_ERROR;
		break;
	    }
	    ...
	    */
		...
		if ((DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, curpage) & 
		    DMPP_LEAF) == 0)
		{
		    dm1cxlog_error( E_DM93EA_NOT_A_LEAF_PAGE, r, curpage,
			t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 0);
		    s = E_DB_ERROR;
		    *err_code = E_DM9264_DM1B_SEARCH;
		    break;
		}

		if (row_locking(r))
		{
		    dm0p_lock_buf_macro(tbio, r->rcb_lk_id, &current);
		    *buf_locked = TRUE;
		}
		...
		*/
		...
		{
		    s = dm1bxclean(r, curpage, err_code);
		    if (s != E_DB_OK)
			break;
		}
		...
		i2	TidSize = DM1B_VPT_GET_BT_TIDSZ_MACRO(
				    t->tcb_rel.relpgtype, curpage);

		if ( dm1cxhas_room(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
		    curpage, DM1B_INDEX_COMPRESSION(r),
		    (i4)100 /* fill factor */,
		    at_sprig_level ? t->tcb_kperleaf : t->tcb_kperpage,
		    at_sprig_level ? 
		    (t->tcb_klen + t->tcb_comp_katts_count + TidSize) :
		    (t->tcb_ixklen + t->tcb_comp_katts_count + TidSize)) == FALSE)
		{   
		    s = dm1bxsplit(r, &current, (i4)pos, parent, 
			at_sprig_level ? leaf_key : index_key,
			keys_given, buf_locked, err_code); 
	...
	} while (!(DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, parpage) & 
	    DMPP_LEAF));
	...
	*/
	...
	{
	    s = dm1b_requalify_leaf(r, mode, &current,
		    leaf_key, keys_given, buf_locked, err_code);
	    if (s != E_DB_OK)
		break;
	}

	leafpage = (DM1B_INDEX *)((current));
	...
	*/
	...
	state = E_DB_OK;
	s = dm1bxdupkey(r, &current, DupKey, &state, err_code);
	if (s != E_DB_OK)
	    break;
	if ( state != E_DB_OK )
	    DupKey = NULL;
	...
	*/
	...
	{
	    s = dm1bxsplit(r, &current, (i4) pos, parent, leaf_key, 
		    keys_given, buf_locked, err_code); 
	    if (s != E_DB_OK)
		break;
	    leafpage = ((DM1B_INDEX *)current);
	...
	*/
	...
	if (DMZ_AM_MACRO(9))
	{
	    ...
	    dmd_prindex(r, (DM1B_INDEX *)current, (i4)0);
	}
	...
	*/
	...
	if (direction == DM1C_EXACT || mode == DM1C_RANGE)
	{
	    for (;;)
	    {
		s = dm1bxsearch(r, leafpage, mode, direction, leaf_key, 
		...
		*/
		    ...
		    leafpage) != 0))
		{
		    ...
		    /* mode == DM1C_TID */
		    s = dm1b_compare_key(r, leafpage, DM1B_RRANGE, leaf_key,
				keys_given, &compare, err_code);
		    ...
		    */
		    if (*buf_locked == TRUE)
		    {
			dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, &current);
			*buf_locked = FALSE;
		    }
		    s = dm0p_unfix_page(r, DM0P_UNFIX, &current, err_code);
		    if (s != E_DB_OK)
	...
	*/
	...
	if (mode == DM1C_LAST)
	{
		...
		s = dm1b_find_ovflo_pg(r, leaf_fix_action, &current, LAST_OVFL,
	    ...
	    pos = DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leafpage); 
	}
	...
	*/
	...
	{
	    if (dm1cxhas_room(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
		leafpage, DM1B_INDEX_COMPRESSION(r),
		(i4)100 /* fill factor */,
		t->tcb_kperleaf, 
		(t->tcb_klen + t->tcb_comp_katts_count + 
	...
	*/
	...
	if (mode == DM1C_FIND )
	{
	    if (DM1B_VPT_GET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype, leafpage) != 
		NULL)
	    {
		if (DupKey == NULL || samekey != DM1B_SAME)
		{
		...
		if ( record )
		    entry_ptr = record;
					    ...
					    &entry_ptr, &AllocBuf, err_code) )
		    break;
		entry_len = t->tcb_klen;
		s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
			t->tcb_acc_tlv, t->tcb_leafatts, t->tcb_leafattcnt,
			pos, &entry_ptr, (DM_TID *)0, (i4*)0,
			&entry_len,
			DM1B_INDEX_COMPRESSION(r), NULL);
    ...
    if ((*err_code > E_DM_INTERNAL) && (*err_code != E_DM9CB1_RNL_LSN_MISMATCH))
    {
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM9264_DM1B_SEARCH;
    }
    ...
    i4		*err_code)
{
    ...
    char	    key_buf[DM1B_MAXSTACKKEY];
    i4	    	    key_len;
    DMPP_PAGE       *data;              /* Pointer to a data page. */
    DM1B_INDEX      *leafpage; 
                                        /* Pointer to pages of type index. */
	...
	*err_code = E_DM923D_DM0M_NOMORE;
    ...
    savepageno = DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leafpage);
	...
	*/
	...
	if ((i4)(localbid.tid_tid.tid_line) == 
	   DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leafpage))
	{
	    s = dm1bxchain(r, leaf, &localbid, (i4)FALSE, leafkey, 
			buf_locked, err_code);
	    if (s != E_DB_OK)
	    {
	...
	*/
	...
	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
			...
			&key_len, DM1B_INDEX_COMPRESSION(r), low_tran_ptr );
	...
	for (error = E_DB_OK, s = E_DB_OK;;)
	{
		...
		*redo_dupcheck = TRUE;
		...
		s = (*t->tcb_acc_plv->dmpp_get)(t->tcb_rel.relpgtype,
		    t->tcb_rel.relpgsize, data, &localtid, &wrec_size,
		    &wrec_ptr, row_ver_ptr, NULL, (DMPP_SEG_HDR *)0);

		/*
		** If row locking, we have a value lock, so we should
		** not need to look at deleted records
		*/
		if (row_locking(r) && s == E_DB_WARN)
		{
		    s = E_DB_OK;
		    break;
		}
		...
		if (s == E_DB_OK &&
		    ...
		    DMPP_VPT_PAGE_HAS_SEGMENTS(t->tcb_rel.relpgtype)))
		{
		    s = dm1c_get(r, data, &localtid, wrec, err_code);
		    if (s && *err_code != E_DM938B_INCONSISTENT_ROW)
			break;
		    wrec_ptr = wrec;
		}
		...
		if (compare == 0)
		{
		    error = E_DM0046_DUPLICATE_RECORD;
		    break;
		}
	    }
	    ...
	    savepageno) 
	{
		...
		*buf_locked = FALSE;
	    ...
	    s = dm0p_unfix_page(r, DM0P_UNFIX, (DMPP_PAGE **)leaf, 
		&local_err_code);
	    if (s != E_DB_OK)
		break;
	    s = dm0p_fix_page(r, savepageno, 
                    (DM0P_WRITE | DM0P_NOREADAHEAD),
		    (DMPP_PAGE **)leaf, &local_err_code);
	    if (s != E_DB_OK)
		break;
	    if (row_locking(r))
	    {
...
**      entry and exitin rcb->rcb_other_page_ptr. The direction to search 
    ...
    i4                 *err_code)
{
	...
	*err_code = E_DM002A_BAD_PARAMETER;
	...
	if (getnextovfl == FALSE && getnextleaf == FALSE)
	{
	    ...
	    if (get_status == E_DB_WARN)
	    {
		if (DM1B_SKIP_DELETED_KEY_MACRO(r, leafpage, *low_tran_ptr))
		    continue;
		else
	...
	*/
	...
	s = dm0p_fix_page(r, savepageno, fix_action, &r->rcb_other_page_ptr, 
			  err_code);
	if (s != E_DB_OK)
	    break;
	leaf = r->rcb_other_page_ptr; 
	...
	*/
	if (t->tcb_rel.relpgtype != TCB_PG_V1)
	{
	    ...
	    page_attcnt = DM1B_VPT_GET_BT_ATTCNT(t->tcb_rel.relpgtype, leafpage);
	    if (page_attcnt != t->tcb_leafattcnt)
		TRdisplay("BTREE leaf page key count error %d %d\n",
			t->tcb_leafattcnt, page_attcnt);
	    for (attno = 1, atts = atts_array; attno <= page_attcnt; attno++)
	    {
		atr = *(atts++);
		DM1B_VPT_GET_ATTR(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
				leafpage, attno, &attdesc);
	...
	if (r->rcb_repos_clean_count != DM1B_CC_INVALID)
	    r->rcb_repos_clean_count = DM1B_CC_TIDEOF_LEAF;
	start = TRUE;
...
**      entry and exit in rcb->rcb_other_page_ptr. There is currently no
    ...
    i4                 *err_code)
{
    ...
    */
    ...
    compression = DM1B_INDEX_COMPRESSION(r);
    ...
    */
    ...
    {
        *err_code = E_DM0055_NONEXT;  /* reached begining of table */
        return (E_DB_ERROR);
    }

    if (bid->tid_tid.tid_page != 
	DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, r->rcb_other_page_ptr))
    {
        *err_code = E_DM002A_BAD_PARAMETER;
        return (E_DB_ERROR);
    }
    ...
    for (;;)
    {
        ...
        if (got_tid)
        {
			...
			&key_len, compression, low_tran_ptr);
	    ...
	    DMPP_CHAIN)
	{
	    ...
	    s = dm1b_find_ovflo_pg(r, fix_action, (DMPP_PAGE **)&leafpage, 
		...
		leafpage) & DMPP_CHAIN ?
		DM1B_VPT_GET_PAGE_MAIN_MACRO(t->tcb_rel.relpgtype, leafpage) :
		/* out leafs page_page */
		DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leafpage);
		/* our page_page */
		...
		*buf_locked = FALSE;
	    ...
	    /* 
	    ** About to unfix leafpage, make sure we clear duplicate
	    ** pointer in r->rcb_other_page_ptr 
	    ** Otherwise if we get an error in btree_search, (like deadlock)
	    ** we will incorrectly try to unfix this page twice
	    */
	    if ((DM1B_INDEX *)r->rcb_other_page_ptr == leafpage)
		r->rcb_other_page_ptr = NULL;
            s = dm0p_unfix_page(r, DM0P_UNFIX, (DMPP_PAGE **)&leafpage,
		err_code);
            if (s != E_DB_OK)
                break;
	    ...
	    ** transform it again into another (wrong) INDEX key.
		...
		s = btree_search(r, ixkeybuf, keys_given, DM1C_RANGE,
		   DM1C_PREV, (DMPP_PAGE **)&leafpage, bid, tid,
		   buf_locked, NULL, err_code);
	    ** end of FIXME */
	       ...
	       buf_locked, NULL, err_code);
            ...
            while (DM1B_VPT_GET_BT_NEXTPAGE_MACRO(t->tcb_rel.relpgtype, leafpage)
		&& DM1B_VPT_GET_BT_NEXTPAGE_MACRO(t->tcb_rel.relpgtype, leafpage)
		!= savepg)
            {
		...
		if (*buf_locked == TRUE)
		{
		    ...
		    *buf_locked = FALSE;
                ...
                s = dm0p_unfix_page(r, DM0P_UNFIX,
                       (DMPP_PAGE **)&leafpage, err_code);
                if (s != E_DB_OK)
                    break;

                s = dm0p_fix_page(r, nextpg, fix_action,
                     (DMPP_PAGE **)&leafpage, err_code);
                if (s != E_DB_OK)
                    break;
	    ...
	    if (DM1B_VPT_GET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype, leafpage))
	    {
	    	...
	    	s = dm1b_find_ovflo_pg(r, fix_action, (DMPP_PAGE **)&leafpage, 
		    LAST_OVFL, 
		    DM1B_VPT_GET_PAGE_OVFL_MACRO(t->tcb_rel.relpgtype, leafpage),
		    err_code);
	    	if (s != E_DB_OK)
		    break;
	    }
        }   /* end - if page is a regular leaf and we re-searched index */
        r->rcb_other_page_ptr = (DMPP_PAGE *)leafpage; /* update rcb */
        start = TRUE;
        bid->tid_tid.tid_page = DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype,
	    leafpage);
        bid->tid_tid.tid_line = (DM_TIDEOF);
    ...
    i4         *err_code)
{
    ...
    for (retry = 0; ; retry++)
    {
        if (row_locking(r) && bid->tid_i4 != r->rcb_alloc_bid.tid_i4)
	{
	    TRdisplay("findpage bid %d != alloc_bid %d\n",
	    	bid->tid_i4, r->rcb_alloc_bid.tid_i4);
	    *err_code = E_DM93F5_ROW_LOCK_PROTOCOL;
	    s = E_DB_ERROR;
	    break;
	}

	/* Lock leaf again and reposition */
	if (retry && (r->rcb_alloc_clean_count != DM1B_CC_INVALID))
	{
	    tmp_bid.tid_i4 = bid->tid_i4;
		...
		*buf_locked = TRUE;
	    ...
	    if (retry > 1)
		TRdisplay("dm1b retry %d bid %d,%d->%d,%d split %d->%d lsn %d->%d cc %d->%d \n",
		retry, tmp_bid.tid_tid.tid_page, tmp_bid.tid_tid.tid_line,
		bid->tid_tid.tid_page, bid->tid_tid.tid_line,
		prev_lsn_low,
		DM1B_VPT_GET_SPLIT_LSN_LOW_MACRO(t->tcb_rel.relpgtype,leafpage),
		r->rcb_alloc_lsn.lsn_low,
		DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(t->tcb_rel.relpgtype,leafpage),
		r->rcb_alloc_clean_count,
		DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype, leafpage));


	    /* Adjust page_lsn, page_clean_count */
	    r->rcb_alloc_bid.tid_i4 = bid->tid_i4;
	    DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype,
		leafpage, r->rcb_alloc_lsn);
	    r->rcb_alloc_clean_count = 
	...
	assoc_datapg = DM1B_VPT_GET_BT_DATA_MACRO(t->tcb_rel.relpgtype, leafpage);
	...
	** - Fixing data page may need to wait for a lock
	** - Allocating row may need to lock escalate (wait)
	...
	*/
	if (*buf_locked)
	{
	    prev_lsn_low = DM1B_VPT_GET_SPLIT_LSN_LOW_MACRO(
		t->tcb_rel.relpgtype, leafpage);		
	    dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, leaf);
	    *buf_locked = FALSE;
	}
	...
	if (*data == 0)
	{
	    ...
	    s = dm0p_fix_page(r, assoc_datapg, fix_action, data, err_code);
	    if (s != E_DB_OK)
	    {
		return (s);
	    }
	...
	s = dm1r_allocate(r, *data, record, need, tid, &data_buf_locked, err_code);
    ...
    */
	...
	if (row_locking(r))
	{
	    /* should have leaf locked, data buf unlocked */
	    if (*buf_locked != TRUE || data_buf_locked != FALSE)
	    {
		TRdisplay("dm1b findpage E_DM93F5_ROW_LOCK_PROTOCOL\n");
		*err_code = E_DM93F5_ROW_LOCK_PROTOCOL;
		s = E_DB_ERROR;
		break;
	    }
	...
	*/
	s = dm1p_getfree(r, dm1p_flag, &newdata, err_code);
	if (s != E_DB_OK)
	    break;
	...
	*/
	...
	{
	    s = dm0p_before_image_check(r, (DMPP_PAGE *) leafpage, err_code);
	    if (s != E_DB_OK)
	        break;
	    s = dm0p_before_image_check(r, *data, err_code);
	    if (s != E_DB_OK)
	        break;
	    s = dm0p_before_image_check(r, newdata, err_code);
	    if (s != E_DB_OK)
	        break;
	}
	...
	*/
	loc_id = DM2F_LOCID_MACRO(tbio->tbio_loc_count,
	    (i4) DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leafpage));
	...
	*/
	if (r->rcb_logging & RCB_LOGGING)
	{
		...
		*err_code = E_DM9257_DM1B_FINDDATA;
		s = E_DB_ERROR;
		break;
	    }
	    ...
	    ** changing.
	    */
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&leafpage);
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, data);
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, &newdata);
	    ...
	    */
	    ...
	    if (s != E_DB_OK)
	    {
		dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, 
			(DMPP_PAGE **)&leafpage);
		dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, data);
		dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, &newdata);
		break;
	    }

	    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, leafpage, lsn);
	    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, (*data), lsn);
	    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, newdata, lsn);
	}
	else
	{
	    dm0p_mutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&leafpage);
	...
	*/
	...
	/* Associate new data page with leaf. */
        ...
        DM1B_VPT_SET_BT_DATA_MACRO(t->tcb_rel.relpgtype, leafpage, 
	    DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, newdata)); 
	DM1B_VPT_SET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, leafpage, DMPP_MODIFY);

	/*
	** Unlock the buffers after completing the updates.
	*/
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, (DMPP_PAGE **)&leafpage);
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, data);
	dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, &newdata);
	...
	*/
	...
	if (*data)
	    s = dm0p_unfix_page(r, DM0P_UNFIX, data, &local_err_code);
	if (newdata)
	    s = dm0p_unfix_page(r, DM0P_UNFIX, &newdata, &local_err_code);
	return (E_DB_ERROR);
    }
...
**	page UNFIXED.
**
**	If data == rcb->rcb_data_page_ptr then data must be a copy of
**	the pointer.  It can't be fixed independently.  This code will
**	zero rcb->rcb_data_page_ptr when it equals data, and it will
**	unfix data.
**
** Inputs:
**      rcb                             Pointer to record access context
    ...
    DB_STATUS   *err_code)
{
    ...
    if (row_locking(r))
    {
        status = dm0p_trans_page_lock(r, t,
			...
			(i4)DM0P_NOWAIT, LK_X, &lock_action, (LK_LKID *)0,
                        err_code);
    ...
    */
    ...
    {
	if (next_rcb->rcb_data_page_ptr == *data)
	{
	    status = dm0p_unfix_page(next_rcb, DM0P_UNFIX,
		&next_rcb->rcb_data_page_ptr, err_code);
	    if (status != E_DB_OK)
		return (status);
	}
    }
...
i4		*err_code)
{
    ...
    if (t->tcb_rel.relpgtype != TCB_PG_V1)
    {
	TRdisplay("dm1b_find_ovfl_space called for V2 format table");
	*err_code = E_DM9C16_DM1B_FIND_OVFL_SP;
	return (E_DB_ERROR);
    }
	...
	*/
	...
	/*
	** Fix the 1st overflow page in order to check for space there.
	*/
	...
	*/
	...
	status = dm0p_unfix_page(r, DM0P_UNFIX, &ovfl_page, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
    if (ovfl_page)
    {
	status = dm0p_unfix_page(r, DM0P_UNFIX, &ovfl_page, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
...
i4		*err_code)
{
    DMP_RCB	    *r = rcb;
    DMP_TCB	    *t = rcb->rcb_tcb_ptr;
    DM1B_INDEX	    *leaf = (DM1B_INDEX *)((*caller_page)); 
    ADF_CB	    *adf_cb = (ADF_CB *)r->rcb_adf_cb;
    i4	    	    state = 0;
    i4	    	    child;
    i4   	    compare;
    ...
    if (t->tcb_rel.relpgtype != TCB_PG_V1)
    {
	TRdisplay("newduplicate called for V2 format table");
	*err_code = E_DM9C17_DM1B_NEWDUP;
	return (E_DB_ERROR);
    }
	...
	*/
	...
	{
	    state = E_DB_OK;
	    status = dm1bxdupkey(r, caller_page, KeyPtr, &state, err_code);
	...
	else
	{
	    ...
	    */
	    ...
	    status = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leaf,
			t->tcb_acc_tlv, t->tcb_leafatts, 
			t->tcb_leafattcnt, child, &key_ptr, 
			(DM_TID *)0, (i4*)0,
			&key_len, compression_type, NULL);
    ...
    */
    ...
    {
	ule_format(E_DM9C18_DM1B_NEWDUP_NOKEY, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, 3, 0,
	    DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, leaf),
...

/*{
** Name: dm1b_badtid - Flag bad BTREE tid
**
** Description:
    ...
    char		*key)
{
    ...
    */
    MEmove(r->rcb_tcb_ptr->tcb_klen, (PTR)key, ' ', 12, (PTR)local_1buf);
    MEfill(24, ' ', (PTR)local_2buf);
    for (k = 0; (k < 12) && (k < r->rcb_tcb_ptr->tcb_klen); k++)
    {
    ...
    ule_format(E_DM9351_BTREE_BAD_TID, (CL_SYS_ERR *)0, ULE_LOG, NULL,
    ...
    i4	*err_code)
{
    ...
    if (r->rcb_tcb_ptr->tcb_rel.relpgtype != TCB_PG_V1)
    {
	TRdisplay("dm1b_find_ovflo_pg called for V2 format table");
	return (E_DB_ERROR);
    }
	...
	/* done with initial or current leafpage, unfix it now */
        if ((s = dm0p_unfix_page(r, DM0P_UNFIX, current, err_code)) != E_DB_OK)
	    break;

	/* get the first/next page in dup chain */
        if ((s = dm0p_fix_page(r, nextpg, fix_action, 
                     current, err_code)) 
	    != E_DB_OK)
           	break;
    ...
    i4         *err_code)
{
    ...
    */
    ...
    {
	s = dm0p_fix_page(r, (DM_PAGENO)r->rcb_lowtid.tid_tid.tid_page, 
	    DM0P_LOCKREAD, &r->rcb_other_page_ptr, err_code);
    }
    ...
    */
	...
	if (t->tcb_rel.relpgtype == TCB_PG_V1)
	{
	    count += DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, leaf);
	}
	else
	{
	    count += dm1cxkeycount(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leaf);
	}
	...
	if ((s = dm0p_unfix_page(r, DM0P_UNFIX, (DMPP_PAGE **)&leaf, 
	    err_code)) != E_DB_OK)
	    break;
	...
	*/
	while (nextovfl)       
	{
	    if ((s = dm0p_fix_page(r, (DM_PAGENO)nextovfl, 
		fix_action | DM0P_NOREADAHEAD,
		(DMPP_PAGE**)&leaf, err_code)) != E_DB_OK)
		break;
	...
	s = dm0p_fix_page(r, nextleaf, fix_action, 
                (DMPP_PAGE**)&leaf, err_code);
	if (s != E_DB_OK)
	    break;
    }

    /* this value is stale so ensure it points to the last page used */
    r->rcb_other_page_ptr =  (DMPP_PAGE *)leaf;
...

/*{
** Name: dm1b_reposition - Determines if reposition to previous key needed 
**
** Description:
...
**
**      bid                             Pointer to bid.
**      bid_lsn                         Pointer to lsn for bid
**      bid_clean_count                 Clean count for bid
**      key_ptr                         key to reposition to      
**      tid                             data tid (tidp) to reposition to
**
** Outputs:
...
**      17-dec-96 (stial01)
**          dm1b_reposition() Reposition to next page, 'overflow_page' not init
**          Replaced this variable with assignment to pageno.
**	31-dec-1996 (nanpr01, stial01)
**	    Bug 79925 : We can return from this routine immediately if we 
**	    NEVER were row locking.
**      12-feb-97 (dilma04)
**          Set Lockmode Cleanup:
**          Use macro to check for serializable transaction.
**      07-apr-97 (stial01)
**          dm1b_reposition() NonUnique primary btree (V2), dups can span leaf
**      21-apr-97 (stial01)
**          Search mode is DM1C_FIND for unique keys, DM1C_TID for non-unique
**          keys, since we re-use unique entries in dm1b_allocate, the
**          tidp part of the leaf entry may not match.
**      21-may-97 (stial01)
**          dm1b_reposition: leaf is parameter instead of rcb_other_page_ptr
**          No more LK_PH_PAGE locks, buffer locked with dm0p_lock_buf_macro()
*/          
DB_STATUS
dm1b_reposition(
...
i4        *err_code)
{
    ...
    status = btree_reposition(r, &r->rcb_other_page_ptr, bid,
	    &r->rcb_fet_lsn, r->rcb_fet_clean_count, r->rcb_fet_nextleaf,
	    r->rcb_fet_key_ptr, tid, DM1C_BYTID, &buf_locked, err_code);
	...
	/* Adjust page_lsn, page_clean_count */
	DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype,
		r->rcb_other_page_ptr, r->rcb_fet_lsn);
	r->rcb_fet_clean_count = 
		DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype,
		r->rcb_other_page_ptr);
	r->rcb_fet_nextleaf =
		DM1B_VPT_GET_BT_NEXTPAGE_MACRO(t->tcb_rel.relpgtype,
    ...
    {
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM93F7_DM1B_REPOSITION;
    }
    return (status);
}
...

/*{
** Name: btree_reposition - Determines if reposition to previous key needed 
**
** Description:
**      This routines checks the page lsn and page clean count to determine
**      if the specified position (bid) is still valid
...
**      bid_lsn                         Pointer to lsn for bid
**      bid_clean_count                 Clean count for bid
**      bid_next_leaf                   next leaf change indicates split
**      key_ptr                         key to reposition to      
**      tid                             data tid (tidp) to reposition to
**      buf_locked
**
    ...
    i4     *err_code)
{
    ...
    */
    if (bid_clean_count == DM1B_CC_TIDEOF_LEAF)
    {
    ...
    if (bid_clean_count == DM1B_CC_INVALID)
    {
	if ( (opflag & DM1C_GETNEXT) || (opflag & DM1C_GETPREV))
	{
	    s = btree_position(r, buf_locked, err_code);
	    if (s == E_DB_OK)
    ...
    if (row_locking(r) && *buf_locked == 0)
    {
	TRdisplay("WARNING reposition without buf locked\n");
	dm0p_lock_buf_macro(tbio, r->rcb_lk_id, leaf);
	*buf_locked = TRUE;
    }
	...
	TRdisplay("WARNING reposition with bid page %d leaf page %d\n",
	    bid->tid_tid.tid_page,
	    DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, *leaf));
	*err_code = E_DM93F6_BAD_REPOSITION;
	return (E_DB_ERROR);
    }

    DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, *leaf,
			    page_lsn);
    page_clean_count = DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype,
			    *leaf);

    /* See if leaf page has changed */
    if ((LSN_EQ(bid_lsn, &page_lsn) == TRUE)
	&& (bid_clean_count == page_clean_count))
    {
	return (E_DB_OK);
    }

    /* Reposition to key specified */
    ...
    */
	...
	if (*leaf == NULL)
	{
	    s = dm0p_fix_page(r, pageno, fix_action, leaf, 
                    err_code);
	    if (s != E_DB_OK)
		 break;
	    if (row_locking(r))
	...
	if (bid_nextleaf == 
		DM1B_VPT_GET_BT_NEXTPAGE_MACRO(t->tcb_rel.relpgtype, page))
	{
	    correct_leaf = TRUE;
	}
	else
	{
	    ...
	    split_done = TRUE;
	    s = dm1b_compare_key(r, page, DM1B_LRANGE, key_ptr,  t->tcb_keys, 
		    &compare, err_code);
	    if (s != E_DB_OK)
		break;
	    if (compare < DM1B_SAME)
	    ...
	    s = dm1b_compare_range( r, page, key_ptr, &correct_leaf, err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	*/
	if (correct_leaf == TRUE)
	{
	    ...
	    */
	    s = dm1bxsearch(r, page, mode, DM1C_EXACT,
		    key_ptr, t->tcb_keys, tid, &tidpartno,
		    &pos, NULL, err_code);
	    if (s != E_DB_OK)
	    {
		if (*err_code == E_DM0056_NOPART)
		{
		    ...
		    */
		    s = dm1b_compare_key(r, page, DM1B_RRANGE, key_ptr,
				t->tcb_keys, &compare, err_code);
		    if (s != E_DB_OK)
			break;
	...
	if (*leaf != NULL)
	{
	    dm0p_unfix_page(r, DM0P_UNFIX, leaf, err_code);
	}
	*err_code = E_DM93F6_BAD_REPOSITION;
	return (E_DB_ERROR);
    }
...
i4        *err_code)
{
    ...
    status = btree_position(r, &buf_locked, err_code);
    if (buf_locked == TRUE)
    {
	dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, &r->rcb_other_page_ptr);
    }
    ...
    /* Init clean count for FETCHED row */
    r->rcb_fet_clean_count = DM1B_CC_INVALID;
...
i4        *err_code)
{
    ...
    if (r->rcb_other_page_ptr && r->rcb_p_lowtid.tid_tid.tid_page != 
	DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype, r->rcb_other_page_ptr))
    {
	if (*buf_locked)
	{
	...
	*buf_locked = TRUE;
    ...
    DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, r->rcb_other_page_ptr,
			    page_lsn);
    page_clean_count = DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype,
			    r->rcb_other_page_ptr);

    /* check to see if page has been changed */
    if ((LSN_EQ(&r->rcb_pos_lsn, &page_lsn) == TRUE) 
         && (r->rcb_pos_clean_count == page_clean_count))
    {
	if (DMZ_TBL_MACRO(19))
	{
	...
	*buf_locked = FALSE;
    ...
    status = dm0p_unfix_page(r, DM0P_UNFIX, &r->rcb_other_page_ptr, err_code);
    if (status != E_DB_OK)
	return (E_DB_ERROR);
    ...
    {
	/* Position to bottom */
	status = btree_search(r, (char *)0, (i4)0,
		DM1C_LAST, DM1C_NEXT, &r->rcb_other_page_ptr,
		&r->rcb_lowtid, &r->rcb_currenttid, buf_locked, NULL, err_code);
    }
	...
	status = btree_search(r, (char *)0, (i4)0, DM1C_EXTREME, 
	...
	*/
	...
	if (r->rcb_p_hk_type == RCB_P_EQ)
	{
		...
		status = btree_search(r, r->rcb_hl_ptr, r->rcb_hl_given, 
		    ...
		    &r->rcb_currenttid, buf_locked, NULL, err_code);
		if (status != E_DB_OK)
		    return (status);
		r->rcb_lowtid.tid_tid.tid_line--;
	    }
	...
	/* Adjust page_lsn, page_clean_count */
	DM1B_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype,
		r->rcb_other_page_ptr, r->rcb_pos_lsn);
	r->rcb_pos_clean_count = 
		DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype,
		r->rcb_other_page_ptr);
	r->rcb_pos_nextleaf =
...
i4     *err_code)
{
	...
	if ( s = dm1b_AllocKeyBuf(t->tcb_rngklen, keybuf,
				    &tmpkey, &AllocKbuf, err_code) )
	    return(s);

	tmpkey_len = t->tcb_rngklen;
	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leaf,
		    t->tcb_acc_tlv, t->tcb_rngatts, t->tcb_rngattcnt, lineno,
		    &tmpkey, (DM_TID *)0, (i4*)0,
		    &tmpkey_len, compression_type, NULL);
	...
	tmpkey_len = t->tcb_klen;
	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leaf,
		    t->tcb_acc_tlv, t->tcb_leafatts, t->tcb_leafattcnt, lineno,
		    &tmpkey, (DM_TID *)0, (i4*)0,
		    &tmpkey_len, compression_type, NULL);
...
i4     *err_code)
{
	...
	*/
	...
	if (s != E_DB_OK)
	    break;
	s = dm0p_fix_page(r, nextpageno, leaf_fix_action, leaf, err_code);
	leafpage = (DM1B_INDEX *)((*leaf));
	if (s != E_DB_OK)
	    break;
    }
    ...
    i4        *err_code)
{
    ...
    LG_LSN          data_lsn;
    LG_LSN          leaf_lsn;
    i4              leaf_clean_count, page_clean_count;
    DM_TID          save_bid;
    ...
    */
	...
	s = dm0p_fix_page(r, (DM_PAGENO )tid_to_lock->tid_tid.tid_page,
		fix_action | DM0P_NOWAIT, &r->rcb_data_page_ptr, err_code);

	if (s == E_DB_OK) 
	    return (E_DB_OK);

	if (*err_code != E_DM004C_LOCK_RESOURCE_BUSY)
	    return (E_DB_ERROR);
    ...
    {
	if (!dmxe_xn_active(*low_tran_ptr))
	{
	...
	*/
	...
	if (lock_status == E_DB_OK)
	{
	    ...
	    **
	    ** Note that row locking isn't currently allowed in
	    ** CLUSTERED tables, but I'm hedging my bets...
	    */
	    if ( t->tcb_rel.relstat & TCB_CLUSTERED )
	    {
		i4	k;
	    ...
	    r->rcb_repos_nextleaf =
		    DM1B_VPT_GET_BT_NEXTPAGE_MACRO(t->tcb_rel.relpgtype,
		    r->rcb_other_page_ptr);
	    r->rcb_repos_tid.tid_i4 = r->rcb_currenttid.tid_i4;
	    r->rcb_repos_bid.tid_i4 = r->rcb_lowtid.tid_i4;
	...
	*/
	if (data_buf_locked && *data_buf_locked)
	{
	    data_was_locked = TRUE;
	    DMPP_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, data, data_lsn);
	    dm0p_unlock_buf_macro(tbio, r->rcb_lk_id, &data);
	    *data_buf_locked = FALSE;
	}
    ...
    */
    ...
    page_clean_count = DM1B_VPT_GET_BT_CLEAN_COUNT_MACRO(t->tcb_rel.relpgtype, 
						     r->rcb_other_page_ptr);
    if (LSN_EQ(&leaf_lsn, &page_lsn) && leaf_clean_count == page_clean_count)
    {
	/*
	...
	*/
	if (data_was_locked)
	{
	    dm0p_lock_buf_macro(tbio, r->rcb_lk_id, &r->rcb_data_page_ptr);
	    *data_buf_locked = TRUE;
	    DMPP_VPT_GET_PAGE_LOG_ADDR_MACRO(t->tcb_rel.relpgtype, data, page_lsn);
...
#ifdef xDEBUG
    TRdisplay("REPOSITION after waiting for row lock for table %d tid %d,%d acc %d\n",
	t->tcb_rel.reltid.db_tab_base, tid_to_lock->tid_tid.tid_page,
	tid_to_lock->tid_tid.tid_line, access);
#endif
    ...
    ** Note even if we haven't fetched any keys from this leaf yet 
    ** we cannot assume starting at the top is ok. That is because
    ** a split may have moved keys (that we have already seen) from the
    ** previous leaf to the current leaf.
    ...
    */
	...
	if (r->rcb_lowtid.tid_tid.tid_page !=
	    r->rcb_repos_bid.tid_tid.tid_page)
	{
		...
		*buf_locked = FALSE;
	    ...
	    s = dm0p_unfix_page(r, DM0P_UNFIX,&r->rcb_other_page_ptr, err_code);
	    if (s != E_DB_OK)
		return (E_DB_ERROR);

	    s = dm0p_fix_page(r, (DM_PAGENO)r->rcb_repos_bid.tid_tid.tid_page, 
			fix_action, &r->rcb_other_page_ptr, err_code);
	    if (s != E_DB_OK)
		return (E_DB_ERROR);
	...
	s = btree_reposition(r, &r->rcb_other_page_ptr, &r->rcb_repos_bid, 
		&r->rcb_repos_lsn, r->rcb_repos_clean_count, 
		r->rcb_repos_nextleaf, r->rcb_repos_key_ptr, &r->rcb_repos_tid,
		opflag, buf_locked, err_code);
	r->rcb_lowtid.tid_i4 = r->rcb_repos_bid.tid_i4;
	...
	if (*err_code != E_DM0055_NONEXT)
		...
		*err_code, t->tcb_rel.reltid.db_tab_base);
	return (E_DB_ERROR);
    }
    else
	return (E_DB_WARN);
    ...
    i4			*err_code)
{
    ...
    DM_TID		*tid = &r->rcb_currenttid;
    DMPP_PAGE		*data, *leaf;   /* Pointers to data/leaf pages. */
    DM1B_INDEX		*leafpage;
    i4			fix_action;     /* Type of access to page. */
    char		*rec_ptr = record;
    DM_TID		unlock_tid;
...
tid = (%d,%d) rnl_btree_flags = 0x%x\n",
	...
	leafpage = ((DM1B_INDEX *)leaf);
	if (leaf)
	    assoc = DM1B_VPT_GET_BT_DATA_MACRO(t->tcb_rel.relpgtype, leafpage);

	if (rnl->rnl_btree_flags & RNL_NEED_LEAF)
	{
	    if (leaf && (rnl->rnl_btree_flags & RNL_NEXT))
	    {
		...
		if ((t->tcb_dcb_ptr->dcb_status & DCB_S_ROLLFORWARD) &&
		    ...
		    (s == E_DB_INFO))
		{
		    if (*err_code == E_DM9CB2_NO_RNL_LSN_ENTRY)
		    {
			...
			*err_code = 0;
		    ...
		    if (*err_code == E_DM9CB1_RNL_LSN_MISMATCH)
		    {
			DB_STATUS	locstat = E_DB_OK;
			locstat = dm0p_unfix_page(r, DM0P_UNFIX, &r->rcb_other_page_ptr,
						err_code);
			if (locstat)
			    s = locstat;
		    }
...
needs to be read rnl_btree_flags=0x%x\n",
		...
		s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
				leafpage, t->tcb_acc_tlv, t->tcb_leafatts, 
				t->tcb_leafattcnt, bid->tid_tid.tid_line, 
				&key, tid, &r->rcb_partno,
				&key_len, DM1B_INDEX_COMPRESSION(r), NULL);
...
tid = (%d,%d) rnl_btree_flags = %x\n",
	...
	*/ 
        ...
        ** before we lock the new page/row
        */
	if (r->rcb_data_page_ptr && (tid->tid_tid.tid_page !=
			DMPP_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype,
			r->rcb_data_page_ptr)))
	{
	    if (DMZ_SRT_MACRO(13))
	    ...
	    s = dm0p_unfix_page(r, DM0P_UNFIX, &r->rcb_data_page_ptr, err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	if (r->rcb_data_page_ptr == NULL)
	{
		...
		{
		    DB_STATUS       locstat = E_DB_OK;
		    locstat = dm0p_unfix_page(r, DM0P_UNFIX, &r->rcb_data_page_ptr,
						err_code);
		    if (locstat)
			s = locstat;
		}
	...
	*/
	...
	if (s == E_DB_OK && 
	    ...
	    DMPP_VPT_PAGE_HAS_SEGMENTS(t->tcb_rel.relpgtype)))
	{
	    s = dm1c_get(r, data, tid, record, err_code);
	    if (s && *err_code != E_DM938B_INCONSISTENT_ROW)
		break;
	    rec_ptr = record;
	}
...
**
** Outputs:
**	leaf				Non-null if leaf still fixed.
    ...
    i4     	*err_code)
{
	...
	*leaf = (DMPP_PAGE*)NULL;
    ...
    if ( DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, leafpage) & DMPP_LEAF &&
	...
	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, leafpage,
		    ...
		    DM1B_INDEX_COMPRESSION(r), (u_i4*)NULL);
    ...
    if ( DM1B_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, leafpage) & DMPP_LEAF )
    {
	s = dm1bxsearch(r, leafpage, DM1C_TID, DM1C_EXACT, ClusterKey,
			t->tcb_keys, TrueTid, &tidpartno, &pos, record, err_code);

	if ( s == E_DB_OK || *err_code != E_DM0056_NOPART )
	    return(s);
    }

    /* btree_search will unfix leaf */
    if ( *leaf == r->rcb_other_page_ptr )
	r->rcb_other_page_ptr = (DMPP_PAGE*)NULL;
    ...
    return( btree_search(r, ClusterKey, t->tcb_keys, 
		    DM1C_FIND, DM1C_EXACT,
		    leaf, TrueTid, 
		    &localtid, &buf_locked, record, err_code) );
}

/*{
...
i4		*err_code)
{
	...
	*err_code = E_DM9425_MEMORY_ALLOCATE;
	ule_format(*err_code, (CL_ERR_DESC*)NULL, ULE_LOG, NULL,
