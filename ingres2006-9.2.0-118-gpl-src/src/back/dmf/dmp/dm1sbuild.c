...
i4         *err_code)
{
    ...
    if (m->mct_rebuild & MCT_KEEPDATA)
    {
	do
	{
	    status = dm1xstart(m, DM1X_KEEPDATA , err_code);
	    if (status != E_DB_OK)
		break;

	    status = dm1xnextpage(mct, &page, err_code);
	    if ( status != E_DB_OK )
		break;
	    ...
	    */
	    ...
	    status = dm1xnewpage(m, 0, &m->mct_curdata, err_code);
	    if ( status != E_DB_OK )
		break;
	...
	(VOID) dm1xfinish(m, DM1X_CLEANUP, &local_err_code);
	log_error(E_DM924B_DM1S_BBEGIN, err_code);
	return (status);
    }
...
i4         *err_code)
{
    ...
    if (DMPP_VPT_PAGE_HAS_SEGMENTS(mct->mct_page_type) && mct->mct_relwid > max)
    {
	i4	nextpage;
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    log_error(E_DM924C_DM1S_BEND, err_code);
	    return (status);
	}
    ...
    status = dm1xfinish(m, DM1X_COMPLETE, err_code);
    if ( status == E_DB_OK )
    {
	return(E_DB_OK );
    }
...
i4         *err_code)
{
    ...
    */
    ...
    while (dm1xbput(mct, mct->mct_curdata, rec, rec_size, mct->mct_compressed,
            0, 0, mct->mct_ver_number, err_code) == E_DB_WARN)
    {
	/*
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    log_error(E_DM924D_DM1S_BPUT, err_code);
	    return (status);
	}
