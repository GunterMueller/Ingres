...
/*
**
**  Name:  DM1CCMPH.C - Normal tuple-level format high compression routines
**
**  Description:
...
**
**	dm1cn_plv - page level vector for normal format
**
**	dm1ch_tlv - tuple level vector for normal format
**
**	The normal (dm1ch_) functions defined in this file are:
**
**	compress - compress a record
**	uncompress - uncompress a record
**	compexpand - how much can a tuple expand when compressed
**
**     N.B. BTREE index compression uses DM1CCMPN routines
**
...

/*
**  Name: dm1ch_tlv - Normal format tuple level vector of accessors
**
**  Description:
**	This is the accessor structure that dm1c_getaccessors will use when
**	passed parameters asking for standard accessors with the
...

{
	...
	if (*pIn++ == (char)FALSE)
	{
    	...
    	/* then copy the data to the output area */
    	    ...
    	    status = dm1r_cvt_row(atts_array, atts_count, pIn, pOut,
...
**
** Algorithm:
**	If the size expands past the source size, then the source is used.
**	Allot an extra byte for the compressed indicator (first byte) and
**	one for each nullable field.
**
**
** Inputs:
...
dm1ch_compexpand(
			...
			i4		att_cnt)
{
	i4        expansion = 1;  /* allow for compression flag */
