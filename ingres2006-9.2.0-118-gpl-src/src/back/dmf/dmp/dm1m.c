    ...
    i4     *err_code)
{
    ...
    */
	...
	atts_array = t->tcb_data_atts;
	atts_count = t->tcb_rel.relatts;
    }
    else
    {
	char            *pos = key;
    ...
    */
	...
	s = dm1m_check_range(pgtype, pgsize, leafpage, adf_cb, 
			t->tcb_acc_klv, key, &in_range, err_code);
	if (s || ! in_range)
	    break;
	...
	*/
	...
	if (dm1cxhas_room(pgtype, pgsize, leafpage, compression_type,
			    (i4)100 /* fill factor */, 
	...
	*/
	s = dm1mxsearch(r, leafpage, DM1C_OPTIM, DM1C_EXACT, key, adf_cb, 
			t->tcb_acc_klv,
			pgtype, pgsize, &localtid, &lineno, err_code);
    ...
    */
    ...
    {
        s = findpage(r, *leaf, data, record, need, tid, err_code);
	...
	*/
	...
	else
	{
	    ...
	    _VOID_ dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
			    ...
			    atts_array, atts_count, bid->tid_tid.tid_line,
    ...
    i4	*err_code)
{
	...
	*/
	    ...
	    r->rcb_other_page_ptr) != localbid.tid_tid.tid_page))
	{
		...
		(DM0P_WRITE | DM0P_NOREADAHEAD), &r->rcb_other_page_ptr,
		err_code);
	    if (s != E_DB_OK)
		break;
	}
	...
	*/
	if (base_table)
	{
	    ...
	    */
	    s = dm1r_delete(r, &localtid, (i4)0, (i4)DM1C_MDELETE, 
			err_code);
	    if (s != E_DB_OK)
		break;
	...
	*/
	s = dm1mxdelete(r, leafpage, localbid.tid_tid.tid_line,
			LOG_REQUIRED, MUTEX_REQUIRED, err_code);
	if (s != E_DB_OK)
	    break;
	...
	*/
	s = dm1m_rcb_update(r, &localbid, (DM_TID *)0, (i4)0,
			 DM1C_MDELETE, err_code);
	...
	*/
	s = adjust_mbrs(r, leafpage, err_code);
	if (s != E_DB_OK)
	    break;
	...
	else
	{
		...
		*/
		...
		local_status = dm1cxget(t->tcb_rel.relpgtype, 
			    ...
			    t->tcb_acc_tlv, t->tcb_key_atts, t->tcb_keys,
			    r->rcb_lowtid.tid_tid.tid_line,
			    &key_ptr, (DM_TID *)0, (i4*)0,
			    &key_len,
			    DM1B_INDEX_COMPRESSION(r), NULL);
    ...
    i4	    *err_code)
{
	...
	*/
	s = dm1m_rcb_update(r, bid, (DM_TID *)0, (i4)0,
		    DM1C_MPUT, err_code);
	if (s != E_DB_OK)
	    break;
    ...
    i4         *err_code)
{
	...
	*/
	if ( ! inplace_update)
	{
	    ...
	    s = dm1mxdelete(r, leaf, bid->tid_tid.tid_line,
			LOG_REQUIRED, MUTEX_REQUIRED, err_code);
	    if (s != E_DB_OK)
		break;
	    ...
	    */
	    if (leaf != newleaf)
	    {
	    	s = adjust_mbrs(r, newleaf, err_code);
	    	if (s != E_DB_OK)
    	    	    break;
	    }
	}
	...
	*/
	s = dm1m_rcb_update(r, bid, newbid, (i4)0, DM1C_MREPLACE,err_code);
	if (s != E_DB_OK)
    	    break;
    ...
    *err_code = E_DB_OK;
	...
	do
	{
	    ...
	    */
		...
		(next_rcb->rcb_other_page_ptr != NULL) &&
		(next_rcb->rcb_other_page_ptr == next_rcb->rcb_1rnl_page ||
		 next_rcb->rcb_other_page_ptr == next_rcb->rcb_2rnl_page) &&
		(DMPP_VPT_GET_PAGE_PAGE_MACRO(r->rcb_tcb_ptr->tcb_rel.relpgtype,
		next_rcb->rcb_other_page_ptr) == localbid.tid_tid.tid_page))
	    {
		 continue;
	    }
    ...
    i4            *err_code)
{
    ...
    i4		 pgsize = t->tcb_rel.relpgsize;
    i4           pgtype = t->tcb_rel.relpgtype;
    DMPP_PAGE	 *parent, *current;     /* Pointers to pages. */
    DM1B_INDEX	 *parpage, *curpage, *leafpage; /* Pointers to index pages. */
...
# endif
	...
	if (mode == DM1C_RANGE)
	{
	    ...
	    s = find_first(r, &current, bid, tid, err_code);
	    if (s != E_DB_OK)
		break;
	    (*leaf) = current;     
	    return E_DB_OK;
	...
	*/
	...
	do
	{
	    s = dm1mxsearch(r, parpage, xmode, direction, key, adf_cb, 
	    		    t->tcb_acc_klv,
			    pgtype, pgsize, &child, &pos, err_code);
	    ...
	    */
	    at_sprig_level =
		(DM1B_VPT_GET_PAGE_STAT_MACRO(pgtype, parpage) & DMPP_SPRIG) != 0;
	    ...
	    */
	    if (at_sprig_level)
	    {
		if ((DM1B_VPT_GET_PAGE_STAT_MACRO(pgtype, curpage) &DMPP_LEAF) == 0)
		{
	    ...
	    else if ((DM1B_VPT_GET_PAGE_STAT_MACRO(pgtype, curpage) &
	    ...
	    */
	    s = requalify_page(r, &current, key, mode, direction, err_code);
	    if (s)
	    	break;
	    /*
	    ...
	    */
	    ...
	    parpage = ((DM1B_INDEX *)parent);
	    current = NULL;
	} while (!(DM1B_VPT_GET_PAGE_STAT_MACRO(pgtype, parpage) & DMPP_LEAF));
	...
	if (direction == DM1C_EXACT)
	{
		...
		DM_PAGENO	nextpageno;
	    	...
	    	s = dm1mxsearch(r, leafpage, mode, direction, key, adf_cb, 
				t->tcb_acc_klv,
				pgtype, pgsize, tid, &pos, err_code);
	    	if ((mode != DM1C_TID) || 	/* Not looking for exact TID */
		    (s == E_DB_OK) || 		     /* or Found exact match */
		    (*err_code != E_DM0056_NOPART))   /* or Something rotten */
		    break;
		/*
...
# define 	MAX_LOCKS 	RCB_MAX_RTREE_LEVEL * (MAX_SPLIT_POLICY + 1)
		...
		s = handle_overflow(r, &locked, sibling, &numsiblings, &newsib,
				    key, splitkey, &lsn, err_code);
		if (s != E_DB_OK)
		    break;

		s = adjust_tree(r, &locked, sibling, numsiblings, newsib, key,
				splitkey, &lsn, err_code);
		{
		    DB_STATUS	local_s =
			unlock_tree(r, locks, sibling[0], err_code);
		    if (s == E_DB_OK)
		    {
		    	s = local_s;
		    }
	...
	if (mode == DM1C_FIND )
	{
	    ...
	    s = dm1cxget(pgtype, pgsize, leafpage, 
			t->tcb_acc_tlv, atts_array, atts_count, pos, 
	    		&entry_ptr, (DM_TID *)0, (i4*)0,
			&entry_len, DM1B_INDEX_COMPRESSION(r), NULL);
	    if (t->tcb_rel.relpgtype != TCB_PG_V1 
	    		    && s == E_DB_WARN)
	    {
	        s = E_DB_OK;
    ...
    i4     *err_code)
{
	...
	*err_code = E_DM002A_BAD_PARAMETER;
    ...
    s = find_next_leaf(r, current, leaf_bid, leaf_tid, err_code);
    ...
    if (s != E_DB_OK)
    {
        ...
        else if (*err_code > E_DM_INTERNAL)
    	{
	    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	    *err_code = E_DM9264_DM1B_SEARCH;
    	}
...
**      entry and exit in rcb->rcb_other_page_ptr. The direction to search
    ...
    i4            *err_code)
{
    ...
    if (!(DMPP_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, r->rcb_other_page_ptr)
	  & DMPP_LEAF)
    || (r->rcb_lowtid.tid_tid.tid_page !=
	...
	*err_code = E_DM002A_BAD_PARAMETER;
    ...
    s = find_next_leaf(r, &r->rcb_other_page_ptr, &r->rcb_lowtid, 
    ...
    i4         *err_code)
{
    ...
    /*
    ** Set up attributes array and count, depending on whether
    ** this is a secondary index or not
    */
    if (t->tcb_rel.relstat & TCB_INDEX)
    {
    ...
    searchlen = atts_array[0]->length; /* NBR length */
	...
	*/
		...
		DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, page))
	{
	    ...
	    */
	    s = pushstack(r, leaf_tid->tid_tid.tid_page, page_ptr, err_code);
	    if (s != E_DB_OK)
	    	return s;
	...
	*/
	...
	s = popstack(r, page_ptr, err_code);
	if (s != E_DB_OK)
	{
	    return s;
	}
	...
	while (leaf_tid->tid_tid.tid_page != savepageno)
	{
		...
		keylen = searchlen;
	    	...
	    	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, page, 
			    ...
			    atts_array, atts_count, bid->tid_tid.tid_line,
			    &key, leaf_tid, (i4*)NULL,
			    &keylen, DM1B_INDEX_COMPRESSION(r), NULL);
	    	if (s != E_DB_OK)
	    	{
		    dm1cxlog_error(E_DM93E3_BAD_INDEX_GET, r, page,
		    		  t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
    ...
    i4         *err_code)
{
    ...
    s = dm0p_unfix_page(r, DM0P_RELEASE, page_ptr, err_code);
    if (s != E_DB_OK)
       	return s;
	...
	s = dm0p_fix_page(r, pageno, fix_type, page_ptr, err_code);
    }
    else
    {
        s = E_DB_FATAL;
    ...
    i4         *err_code)
{
    ...
    if (savepageno)
    {
	s = dm0p_fix_page(r, savepageno, fix_type, page_ptr, err_code);
    }
    else
    {
        s = E_DB_ERROR;
    	...
    	i4		*err_code)
{
    ...
    */
	...
	status =  dm0p_fix_page(r, nextpage,
    ...
    */
    ...
    if (avgkids + 1 >
    	 DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, ((DM1B_INDEX *)*sibling)))
	hasroom = FALSE;
    ...
    */
    status = dm1mxdistribute(r, sibling, *numsiblings, kids, lsn, err_code);
    if (status != E_DB_OK)
        goto ErrorExit;
    /*
...
ErrorExit:
	...
	for (sib = sibling; sib < sibling + *numsiblings; sib++)
	    dm0p_unmutex(tbio, (i4)0, r->rcb_lk_id, sib);
	pages_mutexed = FALSE;
    }
    /*
    ...
    */
    ...
    {
	status = dm0p_unfix_page(r, unfix_type, sib, err_code);
	if (status != E_DB_OK)
	    break;
    }
	...
	i4		*err_code)
{
	DMP_TCB		*t = r->rcb_tcb_ptr;	/* Table control block. */
    	...
    	DB_ATTS	    	**atts_array;
    	i4	    	atts_count;
    	DM1B_INDEX	*curpage;		/* Page pointer */
    	...
    	i4		*err_code)
{
	...
	DM1B_INDEX	*sibpage;		/* Sibling page */
	DMPP_PAGE	**target = sibling;	/* Ptr to target page of key */
	DM1B_INDEX	*targpage;		/* Target (leaf) page of key */
    	DMPP_PAGE	*parent[MAX_SPLIT_POLICY + 1];
	    					/* Set of parent pages */
	DM1B_INDEX	*parpage;		/* Parent page */
	...
	*/
	...
	s = dm0p_fix_page(r, tidp->tid_tid.tid_page, DM0P_WPHYS, parent,
			  err_code);
	if (s)
	    break;
	s = requalify_parent(r, parent, sibpage, &loc_ancestor_level, err_code);
	if (s)
	    break;
	tidp = &r->rcb_ancestors[loc_ancestor_level];
	*(*locked)++ = *parent;
	parptr = parent;
	parpage = ((DM1B_INDEX *) *parptr);
	numparents = 1;
	/*
	     ...
	     sibptr < sibling + numsiblings; ++sibptr)
	{
			...
			DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, parpage))
		{
		    	...
		    	*err_code = E_DM0055_NONEXT;
	    	    ...
	    	    s = dm0p_fix_page(r, next, DM0P_WPHYS, ++parptr, err_code);
	    	    if (s)
		    	break;
	    	    ++numparents;
	    	    *(*locked)++ = *parptr;
	...
	if (newsib)
	{
		...
		*/
		...
		s = handle_overflow(r, locked, parptr, &numparents, &newparent,
				    NULL, parsplitkey, &parlsn, err_code);
		numparents += (parptr - parent);
		if (s)
		    break;
	    } /* if (! dm1cxhas_room...) */
	...
	*/
	...
	for (sibptr = sibling; sibptr < sibling + numsiblings; ++sibptr)
	{
		...
		*/
		...
		klen = parkeylen;
	    	s = dm1cxget(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
			    parpage, t->tcb_acc_tlv, atts_array, atts_count, 
		    	    DM1B_VPT_GET_BT_KIDS_MACRO(t->tcb_rel.relpgtype, 
			     			    parpage) - 1,
			    &key_ptr, (DM_TID *)NULL, (i4*)0,
			    &klen,
			    DM1B_INDEX_COMPRESSION(r), NULL);
	    	if (s != E_DB_OK)
	    	{
					...
					t->tcb_hilbertsize))
		{
		    ...
		    s = dm1mxadjust(r, t, parpage, t->tcb_rel.relpgsize,
				    ...
				    NO_LOG_REQUIRED, MUTEX_REQUIRED, err_code);
		    if (s)
		    	break;
		}
	    	parpage = ((DM1B_INDEX *) *parptr);
    ...
    */
    if (newsib)
      for (;;){
	sibpage = ((DM1B_INDEX *) newsib);
        s = dm1mxnewroot(r, sibling, err_code);
        if (s)
    	    break;
	/*
	...
	*/
	...
	tid.tid_tid.tid_page = DM1B_VPT_GET_PAGE_PAGE_MACRO(t->tcb_rel.relpgtype,
	    			 		        sibpage);
	parpage = ((DM1B_INDEX *) *sibling);
	s = dm1mxinsert(r, t, t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
	        	atts_array, atts_count, DM1B_INDEX_COMPRESSION(r),
    	...
    	i4		*err_code)
{
	...
	*/
	...
	s = dm1mxadjust(r, t, childpage, t->tcb_rel.relpgsize, klen, rangekey,
			DM1B_RRANGE, NO_LOG_REQUIRED, MUTEX_REQUIRED, err_code);
	if (s)
	    break;
    	/*
	...
	*/
	if (!(DMPP_VPT_GET_PAGE_STAT_MACRO(t->tcb_rel.relpgtype, childpage)
	      & DMPP_LEAF))
	{
	    s = dm0p_unfix_page(r, DM0P_RELEASE, ((DMPP_PAGE **)&childpage),
	    			&local_err_code);
	    if (s)
	    	ule_format(local_err_code, 0, ULE_LOG, NULL, 0, 0, 0,
		    	   &local_err_code, 0);
	...
	*/
	s = dm1mxadjust(r, t, parpage, t->tcb_rel.relpgsize, klen, rangekey,
			pos, NO_LOG_REQUIRED, MUTEX_REQUIRED, err_code);
	if (s)
	    break;
	/*
    	...
    	i4		*err_code)
{
	...
	*/
	...
	for (;;)
	{
	    ...
	    s = dm0p_unfix_page(r, unfix_type, page, err_code);
	    if (s)
	        break;
	    tidp->tid_tid.tid_page = nextpageno;
	    tidp->tid_tid.tid_line = (DM_TIDEOF);
	    s = dm0p_fix_page(r, tidp->tid_tid.tid_page, fix_type,
	        	      page, err_code);
	    if (s)
	        break;
	    curpage = ((DM1B_INDEX *) *page);
	...
	*/
	...
	for (;;)
	{
	    ...
	    */
	    ...
	    s = dm0p_fix_page(r, tidp->tid_tid.tid_page, DM0P_WPHYS,
	        	      parent, err_code);
	    if (s)
	        return s;
	    parpage = ((DM1B_INDEX *) *parent);
	...
	*pages_mutexed = TRUE;
	...
	for (page = sibling; page < sibling + numsiblings; page++)
	{
	    ...
	    s = dm0l_ai(r->rcb_log_id, dm0l_flag,
		    ...
		    t->tcb_rel.relpgsize, (DMPP_PAGE *) curpage,
		    (LG_LSN *) 0, &lsn, err_code);
	    if (s != E_DB_OK)
	    	break;
    ...
    i4         *err_code)
{
    ...
    return (dm1r_allocate(r, *data, record, need, tid, &buf_locked, err_code));
} /* findpage( */
    ...
    i4         *err_code)
{
    ...
    */
	...
	s = dm0p_fix_page(r, nextleaf, DM0P_READ, (DMPP_PAGE**)&leaf, err_code);
	if (s != E_DB_OK)
	    break;
    }
	...
	i4 indexpagetype)
{
    ...
    */
    key_len -= DM1B_TID_S;
    return (dm1cxkperpage(page_type, page_size, relcmptlvl, TCB_RTREE, 0, key_len, indexpagetype, sizeof(DM_TID), 0));
}
