...
static DB_STATUS    add_to_overflow(
				    ...
				    char		*key,
				    i4		*err_code);
static VOID	    log_error(i4 status, i4 *err_code);

/*{
...
i4         *err_code)
{
    ...
    /*
    ** Ensure that index compression information is present
    */
    if (mct->mct_index_atts == (DB_ATTS **)0 || mct->mct_index_attcount == 0)
    {
	ule_format(E_DM93C3_BBBEGIN_BADPARAM,
		    (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		    (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM93C1_BBBEGIN_ERROR;
	return (E_DB_ERROR);
    }
    ...
    */
    ...
    status = dm1xstart(mct, DM1X_SAVE, err_code);
    ...
    ** not in INDEX pages.
    */
    mct->mct_cx_atts[MCT_PLEAF] = mct->mct_index_atts;
    mct->mct_cx_attcnt[MCT_PLEAF] = mct->mct_index_attcount;
    mct->mct_cx_keys[MCT_PLEAF] = mct->mct_key_atts;
    mct->mct_cx_klen[MCT_PLEAF] = mct->mct_klen;
    mct->mct_cx_maxentry[MCT_PLEAF] = mct->mct_index_maxlen + mct->mct_tidsize;
    /* Index pages never have TID8s */
    mct->mct_cx_maxentry[MCT_PINDEX] = mct->mct_index_maxlen + sizeof(DM_TID);
    ...
    */
    if ( mct->mct_index && mct->mct_keys != mct->mct_index_attcount)
    {
	atts = mct->mct_cx_keys[MCT_PLEAF];
	klen = 0;
	...
	mct->mct_cx_atts[MCT_PINDEX] = &mct->mct_cx_ix_keys[0];
	mct->mct_cx_attcnt[MCT_PINDEX] = j;
	mct->mct_cx_keys[MCT_PINDEX] = &mct->mct_cx_ix_keys[0];
	mct->mct_cx_klen[MCT_PINDEX] = klen;
	mct->mct_cx_maxentry[MCT_PINDEX] = (*mct->mct_acc_tlv->dmppxcompexpand)
		(mct->mct_cx_atts[MCT_PINDEX], mct->mct_cx_attcnt[MCT_PINDEX])
		+ mct->mct_cx_klen[MCT_PINDEX]
		+ sizeof(DM_TID); /* TID8's only on LEAF page */
    }
    else
    {
	mct->mct_cx_atts[MCT_PINDEX] = mct->mct_cx_atts[MCT_PLEAF];
	mct->mct_cx_attcnt[MCT_PINDEX] = mct->mct_cx_attcnt[MCT_PLEAF];
	mct->mct_cx_keys[MCT_PINDEX] = mct->mct_cx_keys[MCT_PLEAF]; 
	mct->mct_cx_klen[MCT_PINDEX] = mct->mct_cx_klen[MCT_PLEAF];
	mct->mct_cx_maxentry[MCT_PINDEX] = mct->mct_cx_maxentry[MCT_PLEAF];
    }
    ...
    */
    mct->mct_cx_kperpage[MCT_PINDEX] = 
		dm1b_kperpage(mct->mct_page_type, mct->mct_page_size,
				DMF_T_VERSION, mct->mct_cx_attcnt[MCT_PINDEX], 
				mct->mct_cx_klen[MCT_PINDEX], DM1B_PINDEX,
				sizeof(DM_TID), 0);
    mct->mct_cx_kperpage[MCT_PLEAF] = 
		dm1b_kperpage(mct->mct_page_type, mct->mct_page_size,
				DMF_T_VERSION, mct->mct_cx_attcnt[MCT_PLEAF], 
				mct->mct_cx_klen[MCT_PLEAF], DM1B_PLEAF,
				mct->mct_tidsize, 0);
	...
	TRdisplay(
	    ...
	    mct->mct_cx_attcnt[MCT_PLEAF], mct->mct_cx_klen[MCT_PLEAF],
	    mct->mct_cx_maxentry[MCT_PLEAF], mct->mct_cx_kperpage[MCT_PLEAF],
	    mct->mct_l_fill, mct->mct_cx_kperpage[MCT_PLEAF] 
				* mct->mct_l_fill / 100,
	    mct->mct_tidsize);

	TRdisplay(
	    "Index attcnt=%d klen=%d maxent=%d kperpage %d fill=%d(%d)\n",
	    mct->mct_cx_attcnt[MCT_PINDEX], mct->mct_cx_klen[MCT_PINDEX],
	    mct->mct_cx_maxentry[MCT_PINDEX], mct->mct_cx_kperpage[MCT_PINDEX],
	    mct->mct_i_fill, mct->mct_cx_kperpage[MCT_PINDEX] 
				* mct->mct_i_fill / 100);
    }
	...
	TRdisplay("dm1bbuild: LEAF atts %d entrylen %d \n", 
		mct->mct_cx_attcnt[MCT_PLEAF], mct->mct_cx_klen[MCT_PLEAF]);
	for (i = 0; i < mct->mct_cx_attcnt[MCT_PLEAF]; i++)
	    TRdisplay("ATT %t (%d %d)\n", sizeof(DB_ATT_NAME),
	    &mct->mct_cx_atts[MCT_PLEAF][i]->name.db_att_name,
	    mct->mct_cx_atts[MCT_PLEAF][i]->offset, 
	    mct->mct_cx_atts[MCT_PLEAF][i]->key_offset);
	for (i = 0; i < mct->mct_keys; i++)
	    TRdisplay("KEY %t (%d %d)\n", sizeof(DB_ATT_NAME),
	    &mct->mct_cx_keys[MCT_PLEAF][i]->name.db_att_name,
	    mct->mct_cx_keys[MCT_PLEAF][i]->offset, 
	    mct->mct_cx_keys[MCT_PLEAF][i]->key_offset);

	TRdisplay("dm1bbuild: INDEX atts %d entrylen %d \n", 
		mct->mct_cx_attcnt[MCT_PINDEX], mct->mct_cx_klen[MCT_PINDEX]);
	for (i = 0; i < mct->mct_cx_attcnt[MCT_PINDEX]; i++)
	    TRdisplay("ATT %t (%d %d)\n", sizeof(DB_ATT_NAME),
	    &mct->mct_cx_atts[MCT_PINDEX][i]->name.db_att_name,
	    mct->mct_cx_atts[MCT_PINDEX][i]->offset, 
	    mct->mct_cx_atts[MCT_PINDEX][i]->key_offset);
	for (i = 0; i < mct->mct_keys; i++)
	    TRdisplay("KEY %t (%d %d)\n", sizeof(DB_ATT_NAME),
	    &mct->mct_cx_keys[MCT_PINDEX][i]->name.db_att_name,
	    mct->mct_cx_keys[MCT_PINDEX][i]->offset, 
	    mct->mct_cx_keys[MCT_PINDEX][i]->key_offset);
    }
    ...
    */
    ...
    if (status == E_DB_OK)
	status = dm1xnewpage(mct, 0, (DMPP_PAGE **)&mct->mct_curleaf, err_code);
    if (!mct->mct_index)
	status = dm1xnewpage(mct, 0, &mct->mct_curdata, err_code);
    if (status != E_DB_OK)
    {
	(VOID) dm1xfinish(mct, DM1X_CLEANUP, &local_err_code);
	log_error(E_DM925A_DM1B_BEGIN, err_code);
	return (status);
    }
    ...
    status = dm1bxformat(mct->mct_page_type, mct->mct_page_size,
	    ...
	    mct->mct_cx_kperpage[MCT_PINDEX],
	    mct->mct_cx_klen[MCT_PINDEX], 
	    mct->mct_cx_atts[MCT_PINDEX],
	    mct->mct_cx_attcnt[MCT_PINDEX],
	    mct->mct_cx_keys[MCT_PINDEX],
	    mct->mct_keys,
	    DM1B_VPT_GET_PAGE_PAGE_MACRO(mct->mct_page_type, mct->mct_curindex),
	    DM1B_PSPRIG,
	    mct->mct_index_comp, mct->mct_acc_plv, 
	    sizeof(DM_TID), 0, err_code);
    if (status != E_DB_OK)
    {
	(VOID) dm1xfinish(mct, DM1X_CLEANUP, &local_err_code);
	log_error(E_DM925A_DM1B_BEGIN, err_code);
    ...
    status = dm1cxallocate(mct->mct_page_type, mct->mct_page_size, 
			...
			(DB_TRAN_ID *)0, (i4)0, 0, (i4)sizeof(DM_TID));
    if (status != E_DB_OK)
    {
    ...
    status = dm1bxformat(mct->mct_page_type, mct->mct_page_size,
	    ...
	    mct->mct_cx_kperpage[MCT_PLEAF],
	    mct->mct_cx_klen[MCT_PLEAF], 
	    mct->mct_cx_atts[MCT_PLEAF],
	    mct->mct_cx_attcnt[MCT_PLEAF],
	    mct->mct_cx_keys[MCT_PLEAF],
	    mct->mct_keys,
	    DM1B_VPT_GET_PAGE_PAGE_MACRO(mct->mct_page_type, mct->mct_curleaf),
	    DM1B_PLEAF,
	    mct->mct_index_comp, mct->mct_acc_plv,
	    mct->mct_tidsize, 0, err_code);
    if (status != E_DB_OK)
    {
	(VOID) dm1xfinish(mct, DM1X_CLEANUP, &local_err_code);
	log_error(E_DM925A_DM1B_BEGIN, err_code);
...
i4         *err_code)
{
    ...
    */
    ...
    else
    {
		...
		(DMPP_PAGE **)&mct->mct_curleaf, err_code);
	    if (status != E_DB_OK)
		break;
    ...
    */
    s = dm1xfinish(mct, DM1X_COMPLETE, &local_err_code);
    if (s != E_DB_OK)
    {
        i4 ignore;

        status = s, *err_code = local_err_code;
        (VOID) dm1xfinish(mct, DM1X_CLEANUP, &ignore);
    }


    if (status != E_DB_OK)
	log_error(E_DM9259_DM1B_BEND, err_code);
...
i4         *err_code)
{
    ...
    */
	...
	if (mct->mct_compressed != TCB_C_NONE)
	{
	    rec = mct->mct_crecord;
	    status = (*mct->mct_acc_tlv->dmpp_compress)(mct->mct_data_atts,
				mct->mct_relatts, record, record_size,
				rec, &rec_size);
	    if (status != E_DB_OK)
	    {
    ...
    ** less than or equal to mct->mct_cx_kperpage[MCT_PLEAF] and by the type
	...
	*/
	...
	if ( ! mct->mct_index)
	{
	    ...
	    while (dm1xbput(mct, mct->mct_curdata, rec, rec_size,
			mct->mct_compressed, mct->mct_db_fill, &tid,
			mct->mct_ver_number, err_code) == E_DB_WARN)
	    {
		/* 
		...
		*/
		...
		if (status != E_DB_OK)
		{
		    log_error(E_DM925B_DM1B_BPUT, err_code);
		    return (status);
		}
		DMPP_VPT_INIT_PAGE_STAT_MACRO(mct->mct_page_type, mct->mct_curdata,
		    DMPP_DATA | DMPP_ASSOC | DMPP_PRIM );
	...
	status = dm1bxinsert((DMP_RCB *) 0, t, 
		    ...
		    mct->mct_cx_atts[MCT_PLEAF], mct->mct_cx_attcnt[MCT_PLEAF],
		    mct->mct_index_comp, mct->mct_cx_klen[MCT_PLEAF],
		    mct->mct_curleaf, key, &tid, partno,
		    (DM_LINE_IDX)DM1B_VPT_GET_BT_KIDS_MACRO(mct->mct_page_type,
		    mct->mct_curleaf), (i4) FALSE, (i4) FALSE, err_code);
	if (status != E_DB_OK)
	{
	    return (status);
	}
    ...
    **
    ** If the page type != TCB_PG_V1, we can always get a new leaf,
    ** as we no longer have overflow pages for duplicate keys in primary
    ** non unique btrees.
    **
    ...
    */
    ...
    if (dup == FALSE || (mct->mct_page_type != TCB_PG_V1))
...
/*	    			indkey, err_code);   */
    ...
    status = update_parent( mct, indkey, err_code );
...
i4		*err_code)
{
    ...
    if (mct->mct_cx_attcnt[MCT_PLEAF] != mct->mct_cx_attcnt[MCT_PINDEX])
    {
	indexkey = temp_buf;
	dm1b_build_key(mct->mct_keys, mct->mct_cx_keys[MCT_PLEAF], leafkey,
			mct->mct_cx_keys[MCT_PINDEX], indexkey);
    }
    else
	indexkey = leafkey;
    ...
    if ((indexbuf = (DM1B_INDEX *)dm0m_pgalloc(mct->mct_page_size)) == 0)
    {
	*err_code = E_DM93C0_BBPUT_ERROR;
	return (E_DB_ERROR);
    }
	...
	*/
	...
	status = dm1xreadpage(mct, DM1X_FORUPDATE,
	    ...
	    (DMPP_PAGE **)&mct->mct_curindex, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	has_room = dm1cxhas_room(mct->mct_page_type, mct->mct_page_size,
				...
				mct->mct_i_fill,
				mct->mct_cx_kperpage[MCT_PINDEX],
				mct->mct_cx_maxentry[MCT_PINDEX]);
	if (has_room)
	    break;
	...
	*/
    ...
    if (status == E_DB_OK && level < mct->mct_top)
    {
		...
		mct->mct_cx_atts[MCT_PINDEX], mct->mct_cx_attcnt[MCT_PINDEX],
		mct->mct_index_comp,
		mct->mct_cx_klen[MCT_PINDEX], mct->mct_curindex,
		indexkey, &temptid, (i4)0,
		(DM_LINE_IDX)(DM1B_VPT_GET_BT_KIDS_MACRO(mct->mct_page_type,
		mct->mct_curindex) - 1), (i4) FALSE, (i4) FALSE, err_code);
	...
	*err_code = E_DM0057_NOROOM;
	...
	MEcopy((PTR)mct->mct_curindex, mct->mct_page_size,
			(PTR)indexbuf);
	status = dm1xnextpage(mct, (i4 *)&page, err_code);
    }
    if (status != E_DB_OK)
    {
	log_error(E_DM925B_DM1B_BPUT, err_code);
	...
	*err_code = E_DM93C0_BBPUT_ERROR;
    ...
    status = dm1xnewpage(mct, 0, (DMPP_PAGE **)&mct->mct_curindex, err_code);
    if (status == E_DB_OK)
    {
	/*
...
i4		*err_code)
{
	...
	*/
	lastkid = DM1B_VPT_GET_BT_KIDS_MACRO(mct->mct_page_type, mct->mct_curleaf)
				 - 1;
	endkey = indexkey;
	endkey_len = mct->mct_cx_klen[MCT_PLEAF];
	status = dm1cxget(mct->mct_page_type, mct->mct_page_size, 
		    mct->mct_curleaf, t->tcb_acc_tlv, 
		    mct->mct_cx_atts[MCT_PLEAF], mct->mct_cx_attcnt[MCT_PLEAF],
		    lastkid, &endkey, (DM_TID *)0, (i4*)0,
		    &endkey_len,
		    mct->mct_index_comp, NULL);
	if (status != E_DB_OK)
	{
	...
	{
	    status = dm1cxcomp_rec(t, mct->mct_cx_atts[MCT_PLEAF], 
		    mct->mct_cx_attcnt[MCT_PLEAF], indexkey, 
		    mct->mct_cx_klen[MCT_PLEAF], cindexkey_buf, &indkey_len);
	    if (status != E_DB_OK)
	    {
		/*FIX_ME -- need to log something here */

		*err_code = E_DM93E7_INCONSISTENT_ENTRY;
		break;
	    }
	...
	*/
	...
	{
	    status = dm1xreadpage(mct, DM1X_FORUPDATE,
		mct->mct_lastleaf, (DMPP_PAGE **)&mct->mct_curleaf, err_code);
	    if (status == E_DB_OK)
	    {
	...
	*/
	if ( ! mct->mct_index)
	{
		...
		dm1xbput (mct, mct->mct_curdata, rec, rec_size,
			mct->mct_compressed, 0, &tid,
			mct->mct_ver_number, err_code);
	    }
	}
	else
	{
	...
	*/
	...
	/* Put new record on new leaf. */
        status = dm1bxinsert((DMP_RCB *) 0, t,
		    ...
		    mct->mct_cx_atts[MCT_PLEAF], mct->mct_cx_attcnt[MCT_PLEAF],
		    mct->mct_index_comp,
		    mct->mct_cx_klen[MCT_PLEAF], mct->mct_curleaf,
		    key, &tid, partno, 0, 
		    (i4) FALSE, (i4) FALSE, err_code);
	if (status != E_DB_OK)
	    break;
...
i4		*err_code)
{
	...
	** page number 0.
	*/
	if ((index = (DM1B_INDEX *)dm0m_pgalloc(mct->mct_page_size)) == 0)
	{
	    status = E_DB_ERROR;
	    *err_code = E_DM93C0_BBPUT_ERROR;
	    break;
	...
	status = dm1cxrshift(mct->mct_page_type, mct->mct_page_size, 
				...
				mct->mct_dupstart,
				mct->mct_cx_klen[MCT_PLEAF] + mct->mct_tidsize); 
	if (status != E_DB_OK)
	{
	...
	*/
	...
	status = dm1cxget(mct->mct_page_type, mct->mct_page_size, 
			    mct->mct_curleaf, t->tcb_acc_tlv, 
			    mct->mct_index_atts, mct->mct_index_attcount,
			    lastkid, &endkey, (DM_TID *)0, (i4*)0,
			    &endkey_len, 
			    mct->mct_index_comp, NULL);
	if (status != E_DB_OK)
	...
	if (mct->mct_index_comp != DM1CX_UNCOMPRESSED)
	{
	    status = dm1cxcomp_rec(t, mct->mct_index_atts, 
		    mct->mct_index_attcount, indexkey, 
		    mct->mct_cx_klen[MCT_PLEAF], cindexkey_buf, &indkey_len );
	    if (status != E_DB_OK)
	    {
		/*FIX_ME -- need to log something here */
...
i4		*err_code)
{
	...
	*/
	    ...
	    DMPP_LEAF)
	{
		...
		*err_code = E_DM93C0_BBPUT_ERROR;
	    ...
	    {
		status = dm1cxcomp_rec(t, mct->mct_index_atts,
				mct->mct_index_attcount,
				indexkey, mct->mct_cx_klen[MCT_PLEAF],
	...
	*/
	...
	bounds_len[0] = mct->mct_cx_klen[MCT_PLEAF];
	/*
	...
	*/
	...
	if (save_infinity[0] == FALSE)
	{
	    status = dm1cxget(mct->mct_page_type, mct->mct_page_size, 
			    mct->mct_curleaf, t->tcb_acc_tlv, 
			    mct->mct_index_atts, mct->mct_index_attcount,
			    DM1B_LRANGE, &endkey, (DM_TID *)0, (i4*)0,
			    &bounds_len[0],
			    mct->mct_index_comp, NULL);
	    if (status != E_DB_OK)
	    {
	...
	bounds_len[1] = mct->mct_cx_klen[MCT_PLEAF];
	/*
	...
	*/
	...
	if (save_infinity[1] == FALSE)
	{
	    status = dm1cxget(mct->mct_page_type, mct->mct_page_size, 
			    mct->mct_curleaf, t->tcb_acc_tlv,
			    mct->mct_index_atts, mct->mct_index_attcount,
			    DM1B_RRANGE, &endkey, (DM_TID *)0, (i4*)0,
			    &bounds_len[1],
			    mct->mct_index_comp, NULL);
	    if (status != E_DB_OK)
	    {
	...
	if (mct->mct_index_comp != DM1CX_UNCOMPRESSED)
	{
	    ...
	    */
	    ...
	    {
		status = dm1cxcomp_rec(t, mct->mct_index_atts,
				mct->mct_index_attcount, bounds_buf[0],
	    ...
	    {
		status = dm1cxcomp_rec(t, mct->mct_index_atts,
				mct->mct_index_attcount, bounds_buf[1],
