    ...
    i4	    *err_code)
{
	...
	*/
	if (log_required)
	{
	    dm0l_flag = (is_journal(r) ? DM0L_JOURNAL : 0) | DM0L_PHYS_LOCK;
		    ...
		    compression_type, tcb->tcb_rel.relloccount, loc_config_id,
	...
	*/
        status = dm1cxput(tbio->tbio_page_type, tbio->tbio_page_size, b,
			   compression_type, DM1C_DIRECT, tran_id,
    ...
    i4	    *err_code)
{
	...
	*/
	status = dm1cxdel(tbio->tbio_page_type, tbio->tbio_page_size, b,
			    ...
			    &r->rcb_tran_id, r->rcb_seq_number,
	...
	i4		*err_code)
{
    ...
    */
    ...
    {
        rightpage = (DM1B_INDEX *)sibling[right];
	...
	needkids = avgkids - rightkids;
	    ...
	    */
	    ...
	    {
		leftpage = (DM1B_INDEX *)sibling[left];
	...
	*/
	...
	get_range_entries(leftpage, NULL, t->tcb_acc_klv,
			  t->tcb_rel.relpgtype, t->tcb_rel.relpgsize, 
			  range, range_len, infinity, err_code);
	if (left > 0)
	{
    ...
    i4     *err_code)
{
    ...
    */
    ...
    {
	atts_array = r->rcb_tcb_ptr->tcb_data_atts;
	atts_count = r->rcb_tcb_ptr->tcb_rel.relatts;
    }
    else
    {
	atts_array = r->rcb_tcb_ptr->tcb_key_atts;
	atts_count = r->rcb_tcb_ptr->tcb_keys;
    }
	...
	*/
	...
	get_range_entries(curpage, NULL, t->tcb_acc_klv,
			  ...
			  range_keys, range_lens, infinities, err_code);
	...
	if (leaf_split)
	{
	    ...
	    */
		...
		s = dm1cxget(tbio->tbio_page_type, tbio->tbio_page_size,
		    curpage, t->tcb_acc_tlv, atts_array, atts_count,
	...
	if (r->rcb_logging & RCB_LOGGING)
	{
	    ...
	    */
	    ...
	    s = dm0l_bi(r->rcb_log_id, dm0l_flag,
		  ...
		  t->tcb_rel.relpgsize, (DMPP_PAGE *)sibpage,
		  (LG_LSN *) 0, &lsn, err_code);
	    if (s != E_DB_OK)
		break;
	    ...
	    if (data)
	    {
		    ...
		    t->tcb_rel.relpgsize, (DMPP_PAGE *)data,
		    (LG_LSN *) 0, &lsn, err_code);
	    	if (s != E_DB_OK)
		    break;
	...
	*/
	s = dm1mxformat(tbio->tbio_page_type, tbio->tbio_page_size, 
			r, t, sibpage, kperpage, klen,
	    		DM1B_VPT_GET_PAGE_PAGE_MACRO(tbio->tbio_page_type, sibpage),
	    		ptype, compression_type, t->tcb_acc_plv, err_code);
	if (s != E_DB_OK)
	    break;
	...
	*/
	s = update_range_entries(curpage, sibpage, adf_cb,
				 ...
				 disc_key, klen - DM1B_TID_S, err_code);
	if (s != E_DB_OK)
	    break;
	/*
	...
	*/
	s = dm1m_rcb_update(r, &bid, &newbid, *split, DM1C_MSPLIT, err_code);
	if (s != E_DB_OK)
	    break;
    ...
    i4		*err_code)
{
	...
	*/
	...
	s = put_range_entries(cur, compression_type, hilbertsize,
			      page_type, page_size, tran_id, seq_number,
    ...
    DM_TID		infinity[])
{
	...
	*/
    ...
    s = dm1cxput(page_type, page_size, cur, compression_type, 
		DM1C_DIRECT, tran_id, seq_number, DM1B_LRANGE,
    	      	lkeyptr, keylen, &infinity[DM1B_LEFT], (i4)0 );
    if (s)
    {
        dm1cxlog_error( E_DM93E4_BAD_INDEX_PUT, (DMP_RCB *)0, cur,
        		    page_type, page_size, DM1B_LRANGE );
        return s;
    }
    s = dm1cxput(page_type, page_size, cur, compression_type, 
		DM1C_DIRECT, tran_id, seq_number, DM1B_RRANGE,
