...
/* Put the prototype here because <dma.h> caused many errors */
...
i4             *err_code)
{
	...
	*/
	if (req_access_mode == DM2T_A_SYNC_CLOSE
	    && lock_mode != DM2T_X)
	{
	    *err_code = E_DM003E_DB_ACCESS_CONFLICT;
	    return (E_DB_ERROR);
	}
	...
	** them to Page Mode.
	*/
	if (table_id->db_tab_base <= DM_SCONCUR_MAX
	...
	else
	{
		...
		timeout, &grant_mode, &lockid, err_code);
	    if (status != E_DB_OK)
		break;
	    table_lock = TRUE;
	}
	...
	*/
	if (req_access_mode == DM2T_A_OPEN_NOACCESS)
	    ...
	    flags, dcb, &tcb, err_code);
        if (status != E_DB_OK)
            break;

	/* If this is HEAP etab, use table locking */
	if ((tcb->tcb_rel.relstat2 & TCB2_EXTENSION)
			&& tcb->tcb_rel.relspec == DB_HEAP_STORE)
	{
	    ...
	    && req_access_mode != DM2T_A_READ)
	{
	    *err_code = E_DM006A_TRAN_ACCESS_CONFLICT;
        ...
        /* it's a system catalog */
            ...
            {
                   *err_code = E_DM0181_PROD_MODE_ERR;
                   status = E_DB_ERROR;
                   break;
            }
        }
	...
	/*
	** Trace the Table Lock if one was granted - we do this after fixing
	** the TCB so so that we can include the table name in the message.
	*/
	if (DMZ_SES_MACRO(1) && (lock_mode != DM2T_N))
	{
	    MEfill(sizeof(lock_key), '\0', (char *)&lock_key);
        ...
        if (tcb->tcb_rel.relstat2 & TCB2_READONLY)
            if (req_access_mode == DM2T_A_READ)
                lock_mode = DM2T_S;
                        ...
                        (req_access_mode == DM2T_A_MODIFY))
            {
                *err_code = E_DM0067_READONLY_TABLE_ERR;
                status = E_DB_ERROR;
                break;
            }
	...
	*/
	...
	if ( (tcb->tcb_rel.relstat2 & TCB2_PHYS_INCONSISTENT ) &&
	     (req_access_mode != DM2T_A_OPEN_NOACCESS) )
	{
	    *err_code = E_DM0162_PHYS_INCONSISTENT_TABLE_ERR;
	    status = E_DB_ERROR;
	    break;
	}
	/*
	...
	*/
	if ((tcb->tcb_status & TCB_INCONS_IDX) &&
	    ...
	    (req_access_mode != DM2T_A_MODIFY))
	{
	    *err_code = E_DM0168_INCONSISTENT_INDEX_ERR;
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	...
	else
	{
		...
		case    DM2T_A_WRITE:
			    ...
			    */
			    if (r->rcb_tcb_ptr->tcb_rel.relstat & TCB_GW_NOUPDT)
			    {
				*err_code = E_DM0137_GATEWAY_ACCESS_ERROR;
				status=E_DB_ERROR;
				break;
			    }
			    r->rcb_access_mode = RCB_A_WRITE;
	...
	*/
	...
	r->rcb_k_mode = lock_mode;
        ...
        else if ( ((row_lock == TRUE) || (DMZ_TBL_MACRO(16))) &&
		(dm2d_row_lock(dcb) == TRUE) &&
		(dm2t_row_lock(tcb) == TRUE))
	{
	    r->rcb_k_type = RCB_K_ROW;
	}
	else
        {
	...
	*/
        ...
        else if (lock_mode == DM2T_N)
	{
	    ...
	    */
	    if (DMPP_VPT_PAGE_HAS_SEGMENTS(tcb->tcb_rel.relpgtype))
		rnl_buf_cnt = 3;
	    else
		rnl_buf_cnt = 2;
	    /* Get LongTerm memory */
		...
		(i4)DM0M_LONGTERM, (i4)MISC_CB, (i4)MISC_ASCII_ID,
                (char *)r, (DM_OBJECT **)&r->rcb_rnl_cb, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	if (tcb->tcb_rel.relstat & TCB_GATEWAY)
	{
	    status = dmf_gwt_open(r, err_code);
	    if (status != E_DB_OK)
	    {
		/*
	...
	*/
	if ((tcb->tcb_rel.relfhdr != DM_TBL_INVALID_FHDR_PAGENO ) &&
	    ...
	    (req_access_mode != DM2T_A_OPEN_NOACCESS))
	{
	    status = dm1p_checkhwm(r, err_code);
	...
	*/
	    ...
	    (DMZ_SES_MACRO(32) == 0 || dmd_reptrace() == FALSE))
	{
	    ...
	    */
		...
		tran_id, lock_id, log_id, err_code);
	    if (status != E_DB_OK)
		break;
	    /*
	    ...
	    */
		...
		&r->rep_shad_rcb, (DML_SCB *)0, err_code);
	    if (status != E_DB_OK)
		break;
		...
		req_access_mode, timeout, maxlocks, sp_id, log_id, lock_id,
		sequence, iso_level, db_lockmode, tran_id, &local_timestamp,
		&r->rep_arch_rcb, (DML_SCB *)0, err_code);
	    if (status != E_DB_OK)
		break;
		...
		req_access_mode, timeout, maxlocks, sp_id, log_id, lock_id,
		sequence, iso_level, db_lockmode, tran_id, &local_timestamp,
		&r->rep_shadidx_rcb, (DML_SCB *)0, err_code);
	    if (status != E_DB_OK)
		break;
	    ...
	    if (rep_prio_tabid.db_tab_base)
	    {
		    ...
		    &local_timestamp, &r->rep_prio_rcb, (DML_SCB *)0, err_code);
		if (status != E_DB_OK)
		    break;
	    }
	    if (rep_cdds_tabid.db_tab_base)
	    {
		    ...
		    DM2T_A_READ, timeout, maxlocks, sp_id, 
		    log_id, lock_id, sequence, iso_level, db_lockmode, tran_id,
		    &local_timestamp, &r->rep_cdds_rcb, (DML_SCB *)0, err_code);
		if (status != E_DB_OK)
		    break;
	    }
	}
	...
	** if it is then there are no individual locks to drop (kibro01)
	*/
	if (tcb->tcb_rel.relgwid == GW_IMA && lock_mode != DM2T_N &&
		(dcb->dcb_status & DCB_S_EXCLUSIVE) == 0)
	{
	    ...
	    if (stat != OK)
	    {
		ule_format(stat, &sys_err, ULE_LOG, NULL, (char *)0, (i4)0,
		    (i4 *)0, &local_error, 0);
    ...
    */
    ...
    {
	if (r->rep_shad_rcb)
	    local_status = dm2t_close(r->rep_shad_rcb, (i4)0, err_code);

	if (r->rep_arch_rcb)
	    local_status = dm2t_close(r->rep_arch_rcb, (i4)0, err_code);

	if (r->rep_shadidx_rcb)
	    local_status = dm2t_close(r->rep_shadidx_rcb, (i4)0, err_code);

	if (r->rep_prio_rcb)
	    local_status = dm2t_close(r->rep_prio_rcb, (i4)0, err_code);
	...
	*/
	...
	local_status = dm2r_release_rcb(&rcb, &local_error);
	if (local_status)
	{
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
    ...
    */
    if (tcb)
    {
	local_status = dm2t_unfix_tcb(&tcb, DM2T_VERIFY, lock_id, log_id, &local_error);
	if (local_status)
	{
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
    ...
    */
        ...
        if (lock_mode == DM2T_N)
	{
		...
		LK_N, (i4)0, timeout, &local_error);
	    if (local_status)
	    {
		ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG,
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
		    &local_error, (i4)0);
	    }
	}
	else if ((lockid.lk_unique) &&
		    ((dcb->dcb_status & DCB_S_EXCLUSIVE) == 0))
	{
	    stat = LKrelease((i4)0, lock_id, (LK_LKID *)&lockid,
		(LK_LOCK_KEY *)0, (LK_VALUE *)0, &sys_err);
    ...
    {
        ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
        *err_code = E_DM9276_TBL_OPEN;
    }
...
i4		*err_code)
{
	...
	*/
	if (r->rep_shad_rcb)
	{
	    status = dm2t_close(r->rep_shad_rcb, (i4)0, err_code);
	    if (status != E_DB_OK)
		break;
	    r->rep_shad_rcb = NULL;
	}
	if (r->rep_arch_rcb)
	{
	    status = dm2t_close(r->rep_arch_rcb, (i4)0, err_code);
	    if (status != E_DB_OK)
		break;
	    r->rep_arch_rcb = NULL;
	}
	if (r->rep_shadidx_rcb)
	{
	    status = dm2t_close(r->rep_shadidx_rcb, (i4)0, err_code);
	    if (status != E_DB_OK)
		break;
	...
	if (r->rep_prio_rcb)
	{
	    status = dm2t_close(r->rep_prio_rcb, (i4)0, err_code);
	    if (status != E_DB_OK)
		break;
	    r->rep_prio_rcb = NULL;
	}
	if (r->rep_cdds_rcb)
	{
	    status = dm2t_close(r->rep_cdds_rcb, (i4)0, err_code);
	    if (status != E_DB_OK)
		break;
        ...
        */
        if (r->rcb_iso_level == RCB_CURSOR_STABILITY ||
            r->rcb_iso_level == RCB_REPEATABLE_READ)
        {
	...
	** Release them now.
        */
		...
		if (status != E_DB_OK)
		{
		    ...
		    &r->rcb_data_lkid, (LK_VALUE *)0, err_code);
                if (status != E_DB_OK)
		{
		    ...
		    &tcb->tcb_rel.relid, &r->rcb_tran_id,
		    &r->rcb_leaf_lkid, (LK_VALUE *)0, err_code);
		if (status != E_DB_OK)
		{
	...
	*/
	...
	(DM_OBJECT **)&((DM2R_L_CONTEXT*)tcb->tcb_lct_ptr)->lct_mct.mct_buffer);
        ...
        /*
        ** (ICL phil.p)
        ** If we were called by the transaction abort code,
        ** covert the TCB lock down FROM LK_X to LK_S.
        */
        if (r->rcb_state & RCB_RECOVER)
        {
            stat = LKrequestWithCallback(tcb->tcb_table_io.tbio_cache_flags &
				TBIO_CACHE_MULTIPLE ? LK_PHYSICAL | LK_CONVERT :
				LK_PHYSICAL | LK_CONVERT | LK_LOCAL,
                dmf_svcb->svcb_lock_list, (LK_LOCK_KEY *)0,
                LK_S, (LK_VALUE * )&lockvalue, tcb->tcb_lkid_ptr, 0L,
		(LKcallback) dm2t_tcb_callback, (PTR) tcb, &sys_err);

	    if (stat != OK)
	    {
	        ule_format(stat, &sys_err, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	        ule_format(E_DM901C_BAD_LOCK_REQUEST, &sys_err, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, (i4)2,
	...
	status = dm2r_return_rcb(&r, err_code);
	if (status != E_DB_OK)
	    break;
        ...
        */
        ...
        if (unlock_table)
        {
            ...
            status = LKrelease((i4)0, lock_list,
                (LK_LKID *)0, (LK_LOCK_KEY *)&lock_key, (LK_VALUE *)0,
                &sys_err);
            if (status != OK)
            {
                ule_format(status, &sys_err, ULE_LOG, NULL, 0, 0, 0,
                            &local_error, 0);
                ule_format(E_DM901B_BAD_LOCK_RELEASE, &sys_err, ULE_LOG, NULL,
                    (char *)0, 0L, (i4 *)0, &local_error, 1, 0, lock_list);
                *err_code = E_DM926C_TBL_CLOSE;
                break;
            }
            if (DMZ_SES_MACRO(1))
            {
...
i4             *err_code)
{
	...
	*/
	...
	if (t == NULL)
	{
		...
		*err_code = W_DM9C50_DM2T_FIX_NOTFOUND;
		status = E_DB_WARN;
		break;
	    }
	    ...
	    */
		...
		lock_list, log_id, sync_flag, flags, &t, err_code);
	    if (status != E_DB_OK)
	    {
		dm0s_munlock(hash_mutex);
		if (status == E_DB_WARN)
	    ...
	    built = TRUE;
            ...
            /*
            ** (ICL phil.p)
            ** Under DMCM protocol TCB locked LK_S.
            ** Verify_tcb call can be skipped while
            ** TCB_VALIDATED flag is set.
            */
            if (t->tcb_dcb_ptr->dcb_status & DCB_S_DMCM)
                t->tcb_status |= TCB_VALIDATED;
	}
	...
	*/
	if (t->tcb_status & TCB_BUSY)
	{
	    ...
	    {
		dm0s_munlock(hash_mutex);
		*err_code = W_DM9C51_DM2T_FIX_BUSY;
		status = E_DB_WARN;
		break;
	    }
	...
	*/
	  ...
	  )
	{
	    ...
	    */
	    ...
	    status = dm2t_release_tcb(&t, (i4)0, lock_list, log_id, err_code);
	    dm0s_munlock(hash_mutex);
	    if (status != E_DB_OK)
		break;
	    continue;
	...
	*/
            ...
            ((t->tcb_status & TCB_VALIDATED) == 0))
	{
		...
		*/
		status = dm2t_release_tcb(&t, DM2T_TOSS, lock_list, log_id, err_code);
		dm0s_munlock(hash_mutex);
		if (status != E_DB_OK)
		    break;
		continue;
            ...
            /*
            ** (ICL phil.p)
            ** DMCM. TCB has LK_S lock ensuring validity.
            */
            if (t->tcb_dcb_ptr->dcb_status & DCB_S_DMCM)
                t->tcb_status |= TCB_VALIDATED;
	}
    ...
    */
    ...
    {
	*tcb = return_tcb;
	if ( (return_tcb != NULL) && (*err_code == W_DM9C50_DM2T_FIX_NOTFOUND) )
	     *err_code = E_DB_OK;
		    ...
		    || return_tcb->tcb_rel.relspec == TCB_RTREE))
	{
		...
		t->tcb_leafattcnt, t->tcb_klen,
		t->tcb_kperleaf);
	    TRdisplay(
	    "fix_tcb(%d,%d): Index attcnt=%d klen=%d kperpage %d\n",
		t->tcb_rel.reltid.db_tab_base, t->tcb_rel.reltid.db_tab_index,
		t->tcb_ixattcnt, t->tcb_ixklen,
		t->tcb_kperpage);
    ...
    if (*err_code > E_DM_INTERNAL)
    {
        ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, (i4)0);
        *err_code = E_DM9C8A_DM2T_FIX_TCB;
    }
    return (E_DB_ERROR);
}

...
i4		*err_code)
{
	...
	*/
             ...
             (flags & DM2T_PURGE) && (base_tcb->tcb_ref_count > 1))
	{
		...
		*/
		ule_format(E_DM9C5D_DM2T_PURGEFIXED_WARN, (CL_SYS_ERR *)0,
	    ...
	    */
	    ...
	    {
		dm0s_munlock(hash_mutex);
		ule_format(E_DM9C5F_DM2T_PURGEBUSY_WARN, (CL_SYS_ERR *)0,
		    ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0,
		    (i4 *)0, err_code, (i4)6,
		    sizeof(DB_DB_NAME), dcb->dcb_name.db_db_name,
		    sizeof(DB_TAB_NAME), base_tcb->tcb_rel.relid.db_tab_name,
		    ...
		    0, base_tcb->tcb_open_count, 0, base_tcb->tcb_ref_count,
		    0, base_tcb->tcb_status);
		*err_code = E_DM9C53_DM2T_UNFIX_TCB;
		status = E_DB_ERROR;
		break;
	    }
	...
	*/
	...
	{   
	    ...
	    */
	    ...
	    {
		ule_format(E_DM9C52_VALID_COUNT_MISMATCH, (CL_SYS_ERR *)0,
		    ULE_LOG, (DB_SQLSTATE *)NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, (i4)3,
		    sizeof(dcb->dcb_name), &dcb->dcb_name,
	    ...
	    ** of whether the KEEP flag was specified).  In this case,
	    ** we may queue the tcb onto the free queue and return an error
	    ** to the release requestor.
	    **
...
i4		*err_code)
{
	...
	*err_code = 0;
	...
	/*
	** Verify that this is a base table TCB.
	...
	*/
	...
	{
	    ule_format(E_DM9C70_DM2T_RELEASE_IDXERR, (CL_SYS_ERR *)0, ULE_LOG,
	...
	*/
	if ( (t->tcb_page_adds || t->tcb_tup_adds) &&
	     (flag & (DM2T_TOSS | DM2T_NORELUPDAT)) == 0 &&
	     ...
	     (t->tcb_dcb_ptr->dcb_status & DCB_S_RECOVER) == 0 )
	{
	    ...
	    */
	    ...
	    dm0s_munlock(t->tcb_hash_mutex_ptr);
    	    ...
    	    update_rel(t, lock_list, err_code);

	    /* Unpin the TCB, wait for other fixers if ! DM2T_KEEP */
	    dm0s_mlock(t->tcb_hash_mutex_ptr);
	...
	*/
	...
	if (close_page_needed)
	{
	    ...
	    ** flag is specified, allow pages to remain in cache.
	    */
	    close_flag = DM0P_CLCACHE;
	    if (flag & (DM2T_PURGE | DM2T_TOSS))
		close_flag = DM0P_CLOSE;
	    if (t->tcb_temporary == TCB_TEMPORARY)
		close_flag = 0;
	    status = dm0p_close_page(t, lock_list, log_id, close_flag, err_code);
	    if (status != E_DB_OK)
	    {
		ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE*)NULL,
		    (char *)0, (i4)0, (i4 *)0, err_code, 0);
		*err_code = E_DM9371_RELEASE_TCB_WARNING;
		status = E_DB_WARN;
		break;
	    }
	}
        ...
        /*
        ** (ICL phil.p)
        ** Convert tcb cache lock to null and return
        */
        if (flag & DM2T_FLUSH)
        {
          status = dm2t_convert_tcb_lock(t, LK_N, err_code);
          if (status != E_DB_OK)
              *err_code = E_DM926A_TBL_CACHE_LOCK;

          t->tcb_status &= ~(TCB_FLUSH | TCB_VALIDATED);
          break;
        }
	...
	*/
	  ...
	  && (t->tcb_status & TCB_SHOWONLY) == 0)
	{
		...
		DM0P_MODIFY, lock_list, log_id, err_code);
	    if (status != E_DB_OK)
	    {
		/*
	...
	*/
	if (t->tcb_table_io.tbio_status & TBIO_OPEN)
	{
		...
		*/
		...
		t->tcb_table_io.tbio_loc_count, release_fcb_flag, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	while (t->tcb_iq_next != (DMP_TCB*) &t->tcb_iq_next)
	{
		...
		status = dm2f_release_fcb(lock_list, log_id,
		    it->tcb_table_io.tbio_location_array,
		    it->tcb_table_io.tbio_loc_count, release_fcb_flag, err_code);
		if (status != E_DB_OK)
		    break;
	    }
		...
		status = dm2r_release_rcb(&trcb, err_code);
		if (status != E_DB_OK)
		    break;
	    }
	    if (status != E_DB_OK)
	...
	*/
	if ((dcb->dcb_served == DCB_MULTIPLE) &&
	    ((t->tcb_lk_id.lk_unique != 0) || (t->tcb_lk_id.lk_common != 0)))
	{
            /*
            ** (ICL phil.p)
            ** Releasing lock. TCB will may no longer be valid.
            */
	    ...
	    if (flag & DM2T_PURGE)
	    {
                if (dcb->dcb_status & DCB_S_DMCM)
                {
                    /*
                    ** (ICL phil.p)
                    ** Must convert TCB lock on server lock list to NULL
                    ** before bumping on TX lock list, otherwise we'll
                    ** deadlock on the bump.
                    */
                    status = dm2t_convert_bumplock_tcb( t, LK_N, err_code);
                    if (status != E_DB_OK)
                        break;
                }
		else
		{
		    status = dm2t_bumplock_tcb(dcb, &t->tcb_rel.reltid,
		        lock_list, err_code);
		    if (status != E_DB_OK)
		        break;
		}
	...
	*/
	...
	if ( t->tcb_rel.reltid.db_tab_index == 0 )
	    hcb->hcb_tcbcount--; /* keep track of base TCBs allocated */
	...
	*/
	...
	if ( t->tcb_rel.relstat & TCB_INDEX )
	{
		...
		if ( --t->tcb_parent_tcb_ptr->tcb_unique_count == 0 )
		    t->tcb_parent_tcb_ptr->tcb_index_type &= ~TCB_S_UNIQUE;
	    }
	}
...
i4		    *err_code)
{
	...
	/*
	** If the free queue is empty or if we've cycled all the way around
	** without having found a TCB we could free, return an error.
	*/
	if ((hcb->hcb_ftcb_list.q_next == &hcb->hcb_ftcb_list) ||
	    (hcb->hcb_ftcb_list.q_next == first_tcb))
	{
	    dm0s_munlock(&hcb->hcb_tq_mutex);
	    *err_code = E_DM9328_DM2T_NOFREE_TCB;
	    status = E_DB_ERROR;
	    break;
	}

	/*
	** Get the base offset for the first TCB in the queue.
	...
	*/
	base_tcb = (DMP_TCB *)( (char *)(hcb->hcb_ftcb_list.q_next) -
	...
	** because we want to make sure nobody fixes the TCB when we
	** let go of the free-list mutex.
	*/
	...
	*/
	status = dm2t_release_tcb(&base_tcb, (i4)0, lock_list, 
				  log_id, err_code);
	if (status == E_DB_WARN)
	{
	    ...
	    /*
	    ** Remember first failed release in an attempt to keep from
	    ** cycling around the free list retrying the same tcb's.
	    */
	    if (first_tcb == 0)
		first_tcb = &base_tcb->tcb_ftcb_list;
	    ...
	    *err_code = E_DM9328_DM2T_NOFREE_TCB;
        ...
        hcb->hcb_tcbreclaim++;/* keep track of how many times needed 
				 to reclaim */
	dm0s_munlock(hash_mutex);
	break;
    }
    ...
    if ((*err_code > E_DM_INTERNAL) && (*err_code != E_DM9328_DM2T_NOFREE_TCB))
    {
        ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, (i4)0);
	*err_code = E_DM926E_RECLAIM_TCB;
    }
    return (E_DB_ERROR);
}

...
i4             *err_code)
{
    ...
    */
    ...
    {
	status = LKrelease((i4)0, lock_list,
	    (LK_LKID *)0, (LK_LOCK_KEY *)&ctrl_lock_key, (LK_VALUE *)0,
	    &sys_err);
	if (status != OK)
	{
	    ule_format(status, &sys_err, ULE_LOG, NULL, 0, 0, 0, &error, 0);
	    ule_format(E_DM901B_BAD_LOCK_RELEASE, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, &error, 1, 0, lock_list);
    ...
    status = LKrequest((flags | LK_PHYSICAL), lock_list, &ctrl_lock_key,
	lock_mode, (LK_VALUE *)0, (LK_LKID *)0, timeout, &sys_err);
    if (status == OK)
	return (E_DB_OK);
    ...
    */
	...
	ule_format(E_DM9045_TABLE_DEADLOCK, &sys_err,
	    ULE_LOG, NULL, 0, 0, 0, &error, 5, sizeof(ownername), &ownername,
	...
	*err_code = E_DM0042_DEADLOCK;
	...
	if ( dmf_svcb->svcb_status & SVCB_C2SECURE )
	     _VOID_ dma_write_audit(
		    ...
		    FALSE, /* Not force */
		    &error, NULL);
	ule_format(E_DM9046_TABLE_NOLOCKS, &sys_err,
	    ULE_LOG, NULL, 0, 0, 0, &error, 2, sizeof(tablename), &tablename,
	...
	*err_code = E_DM004B_LOCK_QUOTA_EXCEEDED;
	...
	if ( dmf_svcb->svcb_status & SVCB_C2SECURE )
	     _VOID_ dma_write_audit(
		    ...
		    FALSE, /* Not force */
		    &error, NULL);
	ule_format(E_DM9043_LOCK_TIMEOUT, &sys_err,
	    ULE_LOG, NULL, 0, 0, 0, &error, 6, sizeof(tablename), &tablename,
	...
	*err_code = E_DM0065_USER_INTR;
	...
	if ( DMZ_SES_MACRO(35) )
	{
	    ...
	    {
		ule_format(E_DM9066_LOCK_BUSY, &sys_err,
		    ULE_LOG, NULL, 0, 0, 0, &error, 6, sizeof(tablename), &tablename,
		    sizeof(DB_DB_NAME), &dcb->dcb_name, 0, lock_mode,
		    sizeof(ownername), &ownername, 0, "CONTROL", 
		    0, sys_err.moreinfo[0].data.string);
...
i4		    *err_code)
{
	...
	*err_code = 0;
	for (k = 0; k < t->tcb_table_io.tbio_loc_count; k++)
	{
		...
		{
		    *err_code = E_DM001D_BAD_LOCATION_NAME;
		    break;
		}
	    }
	    if (*err_code)
	...
	for ( i = 1; i <= attr_count; i++)
	{
	    ...
	    t->tcb_atts_ptr[i].precision = attr_array[i-1]->attr_prec;
	    t->tcb_atts_ptr[i].flag = attr_array[i-1]->attr_flags;
	    ...
	    ** data compression. Note that the tcb_data_atts and
	    ** tcb_key_atts lists are indexed from 0, not from 1.
	    */
	    t->tcb_data_atts[i - 1] = &(t->tcb_atts_ptr[i]);
	...
	{
	    *err_code = E_DM0075_BAD_ATTRIBUTE_ENTRY;
	    break;
	}

	/*
	** Compute the compression expansion factors for table's attributes.
	** This gives us the worst-case overhead which might be added by
	** the compression routines on the tuple.
	*/
	t->tcb_comp_atts_count =
	    (*t->tcb_acc_tlv->dmpp_compexpand)(t->tcb_data_atts,
							    attr_count);
	...
	for (i = 0; i < key_count; i++)
	{
	    ...
	    if (compare == FALSE)
	    {
		*err_code = E_DM001C_BAD_KEY_SEQUENCE;
		break;
	    }
	...
	t->tcb_tperpage = ((t->tcb_rel.relpgsize - 
		    DMPPN_VPT_OVERHEAD_MACRO(t->tcb_rel.relpgtype))/
		    (t->tcb_rel.relwid + 
		     DM_VPT_SIZEOF_LINEID_MACRO(t->tcb_rel.relpgtype) +
		     DMPP_VPT_SIZEOF_TUPLE_HDR_MACRO(t->tcb_rel.relpgtype)));
	/*
	** Init tcb fields for BTREE temporary tables and indexes
	** Session temporary tables always have table version DMF_T_VERSION
	*/
	if ( t->tcb_rel.relspec == TCB_BTREE )
	{
		...
		if ( t->tcb_rel.relstat & TCB_CLUSTERED )
		{
			...
			*/
		    ...
		    t->tcb_leafatts  = t->tcb_data_atts;
		    t->tcb_leafattcnt  = t->tcb_rel.relatts;
		    t->tcb_leafkeys  = t->tcb_key_atts;
		    t->tcb_klen = t->tcb_rel.relwid;

		    /* tcb_ixkeys point to the INDEX DB_ATTS in tcb_ixatts_ptr */
		...
		else
		{
		    ...
		    t->tcb_leafatts  = t->tcb_key_atts;
		    t->tcb_leafattcnt  = t->tcb_keys;
		    t->tcb_leafkeys  = t->tcb_key_atts;
	...
	*/
	if ( t->tcb_rel.relstat & TCB_INDEX )
	{
	    ...
	    if ( t->tcb_rel.relstat & TCB_UNIQUE )
	    {
		BaseTCB->tcb_index_type |= TCB_S_UNIQUE;
	    ...
	    flag != DM2T_TMP_INMEM_MOD)
	{
	    ...
	    status = dm2r_rcb_allocate(t, (DMP_RCB *)0,
			    tran_id, lock_id, log_id, &tmp_rcb, err_code);
	    if (status != E_DB_OK)
		break;

	    status = dm1s_deallocate(tmp_rcb, err_code);
	    if (status != E_DB_OK)
	    {
		_VOID_ dm2r_release_rcb(&tmp_rcb, &local_err_code);
		break;
	    }

	    status = dm2r_release_rcb(&tmp_rcb, err_code);
	    if (status != E_DB_OK)
		break;
	    tmp_rcb = 0;
	}
	...
	*tcb = t;
    ...
    {
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM9271_BUILD_TEMP_TCB;
    }
    if (t && (*err_code != E_DM9246_DM1S_DEALLOCATE))
    {
	/*
	...
	*/
	...
	if (t->tcb_table_io.tbio_status & TBIO_OPEN)
	{
		...
		t->tcb_table_io.tbio_location_array,
		t->tcb_table_io.tbio_loc_count, 0, &local_error);
	}
...
**      dcb                             The database to search.
**	table_id			The table to search for.
**      opflag                          Value to indicate if it should
**                                      not be destroyed if in use.
**
** Outputs:
...
i4             	    *err_code)
{
	...
	{
	    status = E_DB_ERROR;
	    *err_code = E_DM005D_TABLE_ACCESS_CONFLICT;
	    break;
	}
	...
	*/
	...
	if ( t )
	{
	    ...
	    if ( DestTCB )
	    {
		status = dm2t_release_tcb(&DestTCB, (i4)0, lock_list, log_id, err_code);
		...
		*/
		if ( status == E_DB_OK && lock_list )
		{
		    ...
		    if ( LkStatus )
		    {
			ule_format(LkStatus, &SysErr, ULE_LOG, NULL, (char * )0,
					0L, (i4 *)0, err_code, 0);
			ule_format( E_DM901B_BAD_LOCK_RELEASE, &SysErr,
				    ULE_LOG, NULL,(char * )0, 0L, (i4 *)0, err_code,
				    1, 0, lock_list);
			*err_code = E_DM009D_BAD_TABLE_DESTROY;
			status = E_DB_ERROR;
		    }
		}
	    }
...
i4             *err_code)
{
	...
	*/
	if (dcb->dcb_bm_served == DCB_MULTIPLE)
	{
		...
		DM0P_MODIFY, lock_id, log_id, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	status = dm2t_fix_tcb(dcb->dcb_id, table_id, NULL,
		lock_id, log_id, DM2T_NOBUILD, dcb, &tcb, err_code);
	if (status == E_DB_OK)
	{
	    ...
	    */
	    status = dm2t_unfix_tcb(&tcb, flag, lock_id, log_id, err_code);
	    if (status != E_DB_OK)
		break;
	}
	else
	{
	    ...
	    ** not on an index or partition id.
	    */
    	    if (dcb->dcb_served == DCB_MULTIPLE)
	    {
		base_tabid.db_tab_base = table_id->db_tab_base;
		base_tabid.db_tab_index = 0;
		status = dm2t_bumplock_tcb(dcb, &base_tabid, lock_id, err_code);
		if (status != E_DB_OK)
		    break;
	    }
	}
    ...
    {
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DM9330_DM2T_PURGE_TCB;
    }
    return(E_DB_ERROR);
}

...
**	The returned row count is clipped at 0, the page count at 1.
**
...
dm2t_current_counts(DMP_TCB *tcb, i4 *reltups, i4 *relpages)
{
    ...
    tups = tups + tcb->tcb_rel.reltups + tcb->tcb_tup_adds;
    pages = pages + tcb->tcb_rel.relpages + tcb->tcb_page_adds;
    if (tups < 0) tups = 0;
    if (pages <= 0) pages = 1;
    *reltups = tups;
    *relpages = pages;
...
i4	*err_code)
{
    ...
    if (t->tcb_rel.relstat2 & TCB2_PARTITION)
    {
	*err_code = E_DM93F8_DM2T_NOT_MASTER;
	return (E_DB_ERROR);
    }
	...
	if (t->tcb_low_logical_key >= max_low_value)
	{
	    ...
	    {
		*err_code = E_DM9382_OBJECT_KEY_OVERFLOW;
		return (E_DB_ERROR);
	    }
	...
	status = dm2r_rcb_allocate(t->tcb_dcb_ptr->dcb_rel_tcb_ptr, 0,
	    ...
	    input_rcb->rcb_lk_id,
	    input_rcb->rcb_log_id, &rcb, err_code);
	if (status != E_DB_OK)
            break;
	...
	*/
	  ...
	  || relrecord.reltid.db_tab_index != t->tcb_rel.reltid.db_tab_index)
	{
	    ...
	    *err_code = E_DM9384_DM2T_UPD_LOGKEY_ERR;
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	...
	if (rcb->rcb_logging)
	{
	    if ((status = dm0l_bm(rcb, &bm_lsn, err_code)) != E_DB_OK)
		break;
	}

	status = dm2r_replace(rcb, &t->tcb_iirel_tid,
			    DM2R_BYTID | DM2R_RETURNTID,
			    (char *)&relrecord, (char *)0, err_code);
	...
	*/
	...
	if (rcb->rcb_logging)
	{
	    if ((status = dm0l_em(rcb, &bm_lsn, &lsn, err_code))
							    != E_DB_OK)
		break;
	}

	/* 21-Jan-1995 (canor01) moved from above */
	status = dm2r_release_rcb(&rcb, err_code);
	break;
    }
    if (status != E_DB_OK)
    {
	if (*err_code == E_DM9382_OBJECT_KEY_OVERFLOW)
	{
	...
	else
	{
	    ...
	    ule_format(E_DM9383_DM2T_UPD_LOGKEY_ERR,
		0, ULE_LOG , NULL, (char * )0,
		0L, (i4 *)0, err_code, sizeof(DB_DB_NAME),
...
i4	    lock_list)
{
    ...
    dm0s_eset(lock_list, DM0S_TCBWAIT_EVENT, tcb->tcb_unique_id);
    dm0s_munlock(tcb->tcb_hash_mutex_ptr);

    /* Wait for the TCB to become free */
    dm0s_ewait(lock_list, DM0S_TCBWAIT_EVENT, tcb->tcb_unique_id);
    return;
}

/*
...
i4             *err_code)
{
	...
	(flags | DM2T_NOBUILD), (DMP_DCB *)0, &tcb, err_code);
	...
	*tabio_cb = NULL;
	...
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, (i4)0);
	*err_code = E_DM9C55_DM2T_FIX_TABIO;
    }
...
i4		*err_code)
{
    ...
    tcb = (DMP_TCB *)((char *)tabio_cb - CL_OFFSETOF(DMP_TCB, tcb_table_io));
    status = dm2t_unfix_tcb(&tcb, flags, lock_id, log_id, err_code);
    if (status == E_DB_OK)
	return (E_DB_OK);
	...
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, (i4)0);
	*err_code = E_DM9C56_DM2T_UFX_TABIO;
    }
...
i4		*err_code)
{
    ...
    DB_STATUS		local_status;
    i4		local_error;
    char		sem_name[CS_SEM_NAME_LEN + 10];
    char		*cptr;
    i4			i;
	...
	*/
	...
	if (tcb != NULL)
	{
	    ...
	    */
	    ...
	    {
		dm0s_munlock(tcb_hm);
		if (*err_code == W_DM9C50_DM2T_FIX_NOTFOUND)
		    *err_code = E_DB_OK;
		return (E_DB_OK);
	    }
	}
	...
	*/
	if ((table_id->db_tab_index != 0) && (tcb == 0))
	{
		...
		** just make one up.
		*/
		base_table_name = DB_NOTTAB_NAME;
    
		status = dm2t_init_tabio_cb(dcb, &base_table_id,
		    (DB_TAB_NAME *)base_table_name, table_owner,
		    (i4)0, lock_id, log_id, page_type, page_size, err_code);
		if (status != E_DB_OK)
		    break;
	...
	*/
	status = dm2t_alloc_tcb((DMP_RELATION *)0, (i4)loc_count,
			dcb, &tcb, err_code);
	...
	*/
	if (table_id->db_tab_index <= 0)
	{
	    ...
	    */
	    if (dcb->dcb_served == DCB_MULTIPLE)
	    {
		status = lock_tcb(tcb, err_code);
		if (status != E_DB_OK)
		    break;

                /*
                ** (ICL phil.p)
                ** DMCM. Locked TCB need not be verified.
                */
                if (dcb->dcb_status & DCB_S_DMCM)
	    ...
	    */
	    ...
	    {
		status = dm0p_tblcache(dcb->dcb_id, table_id->db_tab_base,
		    DM0P_VERIFY, lock_id, log_id, err_code);
		if (status != E_DB_OK)
		    break;
	    }
	...
	*/
	if ((dcb->dcb_served == DCB_MULTIPLE) &&
	    ((tcb->tcb_lk_id.lk_unique != 0) || (tcb->tcb_lk_id.lk_common != 0))
	   )
	{
	    local_status = unlock_tcb(tcb, &local_error);

            /*
            ** (ICL phil.p)
            ** DMCM. TCB unlocked, may be invalidated by other server.
            */
            if (dcb->dcb_status & DCB_S_DMCM)
                tcb->tcb_status &= ~(TCB_VALIDATED);
	...
	dm2t_awaken_tcb(tcb, lock_id);
	if (tcb->tcb_status & TCB_WAIT)
	    dm0s_erelease(lock_id, DM0S_TCBWAIT_EVENT, tcb->tcb_unique_id);
	dm0s_munlock(tcb_hm);
    }
    ...
    if (*err_code > E_DM_INTERNAL)
    {
        ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, (i4)0);
	*err_code = E_DM9C57_DM2T_INIT_TABIO;
    }
...
#define MAX_RETRY 300
	...
	*/
	...
	{
	    ule_format(E_DM9C5C_DM2T_ADD_TBIO_LOCERR, (CL_SYS_ERR *)0,
		ULE_LOG, (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
		err_code, (i4)6,
		sizeof(DB_DB_NAME), dcb->dcb_name.db_db_name,
		sizeof(DB_TAB_NAME), table_name->db_tab_name,
		0, location_id, 0, location_count,
		0, location_cnf_id, 0, dcb->dcb_ext->ext_count);
	    *err_code = E_DM9C58_DM2T_ADD_TABIO;
	    break;
	}
	...
	*/
	status = dm2t_fix_tcb(dcb->dcb_id, table_id, (DB_TRAN_ID *)0, lock_id,
	...
	if (status != E_DB_OK)
	{
		...
		location_count, lock_id, log_id, page_type, page_size, err_code);
	    if (status != E_DB_OK)
		break;
	...
	*/
	status = open_tabio_cb(&tcb->tcb_table_io, lock_id, log_id,
	    (i4)DM2F_OPEN, dcb->dcb_sync_flag, err_code);
	if (status != E_DB_OK)
	     break;
	...
	*/
	status = dm2t_unfix_tcb(&tcb, (i4)DM2T_VERIFY, lock_id, log_id, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
    if (retry >= MAX_RETRY)
	*err_code = E_DM9C5E_DM2T_ADD_RETRY_CNT;
	...
	*err_code = E_DM9C58_DM2T_ADD_TABIO;
    ...
    ule_format(E_DM9C71_DM2T_ADD_TBIO_INFO, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	(char *)0, (i4)0, (i4 *)0, err_code, (i4)7,
	sizeof(DB_DB_NAME), dcb->dcb_name.db_db_name,
	sizeof(DB_TAB_NAME), table_name->db_tab_name,
...
#define PCACHE_INITIAL_ME 500	/* If we have to allocate, start here */
	...
	** Don't check the quota for partitions, there might be lots of them.
	**
	** We'll soon discard this dummy TCB, so get ShortTerm memory.
	*/
	if (hcb->hcb_tcbcount < hcb->hcb_tcblimit || opening_partition)
	{
	    status = dm0m_allocate((i4)(sizeof(DMP_TCB)),
		(i4)(DM0M_ZERO | DM0M_SHORTTERM), (i4)TCB_CB, (i4)TCB_ASCII_ID, 
		(char *)dcb, (DM_OBJECT **)&dummy_tcb, err_code);
	}
	else
	{
	...
	*/
	status = dm2r_rcb_allocate(dcb->dcb_rel_tcb_ptr,
	     (DMP_RCB *)0, tran_id, lock_id, log_id,
	     &rel_rcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	if (opening_partition)
	{
	    ...
	    */
	    ...
	    if (status != E_DB_OK)
	    {
		*err_code = E_DM93F9_DM2T_MISSING_PP;
		ule_format(*err_code, NULL, ULE_LOG, NULL, NULL, 0, 0, err_code,
	...
	do
	{
	    ...
	    */
		...
		if (dmf_svcb->svcb_status & SVCB_SINGLEUSER)
		    status = dm0p_alloc_pgarray(relation.relpgsize, err_code);
		...
		if (status != E_DB_OK)
		{
		    *err_code = E_DM0157_NO_BMCACHE_BUFFERS;
		    status = E_DB_ERROR;
		    break;
		}
	    }
	    ...
	    */
	    ...
	    /*
	    ** Sanity check TCB_CLUSTERED, which replaces the deprecated 
	    ** TCB_VBASE bit.
	    **
	    ** Clustered tables evolved with Ingres 2007 (S8_VERSION);
	    ** anything prior to that can't possibly be clustered.
	    ** (upgradedb ought to fix this, if we remember).
	    */
	    if ( MEcmp((char*)&relation.relcmptlvl, DMF_S8_VERSION,
			sizeof(relation.relcmptlvl)) < 0 )
		...
		*/
		if (relation.relstat2 & TCB2_PARTITION)
		{
			...
			*/
		    ...
		    /* Count another of Master's partitions found */
		    partitions_found++;
	    ...
	    */
		...
		if ((status == E_DB_OK) || (*err_code != E_DM923D_DM0M_NOMORE))
		    break;
	    ...
	    */
		...
		(DMPP_PAGE *)rel_rcb->rcb_data_page_ptr)), tcb->tcb_iirel_lsn); 
	    ...
	    */
	    if ( (! tcb->tcb_nofile) && (tcb->tcb_status & TCB_SHOWONLY) == 0 )
	    {
                ...
                */
                ...
                status = open_tabio_cb(&tcb->tcb_table_io, lock_id, log_id,
                    open_tabio_flags, dcb->dcb_sync_flag, err_code);
                if (status != E_DB_OK)
                    break;
	    }
	    ...
	    */
	    if ( !opening_partition && base_tcb &&
		 ...
		 base_tcb->tcb_rel.relnparts == partitions_found )
	    {
		break;
	    }
	...
	*/
	status = dm2r_release_rcb(&rel_rcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	if (dcb->dcb_served == DCB_MULTIPLE)
	{
	    status = lock_tcb(base_tcb, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	...
	else if (tcb_list)
	{
		...
		*err_code = E_DM9331_DM2T_TBL_IDX_MISMATCH;
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	*/
	    ...
	    (rep_catalog(base_tcb->tcb_rel.relid.db_tab_name) == FALSE))
	{
	    ...
	    */
		...
		&rep_rcb, (DML_SCB *)0, err_code);
	    if (status != E_DB_OK)
		break;
	    status = dm2t_open(dcb, &dcb->rep_regist_tab_idx, DM2T_IS,
		DM2T_UDIRECT, DM2T_A_READ, (i4)0, (i4)10, 0, log_id,
		lock_id, (i4)0,(i4)0, 0, tran_id, &timestamp,
		&rep_rcb_idx, (DML_SCB *)0, err_code);
	    if (status != E_DB_OK)
		break;
	    /*
		...
		*/
		    ...
		    *(rep_rec + rep_tcb->tcb_atts_ptr[6].offset) != ' ')
		{
		    ...
		    */
			...
			&timestamp, &rep_rcb_col, (DML_SCB *)0, err_code);
	    	    if (status != E_DB_OK)
		        break;
		    ...
		    for (i = 0;;i++)
		    {
		        status = dm2r_get(rep_rcb_col, &tid, DM2R_GETNEXT,
		    	    reg_col_rec, err_code);
		        if (status != E_DB_OK)
		        {
			    if (*err_code == E_DM0055_NONEXT && i != 0)
			    {
			        /* out of records */
			        status = E_DB_OK;
		    	        break;
			...
			** compacted)
		        */
		        ...
		        {
			    /* no attribute found */
			    *err_code = E_DM9552_REP_NO_ATTRIBUTE;
        		    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG,
            		        (DB_SQLSTATE *)NULL, (char *)0, (i4)0,
			        (i4 *)0, err_code, (i4)2, DB_MAXNAME,
		    ...
		    */
		    status = dm2t_close(rep_rcb_col, (i4)0, err_code);
		    if (status != E_DB_OK)
		        break;
	    ...
	    */
	    status = dm2t_close(rep_rcb, (i4)0, err_code);
	    if (status != E_DB_OK)
		break;
	    status = dm2t_close(rep_rcb_idx, (i4)0, err_code);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	...
	{
	    dm0s_mlock(&hcb->hcb_tq_mutex);
	    if ( !(base_tcb->tcb_rel.relstat2 & TCB2_PARTITION) )
		hcb->hcb_tcbcount++;	/* keep track of base TCBs allocated */
    ...
    */
    ...
    if (rep_rcb)
	local_status = dm2t_close(rep_rcb, (i4)0, err_code);

    if (rep_rcb_idx)
	local_status = dm2t_close(rep_rcb_idx, (i4)0, err_code);
    ...
    if (rel_rcb)
    {
	local_status = dm2r_release_rcb(&rel_rcb, &local_error);
	if (local_status)
	{
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
    ...
    */
	...
	if (tcb->tcb_table_io.tbio_status & TBIO_OPEN)
	{
		...
		tcb->tcb_table_io.tbio_loc_count, 0, &local_error);
	    if (local_status)
	    {
        	ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG,
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
    ...
    */
	...
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
	ule_format(E_DM9C8B_DM2T_TBL_INFO, (CL_ERR_DESC *)0, ULE_LOG,
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
		    &local_error, (i4)3,
		    sizeof(dcb->dcb_name), &dcb->dcb_name,
		    0, table_id->db_tab_base,
		    0, table_id->db_tab_index);
	*err_code = E_DM9C89_DM2T_BUILD_TCB;
    }
...
copy_from_master(DMP_TCB *tcb, DMP_TCB *master_tcb)
{
    ...
    */
    ...
    tcb->tcb_data_atts = master_tcb->tcb_data_atts;
    ...
    */
    ...
    tcb->tcb_index_type = master_tcb->tcb_index_type;
...
i4		*err_code)
{
	...
	*/
	...
	if (t->tcb_table_io.tbio_loc_count > 1)
	{
	    ...
	    */
		...
		tran_id, &timestamp, &dev_rcb, (DML_SCB *)0, err_code);
	    if (status != E_DB_OK)
		break;
		...
		*/
		...
		{
		    ule_format(E_DM9C72_DM2T_LOCCOUNT_MISMATCH, (CL_SYS_ERR *)0,
		    ...
		    *err_code = E_DM007A_BAD_DEVICE_ENTRY;
		    status = E_DB_ERROR;
		    break;
		}
	    ...
	    status = dm2t_close(dev_rcb, 0, err_code);
	    if (status != E_DB_OK)
		break;
	    dev_rcb = 0;
	}
	...
	*/
	if (k != t->tcb_table_io.tbio_loc_count)
	{
	    ule_format(E_DM9C73_DM2T_LOCCOUNT_ERROR, (CL_SYS_ERR *)0,
		ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0,
		&local_error, (i4)5,
		sizeof(DB_DB_NAME), t->tcb_dcb_ptr->dcb_name.db_db_name,
		sizeof(DB_TAB_NAME), t->tcb_rel.relid.db_tab_name,
	...
	*/
	for (k=0; k < t->tcb_table_io.tbio_loc_count; k++)
	{
		...
		if (compare != 0)
		{
		    ule_format(E_DM9C74_DM2T_LOCNAME_ERROR, (CL_SYS_ERR *)0,
			ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0,
			&local_error, (i4)4,
			sizeof(DB_DB_NAME), t->tcb_dcb_ptr->dcb_name.db_db_name,
			sizeof(DB_TAB_NAME), t->tcb_rel.relid.db_tab_name,
	...
	*/
	status = dm2r_rcb_allocate(dcb->dcb_att_tcb_ptr, (DMP_RCB *)0,
	    tran_id, lock_id, log_id, &att_rcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	for (;;)
	{
	    ...
	    ** reltotwid < relwid
	    */
	    if (( att_rcb->rcb_tcb_ptr->tcb_rel.reltotwid <
		att_rcb->rcb_tcb_ptr->tcb_rel.relwid) &&
	    ...
	    ** reltotwid < relwid
	    */
	    if (( att_rcb->rcb_tcb_ptr->tcb_rel.reltid.db_tab_base == 3 ) &&
		( att_rcb->rcb_tcb_ptr->tcb_rel.reltotwid <
		att_rcb->rcb_tcb_ptr->tcb_rel.relwid) )
	    {
		attribute.attintl_id = attribute.attid;
		attribute.attver_added = 0;
	    ...
	    */
		...
		(attribute.attid <= 0))
	    {
		ule_format(E_DM00A8_BAD_ATT_ENTRY, 
		    (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0, (i4)0, 
		    (i4 *)0, &local_error, (i4)5, 0, attribute.attid,
		    sizeof(DB_OWN_NAME), t->tcb_rel.relowner.db_own_name,
		    sizeof(DB_TAB_NAME), t->tcb_rel.relid.db_tab_name,
		    sizeof(DB_DB_NAME), t->tcb_dcb_ptr->dcb_name.db_db_name,
		    0, t->tcb_rel.relatts);
		*err_code = E_DM9C59_DM2T_READ_TCB;
		status = E_DB_ERROR;
		break;
	    }
	    ...
	    {
		ule_format(E_DM00A9_BAD_ATT_COUNT, 
		    (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0, (i4)0, 
		    (i4 *)0, &local_error, (i4)5, 0, att_count, 
		    sizeof(DB_OWN_NAME), t->tcb_rel.relowner.db_own_name,
		    sizeof(DB_TAB_NAME), t->tcb_rel.relid.db_tab_name,
		    sizeof(DB_DB_NAME), t->tcb_dcb_ptr->dcb_name.db_db_name,
		    0, t->tcb_rel.relatts);
		*err_code = E_DM9C59_DM2T_READ_TCB;
	    ...
	    t->tcb_atts_ptr[attribute.attintl_id].name = attribute.attname;
	    t->tcb_atts_ptr[attribute.attintl_id].offset = attribute.attoff;
	    t->tcb_atts_ptr[attribute.attintl_id].type = attribute.attfmt;
	    t->tcb_atts_ptr[attribute.attintl_id].length = attribute.attfml;
	    t->tcb_atts_ptr[attribute.attintl_id].precision = attribute.attfmp;
	    t->tcb_atts_ptr[attribute.attintl_id].key = attribute.attkey;
	    t->tcb_atts_ptr[attribute.attintl_id].flag = attribute.attflag;
	    t->tcb_atts_ptr[attribute.attintl_id].key_offset = 0;

	    COPY_DEFAULT_ID( attribute.attDefaultID,
		t->tcb_atts_ptr[attribute.attintl_id].defaultID );

	    t->tcb_atts_ptr[attribute.attintl_id].defaultTuple = 0;
	    t->tcb_atts_ptr[attribute.attintl_id].collID =
	    				attribute.attcollID;
	    t->tcb_atts_ptr[attribute.attintl_id].replicated = FALSE;
	    t->tcb_atts_ptr[attribute.attintl_id].rep_key_seq = 0;
	    t->tcb_atts_ptr[attribute.attintl_id].intl_id =
	    				attribute.attintl_id;
	    t->tcb_atts_ptr[attribute.attintl_id].ver_added =
	    				attribute.attver_added;
	    t->tcb_atts_ptr[attribute.attintl_id].ver_dropped =
	    				attribute.attver_dropped;
	    t->tcb_atts_ptr[attribute.attintl_id].val_from =
	    				attribute.attval_from;
	    t->tcb_atts_ptr[attribute.attintl_id].ordinal_id =
	    				attribute.attid;
	    t->tcb_atts_ptr[attribute.attintl_id].ver_altcol =
	    				attribute.attver_altcol;
	    t->tcb_atts_ptr[attribute.attintl_id].collID =
	    				attribute.attcollID;
	    ...
	    ** tcb_key_atts lists are indexed from 0, not from 1.
	    */
	    t->tcb_data_atts[attribute.attintl_id - 1] =
			&(t->tcb_atts_ptr[attribute.attintl_id]);
	    ...
	    */
		...
		{
	    	    ule_format(E_DM00AA_BAD_ATT_KEY_COUNT, 
			(CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0, 
			(i4)0, (i4 *)0, &local_error, (i4)6, 
			sizeof(DB_OWN_NAME), t->tcb_rel.relowner.db_own_name,
			sizeof(DB_TAB_NAME), t->tcb_rel.relid.db_tab_name,
			sizeof(DB_DB_NAME), t->tcb_dcb_ptr->dcb_name.db_db_name,
		...
		t->tcb_key_atts[attribute.attkey - 1] =
			      &t->tcb_atts_ptr[attribute.attintl_id];
		t->tcb_klen += attribute.attfml;
	    }

	    if (attribute.attflag & ATT_SEC_KEY)
		t->tcb_sec_key_att = &(t->tcb_atts_ptr[attribute.attintl_id]);
	...
	*/
	status = dm2r_release_rcb(&att_rcb, err_code);
	if (status != E_DB_OK)
	    break;
	att_rcb = 0;
	...
	*/
	...
	/*
	** Now we can compute the compression expansion factors for table
	** and key attributes.  This gives us the worst-case overhead which
	** might be added by the compression routines on the tuple or its key.
	*/
	t->tcb_comp_atts_count =
	    (*t->tcb_acc_tlv->dmpp_compexpand)(t->tcb_data_atts, att_count);
	t->tcb_comp_katts_count =
	    (*t->tcb_acc_tlv->dmppxcompexpand)(t->tcb_key_atts, t->tcb_keys);
	...
	*/
	if ( t->tcb_rel.relspec == TCB_BTREE )
	{
		...
		if ( t->tcb_rel.relstat & TCB_CLUSTERED )
		{
		    ...
		    /* Recompute katts_count for all atts in leaf */
		    t->tcb_comp_katts_count =
			(*t->tcb_acc_tlv->dmppxcompexpand)(t->tcb_leafatts, t->tcb_leafattcnt);
		}
		else
		{
		    /* For nonclustered primary btree, att/keys are same on LEAF/INDEX pages */
	    ...
	    t->tcb_kperpage = 
		dm1b_kperpage(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
		t->tcb_rel.relcmptlvl, t->tcb_ixattcnt, t->tcb_ixklen, DM1B_PINDEX,
		sizeof(DM_TID), t->tcb_rel.relstat & TCB_CLUSTERED);

	    /* Global SI will have TID8-sized tids on LEAF pages */
	    t->tcb_kperleaf = 
		dm1b_kperpage(t->tcb_rel.relpgtype, t->tcb_rel.relpgsize,
		t->tcb_rel.relcmptlvl, t->tcb_leafattcnt, t->tcb_klen, DM1B_PLEAF,
    ...
    if (dev_rcb)
    {
	local_status = dm2t_close(dev_rcb, 0, &local_error);
	if (local_status)
	{
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
    ...
    if (att_rcb)
    {
	local_status = dm2r_release_rcb(&att_rcb, &local_error);
	if (local_status)
	{
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
    ...
    if (*err_code > E_DM_INTERNAL && *err_code != E_DM9C59_DM2T_READ_TCB)
    {
...
i4		*err_code)
{
	...
	*/
	status = dm2r_rcb_allocate(dcb->dcb_idx_tcb_ptr, (DMP_RCB *)0,
	    tran_id, lock_id, log_id, &idx_rcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	for (;;)
	{
	    ...
	    */
		...
		{
		    /* XXXX Add better error message */
		    *err_code = E_DM9334_DM2T_MISSING_IDX;
		    status = E_DB_ERROR;
		    break;
		}
	    }
	    ...
	    if (t->tcb_rel.relspec == TCB_RTREE)
	    {
	       ...
	       */
	       ...
	       if (rtree_rcb == 0)
	       {
			...
			DM2T_A_READ, (i4)0, (i4)20, (i4)0,
			log_id, lock_id, (i4)0, (i4)0, DM2T_IS,
			tran_id, &timestamp, &rtree_rcb, (DML_SCB *)0, 
			err_code);
	         if (status != E_DB_OK)
		   break;
		 ...
		 &t->tcb_rel.reltid.db_tab_index;
	       ...
	       adf_cb = (ADF_CB *)rtree_rcb->rcb_adf_cb;
	       ...
	       if (status)
	       {
	          *err_code = E_DM9332_DM2T_IDX_DOMAIN_ERROR;
	          status = E_DB_ERROR;
	          break;
	       }
	      ...
	      status = dm2t_close(rtree_rcb, 0, err_code);
	      if (status != E_DB_OK)
	        break;
	      rtree_rcb = 0;
	    ...
	    */
	    if (t->tcb_rel.relstat & TCB_UNIQUE)
	    {
		base_tcb->tcb_index_type |= TCB_S_UNIQUE;
	...
	*/
	if (*index_list)
	{
	    ...
	    */
	    ...
	    {
		/* XXXX Add better error message */
		*err_code = E_DM9333_DM2T_EXTRA_IDX;
		status = E_DB_ERROR;
		break;
	    }
	}
	...
	*/
	status = dm2r_release_rcb(&idx_rcb, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
    if (idx_rcb)
    {
	local_status = dm2r_release_rcb(&idx_rcb, &local_error);
	if (local_status)
	{
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
    ...
    if (rtree_rcb)
    {
	local_status = dm2t_close(rtree_rcb, 0, &local_error);
	if (local_status)
	{
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
    ...
    if (*err_code > E_DM_INTERNAL)
    {
        ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
	*err_code = E_DM9C5A_DM2T_BUILD_INDX;
    }
...
i4		*err_code)
{
    ...
    CL_SYS_ERR       sys_err;
    STATUS	     stat;
    i4	lock_flags = tcb->tcb_table_io.tbio_cache_flags &
        TBIO_CACHE_MULTIPLE ? LK_PHYSICAL | LK_CONVERT :
        LK_PHYSICAL | LK_CONVERT | LK_LOCAL;
    ...
    else
    {
        stat = LKrequest( lock_flags,
			dmf_svcb->svcb_lock_list, (LK_LOCK_KEY *)0, LK_N,
	...
	ule_format(stat, &sys_err, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	ule_format(E_DM901C_BAD_LOCK_REQUEST, &sys_err, ULE_LOG, NULL,
	    0, 0, 0, err_code, 2, 0, LK_N, 0, dmf_svcb->svcb_lock_list);
	*err_code = E_DM9274_TBL_VERIFY;
	return (E_DB_ERROR);
    }

    if ((tcb->tcb_table_io.tbio_cache_valid_stamp != lockvalue.lk_low) ||
	(stat == LK_VAL_NOTVALID))
    {
...
i4		    *err_code)
{
    ...
    status = dm2r_rcb_allocate(t->tcb_dcb_ptr->dcb_rel_tcb_ptr, 0,
	&tran_id, lock_list, 0, &rcb, err_code);
    if (status != E_DB_OK)
	return (E_DB_OK);
	...
	*/
	if ((t->tcb_rel.relstat & TCB_IS_PARTITIONED) == 0)
	{
	    ...
	    */
		...
		TRdisplay("%@(dm2t)update_rel expected (%d,%d) at %d, got (%d,%d)\n",
		    ...
		    t->tcb_iirel_tid.tid_i4,
		    relrecord.reltid.db_tab_base, relrecord.reltid.db_tab_index);
		*err_code = E_DM9272_REL_UPDATE_ERROR;
		status = E_DB_ERROR;
		break;
	    }
    ...
    (void) dm2r_release_rcb(&rcb, err_code);
    ...
    */
    ...
    {
	ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	ule_format(E_DM9273_TBL_UPDATE, 0, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	*err_code = E_DB_OK;
    }
    return (E_DB_OK);
}
...
i4		*err_code)
{
	...
	*/
	if ((tabio->tbio_status & TBIO_OPEN) == 0)
	{
		...
		dm2f_flags, DM2F_TAB,
		&tabio->tbio_reltid, 0, 0, tabio, sync_flag, err_code);
	    if (status != E_DB_OK)
		break;
	}
	else
    ...
    */
    if (*err_code == E_DM0114_FILE_NOT_FOUND)
	*err_code = E_DM9291_DM2F_FILENOTFOUND;

    if (*err_code > E_DM_INTERNAL)
    {
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, (DB_SQLSTATE *)NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, (i4)0);
	*err_code = E_DM9C5B_DM2T_OPEN_TABIO;
    }
...
i4		*err_code)
{
    ...
    lock_flags = tcb->tcb_table_io.tbio_cache_flags & TBIO_CACHE_MULTIPLE ?
        LK_PHYSICAL : LK_PHYSICAL | LK_LOCAL;
    /*
    ** (ICL phil.p)
    ...
    */
	...
	if ( tcb->tcb_lkid_ptr->lk_unique == 0 &&
	     tcb->tcb_lkid_ptr->lk_common == 0 )
	{
            stat = LKrequestWithCallback( lock_flags,
        ...
        {
            stat = LKrequestWithCallback( lock_flags | LK_CONVERT,
                dmf_svcb->svcb_lock_list, (LK_LOCK_KEY *)0,
		lock_mode, (LK_VALUE * )&lockvalue, tcb->tcb_lkid_ptr, 0L,
		(LKcallback) dm2t_tcb_callback, (PTR) tcb, &sys_err);
        }
    }
    else
	...
	if (stat == LK_NOLOCKS)
	{
	    ...
	    ule_format(E_DM9046_TABLE_NOLOCKS, &sys_err, ULE_LOG,
		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
		err_code, 2,
	...
	{
	    ule_format(E_DM901C_BAD_LOCK_REQUEST, &sys_err, ULE_LOG,
		(DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
...
i4		*err_code)
{
    ...
    {
	ule_format(stat, &sys_err, ULE_LOG, NULL, 0, 0, 0, err_code, 0);
	ule_format(E_DM901B_BAD_LOCK_RELEASE, &sys_err, ULE_LOG, (DB_SQLSTATE*)NULL,
	    (char *)0, (i4)0, (i4 *)0, err_code, (i4)1,
	    0, dmf_svcb->svcb_lock_list);
	*err_code = E_DM926F_TBL_CACHE_CHANGE;
	status = E_DB_ERROR;
    }
...
**	referenced.
**
**	This routine operates without use of a TCB pointer so that operations
...
**
** Inputs:
**	tcb				Database Control Block
**	table_id			Pointer to the Table Id
**	lock_list			Lock List on which to request lock
**
** Outputs:
**      err_code
**
...
i4		*err_code)
{
    ...
    {
	stat = LKrequest(dcb->dcb_bm_served == DCB_SINGLE ?
                         LK_PHYSICAL | LK_NONINTR | LK_LOCAL :
                         LK_PHYSICAL | LK_NONINTR,
                         lock_list, &lockkey, LK_X, (LK_VALUE * )&lockvalue,
                         &lockid, 0L, &sys_err);
	...
	stat = LKrelease(0, lock_list, &lockid, 0,
		...
		i4		*err_code)
{
	...
	*/
	status = dm2r_rcb_allocate(dcb->dcb_rel_tcb_ptr, (DMP_RCB *)0,
	    tran_id, lock_id, log_id, &rel, err_code);
	if (status != E_DB_OK)
	{
	    check = 1;
	    break;
	}
	status = dm2r_rcb_allocate(dcb->dcb_relidx_tcb_ptr, (DMP_RCB *)0,
	    tran_id, lock_id, log_id, &r_idx, err_code);
	if (status != E_DB_OK)
	{
	    check = 2;
	    break;
	...
	** cleanup
	*/
	status = dm2r_release_rcb(&rel, err_code);
	if (status != E_DB_OK)
	{
	    check = 18;
	    break;
	...
	status = dm2r_release_rcb(&r_idx, err_code);
	if (status != E_DB_OK)
	{
	    check = 19;
	    break;
    ...
    */
    ...
    if (r_idx)
	local_stat = dm2r_release_rcb(&r_idx, err_code);

    if (rel)
	local_stat = dm2r_release_rcb(&rel, err_code);
		   ...
		   LK_LOCK_KEY  *lock_key)
{
    ...
    STATUS           cl_status = OK;
    i4          page_count = 0;
    i4          flags = DM2T_NOBUILD;
    i4          release_flags = DM2T_FLUSH;
    ...
    status = LKshow( LK_S_SLOCKQ, (i4 ) 0, lockid,
			(LK_LOCK_KEY *)0, sizeof(info_block),
    ...
    */
    ...
    /*
    ** Why are we being called if it isn't a LK_SV_TABLE lock
    */
    if ( info_block.lkb_key[0] != LK_SV_TABLE )
    {
	dmd_check( E_DM93F3_NOT_TCB_CACHE_LOCK );
    }

    dcb_id = info_block.lkb_key[1];
    reltid.db_tab_base = info_block.lkb_key[2];
    reltid.db_tab_index = 0;

    /*
    ** Check if the lkb has been reused or not
    */
    if (lock_key->lk_key1 != dcb_id ||
	lock_key->lk_key2 != reltid.db_tab_base)
	return(E_DB_WARN);
    ...
    for (;;)
    {
        ...
        status = dm2t_fix_tcb( dcb_id, &reltid, (DB_TRAN_ID *) 0,
            	dmf_svcb->svcb_lock_list, (i4 ) 0, flags,
            	(DMP_DCB *) 0, &newtcb, &err_code);
	...
	if (status != E_DB_OK)
        {
            ...
            {
                status = E_DB_OK;
                page_count = dm0p_count_pages( dcb_id, reltid.db_tab_base);
        ...
        */
        ...
        status = dm2t_unfix_tcb(&newtcb, (i4) release_flags,
            dmf_svcb->svcb_lock_list, (i4)0, &err_code);
...
**                                svcb_lock_list, to the requested mode.
**
** Description:
**
**
...
i4		*err_code)
{
    ...
    if (tcb->tcb_dcb_ptr->dcb_status & DCB_S_DMCM)
    {
        cl_status = LKrequestWithCallback( lock_flags,
            dmf_svcb->svcb_lock_list, (LK_LOCK_KEY *)0,
            (i4) lock_mode, (LK_VALUE * )&lockvalue, tcb->tcb_lkid_ptr, 
    ...
    else
    {
        cl_status = LKrequest( lock_flags,
            dmf_svcb->svcb_lock_list, (LK_LOCK_KEY *)0, (i4) lock_mode,
            (LK_VALUE * )&lockvalue, tcb->tcb_lkid_ptr, 
	    0L, &sys_err);
...
/*
** dm2t_xlock_tcb - request and release an exclusive SV_TABLE lock.
**
** Description :-
** 	    As a consequence of the Distributed Multi-Cache Management
** 	(DMCM) protocol, dirty pages are left in cache at the end of a
** 	transaction, even when running in a multi-cache environment.
** 	Certain algorithms, such as a Smart Disk scan, require all dirty
** 	pages to be written out before they progress because they bypass the
**	buffer manager. We therefore must provide a mechanism for signalling
**	to other caches, that all dirty pages for a particular table must be
**	written out before one of these operations begins.
** 	The  mechanism to enable this is that of TCB callbacks. When a
**	contesting lock request is made on a TCB, its callback is triggered
**	and all its dirty pages are flushed.
** 	This routine is used - by algorithms such as the Smart Disk scan - to
** 	cause a contending lock request for the TCB of the table to be scanned.
** 	Hence before the Scan proceeds, all dirty pages for the table are
**	written to disk.
**
** History :-
**	01-aug-1996 (nanpr01 for ICL phil.p)
...
**	    (DMCM) protocol.
**	01-aug-1996 (nanpr01)
**	    Copied the lockid if new lock is requested. Also deleted the
**	    LK_TEMPORARY flag from the LKrequestWithCallback calls.
**  29-Sep-2004 (fanch01)
**      Conditionally add LK_LOCAL flag if database/table is confined to one node.
*/
DB_STATUS
...
i4		*err_code)
{
	...
	{
            stat = LKrequestWithCallback( lock_flags,
                dmf_svcb->svcb_lock_list, &lockkey, LK_X,
                (LK_VALUE * )&lockvalue, tcb->tcb_lkid_ptr, 0L,
		(LKcallback) dm2t_tcb_callback, (PTR) tcb, &sys_err);
	}
        else
        {
            stat = LKrequestWithCallback( lock_flags | LK_CONVERT,
                dmf_svcb->svcb_lock_list, (LK_LOCK_KEY *)0, LK_X,
                (LK_VALUE * )&lockvalue, tcb->tcb_lkid_ptr, 0L,
		(LKcallback) dm2t_tcb_callback, (PTR) tcb, &sys_err);
	...
	if (stat != OK && stat != LK_VAL_NOTVALID)
	{
	    ...
	    *err_code = E_DM926F_TBL_CACHE_CHANGE;
        ...
        /*
        ** Set state to indicate that should transaction abort
        ** now occur, dm2t_close should convert the exclusive
        ** lock down to shared.
        */
        rcb->rcb_state |= RCB_RECOVER;
...
/*{
** Name: dm2t_convert_bumplock_tcb      - Bump the TCB cache lock value.
**
** Description:
**	This routine increments the TCB lock value in order to invalidate
**	cached TCBs in other servers.
...
** History:
**	01-aug-1996 (nanpr01)
**	    Created as part of DMCM project. We cannot call convert to
**	    drop the TCB cache lock to NULL and then acquire X lock
**	    to bump the value. This opens window of inconsistency.
**  29-Sep-2004 (fanch01)
**      Conditionally add LK_LOCAL flag if database/table is confined to one node.
*/
DB_STATUS
dm2t_convert_bumplock_tcb(
DMP_TCB         *tcb,
i4         lock_mode,
i4         *err_code)
{
    ...
    LK_VALUE	lockvalue;
    CL_SYS_ERR	sys_err;
    i4	lock_flags = tcb->tcb_table_io.tbio_cache_flags & TBIO_CACHE_MULTIPLE ?
	...
	char	*tabname)
{
    ...
    for (i=0; Rep_cats[i] != 0; i++)
    {
	if (!MEcmp(tabname, Rep_cats[i], DB_MAXNAME)) /* names match */
	    return(TRUE);
    }
    return(FALSE);
}
...
i4		*err_code)
{
    ...
    */
	...
	if ( (rel->relstat2 & TCB2_PARTITION) == 0 )
	{
	    ...
	    if (rel->relkeys)
	    {
		atts_key_size = (rel->relkeys + 1) * sizeof(i2);
		key_atts_size = (rel->relkeys * sizeof(DB_ATTS *));
		/* Add an amount for rounding up */
		key_atts_size += sizeof(PTR);
		...
		**
		** The keys for leaf/index pages are the same except that 
		** offset/key_offset of the tidp entry will differ for new indexes
		** defined with non key fields.
		**
		** However note, for old-style (pre Ingres 2.5), index page entries
		** are the same as leaf page entries. That is, index page entries
		** include non-key fields.
		**
		** If CLUSTERED Btree, we need separate copies of key DB_ATTS,
		** one for the Leaf pages, and one for the Index pages.
		** The Leaf attributes will have key_offset set to the 
		...
		*/
		if ( rel->relspec == TCB_BTREE )
		{
			...
			*/
			bt_key_atts_size = key_atts_size + sizeof(DB_ATTS);
		    }
		    else if ( rel->relstat & TCB_CLUSTERED )
		    {
			/* 
			** Allocate another array for a separate INDEX
			** key array and another set of key DB_ATTS.
			*/
			bt_key_atts_size = key_atts_size;
	...
	if (rel->relspec == TCB_RTREE)
	    rtree_size = (i4) ME_ALIGN_MACRO(sizeof(DMPP_ACC_KLV), 
		sizeof(ALIGN_RESTRICT))  +  /* round to f8 boundary */
		DB_MAXRTREE_DIM * sizeof(f8) * 2;
    ...
    */
	...
	next_slot += atts_size;
	t->tcb_data_atts = (DB_ATTS**)next_slot;
	next_slot += data_atts_size;
    }
    else
    {
	t->tcb_atts_ptr = (DB_ATTS*)NULL;
	t->tcb_data_atts = (DB_ATTS**)NULL;
    }

    t->tcb_leafatts = (DB_ATTS **)NULL;
    t->tcb_ixatts = (DB_ATTS **)NULL;
    t->tcb_leafattcnt = 0;
    t->tcb_ixattcnt = 0;
    ...
    t->tcb_rngkeys = (DB_ATTS **)NULL;
    t->tcb_rngatts = (DB_ATTS **)NULL;
    t->tcb_rngklen = 0;
    t->tcb_rngattcnt = 0;
	...
	*/
    ...
    t->tcb_update_idx = FALSE;
    t->tcb_no_tids = FALSE;
    t->tcb_index_type = TCB_S_NONUNIQUE;
    ...
    t->tcb_kperleaf = 0;
    t->tcb_sec_key_att = 0;
    t->tcb_comp_atts_count = 0;
    t->tcb_comp_katts_count = 0;
    t->tcb_table_type = 0;
    t->tcb_acc_plv = 0;
    t->tcb_acc_tlv = 0;
    t->tcb_rep_info = 0;
	...
	*/
	...
	/*
	** Fill in the tcb accessor lists for this table.
	** These give the routines to call for various page and tuple access
	** actions and are dependent upon the page and compression type.
	*/
	dm1c_getaccessors(t->tcb_rel.relcomptype, t->tcb_rel.relpgtype,
			 t->tcb_rel.relpgsize,
			 &t->tcb_acc_plv, &t->tcb_acc_tlv);
    }
    else
    {
...
** Name:	dm2t_row_lock
**
** Description: Check if row locking supported for a table
**
** Inputs:
**      tcb             - pointer to the TCB.
**
...
DMP_TCB         *t)
{
    ...
    /*
    ** Btree secondary-indices can include data in the key which
    ** is not treated as a keyed column.  It is carried around
    ** in the index, but is never used for key comparisons.
    **
    ** Since these non-key fields are carried around in the
    ** index, they are compressed along with other keyed fields
    ** if key compression is specified.  So for btree secondaries
    ** we must adjust our key compression count to include all
    ** columns of the index, not just the keyed ones.
    **
    ** NOTE this is really the compression adjustment for LEAF pages,
    ** where non-key fields always exist.
    ** Index pages may or may not have non-index fields, depending on
    ** relcmptlvl. However for simplicity.... just always use the leaf
    ** compression adjustment.
    ** FIX ME, we really should be compressing the key and calling 
    ** dm1cxhas_room with the compressed keylen, not the maximum compressed
    ** keylen, which can result in unecessary index/leaf page splits.
    */
    t->tcb_comp_katts_count =
	(*t->tcb_acc_tlv->dmppxcompexpand)(t->tcb_data_atts, t->tcb_rel.relatts);
    ...
    /* For old-style index, att/keys are same on LEAF/INDEX pages */
    ...
    {
	t->tcb_ixatts = t->tcb_data_atts;  /* index atts = leaf atts */
	t->tcb_ixattcnt = t->tcb_rel.relatts;
	...
	** index or a new-style (post release 6) one.
	**
	...
	*/
	...
	/* We may have to build different index att/key arrays */

	/*
	** This says "if all attributes are keys OR there are non-key
	** attributes and the TID is not part of the key"
	*/
	if (t->tcb_rel.relatts == t->tcb_keys ||
	    t->tcb_atts_ptr[t->tcb_rel.relatts].key == 0)
	{
	    /* Leaf/index key offsets are the same */

	    /* 
	    ** After DMF_S8_VERSION, non-key fields are excluded
	    ** from LRANGE/RRANGE leaf entries and range entries
	    ** are read and updated using ixatts, ixattcnt, ixklen,
	    ** not leafatts, leafattcnt, and klen.
	    */
	    if ( MEcmp((char*)&t->tcb_rel.relcmptlvl, 
				DMF_S8_VERSION,
				sizeof(t->tcb_rel.relcmptlvl)) < 0 )
		t->tcb_ixatts = t->tcb_data_atts;  /* index atts = key+non-key  */
	    else
		t->tcb_ixatts = t->tcb_key_atts;  /* index atts = keys only */
	    t->tcb_ixkeys = t->tcb_key_atts;   /* index keys = leaf keys */
	...
	else
	{
	    ...
	    */
	    ...
	    t->tcb_ixatts = t->tcb_ixkeys; /* index atts = index keys */
	    t->tcb_ixklen = ixkeys[tidp_i]->key_offset + 
				ixkeys[tidp_i]->length;
	}
    }

    if (MEcmp((PTR)&t->tcb_rel.relcmptlvl, (char *)DMF_S8_VERSION,
		sizeof(t->tcb_rel.relcmptlvl)) >= 0)
    {
	/* New style secondary LEAF range entries just have keys */
	t->tcb_rngkeys = t->tcb_ixkeys;
	t->tcb_rngatts = t->tcb_ixatts;
	t->tcb_rngklen = t->tcb_ixklen;
	t->tcb_rngattcnt = t->tcb_ixattcnt;
	...
	t->tcb_rngatts = t->tcb_leafatts;
	t->tcb_rngklen = t->tcb_klen;
	t->tcb_rngattcnt = t->tcb_leafattcnt;
    }
	...
	TRdisplay("LEAF atts %d entrylen %d keys %d\n", 
		    t->tcb_leafattcnt, t->tcb_klen, t->tcb_keys);
	for (i = 0; i < t->tcb_leafattcnt; i++)
	    TRdisplay("ATT %t (%d %d)\n", sizeof(DB_ATT_NAME),
	    &t->tcb_leafatts[i]->name.db_att_name,
	    t->tcb_leafatts[i]->offset, t->tcb_leafatts[i]->key_offset);
	...
	TRdisplay("INDEX atts %d entrylen %d, keys %d\n",
		    t->tcb_ixattcnt, t->tcb_ixklen, t->tcb_keys);
	for (i = 0; i < t->tcb_ixattcnt; i++)
	    TRdisplay("ATT %t (%d %d)\n", sizeof(DB_ATT_NAME),
	    &t->tcb_ixatts[i]->name.db_att_name,
	    t->tcb_ixatts[i]->offset, t->tcb_ixatts[i]->key_offset);
	for (i = 0; i < t->tcb_keys; i++)
	    TRdisplay("KEY %t (%d %d)\n", sizeof(DB_ATT_NAME),
...
DMT_CB		*dmt)
{
    ...
    **
    ** As we're running under the check_term thread,
    ** we don't have a lock list or log id, but
    ** we're careful not to do anything that might
    ** need either, fingers crossed.
    */
	...
	** by some sort of DMU or Alter statement run by another server
	*/
	if ( (verify_tcb(t, &error)) == E_DB_INFO )
	{
	    /*
	    ** Release this invalid TCB.  Specify to toss any cached pages
	    ** for this table as they are probably invalid as well.
	    */
	    (VOID) dm2t_release_tcb(&t, DM2T_TOSS, 
				    (i4)0,		/* lock_list */ 
				    (i4)0,		/* log_id */ 
				    &error);
	}
