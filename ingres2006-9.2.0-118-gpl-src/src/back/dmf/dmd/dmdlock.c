...
NOINTERRUPT,SHARED,PSHARED,ENQWAIT,G_DEADLOCK_SEARCH,STALL_ENQ,MASTER_CSP,\
ON_DLOCKQ,DEADLOCK,XSVDB,XROW,MULTITHREAD",
                llb->llb_status);

	    TRdisplay("\n");
		
	    if (llb->llb_status & LLB_INFO_EWAIT)
	    ...
	    */
		...
		if (cluster)
		{
		    ...
		    TRdisplay("lk_attr: %v ",
			"PHYS,XbadX,CANCEL,ENQWAIT,CONVCANCEL,PROPAG,ON_DLOCK_L,DLOCKED,ON_LLB_W,ON_CBACK_L,DISTRIB_LOCK,RSB_MUTEXED,XbadX,XbadX,XbadX,IS_RSBS",
				lkb->lkb_flags);
		}
		else
		{
		...
		*/
		if (lkb->lkb_grant_mode != lkb->lkb_request_mode &&
		    (lkb->lkb_pid != llb->llb_pid || lkb->lkb_sid != llb->llb_sid))
		{
	...
	for (context = 0;;)
	{
		...
		rsb->rsb_id,
		"N,IS,IX,S,SIX,U,X", rsb->rsb_grant,
		"N,IS,IX,S,SIX,U,X", rsb->rsb_convert,
                rsb->rsb_cback_count,
