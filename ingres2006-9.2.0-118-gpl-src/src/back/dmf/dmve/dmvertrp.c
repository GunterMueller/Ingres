...
DMVE_CB		*dmve)
{
	...
	*/
	if ((log_rec->rtr_header.type != DM0LRTREP) ||
	    (log_rec->rtr_header.length != (i4)
		(sizeof(DM0L_RTREP) - 
			(DB_MAXRTREE_KEY - log_rec->rtr_okey_size) -
			(DB_MAXRTREE_KEY - log_rec->rtr_nkey_size) -
			(RCB_MAX_RTREE_LEVEL * sizeof(DM_TID) -
					log_rec->rtr_stack_size) -
			(DB_MAXNAME - log_rec->rtr_tab_size) -
			(DB_MAXNAME - log_rec->rtr_own_size))))
	{
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->rtr_tbl_id, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->rtr_pg_type, 
		log_rec->rtr_page_size, &loc_plv, &loc_tlv);
	...
	*/
	...
	case DMVE_REDO:
          if (LSN_GTE(
		DM1B_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->rtr_pg_type, page), 
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->rtr_pg_type, page),
			0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtr_pg_type, 
			    page),
			0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->rtr_pg_type,
			    page), 
			0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->rtr_pg_type,
			    page),
	...
	case DMVE_UNDO:
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    0, DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->rtr_pg_type, page),
		    0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtr_pg_type, page),
		    0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->rtr_pg_type, 
			page), 
		    0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->rtr_pg_type,
			page),
	...
	*/
	if (page)
	{
	    ...
	    if (log_rec->rtr_header.flags & DM0L_CLR)
            ...
            status = dmv_rertree_rep(dmve, tbio, page, &leaf_bid, log_rec, 
					loc_plv, recovery_action, &error);
...
i4             *err_code)
{
	...
	*/
	status = dm1cxput(log_rec->rtr_pg_type, log_rec->rtr_page_size, page,
			log_rec->rtr_cmp_type, DM1C_DIRECT,
	...
	if (status != E_DB_OK)
	{
	  dm1cxlog_error(E_DM93E4_BAD_INDEX_PUT, (DMP_RCB *)0, page, 
			   log_rec->rtr_pg_type, log_rec->rtr_page_size, child);
	}

    /*
    ** Write the LSN of the Replace log record to the updated page.
    */
    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rtr_pg_type, page, *log_lsn);

    DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->rtr_pg_type, page, DMPP_MODIFY);
