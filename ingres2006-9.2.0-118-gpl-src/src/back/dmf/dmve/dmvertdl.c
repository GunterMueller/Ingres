...
DMVE_CB		*dmve)
{
	...
	*/
	if ((log_rec->rtd_header.type != DM0LRTDEL) ||
	    (log_rec->rtd_header.length != (i4)
		(sizeof(DM0L_RTDEL) - 
			(RCB_MAX_RTREE_LEVEL * sizeof(DM_TID) -
			log_rec->rtd_stack_size) -
			(DB_MAXRTREE_KEY - log_rec->rtd_key_size) -
			(DB_MAXNAME - log_rec->rtd_tab_size) -
			(DB_MAXNAME - log_rec->rtd_own_size))))
	{
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->rtd_tbl_id, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->rtd_pg_type, 
		log_rec->rtd_page_size, &loc_plv, &loc_tlv);
	...
	*/
	for (;;)
	{
	    ...
	    */
		...
		log_rec->rtd_page_size, (DMPP_PAGE *) page, loc_plv, "Page");
	    ...
	    */
	    if ( (requalify_needed) ||
	    	((dmve->dmve_action == DMVE_UNDO) &&
                 (LSN_GT(DM1B_VPT_ADDR_BT_SPLIT_LSN_MACRO(log_rec->rtd_pg_type,
		  page), log_lsn)) ) )
		...
		*/
	        if (requalify_needed == FALSE)
		{
		    ...
		    is_leaf = DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtd_pg_type, page)
			   & DMPP_LEAF;
		    if (is_leaf)
		...
		status = 
		  dm1m_check_range(log_rec->rtd_pg_type, log_rec->rtd_page_size,
		...
		if (in_range)
		{
		    ...
		    if (status != E_DB_OK && error != E_DM0056_NOPART)
		        break;
		    entry_found = TRUE;
		    status = E_DB_OK;
		    break;
		...
		{	  
		    next_page = 
			DM1B_VPT_GET_BT_NEXTPAGE_MACRO(log_rec->rtd_pg_type, 
							page);
		    if (next_page)
		    {
	        ...
	        ule_format(E_DM9671_DMVE_BTUNDO_FAILURE, (CL_ERR_DESC *)0,
		    	ULE_LOG, NULL, (char *)0, (i4)0, (i4)0,
			&error, 0);
	        error = E_DM9669_DMVE_BTUNDO_CHECK;
	        break;
	...
	*/
	...
	case DMVE_REDO:
          if (LSN_GTE(
		DM1B_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->rtd_pg_type, page), 
		log_lsn) && ((dmve->dmve_flags & DMVE_ROLLDB_BOPT) == 0))
	  {
		if (dmve->dmve_action == DMVE_DO)
		{
		  ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
			...
			DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->rtd_pg_type, page),
			0, 
			DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtd_pg_type, page),
			0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->rtd_pg_type,
			    page), 
			0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->rtd_pg_type,
			    page),
	...
	case DMVE_UNDO:
		...
		log_lsn))
	  {
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    0, DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->rtd_pg_type, page),
		    0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtd_pg_type, page),
		    0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->rtd_pg_type, 
			page), 
		    0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->rtd_pg_type,page),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
	...
	*/
	if (page)
	{
	    ...
	    switch (recovery_action)
	    {
	      case DMVE_DO:
	      case DMVE_REDO:
	        status = dmv_rertree_del(dmve, &adf_scb, tbio, page,
			 &leaf_bid, log_rec, loc_plv, &loc_klv, &error);
	        break;

	      case DMVE_UNDO:
	        status = dmv_unrtree_del(dmve, &adf_scb, tbio, page,
			 &leaf_bid, log_rec, loc_plv, &loc_klv, &error);
	      break;
	    }
...
i4             *err_code)
{
    ...
    */
    ...
    index_update = ((DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtd_pg_type, page) & 
	DMPP_INDEX) != 0);
    ...
    */
    ...
    dm1cxrecptr(log_rec->rtd_pg_type, log_rec->rtd_page_size, page, childkey, &key_ptr);
    dm1cxtget(log_rec->rtd_pg_type, log_rec->rtd_page_size, page, childtid, 
		&deltid, (i4*)NULL);
    ...
    */
    ...
    if (log_rec->rtd_cmp_type != DM1CX_UNCOMPRESSED)
    {
	dm1cx_klen(log_rec->rtd_pg_type, log_rec->rtd_page_size, page, 
		childkey, &key_len);
    }
	...
	*/
	...
	{
	    dm1cxtget(log_rec->rtd_pg_type, log_rec->rtd_page_size, 
		page, childkey, &deltid, (i4*)NULL);
	    status = dm1cxtput(log_rec->rtd_pg_type, log_rec->rtd_page_size,
		page, childtid, &deltid, (i4)0);
	    if (status != E_DB_OK)
	    {
		dm1cxlog_error(E_DM93EB_BAD_INDEX_TPUT, (DMP_RCB *)0, page, 
			log_rec->rtd_pg_type, log_rec->rtd_page_size, childtid);
		break;
	    }
	}

	status = dm1cxdel(log_rec->rtd_pg_type, log_rec->rtd_page_size, page,
		    DM1C_DIRECT, log_rec->rtd_cmp_type,
		    &dmve->dmve_tran_id, (i4)0, (i4)DM1CX_RECLAIM, childkey);
	if (status != E_DB_OK)
	{
	    dm1cxlog_error(E_DM93E2_BAD_INDEX_DEL, (DMP_RCB*)0, page, 
		       log_rec->rtd_pg_type, log_rec->rtd_page_size, childkey);
	    break;
	}
    ...
    */
    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rtd_pg_type, page, *log_lsn);

    DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->rtd_pg_type, page, DMPP_MODIFY);
...
i4             *err_code)
{
    ...
    */
    loc_id = DM2F_LOCID_MACRO(tabio->tbio_loc_count, 
	      (i4) DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->rtd_pg_type, page));
    ...
    */
    if (dmve->dmve_logging)
	  DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rtd_pg_type, page, lsn);
	...
	*/
	status = dm1cxallocate(log_rec->rtd_pg_type, log_rec->rtd_page_size,
			page, DM1C_DIRECT, log_rec->rtd_cmp_type, 
	...
	*/
	status = dm1cxput(log_rec->rtd_pg_type, log_rec->rtd_page_size, 
			page, log_rec->rtd_cmp_type, DM1C_DIRECT, 
			&dmve->dmve_tran_id, (i4)0, childkey,
