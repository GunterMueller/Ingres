...
DMVE_CB		*dmve)
{
	...
	*/
	if ((log_rec->rtp_header.type != DM0LRTPUT) ||
	    (log_rec->rtp_header.length !=  (i4)(sizeof(DM0L_RTPUT) - 
			(DB_MAXRTREE_KEY - log_rec->rtp_key_size) -
			(RCB_MAX_RTREE_LEVEL * sizeof(DM_TID) - 
				log_rec->rtp_stack_size) -
			(DB_MAXNAME - log_rec->rtp_tab_size) -
			(DB_MAXNAME - log_rec->rtp_own_size))))
        {
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->rtp_tbl_id, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->rtp_pg_type, 
		log_rec->rtp_page_size, &loc_plv, &loc_tlv);
	...
	*/
	...
	for (;;)
	{
	    ...
	    */
	    ...
	    page = (DM1B_INDEX *)page_ptr;
    	    ...
    	    is_leaf = ((DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtp_pg_type, page) &
			DMPP_LEAF) != 0);
	    ...
	    */
		...
		log_rec->rtp_page_size, (DMPP_PAGE *) page, loc_plv, "Page");
	    ...
	    */
	    if ( (requalify_needed) ||
	    	 ((dmve->dmve_action == DMVE_UNDO) &&
	          (LSN_GT(DM1B_VPT_ADDR_BT_SPLIT_LSN_MACRO(log_rec->rtp_pg_type, page),
	         log_lsn)) ) )
	    {
	    	/*
		...
		*/
	    	for ( ;; )
		{
			...
			DM1B_VPT_GET_BT_KIDS_MACRO(log_rec->rtp_pg_type, page))
		    {
		        dm1cxtget(log_rec->rtp_pg_type, log_rec->rtp_page_size,
		...
		{
		    next_page = 
			DM1B_VPT_GET_BT_NEXTPAGE_MACRO(log_rec->rtp_pg_type, 
						page);
		    if ( next_page)
		    {
			...
			status = dm0p_uncache_fix(tbio, DM0P_UNFIX,
				dmve->dmve_lk_id, dmve->dmve_log_id,
	    	 		&dmve->dmve_tran_id, &page_ptr, &error);
	  		if (status != E_DB_OK)
	    		    break;
	...
	if ( entry_found == FALSE)
	{
	    ...
	    ule_format(E_DM9671_DMVE_BTUNDO_FAILURE, (CL_ERR_DESC *)0,
			ULE_LOG, NULL, (char *)0, (i4)0, (i4*)0,
			&error, 0);
	...
	*/
	switch (dmve->dmve_action)
	{
	  ...
	  case DMVE_REDO:
	    if (LSN_GTE(
			DM1B_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->rtp_pg_type,  
			page), log_lsn) && ((dmve->dmve_flags & DMVE_ROLLDB_BOPT) == 0))
	    {
	       if (dmve->dmve_action == DMVE_DO)
	       {
	            ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
	      		...
	      		DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->rtp_pg_type, page),
	      		0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtp_pg_type,	
	      		page),
	      		0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->rtp_pg_type,	
	      		page), 
	      		0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->rtp_pg_type,
	      		page),
	...
	case DMVE_UNDO:
      	  if (LSN_LT(
	         DM1B_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->rtp_pg_type, page), 
	         log_lsn))
	  {
	    ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
	    	ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
	      	...
	      	sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
	       	0, DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->rtp_pg_type, page),
	       	0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtp_pg_type, page),
	       	0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->rtp_pg_type, 
	    	page), 
	       	0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->rtp_pg_type,
	    	page),
	...
	*/
	...
	{  
	    ...
	    switch (recovery_action)
	    {
	        case DMVE_DO:
	        case DMVE_REDO:
	          status = dmv_rertree_put(dmve, &adf_scb, tbio, page,
		  	 &index_bid, log_rec,  loc_plv, &loc_klv, &error);
	          break;

	        case DMVE_UNDO:
	          status = dmv_unrtree_put(dmve, &adf_scb, tbio, page,
		  	 &index_bid, log_rec, loc_plv, &loc_klv, &error);
	          break;
	    }
	  ...
	  */
	           ...
	           is_leaf)
	      {
	          ...
	          status = dmve_rtadjust_mbrs(dmve, &adf_scb, tbio,
				...
				(DMPP_PAGE *)page, loc_plv, &loc_klv,
				fix_action, &error);
	          if (status != E_DB_OK)
	             break;
	      }
    ...
    */
    if (page_ptr)
    {
     	tmp_status = dm0p_uncache_fix(tbio, DM0P_UNFIX, dmve->dmve_lk_id, 
    			dmve->dmve_log_id, &dmve->dmve_tran_id, &page_ptr,
		 	&loc_error);
...
i4             *err_code)
{
    ...
    */
    ...
    index_update = ((DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->rtp_pg_type, page) & 
	                 DMPP_INDEX) != 0);
	...
	*/
	status = dm1cxallocate(log_rec->rtp_pg_type, log_rec->rtp_page_size,
			page, DM1C_DIRECT, log_rec->rtp_cmp_type, 
			&dmve->dmve_tran_id, (i4)0, child,
			log_rec->rtp_key_size + DM1B_TID_S);
	if (status != E_DB_OK)
	{
	    dm1cxlog_error(E_DM93E0_BAD_INDEX_ALLOC, (DMP_RCB *)0, page, 
			   log_rec->rtp_pg_type, log_rec->rtp_page_size, child);
	    break;
	}
	...
	*/
	status = dm1cxput(log_rec->rtp_pg_type, log_rec->rtp_page_size, page,
			log_rec->rtp_cmp_type, DM1C_DIRECT, 
			&dmve->dmve_tran_id, (i4)0, child,
	...
	*/
	if (index_update)
	{
	    status = dm1cxtput(log_rec->rtp_pg_type, log_rec->rtp_page_size,
			page, child + 1, &log_rec->rtp_tid, (i4)0);
	    if (status != E_DB_OK)
	    {
		dm1cxlog_error(E_DM93EB_BAD_INDEX_TPUT, (DMP_RCB *)0, page,
			log_rec->rtp_pg_type, log_rec->rtp_page_size, child + 1);
		break;
	    }
	    status = dm1cxtput(log_rec->rtp_pg_type, log_rec->rtp_page_size,
    ...
    */
    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rtp_pg_type, page, *log_lsn);

    DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->rtp_pg_type, page, DMPP_MODIFY);
...
i4             *err_code)
{
    ...
    */
	...
	(i4) DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->rtp_pg_type, page));
    loc_config_id = tabio->tbio_location_array[loc_id].loc_config_id;

    /*
    ** Lookup the key/tid pair to delete.
    */
    dm1cxrecptr(log_rec->rtp_pg_type, log_rec->rtp_page_size, 
		page, child, &key_ptr);
    dm1cxtget(log_rec->rtp_pg_type, log_rec->rtp_page_size, 
    ...
    */
    ...
    if (log_rec->rtp_cmp_type != DM1CX_UNCOMPRESSED)
    {
	  dm1cx_klen(log_rec->rtp_pg_type, log_rec->rtp_page_size, 
		page, child, &key_len);
    }
    ...
    */
    if (dmve->dmve_logging)
	DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->rtp_pg_type, page, lsn);
	...
	*/
	...
	status = dm1cxdel(log_rec->rtp_pg_type, log_rec->rtp_page_size, page,
			DM1C_DIRECT, log_rec->rtp_cmp_type,
			&dmve->dmve_tran_id, (i4)0,
...
i4             *err_code)
{
    DMP_DCB		*dcb = dmve->dmve_dcb_ptr;
    DB_STATUS		s = E_DB_OK;
    DM1B_INDEX		*childpage = (DM1B_INDEX *)page;
    DM1B_INDEX		*parpage = NULL;
    DM_TID	 	*tidp;
    DM_TID		childtid;
    DM_TID		origtid;
    DMPP_PAGE		**parent;
	...
	** Mutex the page while updating it
	*/
	dm0p_mutex(tbio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **)&childpage);

	s = dm1cxput(page_type, page_size, childpage,
			compression, DM1C_DIRECT, &dmve->dmve_tran_id,
	 		(i4)0, DM1B_RRANGE,
			rangekey, key_size, (DM_TID *)NULL, (i4)0);

	dm0p_unmutex(tbio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **)&childpage);
	...
	*/
	dm0p_mutex(tbio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **)&parpage);

	s = dm1cxput(page_type, page_size, parpage, 
			compression, DM1C_DIRECT, &dmve->dmve_tran_id,
			(i4)0, pos, rangekey, key_size, &childtid, (i4)0);

	dm0p_unmutex(tbio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **)&parpage);

	if (s)
	{
	    dm1cxlog_error(E_DM93E4_BAD_INDEX_PUT, NULL, parpage,
	...
	*/
	...
	else
	{
		...
		** at this level
	        */
	        s = E_DB_ERROR;	
	     	*err_code = E_DM9671_DMVE_BTUNDO_FAILURE;
	    }
	}
	     	 
	loc_s = dm0p_uncache_fix(tbio, DM0P_UNFIX,
	     		dmve->dmve_lk_id, dmve->dmve_log_id,
			&dmve->dmve_tran_id, parent, &loc_err_code);
