...
DMVE_CB		*dmve_cb)
{
    ...
    DM1P_FMAP		*fmap = NULL;
    DMPP_PAGE		*free = NULL;
    DM1P_FHDR		*fhdr_page = NULL;
    DM1P_FMAP		*fmap_page = NULL;
    DMPP_PAGE		*free_page = NULL;
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->all_tblid, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->all_pg_type, 
		log_rec->all_page_size, &loc_plv, &loc_tlv);
	...
	if ((dcb->dcb_status & DCB_S_EXCLUSIVE) == 0)
	{
		...
		** implicitly protected by the FHDR lock.
		...
		if (fhdr_page_recover || fmap_page_recover)
		{
			...
			&fhdr_lockid, (i4 *)0, (LK_VALUE *)0, &error);
		    if (status != E_DB_OK)
			break;
		...
		*/
		if (free_page_recover)
		{
			...
			tbio->tbio_relid, &dmve->dmve_tran_id,
			&page_lockid, (i4 *)0, (LK_VALUE *)0, &error);
		    if (status != E_DB_OK)
			break;
	...
	*/
	...
	if (free_page_recover)
	{
	    ...
	    if (dmve->dmve_action != DMVE_UNDO)
		dm0p_pagetype(tbio, free_page, dmve->dmve_log_id, DMPP_DATA);
	}
	fhdr = fhdr_page;
	fmap = fmap_page;
	free = free_page;
	...
	*/
	if (DMZ_ASY_MACRO(15))
	{
	    dmve_trace_page_info(log_rec->all_page_size, log_rec->all_pg_type, 
	...
	*/
	...
	case DMVE_REDO:
            if (fhdr && LSN_GTE(
		DM1P_VPT_ADDR_FHDR_PG_LOGADDR_MACRO(log_rec->all_pg_type, fhdr), 
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->all_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_PAGE_STAT_MACRO(log_rec->all_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_LOGADDR_HI_MACRO(log_rec->all_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_LOGADDR_LOW_MACRO(log_rec->all_pg_type,
                            fhdr), 0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		}
		fhdr = NULL;
	    }
            if (fmap && LSN_GTE(
		DM1P_VPT_ADDR_FMAP_PG_LOGADDR_MACRO(log_rec->all_pg_type, fmap), 
		log_lsn))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			0, DM1P_VPT_GET_FMAP_PAGE_PAGE_MACRO(log_rec->all_pg_type,
			    fmap),
			0, DM1P_VPT_GET_FMAP_PAGE_STAT_MACRO(log_rec->all_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_LOGADDR_HI_MACRO(log_rec->all_pg_type,
			    fmap), 0, 
			DM1P_VPT_GET_FMAP_LOGADDR_LOW_MACRO(log_rec->all_pg_type,
			    fmap), 0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		}
		fmap = NULL;
	    }
            if (free && LSN_GTE(
		DMPP_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->all_pg_type, free), 
		log_lsn))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			sizeof(*tbio->tbio_relid), tbio->tbio_relid,
			sizeof(*tbio->tbio_relowner), tbio->tbio_relowner, 0, 
			DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->all_pg_type, free), 
			0, 
			DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->all_pg_type, free),
			0, 
			DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->all_pg_type, 
			    free), 
			0, 
			DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->all_pg_type, 
			    free),
	...
	case DMVE_UNDO:
		...
		(LSN_LT(DM1P_VPT_ADDR_FHDR_PG_LOGADDR_MACRO(log_rec->all_pg_type, 
		fhdr), log_lsn)))
	    {
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ULE_LOG, NULL, (char *)0, (i4)0, (i4 *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->all_pg_type,
			fhdr), 
		    0, DM1P_VPT_GET_FHDR_PAGE_STAT_MACRO(log_rec->all_pg_type,
			fhdr),
		    0, DM1P_VPT_GET_FHDR_LOGADDR_HI_MACRO(log_rec->all_pg_type,
			fhdr), 
		    0, DM1P_VPT_GET_FHDR_LOGADDR_LOW_MACRO(log_rec->all_pg_type,
			fhdr),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    0, DM1P_VPT_GET_FMAP_PAGE_STAT_MACRO(log_rec->all_pg_type,
			fmap),
		    0, DM1P_VPT_GET_FMAP_LOGADDR_HI_MACRO(log_rec->all_pg_type,
			fmap), 
		    0, DM1P_VPT_GET_FMAP_LOGADDR_LOW_MACRO(log_rec->all_pg_type,
			fmap),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		error = E_DM9666_PAGE_LSN_MISMATCH;
		status = E_DB_ERROR;
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->all_pg_type, free), 
		    0, DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->all_pg_type, free),
		    0, 
		    DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->all_pg_type, free), 
		    0, 
		    DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->all_pg_type, free),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		error = E_DM9666_PAGE_LSN_MISMATCH;
		status = E_DB_ERROR;
	    }
	...
	*/
	...
	case DMVE_DO:
	case DMVE_REDO:
	    status = dmv_realloc(dmve, tbio, fhdr, fmap, free, log_rec, 
		loc_plv, &error);
	    break;
	...
	case DMVE_UNDO:
	    ...
	    status = dmv_unalloc(dmve, tbio, fhdr, fmap, free, log_rec, 
		loc_plv, &error);
	    break;
	}
    ...
    */
	...
	tmp_status = dm0p_uncache_fix(tbio, DM0P_UNFIX, dmve->dmve_lk_id, 
		dmve->dmve_log_id, &dmve->dmve_tran_id, &free_page, &loc_error);
	...
	tmp_status = dm0p_unlock_page(dmve->dmve_lk_id, dcb, 
	    ...
	    tbio->tbio_relid, &dmve->dmve_tran_id,
	    &fhdr_lockid, (LK_VALUE *)0, &loc_error);
	if (tmp_status != E_DB_OK)
	{
	    ule_format(loc_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &loc_error, 0);
    ...
    if (status != E_DB_OK)
	dmve->dmve_error.err_code = E_DM9625_DMVE_ALLOC;
...
i4             *err_code)
{
    ...
    */
    ...
    free_bit = log_rec->all_free_pageno % 
		DM1P_FSEG_MACRO(log_rec->all_pg_type, log_rec->all_page_size);
    ...
    */
    ...
    {
	ule_format(E_DM9672_DMVE_ALLOC_WRONGMAP, (CL_ERR_DESC *)0, ULE_LOG,
	...
	*err_code = E_DM964D_REDO_ALLOC;
	return(E_DB_ERROR);
    }

    if (fmap && (BITMAP_TST_MACRO(
	DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->all_pg_type, fmap), free_bit) == 0))
    {
	ule_format(E_DM9673_DMVE_ALLOC_MAPSTATE, (CL_ERR_DESC *)0, ULE_LOG,
	    ...
	    0, (free ? DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->all_pg_type, free) :
		-1), 4, "FREE");
	dmd_log(1, log_rec, 4096);
	ule_format(E_DM964D_REDO_ALLOC, (CL_ERR_DESC *)0, ULE_LOG,
    ...
    */
    if (fhdr)
    {
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
	if (log_rec->all_fhdr_hwmap)
	    DM1P_VPT_SET_FHDR_HWMAP_MACRO(log_rec->all_pg_type, fhdr, 
		log_rec->all_map_index + 1);
	if (log_rec->all_fhdr_hint)
	    VPS_CLEAR_FREE_HINT_MACRO(log_rec->all_pg_type,
			(DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->all_pg_type, fhdr, 
			log_rec->all_map_index)));

	DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(log_rec->all_pg_type,fhdr,DMPP_MODIFY);
	DM1P_VPT_SET_FHDR_PG_LOGADDR_MACRO(log_rec->all_pg_type, fhdr, *log_lsn);
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
    }
    ...
    */
    if (fmap)
    {
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fmap);
	BITMAP_CLR_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->all_pg_type, fmap), 
	    free_bit);
	...
	*/
	if (free_bit >= 
		DM1P_VPT_GET_FMAP_FIRSTBIT_MACRO(log_rec->all_pg_type, fmap))
	    DM1P_VPT_SET_FMAP_FIRSTBIT_MACRO(log_rec->all_pg_type, 
		fmap, free_bit + 1);
	...
	*/
	if ((DM1P_VPT_GET_FMAP_HW_MARK_MACRO(log_rec->all_pg_type, fmap) ==
	      DM1P_FSEG_MACRO(log_rec->all_pg_type, log_rec->all_page_size)) ||
	    (free_bit > 
	      (i4)DM1P_VPT_GET_FMAP_HW_MARK_MACRO(log_rec->all_pg_type, fmap)))
	    DM1P_VPT_SET_FMAP_HW_MARK_MACRO(log_rec->all_pg_type, fmap, free_bit);

	DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(log_rec->all_pg_type,fmap,DMPP_MODIFY);
	DM1P_VPT_SET_FMAP_PG_LOGADDR_MACRO(log_rec->all_pg_type, fmap, *log_lsn);
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fmap);
    }
    ...
    */
    if (free)
    {
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, &free);
        (*loc_plv->dmpp_format)(log_rec->all_pg_type, log_rec->all_page_size,
				free, log_rec->all_free_pageno, DMPP_DATA,
				DM1C_ZERO_FILL);
        DMPP_VPT_SET_PAGE_TRAN_ID_MACRO(log_rec->all_pg_type, free, 
	    dmve->dmve_tran_id);
        DMPP_VPT_SET_PAGE_SEQNO_MACRO(log_rec->all_pg_type, free, 0);

        DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->all_pg_type, free, DMPP_MODIFY);
        DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->all_pg_type, free, *log_lsn);
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, &free);
    }
...
i4             *err_code)
{
    ...
    */
	...
	ule_format(E_DM9672_DMVE_ALLOC_WRONGMAP, (CL_ERR_DESC *)0, ULE_LOG,
	    ...
	    0, log_rec->all_map_index);
	dmd_log(1, log_rec, 4096);
	*err_code = E_DM964C_UNDO_ALLOC;
	return(E_DB_ERROR);
    }

    if (fmap && BITMAP_TST_MACRO(
	DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->all_pg_type, fmap), free_bit))
    {
	ule_format(E_DM9673_DMVE_ALLOC_MAPSTATE, (CL_ERR_DESC *)0, ULE_LOG,
	    NULL, (char *)0, (i4)0, (i4 *)0, err_code, 9,
	    sizeof(DB_DB_NAME), tabio->tbio_dbname->db_db_name,
		...
		-1), 4, "USED");
	dmd_log(1, log_rec, 4096);
	ule_format(E_DM964C_UNDO_ALLOC, (CL_ERR_DESC *)0, ULE_LOG,
	    NULL, (char *)0, (i4)0, (i4 *)0, err_code, 0);
    }
    ...
    */
    if (fhdr && 
	VPS_TEST_FREE_HINT_MACRO(log_rec->all_pg_type,
		(DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->all_pg_type, 
    ...
    */
    if (fhdr)
    {
	if (VPS_TEST_FREE_HINT_MACRO(log_rec->all_pg_type, 
		(DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->all_pg_type, fhdr,
			log_rec->all_map_index))) == 0)
	{
	    dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
	    VPS_SET_FREE_HINT_MACRO(log_rec->all_pg_type, 
		(DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->all_pg_type,
		fhdr, log_rec->all_map_index)));
	    DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(log_rec->all_pg_type, fhdr, 
		DMPP_MODIFY);
	    if (dmve->dmve_logging)
		DM1P_VPT_SET_FHDR_PG_LOGADDR_MACRO(log_rec->all_pg_type,fhdr,lsn);
	    dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
	}
    }
    ...
    */
    ...
    {
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fmap);
	BITMAP_SET_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->all_pg_type, fmap), 
	    free_bit);

	if (free_bit < 
	    (i4)DM1P_VPT_GET_FMAP_FIRSTBIT_MACRO(log_rec->all_pg_type, fmap))
	    DM1P_VPT_SET_FMAP_FIRSTBIT_MACRO(log_rec->all_pg_type,fmap,free_bit);

	DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(log_rec->all_pg_type,fmap,DMPP_MODIFY);
	if (dmve->dmve_logging)
	    DM1P_VPT_SET_FMAP_PG_LOGADDR_MACRO(log_rec->all_pg_type, fmap, lsn);
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fmap);
    }
    ...
    */
	...
	DMPP_VPT_REC_PAGE_LOG_ADDR_MACRO(log_rec->all_pg_type, save_lsn, free);
	...
	if (dmve->dmve_logging)
        {
	    DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->all_pg_type, free, lsn);
        }
        else
        {
            DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->all_pg_type, free, 
							save_lsn);
        }
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, &free);
    }
