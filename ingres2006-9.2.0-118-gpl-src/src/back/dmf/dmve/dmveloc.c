...
DMVE_CB		*dmve_cb)
{
	...
	case DMVE_UNDO:
	    ...
	    */
	    ...
	    status = dm0c_close(cnf, DM0C_UPDATE | DM0C_COPY, &error);
	    if (status != E_DB_OK)
		break;
	    config = 0;
            error = E_DB_OK;
	...
	case DMVE_DO:
		...
		error = E_DM007E_LOCATION_EXISTS;
		break;
	    }
	    ...
	    status = dm0c_close(cnf, DM0C_UPDATE, &error);
	    if (status != E_DB_OK)
		break;
	    config = 0;
            error = E_DB_OK;
	...
	ule_format(error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
	    (char *)0, (i4)0, (i4 *)0, &error, 0);
	dmve->dmve_error.err_code = E_DM9617_DMVE_LOCATION;
    }
    else
...
DMVE_CB		*dmve_cb)
{
	  ...
	  case DMVE_UNDO:
	    ...
	    */
		...
		status = dm0l_ext_alter(dmve->dmve_log_id, dm0l_flags, 
		    ...
		    &log_rec->ext_lname, log_lsn, &error);
		if (status != E_DB_OK)
		{
		    /* XXXX Better error message and continue after logging. */
		    TRdisplay(
	    ...
	    ** Open the configuration file. 
	    */
            status = dm0c_open(dcb, DM0C_NOLOCK, lock_list, &cnf, &error);
	    if (status != E_DB_OK)
		break;
	    config = cnf;
	    ...
	    */
		...
		error = E_DM92A0_DMVE_ALTER_UNDO;
		status = E_DB_ERROR;
		break;
	    }
	...
	case DMVE_DO:
	    ...
	    */
	    ...
	    status = dm2d_alter_db(&dm2d, &error);
