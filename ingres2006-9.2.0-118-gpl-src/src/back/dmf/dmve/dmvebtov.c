...
DMVE_CB		*dmve_cb)
{
    ...
    DM1B_INDEX		*leaf = NULL;
    DMPP_PAGE		*ovfl_page = NULL;
    DMPP_PAGE		*leaf_page = NULL;
    ...
    MEfill(sizeof(LK_LKID), 0, &leaf_lockid);
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->bto_tbl_id, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->bto_pg_type, 
		log_rec->bto_page_size, &loc_plv, &loc_tlv);
	...
	*/
	if ((dcb->dcb_status & DCB_S_EXCLUSIVE) == 0)
	{
	    ...
	    */
	    status = dm2t_lock_table(dcb, &log_rec->bto_tbl_id, DM2T_IX,
		...
		*/
		...
		if (log_rec->bto_header.flags & DM0L_PHYS_LOCK)
		...
		*/
		if (ovfl_page_recover)
		{
			...
			&log_rec->bto_tbl_id, log_rec->bto_ovfl_pageno,
		...
		*/
		...
		{
		    status = dm0p_lock_page(dmve->dmve_lk_id, dcb, 
			&log_rec->bto_tbl_id, leaf_page_number,
			LK_PAGE, LK_X, lock_action, (i4)0,
			tbio->tbio_relid, &dmve->dmve_tran_id, &leaf_lockid, 
                        (i4 *)0, (LK_VALUE *)0, &error);
		    if (status != E_DB_OK)
			break;
	...
	*/
	if ((dmve->dmve_action == DMVE_UNDO) &&
	    (leaf_page_recover) &&
	    (LSN_GT(DM1B_VPT_ADDR_BT_SPLIT_LSN_MACRO(log_rec->bto_pg_type, leaf), 
		log_lsn)) &&
	    ( ! undo_check_done))
	{
	    /*
	    ...
	    */
	    leaf_page_number = 
		DM1B_VPT_GET_PAGE_MAIN_MACRO(log_rec->bto_pg_type, ovfl);
	    ...
	    */
		...
		dmve->dmve_log_id, &dmve->dmve_tran_id, &leaf_page, &error);
	    if (status != E_DB_OK)
		break;
	    ...
	    status = dm2t_ufx_tabio_cb(tbio, DM2T_VERIFY, dmve->dmve_lk_id, 
				       dmve->dmve_log_id, &error);
	    if (status != E_DB_OK)
		break;
	    tbio = NULL;
	...
	*/
	if ((dmve->dmve_action == DMVE_UNDO) &&
	    ...
	    (ovfl_page_recover))
	{
	    if ((DM1B_VPT_GET_PAGE_OVFL_MACRO(log_rec->bto_pg_type, leaf) != 
		    DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->bto_pg_type, ovfl)) ||
		(DM1B_VPT_GET_PAGE_MAIN_MACRO(log_rec->bto_pg_type, ovfl) != 
		    DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->bto_pg_type, leaf)))
	    {
		...
		TRdisplay("\tLeaf page: %d, overflow pointer %d\n",
		    DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->bto_pg_type, leaf), 
		    DM1B_VPT_GET_PAGE_OVFL_MACRO(log_rec->bto_pg_type, leaf));
	...
	*/
	...
	case DMVE_REDO:
	    if (ovfl && LSN_GTE(
		DM1B_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->bto_pg_type, ovfl), 
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->bto_pg_type, ovfl),
			0, 
			DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->bto_pg_type, ovfl),
			0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->bto_pg_type,
			    ovfl), 
			0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->bto_pg_type,
			    ovfl),
	    ...
	    if (leaf && LSN_GTE(
		DM1B_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->bto_pg_type, leaf), 
		log_lsn) && ((dmve->dmve_flags & DMVE_ROLLDB_BOPT) == 0))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			sizeof(*tbio->tbio_relowner), tbio->tbio_relowner, 0, 
			DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->bto_pg_type, leaf),
			0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->bto_pg_type, 
			    leaf),
			0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->bto_pg_type,
			    leaf), 
			0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->bto_pg_type,
	...
	case DMVE_UNDO:
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    0, DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->bto_pg_type, ovfl),
		    0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->bto_pg_type, ovfl),
		    0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->bto_pg_type, 
			ovfl), 
		    0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->bto_pg_type,
			ovfl),
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->bto_pg_type, leaf),
		    0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->bto_pg_type, leaf),
		    0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->bto_pg_type, 
			leaf), 
		    0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->bto_pg_type,
			leaf),
	...
	*/
	...
	case DMVE_DO:
	case DMVE_REDO:
	    status = dmv_rebtovfl(dmve, tbio, ovfl, leaf, log_rec, 
		loc_plv, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_unbtovfl(dmve, tbio, ovfl, leaf, log_rec, 
		loc_plv, &error);
	    break;
	}
    ...
    */
	...
	tmp_status = dm0p_unlock_page(dmve->dmve_lk_id, dcb, 
	    &log_rec->bto_tbl_id, log_rec->bto_ovfl_pageno, 
	    LK_PAGE, tbio->tbio_relid, &dmve->dmve_tran_id, &ovfl_lockid, 
    ...
    if (status != E_DB_OK)
        dmve->dmve_error.err_code = E_DM9657_DMVE_BTREE_OVFL;
...
i4             *err_code)
{
    ...
    */
    if (leaf)
    {
	DM1B_VPT_SET_PAGE_OVFL_MACRO(log_rec->bto_pg_type, leaf,  
	    log_rec->bto_ovfl_pageno);

	DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->bto_pg_type, leaf, DMPP_MODIFY);
	DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->bto_pg_type, leaf, *log_lsn);
    }
	...
	*/
	kperpage = dm1cxkperpage(log_rec->bto_pg_type, log_rec->bto_page_size, 
	...
	*/
	status = dm1cxformat(log_rec->bto_pg_type, log_rec->bto_page_size, ovfl,
	    loc_plv, log_rec->bto_cmp_type,
	...
	if (status != E_DB_OK)
	{
	    dm1cxlog_error(E_DM93E5_BAD_INDEX_FORMAT, (DMP_RCB *)0, ovfl, 
			   log_rec->bto_pg_type, log_rec->bto_page_size, 0);
	    *err_code = E_DM9659_REDO_BTREE_OVFL;
	    break;
	}
	...
	*/
	status = dm1cxreplace(log_rec->bto_pg_type, log_rec->bto_page_size,
	    ovfl, DM1C_DIRECT, log_rec->bto_cmp_type,
	    (DB_TRAN_ID *)0, (i4)0, 
	...
	if (status != E_DB_OK)
	{
		...
		ovfl, log_rec->bto_pg_type, log_rec->bto_page_size,DM1B_LRANGE);
	    *err_code = E_DM9659_REDO_BTREE_OVFL;
	    break;
	}
	...
	{
	    dm1cxlog_error(E_DM93E6_BAD_INDEX_REPLACE, (DMP_RCB *)0, 
		ovfl, log_rec->bto_pg_type, log_rec->bto_page_size,DM1B_RRANGE);
	    *err_code = E_DM9659_REDO_BTREE_OVFL;
	    break;
	}
	...
	*/
	DM1B_VPT_SET_PAGE_MAIN_MACRO(log_rec->bto_pg_type, ovfl, 
	    log_rec->bto_mainpage);
	DM1B_VPT_SET_PAGE_OVFL_MACRO(log_rec->bto_pg_type, ovfl,
	    log_rec->bto_nextovfl);
	DM1B_VPT_SET_BT_NEXTPAGE_MACRO(log_rec->bto_pg_type, ovfl,
	    log_rec->bto_nextpage);

	DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->bto_pg_type, ovfl, DMPP_MODIFY);
	DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->bto_pg_type, ovfl, *log_lsn);
	break;
    }
...
i4             *err_code)
{
    ...
    */
	...
	if (dmve->dmve_logging)
	{
		...
		log_lsn, &lsn, log_rec->bto_tidsize, err_code);
	    if (status != E_DB_OK)
	    {
		if (ovfl) 
		    dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &ovfl);
		...
		*err_code = E_DM9658_UNDO_BTREE_OVFL;
		return(E_DB_ERROR);
	    }
	}
    }
    ...
    */
	...
	DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->bto_pg_type, leaf, DMPP_MODIFY);
	if (dmve->dmve_logging)
	    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->bto_pg_type, leaf, lsn);
    }
	...
	DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->bto_pg_type, ovfl, DMPP_MODIFY);
	if (dmve->dmve_logging)
	    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->bto_pg_type, ovfl, lsn);
    }
