...
DMVE_CB		*dmve_cb)
{
	...
	** the TCB below.
	*/
	status = dm2t_open(dmve->dmve_dcb_ptr, &log_rec->dul_tbl_id, DM2T_X,
	    DM2T_UDIRECT, DM2T_A_OPEN_NOACCESS, (i4)0, (i4)20, 0,
	    dmve->dmve_log_id, dmve->dmve_lk_id, 0, 0, dmve->dmve_db_lockmode,
	    &dmve->dmve_tran_id, &timestamp, &rcb, (DML_SCB *)0, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	if ( ! log_rec->dul_recreate)
	{
		...
		*/
		...
		status = dm1p_lastused(rcb, 0, &last_page, (DMPP_PAGE **)0,
					&error);	
		if (status != E_DB_OK)
		    break;
		...
		*/
		status = dmve_cachefix_page(dmve, log_lsn,
					    ...
					    (DM0P_READAHEAD | DM0P_TABLE_LOCKED_X), 
					    loc_plv,
					    &page_ptr, &error);
		if (status != E_DB_OK)
		    break;
		...
		*/
	    ...
	    /* When we updated relpages and reltups for the original
	    ** load we logged a SREP for this operation. This will already
	    ** have been rolled back and hence reltups and relpages will
	    ** reflect their correct values. So initialise the following to
	    ** zero so that we don't try to update relpages or reltups
	    ** when we close the table. Note that the tcb's relpages, etc
	    ** will be out of date but we close the table with DM2T_PURGE
	    ** so the tcb will be reinitialised the next time it is used
	...
	*/
	status = dm2t_close(rcb, DM2T_PURGE, &error);
	rcb = (DMP_RCB *) 0;
	if (status != E_DB_OK)
	    break;
	...
	*/
	    ...
	    ((log_rec->dul_header.flags & DM0L_CLR) == 0))
	{
	    ...
	    status = dm0l_load(dmve->dmve_log_id, dm0l_flags, 
				...
				log_rec->dul_lastpage, log_lsn, &lsn, &error);
	    if (status != E_DB_OK) {
		/*
		 * Bug56702: return logfull indication.
		 */
		dmve->dmve_error.err_data = error;
		break;
	    }
    ...
    */
    ...
    if (rcb)
    {
	status = dm2t_close(rcb, DM2T_PURGE, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
	...
	(i4)0, (i4 *)0, &error, 0);
    dmve->dmve_error.err_code = E_DM961C_DMVE_LOAD;
