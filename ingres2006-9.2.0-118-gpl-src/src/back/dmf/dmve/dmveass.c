...
DMVE_CB		*dmve_cb)
{
    ...
    i4		error = E_DB_OK;
    DM1B_INDEX		*leaf_page = NULL;
    DMPP_PAGE		*old_data = NULL;
    DMPP_PAGE		*new_data = NULL;
    DM1B_INDEX		*leaf;
    DMPP_PAGE		*odata;
    DMPP_PAGE		*ndata;
    DMP_TCB             *t;
    ...
    LK_LKID		lockid;
    LK_LKID		leaf_page_lockid;
    LK_LKID		old_data_lockid;
    LK_LKID		new_data_lockid;
    i4		lock_action;
    i4		grant_mode;
    i4		loc_error;
    bool		leaf_recover;
    ...
    bool		physical_leaf_page_lock = FALSE;
    bool		physical_old_data_lock = FALSE;
    bool		physical_new_data_lock = FALSE;
    i4                  fix_action = 0;
    i4		recovery_action;

    MEfill(sizeof(LK_LKID), 0, &lockid);
    MEfill(sizeof(LK_LKID), 0, &leaf_page_lockid);
    MEfill(sizeof(LK_LKID), 0, &old_data_lockid);
    MEfill(sizeof(LK_LKID), 0, &new_data_lockid);

    for (;;)
    {
	if ( (log_rec->ass_header.type != DM0LASSOC) ||
	     (log_rec->ass_header.length !=
                (sizeof(DM0L_ASSOC) -
                        (DB_MAXNAME - log_rec->ass_tab_size) -
                        (DB_MAXNAME - log_rec->ass_own_size))) )
	{
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->ass_tbl_id, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->ass_pg_type,
	    log_rec->ass_page_size, &loc_plv, &loc_tlv);
	...
	*/
	if ((dcb->dcb_status & DCB_S_EXCLUSIVE) == 0)
	{
	    ...
	    */
	    status = dm2t_lock_table(dcb, &log_rec->ass_tbl_id, DM2T_IX,
	    ...
	    {
		/*
		** Page locks required for all three pages.
		*/
		lock_action = LK_LOGICAL;            
		if (log_rec->ass_header.flags & DM0L_PHYS_LOCK)
		...
		*/
		if (leaf_recover)
		{
			...
			&log_rec->ass_tbl_id, log_rec->ass_leaf_page, 
			LK_PAGE, LK_X, lock_action, (i4)0, 
			tbio->tbio_relid, &dmve->dmve_tran_id, &leaf_page_lockid, 
                        (i4 *)0, (LK_VALUE *)0, &error);
		    if (status != E_DB_OK)
			break;

		    if (lock_action == LK_PHYSICAL)
			physical_leaf_page_lock = TRUE;
		}

		if (new_data_recover)
		{
		    status = dm0p_lock_page(dmve->dmve_lk_id, dcb, 
			&log_rec->ass_tbl_id, log_rec->ass_new_data, 
			LK_PAGE, LK_X, lock_action, (i4)0, 
			tbio->tbio_relid, &dmve->dmve_tran_id, &new_data_lockid, 
                        (i4 *)0, (LK_VALUE *)0, &error);
		    if (status != E_DB_OK)
			break;

		    if (lock_action == LK_PHYSICAL)
			physical_new_data_lock = TRUE;
		}

		if (old_data_recover)
		{
		    status = dm0p_lock_page(dmve->dmve_lk_id, dcb, 
			&log_rec->ass_tbl_id, log_rec->ass_old_data, 
			LK_PAGE, LK_X, lock_action, (i4)0, 
			tbio->tbio_relid, &dmve->dmve_tran_id, &old_data_lockid,
                        (i4 *)0, (LK_VALUE *)0, &error);
		    if (status != E_DB_OK)
			break;

		    if (lock_action == LK_PHYSICAL)
			physical_old_data_lock = TRUE;
		}
	    }
            else
	...
	*/
	if (leaf_recover)
	{
	    status = dmve_cachefix_page(dmve, log_lsn,
					tbio, log_rec->ass_leaf_page,
					fix_action, loc_plv,
					(DMPP_PAGE**)&leaf_page, &error);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	if (DMZ_ASY_MACRO(15))
	{
		...
		(DMPP_PAGE *) leaf, loc_plv, "LEAF");
	    dmve_trace_page_info(log_rec->ass_pg_type, log_rec->ass_page_size,
		odata, loc_plv, "OldDATA"); 
	    dmve_trace_page_info(log_rec->ass_pg_type, log_rec->ass_page_size,
		ndata, loc_plv, "NewDATA"); 
	}
	...
	*/
	...
	case DMVE_REDO:
	    if (leaf && LSN_GTE(
		DM1B_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, leaf), 
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->ass_pg_type, leaf),
			0,
			DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->ass_pg_type, leaf),
			0, DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->ass_pg_type,
			    leaf),
			0, DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->ass_pg_type,
			    leaf),
	    ...
	    if (odata && LSN_GTE(
		DMPP_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, odata), 
		log_lsn))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			sizeof(*tbio->tbio_relid), tbio->tbio_relid,
			sizeof(*tbio->tbio_relowner), tbio->tbio_relowner, 0, 
			DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->ass_pg_type, odata),
			0, 
			DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->ass_pg_type, odata),
			0, DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->ass_pg_type, 			    odata), 
			0, DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->ass_pg_type,
			    odata),
			0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		}
		odata = NULL;
	    ...
	    if (ndata && LSN_GTE(
		DMPP_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, ndata), 
		log_lsn))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			sizeof(*tbio->tbio_relid), tbio->tbio_relid,
			sizeof(*tbio->tbio_relowner), tbio->tbio_relowner, 0, 
			DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->ass_pg_type, ndata),
			0, 
			DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->ass_pg_type, ndata),
			0, DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->ass_pg_type,
			    ndata), 
			0, DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->ass_pg_type,
			    ndata),
			0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
	...
	case DMVE_UNDO:
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    0, DM1B_VPT_GET_PAGE_PAGE_MACRO(log_rec->ass_pg_type, leaf), 
		    0, DM1B_VPT_GET_PAGE_STAT_MACRO(log_rec->ass_pg_type, leaf),
		    0, 
		    DM1B_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->ass_pg_type, leaf), 
		    0, 
		    DM1B_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->ass_pg_type, leaf),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->ass_pg_type, odata),
		    0, DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->ass_pg_type, odata),
		    0, 
		    DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->ass_pg_type, odata),
		    0, 
		    DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->ass_pg_type, odata),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		error = E_DM9666_PAGE_LSN_MISMATCH;
		status = E_DB_ERROR;
	    }
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->ass_pg_type, ndata),
		    0, DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->ass_pg_type, ndata),
		    0, 
		    DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->ass_pg_type, ndata),
		    0, 
		    DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->ass_pg_type, ndata),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		error = E_DM9666_PAGE_LSN_MISMATCH;
		status = E_DB_ERROR;
	    }
	...
	*/
	...
	if (log_rec->ass_header.flags & DM0L_CLR)
        ...
        case DMVE_DO:
        case DMVE_REDO:
            status = dmv_reassoc(dmve, tbio, leaf, odata, ndata, log_rec, 
		&error);
            break;

        case DMVE_UNDO:
	    status = dmv_unassoc(dmve, tbio, leaf, odata, ndata, log_rec, 
		&error);
            break;
        }
    ...
    */
    ...
    {
	tmp_status = dm0p_uncache_fix(tbio, DM0P_UNFIX, dmve->dmve_lk_id, 
		dmve->dmve_log_id, &dmve->dmve_tran_id, &new_data, &loc_error);
	if (tmp_status != E_DB_OK)
	{
	    ule_format(loc_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &loc_error, 0);
    ...
    {
	tmp_status = dm0p_uncache_fix(tbio, DM0P_UNFIX, dmve->dmve_lk_id, 
		dmve->dmve_log_id, &dmve->dmve_tran_id, &old_data, &loc_error);
	if (tmp_status != E_DB_OK)
	{
	    ule_format(loc_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &loc_error, 0);
    ...
    /*
    ** If short term physical locks were acquired on the pages
    ** then release them.
    */
    if (physical_leaf_page_lock)
    {
	tmp_status = dm0p_unlock_page(dmve->dmve_lk_id, dcb,
	    &log_rec->ass_tbl_id, log_rec->ass_leaf_page,
	    LK_PAGE, tbio->tbio_relid, &dmve->dmve_tran_id, &leaf_page_lockid, 
	    (LK_VALUE *)0, &error);
	if (tmp_status != E_DB_OK)
	{
	    ule_format(loc_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
    ...
    if (physical_new_data_lock)
    {
	tmp_status = dm0p_unlock_page(dmve->dmve_lk_id, dcb,
	    &log_rec->ass_tbl_id, log_rec->ass_new_data,
	    LK_PAGE, tbio->tbio_relid, &dmve->dmve_tran_id, &new_data_lockid, 
	    (LK_VALUE *)0, &error);
	if (tmp_status != E_DB_OK)
	{
	    ule_format(loc_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
    ...
    if (physical_old_data_lock)
    {
	tmp_status = dm0p_unlock_page(dmve->dmve_lk_id, dcb,
	    &log_rec->ass_tbl_id, log_rec->ass_old_data,
	    LK_PAGE, tbio->tbio_relid, &dmve->dmve_tran_id, &old_data_lockid, 
	    (LK_VALUE *)0, &error);
	if (tmp_status != E_DB_OK)
	{
	    ule_format(loc_error, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
    ...
    */
    ...
    if (status != E_DB_OK)
	dmve->dmve_error.err_code = E_DM9627_DMVE_ASSOC;
...
i4             *err_code)
{
    ...
    */
    if (leaf)
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **)&leaf);
    if (odata)
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, &odata);
    if (ndata)
	dm0p_mutex(tabio, (i4)0, dmve->dmve_lk_id, &ndata);
    ...
    */
    if (leaf)
    {
	DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->ass_pg_type, leaf, DMPP_MODIFY);
	DM1B_VPT_SET_BT_DATA_MACRO(log_rec->ass_pg_type, leaf,
	    log_rec->ass_new_data);
	DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, leaf, *log_lsn);
    }
    if (ndata)
    {
	DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->ass_pg_type, ndata, 
	    (DMPP_MODIFY | DMPP_PRIM | DMPP_ASSOC));
	DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, ndata, *log_lsn);
    }
    if (odata)
    {
	DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->ass_pg_type, odata, DMPP_MODIFY);
	DMPP_VPT_CLR_PAGE_STAT_MACRO(log_rec->ass_pg_type, odata, DMPP_ASSOC);
	DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, odata, *log_lsn);
    }

    if (leaf)
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **)&leaf);
    if (odata)
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, &odata);
    if (ndata)
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, &ndata);
...
i4             *err_code)
{
    ...
    */
	...
	if (dmve->dmve_logging)
	{
		...
		log_rec->ass_new_data, log_lsn, &lsn, err_code);
	    if (status != E_DB_OK)
	    {
		if (leaf)
    ...
    */
	...
	DM1B_VPT_SET_PAGE_STAT_MACRO(log_rec->ass_pg_type, leaf, DMPP_MODIFY);
    	DM1B_VPT_SET_BT_DATA_MACRO(log_rec->ass_pg_type, leaf, 
	    log_rec->ass_old_data);
    }

    if (ndata)
    {
	DMPP_VPT_CLR_PAGE_STAT_MACRO(log_rec->ass_pg_type, ndata, DMPP_ASSOC);
	DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->ass_pg_type, ndata, DMPP_MODIFY);
    }

    if (odata)
    {
	DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->ass_pg_type, odata, DMPP_ASSOC);
	DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->ass_pg_type, odata, DMPP_MODIFY);
    }
    ...
    */
    ...
    {
	if (leaf) 
	    DM1B_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, leaf, lsn);
        if (ndata) 
	    DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, ndata, lsn);
	if (odata) 
	    DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->ass_pg_type, odata, lsn);
    }
