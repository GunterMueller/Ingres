...
DMVE_CB		*dmve_cb)
{
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->bi_tbl_id, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->bi_pg_type, 
		log_rec->bi_page_size, &loc_plv, &loc_tlv);
	...
	*/
	if ((dcb->dcb_status & DCB_S_EXCLUSIVE) == 0)
	{
	    ...
	    */
	    status = dm2t_lock_table(dcb, &log_rec->bi_tbl_id, DM2T_IX,
		...
		*/
		if (page_recover)
		{
			...
			&log_rec->bi_tbl_id, (DM_PAGENO) log_rec->bi_pageno,
	...
	*/
	if (page_recover)
	{
	    fix_option |= (dump_processing ? DM0P_SCRATCH : 0);

	    status = dmve_cachefix_page(dmve, log_lsn,
					tbio, log_rec->bi_pageno,
					fix_option, loc_plv,
					&page_ptr, &error);
	    if (status != E_DB_OK)
		break;
	...
	*/
	...
	case DMVE_REDO:
	    if (page && LSN_GTE(
		DMPP_VPT_ADDR_PAGE_LOG_ADDR_MACRO(log_rec->bi_pg_type, page), 
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			0, 
			DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->bi_pg_type, page),
			0, 
			DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->bi_pg_type, page),
			0, DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->bi_pg_type, 
			    page), 
			0, DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->bi_pg_type,
			    page),
	...
	case DMVE_UNDO:
	    ...
	    */
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DMPP_VPT_GET_PAGE_PAGE_MACRO(log_rec->bi_pg_type, page), 
		    0, DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->bi_pg_type, page),
		    0, 
		    DMPP_VPT_GET_LOG_ADDR_HIGH_MACRO(log_rec->bi_pg_type, page), 
		    0, DMPP_VPT_GET_LOG_ADDR_LOW_MACRO(log_rec->bi_pg_type, page),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
	...
	*/
	...
	{
	case DMVE_UNDO:
	    status = dmv_unbipage(dmve, tbio, page, log_rec, loc_plv, &error);
	    break;
    ...
    */
    ...
    if (status != E_DB_OK)
	dmve->dmve_error.err_code = E_DM9603_DMVE_BIPAGE;
...
i4             *err_code)
{
    ...
    */
	...
	status = dm0l_bi(dmve->dmve_log_id, dm0l_flags, 
	    ...
	    &log_rec->bi_page, log_lsn, &lsn, err_code);
	if (status != E_DB_OK)
	{
    ...
    */
    ...
    DMPP_VPT_SET_PAGE_STAT_MACRO(log_rec->bi_pg_type, page, DMPP_MODIFY);

    /*
    ** Move page lsn forward unless we are doing DUMP processing.
    */
    if (((log_rec->bi_header.flags & DM0L_DUMP) == 0) && (dmve->dmve_logging))
	DMPP_VPT_SET_PAGE_LOG_ADDR_MACRO(log_rec->bi_pg_type, page, lsn);
    ...
    */
	...
	(DMPP_VPT_GET_PAGE_STAT_MACRO(log_rec->bi_pg_type, page) & mask));
