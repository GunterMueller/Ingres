...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	{
	case DMVE_DO:
	    status = dmv_reindex(dmve, log_rec, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_unindex(dmve, log_rec, &error);
	    break;
	}
    ...
    */
    ...
    dmve->dmve_error.err_code = E_DM960B_DMVE_INDEX;
...
i4		*err_code)
{
	...
	*/
	...
	/* Get attribute count, key count is subset of attribute count  */
	keycnt = log_rec->dui_acount;
	...
	*/
	status = dm0m_allocate(sizeof(DM2U_INDEX_CB) +
	    ...
	    (char *)0, (DM_OBJECT **)&indx_cb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	indx_cb->indxcb_xcb = (DML_XCB *)0;
	indx_cb->indxcb_index_name = (DB_TAB_NAME *)0; /* not logged */
	if (log_rec->dui_loc_count)
	    indx_cb->indxcb_location = log_rec->dui_location;
	else
	    indx_cb->indxcb_location = (DB_LOC_NAME *)0;
	...
	indx_cb->indxcb_kcount = attcnt; /* all atts in index */
	indx_cb->indxcb_key = keyptrs;
	if (log_rec->dui_structure == TCB_RTREE)
	    indx_cb->indxcb_acount = 0;
	else
	    indx_cb->indxcb_acount = keycnt; /* which cols are keys */
	...
	if (log_rec->dui_structure == TCB_RTREE)
	    ...
	    indx_cb->indxcb_range = (f8 *)0;
	indx_cb->indxcb_index_flags = 0;
	indx_cb->indxcb_gwattr_array = (DM_PTR *)0;
	indx_cb->indxcb_gwchar_array = (DM_DATA *)0;
	indx_cb->indxcb_gwsource = (DMU_FROM_PATH_ENTRY *)0;
	...
	indx_cb->indxcb_char_array = (DM_DATA *)0; /* Not used if rollforward */
	indx_cb->indxcb_tbl_pri = 0; /* Not used if rollforward */
	indx_cb->indxcb_errcb = &errcb;
	...
	*/
	     ...
	     ((log_rec->dui_header.flags & DM0L_CLR) == 0))
	{
	    ...
	    status = dm0l_index(dmve->dmve_log_id, dm0l_flags, 
			...
			log_rec->dui_buckets,
			&log_rec->dui_key[0], log_rec->dui_acount,
