...
DMVE_CB		*dmve_cb)
{
	...
	*/
	status = dmve_fix_tabio(dmve, &log_rec->ext_tblid, &tbio, &error);
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, log_rec->ext_pg_type, 
		log_rec->ext_page_size, &loc_plv, &loc_tlv);
	...
	*/
	...
	case DMVE_REDO:
	    if (fhdr && LSN_GTE(
		DM1P_VPT_ADDR_FHDR_PG_LOGADDR_MACRO(log_rec->ext_pg_type, fhdr),
		...
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->ext_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_PAGE_STAT_MACRO(log_rec->ext_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_LOGADDR_HI_MACRO(log_rec->ext_pg_type,
			    fhdr), 0,
			DM1P_VPT_GET_FHDR_LOGADDR_LOW_MACRO(log_rec->ext_pg_type,
	    ...
	    if (fmap && LSN_GTE(
		DM1P_VPT_ADDR_FMAP_PG_LOGADDR_MACRO(log_rec->ext_pg_type, fmap), 
		log_lsn) && ((dmve->dmve_flags & DMVE_ROLLDB_BOPT) == 0))
	    {
		if (dmve->dmve_action == DMVE_DO)
		{
			...
			sizeof(*tbio->tbio_relid), tbio->tbio_relid,
			sizeof(*tbio->tbio_relowner), tbio->tbio_relowner, 0,
			DM1P_VPT_GET_FMAP_PAGE_PAGE_MACRO(log_rec->ext_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_PAGE_STAT_MACRO(log_rec->ext_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_LOGADDR_HI_MACRO(log_rec->ext_pg_type,
			    fmap), 0,
			DM1P_VPT_GET_FMAP_LOGADDR_LOW_MACRO(log_rec->ext_pg_type,
	...
	case DMVE_UNDO:
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    0, DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->ext_pg_type,
			fhdr),
		    0, DM1P_VPT_GET_FHDR_PAGE_STAT_MACRO(log_rec->ext_pg_type,
			fhdr),
		    0, DM1P_VPT_GET_FHDR_LOGADDR_HI_MACRO(log_rec->ext_pg_type,
			fhdr), 
		    0, DM1P_VPT_GET_FHDR_LOGADDR_LOW_MACRO(log_rec->ext_pg_type,
			fhdr),
		...
		ule_format(E_DM9665_PAGE_OUT_OF_DATE, (CL_ERR_DESC *)0, 
		    ...
		    sizeof(*tbio->tbio_relid), tbio->tbio_relid,
		    sizeof(*tbio->tbio_relowner), tbio->tbio_relowner,
		    0, DM1P_VPT_GET_FMAP_PAGE_PAGE_MACRO(log_rec->ext_pg_type,
			fmap),
		    0, DM1P_VPT_GET_FMAP_PAGE_STAT_MACRO(log_rec->ext_pg_type,
			fmap),
		    0, DM1P_VPT_GET_FMAP_LOGADDR_HI_MACRO(log_rec->ext_pg_type,
			fmap), 
		    0, DM1P_VPT_GET_FMAP_LOGADDR_LOW_MACRO(log_rec->ext_pg_type,
			fmap),
		    0, log_lsn->lsn_high, 0, log_lsn->lsn_low);
		error = E_DM9666_PAGE_LSN_MISMATCH;
		status = E_DB_ERROR;
	...
	*/
	...
	case DMVE_DO:
	case DMVE_REDO:
	    status = dmv_reextend(dmve, tbio, fhdr, fmap, log_rec, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_unextend(dmve, tbio, fhdr, fmap, log_rec, &error);
	    break;
	}
    ...
    */
    ...
    if (status != E_DB_OK)
	dmve->dmve_error.err_code = E_DM9628_DMVE_EXTEND;
...
i4             *err_code)
{
    ...
    */
    if (fhdr) 
    {
	if ((DM1P_VPT_GET_FHDR_PAGES_MACRO(log_rec->ext_pg_type, fhdr) != 
		log_rec->ext_old_pages) ||
	    (DM1P_VPT_GET_FHDR_COUNT_MACRO(log_rec->ext_pg_type, fhdr) != 
		(log_rec->ext_map_index + 1)))
	{
	    ule_format(E_DM9675_DMVE_EXT_FHDR_STATE, (CL_ERR_DESC *)0, ULE_LOG,
		...
		0, DM1P_VPT_GET_FHDR_PAGE_PAGE_MACRO(log_rec->ext_pg_type, fhdr),
		0, DM1P_VPT_GET_FHDR_PAGES_MACRO(log_rec->ext_pg_type, fhdr), 
		0, log_rec->ext_old_pages, 
		0, DM1P_VPT_GET_FHDR_COUNT_MACRO(log_rec->ext_pg_type, fhdr), 
		0, (log_rec->ext_map_index + 1));
	    dmd_log(1, log_rec, 4096);
	    ule_format(E_DM9646_REDO_EXTEND, (CL_ERR_DESC *)0, ULE_LOG,
    ...
    */
	...
	for (i = 0; i < log_rec->ext_loc_cnt; i++)
	{
	    ...
	    */
		...
		&dmve->dmve_dcb_ptr->dcb_name, &last_page, err_code);
	    if (status != E_DB_OK)
		return(E_DB_ERROR);
    ...
    */
	...
	DM1P_VPT_SET_FHDR_PAGES_MACRO(log_rec->ext_pg_type, fhdr, 
	    log_rec->ext_new_pages);
	DM1P_VPT_INCR_FHDR_EXTEND_CNT_MACRO(log_rec->ext_pg_type, fhdr);

	if (log_rec->ext_fhdr_hwmap)
	    DM1P_VPT_SET_FHDR_HWMAP_MACRO(log_rec->ext_pg_type, fhdr, 
		(log_rec->ext_map_index + 1));
	if (log_rec->ext_fhdr_hint)
	    VPS_SET_FREE_HINT_MACRO(log_rec->ext_pg_type, 
		(DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->ext_pg_type, fhdr, 
		log_rec->ext_map_index)));

	DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(log_rec->ext_pg_type,fhdr,DMPP_MODIFY);
	DM1P_VPT_SET_FHDR_PG_LOGADDR_MACRO(log_rec->ext_pg_type, fhdr, *log_lsn);
	...
	*/
	if ( (first_used_page <= last_used_page) &&
	    ((first_used_page / 
	    DM1P_FSEG_MACRO(log_rec->ext_pg_type, log_rec->ext_page_size)) <= 
	    log_rec->ext_map_index) )
	{
	    if (DM1P_VPT_GET_FMAP_FIRSTBIT_MACRO(log_rec->ext_pg_type, fmap) == 
		(DM1P_VPT_GET_FMAP_HW_MARK_MACRO(log_rec->ext_pg_type, fmap) + 1))
	    {
                /*
                ...
                ** free bit. (B77416)
                */
		DM1P_VPT_SET_FMAP_FIRSTBIT_MACRO(log_rec->ext_pg_type, fmap,
		    (last_used_page + 1));
		    ...
		    >= DM1P_FSEG_MACRO(log_rec->ext_pg_type, log_rec->ext_page_size))
		{
		    ...
		    */ 
		    if (fhdr)
		    {
			VPS_CLEAR_FREE_HINT_MACRO(log_rec->ext_pg_type,
			    (DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->ext_pg_type, fhdr,
			    log_rec->ext_map_index)));
		    }
		}
	...
	*/
	...
	DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(log_rec->ext_pg_type,fmap,DMPP_MODIFY);
	DM1P_VPT_SET_FMAP_PG_LOGADDR_MACRO(log_rec->ext_pg_type, fmap, *log_lsn);
...
i4             *err_code)
{
    ...
    */
		...
		(log_rec->ext_map_index + 1)))
	{
		...
		0, (log_rec->ext_map_index + 1));
	    dmd_log(1, log_rec, 4096);
	    ule_format(E_DM9645_UNDO_EXTEND, (CL_ERR_DESC *)0, ULE_LOG,
		NULL, (char *)0, (i4)0, (i4 *)0, err_code, 0);
	}
    }
    ...
    */
	...
	if (dmve->dmve_logging)
	{
		...
		(bool) log_rec->ext_fmap_update, log_lsn, &lsn, err_code);
	    if (status != E_DB_OK)
	    {
    ...
    */
	...
	DM1P_VPT_SET_FHDR_PAGES_MACRO(log_rec->ext_pg_type, fhdr, 
	    log_rec->ext_old_pages);
	DM1P_VPT_DECR_FHDR_EXTEND_CNT_MACRO(log_rec->ext_pg_type, fhdr);
	if (log_rec->ext_fhdr_hint)
	    VPS_CLEAR_FREE_HINT_MACRO(log_rec->ext_pg_type, 
		(DM1P_VPT_GET_FHDR_MAP_MACRO(log_rec->ext_pg_type, fhdr, 
		log_rec->ext_map_index)));
	...
	DM1P_VPT_SET_FHDR_PAGE_STAT_MACRO(log_rec->ext_pg_type,fhdr,DMPP_MODIFY);
	if (dmve->dmve_logging)
	    DM1P_VPT_SET_FHDR_PG_LOGADDR_MACRO(log_rec->ext_pg_type, fhdr, lsn);
	dm0p_unmutex(tabio, (i4)0, dmve->dmve_lk_id, (DMPP_PAGE **) &fhdr);
    }
	...
	*/
	     ...
	     log_rec->ext_map_index) )
	{
		...
		BITMAP_SET_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->ext_pg_type,
                        fmap), bit);
		...
		DM1P_VPT_SET_FMAP_FIRSTBIT_MACRO(log_rec->ext_pg_type, fmap,
		    first_used_page);
	    if ( DM1P_VPT_GET_FMAP_HW_MARK_MACRO(log_rec->ext_pg_type, fmap) >= 
		first_used_page )
		DM1P_VPT_SET_FMAP_HW_MARK_MACRO(log_rec->ext_pg_type, fmap,
			( first_used_page - 1 ));
	}
	...
	*/
	    ...
	    == log_rec->ext_map_index)
	{
		...
		DM1P_FSEG_MACRO(log_rec->ext_pg_type, log_rec->ext_page_size)) - 1;
	    }
	    ...
	    for (bit = first_bit; bit <= last_bit; bit++)
		BITMAP_CLR_MACRO(DM1P_VPT_FMAP_BITMAP_MACRO(log_rec->ext_pg_type,
		    fmap), bit);

	    if ((i4)DM1P_VPT_GET_FMAP_LASTBIT_MACRO(log_rec->ext_pg_type, 
		fmap) >= first_bit)
		DM1P_VPT_SET_FMAP_LASTBIT_MACRO(log_rec->ext_pg_type, fmap,
		    (first_bit - 1));
	}
	DM1P_VPT_SET_FMAP_PAGE_STAT_MACRO(log_rec->ext_pg_type,fmap,DMPP_MODIFY);
	if (dmve->dmve_logging)
	    DM1P_VPT_SET_FMAP_PG_LOGADDR_MACRO(log_rec->ext_pg_type, fmap, lsn);
