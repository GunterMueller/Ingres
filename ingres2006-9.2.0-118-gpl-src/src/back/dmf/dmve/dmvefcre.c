...
DMVE_CB		*dmve_cb)
{
	...
	*/
	...
	{
	case DMVE_DO:
	    status = dmv_refcreate(dmve, log_rec, &error);
	    break;

	case DMVE_UNDO:
	    status = dmv_unfcreate(dmve, log_rec, &error);
	    break;
	}
...
i4             *err_code)
{
	...
	*/
	status = dm2f_close_file(&loc_entry, 1, (DM_OBJECT *)dcb, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = dm2f_release_fcb(dmve->dmve_lk_id, dmve->dmve_log_id,
				&loc_entry, 1, DM2F_ALLOC, err_code);
	if (status != E_DB_OK)
	    break;
    ...
    */
    ...
    {
	status = dm2f_release_fcb(dmve->dmve_lk_id, dmve->dmve_log_id,
				&loc_entry, 1, DM2F_ALLOC, &local_error);
	if (status != E_DB_OK)
	{
    ...
    *err_code = E_DM968A_REDO_FCREATE;
...
i4             *err_code)
{
    ...
    */
    ...
    ule_format(*err_code, (CL_ERR_DESC *)0, ULE_LOG, NULL, 
	(char *)0, (i4)0, (i4 *)0, err_code, 0);
    *err_code = E_DM967B_UNDO_FCREATE;
