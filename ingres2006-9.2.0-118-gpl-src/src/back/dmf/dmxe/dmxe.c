...
i4		    *err_code)
{
    ...
    */ 
    ...
    { 
	status = dmf_gwx_abort(odcb->odcb_scb_ptr, err_code); 
        if (status != E_DB_OK) 
        { 
    ...
    */
	...
	if (stat)
	{
	    ule_format(stat, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, err_code, 0);
	}
	ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, err_code, 1, 0, LG_S_STRANSACTION);
	*err_code = E_DM9503_DMXE_ABORT;
	return (E_DB_ERROR);
    }
    ...
    */
    ...
    {
	status = disconnect_willing_commit(tran_id, lock_id, log_id, err_code);
	if (status != E_DB_OK)
	{
	...
	if ( tr.tr_status & TR_SHARED )
	{
		...
		*/
		...
		if (stat != OK)
		{
		    ...
		    ule_format(stat, &sys_err, ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);
		    ule_format(E_DM9019_BAD_LOCK_ALTER, &sys_err, ULE_LOG, NULL,
	...
	*/
	status = dm0c_open(dcb, 0, lock_id, &cnf, err_code);
	if (status == E_DB_OK)
	{
	...
	*/
	status = dm0l_force(log_id, &tr.tr_last_lsn, 0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	if ( (status = 
		dm0m_allocate(sizeof(DMP_LCTX) +
		    ...
		    (DM_OBJECT **)&lctx, err_code)) == E_DB_OK )
	{
	...
	*/
	status = dm0l_position(DM0L_TRANS, log_id, 0, lctx, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	do
	{
		...
		&dmve.dmve_lg_addr, &dmve.dmve_error.err_code);
	    if (status != E_DB_OK)
            {
                ...
                */
                if (bad_read < 5 && (dmve.dmve_error.err_code == E_DM9238_LOG_READ_EOF))
                {
                   ...
                   ule_format( 0, 0, ULE_MESSAGE, 0, buffer, STlength(buffer), 0, err_code, 0);
	    ...
	    {
                status = E_DB_ERROR;
		dmve.dmve_error.err_code = E_DM9503_DMXE_ABORT;
                break;
	    }
...
*/
    ...
    */
	...
	if (et_found == FALSE && dcb->dcb_access_mode != DCB_A_READ)
	{
		...
		*savepoint_lsn = lsn;
	    ...
	    {
		status = dm0p_force_pages(lock_id, tran_id, log_id, 0,err_code);
		if (status != E_DB_OK)
		    break;
	    }
	...
	*/
	if (sp_id == 0)
	{
		...
		ule_format(E_DM901B_BAD_LOCK_RELEASE, &sys_err, ULE_LOG, NULL,
		    (char*)0, (i4)0, (i4*)0, err_code, 1, 0, lock_id);
		ule_format(E_DM9503_DMXE_ABORT, &sys_err, ULE_LOG, NULL,
			    0, 0, 0, err_code, 0);
    ...
    **
    */
    if ((*err_code != E_DM010C_TRAN_ABORTED) && (sp_id == 0))
    {
	/*
	...
	*/
	...
	ule_format(E_DM9509_DMXE_PASS_ABORT, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    ule_format(E_DM9508_DMXE_TRAN_INFO, (CL_SYS_ERR *)0, ULE_LOG, NULL,
...
i4		    *err_code)
{
	...
	*/
	if (cl_status != OK)
	{
		     ...
		     cl_status != LG_DUPLICATE_TXN) )
		{
			...
			(char *)0, 0L, (i4 *)0, err_code, 0);
		    (VOID)ule_format(E_DM900C_BAD_LOG_BEGIN, &sys_err, 
			    ULE_LOG, NULL, 0, 0, 0,
			    err_code, 1, 0, dcb_id);
		}
		if (cl_status == LG_EXCEED_LIMIT)
		    ...
		    *err_code = E_DM0120_DIS_TRAN_UNKNOWN;
		else if ( cl_status == LG_DUPLICATE_TXN )
		    *err_code = E_DM0119_TRAN_ID_NOT_UNIQUE;
		else
	...
	if ( cl_status == LG_CONTINUE )
	{
		...
		if ( cl_status )
		    ule_format(cl_status, &sys_err, ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, err_code, 0);
		(VOID)ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, 
			ULE_LOG, NULL, 0, 0, 0,
	...
	else
	{
	    ...
	    */
		...
		{
		    ule_format(E_DM9C07_OUT_OF_LOCKLISTS, &sys_err, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, err_code, 0);
	    ...
	    */
		...
		ule_format(cl_status, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, err_code, 0);
		(VOID)ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, 
			ULE_LOG, NULL, 0, 0, 0,
	...
	*/
	if (((flags & (DMXE_DELAYBT|DMXE_CONNECT)) == 0) && (mode == DMXE_WRITE))
	{
	    ...
	    */
	    status = dm2d_check_db_backup_status( dcb, err_code );
	    if (status != E_DB_OK)
		break;
	}
...
i4		    *err_code)
{
	...
	**
	** status = dm0l_force(log_id, 0, 0, err_code);
	*/
	...
	*/
	...
	status = dm0l_et(log_id, et_flags, DM0L_COMMIT, 0, 0, ctime, err_code);
	if (status != E_DB_OK)
	    break;
...
i4		    *err_code)
{
    ...
    */
	...
	(DB_OWN_NAME *)xcb->xcb_username.db_own_name, &lsn, err_code);
	...
	*/
	...
	ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
	    (i4)0, (i4 *)0, err_code, 0);
	*err_code = E_DM9504_DMXE_WRITEBT;
    }
...
i4		    *err_code)
{
    ...
    if (stat != OK)
    {
	ule_format(stat, (CL_SYS_ERR *)0, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, err_code, 0);
	*err_code = E_DM0122_CANT_SECURE_IN_CLUSTER;
	return (E_DB_ERROR);    
    }
    /* 
    ...
    */
	...
	ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, err_code, 1, 0, 
		    LG_S_DIS_TRAN);
	*err_code = E_DM9505_DMXE_WILLING_COMMIT;
	return (E_DB_ERROR);
    }
    if (length)
    {
	...
	*/
    ...
    /*	
    ** Force the log records and alter the state of the
    ** transaction to be WILLING COMMIT. 
    **
    ** SHARED transactions enter the WILLING COMMIT state
    ** when all connections have done this.
    **
    ** The global transaction's pages will be forced from
    ** the cache by dm0l_secure() when all connected HANDLEs
    ** have done this.
    */
    status = dm0l_secure(log_id, lock_id, tran_id, dis_tran_id, err_code);
...
i4		    *err_code)
{
    ...
    status = dm0p_force_pages(lock_id, tran_id, log_id, DM0P_TOSS, err_code);
    if (status != E_DB_OK)
    {
	if (*err_code > E_DM_INTERNAL)
	{
	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		(i4)0, (i4 *)0, err_code, 0);
	    *err_code = E_DM9506_DMXE_DISCONNECT;
	}
	return (status);
    }
...
i4		   *err_code)
{
	...
	*/
    	...
    	*err_code = E_DM0134_DIS_DB_UNKNOWN;
	return (E_DB_ERROR);
    }
	...
	*err_code = E_DM0121_DIS_TRAN_OWNER;
	return (E_DB_ERROR);
    }
	...
	*err_code = E_DM9507_DMXE_RESUME;
	...
	ule_format(stat, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, err_code, 0);
	(VOID) ule_format(E_DM901A_BAD_LOCK_CREATE, &sys_err, ULE_LOG, NULL, 
	    (char *)0, 0L, (i4 *)0, err_code, 0);
...
bool		*abort_state)
{
    ...
    */
	...
	ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL,
	    (char *)0, 0L, (i4 *)0, &error, 1, 0, LG_S_ATRANSACTION);
	*abort_state = FALSE;
	return (E_DB_WARN);
    }
...
i4		    *err_code)
{
	...
	*/
	...
	if (stat != E_DB_OK)
	{
	    ...
	    *err_code = E_DM9503_DMXE_ABORT; /* XXXX */
	    status = E_DB_ERROR;
	    break;
	}
	...
	*/
	...
	if (stat != OK)
	{
		...
		(char *)0, 0L, (i4 *)0, err_code, 0);
	    ule_format(E_DM900B_BAD_LOG_ALTER, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, err_code, 1, 0, LG_A_PASS_ABORT);
	    *err_code = E_DM9503_DMXE_ABORT; /* XXXX */
	    status = E_DB_ERROR;
	    break;
	}
