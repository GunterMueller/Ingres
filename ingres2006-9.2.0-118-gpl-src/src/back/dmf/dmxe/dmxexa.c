...
i4		    *err_code)
{
	...
	*err_code = E_DM016A_ERROR_XA_DISASSOC;
    }
...
i4		    *err_code)
{
	...
	*/
	...
	if ( stat == LG_CONTINUE )
	{
	    ...
	    */
	    status = dm0p_force_pages(tr.tr_lock_id, &tr.tr_eid, tr.tr_id, DM0P_TOSS, err_code);
	}
	else if ( stat )
	{
	    ule_format(stat, &sys_err, ULE_LOG, NULL,
...
i4		    *err_code)
{
    ...
    */
	...
	ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL,
		    (char *)0, 0L, (i4 *)0, err_code, 1, 0, 
		    LG_S_XA_COMMIT);
	*err_code = E_DM012C_ERROR_XA_COMMIT;
    }
    else if ( length > 0 )
    {
...
i4		    *err_code)
{
    ...
    */
	...
	ule_format(E_DM9017_BAD_LOG_SHOW, &sys_err, ULE_LOG, NULL,
		(char *)0, 0L, (i4 *)0, err_code, 1, 0, 
		LG_S_XA_ROLLBACK);
	*err_code = E_DM012D_ERROR_XA_ROLLBACK;
    }
    else if ( length > 0 )
    {
