...
i4		*err_code)	    
{
    ...
    status = dm2u_ckp_lock(dcb, tab_name, tab_owner, xcb, err_code);
    if (status != E_DB_OK)
	return (E_DB_ERROR);

    dmf_svcb->svcb_stat.utl_destroy++;	    /* # times delete utility evoked */

    error = E_DB_OK;		/* success -- reset if failure */
    ...
    */
	...
	{   
		...
		*has_extensions = dest_rcb->rcb_tcb_ptr->tcb_rel.relstat2 &
		...
		status = dm2t_close(dest_rcb, DM2T_KEEP, &error);
		if (status != E_DB_OK)
		{
		    ule_format(error, (CL_SYS_ERR *)0, ULE_LOG,
				NULL, (char *)0, (i4)0,
				(i4 *)0, &local_error, (i4)0);
		}
	    ...
	    if ( tcb->tcb_rel.relstat & TCB_INDEX &&
                (tcb->tcb_parent_tcb_ptr->tcb_rel.relstat2 & TCB2_READONLY))
            {
                ...
                error = E_DM0170_READONLY_TABLE_INDEX_ERR; 
                break;
            }
		...
		if ( !(ustat & DU_UAUDITOR)) /* no auditor priv */
		{
			...
			*/
			status = dma_write_audit(SXF_E_SECURITY, 
			    ...
			    I_SX2042_REMOVE_TABLE, FALSE, &e_error, NULL);
		    }
		    ...
		    error = E_DM201E_SECURITY_DML_NOPRIV;
		    break;
		}
	    }
	    ...
	    */
		...
		no_file = FALSE;
            ...
            */
            ...
            {
	        status = dm2t_close(dest_rcb, DM2T_PURGE, &error);
                if (status)
                    break;
            }
	...
	*/
	...
	for (;;)	/* loop until we find the iirelation tuple we want */
	{
		...
		*/
		...
		{
		    error = E_DM0129_SECURITY_CANT_UPDATE;
		    break;
		}	        
		...
		*/
		...
		{
                    error = E_DM0067_READONLY_TABLE_ERR; 
		    break;
		}
	    ...
	    */
	    ...
	    status = dm2t_close(rel_rcb, DM2T_NOPURGE, &error);
	    if (status != E_DB_OK)
		break;
    ...
    if (error != E_DB_OK)  /* we cant continue deleteing this file */
    {
	...
	*err_code = error;
	...
	{
	    ule_format(log_err, 0, ULE_LOG, NULL,
	...
	*/
	if (tbl_id->db_tab_index >= 0)
	{
	    ...
	    */
	    ...
	    if (status != OK)
	    {
		ule_format(status, (CL_SYS_ERR *)&sys_err, ULE_LOG,
				NULL, (char *)0, (i4)0,
				(i4 *)0, &local_error, (i4)0);
		if (status == LK_NOLOCKS)
		{	
		    ule_format(E_DM9046_TABLE_NOLOCKS, &sys_err, ULE_LOG, NULL, 0, 
			0, 0, &local_error, 2, sizeof(DB_TAB_NAME), &table_name,
			sizeof(DB_DB_NAME), &dcb->dcb_name);
		    error = E_DM004B_LOCK_QUOTA_EXCEEDED;
		    break;
		}
		...
		ule_format(E_DM9025_BAD_TABLE_DESTROY, &sys_err, ULE_LOG , NULL,
	            (char * )0, 0L, (i4 *)0, 
	...
	*/
	...
	status = dm2t_open(dcb, &table_id, DM2T_IX,
	    ...
	    xcb->xcb_log_id, xcb->xcb_lk_id, (i4)0, (i4)0, 
            db_lockmode, &xcb->xcb_tran_id, &timestamp,
	    &index_rcb, (DML_SCB *)0, &error);
	if (status)
	{
	    ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
	    log_err = E_DM9027_INDEX_UPDATE_ERR;
	    break;
	}
	...
	*/
	...
	index_rcb->rcb_xcb_ptr = xcb;
        ...
        /* if we didn't exit due to an open error, assure that status is good */
	error = E_DB_OK;
	break;
	...
	if (index_rcb)
	{
	    local_status = dm2t_close(index_rcb, DM2T_NOPURGE, &local_error);
	    if (local_status != E_DB_OK)
	    {
		ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG,
				NULL, (char *)0, (i4)0,
	...
	*/
	...
	for (;;)
	{
	    ...
	    */
	    ...
	    status = dm2r_unfix_pages(rel_rcb, &error);
	    if (status)
	    {
		ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		...
		*/
		...
		status = dm2r_unfix_pages(index_rcb, &error);
		if (status)
		{
		    ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, &local_error, 
	    ...
	    */
		...
		for (;;)
		{
		    ...
		    */
		    ...
		    status = dm2r_delete(attr_rcb, &tid, DM2R_BYPOSITION, &error);
		    if (status != E_DB_OK )
		    {
			ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
				(char *)0, (i4)0, (i4 *)0, &local_error, 0);
		...
		if (error == E_DM0055_NONEXT && attr_count > 0)
		    error = E_DB_OK;
		if (error != E_DB_OK)
		    break;
		...
		/* Got a qualifying record, delete it. */
	        ...
	        status = dm2r_delete(rtree_rcb, &tid, DM2R_BYPOSITION, &error);
	        if (status != E_DB_OK )
 	        {
	          ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, &local_error, 0);
		...
		break;
	      ...
	      status = dm2r_unfix_pages(rtree_rcb, &error);
	      if (status != E_DB_OK )
	      {
		ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		    (char *)0, (i4)0, (i4 *)0, &local_error, 0);
		...
		*/
		...
		status = dm2r_unfix_pages(dev_rcb, &error);
		if (status != E_DB_OK )
		{
		    ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, &local_error, 
	...
	if (error == E_DM0055_NONEXT && dest_count > 0)
	    error = E_DB_OK;
	if (error != E_DB_OK)
	    break;
    ...
    */
    ...
    if (rtree_rcb)
    {
	local_status = dm2t_close(rtree_rcb, DM2T_NOPURGE, &local_error);
	if (local_status != E_DB_OK)
	{
	    if (error == E_DB_OK)
	    {
