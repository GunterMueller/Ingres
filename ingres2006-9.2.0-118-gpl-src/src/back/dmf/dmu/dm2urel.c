...
i4		*err_code)
{
    ...
    /*
    ** Relocate operations operate on files created here, and
    ** employ DI_USER_SYNC_MASK.  A force must be done at the end of
    ** of the operation.
    */
    ...
    */
	...
	{	
	    ...
	    if (compare != 0)
	    {
		*err_code = E_DM0187_BAD_OLD_LOCATION_NAME;
		if (loc_cb != 0)
	...
	*/
	...
	if (status != E_DB_OK)
	{
	    dm2t_close(reloc_rcb, DM2T_NOPURGE, &error);
	    reloc_rcb = 0;
	    break;
	}
	...
	for (k = 0; k < loc_count; k++)
	{
	    ...
	    if (compare != 0)
	    {
		status = dm2t_close(reloc_rcb, DM2T_NOPURGE, &error);
		reloc_rcb = 0;
		error = E_DM0187_BAD_OLD_LOCATION_NAME;
		*err_data = k;
		break;
	    }
	}
	...
	*/
	status = dm2t_close(reloc_rcb, DM2T_PURGE, &error);
	if (status != E_DB_OK)
	    break;
	...
	if (status != E_DB_OK)
	{
	    ...
	    ** page array.
	    */
	    if (status == E_DM923D_DM0M_NOMORE)
	    {
		mwrite_blocks = 1;
		allocate_size = sizeof(DMP_MISC) +
	...
	*/
	...
	status = dm2f_build_fcb(xcb->xcb_lk_id, xcb->xcb_log_id,
	    ...
	    tbl_id, 0, 0, (DMP_TABLE_IO *)0, db_sync_flag, &error);
	if (status == E_DB_OK)
	{
	    status = dm2f_open_file(xcb->xcb_lk_id, xcb->xcb_log_id,
		newloc_info, loc_count, (i4)page_size,
		DM2F_E_WRITE, db_sync_flag, (DM_OBJECT *)dcb, &error);
	}
	if (status == E_DB_OK ||
	    (status != E_DB_OK && error != E_DM9291_DM2F_FILENOTFOUND))
	{
	    if (status == E_DB_OK)
	    {
		if (!has_raw_location)
		{
		    ule_format(E_DM9029_UNEXPECTED_FILE, 0, ULE_LOG, NULL, 0,0,0,
			&error, 2, newloc_info[0].loc_ext->phys_length, 
			&newloc_info[0].loc_ext->physical, sizeof(DM_FILE), 
			&newloc_info[0].loc_fcb->fcb_filename);
		    error = E_DM0104_ERROR_RELOCATING_TABLE;
		}
	    }
	    if (!has_raw_location)
	    {
	        local_status = dm2f_release_fcb(xcb->xcb_lk_id, xcb->xcb_log_id,
		    newloc_info, loc_count, DM2F_ALLOC, err_code);
	    }
	    break;
	}
	error = E_DB_OK;
	...
	*/
	status = dm2u_file_create(dcb, xcb->xcb_log_id, xcb->xcb_lk_id,
	    tbl_id, db_sync_flag, &newloc_info[k], 1, page_size,
	...
	*/
	...
	status = dm2f_open_file(xcb->xcb_lk_id, xcb->xcb_log_id,
	    ...
	    db_sync_flag, (DM_OBJECT *)dcb, &error);
	if (status != E_DB_OK)
	    break;
	oldfile_open = TRUE;
	...
	*/
	status = dm2f_sense_file(&oldloc_info[k], 1, 
	    &table_name, &dcb->dcb_name, &endpage, &error); 
	if (status != E_DB_OK)
	    break;
	...
	status = dm2f_alloc_file(&newloc_info[k], 1, 
	    &table_name, &dcb->dcb_name, oldloc_endpage[k], &page, &error); 
	if (status != E_DB_OK)
	    break;
	...
	*/
	oldfile_open = FALSE;
	status = dm2f_close_file(&oldloc_info[k], 1, (DM_OBJECT *)dcb, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	newfile_open = FALSE;
	status = dm2f_close_file(&newloc_info[k], 1, (DM_OBJECT *)dcb, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = dm2f_release_fcb(xcb->xcb_lk_id, xcb->xcb_log_id,
				 &oldloc_info[k], 1, DM2F_ALLOC, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = dm2f_release_fcb(xcb->xcb_lk_id, xcb->xcb_log_id,
			     newloc_info, k, DM2F_ALLOC, &error);
	if (status != E_DB_OK)
	    break;
	...
	*/
	status = dm2t_close(rel_rcb, DM2T_PURGE, &error);
	rel_rcb = 0;
	if (status != E_DB_OK)
	    break;
	...
	if (dev_rcb)
	{
	    status = dm2t_close(dev_rcb, DM2T_PURGE, &error);
	    dev_rcb = 0;
	    if (status != E_DB_OK)
		break;
	}
    ...
    */
    ...
    {
	status = dm2f_release_fcb(xcb->xcb_lk_id, xcb->xcb_log_id,
				 &oldloc_info[k], k, DM2F_ALLOC, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
    ...
    {
	status = dm2f_release_fcb(xcb->xcb_lk_id, xcb->xcb_log_id,
				 newloc_info, k, DM2F_ALLOC, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
    ...
    if (rel_rcb)
    {
	status = dm2t_close(rel_rcb, DM2T_PURGE, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
    ...
    if (dev_rcb)
    {
	status = dm2t_close(dev_rcb, DM2T_PURGE, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
