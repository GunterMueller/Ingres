...
**	table_flags       		A mask indicating attributes of the
**					table - one or more of the following:
**					  DM2U_JOURNAL 	    - journal rel
**					  DM2U_DUPLICATES   - allow duplicates
**					  DM2U_CATALOG 	    - base catalogue
**					  DM2U_EXTCATALOG   - extended catalogue
**					  DM2U_SYS_MAINTAINED -
**						contains at least one attribute
**						(ie. a logical key, ...)
**					  DM2U_GATEWAY	    - gateway table
**					  DM2U_SYSMOD	    - called from
**						sysmod.
**					  DM2U_VGRANT_OK - DMT_VGRANT_OK must be
**						set
**						which is sytem maintained
**						(i.e. a logical key ...)
**					  DM2U_TABLE_EXTENSION - is a table
**						extension
**					  DM2U_HAS_EXTENSIONS - if the table has
...
DB_ERROR	    *errcb)
{
    ...
    status = dm2u_ckp_lock(dcb, table_name, owner, xcb, &(errcb->err_code));
    if (status != E_DB_OK)
	return (E_DB_ERROR);

    dmf_svcb->svcb_stat.utl_create++;

    if (table_flags & DM2U_JOURNAL)
	journal = 1;
    if (table_flags & DM2U_DUPLICATES)
	duplicates = 1;
    if (table_flags & DM2U_SYS_MAINTAINED)
	sys_maintained = 1;
    if (table_flags & DM2U_GATEWAY)
	gateway = 1;
    if ((gateway) && (table_flags & DM2U_GW_UPDT) == 0)
	gwnoupdt = 1;
    if (table_flags & DM2U_CATALOG)
	relstat |= TCB_CATALOG;
    if (table_flags & DM2U_TABLE_EXTENSION)
	extension = 1;
    if (table_flags & DM2U_HAS_EXTENSIONS)
	has_extensions = 1;

    if (table_flags & DM2U_EXTCATALOG)
	relstat |= TCB_EXTCATALOG;
    ...
    */
    if (view || gateway || (table_flags & TCB_IS_PARTITIONED))
    {
	nofile_create = TRUE;
	...
	** Etabs and partitions have ii-names but they are NOT catalogs.
	...
	{
	    syscat = TRUE;
	    if  (syscat && (scb->scb_s_type & SCB_S_SYSUPDATE) == 0)
	    {
		*err_code = E_DM005E_CANT_UPDATE_SYSCAT;
		return (E_DB_ERROR);
	    }
	    if (syscat)
	    {
	...
	if (!extension && (relstat2 & TCB2_PARTITION) == 0)
	{
	    ...
	    */
		...
		if (status == E_DB_OK)
		{
		    ...
		    {
			error = E_DM0039_BAD_TABLE_NAME;
			break;
		    }
		}
	    } while (error != E_DM0055_NONEXT);
	    if (error != E_DM0055_NONEXT)
		break;

	    /* Close the index table, no longer needed. */

	    status = dm2t_close(relindex_rcb, DM2T_NOPURGE, &error);
	    relindex_rcb = NULL;
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	if (!extension && (relstat2 & TCB2_PARTITION) == 0)
	{
	    ...
	    */
		...
		{
		    ule_format(E_DM9046_TABLE_NOLOCKS, &sys_err, ULE_LOG, NULL,
			0, 0, 0,
		...
		ule_format( E_DM901F_BAD_TABLE_CREATE, &sys_err, ULE_LOG , NULL,
			(char * )0, 0L, (i4 *)0,
		...
		{
		    error = E_DM0042_DEADLOCK;
		    *err_code = error;
		    return (E_DB_ERROR);
		}
		else
		{
		    error = E_DM0077_BAD_TABLE_CREATE;
		    *err_code = error;
		    return (E_DB_ERROR);
		}
	    }
	...
	{
	    local_status = dm2t_close(relindex_rcb, DM2T_NOPURGE, &local_error);
	    relindex_rcb = NULL;
	}
    ...
    */
	...
	status = dm2t_open(dcb, &table_id, DM2T_IX,
	    ...
	    xcb->xcb_log_id, xcb->xcb_lk_id, (i4)0,(i4)0,
            db_lockmode,
            &xcb->xcb_tran_id, &timestamp,
	    &attr_rcb, (DML_SCB *)0, &error);
	if (status)
	{
	    local_status = dm2t_close(rel_rcb, DM2T_NOPURGE, &local_error);
	    *err_code = error;
	    return (E_DB_ERROR);
	}
	...
	*/
	...
	status = dm2t_open(dcb, &table_id, DM2T_IX,
	    ...
	    xcb->xcb_log_id, xcb->xcb_lk_id, (i4)0,(i4)0,
            db_lockmode,
            &xcb->xcb_tran_id, &timestamp,
	    &dev_rcb, (DML_SCB *)0, &error);
	if (status)
	{
	    local_status = dm2t_close(rel_rcb, DM2T_NOPURGE, &local_error);
	    if (attr_rcb != NULL)
		local_status = dm2t_close(attr_rcb, DM2T_NOPURGE, &local_error);
	    *err_code = error;
	    return (E_DB_ERROR);
	}
    ...
    */
	...
	{
	    (VOID)MEcopy((PTR)systab[k].name, DB_MAXNAME, (PTR)tempstr);
	    if (upcase)
	    {
	...
	*/
    ...
    if ((loc_count > 1) &&
        ((relstat & TCB_CATALOG) || (relstat & TCB_EXTCATALOG)))
    {
	local_status = dm2t_close(rel_rcb, DM2T_NOPURGE, &local_error);
	if (attr_rcb != NULL)
	    local_status = dm2t_close(attr_rcb, DM2T_NOPURGE, &local_error);
	...
	*err_code = E_DM0071_LOCATIONS_TOO_MANY;
    ...
    if ((relstat == 0) || (relstat & TCB_EXTCATALOG))
    {
	    ...
	    */
	    ...
	    else
	    {
	        ...
	        */
	        i = 1;
	        if (table_flags & TCB_IS_PARTITIONED)
	...
	*/
	dm1c_getaccessors(TCB_C_DEFAULT, page_type, page_size, 
		&loc_plv, &loc_tlv);
	    ...
	    */
		...
		for (i = 0; i < dcb->dcb_ext->ext_count; i++)
		{
			...
			if (dcb->dcb_ext->ext_entry[i].flags & DCB_RAW)
			{
				...
				compare = 1;
		    		errcb->err_data = k;
				error = E_DM0190_RAW_LOCATION_OCCUPIED;
				status = E_DB_ERROR;	
				break;
			    }
	...
	*/
	...
	if (view)
	{
	    ...
	    if (table_flags & DM2U_VGRANT_OK)
		relrecord.relstat |= TCB_VGRANT_OK;
	...
	if (table_flags & TCB_CLUSTERED)
	    relrecord.relstat |= TCB_CLUSTERED;
	...
	*/
	if(relstat & TCB_CATALOG)
	{
		...
		*/
		...
		{
			if(attr_entry[i]->attr_flags & (DMU_F_SEC_KEY))
			{
				/*
	...
	*/
        if (gateway)
	{
	    relrecord.relstat |= TCB_GATEWAY;
	    if (gwnoupdt)
		relrecord.relstat |= TCB_GW_NOUPDT;
	...
	if (has_extensions)
	    relrecord.relstat2 |= (TCB2_HAS_EXTENSIONS | TCB2_BSWAP);
	...
	*/
	status = dm2r_put(rel_rcb, DM2R_DUPLICATES, (char *)&relrecord, &error);
	if (status != E_DB_OK)
	{
	    ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
		(char *)0, (i4)0, (i4 *)0, &local_error, 0);
	    log_err = E_DM9026_REL_UPDATE_ERR;
	    break;
	}
	...
	if (attr_rcb != NULL)
	{
		...
		*/
		if ((relstat & TCB_CATALOG) &&
		     attr_entry[i]->attr_flags & (DMU_F_SEC_KEY))
			    continue;
		...
		attrrecord.attfmp = attr_entry[i]->attr_prec;
		attrrecord.attkey = 0;
		attrrecord.attflag = attr_entry[i]->attr_flags;
		...
		if (status != E_DB_OK)
		{
		    ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			    (char *)0, (i4)0, (i4 *)0, &local_error, 0);
		    log_err = E_DM9028_ATTR_UPDATE_ERR;
		    break;
		}
	    }
	    ...
	    */
	    status = dm2r_unfix_pages(attr_rcb, &error);
	    if (status != E_DB_OK)
	    {
		ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, &local_error, 0);
	...
	if (has_extensions && !view)
	{
		...
		*/
		status = dmpe_create_extension(xcb, &relrecord,
			...
			loc_array, loc_count,
			i + 1, attr_entry[i]->attr_type, &etab_record, &error);
	...
	if (loc_count > 1)
	{
		...
		status = dm2r_put(dev_rcb, DM2R_DUPLICATES,
                                  (char *)&devrecord, &error);
		if (status != E_DB_OK)
		{
		    ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
			(char *)0, (i4)0, (i4 *)0, &local_error, 0);
		    log_err = E_DM9048_DEV_UPDATE_ERROR;
	...
	if (gateway)
	{
	    ...
	    */
		...
		if (local_status)
		{
		    status = local_status;
		    error = local_error;
		    ule_format(local_error, 0, ULE_LOG, NULL, 0, 0, 0,
				&local_error, 0);
		    break;
		}
	    }
		...
		xcb, gwsource, gw_id, &tup_cnt, &pg_cnt, char_array, &error);
	    if (status)
		break;
	    ...
	    */
						...
						table_id.db_tab_index))
		{
			...
			(char*)&base_relrecord, (char *)0, &error);
		    if (status != E_DB_OK )
			break;
		}
	    }
	...
	*/
	if ((xcb->xcb_flags & XCB_NOLOGGING) == 0)
	{
		...
		table_name, owner, (i4)DM0LCREATE, (LG_LSN *)0, &error);
	    if (status != E_DB_OK)
		break;
	}
	...
	if (! nofile_create)
	{
	    ...
	    status = dm2f_release_fcb(xcb->xcb_lk_id, xcb->xcb_log_id,
		    loc_array, loc_count, DM2F_ALLOC, &error);
	    if (status != E_DB_OK)
		break;
	}
	...
	if (log_err && error > E_DM_INTERNAL)
	{
	    ule_format(log_err, (CL_SYS_ERR *)0, ULE_LOG, NULL,
    ...
    */
	...
	local_status = dm2f_release_fcb(xcb->xcb_lk_id, xcb->xcb_log_id,
	    loc_array, loc_count, DM2F_ALLOC, &local_error);
	if (local_status)
	{
	    if (error == E_DB_OK)
		error = local_error;
