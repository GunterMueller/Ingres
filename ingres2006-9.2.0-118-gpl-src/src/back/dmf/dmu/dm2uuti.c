...
i4		    *err_code)
{
	...
	/*
	** Use LZW compression for data (nonindex)
	**  non-CAfs pages
	*/
	if (m->mx_compressed)
	{
	    if (t->tcb_rel.relstat & TCB_CATALOG)
		mct->mct_compressed = TCB_C_DEFAULT;
	    else
	    {
	      if (m->mx_compressed == DMU_C_ON)
		mct->mct_compressed = TCB_C_DEFAULT;
	      else
		mct->mct_compressed = TCB_C_HICOMPRESS;
	    }
	}
	else
	{
	    mct->mct_compressed = TCB_C_NONE;
	}
	...
	*/
	...
	/* Initialise the compression accessors in the MCT */
	dm1c_getaccessors(mct->mct_compressed,
	    m->mx_page_type, m->mx_page_size,
	    &mct->mct_acc_plv, &mct->mct_acc_tlv);
	...
	**	compression is in use, the build routines need the attributes for the
	**	index entry -- for a primary table, this is the set of 'key'
	**	attributes, whereas for a secondary index, this is the full set of
	**	attributes.
	*/
	...
	if (m->mx_operation == MX_INDEX)
	{
	    ...
	    mct->mct_index = TRUE;
	    mct->mct_relatts = m->mx_ab_count + 1;
	    mct->mct_data_atts = m->mx_data_atts;
	    mct->mct_index_atts = m->mx_data_atts;
	    mct->mct_index_attcount = mct->mct_relatts;
	    mct->mct_index_maxlen = m->mx_width + m->mx_comp_katts_count;
	}
	else
	{
	    mct->mct_relatts = t->tcb_rel.relatts;
	    mct->mct_data_atts = t->tcb_data_atts;
	    mct->mct_index = t->tcb_rel.relstat & TCB_INDEX;
	    if ( mct->mct_index )
	    {
		mct->mct_key_atts = m->mx_i_key_atts;
		...
		mct->mct_index_atts = mct->mct_key_atts;
		mct->mct_index_attcount = mct->mct_keys;
		mct->mct_index_maxlen = m->mx_kwidth + m->mx_comp_katts_count;
	    }
	}
    }
	...
	*/
	...
	else
	{
		...
		*err_code = 0;
		...
		if ( tp->tpcb_srt )
		    status = dmse_input_end(tp->tpcb_srt, err_code);
	    }
	}
	if (DMZ_SRT_MACRO(11))
...

	...
	*/
	...
	else
	{
	    ...
	    if (!(m->mx_flags & MX_ROLLDB_RESTART_LOAD))
		status = OpenAllTargets(m, err_code);

	    switch_counter = 1000;
	    while ( !m->mx_truncate && status == E_DB_OK )
	    {
		if ( m->mx_state & MX_PRESORT )
		    status = NextFromAllSorts(m, &tp, &record, err_code);
		else
		    status = NextFromAllSources(m, &sp, &tp, &record, 
						    &tid8, err_code);
		...
		if ( DB_SUCCESS_MACRO(status) )
		    status = PutToTarget(m, tp, record, status, err_code);
		...
		*err_code = 0;
	    ...
	    if ( status == E_DB_OK )
		status = CloseAllTargets(m, err_code);
	}
    }

	...
	*err_code = m->mx_err_code;
	...
	if ( tp->tpcb_srt )
	{
	    local_status = dmse_end(tp->tpcb_srt, &local_err_code);
	    /* Don't log errors that are artifacts of error propagation */
	    if ( local_status != E_DB_OK
	      && local_err_code != E_CU0204_ASYNC_STATUS
	      && local_err_code != E_DM9714_PSORT_CFAIL)
	    {
	...
	*/
	for (tp = m->mx_tpcb_next; tp; tp = tp->tpcb_next)
	{
	    local_status = dm2f_force_file(tp->tpcb_locations, 
			...
			&m->mx_dcb->dcb_name, &local_err_code);
	...
	*err_code = E_DM9100_LOAD_TABLE_ERROR;
    }
    if (DMZ_SRT_MACRO(11))
...
i4		*err_code)
{
    ...
    {
	status = NextFromSort(m, tp, tp_p, record, err_code);
...
i4		*err_code)
{
    ...
    if ( Ptp->tpcb_srt )
    {
	status = dmse_get_record(Ptp->tpcb_srt, record, err_code);
	...
	{
	    /* End this sort */
	    (VOID)dmse_end(Ptp->tpcb_srt, &local_error);
...
i4		*err_code)
{
    ...
    {
	status = NextFromSource(m, sp, tp_p, record, tid8p, err_code);
...
(%d,%d)\n",
	...
	*err_code = 0;
	...
	if ( sp->spcb_rcb->rcb_xcb_ptr )
	    dmxCheckForInterrupt(sp->spcb_rcb->rcb_xcb_ptr, err_code);
	...
	*/
	...
	else
	{
	    ...
	    {
		adf_cb = (ADF_CB*)sp->spcb_rcb->rcb_adf_cb;
		...
		*/
		...
		if ( (*tp_p = m->mx_tpcb_ptrs[partno]) != NULL )
		{
			...
			ule_format(E_DM0025_PARTITION_MISMATCH, 0, ULE_LOG, NULL,
			    (char *) 0, 0L, (i4 *)0, &local_error, 4,
		...
		else
		{
		    ...
		    t = sp->spcb_rcb->rcb_tcb_ptr;
                    ...
                    ule_format(E_DM0024_NO_PARTITION_INFO, 0, ULE_LOG, NULL,
...
i4		*err_code)
{
    ...
    while ( tp && (status = OpenTarget(tp, err_code)) == E_DB_OK )
	tp = tp->tpcb_next;
...
i4		*err_code)
{
    ...
    {
	case TCB_HEAP:
	    status = dm1sbbegin(&tp->tpcb_mct, err_code);
	    break;
	case TCB_ISAM:
	    status = dm1ibbegin(&tp->tpcb_mct, err_code);
	    break;
	case TCB_HASH:
	    status = dm1hbbegin(&tp->tpcb_mct, err_code);
	    break;
	case TCB_BTREE:
	    status = dm1bbbegin(&tp->tpcb_mct, err_code);
	    break;
	case TCB_RTREE:
	    status = dm1mbbegin(&tp->tpcb_mct, err_code);
	    break;
    }
...
i4		*err_code)
{
    ...
    while ( tp && (status = CloseTarget(tp, err_code)) == E_DB_OK )
	tp = tp->tpcb_next;
...
i4		*err_code)
{
    ...
    {
	case TCB_HEAP:
	    status = dm1sbend(&tp->tpcb_mct, err_code);
	    break;
	case TCB_ISAM:
	    status = dm1ibend(&tp->tpcb_mct, err_code);
	    break;
	case TCB_HASH:
	    status = dm1hbend(&tp->tpcb_mct, err_code);
	    break;
	case TCB_BTREE:
	    status = dm1bbend(&tp->tpcb_mct, err_code);
	    break;
	case TCB_RTREE:
	    status = dm1mbend(&tp->tpcb_mct, err_code);
	    break;
    }
	...
	*/
	...
	else
	{
	    ...
	    */
	    if (m->mx_structure == TCB_HASH)
	    {
		bucket = dm1h_newhash(m->mx_i_key_atts, m->mx_ai_count,
		    tp->tpcb_irecord, mct->mct_buckets); 
	...
	if ( m->mx_structure == TCB_HASH )
	{
	    ...
	    */
	    bucket = dm1h_newhash(katts, kcount, record,
				  mct->mct_buckets);
    ...
    if ( status == E_DB_OK )
	status = dmse_put_record(Ptp->tpcb_srt, record, err_code);
...
i4		*err_code)
{
	...
	*/
	...
	status = dm0m_allocate((i4)sizeof(DMP_MISC) + size,
	    ...
	    (char *)0, (DM_OBJECT**)&tp->tpcb_srt_wrk, err_code);
...
i4		*err_code)
{
	...
	if  ((m->mx_dcb->dcb_status & DCB_S_ROLLFORWARD) == 0)
	    local_status = dm2u_put_dupchktbl(tp, record, err_code);
	...
	*err_code = E_DM0045_DUPLICATE_KEY;
	...
	case TCB_HEAP:
	    ...
	    status = dm1sbput(mct, record, m->mx_width, (i4)0, err_code);
	    break;
	    ...
	    */
		...
		else while ( (m->mx_state & MX_ABORT) == 0 && status == E_DB_OK )
		{
		    ...
		    */
		    ...
		    else if ( status && err_code == E_DM0055_NONEXT &&
			     m->mx_state & MX_SYMMETRIC )
		    {
			/*
			...
			*/
			...
			if ( status && err_code == E_DM0055_NONEXT )
			{
			    ...
			    if ( (tp->tpcb_state & MX_OPENED) == 0 )
				local_status = OpenTarget(tp, &local_err);

			    if ( local_status == E_DB_OK )
				local_status = CloseTarget(tp, &local_err);
		...
		*/
		if ( state & MX_END )
		{
		    ...
		    */
			...
			{
			    status = CloseTarget(tp, &err_code);
			}
		    }
		}
		...
		else if ( state & MX_LOADFROMSORT && Ptp->tpcb_srt )
		{
		    ...
		    status = dmse_input_end(Ptp->tpcb_srt, &err_code);
			...
			if (++ counter >= 10000 /* arbitrary */ )
			{
			    ...
			    */
				...
				status = E_DB_ERROR;
				err_code = E_CU0204_ASYNC_STATUS;
				break;
			    }
			    counter = 0;
			}
			...
			*/
			...
			if ( DB_SUCCESS_MACRO(status) )
			{
			    ...
			    */
			    ...
			    {
				if ( (PrevTtp &&
				       CloseTarget(PrevTtp, &err_code))
				    || OpenTarget(Ttp, &err_code) )
				{
				    status = E_DB_ERROR;
				}
				else
		    ...
		    */
		    if ( status && err_code == E_DM0055_NONEXT )
			status = CloseTarget(Ttp, &err_code);
		} /* if LOADFROMSORT */
...
i4             	    *err_code)
{
	...
	if (m->mx_reorg == 0)
	{
	    ...
	    */
	    ...
	    status = dm2t_close(r, 0, err_code);
	    if (status != E_DB_OK)
		break;
	    r = 0;
		...
		*/
		...
		status = dm2t_close(r, DM2T_NOPURGE, err_code);
		if (status != E_DB_OK)
		    break;
		r = 0;
	    }
		...
		*/
		for (devices_rows = 0;; devices_rows++)
		{
		    ...
		    */
		    ...
		    status = dm2r_replace(r, &tid, DM2R_BYPOSITION,
				 (char *)&devrecord, (char *)0, err_code);
		    if (status != E_DB_OK)
			break;
		}
		if (status != E_DB_OK)
	...
	*/
	...
	for (;;)
	{
	    ...
	    ** including partitions, just what we want 
	    */
	    status = dm2r_get(r, &tid, DM2R_GETNEXT,
                         (char *)&rel, err_code);
	    if (status != E_DB_OK)
		...
		*/
		...
		if (m->mx_reorg == 0)
		{
		    ...
		    /*
		    ** Use LZW (HI-COMPRESS) for data pages (non-index),
		    ** standard, and non-catalog tables.
		    */
		    if (m->mx_compressed)
		    {
			rel.relstat |= TCB_COMPRESSED;
			if (rel.relstat & TCB_CATALOG)
			    rel.relcomptype = TCB_C_DEFAULT;
			else
			{
			  if (m->mx_compressed == DMU_C_ON)
			    rel.relcomptype = TCB_C_DEFAULT;
			  else
			    rel.relcomptype = TCB_C_HICOMPRESS;
			}
		    }
		    else
			rel.relcomptype = TCB_C_NONE;
	...
	if (status != E_DB_OK)
	{
	    ule_format(*err_code, 0, ULE_LOG, NULL, 0, 0, 0, &local_error, 0);
	    if (*err_code > E_DM_INTERNAL)
	    {
		ule_format(E_DM9026_REL_UPDATE_ERR, 0, ULE_LOG, NULL,
    ...
    */
	...
	if ( status == E_DB_OK )
	{
	    ...
	    */
		...
		(i4)XCCB_ASCII_ID, (char *)m->mx_xcb,
		...
		STRUCT_ASSIGN_MACRO(locArray[k].loc_ext->physical,
					xccb->xccb_p_name);
		xccb->xccb_q_next = m->mx_xcb->xcb_cq_next;
		xccb->xccb_q_prev = (DML_XCCB*) &m->mx_xcb->xcb_cq_next;
		m->mx_xcb->xcb_cq_next->xccb_q_prev = xccb;
		m->mx_xcb->xcb_cq_next = xccb;
		xccb->xccb_xcb_ptr = m->mx_xcb;
		xccb->xccb_sp_id = m->mx_xcb->xcb_sp_id;
...
i4		*err_code)	/* output, error code */
{
    DM0C_CNF	*config;
    DB_STATUS	status;
    i4	    	id;

    status = dm0c_open(dcb, DM0C_PARTIAL, xcb->xcb_lk_id, &config, err_code);

    if ( status == E_DB_OK )
    {
	/*	Compute next table id. */

	/* Check that "numids" doesn't blow through 0x3ffffff */
	if (config->cnf_dsc->dsc_last_table_id + numids >= 0x4000000)
	{
	    (VOID) dm0c_close(config, 0, err_code);
	    *err_code = E_DM0021_TABLES_TOO_MANY;
	    status = E_DB_ERROR;
	}
	else
	{
	    ...
	    status = dm0c_close(config, DM0C_UPDATE | DM0C_COPY, err_code);
	}
    }
...
i4	    *err_code)
{
	...
	*err_code = E_DM004B_LOCK_QUOTA_EXCEEDED;
    ...
    {
          ule_format(E_DM9071_LOCK_TIMEOUT_ONLINE_CKP, &sys_err, ULE_LOG, NULL,
...
i4             *err_code)
{
	...
	*/
	if ( m->mx_readonly == MXCB_UNSET_READONLY )
	{
		...
		*/
	    ...
	    /*
	    ** If error updating the secondaru indicies then bail
	    */
	    if ( status != E_DB_OK )
		break;
        }
	...
	*/
	...
	ule_format( E_DM930D_DM2U_READONLY_UPD_CATS, (CL_SYS_ERR *)0,
		    ULE_LOG, NULL, (char *)0,
...
i4		*err_code)
{
	...
	*/
	status = dm2f_create_file(lock_id, log_id, loc_entry, 1, page_size,
	    db_sync_flag, (DM_OBJECT *)dcb, err_code);
	...
	if (*err_code > E_DM_INTERNAL)
	{
	    ...
	    *err_code = E_DM9441_DM2U_FILE_CREATE;
	}
	return (E_DB_ERROR);
    }
...
i4             *err_code)
{
	...
	*/
	...
	status = dm2t_open(m->mx_dcb, &table_id, DM2T_IX,
		DM2T_UDIRECT, DM2T_A_WRITE,
		(i4)0, (i4)20, (i4)0,
		m->mx_xcb->xcb_log_id,m->mx_lk_id,
		(i4)0, (i4)0, DM2T_S, &m->mx_xcb->xcb_tran_id,
                &timestamp, &rcb, (DML_SCB *)0, err_code);
	if (status != E_DB_OK)
	    break;
	...
	*/
	...
	for (;;)
	{
		...
		*/
		if ( rel.reltid.db_tab_base != m->mx_table_id.db_tab_base ||
		    !(m->mx_rcb->rcb_tcb_ptr->tcb_rel.relstat 
			    & TCB_IS_PARTITIONED) ||
		     rel.relstat & TCB_INDEX )
		{
		    continue;
	    ...
	    */
	    if (m->mx_new_relstat2 & DM2U_2_LOG_INCONSISTENT)
	      ...
	      rel.relstat2 &= ~TCB2_LOGICAL_INCONSISTENT;
	    else if (m->mx_new_relstat2 & DM2U_2_PHYS_INCONSISTENT)
	      rel.relstat2 |= TCB2_PHYS_INCONSISTENT;
	    else if (m->mx_new_relstat2 & DM2U_2_PHYS_CONSISTENT)
	      rel.relstat2 &= ~TCB2_PHYS_INCONSISTENT;
	    else if (m->mx_new_relstat2 & DM2U_2_TBL_RECOVERY_DISALLOWED)
	      rel.relstat2 |= TCB2_TBL_RECOVERY_DISALLOWED;
	    else if (m->mx_new_relstat2 & DM2U_2_TBL_RECOVERY_ALLOWED)
	      rel.relstat2 &= ~TCB2_TBL_RECOVERY_DISALLOWED;
	    else if (m->mx_new_relstat2 & DM2U_2_PERSISTS_OVER_MODIFIES)
	      rel.relstat2 |= TCB_PERSISTS_OVER_MODIFIES;
	    else if (m->mx_new_relstat2 & DM2U_2_STATEMENT_LEVEL_UNIQUE)
...
i4		*err_code)
{
    ...
    */
    ...
    status = dm2r_position(r, DM2R_QUAL, qual_list, (i4)1, (i4)0, 
			   (i4)0, (DM_TID *)0, err_code);
    ...
    for (;;)
    {
	status = dm2r_get(r, &tid, DM2R_GETNEXT, (char *)&dbcomment, err_code);
	if (status != E_DB_OK)
	{
	    if (*err_code == E_DM0055_NONEXT)
		status = E_DB_OK;
	    ...
	    dbcomment.dbc_tabid.db_tab_index == m->mx_table_id.db_tab_index )
	{
	   if (attr_map[dbcomment.dbc_attid] == 1)
	   {
	     ...
	     *comment = 1;
	     /* replace the row */
             status = dm2r_replace(r, &tid, DM2R_BYPOSITION,
                             (char *)&dbcomment, (char *)0, err_code);
	     if (status != E_DB_OK)
		break;
	   }
	}
...
i4		    *err_code)
{
    ...
    status = dm2t_open(dcb, &table_id, DM2T_IX,
		...
		&xcb->xcb_tran_id, &timestamp, &rcb, (DML_SCB *)0, err_code);
				...
				sizeof(DB_LOC_NAME)) == 0)
	{
	    ...
	    *err_code = E_DM0190_RAW_LOCATION_OCCUPIED;
	    return(E_DB_ERROR);
	}
    }
    /* Close the table. */
...
)
{
	...
	if ((mct->mct_index) && !(mct->mct_unique) && 
	    ...
	    (m->mx_atts_ptr[mct->mct_relatts].key != 0))
	{
	...
	if (!(tp->tpcb_dupchk_rcb) && !(new_tabid->db_tab_base))
	{
	    ...
	    */
	    ...
	    mem_needed = sizeof(DMP_MISC) +
		    (i4)(attr_count * sizeof(DM2T_ATTR_ENTRY)) +
		    (i4)(attr_count * sizeof(DM2T_ATTR_ENTRY *)) +
	    ...
	    attrs = (DM2T_ATTR_ENTRY *)((char *)misc_cb + sizeof(DMP_MISC));
	    misc_cb->misc_data = (char*)attrs;
	    attr_ptrs = (DM2T_ATTR_ENTRY **)((char *)attrs + 
			    (attr_count * sizeof(DM2T_ATTR_ENTRY)));
	    pos = key = (char *)attr_ptrs + (attr_count * sizeof(DM2T_ATTR_ENTRY *));
	    ...
	    */
		...
		attrs[i].attr_prec = keyatt->precision;
		attrs[i].attr_flags = keyatt->flag;
...
columns = %d, length = %d\n",
	...
	if (!(tp->tpcb_dupchk_rcb))
	{
           /* Open $dupchk table */
            ...
            if (status)
            {
                ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char * )0,
                                    (i4)0, (i4 *)0, err_code, 0);
                *err_code = E_DM008F_ERROR_OPENING_TABLE;
                break;
            }
	...
	status = dm2r_put(tp->tpcb_dupchk_rcb, DM2R_DUPLICATES, key, err_code);
	if (status)
	{
	    DMP_TCB	*dupchk_tcb = tp->tpcb_dupchk_rcb->rcb_tcb_ptr;

	    ule_format(*err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char *)0,
		    (i4)0, (i4 *)0, err_code, 0);
	    ule_format(E_DM904D_ERROR_PUTTING_RECORD, 0, ULE_LOG, NULL,
		(char * )0, 0L, (i4 *)0, err_code, 3,
	    ...
	    *err_code = E_DM008B_ERROR_PUTTING_RECORD;
	}
	if (DMZ_SRT_MACRO(13))
...
i4		*err_code)
{
    ...
    */
	...
	if (status != OK && status != LK_VAL_NOTVALID)
	{
		...
		ule_format(E_DM9046_TABLE_NOLOCKS, &sys_err,
		    ULE_LOG, NULL, 0, 0, 0, &local_error, 2, 9, "TEMPORARY",
		...
		*err_code = E_DM0065_USER_INTR;
	    ...
	    ule_format( status, &sys_err, ULE_LOG , NULL, (char * )0, 0L,
		    (i4 *)0, &local_error, 0);
		...
		*err_code = E_DM0042_DEADLOCK;
	    ...
	    {
		*err_code = E_DM0077_BAD_TABLE_CREATE;
		break;
	    }
	}

	if (tti_k_value.lk_low == 0)
	    tti_k_value.lk_low = MINI4;
	if (++tti_k_value.lk_low == 0xffffffff)
	    tti_k_value.lk_low = MINI4;
	...
	*/
	...
	if (status != OK)
	{
	    ...
	    *err_code = E_DM901B_BAD_LOCK_RELEASE;
	    break;
	}
	...
	*/
	...
	if ( status != LK_NEW_LOCK)
	{
	    ...
	    {
		ule_format(E_DM9046_TABLE_NOLOCKS, &sys_err,
			   ULE_LOG, NULL, 0, 0, 0, &local_error, 2, 9,
			   "TEMPORARY",
			   sizeof(DB_DB_NAME),
			   &dcb->dcb_name);
		*err_code = E_DM004B_LOCK_QUOTA_EXCEEDED;
	...
	*/
				    ...
				    sizeof(DB_LOC_NAME)) == 0 )
	{
	    ...
	    */
	    ...
	    if (avail_locs < 1)
	    {
		*err_code = E_DM013E_TOO_FEW_LOCS;
		break;
	    }
	    if (avail_locs > DM_WRKLOC_MAX)
	    {
		*err_code= E_DM013F_TOO_MANY_LOCS;
		break;
	    }
	    num_locs = avail_locs;
