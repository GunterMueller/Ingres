...
i4             *err_code)
{
	...
	*err_code = E_DM0188_PINDEX_CRERR;
	return (E_DB_ERROR);
    }
    ...
    */
    ...
    status = dm0m_allocate((i4)sizeof(DMP_MISC) + exch_tot_size + reclen,
                ...
                (char *)0, (DM_OBJECT **)&buf_p, err_code);
    if (status != E_DB_OK)
    {
	return(status); 
    }
    ...
    status = create_index_threads(mxcbs, ccb, exch_hdr, 
			(EXCH_BUF *)exch_buf, no_of_ind, err_code);
    if ( (status == E_DB_OK) && 
	...
	*/
	...
	if ( local_status == E_DB_OK && *(m->mx_rcb->rcb_uiptr) )
	{
	    *err_code = 0;
		...
		dmxCheckForInterrupt(m->mx_rcb->rcb_xcb_ptr, err_code);
    ...
    */
    ...
    if ( status == E_DB_OK )
	status = check_child_status(ccb, no_of_ind, err_code);
...
i4		*err_code)
{
	...
	ccb->ccb_status = 0;
	ccb->ccb_error.err_code = 0;
	ccb->ccb_error.err_data = 0;
...
build_pindex(SCF_FTX *ftx)
{
    ...
    if ( (ccb->ccb_status = init_child_thread(tp, &wloc_mask, &ccb->ccb_error.err_code)) )
    ...
    */
	...
	if ( noofrecord = cur_buf->exch_noofrec )
	{
		...
		nth = cur_buf->exch_noofrec - noofrecord;
            	get_record(exch_hdr, cur_buf, 
					   tp, nth, reclen, &tid);
 	    	ccb->ccb_status = load_sorter(tp, &tid, &ccb->ccb_error.err_code);		
	    	noofrecord--;
	    }
	...
	{
	    if (allread)
		ccb->ccb_status = dmse_input_end(tp->tpcb_srt, &ccb->ccb_error.err_code);
    ...
    if ( ccb->ccb_status == E_DB_OK )
        ccb->ccb_status = load_index(tp, wloc_mask, &ccb->ccb_error.err_code);
    else 
    {
	if ( dmse_end(tp->tpcb_srt, &local_err_code) )
        {
            ule_format(local_err_code, (CL_SYS_ERR *)0, ULE_LOG, NULL,
                    (char *)0, (i4)0, (i4 *)0, &local_err_code, 0);
	    if (local_err_code > ccb->ccb_error.err_code)
		ccb->ccb_error.err_code = local_err_code;
        }
	tp->tpcb_srt = 0;
    }
    if (wloc_mask != NULL)
...
)
{
    ...
    *err_code = 0;
	...
	if ( ccb->ccb_status )
	{
	    ...
	    if (ccb->ccb_error.err_code > *err_code)
		*err_code = ccb->ccb_error.err_code;
	    status = E_DB_ERROR;
	}
    }
    return(status);
...
i4	*err_code)
{
	...
	if (local_status == E_DB_ERROR && *err_code == E_DM0055_NONEXT)
	{
...
i4    *err_code)
{
    ...
    */
    ...
    status = dmse_put_record(tp->tpcb_srt, tp->tpcb_irecord, err_code);
    return(status);
}
...
i4	    *err_code)
{
    ...
    */
	...
	case TCB_HASH:
		...
		status = dm1hbput(mct, bucket,
		    tp->tpcb_irecord, m->mx_width, (i4)0, err_code);
		if (status != E_DB_OK)
		    break;
		tp->tpcb_newtup_cnt++;
	    }
    ...
    */
	...
	if (tp->tpcb_srt)
	{
	    status = dmse_end(tp->tpcb_srt, err_code);
	    tp->tpcb_srt = 0;
	    if (status != E_DB_OK)
		break;
	}
...
i4    *err_code)
{
    ...
    /*	Initialize the MCT for the load operations. */
    ...
    */
    if (m->mx_compressed)
    {
	if (m->mx_rcb->rcb_tcb_ptr->tcb_rel.relstat & TCB_CATALOG)
    ...
    mct->mct_allocation = m->mx_allocation;
    ...
    /*
    ** If creating a Global index, extract this
    ** partition's partition number, set the
    ** TID size appropriately.
    */
    if ( m->mx_new_relstat2 & TCB2_GLOBAL_INDEX )
    {
	mct->mct_partno = m->mx_rcb->rcb_tcb_ptr->tcb_partno;
    ...
    mct->mct_guarantee_on_disk =
		(m->mx_rcb->rcb_tcb_ptr->tcb_temporary == TCB_PERMANENT);
	...
	*/
	...
	if (m->mx_duplicates ||
	    (m->mx_rcb->rcb_tcb_ptr->tcb_rel.relstat & TCB_INDEX) ||
	    mct->mct_unique)
	{
    		sort_flag = 0;
	}
	...
	switch (m->mx_structure)
	{
		   ...
		   if (mct->mct_index == 0 || m->mx_unique)
		       sort_flag |= DMSE_CHECK_KEYS;
		   break;
	}
	...
	*/
	...
	status = dmse_begin(sort_flag, 
			...
			m->mx_rcb->rcb_ucollation, err_code);
	...
	*/
    ...
    if (status != E_DB_OK)
    {
	    DB_STATUS local_status = dmse_end(tp->tpcb_srt, &local_err_code);
	    if (local_status != E_DB_OK)
	    {
