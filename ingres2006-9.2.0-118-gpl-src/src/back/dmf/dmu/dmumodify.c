...
dmu_modify(DMU_CB    *dmu_cb)
{
    ...
    u_i4		statementLevelUnique = 0;
    u_i4		persistsOverModifies = 0;
    u_i4		systemGenerated = 0;
    u_i4		supportsConstraint = 0;
    u_i4		notUnique = 0;
    u_i4		notDroppable = 0;
    u_i4		rowSecAudit=0;
    bool		got_action;
    ...
    */
	...
	for (i = 0; i < chr_count; i++)
	{
		...
		{
		    error = E_DM0157_NO_BMCACHE_BUFFERS;
		    break;
		}		    
		else
		{
	    ...
	    case DMU_COMPRESSED:
		mcb->mcb_compressed = chr[i].char_value == DMU_C_ON;
		continue;
	...
	*/
	...
	else
	{
		...
		if (dmu->dmu_location.data_address &&
		    ...
		    error = E_DM001F_LOCATION_LIST_ERROR;
		break;	    
	    }
	}
	...
	*/
	...
	else if (status == E_DB_OK && has_extensions)
	{
		...
		status = dmpe_modify(dmu, odcb->odcb_dcb_ptr, xcb,
			    ...
			    truncate, (i4)0, blob_add_extend, &error);
	    }
	}
	...
	else
	{
		...
		error = E_DM0091_ERROR_MODIFYING_TABLE;
            }
	    switch (error)
	    {
