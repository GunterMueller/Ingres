...
DM2U_INDEX_CB   *index_cb)
{
    ...
    i4		    buf_size;
    char	    *bufArray;
    DMP_LOCATION    *locArray = (DMP_LOCATION*)NULL;
    DMP_LOCATION    *TlocArray;
    DM2U_SPCB	    *sp;
    DM2U_TPCB	    *tp;
    DM2U_M_CONTEXT  *mct;
    ...
    error = 0;
    err_data = &index_cb->indxcb_errcb->err_data;
    err_code = &index_cb->indxcb_errcb->err_code;
    *err_data = 0;
    *err_code = 0;
    ...
    */
	...
	/* TempTable table locks are on SCB lock list */
	if ( TempTable )
	...
	if ( TempTable )
	{
		...
		{
		    *err_code = E_DM0078_TABLE_EXISTS;
		    break;
		}
	    }
	    if ( *err_code )
	...
	** request.
	*/
	if ((!concurrent_index) && (!sconcur) && (ref_count > 1 ))
	{	
	...
	if ( concurrent_index )
	{
	    ...
	    */
	    ...
	    {
	        *err_code = E_DM0069_CONCURRENT_IDX_ERR;
		break;
	    }
	}
	...
	if ((dcb->dcb_status & DCB_S_ROLLFORWARD) == 0)
	{
	    ...
	    */
	    ...
	    {
		*err_code = E_DM005F_CANT_INDEX_CORE_SYSCAT;
		break;
	...
	{	
	    ...
	    for ( ClusterKeyCount; ClusterKeyCount < t->tcb_keys; ClusterKeyCount++ )
	...
	/* Allocate a set of 4 record buffers for each input source */
        ...
        buf_size += ClusterKeyLen + sizeof(DM_TID8) + sizeof(i4);
	...
	/* Allocate zero-filled memory */
        status = dm0m_allocate(sizeof(DM2U_MXCB) +
		 ...
		 buf_size,
                 ...
                 (char *)xcb, (DM_OBJECT **)&mxcb_cb, err_code);
        if (status != E_DB_OK)
            break;
        ...
        **
        **  The expression
        **  ((t->tcb_rel.relwid + m->mx_tidsize + sizeof(i4)
	...
	**	target locations
	*/
        sp = (DM2U_SPCB*)((char*)m->mx_att_map + (AllAttsCount + 3) * sizeof(i4));
	...
	/* Remember, "acount" is the count of key attributes... */
	...
	*err_code = E_DB_OK;
        ...
        {
	    error = E_DM001D_BAD_LOCATION_NAME;
                ...
                sizeof(DB_LOC_NAME)) == 0)
            {
                ...
                STRUCT_ASSIGN_MACRO(dcb->dcb_root->logical, locArray[k].loc_name);
                locArray[k].loc_id = k;
	...
	*/
	if ( dm2uMakeIndAtts(index_cb, m, r, att_list, &NumCreAtts, 
			FALSE, NULL, err_code, err_data) )
	    break;

        setrelstat2 = index_cb->indxcb_relstat2;              
					/* initialize 2nd flags column */

	/* Note this includes TCB2_GLOBAL_INDEX bit */
        m->mx_new_relstat2 = setrelstat2;
	...
	*/
	if ( TempTable )
	{
		...
		*index_cb->indxcb_tup_info = rel->reltups;
	    ...
	    /* Close the Base Temp Table from which it was built */
            (VOID)dm2t_close(r, DM2T_NOPURGE, &local_error);
	...
	if ((dcb->dcb_status & DCB_S_ROLLFORWARD) == 0)
	{
	    status = dm2u_create(dcb, xcb, 
	      ...
	      m->mx_range, m->mx_tbl_pri, NULL, 0, &local_errcb);
	    if (status != E_DB_OK)
	    {
	        /*
	        ...
	        */
	        ...
	        {
		    *err_code = local_errcb.err_code;
		    *err_data = local_errcb.err_data;
		    break;
	        }
	        status = E_DB_OK;
	    }
	    ...
	    */
	    status = dm0p_close_page(t, lk_id, log_id, DM0P_CLFORCE, err_code);
	    if (status != E_DB_OK)
		break;
	}
...

	...
	**  Data fields are supported for all index types.
	  ...
	  ** filename collision concerns.)
          */
          status = dm2f_build_fcb(lk_id, log_id,
			      locArray, loc_count, index_cb->indxcb_page_size,
                              DM2F_ALLOC, DM2F_MOD, &m->mx_table_id,
                              name_id, name_gen,
                              (DMP_TABLE_IO *)0, db_sync_flag, err_code);
          if (status != E_DB_OK)
            break;
	  ...
	  if ((dcb->dcb_status & DCB_S_ROLLFORWARD) == 0 && !online_index_build)
	  {
	      ...
	      */
	      status = dm2u_file_create(dcb, log_id, lk_id,
			&m->mx_table_id, db_sync_flag, locArray,
			loc_count,  m->mx_page_size,
	  ...
	  else
	  {
	      ...
	      */
		  ...
		  /* we want dm2u_load_table to initialize mct block only; do not sort */
	  ...
	  *index_cb->indxcb_tup_info = m->mx_newtup_cnt;
          ...
          */
          ...
          {
            status = dm2f_force_file(locArray, loc_count,
          ...
          */
          modfile_open = FALSE;
          status = dm2f_close_file(locArray, loc_count, (DM_OBJECT*)dcb, err_code);
          if (status != E_DB_OK)
            break;
        }
        else
	  ...
	  /* Use target's TPCB and context */
	  tp = m->mx_tpcb_next;
	  mct = &tp->tpcb_mct;

          switch (bbase_tcb->tcb_rel.relgwid)
          {
              case    DMGW_RMS:  /* or GW_RMS ? */
                tp->tpcb_newtup_cnt = 0;
                *index_cb->indxcb_tup_info = 0;
                break;
	...
	if (dcb->dcb_status & DCB_S_ROLLFORWARD)
	{
            status = dm2f_release_fcb(m->mx_lk_id, m->mx_log_id,
		     locArray, loc_count, DM2F_ALLOC, err_code);
            if (status != E_DB_OK)
                break;
	    ...
	    */
        ...
        status = dm2u_update_catalogs(m, journal, err_code);
        if (status != E_DB_OK)
            break;
    ...
    */
    if (modfile_open)
    {
        status = dm2f_close_file(locArray, loc_count,
    ...
    */
    if ((locArray) && (locArray[0].loc_fcb))
    {
        status = dm2f_release_fcb(lk_id, log_id,
		     locArray, loc_count, DM2F_ALLOC, &local_error);
        if (status != E_DB_OK)
        {
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL,
            (char *)0, (i4)0, (i4 *)0, &local_error, 0);
...
**	    indxcb_acount		Number of key attributes
**	    indxcb_kcount		Number of key+non-key attributes
**	m				DM2U_MXCB allocated by the caller
**					and presumed be correctly sized.
**	r				Base Table DMP_RCB.
**	CreAtts				A place to build DM2T_ATTR_ENTRY
...
**	    mx_kwidth
**	    mx_comp_katts_count
**	NumCreAtts			Number of DM2T_ATTR_ENTRY things
...
i4		*error_data)
{
	...
	upcase = *index_cb->indxcb_xcb->xcb_scb_ptr->scb_dbxlate & CUI_ID_REG_U;
	...
	** cx indexes the DM2T_ATTR_ENTRY atts for dm2u_create (CreAtts)
	** dx indexes the data attributes (mx_data_atts) for compression
	...
	*/
	...
	/*
	** If ISAM, reserve 1st compare attribute for hash bucket;
	** If RTREE, reserve 1st two compare attributes, first for
	** HILBERT value, second for TIDP.
	*/
        if (index_cb->indxcb_structure == TCB_HASH)
	  mx = 1;
	else if (index_cb->indxcb_structure == TCB_RTREE)
	  mx = 2;

	/* For each user-named attribute in the Index... */
        for ( i = 0; i < index_cb->indxcb_kcount; i++, ax++, bx++, cx++, dx++ )
        {
	    ...
	    **  e.g. hilbertsize = 6 => NBR is 6 bytes * 2 or 12
            ...
            else
	    {
	      ...
	      coltype = t->tcb_atts_ptr[j].type;
            ...
            CreAtts[cx]->attr_prec = t->tcb_atts_ptr[j].precision;
            CreAtts[cx]->attr_flags = t->tcb_atts_ptr[j].flag;
	    ...
	    */
	    ...
	    STRUCT_ASSIGN_MACRO(t->tcb_atts_ptr[j], m->mx_atts_ptr[ax]);
            ...
            m->mx_data_atts[dx] = &m->mx_atts_ptr[ax];

            if (i < index_cb->indxcb_acount)
            {
		...
		OnlyKeyLen += collength;
            if (i < index_cb->indxcb_acount || index_cb->indxcb_structure == TCB_BTREE) 
	...
	if ( t->tcb_rel.relstat & TCB_CLUSTERED )
	{
		...
		*/
		...
		if ( !(AttMap[k >> 5] & (1 << (k & 0x1f))) )
		{
		    ...
		    CreAtts[cx]->attr_prec = t->tcb_key_atts[j]->precision;
		    CreAtts[cx]->attr_flags = t->tcb_key_atts[j]->flag;
		    ...
		    */
		    ...
		    if ( index_cb->indxcb_structure == TCB_BTREE )
	...
	*/
        ...
        /*  For index compression use, calculate the overhead added by
        **  compressing these attributes.
        */
        m->mx_comp_katts_count = (*t->tcb_acc_tlv->dmppxcompexpand)
                                     (m->mx_data_atts, dx);
	    ...
	    m->mx_atts_ptr[ax].offset = m->mx_width;
	    m->mx_atts_ptr[ax].key = index_cb->indxcb_acount;
	    ...
	    m->mx_data_atts[dx] = &(m->mx_atts_ptr[ax]);
	    m->mx_i_key_atts[1] = &(m->mx_atts_ptr[ax]);
	    ...
	    CreAtts[cx]->attr_prec = 0;
	    CreAtts[cx]->attr_flags = 0;
	...
	*NumCreAtts = cx;
	...
	if (m->mx_page_type == TCB_PG_INVALID)
	{
        ...
        */
	if (m->mx_width + 
	    (index_cb->indxcb_compressed ? t->tcb_comp_atts_count : 0) >
		dm2u_maxreclen(m->mx_page_type, m->mx_page_size))
	{
	    *error = E_DM0103_TUPLE_TOO_WIDE;
	    break;
	}
	if (index_cb->indxcb_structure == TCB_ISAM)
	{
	    if (m->mx_kwidth > 
	...
	if (index_cb->indxcb_structure == TCB_BTREE)
	{
	    ...
	    */
		...
		(index_cb->indxcb_index_compressed ? m->mx_comp_katts_count:0);
	    i4  max_leaflen;
...
i4		*error)
{
	...
	*/
	...
	if ( m->mx_compressed )
	{
	    ...
	    relrecord.relcomptype = TCB_C_STANDARD;
	}
	else
	...
	relrecord.relcomptype = m->mx_compressed;
