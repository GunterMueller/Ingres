...
**	    queries logivals, the installation code used by CX may
...
char		*lgk_info)
{
    ...
    */
    ...
    {
	lgk_calculate_size (FALSE, &lgkcount, &min_memory);
    }
    if (min_memory)
    ...
    */
	...
	if ( retries )
	{
		...
		ule_format(E_DMA800_LGKINIT_GETMEM, (CL_ERR_DESC *)0,
	...
	if ( mem_exists )
	{
	    ...
	    */
		...
		else
		{
		    ...
		    if (lgk_mem->mem_pid[i])
		    {
			ule_format(E_DMA499_DEAD_PROCESS_INFO, (CL_ERR_DESC *)0,
				ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
				0, lgk_mem->mem_pid[i],
				0, lgk_mem->mem_info[i].info_txt);
		    }
		    lgk_mem->mem_pid[i] = (PID)0;
		    lgk_mem->mem_info[i].info_txt[0] = NULL;
		...
		ule_format(E_DMA80A_LGK_ATTACH_LIMIT, (CL_ERR_DESC *)0,
	    ...
	    {
		ule_format(E_DMA434_LGK_VERSION_MISMATCH, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
	    ...
	    */
	    ...
	    {
		ule_format(E_DMA811_LGK_MT_MISMATCH, (CL_ERR_DESC *)0,
		    ULE_LOG, NULL, 0, 0, 0, &err_code, 2,
	...
	else
	{
	    ...
	    */
	    ...
	    {
		ule_format(ret_val, (CL_ERR_DESC *)0,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
		ret_val = E_DMA802_LGKINIT_ERROR;
...
CL_ERR_DESC *sys_err)
{
    ...
    if (ret_val)
    {
	ule_format(ret_val, (CL_ERR_DESC *)sys_err,
			    ULE_LOG, NULL, 0, 0, 0, &err_code, 0);
...
int	arg;
{
	...
	if ( lgk_mem )
	{
		...
		lgk_mem->mem_info[LGK_base.lgk_pid_slot].info_txt[0] = NULL;
	    }
	    CSv_semaphore(&lgk_mem->mem_ext_sem);
