...
DMC_CB    *dmc_cb)
{
    ...
    if (dmc_cb->dmc_op_type != DMC_SESSION_OP)
	dmc_cb->error.err_code = E_DM000C_BAD_CB_TYPE;
    else if (dmc_cb->dmc_flags_mask != 0L)
	dmc_cb->error.err_code = E_DM001A_BAD_FLAG;
    else if (!dmc_cb->dmc_name.data_address ||
	      dmc_cb->dmc_name.data_in_size != sizeof(DB_OWN_NAME))
	dmc_cb->error.err_code = E_DM002A_BAD_PARAMETER;
    else if ( (scb = GET_DML_SCB(dmc_cb->dmc_session_id)) == 0 || 
	      dm0m_check((DM_OBJECT *)scb, (i4)SCB_CB) )
	dmc_cb->error.err_code = E_DM002F_BAD_SESSION_ID;
    /* now reset the effective user name */
    else 
    {
	MEcopy(dmc_cb->dmc_name.data_address, sizeof(DB_OWN_NAME),
		    (PTR) &scb->scb_user);
	dmc_cb->error.err_code = 0;
	status = E_DB_OK;
    }
