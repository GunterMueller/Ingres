...
**		.dmh_numlocs		number of work avail locations
...

{
    ...
    static READONLY char    rad37[37] = {'a', 'b', 'c', 'd', 'e', 'f', 'g',
					 'h', 'i', 'j', 'k', 'l', 'm', 'n',
					 'o', 'p', 'q', 'r', 's', 't', 'u',
                                         'v', 'w', 'x', 'y', 'z', '0', '1',
                                         '2', '3', '4', '5', '6', '7', '8',
    ...
    numlocs = dmhcb->dmh_numlocs = BTcount((PTR)lm->locm_w_use, lm->locm_bits);

    /* The filename generator is (file counter*512) + (server ID number),
    ...
    *dmhcb->dmh_locidxp = i;		/* save back in caller */
    ...
    ule_doformat(E_DM9711_SR_OPEN_ERROR, &sys_err, ULE_LOG,
		NULL, 0, 0, 0, &dmhcb->error.err_code, 2, 
		locarray[i].phys_length,
		locarray[i].physical.name,
		12, filename);
    if (stat == SR_EXCEED_LIMIT)
	dmhcb->error.err_code = E_DM0112_RESOURCE_QUOTA_EXCEED;
...

{
    ...
    stat = SRwrite((SR_IO *)dmhcb->dmh_file_context, dmhcb->dmh_rbn, 
	dmhcb->dmh_buffer, &sys_err);
	...
	ule_doformat(E_DM9713_SR_WRITE_ERROR, &sys_err, ULE_LOG, NULL, 
	    0, 0, 0, &dmhcb->error.err_code, 2, 
...

{
    ...
    stat = SRread((SR_IO *)dmhcb->dmh_file_context, dmhcb->dmh_rbn, 
	dmhcb->dmh_buffer, &sys_err);
	...
	ule_doformat(E_DM9712_SR_READ_ERROR, &sys_err, ULE_LOG, NULL, 
	    0, 0, 0, &dmhcb->error.err_code, 2, 
	    locarray[locidx].phys_length,
	    locarray[locidx].physical.name, 0, dmhcb->dmh_rbn);
    }
