...
dmc_group_commit(DMC_CB	    *dmc_cb)
{
    ...
    i4	    error_code = E_DM014B_GROUP_COMMIT;
	...
	dmc_cb->dmc_id, timer_interval_ms);
#endif
    ...
    */
    ...
    MEcopy((PTR)DB_GROUPCOMMIT_NAME, sizeof(add_info.ad_dbname),
	...
	if (stat == LG_EXCEED_LIMIT)
	    ...
	    dmc_cb->error.err_code = E_DM014B_GROUP_COMMIT;
	return (E_DB_ERROR);
    }
	...
	MEcopy((PTR)DB_GROUPTHREAD_NAME, sizeof(DB_OWN_NAME), (PTR) &user_name);
    ...
    */
	...
	ule_format(cl_status, (CL_ERR_DESC *)&sys_err, ULE_LOG, NULL,
			(char *)0, 0L, (i4 *)0, &error_code, 0);
	error_code = E_DM014B_GROUP_COMMIT;
	status = E_DB_ERROR;
    }
