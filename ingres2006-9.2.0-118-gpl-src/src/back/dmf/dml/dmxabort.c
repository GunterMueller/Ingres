...
DMX_CB    *dmx_cb)
{
	...
	*/
	    ...
	    ((xcb->xcb_flags & XCB_DELAYBT) == 0))
	{
	    ...
	    ule_format(E_DM9051_ABORT_NOLOGGING, (CL_ERR_DESC *)0, ULE_LOG,
	     ...
	     scb->scb_ui_state & SCB_FORCE_ABORT )
	{
	    ...
	    */
	    ule_format(E_DM9059_TRAN_FORCE_ABORT, (CL_ERR_DESC *)0, 
	...
	*/
	...
	if ( savepoint_id == 0 && (xcb->xcb_seq || xcb->xcb_cseq) )
	    status = dms_end_tran(DMS_TRAN_ABORT, xcb, &error);
	...
	*/
	...
	while (xcb->xcb_rq_next != (DMP_RCB*) &xcb->xcb_rq_next)
	{
	    ...
	    */
	    ...
	    status = dm2t_close(rcb, close_flags, &error);
	    if (status != E_DB_OK)
	    {
	...
	*/
	status = dmxe_abort(odcb, (DMP_DCB *)0, &xcb->xcb_tran_id,
	    ...
	    savepoint_id, &spcb->spcb_lsn, &error); 
	if (status != E_DB_OK)
	{
	    if ((error != E_DM004B_LOCK_QUOTA_EXCEEDED) &&
	    	(error != E_DM0096_ERROR_ABORTING_TRAN) &&
	        (error != E_DM004D_LOCK_TIMER_EXPIRED) &&
	    ...
	    */
	    if (error != E_DM0100_DB_INCONSISTENT)
		break;
	...
	**  just aborted.
	      ...
	      xccb = next_xccb )
	{
	    ...
	    */
	    ...
	    {
		status = dmt_destroy_temp(scb, xccb, &error);
	...
	*/
	...
	if (savepoint_id == 0)
	{
	    ...
	    */
            if (stat != OK)
            {
                ule_format(stat, &sys_err, ULE_LOG, NULL, 0, 0, 0, &error, 0);
                ule_format(E_DM901B_BAD_LOCK_RELEASE, &sys_err, ULE_LOG, NULL,
                    (char*)0, (i4)0, (i4*)0, &error, 1, 0, xcb->xcb_lk_id);
	...
	*/
	...
	{
	    if (savepoint_id)
		error = E_DM0149_SETLG_SVPT_ABORT;
	    else
		error = E_DM0146_SETLG_XCT_ABORT;
	    break;
	}
		...
		*error = E_DM0065_USER_INTR;
		...
		else if ( length )
		{
			...
			if ( !(scb->scb_sess_mask & SCB_LOGFULL_COMMIT) ||
			    (status = dmx_logfull_commit(xcb, error)) )
			{
