...
dmc_cp_timer(DMC_CB	    *dmc_cb)
{
	...
	** was 0.
	*/
	dmc_cb->error.err_code = E_DM014F_CP_TIMER;
	return (E_DB_ERROR);
    }

    MEcopy((PTR)DB_CP_TIMER_NAME, sizeof(add_info.ad_dbname),
    ...
    if (stat != OK)
    {
	ule_format(stat, &sys_err, ULE_LOG, NULL, (char *)0, 0L,
		    (i4 *)0, &error, 0);
	...
	MEcopy((PTR)DB_CPTMRTHREAD_NAME, sizeof(DB_OWN_NAME), (PTR) &user_name);
	...
	if (stat != OK)
	{
	    ule_format(stat, &sys_err, ULE_LOG, NULL, (char *)0, 0L,
		    (i4 *)0, &error, 0);
	    ule_format(E_DM900C_BAD_LOG_BEGIN, &sys_err, ULE_LOG, NULL, (char *)0,
	...
	*/
	if (cl_status != E_CS0009_TIMEOUT)
	{
	    ...
	    */
	    ule_format(cl_status, (CL_ERR_DESC *)0, ULE_LOG, NULL,
			(char *)0, 0L, 
			(i4 *)0, &error_code, 0);
	    error_code = E_DM014A_CHECK_DEAD;
	    status = E_DB_ERROR;
	    break;
	}
