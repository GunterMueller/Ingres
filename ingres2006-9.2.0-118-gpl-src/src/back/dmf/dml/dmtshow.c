...
DMT_SHW_CB  *dmt_show_cb)
{
	...
	{
	    /*	Get the table identifer by name using the relation_idx table. */

	    status = 
             dm2r_rcb_allocate(odcb->odcb_dcb_ptr->dcb_rel_tcb_ptr->tcb_iq_next,
		(DMP_RCB *)0, (DB_TRAN_ID *)&scb->scb_lock_list, 
                scb->scb_lock_list, (i4)0,
		&rel_rcb, &error);
	    if (status != E_DB_OK)
		break;
	    ...
	    **
	    ** When a NONEXT is returned, search through the tables in the
	    ** TCB cache in case the requested table is a temporary which
	    ** will not be described in iirelation.
	    */
	    while ( status == E_DB_OK && rel_rcb )
	    {
		status = dm2r_get(rel_rcb, &tid, DM2R_GETNEXT, 
                                (char *)&rel_index, &error);
		if ( status == E_DB_OK &&
		...
		else if ( status && error == E_DM0055_NONEXT )
		{
		    /*
		    ** Table not found - try looking for temporary table
		    ** in the session's ODCB XCCB list:
		    */
		    (void) dm2r_release_rcb(&rel_rcb, &error);
		    rel_rcb = NULL;
		...
		*/
		status = dm2r_get(rel_rcb, (DM_TID *)&rel_index.tidp, 
			    DM2R_BYTID, (char *)&relation, &error);
		if (status != E_DB_OK)
		{
		    status = E_DB_ERROR;
		    if (error == E_DM0044_DELETED_TID)
			error = E_DM0054_NONEXISTENT_TABLE;
		    else
			error = E_DM010B_ERROR_SHOWING_TABLE;
	...
	*/
	if (!found_in_memory)
	{
		...
		lock_list, LK_S, (i4)0, timeout,   &error);
	...
	else
	{
	    ...
	    ** this is unusual, and may be an error, but let a higher
	    ** level announce any error;  don't puke all over the
	    ** place here.
	    */
		...
		status = dm2t_fix_tcb(dcb->dcb_id, &master_id, &tran_id,
			...
			dcb, &master_tcb, &error);
		if (status != E_DB_OK)
		    break;
	    }
	    status = dm2t_fix_tcb(dcb->dcb_id, &table_id, &tran_id, 
		lock_list, log_id,
		(DM2T_VERIFY | DM2T_NOPARTIAL | DM2T_SHOWONLY),
		dcb, &tcb, &error);
	    if (status != E_DB_OK)
	...
	for (i = 1; i <= tcb->tcb_rel.relatts; i++)	
	{
	    ...
	    /* Special hack to force "histogram" column of iihistogram
		...
		*/ 
			...
			tcb->tcb_rel.relnparts)
		{
			...
			if (!tcb_pp_ptr->pp_tcb)
			{
				...
				DM2T_VERIFY, lock_list, log_id, &local_error);
	    		    if (local_status != E_DB_OK)
			    {
			        ule_format(local_error, NULL, ULE_LOG, NULL,
			...
			} else
			{
				...
				DM1P_FSEG_MACRO(tcb_pp_ptr->pp_tcb->tcb_rel.relpgtype, tcb_pp_ptr->pp_tcb->tcb_rel.relpgsize) + 1;
			}
	...
	*/
    ...
    defined(LP64) || defined(NT_IA64) || defined(NT_AMD64)
		    ...
		    if (dm_ptr == NULL)
		    {
			error = E_DM013A_INDEX_COUNT_MISMATCH;
			status = E_DB_ERROR;
			break;
		    }
		    ix = (DMT_IDX_ENTRY *)dm_ptr->ptr_address;
	...
	*/
	if (control_lock)
	{
	    control_lock = FALSE;
	    status = dm2t_control(odcb->odcb_dcb_ptr, table_id.db_tab_base,
		lock_list, LK_N, (i4)0, (i4)0, &error);
	    if (status != E_DB_OK)
		break;
	}
	...
	*/
	local_status = dm2t_control(odcb->odcb_dcb_ptr, table_id.db_tab_base,
	    lock_list, LK_N, (i4)0, (i4)0, &local_error);
	if (local_status)
	{
            ule_format(local_error, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
		(char *)0, (i4)0, (i4 *)0, &local_error, (i4)0);
...
{                    
	...
	default:
	    dmt_cb->error.err_data = i;
	    dmt_cb->error.err_code = E_DM000D_BAD_CHAR_ID;
	    return (E_DB_ERROR);
	}
    }
    return (E_DB_OK);
