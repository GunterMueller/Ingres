...
**	be called with the DMR_ENLOAD flag.  This will make the table
**	available for normal record access.
**
**	The caller should not issue a SAVEPOINT command while in the process of
**	loading a table.
**
**	The table being loaded may already exist as a formatted Ingres
**	relation (but empty), or it may be a newly created relation with
**	no allocated pages.  If DMR_LOAD is called for an already existing
**	table, the DMR_TABLE_EXISTS flag should be set so that recover will
...
**	flag is not specified then the the table is assumed to be a temporary.
...
**
**      For structurned user tables, the first call to DMR_LOAD caused the
...
**						it is.
**					All flags except DMR_ENLOAD are checked
...
DMR_CB  *dmr_cb)
{
	...
	if (dm0m_check((DM_OBJECT *)rcb, (i4)RCB_CB) == E_DB_OK)
	{
	    ...
	    */
		...
		status = dmxe_writebt(xcb, journal_bt, &error);
		if (status != E_DB_OK)
		{
		...
		*/
		...
		if ((rcb->rcb_state & RCB_LSTART) == 0)
		{
		    ...
		    {
		        status = start_load(dmr, rcb, &error);
		        if (status != E_DB_OK)
			    break;
		    }
		}
	...
	*(rcb->rcb_uiptr) &= ~SCB_USER_INTR;
	...
	ule_format(error, (CL_SYS_ERR *)0, ULE_LOG, NULL, (char * )0,
	    (i4)0, (i4 *)0, &error, 0);
	error = E_DM008B_ERROR_PUTTING_RECORD;
    }
...
DB_STATUS	*error)
{
	...
	*/
	if ((rcb->rcb_lk_mode < RCB_K_X) ||
	    (tcb->tcb_rel.relstat &
                 (TCB_IDXD | TCB_JOURNAL | TCB_GATEWAY)))
	{
	    *error = E_DM0050_TABLE_NOT_LOADABLE;
	    return (E_DB_ERROR);
	}
	...
	*/
	if (table_empty && ((dmr->dmr_flags_mask & DMR_EMPTY) == 0))
	{
	    ...
	    status = dm2r_get(rcb, &tid, DM2R_GETNEXT, rec_buf, error);
	...
	if ((table_empty == 0) && (tcb->tcb_rel.relspec != TCB_HEAP))
	{
	    *error = E_DM0050_TABLE_NOT_LOADABLE;
	    return (E_DB_ERROR);
	}
