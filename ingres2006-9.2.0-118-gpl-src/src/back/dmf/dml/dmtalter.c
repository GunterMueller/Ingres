...
DMT_CB	    *dmt_cb)
{
	...
	*/
	    ...
	    ((ref_count > 1) && (sconcur == 0)))
	{
	    ...
	    if (status != E_DB_OK)
	    {
		ule_format(local_error, 0, ULE_LOG, NULL, (char * )0, 0L, 
		    (i4 *)0, &local_error, 0);
	    }
	    break;
	}
	...
	*/
	...
	{
	    dmt->error.err_data = jparm;
	    error = (gateway) ? E_DM0137_GATEWAY_ACCESS_ERROR
			      : E_DM000D_BAD_CHAR_ID;
	    break;
	}
	...
	*/
	...
	for (;;)
	{
	    ...
	    */
		...
		status = dm2t_open(odcb->odcb_dcb_ptr, &dmt->dmt_id, DM2T_X,
		    ...
		    &dmt->dmt_timestamp, &rcb, scb, &error);
		if (status)
		    break;
	...
	if (relupdate)
	{
	    ule_format(E_DM9026_REL_UPDATE_ERR, (CL_SYS_ERR *)0, ULE_LOG, NULL,
    ...
    if (rel_rcb)
    {
	status = dm2t_close(rel_rcb, DM2T_NOPURGE, &local_error);
	if (status != E_DB_OK)
	{
	    ule_format(local_error, 0, ULE_LOG, NULL, (char * )0, 0L, 
		(i4 *)0, &local_error, 0);
