...
dmc_logwriter(DMC_CB	    *dmc_cb)
{
    ...
    i4	    error_code = E_DM014D_LOGWRITER;
    ...
    MEcopy((PTR)DB_LOGFILEIO_NAME, sizeof(add_info.ad_dbname),
	...
	if (stat == LG_EXCEED_LIMIT)
	    ...
	    dmc_cb->error.err_code = E_DM014D_LOGWRITER;
	return (E_DB_ERROR);
    }
	...
	MEcopy((PTR)DB_LOGTHREAD_NAME, sizeof(DB_OWN_NAME),
		(PTR) &user_name);
