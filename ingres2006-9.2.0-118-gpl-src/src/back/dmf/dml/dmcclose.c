...
DMC_CB    *dmc_cb)
{
	...
	if (scb->scb_x_ref_count)
	{
        ...
        */
        ...
        {
            status = dmc_set_logging(scb, (DMP_DCB *)odcb->odcb_db_id,
                DMC_C_ON, &error);
            if (status != E_DB_OK)
            {
                ule_format(error, (CL_ERR_DESC *)0, ULE_LOG,
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0,
		    &error, 0);
                error = E_DM0087_ERROR_CLOSING_DB;
                break;
            }
        }
	...
	while (odcb->odcb_cq_next != (DML_XCCB*) &odcb->odcb_cq_next)
	{
		...
		** xccb. Set to zero. */
		...
		if (local_status)
		{
		    ...
		    ule_format( E_DM0087_ERROR_CLOSING_DB,
			    0, ULE_LOG , NULL, 
			    (char * )0, 0L, (i4 *)0, &local_error, 0);
		}
