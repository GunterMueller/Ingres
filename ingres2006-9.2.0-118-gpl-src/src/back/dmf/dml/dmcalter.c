...
DMC_CB    *dmc_cb)
{
	...
	for (status = E_DB_ERROR;;)
	{
		...
		*/
		status = set_lockmode(dmc, scb, &error);
		...
		return (E_DB_OK);
            ...
            if (dmc->dmc_flags_mask == DMC_SETTRANSACTION)
            {
                ...
                */
                status = set_transaction(dmc, scb, &error);
                if (DB_FAILURE_MACRO(status))
                {
                    dmc->error.err_code = error;
                    return (E_DB_ERROR);
		...
		*/
		status = set_worklocs(dmc, scb, &error);
		if (DB_FAILURE_MACRO(status))
		{
		    dmc->error.err_code = error;
		    return (E_DB_ERROR);
		...
		switch (chr[i].char_id)
		{
		  case DMC_C_LOGGING:
		    ...
		    */
		    ...
		    status = dmc_set_logging(scb, dcb, chr[i].char_value, 
			&error);
		    break;
...
i4	    *err_code)
{
	...
	error = E_DM0054_NONEXISTENT_TABLE;
	if ((dmc->dmc_flags_mask == DMC_SETLOCKMODE) &&
	...
	*/
	...
	for (i = 0; i < chr_count; i++)
	{
	    ...
	    */
	    ...
	    {
		error = E_DM0060_TRAN_IN_PROGRESS;
		break;
	    }
	...
	if (llevel == DMC_C_ROW)
	{
	    ...
	    /*
	    ** Check if we can do row locking with this database/server
	    ** Ignore SET LOCKMODE ... WHERE LEVEL=ROW for iidbdb
	    ...
	    */
		...
		{
		    /*
		    ** Clustering does not currently support row
		...
		{
		    error = E_DM010E_SET_LOCKMODE_ROW;
		    break;
		}
	    }
	}
	...
	case DMC_SL_TABLE:
		...
		status = dm0m_allocate((i4)sizeof(DML_SLCB), 
			    ...
			    (i4)SLCB_CB,
			    (i4)SLCB_ASCII_ID, 
			    (char *)scb, (DM_OBJECT **)&slcb, &error);
		if (status != E_DB_OK)
		{
		    ule_format( error, 0, ULE_LOG , NULL, 
			(char * )0, 0L, (i4 *)0, 
			&local_error, 0);
		    error = E_DM0107_ERROR_ALTERING_SESSION;
		    break;
		}
...
i4	    *err_code)
{
    ...
    mask_size = (i4)ME_ALIGN_MACRO(mask_size, sizeof(i4));
	...
	*err_code = E_DM0107_ERROR_ALTERING_SESSION;
	return(E_DB_ERROR);
    }
	...
	switch (dmc->dmc_s_type)
	{
	  ...
	  case DMC_S_ADD_LOCS:
		...
		*/
		...
		error = E_DM013B_LOC_NOT_AUTHORIZED;
		dmc->dmc_error_loc = &q->dmc_lname;
		for (i=sizeof(DB_LOC_NAME); i > 0; i--)
		{
		    if (((char *)dmc->dmc_error_loc)[i-1] != ' ')
...
i4	    *err_code)
{
	...
	for (;;)
	{
	    ...
	    */
	    dbstatus = dm2d_check_db_backup_status(dcb, err_code);
	    if (dbstatus)
		break;

	    if (dcb->dcb_status & DCB_S_BACKUP)
	    {
		*err_code = E_DM0147_NOLG_BACKUP_ERROR;
		dbstatus = E_DB_ERROR;
		break;
	    }
	    ...
	    */
	    ule_format(E_DM9060_SESSION_LOGGING_OFF, (CL_ERR_DESC *)0, 
	    ...
	    */
            dbstatus = dm0c_open(dcb, 0, scb->scb_lock_list, &cnf, err_code);
            if (dbstatus != E_DB_OK)
        	break;
	    ...
	    */
	    ...
	    dbstatus = dm0c_close(cnf, (DM0C_UPDATE | DM0C_COPY), err_code);
	    break;
	}
	...
	while (dcb->dcb_status & DCB_S_FASTCOMMIT)
	{
	    ...
	    */
		...
		ule_format(status, &sys_err, ULE_LOG, 
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
		    err_code, 0);
		ule_format(E_DM900C_BAD_LOG_BEGIN, &sys_err, ULE_LOG, 
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
		    err_code, 1, 0, dcb->dcb_log_id);
	    ...
	    */
		...
		ule_format(status, &sys_err, ULE_LOG, 
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
		    err_code, 0);
		ule_format(E_DM900F_BAD_LOG_EVENT, &sys_err, ULE_LOG,
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
		    err_code, 1, 0, LG_E_ECPDONE);
		break;
	    }
	    ...
	    */
		...
		ule_format(status, &sys_err, ULE_LOG, 
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
		    err_code, 0);
		ule_format(E_DM900E_BAD_LOG_END, &sys_err, ULE_LOG,
		    (DB_SQLSTATE *)NULL, (char *)0, (i4)0, (i4 *)0, 
		    err_code, 1, 0, log_id);
		break;
	    }
	...
	*/
	...
	if (logging_off)
	{
	    *err_code = E_DM9061_JNLDB_SESSION_LOGGING_ON;
	    if (! db_is_journaled)
	    {
	    	*err_code = E_DM9062_NON_JNLDB_SESSION_LOGGING_ON;
	    }
	...
	*/
	if (status != OK)
	{
		...
		if (status != OK)
		{
		    ...
		    ule_format(E_DM900E_BAD_LOG_END, &sys_err, ULE_LOG,
			(DB_SQLSTATE *)NULL, (char *)0, (i4)0,
			(i4 *)0, err_code, 1, 0, log_id);
		}
	    }
	    if (*err_code != E_DM0112_RESOURCE_QUOTA_EXCEED)
		*err_code = E_DM0107_ERROR_ALTERING_SESSION;
	    return(E_DB_ERROR);
	}
