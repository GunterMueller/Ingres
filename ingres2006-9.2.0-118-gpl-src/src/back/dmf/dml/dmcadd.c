...
*/

static READONLY DMM_LOC_LIST    loc_list[4] =
{
    {  DMM_L_JNL,
	{ 'i', 'i', '_', 'j', 'o', 'u', 'r', 'n',
	  'a', 'l', ' ', ' ', ' ', ' ', ' ', ' ',
...
static READONLY DB_LOC_NAME	    dbdb_location = 
{
...
DMC_CB    *dmc_cb)
{
	...
	*/
	...
	error = E_DM000F_BAD_DB_ACCESS_MODE;
	...
	if (mode == DMC_A_CREATE)
	{
	    ...
	    */ 
		...
		loc_ptr, collation.db_name, ucollation.db_name, &error);
		...
		if (error > E_DM_INTERNAL)
		{
		    ...
		    error = E_DM0084_ERROR_ADDING_DB;
		}
		break;
	    }
	}
	...
	MEcopy(dcb->dcb_dbcmptlvl, DU_MAXDBCMPTLVL, dmc->dmc_dbcmptlvl);
