...
#define	QEF_PART_TABLE	0x10	    /* a partitioned table */
    i4		vl_tab_id_index;    /* ? why is this needed */
...
struct _QEF_SCROLL
{
    DMT_ATTR_ENTRY  *ahd_rsattr; 	/* ptr to result set "column" desc */
    ...
    i4		    ahd_rsrdmrix; 	/* cbs index to result set 
					** read DMR_CB */
    i4		    ahd_rssplix; 	/* cbs index to result set spool
					** control block */
    ...
    i4		    ahd_rskscnt; 	/* count of keyset keys (or 0 if not
					** a keyset cursor) */
    DMT_ATTR_ENTRY  *ahd_rskattr; 	/* ptr to array of keyset key 
					** descriptors */
...
#define QEF_ERR_DUP	    1			/* error on unique key update */
	...
	*/
    ...
    i4		    ahd_estimates;	/* 1 == no estimates in this node    */
...
typedef struct _QEQ_D2_GET
{
    ...
    i4		    qeq_g1_firstn;	/* Max rows to return from Star query */
}   QEQ_D2_GET;
...
#define QEAHD_ASCII_ID  CV_C_CONST_MACRO('Q', 'E', 'H', 'D')
    i4              ahd_atype;          /* action */
#define QEA_DMU     1                   /* perform a DMF operation */
#define QEA_APP	    2			/* append operation */
#define QEA_SAGG    3                   /* perform a simple aggregate */
#define QEA_PROJ    4                   /* project an aggregate function */
#define QEA_AGGF    5			/* compute an aggregate function */
#define QEA_UPD     6			/* update operation */
#define QEA_DEL     7		        /* delete operation */
#define QEA_GET     8	                /* fetch operation */
#define QEA_UTL     9	                /* utility operation */
#define QEA_RUP	    10			/* update of tuple in associated QP 
					** This action is used by update
					** cursor commands where one
					** QP updates the current tuple
					** in another cursor QP
					*/
#define QEA_RAGGF   11			/* return the aggregate result row
					...
					** aggregates.
					*/
#define QEA_LOAD    12			/* load operation */
#define	QEA_PUPD    13			/* Positioned update */
#define QEA_PDEL    14			/* Positioned delete */
#define	QEA_COMMIT  15			/* Commit for DB procedure */
#define QEA_ROLLBACK 16			/* Rollback for DB procedure */
#define	QEA_IF	    17			/* If stmt for DB procedure */
#define	QEA_RETURN  18			/* return stmt for DB procedure */
#define	QEA_MESSAGE 19			/* message stmt for DB procedure */
#define	QEA_CALLPROC 20			/* Invoke a DB procedure */
#define	QEA_EMESSAGE 21			/* Generate an error message */
#define QEA_IPROC    22			/* perform QEF Internal Procedure */
#define	QEA_RDEL    23			/* Cursor delete action */
#define	QEA_EVREGISTER 	24		/* REGISTER EVENT - data is QEF_EVENT */
#define	QEA_EVDEREG 	25		/* REMOVE EVENT - data is QEF_EVENT */
#define	QEA_EVRAISE 	26		/* RAISE EVENT - data is QEF_EVENT */
#define	QEA_CREATE_RULE		27	/* takes QEF_DDL_STATEMENT */
#define	QEA_CREATE_PROCEDURE	28	/* takes QEF_DDL_STATEMENT */
#define	QEA_CREATE_INTEGRITY	29	/* takes QEF_CREATE_INTEGRITY_STATEMENT;*/
#define	QEA_CREATE_TABLE	30	/* takes QEF_DDL_STATEMENT */
#define	QEA_DROP_INTEGRITY	31	/* takes QEF_DROP_INTEGRITY */
#define	QEA_MODIFY_TABLE	32	/* takes QEF_DDL_STATEMENT */
#define	QEA_PROC_INSERT		33	/* Insert for deferred proc call  */
#define	QEA_CLOSETEMP		34	/* Close temptable */
#define	QEA_INVOKE_RULE		35	/* Invoke a rule action list */

#define QEA_CREATE_SCHEMA       36      /* takes QEF_CREATE_SCHEMA */
#define QEA_EXEC_IMM            37      /* takes QEF_EXEC_IMM */
#define	QEA_CREATE_VIEW		38	/* takes QEF_CREATE_VIEW */

#define QEA_FOR	    39			/* For stmt for DB procedure */
#define QEA_RETROW  40			/* Return row for DB procedure */
#define QEA_HAGGF   41			/* Same as QEA_RAGGF, except it 
					** assumes no input ordering and 
					** groups input rows by hashing */

#define QEA_D0_NIL  100			/* base for all STAR action types, NOT
					** a real action type */
#define	QEA_D1_QRY  101			/* (+Titan) a subquery action to go with
                                        ** QEQ_D1_QRY */
#define	QEA_D2_GET  102			/* (+Titan) a get action, it always
					** follows a retrieval subquery to
					** allow QEF to peform data fetching 
                                        ** to go with QEQ_D2_GET */
#define	QEA_D3_XFR  103			/* (+Titan) a data transfer action 
					** consisting of 3 sequential 
					** subqueries: SELECT/RETRIEVE,
					** CREATE TABLE and COPY FROM STAR 
                                        ** to go with QEQ_D3_XFR */
#define	QEA_D4_LNK  104			/* (+Titan) a CREATE LINK action to
					** finish off a CREATE TABLE statement 
					** uses QEQ_D4_LNK */
#define	QEA_D5_DEF  105			/* (+Titan) an action defining an LDB
					** repeat query, to go with QEQ_D1_QRY 
					** (NOT used) */
#define	QEA_D6_AGG  106			/* (+Titan) an action defining a 
...
#define	QEA_D7_OPN  107			/* (+Titan) an action defining an OPEN
					** CURSOR subquery, uses QEQ_D1_QRY */
#define	QEA_D8_CRE  108			/* (+Titan) an action defining a CREATE
					** AS SUBSELECT subquery, uses 
					** QEQ_D1_QRY */
#define	QEA_D9_UPD  109			/* (+Titan) an action defining a CURSOR
					** UPDATE subquery, uses QEQ_D1_QRY */
#define QEA_D10_REGPROC 110		/* perform Registered Procedure,
