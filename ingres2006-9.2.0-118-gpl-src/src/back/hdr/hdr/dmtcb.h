...
typedef struct
{
    DB_ATT_NAME     attr_name;              /* Column name. */
    i4         attr_type;              /* Column type. */
    i4         attr_size;              /* Column type size. */
    i4         attr_precision;         /* Column type precision. */
    i4         attr_flags;             /* Used to indicate a non-
                                            ** nullable, non-defaultable
                                            ** attribute. */          
#define                 DMT_F_NDEFAULT      0x01 /* NO DEFAULT */
#define                 DMT_F_KNOWN_NOT_NULLABLE  0x02 /* ANSI semantics */ 
#define                 DMT_F_DEFAULT       0x04 /* not used */
#define                 DMT_F_DESCENDING    0x08 /* keyed in descending order */
#define			DMT_F_RULE	    0x10 /* = ATT_RULE in <dmp.h> */
#define                 DMT_F_WORM          0x20 /* attribute can only be 
						 ** written once (on append)
						 */
#define                 DMT_F_SYS_MAINTAINED 0x40 /* attribute can only be 
						 ** changed by the dbms core 
						 ** system.
						 */
#define			DMT_F_HIDDEN	    0x80 /* Attribute should only be
						 ** retrieved by name, not in
						 ** a select * or tab.ALL.
						 */
#define			DMT_F_PERIPHERAL    0x100
						 /*
						 ** Attribute is a peripheral
						 ** datatype.
						 */
#define			DMT_F_SEC_KEY	   0x2000 /* This attribute is the 
						  ** security key for
						  ** the record.
						  */
#define			DMT_F_SEC_LBL	   0x4000 /* This attribute is the 
						  ...
						  ** record.
						  */
#define 		DMT_F_LEGAL_ATTR_BITS (0x01FB|\
					DMT_F_SEC_LBL|\
					DMT_F_SEC_KEY)
    ...
    PTR		    attr_default;	/* points at storage for the default,
					** null if default tuple not made yet */
    i4		    attr_collID;	/* collation ID (for char cols) */
}   DMT_ATTR_ENTRY;

/*}
