...
**      12-Feb-99 (fanra01)
**          Renamed scfint.h header.  Replace scs calls with ascs equivalents.
**	21-jan-1999 (hanch04)
...
**	    replace nat and longnat with i4
**	04-Jan-2001 (jenjo02)
**	    Pass this session's SCD_SCB* to all called functions.
**	30-nov-2005 (abbjo03)
...
SCF_CB             *cb;
{
    ...
    cb->scf_error.err_code = cb->scf_error.err_data = 0;

    if (!Sc_main_cb)
    {
	/* for standalone processing */
	status = ascf_init_dbg();
	if (status)
	    return (status);
    }

#ifdef xDEBUG
    if (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_EXDECL, 0, 0))
    {
	ex_declared = 1;
    ...
    if (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_MEMCHECK, 0, 0))
    {
	...
	{
	    cb->scf_error.err_code = E_SC0023_INTERNAL_MEMORY_ERROR;
	    sc0e_0_put(E_SC0023_INTERNAL_MEMORY_ERROR, 0);
	    sc0e_0_put(internal_status, 0);
	    if (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_EXDECL, 0, 0))
	    {
		if (ex_declared)
		    EXdelete();
    ...
    */
    ...
    {
	error = E_SC0022_BAD_REQUEST_BLOCK;
    }
    else if (cb->scf_facility < DB_MIN_FACILITY ||
	     cb->scf_facility > DB_MAX_FACILITY)
    {
	error = E_SC0018_BAD_FACILITY;
    }
    else if (operation < SCF_MIN_OPERATION || 
	     operation > SCF_MAX_OPERATION)
    {
	error = E_SC0001_OP_CODE;
    }
    else if ( (scb = (SCD_SCB*)cb->scf_session) == (SCD_SCB*)DB_NOSESSION )
    { 
	if ( scb = (SCD_SCB*)Sc_main_cb->sc_proxy_scb )
	{
	    /* Take the semaphore, check again */
	    CSp_semaphore( 0, &Sc_main_cb->sc_misc_semaphore ); /* shared */
	    scb = (SCD_SCB*)Sc_main_cb->sc_proxy_scb;
	...
	case SCU_SINIT:
		...
		char sem_name[CS_SEM_NAME_LEN];
                ...
                cb->scf_error.err_code = cb->scf_error.err_data =
                        status = E_SC_OK;
                if (!cb->scf_ptr_union.scf_semaphore)
                {
                    cb->scf_error.err_code = E_SC0002_BAD_SEMAPHORE;
                    status = E_DB_ERROR;
                    break;
                }
	...
	case SCU_SWAIT:
		...
		if (status == OK)
		{
		    cb->scf_error.err_code = E_SC_OK;
		    status = E_DB_OK;
		}
		else if (status == E_CS0017_SMPR_DEADLOCK)
		{
	...
	case SCU_SRELEASE:
		...
		if (status)
		{
		    cb->scf_error.err_code = E_SC0012_NOT_SET;
		    status = E_DB_ERROR;
		}
	    }
	...
	case SCU_DBADDUSER:
	    status = ascs_dbadduser(cb);
	    break;

	case SCU_DBDELUSER:
	    status = ascs_dbdeluser(cb);
	    break;

	case SCS_DECLARE:
	    status = ascs_declare(cb);
	    break;

	case SCS_CLEAR:
	    status = ascs_clear(cb);
	    break;

	case SCS_DISABLE:
	    status = ascs_disable(cb);
	    break;

	case SCS_ENABLE:
	    status = ascs_enable(cb);
	    break;
	...
	case SCC_RWRITE:
	case SCC_RREAD:
	    cb->scf_error.err_code = E_SC0025_NOT_YET_AVAILABLE;
	    status = E_DB_ERROR;
	    break;
	...
	case SCD_INITSNGL:
	    status = ascd_init_sngluser(cb);
	    break;

	case SCS_ATFACTOT:
	    status = ascs_atfactot(cb);
	    break;
...
scf_handler( EX_ARGS *ex_args )
{
	...
	ule_format(0, 0, ULE_MESSAGE, NULL,
	    a_vio_msg, a_vio_length, &junk, &error, 0);
	ule_format(E_SC0108_UNEXPECTED_EXCEPTION, 0, ULE_LOG, NULL,
	    0, 0, 0, &error, 1, 0, ex_args->exarg_num);
    }
    else
    {
	ule_format(E_SC0108_UNEXPECTED_EXCEPTION, &ex_args->exarg_num, ULE_LOG,
	    NULL, 0, 0, 0, &error, 5, 
