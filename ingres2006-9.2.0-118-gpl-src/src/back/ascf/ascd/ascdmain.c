...
ascd_alloc_scb( SCD_SCB  **scb_ptr, GCA_LS_PARMS  *input_crb, i4  thread_type )
{
    ...
    */
	...
	/* lint truncation warning if size of ptr > int, but code valid */
	offset_outbuffer= (i4)ME_ALIGN_MACRO(offset_outbuffer,
						  sizeof(ALIGN_RESTRICT));
	/* lint truncation warning if size of ptr > int, but code valid */
	offset_tuples   = offset_outbuffer + (i4)sizeof(SCC_GCMSG);
	/* lint truncation warning if size of ptr > int, but code valid */
	offset_tuples   = (i4)ME_ALIGN_MACRO(offset_tuples,
						  sizeof(ALIGN_RESTRICT));
    ...
    */
	...
	MEfill((u_i2)(sizeof(DMC_CB) + Sc_main_cb->sc_dmbytes), 0, 
			scb->scb_sscb.sscb_dmarea);
    }
    ...
    */
	...
	scb->scb_cscb.cscb_outbuffer =
		    (SCC_GCMSG *)(block + offset_outbuffer);
