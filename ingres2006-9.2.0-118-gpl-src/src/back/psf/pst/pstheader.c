	...
	PSS_Q_XLATE	*xlated_qry)
{
    ...
    */
    ...
    {
	header->pst_rptqry = TRUE;
    }
    else
    {
	header->pst_rptqry = FALSE;
    }
    ...
    */
    ...
    if (is_iidd == TRUE)
	header->pst_mask1	 = (i4) PST_CDB_IIDD_TABLES;
    ...
    */
	...
	{
	    /* 
	    ** remember whether the view definition involved singleton 
	    ** subselect(s) 
	    */
	    ...
	    /* 
	    ** remember whether the view definition involved SUBSEL node with 
	    ** OR for an ancestor
	    */
	    if (sess_cb->pss_flattening_flags & PSS_SUBSEL_IN_OR_TREE)
		header->pst_mask1 |= PST_SUBSEL_IN_OR_TREE;
	    ...
	    /* 
	    ** remember whether the view definition involved ALL 
	    */
	    if (sess_cb->pss_flattening_flags & PSS_ALL_IN_TREE)
		header->pst_mask1 |= PST_ALL_IN_TREE;

	    /* 
	    ** remember whether the view definition involved multiple correlated
	    ** attributes
	    */
	    if (sess_cb->pss_flattening_flags & PSS_MULT_CORR_ATTRS)
		header->pst_mask1 |= PST_MULT_CORR_ATTRS;
	    ...
	    /* 
	    ** remember whether the view definition involved correlated 
	    ** aggregates
	    */
	    if (sess_cb->pss_flattening_flags & PSS_CORR_AGGR)
		header->pst_mask1 |= PST_CORR_AGGR;
	...
	{
	    /*
	    ** PST_NO_FLATTEN and PST_CORR_AGGR will be set (if appropriate) in
	    ** pst_modhdr() (by that time we will have all the info about views
    ...
    */
	...
	MEfill(sizeof(PST_STATEMENT), (u_char) NULL, (PTR) snode);
