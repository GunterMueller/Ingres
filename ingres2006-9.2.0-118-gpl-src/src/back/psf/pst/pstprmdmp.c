...
/* Table of 3-letter abbreviations for pst_type codes.  Since this is
** just for debug (PS145), I'll use an Order Sensitive table.
*/
static char * const pst_typestr3[] = {
    "AND", "_OR", "UOP", "BOP", "AOP",
    "COP", "CST", "RSD", "VAR", "RUT",
    "QLE", "TRE", "BYH", "AGH", "SRT",
    "CVL", "NOT", "SUB", "RLV", "TAB",
...
#define NUM_TYPESTR3 (sizeof(pst_typestr3)/sizeof(char *))
	...
	PTR                control)
{
    ...
    default:
	/* 5 blank lines when we don't know what's in it */
	pst_fblank(buf, DB_MAXNAME + 2);
	uld_prnode(control,buf);
	uld_prnode(control,buf);
	uld_prnode(control,buf);
	...
	i4		    max_str_len)
{
    ...
    /* Convert the number to a string */
    CVla((i4) qnode->pst_sym.pst_dataval.db_prec, numbuf);
	...
	PST_QTREE          *header)
{
    ...
    TRdisplay("\tpst_rptqry:\t");
    if (psq_booldmp(header->pst_rptqry) != E_DB_OK)
	return;
    TRdisplay("\n");
...
SUBSEL_IN_OR_TREE,ALL_IN_TREE,MULT_CORR_ATTRS,RNGTREE,,,NEED_SYSCAT,XTABLE_UPDATE,UPD_CONST,INNER_OJ",
	header->pst_mask1);
	...
	char               *buf)
{
    ...
    if (qnode->pst_sym.pst_type >= 0 && qnode->pst_sym.pst_type < NUM_TYPESTR3)
	...
	PST_RESTAB  *restab)
{
    ...
    TRdisplay("\tpst_resjour:\t");
    if (psq_booldmp(restab->pst_resjour) != E_DB_OK)
	return (E_DB_ERROR);
    TRdisplay("\n");
