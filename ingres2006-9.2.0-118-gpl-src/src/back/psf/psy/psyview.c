	...
	i4		*resp_mask)
{
    ...
    */
    if (qmode == PSQ_DEFCURS || qmode == PSQ_VIEW || qmode == PSQ_RULE)
	emode = PSQ_RETRIEVE;
    else
	emode = qmode;
	...
	for (vn = 0; ; vn++)
	{
		...
		*/
			   ...
			   (PTR) &sess_cb->pss_user, sizeof(DB_OWN_NAME)))
		{
		    ...
		    ** using this cursor.
		    */
		    if (qmode == PSQ_DEFCURS && rngvar == sess_cb->pss_resrng)
		    {
	...
	DB_ERROR	*err_blk)
{
	...
	** endif
	*/
	if (is_owner && qmode == PSQ_DEFCURS && rngvar == sess_cb->pss_resrng)
	{
