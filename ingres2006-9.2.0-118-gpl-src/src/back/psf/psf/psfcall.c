	...
	PTR		   sess_cb)
{
    ...
    */
    if ((PSS_SESBLK*) sess_cb == (PSS_SESBLK *) NULL && 
	opcode != PSQ_STARTUP &&
	opcode != PSQ_SRVDUMP && 
	opcode != PSQ_SHUTDOWN &&
	opcode != PSQ_PRMDUMP)
    ...
    {
	cb = (PSS_SESBLK *) sess_cb;
	cb->pss_retry = PSS_REPORT_MSGS;
    }
    ...
    case PSQ_ALTER:
	ret_val = psq_alter(psq_cb, cb);
	break;

    case PSQ_BGN_SESSION:
	ret_val = psq_bgn_session(psq_cb, cb);
	break;

    case PSQ_CLSCURS:
	ret_val = psq_clscurs(psq_cb, cb);
	break;

    case PSQ_DELCURS:
	ret_val = psq_dlcsrrow(psq_cb, cb);
	break;

    case PSQ_COMMIT:
	ret_val = pst_commit_dsql(psq_cb, cb);
	break;

    case PSQ_CURDUMP:
	ret_val = psq_crdump(psq_cb, cb);
	break;

    case PSQ_END_SESSION:
	ret_val = psq_end_session(psq_cb, cb);
	break;

    case PSQ_PARSEQRY:
	/* start out with all bits turned off */
	cb->pss_stmt_flags = cb->pss_dbp_flags = 0L;
	cb->pss_flattening_flags = 0;

	/* Initialize distributed statement flags */
	cb->pss_distr_sflags = PSS_PRINT_WITH;
	...
	*/ 
	...
	ret_val = psq_parseqry(psq_cb, cb);
	...
	*/
	if (   DB_FAILURE_MACRO(ret_val)
	    ...
	    && cb->pss_dependencies_stream
	    && cb->pss_dependencies_stream->psf_mstream.qso_handle !=
	           (PTR) NULL)
	{
	    DB_STATUS	    stat;

	    stat = psf_mclose(cb, cb->pss_dependencies_stream, &psq_cb->psq_error);

	    /*
	    ** ensure that no one tries to use the address that is no longer
	    ** valid
	    */
	    cb->pss_dependencies_stream = (PSF_MSTREAM *) NULL;

	    if (DB_FAILURE_MACRO(stat) && stat > ret_val)
		ret_val = stat;
	}

	if (!psf_retry(cb, psq_cb, ret_val))
	{
	    break;
	}
	...
	** error.
	*/
	cb->pss_retry = (PSS_REFRESH_CACHE | PSS_REPORT_MSGS);
	cb->pss_dbp_flags = PSS_RECREATE;

	ret_val = psq_recreate(psq_cb, cb);
	break;

    case PSQ_RESET_EFF_USER_ID:
	STRUCT_ASSIGN_MACRO(psq_cb->psq_user.db_tab_own, cb->pss_user);
	ret_val = E_DB_OK;
	break;

    case PSQ_OPEN_REP_CURS:
	ret_val = psq_open_rep_cursor(psq_cb, cb);
	break;
	...
	*/
	...
	psq_cb->psq_owner = (char *)&cb->pss_sess_owner.db_own_name;
	ret_val = E_DB_OK;
	break;
    }
	...
	psq_cb->psq_ret_flag = pst_get_stmt_mode(cb, psq_cb->psq_qid, 
			&stmt_info);
	...
	ret_val = E_DB_SEVERE;
	if (psq_cb != (PSQ_CB *) NULL)
	    psq_cb->psq_error.err_code = E_PS0006_UNKNOWN_OPERATION;
	break;
    }
	...
	PSS_SESBLK	*sess_cb)
{
    ...
    psy_cb->psy_error.err_code = E_PS0000_OK;
    ...
    */
	...
	default:
            psy_entry = NULL;
	    ret_val = E_DB_SEVERE;
	    psy_cb->psy_error.err_code = E_PS0006_UNKNOWN_OPERATION;
	...
	switch (qeu_cb.error.err_code)
	{
	    ...
	    {
		db_error.err_code = E_PS0001_USER_ERROR;
		break;
	    }
	    default:
	    {
