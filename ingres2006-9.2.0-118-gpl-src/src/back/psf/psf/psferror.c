...
**      the parser facility.
**
**          psf_error - Format and report a parser facility error.
...

/*{
** Name: psf_error	- Format and report a parser facility error.
**
** Description:
...
/* VARARGS5 */
DB_STATUS
psf_error(errorno, detail, err_type, err_code, err_blk, num_parms, param1,
...
*/
{
	...
	else if (err_type == PSF_INTERR)
	{
	    if (detail)
	    {
		err_blk->err_code = E_PS0007_INT_OTHER_FAC_ERR;
	    }
	    else
	    {
	...
	*/
	if (err_type != PSF_CALLERR)
	{
	    ...
	    {
		uleret = ule_format(detail, 0L, uletype, (DB_SQLSTATE *) NULL,
		...
		*/
		if (uleret != E_DB_OK)
		{
		    ...
		    ule_format(0L, 0L, ULE_MESSAGE, (DB_SQLSTATE *) NULL,
			errbuf, msglen, 0, &ulecode, 0);
		    *err_code = ulecode;
		}
	    }
	    uleret = ule_doformat(errorno, 0L, uletype, &sqlstate, errbuf,
	    ...
	    */
		...
		msglen = STlength(errbuf);
		ule_format(0L, 0L, ULE_MESSAGE, (DB_SQLSTATE *) NULL, errbuf,
		    msglen, 0, &ulecode, 0);
		*err_code = ulecode;
	...
	DB_ERROR    *err_blk)
{
	...
	case E_RD000C_USER_INTR:
	    err_blk->err_code = E_PS0003_INTERRUPTED;
	    break;

	case E_RD000D_USER_ABORT:
	    err_blk->err_code = E_PS0005_USER_MUST_ABORT;
	    break;
	...
	case E_RD002A_DEADLOCK:
	    err_blk->err_code = E_PS0009_DEADLOCK;
	    break;
	...
	DB_ERROR    *err_blk)
{
	...
	{
	    err_blk->err_code = E_PS0008_RETRY;
	    break;
	}
