		   ...
		   DB_ERROR	*err_blk)
{
	...
	*/
	if (att->att_flags & DMT_F_SYS_MAINTAINED)
	    continue;
	...
	*/
	...
	{
	    mandatory = (att->att_flags & DMT_F_NDEFAULT) ? TRUE : FALSE;
	}
	else
	{
	    ...
	    */
	    if ((att->att_flags & DMT_F_NDEFAULT)
		|| ((att->att_type > 0) 
	...
	*/
	...
	/* attach resdom node to query tree;
	** just add to beginning, since order isn't important in an INSERT.
	*/
	resdom_node->pst_left = root_node->pst_left;
	root_node->pst_left   = resdom_node;
