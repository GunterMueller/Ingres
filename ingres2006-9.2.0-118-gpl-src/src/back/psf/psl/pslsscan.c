...
/* 1343 */	188, 'a', 's', 'y', 'm', 'm', 'e', 't', 'r', 'i', 'c', 0,
...
static const u_char     *Key_index[] = {
                                  ...
                                  &Key_string[818], /* Keywords of length 7 */
                                  &Key_string[1026],/* Keywords of length 8 */
                                  &Key_string[1177],/* Keywords of length 9 */
                                  &Key_string[1343],/* Keywords of length 10 */
                                  &Key_string[1440],/* Keywords of length 11 */
                                  &Key_string[1480],/* Keywords of length 12 */
                                  &Key_string[1565],/* Keywords of length 13 */
                                  &Key_string[0],   /* Keywords of length 14 */
                                  &Key_string[1581],/* Keywords of length 15 */
                                  &Key_string[0],   /* Keywords of length 16 */
                                  &Key_string[1598],/* Keywords of length 17 */
...
# endif /* BYTE_ALIGN */
	...
	*/
	...
	/* check for a bit constant (b'<bit>{<bit>...}') */
	case 'B': case 'b':
	    nonwhite++;
	    if (*(next_char + 1) != '\'')
	    {
		/* Not a bit constant, drop through
		** for name/symbol validation */
		...
		for (;;)
		{
			...
			case '\'':
			    ...
			    /* else bad bit constant */
			default:
			    {
				pss_cb->pss_lineno += lineno;
				psf_error(2726L, 0L, PSF_USERERR,
				    &err_code, &psq_cb->psq_error, 2,
				    (i4) sizeof(pss_cb->pss_lineno),
				    &(pss_cb->pss_lineno), (i4) 1, &ch);
				return (-1);
...
# endif /* BYTE_ALIGN */
		...
		{
		    /* It might fit in an i8.  Clearly we can't tell by
		    ** looking at int_number because it's an i8.  It
		    ** turns out that the strtoll function underlying
		    ** CVal8 is quite happy with giant numbers, and it
		    ** simply returns MAXI8 (or MINI8, although negatives
		    ** don't actually happen here).  So if we get MAXI8,
		    ** simply assume that it's didn't fit.
		    */
		...
		/* else fall thru, try decimal.  i8 tops out at roughly 9e18,
		** decimal handles up to 31 digits.
		*/
	    }
