...
** DMU_TEMP_TABLE		          x	  x
*/
#define SQL_MAX_CREATE_CHARS	    8
#define	SQL_GW_MAX_REGISTER_CHARS   8
	...
	PSS_CONS	*cons_list)
{
    ...
    */
	...
	if (logical_key_specified)
	{
	    ...
	    /* sess_cb->pss_restab.pst_resjour = TRUE; */
	}
    }
	...
	PSQ_CB		*psq_cb)
{
	...
	*/
	if ((ssnode->pst_right->pst_sym.pst_type == PST_CONST) &&
	    (abs(ssnode->pst_right->pst_sym.pst_dataval.db_datatype)
						    == DB_LTXT_TYPE))
	{
	...
	i4		first_n)
{
    ...
    */
	...
	(PST_QNODE *) NULL, query_expr, &tree, &pnode, PST_0FULL_HEADER, 
	xlated_qry);
    if (status != E_DB_OK)
    {
	return (status);
	...
	*/
	...
	else
	{
		...
		*/
		...
		status = psf_malloc(sess_cb, &sess_cb->pss_ostream, sizeof(DMU_ATTR_ENTRY),
	...
	PSS_Q_XLATE	*xlated_qry)
{
	...
	*/
    ...
    if (STcompare(keyword, "duplicates") == 0)
    {
	if (PSS_WC_TST_MACRO(PSS_WC_DUPLICATES, with_clauses))
	{
	    duplicate = TRUE;
	    dupchar = "DUPLICATES";
	}
	...
	{
	    PSS_WC_SET_MACRO(PSS_WC_DUPLICATES, with_clauses);

	    /* Add DUPLICATES entry to DMU_CHAR_ARRAY */
	    chr->char_id = DMU_DUPLICATES;
	    chr->char_value = DMU_C_ON;		    /* allow duplicates */
	    dmu_cb->dmu_char_array.data_in_size += sizeof(DMU_CHAR_ENTRY);
	    sess_cb->pss_restab.pst_resdup = TRUE;
	}
    }
    else if (STcompare(keyword, "noduplicates") == 0)
    {
	if (PSS_WC_TST_MACRO(PSS_WC_DUPLICATES, with_clauses))
	{
	    duplicate = TRUE;
	...
	else
	{
	    ...
	    dmu_cb->dmu_char_array.data_in_size += sizeof(DMU_CHAR_ENTRY);
	    sess_cb->pss_restab.pst_resdup = FALSE;
	}
    }
    else if (STcompare(keyword, "nosecurity_audit_key") == 0)
    {
	/*
	...
	*/
	...
	else
	{
	    ...
	    {
		PSS_WC_SET_MACRO(PSS_WC_ROW_SEC_KEY, with_clauses); /* For dup detection */
		PSS_WC_SET_MACRO(PSS_WC_ROW_NO_SEC_KEY, with_clauses); /* No audit key */
	    }
	}
    }
    else if (!STcompare(keyword, "journaling") ||
	     (sess_cb->pss_lang == DB_QUEL && !STcompare(keyword, "logging")))
    {
	if (PSS_WC_TST_MACRO(PSS_WC_JOURNALING, with_clauses))
	{
	    duplicate = TRUE;
	...
	else
	{
	    ...
	    */
		...
		(VOID) psf_error(E_PS0BD2_NOT_SUPP_ON_TEMP, 0L, PSF_USERERR,
				&err_code, err_blk, 1,
				sizeof("WITH JOURNALING")-1, "WITH JOURNALING");
		return (E_DB_ERROR);
	    }

	    PSS_WC_SET_MACRO(PSS_WC_JOURNALING, with_clauses);

	    /* Add JOURNALED entry to DMU_CHAR_ARRAY */
    ...
    else if (!STcompare(keyword, "nojournaling") ||
	     (sess_cb->pss_lang == DB_QUEL && !STcompare(keyword, "nologging")))
    {
	if (PSS_WC_TST_MACRO(PSS_WC_JOURNALING, with_clauses))
	{
	    duplicate = TRUE;
	...
	else
	{
	    ...
	    sess_cb->pss_restab.pst_resjour = FALSE;
	}
    }
    else if (STcompare(keyword, "nopartition") == 0)
    {
	if (PSS_WC_TST_MACRO(PSS_WC_PARTITION, with_clauses))
	{
	    duplicate = TRUE;
	...
	else
	{
	    ...
	    /* Nothing else to do, default is non partitioned */
	}
    }

    /*
    ** SQL only options
    **	    note that [no]compression is allowed only for CREATE TABLE AS SELECT
    **
    ** NOTE: do NOT add any options supported in both QUEL and SQL below this
    **	     block;
    **       add SQL-only options ONLY inside this block;
    **	     add QUEL-only options ONLY into the next block
    */
    else if (sess_cb->pss_lang == DB_SQL)
    {

	if (!STcompare(keyword, "compression"))
	{
	    if (qmode == PSQ_CREATE || qmode == PSQ_DGTT)
	    {
		bad_option = TRUE;
		...
		else
		{
		    ...
		    if (sess_cb->pss_restab.pst_struct == 0)
		    {
	...
	else if (!STcompare(keyword, "nocompression"))
	{
	    if (qmode == PSQ_CREATE || qmode == PSQ_DGTT)
	    {
		bad_option = TRUE;
	...
	else if (STcompare(keyword, "norecovery") == 0)
	{
	    if (PSS_WC_TST_MACRO(PSS_WC_RECOVERY, with_clauses))
	    {
		duplicate = TRUE;
		dupchar = "RECOVERY";
	    }
	    ...
	    {
		/* must be creating a DGTT - otherwise report error */
		...
		if (!sess_cb->pss_restab.pst_temporary)
		{
			...
			sizeof("WITH NORECOVERY")-1, "WITH NORECOVERY");
		    return (E_DB_ERROR);
		}

		PSS_WC_SET_MACRO(PSS_WC_RECOVERY, with_clauses);

		/* Add RECOVERY entry to DMU_CHAR_ARRAY */
		chr->char_id = DMU_RECOVERY;
		chr->char_value = DMU_C_OFF;	/* disallow recovery */
		dmu_cb->dmu_char_array.data_in_size += sizeof(DMU_CHAR_ENTRY);
		sess_cb->pss_restab.pst_recovery = FALSE;
	    }
	}
	else if (!STcompare(keyword, "autostruct"))
	{
	    if (PSS_WC_TST_MACRO(PSS_WC_AUTOSTRUCT, with_clauses))
	    {
		duplicate = TRUE;
		dupchar = "AUTOSTRUCT";
	    }
	    ...
	    {
		PSS_WC_SET_MACRO(PSS_WC_AUTOSTRUCT, with_clauses);

		sess_cb->pss_restab.pst_autostruct = PST_AUTOSTRUCT;
	    }
	}
	else if (!STcompare(keyword, "noautostruct"))
	{
	    if (PSS_WC_TST_MACRO(PSS_WC_AUTOSTRUCT, with_clauses))
	    {
		duplicate = TRUE;
	...
	{
	    /* unknown SQL option */
	    (VOID) psf_error(2007L, 0L, PSF_USERERR, &err_code,
		err_blk, 1, (i4) STtrmwhite(keyword), keyword);
	    return (E_DB_ERROR);    
	}
	...
	/*
	** NOTE:
	**      if we ever add QUEL-only options, they should go into this block
	*/

	/* unknown QUEL option */
	(VOID) psf_error(2046L, 0L, PSF_USERERR, &err_code,
	    err_blk, 1, (i4) STtrmwhite(keyword), keyword);
	return (E_DB_ERROR);    
    }
...
**	list_clause	    Which type of list clause this is.
**	part_locs	    Points to a DM_DATA array to use for location
**			    keyword store if doing partition definition
**	xlated_qry	    For query text buffering 
**
** Outputs:
	...
	i4		temporary)
{
    ...
    */
    ...
    /* Default for journaling is FALSE */
    sess_cb->pss_restab.pst_resjour = FALSE;
	...
	*/
    ...
    {
	/*
	** Allocate the description entries for RMSGW's REGISTER TABLE 
	*/
	...
	*/
	...
	sess_cb->pss_restab.pst_resjour = -1;
	...
	PSS_OBJ_NAME	*tbl_spec)
{
    ...
    */
	...
	if (sess_cb->pss_restab.pst_temporary == 0)
	{
	    (VOID) psf_error(E_PS0BD3_ONLY_ON_TEMP, 0L, PSF_USERERR,
	...
	*/
	if (sess_cb->pss_restab.pst_temporary != 0)
	{
	    ...
	    sess_cb->pss_restab.pst_temporary = 1;
	    if (sess_cb->pss_lang == DB_SQL)
	...
	i4		collationID)
{
    ...
    ** collation is supported for defined collation. */
	...
	if (invalid_col)
	{
	    ...
	    tcode = (collationID <= 2) ? 0 : 1;
	    (VOID) psf_error(6426L, 0L, PSF_USERERR,
		&err_code, &psq_cb->psq_error, 2,
		STlength(collname_array[collationID-1]),
		collname_array[collationID-1],
    ...
    */
	...
	bool		defval_mismatch = FALSE;
	i4		old_flags, new_flags;
	bool		ok_null_switch = FALSE;
	...
	*/
	if ( ((cur_attr->attr_type < 0) && (alt_attr->att_type > 0)) &&
	      !(null_def & PSS_TYPE_USER_DEFAULT) )
	{
	    /* If switching from NOT NULL to WITH NULL, automatically
	    ** add the "with default" default value (kibro01) b118799 
	    */
	    ok_null_switch = TRUE;
	...
	** does orig column have a user def value
	*/
	if ((null_def & PSS_TYPE_USER_DEFAULT) &&
	    (alt_attr->att_defaultID.db_tab_base))
	{
	    RDF_CB		rdf_cb;
	    RDR_INFO	rdrinfo;
	    ...
	    */
	    MEfill(sizeof(RDR_INFO), (u_char)NULL, (PTR)&rdrinfo);
		...
		defval_mismatch = TRUE;
	    else if (STcompare(cur_attr->attr_defaultTuple->dbd_def_value,
		((RDD_DEFAULT *)(alt_attr->att_default))->rdf_default) 
		    != 0)
		defval_mismatch = TRUE;
	    else
...
#define DEF_FLAGS	(DMU_F_NDEFAULT | DMU_F_DEFAULT)
	old_flags = alt_attr->att_flags & DEF_FLAGS;
	new_flags = cur_attr->attr_flags_mask & DEF_FLAGS;

	/* While changing WITH NULL to NOT NULL is invalid, it is possible
	** to change NOT NULL to WITH NULL (kibro01) b118799
	*/
	if (!ok_null_switch &&
	    ( ((cur_attr->attr_type > 0) && (alt_attr->att_type < 0)) ||
	      (old_flags != new_flags) ||
	      (defval_mismatch) ) )
        {
	    /* attempting to alter nullability/default */
