	...
	DB_ERROR	*err_blk)
{
	...
	*/
	...
	/*
	** if the user was stupid enough to specify [C]HEAP as the default
	** index structure, let him have it...
	*/
	if (sstruct == DB_HEAP_STORE || sstruct == DB_SORT_STORE)
	{
	    (VOID) psf_error(5540L, 0L, PSF_USERERR, &err_code, err_blk, 1,
	...
	*/
        ...
        for (i = 0; i < num_atts; i++)
	{
		...
		if(att_entry->att_flags & DMT_F_SEC_KEY)
		{
	...
	DB_ERROR	*err_blk)
{
	...
	*/
	...
	{
	    case PSQ_INDEX:
	    case PSQ_CONS:	/* say "index" if constraint-with error */
	    {
		if (sess_cb->pss_lang == DB_SQL)
		{
		    qry	    = "CREATE INDEX";
