...
yyvars.in_target_clause = FALSE;
yyvars.in_groupby_clause = FALSE;
yyvars.in_update_set_clause = FALSE;
yyvars.in_orderby       = FALSE;
yyvars.in_rule		= 0;
yyvars.index_struct_specified = 0;
yyvars.indx_struct	= cb->pss_idxstruct;
yyvars.sort_list	= 0;
yyvars.sort_by		= 0;
yyvars.agg_func		= 0;
...
QUinit((QUEUE *)&yyvars.dcol_list);

/*
** when an operator is encountered while not processing a qualification, we will
** be checking the 0-th bit of qual_type
*/
yyvars.qual_depth       = 0;
BTclear(0, (char *) yyvars.qual_type);
yyvars.j_qual = &yyvars.first_qual;
...
yyvars.func_args	= &yyvars.initial_args[0];
