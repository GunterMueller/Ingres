...
ULT_VECTOR_MACRO(PSS_TBITS, PSS_TVAO) pss_trace; 
    ...
    i4		    pss_defqry;		/* 0		    - regular query
					...
					*/
    i4		    pss_rsdmno;		/* result domain number */
...
*/
typedef struct PSS_TBL_REF_
{
    struct PSS_TBL_REF_		*pss_next;
    DB_OWN_NAME			*pss_schema_name;   /*
...
#define	    MAX_NESTING	    100
    ...
    i4	    index_struct_specified; /* if index command,
				    ** has structure been specified.
				    */
    i4	    indx_struct;	    /* structure of index */
    ...
    ** for the current function begin at index arg_base and the next
    ** arg to be parsed goes into index arg_ix.  At close-paren time
    ** there are arg_ix - arg_base arguments.
...
#define PSS_INITIAL_ARGS (ADI_MAX_OPERANDS + 10)
    PST_QNODE	    *initial_args[PSS_INITIAL_ARGS];  /* usually suffices */
    PST_QNODE	    **func_args;	/* sel-expr-or-null's */
...
#define PSS_SET_FUNC_IN_OUTERMOST_SUBSEL        0x40
    ...
    i4	    agg_func;		/* TRUE if aggregate with BY list or GROUP BY
				...
				*/
    i4	    aggr_allowed;	
#define PSS_AGG_ALLOWED 1	/* If bit 0 is on, aggregates must not be
				** specified in the current clause.
				*/
...
blow chunks now!
    ...
    PST_J_MASK	    join_tbls;		/*
					...
					*/
    i4		    qual_depth;		/*
					** will be used to offset into qual_type
					*/
    char 	    qual_type[MAX_NESTING / BITS_IN(char) + 1];
					/*
...
#define PSS_CRTAB_AS_SELECT	    0x00000800L
    ...
    i4		    first_n;    /* count in "select first n ..." syntax */
...
*/
    ...
    */
    ...
    i4		    save_pss_rsdmno;	/* across derived table processing */
...
FUNC_EXTERN DB_STATUS	psf_error();
/*
