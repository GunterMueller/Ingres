...
static DB_STATUS    ulhi_alloc( ULH_RCB *ulh_rcb, ULM_RCB *ulm_rcb,
				i4 errcode, i4  flag );
static DB_STATUS    ulhi_chk_status( ULH_RCB *ulh_rcb, STATUS cs_status,
				     i4 error_code );
/*
	  ...
	  i4 max_alias )
{
    ...
    */
    ...
    if (max_obj == 0)
    {
	ulh_rcb->ulh_error.err_code = E_UL0102_TABLE_SIZE;
	return(E_DB_ERROR);
    }
    ...
    */
    ...
    {
	ulh_rcb->ulh_error.err_code = E_UL0118_NUM_ALIASES;
	return(E_DB_ERROR);
    }
    ...
    */
    ...
    {
	ulh_rcb->ulh_error.err_code = E_UL010D_TABLE_MEM;
	ulh_rcb->ulh_error.err_data = ulm_rcb.ulm_error.err_code;
    ...
    */
    ...
    ulh_rcb->ulh_error.err_code = E_UL0000_OK;
    ulh_rcb->ulh_error.err_data = E_UL0000_OK;
    return(E_DB_OK);
}

/*{
...
ulh_close( ULH_RCB *ulh_rcb )
{
    ...
    {
	ulh_rcb->ulh_error.err_code = E_UL0101_TABLE_ID;
	return(E_DB_ERROR);
    }
	    ...
	    i4  obj_type, i4 block_size )
{
    ...
    {
	ulh_rcb->ulh_error.err_code = E_UL0123_NAME_TOO_LONG;
	return(E_DB_ERROR);
    }
    ...
    */
    ...
    {
	status = ulhi_chk_status(ulh_rcb, cs_status, E_UL0113_TABLE_SEM);
	return(status);			 
    }
    /*
...
ulh_destroy( ULH_RCB *ulh_rcb, unsigned char *obj_name, i4  name_len )
{
	...
	*/
	...
	else
	{
	    ...
	    ulh_rcb->ulh_error.err_code = E_UL0109_NFND;
	    return(E_DB_ERROR);
	}
	...
	*/
	...
	{
	    CSv_semaphore(&tcb->ulh_sem);    /* release table sem */
	    ulh_rcb->ulh_error.err_code = E_UL0106_OBJECT_ID;
	    ulh_rcb->ulh_error.err_data = E_UL0000_OK;
	    return(E_DB_ERROR);
	}
...
ulh_rename( ULH_RCB *ulh_rcb, unsigned char *obj_name, i4  name_len )
{
    ...
    */
    ...
    {						/* duplicated name, */
        CSv_semaphore(&tcb->ulh_sem);    /* release table sem */
	ulh_rcb->ulh_error.err_code = E_UL0108_DUPLICATE;
	return(E_DB_ERROR);
    }
    /*
...
ulh_define_alias( ULH_RCB *ulh_rcb, unsigned char *alias, i4  alias_len )
{
    ...
    */
    ...
    {
	ulh_rcb->ulh_error.err_code = E_UL0121_NO_ALIAS;
	return(E_DB_ERROR);
    }
    ...
    */
	...
	{
	    ulh_rcb->ulh_error.err_code = E_UL0120_DUP_ALIAS;
	    return(E_DB_ERROR);
	}
    }
	       ...
	       i4 block_size )
{
	...
	*/
	...
	{				    /* no space for a new class */
	    ...
	    ulhi_destroy_obj(tcb, objp, &ulm_rcb); /* destroy the object */
	    CSv_semaphore(&tcb->ulh_sem);    /* release table sem */
	    ulh_rcb->ulh_error.err_code = E_UL0105_FULL_CLASS;
	    return(E_DB_ERROR);
	}
...
ulh_info( ULH_RCB *ulh_rcb, i4  ulh_icode, PTR bufptr )
{
    ...
    */
	...
	default:
	    ulh_rcb->ulh_error.err_code = E_UL010C_UNKNOWN;
	    return(E_DB_ERROR);
    }
		 ...
		 i4  name_len, i4  obj_type, i4 block_size )
{
    ...
    */
    ...
    {
	ulh_rcb->ulh_error.err_code = E_UL0104_FULL_TABLE;
	return(E_DB_ERROR);
    }
    ...
    */
	...
	{
	    ulh_rcb->ulh_error.err_code = E_UL0111_OBJ_MEM;
	    ulh_rcb->ulh_error.err_data = ulm_rcb.ulm_error.err_code;
	    return(status);
	}
	if ((tcb->lru_cnt[1]) && (tcb->lru_cnt[0] <= tcb->lru1_limit))
...
ulhi_mem_for_alias( ULH_RCB *ulh_rcb, ULM_RCB *ulm_rcb, ULH_HOBJ *alias_obj )
{
	...
	*/
    ...
    if (objp == (ULH_HOBJ *) NULL)
    {
	ulh_rcb->ulh_error.err_code = E_UL0119_ALIAS_MEM;
	return(E_DB_ERROR);
    }
...
ulhi_alloc( ULH_RCB *ulh_rcb, ULM_RCB *ulm_rcb, i4 errcode, i4  flag )
{
    ...
    {
	ulh_rcb->ulh_error.err_code = errcode;
	ulh_rcb->ulh_error.err_data = ulm_rcb->ulm_error.err_data;
...

/*{
** Name: ulhi_chk_status	- Check status from CSx_semaphore
**
** Description:
**
** Inputs:
**      ulh_rcb                         pointer to ULH_RCB.
**	cs_status			status from CSx_semaphore
**      error_code			error code specific to ULH operation
**
** Outputs:
**      none.
**	Returns:
...
** History:
**      30-nov-87 (puree)
**          created for SCF_SEMAPHORE conversion.
*/
static DB_STATUS
ulhi_chk_status( ULH_RCB *ulh_rcb, STATUS cs_status, i4 error_code )
{
    DB_STATUS	status;

    ulh_rcb->ulh_error.err_code = error_code;
    ulh_rcb->ulh_error.err_data = cs_status;
