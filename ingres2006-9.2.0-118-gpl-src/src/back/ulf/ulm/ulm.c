...
static DB_STATUS ulm_pinit( SIZE_TYPE  poolsize_pag, SIZE_TYPE  pref_chunksize_pag,
			    SIZE_TYPE  chunksize_pag, i4  owner, i4 pblksize,
			    PTR chunk, PTR *pool, i4 *err_code );
static DB_STATUS ulm_nalloc( PTR pool, i4 size, PTR *block,
			     i4 *err_code, i4 *err_data );
static DB_STATUS ulm_free( PTR block, PTR pool, i4 *err_code );
...
#define ULM_PAD_OVERHEAD (sizeof(i4) + sizeof(SYSTIME))
...
static i4	 ulm_conchk( ULM_PHEAD *pool, i4 *failed_check );
...
ulm_startup( register ULM_RCB *ulm_rcb )
{
    ...
    */
    ...
    {
	ulm_rcb->ulm_error.err_code = E_UL0014_BAD_ULM_PARM;
	return (E_DB_ERROR);
    }
    ...
    */
	...
	ulm_rcb->ulm_error.err_code = E_UL0005_NOMEM;
	ulm_rcb->ulm_error.err_data = 1;
	return (E_DB_ERROR);
    }
	...
	*/
	...
	ulm_rcb->ulm_error.err_code = E_UL0005_NOMEM;
	ulm_rcb->ulm_error.err_data = 2;
	return (E_DB_ERROR);
    }

    /* We have memory pool allocated from SCF, now initialize it. */
    status = ulm_pinit(tot_psize_pag, pch_psize_pag, act_psize_pag, ulm_rcb->ulm_facility,
					bsize, chunk_ptr, &pool_ptr, &err_code);
    if (DB_FAILURE_MACRO(status))
    {
	ulm_rcb->ulm_error.err_code = err_code;
...
ulm_shutdown( register ULM_RCB *ulm_rcb )
{
    ...
    {
	ulm_rcb->ulm_error.err_code = E_UL000B_NOT_ALLOCATED;
	return (E_DB_ERROR);
    }
...
ulm_openstream( register ULM_RCB *ulm_rcb )
{
    ...
    *streamid_p = (PTR)NULL;
    ...
    if (ulm_rcb->ulm_error.err_code = 
		ulm_conchk(pool, &ulm_rcb->ulm_error.err_data))
	return (E_DB_ERROR);
#endif /* xDEV_TEST */
    ...
    {
        TRdisplay ("E_UL0005 at point 3;  %d > (%p) %d \n",req_size , ulm_rcb->ulm_memleft, *ulm_rcb->ulm_memleft);

        TRdisplay("DIAG: %s:%d ulm_poolid = %p ulm_facility = %d\n",
       		__FILE__, __LINE__, ulm_rcb->ulm_poolid,
		   ulm_rcb->ulm_facility);
	ulm_rcb->ulm_error.err_code = E_UL0005_NOMEM;
	ulm_rcb->ulm_error.err_data = 3;
	status = E_DB_ERROR;
    }
    else
    {
	...
	{
	    ulm_rcb->ulm_error.err_code = E_UL0009_MEM_SEMWAIT;
	...
	i4	    chkerr;
	i4	    chkdata;
	if (chkerr = ulm_conchk(pool, &chkdata))
	{
	    ulm_rcb->ulm_error.err_code = chkerr;
	    ulm_rcb->ulm_error.err_data = chkdata;
	    status = E_DB_ERROR;
	}
    }
...
ulm_closestream( register ULM_RCB *ulm_rcb )
{
    ...
    */
	...
	{
	    status = E_DB_ERROR;
	    ulm_rcb->ulm_error.err_code = E_UL0004_CORRUPT;
	    break;
	}
#endif /* xDEBUG */
	...
	status = ulm_free((PTR)block, (PTR)pool, &ulm_rcb->ulm_error.err_code);
	if (DB_SUCCESS_MACRO(status))
	...
	status = ulm_free((PTR)stream, (PTR)pool, &ulm_rcb->ulm_error.err_code);
	if (DB_SUCCESS_MACRO(status))
	    *ulm_rcb->ulm_memleft += size;
    }
    ...
    {
	ulm_rcb->ulm_error.err_code = E_UL000A_MEM_SEMRELEASE;
	return (E_DB_ERROR);
    }
...
ulm_palloc( register ULM_RCB *ulm_rcb )
{
	...
	*/
	...
	if (ulm_rcb->ulm_facility >= DB_MIN_FACILITY &&
	    ...
	    facility = "UNKNOWN";
	TRdisplay("%@ ULM %s alloc ERROR pool 0x%x psize %d memleft %d error %d\n",
		facility, pool, ulm_rcb->ulm_psize, *ulm_rcb->ulm_memleft, 
		ulm_rcb->ulm_error.err_code);
    }
#endif /* xDEV_TEST */
...
ulm_mark( register ULM_RCB *ulm_rcb )
{
...
#endif /* xTR_OUTPUT */
    if (ulm_rcb->ulm_error.err_code = 
		ulm_conchk(stream->ulm_spool, &ulm_rcb->ulm_error.err_data))
	return (E_DB_ERROR);
#endif /* xDEV_TEST */
    ...
    ulm_rcb->ulm_error.err_code = E_UL0000_OK;
    
#ifdef    xDEV_TEST
    {
	i4	    chkerr;
	i4	    chkdata;
	if (chkerr = ulm_conchk(stream->ulm_spool, &chkdata))
	{
	    ulm_rcb->ulm_error.err_code = chkerr;
	    ulm_rcb->ulm_error.err_data = chkdata;
	    return(E_DB_ERROR);
...
ulm_reclaim( register ULM_RCB *ulm_rcb )
{
    ...
    {
	ulm_rcb->ulm_error.err_code = E_UL0015_BAD_STREAM_MARK;
	return (E_DB_ERROR);
    }
...
{
    DB_STATUS		status = E_DB_OK;
    i4		*err_code = &ulm_rcb->ulm_error.err_code;
    i4		*err_data = &ulm_rcb->ulm_error.err_data;
...
#endif /* xTR_OUTPUT */
    if (*err_code = ulm_conchk(pool, err_data))
	return (E_DB_ERROR);
#endif /* xDEV_TEST */
    ...
    if (owner != powner)
    {
	*err_code = E_UL000B_NOT_ALLOCATED;
	return (E_DB_ERROR);
    }
...
ulm_pinit( SIZE_TYPE  poolsize_pag, SIZE_TYPE  pref_chunksize_pag, 
	   ...
	   i4  owner, i4 pblksize, PTR chunk, PTR *pool,
	   i4 *err_code )
{
    ...
    if (CSw_semaphore(&poolhead->ulm_psem, CS_SEM_SINGLE,
    			STprintf( sem_name, "ULM pool (%s)", owner_names[owner])) != OK)
    {
	*err_code = E_UL0009_MEM_SEMWAIT;
	return (E_DB_ERROR);
    }
...
static DB_STATUS
ulm_nalloc( register PTR pool, i4 size, register PTR *block, i4 *err_code,
	    i4 *err_data )
{
    ...
    *err_data = 0;
    ...
    {
	status = E_DB_ERROR;
	*err_code = E_UL0014_BAD_ULM_PARM;
    }
    else
    {
	...
	*/
	...
	if (DB_SUCCESS_MACRO(status)  &&  current == (ULM_NODE *) NULL)
	{
	    ...
	    */
	    ...
	    status = ulm_challoc(poolhead, size, &current, err_code, err_data);
...
static DB_STATUS
ulm_free( register PTR block, register PTR pool, i4 *err_code )
{
    ULM_PHEAD		*poolhead = (ULM_PHEAD *) pool;
    ULM_HEAD		*listhead;
    ULM_HEAD		*freehead;
...
*******************************************************************************/
	...
	/* --------------------------------------- */
	status = ulm_fadd(node, poolhead->ulm_powner, freehead, err_code);
    }
...
static DB_STATUS
ulm_fadd( register ULM_NODE *node, i4  owner, register ULM_HEAD *freehead, i4 *err_code )
{
...
static DB_STATUS
ulm_challoc( register ULM_PHEAD *pool, i4 nodesize, ULM_NODE **node,
	     i4 *err_code, i4 *err_data )
{
    DB_STATUS 		status;
    ...
    */
    ...
    {
        TRdisplay ("E_UL0005 at point 6: need_page = %d, pool = %d, main= %d\n", need_pag, pool->ulm_pchsize_pag, premain_pag);
        TRdisplay ("pool size: %d,  alloc : %d\n",pool->ulm_psize_pag , pool->ulm_pages_alloced);

	*err_code = E_UL0005_NOMEM;
	*err_data = 6;
	*node = (ULM_NODE *) NULL;
	return (E_DB_ERROR);
    }
	...
	TRdisplay("%@ ULM %s Alloc failed: psize_pag %d, pages_alloc %d remain %d\n",
		facility, pool->ulm_psize_pag, pool->ulm_pages_alloced, 
		premain_pag);
	TRdisplay("%@ ULM %s Alloc failed: nodesize %d  needed %d csize %d\n",
	...
	*node = (ULM_NODE *) NULL;
    ...
    {
	TRdisplay("DIAG E_UL0005_NOMEM: %s:%d csize_pag = %d, need_pag = %d\n",
			__FILE__, __LINE__, csize_pag, need_pag);
	/*
	** No, we did not.  Before returning with
	** an error, release memory chunk to SCF.
	*/
	...
	TRdisplay("%@ ULM %s Alloc insufficient: psize_pag %d, pages_alloc %d remain %d\n",
		facility, pool->ulm_psize_pag, pool->ulm_pages_alloced, 
		premain_pag);
	TRdisplay("%@ ULM %s Alloc insufficient: nodesize %d  needed %d csize %d\n",
		facility, nodesize, need_pag, csize_pag);
	...
	*node = (ULM_NODE *) NULL;
    ...
    chunk_ptr->ulm_owner = (PTR)powner;
    ...
    /* ------------------------------- */
    status = ulm_fadd(node_ptr, powner, freelist, err_code);

    
    if (DB_SUCCESS_MACRO(status))
    {
	TRdisplay("%@ ULM %s Extend pool at 0x%p to %d pages\n", facility, pool,pool->ulm_pages_alloced);
	TRdisplay("%@ ULM %s Allocate chunk %d %d pages at 0x%p\n",
	...
	*node = node_ptr;
	...
	TRdisplay("%@ ULM %s ERROR Can't Allocate chunk %d %d pages at 0x%p\n",
		    facility, pool->ulm_pchunks, csize_pag, chunk_ptr);
...
ulm_trace( register ULM_RCB *ulm_rcb )
{
	...
	pool = stream->ulm_spool;
	chkerr = ulm_conchk(pool, &chkdata);
    }
    else
	status = E_DB_ERROR;
...
static i4
ulm_conchk( ULM_PHEAD *pool, i4 *failed_check )
{
    i4		err = E_UL0000_OK;
    ULM_HEAD            *listhead;
    ULM_HEAD            *freehead;
	...
	if (!err  &&  prev != listhead->ulm_hlast)
	{
	    /*
	...
	if (!err  &&  prev != freehead->ulm_hlast)
	{
	    /*
	...
	TRdisplay("****  which_check = %2d %27s****\n",
		    which_check, conchk_names[which_check]);
	TRdisplay("******************************************************\n");
    }
...
ulm_pad_error(ULM_NODE *node, char *padchar)
{
    ...
    orig_len = *(i4 *) (((char *)node) + node->ulm_nsize - sizeof(i4));
    TRdisplay("ULM pad error %x %x (%x %x %x %d %d)\n", 
	padchar, *padchar,
	node, node->ulm_nnext, node->ulm_nprev, node->ulm_nsize, 
	orig_len);
    ule_format(E_UL0017_DIAGNOSTIC, (CL_SYS_ERR *)0, ULE_LOG, NULL, 
...
ulm_printpool(PTR pool_ptr, char *facility)
{
		    ...
		    current = current->ulm_nprev)
	{
		...
		orig_len = 
		  *(i4 *) (((char *)current) + current->ulm_nsize - sizeof(i4));
		/*
