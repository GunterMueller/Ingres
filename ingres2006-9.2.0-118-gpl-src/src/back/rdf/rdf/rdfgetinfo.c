...
} special_proc [] =
{
	...
	DB_INGRES_NAME,
        /*12345678901234567890123456789012345678901234567890  - 50 char/line */
        {"create procedure iiQEF_finddbs (loc_name= char(DB_",
	...
	DB_INGRES_NAME,
	 ...
	 "cmptlvl = char(4), cmptminor = integer, access =  ",
...
static READONLY struct _QOPEN_INFO {
	    ...
	    i4		    datasize)
{
    ...
    READONLY struct _QOPEN_INFO *info_ptr;
    ...
    */
    static READONLY char    almval_table = DBOB_TABLE;
    ...
    */
	...
	if (DB_FAILURE_MACRO(status))
	    rdu_ferror(global, status, global->rdf_qeucb.error.err_code,
		E_RD0077_QEU_OPEN,0);
	else
	{
	...
	do
	{
		...
		if (statp->db_atno == rdfcb->rdf_rb.rdr_hattr_no)
		{
		    ...
		    */
			...
			has_version = FALSE;
			rdd_statp->shistlength = rdfcb->rdf_rb.rdr_cellsize;
		    }
	...
	if (read_symsize > 0)
	{
		...
		*/
		...
		case PST_2_VSN:
		{
			...
			= tstate->rdf_2op_node.pst_opmeta;
		    pst_qnode->pst_sym.pst_value.pst_s_op.pst_escape = 0;
		    pst_qnode->pst_sym.pst_value.pst_s_op.pst_isescape 
			= PST_DOESNT_APPLY;
		...
		{   
		    ...
		    pst_qnode->pst_sym.pst_value.pst_s_op.pst_opmeta
			= tstate->rdf_3op_node.pst_opmeta ;
	...
	*/
	...
	{   /* update the address ptr to the ADF function instance descriptor */
		...
		{	
			...
			rdu_ferror(global, status, adferror.err_code,
	    ...
	    i4              treetype)
{
	...
	** space on the stack is a problem then RDF_TUPLE_RETRIEVE can be
    ...
    request = treetype;			    /* need to make sure correct type
					    ** is used for keying */
    global->rdfcb->rdf_rb.rdr_rec_access_id = NULL;
    global->rdfcb->rdf_rb.rdr_qtuple_count = 0;
...
rdu_view(RDF_GLOBAL *global)
{
    ...
    {
	viewp = sys_infoblk->rdr_view;
	if (viewp)
	{
	    /* system copy exists, so use it directly in the user copy */
	...
	return(status);
    viewp = sys_infoblk->rdr_view;
    /* check if system copy has been created prior to rdu_xlock call */
    if (viewp)
    {	/* system copy exists, so use it directly in the user copy */
	usr_infoblk->rdr_view = sys_infoblk->rdr_view;
	Rdi_svcb->rdvstat.rds_c4_view++;
...
rdu_special_proc( char *prcname, char *owner, bool upcase)
{
	...
	if (upcase)
	{
	   if (STbcompare((PTR)prcname, DB_MAXNAME, 
				(PTR)special_proc[i].proc_name, DB_MAXNAME, 1))
	     continue;
	   if (STbcompare((PTR)owner, DB_MAXNAME, 
				(PTR)special_proc[i].proc_own, DB_MAXNAME, 1))
	     continue;
	}
	else
	{
	   if (MEcmp((PTR)prcname,(PTR)special_proc[i].proc_name, DB_MAXNAME))
	   	continue;
	   if (MEcmp((PTR) owner, (PTR) special_proc[i].proc_own, DB_MAXNAME))
	        continue;
	}
		...
		bool		   *refreshed)
{
	...
	*/
	/*
	MEcopy( (PTR) special_proc[special_id].proc_name, DB_MAXNAME,
		(PTR) &proc_tuple.db_dbpname);
	MEcopy( (PTR)  special_proc[special_id].proc_own, DB_MAXNAME,
...
rdu_usrbuf_read(RDF_GLOBAL	*global)
{
	...
	(rdfcb->rdf_rb.rdr_2types_mask & 
	    (RDR2_INDEP_PRIVILEGE_LIST | RDR2_KEYS | RDR2_SYNONYM)==0)
