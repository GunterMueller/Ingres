...

/*{
** Name: rdu_warn	- this routine will report an RDF warning.
**
** Description:
	    ...
	    RDF_ERROR	warn)
{
    ...
    ule_status = ule_format( warn, 0L, ULE_LOG, &sqlstate, msg_buffer,
...

/*{
** Name: rdu_ierror	- this routine will report the RDF error
**
** Description:
	    ...
	    RDF_ERROR		local_error)
{
    ...
    ule_status = ule_format( local_error, 0L, log, &sqlstate, msg_buffer,
	(i4) (sizeof(msg_buffer)-1), &msg_buf_length, &ule_error, 0);
    if (ule_status != E_DB_OK)
    {
        STprintf(msg_buffer,
	    "ULE error = %x, RDF message cannot be found - error no. = %x\n",
	    ule_error, local_error );
	TRdisplay("%s", msg_buffer);
    }
    else
	msg_buffer[msg_buf_length] = 0;	/* null terminate */
...
**	err_code			error code returned from external
**					    facility called.
	    ...
	    RDF_ERROR   suppress)
{
    ...
    ** cannot handle sparse switch statements */
    rdf_error->err_code = interror;
    rdf_error->err_data = err_code;
    if ((err_code >> 16) == DB_ULF_ID)
    {
	switch (err_code)
	{
	    case E_UL0004_CORRUPT:
		rdf_error->err_code = E_RD0006_MEM_CORRUPT;
		break;

	    case E_UL0005_NOMEM:
		rdf_error->err_code = E_RD0001_NO_MORE_MEM;
		break;

	    case E_UL0009_MEM_SEMWAIT:
		rdf_error->err_code = E_RD0008_MEM_SEMWAIT;
		break;

	    case E_UL000A_MEM_SEMRELEASE:
		rdf_error->err_code = E_RD0009_MEM_SEMRELEASE;
		break;

	    case E_UL000B_NOT_ALLOCATED:
		rdf_error->err_code = E_RD0007_MEM_NOT_OWNED;
		break;

	    case E_UL0006_BADPTR:
	    case E_UL0007_CANTFIND:
		rdf_error->err_code = E_RD000A_MEM_NOT_FREE;
		break;

	    case E_UL0101_TABLE_ID:
		rdf_error->err_code = E_RD0041_BAD_FCB;
		break;

	    case E_UL0103_NOT_EMPTY:
		rdf_error->err_code = E_RD0042_CACHE_NOT_EMPTY;
		break;

	    case E_UL0104_FULL_TABLE:
	    case E_UL0110_CHDR_MEM:
	    case E_UL0111_OBJ_MEM:
		rdf_error->err_code = E_RD0043_CACHE_FULL;
		break;

	    case E_UL0106_OBJECT_ID:
		rdf_error->err_code = E_RD0044_BAD_INFO_BLK;
		break;
		...
		** RDF at server startup.  So, just return */
		rdf_error->err_code = E_RD0145_ALIAS_MEM;
		return;
    ...
    else if ((err_code >> 16) == DB_DMF_ID)
    {
	switch (err_code)
	{
	    case E_DM0042_DEADLOCK:
		rdf_error->err_code = E_RD002A_DEADLOCK;
		break;

	    case E_DM0065_USER_INTR:
		rdf_error->err_code = E_RD000C_USER_INTR;
		break;

	    case E_DM0064_USER_ABORT:
		rdf_error->err_code = E_RD000D_USER_ABORT;
		break;

	    case E_DM0054_NONEXISTENT_TABLE:
		rdf_error->err_code = E_RD0002_UNKNOWN_TBL;
		...
		rdf_error->err_code = E_RD001A_RESOURCE_ERROR;
		return;			    /* exceeded some internal DMF
					    ** resource */
	    case E_DM0114_FILE_NOT_FOUND:
		rdf_error->err_code = E_RD0021_FILE_NOT_FOUND;
		break;

	    case E_DM004D_LOCK_TIMER_EXPIRED:
		rdf_error->err_code = E_RD002B_LOCK_TIMER_EXPIRED;
    ...
    else if ((err_code >> 16) == DB_QEF_ID)
    {
	switch (err_code)
	{
	    case E_QE0002_INTERNAL_ERROR:
		rdf_error->err_code = E_RD0010_QEF_ERROR;
		break;

	    case E_QE0011_AMBIGUOUS_REPLACE:
		rdf_error->err_code = E_RD0015_AMBIGUOUS_REPLACE;
		break;

	    case E_QE0015_NO_MORE_ROWS:
		rdf_error->err_code = E_RD0013_NO_TUPLE_FOUND;
		break;

	    case E_QE0017_BAD_CB:
	    case E_QE0018_BAD_PARAM_IN_CB:
		rdf_error->err_code = E_RD0020_INTERNAL_ERR;
		break;
		...
		rdf_error->err_code = E_RD0050_TRANSACTION_ABORTED;
		break;

	    case E_QE0025_USER_ERROR:
		rdf_error->err_code = E_RD0025_USER_ERROR;
		...
		rdf_error->err_code = E_RD0017_TABLE_EXISTS;
		break;

	    case E_QE0027_BAD_QUERY_ID:
		rdf_error->err_code = E_RD0014_BAD_QUERYTEXT_ID;
		break;

	    case E_QE0031_NONEXISTENT_TABLE:
		rdf_error->err_code = E_RD0018_NONEXISTENT_TABLE;
		break;
		...
		rdf_error->err_code = E_RD0016_KEY_SEQ;
		break;

	    case E_QE0051_TABLE_ACCESS_CONFLICT:
		rdf_error->err_code = E_RD0019_TABLE_ACCESS_CONFLICT;
		break;
		...
		rdf_error->err_code = E_RD0210_ABANDONED_OBJECTS;
		return;

	    case E_QE0255_NO_SCHEMA_TO_DROP:
		rdf_error->err_code = E_RD0211_NONEXISTENT_SCHEMA;
		return;

	    case E_QE0256_NONEMPTY_SCHEMA:
		rdf_error->err_code = E_RD0212_NONEMPTY_SCHEMA;
		return;

	    case E_QE011C_NONEXISTENT_SYNONYM:
		rdf_error->err_code = E_RD014A_NONEXISTENT_SYNONYM;
		return;
    ...
    else if ((err_code >> 16) == DB_CLF_ID)
    {
	switch (err_code)
	{
	    case E_CS0004_BAD_PARAMETER:
	    case E_CS000A_NO_SEMAPHORE:
		rdf_error->err_code = E_RD0031_SEM_INIT_ERR;
		break;

	    case E_CS0017_SMPR_DEADLOCK:
		rdf_error->err_code = E_RD0032_NO_SEMAPHORE;
		break;
	    default:
		break;
	}
...
log all messages for now so that DMT_SHOW errors etc. are reported.
	...
	ule_status = ule_format( rdf_error->err_code, 0L, log, &sqlstate,
    ...
    "ULE error = %x\nRDF cannot find error no = %x Status = %x Facility error = %x \n",
		ule_error, rdf_error->err_code , status, rdf_error->err_data);
	}
	else
	    msg_buffer[msg_buf_length] = 0;	/* null terminate */
	...
	** unless it is simple (like cannot find message) */
	{   /* report any errors in the range 0 to 127 to the user */
	    ...
	    scf_status = rdu_sccerror(status, &sqlstate,
		    rdf_error->err_data,  msg_buffer, (i4)msg_buf_length);
	    if (scf_status != E_DB_OK)
	    {
		TRdisplay(
		    "RDF error = %x Status = %x Facility error = %x \n",
		    rdf_error->err_code , status, rdf_error->err_data);
	    }
	}
	ule_status = ule_format( rdf_error->err_data, 0L, log, &sqlstate,
	    msg_buffer, (i4) (sizeof(msg_buffer)-1), &msg_buf_length,
	    &ule_error, 0);
	if (ule_status != E_DB_OK)
	{
		...
		RDF_ERROR	    errcode)
{
	...
	status = orig_status;
	rdu_ferror(global, status, global->rdf_ulmcb.ulm_error.err_code,
	    errcode,0);
    }
    return(status);
}
		...
		RDF_STATE	   *state)
{
	...
	*/
	if (infoblk->rdr_2_types & RDR2_DEFAULT)
	{
	    ...
	    */
		...
		if ( attr && attr->att_default)
		{
			...
			t_status = rdu_i_deulh(global, attr);
		    else
			/* release the default cache object */
			t_status = rdu_r_deulh(global, attr);
    ...
    {
	rdu_ferror(global, status,
	    ulm_rcb->ulm_error.err_code, E_RD011C_ULM_CLOSE,0);
    }
    else
    {
...
rdu_iulh(RDF_GLOBAL *global)
{
    ...
    */
    ...
    {
	/* this is a relation cahce object, so see if it currently has any
	...
	*/
	...
	if (infoblk->rdr_2_types & RDR2_DEFAULT )
	{
	    ...
	    */
		...
		if ( attr && attr->att_default)
		{
		    ...
		    if (global->rdf_de_ulhobject->rdf_state == RDF_SBAD)
			/* destroy the default cache object */
			status = rdu_i_deulh(global, attr);
		    else
			/* release the default cache object */
			status = rdu_r_deulh(global, attr);
		    global->rdf_de_ulhobject = 0;
    ...
    {
	rdu_ferror(global, ulh_status, ulh_rcb->ulh_error.err_code,
	    E_RD0040_ULH_ERROR,0);
	status = ulh_status;
    }
    return(status);
...
*/
DB_STATUS
rdu_r_deulh(RDF_GLOBAL *global, DMT_ATT_ENTRY *att_ptr)
{
    /* call ULH to release lock on the system info block */
    ULH_RCB	    *ulh_rcb;
    DB_STATUS	    ulh_status;
...
*/
DB_STATUS
rdu_i_deulh(RDF_GLOBAL *global, DMT_ATT_ENTRY *att_ptr)
{
    /* call ULH to release lock on the system info block */
    ULH_RCB	    *ulh_rcb;
    DB_STATUS	    ulh_status;
...
**	    block's ptuples etc.
**	25-Aug-06 (kibro01) bug 116147
**	    If we already have a private area set up for this object,
**	    we should use that instead of trying to lock the system copy
**	    of the object.  This can happen if this thread overtakes the
**	    one which originally started updating this object.
**	    By using the private copy, we still do everything as we would
**	    have done before, such as updating the counter to say we're
**	    linked to the object, but now we allow the original thread
**	    which started updating to carry on the job.
**	    This caused a crash in psy_view because the object was grabbed
**	    for update, meaning that sys_infoblk was updated, but the
**	    thread already had a private area for the object, which was
**	    left containing null pointers.
[@history_template@]...
*/
DB_STATUS
	    ...
	    RDR_INFO           **infoblkpp)
{
    ...
    */
	...
	priv_flag
	||
	(sys_infoblk != global->rdfcb->rdf_info_blk)
       )
    {
	...
	*/
	...
	{   /* a private stream has already be set up for the user so place
	    ** there is no need to create another one
	    */
	    *infoblkpp = global->rdfcb->rdf_info_blk;

	    /* update statistics indicating priv infoblk is locked */
...
rdd_setsem(RDF_GLOBAL *global, i4 sem_type)
{
    ...
    if (DB_FAILURE_MACRO(status))
    {
	rdu_ferror(global, E_DB_ERROR, status, E_RD0031_SEM_INIT_ERR,0);
	status = E_DB_ERROR;
    }
    else
    {
...
rdd_resetsem(RDF_GLOBAL *global, i4 sem_type)
{
    ...
    if (DB_FAILURE_MACRO(status))
    {
	rdu_ferror(global, E_DB_ERROR, status, E_RD0034_RELEASE_SEMAPHORE,0);
	status = E_DB_ERROR;
    }
    else
    {
