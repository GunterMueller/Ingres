...
*/
		...
		*flagmask |= RDU_1SYN;
		...
		if (DB_FAILURE_MACRO(status))
		{
			...
			rdu_ferror( global, status, 
				    global->rdf_ulhcb.ulh_error.err_code,
	...
	*/
	...
	if (dmt_shw_cb->error.err_code == E_DM004D_LOCK_TIMER_EXPIRED)
	{
	    ...
	    ** in psf.
	    */
	    rdu_ferror(global, status, dmt_shw_cb->error.err_code,
	    ...
	    bool	*refreshed)
{
    ...
    */
	...
	if (DB_FAILURE_MACRO(status))
	{
	    ...
	    {
		rdu_ferror(global, status, global->rdf_ulhcb.ulh_error.err_code,
...
rdu_key_ary_bld(RDF_GLOBAL *global)
{
	...
	** so that the keys of the shared descriptor can be used */
    ...
    {
	i4		max_attno;	    /* maximum key sequence number
					    ** for a consistency check */
	i4		no_of_key;
	DMT_ATT_ENTRY 	**attr;
	i4		keys[DB_MAXKEYS];   /* temp array to store key
					    ** attributes of storage
					    ** structure */
	bool		toosmall;	    /* TRUE if more than DB_MAXKEY
	...
	no_of_key = 0;
	max_attno = 0;
	MEfill(sizeof(keys), (u_char)0, (PTR)&keys[0]);
	{
	    i4			attno;
	    for (attno= 1; attno <= usr_infoblk->rdr_no_attr; attno++)
	    {   /* first attempt to build key array structure in local stack memory
		*/
		if (attr[attno]->att_key_seq_number > 0)
		{
		    no_of_key++;
		    if (attr[attno]->att_key_seq_number > max_attno)
			max_attno = (i4)attr[attno]->att_key_seq_number;
		    if (attr[attno]->att_key_seq_number <= DB_MAXKEYS)
		    {
			if (keys[attr[attno]->att_key_seq_number-1])
			{   /* if non zero then a duplicate key field is found
			    ** so catalogs are inconsistent */
			    status = E_DB_SEVERE;
			    rdu_ierror(global, status, E_RD0113_KEYS);
			    return(status);
			}
			keys[attr[attno]->att_key_seq_number-1]
			    = attr[attno]->att_number;
		    }
		    else
			toosmall = TRUE;
		}
	...
	if (toosmall)
	{   /* traverse again to pickup the extra long storage structure */
	    i4	    lattno;
	    MEfill(sizeof(i4) * (no_of_key + 1),
		(u_char)0, (PTR)rdr_keys->key_array);
	    for (lattno = 1; lattno <= usr_infoblk->rdr_no_attr; lattno++)
	    {
		if (attr[lattno]->att_key_seq_number)
		{
		    if (rdr_keys->key_array[attr[lattno]->att_key_seq_number-1])
		    {   /* if non zero then a duplicate key field is found
			** so catalogs are inconsistent */
			status = E_DB_SEVERE;
			rdu_ierror(global, status, E_RD0113_KEYS);
		    ...
		    rdr_keys->key_array[attr[lattno]->att_key_seq_number-1] 
			= attr[lattno]->att_number;
		}
	    }
	}
	else
	{   /* just copy from the temp array */
	    MEcopy ((PTR)&keys[0], sizeof(i4) * no_of_key,
		(PTR)rdr_keys->key_array );
	}
...
{	
    ...
    */
    global->rdfcb = rdfcb;
    rdfcb->rdf_error.err_code = E_RD0000_OK;
    status = E_DB_OK;
...
**	only appears one time in the DMT_ATTR_ENTRY and is therefore only 
