...
iidd_ddb_ldbids a, iidd_ddb_ldb_dbcaps b where a.ldb_id =",
	...
	*/
	...
	if (qefrcb->error.err_code == E_QE0506_CANNOT_GET_ASSOCIATION)
	    global->rdfcb->rdf_error.err_code = E_RD0277_CANNOT_GET_ASSOCIATION;
	else 
	    rdu_ferror(global, status, qefrcb->error.err_code,
		...
		i4		   ldb_id)
{
	...
	*/
	...
	rdu_ferror(global, status, global->rdf_dist_ulhcb.ulh_error.err_code,
	    ...
	    ADF_CB	*adfcb)
{
    ...
    if (DB_FAILURE_MACRO(status))
	rdu_ferror(global, status, adfcb->adf_errcb.ad_errcode,
...
rdd_username(RDF_GLOBAL *global)
{
    ...
    {
	status = E_DB_ERROR;
	global->rdfcb->rdf_error.err_code = E_RD026D_USER_NOT_OWNER;
    }	 
...
column_defaults, column_sequence from iicolumns where table_name =",
		...
		*/
		...
		if ( column.sequence != star_attr->att_number ||
		     (
		       ...
		       &&
		       !(star_attr->att_flags & DMT_F_NDEFAULT)
		     )
		   )
		{
		    status = E_DB_ERROR;
    ...
    */
	...
	{
	    status = E_DB_ERROR;
	    global->rdfcb->rdf_error.err_code = E_RD0269_OBJ_DATEMISMATCH; 
	}
	else
	    status = E_DB_OK;
    }
...
overflow_pages from iitables where table_name =",
	...
	{  /* no object was found */
	    status = E_DB_ERROR;
	    global->rdfcb->rdf_error.err_code = E_RD0268_LOCALTABLE_NOTFOUND; 
	    return(status);
	}
	...
	*/
	...
	if (status)
	   ...
	   ** to check the link is not exist.*/
	    global->rdfcb->rdf_error.err_code = E_RD0002_UNKNOWN_TBL; 
	    return (E_DB_ERROR); 
	}
    ...
    ** from user defined length */
	...
	{   /* attribute starts from 1 */
	    ...
	    ** to DMT_F_NDEFAULTS */ 
	    ...
	    ? (col_ptr->att_flags = DMT_F_NDEFAULT)
	    : (col_ptr->att_flags = 0); 
	...
	if (ddr_p->qer_d5_out_info.qed_o3_output_b)
	{
		...
		if (save_idx)
		{
			...
			** display error in rdf */
			...
			global->rdfcb->rdf_error.err_code = E_RD026A_OBJ_INDEXCOUNT;
			status = rdd_flush(global);
			return(E_DB_ERROR);
		    }
		}
