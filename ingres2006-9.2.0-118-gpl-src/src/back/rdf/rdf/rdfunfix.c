	    ...
	    RDF_CB	*rdf_cb)
{
    ...
    */
    ...
    rdf_cb->rdf_error.err_code = E_RD0000_OK;
...

	...
	*/
		...
		for (i=DB_IMTID+1; i <= usr_infoblk->rdr_no_attr; i++)
		{
			...
			*/
			...
			if (global->rdf_de_ulhobject->rdf_state == RDF_SBAD)
			    /* destroy the distributed object */
			    ulh_status = rdu_i_deulh(global, attr);
			else
			    /* release the distributed object */
			    ulh_status = rdu_r_deulh(global, attr);
		    }
		}
	    }
	}
	...
	{   /* This is NOT a private infoblk, so it is the shared one */
	    ...
	    {
		/*
		** This is a distributed object, so clear ldb descriptor before
		** releasing the ulh object.  This must be done under semaphore
		** protection.
		*/
		status = rdu_gsemaphore(global);
		if (DB_SUCCESS_MACRO(status))
		{
		    /* if no other session is using the object, unfix the
		    ** LDBdesc cache object and clear ldb desc ptr
		    */
		    if (global->rdf_ulhobject->rdf_shared_sessions < 1)
		    {
			/* no one is using this infoblk, so unfix the LDBdesc
			** cache object and clear the LDBdesc ptr
			*/
			if (sys_infoblk->rdr_obj_desc->dd_o9_tab_info.
				dd_t9_ldb_p )
			{
			    /* unfix this LDBdesc cache object */
			    if (global->rdf_d_ulhobject->rdf_state == RDF_SBAD)
				/* destroy the distributed object */
				ulh_status = rdu_i_dulh(global);
			    else
		    ...
		    /* let's not worry return status here */
		    status = rdu_rsemaphore(global);    /* release semaphore */
	    ...
	    {
		/* if there are any defaults cache objects attached to this
		** infoblk and we are the last thread to unfix the infoblk,
		** we need to unfix the defaults cache objects and unchain
		** them from this infoblk.  This can only be performed under
		...
		*/
		...
		if (DB_SUCCESS_MACRO(status))
		{
		    ...
		    {
			i4		i;
			DMT_ATT_ENTRY   *attr;  /* ptr to DMT_ATT_ENTRY array */

			/* When RDF built this infoblk, it fixed one or
			** more defaults cache entries to attribute descriptors.
			** Unfix them now.
			*/
			for (i=DB_IMTID+1; i<=usr_infoblk->rdr_no_attr; i++)
			{
				...
				if (global->rdf_de_ulhobject->rdf_state ==
				    RDF_SBAD
				   )
				    /* destroy the distributed object */
				    ulh_status = rdu_i_deulh(global, attr);
