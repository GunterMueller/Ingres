...
**	5-apr-2005 (mutma03)
**	    Suppressed the error messages if the function sce_econnect has been **	    called by the recovery server as the EV_SEGMENT may not have been
...
sce_edump( SCEV_CONTEXT	*evc )
{
    ...
    (*etrc)(STprintf(stbuf,
	    ...
	    sizeof(ecb->evs_segname), ecb->evs_segname));
    (*etrc)(STprintf(stbuf,
	    "  evs_orig_pid = 0x%x\n", (i4)ecb->evs_orig_pid));
    (*etrc)(STprintf(stbuf,
	...
	(*etrc)(STprintf(stbuf, "    event_name: %#s\n",
		sce_ename(eptr->ev_relname, eptr->ev_rename, evname), evname));
	...
	if (sentry->ev_svrinstoff != 0)
	{
	  ...
	  while ((PTR)inst != (PTR)ecb)
	  {
		...
		sce_ename(inst->ev_inlname, inst->ev_inname, evname), evname));
