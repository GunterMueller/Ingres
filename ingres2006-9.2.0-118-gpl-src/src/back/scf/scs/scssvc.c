	    ...
	    || (eid == CS_KILL_EVENT))
	{
				...
				(list->sc_afcn[fac].afcn_addr)))
		{
			...
			switch (fac)
			{
				...
				break;
			    case DB_ULF_ID:
				fac_scb = scb->scb_sscb.sscb_ulscb;
				break;
...
scs_i_interpret(SCD_SCB *scb )
{
    ...
    status = IIGCa_call(GCA_INTERPRET,
			...
			&error);
    if (status)
	return(it_parms.gca_status);

    scb->scb_sscb.sscb_rsptype = GCA_ACK;
    buf = (GCA_AT_DATA *) it_parms.gca_data_area;
...

{
	...
	*/
	...
	{
		i4	qlen = scb->scb_sscb.sscb_ics.ics_l_qbuf;
		PTR	qPtr = scb->scb_sscb.sscb_ics.ics_qbuf;

		if (qlen == 0)
		{
		    qlen = scb->scb_sscb.sscb_ics.ics_l_lqbuf;
		    qPtr = scb->scb_sscb.sscb_ics.ics_lqbuf;
		}
        	TRformat(sc0e_tput, &print_mask, buf, sizeof(buf) - 1,
    		"    Query: %t\n", (qlen ? qlen : 0), (qlen ? qPtr : ""));
	}
	else
	{
		i2 qlen = 0, oLen = 0;
