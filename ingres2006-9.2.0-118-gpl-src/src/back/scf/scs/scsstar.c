...
scs_dctdesc(SCD_SCB *scb )
{
    ...
    DB_STATUS    status;
    DB_ERROR     error;
    QEF_RCB      *qefrcb = (QEF_RCB *)scb->scb_sscb.sscb_qeccb;
    GCA_TD_DATA	 *td;
    i4	 ncols;
    PTR		block;
    ...
    it = (GCA_IT_PARMS *)scb->scb_sscb.sscb_dccb->dc_dcblk.sdc_it;    
    ...
    MEcopy((PTR)(it->gca_data_area + size), sizeof(td->gca_l_col_desc), 
	   &ncols);
    ...
    */
    ...
    scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length =
	    Sc_main_cb->sc_gchdr_size + size + it->gca_d_length;
	
    status = IIGCa_call(GCA_FORMAT,
			(GCA_PARMLIST *)&scb->scb_cscb.cscb_gcp.gca_fo_parms,
	       		GCA_SYNC_FLAG, (PTR)0, -1, (i4 *)&error);
    ...
    darea = scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_data_area;
	...
	it = (GCA_IT_PARMS *)((*dc)->sdc_it);
	MEcopy(it->gca_data_area, it->gca_d_length, (PTR)(darea + size));
	size += it->gca_d_length;
	if (it->gca_end_of_data == TRUE)
	    break;
...
#define SCP_DONE     30 /* Done reading copy map, Next state : None */
    ...
    if(( ((GCA_IT_PARMS *)((*dc)->sdc_it))->gca_message_type == GCA1_C_FROM ) ||
       ( ((GCA_IT_PARMS *)((*dc)->sdc_it))->gca_message_type == GCA1_C_INTO ))
...
*/
    ...
    */
    ...
    {
	it = (GCA_IT_PARMS *)((*dc)->sdc_it);
	data = it->gca_data_area;
	nleft = it->gca_d_length;
	...
	*/
	...
	msg->scg_mtype = it->gca_message_type;
	if ((fe_version >= GCA_PROTOCOL_LEVEL_50) &&
	    (it->gca_message_type == GCA_C_FROM))
		    msg->scg_mtype = GCA1_C_FROM;
	else if ((fe_version >= GCA_PROTOCOL_LEVEL_50) &&
	    (it->gca_message_type == GCA_C_INTO))
		    msg->scg_mtype = GCA1_C_INTO;
	else if ((fe_version < GCA_PROTOCOL_LEVEL_50) &&
	     (it->gca_message_type == GCA1_C_FROM))
		    msg->scg_mtype = GCA_C_FROM;
	else if ((fe_version < GCA_PROTOCOL_LEVEL_50) &&
	     (it->gca_message_type == GCA1_C_INTO))
	...
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_buffer =
	    msg->scg_marea = (PTR)msg + sizeof(SCC_GCMSG);
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length =
	    Sc_main_cb->sc_gchdr_size + msg->scg_msize;
	...
	cpmap = scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_data_area;
	cpptr = cpmap + cpsiz;
	...
	if ((it->gca_end_of_data == TRUE) || (DB_FAILURE_MACRO(status)))
	    break;
...
**	it		Address to return address of formatted GCA message.
**
** Outputs:
**	it		Addres of formatted GCA message.
**
**    Returns:
             ...
             GCA_IT_PARMS **it )
{
    ...
    *it = &scb->scb_cscb.cscb_gcp.gca_it_parms;
    return (E_DB_OK);
}

/*{
...
**	it	- Pointer to a GCA message block (GCA_IT_PARM)
**	message - Address to return address of formatted message
**	copy	- Allocate a new buffer?
**
** Outputs:
**	message - Address of formatted message.
**
**    Returns:
	   ...
	   bool   copy )
{
    ...
    if (copy)
	size += (Sc_main_cb->sc_gchdr_size + it->gca_d_length);
    ...
    *message = (SCC_GCMSG *)block;
    ...
    (*message)->scg_mtype = it->gca_message_type;
    (*message)->scg_msize = it->gca_d_length;
    if ((it->gca_message_type == GCA_TUPLES) ||
	(it->gca_message_type == GCA_CDATA))
        (*message)->scg_mdescriptor = scb->scb_sscb.sscb_dccb->dc_tdesc;
    else
        (*message)->scg_mdescriptor = NULL;	
    if (it->gca_end_of_data == FALSE)
	...
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_buffer =
	    (*message)->scg_marea =
		(PTR)(*message) + sizeof(SCC_GCMSG);
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length =
	    Sc_main_cb->sc_gchdr_size + it->gca_d_length;
	status = IIGCa_call(GCA_FORMAT,
			   (GCA_PARMLIST *)&scb->scb_cscb.cscb_gcp.gca_fo_parms,
			   GCA_SYNC_FLAG,
			   (PTR)0, -1, (i4 *)&error);
	...
	MEcopy(it->gca_data_area, it->gca_d_length,
	       scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_data_area);
    }
    else
    {
	(*message)->scg_marea = it->gca_buffer;
    }
	...
	*/
	...
	erdata = (GCA_ER_DATA *)
			((*message)->scg_marea + Sc_main_cb->sc_gchdr_size);
...
**
** Inputs:
**      it	GCA_IT_PARM, GCA message block from FE.
**
** Outputs:
**	None
**
...
scs_dcmsg(GCA_IT_PARMS *it )
{
    ...
    if (it->gca_message_type != GCA_QUERY)
	return (FALSE);

    if (it->gca_end_of_data == FALSE)
	return (FALSE);

    qptr = ((GCA_Q_DATA *)it->gca_data_area)->gca_qdata;
		...
		GCA_IT_PARMS **it )
{
    ...
    itp.gca_buffer = scb->scb_cscb.cscb_intr_buffer;
    
    status = IIGCa_call(GCA_INTERPRET, (GCA_PARMLIST *)&itp,
			GCA_SYNC_FLAG, (PTR)0, -1, (i4 *)&error);
    ...
    status = scs_dccont(scb, SDC_WRITE, GCA_ATTENTION, (PTR)&itp);
    if (DB_FAILURE_MACRO(status))
    {
        scb->scb_sscb.sscb_flags &= ~SCS_STAR_INTERRUPT;
	return(status);
    ...
    do
    {
        ...
        *it = (GCA_IT_PARMS *)dc->sdc_it;
    } while (((*it)->gca_message_type != GCA_IACK) ||
         ((*it)->gca_end_of_data != TRUE));
            ...
            PTR   msg )
{
    ...
    status = qef_call(QED_C2_CONT, ( PTR ) qe_ccb);
...
scs_dcendcpy( SCD_SCB *scb )
{
    ...
    status1 = qef_call( qef_op, ( PTR ) qe_ccb );
    scb->scb_sscb.sscb_cfac = 0;
            ...
            i4	opcode )
{
	...
	*/
	...
	scb->scb_sscb.sscb_cfac = DB_QEF_ID;
	status = qef_call(QED_C1_CONN, ( PTR ) qe_ccb);
	scb->scb_sscb.sscb_cfac = 0;
	if (status != E_DB_OK)
	{
    ...
    */
	...
	if ((scb->scb_sscb.sscb_dccb->dc_tdescmsg != NULL)
		&& ((it->gca_message_type != GCA_ACK) &&
		    (it->gca_message_type != GCA_CDATA) &&
		    (it->gca_message_type != GCA_RESPONSE)))
	{
	...
	if ((it->gca_message_type == GCA_QUERY) ||
	    (it->gca_message_type == GCA_RELEASE) ||
	    (fe_released == TRUE))
	{
	    /*
		...
		*/
		scb->scb_sscb.sscb_cfac = DB_QEF_ID;
		status = qef_call(QED_C3_DISC, ( PTR ) qe_ccb);
		...
		if ((it->gca_message_type == GCA_RELEASE) ||
		    (fe_released == TRUE))
		{
		    scb->scb_sscb.sscb_state = SCS_TERMINATE;
	...
	/* 2. Send message to ldb */
	status = scs_dccont(scb, SDC_WRITE, it->gca_message_type, (PTR)it);
    ...
    it = (GCA_IT_PARMS *)scb->scb_sscb.sscb_dccb->dc_dcblk.sdc_it;
    scb->scb_sscb.sscb_rsptype = it->gca_message_type;
	...
	default :
	    ...
	    sc0e_put( E_SC0226_BAD_GCA_TYPE, 0, 1, 
		     sizeof( it->gca_message_type ),
		     (PTR)&it->gca_message_type,
		     0, (PTR)0,
		     0, (PTR)0,
		     0, (PTR)0,
		     0, (PTR)0,
    ...
    if (it->gca_message_type == GCA_TDESCR) 
	status = scs_dctdesc(scb);  /* Fetch LDB tuple descriptor */
    else if ((it->gca_message_type == GCA_C_INTO) ||
	     (it->gca_message_type == GCA_C_FROM) ||
	     (it->gca_message_type == GCA1_C_INTO) ||
	     (it->gca_message_type == GCA1_C_FROM))
	status = scs_dccpmap(scb);  /* Fetch LDB copy map */
    else
	status = scs_dcsend(scb, it, &message, FALSE);  /* Relay msg */

    if ((it->gca_message_type == GCA_RETPROC) && (peer != fe))
    {
	...
	*/
	if (peer >= GCA_PROTOCOL_LEVEL_60)
	{
	    ...
	    while (it->gca_message_type != GCA_RESPONSE)
	    {
		status = scs_dccont(scb, SDC_READ, 0, NULL);
		if ((DB_FAILURE_MACRO(status)) || 
		    (scb->scb_sscb.sscb_interrupt != 0))
	   ...
	   i4  opcode )
{
	...
	*/
	...
	/* 2. Connect to ldb. */
	scb->scb_sscb.sscb_cfac = DB_QEF_ID;
	status = qef_call(QED_B_COPY, ( PTR ) qe_ccb);
	scb->scb_sscb.sscb_cfac = 0;
	if (DB_FAILURE_MACRO(status))
	{
	...
	{   /* 4. Read LDB response */
	    ...
	    it = (GCA_IT_PARMS *)
			scb->scb_sscb.sscb_dccb->dc_dcblk.sdc_it;

	    if ((it->gca_message_type != GCA_C_FROM) &&
		(it->gca_message_type != GCA_C_INTO) &&
		(it->gca_message_type != GCA1_C_INTO) &&
		(it->gca_message_type != GCA1_C_FROM) &&
		(it->gca_message_type != GCA_RESPONSE))
	    {
		status = scs_dcsend(scb, it, &message, TRUE);
		if ((DB_FAILURE_MACRO(status)) ||
		    (scb->scb_sscb.sscb_interrupt != 0))
		{
		     break;
	...
	else 
	{
	    ...
	    re = (GCA_RE_DATA *)it->gca_data_area;
    ...
    */
    ...
    if (it->gca_message_type != GCA_RESPONSE) 
    {
	/* 3. LDB did not send a GCA_RESPONSE, keep reading. */
	status = scs_dcsend(scb, it, &message, FALSE); /* Relay to FE */
	if (DB_FAILURE_MACRO(status))
	{
	    sc0e_put(status, 0, 0,
		     0, (PTR)0,
	...
	*next_op = CS_OUTPUT;
	...
	re = (GCA_RE_DATA *)it->gca_data_area;	/* Build response block */
	scb->scb_sscb.sscb_cquery.cur_row_count = re->gca_rowcount;
	   ...
	   i4  opcode )
{
    ...
    *next_op = CS_OUTPUT;
	...
	if ((it->gca_message_type ==  GCA_S_ETRAN) ||
	    (it->gca_message_type ==  GCA_DONE) ||
	    (it->gca_message_type ==  GCA_REFUSE) ||
	    (it->gca_message_type ==  GCA_RESPONSE))
	{
	...
	else if ((it->gca_message_type == GCA_TRACE) ||
		 (it->gca_message_type == GCA_ERROR) ||
		 (it->gca_message_type == GCA_EVENT))
	{
	    /* 
