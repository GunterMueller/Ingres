...
void scs_compress(GCA_TD_DATA *tup_desc, char *local_row_buf, 
...

{
    ...
    i4			length;                 /* Length of column */
    bool 		has_varchar=FALSE;      /* Does col have varchar? */
    ...
    *total_dif = 0;
    ...
    { 
	  ...
	  length = cur_att->gca_attdbv.db_length;
#endif
          switch (abs(type))
          {
             case DB_LVCH_TYPE:
                ...
                else if (!null_flag && (type > 0))
		{
	     ...
	     case DB_NVCHR_TYPE:
             ...
             case DB_LTXT_TYPE:
             {
                if (type < 0 ) 
                {
                   if (*(((u_char *)local_row_buf) + 
                      (length - sizeof(char))) & ADF_NVL_BIT)
                   {
                      local_length = 0;
                      MECOPY_CONST_MACRO(&local_length, sizeof(u_i2),
                         (PTR)local_row_buf); 
                      null_flag = ADF_NVL_BIT;
                   }
                   else
                   {
                      null_flag = 0;
		   ...
		   if (type == -DB_NVCHR_TYPE)
		      local_length = local_length * DB_ELMSZ;
                   adj_length = local_length + sizeof(u_i2) + sizeof(char);
                }
                else
                {
                   MECOPY_CONST_MACRO((PTR)local_row_buf, sizeof(u_i2),
		   ...
		   if (type == DB_NVCHR_TYPE)
		      local_length = local_length * DB_ELMSZ;
                   adj_length = local_length + sizeof(u_i2);
                }
                dif = length - adj_length;
                *total_dif += dif;
                MEcopy((PTR)local_row_buf,adj_length,
                   ((PTR)compress_buf + total_len));
                total_len += adj_length;
                if (type < 0 ) 
                {
                   MECOPY_CONST_MACRO(&null_flag,sizeof(char),
                      compress_buf+total_len - sizeof(char));
                }
                local_row_buf += length; 
                break;
             }
             default:
             {
                MEcopy((PTR)local_row_buf,
                   length, ((PTR)compress_buf+total_len));
                total_len += length;
                local_row_buf += length;;
                break;
             }       
