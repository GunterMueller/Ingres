...
/* bug 70170 */
		...
		if (DB_SUCCESS_MACRO(status))
		{
		    ...
		    if (qmode == PSQ_RETRIEVE || qmode == PSQ_RETCURS
		      || rowproc)
		    {
			scs_emit_rows(qmode, rowproc, scb, qe_ccb, message);
		    }
		    else
		    {
			scb->scb_sscb.sscb_state = SCS_INPUT;
...

		...
		** unless an application code is set
		*/
...
static void
scs_emit_rows(i4 qmode, bool rowproc,
	SCD_SCB *scb, QEF_RCB *qe_ccb, SCC_GCMSG *message)

{
	...
	*/
    ...
    if (tupbuf_used == 0)
	return;			/* No rows produced at all */
    ...
    */
	...
	if (tdesc)
	{	
	    scs_compress(tdesc, scb->scb_cscb.cscb_tuples,
