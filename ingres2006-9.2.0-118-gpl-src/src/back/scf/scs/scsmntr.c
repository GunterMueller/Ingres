	       ...
	       i4  *next_op )
{
	...
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length =
		Sc_main_cb->sc_gchdr_size + sizeof(GCA_TD_DATA)
			- sizeof(tdescr->gca_col_desc[0].gca_attname);
	...
	if (status)
	{
	    scs_gca_error(status, error, E_SC0024_INTERNAL_ERROR);
	    scb->scb_sscb.sscb_state = SCS_TERMINATE;
	    *next_op = CS_TERMINATE;
	    status = E_DB_SEVERE;
	    break;
	...
	tdescr =
	    (GCA_TD_DATA *) scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_data_area;
...
/*	status = sc0m_allocate(0,
	...
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_buffer =
		...
		((char *) message + sizeof(SCC_GCMSG));
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length =
		Sc_main_cb->sc_gchdr_size + scb->scb_cscb.cscb_comm_size;
	...
	text_string =
	    (DB_TEXT_STRING *) scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_data_area;
