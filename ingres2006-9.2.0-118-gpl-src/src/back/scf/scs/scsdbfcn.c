...
/*
** This is the id of IIDBDB.  It is defined in DUCREATE.C and hardcoded
** in SCSINIT.C.
*/

#define		IIDBDB_ID	1	/* aka DU_DBIDDBDB */
	   ...
	   SCV_DBCB  **dbcb_loc )
{
        ...
        */
		...
		&sys_err);
            if (lk_status != OK)
            {
                ...
                error->err_code = E_SC0389_LOCK_CREATE;
                break;
            }
	    ...
	    cnf_lkid.lk_common = 0;
            ...
            if (lk_status != OK)
            {
                ule_format(lk_status, &sys_err, ULE_LOG, NULL, 0, 0, 0,
                    &local_error, 0);
                error->err_code = E_SC0390_BAD_LOCK_REQUEST;
                break;
            }
	...
	if (flag & SCV_NOOPEN_MASK)
	{
            if(cnf_locked)
            {
                ...
                if (lk_status != OK)
                {
                    ule_format(lk_status, &sys_err, ULE_LOG, NULL,
                        (char *)0, 0L, (i4 *)0, &local_error, 0);
                    error->err_code = E_SC0391_BAD_LOCK_RELEASE;
                }
	    ...
	    if (notfound)
	    {
		error->err_code = E_SC012D_DB_NOT_FOUND;
		return(E_DB_ERROR);
	    }
	    if (dbcb_loc)
	    {
	...
	*/
	...
	if (dmc->error.err_code != E_DM004C_LOCK_RESOURCE_BUSY)
	{
	    ...
	    {
		sc0e_put(E_SC010D_DB_LOCATION, 0, 3,
	...
	*/
	sc0e_0_put(E_SC0202_SEM_RELEASE, 0);
    ...
    {
	ret_val = E_DB_ERROR;
	error->err_code = E_US0051_DB_SERVER_MISMATCH;
	sc0e_uput( E_US0050_DB_SERVER_INCOMPAT, 0, 2,
             ...
             DMC_CB *dmc )
{
	...
	error->err_code = E_SC010B_SCB_DBCB_LINK;
	...
	*/
	...
	for (;;)
	{
		...
		if (dmc->error.err_code == E_DM0060_TRAN_IN_PROGRESS)
		{
		    ...
		    qef_status = qef_call(QET_ABORT, ( PTR ) qe_ccb);
	...
	if (dbcb->db_ucount == 0)
	{
	    ...
	    (VOID) scd_dbdelete(dbcb, error, dmc, &scf_cb);
	    if (dbop_status)
		dmc->error.err_code = error->err_code;
	    dbcb->db_next->db_prev = dbcb->db_prev;
	    dbcb->db_prev->db_next = dbcb->db_next;
	    dbcb_delete = dbcb;
	}
	...
	if (ret_val = dbop_status)
	{
	    ...
	    sc0e_put(E_SC0122_DB_CLOSE, 0, 2,
		     sizeof(dbcb->db_dbname), (PTR)&dbcb->db_dbname,
		     sizeof(dbcb->db_dbowner), (PTR)&dbcb->db_dbowner,
		     0, (PTR)0,
		     0, (PTR)0,
	...
	{
	    sc0e_0_put(E_SC0201_SEM_WAIT, 0);
	    sc0e_put(E_SC0122_DB_CLOSE, 0, 2,
		     sizeof(dbcb->db_dbname), (PTR)&dbcb->db_dbname,
		     sizeof(dbcb->db_dbowner), (PTR)&dbcb->db_dbowner,
		     0, (PTR)0,
...
scs_dbms_task(SCD_SCB *scb )
{
    ...
    */
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC0238_FAST_COMMIT_EXIT, 0);
	}
	...
	*/
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC037C_REP_QMAN_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC031E_RECOVERY_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC0320_LOGWRITER_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC0322_CHECK_DEAD_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC0324_GROUP_COMMIT_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC0326_FORCE_ABORT_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC0330_CP_TIMER_EXIT, 0);
	}
	break;
	...
	scb->scb_sscb.sscb_cfac = DB_DMF_ID;
        ...
        { 
            sc0e_0_put(dmc->error.err_code, 0);
            sc0e_0_put(E_SC037F_LK_CALLBACK_THREAD_EXIT, 0);
        }
        break; 
	...
	*/
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC0366_WRITE_ALONG_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC0364_READ_AHEAD_EXIT, 0);
	}
	break;
	...
	scb->scb_sscb.sscb_cfac = DB_DMF_ID;
        ...
        { 
            sc0e_0_put(dmc->error.err_code, 0);
            sc0e_0_put(E_SC0372_DEADLOCK_THREAD_EXIT, 0);
        }
        break; 
	...
	scb->scb_sscb.sscb_cfac = DB_DMF_ID;
        ...
        { 
            sc0e_0_put(dmc->error.err_code, 0);
            sc0e_0_put(E_SC0397_DIST_DEADLOCK_THR_EXIT, 0);
        }
        break; 
	...
	scb->scb_sscb.sscb_cfac = DB_DMF_ID;
        ...
        { 
            sc0e_0_put(dmc->error.err_code, 0);
            sc0e_0_put(E_SC0399_GLC_SUPPORT_THR_EXIT, 0);
        }
        break; 
	...
	*/
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC02C1_CSP_MAIN_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC02C3_CSP_MSGMON_EXIT, 0);
	}
	break;
	...
	{
	    sc0e_0_put(dmc->error.err_code, 0);
	    sc0e_0_put(E_SC02C5_CSP_MSGTHR_EXIT, 0);
	}
	break;
	       ...
	       DB_ERROR *error )
{
    ...
    */
	...
	if (rdf_call(RDF_DDB, (PTR)&rdf_cb) != E_DB_OK)
	{
	    ...
	    /* release semaphore */
	    CSv_semaphore(&Sc_main_cb->sc_kdbl.kdb_semaphore);
	    sc0e_0_put(error->err_code = E_US0014_DB_UNAVAILABLE, 0);
	    return(E_DB_ERROR);
	}
	...
	status = qef_call(QED_3SCF_CLUSTER_INFO, ( PTR ) qef_cb);
    ...
    */
	...
	if (rdf_call(RDF_DDB, (PTR)&rdf_cb) != E_DB_OK)
	{
	    ...
	    /* release semaphore */
            CSv_semaphore(&Sc_main_cb->sc_kdbl.kdb_semaphore);
	    sc0e_put(error->err_code = E_SC0395_RDF_DDB_FAILURE, 0, 1, 
		sizeof(scb->scb_sscb.sscb_ics.ics_dbname),
		(PTR)&scb->scb_sscb.sscb_ics.ics_dbname,
                0, (PTR)0, 0, (PTR)0, 0, (PTR)0, 0, (PTR)0, 0, (PTR)0);
	    return(E_DB_ERROR);
	}
    ...
    scb->scb_sscb.sscb_dbcb = dbcb;
    scb->scb_sscb.sscb_ics.ics_opendb_id = (PTR)dbcb->db_ddb_desc.dd_d7_uniq_id;
	...
	/* release semaphore */
	CSv_semaphore(&Sc_main_cb->sc_kdbl.kdb_semaphore);
	sc0e_0_put(error->err_code = E_US000C_INVALID_REAL_USER, 0);
	return(E_DB_ERROR);
    }
    ...
    */
	...
	if (rdf_call(RDF_DDB, (PTR)&rdf_cb) != E_DB_OK)
	{
	    ...
	    /* release semaphore */
	    CSv_semaphore(&Sc_main_cb->sc_kdbl.kdb_semaphore);
	    sc0e_0_put(error->err_code = E_US0041_DBDB_CONFIG_INCON, 0);
	    return(E_DB_ERROR);
	}
    }
    else
    ...
    */
    ...
    {
	sc0e_0_put(error->err_code = E_US0010_NO_SUCH_DB, 0);
	return(E_DB_ERROR);
    }	
    qef_dcb->qer_d2_ldb_info_p = ddb_descp->dd_d4_iidbdb_p;
    status = qef_call(QED_2SCF_TERM_ASSOC, ( PTR ) qef_cb);
    if ( status != E_DB_OK )
    {
	return( E_DB_ERROR );
    }
...
scs_audit_thread( SCD_SCB *scb )
{
    ...
    DB_DB_OWNER	db_owner;
    SCV_LOC	db_loc;
    DMC_CB	*dmc = (DMC_CB *) scb->scb_sscb.sscb_dmccb;
    SXF_RCB	*sxf_rcb = (SXF_RCB *) scb->scb_sscb.sscb_sxccb;
	...
	for (;;)	/* The main audit thread loop */
	{
		...
		*/
		...
		if (dbdb_open &&
		    ...
		    sc0e_0_put(local_err.err_code, 0);
		break;
	    }
    ...
    if (status != E_DB_OK)
	sc0e_0_put(E_SC032A_AUDIT_THREAD_EXIT, 0);
...
scs_check_term_thread( SCD_SCB *scb )
{
    ...
    */
    ...
    if (status != E_DB_OK)
	sc0e_0_put(E_SC0341_TERM_THREAD_EXIT, 0);
	...
	*/
	if(scb->scb_sscb.sscb_is_idle)
	{
		...
		*/
	    ...
	    sc0e_put(I_SC0344_SESSION_DISCONNECT, 0, 3,
...
scs_secaudit_writer_thread( SCD_SCB *scb )
{
    ...
    if (status != E_DB_OK)
	sc0e_0_put(E_SC0361_SEC_WRITER_THREAD_EXIT, 0);
		...
		DB_ERROR *error)
{
    ...
    {
    	sc0e_0_put(dmc->error.err_code, 0);
    	sc0e_0_put(E_SC0365_IOMASTER_OPENDB, 0);
    }
...
scs_dbadduser(SCF_CB	*scf_cb, SCD_SCB *scb )
{
    ...
    */
    ...
    if (notfound)
    {
	scf_cb->scf_error.err_code = E_SC012D_DB_NOT_FOUND;
	sc0e_0_put(E_SC012D_DB_NOT_FOUND, 0);
