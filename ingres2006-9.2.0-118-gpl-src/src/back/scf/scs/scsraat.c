	    ...
	    */
		...
		case RAAT_TABLE_OPEN:
		{
		    ...
		    */
		    ...
		    {
                        status = psq_call(PSQ_SESSOWN, &psq_cb, (char *)NULL);
      ...
      defined(axp_lnx)
		    ...
		    */
			...
			if (owner_given)
		    	...
		    	status = psq_call(PSQ_RESOLVE, &psq_cb, (PTR)NULL);
		    ...
		    */
			...
			scb->scb_sscb.sscb_ics.ics_opendb_id;
		    dmt_cb.dmt_tran_id		=
			((QEF_CB *)scb->scb_sscb.sscb_qescb)->qef_dmt_id;
		    dmt_cb.dmt_id		= dmt_tbl->tbl_id;
		    dmt_cb.dmt_sequence		= 
			((QEF_CB *)scb->scb_sscb.sscb_qescb)->qef_stmt;
...
/* FIX ME !!!!! what should be subtracted from comm_size that
		will equal 4040 ? */
		    	buf_avail = scb->scb_cscb.cscb_comm_size -
			    (2 * Sc_main_cb->sc_gchdr_size) - buf_used;
      ...
      defined(axp_lnx) 
		...
		case RAAT_RECORD_GET:
		{
		    ...
		    */
		    ...
		    status = allocate_big_buffer(scb, buf_size);
		    if (status)
		    {
		       sc0e_0_put(status, 0);
		       err_code = E_SC0372_NO_MESSAGE_AREA;
...
/* FIX ME !!!!! what should be subtracted from comm_size that
      ...
      defined(axp_lnx)
		...
		case RAAT_BLOB_GET:
		{
		    ...
		    */
		    ...
		    status = allocate_big_buffer(scb, reslen+space_needed,
			&err_code);
		    if (status)
		    {
			sc0e_0_put(status, 0);
		    ...
		    adf_scb = (ADF_CB *)scb->scb_sscb.sscb_adscb;
    ...
    defined(axp_lnx)
	    ...
	    */
	    ...
	    {
		status = sc0m_allocate(0,
		    (i4)(sizeof(SCC_GCMSG) + Sc_main_cb->sc_gchdr_size +
...
#endif
    format_msg.gca_b_length =
	scb->scb_cscb.cscb_comm_size;

    _VOID_ IIGCa_call(GCA_FORMAT, (GCA_PARMLIST *)&format_msg,
	GCA_SYNC_FLAG, (PTR)0, -1, &gca_stat);
...
scs_raat_load(SCD_SCB *scb)
{
    ...
    message = (SCC_GCMSG *)&raat_mptr->message;
    message->scg_msize = scb->scb_cscb.cscb_gcp.gca_it_parms.gca_d_length;
    MEcopy((char *)
...
scs_raat_parse(SCD_SCB *scb)
{
    ...
    /* do allocation */
    status = allocate_big_buffer(scb, size);
    if (status)
    {
	sc0e_0_put(status, 0);
	return(status);;
...
/*
** Name: allocate_big_buffer - allocate scb->scb_sscb.sscb_big_buffer 
**
** Description:
**
** Inputs:
...
*/
STATUS
allocate_big_buffer(SCD_SCB *scb, i4 buf_size)
{
    SCS_BIG_BUF *bptr;
    SCC_GCMSG	*msg;
    GCA_FO_PARMS format_msg;
    ...
    if (bptr == NULL)
    {
        status = sc0m_allocate(0, (i4)(Sc_main_cb->sc_gchdr_size +
...
scs_raat_term_msg(SCD_SCB *scb, i4 err_code)
{
    ...
    scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_buffer = raaterr;
    scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length = 
	Sc_main_cb->sc_gchdr_size + sizeof(i4);
    status = IIGCa_call(GCA_FORMAT, 
	(GCA_PARMLIST *)&scb->scb_cscb.cscb_gcp.gca_fo_parms,
	GCA_SYNC_FLAG, (PTR)0, -1, &gc_error);
    if (status)
	return(status);

    *((i4 *) scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_data_area) = err_code;
    scb->scb_cscb.cscb_gcp.gca_sd_parms.gca_association_id =
			    scb->scb_cscb.cscb_assoc_id;
    scb->scb_cscb.cscb_gcp.gca_sd_parms.gca_buffer = raaterr;
