...
# endif /* UNIX */
    ...
    if ( Sc_server_type == SC_RECOVERY_SERVER  )
      {
       gca_status = IIGCa_call(GCA_TERMINATE,
                              (GCA_PARMLIST *)&gca_cb,
                              GCA_SYNC_FLAG,
                              (PTR)0,
                              -1,
                              &err_code);
       if ( gca_status || ( gca_cb.gca_status != E_GC0000_OK ) )
       {
        ule_format( E_SC012B_SESSION_TERMINATE, (CL_SYS_ERR *)0, ULE_LOG, NULL,
                    (char *)0, (i4)0, (i4 *)0, &err_code, 0);
        TRdisplay("%@ RCP GCA session termination, gca_status= %d .\n",
                   gca_status );
       }
      }
...
scd_alloc_scb( SCD_SCB  **scb_ptr, GCA_LS_PARMS  *input_crb, i4  thread_type )
{
    ...
    */
    ...
    mem_needed = (i4)ME_ALIGN_MACRO(mem_needed, sizeof(ALIGN_RESTRICT));
	...
	if (ftc->ftc_data && ftc->ftc_data_length)
	{
	    ...
	    mem_needed = (i4)ME_ALIGN_MACRO(mem_needed,
	...
	** price to pay to allow display of cursor text. */
	mem_needed += Sc_main_cb->sc_acc * ER_MAX_LEN;
	offset_inbuffer = offset_curstext + 
				Sc_main_cb->sc_acc * ER_MAX_LEN;
	    
	/* lint truncation warning if size of ptr > int, but code valid */
	offset_inbuffer = (i4)ME_ALIGN_MACRO(offset_inbuffer,
						  sizeof(ALIGN_RESTRICT));

	offset_rdata	= offset_inbuffer +
			  (crb->gca_size_advise ? crb->gca_size_advise : 256);
	/* lint truncation warning if size of ptr > int, but code valid */
	offset_rdata    = (i4)ME_ALIGN_MACRO(offset_rdata,
						  sizeof(ALIGN_RESTRICT));
	offset_rpdata   = offset_rdata +
			  (i4)Sc_main_cb->sc_gchdr_size +
			  (i4)sizeof(GCA_RE_DATA);
	/* lint truncation warning if size of ptr > int, but code valid */
	offset_rpdata   = (i4)ME_ALIGN_MACRO(offset_rpdata,
						  sizeof(ALIGN_RESTRICT));
	offset_evdata   = offset_rpdata +
			  (i4)Sc_main_cb->sc_gchdr_size +
			  (i4)sizeof(GCA_RP_DATA);
	/* lint truncation warning if size of ptr > int, but code valid */
	offset_evdata   = (i4)ME_ALIGN_MACRO(offset_evdata,
						  sizeof(ALIGN_RESTRICT));
	offset_dbuffer  = offset_evdata +
			  (i4)Sc_main_cb->sc_gchdr_size +
			  (i4)sizeof(SCC_GCEV_MSG);
	/* lint truncation warning if size of ptr > int, but code valid */
	offset_dbuffer  = (i4)ME_ALIGN_MACRO(offset_dbuffer,
						  sizeof(ALIGN_RESTRICT));
    ...
    */
    ...
    /* SCF */
    scb->scb_sscb.sscb_scarea = scb->scb_sscb.sscb_scccb = block + offset_scf;
    scb->scb_sscb.sscb_scscb = (char *)scb;

    /* ADF */
    if (need_facilities & (1 << DB_ADF_ID))
    {
	scb->scb_sscb.sscb_adarea = scb->scb_sscb.sscb_adccb = 
		    scb->scb_sscb.sscb_adscb = block + offset_adf;
    }

    /* DMF */
    if (need_facilities & (1 << DB_DMF_ID))
    {
	scb->scb_sscb.sscb_dmarea = scb->scb_sscb.sscb_dmccb =
						block + offset_dmf;
    ...
    if (need_facilities & (1 << DB_GWF_ID))
    {
	scb->scb_sscb.sscb_gwarea = scb->scb_sscb.sscb_gwccb =
						block + offset_gwf;
    ...
    if (need_facilities & (1 << DB_SXF_ID))
    {
	scb->scb_sscb.sscb_sxarea = scb->scb_sscb.sscb_sxccb =
					    block + offset_sxf;
    ...
    if (need_facilities & (1 << DB_OPF_ID))
    {
	scb->scb_sscb.sscb_oparea = scb->scb_sscb.sscb_opccb =
					    block + offset_opf;
    ...
    if (need_facilities & (1 << DB_PSF_ID))
    {
	scb->scb_sscb.sscb_psarea = scb->scb_sscb.sscb_psccb =
					    block + offset_psf;
	scb->scb_sscb.sscb_psscb = (Sc_main_cb->sc_psbytes > 0)
			? (char *)scb->scb_sscb.sscb_psccb + sizeof(PSQ_CB)
			: NULL;
    }
    ...
    if (need_facilities & (1 << DB_QEF_ID))
    {
	scb->scb_sscb.sscb_qearea = scb->scb_sscb.sscb_qeccb =
					    block + offset_qef;
	scb->scb_sscb.sscb_qescb = (Sc_main_cb->sc_qebytes > 0)
			? (char *)scb->scb_sscb.sscb_qeccb + sizeof(QEF_RCB)
			: NULL;
    }
    ...
    if (need_facilities & (1 << DB_QSF_ID))
    {
	scb->scb_sscb.sscb_qsarea = scb->scb_sscb.sscb_qsccb =
					    block + offset_qsf;
	scb->scb_sscb.sscb_qsscb = (Sc_main_cb->sc_qsbytes > 0)
			? (char *)scb->scb_sscb.sscb_qsccb + sizeof(QSF_RCB)
			: NULL;
    }
    ...
    if (need_facilities & (1 << DB_RDF_ID))
    {
	scb->scb_sscb.sscb_rdarea = scb->scb_sscb.sscb_rdccb =
					    block + offset_rdf;
    ...
    */
	...
	status = sc0m_allocate(0,
	   sizeof(SCC_GCMSG) + Sc_main_cb->sc_gchdr_size+ Sc_main_cb->sc_maxtup,
	...
	MEfill((sizeof(SCC_GCMSG) + Sc_main_cb->sc_gchdr_size+ Sc_main_cb->sc_maxtup) - sizeof(SC0M_OBJECT), 0,
	...
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_buffer =
	    scb->scb_cscb.cscb_rspmsg.scg_marea =
		    (block + offset_rdata);
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length =
				Sc_main_cb->sc_gchdr_size
				    + sizeof(GCA_RE_DATA);
	...
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_buffer =
	    scb->scb_cscb.cscb_rpmmsg.scg_marea =
		    (block + offset_rpdata);
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length =
				Sc_main_cb->sc_gchdr_size
				    + sizeof(GCA_RP_DATA);
	...
	{
	    scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_buffer =
		scb->scb_cscb.cscb_evmsg.scg_marea =
			    (block + offset_evdata);
	    scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length =
			    Sc_main_cb->sc_gchdr_size + sizeof(SCC_GCEV_MSG);
	    status = IIGCa_call(GCA_FORMAT,
		    (GCA_PARMLIST *)&scb->scb_cscb.cscb_gcp.gca_fo_parms,
		    GCA_SYNC_FLAG,
		    (PTR)0,
	...
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_buffer =
	    ...
	    scb->scb_cscb.cscb_dbuffer = 
		    (block + offset_dbuffer);
	scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_b_length = 512;
	status = IIGCa_call(GCA_FORMAT,
		    (GCA_PARMLIST *)&scb->scb_cscb.cscb_gcp.gca_fo_parms,
		    GCA_SYNC_FLAG,
		    (PTR)0,
	...
	scb->scb_cscb.cscb_darea = (char *)
	    scb->scb_cscb.cscb_gcp.gca_fo_parms.gca_data_area;
	scb->scb_cscb.cscb_dsize = 
...
scd_disconnect( SCD_SCB  *scb )
{
    ...
    {
	fparms.gca_buffer = scb->scb_cscb.cscb_inbuffer;
	fparms.gca_b_length = scb->scb_cscb.cscb_comm_size;
	status = IIGCa_call(GCA_FORMAT,
				(GCA_PARMLIST *)&fparms,
				GCA_SYNC_FLAG,
				(PTR)0,
				-1,
				&Sc_main_cb->sc_gcdc_sdparms.gca_status);
	if (status || fparms.gca_status)
	{
	    if (status)
		sc0e_0_put(Sc_main_cb->sc_gcdc_sdparms.gca_status, 0);
	    else
		sc0e_0_put(fparms.gca_status, 0);
	    return;
	}

	ebuf = (GCA_ER_DATA *) fparms.gca_data_area;
