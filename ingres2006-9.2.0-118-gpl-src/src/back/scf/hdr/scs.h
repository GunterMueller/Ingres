...
#define		SCS_USR_TRUSTED		0x0400L /* User has TRUSTED */
    ...
    i4		    ics_dbserv;		/* DU_DATABASE.du_dbservice flags */
...

/*}
**  Name: SCS_COPY - Control variables for copy involving large objects
**
**  Description:
**	This structure contains information used for the processing of
**	large objects during a copy.  This information is maintained
**      in the scb to allow it to be shared over sequencer calls.  The
**      sequencer will may be called multiple times between QEF calls
**      as it expands or couponifies large objects contained in the
**      copy stream.  This are just provides storage for its state
**      information.
**
**  History:
**	24-Feb-1993 (fred)
**	    Created.
*/
typedef struct _SCS_COPY SCS_COPY;
struct _SCS_COPY
{
    i4 	    cp_cur_att;        /* att # on which we're working */
    i4         cp_att_need;       /* Amt needed to finish it */
    i4         cp_in_offset;      /* current offset in inp buffer */
    i4         cp_out_offset;     /* Ditto for output */
    i4         cp_in_lobj;        /* is current column large */
    i4         cp_att_count;      /* Att count in current copy */
};
/*}
...
#define			SCS_ASIGNORE	4	/* Ignore GCA completion */
    ...
    PTR		    sscb_adscb;       /* address of session blk for adf */
    PTR		    sscb_adccb;	      /* call control block */
    PTR             sscb_dmscb;
    PTR		    sscb_dmccb;       /*  (and so forth) */
    PTR             sscb_opscb;
    PTR		    sscb_opccb;
    PTR		    sscb_psscb;
    PTR             sscb_psccb;
...
#define                 SCS_SQPARAMS    16
    ...
    SCS_COPY        sscb_copy;          /* copy control info */
...
FUNC_EXTERN DB_STATUS scs_scopy(SCD_SCB *scb, QEU_COPY *qe_copy );

FUNC_EXTERN VOID scs_copy_error(SCD_SCB *scb, 
				QEF_RCB *qe_ccb,
				i4 copy_done );

FUNC_EXTERN VOID scs_cpsave_err(QEU_COPY *qe_copy, i4 local_errno );

FUNC_EXTERN VOID scs_cpxmit_errors(SCD_SCB *scb, i4  end_stream );

FUNC_EXTERN VOID scs_cpinterrupt(SCD_SCB *scb );

FUNC_EXTERN DB_STATUS scs_cp_cpnify(SCD_SCB *scb, 
				    char *inbuffer,
				    i4  insize, 
				    char *outbuffer,
				    i4  outsize);

FUNC_EXTERN DB_STATUS scs_cp_redeem(SCD_SCB *scb,
				    char *inbuffer,
				    i4  insize, 
				    char *outbuffer, 
				    i4  outsize);
...
FUNC_EXTERN STATUS scs_dcinput(SCD_SCB *scb, GCA_IT_PARMS **it );
...
FUNC_EXTERN bool scs_dcmsg(GCA_IT_PARMS *it );

FUNC_EXTERN DB_STATUS scs_dcinterrupt(SCD_SCB *scb, GCA_IT_PARMS **it );
