...
scu_malloc(SCF_CB  *scf_cb, SCD_SCB *scb )
{
	...
	*/
	...
	status = E_SC0015_BAD_PAGE_COUNT;
	if (scf_cb->scf_scm.scm_in_pages < 0)
	    break;

	status = E_SC0019_BAD_FUNCTION_CODE;
	...
	*/
	...
	{
	    sc0e_put(E_SC0102_MEM_CORRUPT, 0, 0);
	    scf_cb->scf_error.err_code = E_SC0102_MEM_CORRUPT;
	    ret_val = E_DB_FATAL;
	    break;
	}
	...
	if (scf_cb->scf_scm.scm_in_pages
		== scf_cb->scf_scm.scm_out_pages)
	{
	...
	{
	    scf_cb->scf_error.err_code = E_SC0005_LESS_THAN_REQUESTED;
	    ret_val = E_DB_WARN;
	}
	if (delete)
	{
	...
	ule_format(status, 0, ULE_LOG, NULL, 0, 0, 0, &error, 0);
    }
    return(ret_val);
}

...
scu_mfree(SCF_CB  *scf_cb, SCD_SCB *scb )
{
    ...
    */
	...
	ret_val = E_DB_FATAL;	    /* therefore, errors are real bad */
	...
	status = sc0m_free_pages(scf_cb->scf_scm.scm_in_pages,
				 &scf_cb->scf_scm.scm_addr);
	      ...
	      i4  *deleted )
{
	...
	*/
    ...
    {
	if (cur_mcb == Sc_main_cb->sc_mcb_list)
	    ret_val = E_SC0006_NOT_ALLOCATED;
	else if (cur_mcb->mcb_page_count < scf_cb->scf_scm.scm_in_pages)
	    ret_val = E_SC0020_WRONG_PAGE_COUNT;
	else
	    ret_val = E_SC0021_BAD_ADDRESS;
    }
	...
	*mcb = cur_mcb;
    ...
    {
	if (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_MEMLOG, 0, 0))
	...
	cur_mcb->mcb_start_address = (PTR) ((char *) cur_mcb->mcb_start_address
					+ (scf_cb->scf_scm.scm_in_pages *
						SCU_MPAGESIZE));
	if (ult_check_macro(&Sc_main_cb->sc_trace_vector, SCT_MEMLOG, 0, 0))
	    TRdisplay("MFREE: losing %x pages yeilds -- pages: %x, addr: %p\n",
		    scf_cb->scf_scm.scm_in_pages,
	...
	*/
	...
	ret_val = sc0m_allocate(0, (i4)sizeof(SCS_MCB), MCB_ID,
