...
i4		function )
{
    ...
    i4		rowno, rowstat;
    i4 		qmode = dsh->dsh_qp_ptr->qp_qmode;
    i4		qp_status = dsh->dsh_qp_ptr->qp_status;
    i4		rowlim = action->qhd_obj.qhd_qep.ahd_firstn;
    i4		error;
	...
	** buffer for qea_fetch to materialize the result row. */
	...
	sptr = (QEE_RSET_SPOOL *)dsh->dsh_cbs[action->qhd_obj.qhd_qep.
						ahd_scroll->ahd_rssplix];
	...
	** and this was the "n"th. */
	...
	if ((!scroll && rowlim && rowlim <= ++(dsh->dsh_srowcount) ||
...
{
    QEF_CB		*qef_cb = dsh->dsh_qefcb;
    QEE_RSET_SPOOL	*sptr = (QEE_RSET_SPOOL *)dsh->dsh_cbs[action->
				qhd_obj.qhd_qep.ahd_scroll->ahd_rssplix];
	...
	** and add in line no. */
	...
	temptid *= 512;			/* shift to page no location */
	temptid += (position-1) % sptr->sp_rowspp;
					/* add in line no. */
    }
...

{
    ...
    ADF_CB	*adf_cb = dsh->dsh_adf_cb;
    DMT_ATTR_ENTRY *attp = scrollp->ahd_rskattr;
    DB_STATUS	status;
    i4		i, off1, *off2;
...

{
    ...
    if (scroll)
    {
	rsptr = (QEE_RSET_SPOOL *)dsh->dsh_cbs[action->qhd_obj.qhd_qep.
					ahd_scroll->ahd_rssplix];
...

{
	...
	dmtcb->dmt_attr_array.ptr_in_count = 1;
	...
	*/
	...
	if (sptr->sp_rowspp > 511)
	    sptr->sp_rowspp = 511;		/* max of 511 rows (line
						** ID capacity) */
