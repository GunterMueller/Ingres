...
static READONLY struct 
  {
	...
	{   /* iiQEF_make_udt_defaults */
	    {{ 'i', 'i', 'q', 'e', 'f', '_', 'm', 'a', 'k', 'e', '_', 'u',
 	      'd', 't', '_', 'd', 'e', 'f', 'a', 'u', 'l', 't', 's', ' ' ,
	      ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '}},
	    {{ '$', 'i', 'n', 'g', 'r', 'e', 's', ' ', ' ', ' ', ' ', ' ',
 	       ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ' ,
	      ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '}},
	...
	*/
	for (proc_case=NONEXISTENT_PROC, i=0; i<PROC_CNT; i++)
	{
		...
		*/
		...
		for (from = (char *) &qef_procs[i].dbp_nam,
		     last = from + sizeof(qef_procs[i].dbp_nam),
		     to = (char *) dbpname_p;
		...
		for (from = (char *) &qef_procs[i].dbp_own,
		     last = from + sizeof(qef_procs[i].dbp_own),
		     to = (char *) ownname_p;
	    ...
	    {
		dbpname_p = (DB_DBP_NAME *)&qef_procs[i].dbp_nam;
		ownname_p = (DB_OWN_NAME *)&qef_procs[i].dbp_own;
	    }
	...
	*/
	...
	for (i=0; i<exp_parm[proc_case].exp_num; i++)
	{
		...
		*/
		...
		for (from = (char *) &exp_parm[proc_case].p[i].name,
		     last = from + sizeof(exp_parm[proc_case].p[i].name),
		     to = (char *) attname_p;
	    ...
	    {
		attname_p = (DB_ATT_NAME *)&exp_parm[proc_case].p[i].name;
	    }
...
*/
    ...
    MEmove(sizeof(DU_CUR_DBCMPTLVL) - 1, DU_CUR_DBCMPTLVL, ' ',
	    sizeof(database.du_dbcmptlvl), database.du_dbcmptlvl);
...
*/
	...
	*/
	...
	{    
	    ...
	    if ((du_extend_type & (DU_EXT_WORK | DU_EXT_AWORK)) &&
		(dbextend.du_status & (DU_EXT_WORK | DU_EXT_AWORK)))
	    {
		if (MEcmp((PTR)dbextend.du_lname,
                    (PTR)&dmm_cb.dmm_location_name,
                    dbextend.du_l_length) == 0) 
...
QEE_DSH			*dsh )
{
    ...
    */
	...
	{    
	    ...
	    /* jump out of the loop if we've found a matching location */
	    if (MEcmp( ( PTR ) dbextend.du_lname,
			( PTR ) &dmm_cb.dmm_location_name,
			dbextend.du_l_length) == 0)
	    {
...
**	procedure return value.  Note that dsc_dbcmptlvl is cohersed to an i4
**	to return, and must be cohersed back to a character array 4 bytes long.
**
** Inputs:
**    qef_rcb				QEF Reqeust Block
**	.qef_cb				    QEF Session Control Block ptr
			...
			QEE_DSH		*dsh)
{
	...
	*/
	...
	STRUCT_ASSIGN_MACRO(db_parm[4].dbp_dbv, row_dbv);
	MEmove(row_dbv.db_length,
	    (char *)dsh->dsh_row[db_parm[4].dbp_rowno] + db_parm[4].dbp_offset,
	    ' ', sizeof(dmm_cb.dmm_cmptlvl), dmm_cb.dmm_cmptlvl);
    }
...
QEE_DSH			*dsh )
{
	...
	*/
	...
	for ( ; ; )
	{
	    rqeu->qeu_qual = (PTR) UDTsieve;
	    rqeu->qeu_qarg = (PTR) NULL;
...
**
** Inputs:
**      qual_loc		This is NULL.  No qualification args needed.
**      tuple		The current tuple being considered
**
** Outputs:
**      None.
**
**      Returns:
**          DMR_F_RETURN        Match
**          DMR_F_NEXT          Continue searching
**          DMR_F_ABORT         Fatal error
**      Exceptions:
**          none
**
** Side Effects:
...
)
{
    ...
    {
        return(DMR_F_RETURN);                       /* Qualified! */
    }

    return(DMR_F_NEXT);                             /* Unqualified.... */
}

/*{
