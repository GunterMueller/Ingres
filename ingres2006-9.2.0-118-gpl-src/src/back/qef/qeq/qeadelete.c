...
i4		state )
{
	...
	for (;;)
	{
	    /* Skip all this stuff if we're returning from a BEFORE trigger. */
		...
		*/
		...
		{
		    status = qeq_part_open(qef_rcb, action, dsh, NULL, 0, 
...
i4		state )
{
	...
	for (;;)
	{
		...
		*/
	    ...
	    /* If resuming after a BEFORE trigger, save DMR fields
	    ** again, to be restored after the DMR_DELETE. */
	    if (state == DSH_CT_CONTINUE3)
	    {
		old_flags_mask = dmr_cb->dmr_flags_mask;
		old_dmr_tid = dmr_cb->dmr_tid;
