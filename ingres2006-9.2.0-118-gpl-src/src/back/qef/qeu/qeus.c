...
**		<dmu_attr_array> are of type DMU_ATTR_ENTRY
**		attr_name
**		attr_type
**		attr_size
**		attr_precision
...

	...
	*/
	...
	if ( (opcode == DMU_DESTROY_TABLE) || (dmu_atbl_dropcol) )
	{
	    ...
	    */
	    if (dmt_tbl_entry.tbl_status_mask & DMT_GATEWAY)
	    {
		i4	    rem_err;
	...
	else if (opcode != DMU_DESTROY_TABLE)	
	{
	    ...
	    */
	    ...
	    /* Setting rebuild_pindex if we're operating on an index is bad Karma */
	    if ( opcode != DMU_RELOCATE_TABLE && dmu_ptr->dmu_tbl_id.db_tab_index <= 0 )
	    {
	        chr = * ((DMU_CHAR_ENTRY *) dmu_cb.dmu_char_array.data_address);
		if (chr.char_id == DMU_STRUCTURE
		  || chr.char_id == DMU_TRUNCATE
		  || (chr.char_id == DMU_VERIFY && chr.char_value != DMU_V_DEBUG) )
		{
		    rebuild_pindex = TRUE;
		}
	    }
...
case E_DM006A_TRAN_ACCESS_CONFLICT:
		...
		*/
		...
		{
		    /*
		    ** now recreate the persistent indices using the DMU_CBs we
		    ** built earlier.
		    */
		    ...
		    if ( numberOfPersistentIndices > 0 )
		    {
		        ...
		        status = createIndices( qef_cb, &qef_rcb, &ulm,
			    persistentIndexDMU_CBs, numberOfPersistentIndices,
			    &newPersistentIndexIDs );
		        if ( status != E_DB_OK )
			{
			    err = qef_rcb.error.err_code;
			    goto exit;
...
i4	    *error)
{
    ...
    if (gw < DMGW_NONE  ||  gw > DMGW_MAX_GW_ID)
    {
...

{
    ...
    DMU_ATTR_ENTRY **dmu_attp_base;	/* Base of dmu pointer array */
    DMU_ATTR_ENTRY **dmu_attpp;		/* A dmu pointer pointer */
    DMU_ATTR_ENTRY *dmu_att_base;	/* Base of dmu attr entries */
    DMU_ATTR_ENTRY *dmu_attp;		/* A dmu entry pointer */
    i4 i;
    ...
    */
    -- natts;
    ulm->ulm_psize = natts * (sizeof(DMU_ATTR_ENTRY **) + sizeof(DMU_ATTR_ENTRY));
    status = qec_malloc(ulm);
    if (DB_FAILURE_MACRO(status))
    {
	*err_code = ulm->ulm_error.err_code;
    ...
    dmu_attp_base = (DMU_ATTR_ENTRY **) ulm->ulm_pptr;
    dmucb->dmu_attr_array.ptr_address = (PTR) dmu_attp_base;
    dmu_att_base = (DMU_ATTR_ENTRY *) ((PTR) dmu_attp_base + natts * sizeof(DMU_ATTR_ENTRY **));
...

/*{
** Name: createIndices - pump an array of index creating DMU_CBs at DMF
**
** Description:
**	This routine is passed an array of DMU_CBs, each suitable for
**	creating an index.  This routine calls DMF to create the indices.
**
** Inputs:
**	qef_rcb		request control block
...
)
{
    ...
    DMU_CB		*dmu_cb;
    DB_TAB_ID		*indexIDs;
    i4		val_1, val_2, par_index = 1;

    /* allocate an array of index IDs */
    GET_MEMORY( numberOfIndices * sizeof( DB_TAB_ID ), &indexIDs );
    *newIndexIDs = indexIDs;

    if (ult_check_macro(&qef_cb->qef_trace, QEF_TRACE_NO_PAR_INDEX_70,
			&val_1, &val_2))
	par_index = 0;
	
    if ((numberOfIndices > 1) && (par_index))
    {
	dmu_cb = indexDMU_CBs;
#ifdef xxx
/* FIX ME skip createIndices, already done on modify call */
	status = dmf_call( DMU_PINDEX_TABLE, dmu_cb );
#endif
        if ( status != E_DB_OK )
        {   QEF_ERROR( dmu_cb->error.err_code ); return( status );  }
 
        for ( i = 0, dmu_cb = indexDMU_CBs;
	      i < numberOfIndices;
	      i++, dmu_cb = ( DMU_CB * ) dmu_cb->q_next )
          MEcopy( ( PTR ) &dmu_cb->dmu_idx_id, sizeof( DB_TAB_ID ),
                  ( PTR ) &indexIDs[ i ] );
    }
    else {
        /* create the indices and remember their IDs */
...
#ifdef xxx
/* FIX ME skip createIndices, already done on modify call */
            status = dmf_call( DMU_INDEX_TABLE, dmu_cb );
#endif
            if ( status != E_DB_OK )
            {   QEF_ERROR( dmu_cb->error.err_code ); return( status );	}
...
)
{
    ...
    DMU_ATTR_ENTRY	**attributePointers =
			( DMU_ATTR_ENTRY ** ) dm_ptr->ptr_address;
    DMU_ATTR_ENTRY	*attribute;
