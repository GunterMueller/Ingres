...
QEUQ_CB		*qeuq_cb)
{
	...
	*/
	...
	schemaqeu.qeu_qual = schemaqeu.qeu_f_qual = 0;
	schemaqeu.qeu_qarg = schemaqeu.qeu_f_qarg = 0;
	...
	*/
	...
	dbpqeu.qeu_qarg = (PTR) &schema_to_drop->db_schname;

	dbpqeu.qeu_qual = ( PTR ) qeu_dbp_qual_func;
	...
	evqeu.qeu_qual = ( PTR ) qeu_ev_qual_func;
	...
	synqeu.qeu_qual = ( PTR ) qeu_syn_qual_func;
	...
	idxqeu.qeu_qual = ( PTR ) qeu_idx_qual_func;
...
**
** Input:
**	schema_name	    name of the schema being dropped
**	relidx_tuple	    current IIREL_IDX tuple
**	    relowner	    schema name (in 6.5, same as relation owner name)
**
** Output
**	none
**
** Returns:
**	DMR_F_NEXT if schema names do not match, DMR_F_RETURN otherwise.
*/
static i4
qeu_idx_qual_func(
    DB_SCHEMA_NAME	*schema_name,
...
{
    return(
	(MEcmp((PTR) &relidx_tuple->relowner, (PTR) schema_name,
	    DB_MAXNAME)) ? DMR_F_NEXT : DMR_F_RETURN);
}

/*
...
** Input:
**	schema_name	    name of the schema being dropped
**	dbp_tuple	    current IIPROCEDURE tuple
**	    db_owner	    schema name (in 6.5, same as dbproc owner name)
**
** Output
**	none
**
...
{
    return(
	(MEcmp((PTR) &dbp_tuple->db_owner, (PTR) schema_name,
	    DB_MAXNAME)) ? DMR_F_NEXT : DMR_F_RETURN);
}

/*
...
** Input:
**	schema_name	    name of the schema being dropped
**	syn_tuple	    current IISYNONYM tuple
**	    db_synowner	    schema name (in 6.5, same as synonym owner name)
**
** Output
**	none
**
...
{
    return(
	(MEcmp((PTR) &syn_tuple->db_synowner, (PTR) schema_name,
	    DB_MAXNAME)) ? DMR_F_NEXT : DMR_F_RETURN);
}

/*
...
** Input:
**	schema_name	    name of the schema being dropped
**	ev_tuple	    current IIEVENT tuple
**	    dbe_owner 	    schema name (in 6.5, same as dbevent owner name)
**
** Output
**	none
**
...
{
    return(
	(MEcmp((PTR) &ev_tuple->dbe_owner, (PTR) schema_name,
	    DB_MAXNAME)) ? DMR_F_NEXT : DMR_F_RETURN);
}
