...
**
**  NOTE 2: If select type is SEL_ALL, either there are no inner rows or all
...
**
**  NOTE 3: If the select type is SEL_ZERO_ONE, either no rows or exactly
...
i4		function)
{
    ...
    ** of the query tree do we continue executing the function. */
    ...
    sel_type = node->node_qen.qen_sejoin.sejn_seltype;
    ...
    ** at this node if sejn_hcreate is FALSE */
	...
	if (qen_status->node_status != QEN0_INITIAL)
	{
		...
		{	
		    ...
		    if (status)
		    {
			rcb->error.err_code = dmt_cb->error.err_code;
			return(status);
		    }
	...
	** file and a few other things */
	...
	if (qen_status->node_status == QEN0_INITIAL)  
	{
	    ...
	    ** this node or a shared hold node below */
		...
		dmt_cb->dmt_flags_mask	= 0;
		if ((status = dmf_call(DMT_OPEN, dmt_cb)) != E_DB_OK)
		{
		    dsh->dsh_error.err_code = dmt_cb->error.err_code;
		    break;  /* off to return DMF error */
		}
	...
	{				
		...
		*/
		...
		if (status != E_DB_OK)
		{
			...
			if (i_qual == TRUE || sel_type == SEL_ALL)
			{
	    ...
	    ** SEL_ZERO_ONE */
	    if (sel_type == SEL_ZERO_ONE && i_fetched == TRUE)
	    {
		qen_status->node_status = QEN7_FAILED;
	    ...
	    */
	    ...
	    {
		i_qual = TRUE;
		if (sel_type == SEL_ALL || sel_type == SEL_ZERO_ONE)
		{
