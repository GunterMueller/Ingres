    ...
    i4		*err_code)
{
    ...
    MEfill(SA_MAX_EVENTLEN, NULL, evtype);
    MEfill(SA_MAX_MESSTXT, NULL, message);
    MEfill(SA_MAX_PRIVLEN, NULL, user_priv);
    MEfill(SA_MAX_PRIVLEN, NULL, object_priv);
    MEfill(SA_MAX_ACCLEN, NULL, acc_type);
	...
	*/
	if (audit_rec->sxf_detail_txt)
	{
		...
		detail_text);
	    detail_text[audit_rec->sxf_detail_len] = NULL;
