...
if not exist %ING_ROOT% echo Directory %ING_ROOT% does not exist& goto TRUNK_LOOP
...
:XERCES_LOOP
SET /P XERCESCROOT=Root location of Xerces source (default is %ING_ROOT%\xerces-c_2_5_0-windows_nt-msvc_60):
if "%XERCESCROOT%"=="" SET XERCESCROOT=%ING_ROOT%\xerces-c_2_5_0-windows_nt-msvc_60
if not exist %XERCESCROOT% echo %XERCESCROOT% does not exist& goto XERCES_LOOP
SET XERCESLOC=%XERCESCROOT%\lib

:KERBEROS_LOOP
SET /P KRB5HDR=Location of Kerberos header files (default is %ING_ROOT%\Kerberos5:
if "%KRB5HDR%"=="" SET KRB5HDR=%ING_ROOT%\Kerberos5
if not exist %KRB5HDR% echo %KRB5HDR% does not exist& goto KERBEROS_LOOP
