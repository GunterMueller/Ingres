...
case $vers in
   rs4_us5|ris_u64|i64_aix)
            echo "install"  > /tmp/packlist
            ;;
   *)       ls -1 install/* > /tmp/packlist
            ;;
esac
...
echo "Creating $fname ..."
case $vers in
   hp8_us5|ris_us5|rs4_us5|axp_osf|sqs_ptx|sos_us5|sgi_us5|hpb_us5|\
   ris_u64|i64_aix|hp2_us5|*_lnx|int_rpl|i64_hpu|mg5_osx)
	    tar -cvf $tarfile `cat /tmp/packlist` ||
            {
		echo "Tar command failed."
		exit 1
            } 
	    ;;
   *)       tar cvf $tarfile -I /tmp/packlist ||
            {
	       echo "Tar command failed."
	       exit 1
            } 
