...
#
[ "$config" = "rs4_us5" -o "$config" = "ris_u64" ] &&
mkexplist()
{
...
mkexplist()
{
    ls -1 *.o | xargs -n50 /usr/ccs/bin/nm -p |
      awk ' $2=="D"||$2=="B" { print $3 } ' > expsym_list
}
#
#
...
then
    echo ""
    echo " Building lib(PROG0PRFX)compat$BITSFX.1.$SLSFX "
    echo ""
    ...
    rm -f $INGLIB/lib(PROG0PRFX)compat.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG0PRFX)compat.1.$SLSFX \*.o $shlink_opts 
    case $config in
    dgi_us5 | dg8_us5 | nc4_us5 | sos_us5 | pym_us5)
...
$shlink_opts \
   ...
   mg5_osx)
	$shlink_cmd -dynamic -o $INGLIB/lib(PROG0PRFX)compat.1.$SLSFX *.o \
        $shlink_opts -framework CoreServices -framework DirectoryService \
    ...
    *)
    ...
    if [ "$config" != "rs4_us5" ] ; then
	  ...
	  exit 4
       	...
       	[ "$config" = "ris_us5" -o "$config" = "ris_u64" ] \
	  && archive_lib lib(PROG0PRFX)compat.1
...
then
    echo ""
    echo " Building lib(PROG0PRFX)q$BITSFX.1.$SLSFX "
    echo ""
...
if [ "$ming_bld" ] ; then
    ...
    [ "$config" = "ris_us5" -o "$config" = "rs4_us5" -o \
      "$config" = "ris_u64" -o "$config" = "i64_aix" ] && mkexplist
    rm -f $INGLIB/lib(PROG0PRFX)q.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG0PRFX)q.1.$SLSFX \*.o $shlink_opts 
    case $config in
    axp_osf)
    ...
    pym_us5)
        $shlink_cmd -o $INGLIB/lib(PROG0PRFX)q.1.$SLSFX *.o \
        $INGLIB/libabfrt.a -hlib(PROG0PRFX)q.1.$SLSFX \
        $shlink_opts
        ;;
    ts2_us5)
	$shlink_cmd -o $INGLIB/lib(PROG0PRFX)q.1.$SLSFX *.o \
   ...
   mg5_osx)
	...
	-L$INGLIB -l(PROG0PRFX)compat.1 \
        $shlink_opts -install_name lib(PROG0PRFX)q.1.$SLSFX
        ;;
    *)
	$shlink_cmd -o $INGLIB/lib(PROG0PRFX)q.1.$SLSFX *.o \
	$INGLIB/lib(PROG0PRFX)compat.1.$SLSFX \
    ...
    if [ "$config" != "rs4_us5" ] ; then
      if [ $? != 0 -o ! -r $INGLIB/lib(PROG0PRFX)q.1.$SLSFX ]
        then
          echo $0 Failed,  aborting !!
          exit 4
        else
...
then
    echo ""
    echo " Building lib(PROG0PRFX)frame$BITSFX.1.$SLSFX "
    echo ""
    [ -r $INGLIB/lib(PROG0PRFX)compat.1.$SLSFX -a -r $INGLIB/lib(PROG0PRFX)q.1.$SLSFX ] ||
      { echo Need to build lib(PROG0PRFX)compat.1.$SLSFX, and lib(PROG0PRFX)q.1.$SLSFX first, exiting ...;  exit 5 ;}
    ...
    [ "$config" = "ris_us5" -o "$config" = "rs4_us5" -o \
      "$config" = "ris_u64" -o "$config" = "i64_aix" ] && mkexplist
    rm -f $INGLIB/lib(PROG0PRFX)frame.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG0PRFX)frame.1.$SLSFX \*.o $shlink_opts 
    case $config in
    i64_hpu|\
    hp2_us5|\
    hpb_us5|\
    ...
    pym_us5)
        $shlink_cmd -o $INGLIB/lib(PROG0PRFX)frame.1.$SLSFX *.o  \
        -hlib(PROG0PRFX)frame.1.$SLSFX $shlink_opts
        ;;
    ts2_us5)
	$shlink_cmd -o $INGLIB/lib(PROG0PRFX)frame.1.$SLSFX *.o  \
...
then
    echo ""
    echo " Building lib(PROG0PRFX)interp$BITSFX.1.$SLSFX "
    echo ""
    [ -r $INGLIB/lib(PROG0PRFX)compat.1.$SLSFX -a -r $INGLIB/lib(PROG0PRFX)q.1.$SLSFX -a  \
      -r $INGLIB/lib(PROG0PRFX)frame.1.$SLSFX ] ||
     { echo Need to build lib(PROG0PRFX)frame.1.$SLSFX etc first, exiting ...;  exit 5 ;}
    ...
    [ "$config" = "ris_us5" -o "$config" = "rs4_us5" -o \
      "$config" = "ris_u64" -o "$config" = "i64_aix" ] && mkexplist
    rm -f $INGLIB/lib(PROG0PRFX)interp.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG0PRFX)interp.1.$SLSFX \*.o $shlink_opts
    case $config in
    i64_hpu|\
    hp2_us5|\
    hpb_us5|\
    ...
    pym_us5)
        $shlink_cmd -o $INGLIB/lib(PROG0PRFX)interp.1.$SLSFX *.o \
        -hlib(PROG0PRFX)interp.1.$SLSFX $shlink_opts
        ;;
    ts2_us5)
	$shlink_cmd -o $INGLIB/lib(PROG0PRFX)interp.1.$SLSFX *.o \
    ...
    *)
	...
	[ "$config" = "ris_us5" -o "$config" = "ri4_u64" ] \
	  && archive_lib lib(PROG0PRFX)interp.1
        chmod 755 $INGLIB/lib(PROG0PRFX)interp.1.$SLSFX
        ls -l $INGLIB/lib(PROG0PRFX)interp.1.$SLSFX
      fi
...
then
    ...
    echo ""
    echo " Building lib(PROG1PRFX)api$BITSFX.1.$SLSFX "
    echo ""

    # check whether libcompat.1.$SLSFX exists, since api needs it.
    ...
    [ "$config" = "ris_us5" -o "$config" = "rs4_us5" -o \
      "$config" = "ris_u64" -o "$config" = "i64_aix" ] && mkexplist
    rm -f $INGLIB/lib(PROG1PRFX)api.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG1PRFX)api.1.$SLSFX \*.o $shlink_opts 
    case $config in
    axp_osf)
    ...
    pym_us5)
	$shlink_cmd -o $INGLIB/lib(PROG1PRFX)api.1.$SLSFX *.o \
	-hlib(PROG1PRFX)api.1.$SLSFX $shlink_opts
	;;
    ts2_us5)
	$shlink_cmd -o $INGLIB/lib(PROG1PRFX)api.1.$SLSFX *.o \
...
then
    echo ""
    echo " Building oiice$BITSFX.1.$SLSFX for Netscape "
    echo ""
 
    # check whether libcompat.1.$SLSFX exists, since oiice needs it.
    # If it exists, continue processing. Otherwise exit.
    [ -r $INGLIB/lib(PROG0PRFX)compat.1.$SLSFX ] ||
        { echo Need to build lib(PROG0PRFX)compat.1.$SLSFX first, exiting ...;  exit 5 ;}
    ...
    echo $shlink_cmd -o $INGLIB/oiicens.1.$SLSFX \*.o $shlink_opts
    case $config in
    axp_osf)
...
#	directory.
    ...
    *)
    ...
    if [ $? != 0 -o ! -r $INGLIB/oiicens.1.$SLSFX ]
    then
        echo $0 Failed,  aborting !!
        exit 4
    else
        [ "$config" = "ris_us5" ] && archive_lib oiicens.1
        chmod 755 $INGLIB/oiicens.1.$SLSFX
        ls -l $INGLIB/oiicens.1.$SLSFX
        echo Cleaning up ...
        cp $INGLIB/oiicens.1.$SLSFX $ING_BUILD/ice/bin/$LPHB/ice.1.$SLSFX
        mv $INGLIB/oiicens.1.$SLSFX $ING_BUILD/ice/bin/netscape/$LPHB/oiice.1.$SLSFX
        echo "Moving $INGLIB/oiicens.1.$SLSFX to $ING_BUILD/ice/bin/netscape/$LPHB/oiice.1.$SLSFX"
        cd $workdir; 
        if [ "X$IIOPTIM" = "X" ] ; then
            rm -rf oiicens
        fi
...
then
    echo ""
    echo " Building oiice.1.$SLSFX for Apache "
    echo ""
    ...
    echo $shlink_cmd -o $INGLIB/oiiceap.1.$SLSFX \*.o $shlink_opts
    case $config in
    axp_osf)
...
#	directory.
    ...
    *)
    ...
    if [ $? != 0 -o ! -r $INGLIB/oiiceap.1.$SLSFX ]
    then
        echo $0 Failed,  aborting !!
        exit 4
    else
        [ "$config" = "ris_us5" ] && archive_lib oiiceap.1
        chmod 755 $INGLIB/oiiceap.1.$SLSFX
        ls -l $INGLIB/oiiceap.1.$SLSFX
        echo Cleaning up ...
        mv $INGLIB/oiiceap.1.$SLSFX $ING_BUILD/ice/bin/apache/$LPHB/oiice.1.$SLSFX
        echo "Moving $INGLIB/oiiceap.1.$SLSFX to $ING_BUILD/ice/bin/apache/$LPHB/oiice.1.$SLSFX"
        cd $workdir; 
        if [ "X$IIOPTIM" = "X" ] ; then
            rm -rf oiiceap
        fi
    ...
    *)
    ...
    if [ "$config" != "rs4_us5" ] ; then
      ...
      then
        ...
        [ "$config" = "ris_us5" ] && archive_lib oiddi.1
        chmod 755 $INGLIB/${SLDDIHEAD}oiddi.$SLDDITAIL
        ls -l $INGLIB/${SLDDIHEAD}oiddi.$SLDDITAIL
      fi
    fi
...
then
    echo ""
    echo " Building libneworder.${SLSFX}.2.0 "
    echo ""
...
defs
    ...
    dr6_us5)
      ...
      *_lnx|int_rpl)
	$shlink_cmd -o $ING_BUILD/files/dynamic/${LPHB}/libplay_NewOrder.$SLSFX.2.0 *.o \
	$shlink_opts -soname=libplay_NewOrder.$SLSFX.2.0
	;;
    dgi_us5 | dg8_us5)
        $shlink_cmd -o $ING_BUILD/files/dynamic/${LPHB}/libplay_NewOrder.$SLSFX.2.0 *.o \
        $shlink_opts -z defs -l(PROG0PRFX)compat.1 -l(PROG0PRFX)q.1 -lc -lthread -lsocket \
    ...
    *)
    ...
    then
        ...
        [ "$config" = "ris_us5" ] && archive_lib neworder.1
...
then
    ...
    echo ""
    echo " Building lib(PROG1PRFX)odbcdriver$BITSFX.1.$SLSFX "
    echo ""

    # check whether libcompat.1.$SLSFX exists, since odbcdriver needs it.
    # If it exists, continue processing. Otherwise exit.
    [ -r $INGLIB/libcompat.1.$SLSFX ] ||
      { echo Need to build libcompat.1.$SLSFX first, exiting ...;  exit 5 ;}
    # check whether libiiapi.1.$SLSFX exists, since odbcdriver needs it.
    # If it exists, continue processing. Otherwise exit.
    [ -r $INGLIB/lib(PROG1PRFX)api.1.$SLSFX ] ||
      { echo Need to build lib(PROG1PRFX)api.1.$SLSFX first, exiting ...;  exit 5 ;}
    ...
    [ "$config" = "ris_us5" -o "$config" = "rs4_us5" ] && mkexplist
    rm -f $INGLIB/lib(PROG1PRFX)odbcdriver$RO.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG1PRFX)odbcdriver$RO.1.$SLSFX \*.o $shlink_opts
    case $config in
    axp_osf)
...
then
    ...
    echo ""
    echo " Building lib(PROG1PRFX)odbc.1.$SLSFX "
    echo ""

    # check whether libcompat.1.$SLSFX exists, since odbc cli needs it.
    # If it exists, continue processing. Otherwise exit.
    [ -r $INGLIB/libcompat.1.$SLSFX ] ||
     { echo Need to build libcompat.1.$SLSFX first, exiting ...;  exit 5 ;}

    # check whether libiiapi.1.$SLSFX exists, since odbc cli needs it.
    # If it exists, continue processing. Otherwise exit.
    [ -r $INGLIB/lib(PROG1PRFX)api.1.$SLSFX ] ||
      { echo Need to build lib(PROG1PRFX)api.1.$SLSFX first, exiting ...;  exit 5 ;} 
    # Create a temporary working directory. cd to the dir. Exit, if it fails.
    ...
    [ "$config" = "ris_us5" -o "$config" = "rs4_us5" ] && mkexplist
    rm -f $INGLIB/lib(PROG1PRFX)odbc.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG1PRFX)odbc.1.$SLSFX \*.o $shlink_opts
    case $config in
    axp_osf)
...
then
    ...
    echo ""
    echo " Building lib(PROG1PRFX)odbctrace.1.$SLSFX "
    echo ""

    # check whether libcompat.1.$SLSFX exists, since odbc trace needs it.
    # If it exists, continue processing. Otherwise exit.
    [ -r $INGLIB/libcompat.1.$SLSFX ] ||
     { echo Need to build libcompat.1.$SLSFX first, exiting ...;  exit 5 ;}
    ...
    [ "$config" = "ris_us5" -o "$config" = "rs4_us5" ] && mkexplist
    rm -f $INGLIB/lib(PROG1PRFX)odbctrace.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG1PRFX)odbctrace.1.$SLSFX \*.o $shlink_opts
    case $config in
    axp_osf)
...
then
    echo ""
    echo " Building lib(PROG0PRFX)oiutil$BITSFX.1.$SLSFX "
    echo ""
    ...
    [ "$config" = "ris_us5" -o "$config" = "rs4_us5" -o \
      "$config" = "ris_u64" -o "$config" = "i64_aix" ] && mkexplist
    rm -f $INGLIB/lib(PROG0PRFX)oiutil.1.$SLSFX
    echo $shlink_cmd -o $INGLIB/lib(PROG0PRFX)oiutil.1.$SLSFX \*.o $shlink_opts 
    case $config in
    axp_osf)
    ...
    pym_us5)
	$shlink_cmd -o $INGLIB/lib(PROG0PRFX)oiutil.1.$SLSFX *.o \
	-hlib(PROG0PRFX)oiutil.1.$SLSFX $shlink_opts     \
        -L$INGLIB -l(PROG0PRFX)compat.1 -l(PROG0PRFX)q.1 \
                  -l(PROG0PRFX)frame.1 -lgcf -lutil -linstall
	;;
    ts2_us5)
    ...
    *)
	...
	[ "$config" = "ris_us5"  -o "$config" = "ris_u64" ]  \
	  && archive_lib lib(PROG0PRFX)oiutil.1
        chmod 755 $INGLIB/lib(PROG0PRFX)oiutil.1.$SLSFX
        ls -l $INGLIB/lib(PROG0PRFX)oiutil.1.$SLSFX
      fi
    fi
