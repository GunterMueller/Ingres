...
then
	. readvers
	conf=$config
	[ "$conf_ADD_ON32" ] && conf=$config64 && confd=-D$conf
...
< $ING_SRC/tools/port$noise/conf/CONFIG || exit 1
...
case $conf in
	...
	ibm_lnx)	CPP='/lib/cpp -traditional -Ulinux';;
	int_lnx)	CPP='/lib/cpp -traditional -Ulinux';;
	int_rpl)	CPP='/lib/cpp -traditional -Ulinux';;
	a64_lnx)	CPP='/lib/cpp -traditional -Ulinux';;
	ppc_lnx)	CPP='/lib/cpp -traditional -Ulinux';;
	i64_lnx)	CPP='/lib/cpp -traditional -Ulinux';;
	mg5_osx)	CPP='/usr/bin/cpp';;
	*)		CPP=/lib/cpp ;;
esac

case $conf in
	[a-z][a-z0-9][a-z0-9]_us5)	;;
	[0-9][a-z0-9][a-z0-9]_us5)	conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_u42)	;;
	[0-9][a-z0-9][a-z0-9]_u42)	conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_u43)	;;
	[0-9][a-z0-9][a-z0-9]_u43)	conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_osf)	;;
	[0-9][a-z0-9][a-z0-9]_osf)	conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_ulx)	;;
	[0-9][a-z0-9][a-z0-9]_ulx)	conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_ul5)	;;
	[0-9][a-z0-9][a-z0-9]_ul5)	conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_lnx)	;;
	[0-9][a-z0-9][a-z0-9]_lnx)	conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_rpl)	;;
	[0-9][a-z0-9][a-z0-9]_rpl)	conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_u64)	;;
	[0-9][a-z0-9][a-z0-9]_u64)	conf=x$conf;;
        [a-z][a-z0-9][a-z0-9]_aix)      ;;
        [0-9][a-z0-9][a-z0-9]_aix)      conf=x$conf;;
	[a-z][a-z0-9][a-z0-9]_aos)	;;
	[a-z][a-z0-9][a-z0-9]_cmw)	;;
	[a-z][a-z0-9][a-z0-9]_bos)	;;
	[a-z][a-z0-9][a-z0-9]_prm)	;;
	[a-z][a-z0-9][a-z0-9]_vme)	;;
	[a-z][a-z0-9][a-z0-9]_vad)	;;
	...
	*_kit)				;;
	sqs_ptx)			;;
	*) echo $0: bad config, should be xxx_uyy >&2; exit 1;;
esac
...
then
	res=`echo $sym | cat $ING_SRC/tools/port$noise/conf/CONFIG - | \
		$CPP $incdir -DVERS=$conf -D$conf $optdef -P | sed -e '/^[ 	]*$/d' -e '/^#pragma/d'`
	if [ "$res" = "$sym" ]
	...
	cat $bzarch $ING_SRC/tools/port$noise/conf/CONFIG ${*-"-"} | \
		$CPP $incdir -DVERS=$conf $confd $defs $undefs -P | sed -e '/^[ 	]*$/d' -e '/^#pragma/d'
fi
