...
ris_u64)	echo '# define xCL_065_SHADOW_PASSWD'
		...
		# echo '# define xCL_TLI_OSLAN_EXISTS'
		;;
i64_aix)	echo '# define xCL_065_SHADOW_PASSWD'
		echo '# define xCL_088_FD_EXCEPT_LIST'
		echo '# define xCL_005_GETTIMEOFDAY_EXISTS'
		echo '# define xCL_GETTIMEOFDAY_TIME_AND_VOIDPTR'
                echo "# define xCL_067_GETCWD_EXISTS"
...
su4_us5)        echo '# define xCL_065_SHADOW_PASSWD'
		...
		if [ " $conf_DBL" != " true" -a " $conf_LP64" != " true" ] 
		then
			echo '# define xCL_SUN_LU62_EXISTS'
		fi
		...
		echo '# define xCL_076_USE_UNAME'
                Select=poll
                ;;
su9_us5)        echo '# define xCL_065_SHADOW_PASSWD'
                echo '# define xCL_073_CAN_POLL_FIFOS'
                echo '# define xCL_BLAST_EXISTS'
                echo '# define xCL_TLI_UNIX_EXISTS'
                echo '# define xCL_NO_INADDR_ANY'
...
rux_us5)        echo '# define xCL_065_SHADOW_PASSWD'
                echo "# define xCL_GETTIMEOFDAY_TIMEONLY_V"
                ;;
pym_us5)  	echo '# define xCL_065_SHADOW_PASSWD'
                ;;
 *_lnx|int_rpl)	echo '# define xCL_065_SHADOW_PASSWD'
		...
		Sigvec='sigaction'
		;;
 mg5_osx)	echo '# define xCL_067_GETCWD_EXISTS'
	        echo "# define xCL_SYS_PAGE_MAP"
...
sqs_ptx)
	...
	#
*)	#
	# normal case where inspection of libc.a will work
	#
		...
		dr6_us5|usl_us5|nc4_us5|sos_us5) pre='^.*\|' ;;
		i64_aix) pre='\|';;
		a64_sol) pre='\|';;
		mg5_osx) pre=' _';;
		*)	 pre='_*[^A-Za-z]' ;;
		...
		case $vers in
		ris_us5|rs4_us5|ris_u64)	nm=/bin/nm  ;;
		*)		nm=nm	;;
		esac
		...
		grep "${pre}getrusage" $cnm > /dev/null &&
			echo "# define xCL_003_GETRUSAGE_EXISTS"

		grep "${pre}getdtablesize" $cnm > /dev/null &&
			echo "# define xCL_004_GETDTABLESIZE_EXISTS"

		grep "${pre}waitpid" $cnm > /dev/null &&
			echo "# define xCL_WAITPID_EXISTS"

		grep "${pre}pwrite" $cnm > /dev/null ||
			echo "# define xCL_NO_ATOMIC_READ_WRITE_IO"

		grep "${pre}directio" $cnm > /dev/null &&
...
if [ $vers != "dg8_us5" -a $vers != "usl_us5" -a $vers != "dgi_us5" \
     -a $vers != "sos_us5" -a $vers != "rs4_us5" -a $vers != "ts2_us5" \
     -a $vers != "ris_u64"  -a $vers != "su4_us5" -a $vers != "su9_us5" \
     -a $vers != "i64_aix" -a $vers != "a64_sol" ]
then
    if grep "${pre}gettimeofday" $cnm > /dev/null
    then
	cd /tmp
	echo '
#include <sys/types.h>
...
' > tzone.c
	...
	if (cc -o tzone tzone.c) > /dev/null 2>&1
	then
	    if `tzone`
	    then
		echo "# define xCL_005_GETTIMEOFDAY_EXISTS"
...
# SPARC however is compiled with _SVID_GETTOD defined which
# brings in the one-parameter gettimeofday prototype.
...
case "$vers" in
axp_osf|su4_us5|su9_us5|dg8_us5|dgi_us5|sui_us5|pym_us5|ts2_us5)
...
' > shmdtargs.c
cc -c shmdtargs.c > /dev/null 2>&1 || 
	(cc -c -DxCL_SHMDT_ONE_ARG shmdtargs.c > /dev/null 2>&1 &&
		echo "# define xCL_SHMDT_ONE_ARG")
rm -f shmdtargs.*  ;;
esac

		grep "${pre}mkdir" $cnm > /dev/null &&
			echo "# define xCL_008_MKDIR_EXISTS"

		grep "${pre}rmdir" $cnm > /dev/null &&
			echo "# define xCL_009_RMDIR_EXISTS"

		grep "${pre}fsync" $cnm > /dev/null  &&
			echo "# define xCL_010_FSYNC_EXISTS"
if [ $vers != "su4_us5" ] && [ $vers != "sui_us5" ] && [ $vers != "su9_us5" ] 
then
		grep "${pre}setreuid" $cnm > /dev/null  &&
			echo "# define xCL_044_SETREUID_EXISTS"
fi

		grep "${pre}select" $cnm > /dev/null && [ "$Select" = "any" ] &&
			Select="select"
...
then
		...
		#
		grep "${pre}sigaction" $cnm > /dev/null  &&
			[ "$Sigvec" = "any" ] &&
			Sigvec="sigaction"
		...
		grep "${pre}dup2" $cnm > /dev/null  &&
			echo "# define xCL_012_DUP2_EXISTS"

case "$vers" in
nc4_us5|pym_us5)
		;;
*)		grep "${pre}valloc" $cnm > /dev/null  &&
		...
		#
		grep "${pre}sigvector" $cnm > /dev/null  &&
...
#
usl_us5 | sgi_us5 | rux_us5)	;;
axp_osf)	;;
*)		grep "${pre}vfork" $cnm > /dev/null &&
			echo "# define xCL_023_VFORK_EXISTS"
		;;
esac

		grep "${pre}opendir" $cnm > /dev/null &&
			echo "# define xCL_024_DIRFUNC_EXISTS"

		grep "${pre}execvp" $cnm > /dev/null &&
			echo "# define xCL_029_EXECVP_EXISTS"

		grep "${pre}setitimer" $cnm > /dev/null &&
			echo "# define xCL_030_SETITIMER_EXISTS"

		grep "${pre}ftok" $cnm > /dev/null &&
			echo "# define xCL_039_FTOK_EXISTS"

		grep "${pre}setdtablesize" $cnm > /dev/null &&
			echo "# define xCL_062_SETDTABLESIZE"

if [ $vers != "su4_us5" ] && [ $vers != "sui_us5" ] && [ $vers != "hp8_us5" ] \
          && [ $vers != "hpb_us5" ] && [ $vers != "su9_us5" ] \
          && [ $vers != "axp_osf" ] && [ $vers != "ris_u64" ] \
	  && [ $vers != "hp2_us5" ] && [ $vers != "i64_hpu" ] \
	  && [ $vers != "a64_sol" ]
then
		Getcwd="yes"
		grep "${pre}getwd" $cnm > /dev/null &&
			Getcwd="no"
		if [ "$Getcwd" = "yes" ]
		then
			grep "${pre}getcwd" $cnm > /dev/null &&
				echo "# define xCL_067_GETCWD_EXISTS"
		fi

		grep "${pre}setpriority" $cnm > /dev/null &&
			echo "# define xCL_033_SETPRIORITY_EXISTS"
fi

if [ $vers != "ris_u64" -a $vers != "i64_aix" ]
then
		grep "${pre}getrlimit" $cnm > /dev/null &&
			echo "# define xCL_034_GETRLIMIT_EXISTS"
fi

		grep "${pre}rename" $cnm > /dev/null &&
			echo "# define xCL_035_RENAME_EXISTS"

		grep "${pre}writev" $cnm > /dev/null &&
			echo "# define xCL_074_WRITEV_READV_EXISTS"

if [ $vers != "su4_us5" ] && [ $vers != "sui_us5" ] && [ $vers != "su9_us5" ]
then
		grep "${pre}gethostname" $cnm > /dev/null ||
			(
			grep "${pre}uname" $cnm > /dev/null &&
			echo "# define xCL_076_USE_UNAME"
			)
fi
...
if [ $vers != "ris_us5" -a $vers != "rs4_us5"  -a $vers != "ris_u64" \
  -a $vers != "i64_aix" ]
then
		grep "${pre}mmap" $cnm > /dev/null &&
		grep "${pre}munmap" $cnm > /dev/null &&
		grep "${pre}msync" $cnm > /dev/null &&
			echo "# define xCL_077_BSD_MMAP"
fi

		grep "${pre}mset" $cnm > /dev/null &&
		grep "${pre}mclear" $cnm > /dev/null &&
			echo "# define xCL_078_CVX_SEM"

		grep "${pre}sysconf" $cnm > /dev/null &&
			echo "# define xCL_079_SYSCONF_EXISTS"

if [ $vers != "su4_us5" ] && [ $vers != "sui_us5" ] && [ $vers != "su9_us5" ]
then
		grep "${pre}getpagesize" $cnm > /dev/null &&
			echo "# define xCL_080_GETPAGESIZE_EXISTS"
fi

		grep "${pre}mlock" $cnm > /dev/null &&
			echo "# define xCL_MLOCK_EXISTS"

		grep "${pre}seteuid" $cnm > /dev/null &&
...
[ $vers != "hp3_us5" -a $vers != "hp8_us5" -a $vers != "hp8_bls" -a $vers != "dg8_us5" -a $vers != "dgi_us5" -a $vers != "sqs_ptx" -a $vers != "hpb_us5" -a $vers != "hp2_us5" -a $vers != "i64_hpu" ] &&
...
if [ "$vers" = "rs4_us5" -o $vers = "ris_u64" -o $vers = "i64_aix" ]
then
...
[ -r /usr/include/sys/select.h ] &&
	...
	[ $vers != "ris_us5" -a $vers != "rs4_us5" -a $vers != "ris_u64" \
	-a $vers != "i64_aix" ] &&
...
grep FNDELAY /usr/include/fcntl.h > /dev/null 2>&1 &&
	[ $vers != "ris_us5"  -o $vers != "rs4_us5" -o $vers != "ris_u64" \
	-o $vers != "i64_aix" ] &&
...
# SYSV POLL
[ -r /usr/include/stropts.h ] && [ "$Select" = "any" ] &&
	Select="poll"
...
# TLI_TCP library
case "$vers" in
sos_us5|dg8_us5|dgi_us5|rmx_us5|pym_us5|ts2_us5|i64_hpu|hp2_us5|hpb_us5|hp8_us5|rux_us5) ;;
...
case $vers in
	...
	pym_us5|sui_us5|rmx_us5|ts2_us5|\
	sgi_us5|rux_us5|*_lnx|int_rpl|i64_aix|\
...
rs4_us5)	echo "# define xCL_092_NO_RAW_FSYNC"
		;;
ris_u64)	echo "# define xCL_092_NO_RAW_FSYNC"
		;;
i64_aix)	echo "# define xCL_092_NO_RAW_FSYNC"
		;;
dg8_us5)	echo "# define xCL_092_NO_RAW_FSYNC"
		;;
dgi_us5)	echo "# define xCL_092_NO_RAW_FSYNC"
		;;
ris_us5)	echo "# define xCL_092_NO_RAW_FSYNC"
		;;
esac
...
dgc_us5)        echo "# define xCL_093_MALLOC_OK"
                ;;
ris_us5)        echo "# define xCL_093_MALLOC_OK"
                ;;
rs4_us5)        echo "# define xCL_093_MALLOC_OK"
		echo "# define xCL_094_TLS_EXISTS"
                ;;
ris_u64)        echo "# define xCL_093_MALLOC_OK"
		echo "# define xCL_094_TLS_EXISTS"
                ;;
i64_aix)        echo "# define xCL_093_MALLOC_OK"
		echo "# define xCL_094_TLS_EXISTS"
                ;;
 ...
 *_lnx|int_rpl) echo "# define xCL_093_MALLOC_OK"
                echo "# define xCL_094_TLS_EXISTS"
                ;;
mg5_osx)        echo "# define xCL_093_MALLOC_OK"
                echo "# define xCL_094_TLS_EXISTS"
                ;;
esac
...
#
case $vers in
su9_us5|su4_us5|dg8_us5|dgi_us5|rs4_us5|axp_osf|hpb_us5|sgi_us5|ris_u64|i64_aix|hp2_us5|i64_hpu|a64_sol)
...
su4_us5|\
su9_us5|\
a64_sol)        echo "# define xCL_SYS_PAGE_MAP"
                ;;
esac
