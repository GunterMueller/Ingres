...
#
# For reverse hybrid builds use config64 for config
#
...
eval `egrep "VERS[ 	]*=|SLSFX[ 	]*=|MWSLSFX[	 ]*=" $DEFHDR | \
  sed -e 's/ //g' -e 's// ; /'`
...
case "$config" in
    ...
    rs4_us5)
	use_shared_libs=true
	shlink_cmd="ld -K -bE:expsym_list -G -bnoentry"
	shlink_opts="-lld -liconv -lpthreads -ldl -lm -lc_r -lc"
	# LIBPATH="/lib:$ING_BUILD/lib" 	# for information purposes
	;;
    ris_u64)
	use_shared_libs=true
    ...
    i64_aix)
	use_shared_libs=true
        shlink_cmd="ld -f elf64 -G "
        shlink_opts="-liconv -lpthreads -lm -lc"
	;;
    su4_us5 | sui_us5 | su9_us5 | a64_sol)
	use_shared_libs=true
	use_kerberos=true
	shlink_cmd="ld -G "
	shlink_opts=" -lm -lc -lelf -lposix4"
	krblink_cmd="ld -G"
	krblink_opts="-lm -lc -lelf -lnsl -lsocket"
    ...
    sgi_us5)
	use_shared_libs=true
	shlink_cmd="ld -shared -mips3"
	shlinkHB_cmd="ld -shared -64"
	shlink_opts=" -lm "
	shlinkHB_opts=" -lm "
	krblink_cmd="ld -shared -mips3"
	krblinkHB_cmd="ld -shared -64"
	krblink_opts=""
	krblinkHB_opts=""
	;;
    i64_lnx)
	...
	use_kerberos=true
	shlink_cmd="ld -shared "
	shlinkHB_cmd="i686-unknown-linux-gnu-ld -shared "
	shlink_opts=" -lpthread -lm -lc -ldl -lcrypt -lgcc_s "
	shlinkHB_opts=" -lpthread -lm -lc -ldl -lcrypt "
	krblink_cmd="ld -shared"
	krblinkHB_cmd="i686-unknown-linux-gnu-ld -shared"
	krblink_opts=""
	LD_LIBRARY_PATH="/lib:/usr/lib:$ING_BUILD/lib"
	;;
      int_lnx|\
	...
	shlink_opts=" -lm -lc -lpthread -ldl -lcc_dynamic -undefined dynamic_lookup -dead_strip"
	krblink_cmd="libtool -dynamic"
	krblink_opts="-lSystem -undefined dynamic_lookup"
...
export use_shared_libs use_shared_svr_libs shlink_cmd shlinkHB_cmd shlink_opts shlinkHB_opts krblink_cmd krblink_opts
