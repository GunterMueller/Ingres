...
REM 
REM Syntax:
REM 	FASTI <II_INSTALLATION> [clean]
...
echo
if "%1"=="" echo Usage: fasti {II_INSTALLATION} {clean}& goto END
if "%1"=="clean" echo Usage: fasti {II_INSTALLATION} {clean}& goto END
if "%2" == "" goto FRESH_INSTALLATION
if "%2" == "clean" goto CLEAN_UP_FIRST
...
ingsetenv II_TEMPORARY          "%II_SYSTEM%\ingres\temp"
...
ingsetenv II_GCN%1_LCL_VNODE    %COMPUTERNAME%
ingsetenv II_CHARSET%1          IBMPC850
ingsetenv TERM_INGRES           IBMPC
ingsetenv II_TIMEZONE_NAME      NA-EASTERN
...
echo.
REM This script must be run in the vdba directory.

createdb -u$ingres imadb
cd /D "%II_SYSTEM%\ingres\vdba"
