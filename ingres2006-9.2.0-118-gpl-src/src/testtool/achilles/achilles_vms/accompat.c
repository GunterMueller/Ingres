...
/* Required VMS definitions */
globalvalue int SJC$_PRIORITY;
globalvalue int SJC$_ENTER_FILE;
globalvalue int SJC$_QUEUE;
globalvalue int SJC$_FILE_SPECIFICATION;
globalvalue int SJC$_LOG_SPECIFICATION;
globalvalue int SJC$_NO_LOG_SPOOL;
globalvalue int SJC$_NO_LOG_DELETE;
globalvalue int SJC$_DELETE_FILE;
globalvalue int SJC$_NOTIFY;
globalvalue int SJC$_USERNAME;
globalvalue int SJC$_ENTRY_NUMBER_OUTPUT;
globalvalue int SJC$_SYNCHRONIZE_JOB;
globalvalue int SJC$_ENTRY_NUMBER;
globalvalue int JBC$_NORMAL;
globalvalue int QUI$_CANCEL_OPERATION;
globalvalue int QUI$_DISPLAY_QUEUE;
globalvalue int QUI$_SEARCH_NAME;
globalvalue int QUI$_SEARCH_FLAGS;
globalvalue int QUI$_ENTRY_NUMBER;
globalvalue int QUI$_JOB_PID;
globalvalue int QUI$_QUEUE_NAME;
globalvalue int QUI$_DISPLAY_JOB;
globalvalue int QUI$M_SEARCH_ALL_JOBS;
globalvalue int QUI$M_SEARCH_WILDCARD;
globalvalue int QUI$M_SEARCH_BATCH;
...
i4 waittime;
{
	...
	if ( (rval = sys$setimr(0, tval, alarm_handler, 1) ) != SS$_NORMAL)
		lib$signal(rval);
} /* ACinitAlarm */
...
VOID ACresetAlarm ()
{
	...
	if (!*tval)
		sys$bintim(&tdesc, tval);
	sys$setimr(0, tval, alarm_handler, 1);
} /* ACresetAlarm */

...
{
	unsigned short channel;
	unsigned long rval, iosb[2];
	$DESCRIPTOR(device, "SYS$INPUT:");

	sys$assign(&device, &channel, 0, 0);

	/* Ignore any errors as we may be running as a batch job */
	if ( (rval = sys$qiow(0, channel, IO$_SETMODE|IO$M_CTRLCAST, iosb, 0,
	  0, 0, 0, 0, 0, 0, 0) ) != SS$_NORMAL)
		return;

	if ( (rval = sys$qiow(0, channel, IO$_SETMODE|IO$M_CTRLCAST, iosb, 0,
	  0, abort_handler, 0, 0, 0, 0, 0) ) != SS$_NORMAL)
		return;

	if ( (iosb[0] & 0xffff) != SS$_NORMAL)
		printf("SYS$QIOW returned %d in iosv.\n", iosb[0] & 0xffff);
} /* ACsetupHandlers */
...
unsigned long		*rtnpid,	    /* Addr to return job's pid */
			rtniosb[2];	    /* Addr for job's compl iosb */
i4			*astprm;	    /* AST parm */
...
/* A general item list structure for all those routines that need it */
struct	ITEM_LIST {
	unsigned short	len,
			code;
	long		*buf,
...
struct ITEM_LIST trnlnm_itm[] = {
	...
	queue[trn_len] = '\0';
        itm_1[2].len = trn_len;	/*  When itm_1 structure is inited, there's
				    no length yet. */
...
/* added by Don Johnson... ( end   ) */
	...
	rval = sys$sndjbcw( 0, SJC$_ENTER_FILE, 0, &itm_1, iosb, 0, 0 );
	if ( rval != SS$_NORMAL )
	{
		ACerrMsg( "ACsubmit sys$sndjbcw rval", rval );
		return;
	}
	if ( iosb[0] != JBC$_NORMAL )
	{
		sprintf(  tmp_str
			, "ACsubmit <%s> sys$sndjbcw iosb"
			, itm_1[1].buf );

		ACerrMsg( tmp_str, iosb[0] );
		return;
	}
	...
	do
	{
		rval = sys$getquiw( 0, QUI$_DISPLAY_QUEUE, 0, &itm_3, iosb,0,0);
		if ( rval != SS$_NORMAL )
		{
		...
		if ( iosb[0] != JBC$_NORMAL )
		{
			ACerrMsg( "ACsubmit Display_queue", iosb[0] );
			return;
		}
	...
	do
	{
		rval = sys$getquiw( 0, QUI$_DISPLAY_JOB, 0, &itm_4, iosb, 0, 0);
		if ( rval != SS$_NORMAL )
		{
		...
		if ( iosb[0] != JBC$_NORMAL )
		{
			ACerrMsg( "ACsubmit Display_job", iosb[0] );
			return;
		}
