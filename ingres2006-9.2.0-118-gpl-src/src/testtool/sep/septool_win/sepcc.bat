...
set cflag=false
set nodbg_flag=true
if not "%ING_SRC%"=="" set include_dirs=/I%ING_SRC%\\gl\\hdr\\hdr /I%ING_SRC%\\cl\\hdr\\hdr /I%ING_SRC%\\cl\\clf\\hdr /I%ING_SRC%\\front\\hdr\\hdr /I%ING_SRC%\\front\\frontcl\\hdr /I%ING_SRC%\\common\\hdr\\hdr
...
if "%arg%"=="" goto ENDLOOP2
  ...
  if "%%i"=="-debug" set nodbg_flag=false
  if "%%i"=="-debu" set nodbg_flag=false
  if "%%i"=="-deb" set nodbg_flag=false
  if "%%i"=="-de" set nodbg_flag=false
  if "%%i"=="-d" set nodbg_flag=false
  if "%nodbg_flag%"=="false" set cc_sw=%cc_sw% /Yd
  if "%nodbg_flag%"=="false" goto LOOP2
...
:ENDCASE
if not "%nodbg_flag%"=="true" goto NEXT2
