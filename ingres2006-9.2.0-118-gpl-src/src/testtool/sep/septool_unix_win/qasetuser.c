...
#if defined( axp_osf ) || defined( ris_u64 ) || defined( rs4_us5 ) || \
    defined( ris_us5)
...
# endif
	...
	SecureZeroMemory(ingres_pw, strlen(ingres_pw));
	SecureZeroMemory(pvusr1_pw, strlen(pvusr1_pw));
	SecureZeroMemory(pvusr2_pw, strlen(pvusr2_pw));
	SecureZeroMemory(pvusr3_pw, strlen(pvusr3_pw));
	SecureZeroMemory(pvusr4_pw, strlen(pvusr4_pw));
	SecureZeroMemory(qatest_pw, strlen(qatest_pw));
	SecureZeroMemory(devtest_pw, strlen(devtest_pw));
	SecureZeroMemory(testenv_pw, strlen(testenv_pw));
	SecureZeroMemory(notauth_pw, strlen(notauth_pw));
	SecureZeroMemory(syscat_pw, strlen(syscat_pw));
	SecureZeroMemory(super_pw, strlen(super_pw));
	SecureZeroMemory(ctdba_pw, strlen(ctdba_pw));
	SecureZeroMemory(release_pw, strlen(release_pw));
...
# ifdef mg5_osx
	/* Need to set group ID on Mac too */
	ingpwd = getpwnam("ingres");
        if(!ingpwd) {
	  SIfprintf(stderr, ERx("Could not find user ingres\n"));
	  exit (1);
	}

	if(setgid(ingpwd->pw_uid) == -1) {
	 ...
	 if (getenv(SystemLocationVariable))
	  {
	   ...
	   char lpathnam[] = "LIBPATH";
#else
	   char lpathnam[] = "LD_LIBRARY_PATH";
#endif
	   char lpathfmt[] = "%s=%s:%s/ingres/lib";
	   sprintf(ld_lib, lpathfmt, lpathnam, "/lib:/usr/lib", getenv(SystemLocationVariable));
           DEBUG(ERx("ld_lib is %s\n"), ld_lib);
	   putenv(ld_lib);
	  }
         else
          {
            SIfprintf(stderr, ERx("qasetuser: %s is not set.\n"), SystemLocationVariable ); 
            exit(1);
          }
      
#endif  /* axp_osf ris_us5 rs4_us5 ris_u64 */

/* If int.lnx make sure we pass LD_LIBRARY_PATH on to new user shell */
#if defined(LNX) || defined(mg5_osx)
	if (getenv(SystemLocationVariable))
	{
	    char save_lib[256];
# ifdef mg5_osx
...
# endif
	    ...
	    if ((sizeof(save_lib)) < (strlen(getenv(SystemLocationVariable)) + strlen(ld_prefix) + strlen(ld_suffix)))
	    {
		SIfprintf(stderr, ERx("qasetuser: %s string too long in (DY)LD_LIBRARY_PATH save routine\n"), SystemLocationVariable);
		exit(1);
	    }
	    sprintf(save_lib, "%s%s%s", ld_prefix, getenv(SystemLocationVariable), ld_suffix);
	    putenv(save_lib);
	}
	else
	{
