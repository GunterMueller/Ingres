...
CXdlm_request( u_i4 flags, CX_REQ_CB *preq, LK_UNIQUE *ptranid )
{
	...
	(CX_LKI_F_PCONTEXT|CX_LKI_F_NOTIFY|CX_LKI_F_NAB|
	 CX_LKI_F_NO_DOMAIN|CX_LKI_F_LSN_IN_LVB|CX_LKI_F_USEPROXY);
    preq->cx_status = 0;
	...
	*/
    ...
    if ( CX_F_WAIT & flags )
    {
	dlmstat = sys$enqw( 0, dlmmode,
			    (struct _lksb *)&preq->cx_dlm_ws, dlmflags,
			    &resname, 0, NULL, preq, blkrtn,
			    PSL$C_USER, CX_Proc_CB.cxvms_dlm_rsdm_id, NULL );
	if ( SS$_NORMAL == dlmstat ) dlmstat = preq->cx_dlm_ws.cx_dlm_status;
    }
    else
    {
	dlmstat = sys$enq( 0, dlmmode,
			   (struct _lksb *)&preq->cx_dlm_ws, dlmflags,
		...
		dlm_status_t dlmstat )
{
	...
	CSresume_from_AST( preq->cx_owner.sid );
    return;
} /* cxaxp_cmpl_rtn */
...
cxvms_cmpl_rtn( void *pdata )
{
    ...
    if ( preq->cx_lki_flags & CX_LKI_F_USEPROXY )
    {
	/*
	** Resume session in other process, it will call user funcs if needed.
	*/
	CScp_resume(
...
*/
static void
cx_vms_free_glm( CX_GLC_CTX *glc, CX_GLC_MSG *glm )
{
...
cx_vms_pass_dlm_req( i4 flags, CX_REQ_CB *preq, i4 opcode )
{
	...
	*/
	...
	case CX_DLM_OP_CONV:
	    preq->cx_lki_flags |= CX_LKI_F_USEPROXY;
	    flags |= CX_F_STATUS;
	...
	if ( CS_TAS( &glc->cx_glc_ready ) )
	{
	    /* cx_glc_ready not already set, wake target */
	     ...
	     (opcode == CX_DLM_OP_CANC) )
	{
	    ...
	    */
	    ...
	    /* No double suspend */
	    preq->cx_lki_flags &= ~(CX_LKI_F_QUEUED|CX_LKI_F_USEPROXY);
	}
	else
	{
	    /*
...
/*
** Little utility routine to invert order of singlely linked list.
** Needed because queue is always added to at the head, but we
** need to process queue in FIFO order.
*/
static i4
cx_vms_invert_glms( i4 glmoff, i4 prev )
{
    CX_GLC_MSG	*msg;
    i4		 nextoff;

    msg = (CX_GLC_MSG *)CX_OFFSET_TO_PTR( glmoff );
    nextoff = msg->cx_glm_next;
    msg->cx_glm_next = prev;
    if ( 0 == nextoff )
    {
	return ( glmoff );
    }
    return cx_vms_invert_glms( nextoff, glmoff );
} /* cx_vms_invert_glms */
...
CXdlm_convert( u_i4 flags, CX_REQ_CB *preq )
{
	...
	*/
    ...
    if ( CX_F_WAIT & flags )
    {
	osstat = sys$enqw( 0, dlmmode,
			   (struct _lksb *)&preq->cx_dlm_ws, dlmflags,
	                   NULL, 0, NULL, preq, blkrtn,
			   PSL$C_USER, CX_Proc_CB.cxvms_dlm_rsdm_id, NULL );
	if ( SS$_NORMAL == osstat ) osstat = preq->cx_dlm_ws.cx_dlm_status;
    }
    else
    {
	osstat = sys$enq( 0, dlmmode,
			  (struct _lksb *)&preq->cx_dlm_ws, dlmflags,
...
CXdlm_wait( u_i4 flags, CX_REQ_CB *preq, i4 timeout )
{
    ...
    if ( preq->cx_lki_flags & (CX_LKI_F_QUEUED|CX_LKI_F_USEPROXY) )
    {
	/* 
	...
	** interrupted, or lock request is canceled.
	...
	if ( preq->cx_lki_flags & CX_LKI_F_USEPROXY )
	{
	    ...
	    suspend_func = CSsuspend;
	}
	else
	{
	    cancelflags = 0;
	    suspend_func = CSsuspend_for_AST;
	}
	...
	suspstatus = (*suspend_func)( csflags, timeout, (PTR)&preq->cx_key );
	if ( suspstatus || 
	...
	{ 
		...
		*/
	    ...
	    if ( OK == status )
	    {
		/* Enqueued request canceled, wait for completion */
		status = (*suspend_func)(CS_LOCK_MASK, 0, 
		  (PTR)&preq->cx_key );
		
		/* Intentionally ignoring 2nd CSsuspend return status */
		...
		cmplstatus = preq->cx_status;
                if ( 0 == cmplstatus )
                {
                    ...
                    */
                    status = (*suspend_func)( CS_LOCK_MASK, 0l,
                                              (PTR)&preq->cx_key );
		    cmplstatus = preq->cx_status;
                }
...
**
**	plkmode		Pointer to i4 buffer to return the global
**			lock strength to use for compatibility
**			testing.  This may be greater than the
**			mode requested by preq, because it reflects
**			the highest mode of all locks ahead
**			of preq throughout the cluster.
**
...
*/
STATUS
CXdlm_get_blki( u_i4 flags, CX_REQ_CB *preq,
...
#   define	LKBI_ALLOC_INC	64
    ...
    /*
    ** Single thread this function under VMS since we have
    ** only one buffer to receive data from $getlkiw.
    ** This is acceptable, since under Ingres there are only
    ** two threads which call this, and at present both
    ** cannot be running concurrently, so this semaphore
    ** should always be granted immediately.  
    **
    ** FIX-ME - Once POSIX threads are implemented this
    ...
    ** confirm this.
    */
    status = CSp_semaphore( TRUE, &CX_Proc_CB.cxvms_bli_sem );
    if ( OK == status )
    {
	...
	while (1) /* something to break out of */
	{
	    ...
	    if ( (totallki * sizeof(LKIDEF)) > 
	         CX_Proc_CB.cxvms_bli_size )
	    {
		/* Need to expand/allocate lki buffer */
		if ( CX_Proc_CB.cxvms_bli_buf )
		{
		    /* Free old buffer */
		    MEfree( CX_Proc_CB.cxvms_bli_buf );
		}
		/* Allocate expanded/initial buffer */
		CX_Proc_CB.cxvms_bli_buf = MEreqmem( 0, 
		 totallki * sizeof(LKIDEF), 0, NULL );
		if ( NULL == CX_Proc_CB.cxvms_bli_buf )
		{
		    status = E_CL2C12_CX_E_INSMEM;
		    break;
		}
		CX_Proc_CB.cxvms_bli_size = totallki * sizeof(LKIDEF);
	    }

	    /* Get info from VMS DLM */
	    lki_item_list[3].buflen = CX_Proc_CB.cxvms_bli_size;
	    lki_item_list[3].bufadr = CX_Proc_CB.cxvms_bli_buf;

	    vmsstat = sys$getlkiw( 0, &preq->cx_lock_id, lki_item_list,
				   &iosb, 0, 0, 0 );
	    if ( SS$_NORMAL != vmsstat ) break;
	    ...
	    **
	    ** Include the request mode of this lock.
	    */
	    *plkmode = preq->cx_new_mode;
	    for ( count = 0, plki = (LKIDEF *)CX_Proc_CB.cxvms_bli_buf;
		  count < totallki;
		  plki++, count++ )
	    {
		...
		/*
		** Observed behavior is that VMS does not reliably
		** set requested mode to granted mode after
		** doing a conversion.
		*/
		...
		{
		    /*
		    ** This is the lock we started out with;
		    ** we can end the search right here: we only
		    ** care about locks queued ahead of us and
		    ** ourselves, not behind us.
		    ** Exit out of gather loop.
		    */
		    break;
		}
	    }
	    ...
	    for ( plki = (LKIDEF *)CX_Proc_CB.cxvms_bli_buf;
		  count > 0;
		  plki++, count-- )
	    {
		...
		{
		    /*
		    ** Filter out those LKI records that are
		    ** not blocking us.
		    */
		    ...
		    */
		    ...
		    if ( plki->lki$l_csid ==
		         CX_Proc_CB.cx_ncb->cxvms_csids[0] )
		    {
			/* Blockers on our own node are checked in LK */
			continue;
		    }

		    /* Exclude granted that are compatible */
		    if ( CX_BLKI_GRANTQ == queue &&
		         (cx_ingres_lk_compat[grmode] & (1 << *plkmode)) )
		    {
			/* Compatible granted locks */
			continue;
		    }

		    /* Exclude Converters with a compatible rqmode */
		    if ( CX_BLKI_CONVQ == queue &&
		...
		{
		    /*
		    ** Expand buffer by greater of remaining
		    ** totallki or LKBI_ALLOC_INC
		    */
		    *pblkimax +=
		     totallki > LKBI_ALLOC_INC ? totallki : LKBI_ALLOC_INC;
		    pcxblki = MEreqmem( 0, sizeof(CX_BLKI) * *pblkimax,
		                        0, NULL );
		    if ( pcxblki )
		    {
			if ( outcount > 0 )
			{
			    MEcopy( blkibuf, outcount * sizeof(CX_BLKI),
				    pcxblki );
			    /*
			    ** Caller needs to decide if he will free
			    ** new or old buffer.  We can't do it here
			    ** as original buffer may be a declared
			    ** array.
			    */
			}
			*ppblkibuf = pcxblki;
	        ...
	        while ( idx > 0 )
		{
		    ...
		    if ( plki->lki$l_csid ==
		         CX_Proc_CB.cx_ncb->cxvms_csids[node] )
		    { 
		...
		CX_EXTRACT_LOCK_ID( &pcxblki->cx_blki_lkid, \
		                    &plki->lki$l_lkid );
		...
		if ( flags & CX_F_CVT_CVT )
		{
		    /*
		    ** Check for Cvt - Cvt deadlock 
		    **
		    ** Done here to short cicuit preparation
		    ** of blocking locks list.
		    */
		    if ( (CX_BLKI_CONVQ == queue) &&
		         !(cx_ingres_lk_compat[preq->cx_old_mode] &
			   (1 << rqmode)) )
		    {
			/*
	...
	(void)CSv_semaphore( &CX_Proc_CB.cxvms_bli_sem );
    }
...
# if defined(VMS) /* start _VMS_ */
    if ( !CS_ISSET( &preq->cx_dlm_ws.cx_dlm_posted ) )
    {
	if ( CS_TAS( &preq->cx_dlm_ws.cx_dlm_posted ) )
	{
	    posted = TRUE;
	}
    }
...
# if defined(VMS) /* start _VMS_ */
	...
	{
	    /*
	    ** Get a pointer to queue of pending CX_GLC_MSG messages.
	    */
	    while ( 0 != ( local_list = glc->cx_glc_glms ) )
	    {
		if ( !CScas4( &glc->cx_glc_glms, local_list, 0 ) )
		{
		    break;
		}
	    }
	    ...
	    if ( local_list )
	    {
		/* Invert order of list */
		local_list = cx_vms_invert_glms( local_list, 0 );
	    }
	    else if ( CS_ISSET( &glc->cx_glc_ready ) )
	    {
		CS_ACLR( &glc->cx_glc_ready );
		continue;
	    }
	    else
	    {
		/* Nothing is going on, just go to sleep. */
		...
		else
		{
		    ...
		    {
		    case CX_DLM_OP_RQST:
			lstatus = CXdlm_request( 
			   in_msg->cx_glm_flags & (~CX_F_WAIT),
			   preq, NULL /* VMS ignores this arg */ );
			if ( OK != lstatus )
			{
		    ...
		    case CX_DLM_OP_RLSE:
			lstatus = CXdlm_release( in_msg->cx_glm_flags, preq );
			CScp_resume( &in_msg->cx_glm_caller );
			cx_vms_free_glm( glc, in_msg );
			break;

		    case CX_DLM_OP_CANC:
			lstatus = CXdlm_cancel( in_msg->cx_glm_flags, preq );
		...
		*/
		...
		else if ( insert )
		{
			...
			*/
		    ...
		    CS_TAS( &glc->cx_glc_ready ); /* So resume is not sent. */
