...
**
**	    CXnode_name_from_nickname - Get actual name from nickname.
**
**	    CXcluster_nodes	- Information about nodes configured.
**
**	    CXnickname_from_node_name - Get nickname used by node.
**
...

/*{
** Name: CXnode_name_from_nickname - Get actual node name from nickname.
**
** Description:
**
**	This returns the actual node name for a given nickname.
**
** Inputs:
**	nickname	- Ptr to target nickname.
**	nodename	- Ptr to nodename buffer at least
**			  CX_MAX_NODE_NAME_LEN + 1 chars long.
**
** Outputs:
**	nodename	- filled in with actual nodename or ""
**
**	Returns:
**		TRUE   - if nickname in use.
**	        FALSE  - if nickname not defined.
**		
**	Exceptions:
**	    none
**
...
*/
bool
CXnode_name_from_nickname( char *nickname, char *nodename )
{
    STATUS		 status;
    PM_SCAN_REC 	 scanrec;
    char		*resname, *resvalue, *pmnodename;
    bool		 foundit = FALSE;

    *nodename = '\0';

    /* Call PMinit, just in case.  If already done will return "dup init" */
    if ( OK == PMinit() )
    {
    ...
    scanrec.last = 0;
    status = PMscan( ERx("^II\\.[^.]*\\.CONFIG\\.NICKNAME$"),
		     &scanrec, (char *)NULL,
		     &resname, &resvalue );
    while ( OK == status )
    {
	if ( 0 == STcompare(resvalue, nickname) )
	{
	    foundit = TRUE;
	    pmnodename = PMgetElem( 1, resname );
	    STlcopy(pmnodename, nodename, CX_MAX_NODE_NAME_LEN);
	    break;
	}
    ...
    return foundit;
} /*CXnode_name_from_nickname*/


/*{
** Name: CXnickname_from_node_name - Get nickname used by node.
**
** Description:
**
**	This returns the nickname used by a node, which defaults
**	to the node name itself.
**
** Inputs:
**	nodename	- Ptr to target node name.
**	nickname	- Ptr to nickname buffer at least
**			  CX_MAX_NICK_NAME_LEN + 1 chars long.
**
** Outputs:
**	nickname	- filled in with nickname
**
**	Returns:
**		TRUE   - if nickname differs from node name.
**	        FALSE  - if nickname not defined. In this case
**			 it is possible nodename is invalid
**			 also, since we don't bother to verify it.
**		
**	Exceptions:
**	    none
**
...
**	    Created.
**      15-mar-2005 (devjo01)
**          Do not fill in nickname if no nickname defined.
*/
bool
CXnickname_from_node_name( char *nodename, char *nickname )
{
    CX_NODE_INFO	*pni;
    bool		 foundit = FALSE;

    pni = CXnode_info( nodename, 0 );
    if ( pni && pni->cx_nick_name_l )
    {
        STlcopy(pni->cx_nick_name, nickname, CX_MAX_NICK_NAME_LEN);
        nickname[CX_MAX_NICK_NAME_LEN] = '\0';
        foundit = TRUE;
    }

    return foundit;
} /*CXnickname_from_node_name*/
...
ERx("%@ Cluster configuration: Node %~t has same id as Node %~t.\n"),
		 ...
		 ERx("%@ Cluster configuration: Node %s already exists.\n"),
		    ...
		    */
		    ...
		    pni->cx_nick_name_l = pni->cx_host_name_l =
		     pni->cx_rad_id = 0;

		    STprintf(param,"ii.%s.config.nickname", nodename);
		    if ( OK == PMget(param, &resvalue) )
		    {
			STmove( resvalue, '\0', CX_MAX_NICK_NAME_LEN,
				 pni->cx_nick_name );
			pni->cx_nick_name_l = STnlength( CX_MAX_NICK_NAME_LEN,
			 pni->cx_nick_name );
		    }
...
CXnode_info(char *host, i4 rad)
{
	...
	if ( 0 == rad )
	{
	    if ( ( 0 == STxcompare( host, 0,
		 ...
		 ( pni->cx_nick_name_l &&
		   0 == STxcompare( host, 0,
		      pni->cx_nick_name, pni->cx_nick_name_l,
		      TRUE, FALSE ) ) )
	    {
		/* Found the node name */
		return pni;
