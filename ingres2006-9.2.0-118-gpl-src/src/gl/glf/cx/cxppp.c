...
CXppp_write( CX_PPP_CTX ctx, char *buffer, i4  recordsize, i4 recordcount )
{
	...
	/* Show suspended on BIO write */
        do
        {
	    /* For debugging, force a 30-second timeout; hung writes are serious */
	    status = CSsuspend( CS_BIOW_MASK | CS_TIMEOUT_MASK, 30, 0 );
...
/* mbx_name : Must point to area >= 16 bytes */
{
    ...
    osstatus = sys$getdviw(0, 	    /* event flag */
