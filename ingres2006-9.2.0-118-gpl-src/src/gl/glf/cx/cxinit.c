	...
	*/
	{
	    ...
	    osstat = sys$getjpiw( 0, &CX_Proc_CB.cx_pid, (void *)0,
				  itmlst, &iosb, NULL, 0 );
...
# endif
		...
		$DESCRIPTOR(nodename,"");
		II_VMS_ITEM_LIST_3  syi_item_list [] = {
		      ...
		      i <= CX_MAX_NODES; pnodeinfo++, i++ )
		{
			...
			*/
			...
			syi_item_list[0].bufadr = &sms->cxvms_csids[i];
			osstat = sys$getsyiw(0, NULL, &nodename,
				             &syi_item_list, 0, 0, 0);
			if ( SS$_NORMAL != osstat )
			{
			    break;
		...
		CS_TAS(&sms->cxvms_csp_glc.cx_glc_ready);
...
# endif
	...
	CSw_semaphore( &CX_Proc_CB.cxvms_bli_sem, CS_SEM_SINGLE,
		       ERx("cx_bli_sem") );
	CX_Proc_CB.cxvms_bli_buf = NULL;
	CX_Proc_CB.cxvms_bli_size = 0;
...
# define ALTER_FLAG(flag) \
	...
	CX_Proc_CB.cx_proxy_offset = (i4)pvalue;
	break;
