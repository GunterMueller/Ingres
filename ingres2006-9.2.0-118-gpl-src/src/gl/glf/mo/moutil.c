...

/*{
**  Name: MOlongout - output utility for MO methods to put i4s
**
**  Description:
**
**	Turn the input i4 into a fixed width, 0 padded string and
...
STATUS
MOlongout( STATUS errstat, i4 val, i4  destlen, char *dest )
{
    ...
    /* Ignore errors in CVla -- force to 0 */
    (void)CVla( val, bufa );
    padlen = CV_DEC_PTR_SIZE - (i4)STlength( bufa ) - 1;
...

/*{
**  Name: MOulongout - output utility for MO methods to put unsigned i4s
**
**  Description:
**
**	Turn the input unsigned i4 into a fixed width, 0 padded
...
STATUS
MOulongout( STATUS errstat, u_i4 val, i4  destlen, char *dest )
{
    ...
    if( destlen < CV_DEC_PTR_SIZE )
    	 p = buf + destlen;
    else 
    	 p = buf + CV_DEC_PTR_SIZE; 
    *p-- = EOS;
