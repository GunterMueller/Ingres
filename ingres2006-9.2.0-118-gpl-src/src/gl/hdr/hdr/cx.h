...
#define CX_MAX_NICK_NAME_LEN	 8	/* Maximum length of a node alias */
...
struct _CX_REQ_CB
{
    u_i2	cx_lki_flags;	/* Internal status flags, do not modify */
#define CX_LKI_F_QUEUED         (1<<0)  /* Request queued for async grant */
#define CX_LKI_F_ISCONVERT      (1<<1)  /* Request is a convert */
#define CX_LKI_F_USEPROXY       CX_F_USEPROXY /* Resume logical (LK), not
                                 the actual (CX) lock owner */
...
struct _CX_NODE_INFO
{
    ...
    u_i4	cx_nick_name_l;		/* Length of node alias */
    char	cx_nick_name[CX_MAX_NICK_NAME_LEN]; /* Node alias */
...
bool	 CXnode_name_from_nickname( char *nickname, char *nodename );

bool	 CXnickname_from_node_name( char *nodename, char *nickname );
...
STATUS   CXdlm_get_bli( u_i4 flags, CX_REQ_CB *preq,
