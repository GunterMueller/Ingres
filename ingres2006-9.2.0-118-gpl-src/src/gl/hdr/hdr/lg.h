...
**          Explicitly define LGbegin's flag values as longs. On i64.hpu
**          the define's were not picked up correctly in LGbegin() and this
**          lead to spurious behaviour.
**/

/*
...
FUNC_EXTERN STATUS LGerase( LG_LGID *lg_id, LG_LXID lx_id, i4 bcnt, 
			    i4 bsize,
...
FUNC_EXTERN i4 LGidnum_from_id(i4 handle);
...
FUNC_EXTERN STATUS  LGposition( LG_LXID lx_id, i4  position, i4  direction,
			    ...
			    CL_SYS_ERR *sys_err );
FUNC_EXTERN STATUS  LGread( LG_LXID lx_id, PTR context, i4 l_context,
...
**
**      LG_MAX_RSZ which is the maximum log record size must be kept in sync
**      with back!dmf!hdr dm0j.h DM0J_MRZ, which is the maximum journal
**      record size.
**
**      I have added a consistency check in server startup to validate
**      the LG_MAX_RSZ and DM0J_MRZ.
*/

#define			LG_MAX_RSZ	    131072
